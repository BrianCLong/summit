...
# Fix semgrep action parameters
content = content.replace("""        uses: returntocorp/semgrep-action@713efdd345f3035192eaa63f56867b88e63e4e5d
        with:
          config: "p/ci"
          generateSarif: "true"
          sarifFile: "security-reports/semgrep.sarif"
          auditOn: "push\"""", """        uses: returntocorp/semgrep-action@v1
        with:
          config: "p/ci"
          generateSarif: "1"
...
content = content.replace("""        uses: returntocorp/semgrep-action@713efdd345f3035192eaa63f56867b88e63e4e5d
        with:
          config: "p/ci"
          generateSarif: "1"
          publishToken: ${{ secrets.SEMGREP_APP_TOKEN }}""", """        uses: returntocorp/semgrep-action@v1
        with:
          config: "p/ci"
          generateSarif: "1"
          publishToken: ${{ secrets.SEMGREP_APP_TOKEN }}\"""")

content = content.replace("""        uses: returntocorp/semgrep-action@v1
        with:
          config: "p/ci"
          generateSarif: "1"
          publishToken: ${{ secrets.SEMGREP_APP_TOKEN }}\"""", """        uses: returntocorp/semgrep-action@v1
...
content = content.replace("""      - name: Run Semgrep
        uses: returntocorp/semgrep-action@713efdd345f3035192eaa63f56867b88e63e4e5d
        with:
          config: >-
            p/ci
          generateSarif: "1"
          publishToken: ${{ secrets.SEMGREP_APP_TOKEN }}""", """      - name: Run Semgrep
        run: |
          mkdir -p security-reports
          semgrep ci --json --output=security-reports/semgrep.json || true
          semgrep ci --sarif --output=security-reports/semgrep.sarif || true
        env:
...
# Fix download artifact for aggregated
content = content.replace("merge-multiple: true\n        if-no-artifact-found: warn", "merge-multiple: true")
content = content.replace("merge-multiple: true\n          if-no-artifact-found: warn", "merge-multiple: true")

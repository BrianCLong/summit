...
export function evaluate(policy: PolicyDocument, ctx: Context): Decision {
  ...
  for (const r of policy.rules) {
    ...
    if (labels && (!Array.isArray(ctx.attributes.labels) || !labels.every((l) => ctx.attributes.labels.includes(l)))) continue;
    if (sensitivityAtMost && (ctx.attributes.sensitivity || 'S0') > sensitivityAtMost) continue;
    if (timeWindow) {
      const now = new Date();
      if (Number.isNaN(Date.parse(timeWindow.start)) || Number.isNaN(Date.parse(timeWindow.end))) continue;

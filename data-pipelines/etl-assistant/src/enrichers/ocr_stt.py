    ...
    def enrich(self, data: dict[str, Any], context: EnrichmentContext) -> EnricherResult:
        ...
        try:
            ...
            for field_name, target_ref in ocr_targets.items():
                ...
                result.metadata[f"{field_name}_text_length"] = len(ocr_result.get("text", ""))

            result.add_enrichment("ocr", ocr_data)

        except Exception as e:
            result.add_error(f"OCR processing failed: {e!s}")
...
class STTEnricher(BaseEnricher):
    ...
    def enrich(self, data: dict[str, Any], context: EnrichmentContext) -> EnricherResult:
        ...
        try:
            ...
            result.add_error(f"STT processing failed: {e!s}")
    ...
    def _process_stt(self, target_ref: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "word_count": 8,
            "speakers": [{"speaker_id": "SPEAKER_00", "segments": [{"start": 0.0, "end": 30.5}]}]

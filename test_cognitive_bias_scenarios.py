...
class TestCognitiveBiasScenarios(unittest.TestCase):
    ...
    def test_anchoring_bias(self):
        ...
        decision_context = {
            ...
            "adjustment_process": {"substantiality": "low"},
        }
        agent_state = {"cognitive_load": 0.8, "stress_level": 0.7}

        detections = self.bias_mitigation_system.bias_detector.detect_bias(
            decision_context, agent_state
        )
        ...
        if anchoring_detected:
            anchoring_detection = next(
                d for d in detections if d.bias_type.value == "anchoring_bias"
            )
            debiasing_results = self.bias_mitigation_system.debiasing_engine.apply_debiasing(
                anchoring_detection, decision_context
            )
            self.assertGreater(
                len(debiasing_results),
                0,
                "No debiasing strategies were applied for anchoring bias.",
            )

            # Check if a relevant strategy was applied
            relevant_strategy_applied = any(
                r.strategy_applied.value
                in ["anchor_and_adjust", "actively_seek_disconfirming_evidence"]
                for r in debiasing_results
            )
            self.assertTrue(
                relevant_strategy_applied,
                "A relevant debiasing strategy for anchoring was not applied.",
            )
    ...
    def test_confirmation_bias(self):
        ...
        decision_context = {
            ...
            "evidence": [
                ...
                {
                    "source": "Decoy server logs",
                    "supports": False,
                    "details": "Traffic pattern doesn't match known TTPs.",
                },
                {"source": "Threat intel feed", "supports": True},
                {"source": "Malware analysis", "supports": True},
                {"source": "Dark web chatter", "supports": True},
            ],
            "information_gathering": {"confirming_search": 0.9, "disconfirming_search": 0.1},
        }
        agent_state = {"confidence": 0.9, "emotional_state": {"anxiety": 0.1}}
        ...
        if confirmation_detected:
            confirmation_detection = next(
                d for d in detections if d.bias_type.value == "confirmation_bias"
            )
            debiasing_results = self.bias_mitigation_system.debiasing_engine.apply_debiasing(
                confirmation_detection, decision_context
            )
            self.assertGreater(
                len(debiasing_results),
                0,
                "No debiasing strategies were applied for confirmation bias.",
            )

            self.assertTrue(
                any(
                    r.strategy_applied.value == "actively_seek_disconfirming_evidence"
                    for r in debiasing_results
                ),
    ...
    def test_framing_effect(self):
        ...
        decision_context_gains = {
            ...
            "decision_frame": {"presentation": "gains", "presentation_bias": 0.8},
        }
        ...
        decision_context_losses = {
            ...
            "alternatives": ["Strategy C", "Strategy D"],
            "decision_frame": {"presentation": "losses", "presentation_bias": 0.8},
        }
        agent_state = {"risk_preference": "risk-averse"}

        # Test detection for both frames
        detections_gains = self.bias_mitigation_system.bias_detector.detect_bias(
            decision_context_gains, agent_state
        )
        framing_detected_gains = any(
            d.bias_type.value == "framing_effect" for d in detections_gains
        )
        self.assertTrue(
            framing_detected_gains, "Framing effect was not detected in the gains-framed scenario."
        )

        detections_losses = self.bias_mitigation_system.bias_detector.detect_bias(
            decision_context_losses, agent_state
        )
        framing_detected_losses = any(
            d.bias_type.value == "framing_effect" for d in detections_losses
        )
        self.assertTrue(
            framing_detected_losses,
            "Framing effect was not detected in the losses-framed scenario.",
        )

        if framing_detected_gains:
            framing_detection = next(
                d for d in detections_gains if d.bias_type.value == "framing_effect"
            )
            debiasing_results = self.bias_mitigation_system.debiasing_engine.apply_debiasing(
                framing_detection, decision_context_gains
            )
            self.assertGreater(
                len(debiasing_results),
                0,
                "No debiasing strategies were applied for the framing effect.",
            )

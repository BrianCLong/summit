...
export const DEFAULT_DETECTION_RULES: DetectionRule[] = [
  {
    ...
    pattern: '(?i)(ignore (all )?previous|disable (safety|guardrails)|strip provenance)',
    severity: 'high',
    vector: 'prompt-injection',
    controlExpectation: 'Prompt firewall with downstream citation verification',
  },
  {
    id: 'exfil-guard-001',
    description: 'Detect attempts to move data to unapproved hosts or channels',
    pattern: '(?i)(curl|scp|wget|POST ).*(http|https|s3|scp)',
    severity: 'critical',
    vector: 'data-exfiltration',
    controlExpectation: 'Egress proxy with allowlist and DLP rules',
  },
  {
    id: 'supply-chain-guard-001',
    description: 'Detect dependency tampering or unsigned package insertion',
    pattern: '(?i)(package\\.json|lock|requirements|checksum).*override',
    severity: 'high',
    vector: 'supply-chain',
    controlExpectation: 'Signed dependency policy and checksum enforcement',
  },
  {
    id: 'comm-mitm-001',
    description: 'Detect agent-in-the-middle relay wording or credential forwarding',
    pattern: '(?i)(forward.*token|proxying .*agent|relay channel)',
    severity: 'high',
    vector: 'comm-mitm',
    controlExpectation: 'Mutual TLS between agents and scoped tokens',
  },
  {
    id: 'workflow-hijack-001',
    description: 'Detect unauthorized workflow edits or auto-approval paths',
    pattern: '(?i)(github_token|auto.?approve|bypass review)',
    severity: 'high',
    vector: 'workflow-hijack',
    controlExpectation: 'Protected branches with CODEOWNERS and attestations',
  },
  {
    id: 'backdoor-pr-001',
    description: 'Detect covert PR backdoors or staged payload references',
    pattern: '(?i)(backdoor|payload|stash).*PR',
    severity: 'medium',
    vector: 'backdoor-pr',
    controlExpectation: 'PR policy checks with anomaly detection on diffs',
  },
];

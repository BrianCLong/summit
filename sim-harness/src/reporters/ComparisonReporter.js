...
exports.ComparisonReporter = void 0;
...
class ComparisonReporter {
    logger;
    constructor() {
        this.logger = new Logger_js_1.Logger('ComparisonReporter');
    }
    /**
     * Generate comparison report between baseline and candidate
     */
    generateComparison(baseline, candidate) {
        ...
        // Calculate deltas
        const successRateDelta = candidateAggregated.averageSuccessRate -
            baselineAggregated.averageSuccessRate;
        const performanceDelta = ((baselineAggregated.averageDuration -
            candidateAggregated.averageDuration) /
            baselineAggregated.averageDuration) *
            100;
        const qualityDelta = candidateAggregated.averageF1Score -
            baselineAggregated.averageF1Score;
        // Identify regressions and improvements
        const regressions = [];
        const improvements = [];
        if (successRateDelta < -0.05) {
            regressions.push(`Success rate decreased by ${(Math.abs(successRateDelta) * 100).toFixed(2)}%`);
        }
        else if (successRateDelta > 0.05) {
            improvements.push(`Success rate improved by ${(successRateDelta * 100).toFixed(2)}%`);
        }
        if (performanceDelta < -10) {
            regressions.push(`Performance degraded by ${Math.abs(performanceDelta).toFixed(2)}%`);
        }
        else if (performanceDelta > 10) {
            improvements.push(`Performance improved by ${performanceDelta.toFixed(2)}%`);
        }
        if (qualityDelta < -0.05) {
            regressions.push(`F1 score decreased by ${(Math.abs(qualityDelta) * 100).toFixed(2)}%`);
        }
        else if (qualityDelta > 0.05) {
            improvements.push(`F1 score improved by ${(qualityDelta * 100).toFixed(2)}%`);
        }
        if (candidateAggregated.errorRate >
            baselineAggregated.errorRate * 1.2) {
            regressions.push(`Error rate increased by ${((candidateAggregated.errorRate - baselineAggregated.errorRate) * 100).toFixed(2)}%`);
        }
        else if (candidateAggregated.errorRate <
            baselineAggregated.errorRate * 0.8) {
            improvements.push(`Error rate decreased by ${((baselineAggregated.errorRate - candidateAggregated.errorRate) * 100).toFixed(2)}%`);
        }
        return {
...
*Generated by IntelGraph Simulation Harness*
     ...
     * Save report to file
     */
    async saveReport(report, outputDir, format = 'html') {
...
exports.ComparisonReporter = ComparisonReporter;

...
export class RedteamHarness {
  ...
  constructor(options?: RedteamHarnessOptions) {
    ...
    this.artifactRoot =
      options?.artifactRoot || path.resolve(__dirname, '..', '..', '..', 'artifacts', 'agent-runs');
  ...
  runScenario(
    ...
    if (registryEntry.task_id !== scenario.task_id) {
      throw new Error(`Task ID mismatch between registry (${registryEntry.task_id}) and scenario (${scenario.task_id})`);
    }
    ...
    const provenance = this.buildProvenance(scenario, registryEntry, artifactPath);

...
class ScenarioGenerator {
    ...
    async generate(params) {
        ...
        const config = index_js_1.GRAPH_SIZE_CONFIGS[params.size];
        switch (params.type) {
            case 'fraud-ring':
     ...
     */
    async generateFraudRing(params, config) {
        const entities = [];
        const relationships = [];
        ...
        for (let i = 0; i < accountCount; i++) {
            entities.push({
                type: 'ACCOUNT',
                name: `Account-${(0, uuid_1.v4)().substring(0, 8)}`,
                properties: {
        ...
        return {
            investigation: {
                ...
                description: 'Investigation of suspected financial fraud network involving shell companies and international money transfers',
                type: 'FRAUD_INVESTIGATION',
            },
            entities,
            relationships,
            copilotGoal: 'Identify the key actors in the fraud network, trace money flows, and establish connections between shell companies',
            metadata: {
                seed: this.seed,
     ...
     */
    async generateTerrorCell(params, config) {
        const entities = [];
        const relationships = [];
        ...
        return {
            investigation: {
                ...
                description: 'Network analysis of suspected terror cell with focus on command structure and operational planning',
                type: 'THREAT_ANALYSIS',
            },
            entities,
            relationships,
            copilotGoal: 'Map the organizational structure of the cell, identify the leadership, and predict potential targets or timelines',
            metadata: {
                seed: this.seed,
                generatedAt: new Date().toISOString(),
                parameters: params,
     ...
     */
    async generateCorruptionNetwork(params, config) {
        const entities = [];
        const relationships = [];
        ...
        relationships.push(...this.generateCorruptionRelationships(entities, config));
        this.applyNoise(entities, relationships, params);
        return {
            investigation: {
                name: 'Corruption Network Investigation',
                description: 'Investigation of alleged corruption involving government officials, contractors, and kickback schemes',
                type: 'CORRUPTION_INVESTIGATION',
            },
            entities,
            relationships,
            copilotGoal: 'Trace money flows from government contracts to officials, identify beneficial ownership structures, and establish quid pro quo relationships',
            metadata: {
                seed: this.seed,
                generatedAt: new Date().toISOString(),
                parameters: params,
     ...
     */
    async generateSupplyChain(params, config) {
        const entities = [];
        const relationships = [];
        ...
        return {
            investigation: {
                ...
                description: 'Analysis of complex multi-tier supply chain for risk assessment and optimization',
                type: 'NETWORK_ANALYSIS',
            },
            entities,
            relationships,
            copilotGoal: 'Map the complete supply chain, identify bottlenecks and single points of failure, assess supplier risks',
            metadata: {
                seed: this.seed,
                generatedAt: new Date().toISOString(),
                parameters: params,
     ...
     */
    async generateMoneyLaundering(params, config) {
        const entities = [];
        const relationships = [];
        ...
        relationships.push(...this.generateMoneyLaunderingRelationships(entities, config));
        this.applyNoise(entities, relationships, params);
        return {
            investigation: {
                name: 'Money Laundering Network',
                description: 'Investigation of suspected money laundering operation across multiple jurisdictions',
                type: 'FRAUD_INVESTIGATION',
            },
            entities,
            relationships,
            copilotGoal: 'Trace the flow of funds through placement, layering, and integration stages, identify beneficial owners',
            metadata: {
                seed: this.seed,
                generatedAt: new Date().toISOString(),
                parameters: params,
     ...
     */
    async generateCustom(params, config) {
        const entities = [];
        const relationships = [];
        const entityTypes = [
            'PERSON',
        ...
        const relationshipCount = Math.floor(config.entities * config.relationshipDensity);
        ...
        return {
            investigation: {
                ...
                description: params.customParams?.description || 'Custom generated scenario',
                type: 'INTELLIGENCE_ANALYSIS',
            },
            entities,
            relationships,
            copilotGoal: params.customParams?.copilotGoal || 'Analyze the network and identify key patterns',
            metadata: {
                seed: this.seed,
                generatedAt: new Date().toISOString(),
                parameters: params,
    ...
    generateFraudRelationships(entities, config) {
        const relationships = [];
        ...
        for (const person of people) {
            ...
            for (const org of targetOrgs) {
                relationships.push({
                    type: this.rng.choice(['CONTROLS', 'OWNS']),
                    from: person.index,
                    to: org.index,
                    properties: {
    ...
    generateTerrorRelationships(entities, config) {
        const relationships = [];
        const people = entities
            .map((e, i) => ({ entity: e, index: i }))
            .filter((e) => e.entity.type === 'PERSON');
        ...
        if (people.length > 0) {
            const leaders = people.filter((p) => p.entity.properties.role === 'cell_leader');
            const operatives = people.filter((p) => p.entity.properties.role === 'operative');
            const supporters = people.filter((p) => p.entity.properties.role === 'supporter');
    ...
    generateCorruptionRelationships(entities, config) {
        const relationships = [];
        ...
        for (const official of officials) {
            ...
            for (const intermediary of targets) {
                relationships.push({
                    type: this.rng.choice(['AFFILIATED_WITH', 'RELATED_TO']),
                    from: official.index,
    ...
    generateSupplyChainRelationships(entities, config) {
        const relationships = [];
    ...
    generateMoneyLaunderingRelationships(entities, config) {
        const relationships = [];
        // Group by stage
        ...
        for (const source of layering.slice(-Math.floor(layering.length * 0.4))) {
            ...
            for (const target of targets) {
                relationships.push({
                    type: this.rng.choice(['TRANSACTED_WITH', 'OWNS']),
                    from: source.index,
                    to: target.index,
                    properties: {
    ...
    applyNoise(entities, relationships, params) {
        // Apply missing data
        ...
        if (params.conflictingEvidenceRate > 0) {
            const conflictCount = Math.floor(entities.length * params.conflictingEvidenceRate);
    ...
    buildGroundTruth(entities, relationships) {
        const rankedEntities = entities
            .map((entity, index) => {
            const weight = Number(entity.properties?.risk_score) ||
        ...
        const relationshipWeights = relationships.map((relationship, index) => ({
            index,
            weight: ['AFFILIATED_WITH', 'TRANSACTED_WITH', 'COMMUNICATES_WITH'].includes(relationship.type)
                ? 100
                : ['OWNS', 'CONTROLS', 'EMPLOYED_BY'].includes(relationship.type)
                    ? 75
                    : 25,
        }));
        const criticalRelationshipCount = Math.max(1, Math.floor(relationshipWeights.length * 0.1));

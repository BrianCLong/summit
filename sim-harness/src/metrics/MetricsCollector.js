...
class MetricsCollector {
     ...
     */
    startSession(sessionId, scenarioType = 'custom') {
        const sessionData = {
            sessionId,
            scenarioType,
            startTime: Date.now(),
     ...
     */
    recordStepCompletion(sessionId, stepType, duration) {
     ...
     * Record step failure
     */
    recordStepFailure(sessionId, stepType, duration, error) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    updateSessionMetrics(sessionId, updates) {
        ...
        if (updates.timeToGroundTruth !== undefined &&
    ...
    endSession(sessionId) {
        ...
        const groundTruthCoverage = groundTruthTotal > 0 ? groundTruthFound / groundTruthTotal : 0;
        ...
        const timeToGroundTruth = session.metrics.timeToGroundTruth ||
            (session.groundTruthDetectedAt
                ...
                : undefined);
        const precision = keyEntitiesFound > 0 && entitiesExplored > 0
            ? keyEntitiesFound / entitiesExplored
            : 0;
        const recall = keyEntitiesFound > 0 && keyEntitiesExpected > 0
        ...
        const finalMetrics = {
            ...
            copilotAverageResponseTime: session.metrics.copilotAverageResponseTime || 0,
     ...
     */
    getMetricsByScenario(scenarioType) {
        return this.completedSessions.filter((m) => m.scenarioType === scenarioType);
    }
    /**
    ...
    aggregateMetrics(sessions) {
        ...
        const sum = (arr) => arr.reduce((a, b) => a + b, 0) / arr.length;

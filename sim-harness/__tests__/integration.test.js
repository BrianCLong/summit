...
describe('Simulation Harness Integration', () => {
    const testConfig = index_1.ConfigLoader.getDefaults();
    ...
    testFn('should complete full fraud ring scenario workflow', async () => {
        // 1. Generate scenario
        const generator = new index_1.ScenarioGenerator(42);
        const params = {
            type: 'fraud-ring',
            size: 'small',
            noiseLevel: 0.1,
        ...
        // 3. Run ghost analyst
        const analyst = new index_1.GhostAnalyst(testConfig);
    ...
    testFn('should generate comparison report', async () => {
        ...
        const reporter = new index_1.ComparisonReporter();
    ...
    describe('Configuration Management', () => {
        it('should load default configuration', () => {
            const config = index_1.ConfigLoader.getDefaults();
        ...
        it('should merge custom configuration with defaults', () => {
            ...
            const merged = index_1.ConfigLoader.mergeWithDefaults(customConfig);
        ...
        it('should validate safe configuration', () => {
            const config = index_1.ConfigLoader.getDefaults();
            expect(() => {
                index_1.ConfigLoader.validate(config);
            }).not.toThrow();
        });
        it('should reject unsafe configuration', () => {
            const unsafeConfig = {
                ...index_1.ConfigLoader.getDefaults(),
                safety: {
                    ...index_1.ConfigLoader.getDefaults().safety,
                    nonProdOnly: false,
                },
            };
            expect(() => {
                index_1.ConfigLoader.validate(unsafeConfig);
            }).toThrow();
        });
    });
    describe('Scenario Generation Consistency', () => {
        it('should generate consistent scenarios across types', async () => {
            const generator = new index_1.ScenarioGenerator(12345);
            const scenarioTypes = [
                'fraud-ring',
                'terror-cell',
                'corruption-network',

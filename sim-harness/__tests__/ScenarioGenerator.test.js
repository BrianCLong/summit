...
const ScenarioGenerator_1 = require("../src/generators/ScenarioGenerator");
describe('ScenarioGenerator', () => {
    let generator;
    beforeEach(() => {
        generator = new ScenarioGenerator_1.ScenarioGenerator(12345);
    });
    describe('Deterministic Generation', () => {
        it('should generate identical scenarios with same seed', async () => {
            ...
            const gen1 = new ScenarioGenerator_1.ScenarioGenerator(42);
            const gen2 = new ScenarioGenerator_1.ScenarioGenerator(42);
        ...
        it('should generate different scenarios with different seeds', async () => {
            ...
            const gen1 = new ScenarioGenerator_1.ScenarioGenerator(42);
            const gen2 = new ScenarioGenerator_1.ScenarioGenerator(99);
    ...
    describe('Fraud Ring Scenario', () => {
        ...
        it('should include key fraud patterns', async () => {
            ...
            const controlRels = scenario.relationships.filter((r) => r.type === 'CONTROLS' || r.type === 'OWNS');
            expect(controlRels.length).toBeGreaterThan(0);
            // Check for transaction relationships
            const transactionRels = scenario.relationships.filter((r) => r.type === 'TRANSACTED_WITH');
    ...
    describe('Terror Cell Scenario', () => {
        it('should generate terror cell with hierarchical structure', async () => {
            ...
            const people = scenario.entities.filter((e) => e.type === 'PERSON');
            const leaders = people.filter((p) => p.properties.role === 'cell_leader');
            const operatives = people.filter((p) => p.properties.role === 'operative');
    ...
    describe('Corruption Network Scenario', () => {
        it('should generate corruption network with officials and businesses', async () => {
            ...
            const officials = scenario.entities.filter((e) => e.type === 'PERSON' &&
                e.properties.role === 'government_official');
            expect(officials.length).toBeGreaterThan(0);
            // Check for businesses
            const businesses = scenario.entities.filter((e) => e.type === 'ORGANIZATION');
    ...
    describe('Noise and Data Quality', () => {
        it('should apply noise to entities', async () => {
            ...
            const scenario = await generator.generate(params);
            const noisyEntities = scenario.entities.filter((e) => e.properties.noise_flag);
        ...
        it('should apply missing data', async () => {
            ...
            const actualAvgProps = scenario.entities.reduce((sum, e) => sum + Object.keys(e.properties).length, 0) / scenario.entities.length;
        ...
        it('should apply conflicting evidence', async () => {
            ...
            const scenario = await generator.generate(params);
            const conflictingEntities = scenario.entities.filter((e) => e.properties.conflicting_data);

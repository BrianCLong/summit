...
def cli(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--dashboard-json", type=Path, help="Path to write dashboard comparison data"
    )
    ...
    if args.baseline:
        ...
        for task in report.task_deltas:
            ...
            for metric in task.metric_deltas:
                ...
                print(
                    f"    Metric {metric.metric} delta={metric.delta:.4f} ci=[{metric.ci_low:.4f},{metric.ci_high:.4f}] {status}"
                )
    else:
        if args.dashboard_json:

...
async function runReindex(collectionType, newVersion) {
    ...
    try {
        const schemaPath = path_1.default.resolve(__dirname, `../schemas/${collectionType}@v${newVersion}.json`);
        ...
        try {
            ...
            if (e.httpStatus !== 404)
                console.warn("Check collection warning:", e.message);
        }
        ...
        if (newCount === 0 && currentCount > 0) {
            throw new Error(`ABORTING: New collection ${newCollectionName} is empty, but current has ${currentCount} docs.`);
        }
        if (currentCount > 0) {
            const diff = Math.abs(currentCount - newCount);
            const pct = diff / currentCount;
            if (pct > 0.05) { // 5% tolerance
                console.warn(`WARNING: Doc count mismatch > 5% (${(pct * 100).toFixed(2)}%). Verification needed.`);
            }
        }
        // ALIAS SWAP

...
const defaultComposeGlobs = ['docker-compose.yml', 'docker-compose.dev.yml', 'ops/compose/docker-compose.yml'];
...
function findReferencedServices(service, serviceNames) {
  ...
  if (service.command) {
    pools.push(Array.isArray(service.command) ? service.command.join(' ') : String(service.command));
  }
...
function detectRuntimeRaces(services, options) {
  ...
  Object.keys(services).forEach((serviceName) => {
    ...
    const unhealthyAttempts = health.Log?.filter((entry) => entry.Status === 'unhealthy').length || 0;

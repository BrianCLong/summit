...
describe('Agentic Plan Gate', () => {
    ...
    test('fails/warns when PLAN.md is missing', () => {
        try {
            (0, child_process_1.execSync)(`npx tsx ${SCRIPT_PATH}`, { cwd: TEMP_DIR, encoding: 'utf-8' });
        }
        catch (e) {
...
Check it
    `;
        ...
        const output = (0, child_process_1.execSync)(`npx tsx ${SCRIPT_PATH}`, { cwd: TEMP_DIR, encoding: 'utf-8' });
        const reportPath = path.join(TEMP_DIR, 'artifacts/agentic_policy/report.json');
        const report = JSON.parse(fs.readFileSync(reportPath, 'utf-8'));
...
Check it
    `;
        ...
        (0, child_process_1.execSync)(`npx tsx ${SCRIPT_PATH} test_code.ts`, { cwd: TEMP_DIR, encoding: 'utf-8' });
        const reportPath = path.join(TEMP_DIR, 'artifacts/agentic_policy/report.json');
        const report = JSON.parse(fs.readFileSync(reportPath, 'utf-8'));

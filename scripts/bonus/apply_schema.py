...
def get_owner_id(owner: str) -> str:
    ...
    try:
        ...
        # GraphQL may error if org not found, but user might still be in response
        # Try user-only query as fallback
        q_user = """
        query($login:String!) {
          user(login:$login){ id }
        }
        """
        data = gh_graphql(q_user, {"login": owner})
...
def ensure_field(project_id: str, spec: dict, verbose=False, owner="BrianCLong", project_number=None) -> dict:
    ...
    # Use gh CLI for field creation (simpler than GraphQL)
    # First, find the project number from the ID
    if not project_number:
        ...
        for p in projects["projects"]:
            if p.get("id") == project_id:
                ...
                break
        if not project_number:
            raise RuntimeError(f"Could not find project number for ID {project_id}")

    cmd = [GH, "project", "field-create", str(project_number), "--owner", owner, "--name", fname, "--data-type", dtype]

    if dtype == "SINGLE_SELECT" and spec.get("options"):
        opts_str = ",".join(spec["options"])
        cmd += ["--single-select-options", opts_str]

    subprocess.check_call(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    if verbose:
        print(f"[new] field created: {fname} ({dtype})")
    return get_project_fields(project_id)[fname]
...
def main():
    ...
    for fs in desired_fields:
        fld = ensure_field(pid, fs, verbose=args.verbose, owner=args.owner)

...
async function runTests() {
  ...
  if (!allowed) { console.error('FAILED: Should allow spend within budget'); failures++; }
  else {
  ...
  if (allowed2) { console.error('FAILED: Should block spend exceeding budget'); failures++; }
  else { console.log('PASSED: Hard stop enforced.'); }


  // TEST 2: Forecasting
  console.log('[TEST 2] Testing Forecasting...');
  const forecastBudget = budgetTracker.getDomainBudget(tenantId, CostDomain.AGENT_RUNS);
  if (!forecastBudget) { console.error('FAILED: Budget not found'); failures++; }
  else {
      // Allow 0 forecast if execution is too fast for Date.now() diff
      if (forecastBudget.forecastedSpending >= 0) console.log('PASSED: Forecast generated: ' + forecastBudget.forecastedSpending);
      else { console.error('FAILED: Forecast is invalid'); failures++; }
  }

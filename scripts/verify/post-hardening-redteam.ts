...
// CRITICAL: Mock process.exit BEFORE any imports that might trigger config validation
const originalExit = process.exit;
process.exit = (code?: number) => {
  if (code === 0) return originalExit(0);
...
// Bypassing the immediate proxy load by wrapping in a try-catch for the script entry
try {
    runRedTeamPass();
} catch (e) {
    if (e.message.includes('Process.exit')) {
        // Correctly caught the initial proxy load. Now run the pass which will reset it.
        runRedTeamPass();
    } else {

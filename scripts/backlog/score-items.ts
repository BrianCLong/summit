...
interface BacklogItem {
  ...
  title?: string;
  description?: string;
  source_signals: Array<{ id: string; type: string; severity: string }>;
  category: 'debt' | 'risk' | 'feature' | 'governance' | 'reliability' | 'security';
  ...
  metadata?: {
    debt_severity?: number;
    debt_age_days?: number;
    governance_impact?: number;
    blast_radius?: number;
    strategic_alignment?: string[];
    residual_risk?: number;
    budget_impact?: { risk?: number; debt?: number };
  };
}
...
interface ScoredItem extends BacklogItem {
...
function loadWeights(filePath: string): WeightsConfig {
  ...
  const parsed = loadYaml(contents) as WeightsConfig;
  validateWeights(parsed);
  return parsed;
}

function loadItems(itemPath: string): BacklogItem[] {
  ...
  return files.map((file) => JSON.parse(fs.readFileSync(file, 'utf-8')) as BacklogItem);
}

function validateWeights(weights: WeightsConfig) {
  const required = [
    'risk_reduction',
    'debt_severity',
    'debt_age',
  ...
  const missing = required.filter((key) => typeof weights.weights?.[key as keyof WeightsConfig['weights']] !== 'number');
  if (missing.length > 0) {
    throw new Error(`Weights missing required keys: ${missing.join(', ')}`);
  }

  const outOfRange = required.filter((key) => {
    const value = weights.weights[key as keyof WeightsConfig['weights']];
  ...
  if (
    weights.bands.p0 < weights.bands.p1 ||
...
function validateItems(items: BacklogItem[], schemaPath: string) {
...
function bandFromScore(score: number, bands: WeightsConfig['bands']): ScoredItem['priority_band'] {
  if (score >= bands.p0) return 'P0';
  if (score >= bands.p1) return 'P1';
  if (score >= bands.p2) return 'P2';
  return 'P3';
}

function scoreItem(item: BacklogItem, weights: WeightsConfig): ScoredItem {
  ...
  const rationale =
    `Risk reduction ${riskReduction.toFixed(2)}, debt ${debtSeverity.toFixed(2)} + age ${debtAge.toFixed(2)}, ` +

...
exports.OpenAPIDiffer = exports.GraphQLDiffer = exports.SchemaDiffEngine = void 0;
...
class GraphQLDiffer {
     ...
     */
    static diff(currentPath, snapshotPath) {
        const changes = [];
     ...
     */
    static isBreakingTypeChange(oldType, newType) {
...
exports.GraphQLDiffer = GraphQLDiffer;
// ============================================================================
// OpenAPI Schema Diff
// ============================================================================
class OpenAPIDiffer {
...
exports.OpenAPIDiffer = OpenAPIDiffer;
// ============================================================================
// Main Diff Engine
// ============================================================================
class SchemaDiffEngine {
    ...
    static async run(version = 'v1') {
        ...
        // Categorize changes
        const breaking = allChanges.filter((c) => ['field_removed', 'field_type_changed', 'field_made_required', 'type_removed', 'enum_value_removed', 'endpoint_removed', 'method_changed'].includes(c.type));
        const nonBreaking = allChanges.filter((c) => ['field_added', 'type_added', 'endpoint_added'].includes(c.type));
     ...
     */
    static generateRecommendations(breaking, nonBreaking, deprecated) {
        const recommendations = [];
        if (breaking.length > 0) {
            recommendations.push(`⚠️  BREAKING CHANGES DETECTED: ${breaking.length} breaking change(s) found`);
            ...
            recommendations.push(`✅ Non-breaking changes only: ${nonBreaking.length} additive change(s)`);
     ...
     */
    static calculateHashes(graphqlPath, openApiPath) {
     ...
     */
    static loadSnapshotHashes(versionDir) {
     ...
     */
    static saveDiffReport(result, outputFormat = 'json') {
     ...
     */
    static formatTextReport(result) {
        const lines = [];
...
exports.SchemaDiffEngine = SchemaDiffEngine;
// ============================================================================
// CLI
// ============================================================================
async function main() {

...
async function main() {
    const r = await (0, node_fetch_1.default)(`${PROM}/api/v1/query?query=${encodeURIComponent(Q)}`);
    const j = await r.json();
    const v = parseFloat(j.data.result?.[0]?.value?.[1] || '0');
    if (isNaN(v))
        throw new Error('No metric value');
    if (v > THRESH) {
    ...
    const alertsRes = await (0, node_fetch_1.default)(`${alertmanagerUrl}/api/v2/alerts?active=true&filter=severity=critical,severity=warning');

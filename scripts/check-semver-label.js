...
function main() {
    ...
    if (fixturePath) {
        ...
        try {
            const content = fs_1.default.readFileSync(fixturePath, 'utf8');
            const json = JSON.parse(content);
            if (Array.isArray(json)) {
                prLabels = json;
            }
            else if (json.pull_request) {
                prLabels = (json.pull_request.labels || []).map((l) => l.name);
        ...
        try {
            const content = fs_1.default.readFileSync(eventPath, 'utf8');
    ...
    if (prLabels.length === 0 && prTitle) {
        if (prTitle.startsWith('feat:'))
            ...
            prLabels.push('documentation');
        else if (prTitle.includes('BREAKING CHANGE'))
            prLabels.push('major');
    }
    ...
    if (foundLabels.length === 0) {
        ...
        console.log(`Success: Found valid SemVer label "${foundLabels[0]}" -> ${VALID_LABELS[foundLabels[0]]}`);
    }
    process.exit(0);
}
main();

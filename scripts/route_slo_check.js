...
async function q(expr) {
    const r = await (0, node_fetch_1.default)(`${PROM}/api/v1/query?query=${encodeURIComponent(expr)}`);
    const json = await r.json();
    return Number(json.data.result?.[0]?.value?.[1] || '0');
}
async function burnRate(path, window, budget) {
    const errRatio = await q(`sum(rate(http_requests_total{path="${path}",code=~"5.."}[${window}])) / sum(rate(http_requests_total{path="${path}"}[${window}]))`);

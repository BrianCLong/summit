...
async function main() {
  ...
  for (const rule of rules) {
    ...
    try {
      if (rule.backend === 'postgres') {
        if (!pgClient) { throw new Error("PostgreSQL connection not available"); }
        violations = await runPostgresCheck(pgClient, rule);
      } else if (rule.backend === 'neo4j') {
        if (!neo4jDriver) { throw new Error("Neo4j connection not available"); }
    ...
    if (error) {
      ...
      if (rule.severity === 'BLOCKER') { report.summary.blockers += violations.length; }
      if (rule.severity === 'WARN') { report.summary.warnings += violations.length; }
    } else {
  ...
  // 4. Cleanup
  if (pgClient) {
      try { await pgClient.end(); } catch (e) { console.error("Error closing PG:", e); }
  }
  if (neo4jDriver) {
      try { await neo4jDriver.close(); } catch (e) { console.error("Error closing Neo4j:", e); }
  }

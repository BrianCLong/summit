...
async function main(){
  ...
  if (pkgs.length===0) { console.error("No workspaces found."); process.exit(2); }

  // build a name->dir map
  const nameToDir=new Map();
  for (const p of pkgs){
    const pj = await readJSON(p.pj);
    if (!pj.name){ pj.name = `@${p.group}/${p.name}`; await writeJSON(p.pj,pj); }
  ...
  const rootTs = (await exists(ROOT_TSCONFIG)) ? await readJSON(ROOT_TSCONFIG) : {
    files: [],
    references: [],
    compilerOptions: {
      "composite": true,"declaration": true,"declarationMap": true,"sourceMap": true,
      "module":"NodeNext","moduleResolution":"NodeNext","target":"ES2022",
      "lib":["ES2022","DOM"],"strict": true,"skipLibCheck": true,"outDir":"./dist","rootDir":".","types":["node","vitest"]
    }
  };
  ...
  rootTs.references = Array.from(new Set([...(rootTs.references||[]).map(r=>r.path||r), ...references])).map(p=>({path:p}));
  await writeJSON(ROOT_TSCONFIG, rootTs);

  console.log("✓ Monorepo healed: scripts, exports, workspace deps, tsconfigs, and references synced.");
}

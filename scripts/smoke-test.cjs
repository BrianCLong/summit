...
const defaultDataset = path.join(repoRoot, "data", "quickstart", "quickstart-investigation.json");
const config = {
  ...
  retryDelay: 2000,
  datasetPath: path.resolve(process.env.GOLDEN_PATH_DATASET || defaultDataset),
};
...
class SmokeTest {
  ...
  async run() {
    ...
    await this.test("API /health endpoint", async () => {
      const healthy = await this.httpHealthCheck("API health", `${config.apiBaseUrl}/health`);
      if (!healthy) {
    ...
    await this.test("API /health/detailed endpoint", async () => {
      const health = await this.fetchJson(`${config.apiBaseUrl}/health/detailed`);
    ...
    await this.test("Metrics Endpoint", async () => {
      ...
      if (
        response.status !== 200 ||
        (typeof response.data === "string" && !response.data.includes("http_requests_total"))
      ) {
    ...
    await this.test("Database Schema Verification", async () => {
      ...
      if (health.services?.postgres !== "healthy") {
        throw new Error('PostgreSQL is not healthy. Run "make migrate" to initialize schema.');
      }
    ...
    await this.test("Start Copilot Run", async () => {
      const variables = {
        goal: this.dataset.copilotGoal || "Analyze dataset relationships for summit smoke test",
        investigationId,
      };

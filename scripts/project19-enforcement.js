...
const APP_TSX_PATH = path_1.default.join(process.cwd(), 'apps/web/src/App.tsx');
function checkResilienceCoverage() {
    console.log(`Checking resilience coverage in ${APP_TSX_PATH}...`);
    if (!fs_1.default.existsSync(APP_TSX_PATH)) {
        console.error(`ERROR: App.tsx not found at ${APP_TSX_PATH}`);
        process.exit(1);
    }
    const content = fs_1.default.readFileSync(APP_TSX_PATH, 'utf-8');

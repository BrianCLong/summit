...
function healthCheckScenario(baseUrl, headers) {
  group('Health Checks', () => {
    ...
    const componentsResponse = http.get(`${baseUrl}/health/components`, { headers });
  ...
  group('Workflow Orchestration', () => {
    ...
    if (orchestrateSuccess && orchestrateResponse.json().data) {
      ...
      while (!completed && attempts < maxAttempts) {
        ...
        const statusResponse = http.get(`${baseUrl}/workflow/status/${orchestrationId}`, { headers });
...
function metricsScenario(baseUrl, headers) {
  group('Metrics and Monitoring', () => {
    ...
    const workflowMetricsResponse = http.get(`${baseUrl}/workflows/metrics`, { headers });
...
<body>
    ...
    <table class="summary-table">
        ...
        ${Object.entries(testResults).map(([name, metric]) => `
            <tr>
                <td>${name}</td>

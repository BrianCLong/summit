...
async function runSimulation() {
  console.log(`🚀 Starting Launch Day Simulation: ${REQUESTS_PER_SECOND} rps for ${DURATION_SECONDS}s`);
  ...
  const serverErrors = Object.keys(statusCodes).filter(c => c.startsWith('5')).reduce((a, c) => a + statusCodes[c], 0);

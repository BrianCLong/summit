...
async function installMissingTypes() {
    ...
    try {
        (0, child_process_1.execSync)(`pnpm add -D -w ${missingTypes.join(' ')}`, {
            stdio: 'inherit',
            cwd: process.cwd()
        });
...
async function fixSecurityVulnerabilities() {
    ...
    try {
        for (const [pkg, version] of Object.entries(vulnerablePackages)) {
            try {
                (0, child_process_1.execSync)(`pnpm update ${pkg}@${version} --recursive`, {
                    stdio: 'pipe',
                    cwd: process.cwd()
                });
...
`;
    ...
    try {
        ...
        fs.writeFileSync(path.join(process.cwd(), 'packages/logger/package.json'), JSON.stringify(packageJson, null, 2) + '\n');
        ...
        fs.writeFileSync(path.join(process.cwd(), 'packages/logger/tsconfig.json'), JSON.stringify(tsconfig, null, 2) + '\n');
...
`;
    ...
    try {
        ...
        fs.writeFileSync(path.join(process.cwd(), 'packages/error-handler/package.json'), JSON.stringify(packageJson, null, 2) + '\n');
        ...
        fs.writeFileSync(path.join(process.cwd(), 'packages/error-handler/tsconfig.json'), JSON.stringify(tsconfig, null, 2) + '\n');
...
`;
    ...
    try {
        ...
        fs.writeFileSync(path.join(process.cwd(), 'packages/monitoring/package.json'), JSON.stringify(packageJson, null, 2) + '\n');
        // Create tsconfig.json
        const tsconfig = {
            extends: '../../tsconfig.base.json',
            compilerOptions: {
        ...
        fs.writeFileSync(path.join(process.cwd(), 'packages/monitoring/tsconfig.json'), JSON.stringify(tsconfig, null, 2) + '\n');

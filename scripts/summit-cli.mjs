...
async function down(args) {
  ...
  exec(`docker-compose -f ${config.composeFile} down ${volumes} --remove-orphans`, {
    ignoreError: true,
  });
...
async function status() {
  ...
  try {
    exec(
      `docker-compose -f ${config.composeFile} ps --format "table {{.Name}}\t{{.Status}}\t{{.Ports}}"`,
      { silent: false }
    );
  } catch {
  ...
  switch (subcommand) {
    ...
    case "shell":
      ...
      if (dbType === "postgres") {
        ...
        exec("docker exec -it summit-neo4j cypher-shell -u neo4j -p devpassword", {
          stdio: "inherit",
        });
...
async function newComponent(args) {
  ...
  switch (type) {
    case "service":
      ...
      exec(
        `node companyos/scripts/companyos-cli.mjs new-service --name ${name} --port ${port} --owner ${owner}`
      );
      break;

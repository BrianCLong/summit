...
const STUB_PATH = path_1.default.resolve(__dirname, '../../data/execution/pipeline_stub.json');
const ARTIFACT_PATH = path_1.default.resolve(__dirname, '../../artifacts/execution/scorecard.json');
function parseArgs() {
...
function getGitMetrics(startDate, endDate) {
    try {
        ...
        const commitsCount = parseInt((0, child_process_1.execSync)(commitsCmd, { encoding: 'utf8' }).trim(), 10) || 0;
        // Merged PRs count (counting merge commits)
        const mergesCmd = `git log --since="${since}" --until="${until}" --merges --oneline | wc -l`;
        const mergedPrsCount = parseInt((0, child_process_1.execSync)(mergesCmd, { encoding: 'utf8' }).trim(), 10) || 0;
...
function main() {
    ...
    // Load stub data
    if (!fs_1.default.existsSync(STUB_PATH)) {
        console.error(`Stub file not found at ${STUB_PATH}`);
        process.exit(1);
    }
    const stubData = JSON.parse(fs_1.default.readFileSync(STUB_PATH, 'utf8'));
    ...
    fs_1.default.writeFileSync(ARTIFACT_PATH, jsonOutput);

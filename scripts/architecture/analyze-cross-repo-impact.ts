...
function findMatchingInterfaces(changedFiles: string[], interfaces: PublicInterface[]): PublicInterface[] {
  return interfaces.filter((entry) =>
    changedFiles.some((file) => file.startsWith(entry.path) || file === entry.path),
  );
}

function analyzeImpacts(changedFiles: string[], map: RepoEntry[], interfaces: PublicInterface[]): Impact[] {
  ...
  for (const dependent of dependents) {
    ...
    for (const dep of relevantDeps) {
      ...
      const risk: Impact['risk'] = affectedSurfaces.length > 0 || inferredMatch || dep.dependency_type === 'API'
        ? 'high'
        : 'medium';

...
wt_rm() {
  ...
  marker_dir=$(find_worktree_marker "$current_worktree_root" || true)
  if [ -z "$marker_dir" ]; then
    echo "Current directory is not inside a worktree named <root>--<branch>; aborting." >&2
    exit 1
  fi
  name=$(basename "$marker_dir")
  root="${name%%--*}"
  branch_base="${name#*--}"

  if [ "$root" = "$name" ] || [ -z "$branch_base" ]; then
    echo "Current directory is not inside a worktree named <root>--<branch>; aborting." >&2
    exit 1
  fi

  if [ "$marker_dir" = "$current_worktree_root" ]; then
    branch="$branch_base"
  else
    branch_suffix="${current_worktree_root#"$marker_dir"/}"
    branch="${branch_base}/${branch_suffix}"
  fi

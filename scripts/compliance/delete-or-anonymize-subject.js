...
async function resolveUserId(pool, identifier) {
    const client = await pool.connect();
    try {
        const result = await client.query(`SELECT id FROM users WHERE ($1::uuid IS NOT NULL AND id = $1::uuid)
...
async function anonymizeUser(client, userId, dryRun) {
    ...
    const impersonationCount = await countRows(client, 'SELECT COUNT(*) AS count FROM user_impersonations WHERE admin_user_id = $1 OR target_user_id = $1', [userId]);
       ...
       WHERE user_id = $2`, [userId, userId]);
        const userUpdate = await client.query(`UPDATE users
         SET email = $1,
             ...
             metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('subject_redacted', true),
             updated_at = now()
       WHERE id = $2`, [placeholderEmail, userId]);
...
async function deleteUser(client, userId, dryRun) {
    const roleCount = await countRows(client, 'SELECT COUNT(*) AS count FROM user_roles WHERE user_id = $1', [userId]);
    const impersonationCount = await countRows(client, 'SELECT COUNT(*) AS count FROM user_impersonations WHERE admin_user_id = $1 OR target_user_id = $1', [userId]);
    const auditCount = await countRows(client, 'SELECT COUNT(*) AS count FROM audit_logs WHERE user_id = $1', [userId]);
    if (dryRun) {
...
async function main() {
    ...
    const client = await pool.connect();
    try {
        const result = identifier.mode === 'delete'

...
function findPackageFiles(dir, fileList = []) {
  try {
    ...
    files.forEach(file => {
      ...
      if (stat.isDirectory()) {
        if (file !== 'node_modules' && file !== '.git' && file !== 'dist' && file !== 'build' && file !== '.next' && file !== 'coverage' && file !== '.cache' && file !== '.pnpm-store') {
    ...
    ['dependencies', 'devDependencies', 'peerDependencies'].forEach(type => {
      if (pkg[type]) {
        Object.keys(pkg[type]).forEach(dep => {
          ...
          if (val.startsWith('workspace:') || val.startsWith('file:') || val.startsWith('link:') || val.startsWith('git') || val.startsWith('http') || val.includes('/')) {
             return;
          }

...
function loadLensConfiguration() {
...
function getLens(config, lensId) {
...
// Analysis Functions
// ============================================================================
function analyzeServiceMappings(config, lens) {
    const findings = [];
    ...
    for (const [serviceName, mapping] of Object.entries(serviceMappings)) {
        ...
        const specialCategoryTypes = lens.dataTypeMappings?.special_category ||
            lens.dataTypeMappings?.sensitive_data ||
            [];
        const handlesSpecialCategory = mapping.dataTypes.some((dt) => specialCategoryTypes.includes(dt) ||
        ...
        if (mapping.features.includes('automated_decision_making') ||
            ...
            findings.push({
                ...
                severity: 'high',
                service: serviceName,
                features: mapping.features.filter((f) => f.includes('automated') || f.includes('profiling')),
                obligations: [
...
function analyzeDataFlows(config, lens) {
    const serviceMappings = config.serviceMappings || {};
    const entryPoints = [
        {
    ...
    const processingServices = Object.entries(serviceMappings)
        .filter(([, mapping]) => mapping.regulations.includes(lens.id))
        .map(([name, mapping]) => ({
        name,
        dataTypes: mapping.dataTypes,
        riskLevel: mapping.features.some((f) => f.includes('automated') ||
...
function generateObligationChecklist(config, lens) {
    const checklist = [];
...
function getObligationsForCategory(category, lensId) {
    const obligations = [];
    if (category.id === 'data_subject_rights' && lensId === 'gdpr') {
        obligations.push({ id: 'access', name: 'Right of Access (Art. 15)' }, { id: 'rectification', name: 'Right to Rectification (Art. 16)' }, { id: 'erasure', name: 'Right to Erasure (Art. 17)' }, { id: 'portability', name: 'Right to Data Portability (Art. 20)' }, { id: 'object', name: 'Right to Object (Art. 21)' }, {
            id: 'automated_decisions',
            name: 'Automated Decision Rights (Art. 22)',
        });
    }
    else if (category.id === 'consumer_rights' && lensId === 'ccpa') {
        obligations.push({ id: 'know', name: 'Right to Know' }, { id: 'delete', name: 'Right to Delete' }, { id: 'opt_out', name: 'Right to Opt-Out of Sale' }, { id: 'correct', name: 'Right to Correct' }, { id: 'limit', name: 'Right to Limit Sensitive Data Use' });
    }
    else if (category.id === 'high_risk_ai' && lensId === 'eu_ai_act') {
        obligations.push({ id: 'risk_management', name: 'Risk Management System' }, { id: 'data_governance', name: 'Data Governance' }, { id: 'technical_docs', name: 'Technical Documentation' }, { id: 'human_oversight', name: 'Human Oversight' }, { id: 'transparency', name: 'Transparency Requirements' }, { id: 'conformity', name: 'Conformity Assessment' });
    }
    else if (category.id === 'security') {
        obligations.push({ id: 'encryption', name: 'Encryption at Rest and in Transit' }, { id: 'access_control', name: 'Access Control Measures' }, { id: 'breach_notification', name: 'Breach Notification Process' }, { id: 'incident_response', name: 'Incident Response Plan' });
    }
    else if (category.id === 'accountability') {
        obligations.push({ id: 'records', name: 'Records of Processing Activities' }, { id: 'dpia', name: 'Data Protection Impact Assessment' }, { id: 'dpo', name: 'Data Protection Officer' }, { id: 'policies', name: 'Privacy Policies and Notices' });
    }
    return obligations;
}
function evaluateObligationStatus(obligationId) {
...
function getEvidenceForObligation(obligationId) {
    const evidenceMap = {
...
function getGapsForObligation(obligationId) {
    const gapsMap = {
        portability: [
...
function generateRecommendations(findings) {
    ...
    if (findingsByCategory['high_risk_processing']?.length) {
        recommendations.push({
            ...
            description: 'Perform Data Protection Impact Assessments for all high-risk processing operations identified',
            effort: 'high',
            impact: 'high',
    ...
    if (findingsByCategory['special_category_data']?.length) {
        recommendations.push({
            ...
            description: 'Verify explicit consent or legal basis for all special category data processing',
            effort: 'medium',
            impact: 'high',
    ...
    if (findingsByCategory['automated_decisions']?.length) {
        recommendations.push({
            ...
            description: 'Implement human oversight, explainability, and contestability for automated decisions',
            effort: 'high',
            impact: 'high',
    ...
    recommendations.push({
        ...
        description: 'Keep records of processing activities, DPIAs, and compliance evidence up to date',
        effort: 'medium',
        impact: 'medium',
        relatedFindings: [],
    }, {
...
function calculateComplianceScore(findings, checklist) {
    ...
    const avgFindingSeverity = findings.length > 0
        ? findings.reduce((sum, f) => sum + (findingSeverityScores[f.severity] || 0), 0) / findings.length
        : 1;
    // Calculate checklist score
    const avgChecklistStatus = checklist.length > 0
        ? checklist.reduce((sum, c) => sum + (checklistStatusScores[c.status] || 0), 0) / checklist.length
        : 0;
    // Weighted average
...
// Report Generation
// ============================================================================
function generateReport(config, lens) {
    ...
    const findingCounts = findings.reduce((counts, f) => {
    ...
    return {
        metadata: {
            ...
            disclaimer: 'This is a simulation/aid tool, NOT legal advice. Consult qualified legal counsel for compliance matters.',
        },
        summary: {
            overallScore,
            passingThreshold: config.analysisConfig?.complianceScoring?.passingThreshold || 0.7,
            status: overallScore >= 0.7
                ? 'compliant'
                : overallScore >= 0.5
                    ? 'needs_review'
                    : 'non_compliant',
...
function formatReportAsMarkdown(report) {
    ...
    lines.push('');
    const statusEmoji = report.summary.status === 'compliant'
        ? 'PASS'
    ...
    for (const ep of report.dataFlowAnalysis.entryPoints) {
        lines.push(`| ${ep.name} | ${ep.dataTypes.join(', ')} | ${ep.riskLevel} | ${ep.regulatoryFlags.join(', ')} |`);
    }
    lines.push('');
    ...
    for (const ps of report.dataFlowAnalysis.processingServices) {
        lines.push(`| ${ps.name} | ${ps.dataTypes.join(', ')} | ${ps.riskLevel} | ${ps.regulatoryFlags.join(', ')} |`);
    }
    lines.push('');
    ...
    for (const transfer of report.dataFlowAnalysis.crossBorderTransfers) {
        lines.push(`| ${transfer.source} | ${transfer.destination} | ${transfer.mechanism} | ${transfer.requiresTIA ? 'Yes' : 'No'} |`);
    }
    lines.push('');
    lines.push('---');
    lines.push('');
...
function parseArgs() {
    ...
    const result = { lensIds: [], all: false };

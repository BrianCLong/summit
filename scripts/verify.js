...
async function runVerification(check) {
    const scriptPath = (0, path_1.join)(ROOT_DIR, check.script);
    ...
    return new Promise((resolve) => {
        ...
        const child = (0, child_process_1.spawn)('tsx', [scriptPath], {
            cwd: ROOT_DIR,
...
async function main() {
    ...
    if (filter) {
        checksToRun = VERIFICATION_CHECKS.filter(check => check.name.toLowerCase().includes(filter.toLowerCase()) ||

...
function compareVersions(found, expected) {
...
function runCommand(cmd) {
    try {
        const output = (0, child_process_1.execSync)(cmd, { stdio: ['ignore', 'pipe', 'pipe'] }).toString().trim();
...
const checks = [
    ...
    },
    {
        ...
        run: async () => {
            const connectionString = process.env.DATABASE_URL ?? 'postgres://postgres:postgres@localhost:5432/intelgraph';
            const client = new pg_1.default.Client({ connectionString, connectionTimeoutMillis: 5000 });
            try {
...
async function main() {
    ...
    const root = path_1.default.dirname(url_1.default.fileURLToPath(import.meta.url));
    const reportLines = [

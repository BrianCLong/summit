...
const run = (command: string, args: string[], options: { cwd?: string } = {}) => {
  const result = spawnSync(command, args, { cwd: options.cwd ?? repoRoot, stdio: 'pipe', encoding: 'utf8' })
  if (result.status !== 0) {
...
const buildBody = async (diffStat: string) => {
  ...
  await fs.writeFile(path.join(repoRoot, 'ai-refactor', 'output', 'latest-pr-body.md'), summaryLines.join('\n'), 'utf8')

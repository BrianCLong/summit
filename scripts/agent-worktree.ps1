...
function Remove-Worktree {
    ...
    $marker = Get-WorktreeMarker -StartPath $currentRoot
    ...
    $branchBase = $name.Substring($separatorIndex + 2)

    if ($root -eq $name -or -not $branchBase) {
        Write-Error "Current directory is not inside a worktree named <root>--<branch>; aborting."
        exit 1
    }

    $branchSuffix = $currentRoot.Substring($marker.Length).TrimStart('\', '/')
    if ($branchSuffix) {
        $branch = "$branchBase/$branchSuffix".Replace('\', '/')
    }
    else {
        $branch = $branchBase
    }

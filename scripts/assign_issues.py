...
def main():
    ...
    issues = json.loads(run(["gh", "issue", "list", "--repo", slug, "--label", "mvp1", "--state", "open", "--json", "number,title,labels"]).stdout)
    ...
    for it in issues:
        ...
        if is_epic:
            ...
            run(["gh", "issue", "edit", str(it["number"]), "--repo", slug, "--add-assignee", "BrianCLong"])

  ...
  if command -v syft &> /dev/null; then
    ...
    for dockerfile in Dockerfile*; do
      if [ -f "$dockerfile" ]; then
        ...
        syft scan dir:. -o "cyclonedx-json@$CYCLONEDX_VERSION" --file "$OUTPUT_DIR/${ARTIFACT_NAME}-${service_name}-${VERSION}.cdx.json" 2>/dev/null || \
        syft scan dir:. -o cyclonedx-json --file "$OUTPUT_DIR/${ARTIFACT_NAME}-${service_name}-${VERSION}.cdx.json"
        
        # Generate SPDX format (Targeting $SPDX_VERSION)
        syft scan dir:. -o "spdx-json@$SPDX_VERSION" --file "$OUTPUT_DIR/${ARTIFACT_NAME}-${service_name}-${VERSION}.spdx.json" 2>/dev/null || \
        syft scan dir:. -o spdx-json --file "$OUTPUT_DIR/${ARTIFACT_NAME}-${service_name}-${VERSION}.spdx.json"
        
        # Generate syft table format for human consumption
        syft scan dir:. -o table --file "$OUTPUT_DIR/${ARTIFACT_NAME}-${service_name}-${VERSION}.syft.txt"
      fi
    done
  else
...
if [ -f "package.json" ]; then
  ...
  if command -v syft &> /dev/null; then
    syft scan dir:. -o cyclonedx-json --file "$OUTPUT_DIR/${ARTIFACT_NAME}-npm-${VERSION}-alt.cdx.json"
  fi
fi
...
if [ -f "requirements.txt" ] || [ -f "pyproject.toml" ]; then
  ...
  if command -v syft &> /dev/null; then
    syft scan dir:. -o cyclonedx-json --file "$OUTPUT_DIR/${ARTIFACT_NAME}-python-${VERSION}.cdx.json"
  fi
...
if [ -f "pom.xml" ] || [ -f "build.gradle" ] || [ -f "build.gradle.kts" ]; then
  ...
  if command -v syft &> /dev/null; then
    syft scan dir:. -o cyclonedx-json --file "$OUTPUT_DIR/${ARTIFACT_NAME}-java-${VERSION}.cdx.json"
  fi
fi

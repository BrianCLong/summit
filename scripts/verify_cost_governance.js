...
async function runTests() {
    ...
    const allowed2 = budget_tracker_js_1.budgetTracker.checkBudget(tenantId, types_js_1.CostDomain.AGENT_RUNS, 6.0);
    if (allowed2) {
    ...
    const forecastBudget = budget_tracker_js_1.budgetTracker.getDomainBudget(tenantId, types_js_1.CostDomain.AGENT_RUNS);
    if (!forecastBudget) {
    ...
    budget_tracker_js_1.budgetTracker.on('threshold_reached', alertHandler);
    ...
    budget_tracker_js_1.budgetTracker.trackCost(tenantIdAlert, types_js_1.CostDomain.AGENT_RUNS, 10.0);
    // Give time for event emission
    await new Promise(resolve => setTimeout(resolve, 100));
    budget_tracker_js_1.budgetTracker.off('threshold_reached', alertHandler); // Cleanup
    if (alertCount === 1) {

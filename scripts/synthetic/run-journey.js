...
async function run() {
    ...
    const measure = async (name, fn) => {
        ...
        try {
            ...
            const duration = Date.now() - start;
            results.journeys[name] = { success: false, duration, error: e.message, status: e.response?.status };
            results.latency[name] = duration;
            console.error(`[FAIL] ${name}: ${e.message}`);
            // Critical steps should stop execution to avoid cascading errors or inconsistent state
            if (name.includes('Login') || name.includes('Create Investigation'))
                throw e;
            return null;
        }
    };
    try {
      ...
      `;
        ...
        await measure('J-002: Import Execution', async () => {
            if (!uploadedFilePath)
                throw new Error('No file path from analysis');
            const mapping = {
                source: 'source',
                target: 'target',
                type: 'type',
      ...
      `;
        ...
        await measure('J-004: Trigger Maestro Run', async () => {
            ...
            if (res.status !== 200)
                throw new Error(`Maestro failed: ${res.status}`);
        ...
        await measure('J-006: Evaluate Result', async () => {
            if (!maestroResponse)
                throw new Error('No Maestro response from J-004');
            const { run, results: taskResults } = maestroResponse;
            if (!run || !run.id)
                throw new Error('Maestro response missing run ID');
            ...
            const failedTasks = taskResults.filter((r) => r.task && r.task.status === 'failed');
            if (failedTasks.length > 0) {
                throw new Error(`Maestro tasks failed: ${failedTasks.map((t) => t.task.errorMessage).join(', ')}`);
            }
        ...
        fs_1.default.writeFileSync('synthetic-results.json', JSON.stringify(results, null, 2));
        fs_1.default.writeFileSync('synthetic-latency-histogram.json', JSON.stringify(results.latency, null, 2));
    }
}
run();

 ...
 */
function generateUnitTest(filePath, sourceCode) {
 ...
 */
function generateIntegrationTest(serviceName) {
    return `/**
 ...
 * Generate E2E test
 */
function generateE2ETest(featureName) {
    return `/**
 ...
 */
async function findFilesNeedingTests() {
    const sourceFiles = await (0, glob_1.glob)('**/*.{ts,tsx}', {
        ignore: [
            '**/node_modules/**',
            '**/dist/**',
            '**/build/**',
...
async function main() {
    ...
    if (fs.existsSync(servicesDir)) {
        ...
        for (const service of services) {
            const integrationTestPath = path.join(servicesDir, service, '__tests__', 'integration', `${service}.integration.test.ts`);

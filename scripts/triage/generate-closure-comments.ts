  ...
  while (pageUrl) {
    const response = await fetch(pageUrl, {
      headers: {
        ...
        'User-Agent': 'summit-doc-review-triage',
        ...(options.token ? { Authorization: `Bearer ${options.token}` } : {}),
      },
    });

    if (response.status === 403) {
      const retried = await handleRateLimit(response);
      if (retried) {
        continue;
      }
      throw new Error(
        'GitHub API returned 403 without rate-limit exhaustion. Check token scopes.',
      );
    }

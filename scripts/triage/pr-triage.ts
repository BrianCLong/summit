...
function getPRs(): PR[] {
  try {
    ...
    const output = execSync(
      "gh pr list --state open --json number,title,headRefName,author,updatedAt,labels,checks --limit 50",
    ...
    console.error(
      "To generate a triage report, either install gh CLI or provide a valid prs.json export."
    );
    process.exit(1);
  }
}
...
function generateReport(prs: PR[]) {
  ...
  analyzed.forEach((pr) => {
    console.log(
      `| #${pr.number} ${pr.title.slice(0, 40)}... | ${pr.status} | ${pr.author} | ${new Date(pr.updatedAt).toLocaleDateString()} |`
    );
  });

...
const findings = [];
function checkRunnerValue(value, fileName, context) {
...
function traverse(node, pathStack, fileName) {
...
function checkWorkflowStructure(fileName, doc) {
    if (!doc || !doc.jobs)
        return;
                ...
                if (job.strategy && job.strategy.matrix) {
                    ...
                    for (const key in matrix) {
                        ...
                        if (Array.isArray(val)) {
                            val.forEach(v => {
                                if (typeof v === 'string')
                                    checkRunnerValue(v, fileName, `Job: ${jobId}, Matrix: ${key}`);
                            });
                        }
                    }
                    // Handle include block
                    if (matrix.include && Array.isArray(matrix.include)) {
                        matrix.include.forEach((item, idx) => {
                            for (const k in item) {
                                if (typeof item[k] === 'string')
                                    checkRunnerValue(item[k], fileName, `Job: ${jobId}, Matrix Include[${idx}]`);
                            }
                        });
                    }
                }
...
function main() {
    ...
    // Deduplicate
    const uniqueFindings = findings.filter((value, index, self) => index === self.findIndex((t) => (t.file === value.file && t.message === value.message && t.level === value.level)));

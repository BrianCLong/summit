...
test('Evidence Pack - Offline Mode', (t) => {
  ...
  try {
    execSync(`npx tsx ${scriptPath} --mode=offline --fixturesDir=${fixturesDir} --outputDir=${TEST_OUTPUT_DIR}`, {
        stdio: 'inherit'
    });
  } catch (e) {
  ...
  assert.ok(fs.existsSync(path.join(artifactsDir, 'docs/compliance/EVIDENCE_INDEX.md')), 'Should copy deep artifact');

  // Assert outputs
  const outputsDir = path.join(packDir, 'outputs');
  assert.ok(fs.existsSync(path.join(outputsDir, 'baseline-report.json')), 'Should copy baseline-report.json');

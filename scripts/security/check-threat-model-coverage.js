...
function findCoverageMapping(file) {
    return COVERAGE_MAPPINGS.find((mapping) => mapping.pattern.test(file));
}
function extractLastUpdated(threatModelPath) {
...
function generateSummary(results) {
    ...
    const hasCriticalIssues = dedupedResults.some((r) => r.riskTier === 'Critical' && r.status !== 'present');
    const hasHighIssues = dedupedResults.some((r) => r.riskTier === 'High' && r.status !== 'present');
    return {
...
function formatMarkdownOutput(summary) {
    const lines = [];
    lines.push('## Threat Model Coverage Check');
    lines.push('');
    if (summary.results.length === 0) {
    ...
    for (const r of summary.results) {
        const statusEmoji = r.status === 'present' ? 'Current' : r.status === 'stale' ? 'Stale' : 'Missing';
        const link = r.status !== 'missing' ? `[View](${r.threatModel})` : 'Not found';
    ...
    if (missing.length > 0 || stale.length > 0) {
        ...
        lines.push('');
        if (missing.length > 0) {
            lines.push('- **Missing threat models**: Please create threat models for the features listed above.');
            lines.push('  Use the [template](docs/security/threat-models/template.md) as a starting point.');
        }
        if (stale.length > 0) {
            lines.push('- **Stale threat models**: Please review and update the threat models listed above.');
            lines.push('  Update the "Last Updated" date after review.');
        }
        lines.push('');
        lines.push('See [Threat Modeling Framework](docs/security/threat-modeling-framework.md) for guidance.');
    }
    return lines.join('\n');
}
function formatJsonOutput(summary) {
...
function main() {
    ...
    if (changedFiles.length === 0) {
        ...
        console.log('');
        console.log('Usage:');
        console.log('  npx ts-node scripts/security/check-threat-model-coverage.ts --changed-files "file1.ts,file2.ts"');
        console.log('  npx ts-node scripts/security/check-threat-model-coverage.ts --base-ref main');
        console.log('');
        console.log('Options:');

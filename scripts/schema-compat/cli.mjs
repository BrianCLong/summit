...
function parseArgs(argv) {
  ...
  if (!args.baseline || !args.current) {
    throw new Error('Usage: node scripts/schema-compat/cli.mjs --baseline <path> --current <path> [--report <file>] [--compat <file>]');
  }
...
export async function run(argv) {
  ...
  const diff = await compareDirectories({ baselineDir: args.baselineDir, currentDir: args.currentDir, compatibility });

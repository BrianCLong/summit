...
function getMockPRs(): PRStatus[] {
  ...
  return Array.from({ length: 494 }, (_, i) => ({
    ...
    autoMergeRequest:
      Math.random() > 0.5 ? { enabledAt: new Date().toISOString(), mergeMethod: "SQUASH" } : null,
    statusCheckRollup: {
...
function fetchOpenPRs(): PRStatus[] {
  try {
    ...
    const fields =
      "number,title,state,mergeable,autoMergeRequest,statusCheckRollup,author,createdAt";
...
function printReport(stats: BacklogStats) {
  ...
  console.log("\n🤖 Auto-Merge Status:");
  console.log(
    `   Enabled:  ${stats.autoMergeEnabled.toString().padEnd(5)} ${generateProgressBar(stats.autoMergeEnabled, stats.totalOpen)}`
  );
  console.log(
    `   Disabled: ${(stats.totalOpen - stats.autoMergeEnabled).toString().padEnd(5)} ${generateProgressBar(stats.totalOpen - stats.autoMergeEnabled, stats.totalOpen)}`
  );

  console.log("\n🚦 CI Health:");
  console.log(
    `   ✅ Passing: ${stats.ciStatus.passing.toString().padEnd(5)} ${generateProgressBar(stats.ciStatus.passing, stats.totalOpen)}`
  );
  console.log(
    `   ❌ Failing: ${stats.ciStatus.failing.toString().padEnd(5)} ${generateProgressBar(stats.ciStatus.failing, stats.totalOpen)}`
  );
  console.log(
    `   ⏳ Pending: ${stats.ciStatus.pending.toString().padEnd(5)} ${generateProgressBar(stats.ciStatus.pending, stats.totalOpen)}`
  );

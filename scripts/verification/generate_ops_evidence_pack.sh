...
copy_file() {
  ...
  if [[ -f "$source_file" ]]; then
    ...
    echo "Error: required file $source_file not found" >&2
    exit 1
  else
...
copy_file "$REPO_ROOT/docs/ops/EVIDENCE_INDEX.md" "$PACK_DIR/docs" true
copy_file "$REPO_ROOT/docs/ops/EVIDENCE_INDEX.json" "$PACK_DIR/docs" true

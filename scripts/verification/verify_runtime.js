...
function checkVersions() {
    ...
    try {
        const nodeVersion = (0, node_child_process_1.execSync)('node --version', { encoding: 'utf8' }).trim();
        const nodeMatch = nodeVersion.match(/v(\d+)\.(\d+)/);
        const nodeMajor = nodeMatch ? parseInt(nodeMatch[1], 10) : 0;
        check('Node.js Version', nodeMajor >= 18, `${nodeVersion} (required: >=18.18)`, nodeMajor < 18 ? ['Upgrade Node.js to v18.18 or later'] : undefined);
    }
    catch {
        check('Node.js Version', false, 'Failed to detect Node.js version');
    }
    try {
        const pnpmVersion = (0, node_child_process_1.execSync)('pnpm --version', { encoding: 'utf8' }).trim();
        const pnpmMatch = pnpmVersion.match(/^(\d+)/);
        const pnpmMajor = pnpmMatch ? parseInt(pnpmMatch[1], 10) : 0;
        check('pnpm Version', pnpmMajor >= 9, `v${pnpmVersion} (required: >=9.0.0)`, pnpmMajor < 9 ? ['Upgrade pnpm: npm install -g pnpm@latest'] : undefined);
    }
    catch {
...
function checkLockfile() {
    ...
    check('pnpm-lock.yaml exists', lockfileExists, lockfileExists ? 'Present' : 'Missing', lockfileExists ? undefined : ['Run: pnpm install to generate lockfile']);
    if (lockfileExists) {
        const npmrcPath = (0, node_path_1.join)(ROOT_DIR, '.npmrc');
        if ((0, node_fs_1.existsSync)(npmrcPath)) {
            const npmrc = (0, node_fs_1.readFileSync)(npmrcPath, 'utf8');
...
function findPackages() {
    ...
    for (const dir of workspaceDirs) {
        const fullPath = (0, node_path_1.join)(ROOT_DIR, dir);
        if (!(0, node_fs_1.existsSync)(fullPath))
            continue;
        // Check if the directory itself is a package
        if ((0, node_fs_1.existsSync)((0, node_path_1.join)(fullPath, 'package.json'))) {
            packages.push(fullPath);
        }
        // Check subdirectories
        try {
            const entries = (0, node_fs_1.readdirSync)(fullPath);
            for (const entry of entries) {
                const entryPath = (0, node_path_1.join)(fullPath, entry);
                if ((0, node_fs_1.statSync)(entryPath).isDirectory()) {
                    if ((0, node_fs_1.existsSync)((0, node_path_1.join)(entryPath, 'package.json'))) {
...
function analyzePackageRunner(pkgPath) {
    const pkgJsonPath = (0, node_path_1.join)(pkgPath, 'package.json');
    ...
    try {
        ...
        const hasJestConfig = (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'jest.config.js')) ||
            (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'jest.config.cjs')) ||
            (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'jest.config.ts')) ||
            (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'jest.config.mjs'));
        const hasVitestConfig = (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'vitest.config.ts')) ||
            (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'vitest.config.js')) ||
            (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'vitest.config.mjs'));
...
function checkRunnerConfigs() {
    section('Runner Configuration Consistency');
    const packages = findPackages();
    const dualRunnerPackages = [];
    const orphanConfigPackages = [];
    let jestCount = 0;
    let vitestCount = 0;
    let nodeTestCount = 0;
    for (const pkgPath of packages) {
        const info = analyzePackageRunner(pkgPath);
        if (!info || !info.testScript)
            continue;
        if (info.usesJest)
            jestCount++;
        if (info.usesVitest)
            vitestCount++;
        if (info.usesNodeTest)
            nodeTestCount++;
    ...
    check('No dual-runner packages', dualRunnerPackages.length === 0, dualRunnerPackages.length === 0
        ? 'All packages use single runner'
        : `${dualRunnerPackages.length} package(s) use multiple runners`, dualRunnerPackages.length > 0
        ? [...dualRunnerPackages, '', 'FIX: Choose one runner per package']
        : undefined);
    check('No orphan runner configs', orphanConfigPackages.length === 0, orphanConfigPackages.length === 0
        ? 'All configs are in use'
        : `${orphanConfigPackages.length} orphan config(s) found`, orphanConfigPackages.length > 0
...
function checkModuleResolution() {
    ...
    for (const mod of criticalModules) {
        try {
            ...
            const result = (0, node_child_process_1.execSync)(`node -e "require.resolve('${mod.name}')"`, { encoding: 'utf8', cwd: ROOT_DIR, stdio: ['pipe', 'pipe', 'pipe'] });
            check(`Module: ${mod.name}`, true, 'Resolvable');
        }
        catch (e) {
            const passed = !mod.required;
            check(`Module: ${mod.name}`, passed, passed ? 'Not installed (optional)' : 'MISSING - required dependency', passed ? undefined : [`Install: pnpm add ${mod.name}`]);
        }
    }
  ...
  `;
    try {
        (0, node_child_process_1.execSync)(`node --input-type=module -e "${esmCheck}"`, {
            encoding: 'utf8',
            cwd: ROOT_DIR,
...
function checkPackageType() {
    ...
    const rootPkgPath = (0, node_path_1.join)(ROOT_DIR, 'package.json');
    try {
        const rootPkg = JSON.parse((0, node_fs_1.readFileSync)(rootPkgPath, 'utf8'));
        const isEsm = rootPkg.type === 'module';
        check('Root package type', isEsm, isEsm ? '"module" (ESM)' : `"${rootPkg.type || 'commonjs'}" - should be "module"`, isEsm ? undefined : ['Set "type": "module" in root package.json']);
        // Check that jest config uses .cjs
        const jestConfigCjs = (0, node_fs_1.existsSync)((0, node_path_1.join)(ROOT_DIR, 'jest.config.cjs'));
        const jestConfigJs = (0, node_fs_1.existsSync)((0, node_path_1.join)(ROOT_DIR, 'jest.config.js'));
        const jestConfigTs = (0, node_fs_1.existsSync)((0, node_path_1.join)(ROOT_DIR, 'jest.config.ts'));
        if (isEsm) {
            check('Jest config extension', jestConfigCjs && !jestConfigJs, jestConfigCjs ? 'Uses .cjs (correct for ESM)' : 'Should use .cjs for ESM compatibility', !jestConfigCjs ? ['Rename jest.config.js to jest.config.cjs'] : undefined);
        }
    }
    catch {
...
function checkConfigCompatibility() {
    ...
    for (const pkgPath of packages) {
        ...
        try {
            ...
            if (isEsm) {
                const hasJsConfig = (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'jest.config.js'));
                const hasCjsConfig = (0, node_fs_1.existsSync)((0, node_path_1.join)(pkgPath, 'jest.config.cjs'));
    ...
    check('ESM/CJS config compatibility', incompatibleConfigs.length === 0, incompatibleConfigs.length === 0
        ? 'All configs use correct extensions'
        : `${incompatibleConfigs.length} incompatible config(s)`, incompatibleConfigs.length > 0
...
function checkTestSetupFiles() {
    ...
    for (const file of setupFiles) {
        const fullPath = (0, node_path_1.join)(ROOT_DIR, file.path);
    ...
    check('Test setup files exist', allExist, allExist ? 'All required setup files present' : `Missing ${missingFiles.length} file(s)`, missingFiles.length > 0 ? missingFiles : undefined);
}
// ============================================================================
// Check 8: Transform Patterns
// ============================================================================
function checkTransformPatterns() {
    section('Transform Configuration');
    // Check that transformIgnorePatterns is configured for ESM packages
    const jestConfigPath = (0, node_path_1.join)(ROOT_DIR, 'jest.config.cjs');
    if (!(0, node_fs_1.existsSync)(jestConfigPath)) {
        check('Transform patterns', false, 'jest.config.cjs not found');
        return;
    }
    try {
        const configContent = (0, node_fs_1.readFileSync)(jestConfigPath, 'utf8');
        // Check for transformIgnorePatterns
        const hasTransformIgnore = configContent.includes('transformIgnorePatterns');
        check('transformIgnorePatterns configured', hasTransformIgnore, hasTransformIgnore ? 'Present' : 'Missing - may cause ESM import issues', hasTransformIgnore ? undefined : ['Add transformIgnorePatterns to handle ESM-only packages']);
        // Check for moduleNameMapper for .js extensions
        const hasJsMapper = configContent.includes('.js$');
        check('JS extension mapper', hasJsMapper, hasJsMapper ? 'Configured' : 'Missing - may cause ESM resolution issues', hasJsMapper ? undefined : ['Add moduleNameMapper: { "^(.*)\\.js$": "$1" }']);
    }
    catch {

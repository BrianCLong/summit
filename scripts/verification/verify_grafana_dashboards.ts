...
function isDashboardCandidate(value: unknown): value is Dashboard {
  if (!value || typeof value !== 'object') {
    return false;
  }
  const candidate = value as Dashboard;
  return (
...
function parseDashboard(file: string): Dashboard | null {
  try {
    const raw = JSON.parse(readFileSync(join(ROOT_DIR, file), 'utf-8'));
    if (!isDashboardCandidate(raw)) {
      return null;
    }
    const dashboard = ('dashboard' in raw)
      ? (raw as { dashboard: Dashboard }).dashboard
      : (raw as Dashboard);
    return isDashboardCandidate(dashboard) ? dashboard : null;
  } catch (error) {
...
function validatePanels(
  ...
  panels.forEach((panel, index) =>
    validatePanel(panel as Panel, file, `${basePath}/panels/${index}`, errors),
  );
}

function validateRows(rows: unknown, file: string, basePath: string, errors: ValidationError[]): void {
  ...
  rows.forEach((row, index) => {
    if (!row || typeof row !== 'object') {
      errors.push({
        file,
        path: `${basePath}/rows/${index}`,
        message: 'row must be an object',
      });
      return;
    }
    const panels = (row as { panels?: unknown }).panels;
    validatePanels(panels, file, `${basePath}/rows/${index}`, errors);
  });
}
  ...
  targets.forEach((target, index) => {
    ...
    if (!isNonEmptyString(queryField)) {
      errors.push({
        ...
        path: `${path}/expr`,
        message:
          'target is missing a query expression (expr/query/target/rawSql/metric)',
      });
    }
  });
}
...
function validatePanel(panel: Panel, file: string, path: string, errors: ValidationError[]): void {
  ...
  const needsTargets =
    panelType !== 'row' && panelType !== 'text' && panelType !== 'table-old';
...
function validateDashboard(dashboard: Dashboard, file: string): ValidationError[] {
  ...
  if (!isNonEmptyString(dashboard.title)) {
    errors.push({
      file,
      path: '/title',
      message: 'dashboard title must be a non-empty string',
    });
  }
  ...
  if (dashboard.panels) {
    ...
    validateRows(dashboard.rows as unknown, file, '', errors);
  } else {
    errors.push({
      file,
      path: '/panels',
      message: 'dashboard must define panels',
    });
  }

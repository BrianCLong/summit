...
function checkBudget(name, current, baselineVal, isMaxLimit) {
  if (current === null || current === 0) {
    // Treating 0 as invalid for these metrics
...
if (!checkBudget("API Throughput", currentThroughput, baseline.api_throughput_rps, false))
  failed = true;

if (baseline.queue_throughput_rps && currentQueueThroughput !== null) {
  if (
    !checkBudget("Queue Throughput", currentQueueThroughput, baseline.queue_throughput_rps, false)
  )
    failed = true;
}

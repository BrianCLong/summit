...
function runPolicySuite(options = {}) {
  const baseRef =
    options.baseRef || process.env.TEST_POLICY_BASE || process.env.GITHUB_BASE_REF || "origin/main";
  const results = [];
  ...
  if (!report.summary.passed) {
    ...
    throw new Error(
      failed ? `Test policy requirements failed: ${failed}` : "Test policy requirements failed."
    );
  }
  return report;
}

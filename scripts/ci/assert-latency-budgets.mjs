...
const budgetsDoc = YAML.parse(
  fs.readFileSync(path.resolve('perf/models.yaml'), 'utf8'),
);
...
const journeyBudget =
  journeys.find((journey) => journey.id === journeyId) ?? journeys[0];
...
const getPercentile = (metric, key) => {
  ...
  const source =
    metric.percentiles || metric.values || metric.thresholds || metric.data;
...
fail(
  Number.isNaN(p95),
  'p95 latency missing from k6 summary output',
  'p95',
);
fail(
  Number.isNaN(p99),
  'p99 latency missing from k6 summary output',
  'p99',
);
fail(
  Number.isNaN(errorRate),
  'Error rate missing from k6 summary output',
  'errorRate',
);

...
function usage(): never {
  ...
  console.error(
    "Usage: ts-node scripts/ci/validate-pr-metadata.ts --body <path> [--registry <path>] [--output <artifact>]"
  );
  process.exit(1);
}
...
function validateMetadataShape(metadata: AgentMetadata): void {
  ...
  if (
    ...
    !Array.isArray(metadata.declared_scope.paths) ||
    metadata.declared_scope.paths.length === 0
  ) {
...
function main(): void {
  ...
  const diffViolations = changedFiles.filter(
    (file) => !declaredPaths.some((allowed) => file.startsWith(allowed))
  );
  ...
  if (
    prompt.scope?.domains &&
    metadata.domains.some((domain) => !prompt.scope.domains?.includes(domain))
  ) {

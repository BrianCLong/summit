...
function main() {
    ...
    if (!fs.existsSync(REGISTRY_FILE)) {
        console.error('❌ Registry not found! Run scripts/compliance/generate_debt_registry.cjs first.');
        process.exit(1);
    }
    ...
    if (retiredDebt.length > 0) {
        console.log(`💡 NOTE: ${retiredDebt.length} items have been retired. Consider updating the registry.`);

...
function runGit(command) {
    try {
        return (0, node_child_process_1.execSync)(command, { encoding: 'utf8', cwd: rootDir }).trim();
    }
    catch (e) {
...
function loadJson(filepath) {
    const fullPath = node_path_1.default.resolve(rootDir, filepath);
...
function getFileContent(filepath) {
    const fullPath = node_path_1.default.resolve(rootDir, filepath);
    if (node_fs_1.default.existsSync(fullPath) && node_fs_1.default.lstatSync(fullPath).isFile()) {
...
function checkAgentRules(changedFiles, violations, actor) {
    const rulesConfig = loadJson(node_path_1.default.join(POLICY_DIR, 'agent-rules.json'));
    if (!rulesConfig)
        return;
    const isAgent = actor === 'Jules' || actor === 'github-actions[bot]' || (process.env.IS_AGENT === 'true');
    for (const rule of rulesConfig.rules) {
        // File path checks
        if (rule.files) {
            const protectedMatches = changedFiles.filter(file => rule.files.some(pattern => matchGlob(file, pattern)));
...
function main() {
    ...
    node_fs_1.default.writeFileSync(ARTIFACT_PATH, JSON.stringify(provenance, null, 2));

...
const agent_gym_1 = require("@summit/agent-gym");
const agent_gym_metrics_1 = require("@summit/agent-gym-metrics");
...
async function runSuite() {
    ...
    const envs = [
        new agent_gym_1.BrowserOpsEnvironment(),
        new agent_gym_1.SpreadsheetOpsEnvironment(),
        new agent_gym_1.MCPToolChainEnvironment()
    ];
    const agent = new RandomAgent();
    const allResults = [];
    for (const env of envs) {
        console.log(`Running episodes for ${env.name}...`);
        const runner = new agent_gym_1.GymRunner(env, agent, { maxSteps: 5 });
    ...
    // Calculate Metrics
    const successRate = agent_gym_metrics_1.AgentMetrics.calculateSuccessRate(allResults);
    const avgSteps = agent_gym_metrics_1.AgentMetrics.calculateAverageSteps(allResults);
    // Looping index for all turns
    const allTurns = allResults.flatMap(r => r.turns);
    const looping = agent_gym_metrics_1.AgentMetrics.calculateLoopingIndex(allTurns);
    console.log('\n--- Agent Gym Report ---');

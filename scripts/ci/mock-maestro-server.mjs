...
const errorRate = Math.min(
  1,
  Math.max(0, Number(process.env.MOCK_ERROR_RATE || 0)),
);
...
const maybeInjectFailure = (res) => {
  ...
  jsonResponse(
    res,
    { status: 'error', message: 'Mock error injection triggered' },
    500,
  );
  return true;
};
...
const server = http.createServer(async (req, res) => {
  ...
  if (
    req.method === 'POST' &&
    url.pathname === '/api/conductor/pricing/simulate-selection'
  ) {
    await readBody(req);
    jsonResponse(res, {
      success: true,

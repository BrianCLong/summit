...
const OUTPUT_DIR = (0, node_path_1.join)(ROOT_DIR, '.dx-metrics');
function measureBuildTime(command) {
    const start = Date.now();
    try {
        (0, node_child_process_1.execSync)(command, { cwd: ROOT_DIR, stdio: 'pipe' });
    }
    catch {
...
function getBuildMetrics() {
    ...
    (0, node_child_process_1.spawnSync)('pnpm', ['typecheck'], { cwd: ROOT_DIR, stdio: 'pipe' });
    const typeCheckTime = Date.now() - typeCheckStart;
    // Lint time
    const lintStart = Date.now();
    (0, node_child_process_1.spawnSync)('pnpm', ['lint'], { cwd: ROOT_DIR, stdio: 'pipe' });
...
function getTestMetrics() {
    ...
    try {
        const result = (0, node_child_process_1.spawnSync)('pnpm', ['test', '--', '--json', '--passWithNoTests'], {
            cwd: ROOT_DIR,
            encoding: 'utf-8',
            timeout: 300000,
        });
...
function getCodebaseMetrics() {
    ...
    try {
        ...
        const result = (0, node_child_process_1.spawnSync)('find', ['.', '-name', '*.ts', '-o', '-name', '*.tsx', '-type', 'f'], { cwd: ROOT_DIR, encoding: 'utf-8' });
        ...
        for (const file of files.slice(0, 1000)) {
            try {
                const content = (0, node_fs_1.readFileSync)((0, node_path_1.join)(ROOT_DIR, file), 'utf-8');
...
function getDevEnvMetrics() {
    console.log('Collecting dev environment metrics...');
    let nodeVersion = '';
    let pnpmVersion = '';
    let dockerAvailable = false;
    try {
        nodeVersion = (0, node_child_process_1.execSync)('node --version', { encoding: 'utf-8' }).trim();
    }
    catch { }
    try {
        pnpmVersion = (0, node_child_process_1.execSync)('pnpm --version', { encoding: 'utf-8' }).trim();
    }
    catch { }
    try {
...
async function main() {
    ...
    // Save JSON metrics
    const jsonPath = (0, node_path_1.join)(OUTPUT_DIR, 'dx-metrics.json');
    (0, node_fs_1.writeFileSync)(jsonPath, JSON.stringify(metrics, null, 2));
    console.log(`\nJSON metrics saved to: ${jsonPath}`);
    // Generate and save report
    const report = generateReport(metrics);
    const reportPath = (0, node_path_1.join)(OUTPUT_DIR, 'dx-report.md');

...
for (const sf of files) {
  ...
  if (bp) {
    ...
    if (!hasExpressDefault) {
      ...
      else sf.addImportDeclaration({ defaultImport: "express", moduleSpecifier: "express" });
      fileChanged = true;
    }
  }
  ...
  if (expressImport) {
    const nextNamed = expressImport.getNamedImports().find((n) => n.getName() === "NextFunction");
    if (nextNamed) {
  ...
  const candidates = sf.getDescendants().filter(
    (n) => n.getKind() === SyntaxKind.ArrowFunction || n.getKind() === SyntaxKind.FunctionExpression,
  );
  ...
  for (const fn of candidates) {
    ...
    const nextRefs = body.getDescendantsOfKind(SyntaxKind.Identifier).filter((i: any) => i.getText() === "next");

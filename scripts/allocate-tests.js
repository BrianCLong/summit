...
function main() {
  ...
  if (!shardIndex || !totalShards || shardIndex < 1 || shardIndex > totalShards) {
    console.error("Usage: node allocate-tests.js <shardIndex> <totalShards> [timingFile]");
  ...
  try {
    if (fs.existsSync(timingFile)) {
      ...
      console.error(`⚠️ Warning: Timing file ${timingFile} not found, using equal distribution`);
    }
  } catch (error) {
  ...
  if (Object.keys(timings).length === 0) {
    console.error("📊 No timing data available, falling back to file-based distribution");
    timings = getDefaultTestFiles();
  }

  // Sort tests by duration (longest first for better balancing)
  const sortedTests = Object.entries(timings).sort((a, b) => (b[1] || 1) - (a[1] || 1));
  ...
  if (process.env.VERBOSE === "1") {
    ...
    buckets.forEach(bucket => {
      console.error(`  Shard ${bucket.index}: ${bucket.files.length} files, ~${Math.round(bucket.totalTime)}s`);
    });
    ...
    const skew = maxTime > 0 ? ((maxTime - minTime) / maxTime * 100).toFixed(1) : 0;

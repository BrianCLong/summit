...
class TransparencyReportGenerator {
    ...
    async collectDataUsage(tenantId, periodStart, periodEnd) {
        return {
    ...
    async collectAgentUsage(tenantId, periodStart, periodEnd) {
        // Total agent runs
        const totalRunsResult = await pg_js_1.pg.query(`SELECT COUNT(*) as count
       FROM agent_runs
       WHERE tenant_id = $1
         AND created_at >= $2
         AND created_at <= $3`, [tenantId, periodStart, periodEnd]);
        // Success rate
        const successRateResult = await pg_js_1.pg.query(`SELECT
       ...
       WHERE tenant_id = $1
         ...
         AND created_at <= $3`, [tenantId, periodStart, periodEnd]);
        // Average duration
        const avgDurationResult = await pg_js_1.pg.query(`SELECT AVG(EXTRACT(EPOCH FROM (completed_at - started_at))) as avg_duration
       FROM agent_runs
       WHERE tenant_id = $1
         AND created_at >= $2
         AND created_at <= $3
         AND status = 'completed'`, [tenantId, periodStart, periodEnd]);
        // Top agents
        const topAgentsResult = await pg_js_1.pg.query(`SELECT
         agent_id,
         COUNT(*) as runs,
         COUNT(*) FILTER (WHERE status = 'completed') as successful
       FROM agent_runs
       ...
       GROUP BY agent_id
       ORDER BY runs DESC
       LIMIT 10`, [tenantId, periodStart, periodEnd]);
    ...
    async collectSecurityPosture(tenantId, periodStart, periodEnd) {
        // Authentication events
        const authEventsResult = await pg_js_1.pg.query(`SELECT
       ...
       WHERE tenant_id = $1
         AND timestamp >= $2
         AND timestamp <= $3`, [tenantId, periodStart, periodEnd]);
        // Authorization decisions
        const authzEventsResult = await pg_js_1.pg.query(`SELECT
       ...
       WHERE tenant_id = $1
        ...
        const incidentsResult = await pg_js_1.pg.query(`SELECT
       ...
       WHERE tenant_id = $1
         AND timestamp >= $2
         AND timestamp <= $3
         AND event_type IN ('security_incident', 'policy_violation', 'suspicious_activity')`, [tenantId, periodStart, periodEnd]);
        // Data access events
        const dataAccessResult = await pg_js_1.pg.query(`SELECT
    ...
    async collectCompliancePosture(tenantId) {
        // Compliance frameworks
        const frameworksResult = await pg_js_1.pg.query(`SELECT
         framework,
       ...
       WHERE tenant_id = $1
       GROUP BY framework`, [tenantId]);
        // Audit trail stats
        const auditTrailResult = await pg_js_1.pg.query(`SELECT COUNT(*) as total_events
       FROM audit.events
    ...
    async verifyProvenance(tenantId, periodStart, periodEnd) {
        // Verify hash chain integrity (sample check)
        const integrityResult = await pg_js_1.pg.query(`SELECT
         COUNT(*) as total,
       ...
       WHERE tenant_id = $1
         AND timestamp >= $2
         AND timestamp <= $3
       LIMIT 1000`, [tenantId, periodStart, periodEnd]);
    ...
    calculateReportHash(report) {
        ...
        return crypto_1.default.createHash('sha256').update(reportJson).digest('hex');
    }
    async renderMarkdown(report) {
...
commander_1.program
    ...
    const report = await generator.generate(options.tenant, new Date(options.start), new Date(options.end));
    let output;
    ...
    if (options.output) {
        await promises_1.default.writeFile(options.output, output, 'utf-8');

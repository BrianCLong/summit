...
function getGitInfo() {
    const run = (args) => {
        const result = (0, node_child_process_1.spawnSync)('git', args, { encoding: 'utf8', stdio: 'pipe' });
...
function getToolchainInfo() {
    ...
    try {
        const result = (0, node_child_process_1.spawnSync)('pnpm', ['--version'], { encoding: 'utf8', stdio: 'pipe' });
...
function getWhitelistedEnv() {
    ...
    for (const [key, value] of Object.entries(process.env)) {
        ...
        const isWhitelisted = ENV_WHITELIST_PREFIXES.some((prefix) => key === prefix || key.startsWith(prefix));
        if (!isWhitelisted)
            continue;
...
function runCheck(name, command, args, env = {}) {
...
function runSmokeCheck(smokeUrl) {
    const startedAt = new Date().toISOString();
    const startMs = Date.now();
    const smokePath = node_path_1.default.join(process.cwd(), 'scripts/go-live/smoke-prod.sh');
    if (!node_fs_1.default.existsSync(smokePath)) {
    ...
    const result = (0, node_child_process_1.spawnSync)('bash', [smokePath, ...args], {
        env: process.env,
        encoding: 'utf8',
        stdio: ['inherit', 'pipe', 'pipe'],
...
function generateMarkdownSummary(evidence) {
...
async function main() {
    ...
    // Output directory
    const outDir = node_path_1.default.join('artifacts', 'evidence', 'go-live', git.sha);
    ...
    // Write outputs
    const evidencePath = node_path_1.default.join(outDir, 'evidence.json');
    const mdPath = node_path_1.default.join(outDir, 'evidence.md');
    const checksumPath = node_path_1.default.join(outDir, 'checksums.txt');
    node_fs_1.default.writeFileSync(evidencePath, JSON.stringify(evidence, null, 2));
    console.log(`\n[evidence] Wrote ${evidencePath}`);
    node_fs_1.default.writeFileSync(mdPath, generateMarkdownSummary(evidence));

...
def main() -> int:
    ...
    # Load governed exceptions if they exist
    timestamp_exceptions = set()
    exc_path = EVID / "governed_exceptions.json"
    if exc_path.exists():
        try:
            exc_data = load(exc_path)
            timestamp_exceptions = set(exc_data.get("timestamps", []))
        except Exception:
            pass
    ...
    for p in EVID.rglob("*"):
        ...
        if p.name == "stamp.json" or p.is_dir() or p.suffix not in {".json", ".md", ".yml", ".yaml", ".jsonl"} or p.name.endswith(".schema.json"):
            continue
        if p.name in IGNORE or any(d in p.parts for d in IGNORE_DIRS) or rel_p in timestamp_exceptions:
            continue
        try:

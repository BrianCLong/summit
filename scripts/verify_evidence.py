...
def main() -> int:
    ...
    IGNORE = {
        ...
        ,"evidence/governed_exceptions.json", "evidence/ai-influence-ops/report.json", "evidence/bundles/SHINY-SAAS-VISH-20260130/report.json", "evidence/ga/v5.3.2/ATTESTATION_SUMMARY.md"
    }
    ...
    IGNORE_PATHS = {
        ...
        "evidence/EVD-NARRATIVE_IOPS_20260129-FRAMES-001/metrics.json",
    ...
    for p in EVID.rglob("*"):
        if p.name == "stamp.json" or p.is_dir() or p.suffix not in {".json", ".md", ".yml", ".yaml", ".jsonl"} or p.name.endswith(".schema.json"):
            continue
        if p.name in IGNORE or any(d in p.parts for d in IGNORE_DIRS) or str(p.relative_to(ROOT)) in IGNORE_PATHS:
            continue
        try:

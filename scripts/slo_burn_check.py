...
BUDGET_KEYS = [
  ("graphql_read_p95_ms", "histogram_quantile(0.95, sum by(le,job) (rate(http_request_duration_seconds_bucket{route=~\"/graphql.*\",job=~\"api|gateway\"}[5m]))) * 1000"),
  ("graphql_write_p95_ms", "histogram_quantile(0.95, sum by(le,job) (rate(http_request_duration_seconds_bucket{route=~\"/graphql.*\",method=\"POST\",job=~\"api|gateway\"}[5m]))) * 1000"),
]
...
if __name__ == "__main__":
    ...
    budget_path = sys.argv[sys.argv.index('--budget')+1] if '--budget' in sys.argv else '.maestro/ci_budget.json'

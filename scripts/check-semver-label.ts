...
export function mapLabelToLevel(label: string): SemverLevel | null {
  return SEMVER_LABEL_MAP[label.toLowerCase() as SemverLabel] ?? null;
}
...
export function evaluateLabels(labels: string[]): CheckReport {
  ...
  const semverLabels = normalized
    .map((label) => ({ raw: label, bump: mapLabelToLevel(label) }))
  ...
  const selected = semverLabels[0] ?? null;
  ...
  return {
    ...
    labels: normalized,
    status,
    label: (selected?.raw as SemverLabel | undefined) ?? null,
...
async function loadLabelsFromEventPayload(): Promise<{ labels: string[]; source: CheckReport['source'] }> {
  ...
  if (process.env.PR_LABELS) {
    const labels = process.env.PR_LABELS.split(',').map((value) => value.trim());
...
export async function run(options: CheckOptions): Promise<{ report: CheckReport; exitCode: number }> {
  const { fixturePath, warnOnly = false, outputPath } = options;
  ...
  const exitCode = warnOnly ? 0 : report.status === 'fail' ? 1 : 0;
...
async function main(): Promise<void> {
  ...
  const { report, exitCode } = await run({ fixturePath, warnOnly, outputPath });
  printReport(report);

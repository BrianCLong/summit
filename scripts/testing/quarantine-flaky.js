...
const loadMetadata = async (metadataPath) => {
    ...
    const content = await (0, promises_1.readFile)(metadataPath, 'utf8');
    try {
        const parsed = JSON.parse(content);
        return parsed.tests || [];
    }
    catch (error) {
        console.warn(`[quarantine] Unable to parse metadata at ${metadataPath}: ${error.message}`);
        return [];
    }
};
const persistMetadata = async (metadataPath, tests) => {
    const payload = { tests };
    await (0, promises_1.mkdir)(path_1.default.dirname(metadataPath), { recursive: true });
    await (0, promises_1.writeFile)(metadataPath, JSON.stringify(payload, null, 2));
};
const parseReport = async (reportPath) => {
    ...
    const content = await (0, promises_1.readFile)(reportPath, 'utf8');
    try {
        const parsed = JSON.parse(content);
        ...
        console.warn(`[quarantine] Unable to parse report at ${reportPath}: ${error.message}`);
        return [];
    }
};
const extractTestsFromArgs = (tests) => tests
    .filter(Boolean)
    .map((entry) => {
...
const parseArgs = () => {
    ...
    const tests = [];
    const options = { applyTags: false, status: 'quarantined' };
    ...
    return {
        ...
        metadataPath: options.metadataPath || path_1.default.join('testing', 'flaky-metadata.json'),
        applyTags: Boolean(options.applyTags),
    };
};
const findTestInsertionPoint = (content, testName) => {
...
const tagTestInFile = async (filePath, testName) => {
    ...
    await (0, promises_1.writeFile)(filePath, taggedContent);
};
const mergeRecords = (existing, incoming, defaults) => {
    ...
    incoming.forEach((record) => {
        ...
        const next = {
            ...
            status: (record.status || prior?.status || defaults.status),

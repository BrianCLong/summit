...
- `health-check.mjs` - Validate system integrity
...
```bash
GITHUB_TOKEN=... DRY_RUN=false MAX_FIX_SCOPE=50 PROJECT19_SCHEMA=scripts/config/project19-field-schema.json PROJECT_ID=project-19-id npm run ensure-fields
```
...
```bash
GITHUB_TOKEN=... DRY_RUN=false MAX_FIX_SCOPE=200 PROJECT_ID=project-19-id EVENT_PATH=path/to/event.json npm run process:event
```
...
```bash
GITHUB_TOKEN=... DRY_RUN=false MAX_FIX_SCOPE=500 PROJECT_ID=project-19-id npm run reconcile:project19
```
...
Labels on issues/PRs are automatically mapped to Project fields using the mapping in `project19-label-map.json`:

- Security-related labels → `Delivery Class: Security`, `Governance Gate: Security`
- Milestones → `Release Train` field
- Priority labels (P0, P1, etc.) → `Priority` field
- Assignees → `Primary Agent` field

### CI/CD Artifacts to Fields

Workflow artifacts are parsed and mapped to fields:

- `stamp.json` → `CI Status Snapshot`, `Evidence Bundle ID`, `Determinism Risk`
- `report.json` → Gate statuses, evidence completeness
- `coverage-summary.json` → `Test Coverage Delta`
- Security artifacts → `Audit Criticality`, `External Audit Scope`
...
5. **Computed field protection** - Prevents manual edits to calculated fields
6. **Comprehensive reporting** - All runs produce detailed reports and summaries

## Executive and Audit Dashboards

The system supports these standardized Project Views:

### Executive Views

- **Exec / Release Readiness** - GA and upcoming release progress tracking
- **Exec / Top Blockers** - Critical items blocking progress
- **Exec / Portfolio Allocation** - Work distribution across delivery classes and strategic themes
- **Exec / Automation Throughput** - Agent and automation effectiveness tracking

### Audit Views

- **Audit / Scope WIP** - Audit-relevant work in progress
- **Audit / Evidence Missing** - Items requiring evidence that aren't complete
- **Audit / Determinism Exceptions** - Items with CI/CD health issues
- **Audit / Change Control** - Items requiring explicit approvals
...
1. **"Field not found" errors** - Run `ensure-fields` to create missing fields

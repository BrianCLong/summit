...
```bash
GITHUB_TOKEN=... DRY_RUN=false MAX_FIX_SCOPE=200 PROJECT_ID=project-19-id WORKFLOW_RUN_PATH=path/to/workflow-run.json npm run process:ci-signals
```
...
- **Exec / Portfolio Allocation** - Work distribution across delivery classes
- **Exec / Automation Throughput** - Agent and automation effectiveness
...
- **Projects store only pointers and summaries** (e.g., Evidence Complete = Yes/No; Evidence Bundle ID = string)
- **Full evidence remains in CI artifacts** and governance docs

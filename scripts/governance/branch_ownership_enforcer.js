...
function getOwnerForFile(file, rules) {
...
async function main() {
    ...
    try {
        if (prNumber && process.env.GITHUB_ACTIONS) {
            ...
            const out = (0, child_process_1.execSync)(`gh pr view ${prNumber} --json files --jq '.files.[].path'`, { encoding: 'utf-8' });
            changedFiles = out.split('\n').filter(Boolean);
        }
        else {
            // Local test fallback
            console.log(`Fetching changed files via git diff...`);
            const out = (0, child_process_1.execSync)(`git diff --name-only HEAD~1 HEAD`, { encoding: 'utf-8' });
            changedFiles = out.split('\n').filter(Boolean);
        }
    }
    catch (e) {
    ...
    if (prNumber) {
        try {
            ...
            (0, child_process_1.execSync)(`gh pr edit ${prNumber} ${addArgs.trim()}`);

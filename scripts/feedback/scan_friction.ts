...
function scanFriction(): FrictionMetrics {
  ...
  try {
    const todoOutput = execSync(
      'grep -r "TODO" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=dist || true',
      { encoding: "utf-8" }
    );
    metrics.todoCount = todoOutput.split("\n").filter(Boolean).length;

    const fixmeOutput = execSync(
      'grep -r "FIXME" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=dist || true',
      { encoding: "utf-8" }
    );
    metrics.fixmeCount = fixmeOutput.split("\n").filter(Boolean).length;

    // Check for skipped tests in common test files
    const skippedOutput = execSync(
      'grep -r "\.skip" . --include="*.test.ts" --include="*.spec.ts" --exclude-dir=node_modules --exclude-dir=.git || true',
      { encoding: "utf-8" }
    );

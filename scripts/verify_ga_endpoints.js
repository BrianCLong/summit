...
const index_js_1 = require("../server/src/feature-flags/index.js");
...
async function runGAVerification() {
    ...
    // 2. Readyz
    try {
        const res = await axios_1.default.get(`${BASE_URL}/readyz`);
    ...
    // 3. Analyze (Valid)
    try {
        const res = await axios_1.default.post(`${BASE_URL}/api/v1/analyze`, {
            text: 'Test content',
    ...
    try {
        await axios_1.default.post(`${BASE_URL}/api/v1/analyze`, {
            context: { locale: 'en-US' }
        });
    ...
    try {
        await axios_1.default.post(`${BASE_URL}/api/v1/counter`, {
            narrative_id: '123',
    ...
    try {
        const res = await axios_1.default.post(`${BASE_URL}/api/v1/counter`, {
            narrative_id: '123',
            mode: 'prebunk',
    ...
    // 7. Feature Flags
    if ((0, index_js_1.isGAFlagEnabled)('detector_v3') && !(0, index_js_1.isGAFlagEnabled)('ephemeral_frame_detector')) {

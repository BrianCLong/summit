...
function getCommitCount(fromRef, toRef) {
...
function identifyRisks(filePath) {
    const risks = [];
...
function aggregateComponents(changes) {
...
function calculateComponentRisk(component) {
    // Check for critical risk factors
    const criticalRisks = component.riskFactors.filter(r => RISK_PATTERNS.find(p => p.risk === r && p.level === 'critical'));
    if (criticalRisks.length > 0)
        return 'critical';
    // Check for high risk factors
    const highRisks = component.riskFactors.filter(r => RISK_PATTERNS.find(p => p.risk === r && p.level === 'high'));
    if (highRisks.length > 0)
        return 'high';
    // Large changes are risky
    if (component.additions + component.deletions > 500)
        return 'high';
    if (component.additions + component.deletions > 200)
        return 'medium';
    // Medium risk factors
    if (component.riskFactors.length > 0)
        return 'medium';
    // Component-based risk
    if (['Database', 'Authentication', 'Security'].includes(component.name))
        return 'medium';
    return 'low';
}
function aggregateRisks(changes) {
...
function calculateOverallRisk(components, risks, totalChanges) {
    let score = 0;
...
function analyzeTestCoverage(changes) {
    ...
    for (const change of nonTestChanges) {
        if (change.path.endsWith('.ts') || change.path.endsWith('.tsx') || change.path.endsWith('.js')) {
            const hasTest = testFiles.some(t => t.path.includes(node_path_1.default.basename(change.path, node_path_1.default.extname(change.path))));
...
|-----------|------|-------|---------|--------------|
${report.components.map(c => `| ${c.name} | ${c.riskLevel.toUpperCase()} | ${c.files} | +${c.additions}/-${c.deletions} | ${c.riskFactors.join(', ') || '-'} |`).join('\n')}`);
    // Risks
    if (report.risks.length > 0) {
...
`);
    ...
    if (positionalArgs.length >= 1)
        fromRef = positionalArgs[0];
    if (positionalArgs.length >= 2)
        toRef = positionalArgs[1];
    console.log('========================================');
    ...
    const { level: riskLevel, score: riskScore } = calculateOverallRisk(components, risks, totalAdditions + totalDeletions);
    const report = {
    ...
    if (outputPath && content) {
        node_fs_1.default.writeFileSync(outputPath, content);

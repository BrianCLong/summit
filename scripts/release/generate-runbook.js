...
function getPreviousVersion(currentTag) {
...
function getCommitCount(fromTag, toTag) {
    const result = run('git', ['rev-list', '--count', `${fromTag}..${toTag}`]);
    return parseInt(result.output, 10) || 0;
}
function getChangedFiles(fromTag, toTag) {
    const result = run('git', ['diff', '--name-only', fromTag, toTag]);
    return result.output.split('\n').filter(Boolean);
}
function hasBreakingChanges(fromTag, toTag) {
    const result = run('git', ['log', '--oneline', '--grep=BREAKING', `${fromTag}..${toTag}`]);
    return result.output.split('\n').filter(Boolean);
}
function hasMigrations(changedFiles) {
    return changedFiles.some(f => f.includes('migration') ||
        f.includes('migrate') ||
        f.match(/prisma\/.*\.sql/) ||
        f.match(/knex\/.*\.js/));
}
function generateMarkdownRunbook(config) {
    const typeEmoji = {
        standard: '🚀',
        hotfix: '🔥',
        rollback: '⏪',
...
**Environment:** ${envEmoji[config.environment] || '🔹'} ${config.environment}
...
function generateHtmlRunbook(config) {

...
function checkCompatibility(bundleIndex) {
  ...
  if (bundleMajor > SUPPORTED_MAJOR_VERSION) {
    return {
      ...
      details: {
        bundleVersion: bundleIndex.schemaVersion,
        supportedVersion: `${SUPPORTED_MAJOR_VERSION}.x.x`,
      },
    };
  }
    ...
    try {
      ...
      try {
        ...
        throw new ReleaseBundleError(
          "INVALID_JSON",
          `Failed to parse bundle-index.json: ${e.message}`,
      ...
      if (indexJson.files && Array.isArray(indexJson.files)) {
        ...
        indexJson.files.forEach((f) => {
          ...
          if (!canonicalHashes.has(f.path)) {
            addError(
              "INDEX_EXTRA_FILE",
              `bundle-index.json lists file not in SHA256SUMS: ${f.path}`
            );
      ...
      if (indexJson.pointers) {
        ...
        for (const [ptrName, ptrTarget] of Object.entries(indexJson.pointers)) {
          if (!canonicalHashes.has(ptrTarget)) {
            addError(
              "POINTER_INVALID",
              `Pointer ${ptrName} -> ${ptrTarget} targets file not in SHA256SUMS`
            );
          }
        }
      ...
      if (provJson.subject && Array.isArray(provJson.subject)) {
        ...
        provJson.subject.forEach((sub) => {
          ...
          if (!canonicalHashes.has(sub.name)) {
            ...
            if (sub.digest && sub.digest.sha256) {
              if (
                sub.name !== "provenance.json" &&
                sub.digest.sha256 !== canonicalHashes.get(sub.name)
              ) {
        ...
        if (missingSubjects.length > 0) {
          missingSubjects.forEach((f) => {
            addError(
              "PROV_MISSING_SUBJECT",
              `SHA256SUMS file missing from Provenance subjects: ${f}`
            );
          });
        } else {

...
function getUser() {
    ...
    return gitUser.success ? gitUser.output : node_os_1.default.userInfo().username;
}
function generateId() {
    return node_crypto_1.default.randomBytes(4).toString('hex');
}
...
function readSchedule() {
    const filePath = node_path_1.default.join(process.cwd(), SCHEDULE_FILE);
...
function writeSchedule(releases) {
    const filePath = node_path_1.default.join(process.cwd(), SCHEDULE_FILE);
    node_fs_1.default.writeFileSync(filePath, JSON.stringify(releases, null, 2) + '\n');
}
function scheduleRelease(version, scheduledAt, environment, notify, dryRun) {
    const releases = readSchedule();
    // Check for conflicts
    const conflicting = releases.find(r => r.status === 'pending' &&
...
function rescheduleRelease(id, newTime) {
    const releases = readSchedule();
    const idx = releases.findIndex(r => r.id === id);
    if (idx === -1) {
        console.error(`Release not found: ${id}`);
...
function listReleases(showAll) {
    ...
    const now = new Date();
    for (const release of filtered.sort((a, b) => new Date(a.scheduledAt).getTime() - new Date(b.scheduledAt).getTime())) {
...
function checkAndExecuteDue() {
    const releases = readSchedule();
    const now = new Date();
    const dueReleases = releases.filter(r => r.status === 'pending' &&

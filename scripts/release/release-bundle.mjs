...
const resolveScript = (name) => {
  const locations = [resolve("scripts", name), resolve("scripts/release", name)];
...
async function main() {
  ...
  if (!tagSha) {
    console.warn(
      `⚠️  Tag ${values.tag} not found locally. Proceeding with caution (assuming it will be created or is a target).`
    );
  } else {
  ...
  try {
    const files = runSilent(`find ${DIST_RELEASE} -type f -not -name "checksums.txt"`)
      .split("\n")
      .filter(Boolean);
    let checksums = "";
    for (const file of files) {
      ...
      const shasum =
        runSilent(`shasum -a 256 "${file}" | awk '{print $1}'`) ||
        runSilent(`sha256sum "${file}" | awk '{print $1}'`);
      if (shasum) {

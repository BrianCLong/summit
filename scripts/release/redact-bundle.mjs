...
const SENSITIVE_KEYS = [
  ...
  "sender",
  "pusher",
  "committer", // broad stroke on usernames
];
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (isSensitiveKey) {
        ...
        if (
          ...
          (lowerVal.includes("github.com") || lowerVal.includes("gitlab"))
        ) {
...
try {
  ...
  const files = getAllFiles(BUNDLE_DIR).filter(
    (f) => !f.endsWith("checksums.txt") && !f.endsWith("SHA256SUMS")
  );
  let checksums = "";

...
try {
  ...
  try {
    ...
    console.log(
      `Resolved ${values.from} to ${sha} (warning: treated as direct ref, not dereferenced tag)`
    );
  } catch (e2) {
...
if (branchExists) {
  const branchSha = execSync(`git rev-parse refs/heads/${branchName}`, {
    encoding: "utf-8",
  }).trim();
  if (branchSha !== sha) {
    console.error(
      `Error: Branch ${branchName} already exists and points to a different commit (${branchSha}).`
    );

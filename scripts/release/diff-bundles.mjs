...
function printReport(result) {
  ...
  if (
    ...
    if (result.files.removed.length > 0)
      console.log(`  [-] Removed: ${result.files.removed.join(", ")}`);
    if (result.files.changed.length > 0)
      console.log(`  [~] Changed: ${result.files.changed.join(", ")}`);
  }
  ...
  if (result.schemaWarnings.length > 0) {
    ...
    for (const warning of result.schemaWarnings) {
      console.log(
        `  ! ${warning.file}: schemaVersion changed from "${warning.a}" to "${warning.b}". Possible incompatible bundle consumers.`
      );
    }
  }
  console.log(`\n----------------------`);

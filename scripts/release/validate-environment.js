...
function checkUrl(url, expectedStatus = 200, timeout = 10) {
    return new Promise(resolve => {
        const startTime = Date.now();
        const urlObj = new URL(url);
        const protocol = urlObj.protocol === 'https:' ? node_https_1.default : node_http_1.default;
...
function checkKubernetesConnectivity(context) {
...
function checkKubernetesNamespace(namespace, context) {
    const args = context
...
function checkKubernetesSecret(name, namespace, context) {
    const args = context
...
function checkKubernetesConfigMap(name, namespace, requiredKeys, context) {
    const args = context
...
function checkDatabaseConnectivity(name, type, connectionString) {
...
function checkResourceAvailability(namespace, minPods, context) {
    if (!minPods) {
        return {
...
function checkHelmRelease(releaseName, namespace, context) {
    const args = context
...
async function validateEnvironment(config, skipChecks, timeout) {
    const checks = [];
    ...
    if (config.resources && config.cluster?.namespace && !skip.has('resources')) {
        checks.push(checkResourceAvailability(config.cluster.namespace, config.resources.minPods, config.cluster.context));
    }

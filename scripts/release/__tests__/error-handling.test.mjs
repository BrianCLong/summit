...
const SCRIPT_PATH_VALIDATE = join(
  process.cwd(),
  "scripts",
  "release",
  "validate-bundle-schemas.mjs"
);
...
describe("Release Bundle Error Handling", () => {
  ...
  it("throws SCHEMA_MAJOR_UNSUPPORTED for unsupported major version", () => {
    ...
    assert(error, "Script should have failed");
    assert(
      output.includes("[SCHEMA_MAJOR_UNSUPPORTED]"),
      `Expected SCHEMA_MAJOR_UNSUPPORTED, got: ${output}`
    );
  });
...
describe("Schema Validation Error Handling", () => {
  ...
  it("throws SCHEMA_INVALID for invalid release-status.json", () => {
    ...
    try {
      execSync(`node ${SCRIPT_PATH_VALIDATE} --dir=${artifactDir} --strict`, {
        cwd: tempDir,
        encoding: "utf8",
        stdio: "pipe",
      });

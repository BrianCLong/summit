...
describe('bundle index determinism', () => {
  it('writes bundle-index.json with deterministic ordering', () => {
    ...
    try {
      ...
      writeFileSync(join(tempDir, 'alpha.txt'), 'a');
      writeFileSync(join(tempDir, 'mango.txt'), 'm');
      ...
      const basenames = index.files.map(file => basename(file.path));
      const sorted = [...basenames].sort();
      assert.deepEqual(
        basenames,
        sorted,
        `Expected sorted file order, got: ${basenames.join(', ')}`,
      );
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
    }

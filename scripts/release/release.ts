...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === '--bump') args.bump = argv[++i] as Args['bump'];
    else if (arg === '--tag-prefix') args.tagPrefix = argv[++i];
    else if (arg === '--notes-from') args.notesFrom = argv[++i];
    else if (arg === '--dry-run') args.dryRun = true;
    else if (arg === '--skip-tag') args.skipTag = true;
    else if (arg === '--skip-gh-release') args.skipGhRelease = true;
    else if (arg === '--release-line') args.releaseLine = argv[++i];
...
function ensureCleanWorkingTree(force: boolean): void {
...
function ensureRequiredFiles(force: boolean): void {
  if (!existsSync('docs/releases') && !force) {
    fail('docs/releases directory missing. Use --force to bypass.');
  }
}

function ensureGaVerifyStamp(force: boolean): string {
...
function bumpVersion(current: string, bump: NonNullable<Args['bump']>): string {
...
function updateVersions(newVersion: string, dryRun: boolean): string[] {
  const updated: string[] = [];
  for (const path of listPackageJsons()) {
    const pkg = readJson(path);
    if (pkg.version && pkg.version !== newVersion) {
      pkg.version = newVersion;
      if (!dryRun) writeJson(path, pkg);
      updated.push(path);
    }
  }
  return updated;
...
function getLatestSemverTag(prefix: string): string | null {
  ...
  const normalize = (tag: string) => `v${tag.slice(prefix.length)}`;
  const sorted = [...tags].sort((a, b) => compareTags(normalize(a), normalize(b)));
  return sorted[sorted.length - 1];
}

function getGitRange(tagPrefix: string, notesFrom?: string): string {
  if (notesFrom) return notesFrom;
...
function classifyCommit(commit: Commit): 'server' | 'ui' | 'infra' | 'security' | 'misc' {
  const files = commit.files;
  const has = (prefixes: string[]) => files.some(f => prefixes.some(p => f.startsWith(p)));
  const contains = (tokens: string[]) =>
    files.some(f => tokens.some(t => f.toLowerCase().includes(t)));

  if (contains(['security', 'policy', 'opa'])) return 'security';
  if (has(['infra/', 'terraform/', 'k8s/', 'helm/', 'ops/', '.github/'])) return 'infra';
  if (has(['server/'])) return 'server';
  if (has(['client/', 'apps/web/', 'apps/mobile/'])) return 'ui';
  return 'misc';
}

function extractBreaking(commits: Commit[]): Commit[] {
  return commits.filter(c => c.subject.includes('!:') || c.body.includes('BREAKING CHANGE'));
}

function buildHighlights(commits: Commit[]): Commit[] {
  ...
  for (const key of order) {
    if (buckets[key].length) picks.push(buckets[key][0]);
  }
  if (picks.length < 3) {
    for (const commit of commits) {
      if (!picks.includes(commit)) picks.push(commit);
...
function formatCommitBullet(commit: Commit): string {
  return `- ${commit.subject} (${commit.hash.slice(0, 7)})`;
}

function buildReleaseNotes(releaseLine: string, version: string, range: string, commits: Commit[]): string {
  ...
  if (highlights.length) highlights.forEach(c => lines.push(formatCommitBullet(c)));
  else lines.push('- Not evidenced');
  lines.push('');
...
function buildEvidencePack(releaseLine: string, version: string, range: string, stampPath: string): string {
...
function updateChangelog(version: string, releaseLine: string, notesPath: string, dryRun: boolean): void {
  ...
  if (!dryRun) writeFileSync('CHANGELOG.md', updated);
}

function ensureArtifactsDir(tag: string): string {
  const dir = join('artifacts', 'release', tag);
  mkdirSync(dir, { recursive: true });
  return dir;
}

function createReleaseCommit(tag: string, dryRun: boolean): void {
  if (dryRun) return;
...
function createTag(tag: string, dryRun: boolean, skipTag: boolean): void {
  if (skipTag || dryRun) return;
  run(`git tag -a ${tag} -m "Release ${tag}"`);
}

function ghRelease(tag: string, notesPath: string, dryRun: boolean, skip: boolean): void {
  if (skip) return;
...
function main(): void {
  ...
  const currentVersion = readJson('package.json').version as string;
  ...
  if (args.dryRun) console.log('Dry run: no repository files were modified.');
}

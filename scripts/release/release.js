...
const semver_tags_mjs_1 = require("./semver-tags.mjs");
...
function ensureRequiredFiles(force) {
    if (!(0, node_fs_1.existsSync)('docs/releases') && !force) {
        fail('docs/releases directory missing. Use --force to bypass.');
    }
}
function ensureGaVerifyStamp(force) {
    const sha = run('git rev-parse HEAD');
    const stampPath = (0, node_path_1.join)('artifacts', 'ga-verify', sha, 'stamp.json');
    if (!(0, node_fs_1.existsSync)(stampPath) && !force) {
...
function listPackageJsons() {
    ...
    if ((0, node_fs_1.existsSync)('packages')) {
        const dirs = (0, node_fs_1.readdirSync)('packages', { withFileTypes: true });
        for (const dir of dirs) {
            if (dir.isDirectory()) {
                paths.push((0, node_path_1.join)('packages', dir.name, 'package.json'));
            }
        }
    }
    return paths.filter(p => (0, node_fs_1.existsSync)(p));
}
...
function getLatestSemverTag(prefix) {
    ...
    const sorted = [...tags].sort((a, b) => (0, semver_tags_mjs_1.compareTags)(normalize(a), normalize(b)));
...
function buildEvidencePack(releaseLine, version, range, stampPath) {
    const stamp = (0, node_fs_1.existsSync)(stampPath) ? readJson(stampPath) : null;
    const lines = [];
...
function updateChangelog(version, releaseLine, notesPath, dryRun) {
    if (!(0, node_fs_1.existsSync)('CHANGELOG.md'))
        return;
    const content = (0, node_fs_1.readFileSync)('CHANGELOG.md', 'utf8');
    ...
    if (!dryRun)
        (0, node_fs_1.writeFileSync)('CHANGELOG.md', updated);
}
function ensureArtifactsDir(tag) {
    const dir = (0, node_path_1.join)('artifacts', 'release', tag);
...
function main() {
    ...
    const notesPath = (0, node_path_1.join)('docs', 'releases', `${releaseLine}_RELEASE_NOTES.md`);
    const evidencePath = (0, node_path_1.join)('docs', 'releases', `${releaseLine}_EVIDENCE_PACK.md`);
    ...
    if (!args.dryRun) {
        ...
        (0, node_fs_1.writeFileSync)(notesPath, notes);
        (0, node_fs_1.writeFileSync)(evidencePath, evidence);
        updateChangelog(version, releaseLine, notesPath, args.dryRun);
    }
    const artifactsDir = ensureArtifactsDir(tag);
    (0, node_fs_1.writeFileSync)((0, node_path_1.join)(artifactsDir, 'github_release.md'), notes);
    (0, node_fs_1.writeFileSync)((0, node_path_1.join)(artifactsDir, 'release_notes.md'), notes);
    (0, node_fs_1.writeFileSync)((0, node_path_1.join)(artifactsDir, 'evidence_pack.md'), evidence);
    createReleaseCommit(tag, args.dryRun);
    createTag(tag, args.dryRun, args.skipTag);
    ghRelease(tag, (0, node_path_1.join)(artifactsDir, 'github_release.md'), args.dryRun, args.skipGhRelease);

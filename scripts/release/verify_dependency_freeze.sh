...
extract_versions() {
  ...
  echo "$lockfile_content" | grep -E "^  ['\"]?[a-zA-Z@]" | head -100 || true >&2
}
...
compare_lockfiles() {
  ...
  echo "$baseline_content" > "$baseline_file" >&2
  echo "$current_content" > "$current_file" >&2
...
generate_recommendations() {
  ...
  if [[ "$status" == "PASS" ]]; then
    echo "Dependencies are frozen and stable. No action required." >&2
  elif [[ "$status" == "WARN" ]]; then
    echo "- Review the patch version changes" >&2
    echo "- Ensure changes are intentional security updates" >&2
    echo "- Consider regenerating RC if changes are substantial" >&2
  else
    echo "- **FREEZE VIOLATION:** Dependencies have changed since RC baseline" >&2
    echo "- Investigate the source of dependency changes" >&2
    echo "- Either revert changes or cut a new RC" >&2
    echo "- Do not proceed with GA promotion until resolved" >&2
  fi
}
...
main() {
  ...
  echo "{"timestamp": "${CHECK_TIMESTAMP}", "baseline": "${BASELINE}", "lockfile": "${LOCKFILE}", "status": "${status}", "has_changes": ${has_changes}, "strict_mode": ${STRICT_MODE}, "allow_patch": ${ALLOW_PATCH}}"

...
if (!params.repo) {
  console.error(
    "Error: --repo <owner/name> is required or must be set via GITHUB_REPOSITORY or detected via git remote"
  );
  process.exit(1);
}
...
async function fetchGitHub(path) {
  if (token) {
    ...
    try {
      ...
      throw new Error(
        `Failed to fetch from GitHub via gh CLI. Ensure gh is installed and authenticated, or set GITHUB_TOKEN. Error: ${e.message}`
      );
    }
  }
}

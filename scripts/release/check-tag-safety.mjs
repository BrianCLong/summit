...
function main() {
  ...
  if (isAnnotated) {
    ...
    try {
      ...
      } else if (stderr.includes('No public key') || stderr.includes('gpg: Can\'t check signature')) {
  ...
  console.log(`   Signature: ${tagSafety.signature.present ? 'PRESENT' : 'NO'} (${tagSafety.signature.note})`);

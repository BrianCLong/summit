...
if (!csvPath) {
  console.error("Usage: node scripts/batch-correlate-osint.js <csvPath> [endpoint]");
  process.exit(1);
}

const endpoint = endpointArg || process.env.GRAPHQL_ENDPOINT || "http://localhost:4000/graphql";
...
async function validateSchema(url) {
  const introspectionQuery = "query { __schema { mutationType { fields { name } } } }";
  ...
  const fields = json.data?.__schema?.mutationType?.fields?.map((f) => f.name) || [];

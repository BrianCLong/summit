...
const schemas = {
  changeplan: {
    ...
    patterns: ["**/*changeplan.yaml", "**/*ChangePlan.yaml", "docs/examples/changeplan.yaml"],
  },
  experiment: {
  ...
  migration: {
    ...
    patterns: ["**/*migration.yaml", "**/*migrations.yaml", "docs/examples/migration.yaml"],
  },
  perf: {
  ...
  cbl: {
    ...
    patterns: ["**/*.cbl.yaml", "**/*.cbl.yml", "docs/examples/cbl.example.yaml"],
  },
  airgap: {
  ...
  for (const it of items) {
    if (it.status === "pass") summary.push(`- ✅ ${it.file ?? it.reason ?? ""}`);
      ...
      it.errors
        .slice(0, 10)
        .forEach((e) =>
          summary.push(`  - ${e.instancePath || "/"} — ${e.message} (${e.schemaPath})`)
        );

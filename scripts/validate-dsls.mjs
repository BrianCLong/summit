...
for (const [key, cfg] of Object.entries(schemas)) {
  ...
  for (const file of files) {
    ...
    let data;
    try { data = (ext === '.yaml' || ext === '.yml') ? yaml.load(raw) : JSON.parse(raw); }
    catch (e) { results.push({ key, file, status:'error', errors:[{ message:`Parse error: ${e.message}` }] }); continue; }
    ...
    } else { results.push({ key, file, status:'pass' }); }
  }
}

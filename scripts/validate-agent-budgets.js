...
function validateManifest(manifest, agentName, schema) {
  ...
  if (typeof risk.residual === 'number' && typeof risk.riskCeiling === 'number' && risk.residual > risk.riskCeiling) {
  ...
  if (
    telemetry.emitBudgetEvents !== undefined &&
    typeof telemetry.emitBudgetEvents !== 'boolean'
  ) {
    errors.push('telemetry.emitBudgetEvents must be boolean when provided');
  }
  if (
    telemetry.sampleRate !== undefined &&
    (typeof telemetry.sampleRate !== 'number' || telemetry.sampleRate < 0 || telemetry.sampleRate > 1)
  ) {
    errors.push('telemetry.sampleRate must be a number between 0 and 1 when provided');
  }

  if ('billing' in manifest) {
    errors.push('billing is not allowed; budgets must be deterministic and not reference external billing');
  }

...
class BuildMetrics {
    ...
    try {
      ...
      switch (serviceName) {
        ...
        case 'cognitive-insights':
          buildCmd = 'cd cognitive-insights && docker build . -t ci-temp 2>/dev/null || echo "build-placeholder"';
          break;
        case 'active-measures-module':
          buildCmd = 'cd active-measures-module && docker build . -t am-temp 2>/dev/null || echo "build-placeholder"';
          break;
        case 'reliability-service':
          buildCmd = 'cd reliability-service && docker build . -t rs-temp 2>/dev/null || echo "build-placeholder"';
          break;
        default:
  ...
  async benchmarkRun() {
    ...
    for (const service of TOP_SERVICES) {
      ...
      if (baseline) {
        const improvement = ((baseline.duration - timing.duration) / baseline.duration * 100).toFixed(1);
  ...
  generateReport() {
    ...
    for (const service of TOP_SERVICES) {
      ...
      if (baseline && current && baseline.status === 'success' && current.status === 'success') {
        ...
        console.log(`${service.padEnd(20)} ${baseline.duration.toString().padStart(8)}ms ${current.duration.toString().padStart(8)}ms ${status} ${improvement.toFixed(1)}%`);
      }
    }

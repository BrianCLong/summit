...
class TenantBudgetReporter:
    ...
    def query_prometheus(self, query: str, time_range: str = "1d") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if data['status'] != 'success':
                print(f"⚠️ Prometheus query failed: {data.get('error', 'unknown error')}", file=sys.stderr)
                return []
    ...
    def get_tenant_budget_status(self, tenant: str) -> Dict[str, Any]:
        ...
        # Calculated metrics
        utilization = self.query_prometheus(f'mc:tenant_budget_utilization:rate5m{{tenant="{tenant}"}}')
        burn_rate = self.query_prometheus(f'mc:tenant_budget_burn_rate:1h{{tenant="{tenant}"}}')
        remaining = self.query_prometheus(f'mc:tenant_budget_remaining:current{{tenant="{tenant}"}}')
    ...
    def get_budget_alerts(self, tenant: str, utilization: float, burn_rate: float) -> List[str]:
        ...
        if utilization >= 90:
            alerts.append(f"CRITICAL: Budget utilization at {utilization:.1f}% - immediate action required")
        elif utilization >= 80:
            alerts.append(f"WARNING: Budget utilization at {utilization:.1f}% - monitor closely")

        if burn_rate > 1.2:
            alerts.append(f"WARNING: Burn rate {burn_rate:.1f}x above projected - review usage patterns")

        if burn_rate > 1.5:
            alerts.append(f"CRITICAL: Burn rate {burn_rate:.1f}x above projected - budget overrun likely")
    ...
    def generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        avg_utilization = sum(t['utilization_percent'] for t in tenant_reports) / len(tenant_reports) if tenant_reports else 0
        ...
        return {
            ...
            "aggregate_summary": {
                ...
                "overall_utilization_percent": (total_cost / total_budget * 100) if total_budget > 0 else 0,
                ...
                "healthy_tenants_count": len(tenant_reports) - len(critical_tenants) - len(warning_tenants)
            },
    ...
    def generate_action_items(self, tenant_reports: List[Dict[str, Any]]) -> List[str]:
        ...
        for tenant in critical_tenants:
            actions.append(f"URGENT: Review {tenant['tenant']} usage - {tenant['utilization_percent']:.1f}% budget utilization")

        high_burn_rate = [t for t in tenant_reports if t['burn_rate_multiplier'] > 1.3]
        for tenant in high_burn_rate:
            actions.append(f"REVIEW: {tenant['tenant']} burn rate {tenant['burn_rate_multiplier']:.1f}x - optimize resources")

        low_remaining = [t for t in tenant_reports if t['days_remaining'] < 7]
        for tenant in low_remaining:
            actions.append(f"PLAN: {tenant['tenant']} budget exhaustion in {tenant['days_remaining']:.1f} days")
    ...
    def generate_recommendations(self, tenant_reports: List[Dict[str, Any]]) -> List[str]:
        ...
        if avg_utilization > 70:
            recommendations.append("Consider implementing cost optimization strategies across high-utilization tenants")

        high_variance = max(t['utilization_percent'] for t in tenant_reports) - min(t['utilization_percent'] for t in tenant_reports) if tenant_reports else 0
        if high_variance > 50:
            recommendations.append("Large budget utilization variance detected - review resource allocation policies")

        over_budget = len([t for t in tenant_reports if t['utilization_percent'] > 100])
        if over_budget > 0:
            recommendations.append(f"{over_budget} tenants over budget - implement cost controls and alerts")
...
def main():
    ...
    parser.add_argument("--tenants", default="TENANT_001,TENANT_002,TENANT_003,TENANT_004,TENANT_005", help="Comma-separated tenant list")
    parser.add_argument("--output", default="evidence/v0.3.3/budgets/tenant-budget-report.json", help="Output file path")
    ...
    try:
        ...
        if args.format == "json":
            ...
            print(f"Overall utilization: {report['aggregate_summary']['overall_utilization_percent']:.1f}%")

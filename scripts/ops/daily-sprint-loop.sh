...
if ! date -u -d "$RUN_DATE" +%F >/dev/null 2>&1; then
  echo "invalid calendar date '$RUN_DATE'" >&2
  exit 2
fi
...
if command -v gh >/dev/null 2>&1; then
  if ! gh auth status >/dev/null 2>&1; then
    echo "gh CLI is installed but not authenticated" >"$PR_ERR"
    echo "gh CLI is installed but not authenticated" >"$ISSUE_ERR"
  else
...
report = {
    ...
    "date": run_date,
    "tooling": {
        "gh_available": not pr_err.startswith("gh CLI unavailable"),
        "gh_authenticated": not pr_err.startswith("gh CLI is installed but not authenticated"),
    },
    "evidence": {
...
metrics = {
    ...
    "gh_available": report["tooling"]["gh_available"],
    "gh_authenticated": report["tooling"]["gh_authenticated"],
}

...
try {
    // Try to use gh CLI
    const prsJson = (0, child_process_1.execSync)('gh pr list --state open --limit 100 --json number,title,labels,mergeable', { encoding: 'utf-8', stdio: ['ignore', 'pipe', 'ignore'] });
    const prs = JSON.parse(prsJson);
    const conflicts = prs.filter((p) => p.mergeable === 'CONFLICTING');
    const mergeable = prs.filter((p) => p.mergeable === 'MERGEABLE');
    ...
    const highPriority = prs.filter((p) => p.labels.some((l) => l.name.toLowerCase().includes('priority') || l.name.toLowerCase().includes('ga')));

...
async function fetchOpenPrs() {
    ...
    while (true) {
        try {
            const res = await api.get(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls`, {
                params: { state: 'open', per_page: 100, page },
            });
            if (res.data.length === 0)
                break;
            prs = prs.concat(res.data);
            if (prs.length >= MAX_PRS_TO_PROCESS)
                break;
            page++;
        }
        catch (error) {
...
async function fetchPrFiles(prNumber) {
    try {
        const res = await api.get(`/repos/${REPO_OWNER}/${REPO_NAME}/pulls/${prNumber}/files`, {
            params: { per_page: 100 },
        });
...
async function fetchCompare(base, head) {
    try {
        const res = await api.get(`/repos/${REPO_OWNER}/${REPO_NAME}/compare/${base}...${head}`);
        return res.data;
    }
    catch (error) {
...
function getDirectory(filePath, depth = 2) {
    const parts = filePath.split('/');
    return parts.slice(0, Math.min(parts.length - 1, depth)).join('/') + '/';
}
function calculateScoreAndAdvice(pr, allPrs, hotspotMap) {
    let score = 0;
    const factors = [];
    // 1. Hotspot Overlap
    let hotspotHits = 0;
...
async function main() {
    ...
    for (const pr of analyzedPrs) {
        ...
        const overlaps = analyzedPrs.filter(other => other.number !== pr.number &&
    ...
    // JSON
    fs_1.default.writeFileSync(path_1.default.join(OUTPUT_DIR, 'PR_CONFLICT_FORECAST.json'), JSON.stringify(output, null, 2));
    ...
    fs_1.default.writeFileSync(path_1.default.join(OUTPUT_DIR, 'PR_CONFLICT_FORECAST.md'), md);

...
const DOCS_OPS_DIR = node_path_1.default.resolve(ROOT_DIR, 'docs/ops');
const SNAPSHOTS_DIR = node_path_1.default.resolve(__dirname, 'snapshots');
const OWNERS = {
...
function getGovernanceData(ctx) {
    const govData = {
    ...
    if (ctx.mode === 'live') {
        // 1. Compliance Drift
        try {
            (0, node_child_process_1.execSync)('npx tsx scripts/compliance/check_drift.ts', { stdio: 'pipe' });
        ...
        try {
            ...
            node_fs_1.default.rmSync(reportPath, { recursive: true, force: true });
            (0, node_child_process_1.execSync)(`node scripts/ci/verify_governance_docs.mjs --out ${reportPath}`, { stdio: 'pipe' });
            govData.docs.status = 'PASS';
            const report = readJsonFile(node_path_1.default.join(reportPath, 'report.json'));
        ...
        try {
            ...
            (0, node_child_process_1.execSync)(`node scripts/ci/verify_evidence_id_consistency.mjs --evidence-map-path=docs/evidence/evidence-map.yml --output=${reportPath} --sha=latest`, { stdio: 'pipe' });
            govData.evidence.status = 'PASS';
            const report = readJsonFile(node_path_1.default.join(reportPath, 'latest', 'report.json'));
...
function runGhCommand(args) {
    try {
        const result = (0, node_child_process_1.spawnSync)('gh', args, { encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
        if (result.error)
            throw result.error;
        if (result.status !== 0)
...
function getData(ctx) {
    ...
    if (ctx.mode === 'offline') {
        ...
        return {
            prs: readJsonFile(node_path_1.default.join(ctx.snapshotsDir, 'pr_list.json')),
            ...
            prChecks: readJsonFile(node_path_1.default.join(ctx.snapshotsDir, 'pr_checks.json')),
            issues: readJsonFile(node_path_1.default.join(ctx.snapshotsDir, 'issues.json')),
            codeScanning: readJsonFile(node_path_1.default.join(ctx.snapshotsDir, 'code_scanning.json')),
            dependabot: readJsonFile(node_path_1.default.join(ctx.snapshotsDir, 'dependabot.json')),
            govData,
        };
    }
    else {
...
function clusterFailures(pr) {
    ...
    for (const f of failures) {
        ...
        if (CLUSTERS.ERR_MODULE_NOT_FOUND.test(desc))
            return 'ERR_MODULE_NOT_FOUND';
        if (CLUSTERS.ESM_CJS.test(desc))
            return 'ESM/CJS_ISSUES';
        if (CLUSTERS.DUPLICATE_METRIC.test(desc))
            return 'DUPLICATE_METRIC';
        if (CLUSTERS.NETWORK.test(desc))
            return 'NETWORK_LEAK';
        if (CLUSTERS.TIMEOUT.test(desc))
            return 'TIMEOUT_HANG';
        if (CLUSTERS.LINT_BUILD.test(desc))
            return 'LINT_BUILD_FAIL';
    }
    return 'OTHER_FAILURE';
}
function determineMergePriority(pr, cluster, owner) {
    // Lower is better
    if (cluster === 'GREEN')
        return 10;
    if (pr.labels.some(l => l.name === 'P0' || l.name === 'security'))
        return 20;
    if (pr.title.startsWith('fix:'))
        return 30;
    if (owner === OWNERS.JULES)
        return 40; // Low risk
    if (owner === OWNERS.CLAUDE)
        return 50; // High leverage but risky
    if (owner === OWNERS.QWEN)
        return 60;
    return 100;
}
// --- Output Generation ---
function generateMarkdown(data) {
    ...
    // Sort by priority
    processedPRs.sort((a, b) => a.priority - b.priority);
    // Stats
    const openIssueCount = issues.length;
    const p0Issues = issues.filter((i) => i.labels.some(l => l.name === 'P0'));
    ...
    md += `| :--- | :--- | :--- | :--- | :--- |\n`;
    processedPRs.slice(0, 10).forEach((pr, idx) => {
        md += `| ${idx + 1} | #${pr.number} | ${pr.owner.split(' ')[0]} | ${pr.cluster} | ${pr.title} |\n`;
    });
    md += `\n`;
    // Fail Clusters & Routing
    md += `## 3. Failure Clusters & Routing\n`;
    const byCluster = processedPRs.reduce((acc, pr) => {
        if (pr.cluster === 'GREEN' || pr.cluster === 'PENDING')
            return acc;
    ...
    md += `2. **Merge Green**: Clear the ${processedPRs.filter((p) => p.cluster === 'GREEN').length} green PRs.\n`;
    ...
    if (data.govData) {
        ...
        if (drift.status === 'FAIL')
            md += `  - Drift Output: ${drift.output.split('\n')[0]}...\n`;
...
async function main() {
    ...
    const ctx = {
        mode,
        snapshotsDir,
        outputFile: node_path_1.default.join(DOCS_OPS_DIR, 'COMMAND_CENTER.md'),
        failOnP0
    };
    console.log(`Starting Command Center Generator (${mode})...`);
    const data = getData(ctx);
    const markdown = generateMarkdown(data);
    node_fs_1.default.mkdirSync(node_path_1.default.dirname(ctx.outputFile), { recursive: true });
    node_fs_1.default.writeFileSync(ctx.outputFile, markdown);
    console.log(`Report generated at: ${ctx.outputFile}`);
    if (ctx.failOnP0) {
        const p0Count = data.issues.filter((i) => i.labels.some(l => l.name === 'P0')).length;
        if (p0Count > 0) {

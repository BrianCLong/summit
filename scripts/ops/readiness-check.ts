...
async function main() {
  ...
  for (const pkgPath of packageJsonPaths) {
    ...
    if (await checkMixedTestRunners(pkgPath)) {
      blockers.push({
        id: "P1-DEBT-001",
        message: "Mixed test runners (Jest and Vitest)",
        file: pkgPath,
      });
    }
  }
  ...
  try {
    ...
    blockers.push({
      id: "P0-BUILD-002",
      message: "apps/mobile-interface build failed",
      file: "apps/mobile-interface",
    });
  }
  ...
  try {
    ...
    blockers.push({
      id: "P1-TEST-001",
      message: "apps/a11y-lab tests failed",
      file: "apps/a11y-lab",
    });
  }
  ...
  try {
    ...
    if (zodVersions.size > 1) {
      blockers.push({
        id: "P1-DEBT-002",
        message: `Zod version mismatch found: ${[...zodVersions].join(", ")}`,
        file: "pnpm-lock.yaml",
      });
    }
  } catch (err) {
    blockers.push({
      id: "P1-DEBT-002",
      message: "Failed to check for Zod version mismatch",
      file: "pnpm-lock.yaml",
    });
  }

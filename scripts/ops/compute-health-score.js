...
const OUTPUT_FILE_MD = path_1.default.join(DOCS_DIR, 'dashboard-health.md');
// Component Weights
const WEIGHTS = {
    reliability: 0.35,
    availability: 0.20,
...
function getReliabilityMetrics() {
    ...
    for (const file of files) {
        const p = path_1.default.join(METRICS_DIR, file);
...
function getAvailabilityMetrics() {
    ...
    const p = path_1.default.join(METRICS_DIR, 'slo-results.json');
    if ((0, fs_1.existsSync)(p)) {
        try {
            const content = JSON.parse((0, fs_1.readFileSync)(p, 'utf-8'));
...
function getQualityMetrics() {
    ...
    const p = path_1.default.join(METRICS_DIR, 'issues.json');
    if ((0, fs_1.existsSync)(p)) {
        try {
            const content = JSON.parse((0, fs_1.readFileSync)(p, 'utf-8'));
...
function getSecurityMetrics() {
    ...
    let files = [];
    try {
        files = (0, fs_1.readdirSync)(METRICS_DIR).filter((fn) => fn.startsWith('vulns-') && fn.endsWith('.txt'));
    }
    catch (e) {
    ...
    for (const file of files) {
        try {
            const content = (0, fs_1.readFileSync)(path_1.default.join(METRICS_DIR, file), 'utf-8');
            const critMatch = content.match(/Critical:\s*(\d+)/i);
            const highMatch = content.match(/High:\s*(\d+)/i);
            if (critMatch)
                critical += parseInt(critMatch[1], 10);
            if (highMatch)
                high += parseInt(highMatch[1], 10);
        }
        catch (e) {
...
function getEfficiencyMetrics() {
    // budget.json
    const p = path_1.default.join(METRICS_DIR, 'budget.json');
    if ((0, fs_1.existsSync)(p)) {
        try {
            const content = JSON.parse((0, fs_1.readFileSync)(p, 'utf-8'));
...
function getVelocityMetrics() {
    // velocity.json
    const p = path_1.default.join(METRICS_DIR, 'velocity.json');
    if ((0, fs_1.existsSync)(p)) {
        try {
            const content = JSON.parse((0, fs_1.readFileSync)(p, 'utf-8'));
...
function calculateScore(metrics) {
    ...
    // 2. Availability
    let sAvailability = 0;
    if (metrics.availability.errorRate < 0.1)
        sAvailability = 100;
    else if (metrics.availability.errorRate < 1)
        sAvailability = 80;
    else if (metrics.availability.errorRate < 5)
        sAvailability = 50;
    else
        sAvailability = 0;
    // 3. Quality
    ...
    // 5. Efficiency
    let sEfficiency = 50;
    if (metrics.efficiency.cost <= metrics.efficiency.budget)
        sEfficiency = 100;
    else if (metrics.efficiency.cost <= 1.1 * metrics.efficiency.budget)
        sEfficiency = 80;
    // 6. Velocity
    let sVelocity = 50;
    if (metrics.velocity.leadTimeHours < 24)
        sVelocity = 100;
    else if (metrics.velocity.leadTimeHours < 48)
        sVelocity = 80;
    // Total
    const totalScore = (WEIGHTS.reliability * sReliability) +
...
function main() {
    ...
    (0, fs_1.writeFileSync)(OUTPUT_FILE_JSON, JSON.stringify(outputJson, null, 2));
...
`;
    if (!(0, fs_1.existsSync)(DOCS_DIR)) {
        (0, fs_1.mkdirSync)(DOCS_DIR, { recursive: true });
    }
    (0, fs_1.writeFileSync)(OUTPUT_FILE_MD, mdContent);

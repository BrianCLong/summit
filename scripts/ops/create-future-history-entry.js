...
async function fetchPRInfo(prNumber) {
    try {
        ...
        const result = (0, child_process_1.execSync)(`gh pr view ${prNumber} --json number,title,body,author,additions,deletions,changedFiles,mergedAt,labels,url 2>/dev/null`, { encoding: 'utf-8', timeout: 10000 });
...
function inferCategory(prInfo) {
    ...
    if (combined.includes('perf') ||
        combined.includes('optim') ||
        combined.includes('latency')) {
        return 'Performance';
    }
    if (combined.includes('governance') ||
...
function generateDiffSummary(prInfo) {
...
function generateEntry(options, prInfo) {
...
function generateIndexEntry(options, prInfo) {
    const date = new Date().toISOString().split('T')[0];
    const title = options.summary || prInfo.map((pr) => pr.title).join('; ');
    const category = options.category || inferCategory(prInfo);
    const anchor = title
...
function appendToLog(entry, indexEntry, logPath) {
...
function writeStandaloneEntry(entry, options) {
    const date = new Date().toISOString().split('T')[0];
...
async function main() {
    ...
    const prInfo = prInfoResults.filter((pr) => pr !== null);
    // Generate entry

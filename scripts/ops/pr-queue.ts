...
try {
  ...
  const prsJson = execSync(
    "gh pr list --state open --limit 100 --json number,title,labels,mergeable",
  ...
  const highPriority = prs.filter((p: any) =>
    p.labels.some(
      (l: any) => l.name.toLowerCase().includes("priority") || l.name.toLowerCase().includes("ga")
    )
  );

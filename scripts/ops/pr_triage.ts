...
function getMockPRs() {
  return [
    {
      number: 101,
      title: "feat: add new ingestion pipeline",
      user: { login: "dev-1" },
      labels: [{ name: "feature" }],
      ...
      deletions: 20,
      base: { ref: "main" },
      head: { ref: "feat/ingestion", sha: "mocksha1" },
    },
    {
      number: 102,
      title: "fix: security policy bypass",
      user: { login: "sec-eng" },
      ...
      deletions: 10,
      base: { ref: "main" },
      head: { ref: "fix/security", sha: "mocksha2" },
    },
    {
      number: 103,
      ...
      deletions: 1000,
      base: { ref: "main" },
      head: { ref: "chore/deps", sha: "mocksha3" },
    },
  ];
}
...
async function analyze() {
  ...
  for (const pr of processedPrs) {
    ...
    const touchesCritical = pr.overlap.files.some((f) =>
      CRITICAL_PATHS.some((cp) => f.startsWith(cp))
    );
    if (touchesCritical) {
      score += 15;

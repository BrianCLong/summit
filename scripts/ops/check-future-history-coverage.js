...
function getPRStats() {
    ...
    // Try gh CLI
    try {
        const result = (0, child_process_1.execSync)('gh pr view --json additions,deletions,changedFiles,labels,title,body,number 2>/dev/null', { encoding: 'utf-8', timeout: 10000 });
    ...
    try {
        ...
        // Get diff stats
        const diffStat = (0, child_process_1.execSync)(`git diff --stat ${baseBranch}...HEAD 2>/dev/null || git diff --stat HEAD~1`, {
            encoding: 'utf-8',
            timeout: 10000,
        });
...
function hasSkipMarker(stats) {
    return (stats.body.includes(CONFIG.skipMarker) ||
        stats.title.includes(CONFIG.skipMarker));
}
// Check if PR has epic-level labels
function hasEpicLabels(stats) {
    return stats.labels.some((label) => CONFIG.epicLabels.some((epic) => label.toLowerCase().includes(epic)));
}
// Check if PR exceeds thresholds
function exceedsThresholds(stats) {
    const totalLines = stats.additions + stats.deletions;
    return (totalLines >= CONFIG.linesThreshold ||
        stats.changedFiles >= CONFIG.filesThreshold);
}
// Check if Future History entry exists for this PR
function hasFutureHistoryEntry(stats) {
...
function changesFutureHistory() {
    try {
        const baseBranch = process.env.GITHUB_BASE_REF || 'main';
        const changedFiles = (0, child_process_1.execSync)(`git diff --name-only ${baseBranch}...HEAD 2>/dev/null || git diff --name-only HEAD~1`, { encoding: 'utf-8', timeout: 10000 });
...
function runCheck() {
    ...
    const totalLines = stats.additions + stats.deletions;
    console.log(`\n📊 PR Stats:`);
    console.log(`   Lines changed: ${totalLines} (+${stats.additions}/-${stats.deletions})`);
    console.log(`   Files changed: ${stats.changedFiles}`);
    console.log(`   Labels: ${stats.labels.length > 0 ? stats.labels.join(', ') : 'none'}`);
    console.log(`   Thresholds: ${CONFIG.linesThreshold} lines, ${CONFIG.filesThreshold} files\n`);
...
function annotateGitHub(result) {
    if (process.env.GITHUB_ACTIONS !== 'true') {
        return;
    }
    if (!result.pass) {
        const level = CONFIG.strict ? 'error' : 'warning';
        console.log(`::${level}::Future History entry recommended for this PR. ${result.reason}`);
    }
}
// Main execution
function main() {
    console.log('🔍 Future History Coverage Check\n');
    console.log(`   Mode: ${CONFIG.strict ? 'Strict (blocking)' : 'Advisory (non-blocking)'}`);

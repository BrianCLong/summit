...
async function fetchPRData(prNumber: number): Promise<{ meta: PRMetadata; files: FileChange[] }> {
  ...
  if (!token) {
    console.warn(
      "Warning: GITHUB_TOKEN not found. If this script is running outside of a GitHub Action, it may fail or be rate limited."
    );
  ...
  try {
    ...
    while (true) {
      const filesRes = await fetch(
        `${baseUrl}/repos/${repo}/pulls/${prNumber}/files?per_page=100&page=${page}`,
        { headers }
      );
...
function mockPRData(prNumber: number): { meta: PRMetadata; files: FileChange[] } {
  return {
    ...
    files: [
      ...
      {
        filename: "client/src/components/Header.tsx",
        status: "modified",
        additions: 20,
        deletions: 5,
      },
...
function checkMegaPR(
  meta: PRMetadata,
  criticalCount: number,
  config: Config
): { isMega: boolean; reasons: string[] } {
  const reasons: string[] = [];
  if (meta.changed_files >= config.thresholds.filesChanged) {
    reasons.push(
      `Files changed (${meta.changed_files}) >= threshold (${config.thresholds.filesChanged})`
    );
  }
  if (meta.additions + meta.deletions >= config.thresholds.linesChanged) {
    reasons.push(
      `Lines changed (${meta.additions + meta.deletions}) >= threshold (${config.thresholds.linesChanged})`
    );
  }
  if (criticalCount > 0 && meta.changed_files >= config.thresholds.criticalSize) {
    reasons.push(
      `Touching critical paths with size (${meta.changed_files}) >= critical threshold (${config.thresholds.criticalSize})`
    );
  }
...
function generatePlan(
  prData: { meta: PRMetadata; files: FileChange[] },
  config: Config
): SplitPlan {
  ...
  return {
    ...
    slices,
    rollbackStrategy:
      "Revert slices in reverse order (Sn -> S1). If S1 (Critical) is reverted, verify environment stability immediately.",
  };
}

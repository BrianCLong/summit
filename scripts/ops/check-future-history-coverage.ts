    ...
    try {
      ...
      if (event.pull_request) {
        return {
          ...
          labels: (event.pull_request.labels || []).map((l: { name: string }) => l.name),
...
function hasSkipMarker(stats: PRStats): boolean {
  return stats.body.includes(CONFIG.skipMarker) || stats.title.includes(CONFIG.skipMarker);
}
...
function exceedsThresholds(stats: PRStats): boolean {
  ...
  return totalLines >= CONFIG.linesThreshold || stats.changedFiles >= CONFIG.filesThreshold;
}

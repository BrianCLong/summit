...
async function refreshStatistics() {
  ...
  const tables =
    process.env.PLAN_REGRESSION_ANALYZE_TABLES?.split(',').map((table) => table.trim()).filter(Boolean) ??
    [
  ...
  const analyzeVerbosity =
    process.env.PLAN_REGRESSION_ANALYZE_VERBOSE === 'true';

  const pool = new Pool({ connectionString });
  try {
    await pool.query('SET search_path TO plan_regression, public;');
    for (const table of tables) {
      const statement = analyzeVerbosity

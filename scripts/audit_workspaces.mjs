...
function internalNameSet(ws) {
  return new Set(ws.map((w) => JSON.parse(w.pkgJsonRaw).name).filter(Boolean));
}
...
(async () => {
  ...
  for (const w of ws) {
    if (!w.hasBuild) {
      warn(`[${w.name}] missing "build" script`);
    }
    if (w.hasTs && !w.hasTsconfig) {
      err(`[${w.name}] TS sources but no local tsconfig.json`);
    }

...
function ghJson(args) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      catch (e) { reject(new Error(`Failed to parse gh JSON: ${e.message}\nRAW:\n${out}`)); }
    });
  });
}
...
class PROrchestrator {
  ...
  async run() {
    ...
    try {
      ...
      for (let i = 0; i < prsToProcess.length; i += config.CONCURRENCY) {
        ...
        console.log(`Processing batch ${Math.floor(i / config.CONCURRENCY) + 1}/${Math.ceil(prsToProcess.length / config.CONCURRENCY)}`);
  ...
  async calculatePriorityScore(pr) {
    ...
    const hasFailingChecks = checks.some(check => 
      config.REQUIRED_CHECKS.includes(check.name) && check.conclusion !== 'success'
    );
    if (hasFailingChecks) {
  ...
  async processPR(pr) {
    ...
    try {
      ...
      await this.postStatusComment(pr.number, `Sync with ${config.BASE_BRANCH}: ${syncResult ? '✅ done' : '⏳ pending'}`);
      ...
      await this.postStatusComment(pr.number, `Repro CI locally: ${testResults.success ? '✅ done' : '❌ failed'}`);
      ...
      await this.postStatusComment(pr.number, `Primary failure class: ${diagnosis.primaryIssue || 'none'}`);
      ...
      if (config.AUTO_FIX && diagnosis.lowRisk) {
        ...
        await this.postStatusComment(pr.number, `Minimal patch proposed: ${fixResult ? '✅ yes' : '❌ no'}`);
      }
      
      // 6. Review Comment - Post structured review
      console.log(`  6️⃣ Review - Posting structured review for PR #${pr.number}`);
      const review = await this.createReviewComment(pr, metadata, testResults, diagnosis, fixResult);
      ...
      await this.postStatusComment(pr.number, `Required checks: ${metadata.checksStatus || 'unknown'}`);
  ...
  async fetchPRMetadata(prNumber) {
    const fields = [
      'number','title','labels','baseRefName','headRefName','author','createdAt','updatedAt',
      'mergeable','reviewDecision','additions','deletions','commits','files','reviewRequests',
    ...
    return {
      ...
      files: (data.files || []).map(f => ({ path: f.path, additions: f.additions ?? 0, deletions: f.deletions ?? 0 })),
  ...
  async syncBranch(prNumber, branchName) {
    try {
      ...
      const result = spawnSync('git', ['merge', `origin/${config.BASE_BRANCH}`], { stdio: 'inherit' });
  ...
  async runTestSuite() {
    ...
    const lintResult = spawnSync(config.LINT_COMMAND, { shell: true, stdio: 'pipe' });
    
    console.log('    Running typecheck...');
    const typecheckResult = spawnSync(config.TYPECHECK_COMMAND, { shell: true, stdio: 'pipe' });
    
    console.log('    Running tests...');
    const testResult = spawnSync(config.TEST_COMMAND, { shell: true, stdio: 'pipe' });
    
    console.log('    Running security scan...');
    const securityResult = spawnSync(config.SECURITY_COMMAND, { shell: true, stdio: 'pipe' });
    
    return {
      success: lintResult.status === 0 && 
      ...
      details: {
        lint: { status: lintResult.status, stderr: lintResult.stderr?.toString() },
        typecheck: { status: typecheckResult.status, stderr: typecheckResult.stderr?.toString() },
        test: { status: testResult.status, stderr: testResult.stderr?.toString() },
        security: { status: securityResult.status, stderr: securityResult.stderr?.toString() }
      }
    };
  }
  ...
  async diagnoseFailures(testResults) {
    ...
    const lowRisk = failures.length === 1 && 
  ...
  async applySafeFixes(diagnosis) {
    ...
    try {
      if (diagnosis.failures.includes('lint')) {
        ...
        const result = spawnSync(config.LINT_COMMAND + ' --fix', { shell: true });
        if (result.status === 0) {
          // Commit the changes
          if (!config.DRY_RUN) {
            spawnSync('git', ['add', '.']);
      ...
      if (diagnosis.failures.includes('types')) {
        ...
        return { type: 'types', applied: false, message: 'Type fixes require manual intervention' };
      }
  ...
  async createReviewComment(pr, metadata, testResults, diagnosis, fixResult) {
    const status = diagnosis.failures.length === 0 ? 'Ready' : 
                  fixResult?.applied ? 'Ready after fixes' : 'Blocked';
...
`;
  ...
  async triggerCI(prNumber) {
    ...
    try {
      ...
      spawnSync('git', ['commit', '--allow-empty', '-m', `ci: trigger build for PR #${prNumber}`], { stdio: 'inherit' });
  ...
  async canMergePR(prNumber, metadata) {
    ...
    const isApproved = metadata.reviewDecision === 'APPROVED' || 
  ...
  async generateRunSummary() {
    ...
    this.runSummary.forEach(item => {
      console.log(`| #${item.pr} | ${item.title.substring(0, 30)}${item.title.length > 30 ? '...' : ''} | ${item.score} | ${item.primaryIssue} | ${item.actionTaken} | ${item.checks} | ${item.state} |`);
    });
    
    // Generate run digest
    const total = this.runSummary.length;
    const merged = this.runSummary.filter(item => item.state === 'Merged').length;
    const deferred = this.runSummary.filter(item => item.actionTaken.includes('Instructions left')).length;
    const needsAuthor = this.runSummary.filter(item => item.state === 'Needs author').length;
    
    console.log('\n📋 Run Digest:');
    console.log(`- Processed: ${total}/${total}; Merged: ${merged}; Deferred: ${deferred}; Needs author: ${needsAuthor}`);

...
const {
  HybridEntityResolutionService,
} = require('../server/src/services/HybridEntityResolutionService');
const fs = require('fs');
...
class ERPrecisionGate {
  ...
  async checkPrecision(
    ...
    try {
      ...
      const testDataPath = path.join(
        ...
        '..',
        'ml',
        'test_training_data.json',
      );
      const feedbackDataPath = path.join(
        __dirname,
        '..',
        'ml',
        'test_feedback.json',
      ...
      if (!fs.existsSync(testDataPath)) {
        ...
        return await this.useBaselineMetrics(
          entityType,
          threshold,
          prNumber,
          commitSha,
  ...
  async recordCIMetric(prNumber, commitSha, entityType, results, threshold) {
    ...
    try {
      ...
      console.log(
        `📝 Recorded CI metric for PR #${prNumber} (${commitSha.substring(0, 7)})`,
      );
    } catch (error) {
  ...
  async generateMetricsJSON(results, entityType, threshold) {
    ...
    const metricsPath = path.join(
      metricsDir,
      `er-metrics-${entityType.toLowerCase()}.json`,
    );
  ...
  generateReport(results, entityType, threshold) {
    ...
    if (meetsThreshold) {
      console.log(
        `✅ PASS: Precision ${(results.precision * 100).toFixed(2)}% >= ${(threshold * 100).toFixed(1)}%`,
      );
      console.log('🚀 GA Core Ready - ER precision gate PASSED');
      return { success: true, precision: results.precision, threshold };
    } else {
      console.log(
        `❌ FAIL: Precision ${(results.precision * 100).toFixed(2)}% < ${(threshold * 100).toFixed(1)}%`,
      );

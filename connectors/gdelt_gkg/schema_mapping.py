...
def derive_observation_id(gkg_record_id: str, element_type: str, element_code: str) -> str:
    source = f"{gkg_record_id}:{element_type}:{element_code}"
    return f"obs:{sha256(source.encode('utf-8')).hexdigest()[:24]}"
...
def map_gkg_to_intelgraph(record: GKGRecord) -> tuple[list[dict], list[dict]]:
    ...
    for theme in sorted(set(record.themes)):
        ...
        obs_id = derive_observation_id(record.gkg_record_id, "theme", theme)
        ...
        relationships.extend(
            [
                ...
                },
                {
                    "type": "EVIDENCED_BY",
                    "source_id": obs_id,
                    "target_id": record.gkg_record_id,
                },
                {
                    "type": "TARGETS",
                    "source_id": obs_id,
    ...
    for location in sorted(set(record.locations)):
        ...
        obs_id = derive_observation_id(record.gkg_record_id, "location", location)
        entities.append(

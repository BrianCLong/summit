...
class TestJsonConnector(unittest.TestCase):
    ...
    def setUp(self):
        self.sample_json_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../json_connector/sample.json'))
    ...
    def test_map_json_to_intelgraph_enhanced(self):
        ...
        actual_relationships_simplified.sort(key=lambda x: (x['type'], x['source_id'], x['target_id']))

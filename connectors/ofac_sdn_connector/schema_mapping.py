...
class OFACSdnMapper:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.sdn_list_url = self.config.get('sdn_list_url',
            'https://www.treasury.gov/ofac/downloads/sdn.csv')
        self.alt_names_url = self.config.get('alt_names_url',
            'https://www.treasury.gov/ofac/downloads/alt.csv')
        self.addresses_url = self.config.get('addresses_url',
    ...
    def map_sdn_to_entities(self, sdn_df: pd.DataFrame) -> List[Dict]:
        ...
        for _, row in sdn_df.iterrows():
            try:
                ...
                sanctions_programs = self.extract_sanctions_programs(
                    row.get('PROGRAMS', '')
                )
                ...
                entity = {
                    ...
                    'properties': {
                        'id': self.generate_entity_id(
                            str(row.get('ENT_NUM', '')),
                            self.clean_text(row.get('SDN_NAME', ''))
                        ),
                ...
                if entity_type == "PERSON":
                    entity['properties'].update({
                        'entity_category': 'sanctioned_individual',
                        'risk_level': 'high'
                    })
                elif entity_type == "ORGANIZATION":
                    entity['properties'].update({
                        'entity_category': 'sanctioned_organization',
                        'risk_level': 'high'
                    })
    ...
    def map_addresses(self, addr_df: pd.DataFrame, entities: List[Dict]) -> List[Dict]:
        ...
        relationships = []
        entity_lookup = {e['properties']['sdn_id']: e['properties']['id'] for e in entities if 'sdn_id' in e['properties']}
        ...
        for _, row in addr_df.iterrows():
            try:
                ...
                address_parts = []
                for field in ['ADDRESS_1', 'ADDRESS_2', 'ADDRESS_3', 'CITY', 'STATE_PROVINCE', 'POSTAL_CODE', 'COUNTRY']:
...
def map_ofac_sdn_to_intelgraph(config: Dict = None) -> Tuple[List[Dict], List[Dict]]:
    ...
    try:
        ...
        logger.info(f"Successfully mapped {len(entities)} entities and {len(all_relationships)} relationships")

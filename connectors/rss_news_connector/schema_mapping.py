...
class RSSNewsMapper:
    ...
    def parse_feed_item(self, entry: Dict, feed_url: str, feed_info: Dict) -> Optional[FeedItem]:
    ...
    def create_extracted_entities(self, item: FeedItem, extracted: List[Dict]) -> List[Dict]:
        ...
        for ext in extracted:
            entity_id = f"{ext['type'].lower()}-{hashlib.md5(ext['value'].encode()).hexdigest()[:12]}"
    ...
    def create_relationships(self, article_entity: Dict, extracted_entities: List[Dict]) -> List[Dict]:
    ...
    def process_feed_items(self, feed: feedparser.FeedParserDict, feed_url: str) -> Tuple[List[Dict], List[Dict]]:
        ...
        for entry in feed.entries:
            try:
                ...
                if extracted:
                    ...
                    item_relationships = self.create_relationships(article_entity, extracted_entities)
...
def map_rss_feeds_to_intelgraph(feed_urls: List[str], config: Dict = None) -> Tuple[List[Dict], List[Dict]]:
    """
    ...
    logger.info(f"Successfully processed {len(all_entities)} entities and {len(all_relationships)} relationships from {len(feed_urls)} feeds")

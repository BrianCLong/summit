...
_Focus: Calculating metrics on static datasets._
...
_Focus: Production monitoring and debugging._
...
_Focus: A/B testing and prompt refining._
...
| **Metric Depth**             | Outcome accuracy, simple "coherence" | **Step-wise Consistency**: Evaluating the _logic_ of intermediate steps, not just the final answer.                               |
...
**Novelty**: Most tracing tools (OpenTelemetry) are tree-structured (spans). Explicitly modeling _backtracking_ (correction) and _merging_ (consensus) as graph operations allows for novel metrics like "Reasoning Efficiency Ratio" (nodes on optimal path vs. total nodes).
...
**Novelty**: Applying "Active Learning" to the _evaluation_ phase to minimize compute spend while maximizing signal on model failure modes.

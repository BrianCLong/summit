...
const git_workflow_js_1 = require("../src/lib/git-workflow.js");
/**
 * Helper to execute git commands
 */
async function execGit(args, cwd) {
    return new Promise((resolve, reject) => {
        const proc = (0, child_process_1.spawn)('git', args, { cwd, stdio: ['pipe', 'pipe', 'pipe'] });
        let stdout = '';
        let stderr = '';
...
describe('Git Workflow', () => {
    ...
    describe('isGitRepo', () => {
        it('returns true for git repository', async () => {
            expect(await (0, git_workflow_js_1.isGitRepo)(repoRoot)).toBe(true);
        });
        it('returns false for non-git directory', async () => {
            const nonGitDir = path.join(tempDir, 'not-a-repo');
            fs.mkdirSync(nonGitDir);
            expect(await (0, git_workflow_js_1.isGitRepo)(nonGitDir)).toBe(false);
        });
    });
    describe('findRepoRoot', () => {
        it('finds repo root from subdirectory', async () => {
            ...
            const found = await (0, git_workflow_js_1.findRepoRoot)(subDir);
        ...
        it('returns null for non-repo directory', async () => {
            const nonGitDir = path.join(tempDir, 'not-a-repo');
            fs.mkdirSync(nonGitDir);
            const found = await (0, git_workflow_js_1.findRepoRoot)(nonGitDir);
            expect(found).toBeNull();
        });
    });
    describe('getGitStatus', () => {
        it('returns clean status for clean repo', async () => {
            const status = await (0, git_workflow_js_1.getGitStatus)(repoRoot);
    ...
    describe('GitWorkflow', () => {
        describe('initialize', () => {
            it('succeeds for clean repo', async () => {
                const workflow = (0, git_workflow_js_1.createGitWorkflow)({ repoRoot });
                await expect(workflow.initialize()).resolves.toBeUndefined();
            });
            it('throws for non-git directory', async () => {
                const nonGitDir = path.join(tempDir, 'not-a-repo');
                fs.mkdirSync(nonGitDir);
                const workflow = (0, git_workflow_js_1.createGitWorkflow)({ repoRoot: nonGitDir });
                await expect(workflow.initialize()).rejects.toThrow(git_workflow_js_1.GitWorkflowError);
                try {
                    await workflow.initialize();
                }
                catch (error) {
                    expect(error).toBeInstanceOf(git_workflow_js_1.GitWorkflowError);
            ...
            it('throws for dirty repo when allowDirty is false', async () => {
                ...
                const workflow = (0, git_workflow_js_1.createGitWorkflow)({
                    repoRoot,
                    allowDirty: false,
                });
                await expect(workflow.initialize()).rejects.toThrow(git_workflow_js_1.GitWorkflowError);
        ...
        describe('stageFiles', () => {
            it('stages specified files', async () => {
                ...
                const workflow = (0, git_workflow_js_1.createGitWorkflow)({ repoRoot, allowDirty: true });
                await workflow.initialize();
        ...
        describe('commit', () => {
            ...
            it('throws when nothing staged', async () => {
                const workflow = (0, git_workflow_js_1.createGitWorkflow)({ repoRoot });
                await workflow.initialize();
                await expect(workflow.commit('Empty commit')).rejects.toThrow(git_workflow_js_1.GitWorkflowError);
                try {
    ...
    describe('GitWorkflowError', () => {
        it('formats error with sorted details', () => {
            const error = new git_workflow_js_1.GitWorkflowError('Test error', 'test_reason', ['detail_z', 'detail_a', 'detail_m']);
        ...
        it('has correct exit code', () => {
            const error = new git_workflow_js_1.GitWorkflowError('Test', 'reason', [], git_workflow_js_1.GIT_WORKFLOW_EXIT_CODE);

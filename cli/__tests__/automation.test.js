...
const automation_js_1 = require("../src/automation.js");
describe('automation workflows', () => {
    test('runs steps sequentially with provided runner', async () => {
        const commands = [];
        const report = await (0, automation_js_1.runAutomationWorkflow)('init', {
            runner: async (command) => {
                commands.push(command);
                return { stdout: 'ok', stderr: '', exitCode: 0 };
            },
        });
        expect(commands).toEqual(automation_js_1.AUTOMATION_WORKFLOWS.init.map((step) => step.command));
    ...
    test('marks failures and preserves output', async () => {
        const report = await (0, automation_js_1.runAutomationWorkflow)('check', {
            runner: async (command) => {

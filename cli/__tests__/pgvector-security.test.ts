...
describe("PgVectorSync Security", () => {
  it("should reject invalid table names to prevent SQL injection", async () => {
    ...
    await expect(sync.ensureTable(maliciousOptions)).rejects.toThrow(/Invalid identifier/);
  });
  ...
  it("should reject invalid column names", async () => {
    ...
    await expect(sync.ensureTable({ idColumn: "id; --" })).rejects.toThrow(/Invalid identifier/);
  });
});

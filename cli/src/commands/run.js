...
const policy_js_1 = require("../lib/policy.js");
const sandbox_js_1 = require("../lib/sandbox.js");
const git_workflow_js_1 = require("../lib/git-workflow.js");
const provider_js_1 = require("../lib/provider.js");
const session_js_1 = require("../lib/session.js");
const replay_js_1 = require("../lib/replay.js");
const event_logger_js_1 = require("../lib/event-logger.js");
/**
...
function handleRunError(error) {
    if (error instanceof policy_js_1.PolicyError) {
        console.error(error.format());
        process.exit(error.exitCode);
    }
    if (error instanceof sandbox_js_1.SandboxError) {
        console.error(error.format());
        process.exit(error.exitCode);
    }
    if (error instanceof git_workflow_js_1.GitWorkflowError) {
        console.error(error.format());
        process.exit(error.exitCode);
    }
    if (error instanceof provider_js_1.ProviderError) {
        console.error(error.format());
        process.exit(error.exitCode);
    }
    if (error instanceof provider_js_1.BudgetExceededError) {
        console.error(error.format());
        process.exit(error.exitCode);
    }
...
async function initializeContext(options) {
    ...
    const policyGate = new policy_js_1.PolicyGate({
        ci: options.ci,
    ...
    const gitWorkflow = (0, git_workflow_js_1.createGitWorkflow)({
        repoRoot,
    ...
    const provider = (0, provider_js_1.createProviderWrapper)('cli', {
    ...
    const eventLogger = (0, event_logger_js_1.createEventLogger)({
        sessionDir,
...
async function executeAction(ctx, action) {
    ...
    // Evaluate policy
    const decision = ctx.policyGate.evaluate('run', { write: action.type === 'write_patch' }, [action]);
    // Record policy evaluation
    ctx.session.recordOperation('policy', action.type, decision.allow ? 'allowed' : 'denied', { action_type: action.type }, Date.now() - startTime);
    if (!decision.allow) {
        return {
            success: false,
    ...
    switch (action.type) {
        case 'read_file': {
            ...
            try {
                ...
                ctx.session.recordOperation('read', readAction.path, 'success', { size: content.length }, duration);
                ...
                ctx.session.recordOperation('read', readAction.path, 'failure', { error: error instanceof Error ? error.message : String(error) }, Date.now() - opStart);
                return {
                    success: false,
        ...
        case 'write_patch': {
            ...
            if (ctx.options.dryRun) {
                ctx.session.recordOperation('write', writeAction.files.join(','), 'success', { dryRun: true, diffBytes: writeAction.diff_bytes });
                return {
                    success: true,
                    result: {
                        dryRun: true,
            ...
            ctx.session.recordOperation('write', writeAction.files.join(','), 'success', { filesCount: writeAction.files.length, diffBytes: writeAction.diff_bytes });
        ...
        case 'exec_tool': {
            ...
            try {
                ...
                const duration = Date.now() - opStart;
                ctx.session.recordOperation('exec', execAction.tool, result.exitCode === 0 ? 'success' : 'failure', { args: execAction.args, exitCode: result.exitCode }, duration);
                // Log tool execution
                ...
                const duration = Date.now() - opStart;
                ctx.session.recordOperation('exec', execAction.tool, 'failure', { args: execAction.args, error: error instanceof Error ? error.message : String(error) }, duration);
        ...
        case 'network': {
            ...
            try {
                ...
                ctx.session.recordOperation('network', 'check', 'denied', { error: error instanceof Error ? error.message : String(error) }, Date.now() - opStart);
                return {
                    success: false,
                    error: error instanceof Error ? error.message : String(error),
                };
...
async function runStatus(options) {
    const repoRoot = (0, sandbox_js_1.detectRepoRoot)(process.cwd());
    const gitStatus = await (0, git_workflow_js_1.getGitStatus)(repoRoot);
    const status = {
        repoRoot,
        git: gitStatus,
        policy: {
 ...
 */
async function runExec(tool, args, options) {
    let ctx;
    const startTime = Date.now();
    try {
 ...
 */
async function runCommit(message, options) {
    let ctx;
    try {
 ...
 */
async function runBranch(branchName, options) {
    let ctx;
    try {
        ctx = await initializeContext({
            ...options,
 ...
 */
async function runReview(baseBranch, options) {
    let ctx;
    try {
        ctx = await initializeContext(options);
        // Log run start
        ...
        const reviewPath = await ctx.gitWorkflow.writeReviewFile(review, options.reviewPath || 'review.md');
        ctx.session.recordOperation('write', reviewPath, 'success', { type: 'review' });
        ctx.eventLogger.logAction({
            action_type: 'write',
            affected_files: [reviewPath],
...
function registerRunCommands(program) {
    ...
    const addGlobalOptions = (cmd) => {
        return cmd
            ...
            .option('--unsafe-allow-sensitive-paths', 'Disable hardcoded security patterns (dangerous)', false)
    ...
    const parseOptions = (opts) => ({
        ...opts,
        toolTimeoutMs: parseInt(opts.toolTimeoutMs, 10),
        maxRetries: parseInt(opts.maxRetries, 10),
        initialBackoffMs: parseInt(opts.initialBackoffMs, 10),
        maxBackoffMs: parseInt(opts.maxBackoffMs, 10),
        timeoutMs: parseInt(opts.timeoutMs, 10),
        budgetMs: opts.budgetMs ? parseInt(opts.budgetMs, 10) : null,
        maxRequests: opts.maxRequests ? parseInt(opts.maxRequests, 10) : null,
        tokenBudget: opts.tokenBudget ? parseInt(opts.tokenBudget, 10) : null,
    ...
    addGlobalOptions(run
        .command('status')
        .description('Show git, policy, and sandbox status')).action(async (opts) => {
        await runStatus(parseOptions(opts));
    });
    // run check
    addGlobalOptions(run
        .command('check')
        .description('Validate policy and sandbox configuration')).action(async (opts) => {
        await runCheck(parseOptions(opts));
    });
    // run exec <tool> [args...]
    addGlobalOptions(run
        .command('exec <tool> [args...]')
        .description('Execute a tool with sandbox restrictions')).action(async (tool, args, opts) => {
        await runExec(tool, args, parseOptions(opts));
    });
    // run commit <message>
    addGlobalOptions(run
        .command('commit <message>')
        .description('Create a commit with optional review artifact')).action(async (message, opts) => {
        await runCommit(message, parseOptions(opts));
    });
    // run branch <name>
    addGlobalOptions(run
        .command('branch <name>')
        .description('Create or switch to a branch')).action(async (name, opts) => {
        await runBranch(name, parseOptions(opts));
    });
    // run review [base-branch]
    addGlobalOptions(run
        .command('review [base-branch]')
        .description('Generate review.md artifact')).action(async (baseBranch = 'main', opts) => {
    ...
    sessions
        ...
        const repoRoot = (0, sandbox_js_1.detectRepoRoot)(process.cwd());
        const sessionDir = `${repoRoot}/.claude/sessions`;
        const allSessions = (0, session_js_1.listSessions)(sessionDir);
    ...
    sessions
        ...
        const session = (0, session_js_1.loadSession)(sessionFile);
        if (!session) {
    ...
    sessions
        ...
        const cleaned = (0, session_js_1.cleanOldSessions)(sessionDir, maxAgeDays);
    ...
    run
        ...
        const sessionDir = `${repoRoot}/.claude/sessions`;
        try {
            const { events, summary } = (0, replay_js_1.replaySession)({
                sessionDir,
                runId: sessionId,
            });
            if (events.length === 0) {
                console.error(`Session not found or no events: ${sessionId}`);
                process.exit(constants_js_1.EXIT_CODES.POLICY_ERROR);
            }
            // Output summary
            if (opts.output === 'json') {
                console.log((0, replay_js_1.formatSummaryJson)(summary));
            }
            else {
                console.log((0, replay_js_1.formatSummaryText)(summary));
            }
            // Write report if requested
            if (opts.writeReport) {
                const reportPath = (0, replay_js_1.writeReportArtifact)({ sessionDir, runId: sessionId }, summary, events);

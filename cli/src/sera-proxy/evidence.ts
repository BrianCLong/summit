...
export class SeraProxyEvidenceStore {
  ...
  recordExchange(requestBody: string, responseBody: string, policyDecisionId: string): void {
    const requestSha256 = sha256Hex(requestBody);
    const responseSha256 = sha256Hex(responseBody);
    const previousHash =
      this.entries.length > 0 ? this.entries[this.entries.length - 1].entryHash : null;

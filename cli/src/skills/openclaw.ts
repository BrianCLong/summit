...
const RISK_PATTERNS = [/raw\s+transaction/i, /transaction\s+json/i, /calldata/i, /submit\s+raw/i];
...
export function ingestOpenClawRepo(options: {
  ...
  fs.writeFileSync(path.join(outputDir, "skill-index.json"), JSON.stringify(index, null, 2));
...
function normalizeMetadata(value: unknown): OpenClawSkillFrontmatter["metadata"] | undefined {
...
function normalizeClawdbot(
  value: unknown
): OpenClawSkillFrontmatter["metadata"]["clawdbot"] | undefined {
  if (!value || typeof value !== "object") {
    return undefined;
  }
...
function normalizeRequires(
  value: unknown
): OpenClawSkillFrontmatter["metadata"]["clawdbot"]["requires"] | undefined {
  if (!value || typeof value !== "object") {
    return undefined;
  }

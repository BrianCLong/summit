...
export interface SwitchboardRegistryValidationResult {
  registry: SwitchboardRegistry;
  stats: SwitchboardRegistryStats;
  marker: string;
}
...
export function buildSwitchboardRegistryPassMarker(stats: SwitchboardRegistryStats): string {
  return `REGISTRY PASS (servers=${stats.servers}, tools=${stats.tools}, capabilities=${stats.capabilities})`;
}

export function validateSwitchboardRegistry(
  registryPath: string
): SwitchboardRegistryValidationResult {
  const registry = loadSwitchboardRegistry(registryPath);
  const stats = getSwitchboardRegistryStats(registry);
  return {
    registry,
    stats,
    marker: buildSwitchboardRegistryPassMarker(stats),
  };
}

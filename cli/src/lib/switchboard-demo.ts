...
import { z } from 'zod';
import {
  buildSwitchboardRegistryPassMarker,
  findSwitchboardTool,
...
const DEMO_TIMESTAMP = '2026-02-10T00:00:00.000Z';
...
function loadDemoPolicy(policyPath: string): z.infer<typeof DemoPolicySchema> {
  ...
  const result = DemoPolicySchema.safeParse(parsed);
  if (!result.success) {
...
export function runSwitchboardDemo(repoRoot: string, paths?: Partial<SwitchboardDemoPaths>): SwitchboardDemoResult {
  ...
  const validation = validateSwitchboardRegistry(resolvedPaths.registryPath);
  const registry = validation.registry;
  const registryStats = validation.stats;
  console.log(buildSwitchboardRegistryPassMarker(registryStats));

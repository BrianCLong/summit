...
const switchboard_policy_js_1 = require("./switchboard-policy.js");
...
function copyPath(src, dest, allowMissing) {
...
function bootstrapWritePath(baseDir, normalizedPath, isDirHint) {
    if (!normalizedPath) {
        return;
    }
...
function buildEnv(manifest, secrets) {
    const env = {
...
function normalizeCapsulePaths(paths) {
    return paths.map((entry) => {
        const normalized = (0, switchboard_capsule_js_1.normalizeRelativePath)(entry);
        if (!normalized) {
...
function evaluatePolicy(ledger, gate, action, target) {
...
function loadSecrets(step, gate, ledger) {
    const secrets = {};
...
function evaluatePathActions(step, gate, ledger) {
...
async function runCapsule(options) {
    const manifest = (0, switchboard_capsule_js_1.loadCapsuleManifest)(options.manifestPath);
    ...
    const ledger = new switchboard_ledger_js_1.CapsuleLedger(sessionDir, sessionId);
    ...
    const gate = new switchboard_policy_js_1.CapsulePolicyGate(manifest, options.waiverToken);
    ...
    for (const capsulePath of readPaths) {
        copyPath(path.join(repoRoot, capsulePath.normalized), path.join(snapshotDir, capsulePath.normalized), false);
        copyPath(path.join(repoRoot, capsulePath.normalized), path.join(workspaceDir, capsulePath.normalized), false);
    }
    for (const capsulePath of writePaths) {
        copyPath(path.join(repoRoot, capsulePath.normalized), path.join(snapshotDir, capsulePath.normalized), true);
        copyPath(path.join(repoRoot, capsulePath.normalized), path.join(workspaceDir, capsulePath.normalized), true);
    ...
    try {
        for (let index = 0; index < manifest.steps.length; index += 1) {
            ...
            const result = (0, child_process_1.spawnSync)(step.command, step.args, {
                cwd: workspaceDir,
                env,
                encoding: 'utf8',
            });
        ...
        const diffResult = (0, child_process_1.spawnSync)('git', ['diff', '--no-index', '--', 'snapshot', 'workspace'], {
            encoding: 'utf8',
            cwd: sessionDir,
        });

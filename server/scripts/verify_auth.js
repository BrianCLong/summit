...
async function unifiedAuthMiddleware(req, res, next) {
    ...
    if (!principal) {
        ...
        if (apiKey) {
            ...
            // Checking DB mock
            const dbResult = await mockPool.query('SELECT * FROM api_keys WHERE key_hash = $1 AND is_active = true', [hashedKey]);
...
async function run() {
    ...
    const hashed = crypto_1.default.createHash('sha256').update(realKey).digest('hex');

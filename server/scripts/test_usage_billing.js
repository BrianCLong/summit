...
async function main() {
    ...
    try {
        await (0, database_js_1.connectPostgres)();
        const pool = (0, database_js_1.getPostgresPool)();
        // 1. Setup Data
        const tenantId = 'test-tenant-' + Date.now();
        console.log(`Using Tenant: ${tenantId}`);
        // Create a plan
        const planRes = await pool.query(`INSERT INTO plans (name, limits) VALUES ($1, $2) RETURNING id`, ['Test Plan ' + Date.now(), JSON.stringify({
                'test.tokens': { hardCap: 1000, monthlyIncluded: 100, unitPrice: 0.01, softThresholds: [800] }
            })]);
        const planId = planRes.rows[0].id;
        // Assign plan
        await pool.query(`INSERT INTO tenant_plans (tenant_id, plan_id) VALUES ($1, $2)`, [tenantId, planId]);
        // 2. Test Metering
        ...
        const res1 = await QuotaService_js_1.QuotaService.checkQuota({
            tenantId,
            kind: 'test.tokens',
            quantity: 100
        });
        ...
        const res2 = await QuotaService_js_1.QuotaService.checkQuota({
            tenantId,
            kind: 'test.tokens',
            quantity: 300
        });
        ...
        await pool.query(`INSERT INTO usage_summaries (tenant_id, period_start, period_end, kind, total_quantity, unit)
             VALUES ($1, $2, $3, 'test.tokens', 800, 'tokens')`, [tenantId, start.toISOString(), end.toISOString()]);
        const invoice = await PricingEngine_js_1.PricingEngine.generateInvoice(tenantId, start.toISOString(), end.toISOString());

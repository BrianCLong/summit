...
class SecurityVerifier {
    ...
    constructor() {
        ...
        const currentDir = typeof __dirname !== 'undefined'
            ? __dirname
            : path.dirname((0, url_1.fileURLToPath)(import.meta.url));
    ...
    verifyAuthCoverage() {
        ...
        for (const route of criticalRoutes) {
            ...
            const hasAuth = content.includes('ensureAuthenticated') ||
        ...
        if (missingAuth.length === 0) {
            this.addResult('Auth Coverage', true, 'All critical routes have authentication middleware');
        }
        else {
            this.addResult('Auth Coverage', false, 'Some critical routes missing authentication middleware', missingAuth);
        }
    }
    /**
    ...
    verifyTenantIsolation() {
        ...
        const hasTenantMiddleware = appFile.includes('tenantContextMiddleware') ||
            appFile.includes('TenantIsolationGuard');
        const hasResidencyEnforcement = appFile.includes('residency') ||
            appFile.includes('Residency');
        if (hasTenantMiddleware && hasResidencyEnforcement) {
            this.addResult('Tenant Isolation', true, 'Tenant context and residency enforcement middleware are present');
        }
        else {
            const missing = [];
            if (!hasTenantMiddleware)
                missing.push('tenant context middleware');
            if (!hasResidencyEnforcement)
                missing.push('residency enforcement');
            this.addResult('Tenant Isolation', false, 'Missing tenant isolation components', missing);
        }
    }
    /**
    ...
    verifyAdminAuthorization() {
        ...
        for (const route of adminRoutes) {
            ...
            if (!content)
                continue;
            const hasAuthz = content.includes('authorize(') ||
                content.includes('requirePermission(') ||
        ...
        if (missingAuthz.length === 0) {
            this.addResult('Admin Authorization', true, 'All admin routes have authorization checks');
        }
        else {
            this.addResult('Admin Authorization', false, 'Some admin routes missing authorization checks', missingAuthz);
        }
    }
    /**
    ...
    verifyRateLimiting() {
        ...
        const importsPublicRateLimit = appFile.includes('publicRateLimit') &&
            (appFile.includes("from './middleware/rateLimiter.js'") ||
                appFile.includes("from './middleware/rateLimiter.ts'"));
        const importsAuthRateLimit = appFile.includes('authenticatedRateLimit') &&
            (appFile.includes("from './middleware/rateLimiter.js'") ||
                appFile.includes("from './middleware/rateLimiter.ts'"));
        // Check for usage
        const usesPublicRateLimit = /app\.use\(publicRateLimit\)/.test(appFile);
        const usesAuthRateLimit = /app\.use\(\[['"]\/api['"],\s*['"]\/graphql['"]\],\s*authenticatedRateLimit\)/.test(appFile) ||
            appFile.includes('authenticatedRateLimit');
        const hasTieredLimits = appFile.includes('TieredRateLimitMiddleware') ||
            appFile.includes('TenantIsolationGuard');
        ...
        if (allChecks) {
            this.addResult('Rate Limiting', true, 'Rate limiting is configured for public, authenticated, and tenant-based access');
        }
        else {
            const missing = [];
            if (!importsPublicRateLimit)
                missing.push('publicRateLimit import');
            if (!importsAuthRateLimit)
                missing.push('authenticatedRateLimit import');
            if (!usesPublicRateLimit)
                missing.push('publicRateLimit usage in app.use()');
            if (!usesAuthRateLimit)
                missing.push('authenticatedRateLimit usage for /api & /graphql');
            if (!hasTieredLimits)
                missing.push('tiered/tenant rate limiting');
            if (!hasRateLimiterConfig)
                missing.push('rate limiter configuration file');
            this.addResult('Rate Limiting', false, 'Rate limiting configuration incomplete', missing);
        }
    }
    /**
    ...
    verifyInputValidation() {
        ...
        const hasSanitization = appFile.includes('sanitizeInput') ||
            appFile.includes('mongo-sanitize');
        const hasPiiGuard = appFile.includes('piiGuardMiddleware') ||
            appFile.includes('pii-guard');
        const validatorFile = this.readFile('src/middleware/request-schema-validator.ts');
        const hasSchemaValidation = validatorFile !== null && (validatorFile.includes('zod') || validatorFile.includes('joi'));
        if (hasSanitization && hasPiiGuard && hasSchemaValidation) {
            this.addResult('Input Validation', true, 'Input sanitization, PII guard, and schema validation are configured');
        }
        else {
            const missing = [];
            if (!hasSanitization)
                missing.push('input sanitization (mongo-sanitize)');
            if (!hasPiiGuard)
                missing.push('PII guard middleware');
            if (!hasSchemaValidation)
                missing.push('schema validation (Zod/Joi)');
            this.addResult('Input Validation', false, 'Input validation stack incomplete', missing);
        }
    }
    /**
     * Verification 6: Logging redaction configured
     */
    verifyLoggingRedaction() {
        ...
        const redactsAuth = loggingFile.includes('req.headers.authorization') ||
            loggingFile.includes('authorization');
        const redactsCookie = loggingFile.includes('req.headers.cookie') ||
            loggingFile.includes('cookie');
        const hasCorrelationId = loggingFile.includes('correlationId');
        if (redactsAuth && redactsCookie && hasCorrelationId) {
            this.addResult('Logging Redaction', true, 'Logging redacts sensitive headers and includes correlation tracking');
        }
        else {
            const missing = [];
            if (!redactsAuth)
                missing.push('authorization header redaction');
            if (!redactsCookie)
                missing.push('cookie header redaction');
            if (!hasCorrelationId)
                missing.push('correlation ID tracking');
            this.addResult('Logging Redaction', false, 'Logging configuration incomplete', missing);
        }
    }
    /**
    ...
    verifySecurityHeaders() {
        ...
        if (missing.length === 0 && usesHelmet) {
            this.addResult('Security Headers', true, 'All required security headers are configured via Helmet');
        }
        else {
            const missingDetails = [];
            if (!usesHelmet)
                missingDetails.push('helmet middleware');
            missingDetails.push(...missing.map(config => `${config} configuration`));
            this.addResult('Security Headers', false, 'Some security headers configuration is missing', missingDetails);
        }
    }
    /**
    ...
    verifyCORS() {
        ...
        const hasCorsOriginCheck = corsFile.includes('CORS_ORIGIN') ||
            corsFile.includes('origin');
        const hasCredentials = corsFile.includes('credentials: true');
        const hasProductionCheck = corsFile.includes('NODE_ENV') &&
            corsFile.includes('production');
        if (hasCorsOriginCheck && hasCredentials && hasProductionCheck) {
            this.addResult('CORS Configuration', true, 'CORS is configured with origin whitelist and credentials support');
        }
        else {
            const missing = [];
            if (!hasCorsOriginCheck)
                missing.push('origin validation');
            if (!hasCredentials)
                missing.push('credentials support');
            if (!hasProductionCheck)
                missing.push('production environment check');
            this.addResult('CORS Configuration', false, 'CORS configuration incomplete', missing);
        }
    }
    /**
     * Verification 9: GraphQL security (shield, complexity, introspection)
     */
    verifyGraphQLSecurity() {
        ...
        const hasShield = graphqlFile.includes('graphql-shield') ||
            graphqlFile.includes('shield');
        const hasComplexity = graphqlFile.includes('complexity') ||
            graphqlFile.includes('queryComplexity');
        const hasIntrospectionControl = graphqlFile.includes('introspection');
        if (hasShield && hasComplexity && hasIntrospectionControl) {
            this.addResult('GraphQL Security', true, 'GraphQL has shield rules, complexity limits, and introspection control');
        }
        else {
            const missing = [];
            if (!hasShield)
                missing.push('graphql-shield or equivalent');
            if (!hasComplexity)
                missing.push('query complexity limiting');
            if (!hasIntrospectionControl)
                missing.push('introspection control');
            this.addResult('GraphQL Security', false, 'GraphQL security configuration incomplete', missing);
        }
    }
    /**
    ...
    verifyProductionSecrets() {
        ...
        const hasSecretValidation = configFile.includes('JWT_SECRET') &&
            configFile.includes('min(32)') ||
            configFile.includes('length') &&
                configFile.includes('32');
        const hasTestSecretBlock = configFile.includes('devpassword') ||
            configFile.includes('changeme') ||
            configFile.includes('insecure');
        if (hasSecretValidation && hasTestSecretBlock) {
            this.addResult('Production Secrets', true, 'Production secret validation and test secret blocking are configured');
        }
        else {
            const missing = [];
            if (!hasSecretValidation)
                missing.push('JWT secret length validation (min 32 chars)');
            if (!hasTestSecretBlock)
                missing.push('test secret blocking in production');
            this.addResult('Production Secrets', false, 'Production secret validation incomplete', missing);
        }
    }
    /**
    ...
    verifyAuditLogging() {
        ...
        const hasAuditMiddleware = appFile.includes('auditLogger') ||
        ...
        if (hasAuditMiddleware && hasPermissionAudit) {
            this.addResult('Audit Logging', true, 'Audit logging is configured for requests and permission checks');
        }
        else {
            const missing = [];
            if (!hasAuditMiddleware)
                missing.push('audit middleware in app.ts');
            if (!hasPermissionAudit)
                missing.push('permission check audit logging');
            this.addResult('Audit Logging', false, 'Audit logging configuration incomplete', missing);
        }
    }
    /**
    ...
    verifyStepUpAuth() {
        ...
        if (!stepupFile) {
            this.addResult('Step-Up Auth', false, 'Step-up authentication middleware not found', ['Expected middleware/stepup.ts or src/middleware/stepup.ts']);
            return;
        }
        const hasStepUpFunction = stepupFile.includes('requireStepUp') ||
            stepupFile.includes('x-mfa-level');
        if (hasStepUpFunction) {
            this.addResult('Step-Up Auth', true, 'Step-up authentication middleware is configured');
        }
        else {
            this.addResult('Step-Up Auth', false, 'Step-up authentication not properly configured', ['Missing requireStepUp or x-mfa-level validation']);
        }
    }
    /**

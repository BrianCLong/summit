...
const videoPath = path_1.default.resolve(VIDEO_PATH_ARG);
...
async function testVideoExtraction() {
    ...
    try {
        ...
        const submitResponse = await axios_1.default.post(`${API_BASE_URL}/extract-video`, {
            mediaPath: videoPath,
            mediaType: 'VIDEO',
        ...
        while (jobStatus !== 'completed' && jobStatus !== 'failed') {
            ...
            const statusResponse = await axios_1.default.get(`${API_BASE_URL}/job-status/${jobId}`, {
                headers: {
                    Authorization: `Bearer ${AUTH_TOKEN}`,
                },
            });

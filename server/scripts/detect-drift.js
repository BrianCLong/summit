...
const schemaPath = path_1.default.resolve(__dirname, '../src/graphql/schema.ts');
...
async function checkPostgresDrift() {
    ...
    const sqlDir = path_1.default.resolve(__dirname, '../db/migrations/postgres');
    ...
    `);
        ...
        const files = fs_1.default.readdirSync(sqlDir).filter(f => f.endsWith('.sql'));
        // Check for pending
...
async function checkNeo4jDrift() {
    ...
    for (const def of doc.definitions) {
        if (def.kind === 'ObjectTypeDefinition' && !['Query', 'Mutation', 'Subscription'].includes(def.name.value)) {
            ...
            if (typeName.endsWith('Result') || typeName.endsWith('Payload') || typeName.endsWith('Response') || typeName.endsWith('Edge') || typeName.endsWith('Connection'))
                continue;
    ...
    try {
        ...
        const constraints = result.records.map(r => {
            ...
            let labels = [];
            let properties = [];
            if (obj.labelsOrTypes)
                labels = Array.isArray(obj.labelsOrTypes) ? obj.labelsOrTypes : [obj.labelsOrTypes];
            if (obj.properties)
                properties = Array.isArray(obj.properties) ? obj.properties : [obj.properties];
            return {
        ...
        for (const [typeName, fields] of typeMap.entries()) {
            for (const field of fields) {
                const exists = constraints.some(c => (c.type === 'UNIQUENESS' || c.type === 'UNIQUE') &&

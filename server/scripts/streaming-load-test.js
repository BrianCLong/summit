...
const EventFactory_js_1 = require("../src/streaming/EventFactory.js");
const Logger_js_1 = require("../src/streaming/Logger.js");
const logger = new Logger_js_1.Logger('LoadTest');
...
async function run() {
    ...
    const registry = USE_GLUE
        ? new SchemaRegistry_js_1.GlueSchemaRegistry(REGION, REGISTRY)
        : new SchemaRegistry_js_1.MockSchemaRegistry();
    if (!USE_GLUE) {
        // Pre-register if mock
        await registry.register(TOPIC, {});
    }
    const producer = new KafkaProducer_js_1.KafkaProducerWrapper({
    ...
    const timer = setInterval(async () => {
        ...
        for (let i = 0; i < batchSize; i++) {
            messages.push(EventFactory_js_1.EventFactory.createEvent('load.test', 'load-generator', {
                seq: sent + i,
                ts: Date.now(),

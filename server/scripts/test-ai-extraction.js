...
const config = {
  ...
  tempPath:
    process.env.AI_TEMP_PATH || path.join(os.tmpdir(), 'intelgraph-test'),
...
function createTestImage() {
  ...
  try {
    ...
    fs.writeFileSync(
      fallbackPath,
      'This is a test document for AI extraction testing. It contains sample text for analysis.',
    );
    return fallbackPath;
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    python.on('close', (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(`Python script failed (code ${code}): ${stderr || stdout}`),
        );
      }
    });
...
async function testObjectDetection(imagePath) {
  ...
  try {
    ...
    if (result.detections && Array.isArray(result.detections)) {
      log(
        'SUCCESS',
        `Object detection: Found ${result.detections.length} objects`,
      );
      if (result.detections.length > 0) {
        log(
          'INFO',
          `Detected: ${result.detections.map((d) => d.class_name).join(', ')}`,
        );
      }
      return true;
    } else if (result.error) {
      ...
      log(
        'WARNING',
        'Object detection: No detections found (this may be normal for test images)',
      );
...
async function testWhisperTranscription(audioPath) {
  ...
  try {
    ...
    if (result.segments && Array.isArray(result.segments)) {
      log(
        'SUCCESS',
        `Speech transcription: Processed ${result.segments.length} segments`,
      );
      if (result.text) {
      ...
      log(
        'WARNING',
        'Speech transcription: No segments found (expected for silent audio)',
      );
      return true;
    }
  } catch (error) {
...
async function testDependencies() {
  ...
  const dependencies = [
    {
      name: 'torch',
      import: 'import torch; print(f"PyTorch {torch.__version__}")',
    },
    ...
    },
    {
      ...
      import: 'import sentence_transformers; print("Sentence Transformers OK")',
    },
    { name: 'whisper', import: 'import whisper; print("Whisper OK")' },
    {
      name: 'ultralytics',
      import: 'import ultralytics; print("Ultralytics OK")',
    },
    {
      name: 'spacy',
      import: 'import spacy; print(f"spaCy {spacy.__version__}")',
    },
  ];
...
async function runExtractorTests() {
  ...
  const tests = [
    ...
    { name: 'Object Detection', fn: () => testObjectDetection(imagePath) },
    {
      ...
      fn: () => testWhisperTranscription(audioPath),
    },
  ];
  ...
  try {
    ...
    if (fs.existsSync(config.tempPath))
      fs.rmdirSync(config.tempPath, { recursive: true });
  } catch (error) {
...
async function main() {
  console.log(
    `${colors.cyan}🧪 IntelGraph AI Extraction Testing${colors.reset}`,
  );
  console.log(
  ...
  try {
    ...
    if (successCount === totalTests) {
      ...
      console.log(
        `${colors.green}The IntelGraph AI engines are ready for use.${colors.reset}`,
      );
      process.exit(0);
    } else {
      console.log(`\n${colors.yellow}⚠️  Some tests failed.${colors.reset}`);
      console.log(
        `${colors.yellow}Please check the error messages above and ensure all dependencies are installed.${colors.reset}`,
      );
      console.log(
        `${colors.yellow}Run './scripts/setup-ai-models.sh' to install missing dependencies.${colors.reset}`,
      );
      process.exit(1);
    }
  } catch (error) {

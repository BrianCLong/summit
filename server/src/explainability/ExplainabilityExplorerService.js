...
exports.ExplainabilityExplorerService = void 0;
 ...
 */
class ExplainabilityExplorerService {
    static instance;
    constructor() {
     ...
     */
    async listRuns(tenantId, filter = {}, requesterId) {
      ...
      `;
            ...
            const runs = await Promise.all(result.rows.map((row) => this.mapRowToExplainableRun(row)));
     ...
     * Enforces tenant isolation.
     */
    async getRun(runId, tenantId, requesterId) {
        const requestId = (0, uuid_1.v4)();
        const queriedAt = new Date().toISOString();
        try {
            const sql = `
      ...
      `;
            const result = await (0, timescale_js_1.query)(sql, [runId, tenantId]);
     ...
     */
    async getLineage(runId, tenantId, depth = 3) {
        const requestId = (0, uuid_1.v4)();
        const queriedAt = new Date().toISOString();
        try {
            // Get the run first
            ...
            const nodes = [{ id: runId, type: 'run', data: run }];
            const edges = [];
            // Add provenance chain if exists
            if (run.provenance_links.provenance_chain_id && depth > 0) {
                const provenanceService = provenance_ledger_beta_js_1.ProvenanceLedgerBetaService.getInstance();
                // Add claims
     ...
     */
    async compareRuns(runIdA, runIdB, tenantId) {
        const requestId = (0, uuid_1.v4)();
        const queriedAt = new Date().toISOString();
        try {
            ...
            const comparison = {
                ...
                deltas: {
                    ...
                    duration_delta_ms: runA.duration_ms !== null && runB.duration_ms !== null
                        ? runB.duration_ms - runA.duration_ms
                        : null,
                    different_capabilities: this.arrayDiff(runA.capabilities_used, runB.capabilities_used),
                    different_policies: this.arrayDiff(runA.policy_decisions.map((d) => d.policy_name), runB.policy_decisions.map((d) => d.policy_name)),
                },
            };
            return {
                success: true,
     ...
     */
    async verifyLinkage(runId, tenantId) {
        const requestId = (0, uuid_1.v4)();
        const queriedAt = new Date().toISOString();
        try {
            // Get the run
     ...
     */
    async mapRowToExplainableRun(row) {
    ...
    async fetchRunClaims(runId) {
        try {
            const sql = `
        SELECT * FROM explainable_run_claims
        WHERE run_id = $1
      `;
            const result = await (0, timescale_js_1.query)(sql, [runId]);
            ...
            logger_js_1.default.warn({ message: 'Failed to fetch claims', run_id: runId, error: err });
            return [];
        }
    }
    async fetchRunEvidence(runId) {
        try {
            const sql = `
      ...
      `;
            ...
            logger_js_1.default.warn({ message: 'Failed to fetch evidence', run_id: runId, error: err });
            return [];
        }
    }
    async fetchRunSources(runId) {
        try {
            const sql = `
      ...
      `;
            ...
            logger_js_1.default.warn({ message: 'Failed to fetch sources', run_id: runId, error: err });
            return [];
        }
    }
    async fetchRunTransforms(runId) {
        try {
            const sql = `
      ...
      `;
            ...
            logger_js_1.default.warn({ message: 'Failed to fetch transforms', run_id: runId, error: err });
            return [];
        }
    }
    hashObject(obj) {
...
exports.ExplainabilityExplorerService = ExplainabilityExplorerService;

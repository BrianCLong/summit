...
class KnowledgeRepository {
    ...
    async upsertKnowledgeObject(obj) {
        ...
        try {
            await client.query(`INSERT INTO knowledge_objects (
    ...
    async upsertEmbedding(emb) {
        ...
        try {
            ...
            await client.query(`INSERT INTO embedding_records (
    ...
    async deleteObject(tenantId, objectId) {
        await this.pool.query(`DELETE FROM knowledge_objects WHERE id = $1 AND tenant_id = $2`, [objectId, tenantId]);
    }
    // --- Search Methods ---
    async searchKeyword(query) {
        ...
        try {
            ...
            const qIdx = pIdx;
            const parsedTopK = typeof query.topK === 'number'
                ? query.topK
    ...
    async searchVector(query, embeddingVector) {
        const client = await this.pool.connect();
        try {

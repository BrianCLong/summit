...
export function applyDisclosurePolicy(args: {
  ...
  const deniedArtifacts = requested.filter(
    (artifact) => !policy.allowedArtifacts.includes(artifact),
  );
  ...
  const allowedFields = policyAllowlist
    ? requestedAllowlist && requestedAllowlist.length
      ...
      : policyAllowlist
    : requestedAllowlist;
  const allowlistTrimmed =
    Boolean(requestedAllowlist?.length) &&
    (allowedFields?.length ?? 0) > 0 &&
    (allowedFields?.length ?? 0) < requestedAllowlist!.length;
  return {
    artifacts,
    allowedFields,
...
export class DisclosureExportService {
  ...
  async createJob(input: unknown): Promise<DisclosureExportJob> {
    ...
    if (disclosurePolicy.deniedArtifacts.length > 0) {
      warnings.push(
        ...disclosurePolicy.deniedArtifacts.map(
          (artifact) => `artifact_not_allowed:${artifact}`,
        ),
      );
    }
    if (disclosurePolicy.allowlistTrimmed) {
      warnings.push('allowed_fields_trimmed');
    }

...
exports.DEFAULT_TAXONOMY_NAME = exports.TaxonomyManager = exports.defaultTaxonomyDefinition = void 0;
const DEFAULT_TAXONOMY_NAME = 'global-default';
exports.DEFAULT_TAXONOMY_NAME = DEFAULT_TAXONOMY_NAME;
const buildNode = (id, label, piiTypes, severity, categories, description, policyTags = []) => ({
    id,
    label,
    piiTypes,
    severity,
...
exports.defaultTaxonomyDefinition = {
    name: DEFAULT_TAXONOMY_NAME,
    version: '1.0.0',
    nodes: [
        buildNode('identity.core', 'Identity Core', [
            'fullName',
            'firstName',
            'lastName',
            'middleName',
            ...
            'alias',
            'preferredName',
        ], 'medium', ['identity'], 'Personally identifying names'),
        buildNode('identity.credentials', 'Identity Credentials', [
            'username',
            'password',
            'passwordHint',
            'accountToken',
            ...
            'sessionId',
            'cookieId',
        ], 'high', ['identity', 'credentials'], 'Account credentials require high protection', ['restricted']),
        ...
        buildNode('location.address', 'Physical Address', [
            'homeAddress',
            'mailingAddress',
            'shippingAddress',
            'billingAddress',
            ...
            'state',
            'country',
        ], 'high', ['location'], 'Address and locality data', ['restricted']),
        ...
        buildNode('government.identity', 'Government Identifiers', [
            'nationalId',
            ...
            'licensePlate',
        ], 'critical', ['identity', 'regulatory'], 'Government issued identifiers', ['restricted', 'audit']),
        buildNode('financial.accounts', 'Financial Accounts', [
            'creditCardNumber',
            'debitCardNumber',
            'cardExpiration',
            'cardSecurityCode',
        ...
        ], 'critical', ['financial'], 'Financial account and transaction data', ['pci']),
        ...
        buildNode('biometric', 'Biometric Identifiers', [
            'biometricFingerprint',
            'biometricFace',
            'biometricVoice',
            'biometricRetina',
            'biometricDNA',
        ], 'critical', ['biometric'], 'Biometric measurements', ['phi', 'restricted']),
...
class TaxonomyManager {
    taxonomies = new Map();
    cache = new Map();
    constructor(baseDefinition = exports.defaultTaxonomyDefinition) {
        this.registerTaxonomy(baseDefinition, { overwrite: true });
    }
    registerTaxonomy(definition, options = {}) {
    ...
    getNodeForType(type, taxonomyName = DEFAULT_TAXONOMY_NAME) {
        const taxonomyCache = this.ensureCache(taxonomyName);
        return taxonomyCache.get(type);
    }
    classify(type, taxonomyName = DEFAULT_TAXONOMY_NAME) {
    ...
    getDefinition(name = DEFAULT_TAXONOMY_NAME) {

...
exports.MetadataStoreImpl = void 0;
...
class MetadataStoreImpl {
     ...
     */
    async storeCatalogMetadata(entry) {
        if (!this.postgresClient) {
            throw new Error('PostgreSQL client not configured');
        }
        const sql = (0, metadata_js_1.toSQLColumns)(entry.sensitivity);
     ...
     */
    async getCatalogMetadata(catalogId) {
        if (!this.postgresClient) {
            throw new Error('PostgreSQL client not configured');
        }
        const result = await this.postgresClient.query(`SELECT * FROM ${this.catalogTable} WHERE catalog_id = $1`, [catalogId]);
     ...
     */
    async queryCatalogBySensitivity(sensitivityClass) {
        if (!this.postgresClient) {
            throw new Error('PostgreSQL client not configured');
        }
        const result = await this.postgresClient.query(`SELECT * FROM ${this.catalogTable} WHERE sensitivity_class = $1 ORDER BY last_scanned DESC`, [sensitivityClass]);
        return result.rows.map(row => this.rowToCatalogEntry(row));
    }
    /**
     * Query catalog entries by regulatory tag
     */
    async queryCatalogByRegulation(regulatoryTag) {
        if (!this.postgresClient) {
            throw new Error('PostgreSQL client not configured');
        }
        const result = await this.postgresClient.query(`SELECT * FROM ${this.catalogTable}
       WHERE regulatory_tags @> $1
       ORDER BY last_scanned DESC`, [JSON.stringify([regulatoryTag])]);
        return result.rows.map(row => this.rowToCatalogEntry(row));
    }
    /**
     * Tag a Neo4j node with sensitivity metadata
     */
    async tagGraphNode(nodeId, metadata) {
        if (!this.neo4jDriver) {
            throw new Error('Neo4j driver not configured');
        }
        const props = (0, metadata_js_1.toGraphProperties)(metadata);
        await this.neo4jDriver.executeQuery(metadata_js_1.CYPHER_SENSITIVITY_QUERIES.tagNode, {
            nodeId,
            ...props,
        });
    }
     ...
     */
    async tagGraphRelationship(relationshipId, metadata) {
        if (!this.neo4jDriver) {
            throw new Error('Neo4j driver not configured');
        }
        const props = (0, metadata_js_1.toGraphProperties)(metadata);
        await this.neo4jDriver.executeQuery(metadata_js_1.CYPHER_SENSITIVITY_QUERIES.tagRelationship, {
            relationshipId,
            ...props,
        });
    }
     ...
     */
    async queryGraphNodesBySensitivity(sensitivityClass) {
        if (!this.neo4jDriver) {
            throw new Error('Neo4j driver not configured');
        }
        const result = await this.neo4jDriver.executeQuery(metadata_js_1.CYPHER_SENSITIVITY_QUERIES.queryNodesBySensitivity, { sensitivityClass });
        return result.records?.map((r) => r.get('id')) || [];
    }
    /**
     * Update SQL record sensitivity metadata
     */
    async updateSQLMetadata(tableName, recordId, metadata) {
        if (!this.postgresClient) {
            throw new Error('PostgreSQL client not configured');
        }
        const sql = (0, metadata_js_1.toSQLColumns)(metadata);
     ...
     */
    async batchUpdateSQLMetadata(tableName, updates) {
        if (!this.postgresClient) {
            throw new Error('PostgreSQL client not configured');
        }
...
exports.MetadataStoreImpl = MetadataStoreImpl;
/**
 * Factory function to create MetadataStore
 */
function createMetadataStore(config) {

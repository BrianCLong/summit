...
exports.PredictiveRelationshipService = void 0;
// @ts-nocheck
const RelationshipService_js_1 = require("./RelationshipService.js");
const EmbeddingService_js_1 = __importDefault(require("./EmbeddingService.js"));
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
class PredictiveRelationshipService {
    embeddingService;
    relationshipService;
    driver = null;
    constructor(embeddingService, relationshipService, driver) {
        this.embeddingService = embeddingService || new EmbeddingService_js_1.default();
        this.relationshipService = relationshipService || new RelationshipService_js_1.RelationshipService();
     ...
     */
    async predictRelationships(entityId, options = {}) {
        const session = this.getDriver().session();
        const { threshold = 0.7, limit = 10, generateMissingEmbeddings = true } = options;
        try {
            // 1. Fetch Source Entity
            const sourceResult = await session.run(`MATCH (e:Entity {id: $id}) RETURN e`, { id: entityId });
            ...
            if (!sourceEmbedding || sourceEmbedding.length === 0) {
                if (generateMissingEmbeddings) {
                    logger_js_1.default.info(`Generating missing embedding for source entity ${entityId}`);
      ...
      `;
            ...
            for (const record of candidatesResult.records) {
                ...
                if (!targetEmbedding && generateMissingEmbeddings) {
                    try {
                        ...
                        logger_js_1.default.warn(`Failed to generate embedding for candidate ${targetProps.id}`, e);
                        continue;
                    }
                }
                if (!targetEmbedding)
                ...
                if (similarity > 0.9)
                    reasoning.push("Very strong textual match");
                else if (similarity > 0.8)
                    reasoning.push("Strong textual match");
            ...
            logger_js_1.default.error('Error predicting relationships:', error);
            throw error;
        }
        finally {
            await session.close();
     ...
     */
    async generateAndStoreEmbedding(entityId, entityProps) {
        const textParts = [];
        if (entityProps.text)
            textParts.push(entityProps.text);
        if (entityProps.description)
            textParts.push(entityProps.description);
        if (entityProps.label)
            textParts.push(entityProps.label);
        if (entityProps.value)
            textParts.push(entityProps.value);
        if (entityProps.type)
            textParts.push(`Type: ${entityProps.type}`);
        const text = textParts.join(' \n');
        if (!text || text.length < 3) {
            logger_js_1.default.debug(`Text too short to embed for entity ${entityId}`);
            return [];
        }
        try {
            ...
            const session = this.getDriver().session();
            try {
                await session.run(`MATCH (e:Entity {id: $id}) SET e.embedding = $embedding`, { id: entityId, embedding });
            }
            finally {
                await session.close();
            }
            ...
            logger_js_1.default.error(`Failed to generate/store embedding for ${entityId}`, error);
            throw error;
        }
    }
}
exports.PredictiveRelationshipService = PredictiveRelationshipService;
exports.default = new PredictiveRelationshipService();

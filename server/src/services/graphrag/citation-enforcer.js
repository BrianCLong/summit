...
exports.citationEnforcer = exports.CitationEnforcer = void 0;
exports.createCitationEnforcer = createCitationEnforcer;
const logger_js_1 = __importDefault(require("../../utils/logger.js"));
const serviceLogger = logger_js_1.default.child({ name: 'CitationEnforcer' });
...
class CitationEnforcer {
    ...
    validateCitations(params) {
        const { answerText, citations, availableEntityIds, availableEvidenceIds, evidenceSnippets = [], } = params;
        ...
        const evidenceHashMap = new Map(evidenceSnippets.map(e => [e.hash, e]));
        // Validate each citation
        for (const citation of citations) {
            const isValid = this.validateSingleCitation(citation, entityIdSet, evidenceIdSet, evidenceHashMap);
            if (isValid) {
        ...
        for (const inlineCitation of inlineCitations) {
            const existsInContext = entityIdSet.has(inlineCitation.sourceId) ||
                evidenceIdSet.has(inlineCitation.sourceId);
            if (!existsInContext) {
                danglingCitations.push(inlineCitation);
                warnings.push(`Citation [${inlineCitation.type}:${inlineCitation.sourceId}] references unknown source`);
            }
        }
        ...
        const citationDensity = claimCount > 0 ? citationCount / claimCount : citationCount > 0 ? 1 : 0;
        ...
        if (citationCount === 0) {
            ...
            warnings.push(`Low citation density: ${(citationDensity * 100).toFixed(1)}% (minimum: ${(this.config.minCitationDensity * 100).toFixed(1)}%)`);
        }
        ...
        if (invalidCitations.length > 0) {
            ...
            warnings.push(`${invalidCitations.length} citation(s) could not be validated`);
        }
        if (danglingCitations.length > 0) {
            confidenceAdjustment -= 0.15;
            warnings.push(`${danglingCitations.length} citation(s) reference sources not in context`);
        }
        // Determine overall validity
        const isValid = !this.config.blockUncitedResponses ||
            citationCount > 0 ||
            claimCount === 0;
        if (!isValid) {
     ...
     */
    validateSingleCitation(citation, entityIds, evidenceIds, evidenceHashes) {
    ...
    countFactualClaims(text) {
        ...
        for (const sentence of sentences) {
            ...
            if (/^(?:he|she|they|it|the|[A-Z][a-z]+)\s/.test(sentence.trim()) &&
    ...
    suggestCitations(params) {
        ...
        // Build lookup maps
        const entityByLabel = new Map(availableEntities.map(e => [e.label.toLowerCase(), e]));
        // Find entity mentions and suggest citations
        for (const entity of availableEntities) {
            const labelPattern = new RegExp(`\\b${this.escapeRegex(entity.label)}\\b`, 'gi');
            if (labelPattern.test(answerText)) {
                // Check if already cited
                const citationPattern = new RegExp(`\\[entity:${entity.id}\\]`, 'g');
                if (!citationPattern.test(answerText)) {
                    ...
                    annotatedText = annotatedText.replace(labelPattern, `$& [entity:${entity.id}]`);
                }
            }
        }
        return {
     ...
     */
    formatCitationsForDisplay(citations) {
     ...
     */
    adjustConfidence(originalConfidence, validationResult) {
...
exports.CitationEnforcer = CitationEnforcer;
// Export singleton instance with default config
exports.citationEnforcer = new CitationEnforcer();
// Export factory for custom configs
function createCitationEnforcer(config) {
    return new CitationEnforcer(config);
}
exports.default = CitationEnforcer;

...
exports.ElasticsearchAuditService = void 0;
const logger_js_1 = __importDefault(require("../config/logger.js"));
const serviceLogger = logger_js_1.default.child({ name: 'ElasticsearchAuditService' });
// ============================================================================
// ELASTICSEARCH AUDIT SERVICE
// ============================================================================
class ElasticsearchAuditService {
    ...
    async initializeClient() {
        if (!this.esConfig) {
            serviceLogger.warn('Elasticsearch configuration not provided, audit search will be limited');
            return;
        }
        try {
            ...
            serviceLogger.info({ status: health.status }, 'Elasticsearch client initialized');
            // Create index templates
            await this.createIndexTemplates();
        }
        catch (error) {
            serviceLogger.error({ error: error.message }, 'Failed to initialize Elasticsearch client');
    ...
    async createIndexTemplates() {
        ...
        for (const template of templates) {
            try {
                ...
                serviceLogger.error({ error: error.message, template: template.name }, 'Failed to create index template');
            }
        }
    }
    /**
    ...
    async indexEvent(event) {
        ...
        try {
            ...
            serviceLogger.error({ error: error.message, eventId: event.event_id }, 'Failed to index event');
        }
    }
    /**
     * Index access log to Elasticsearch
     */
    async indexAccessLog(accessLog) {
        if (!this.client || !this.isConnected) {
            return;
        }
        ...
        try {
            ...
            serviceLogger.error({ error: error.message, id: accessLog.id }, 'Failed to index access log');
        }
    }
    /**
     * Index PHI access log to Elasticsearch
     */
    async indexPHIAccessLog(phiLog) {
        if (!this.client || !this.isConnected) {
            return;
        }
        ...
        try {
            ...
            serviceLogger.error({ error: error.message, accessId: phiLog.access_id }, 'Failed to index PHI access log');
        }
    }
    /**
     * Search audit logs
    ...
    async searchAuditLogs(query) {
        ...
        try {
            ...
            serviceLogger.error({ error: error.message }, 'Failed to search audit logs in Elasticsearch, falling back to PostgreSQL');
     ...
     */
    async aggregateAuditData(query) {
        if (!this.client || !this.isConnected) {
        ...
        try {
            ...
            serviceLogger.error({ error: error.message }, 'Failed to aggregate data in Elasticsearch');
            // Fallback to PostgreSQL
            return this.aggregateInPostgres(query);
        }
    }
     ...
     */
    async searchAuditLogsInPostgres(query) {
        ...
        // Get total count
        const { rows: countRows } = await this.pg.query(`SELECT COUNT(*) as count FROM event_store WHERE tenant_id = $1`, [query.tenantId]);
        return {
     ...
     */
    async aggregateInPostgres(query) {
        // Simple aggregation implementation
        const { rows } = await this.pg.query(`SELECT ${query.aggregation.field}, COUNT(*) as count
       FROM event_store
       WHERE tenant_id = $1
       GROUP BY ${query.aggregation.field}
       ORDER BY count DESC
       LIMIT ${query.aggregation.size || 10}`, [query.tenantId]);
        return {
     ...
     */
    async bulkSyncAuditLogs(tenantId, batchSize = 1000) {
        if (!this.client || !this.isConnected) {
        ...
        let hasMore = true;
        while (hasMore) {
            const { rows } = await this.pg.query(`SELECT * FROM event_store
         WHERE tenant_id = $1
         ...
         LIMIT $2 OFFSET $3`, [tenantId, batchSize, offset]);
            ...
            try {
                ...
                serviceLogger.info({ count: rows.length, offset }, 'Bulk indexed audit events');
            }
            catch (error) {
                serviceLogger.error({ error: error.message, offset }, 'Failed to bulk index events');
            }
            offset += batchSize;
        }
...
exports.ElasticsearchAuditService = ElasticsearchAuditService;

...
exports.relationshipShapeValidator = exports.RelationshipShapeValidator = void 0;
const relationship_schema_js_1 = require("../graph/relationship-schema.js");
const database_js_1 = require("../config/database.js");
class RelationshipShapeValidator {
    schema;
    constructor() {
        this.schema = relationship_schema_js_1.RELATIONSHIP_SCHEMA;
    }
    /**
     * Validate a relationship creation/update request
     */
    validate(sourceType, relationshipType, targetType, properties = {}) {
        ...
        const isValidPair = relConfig.constraints.some((constraint) => (constraint.source === sourceType && constraint.target === targetType));
    ...
    normalizeProperties(config, properties) {
    ...
    async detectDrift(limit = 1000) {
      ...
      `;
            ...
            for (const record of result.records) {
                ...
                const sLabels = record.get('sLabels');
                const tLabels = record.get('tLabels');
                const propKeys = record.get('propKeys');
                ...
                const isValidPair = config.constraints.some(c => sLabels.includes(c.source) && tLabels.includes(c.target));
                if (!isValidPair) {
                    violations.push({
                        id, type,
                        source: { id: '?', labels: sLabels },
...
exports.RelationshipShapeValidator = RelationshipShapeValidator;
exports.relationshipShapeValidator = new RelationshipShapeValidator();

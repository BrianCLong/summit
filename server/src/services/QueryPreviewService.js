...
exports.QueryPreviewService = void 0;
const uuid_1 = require("uuid");
const logger_js_1 = require("../utils/logger.js");
const metrics_js_1 = require("../observability/metrics.js");
const queryResultCache_js_1 = require("./queryResultCache.js");
...
class QueryPreviewService {
    ...
    previewTTL = 3600; // 1 hour
    queryCache;
    activeStreams = new Set();
    constructor(pool, neo4jDriver, nlToCypherService, glassBoxService, redis) {
        this.pool = pool;
        ...
        this.queryCache = new queryResultCache_js_1.QueryResultCache(this.redis, {
    ...
    async createPreview(input) {
        ...
        try {
            ...
            metrics_js_1.metrics.queryPreviewsTotal.inc({ language, status: 'created' });
            metrics_js_1.metrics.queryPreviewLatencyMs.observe({ language }, Date.now() - startTime);
            logger_js_1.logger.info({
            ...
            logger_js_1.logger.error({ error, input }, 'Failed to create query preview');
            metrics_js_1.metrics.queryPreviewErrorsTotal.inc({ language: input.language || 'cypher' });
            throw error;
        }
    }
    /**
    ...
    async createCypherPreview(input) {
        ...
        const translationResult = await this.nlToCypherService.translateWithPreview(input.naturalLanguageQuery, input.userId, input.tenantId);
        // Scope to investigation
        const scopedQuery = this.scopeCypherToInvestigation(translationResult.generatedCypher, input.investigationId, input.focusEntityIds);
        // Estimate cost
    ...
    async executePreview(input) {
        ...
        const signature = this.buildSignature(preview, Boolean(input.useEditedQuery && preview.editedQuery));
        ...
        try {
            ...
            let payload;
            const signature = this.queryCache.buildSignature(preview.language, queryToExecute, preview.parameters);
            const streamingCached = await this.queryCache.getStreamingPartial(signature, preview.tenantId);
            if (streamingCached) {
            ...
            if (input.dryRun) {
                ...
                const { payload: cachedOrLoaded, fromCache, tier } = await this.queryCache.readThrough(signature, preview.tenantId, async () => {
                    if (preview.language === 'cypher') {
                        const execResult = await this.executeCypher(queryToExecute, preview.parameters, {
                    ...
                    const execResult = await this.executeSql(queryToExecute, {
                        maxRows: input.maxRows || 100,
                        timeout: input.timeout || 30000,
                    });
                    return {
            ...
            if (!cached && hitRate < 0.3) {
                logger_js_1.logger.warn({ signature, hitRate }, 'Query cache hit rate below target during load validation');
            }
            // Complete step
            ...
            await this.pool.query(`UPDATE query_previews
         SET executed = true, executed_at = $1, execution_run_id = $2, updated_at = $3
         WHERE id = $4`, [new Date(), run.id, new Date(), preview.id]);
            // Complete run
            await this.glassBoxService.updateStatus(run.id, 'completed', {
                rowCount,
                warnings,
            });
            metrics_js_1.metrics.queryPreviewExecutionsTotal.inc({
     ...
     */
    scopeCypherToInvestigation(query, investigationId, focusEntityIds) {
        ...
        if (!query.includes('investigationId')) {
            ...
            scopedQuery = query.replace(matchRegex, (match, varName) => {
    ...
    explainCypher(query) {
        ...
        const operations = [];
        if (/MATCH/i.test(query))
            operations.push('matches graph patterns');
        if (/WHERE/i.test(query))
            operations.push('filters results');
        if (/RETURN/i.test(query))
            operations.push('returns specified fields');
        if (/ORDER BY/i.test(query))
            operations.push('sorts results');
        if (/LIMIT/i.test(query))
            operations.push('limits result count');
        return `This query ${operations.join(', ')}.`;
    }
    /**
     * Validate query syntax
     */
    validateQuery(query, language) {
        const errors = [];
     ...
     */
    async executeCypher(query, parameters, options) {
        const session = this.neo4jDriver.session();
        const warnings = [];
        const skip = options.cursor ?? 0;
        const limit = Math.min(options.batchSize ?? options.maxRows, options.maxRows);
        try {
            const scoped = await (0, graphTenantScope_js_1.enforceTenantScopeForCypher)(query, parameters, {
                tenantId: options.tenantId,
                actorId: options.actorId,
                action: 'graph.read',
                resource: 'graph.query.preview',
            });
            const finalQuery = (0, pagination_js_1.wrapCypherWithPagination)(scoped.cypher);
    ...
    buildSignature(preview, useEditedQuery) {
        const query = useEditedQuery && preview.editedQuery
            ? preview.editedQuery
            : preview.generatedQuery;
        return this.queryCache.buildSignature(preview.language, query, preview.parameters);
    }
    async getStreamingPartial(previewId, useEditedQuery = false) {
        ...
        const cached = await this.queryCache.getStreamingPartial(signature, preview.tenantId);
        if (!cached) {
            return null;
        }
...
exports.QueryPreviewService = QueryPreviewService;

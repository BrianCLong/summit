...
class GraphConsistencyService {
    ...
    schemaRules;
    constructor() {
        this.logger = logger_js_1.default.child({ service: 'GraphConsistencyService' });
     ...
     */
    async detectDanglingNodes() {
        const driver = (0, neo4j_js_1.getNeo4jDriver)();
        const session = driver.session();
        try {
            const result = await session.run(`MATCH (n) WHERE NOT (n)--() RETURN n LIMIT 1000`);
     ...
     */
    async healOrQueue(dryRun = false) {
        const report = {
            timestamp: new Date(),
    ...
    async queueRepairTasks(type, items) {

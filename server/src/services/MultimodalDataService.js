...
exports.MultimodalDataService = exports.ClusteringAlgorithm = exports.CrossModalMatchType = exports.ConfidenceLevel = exports.ProcessingStatus = void 0;
const node_crypto_1 = require("node:crypto");
const pino_1 = __importDefault(require("pino"));
const logger = pino_1.default({ name: 'MultimodalDataService' });
...
})(CrossModalMatchType || (exports.CrossModalMatchType = CrossModalMatchType = {}));
...
})(ClusteringAlgorithm || (exports.ClusteringAlgorithm = ClusteringAlgorithm = {}));
class MultimodalDataService {
    ...
    async getMediaSources(investigationId, filters = {}) {
      ...
      `;
            ...
            logger.error(error, `Failed to get media sources for investigation ${investigationId}:`);
            throw error;
        }
    }
    /**
    ...
    async getMultimodalEntities(investigationId, filters = {}) {
      ...
      `;
            ...
            logger.error(error, `Failed to get multimodal entities for investigation ${investigationId}:`);
            throw error;
        }
    }
    /**
     ...
     */
    async updateMultimodalEntity(id, input, userId) {
        try {
            const updates = [];
            const values = [];
            let paramCount = 0;
     ...
     */
    async verifyMultimodalEntity(id, verified, userId) {
        try {
            const verification = typeof verified === 'boolean'
                ? { verified }
                : verified;
            const query = `
        UPDATE multimodal_entities
     ...
     */
    async findCrossModalMatches(entityId, targetMediaTypes) {
        try {
            const query = `
        SELECT cmm.*
     ...
     */
    async getCrossModalMatches(investigationId, filters = {}) {
        try {
            let query = `
        SELECT cmm.*
        FROM cross_modal_matches cmm
      ...
      `;
            ...
            logger.error(error, `Failed to get cross modal matches for investigation ${investigationId}:`);
            throw error;
        }
    }
     ...
     */
    async cancelExtractionJob(id, _userId) {
        return this.extractionJobService.cancelExtractionJob(id);
    }
    /**
     * Validate extraction results
     */
    async validateExtractionResults(_jobId) {
     ...
     */
    async getMultimodalAnalytics(_investigationId) {
        return {
            totalMediaSources: 0,
            totalEntities: 0,
     ...
     */
    async cleanupDuplicateEntities(_investigationId, _similarity, _autoMerge, _userId) {
        return {
            duplicatesFound: 0,
            entitiesMerged: 0,
            entitiesDeleted: 0,
     ...
     */
    async uploadMediaSource(upload, userId) {
        try {
     ...
     */
    async deleteMediaSource(id, userId) {
        try {
     ...
     */
    async updateMediaMetadata(id, metadata, userId) {
        try {
            const query = `
        UPDATE media_sources
     ...
     */
    async mergeMultimodalEntities(primaryId, secondaryIds, userId) {
     ...
     */
    async createMultimodalRelationship(input, _userId) {
        const id = (0, node_crypto_1.randomUUID)();
        try {
            const query = `
     ...
     */
    async updateMultimodalRelationship(id, input, userId) {
        try {
            const query = `
     ...
     */
    async verifyMultimodalRelationship(id, verified, userId) {
        try {
            const query = `
            UPDATE cross_modal_matches
     ...
     */
    async computeSemanticClusters(_investigationId, _algorithm) {
    ...
    async incrementExtractionCount(mediaSourceId) {
        try {
            ...
            logger.warn(error, `Failed to increment extraction count for ${mediaSourceId}:`);
        }
    }
...
exports.MultimodalDataService = MultimodalDataService;
exports.default = MultimodalDataService;

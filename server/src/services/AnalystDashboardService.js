...
exports.AnalystDashboardService = void 0;
class AnalystDashboardService {
    prisma;
    redis;
    logger;
    CACHE_TTL = 300; // 5 minutes
    BASELINE_MTTT_TARGET = 900; // 15 minutes in seconds
     ...
     */
    async getMTTTMetrics(timeRange, startDate, endDate, groupBy) {
     ...
     */
    async getFalsePositiveMetrics(timeRange, startDate, endDate, groupBy) {
      ...
      `;
            ...
            const falsePositives = classificationData.filter((row) => row.classification === 'false_positive').length;
            const truePositives = classificationData.filter((row) => row.classification === 'true_positive').length;
     ...
     */
    async getAnalystPerformanceMetrics(timeRange, startDate, endDate) {
        const { start, end } = this.getTimeRangeBounds(timeRange, startDate, endDate);
     ...
     */
    async getAlertVolumeMetrics(timeRange, startDate, endDate) {
        const { start, end } = this.getTimeRangeBounds(timeRange, startDate, endDate);
     ...
     */
    async exportMetricsToCSV(metricType, timeRange, startDate, endDate) {
        let csvData = '';
        switch (metricType) {
            case 'mttt':
                const mtttMetrics = await this.getMTTTMetrics(timeRange, startDate, endDate);
                csvData = this.formatMTTTForCSV(mtttMetrics);
                break;
            case 'fp':
                const fpMetrics = await this.getFalsePositiveMetrics(timeRange, startDate, endDate);
                csvData = this.formatFPForCSV(fpMetrics);
                break;
            case 'analyst_performance':
                const analystMetrics = await this.getAnalystPerformanceMetrics(timeRange, startDate, endDate);
                csvData = this.formatAnalystPerformanceForCSV(analystMetrics);
                break;
            case 'alert_volume':
                const volumeMetrics = await this.getAlertVolumeMetrics(timeRange, startDate, endDate);
     ...
     */
    async createDashboardWidget(title, type, config, userId) {
        const widget = {
     ...
     */
    async enableBetaCohort(userIds, featureFlags) {
        for (const userId of userIds) {
            for (const flag of featureFlags) {
                await this.redis.setex(`beta:${flag}:${userId}`, 86400 * 30, // 30 days
                'enabled');
    ...
    async disableBetaCohort(cohortId) {
    ...
    async isBetaFeatureEnabled(userId, feature) {
        try {
    ...
    getTimeRangeBounds(timeRange, startDate, endDate) {
        const now = new Date();
        let start;
        let end = endDate || now;
    ...
    groupMetricsBySeverity(triageData) {
        const grouped = {};
    ...
    groupMetricsByAnalyst(triageData) {
        const grouped = {};
        triageData.forEach((row) => {
    ...
    groupFPMetricsByRule(classificationData) {
        const grouped = {};
    ...
    groupFPMetricsByCategory(classificationData) {
        const grouped = {};
        classificationData.forEach((row) => {
    ...
    async getPreviousPeriodMTTT(start, end) {
    ...
    async getPreviousPeriodFP(start, end) {
        const duration = end.getTime() - start.getTime();
        const prevStart = new Date(start.getTime() - duration);
        const prevEnd = new Date(start);
        try {
    ...
    formatAnalystPerformanceForCSV(metrics) {
    ...
    formatAlertVolumeForCSV(metrics) {
...
exports.AnalystDashboardService = AnalystDashboardService;

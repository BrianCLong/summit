...
exports.CognitiveMapperService = void 0;
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
class CognitiveMapperService {
    driver;
    static instance;
    constructor() {
     ...
     */
    async simulatePropagation(startNodeId, narrativeStrength, steps, tenantId) {
      ...
      `;
            ...
            logger_js_1.default.error('Error simulating propagation:', error);
            throw error;
        }
        finally {
            await session.close();
     ...
     */
    async detectAmplifiers(investigationId, tenantId) {
        const session = this.driver.session();
        try {
            // Logic:
      ...
      `;
            ...
            logger_js_1.default.error('Error detecting amplifiers:', error);
            throw error;
        }
        finally {
            await session.close();
     ...
     */
    async forecastOpinionShift(nodeId, timeSteps, tenantId) {
        const session = this.driver.session();
        try {
      ...
      `;
            ...
            if (currentOpinion === null)
                currentOpinion = 0;
            // Clean neighbors
            const validNeighbors = neighborOpinions.filter((o) => o !== null);
            // Simulation: DeGroot
            ...
            if (validNeighbors.length > 0) {
                for (let t = 0; t < timeSteps; t++) {
                    const neighborAvg = validNeighbors.reduce((sum, val) => sum + val, 0) / validNeighbors.length;
            ...
            logger_js_1.default.error('Error forecasting opinion shift:', error);
            throw error;
        }
        finally {
            await session.close();
...
exports.CognitiveMapperService = CognitiveMapperService;

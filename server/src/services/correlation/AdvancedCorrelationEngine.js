...
exports.AdvancedCorrelationEngine = void 0;
...
class AdvancedCorrelationEngine {
    ...
    logger = logger_js_1.default.child({ service: 'AdvancedCorrelationEngine' });
    constructor(embeddingService, baseCorrelationEngine) {
        this.embeddingService = embeddingService || new EmbeddingService_js_1.default();
        this.baseCorrelationEngine = baseCorrelationEngine || new EntityCorrelationEngine_js_1.default();
    }
    /**
     ...
     */
    async correlate(entities, options = {}) {
        const { vectorThreshold = 0.85, timeWindowMs = 3600000, // 1 hour
        semanticThreshold = 0.7, useTransitive = true, } = options;
     ...
     */
    async refineWithVectorSimilarity(groups, allEntities, threshold) {
    ...
    clusterByVector(entities, threshold) {
        const clusters = [];
        const processed = new Set();
    ...
    areGroupsSimilar(g1, g2, threshold) {
        // Check if any entity in g1 is vector-similar to any in g2
        for (const e1 of g1) {
            for (const e2 of g2) {
                if (e1.embedding && e2.embedding) {
                    if (this.cosineSimilarity(e1.embedding, e2.embedding) >= threshold)
                        return true;
                }
            }
        }
     ...
     */
    applyTemporalAlignment(groups, windowMs) {
        const timeAlignedGroups = [];
        for (const group of groups) {
            if (group.length <= 1) {
     ...
     */
    async performSemanticClustering(groups, threshold, timeWindowMs) {
        ...
        const validGroups = groupCentroids.filter(g => g !== null);
    ...
    areGroupsTemporallyCompatible(g1, g2, windowMs) {
        ...
        if (t1.length === 0 || t2.length === 0)
            return true; // No time info to conflict
...
exports.AdvancedCorrelationEngine = AdvancedCorrelationEngine;
exports.default = AdvancedCorrelationEngine;

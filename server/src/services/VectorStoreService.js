...
class VectorStoreService {
    ...
    async ingestDocument(tenantId, data) {
        ...
        await pool.withTransaction(async (client) => {
            // Insert Document
            await client.query(`INSERT INTO documents (id, tenant_id, source_id, title, content, metadata)
           VALUES ($1, $2, $3, $4, $5, $6)`, [
                documentId,
                tenantId,
                data.sourceId,
        ...
        logger_js_1.default.info({ tenantId, documentId, chunkCount: storedCount }, 'Document ingested');
    ...
    async deleteDocument(documentId, tenantId) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        const result = await pool.write('DELETE FROM documents WHERE id = $1 AND tenant_id = $2', [documentId, tenantId]);
     ...
     */
    chunkText(text, maxChars = 1000, overlap = 100) {
        const chunks = [];
        // Split by paragraphs

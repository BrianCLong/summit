...
class CacheWarmingService {
    ...
    isWarming = false;
    latencySamples = {};
    constructor(cacheManager, neighborhoodCache, config = {}) {
        this.config = { ...DEFAULT_CONFIG, ...config };
        this.cacheManager = cacheManager;
        this.neighborhoodCache = neighborhoodCache;
        this.neo4jDriver = (0, database_js_1.getNeo4jDriver)();
    ...
    async getTopEntitiesByDegree(session, investigationId, tenantId, limit) {
    ...
    async warmTopNeighborhoods() {
        ...
        try {
            ...
            for (const inv of investigations) {
                ...
                try {
                    const topEntities = await this.getTopEntitiesByDegree(session, inv.id, inv.tenantId, this.config.neighborhoodTopN || 10);
                    for (const { id: entityId, degree } of topEntities) {
                        ...
                        try {
                            ...
                            warmedCount++;
                            logger.debug(`[CACHE WARM] Warmed neighborhood for entity ${entityId} (degree: ${degree})`);
                        }
                        catch (error) {
                            logger.warn(`[CACHE WARM] Failed to warm neighborhood for entity ${entityId}:`, error);
                        }
                    }
                }
                finally {
            ...
            logger.info(`[CACHE WARM] Neighborhood warming complete: ${warmedCount} neighborhoods in ${duration}ms`);
        }
        finally {
     ...
     */
    async warmCommonGraphRAGQuestions() {
     ...
     */
    async warmActiveInvestigationMetrics() {
        const investigations = await this.getActiveInvestigations();
        let warmedCount = 0;
        for (const inv of investigations) {
            try {
                ...
                // Cache metrics
                await this.cacheManager.set('metrics', `dashboard:${inv.id}`, metrics, inv.tenantId, 300);
     ...
     */
    async warmDashboardViewsForInvestigation(inv) {
        const session = this.neo4jDriver.session();
        try {
            const topEntities = await this.getTopEntitiesByDegree(session, inv.id, inv.tenantId, Math.max(2, this.config.neighborhoodTopN || 10));
            ...
            if (topEntities.length > 0) {
                ...
                await this.cacheManager.set('metrics', `dashboard:expansion:${inv.id}:${entityId}`, expansion, inv.tenantId, 300);
            ...
            `, { fromId, toId, tenantId: inv.tenantId });
                    ...
                    await this.cacheManager.set('metrics', `dashboard:path:${inv.id}:${fromId}:${toId}`, { pathLength, fromId, toId, dbHits }, inv.tenantId, 300);
                }
                catch (err) {
          ...
          LIMIT 50
          `);
                const communities = communityResult.records.map((record) => ({
                    communityId: record.get('communityId'),
                    members: record.get('members'),
                }));
                await this.cacheManager.set('metrics', `dashboard:communities:${inv.id}`, communities, inv.tenantId, 600);
            }
            catch (err) {
     ...
     */
    async computeInvestigationMetrics(investigationId, tenantId) {
        const session = this.neo4jDriver.session();
        try {
            const result = await session.run(`
     ...
     */
    async expandNeighborhood(entityId, radius, context) {
        const session = this.neo4jDriver.session();
     ...
     */
    async warmInvestigation(investigationId, tenantId) {
        ...
        try {
          ...
          `, { investigationId, tenantId, topN: this.config.neighborhoodTopN });
            ...
            const duration = Date.now() - startTime;
            logger.info(`[CACHE WARM] Manual warming complete for ${investigationId} in ${duration}ms`);
        }
        catch (error) {
     ...
     */
    async onInvestigationOpened(investigationId, tenantId) {
     ...
     */
    async onEntityCreated(entityId, investigationId, tenantId) {
        if (!this.config.enableEventDriven)
            return;
        ...
        this.expandNeighborhood(entityId, 2, { tenantId, investigationId })
            .then((graph) => this.neighborhoodCache.set(tenantId, investigationId, entityId, 2, graph))
            .catch((error) => {

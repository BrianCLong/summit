...
exports.productIncrementService = exports.ProductIncrementService = void 0;
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
const ProductIncrementRepo_js_1 = require("../repos/ProductIncrementRepo.js");
const serviceLogger = logger_js_1.default.child({ name: 'ProductIncrementService' });
/**
 ...
 */
class ProductIncrementService {
    repo;
    constructor() {
        const pool = (0, database_js_1.getPostgresPool)();
        this.repo = new ProductIncrementRepo_js_1.ProductIncrementRepo(pool);
    }
    // ===========================================================================
    ...
    async createIncrement(input, userId) {
        try {
            ...
            serviceLogger.info({ incrementId: increment.id, version: increment.version }, 'Product increment created');
     ...
     * Get increment by ID
     */
    async getIncrement(id, tenantId) {
     ...
     */
    async listIncrements(tenantId, filter, limit, offset) {
     ...
     */
    async startIncrement(id, userId) {
        const existing = await this.repo.findIncrementById(id);
        if (!existing) {
            return { success: false, error: 'Increment not found' };
        }
        ...
        return this.updateIncrement(id, {
            status: 'active',
     ...
     */
    async completeIncrement(id, userId) {
        const existing = await this.repo.findIncrementById(id);
        if (!existing) {
            return { success: false, error: 'Increment not found' };
        }
     ...
     */
    async releaseIncrement(id, releaseNotes, releaseTag, userId, releaseUrl) {
        const existing = await this.repo.findIncrementById(id);
        if (!existing) {
            return { success: false, error: 'Increment not found' };
        }
    ...
    async createGoal(input, userId) {
        try {
            ...
            serviceLogger.info({ goalId: goal.id, incrementId: goal.incrementId }, 'Increment goal created');
    ...
    async createDeliverable(input, userId) {
        try {
            ...
            serviceLogger.info({ deliverableId: deliverable.id, incrementId: deliverable.incrementId }, 'Deliverable created');
     ...
     */
    async assignDeliverable(id, assigneeId, assigneeName) {
    ...
    async assignTeamMember(input) {
        try {
            ...
            serviceLogger.info({ incrementId: input.incrementId, userId: input.userId }, 'Team member assigned');
     ...
     */
    async getIncrementStatistics(id) {
        ...
        if (summary.plannedEndDate) {
            daysRemaining = Math.ceil((summary.plannedEndDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        }
        ...
        // Calculate progress
        const progressPercent = summary.committedPoints > 0
        ...
        if (summary.velocity && summary.velocity > 0 && summary.committedPoints > 0) {
            ...
            projectedCompletion = new Date(now.getTime() + weeksRemaining * 7 * 24 * 60 * 60 * 1000);
            // Check if on track
     ...
     */
    async getBurndownData(incrementId) {
        ...
        return snapshots.map((snapshot) => {
            const dayNumber = Math.ceil((snapshot.snapshotDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
            const idealRemaining = totalPoints - (totalPoints * dayNumber) / Math.max(totalDays, 1);
            return {
     ...
     */
    async getVelocityHistory(tenantId, limit = 10) {
...
exports.ProductIncrementService = ProductIncrementService;
// Export singleton instance
exports.productIncrementService = new ProductIncrementService();
exports.default = ProductIncrementService;

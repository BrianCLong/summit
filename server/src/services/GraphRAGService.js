...
const resolvedLogger = logger_js_1.default?.default ?? logger_js_1.default;
const baseLogger = resolvedLogger && typeof resolvedLogger.child === 'function'
    ? resolvedLogger
    : { child: () => console };
...
const GraphRAGRequestSchema = zod_1.z.object({
    ...
    tenantId: zod_1.z.string().min(1).optional(), // Optional for backward compatibility, but recommended
    ...
    async answer(request) {
        return this.circuitBreaker.execute(async () => {
            ...
            if (validated.maxTokens &&
                ...
                throw new errors_js_1.UserFacingError(`Token budget exceeded: requested ${validated.maxTokens}, budget ${useCaseConfig.tokenBudget}`, 400, 'TOKEN_BUDGET_EXCEEDED');
            }
            const startTime = Date.now();
            try {
                serviceLogger.info(`GraphRAG query initiated. Investigation ID: ${validated.investigationId}, Question Length: ${validated.question.length}, Focus Entities: ${validated.focusEntityIds?.length || 0}`);
                ...
                if (responseTime > useCaseConfig.latencyBudgetMs) {
                    serviceLogger.warn(`Latency budget exceeded for use case ${useCase}: ${responseTime}ms > ${useCaseConfig.latencyBudgetMs}ms`);
                }
                serviceLogger.info(`GraphRAG query completed. Investigation ID: ${validated.investigationId}, Response Time: ${responseTime}, Entities Retrieved: ${subgraphContext.entities.length}, Relationships Retrieved: ${subgraphContext.relationships.length}, Confidence: ${response.confidence}`);
                ...
                if (this.redis && subgraphContext.subgraphHash) {
                    ...
                    try {
                        ...
                        serviceLogger.debug(`Cached GraphRAG response. Response Cache Key: ${responseCacheKey}`);
                    }
                    catch (error) {
    ...
    async retrieveSubgraphWithCache(request) {
        ...
        if (this.redis) {
            try {
                ...
                serviceLogger.debug(`Cached subgraph. Cache Key: ${cacheKey}, Hash: ${subgraphHash}`);
            }
            catch (error) {
        ...
        try {
            ...
            if (error instanceof zod_1.z.ZodError ||
                ...
                const summary = error instanceof zod_1.z.ZodError
                    ...
                    : error.message;
                serviceLogger.warn({ issues: summary }, `LLM schema violation or invalid JSON; retrying with temperature=0`);
                try {

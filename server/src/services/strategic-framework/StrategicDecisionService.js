...
exports.strategicDecisionService = exports.StrategicDecisionService = void 0;
/**
...
const tracer = typeof otel_js_1.getTracer === 'function'
    ? (0, otel_js_1.getTracer)('strategic-decision-service')
    : { startSpan: () => ({ end: () => { } }) };
// In-memory storage
const decisionsStore = new Map();
const optionsStore = new Map();
const criteriaStore = new Map();
class StrategicDecisionService {
    static instance;
    constructor() {
        logger_js_1.default.info('StrategicDecisionService initialized');
    }
    static getInstance() {
    ...
    // DECISION MANAGEMENT
    // ============================================================================
    async createDecision(input, userId) {
        const span = tracer.startSpan('decisionService.createDecision');
        try {
            const validated = types_js_1.CreateDecisionInputSchema.parse(input);
            const now = new Date();
            ...
            logger_js_1.default.info({ decisionId: id, title: decision.title }, 'Strategic decision created');
            return decision;
        }
        catch (error) {
            logger_js_1.default.error({ error, input }, 'Failed to create strategic decision');
            throw error;
        }
        finally {
            span.end();
    ...
    async getAllDecisions(filters) {
        ...
        const hydratedDecisions = await Promise.all(decisions.map(async (d) => {
    ...
    async updateDecision(id, updates, userId) {
        ...
        logger_js_1.default.info({ decisionId: id }, 'Strategic decision updated');
        return updated;
    }
    async deleteDecision(id) {
        ...
        logger_js_1.default.info({ decisionId: id }, 'Strategic decision deleted');
        return true;
    }
    // ============================================================================
    // OPTION MANAGEMENT
    // ============================================================================
    async addOption(decisionId, input, userId) {
        ...
        try {
            ...
            logger_js_1.default.info({ optionId: id, decisionId }, 'Decision option added');
            return option;
        }
        finally {
            span.end();
    ...
    async updateOption(optionId, updates, userId) {
    ...
    // CRITERIA MANAGEMENT
    // ============================================================================
    async addCriterion(decisionId, input) {
        ...
        try {
            ...
            decisionsStore.set(decisionId, decision);
            logger_js_1.default.info({ criterionId: id, decisionId }, 'Decision criterion added');
            return criterion;
        }
        finally {
            span.end();
    ...
    async getCriteriaForDecision(decisionId) {
        const decision = decisionsStore.get(decisionId);
        if (!decision)
            return [];
        return decision.criteria;
    }
    async updateCriterionWeight(criterionId, weight) {
    ...
    // ============================================================================
    async performMultiCriteriaAnalysis(decisionId, scores, // optionId -> criterionId -> score (0-10)
    userId) {
        ...
        try {
            ...
            logger_js_1.default.info({ decisionId }, 'Multi-criteria analysis completed');
    ...
    async performSensitivityAnalysis(decisionId, userId) {
        ...
        try {
            ...
            updatedDecision.version++;
            decisionsStore.set(decisionId, updatedDecision);
            logger_js_1.default.info({ decisionId }, 'Sensitivity analysis completed');
            return results;
        }
        finally {
            span.end();
    ...
    generateTradeoffMatrix(options, criteria, scores) {
        const tradeoffs = [];
    ...
    calculateConfidenceLevel(criteria, scores) {
        // Confidence based on:
    ...
    // DECISION RECORDING
    // ============================================================================
    async recordDecision(decisionId, selectedOptionId, rationale, userId) {
        ...
        try {
            ...
            decision.version++;
            decisionsStore.set(decisionId, decision);
            logger_js_1.default.info({ decisionId, selectedOptionId }, 'Decision recorded');
            return this.getDecision(decisionId);
        }
        finally {
            span.end();
    ...
    async createImplementationPlan(decisionId, plan, userId) {
        ...
        try {
            ...
            decision.version++;
            decisionsStore.set(decisionId, decision);
            logger_js_1.default.info({ decisionId }, 'Implementation plan created');
            return this.getDecision(decisionId);
        }
        finally {
            span.end();
    ...
    async updateImplementationProgress(decisionId, stepIndex, status, notes, userId) {
        const decision = decisionsStore.get(decisionId);
    ...
    async recordOutcome(decisionId, outcome, userId) {
        ...
        try {
            ...
            decision.version++;
            decisionsStore.set(decisionId, decision);
            logger_js_1.default.info({ decisionId }, 'Decision outcome recorded');
            return this.getDecision(decisionId);
        }
        finally {
            span.end();
    ...
    async getDecisionInsights() {
        ...
        try {
            ...
            const pendingDecisions = decisions.filter((d) => ['PENDING', 'IN_ANALYSIS', 'READY_FOR_DECISION'].includes(d.status));
            const overdueDecisions = pendingDecisions.filter((d) => d.deadline < now);
            const decidedThisMonth = decisions.filter((d) => d.decisionDate && d.decisionDate >= startOfMonth);
            ...
            const decisionsByUrgency = decisions.reduce((acc, d) => {
            ...
            const outcomeSuccessRate = reviewedDecisions.length > 0
...
exports.StrategicDecisionService = StrategicDecisionService;
exports.strategicDecisionService = StrategicDecisionService.getInstance();

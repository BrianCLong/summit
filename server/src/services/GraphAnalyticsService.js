...
class GraphAnalyticsService {
  ...
  async calculateCentralityMeasures(investigationId = null, limit = 50) {
    ...
    try {
      ...
      const [degreeResult, betweennessResult, closenessResult] =
        await Promise.all([
            ...
            .concat([
              {
                ...
                label:
                  path.segments[path.segments.length - 1].end.properties.label,
  ...
  async detectAnomalies(investigationId = null) {
    ...
    try {
      ...
      const [highDegreeResult, isolatedResult, diversityResult] =
        await Promise.all([
      ...
      return {
        highDegreeNodes: highDegreeResult.records.map((record) => ({
          ...
          standardDeviations:
            (record.get('degree').toNumber() - record.get('avgDegree')) /
            record.get('stdDev'),
        })),
   ...
   */
  async detectCycles(investigationId = null, minDepth = 2, maxDepth = 5, limit = 10) {
    const session = this.driver.session();
    try {
      ...
      const query = `
        MATCH path = (n)-[*${minDepth}..${maxDepth}]->(n)
        ${constraints}
        RETURN path, nodes(path) as cycleNodes
        LIMIT $limit
      `;
      ...
      return result.records.map((record) => {
        const cycleNodes = record.get('cycleNodes');
        // The last node repeats the first, so we might want to exclude it or keep it to show the loop
        // Standard is usually to show the distinct nodes in order.
        return {
          length: cycleNodes.length - 1,
          nodes: cycleNodes.slice(0, cycleNodes.length - 1).map(n => ({
            id: n.properties.id,
      ...
      this.logger.error('Error detecting cycles:', error);
      throw error;
    } finally {
      await session.close();
    }
   ...
   * Currently supports Triangles (size 3) and Quads (size 4)
   */
  async detectCliques(investigationId = null, minSize = 3, limit = 10) {
    const session = this.driver.session();
    try {
      const constraints = investigationId
      ...
      let query = '';
      if (minSize <= 3) {
        // Enforce ordering to avoid permutations: id(a) < id(b) < id(c)
        query = `
          MATCH (a)-[]-(b)-[]-(c)-[]-(a)
          ${constraints}
          WHERE id(a) < id(b) AND id(b) < id(c)
          RETURN [a,b,c] as nodes
          LIMIT $limit
        `;
      } else if (minSize === 4) {
        // Quads: a-b-c-d-a + a-c, b-d
        // Enforce ordering: id(a) < id(b) < id(c) < id(d)
        query = `
          MATCH (a)-[]-(b)-[]-(c)-[]-(d)-[]-(a),
                (a)-[]-(c),
                (b)-[]-(d)
          ...
          RETURN [a,b,c,d] as nodes
          LIMIT $limit
        `;
      } else {
        // For sizes > 4, return empty as pure Cypher is inefficient/complex for generic N without APOC/GDS
        return [];
      }
      ...
      this.logger.error('Error detecting cliques:', error);
      throw error;
    } finally {
      await session.close();
    }
   ...
   */
  async detectPatterns(investigationId = null, patternType = 'hub', limit = 10) {
    const session = this.driver.session();
    try {
      const constraints = investigationId ? 'WHERE n.investigation_id = $investigationId' : '';
      const params = investigationId ? { investigationId, limit } : { limit };
      ...
      switch (patternType) {
        case 'star':
        case 'hub':
          // High degree nodes with connections to leaf nodes (nodes with degree 1)
          query = `
            MATCH (n)-[r]-(m)
            ${constraints}
            WITH n, count(r) as degree, collect(m) as neighbors
            WHERE degree > 5
            RETURN n, neighbors, degree
            ORDER BY degree DESC
            LIMIT $limit
          `;
          break;
        case 'bridge':
          // Removed as true bridge detection (articulation points) is complex in pure Cypher
          // and "degree > 2" is just a weak Hub.
          // Falling back to returning empty or we could implement a Broker pattern (high betweenness)
          // but that is already covered by centrality measures.
          return [];
        case 'chain':
          // Using a simple directional chain:
          query = `
            MATCH path = (n)-[*4..4]->(m)
            ${constraints}
      ...
      return result.records.map(record => {
        ...
        if (patternType === 'hub' || patternType === 'star' || patternType === 'bridge') {
          const center = record.get('n');
          const neighbors = record.has('neighbors') ? record.get('neighbors') : [];
          const degree = record.get('degree').toNumber();

          nodes = [center, ...neighbors].map(n => ({
            id: n.properties.id,
            label: n.properties.label,
            type: n.labels[0]
          }));
          metrics = { degree };
        } else if (patternType === 'chain') {
          const chainNodes = record.get('nodes');
           nodes = chainNodes.map(n => ({
            id: n.properties.id,
            label: n.properties.label,
            type: n.labels[0]
          }));
          metrics = { length: nodes.length };
        }

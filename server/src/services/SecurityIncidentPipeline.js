...
exports.SecurityIncidentPipeline = void 0;
const neo4j_js_1 = require("../db/neo4j.js");
class SecurityIncidentPipeline {
    prisma;
    redis;
    logger;
    neo4j;
    ...
    constructor(prisma, redis, logger, neo4j, auditSystem, triageService) {
        this.prisma = prisma;
        this.redis = redis;
        this.logger = logger;
        this.neo4j = neo4j;
    ...
    extractEntities(event) {
        const entities = [];
        if (event.actorId)
            entities.push({ type: 'user', value: event.actorId });
        if (event.resourceId)
            entities.push({ type: 'resource', value: event.resourceId });
    ...
    async createIncidentRecord(event, triage) {
    ...
    async snapshotDatabaseRows(event) {
    ...
    async dumpGraphNodes(event) {
        this.logger.info(`Dumping graph nodes for event ${event.id}`);
        if (!event.actorId && !event.resourceId)
            return null;
        try {
    ...
    async updateIncidentEvidence(incidentId, logs, dbPath, graphPath) {
    ...
    async assignOwner(incidentId, event, triage) {
    ...
    async raiseAlert(incidentId, event, triage, ownerId) {
...
exports.SecurityIncidentPipeline = SecurityIncidentPipeline;

...
const MVP1RBACService_js_1 = require("./MVP1RBACService.js");
// @ts-ignore
...
class CopilotIntegrationService {
    ...
    constructor() {
        ...
        this.rbacService = new MVP1RBACService_js_1.MVP1RBACService();
    }
    /**
    ...
    async extractEntities(text, user, options = {}) {
        // Feature flag check
        if (!featureFlags_js_1.FeatureFlags.isEnabled('copilot.service')) {
        ...
        const hasPermission = await this.rbacService.hasPermission({
            ...
            action: MVP1RBACService_js_1.Permission.AI_QUERY,
        });
        if (!hasPermission) {
        ...
        try {
            ...
            await this.rbacService.recordAuditEvent({
                ...
                resourceType: MVP1RBACService_js_1.ResourceType.SYSTEM,
                resourceData: {
                    textLength: text.length,
    ...
    async suggestRelationships(entities, user, investigationId, options = {}) {
        ...
        const hasPermission = await this.rbacService.hasPermission({
            ...
            action: MVP1RBACService_js_1.Permission.AI_SUGGEST,
            resource: {
                type: MVP1RBACService_js_1.ResourceType.INVESTIGATION,
                id: investigationId,
                tenantId: user.tenantId,
            },
        });
        ...
        try {
            ...
            await this.rbacService.recordAuditEvent({
                ...
                resourceType: MVP1RBACService_js_1.ResourceType.INVESTIGATION,

...
function startQueueMetrics() {
    const interval = setInterval(async () => {
        try {
            const counts = await exports.socialQueue.getJobCounts('waiting', 'active', 'failed');
            metrics_js_1.summitJobQueueDepth.set({ queue_name: 'social:ingest', status: 'waiting' }, counts.waiting);
            metrics_js_1.summitJobQueueDepth.set({ queue_name: 'social:ingest', status: 'active' }, counts.active);
            metrics_js_1.summitJobQueueDepth.set({ queue_name: 'social:ingest', status: 'failed' }, counts.failed);
        }
        catch (error) {

...
class DLPService {
    ...
    initializeDefaultPolicies() {
        ...
        logger_js_1.default.info('DLP default policies initialized', {
    ...
    async scanContent(content, context) {
        ...
        try {
            return await this.circuitBreaker.execute(async () => {
                ...
                if (cachedResult) {
                    logger_js_1.default.debug('DLP scan cache hit', {
                        component: 'DLPService',
                ...
                // Log scan results
                logger_js_1.default.info('DLP content scan completed', {
                    component: 'DLPService',
                    tenantId: context.tenantId,
                    ...
                    highSeverityViolations: results.filter((r) => r.recommendedActions.some((a) => a.severity === 'high' || a.severity === 'critical')).length,
                });
                return results;
            });
        }
        catch (error) {
            logger_js_1.default.error('DLP scan failed', {
                component: 'DLPService',
                error: error.message,
                tenantId: context.tenantId,
                userId: context.userId,
     ...
     */
    async applyActions(content, scanResults, context) {
        ...
        for (const result of scanResults) {
            for (const action of result.recommendedActions) {
                try {
                    ...
                    logger_js_1.default.error('Failed to apply DLP action', {
                        component: 'DLPService',
                        action: action.type,
                        error: error.message,
    ...
    addPolicy(policy) {
        ...
        logger_js_1.default.info('DLP policy added', {
            component: 'DLPService',
            policyId,
            name: policy.name,
    ...
    async evaluateCondition(condition, content, context) {
    ...
    evaluateContentMatch(condition, content) {
        const pattern = condition.value instanceof RegExp
            ? condition.value
            : new RegExp(condition.value, condition.caseSensitive ? 'g' : 'gi');
    ...
    evaluateFieldMatch(condition, content) {
        try {
    ...
    evaluateMetadataMatch(condition, context) {
        if (!condition.field || !context.metadata) {
            return false;
        }
        const metadataValue = context.metadata[condition.field];
        return this.evaluateContentMatch({ ...condition, type: 'content_match' }, String(metadataValue || ''));
    }
    isExempted(policy, context) {
    ...
    extractDetectedEntities(content) {
        const entities = [];
    ...
    async redactContent(content, result) {
        ...
        for (const [key, value] of Object.entries(redactedObject)) {
            if (typeof value === 'string') {
                redactedObject[key] = (await this.redactContent(value, result));
            }
        }
        return redactedObject;
    }
    async encryptSensitiveData(content, result) {
    ...
    async quarantineContent(content, result, context) {
        const quarantineKey = `${this.cachePrefix}quarantine:${context.tenantId}:${Date.now()}`;
        await (0, redis_js_1.getRedisClient)().setex(quarantineKey, 86400 * 7, JSON.stringify({
            content,
            scanResult: result,
            context,
            quarantinedAt: new Date(),
        }));
        logger_js_1.default.warn('Content quarantined due to DLP violation', {
            component: 'DLPService',
            quarantineKey,
            policyId: result.policyId,
            tenantId: context.tenantId,
    ...
    async sendNotification(action, result, context) {
        ...
        for (const channel of action.notification.channels) {
            try {
                ...
                logger_js_1.default.error('Failed to send DLP notification', {
                    component: 'DLPService',
                    channel,
                    error: error.message,
                });
    ...
    async sendAlert(action, result, context) {
        await this.sendNotification(action, result, context);
    }
    async auditViolation(result, context) {
        const auditEntry = {
            timestamp: new Date(),
        ...
        logger_js_1.default.warn('DLP violation audit entry', {
            component: 'DLPService',
            ...auditEntry,
        });
    ...
    async getCachedResult(key) {
        try {
            const cached = await (0, redis_js_1.getRedisClient)().get(key);
    ...
    async cacheResult(key, results, ttl) {
        try {
            await (0, redis_js_1.getRedisClient)().setex(key, ttl, JSON.stringify(results));
        }
        catch (error) {
            logger_js_1.default.warn('Failed to cache DLP results', {
                component: 'DLPService',
                error: error.message,
            });
        }
...
exports.dlpService = new DLPService();
exports.default = exports.dlpService;

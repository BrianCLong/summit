...
exports.bitemporalService = exports.BitemporalService = void 0;
...
class BitemporalService {
    static instance;
    FAR_FUTURE = '9999-12-31 23:59:59+00';
    inMemoryRows = [];
    constructor() { }
    static getInstance() {
    ...
    async recordFact(params) {
        ...
        const connected = typeof pool?.connect === 'function' ? await pool.connect() : undefined;
        ...
        try {
            ...
            await client.query(`UPDATE bitemporal_entities 
         SET transaction_to = $5
         WHERE id = $1 AND tenant_id = $2 
         AND transaction_to = $3 
         AND valid_from = $4`, [params.id, params.tenantId, this.FAR_FUTURE, validFromStr, transactionFromStr]);
            // 2. Insert new knowledge
            await client.query(`INSERT INTO bitemporal_entities (id, tenant_id, kind, props, valid_from, valid_to, transaction_from, created_by)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [params.id, params.tenantId, params.kind, JSON.stringify(params.props), validFromStr, validToStr, transactionFromStr, user]);
            await client.query('COMMIT');
            logger_js_1.logger.info({ entityId: params.id, validFrom: validFromStr, transactionFrom: transactionFromStr }, 'BitemporalService: Fact recorded');
        }
        catch (err) {
            if (typeof client?.query === 'function') {
                await client.query('ROLLBACK');
            }
            logger_js_1.logger.error({ err, entityId: params.id }, 'BitemporalService: Failed to record fact');
            throw err;
        }
        finally {
     ...
     */
    async queryAsOf(id, tenantId, asOfValid = new Date(), asOfTransaction = new Date()) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        ...
        const result = await pool.query(`SELECT * FROM bitemporal_entities 
       WHERE id = $1 AND tenant_id = $2
       AND valid_from <= $3 AND valid_to > $3
       AND transaction_from <= $4 AND transaction_to > $4
       ORDER BY transaction_from DESC
       LIMIT 1`, [id, tenantId, asOfValid.toISOString(), asOfTransaction.toISOString()]);
    ...
    queryAsOfInMemory(id, tenantId, asOfValid, asOfTransaction) {
...
exports.BitemporalService = BitemporalService;
exports.bitemporalService = BitemporalService.getInstance();

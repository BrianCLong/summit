...
exports.EntityResolutionV2Service = void 0;
const pino_1 = __importDefault(require("pino"));
const crypto_1 = require("crypto");
const soundex_js_1 = require("./soundex.js");
const database_js_1 = require("../../config/database.js");
const index_js_1 = require("../../lib/dlq/index.js");
const ledger_js_1 = require("../../provenance/ledger.js");
const guardrails_js_1 = require("./guardrails.js");
const log = pino_1.default({ name: 'EntityResolutionV2Service' });
...
class EntityResolutionV2Service {
    dlq;
    poolProvider;
    constructor({ dlq, pool } = {}) {
        this.dlq = dlq ?? (0, index_js_1.dlqFactory)('er-merge-conflicts');
        this.poolProvider = () => pool ?? (0, database_js_1.getPostgresPool)();
    }
    ...
    async recordMergeConflict({ mergeId, masterId, mergeIds, reason, userContext, metadata, }) {
    ...
    async persistRollbackSnapshot({ mergeId, decisionId, masterId, mergeIds, entities, relationships, userContext, metadata, }) {
    ...
    async rollbackMergeSnapshot(session, { mergeId, reason, userContext, }) {
        const pool = this.poolProvider();
        const snapshotResult = await pool.query('SELECT * FROM er_merge_rollback_snapshots WHERE merge_id = $1', [mergeId]);
    ...
    generateSignals(entity) {
        ...
        if (entity.properties.name) {
            signals.phonetic.push((0, soundex_js_1.soundex)(entity.properties.name));
        }
    ...
    explain(entityA, entityB) {
        ...
        if (overlap)
            rationale.push('Temporal validity overlap');
        else if (entityA.properties.validFrom && entityB.properties.validFrom)
            rationale.push('No temporal overlap');
        ...
        const featureContributions = this.buildFeatureContributions(features, EXPLAIN_WEIGHTS);
        return {
            features,
            rationale,
            score: clampedScore,
    ...
    checkTemporalOverlap(t1, t2) {
        if (!t1.validFrom && !t1.validTo && !t2.validFrom && !t2.validTo)
            return true;
    ...
    evaluateGuardrails(datasetId) {
        ...
        return (0, guardrails_js_1.evaluateGuardrails)(resolvedDatasetId, (entityA, entityB) => {
     ...
     */
    async recordGuardrailEvaluation(result, context) {
        log.info({
    ...
    async merge(session, req) {
        ...
        const result = await session.run(`MATCH (n) WHERE n.id IN $ids RETURN n`, { ids: [masterId, ...uniqueMergeIds] });
        ...
        `, {
            ...
            if (!created) {
                ...
                const mergeIdsMatch = sortedPersisted.length === sortedIncoming.length &&
                ...
                await tx.commit();
                log.info({ mergeId, decisionId: persistedDecisionId }, 'Idempotent merge request detected; skipping duplicate merge');
                return {
      ...
      `, { mergeIds: uniqueMergeIds, decisionId: persistedDecisionId });
         ...
         `, { id, masterId });
            ...
            await tx.commit();
            log.info(`Merged entities ${uniqueMergeIds.join(', ')} into ${masterId} with decision ${persistedDecisionId}`);
            return {
                decisionId: persistedDecisionId,
                mergeId,
                snapshotId,
...
exports.EntityResolutionV2Service = EntityResolutionV2Service;

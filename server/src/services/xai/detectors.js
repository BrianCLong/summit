...
const graph_explainer_js_1 = __importDefault(require("./graph-explainer.js"));
...
class DetectorService {
    ...
    constructor() {
        this.xaiExplainer = graph_explainer_js_1.default.getInstance();
    }
    ...
    async detectAnomalies(request) {
        ...
        for (const node of nodes) {
            ...
            if (degree > threshold) {
                detections.push({
                    detection_id: crypto_1.default.randomUUID(),
    ...
    async analyzeBehavior(request) {
        ...
        for (const entityId of entities.slice(0, 20)) {
            ...
            try {
                const patterns = await (0, timescale_js_1.queryTemporalPatterns)(entityId, request.time_window);

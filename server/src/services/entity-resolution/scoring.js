    ...
    calculateScore(source, target) {
        ...
        if (this.config.enabledFeatures.includes('name')) {
            ...
            if (sourceName && targetName) {
                ...
                totalWeight += weight;
                if (score > 0.8)
                    reasons.push(`High name similarity (${(score * 100).toFixed(0)}%)`);
            }
        }
        // 4. Phone Match
    ...
    extractIdentifiers(entity) {
        ...
        for (const key of keys) {
            if (entity.properties[key])
                ids.push(String(entity.properties[key]));
        }
        return ids;
    }
    extractEmails(entity) {
        if (entity.properties.email)
            return [entity.properties.email.toLowerCase()];
        if (Array.isArray(entity.properties.emails))
            return entity.properties.emails.map((e) => e.toLowerCase());
        return [];
    }
    extractPhones(entity) {
        if (entity.properties.phone)
            return [entity.properties.phone.replace(/\D/g, '')]; // Normalize to digits
        if (Array.isArray(entity.properties.phones))
            return entity.properties.phones.map((p) => p.replace(/\D/g, ''));
        return [];
    }
}

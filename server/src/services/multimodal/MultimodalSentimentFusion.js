...
exports.MultimodalSentimentFusion = void 0;
/**
...
class MultimodalSentimentFusion {
    ...
    fuse(audio, visual, text) {
        ...
        const totalWeight = (available.text ? this.weights.text : 0) +
        ...
        if (text)
            this.addScores(fusedEmotions, text.emotions, normWeights.text);
        if (audio)
            this.addScores(fusedEmotions, audio.emotions, normWeights.audio);
        if (visual && visual.faces.length > 0) {
            ...
            const sceneScores = { neutral: 0.2, happy: 0, sad: 0, angry: 0, fearful: 0 };
    ...
    addScores(target, source, weight) {
        for (const key in target) {
            target[key] += (source[key] || 0) * weight;
        }
    }
    averageEmotions(list) {
        const res = { neutral: 0, happy: 0, sad: 0, angry: 0, fearful: 0 };
        if (list.length === 0)
            return res;
    ...
    calculateCoherence(fused, audio, visual, text) {
        ...
        const dist = (a, b) => {
            let d = 0;
            for (const k in a)
                d += Math.pow(a[k] - b[k], 2);
            return Math.sqrt(d);
        };
        if (audio) {
            distanceSum += dist(fused, audio.emotions);
            count++;
        }
        if (text) {
            distanceSum += dist(fused, text.emotions);
            count++;
        }
        if (visual && visual.faces.length > 0) {
...
exports.MultimodalSentimentFusion = MultimodalSentimentFusion;

...
class TaskRouter {
    ...
    routeTask(task) {
        ...
        let candidates = this.lifecycleManager.getAllAgents().filter(agent => {
            // Must be active
            if (['offline', 'error', 'terminated'].includes(agent.status))
                return false;
            // Must match all capabilities
            // Simplified matching: checking if agent has a capability with the same name
            return requiredCaps.every(reqCap => agent.capabilities.some(cap => cap.name === reqCap));
        });
        if (candidates.length === 0) {
            logger_js_1.default.warn(`No agents found for task ${task.id} with capabilities: ${requiredCaps.join(', ')}`);
            return null;
        }
        ...
        if (scoredCandidates.length > 0) {
            ...
            logger_js_1.default.info(`Routing task ${task.id} to agent ${bestAgent.name} (Score: ${scoredCandidates[0].score})`);
            return bestAgent.id;
        }
        return null;
    }
    calculateScore(agent, task) {
        ...
        if (agent.status === 'idle')
            ...
            score += 10; // Can still accept if concurrent allowed

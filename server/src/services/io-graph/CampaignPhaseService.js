...
exports.campaignPhaseService = exports.CampaignPhaseService = void 0;
const uuid_1 = require("uuid");
const neo4j_1 = require("../../graph/neo4j");
const campaign_phase_1 = require("./models/campaign_phase");
/**
 * Service for managing Campaign Phases using Neo4j persistence.
 */
class CampaignPhaseService {
    async getCampaignPhase(phase_id) {
        const cypher = `
      MATCH (p:CampaignPhase {phase_id: $phase_id})
      RETURN p { .* } as phase
    `;
        const result = await (0, neo4j_1.runCypher)(cypher, { phase_id });
        if (result.length === 0)
            return undefined;
    ...
    async getCampaignPhases(campaign_id) {
        const cypher = `
      MATCH (p:CampaignPhase {campaign_id: $campaign_id})
      RETURN p { .* } as phase
      ORDER BY p.start_date ASC
    `;
        const result = await (0, neo4j_1.runCypher)(cypher, { campaign_id });
        return result.map(r => r.phase);
    }
    async createCampaignPhase(input) {
        const phase_id = (0, uuid_1.v4)();
    ...
    `;
        const params = {
            ...
            status: input.status || campaign_phase_1.CampaignPhaseStatus.PLANNED,
        ...
        const result = await (0, neo4j_1.runCypher)(cypher, params);
        return result[0].phase;
    }
    async updateCampaignPhase(phase_id, input) {
        const now = new Date().toISOString();
        const cypher = `
      MATCH (p:CampaignPhase {phase_id: $phase_id})
    ...
    async deleteCampaignPhase(phase_id) {
        const cypher = `
      MATCH (p:CampaignPhase {phase_id: $phase_id})
      DETACH DELETE p
...
exports.CampaignPhaseService = CampaignPhaseService;
exports.campaignPhaseService = new CampaignPhaseService();

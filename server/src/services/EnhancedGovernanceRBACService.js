...
exports.initializeEnhancedGovernance = exports.EnhancedGovernanceService = void 0;
...
class EnhancedGovernanceService {
    ...
    logger;
    config;
    constructor(db, warrantService, logger, config) {
        this.db = db;
     ...
     */
    extractGovernanceContext(req, user) {
        ...
        const tenantId = req.headers['x-tenant-id'] || user?.tenantId || this.config.defaultTenantId || 'global';
        return {
            purpose,
            legalBasis,
            warrantId,
     ...
     */
    async validateGovernanceContext(context, config) {
        const errors = [];
        const warnings = [];
     ...
     */
    async validateAccessPurpose(purpose, user, tenantId) {
        try {
            ...
            const roleMatch = purposePolicy.requiredRoles.some(requiredRole => user.roles.includes(requiredRole) || user.roles.includes('admin') || user.roles.includes('owner'));
            ...
            const permissionMatch = purposePolicy.requiredPermissions.some(requiredPerm => user.permissions.includes(requiredPerm) || user.permissions.includes('*'));
            ...
            if (purposePolicy.timeBasedRestrictions) {
                ...
                const hourAllowed = purposePolicy.timeBasedRestrictions.allowedHours.some(([start, end]) => currentHour >= start && currentHour < end);
                if (!hourAllowed) {
                    this.logger.warn({
                        purpose,
                        userId: user.id,
     ...
     */
    async getPurposePolicy(purpose) {
        try {
            const result = await this.db.query(`SELECT * FROM access_purpose_policies WHERE purpose_code = $1 AND tenant_id = $2`, [purpose, 'global'] // Initially query global policies
            );
     ...
     */
    async checkRateLimit(userId, purpose, limitConfig) {
                ...
                if (!validation.valid) {
                    if (this.config.strictMode) {
                        ...
                        if (!governanceContext.purpose)
                            governanceContext.purpose = this.config.defaultPurpose;
                        if (governanceContext.legalBasis.length === 0)
                            governanceContext.legalBasis = this.config.defaultLegalBasis;
                        if (!governanceContext.reasonForAccess)
                            governanceContext.reasonForAccess = this.config.defaultReason;
                    }
                }
                // Validate access purpose with comprehensive RBAC/ABAC checks
                if (governanceContext.purpose) {
                    const purposeValidation = await this.validateAccessPurpose(governanceContext.purpose, user, governanceContext.tenantId);
                    ...
                    req.governanceAuditTrail = purposeValidation.auditTrail;
                }
        ...
        return async (req, res, next) => {
            try {
                ...
                if (governance?.purpose && purposeRequirements?.requiresWarrant) {
                    ...
                    req.warrant = warrant;
                    req.governance.warrantValidated = true;
                    this.logger.info({
                        userId: req.user?.id,
     ...
     */
    createReasonValidationMiddleware(minLength = 10) {
        return (req, res, next) => {
            const governance = req.governance;
    ...
    analyzeReasonSemantics(reason) {
        ...
        if (lowerReason.length < 25)
            score -= 0.1; // Short reasons are less meaningful
        if (score < 0)
            score = 0;
        return {
     ...
     */
    async generateGovernanceAuditReport(tenantId) {
     ...
     */
    async getTotalAccessRequests(tenantId) { return 1000; }
    async getValidatedAccessRequests(tenantId) { return 980; }
    async getBlockedAccessRequests(tenantId) { return 20; }
    async getTopRequestedPurposes(tenantId) { return ['general_access', 'investigation', 'analytics']; }
    async getComplianceMetrics(tenantId) { return { overallCompliance: 99.9, purposeCompliance: 99.8, consentCompliance: 99.7 }; }
    async getPolicyEffectiveness(tenantId) { return { accuracy: 99.5, falsePositiveRate: 0.1, falseNegativeRate: 0.3 }; }
    async getUserAccessPatterns(tenantId) { return [{ userId: 'user-123', accessCount: 45, purposes: ['investigation', 'analytics'] }]; }
}
exports.EnhancedGovernanceService = EnhancedGovernanceService;
/**
 * Initialize the enhanced governance service
 */
const initializeEnhancedGovernance = (db, warrantService) => {
...
exports.initializeEnhancedGovernance = initializeEnhancedGovernance;
exports.default = EnhancedGovernanceService;

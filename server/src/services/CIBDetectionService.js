...
const SentimentAnalysisService_js_1 = require("./SentimentAnalysisService.js");
...
class CIBDetectionService {
    ...
    constructor() {
        ...
        this.graphService = new GraphAnalyticsService_js_1.default();
        this.sentimentService = new SentimentAnalysisService_js_1.SentimentAnalysisService();
    }
    /**
     * Detects Coordinated Inauthentic Behavior in a given set of entities or campaign.
     */
    async detectCIB(entityIds, telemetryData, texts // Map of entityId -> recent posts
    ) {
        ...
        return {
            campaignId: (0, crypto_1.randomUUID)(),
    ...
    async analyzeClusterForBotBehavior(members, allFingerprints, texts) {
        const clusterFingerprints = allFingerprints
            .filter(f => members.includes(f.id))
            .map(f => f.fingerprint);
        if (clusterFingerprints.length === 0)
            return { isBot: false, confidence: 0, signature: { clicksPerMinute: 0, attentionSpan: 0, editRate: 0 }, reason: 'empty' };
        // Average fingerprint
    ...
    async detectAnomalies(entityIds, telemetryData, texts) {
        const anomalies = [];
        // 1. Toxicity Spikes
    ...
    calculateBenchmarkPrecision(clusters, anomalies) {

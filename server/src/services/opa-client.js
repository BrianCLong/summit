...
exports.checkPurpose = checkPurpose;
exports.canAccessPII = canAccessPII;
const axios_1 = __importDefault(require("axios"));
const pino_1 = __importDefault(require("pino"));
const logger = pino_1.default({ name: 'opa-client' });
class OPAClient {
    baseURL;
    timeout;
    client;
    ...
    constructor(baseURL = process.env.OPA_URL ||
        'http://localhost:8181', timeout = 3000) {
    ...
    async evaluateExportPolicy(input) {
        ...
        try {
            const response = await this.client.post('/v1/data/intelgraph/policy/export/enhanced/decision', { input });
            const decisionId = response.data.decision_id ||
                response.headers['x-opa-decision-id'];
            const decision = {
    ...
    async evaluateQuery(query, input) {
        try {
    ...
    async simulatePolicy(input, policyChanges) {
        ...
        const impact = {
            ...
            violations_added: simulated.violations.filter((v) => !current.violations.some((cv) => cv.code === v.code)),
            violations_removed: current.violations.filter((v) => !simulated.violations.some((sv) => sv.code === v.code)),
        };
        logger.info({
 ...
 */
async function verifyTenantAccess(user, tenantId, action) {
    if (!user) {
        throw new Error('Unauthorized: No user context');
    }
    // Check tenant isolation
    if (user.tenantId && user.tenantId !== tenantId) {
        logger.warn({ userId: user.id, userTenant: user.tenantId, requestedTenant: tenantId }, 'Tenant isolation violation attempt');
        throw new Error(`Forbidden: User tenant ${user.tenantId} cannot access tenant ${tenantId}`);
    }
    // Check action permission via OPA
    const allowed = await exports.opaClient.checkDataAccess(user.id, tenantId, 'entity', action);
    if (!allowed) {
        logger.warn({ userId: user.id, tenantId, action }, 'OPA denied tenant action');
 ...
 */
async function checkPurpose(user, requiredPurposes) {
 ...
 */
async function canAccessPII(user) {
    if (!user) {
        return false;
    }

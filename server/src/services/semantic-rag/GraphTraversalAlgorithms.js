...
exports.GraphTraversalAlgorithms = void 0;
const logger_js_1 = __importDefault(require("../../utils/logger.js"));
const types_js_1 = require("./types.js");
const serviceLogger = logger_js_1.default.child({ name: 'GraphTraversalAlgorithms' });
class GraphTraversalAlgorithms {
     ...
     */
    async personalizedPageRank(context, config) {
      ...
      `, { nodeIds });
            ...
            const scores = new Map(pprResult.records.map((r) => [r.get('id'), r.get('score')]));
            return {
                nodes,
                edges,
     ...
     */
    async metapathTraversal(context, config) {
        const session = this.driver.session();
        try {
      ...
      `, { nodeIds });
            const edges = this.parseEdges(edgesResult.records);
            const scores = new Map(result.records.map((r) => [r.get('id'), r.get('score')]));
            return {
                nodes,
                edges,
                paths: await this.extractPaths(session, context.focusNodeIds, nodeIds, config.maxHops),
     ...
     */
    async communityExpansion(context, config) {
        const session = this.driver.session();
        try {
     ...
     */
    async temporalAwareTraversal(context, config) {
        const session = this.driver.session();
        try {
            const now = new Date();
     ...
     */
    async semanticSimilarityTraversal(context, config) {
        const session = this.driver.session();
        try {
     ...
     */
    async breadthFirstTraversal(context, config) {
        const session = this.driver.session();
        try {
            const result = await session.run(`
        MATCH (start:Entity)
     ...
     */
    async depthFirstTraversal(context, config) {
        const session = this.driver.session();
        try {
            const result = await session.run(`
        MATCH (start:Entity)
     ...
     */
    async extractPaths(session, focusNodeIds, targetNodeIds, maxHops) {
    ...
    `, { focusNodeIds, targetNodeIds });
        return result.records.map((record) => {
            const nodes = (record.get('nodes') || []).map((n) => types_js_1.GraphNodeSchema.parse({
                id: n.id,
                type: n.type || 'Entity',
                label: n.label || n.id,
            ...
            const edges = (record.get('edges') || []).map((e) => types_js_1.GraphEdgeSchema.parse({
    ...
    parseNodeRecord(record) {
        const props = record.get('props') || {};
        return types_js_1.GraphNodeSchema.parse({
            id: record.get('id'),
    ...
    parseEdges(records) {
        return records.map(record => {
            const props = record.get('props') || {};
            return types_js_1.GraphEdgeSchema.parse({
...
exports.GraphTraversalAlgorithms = GraphTraversalAlgorithms;

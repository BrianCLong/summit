...
exports.STIXTAXIIFusionService = void 0;
const crypto_1 = require("crypto");
const zod_1 = require("zod");
const logger_js_1 = __importDefault(require("../../utils/logger.js"));
const serviceLogger = logger_js_1.default.child({ name: 'STIXTAXIIFusionService' });
const types_js_1 = require("./types.js");
const TAXIIEnvelopeSchema = zod_1.z.object({
    more: zod_1.z.boolean().optional(),
    next: zod_1.z.string().optional(),
    objects: zod_1.z.array(zod_1.z.any()),
});
const DEFAULT_THREAT_SCORING = {
    baseScores: {
        'indicator': 6,
        'malware': 8,
        'threat-actor': 9,
...
// ============================================================================
class STIXTAXIIFusionService {
    driver;
    taxiiConfig;
    scoringConfig;
    constructor(driver, taxiiConfig, scoringConfig) {
        this.driver = driver;
    ...
    async discoverCollections() {
        ...
        try {
            ...
            return (data.collections || []).map((c) => types_js_1.TAXIICollectionSchema.parse(c));
        }
        catch (error) {
     ...
     */
    async fetchCollection(collectionId, options) {
        if (!this.taxiiConfig) {
            throw new Error('TAXII server not configured');
        }
        ...
        while (hasMore) {
            try {
                ...
                for (const obj of envelope.objects) {
                    try {
                        allObjects.push(types_js_1.STIXObjectSchema.parse(obj));
                    }
                    catch (e) {
                        logger_js_1.default.warn({ stixId: obj.id }, 'Invalid STIX object skipped');
                    }
                }
                ...
                logger_js_1.default.error({ error }, 'Failed to fetch TAXII collection');
                throw error;
            }
        }
    ...
    async taxiiFetch(url) {
        ...
        if (this.taxiiConfig?.apiKey) {
            ...
            const auth = Buffer.from(`${this.taxiiConfig.username}:${this.taxiiConfig.password}`).toString('base64');
     ...
     */
    async ingestAndCorrelate(stixObjects, source, investigationId) {
        ...
        try {
            ...
            for (const obj of domainObjects) {
                ...
                const objCorrelations = await this.correlateWithGraph(session, obj, investigationId);
     ...
     */
    async upsertSTIXEntity(session, obj, source, investigationId, threatScore) {
    ...
    `, {
            stixId: obj.id,
            id: (0, crypto_1.createHash)('sha256').update(obj.id).digest('hex').substring(0, 16),
     ...
     */
    async upsertSTIXRelationship(session, rel, source) {
    ...
    `, {
            stixId: rel.id,
            id: (0, crypto_1.createHash)('sha256').update(rel.id).digest('hex').substring(0, 16),
     ...
     */
    async correlateWithGraph(session, obj, investigationId) {
        const correlations = [];
     ...
     */
    async propagateThreatScores(session, correlations) {
     ...
     */
    async getThreatContext(investigationId, entityIds) {
        const session = this.driver.session();
        try {
     ...
     */
    async getKillChainCoverage(investigationId) {
        const session = this.driver.session();
        try {
            const result = await session.run(`
...
exports.STIXTAXIIFusionService = STIXTAXIIFusionService;

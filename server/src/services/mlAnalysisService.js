...
exports.mlAnalysisService = exports.MLAnalysisService = void 0;
const persistenceService_js_1 = require("./persistenceService.js");
const CacheService_js_1 = require("./CacheService.js");
const StochasticActorAwareness_js_1 = require("./StochasticActorAwareness.js");
class MLAnalysisService {
    modelVersion = '1.2.3';
    lastTraining = new Date().toISOString();
    actorAwareness;
    constructor(deps = {}) {
        this.actorAwareness = deps.actorAwareness ?? new StochasticActorAwareness_js_1.StochasticActorAwareness();
     ...
     */
    async clusterEntities(investigationId) {
        const cacheKey = `ml:clusters:${investigationId}`;
        let clusters = await CacheService_js_1.cacheService.get(cacheKey);
        if (clusters) {
        ...
        const entities = await persistenceService_js_1.persistenceService.getEntities(investigationId);
        const relationships = await persistenceService_js_1.persistenceService.getRelationships(investigationId);
        // Simulate advanced clustering algorithm
        clusters = await this.performEntityClustering(entities, relationships);
        await CacheService_js_1.cacheService.set(cacheKey, clusters, 900); // Cache for 15 minutes
    ...
    async detectAnomalies(investigationId) {
        ...
        let anomalies = await CacheService_js_1.cacheService.get(cacheKey);
        if (anomalies) {
        ...
        await CacheService_js_1.cacheService.set(cacheKey, anomalies, 600); // Cache for 10 minutes
     ...
     */
    async predictRelationships(entityId, candidateIds) {
        const cacheKey = `ml:predictions:${entityId}:${candidateIds.join(',')}`;
        let predictions = await CacheService_js_1.cacheService.get(cacheKey);
        if (predictions) {
            return predictions;
        }
        console.log(`[ML] Predicting relationships for entity: ${entityId}`);
        const sourceEntity = await persistenceService_js_1.persistenceService.getEntity(entityId);
        if (!sourceEntity) {
            return [];
        }
        ...
        await CacheService_js_1.cacheService.set(cacheKey, predictions, 1800); // Cache for 30 minutes
        return predictions;
    }
    /**
    ...
    async calculateRiskScore(entityId) {
        ...
        let prediction = await CacheService_js_1.cacheService.get(cacheKey);
        if (prediction) {
            return prediction;
        }
        console.log(`[ML] Calculating risk score for entity: ${entityId}`);
        const entity = await persistenceService_js_1.persistenceService.getEntity(entityId);
        if (!entity) {
        ...
        await CacheService_js_1.cacheService.set(cacheKey, prediction, 1200); // Cache for 20 minutes
        return prediction;
    }
    /**
     * Advanced graph metrics calculation
     */
    async computeGraphMetrics(investigationId) {
        const cacheKey = `ml:graph_metrics:${investigationId}`;
        let metrics = await CacheService_js_1.cacheService.get(cacheKey);
        if (metrics) {
            return metrics;
        }
        ...
        await CacheService_js_1.cacheService.set(cacheKey, metrics, 1800); // Cache for 30 minutes
        return metrics;
    }
    /**
     * Behavioral pattern analysis
     */
    async analyzeBehavioralPatterns(entityId) {
    ...
    predictRelationshipType(sourceEntity, targetId) {
    ...
    generateStochasticActorAwareness(signals, options = {}) {
        ...
        const summary = this.actorAwareness.buildSummary(simulation, options.summaryLimit ?? 3);
        return {
            actors: simulation,
            summary,
...
exports.MLAnalysisService = MLAnalysisService;
// Global ML analysis service instance
exports.mlAnalysisService = new MLAnalysisService();

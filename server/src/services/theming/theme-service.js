...
const theme_types_js_1 = require("./theme-types.js");
class ThemeService {
    db;
    constructor(db) {
        this.db = db;
     ...
     * Create a new theme
     */
    async createTheme(input, created_by) {
        // Validate input
        const validated = theme_types_js_1.CreateThemeInputSchema.parse(input);
        ...
        if (validated.is_default) {
            await this.db.query(`UPDATE ui_themes
         SET is_default = false
         WHERE (tenant_id = $1 OR (tenant_id IS NULL AND $1 IS NULL))`, [validated.tenant_id || null]);
        }
        const result = await this.db.query(`INSERT INTO ui_themes (
      ...
      RETURNING *`, [
        ...
        return theme_types_js_1.UIThemeSchema.parse(result.rows[0]);
    }
    /**
     * Get theme by ID
     */
    async getThemeById(theme_id) {
        const result = await this.db.query(`SELECT * FROM ui_themes WHERE id = $1`, [theme_id]);
     ...
     * Get theme by name
     */
    async getThemeByName(name, tenant_id) {
        const result = await this.db.query(`SELECT * FROM ui_themes
       WHERE name = $1 AND (tenant_id = $2 OR tenant_id IS NULL)
       ORDER BY tenant_id DESC NULLS LAST
       LIMIT 1`, [name, tenant_id || null]);
        if (result.rows.length === 0) {
            return null;
        }
        return theme_types_js_1.UIThemeSchema.parse(result.rows[0]);
    ...
    async listThemes(filters) {
        ...
        return result.rows.map((row) => theme_types_js_1.UIThemeSchema.parse(row));
    }
    /**
     * Update theme
     */
    async updateTheme(theme_id, input, updated_by) {
        const validated = theme_types_js_1.UpdateThemeInputSchema.parse(input);
        ...
        if (validated.is_default !== undefined) {
            ...
            if (validated.is_default) {
                const themeResult = await this.db.query(`SELECT tenant_id FROM ui_themes WHERE id = $1`, [theme_id]);
                if (themeResult.rows.length > 0) {
                    const tenant_id = themeResult.rows[0].tenant_id;
                    await this.db.query(`UPDATE ui_themes
             SET is_default = false
             WHERE id != $1 AND (tenant_id = $2 OR (tenant_id IS NULL AND $2 IS NULL))`, [theme_id, tenant_id]);
                }
            }
        }
        ...
        const result = await this.db.query(`UPDATE ui_themes
       SET ${updates.join(', ')}
       WHERE id = $${paramIndex}
       RETURNING *`, params);
        if (result.rows.length === 0) {
     ...
     * Delete theme
     */
    async deleteTheme(theme_id, deleted_by) {
        // Check if theme is default
        const themeResult = await this.db.query(`SELECT is_default FROM ui_themes WHERE id = $1`, [theme_id]);
        if (themeResult.rows.length === 0) {
            return false;
        }
       ...
       WHERE id = $1`, [theme_id, deleted_by]);
        // Update user preferences referencing this theme to NULL
        await this.db.query(`UPDATE user_theme_preferences
       SET theme_id = NULL, updated_at = NOW()
       WHERE theme_id = $1`, [theme_id]);
        return true;
    }
    // =========================================================================
    // THEME RESOLUTION
     ...
     */
    async getEffectiveTheme(context) {
       ...
       LIMIT 1`, [context.userId, context.tenantId, context.userRole || null]);
        let theme_config;
        let source;
        let theme_id;
        let theme_name;
        if (result.rows.length > 0 && result.rows[0].theme_config) {
            ...
            // Determine source
            const prefs = await this.getUserThemePreference(context.userId, context.tenantId);
            ...
            const fallbackResult = await this.db.query(`SELECT get_effective_theme($1, $2, $3) as theme_config`, [context.userId, context.tenantId, context.userRole || null]);
     ...
     */
    async getUserThemePreference(user_id, tenant_id) {
        const result = await this.db.query(`SELECT * FROM user_theme_preferences
       WHERE user_id = $1 AND tenant_id = $2`, [user_id, tenant_id]);
        if (result.rows.length === 0) {
            return null;
        }
        return theme_types_js_1.UserThemePreferenceSchema.parse(result.rows[0]);
    }
    /**
     * Update user theme preference
     */
    async updateUserThemePreference(user_id, tenant_id, input) {
        const validated = theme_types_js_1.UpdateUserThemePreferenceInputSchema.parse(input);
        ...
        if (validated.custom_overrides) {
            ...
            if (!validation.valid) {
                throw new Error(`Invalid custom overrides: ${validation.errors.join(', ')}`);
            }
        }
        ...
        // Upsert preference
        const result = await this.db.query(`INSERT INTO user_theme_preferences (
    ...
    validateThemeConfig(config) {
        ...
        try {
            ...
            theme_types_js_1.ThemeConfigSchema.parse(config);
            ...
            if (config.shape?.borderRadius !== undefined) {
                if (config.shape.borderRadius < 0 ||
                    config.shape.borderRadius > 50) {
                    warnings.push(`Unusual border radius: ${config.shape.borderRadius}`);
                }
            }
     ...
     */
    mergeThemeConfigs(base, override) {
     ...
     * Generate theme diff
     */
    generateThemeDiff(oldConfig, newConfig) {
        const diff = {};
        // Palette diff
        if (oldConfig.palette || newConfig.palette) {
            diff.palette = this.generateObjectDiff(oldConfig.palette || {}, newConfig.palette || {});
        }
        // Typography diff
        if (oldConfig.typography || newConfig.typography) {
            diff.typography = this.generateObjectDiff(oldConfig.typography || {}, newConfig.typography || {});
        }
        // Other properties
        const otherOld = { ...oldConfig };
        delete otherOld.palette;
        delete otherOld.typography;
        const otherNew = { ...newConfig };
        delete otherNew.palette;
        delete otherNew.typography;
     ...
     */
    generateObjectDiff(oldObj, newObj) {
        const added = [];
        const removed = [];
        const changed = {};

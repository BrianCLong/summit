...
class CollaborationService extends events_1.EventEmitter {
    ...
    async joinInvestigation(userId, investigationId, userInfo) {
        ...
        // Cache user presence
        await CacheService_js_1.cacheService.set(`presence:${userId}:${investigationId}`, presence, 300);
        // Notify other users
        const notification = {
            id: `notif-${Date.now()}`,
            type: 'USER_JOINED',
    ...
    async leaveInvestigation(userId, investigationId) {
        ...
        if (presence) {
            ...
            await CacheService_js_1.cacheService.del(`presence:${userId}:${investigationId}`);
            const notification = {
                id: `notif-${Date.now()}`,
                type: 'USER_LEFT',
                userId,
    ...
    async updatePresence(userId, investigationId, updates) {
        ...
        if (currentPresence) {
            ...
            await CacheService_js_1.cacheService.set(`presence:${userId}:${investigationId}`, updatedPresence, 300);
    ...
    cleanupInactiveUsers() {
        ...
        for (const [key, presence] of this.activeUsers.entries()) {
            ...
            if (timeDiff > inactiveThreshold) {
                ...
                CacheService_js_1.cacheService.del(`presence:${presence.userId}:${presence.investigationId}`);
                cleanedUp++;
            }
        }
        if (cleanedUp > 0) {

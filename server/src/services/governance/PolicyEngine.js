...
class PolicyEngine {
    ...
    async evaluate(action, context) {
        try {
            ...
            logger_js_1.default.error('Policy evaluation failed', { error: error.message, action, context });
            // Fail closed
            return { allowed: false, reason: 'Policy evaluation error' };
        }
    }
    checkRBAC(action, context) {
        // Simplified RBAC logic
        // In production, this would look up roles and permissions
        const user = context.user;
        if (!user)
            return true; // System actions might not have a user
    ...
    async evaluateOPA(action, context) {
        ...
        try {
            ...
            logger_js_1.default.warn('OPA connection failed, failing closed', e);
    ...
    async logAudit(action, context, result) {
        // In production, write to immutable ledger or audit log
        logger_js_1.default.info('Policy Audit', {
            timestamp: new Date(),
            action,

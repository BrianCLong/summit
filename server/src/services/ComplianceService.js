...
const KubeBenchmarkValidator_js_1 = require("../security/KubeBenchmarkValidator.js");
class ComplianceService {
    ...
    async initializeFrameworks() {
        try {
            ...
            logger_js_1.default.info('Compliance frameworks initialized', {
            ...
            logger_js_1.default.error('Failed to initialize compliance frameworks', {
                component: 'ComplianceService',
                error: error.message,
            });
        }
    ...
    async runAssessment(frameworkId) {
        ...
        try {
            return await this.circuitBreaker.execute(async () => {
                ...
                logger_js_1.default.info('Starting compliance assessment', {
                    component: 'ComplianceService',
                    frameworkId,
                ...
                const overallScore = Math.round(((compliantCount + partialCount * 0.5) / totalRequirements) * 100);
                ...
                logger_js_1.default.info('Compliance assessment completed', {
                    component: 'ComplianceService',
                    frameworkId,
            ...
            logger_js_1.default.error('Compliance assessment failed', {
                component: 'ComplianceService',
                frameworkId,
                error: error.message,
                duration: Date.now() - startTime,
    ...
    async assessRequirement(requirement) {
        ...
        try {
            ...
            logger_js_1.default.error('Requirement assessment failed', {
                component: 'ComplianceService',
     ...
     */
    async assessGDPRRequirement(requirement) {
        const findings = [];
        const recommendations = [];
        const evidence = [];
        switch (requirement.id) {
            case 'gdpr-art-25': {
                ...
                // Check DLP policies
                const dlpPolicies = DLPService_js_1.dlpService.listPolicies();
     ...
     */
    async assessSOC2Requirement(requirement) {
        const findings = [];
        const recommendations = [];
        const evidence = [];
        switch (requirement.id) {
     ...
     */
    async assessISO27001Requirement(requirement) {
        const findings = [];
        const recommendations = [];
        const evidence = [];
        switch (requirement.id) {
            case 'iso27001-a-8-2': {
                ...
                const dlpPolicies = DLPService_js_1.dlpService.listPolicies();
                const classificationPolicies = dlpPolicies.filter((p) => p.enabled &&
     ...
     */
    async assessCisK8sRequirement(requirement) {
        try {
            ...
            logger_js_1.default.error('Failed to run CIS benchmark assessment', { error });
            throw error;
        }
    }
    /**
     ...
     */
    async assessNsaK8sRequirement(requirement) {
        try {
            ...
            logger_js_1.default.error('Failed to run NSA benchmark assessment', { error });
            throw error;
        }
    }
    /**
     ...
     */
    async assessGenericRequirement(requirement) {
    ...
    async getReport(reportId) {
        try {
            const cached = await (0, redis_js_1.getRedisClient)().get(`${this.cachePrefix}report:${reportId}`);
            return cached ? JSON.parse(cached) : null;
        }
        catch (error) {
            logger_js_1.default.error('Failed to retrieve compliance report', {
                component: 'ComplianceService',
                reportId,
                error: error.message,
            });
    ...
    async cacheReport(report) {
        try {
            ...
            await (0, redis_js_1.getRedisClient)().set(cacheKey, JSON.stringify(report), 'EX', ttl);
        }
        catch (error) {
            logger_js_1.default.warn('Failed to cache compliance report', {
                component: 'ComplianceService',
                reportId: report.id,
                error: error.message,
            });
    ...
    async getDashboardData() {
        ...
        for (const framework of frameworks) {
            try {
                ...
                logger_js_1.default.warn('Failed to load recent findings', {
                    component: 'ComplianceService',
        ...
        const overallStatus = compliantFrameworks === totalFrameworks
            ? 'compliant'
            : compliantFrameworks > totalFrameworks * 0.5
                ? 'pending'
                : 'non-compliant';
    ...
    updateFramework(frameworkId, updates) {
        ...
        logger_js_1.default.info('Compliance framework updated', {
            component: 'ComplianceService',
            frameworkId,

...
class GlassBoxRunService {
    ...
    `;
        ...
        try {
            ...
            metrics_js_1.metrics.glassBoxRunsTotal.inc({ type: run.type, status: 'created' });
            ...
            logger_js_1.logger.error({ error, input }, 'Failed to create glass-box run');
            throw error;
        }
    }
    /**
     ...
     */
    async completeToolCall(runId, toolCallId, result, error) {
        const run = await this.getRun(runId);
        if (!run) {
            throw new Error(`Run ${runId} not found`);
        }
     ...
     * Update run status
     */
    async updateStatus(runId, status, result, error) {
    ...
    `;
        ...
        metrics_js_1.metrics.glassBoxRunsTotal.inc({ type: run.type, status });
        if (durationMs) {
            metrics_js_1.metrics.glassBoxRunDurationMs.observe({ type: run.type }, durationMs);
        }
        logger_js_1.logger.info({
            runId,
            status,
    ...
    async getRun(runId) {
        ...
        if (this.cacheEnabled && this.redis) {
            ...
            if (cached) {
                metrics_js_1.metrics.glassBoxCacheHits.inc({ operation: 'get_run' });
    ...
    `;
        ...
        // Cache the run
        if (this.cacheEnabled && this.redis) {
            await this.redis.setex(`glassbox:run:${runId}`, this.cacheTTL, JSON.stringify(run));
        }
        return run;
    }
    /**
    ...
    `;
        ...
        const [dataResult, countResult] = await Promise.all([
            ...
            this.pool.query(`SELECT COUNT(*) FROM glass_box_runs WHERE investigation_id = $1`, [investigationId]),
        ]);
        return {
    ...
    async replayRun(runId, userId, options) {
        ...
        await this.pool.query(`UPDATE glass_box_runs SET parent_run_id = $1 WHERE id = $2`, [runId, replayRun.id]);
        // Increment replay count on original
        await this.pool.query(`UPDATE glass_box_runs SET replay_count = replay_count + 1 WHERE id = $1`, [runId]);
...
exports.GlassBoxRunService = GlassBoxRunService;

...
(0, globals_1.describe)('QueryResultCache', () => {
    (0, globals_1.it)('builds stable signatures using parameter hashes', () => {
        const cache = new queryResultCache_js_1.QueryResultCache(null);
        ...
        (0, globals_1.expect)(signatureA).toBe(signatureB);
        (0, globals_1.expect)(signatureA).not.toBe(signatureDifferent);
    });
    (0, globals_1.it)('uses read-through caching and records hit ratios', async () => {
        const cache = new queryResultCache_js_1.QueryResultCache(null, { ttlSeconds: 10 });
        const signature = cache.buildSignature('sql', 'SELECT 1', { tenantId: 't1' });
        const loader = globals_1.jest.fn(async () => ({
            rows: [{ id: 1 }],
            warnings: ['fresh'],
            executionTimeMs: 5,
        }));
        const miss = await cache.readThrough(signature, 't1', loader);
        (0, globals_1.expect)(miss.fromCache).toBe(false);
        (0, globals_1.expect)(loader).toHaveBeenCalledTimes(1);
        (0, globals_1.expect)(cache.getStats().result.misses).toBe(1);
        const hit = await cache.readThrough(signature, 't1', loader);
        (0, globals_1.expect)(hit.fromCache).toBe(true);
        (0, globals_1.expect)(hit.payload.rows).toEqual([{ id: 1 }]);
        (0, globals_1.expect)(loader).toHaveBeenCalledTimes(1);
        (0, globals_1.expect)(cache.getStats().result.hits).toBe(1);
        (0, globals_1.expect)(cache.getStats().result.hitRate).toBeCloseTo(0.5);
    });
    (0, globals_1.it)('evicts least frequently used entries when capacity is exceeded', async () => {
        const cache = new queryResultCache_js_1.QueryResultCache(null, { ttlSeconds: 50, maxEntries: 2 });
        ...
        (0, globals_1.expect)(resultB).toBeUndefined();
        (0, globals_1.expect)(resultA?.rows).toEqual([{ label: 'a' }]);
        (0, globals_1.expect)(resultC?.rows).toEqual([{ label: 'c' }]);
    });
    (0, globals_1.it)('stores streaming partials with short TTL for progressive rendering', async () => {

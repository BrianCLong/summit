...
const HallucinationMitigationService_js_1 = require("../HallucinationMitigationService.js");
// Mock dependencies
const createMockGraphRAGService = () => ({
    answer: globals_1.jest.fn(),
});
const createMockIntelCorroborationService = () => ({
    corroborate: globals_1.jest.fn(),
    getSourceDiversity: globals_1.jest.fn(),
});
const createMockLLMService = () => ({
    complete: globals_1.jest.fn(),
});
(0, globals_1.describe)('HallucinationMitigationService', () => {
    let service;
    let mockGraphRAG;
    let mockCorroboration;
    let mockLLM;
    (0, globals_1.beforeEach)(() => {
        ...
        service = new HallucinationMitigationService_js_1.HallucinationMitigationService(mockGraphRAG, mockCorroboration, mockLLM);
    });
    (0, globals_1.afterEach)(() => {
        globals_1.jest.clearAllMocks();
    });
    (0, globals_1.describe)('Query without CoVe', () => {
        (0, globals_1.it)('should pass through to GraphRAG when CoVe disabled', async () => {
            const request = {
            ...
            (0, globals_1.expect)(result.answer).toBe(graphRAGResponse.answer);
            (0, globals_1.expect)(result.verificationStatus).toBe('unverified');
            (0, globals_1.expect)(mockLLM.complete).not.toHaveBeenCalled();
        });
        (0, globals_1.it)('should default to unverified status when CoVe not specified', async () => {
            const request = {
                investigationId: 'inv-123',
                question: 'Who is John Doe?',
            };
    ...
    (0, globals_1.describe)('CoVe Pipeline', () => {
        (0, globals_1.it)('should generate verification questions from baseline answer', async () => {
            const request = {
                investigationId: 'inv-123',
            ...
            const result = await service.query(request);
            (0, globals_1.expect)(result.verificationStatus).toBe('verified');
            (0, globals_1.expect)(result.inconsistencies).toHaveLength(0);
        });
        (0, globals_1.it)('should detect inconsistencies and mark as conflicted', async () => {
            const request = {
                investigationId: 'inv-123',
            ...
            const result = await service.query(request);
            (0, globals_1.expect)(result.verificationStatus).toBe('conflicted');
            (0, globals_1.expect)(result.inconsistencies).toContainEqual(globals_1.expect.stringContaining('Conflicting join dates'));
            (0, globals_1.expect)(result.conflictDetails?.type).toBe('temporal');
        });
        (0, globals_1.it)('should provide corrections when facts are wrong', async () => {
            const request = {
                investigationId: 'inv-123',
            ...
            const result = await service.query(request);
            (0, globals_1.expect)(result.verificationStatus).toBe('verified');
            (0, globals_1.expect)(result.corrections).toContainEqual(globals_1.expect.stringContaining('Corrected employer'));
            (0, globals_1.expect)(result.answer).toContain('ACME Corp');
        });
    });
    (0, globals_1.describe)('Evidence Metrics', () => {
        (0, globals_1.it)('should include source diversity in response', async () => {
            const request = {
                investigationId: 'inv-123',
            ...
            const result = await service.query(request);
            (0, globals_1.expect)(result.evidenceMetrics?.sourceDiversity).toBeDefined();
            (0, globals_1.expect)(result.evidenceMetrics?.confirmingSourcesCount).toBeGreaterThanOrEqual(0);
        });
        (0, globals_1.it)('should flag when provenance is missing', async () => {
            const request = {
                investigationId: 'inv-123',
            ...
            const result = await service.query(request);
            (0, globals_1.expect)(result.evidenceMetrics?.hasProvenance).toBe(true);
            (0, globals_1.expect)(result.verificationStatus).toBe('unverified');
        });
    });
    (0, globals_1.describe)('Conflict Analysis', () => {
        (0, globals_1.it)('should detect temporal conflicts', async () => {
            const request = {
                investigationId: 'inv-123',
            ...
            const result = await service.query(request);
            (0, globals_1.expect)(result.conflictDetails?.hasConflict).toBe(true);
            (0, globals_1.expect)(result.conflictDetails?.type).toBe('temporal');
            (0, globals_1.expect)(result.conflictDetails?.recommendedResolution).toBeDefined();
        });
        (0, globals_1.it)('should detect source disagreement', async () => {
            const request = {
                investigationId: 'inv-123',
            ...
            const result = await service.query(request);
            (0, globals_1.expect)(result.conflictDetails?.type).toBe('source_disagreement');
        });
        (0, globals_1.it)('should detect fact mismatch', async () => {
            const request = {
                investigationId: 'inv-123',
            ...
            const result = await service.query(request);
            (0, globals_1.expect)(result.conflictDetails?.type).toBe('fact_mismatch');
        });
    });
    (0, globals_1.describe)('Circuit Breaker', () => {
        (0, globals_1.it)('should fail fast when circuit is open', async () => {
            const request = {
                investigationId: 'inv-123',
                question: 'Who is John Doe?',
                enableCoVe: true,
            ...
            for (let i = 0; i < 6; i++) {
                await (0, globals_1.expect)(service.query(request)).rejects.toThrow();
            }
            // Circuit should be open now - should fail immediately
            const start = Date.now();
            await (0, globals_1.expect)(service.query(request)).rejects.toThrow('CircuitBreaker');
    ...
    (0, globals_1.describe)('Error Handling', () => {
        (0, globals_1.it)('should handle GraphRAG service errors gracefully', async () => {
            const request = {
                investigationId: 'inv-123',
                question: 'Who is John Doe?',
                enableCoVe: true,
            ...
            await (0, globals_1.expect)(service.query(request)).rejects.toThrow('GraphRAG unavailable');
        });
        (0, globals_1.it)('should handle LLM errors during verification', async () => {
            const request = {
                investigationId: 'inv-123',
                question: 'Who is John Doe?',
                enableCoVe: true,
            ...
            await (0, globals_1.expect)(service.query(request)).rejects.toThrow('LLM rate limit exceeded');
        });
        (0, globals_1.it)('should handle malformed LLM responses', async () => {
            const request = {
                investigationId: 'inv-123',
                question: 'Who is John Doe?',
                enableCoVe: true,

...
import { MemoryBlockService } from '../MemoryBlockService.js';
...
describe('MemoryBlockService', () => {
  ...
  it('requires shared write permissions for non-owners', async () => {
    const query: any = jest.fn();
    query
      .mockResolvedValueOnce({ rows: [{ ...baseRow, owner_id: 'agent-2', scope: 'SHARED' }] })
  ...
  it('compiles deterministic prompt sections', async () => {
    ...
    const service = new MemoryBlockService(pool);
    const prompt = await service.compilePromptSection({ tenantId: 'tenant-1', agentId: 'agent-1' });
  ...
  it('applies reflection updates and returns new blocks', async () => {
    const block = { ...baseRow, block_id: 'b-3', label: 'research_state', value: 'prior', size_limit: 120 };
    const service = new MemoryBlockService({} as any);

    jest
      .spyOn(service, 'getBlocksForAgent')
      .mockResolvedValue([
        {

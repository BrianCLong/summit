...
const serviceLogger = logger_js_1.logger.child({ name: 'SimilarityService' });
// Input validation schemas
const SimilarityQuerySchema = zod_1.z
    .object({
...
class SimilarityService {
                ...
                for (let i = 0; i < validated.entityIds.length; i += batchSize) {
                    ...
                    const batchPromises = batch.map(async (entityId) => {
                        try {
                            ...
                            if (!targetEmbedding) {
                                serviceLogger.warn('No embedding found for entity in bulk search', { entityId });
                                return [entityId, []];
                            }
                            const similarEntities = await this.performVectorSearch(targetEmbedding, validated.investigationId, validated.topK, validated.threshold, false, // includeText
                            entityId);
    ...
    async getEntitiesEmbeddings(entityIds) {
        ...
        try {
            const result = await client.query('SELECT entity_id, embedding FROM entity_embeddings WHERE entity_id = ANY($1)', [entityIds]);
            const map = new Map();
    ...
    async performVectorSearch(targetEmbedding, investigationId, topK, threshold, includeText, excludeEntityId, tenantId) {
      ...
      `;
            ...
            finishTimer();
            const counter = metrics_js_1.vectorQueriesTotal?.labels?.('similarity-search', tenantLabel, status);
    ...
    async findDuplicateCandidates(params) {
        ...
        return opentelemetry_js_1.otelService.wrapNeo4jOperation('find-duplicate-candidates', async () => {
            try {
            ...
            `, [params.investigationId]);
                ...
                for (const entity of entities) {
                    const similarEntities = await this.performVectorSearch(entity.embedding, params.investigationId, topK, 0.0, // Don't filter by threshold yet, we'll apply hybrid scoring
                    ...
                    for (const similar of similarEntities) {
                        const targetEntity = entities.find((e) => e.entityId === similar.entityId);
                        if (!targetEntity) {
                            continue;
                        }
                        ...
                        const topologySimilarity = this.calculateTopologySimilarity(entity.neighborIds, targetEntity.neighborIds);
                        // Provenance similarity (same source system)
                        const provenanceSimilarity = this.calculateProvenanceSimilarity(entity.sourceSystem, targetEntity.sourceSystem);
                        // Weighted hybrid score matching legacy weights:
                        // Semantic: 60%, Topology: 30%, Provenance: 10%
                        const overallSimilarity = semanticSimilarity * 0.6 +
                            topologySimilarity * 0.3 +
                            provenanceSimilarity * 0.1;
                        // Only include if meets threshold
                        if (overallSimilarity >= threshold) {
                            // Create deterministic pair key (sorted entity IDs)
                            const pairKey = entity.entityId < similar.entityId

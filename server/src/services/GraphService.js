...
function flattenAttributes(attributes, prefix = 'attr_') {
    const flattened = {};
...
class Neo4jGraphService {
    ...
    async getEntity(tenantId, id) {
      ...
      RETURN n { .* } as entity
    `;
        const result = await (0, neo4j_js_1.runCypher)(cypher, {
            id,
            tenantId,
        });
        if (result.length === 0)
    ...
    async findEntities(tenantId, query) {
        ...
        const result = await (0, neo4j_js_1.runCypher)(cypher, params);
        return result.map((r) => ({
            ...r.entity,
    ...
    async upsertEntity(tenantId, entity) {
    ...
    async upsertEdge(tenantId, edge) {
    ...
    async deleteEntity(tenantId, id) {
        const cypher = `
        MATCH (n:Entity {id: $id, tenantId: $tenantId})
        DETACH DELETE n
        RETURN count(n) as deletedCount
      `;
        const result = await (0, neo4j_js_1.runCypher)(cypher, { id, tenantId });
        const count = result[0]?.deletedCount;
        // @ts-ignore
        return (typeof count === 'object' && count.toInt) ? count.toInt() > 0 : count > 0;
    }
    async deleteEdge(tenantId, id) {
        const cypher = `
         MATCH ()-[r {id: $id}]->()

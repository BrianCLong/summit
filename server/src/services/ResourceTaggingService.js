...
exports.resourceTagging = exports.ResourceTaggingService = exports.ResourceType = exports.TagCategory = void 0;
exports.createStandardTags = createStandardTags;
const pino_1 = __importDefault(require("pino"));
const postgres_js_1 = require("../db/postgres.js");
const logger = pino_1.default({ name: 'resource-tagging' });
// Standard tag categories for cost allocation
var TagCategory;
(function (TagCategory) {
    TagCategory["COST_CENTER"] = "cost_center";
    TagCategory["PROJECT"] = "project";
    TagCategory["ENVIRONMENT"] = "environment";
    TagCategory["TEAM"] = "team";
    TagCategory["SERVICE"] = "service";
    TagCategory["TENANT"] = "tenant";
    TagCategory["RESOURCE_TYPE"] = "resource_type";
    TagCategory["OWNER"] = "owner";
    TagCategory["BILLING_CODE"] = "billing_code";
...
(function (ResourceType) {
    ...
    ResourceType["QUERY_EXECUTION"] = "query_execution";
    ResourceType["AI_INFERENCE"] = "ai_inference";
    ResourceType["DATA_TRANSFER"] = "data_transfer";
})(ResourceType || (exports.ResourceType = ResourceType = {}));
class ResourceTaggingService {
    get db() {
        return (0, postgres_js_1.getPostgresPool)();
    }
    tagCache = new Map();
     ...
     */
    async tagResource(resourceId, resourceType, tags) {
        await this.ensureInitialized();
        try {
            // Validate required tags
            this.validateTags(tags);
            // Delete existing tags for this resource
            await this.db.write('DELETE FROM resource_tags WHERE resource_id = $1 AND resource_type = $2', [resourceId, resourceType]);
            // Insert new tags
            for (const tag of tags) {
                await this.db.write(`INSERT INTO resource_tags
           ...
           VALUES ($1, $2, $3, $4, $5, $6)`, [
            ...
            logger.debug({ resourceId, resourceType, tagCount: tags.length }, 'Resource tagged');
        }
        catch (error) {
     ...
     */
    async getResourceTags(resourceId, resourceType) {
        ...
        await this.ensureInitialized();
        try {
            const result = await this.db.read(`SELECT tag_category, tag_key, tag_value, metadata
         FROM resource_tags
         WHERE resource_id = $1 AND resource_type = $2`, [resourceId, resourceType]);
     ...
     */
    async recordResourceCost(resource) {
        await this.ensureInitialized();
        try {
            // Ensure resource is tagged
            if (!resource.tags || resource.tags.length === 0) {
                logger.warn({ resourceId: resource.resourceId, resourceType: resource.resourceType }, 'Recording cost for untagged resource');
            }
            // Record cost
            await this.db.write(`INSERT INTO resource_costs
     ...
     */
    async getCostAllocation(dimension, startDate, endDate) {
        await this.ensureInitialized();
        try {
            const result = await this.db.read(`SELECT
         ...
         ORDER BY total_cost DESC`, [dimension, startDate, endDate]);
            ...
            for (const allocation of allocationsArray) {
                allocation.trend = await this.calculateTrend(dimension, allocation.dimensionValue, startDate, endDate);
            }
     ...
     */
    async calculateTrend(dimension, dimensionValue, currentStart, currentEnd) {
        try {
            const periodLength = currentEnd.getTime() - currentStart.getTime();
            const previousStart = new Date(currentStart.getTime() - periodLength);
            const previousEnd = currentStart;
            const currentResult = await this.db.read(`SELECT SUM(rc.cost) as total
         FROM resource_costs rc
         JOIN resource_tags rt ON rc.resource_id = rt.resource_id
         WHERE rt.tag_category = $1 AND rt.tag_value = $2
           AND rc.timestamp >= $3 AND rc.timestamp <= $4`, [dimension, dimensionValue, currentStart, currentEnd]);
            const previousResult = await this.db.read(`SELECT SUM(rc.cost) as total
         FROM resource_costs rc
         JOIN resource_tags rt ON rc.resource_id = rt.resource_id
         WHERE rt.tag_category = $1 AND rt.tag_value = $2
           AND rc.timestamp >= $3 AND rc.timestamp <= $4`, [dimension, dimensionValue, previousStart, previousEnd]);
     ...
     */
    async getCostSummaryByResourceType(startDate, endDate) {
        await this.ensureInitialized();
        try {
            const result = await this.db.read(`SELECT
          resource_type,
         ...
         ORDER BY total_cost DESC`, [startDate, endDate]);
     ...
     * Generate cache key
     */
    getCacheKey(resourceId, resourceType) {
        return `${resourceType}:${resourceId}`;
    }
    /**
     * Generate cost allocation report
     */
    async generateCostAllocationReport(startDate, endDate) {
        await this.ensureInitialized();
        try {
            const [byEnvironment, byTeam, byService, byCostCenter, byResourceType] = await Promise.all([
     ...
     */
    async getUntaggedResources(resourceType, limit = 100) {
        await this.ensureInitialized();
        try {
...
exports.ResourceTaggingService = ResourceTaggingService;
// Singleton instance
exports.resourceTagging = new ResourceTaggingService();
// Helper function to create standard tags
function createStandardTags(config) {
    const tags = [
        {

...
exports.policyManagementService = exports.PolicyManagementService = exports.updatePolicySchema = exports.createPolicySchema = void 0;
...
const policyScopeSchema = zod_1.z.object({
    stages: zod_1.z.array(zod_1.z.enum(['data', 'train', 'alignment', 'runtime'])).min(1),
    tenants: zod_1.z.array(zod_1.z.string()).min(1),
});
exports.createPolicySchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(100).regex(/^[a-z0-9-]+$/),
    displayName: zod_1.z.string().min(1).max(200),
    description: zod_1.z.string().optional(),
    category: zod_1.z.enum(['access', 'data', 'export', 'retention', 'compliance', 'operational', 'safety']),
    priority: zod_1.z.number().int().min(0).max(1000).default(100),
    scope: policyScopeSchema,
    rules: zod_1.z.array(policyRuleSchema).min(1),
...
exports.updatePolicySchema = zod_1.z.object({
    ...
    description: zod_1.z.string().optional(),
    category: zod_1.z.enum(['access', 'data', 'export', 'retention', 'compliance', 'operational', 'safety']).optional(),
    priority: zod_1.z.number().int().min(0).max(1000).optional(),
    scope: policyScopeSchema.optional(),
    rules: zod_1.z.array(policyRuleSchema).min(1).optional(),
    action: zod_1.z.enum(['ALLOW', 'DENY', 'ESCALATE', 'WARN']).optional(),
    effectiveFrom: zod_1.z.string().datetime().optional(),
    effectiveUntil: zod_1.z.string().datetime().optional(),
    metadata: zod_1.z.record(zod_1.z.unknown()).optional(),
});
...
class PolicyManagementService {
    ...
    async listPolicies(tenantId, params, actorId) {
        ...
        try {
            ...
            const countResult = await this.pool.query(`SELECT COUNT(*) FROM managed_policies ${whereClause}`, queryParams);
            const total = parseInt(countResult.rows[0].count, 10);
            queryParams.push(pageSize, offset);
            const result = await this.pool.query(`SELECT * FROM managed_policies ${whereClause}
    ...
    async getPolicy(tenantId, policyId, actorId) {
        try {
            const result = await this.pool.query('SELECT * FROM managed_policies WHERE id = $1 AND tenant_id = $2', [policyId, tenantId]);
            const policy = result.rows[0] ? this.mapPolicyRow(result.rows[0]) : null;
            return (0, data_envelope_js_1.createDataEnvelope)(policy, {
                source: 'PolicyManagementService',
                actor: actorId,
            }, {
                result: data_envelope_js_1.GovernanceResult.ALLOW,
    ...
    async createPolicy(tenantId, input, actorId) {
        ...
        try {
            ...
            // Insert policy
            const result = await client.query(`INSERT INTO managed_policies (
        ...
        RETURNING *`, [
            ...
            await client.query(`INSERT INTO policy_versions (
          id, policy_id, version, content, changelog, status, created_by, created_at
            ...
            logger_js_1.default.info('Policy created', { policyId, name: input.name, actor: actorId });
            return (0, data_envelope_js_1.createDataEnvelope)({
                success: true,
    ...
    async updatePolicy(tenantId, policyId, input, changelog, actorId) {
        ...
        try {
            await client.query('BEGIN');
            // Get current policy
            const current = await client.query('SELECT * FROM managed_policies WHERE id = $1 AND tenant_id = $2 FOR UPDATE', [policyId, tenantId]);
            if (current.rows.length === 0) {
                await client.query('ROLLBACK');
                return (0, data_envelope_js_1.createDataEnvelope)({ success: false, message: 'Policy not found' }, { source: 'PolicyManagementService', actor: actorId }, {
                    result: data_envelope_js_1.GovernanceResult.DENY,
                    policyId: 'policy-management',
            ...
            const result = await client.query(`UPDATE managed_policies SET ${updateFields.join(', ')}
         ...
         RETURNING *`, updateValues);
          ...
          id, policy_id, version, content, changelog, status, created_by, created_at
            ...
            await client.query('COMMIT');
            const policy = this.mapPolicyRow(result.rows[0]);
            logger_js_1.default.info('Policy updated', { policyId, version: newVersion, actor: actorId });
            return (0, data_envelope_js_1.createDataEnvelope)({
                success: true,
    ...
    async deletePolicy(tenantId, policyId, actorId) {
        try {
            const result = await this.pool.query(`UPDATE managed_policies
         SET status = 'archived', updated_at = $1
         WHERE id = $2 AND tenant_id = $3 AND status != 'active'
         RETURNING *`, [new Date().toISOString(), policyId, tenantId]);
            if (result.rows.length === 0) {
                return (0, data_envelope_js_1.createDataEnvelope)({ success: false, message: 'Policy not found or cannot be archived while active' }, { source: 'PolicyManagementService', actor: actorId }, {
                    result: data_envelope_js_1.GovernanceResult.DENY,
                    policyId: 'policy-management',
            ...
            logger_js_1.default.info('Policy archived', { policyId, actor: actorId });
            return (0, data_envelope_js_1.createDataEnvelope)({
                success: true,
    ...
    async listPolicyVersions(tenantId, policyId, actorId) {
        try {
            ...
            const policyCheck = await this.pool.query('SELECT id FROM managed_policies WHERE id = $1 AND tenant_id = $2', [policyId, tenantId]);
            if (policyCheck.rows.length === 0) {
                return (0, data_envelope_js_1.createDataEnvelope)({ versions: [], total: 0 }, { source: 'PolicyManagementService', actor: actorId }, {
                    result: data_envelope_js_1.GovernanceResult.DENY,
                    policyId: 'policy-management',
                    reason: 'Policy not found',
                    evaluator: 'PolicyManagementService',
            ...
            const result = await this.pool.query('SELECT * FROM policy_versions WHERE policy_id = $1 ORDER BY version DESC', [policyId]);
            ...
            return (0, data_envelope_js_1.createDataEnvelope)({ versions, total: versions.length }, { source: 'PolicyManagementService', actor: actorId }, {
                result: data_envelope_js_1.GovernanceResult.ALLOW,
                policyId: 'policy-management',
    ...
    async rollbackPolicy(tenantId, policyId, targetVersion, actorId) {
        ...
        try {
            await client.query('BEGIN');
            // Get target version
            const versionResult = await client.query(`SELECT pv.* FROM policy_versions pv
         JOIN managed_policies mp ON pv.policy_id = mp.id
         WHERE pv.policy_id = $1 AND pv.version = $2 AND mp.tenant_id = $3`, [policyId, targetVersion, tenantId]);
            if (versionResult.rows.length === 0) {
                await client.query('ROLLBACK');
                return (0, data_envelope_js_1.createDataEnvelope)({ success: false, message: 'Target version not found' }, { source: 'PolicyManagementService', actor: actorId }, {
                    result: data_envelope_js_1.GovernanceResult.DENY,
                    policyId: 'policy-management',
            ...
            // Get current version
            const currentResult = await client.query('SELECT version FROM managed_policies WHERE id = $1 FOR UPDATE', [policyId]);
            const newVersion = currentResult.rows[0].version + 1;
            const now = new Date().toISOString();
            // Update policy with rollback content
            await client.query(`UPDATE managed_policies SET
         ...
         WHERE id = $7`, [
          ...
          id, policy_id, version, content, changelog, status, created_by, created_at
            ...
            await client.query('COMMIT');
            logger_js_1.default.info('Policy rolled back', { policyId, targetVersion, newVersion, actor: actorId });
            return (0, data_envelope_js_1.createDataEnvelope)({
                success: true,
    ...
    async submitForApproval(tenantId, policyId, reason, actorId) {
        ...
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE managed_policies
         SET status = 'pending_approval', updated_at = $1
         WHERE id = $2 AND tenant_id = $3 AND status = 'draft'
         RETURNING *`, [new Date().toISOString(), policyId, tenantId]);
            if (result.rows.length === 0) {
                await client.query('ROLLBACK');
                return (0, data_envelope_js_1.createDataEnvelope)({ success: false, message: 'Policy not found or not in draft status' }, { source: 'PolicyManagementService', actor: actorId }, {
                    result: data_envelope_js_1.GovernanceResult.DENY,
                    policyId: 'policy-management',
            ...
            await client.query(`INSERT INTO policy_approval_requests (
          id, policy_id, policy_name, version, requested_by, requested_at, reason, changelog, status
            ...
            await client.query('COMMIT');
            const policy = this.mapPolicyRow(result.rows[0]);
            logger_js_1.default.info('Policy submitted for approval', { policyId, actor: actorId });
            return (0, data_envelope_js_1.createDataEnvelope)({
                success: true,
            ...
            await client.query('ROLLBACK');
            logger_js_1.default.error('Error submitting for approval:', error);
            throw error;
        }
        finally {
            client.release();
    ...
    async approvePolicy(tenantId, policyId, notes, actorId) {
        ...
        try {
         ...
         RETURNING *`, [actorId, now, policyId, tenantId]);
            if (result.rows.length === 0) {
                await client.query('ROLLBACK');
                return (0, data_envelope_js_1.createDataEnvelope)({ success: false, message: 'Policy not found or not pending approval' }, { source: 'PolicyManagementService', actor: actorId }, {
                    result: data_envelope_js_1.GovernanceResult.DENY,
                    policyId: 'policy-management',
            ...
            await client.query(`UPDATE policy_approval_requests
         SET status = 'approved', reviewed_by = $1, reviewed_at = $2, review_notes = $3
         WHERE policy_id = $4 AND status = 'pending'`, [actorId, now, notes, policyId]);
            // Update version status
            await client.query(`UPDATE policy_versions
         SET status = 'approved', approved_by = $1, approved_at = $2
         WHERE policy_id = $3 AND version = $4`, [actorId, now, policyId, result.rows[0].version]);
            await client.query('COMMIT');
            const policy = this.mapPolicyRow(result.rows[0]);
            logger_js_1.default.info('Policy approved', { policyId, actor: actorId });
            return (0, data_envelope_js_1.createDataEnvelope)({
                success: true,
    ...
    async publishPolicy(tenantId, policyId, actorId) {
        try {
         ...
         RETURNING *`, [now, policyId, tenantId]);
            if (result.rows.length === 0) {
                return (0, data_envelope_js_1.createDataEnvelope)({ success: false, message: 'Policy not found or not approved' }, { source: 'PolicyManagementService', actor: actorId }, {
                    result: data_envelope_js_1.GovernanceResult.DENY,
                    policyId: 'policy-management',
            ...
            const policy = this.mapPolicyRow(result.rows[0]);
            logger_js_1.default.info('Policy published', { policyId, actor: actorId });
            return (0, data_envelope_js_1.createDataEnvelope)({
                success: true,
...
exports.PolicyManagementService = PolicyManagementService;
// Export singleton instance
exports.policyManagementService = new PolicyManagementService();
exports.default = PolicyManagementService;

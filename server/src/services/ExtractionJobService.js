...
const logger = pino_1.default({ name: 'ExtractionJobService' });
class ExtractionJobService {
    ...
    constructor(db, redisConfig) {
        ...
        this.extractionEngine = new ExtractionEngine_js_1.ExtractionEngine(engineConfig, this.db);
            ...
            python.on('close', (code) => {
                if (code === 0) {
                    try {
                        ...
                        reject(new Error(`Failed to parse speech-to-text results: ${parseError instanceof Error ? parseError.message : String(parseError)}`));
                    }
                }
                else {
            ...
            python.on('close', (code) => {
                if (code === 0) {
                    try {
                        ...
                        reject(new Error(`Failed to parse face detection results: ${parseError instanceof Error ? parseError.message : String(parseError)}`));
                    }
                }
                else {
            ...
            python.on('close', (code) => {
                if (code === 0) {
                    try {
                        ...
                        reject(new Error(`Failed to parse text analysis results: ${parseError instanceof Error ? parseError.message : String(parseError)}`));
                    }
                }
                else {

...
export class AIAnalysisService {
  ...
  private relationshipPatterns = [
    {
      pattern:
        /(\w+(?:\s+\w+)*)\s+(works\s+(?:at|for)|employed\s+(?:by|at))\s+(\w+(?:\s+\w+)*)/gi,
      type: 'EMPLOYED_BY',
    },
    {
      pattern:
    ...
    {
      pattern:
        /(\w+(?:\s+\w+)*)\s+(met\s+with|meeting\s+with|talked\s+to)\s+(\w+(?:\s+\w+)*)/gi,
      type: 'MET_WITH',
    },
    {
      pattern:
        /(\w+(?:\s+\w+)*)\s+(located\s+in|based\s+in|from)\s+(\w+(?:\s+\w+)*)/gi,
      type: 'LOCATED_IN',
    },
    {
  ...
  public async analyzeSentiment(text: string): Promise<{
    ...
    if (positiveScore > negativeScore) {
      ...
      confidence = Math.min(
        0.9,
        0.5 + ((positiveScore - negativeScore) / words.length) * 2,
      );
    } else if (negativeScore > positiveScore) {
      sentiment = 'negative';
      confidence = Math.min(
        0.9,
        0.5 + ((negativeScore - positiveScore) / words.length) * 2,
      );
    }
   ...
   */
  private findCooccurrenceContext(
    entity1: string,
    entity2: string,
    text: string,
  ): string | null {

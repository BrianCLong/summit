...
class ExternalVerifier {
    static instance;
    verificationLog = new Map();
    constructor() {
        logger_js_1.default.info({ message: 'External Verifier initialized' });
    }
    static getInstance() {
    ...
    async verifyTrace(request) {
        ...
        try {
            ...
            if (request.verificationLevel === 'standard' || request.verificationLevel === 'comprehensive') {
                result.checks.reproducibility = await this.verifyReproducibility(request.trace, request.tolerances?.scoreTolerance || 0.001);
            ...
            if (request.verificationLevel === 'comprehensive') {
                result.checks.parameterConsistency = await this.verifyParameterConsistency(request.trace);
    ...
    // ============================================================================
    async verifyDigestIntegrity(trace) {
        try {
    ...
    async verifyReproducibility(trace, tolerance) {
        try {
            ...
            const riskResult = trace.modelOutput;
    ...
    async verifyParameterConsistency(trace) {
        try {
            const riskResult = trace.modelOutput;
            ...
            for (const contrib of riskResult.contributions) {
                ...
                if (!saliency) {
                    ...
                    if (contributionDiff > 0.0001) {
                        inconsistencies.push(`Saliency contribution mismatch for ${contrib.feature}: ` +
            ...
            if (expectedBand !== riskResult.band) {
                inconsistencies.push(`Risk band mismatch: score ${score.toFixed(3)} should be '${expectedBand}', ` +
    ...
    verifyModelMetadata(metadata) {
        ...
        if (!metadata.modelVersion)
            issues.push('Missing model version');
        if (!metadata.modelType)
            issues.push('Missing model type');
     ...
     */
    async performParameterSweep(baseFeatures, featureToSweep, minValue, maxValue, steps, computeFunction) {
        logger_js_1.default.info({
        ...
        const explanation = this.generateSweepExplanation(featureToSweep, minValue, maxValue, allMonotonic, sensitivity, results);
        logger_js_1.default.info({
    ...
    generateSweepExplanation(feature, min, max, monotonic, sensitivity, results) {
    ...
    calculateConfidence(checks) {
        ...
        let confidence = 0;
        if (checks.digestIntegrity.passed)
            confidence += weights.digestIntegrity;
        if (checks.signatureValidity.passed)
            confidence += weights.signatureValidity;
        if (checks.reproducibility.passed)
            confidence += weights.reproducibility;
        if (checks.parameterConsistency.passed)
            confidence += weights.parameterConsistency;
        if (checks.modelMetadata.passed)
            confidence += weights.modelMetadata;
        return confidence;
    }
    determineOverallValidity(checks, level) {
    ...
    getVerificationHistory(traceId) {
        return Array.from(this.verificationLog.values()).filter(result => result.checks.digestIntegrity.evidence?.expectedDigest?.includes(traceId));
    }
    clearVerificationLog() {
        this.verificationLog.clear();
        logger_js_1.default.info({ message: 'Verification log cleared' });
    }
}
exports.ExternalVerifier = ExternalVerifier;
// Export singleton instance
exports.externalVerifier = ExternalVerifier.getInstance();

...
export class DetectionContentPackV6Service {
    ...
    try {
      ...
      const contentPack = await this.prisma.contentPack.create({
        data: {
          ...
          description: 'Expanded coverage for credential access, lateral movement, persistence, and C2',
          status: 'deploying',
          deployedAt: new Date(),
        },
      });
      ...
      const metrics: ContentPackMetrics = {
        ...
        attackCoverage: {
          techniques: Object.keys(attackCoverage).reduce((acc, tactic) => 
            acc + Object.keys(attackCoverage[tactic]).length, 0),
      ...
      this.logger.info('Detection Content Pack v6 deployed successfully', metrics);
      return metrics;

    } catch (error) {
      this.logger.error('Failed to deploy Detection Content Pack v6', { error });
      throw error;
    }
  }
  ...
  private async deployCredentialAccessRules(): Promise<DetectionRule[]> {
    const rules: Omit<DetectionRule, 'id' | 'createdAt' | 'updatedAt'>[] = [
      ...
      },
      {
        ...
        description: 'Detects Kerberoasting attacks via service ticket requests',
        category: 'credential_access',
    ...
    const rules: Omit<DetectionRule, 'id' | 'createdAt' | 'updatedAt'>[] = [
      {
        ...
        metadata: {
          ...
          tuningNotes: 'Reduced false positives by excluding legitimate admin tools',
        },
        tests: [
          {
            ...
            testData: {
              events: [{
                ProcessCommandLine: 'wmic /node:192.168.1.100 process call create "cmd.exe"',
                DeviceName: 'ADMIN-PC',
      ...
      },
      {
        ...
        description: 'Detects PSExec-style lateral movement via service creation',
        category: 'lateral_movement',
        subcategory: 'remote_services',
        attackTechniques: ['T1021.002'],
        severity: 'medium',
    ...
    const rules: Omit<DetectionRule, 'id' | 'createdAt' | 'updatedAt'>[] = [
      {
        ...
        description: 'Detects potential Golden Ticket attacks via anomalous Kerberos tickets',
  ...
  private async createDetectionRules(rules: Omit<DetectionRule, 'id' | 'createdAt' | 'updatedAt'>[]): Promise<DetectionRule[]> {
    ...
    for (const rule of rules) {
      ...
      this.logger.info('Detection rule created', { ruleId: created.id, name: rule.name });
    }
      ...
      for (const test of rule.tests) {
        ...
        try {
          ...
          if (result === test.expectedResult) {
            ...
            failedTests++;
            await this.updateTestStatus(test.id, 'failed', `Expected ${test.expectedResult}, got ${result}`);
          }
        } catch (error) {
          failedTests++;
  ...
  private async runDetectionTest(rule: DetectionRule, test: DetectionTest): Promise<string> {
    // Mock implementation - in reality would execute the query against test data
    this.logger.info('Running detection test', { ruleId: rule.id, testId: test.id });
  ...
  private async updateTestStatus(testId: string, status: 'passed' | 'failed', error?: string): Promise<void> {
  ...
  private async generateAttackCoverage(rules: DetectionRule[]): Promise<AttackCoverageMap> {
  ...
  private async calculateFalsePositiveReduction(rules: DetectionRule[]): Promise<number> {

...
type MemoryBlockRow = {
  block_id: string;
  tenant_id: string;
  owner_type: MemoryBlockOwnerType;
  owner_id: string;
  label: string;
  value: string;
...
const mapBlockRow = (row: MemoryBlockRow): MemoryBlock => ({
...
export class MemoryBlockService {
  constructor(
    private readonly pool: ManagedPostgresPool = getPostgresPool(),
  ) {}

  private assertWithinLimit(value: string, sizeLimit: number) {
    if (sizeLimit <= 0) {
      throw new Error('Size limit must be positive');
    }
  ...
  private async assertWriteAccess(
    ...
    if (block.ownerId === actorId && block.ownerType === actorType) return;
  ...
  async createBlock(input: CreateMemoryBlockInput): Promise<MemoryBlock> {
    ...
    const result = await this.pool.query<MemoryBlockRow>(
  ...
  async getBlock(
    ...
    const queryText = tenantId
      ? `SELECT block_id, tenant_id, owner_type, owner_id, label, value, size_limit, description, is_read_only, scope, created_at, updated_at
         FROM memory_blocks WHERE block_id = $1 AND tenant_id = $2`
      : `SELECT block_id, tenant_id, owner_type, owner_id, label, value, size_limit, description, is_read_only, scope, created_at, updated_at
         FROM memory_blocks WHERE block_id = $1`;
    const params = tenantId ? [blockId, tenantId] : [blockId];
    const result = await this.pool.query<MemoryBlockRow>(queryText, params);
  ...
  private truncateValue(value: string, limit: number): string {
    if (limit <= 0) return '';
  ...
  async replaceBlockValue(input: ReplaceMemoryBlockInput): Promise<MemoryBlock> {
    const block = await this.getBlock(input.blockId, input.tenantId);
    if (!block) {
    ...
    await this.assertWriteAccess(block, input.actorId, input.actorType);
  ...
  async compilePromptSection(
    ...
    const normalized = blocks.map((block) => ({
      ...block,
      value: this.truncateValue(block.value, block.sizeLimit),
    }));

    return normalized
      .sort((a, b) =>
        `${a.label}-${a.blockId}`.localeCompare(`${b.label}-${b.blockId}`),
      )
  ...
  async reflectAndUpdateBlocks(options: {
    ...
    for (const block of blocks) {
      try {
        const proposedValue = this.buildReflection(
          block.value,
          options.delta,
          block.sizeLimit,
        );
        ...
        logger.warn(
          ...
          'Skipping memory block reflection update',
        );
      }
    }
  ...
  private buildReflection(existing: string, delta: string, limit: number): string {
    ...
    const combined = `${existing}\n\nReflection:${
      normalizedDelta ? ` ${normalizedDelta}` : ' none'
    }`;

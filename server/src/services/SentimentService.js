      ...
      Object.keys(emotions).forEach((emotion) => {
        const matches = tokens.filter((token) =>
          emotions[emotion].some(
            (emotionWord) => token.includes(emotionWord) || emotionWord.includes(token),
          ),
        );
  ...
  analyzeSentimentTrends(timeSeriesData) {
    try {
      ...
      for (let i = windowSize - 1; i < sentiments.length; i++) {
        ...
        const avgScore =
          window.reduce((sum, item) => sum + item.sentiment.score, 0) / window.length;
      ...
      const recentAverage =
        movingAverages.slice(-3).reduce((sum, item) => sum + item.movingAverage, 0) / 3;
      const earlierAverage =
        movingAverages.slice(0, 3).reduce((sum, item) => sum + item.movingAverage, 0) / 3;

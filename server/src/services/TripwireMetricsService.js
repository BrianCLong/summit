...
exports.tripwireMetricsService = exports.TripwireMetricsService = void 0;
...
// ============================================================================
class TripwireMetricsService {
    circuitBreaker;
    redis = null;
    postgres = null;
    // Thresholds
    TARGET_VIOLATION_RATE = 0.01; // 1% or less is acceptable
    constructor() {
        this.circuitBreaker = new CircuitBreaker_js_1.CircuitBreaker('TripwireMetricsService', {
            failureThreshold: 5,
            resetTimeout: 60000,
        });
    ...
    async initializeConnections() {
        try {
            ...
            logger_js_1.default.error('Failed to initialize TripwireMetricsService connections', {
     ...
     */
    async calculateMetrics(tenantId, metricDate, periodType) {
        try {
            return await this.circuitBreaker.execute(async () => {
                logger_js_1.default.info('Calculating tripwire metrics', { tenantId, metricDate, periodType });
                const { periodStart, periodEnd } = this.getPeriodBounds(metricDate, periodType);
                // Calculate current period metrics
                const currentMetrics = await this.calculatePeriodMetrics(tenantId, periodStart, periodEnd);
                ...
                logger_js_1.default.info('Tripwire metrics calculated', {
                    tenantId,
            ...
            logger_js_1.default.error('Failed to calculate tripwire metrics', {
                error: error instanceof Error ? error.message : String(error),
                tenantId
            });
            throw error;
     ...
     */
    async calculatePeriodMetrics(tenantId, periodStart, periodEnd) {
        if (!this.postgres) {
    ...
    `;
        ...
        return {
            ...
            reasonsProvided,
            reasonComplianceRate: queriesRequiringReason > 0 ? reasonsProvided / queriesRequiringReason : 1.0,
        };
    }
    /**
     * Calculate daily metrics for all tenants (scheduled job)
     */
    async calculateDailyMetricsForAllTenants() {
        ...
        for (const tenantId of tenants) {
            try {
                ...
                logger_js_1.default.error('Failed to calculate daily metrics for tenant', {
                    error: error instanceof Error ? error.message : String(error),
                    tenantId
                });
        ...
        logger_js_1.default.info('Daily metrics calculated for all tenants', { count: tenants.length });
    }
    /**
     * Calculate weekly metrics (scheduled job)
     */
    async calculateWeeklyMetrics(tenantId) {
     ...
     */
    async calculateMonthlyMetrics(tenantId) {
     ...
     */
    async analyzeTrends(tenantId, periodType, lookbackPeriods = 30) {
        ...
        const percentageImprovement = initialViolationRate > 0 ? (totalReduction / initialViolationRate) * 100 : 0;
        ...
        const trendAnalysis = this.calculateLinearTrend(metrics.map((m, i) => ({ x: i, y: m.violationRate })));
        const isImproving = trendAnalysis.slope < 0; // Negative slope = improving
        // Forecast next period
        const projectedViolationRate = Math.max(0, trendAnalysis.slope * metrics.length + trendAnalysis.intercept);
        // Calculate time to zero violations
        let timeToZeroViolations;
        if (isImproving && trendAnalysis.slope < 0) {
            const periodsToTarget = (this.TARGET_VIOLATION_RATE - currentViolationRate) / trendAnalysis.slope;
     ...
     */
    async getImprovementDashboard(tenantId) {
     ...
     */
    async loadMetric(tenantId, metricDate, periodType) {
        if (!this.postgres) {
            throw new Error('PostgreSQL not initialized');
        }
        const query = `
     ...
     */
    async getMetrics(tenantId, periodType, startDate, endDate) {
        if (!this.postgres) {
            throw new Error('PostgreSQL not initialized');
        }
        const query = `
     ...
     */
    getPreviousPeriod(date, periodType) {
     ...
     */
    getStartDate(endDate, periodType, lookbackPeriods) {
...
exports.TripwireMetricsService = TripwireMetricsService;
// Singleton instance
exports.tripwireMetricsService = new TripwireMetricsService();

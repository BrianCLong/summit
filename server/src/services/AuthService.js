...
const securityService_js_1 = require("./securityService.js");
/**
...
class AuthService {
     ...
     */
    constructor(securityService = securityService_js_1.securityService) {
        this.securityService = securityService;
        // Lazy initialized via getter
        try {
            this.metrics = new metrics_js_1.PrometheusMetrics('summit_auth');
            this.metrics.createHistogram('user_registration_duration_seconds', 'Time taken to register a user', { buckets: [0.1, 0.5, 1, 2, 5] });
            this.metrics.createCounter('user_logins_total', 'Total number of user login attempts', ['tenant_id', 'result']);
            this.metrics.createCounter('user_logouts_total', 'Total number of user logouts', ['tenant_id']);
            this.metrics.createHistogram('user_session_duration_seconds', 'Duration of user sessions', { buckets: [60, 300, 900, 1800, 3600] });
        }
        catch (e) {
    ...
    async register(userData) {
        ...
        try {
            ...
            if (process.env.GA_ENROLLMENT_BYPASS !== 'true') {
                const enrollmentCheck = await GAEnrollmentService_js_1.default.checkUserEnrollmentEligibility(userData.email);
    ...
    async externalLogin(email) {
        ...
        try {
            ...
            logger_js_1.default.error('Error logging in user via SSO:', error);
    ...
    async refreshAccessToken(refreshToken) {
        const client = await this.pool.connect();
        try {
            const getScopes = (role) => ROLE_SCOPES[role.toUpperCase()] || [];
            const result = await this.securityService.refreshDbToken(refreshToken, client, getScopes);
            if (!result)
                return null;
            logger_js_1.default.info('Token successfully refreshed with rotation', {
                userId: result.userId,
            });
            return { token: result.token, refreshToken: result.refreshToken };
        }
        catch (error) {
            logger_js_1.default.error('Error refreshing token:', error);
            return null;
        }
        finally {
            client.release();
    ...
    async logout(userId, currentToken) {
        ...
        try {
            ...
            const result = await client.query('UPDATE user_sessions SET is_revoked = true WHERE user_id = $1 RETURNING (SELECT tenant_id, last_login FROM users WHERE id = $1)', [userId]);
            ...
            await client.query('COMMIT');
            logger_js_1.default.info('User logged out successfully', { userId });
            return true;
        }
        catch (error) {
            await client.query('ROLLBACK');
            logger_js_1.default.error('Error during logout:', error);
            return false;
        }
        finally {
            client.release();
    ...
    hasScope(user, scope) {
        if (!user || !user.scopes)
            return false;
        return (0, scopeGuard_js_1.checkScope)(user.scopes, scope);
    }
    /**

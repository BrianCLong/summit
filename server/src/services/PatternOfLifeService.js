...
exports.PatternOfLifeService = void 0;
class PatternOfLifeService {
    /**
    ...
    detectPeriodicity(events) {
        ...
        for (const [type, typeEvents] of eventsByType.entries()) {
            ...
            if (meanInterval < 1)
                continue; // Ignore very high frequency noise for now
            const variance = intervals.reduce((a, b) => a + Math.pow(b - meanInterval, 2), 0) /
                intervals.length;
            ...
            if (cv < 0.2) {
                ...
                patterns.push({
                    ...
                    type: 'periodicity',
                    confidence,
                    description: `Regular activity '${type}' detected every ${meanInterval.toFixed(1)} seconds (+/- ${stdDev.toFixed(1)}s)`,
                    metadata: {
                        actionType: type,
    ...
    detectSequences(events) {
        ...
        for (let n = 2; n <= 5; n++) {
            if (actionSequence.length < n * 2)
                break; // Need at least 2 occurrences
            ...
            for (const [gram, count] of ngrams.entries()) {
                ...
                if (count >= 3) {
                    ...
                    patterns.push({
                        ...
                        type: 'sequence',
                        confidence,
                        description: `Repeated sequence detected: ${sequenceParts.join(' -> ')} (${count} times)`,
                        metadata: {
...
exports.PatternOfLifeService = PatternOfLifeService;

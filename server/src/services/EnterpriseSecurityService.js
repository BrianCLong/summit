...
class EnterpriseSecurityService extends EventEmitter {
  ...
  initializeRBAC() {
    ...
    const roles = [
...

      {
        ...
        permissions: ['INVESTIGATION_READ', 'ENTITY_READ', 'ANALYTICS_VIEW_RESULTS'],
        dataClassifications: ['UNCLASSIFIED', 'INTERNAL'],
  ...
  async createSession(user, clientInfo) {
    ...
    const userRole = this.roles.get(user.role);
    const activeSessions = Array.from(this.sessionManager.values()).filter(
      (s) => s.userId === user.id && s.expiresAt > new Date(),
    );
  ...
  async logSecurityEvent(event) {
    const auditEvent = {
      ...
      outcome:
        event.outcome ||
        (event.type.includes('DENIED') || event.type.includes('FAILED') ? 'FAILURE' : 'SUCCESS'),
  ...
  getSecurityMetrics() {
    const successRate =
      this.securityMetrics.totalLogins > 0
        ? (
            (this.securityMetrics.successfulLogins / this.securityMetrics.totalLogins) *
            100
          ).toFixed(2)
        : '0';

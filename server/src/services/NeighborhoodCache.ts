...
export class NeighborhoodCache {
  ...
  async getNeighborhood(
    ...
    const cacheKey = this.generateNeighborhoodKey(
      nodeId,
      tenantContext,
      maxDepth,
      maxNeighbors,
  ...
  async clearTenantCache(tenantContext: TenantContext): Promise<number> {
    try {
      const pattern = TenantValidator.getTenantCacheKey(
        `${this.keyPrefix}:*`,
        tenantContext,
      );
      const keys = await this.redis.keys(pattern);
      ...
      if (keys.length > 0) {
        ...
        this.stats.invalidations += deleted;
        logger.info(
          `Cleared ${deleted} cache entries for tenant ${tenantContext.tenantId}`,
        );
        return deleted;
      }

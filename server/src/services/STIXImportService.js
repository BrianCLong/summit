...
class STIXImportService {
  ...
  async fetchTaxiiObjects(client, job) {
    ...
    const response = await client.get(
      `/collections/${job.collectionId}/objects/`,
      { params },
    );
  ...
  async processStixBatch(batch, job, session) {
    ...
    for (const sdo of sdos) {
      try {
        const confidence =
          typeof sdo.confidence === 'number' ? sdo.confidence / 100 : 1.0;
    ...
    for (const sro of sros) {
      try {
        const confidence =
          typeof sro.confidence === 'number' ? sro.confidence / 100 : 1.0;
  ...
  async processStixDomainObject(sdo, job, session, confidence = 1.0) {
    ...
    const provenance =
      job.taxiiUrl || job.bundlePath || sdo.created_by_ref || 'STIX';
  ...
  async processStixRelationshipObject(sro, job, session, confidence = 1.0) {
    const relationshipType = this.mapStixRelationshipType(
      sro.relationship_type || sro.type,
    );
    const provenance =
      job.taxiiUrl || job.bundlePath || sro.created_by_ref || 'STIX';
  ...
  mapStixToDomain(stixObj) {
    ...
    switch (stixObj.type) {
      ...
      case 'vulnerability':
        ...
        properties.cvss_score = this.extractCvssScore(
  ...
  extractCvssScore(externalRefs) {
    ...
    const cvssRef = externalRefs.find(
      (ref) =>
        ref.source_name?.includes('cvss') || ref.description?.includes('CVSS'),
    );

...
exports.graphCore = exports.GraphCoreService = void 0;
// @ts-nocheck
...
class GraphCoreService {
     ...
     */
    async saveEntity(tenantId, entityType, data, policyLabels, actorId) {
        const session = neo4j_js_1.neo4jDriver.session();
     ...
     */
    async getEntity(tenantId, id, asOf) {
        const session = neo4j_js_1.neo4jDriver.session();
        try {
            let query = `
        MATCH (e:CanonicalEntity {id: $id, tenantId: $tenantId})
     ...
     * Also bitemporal.
     */
    async createRelationship(tenantId, fromId, toId, relationType, properties = {}, actorId) {
        const session = neo4j_js_1.neo4jDriver.session();
        const now = new Date();
        try {
            const result = await session.executeWrite(async (tx) => {
     ...
     */
    async linkEvidenceToClaim(tenantId, claimId, evidenceId, weight, description, actorId) {
        return this.createRelationship(tenantId, evidenceId, // Evidence supports Claim? Or Claim supported by Evidence? Usually Evidence -> SUPPORTS -> Claim
        claimId, 'SUPPORTS', { weight, description }, actorId);
    }
    parseNodeProperties(props) {
        const p = { ...props };
...
exports.GraphCoreService = GraphCoreService;
exports.graphCore = GraphCoreService.getInstance();

...
export class HybridEntityResolutionService {
  ...
  async resolveEntitiesPair(
    ...
    try {
      ...
      if (deterministicResult.confidence > 0.95) {
        log.info(
          { traceId, method: 'deterministic', score: deterministicResult.score },
          'er_match_deterministic',
        );
        return {
          ...deterministicResult,
          traceId,
      ...
      const threshold =
        GA_PRECISION_THRESHOLDS[entityType as keyof typeof GA_PRECISION_THRESHOLDS] || 0.85;

      if (mlResult.confidence > threshold) {
        log.info(
          { traceId, method: 'ml', score: mlResult.score, confidence: mlResult.confidence },
          'er_match_ml',
        );
        return {
          ...mlResult,
      ...
      if (mlResult.confidence > 0.5 && mlResult.confidence < threshold) {
        ...
        if (clusterResult.confidence > threshold - 0.05) {
          ...
          log.info(
            { traceId, method: 'clustering', score: clusterResult.score },
            'er_match_clustering',
          );
          return {
            ...clusterResult,
  ...
  private async deterministicMatch(entityA: any, entityB: any): Promise<ERServiceResult> {
    ...
    const hasSecondaryMatch =
      (entityA.phone && entityB.phone && entityA.phone === entityB.phone) ||
      (entityA.address &&
        entityB.address &&
  ...
  private async clusterBasedMatch(
    entityA: any,
    entityB: any,
    entityType: string,
  ): Promise<ERServiceResult> {
  ...
  private async hybridDecision(
    ...
    const combinedScore =
      deterministicResult.score * weights.deterministic + mlResult.score * weights.ml;

    const combinedConfidence = Math.max(
      deterministicResult.confidence || 0,

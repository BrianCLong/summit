...
function validateUsageAggregates(usage: UsageAggregates): void {
  const entries: Array<[string, number]> = [
    ['computeMs', usage.computeMs],
    ['storageBytesAverage', usage.storageBytesAverage],
    ['receiptSigningOps', usage.receiptSigningOps],
    ['periodDays', usage.periodDays],
  ];

  for (const [label, value] of entries) {
    if (!Number.isFinite(value) || value < 0) {
      throw new Error(`Invalid usage aggregate: ${label}`);
    }
  }

  if (usage.periodDays === 0) {
    throw new Error('Invalid usage aggregate: periodDays');
  }
}

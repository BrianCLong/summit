...
exports.costAnomalyDetection = exports.CostAnomalyDetectionService = void 0;
const pino_1 = __importDefault(require("pino"));
const postgres_js_1 = require("../db/postgres.js");
const logger = pino_1.default({ name: 'cost-anomaly-detection' });
class CostAnomalyDetectionService {
    ...
    async loadBudgetThresholds() {
        try {
            ...
            logger.info({ count: this.budgetThresholds.size }, 'Budget thresholds loaded');
        }
        catch (error) {
     ...
     */
    async setBudgetThreshold(threshold) {
        await this.ensureInitialized();
        try {
            await this.db.write(`INSERT INTO budget_thresholds
     ...
     */
    async checkBudgetCompliance(dimension, dimensionValue) {
        await this.ensureInitialized();
        ...
        try {
            ...
            const [dailyResult, monthlyResult] = await Promise.all([
                this.db.read(`SELECT COALESCE(SUM(rc.cost), 0) as total
           FROM resource_costs rc
           JOIN resource_tags rt ON rc.resource_id = rt.resource_id
           WHERE rt.tag_category = $1 AND rt.tag_value = $2
             AND rc.timestamp >= $3`, [dimension, dimensionValue, startOfDay]),
                this.db.read(`SELECT COALESCE(SUM(rc.cost), 0) as total
           FROM resource_costs rc
           JOIN resource_tags rt ON rc.resource_id = rt.resource_id
           WHERE rt.tag_category = $1 AND rt.tag_value = $2
             AND rc.timestamp >= $3`, [dimension, dimensionValue, startOfMonth]),
            ]);
     ...
     */
    async detectCostAnomalies(dimension, dimensionValue, config = {}) {
        await this.ensureInitialized();
        ...
        try {
            ...
            const result = await this.db.read(`SELECT DATE(rc.timestamp) as date, SUM(rc.cost) as daily_cost
         FROM resource_costs rc
         JOIN resource_tags rt ON rc.resource_id = rt.resource_id
         WHERE rt.tag_category = $1 AND rt.tag_value = $2
           AND rc.timestamp >= $3
         GROUP BY DATE(rc.timestamp)
         ORDER BY date`, [dimension, dimensionValue, lookbackStart]);
            if (result.rows.length < detectionConfig.minDataPoints) {
                logger.debug({ dimension, dimensionValue, dataPoints: result.rows.length }, 'Insufficient data for anomaly detection');
                return [];
            }
            ...
            const todayResult = await this.db.read(`SELECT COALESCE(SUM(rc.cost), 0) as today_cost
         FROM resource_costs rc
         JOIN resource_tags rt ON rc.resource_id = rt.resource_id
         WHERE rt.tag_category = $1 AND rt.tag_value = $2
           AND rc.timestamp >= $3`, [dimension, dimensionValue, todayStart]);
    ...
    async detectWaste() {
        ...
        try {
            ...
            const result = await this.db.read(`SELECT rc.resource_id, rc.resource_type,
    ...
    async saveAlert(alert) {
        ...
        try {
            await this.db.write(`INSERT INTO cost_alerts
     ...
     */
    async updateAnomalyBaseline(dimension, dimensionValue, baseline, stdDev, sampleCount) {
        try {
            await this.db.write(`INSERT INTO anomaly_baselines
         ...
         DO UPDATE SET
           ...
           calculated_at = NOW()`, [dimension, dimensionValue, baseline, stdDev, sampleCount]);
        }
        catch (error) {
     ...
     * Get recent alerts
     */
    async getRecentAlerts(limit = 50, severity, acknowledged) {
        await this.ensureInitialized();
        try {
            let query = `
...
exports.CostAnomalyDetectionService = CostAnomalyDetectionService;
// Singleton instance
exports.costAnomalyDetection = new CostAnomalyDetectionService();

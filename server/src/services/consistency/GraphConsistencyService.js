...
exports.GraphConsistencyService = void 0;
const logger_js_1 = __importDefault(require("../../config/logger.js"));
class GraphConsistencyService {
    pg;
    neo4j;
    logger = logger_js_1.default.child({ name: 'GraphConsistencyService' });
     ...
     */
    async checkInvestigation(investigationId, tenantId) {
        const session = this.neo4j.session();
        try {
            // 1. Get stats from Postgres
            // We need IDs to identify missing ones
            const { rows: pgEntities } = await this.pg.query(`SELECT id FROM entities
         WHERE tenant_id = $1 AND props->>'investigationId' = $2`, [tenantId, investigationId]);
            const pgEntityIds = new Set(pgEntities.map((r) => r.id));
            const { rows: pgRelationships } = await this.pg.query(`SELECT id FROM relationships
         WHERE tenant_id = $1 AND props->>'investigationId' = $2`, [tenantId, investigationId]);
            const pgRelIds = new Set(pgRelationships.map((r) => r.id));
            // 2. Get stats from Neo4j
            // Assuming investigationId is indexed on nodes
            const neo4jEntitiesResult = await session.run(`MATCH (n:Entity)
         WHERE n.investigationId = $investigationId
         RETURN n.id as id`, { investigationId });
            const neo4jEntityIds = new Set(neo4jEntitiesResult.records.map((r) => r.get('id')));
            const neo4jRelsResult = await session.run(`MATCH ()-[r:RELATIONSHIP]->()
         WHERE r.investigationId = $investigationId
         RETURN r.id as id`, { investigationId });
            const neo4jRelIds = new Set(neo4jRelsResult.records.map((r) => r.get('id')));
            ...
            // 4. Calculate Drift
            const postgresEntitiesMissingInNeo4j = [...pgEntityIds].filter((id) => !neo4jEntityIds.has(id));
            const neo4jEntitiesMissingInPostgres = [...neo4jEntityIds].filter((id) => !pgEntityIds.has(id));
            const postgresRelationshipsMissingInNeo4j = [...pgRelIds].filter((id) => !neo4jRelIds.has(id));
            const neo4jRelationshipsMissingInPostgres = [...neo4jRelIds].filter((id) => !pgRelIds.has(id));
            const isClean = postgresEntitiesMissingInNeo4j.length === 0 &&
     ...
     */
    async repairInvestigation(investigationId, tenantId, report) {
        if (!report) {
        ...
        try {
            ...
            if (report.postgresEntitiesMissingInNeo4j.length > 0) {
                const { rows: entitiesToSync } = await this.pg.query(`SELECT * FROM entities WHERE id = ANY($1)`, [report.postgresEntitiesMissingInNeo4j]);
            ...
            if (report.postgresRelationshipsMissingInNeo4j.length > 0) {
                const { rows: relsToSync } = await this.pg.query(`SELECT * FROM relationships WHERE id = ANY($1)`, [report.postgresRelationshipsMissingInNeo4j]);
    ...
    async runGlobalCheck() {
        const { rows: investigations } = await this.pg.query(`SELECT id, tenant_id FROM investigations WHERE status = 'active'`);
        const reports = [];
...
exports.GraphConsistencyService = GraphConsistencyService;

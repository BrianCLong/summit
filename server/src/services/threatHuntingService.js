...
exports.threatHuntingService = exports.ThreatHuntingService = void 0;
// @ts-nocheck
const events_1 = require("events");
const CacheService_js_1 = require("./CacheService.js");
class ThreatHuntingService extends events_1.EventEmitter {
    iocs = new Map();
    detections = new Map();
    hunts = new Map();
    feedSources = new Map();
    ...
    async createIOC(iocData, createdBy) {
        ...
        await CacheService_js_1.cacheService.set(`ioc:${iocId}`, ioc, 86400); // Cache for 24 hours
    ...
    async createThreatHunt(huntData, createdBy) {
        ...
        await CacheService_js_1.cacheService.set(`hunt:${huntId}`, hunt, 86400 * 30); // Cache for 30 days
    ...
    async executeHuntQuery(huntId, queryId, executedBy) {
        ...
        this.hunts.set(huntId, hunt);
        await CacheService_js_1.cacheService.set(`hunt:${huntId}`, hunt, 86400 * 30);
     ...
     */
    getThreatScore(entityId) {
        const actor = this.threatActors.get(entityId);
        if (actor) {
            let score = 50;
            if (actor.sophistication === 'expert')
                score += 30;
            if (actor.threat_actor_types.includes('nation-state'))
                score += 15;
            return score;
        }
    ...
    cleanupExpiredIOCs() {
        ...
        for (const [iocId, ioc] of this.iocs.entries()) {
            if (ioc.expiresAt && new Date(ioc.expiresAt).getTime() < now) {
                ...
                CacheService_js_1.cacheService.delete(`ioc:${iocId}`);
                cleanedUp++;
            }
        }
        if (cleanedUp > 0) {
...
exports.threatHuntingService = new ThreatHuntingService();

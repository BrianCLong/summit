...
exports.DetectionContentPackV5 = void 0;
class DetectionContentPackV5 {
    prisma;
    logger;
    PACK_VERSION = 'v5.0.0';
    TARGET_FP_RATE = 0.03; // 3% max FP rate
     ...
     */
    async tuneLateralMovementRules() {
     ...
     */
    async addPersistenceTechniques() {
     ...
     */
    async validateDetectionRules(rules) {
        let passed = 0;
        let failed = 0;
        const results = [];
     ...
     */
    async getStagingFalsePositiveRate(ruleIds) {
        const ruleMetrics = [];
        let totalAlerts = 0;
    ...
    async deployDetectionRule(rule) {
        const detectionRule = {
    ...
    async getBaselineMetrics(category) {
    ...
    async getPostTuningMetrics(category) {
    ...
    async applyRuleTuning(ruleId, improvements) {
    ...
    async updateTechniqueDocumentation(technique) {
        const documentation = {
            technique_id: technique.technique_id,
    ...
    async generateCoverageReport(techniques) {
        const report = {
            pack_version: this.PACK_VERSION,
            generated_at: new Date(),
            total_techniques: techniques.length,
            average_coverage: techniques.reduce((sum, t) => sum + t.detection_coverage, 0) /
                techniques.length,
    ...
    groupCoverageByTactic(techniques) {
        const tactics = {};
    ...
    async runTestCase(rule, testCase) {
    ...
    async getRuleMetrics(ruleId, environment) {
...
exports.DetectionContentPackV5 = DetectionContentPackV5;

...
class GNNService {
      ...
      async () => {
        try {
          const payload = {
            ...
            task_mode: taskMode,
            num_epochs:
              options.numEpochs || (taskMode === 'train' ? 50 : undefined),
          };

          const response = await fetch(
            `${this.mlServiceUrl}/gnn/node_classification`,
            {
              method: 'POST',
              headers: {
          ...
          if (!response.ok) {
            throw new Error(
              `GNN node classification failed: ${response.statusText}`,
            );
          }
    ...
    return trackGraphOperation(
      ...
      async () => {
        try {
          ...
          const response = await fetch(
            `${this.mlServiceUrl}/gnn/link_prediction`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
          ...
          if (!response.ok) {
            throw new Error(
              `GNN link prediction failed: ${response.statusText}`,
            );
          }
      ...
      async () => {
        try {
          const payload = {
            ...
            task_mode: taskMode,
            num_epochs:
              options.numEpochs || (taskMode === 'train' ? 100 : undefined),
          };

          const response = await fetch(
            `${this.mlServiceUrl}/gnn/graph_classification`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
          ...
          if (!response.ok) {
            throw new Error(
              `GNN graph classification failed: ${response.statusText}`,
            );
          }
    ...
    return trackGraphOperation(
      ...
      async () => {
        try {
          ...
          const response = await fetch(
            `${this.mlServiceUrl}/gnn/anomaly_detection`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
          ...
          if (!response.ok) {
            throw new Error(
              `GNN anomaly detection failed: ${response.statusText}`,
            );
          }
    ...
    return trackGraphOperation(
      ...
      async () => {
        try {
          ...
          const response = await fetch(
            `${this.mlServiceUrl}/gnn/generate_embeddings`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
          ...
          if (!response.ok) {
            throw new Error(
              `GNN embedding generation failed: ${response.statusText}`,
            );
          }
  ...
  async getModelInfo(modelName, options = {}) {
    try {
      const response = await fetch(
        `${this.mlServiceUrl}/gnn/models/${modelName}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${options.token || ''}`,

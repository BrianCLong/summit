...
exports.SupernodeQueryOptimizer = void 0;
exports.createSupernodeOptimizer = createSupernodeOptimizer;
const logger_js_1 = __importDefault(require("../utils/logger.js"));
const serviceLogger = logger_js_1.default.child({ name: 'SupernodeQueryOptimizer' });
 ...
 */
class SupernodeQueryOptimizer {
    neo4j;
    redis;
    config;
    supernodeCache;
    constructor(neo4jDriver, redisClient, config = {}) {
     ...
     */
    async isSupernodeById(entityId) {
     ...
     */
    async getSupernodeInfo(entityId) {
        // Check cache first
        ...
        `, { entityId });
            ...
            if (info.connectionCount >= this.config.supernodeThreshold) {
                ...
                if (this.redis && this.config.cachePrecomputedStats) {
                    await this.redis.setex(`supernode:${entityId}`, this.config.statsCacheTTL, JSON.stringify(info));
                }
            }
            return info;
        }
     ...
     */
    async detectSupernodes(entityIds) {
        ...
        `, { entityIds: missingIds, threshold: this.config.supernodeThreshold });
            for (const record of result.records) {
                ...
                if (this.redis && this.config.cachePrecomputedStats) {
                    await this.redis.setex(`supernode:${info.entityId}`, this.config.statsCacheTTL, JSON.stringify(info));
                }
            }
            return supernodes;
        }
    ...
    async optimizeTraversalQuery(params) {
        const { startEntityIds, maxHops, relationshipTypes, limit = this.config.paginationLimit, tenantId, } = params;
        ...
        const maxSupernodeConnections = supernodes.reduce((max, s) => Math.max(max, s.connectionCount), 0);
        // Estimate query cost
        ...
        return {
            ...
            estimatedCost,
            warning,
            pagination: strategy === 'paginated' || strategy === 'sampled'
                ? {
                    limit,
                    offset: 0,
                    hasMore: true,
     ...
     */
    buildOptimizedQuery(params) {
        const { startEntityIds, maxHops, relationshipTypes, limit, tenantId, strategy, samplingRate, } = params;
    ...
    async getPaginatedNeighbors(params) {
        const { entityId, relationshipType, direction = 'both', limit, offset, tenantId, } = params;
        const session = this.neo4j.session();
        try {
     ...
     */
    async precomputeSupernodeStats(topN = 100) {
...
exports.SupernodeQueryOptimizer = SupernodeQueryOptimizer;
// Export factory function
function createSupernodeOptimizer(neo4jDriver, redisClient, config) {
    return new SupernodeQueryOptimizer(neo4jDriver, redisClient, config);
}
exports.default = SupernodeQueryOptimizer;

...
class CIBudgetService extends events_1.EventEmitter {
    ...
    constructor(config = {}, db) {
        ...
        this.db = db;
        this.metrics = new metrics_js_1.PrometheusMetrics('ci_budget_service');
    ...
    async loadTenantBudgets() {
      ...
      `);
            for (const row of budgets.rows) {
                const budget = JSON.parse(row.budget_config);
                this.tenantBudgets.set(row.tenant_id, budget);
            }
            logger_js_1.default.info('Loaded tenant CI budgets', { count: budgets.rows.length });
        }
        catch (error) {
            logger_js_1.default.error('Failed to load tenant CI budgets', {
    ...
    startBudgetTracking() {
        if (!this.config.enabled) {
            logger_js_1.default.info('CI budget enforcement disabled');
            return;
        }
        ...
        logger_js_1.default.info('CI budget tracking started', {
            interval: this.config.trackingIntervalMinutes,
            throttlingEnabled: this.config.throttlingEnabled,
        });
    }
    // Pipeline execution control
    async canExecutePipeline(request) {
        return tracing_js_1.tracer.startActiveSpan('ci_budget.can_execute', async (span) => {
            span.setAttributes({
            ...
            try {
                ...
                if (budget.overrides.allowUnlimited ||
                ...
                // Check bypass period
                if (budget.overrides.bypassUntil &&
                ...
                // Check hard limits
                if (this.config.emergencyBreakEnabled &&
                ...
                logger_js_1.default.error('Failed to check pipeline execution', {
    ...
    calculateUtilization(usage, budget) {
        return budget > 0 ? (usage.costs.total / budget) * 100 : 0;
    }
    async applyThrottling(request, budget, utilization) {
    ...
    async startPipeline(request) {
        ...
        logger_js_1.default.info('Pipeline started', {
            tenantId: request.tenantId,
    ...
    async completePipeline(jobId, result) {
        const request = this.activeJobs.get(jobId);
        if (!request) {
            logger_js_1.default.warn('Completed pipeline not found in active jobs', { jobId });
            return;
        }
        ...
        logger_js_1.default.info('Pipeline completed', {
            tenantId: request.tenantId,
            pipelineId: request.pipelineId,
            status: result.status,
            duration: result.actualDuration,
    ...
    async recordPipelineUsage(tenantId, execution) {
        try {
      ...
      `, [
            ...
            logger_js_1.default.error('Failed to record pipeline usage', {
                tenantId,
                error: error instanceof Error ? error.message : String(error),
            });
        }
    }
    async updateTenantUsage(tenantId, execution) {
    ...
    updateBudgetMetrics(tenantId, budget) {
        // Remaining budget
        this.metrics.setGauge('tenant_ci_budget_remaining', budget.budgets.daily - budget.usage.today.costs.total, { tenant_id: tenantId, period: 'daily' });
        this.metrics.setGauge('tenant_ci_budget_remaining', budget.budgets.monthly - budget.usage.thisMonth.costs.total, { tenant_id: tenantId, period: 'monthly' });
        // Utilization
        this.metrics.setGauge('tenant_ci_budget_utilization', this.calculateUtilization(budget.usage.today, budget.budgets.daily), { tenant_id: tenantId, period: 'daily' });
        this.metrics.setGauge('tenant_ci_budget_utilization', this.calculateUtilization(budget.usage.thisMonth, budget.budgets.monthly), { tenant_id: tenantId, period: 'monthly' });
        // Total costs
        this.metrics.setGauge('tenant_ci_cost_total', budget.usage.today.costs.total, { tenant_id: tenantId, period: 'daily' });
        this.metrics.setGauge('tenant_ci_cost_total', budget.usage.thisMonth.costs.total, { tenant_id: tenantId, period: 'monthly' });
        // Cost by category
        this.metrics.incrementCounter('ci_cost_total', { tenant_id: tenantId, category: 'compute', value: budget.usage.today.costs.compute });
        this.metrics.incrementCounter('ci_cost_total', { tenant_id: tenantId, category: 'storage', value: budget.usage.today.costs.storage });
        this.metrics.incrementCounter('ci_cost_total', { tenant_id: tenantId, category: 'network', value: budget.usage.today.costs.network });
        this.metrics.incrementCounter('ci_cost_total', { tenant_id: tenantId, category: 'actions', value: budget.usage.today.costs.actions });
    }
    async updateAllTenantUsage() {
        for (const tenantId of this.tenantBudgets.keys()) {
            try {
                await this.refreshTenantUsage(tenantId);
            }
            catch (error) {
                logger_js_1.default.error('Failed to refresh tenant usage', {
                    tenantId,
                    error: error instanceof Error ? error.message : String(error),
                });
            }
    ...
    async refreshTenantUsage(tenantId) {
    ...
    `, [tenantId]);
        const budget = this.tenantBudgets.get(tenantId);
        if (!budget || dailyUsage.rows.length === 0)
            return;
    ...
    async checkBudgetLimits(tenantId, budget) {
        ...
        if (maxUtilization >= budget.throttling.softLimitPercent) {
            if (!this.throttledTenants.has(tenantId)) {
                ...
                logger_js_1.default.warn('Tenant throttling activated', {
                    tenantId,
            ...
            this.metrics.setGauge('ci_throttled_tenants', this.throttledTenants.size);
            logger_js_1.default.info('Tenant throttling deactivated', {
                tenantId,
                utilization: maxUtilization,
            });
    ...
    hasRecentAlert(tenantId, period, threshold) {
    ...
    async sendBudgetAlert(tenantId, period, utilization, threshold, budget) {
        ...
        logger_js_1.default.warn('CI budget threshold exceeded', alert);
      ...
      `, [tenantId, period, threshold, utilization]);
            ...
            logger_js_1.default.error('Failed to store budget alert', {
                tenantId,
                error: error instanceof Error ? error.message : String(error),
            });
        }
    ...
    async saveBudgetConfig(tenantId, budget) {
        try {
            await this.db.query(`
      ...
      `, [tenantId, JSON.stringify(budget)]);
            ...
            logger_js_1.default.error('Failed to save budget config', {
                tenantId,
                error: error instanceof Error ? error.message : String(error),
            });
        }
    ...
    async setBudget(tenantId, budgetConfig) {
        ...
        await this.saveBudgetConfig(tenantId, budget);
        logger_js_1.default.info('Updated tenant CI budget', { tenantId });
        this.emit('budgetUpdated', { tenantId, budget });
    }
    async generateForecast(tenantId) {
        const budget = this.tenantBudgets.get(tenantId);
        if (!budget) {
    ...
    generateRecommendations(budget, projectedCost) {
    ...
    async setEmergencyOverride(tenantId, enabled, reason) {
        const budget = this.tenantBudgets.get(tenantId);
        if (!budget) {
            throw new Error(`Budget not found for tenant: ${tenantId}`);
        }
        ...
        await this.saveBudgetConfig(tenantId, budget);
        logger_js_1.default.info('Emergency override updated', { tenantId, enabled, reason });
        this.emit('emergencyOverride', { tenantId, enabled, reason });
    }
    async setBudgetBypass(tenantId, until, reason) {
        const budget = this.tenantBudgets.get(tenantId);
        if (!budget) {
            throw new Error(`Budget not found for tenant: ${tenantId}`);
        }
        ...
        budget.overrides.reason = reason;
        await this.saveBudgetConfig(tenantId, budget);
        logger_js_1.default.info('Budget bypass set', { tenantId, until, reason });
        this.emit('budgetBypass', { tenantId, until, reason });
    }
}
exports.CIBudgetService = CIBudgetService;
// Export singleton instance
exports.ciBudgetService = new CIBudgetService({

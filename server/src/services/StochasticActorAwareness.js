...
exports.StochasticActorAwareness = void 0;
/**
 ...
 */
class StochasticActorAwareness {
    random;
    ...
    runSimulation(signals, options = {}) {
        ...
        const baseScores = signals.map((signal) => this.computeBaseScore(signal, halfLife, saturation));
        const epsilon = 1e-6;
        const totalBase = baseScores.reduce((sum, score) => sum + score, 0);
        const normalizedBase = baseScores.map((score) => (score + epsilon) / (totalBase + epsilon * baseScores.length));
        ...
        const results = signals.map((signal, index) => {
            ...
            const variance = Math.max(0, squaredTotals[index] / sampleCount - meanScore * meanScore);
    ...
    buildSummary(results, limit = 3) {
        if (!results || results.length === 0) {
            return 'No actor signals provided for awareness simulation.';
        }
        const focus = results.slice(0, limit);
        const breakdown = focus
            .map((entry) => `${entry.actor}: ${(entry.awarenessScore * 100).toFixed(1)} awareness score, ${(entry.probability * 100).toFixed(1)}% dominant`)
            .join('; ');
    ...
    computeBaseScore(signal, halfLife, saturation) {
    ...
    saturatingScore(value, saturation) {
...
exports.StochasticActorAwareness = StochasticActorAwareness;

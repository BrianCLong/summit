...
exports.DetectionContentPackV6Service = void 0;
class DetectionContentPackV6Service {
    prisma;
    logger;
    ...
    async deployContentPackV6() {
        ...
        try {
            ...
            const metrics = {
                ...
                attackCoverage: {
                    techniques: Object.keys(attackCoverage).reduce((acc, tactic) => acc + Object.keys(attackCoverage[tactic]).length, 0),
     ...
     */
    async deployCredentialAccessRules() {
        const rules = [
            {
     ...
     * Deploy lateral movement detection rules
     */
    async deployLateralMovementRules() {
        const rules = [
            {
     ...
     * Deploy command & control detection rules
     */
    async deployCommandControlRules() {
        const rules = [
            {
    ...
    async createDetectionRules(rules) {
        const createdRules = [];
    ...
    async runComprehensiveTests(rules) {
        let totalTests = 0;
        let passedTests = 0;
        let failedTests = 0;
        for (const rule of rules) {
    ...
    async runDetectionTest(rule, test) {
    ...
    async updateTestStatus(testId, status, error) {
    ...
    async generateAttackCoverage(rules) {
        const coverage = {};
    ...
    async calculateFalsePositiveReduction(rules) {
...
exports.DetectionContentPackV6Service = DetectionContentPackV6Service;

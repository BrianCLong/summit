...
class ClaimValidator {
    ...
    async validateClaim(claim, context) {
        ...
        const totalDelta = verificationResults.reduce((sum, r) => sum + r.confidenceDelta, 0);
...
class CorroborationStrategy {
    ...
    async validate(claim, context) {
        ...
        const corroboratingClaims = context.allClaims.filter((c) => c.id !== claim.id &&
        ...
        const matchingClaims = corroboratingClaims.filter((c) => this.objectsMatch(c.object, claim.object));
        const conflictingClaims = corroboratingClaims.filter((c) => !this.objectsMatch(c.object, claim.object));
 ...
 */
class TemporalConsistencyStrategy {
    ...
    async validate(claim, context) {
        ...
        const overlappingClaims = context.allClaims.filter((c) => c.id !== claim.id &&
            c.subject === claim.subject &&
            c.predicate === claim.predicate &&
            this.temporalOverlap(claim, c) &&
            !this.objectsMatch(c.object, claim.object));
        if (overlappingClaims.length > 0) {
            issues.push(`Temporal conflict with ${overlappingClaims.length} claim(s)`);
        }
        if (issues.length === 0) {
            return {
                verifierId: this.id,

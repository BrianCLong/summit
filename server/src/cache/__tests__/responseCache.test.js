...
describe('responseCache', () => {
    ...
    it('stores and retrieves values with TTL enforcement and index prefixing', async () => {
        const { buildCacheKey, getCachedJson, setCachedJson, flushLocalCache, } = await Promise.resolve().then(() => __importStar(require('../responseCache.js')));
        flushLocalCache();
        const key = buildCacheKey('ns', 'item');
        const payload = { ok: true };
        await setCachedJson(key, payload, { ttlSeconds: 1, indexPrefixes: ['ns'] });
        const cachedValue = await getCachedJson(key, { ttlSeconds: 1 });
        expect(cachedValue).toEqual(payload);
        globals_1.jest.advanceTimersByTime(1500);
        const expired = await getCachedJson(key, { ttlSeconds: 1 });

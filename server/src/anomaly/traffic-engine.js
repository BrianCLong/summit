...
class TrafficEngine {
     ...
     */
    detectBatchAnomalies(flows, contamination = 0.05) {
    ...
    detectCoordinatedAttacks(flows, results) {
        const targetCounts = new Map(); // target -> Set<sourceIp>
        for (const flow of flows) {
            const key = `${flow.destIp}:${flow.destPort}`;
            if (!targetCounts.has(key))
                targetCounts.set(key, new Set());
...
exports.TrafficEngine = TrafficEngine;

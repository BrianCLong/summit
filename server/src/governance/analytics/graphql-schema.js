...
exports.governanceMetricsResolvers = exports.governanceMetricsTypeDefs = void 0;
const graphql_tag_1 = require("graphql-tag");
exports.governanceMetricsTypeDefs = (0, graphql_tag_1.gql) `
  """
...
exports.governanceMetricsResolvers = {
    Query: {
        governanceMetrics: async (_parent, { input }, context) => {
            const metrics = await context.governanceMetricsService.getGovernanceMetrics(input.tenantId, input.timeRange);
            // Add computed fields
            return {
                ...metrics,
                overallCompliance: {
                    isCompliant: metrics.validationRate.validationRate >= 85 &&
                        metrics.complianceGaps.filter((g) => g.severity === 'critical')
                            .length === 0,
                    validationMeetsODNI: metrics.validationRate.validationRate >= 85,
                    criticalGapsCount: metrics.complianceGaps.filter((g) => g.severity === 'critical').length,
                    highGapsCount: metrics.complianceGaps.filter((g) => g.severity === 'high')
                        .length,
                    riskLevel: metrics.riskScore.overall >= 80
                        ? 'LOW'
        ...
        governanceDashboardConfig: async (_parent, _args, context) => {
            return context.governanceMetricsService.getDashboardConfig();
        },
        complianceGaps: async (_parent, args, context) => {
            let gaps = await context.governanceMetricsService.getComplianceGaps(args.tenantId);
            if (args.framework) {
                gaps = gaps.filter((g) => g.framework === args.framework);
            }
            if (args.severity) {
                gaps = gaps.filter((g) => g.severity.toUpperCase() === args.severity);
            }
            if (args.status) {
                gaps = gaps.filter((g) => g.status.toUpperCase() === args.status);
            }
            return gaps;
        },
        validationRateHistory: async (_parent, args, context) => {
            const trends = await context.governanceMetricsService.getIncidentTrends(args.tenantId, args.timeRange);
            return trends.timeline;
        },
        auditTrail: async (_parent, args, context) => {
            const limit = args.limit || 50;
            let events = await context.governanceMetricsService.getRecentAuditEvents(args.tenantId, limit + (args.offset || 0));
            if (args.eventType) {
                events = events.filter((e) => e.eventType.toUpperCase() === args.eventType);
            }
            if (args.offset) {
    ...
    ValidationMetrics: {
        meetsODNIRequirement: (parent) => parent.validationRate >= parent.targetRate,
    },
    ComplianceGap: {
        daysUntilDue: (parent) => {
            if (!parent.dueDate)
    ...
    RiskScoreData: {
        riskLevel: (parent) => {
            if (parent.overall >= 80)
                return 'LOW';
            if (parent.overall >= 60)
                return 'MEDIUM';
            if (parent.overall >= 40)
                return 'HIGH';
            return 'CRITICAL';
        },
    },
    DeploymentMetrics: {
        successRate: (parent) => parent.totalDeployments > 0
            ? (parent.successfulDeployments / parent.totalDeployments) * 100
            : 0,
    },
    BiasMetrics: {
        detectionRate: (parent) => parent.modelsAudited > 0
            ? (parent.biasDetected / parent.modelsAudited) * 100
            : 0,
    },
    ModelGovernanceMetrics: {
        approvalRate: (parent) => parent.totalModels > 0

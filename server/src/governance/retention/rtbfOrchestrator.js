...
exports.RTBFOrchestrator = void 0;
...
class RTBFOrchestrator {
    logger = pino_1.default({ name: 'rtbf-orchestrator' });
    pool;
    repository;
    policyEvaluator;
    redactionEngine;
    ...
    constructor(options) {
        ...
        this.policyEvaluator =
            options.policyEvaluator ??
                new policyEvaluator_js_1.PolicyEvaluator({ pool: options.pool, repository: this.repository });
        this.redactionEngine =
            options.redactionEngine ??
                new redactionEngine_js_1.RedactionEngine({ pool: options.pool, runCypher: options.runCypher });
    ...
    async submitRTBFRequest(request) {
        ...
        this.logger.info({ requestId: fullRequest.id, scope: request.scope }, 'RTBF request submitted');
        // Auto-validate
    ...
    getRequestStatus(requestId) {
        ...
        const jobs = Array.from(this.jobs.values()).filter((j) => j.requestId === requestId);
    ...
    async validateRequest(requestId) {
        ...
        try {
            ...
            if (legalHoldBlocked.length > 0) {
                ...
                await this.rejectRequest(requestId, 'system', `Legal hold active on datasets: ${legalHoldBlocked.join(', ')}`);
                return;
            }
            ...
            this.logger.info({ requestId: request.id }, 'RTBF request validated successfully');
        }
        catch (error) {
            this.logger.error({ error, requestId: request.id }, 'RTBF request validation failed');
            this.addAuditEvent(request, 'validation.failed', 'system', {
                error: error.message,
            });
    ...
    async approveRequest(requestId, approvedBy, notes) {
        ...
        await this.persistRequest(request);
        this.logger.info({ requestId: request.id, approvedBy }, 'RTBF request approved');
    ...
    async rejectRequest(requestId, rejectedBy, reason) {
        ...
        await this.persistRequest(request);
        this.logger.info({ requestId: request.id, rejectedBy, reason }, 'RTBF request rejected');
    }
    /**
     * Execute RTBF request
     */
    async executeRequest(requestId, executedBy) {
        const request = this.requests.get(requestId);
        if (!request) {
            throw new Error(`Request ${requestId} not found`);
        }
        ...
        try {
            ...
            await this.persistRequest(request);
            this.logger.info({ requestId: request.id, jobCount: jobs.length }, 'RTBF request execution completed');
        }
        catch (error) {
            this.logger.error({ error, requestId: request.id }, 'RTBF request execution failed');
            this.updateRequestState(request, 'failed');
            request.execution.failedAt = new Date();
            request.execution.errorMessage = error.message;
            this.addAuditEvent(request, 'execution.failed', executedBy, {
                error: error.message,
            });
            await this.persistRequest(request);
    ...
    async assessImpact(request) {
        ...
        if (request.scope === 'dataset' && request.target.datasetIds) {
            for (const datasetId of request.target.datasetIds) {
                ...
                if (record) {
                    ...
                    record.policy.storageTargets.forEach((sys) => affectedSystems.add(sys));
                }
            }
        }
        ...
        if (estimatedRecordCount > 10000) {
            warnings.push(`Large deletion: ${estimatedRecordCount} records will be affected`);
        }
        if (affectedSystems.size > 3) {
            warnings.push(`Multiple storage systems affected: ${Array.from(affectedSystems).join(', ')}`);
        }
        return {
    ...
    async performDryRun(request) {
        ...
        const totalRecords = results.affectedRecords.reduce((sum, r) => sum + r.recordCount, 0);
     ...
     */
    async dryRunPostgres(datasetId, record) {
        ...
        for (const table of tables) {
            try {
                const result = await this.pool.query(`SELECT COUNT(*) FROM ${table} WHERE dataset_id = $1`, [datasetId]);
     ...
     * Dry-run for Neo4j
     */
    async dryRunNeo4j(datasetId, record) {
        ...
        for (const label of labels) {
            const result = await this.cypherRunner(`MATCH (n:${label} { datasetId: $datasetId }) RETURN count(n) as count`, { datasetId });
            if (result.length > 0) {
     ...
     */
    async createExecutionJobs(request) {
        const jobs = [];
        if (request.target.datasetIds) {
            for (const datasetId of request.target.datasetIds) {
                const record = this.repository.getRecord(datasetId);
     ...
     */
    buildJobTargets(storageSystem, datasetId, record) {
        if (storageSystem === 'postgres') {
            const tableTags = record.metadata.tags.filter((tag) => tag.startsWith('postgres:table:'));
    ...
    async executeJob(job) {
        ...
        try {
            ...
            job.execution.errorMessage = error.message;
            throw error;
        }
        finally {
     ...
     */
    async executePostgresJob(job) {
        const { table, filterColumn = 'dataset_id', filterValue } = job.operation.targets;
        if (!table) {
            throw new Error('Missing table for Postgres job');
        }
        const safeTable = this.assertSafeIdentifier(table, 'postgres table');
        const safeFilterColumn = this.assertSafeIdentifier(filterColumn, 'postgres filter column');
        const resolvedFilterValue = filterValue ?? job.operation.parameters?.datasetId ?? null;
        if (!resolvedFilterValue) {
            throw new Error('Missing filter value for Postgres job');
        }
        if (job.operation.type === 'delete') {
            const result = await this.pool.query(`DELETE FROM ${safeTable} WHERE ${safeFilterColumn} = $1`, [resolvedFilterValue]);
            ...
            const result = await this.pool.query(`SELECT id FROM ${safeTable} WHERE ${safeFilterColumn} = $1`, [resolvedFilterValue]);
            const recordIds = result.rows.map((r) => r.id);
            // Perform redaction
            const redactionResults = await this.redactionEngine.redactPostgresRecords(table, recordIds, rules, { preserveProvenance: true });
            job.execution.recordsAffected = redactionResults.length;
            job.provenance.hashStubsCreated = redactionResults.reduce((sum, r) => sum + r.hashStubs.size, 0);
        }
    }
    /**
     * Execute Neo4j job
     */
    async executeNeo4jJob(job) {
        if (!this.cypherRunner) {
        ...
        if (job.operation.type === 'delete') {
            const result = await this.cypherRunner(`MATCH (n:${label}) WHERE ${query} DETACH DELETE n RETURN count(n) as deleted`);
    ...
    updateRequestState(request, newState) {
        ...
        this.logger.info({ requestId: request.id, oldState, newState }, 'RTBF request state changed');
    }
    /**
     * Add audit event to request
     */
    addAuditEvent(request, eventType, actor, details) {
        const event = {
            timestamp: new Date(),
            eventType,
            actor,
    ...
    async persistRequest(request) {
        try {
            await this.pool.query(`INSERT INTO rtbf_requests (
...
exports.RTBFOrchestrator = RTBFOrchestrator;

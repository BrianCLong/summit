...
exports.RetentionPolicyModel = void 0;
const MS_PER_DAY = 24 * 60 * 60 * 1000;
function buildCaseKey(tenantId, caseId, evidenceType) {
...
class RetentionPolicyModel {
    ...
    constructor(initial) {
        initial?.tenants?.forEach((policy) => this.tenantDefaults.set(policy.tenantId, policy));
        initial?.cases?.forEach((policy) => this.caseOverrides.set(buildCaseKey(policy.tenantId, policy.caseId, policy.evidenceType), policy));
        initial?.evidenceTypes?.forEach((policy) => this.evidenceOverrides.set(`${policy.tenantId}:${policy.evidenceType}`, policy));
    }
    setTenantDefault(policy) {
        this.tenantDefaults.set(policy.tenantId, policy);
    }
    setCaseOverride(policy) {
        this.caseOverrides.set(buildCaseKey(policy.tenantId, policy.caseId, policy.evidenceType), policy);
    }
    setEvidenceTypeOverride(policy) {
        this.evidenceOverrides.set(`${policy.tenantId}:${policy.evidenceType}`, policy);
    }
    resolve(record) {
        const appliedLayers = [
        ...
        if (tenantId && caseId) {
            const casePolicy = this.caseOverrides.get(buildCaseKey(tenantId, caseId, evidenceType)) ??
        ...
        if (tenantId && evidenceType) {
            const evidencePolicy = this.evidenceOverrides.get(`${tenantId}:${evidenceType}`);
            if (evidencePolicy) {
    ...
    calculateDeletionWindow(record) {
        const resolved = this.resolve(record);
        const createdAt = record.metadata.createdAt;
        const expiresAt = new Date(createdAt.getTime() + resolved.retentionDays * MS_PER_DAY);

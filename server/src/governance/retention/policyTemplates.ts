...
export const POLICY_TEMPLATE_LIBRARY: RetentionPolicyTemplate[] = [
  {
    ...
    description:
      'Write-once storage with seven year retention for audit evidence and regulator-mandated records.',
  ...
  },
  {
    ...
    description:
      'Standard one year retention for personal data with privacy review and DSAR support.',
  ...
  },
  {
    ...
    description:
      'Operational telemetry retained for ninety days to support investigations and trend analysis.',
  ...
  },
  {
    ...
    description:
      'Short lived retention for ML feature stores and training extracts pending model promotion.',
    classificationLevel: 'internal',
    retentionDays: 30,
    legalHoldAllowed: true,
    purgeGraceDays: 5,
  ...
  },
  {
    ...
    description:
      'Retention for open-source intelligence artifacts and derived analytics.',
...
export function resolveClassification(metadata: DatasetMetadata): {
  ...
  if (metadata.containsFinancialData) {
    rationale.push(
      'Contains financial attributes requiring regulatory retention.',
    );
    level = 'regulated';
  }
  ...
  if (metadata.containsHealthData) {
    rationale.push(
      'Contains health information subject to HIPAA-equivalent retention.',
    );
    level = 'regulated';
  }
...
export function selectTemplateForDataset(metadata: DatasetMetadata): {
  ...
  const applicableTemplates = POLICY_TEMPLATE_LIBRARY.filter((template) => {
    const levelMatch =
      template.classificationLevel === classification.level ||
      (classification.level === 'regulated' &&
        template.classificationLevel === 'restricted');
    const typeMatch =
      template.applicableDataTypes.includes(metadata.dataType) ||

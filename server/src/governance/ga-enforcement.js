...
exports.defaultGAEnforcementService = exports.GAEnforcementService = exports.BypassableGovernanceError = exports.ProvenanceRequiredError = exports.GovernanceEnforcementError = void 0;
exports.createGovernanceVerdict = createGovernanceVerdict;
exports.wrapWithEnvelope = wrapWithEnvelope;
...
exports.GovernanceEnforcementError = GovernanceEnforcementError;
/**
 * Error thrown when provenance is required but missing
 */
class ProvenanceRequiredError extends GovernanceEnforcementError {
    constructor(message = 'Provenance is required but missing') {
        super(message, 'PROVENANCE_REQUIRED');
        this.name = 'ProvenanceRequiredError';
    }
}
exports.ProvenanceRequiredError = ProvenanceRequiredError;
/**
 * Error thrown when governance verdict is bypassable (security violation)
 */
class BypassableGovernanceError extends GovernanceEnforcementError {
    constructor(message = 'Governance verdict cannot be bypassed', verdictId) {
        super(message, 'GOVERNANCE_BYPASS_ATTEMPTED', { verdictId });
        this.name = 'BypassableGovernanceError';
    }
}
exports.BypassableGovernanceError = BypassableGovernanceError;
/**
 * GA Enforcement Service
 *
 ...
 */
class GAEnforcementService {
    config;
    logger = request_context_js_1.appLogger.child({ component: 'GAEnforcementService' });
    /**
    ...
    constructor(config = {}) {
        this.config = {
            ...
            defaultClassification: config.defaultClassification ?? data_envelope_js_1.DataClassification.INTERNAL,
     ...
     * );
     */
    async enforceGovernance(data, context, options) {
        const startTime = Date.now();
        try {
            // Generate provenance
            const provenance = this.generateProvenance(context, options?.source || 'ga-enforcement', options?.version);
            // Create governance verdict (mandatory in strict mode)
            const verdict = await this.createMandatoryVerdict(context, options?.policyId || 'default-policy', options?.customVerdict);
            // Build envelope
            const envelope = wrapWithEnvelope(data, provenance, verdict, {
            ...
            if (this.config.autoGenerateLineage) {
                envelope.provenance.lineage.push(this.createLineageNode('ga-enforcement', [], context.userId, {
     ...
     */
    validateProvenanceRequired(envelope) {
     ...
     */
    assertNotBypassable(verdict) {
        // Check for bypassable DENY verdicts
        if (verdict.result === data_envelope_js_1.GovernanceResult.DENY) {
            ...
            throw new BypassableGovernanceError(`Access denied by governance policy: ${verdict.reason || 'No reason provided'}`, verdict.verdictId);
        }
        // Check for bypassable REVIEW_REQUIRED verdicts
        if (verdict.result === data_envelope_js_1.GovernanceResult.REVIEW_REQUIRED) {
            if (!verdict.requiredApprovals || verdict.requiredApprovals.length === 0) {
                throw new BypassableGovernanceError('REVIEW_REQUIRED verdict missing required approvals list', verdict.verdictId);
            }
            ...
            throw new BypassableGovernanceError(`Review required by governance policy. Required approvals: ${verdict.requiredApprovals.join(', ')}`, verdict.verdictId);
        }
        // FLAG verdicts are allowed but logged
        if (verdict.result === data_envelope_js_1.GovernanceResult.FLAG) {
     ...
     */
    async createMandatoryVerdict(context, policyId, customVerdict) {
        ...
        if (this.config.strictMode && !customVerdict) {
            ...
            return createGovernanceVerdict(data_envelope_js_1.GovernanceResult.ALLOW, policyId, 'Default allow - strict mode enabled', {
                evaluator: this.config.defaultEvaluator,
            });
        }
        // Use custom verdict if provided
        if (customVerdict) {
            return createGovernanceVerdict(customVerdict.result || data_envelope_js_1.GovernanceResult.ALLOW, customVerdict.policyId || policyId, customVerdict.reason, {
                evaluator: customVerdict.evaluator || this.config.defaultEvaluator,
                requiredApprovals: customVerdict.requiredApprovals,
            });
        }
        // Default verdict
        return createGovernanceVerdict(data_envelope_js_1.GovernanceResult.ALLOW, policyId, 'Default allow', {
            evaluator: this.config.defaultEvaluator,
        });
    }
    /**
     ...
     */
    generateProvenance(context, source, version) {
        return {
            source,
            ...
            provenanceId: `prov-${Date.now()}-${(0, crypto_1.randomUUID)()}`,
        };
    }
    /**
     ...
     */
    createLineageNode(operation, inputs, actor, metadata) {
        return {
            id: `lineage-${Date.now()}-${(0, crypto_1.randomUUID)()}`,
            operation,
            inputs,
            timestamp: new Date(),
            actor,
...
exports.GAEnforcementService = GAEnforcementService;
/**
 ...
 * );
 */
function createGovernanceVerdict(result, policyId, reason, options) {
    return {
        verdictId: `verdict-${Date.now()}-${(0, crypto_1.randomUUID)()}`,
        policyId,
        result,
        decidedAt: new Date(),
        reason,
 ...
 * );
 */
function wrapWithEnvelope(data, provenance, verdict, options) {
    ...
    if (options?.confidence !== undefined) {
        if (options.confidence < 0 || options.confidence > 1) {
            throw new GovernanceEnforcementError('Confidence score must be between 0 and 1', 'INVALID_CONFIDENCE', { confidence: options.confidence });
        }
    }
    return {
        data,
 ...
 */
exports.defaultGAEnforcementService = new GAEnforcementService();

...
exports.migrationFramework = exports.MigrationFramework = void 0;
...
const tracer = api_1.trace.getTracer('migration-framework', '24.3.0');
// Metrics
const migrationOperations = new prom_client_1.Counter({
    name: 'migration_operations_total',
    help: 'Total migration operations',
    labelNames: ['tenant_id', 'migration_type', 'phase', 'result'],
});
const migrationDuration = new prom_client_1.Histogram({
    name: 'migration_duration_seconds',
    help: 'Migration execution time',
    buckets: [1, 5, 10, 30, 60, 300, 600, 1800, 3600],
    labelNames: ['migration_type', 'phase'],
});
const activeMigrations = new prom_client_1.Gauge({
    name: 'active_migrations',
    help: 'Currently running migrations',
    labelNames: ['tenant_id', 'migration_type'],
});
const migrationLockStatus = new prom_client_1.Gauge({
    name: 'migration_lock_status',
    help: 'Migration lock status (1 = locked, 0 = unlocked)',
    labelNames: ['tenant_id'],
});
class MigrationFramework extends events_1.EventEmitter {
    lockPrefix = 'migration_lock';
    statePrefix = 'migration_state';
    lockTTL = 3600; // 1 hour
    constructor() {
        super();
    }
    async executeMigration(migration, tenantId, options = {}) {
        return tracer.startActiveSpan('migration_framework.execute', async (span) => {
            span.setAttributes({
                tenant_id: tenantId,
    ...
    async executePhase(migration, execution, phase, options) {
        ...
        migrationDuration.observe({ migration_type: migration.type, phase }, (Date.now() - startTime) / 1000);
    }
    async executeStep(migration, execution, step, phase, dryRun) {
        return tracer.startActiveSpan('migration_framework.execute_step', async (span) => {
            span.setAttributes({
    ...
    async executeSQLStep(step, execution, dryRun) {
    ...
    async executeSQLInBatches(sql, execution, 
    ...
    async executeCypherStep(step, execution, dryRun) {
        if (dryRun) {
            // For dry run, use EXPLAIN
            const explainQuery = `EXPLAIN ${step.content}`;
            await neo4j_js_1.neo.run(explainQuery, {}, { tenantId: execution.tenantId });
        }
        else {
            const result = await neo4j_js_1.neo.run(step.content, {}, { tenantId: execution.tenantId });
    ...
    async executeJavaScriptStep(step, execution, dryRun) {
        ...
        const sandboxContext = {
            ...
            pool: pg_js_1.pool,
            neo: neo4j_js_1.neo,
            redis: pubsub_js_1.redis,
            // Add utility functions
            sleep: this.sleep,
            log: (message) => console.log(`[${execution.migrationId}:${step.id}] ${message}`),
        };
        const context = vm_1.default.createContext(sandboxContext);
        if (dryRun) {
            // For dry run, validate syntax
            new vm_1.default.Script(step.content);
        }
        else {
            try {
                const script = new vm_1.default.Script(step.content);
                script.runInContext(context);
            }
            catch (error) {
                throw new Error(`JavaScript step failed: ${error.message}`);
            }
        }
    }
    async executeValidationStep(step, execution) {
        ...
        if (validation.type === 'sql') {
            const result = await pg_js_1.pool.query(validation.query);
            ...
            const result = await neo4j_js_1.neo.run(validation.query, {}, { tenantId: execution.tenantId });
    ...
    async evaluateCondition(condition, tenantId) {
        ...
        if (condition.startsWith('sql:')) {
            ...
            const result = await pg_js_1.pool.query(query);
    ...
    async runValidations(migration, tenantId, type) {
        ...
        for (const validation of validations) {
            try {
                ...
                if (validation.critical) {
                    throw new Error(`Critical validation '${validation.name}' failed: ${error.message}`);
                }
            }
        }
    }
    async rollbackMigration(migrationId, tenantId, toPhase) {
        return tracer.startActiveSpan('migration_framework.rollback', async (span) => {
            span.setAttributes({
                tenant_id: tenantId,
    ...
    async acquireLock(migrationId, tenantId, phase) {
        ...
        const lockSet = await pubsub_js_1.redis.setWithTTLIfNotExists(lockKey, JSON.stringify(lock), this.lockTTL);
        if (!lockSet) {
            throw new Error(`Failed to acquire migration lock for ${migrationId}:${phase}`);
        }
        migrationLockStatus.set({ tenant_id: tenantId }, 1);
    }
    async releaseLock(migrationId, tenantId) {
    ...
    getPhasesUntil(toPhase) {
        const allPhases = ['expand', 'migrate', 'contract'];
    ...
    async getMigrationStatus(migrationId, tenantId) {
        const stateKey = `${this.statePrefix}:${tenantId}:${migrationId}`;
        const state = await pubsub_js_1.redis.get(stateKey);
        return state ? JSON.parse(state) : null;
    }
    async listRunningMigrations(tenantId) {
    ...
    generateMigrationReport(executions) {
...
exports.MigrationFramework = MigrationFramework;
exports.migrationFramework = new MigrationFramework();

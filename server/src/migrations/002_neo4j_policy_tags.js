 ...
 */
async function addPolicyTagsToNodes(session) {
    logger_js_1.logger.info('Adding policy tags to all nodes...');
    ...
    logger_js_1.logger.info(`Updated ${updated} nodes with policy tags`);
    return updated;
}
/**
 * Apply policy tags to all existing relationships
 */
async function addPolicyTagsToRelationships(session) {
    logger_js_1.logger.info('Adding policy tags to all relationships...');
    const result = await session.run(`
    MATCH ()-[r]->()
    ...
    const updated = result.records[0]?.get('updated').toNumber() || 0;
    logger_js_1.logger.info(`Updated ${updated} relationships with policy tags`);
    return updated;
}
/**
 * Create indexes for policy tag queries
 */
async function createPolicyTagIndexes(session) {
    logger_js_1.logger.info('Creating indexes for policy tags...');
    const indexes = [
        {
    ...
    for (const index of indexes) {
        try {
            ...
            logger_js_1.logger.info(`Created index: ${index.name}`);
        }
        catch (error) {
            logger_js_1.logger.warn(`Index ${index.name} may already exist: ${error.message}`);
        }
    }
}
/**
 ...
 */
async function detectAndFlagPII(session) {
    logger_js_1.logger.info('Detecting and flagging PII in existing data...');
    const piiPatterns = {
        has_emails: `(?i).*@.*\\..*`,
    ...
    logger_js_1.logger.info(`Flagged PII in ${totalUpdated} entities`);
    return totalUpdated;
}
/**
 * Add policy owner based on entity type
 */
async function assignPolicyOwners(session) {
    logger_js_1.logger.info('Assigning policy owners...');
    const result = await session.run(`
    MATCH (n)
    ...
    `);
    const updated = result.records[0]?.get('updated').toNumber() || 0;
    logger_js_1.logger.info(`Assigned policy owners to ${updated} nodes`);
    return updated;
}
/**
 * Calculate and set policy expiry dates based on retention policies
 */
async function setPolicyExpiryDates(session) {
    logger_js_1.logger.info('Setting policy expiry dates...');
    const result = await session.run(`
    MATCH (n)
    ...
    `);
    const updated = result.records[0]?.get('updated').toNumber() || 0;
    logger_js_1.logger.info(`Set expiry dates for ${updated} nodes`);
    return updated;
}
/**
 * Create constraints for policy tags
 */
async function createPolicyTagConstraints(session) {
    logger_js_1.logger.info('Creating constraints for policy tags...');
    const constraints = [
        {
    ...
    for (const constraint of constraints) {
        try {
            ...
            logger_js_1.logger.info(`Created constraint: ${constraint.name}`);
        }
        catch (error) {
            // Constraints may not be supported in all Neo4j versions
            logger_js_1.logger.warn(`Could not create constraint ${constraint.name}: ${error.message}`);
        }
    }
}
/**
 ...
 */
async function up(driver) {
    const session = driver.session();
    try {
        logger_js_1.logger.info('Starting Neo4j policy tags migration...');
        ...
        try {
            ...
            logger_js_1.logger.info(`PII detection completed: ${piiUpdated} entities flagged`);
        }
        catch (error) {
            logger_js_1.logger.warn('PII detection skipped (may require APOC plugin)', { error: error.message });
        }
        ...
        try {
            ...
            logger_js_1.logger.warn('Constraint creation skipped', { error: error.message });
        }
        logger_js_1.logger.info('Neo4j policy tags migration completed successfully', {
            nodesUpdated,
            relsUpdated,
            ownersAssigned,
            expiryDatesSet,
        ...
        logger_js_1.logger.error('Neo4j policy tags migration failed', { error: error.message });
        throw error;
    }
    finally {
        await session.close();
 ...
 */
async function down(driver) {
    const session = driver.session();
    try {
        logger_js_1.logger.info('Rolling back Neo4j policy tags migration...');
      ...
      `);
        ...
        for (const indexName of indexes) {
            try {
                ...
                logger_js_1.logger.info(`Dropped index: ${indexName}`);
            }
            catch (error) {
                logger_js_1.logger.warn(`Could not drop index ${indexName}: ${error.message}`);
            }
        }
        logger_js_1.logger.info('Neo4j policy tags rollback completed');
    }
    catch (error) {
        logger_js_1.logger.error('Neo4j policy tags rollback failed', { error: error.message });
        throw error;
    }
    finally {
        await session.close();
 ...
 */
async function isMigrationNeeded(driver) {
    const session = driver.session();
    try {
        const result = await session.run(`
      MATCH (n)

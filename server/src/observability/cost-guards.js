...
exports.costGuardService = exports.CostGuardService = void 0;
...
class CostGuardService {
    config;
    costLedger; // tenantId -> entries
    alertsSent; // tenantId -> set of thresholds alerted
    lastReset;
    constructor(config = {}) {
        ...
        if (this.config.enabled) {
            ...
            logger_js_1.default.info('Cost Guard Service initialized', {
     ...
     * Record a cost entry
     */
    recordCost(resourceType, operation, amount, tenantId = 'default', metadata) {
        if (!this.config.enabled) {
            return;
        }
        const entry = {
        ...
        metrics_enhanced_js_1.costAccrued.inc({
            tenant_id: tenantId,
     ...
     */
    recordQueryCost(database, durationMs, tenantId = 'default') {
     ...
     * Record compute cost
     */
    recordComputeCost(cpuSeconds, memoryGbSeconds, tenantId = 'default') {
        const cost = cpuSeconds * COST_RATES.cpu_second +
     ...
     * Record storage cost
     */
    recordStorageCost(storageGb, tier, tenantId = 'default') {
     ...
     */
    async checkBudgetLimit(estimatedCost, tenantId = 'default') {
        if (!this.config.enabled) {
            return {
                allowed: true,
     ...
     */
    getBudgetStatus(tenantId = 'default', budgetType = 'tenant') {
        ...
        // Update Prometheus metrics
        metrics_enhanced_js_1.costBudgetGauge.set({ tenant_id: tenantId, budget_type: budgetType }, remaining);
        metrics_enhanced_js_1.costBudgetUtilization.set({ tenant_id: tenantId, budget_type: budgetType }, utilization);
        return {
            budgetType,
            limit,
            used,
     ...
     */
    getTotalCost(tenantId, budgetType) {
        const entries = this.costLedger.get(tenantId) || [];
        const now = new Date();
        let cutoffTime;
        switch (budgetType) {
     ...
     * Send a cost alert
     */
    sendCostAlert(tenantId, threshold, status) {
        ...
        metrics_enhanced_js_1.costAlertCounter.inc({
            tenant_id: tenantId,
     ...
     */
    getCostBreakdown(tenantId = 'default') {
        const entries = this.costLedger.get(tenantId) || [];
        const breakdown = {};
     ...
     */
    resetDailyBudgets() {
        logger_js_1.default.info('Resetting daily cost budgets');
        // Clear daily alerts
        for (const tenantId of this.costLedger.keys()) {
     ...
     */
    resetMonthlyBudgets() {
        logger_js_1.default.info('Resetting monthly cost budgets');
    ...
    updateConfig(updates) {
        ...
        logger_js_1.default.info('Cost Guard configuration updated', this.config);
    }
    /**
     * Get configuration
     */
...
// Singleton instance
exports.costGuardService = new CostGuardService();
exports.default = exports.costGuardService;

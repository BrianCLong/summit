...
function computeServiceScore(group, service, options) {
    ...
    const traces = related.filter((signal) => signal.kind === 'trace');
    const metrics = related.filter((signal) => signal.kind === 'metric');
    const latencyScore = normalizeScore(percentile(traces.map((trace) => trace.durationMs), 0.95) / options.latencyPenaltyMs);
    const errorScore = normalizeScore((errors.length / related.length) * options.errorWeight);
    const metricDeviation = normalizeScore(metrics.reduce((acc, metric) => acc + (metric.value > metric.expected?.p95 ? 1 : 0), 0) / Math.max(metrics.length, 1));

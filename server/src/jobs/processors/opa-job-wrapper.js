...
exports.withOpaPolicy = withOpaPolicy;
exports.canEnqueueJob = canEnqueueJob;
 ...
 */
async function evaluateJobPolicy(queueName, job) {
    if (!OPA_ENABLED) {
        return {
            allow: true,
    ...
    try {
        ...
        logger_js_1.default.error('OPA job policy evaluation failed', {
            queue: queueName,
            jobId: job.id,
            error: errorMessage,
        ...
        logger_js_1.default.warn('Non-production: allowing job despite policy error', {
            queue: queueName,
            jobId: job.id,
        });
        return {
 ...
 */
function logPolicyDecision(queueName, job, decision) {
    const metadata = job.data ?? {};
    const logEntry = {
    ...
    if (decision.allow) {
        logger_js_1.default.info('Job policy decision: ALLOW', logEntry);
    }
    else {
        logger_js_1.default.warn('Job policy decision: DENY', logEntry);
    }
}
/**
 ...
 */
function withOpaPolicy(queueName, processor) {
    return async (job) => {
        const policyContext = await evaluateJobPolicy(queueName, job);
        if (!policyContext.allow) {
            logger_js_1.default.warn('Job blocked by OPA policy', {
                queue: queueName,
                jobId: job.id,
 ...
 */
async function canEnqueueJob(queueName, jobData) {
    if (!OPA_ENABLED) {
        return { allowed: true };
    }
    const mockJob = {

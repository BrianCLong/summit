...
class IndexingService {
    ...
    constructor(options = {}) {
        ...
        this.pgPool = options.pool || new pg_1.Pool({ connectionString: process.env.DATABASE_URL });
        this.neo4jDriverFactory = () => options.neo4jDriver || (0, database_js_1.getNeo4jDriver)();
        this.useBatchWrites =
            options.batchWritesEnabled !== undefined
                ? options.batchWritesEnabled
                : process.env.BATCH_WRITES_V1 === '1';
    }
    async index(data, ctx) {
    ...
    async syncToNeo4j(data, ctx) {
        const driver = this.neo4jDriverFactory();
        const session = driver.session();
        try {
            if (this.useBatchWrites) {
                if (data.entities.length > 0) {
                    await session.executeWrite((tx) => tx.run(`UNWIND $entities AS entity
               ...
               SET e.kind = entity.kind,
                ...
                if (data.edges.length > 0) {
                    await session.executeWrite((tx) => tx.run(`UNWIND $edges AS edge
               ...
               SET r += edge.props`, {
                ...
                for (const entity of data.entities) {
                    await session.run(`MERGE (e:Entity {id: $id, tenantId: $tenantId})
             SET e.kind = $kind, e.labels = $labels, e += $props`, { id: entity.id, tenantId: ctx.tenantId, kind: entity.kind, labels: entity.labels, props: entity.properties });
                }
                for (const edge of data.edges) {
                    await session.run(`MATCH (from:Entity {id: $fromId, tenantId: $tenantId})
             MATCH (to:Entity {id: $toId, tenantId: $tenantId})
             MERGE (from)-[r:RELATIONSHIP {id: $id, kind: $kind}]->(to)
             SET r += $props`, { fromId: edge.fromEntityId, toId: edge.toEntityId, tenantId: ctx.tenantId, id: edge.id, kind: edge.kind, props: edge.properties });
                }
            }
        }
        catch (e) {
    ...
    async deferConstraints(client, ctx) {
        try {
    ...
    async batchUpsertEntities(client, entities, ctx) {
        ...
        entities.forEach((entity, idx) => {
            ...
            params.push(entity.id, ctx.tenantId, entity.kind, entity.labels || [], JSON.stringify(entity.properties || {}), entity.sourceIds || []);
        });
        const query = `
    ...
    async batchUpsertEdges(client, edges, ctx) {
        if (edges.length === 0)
            return;
        const placeholders = [];
        const params = [];
        edges.forEach((edge, idx) => {
            ...
            params.push(edge.id, ctx.tenantId, edge.fromEntityId, edge.toEntityId, edge.kind, JSON.stringify(edge.properties || {}), edge.sourceIds || []);
        });
        const query = `
    ...
    async batchUpsertDocuments(client, documents, ctx) {
        ...
        documents.forEach((doc, idx) => {
            ...
            params.push(doc.id, ctx.tenantId, doc.title || null, doc.mimeType || null, doc.source ? JSON.stringify(doc.source) : null, doc.text, JSON.stringify(doc.metadata || {}), doc.entityIds || []);
        });
        const query = `
    ...
    async batchUpsertChunks(client, chunks, ctx) {
        if (chunks.length === 0)
            return;
        const placeholders = [];
        const params = [];
        chunks.forEach((chunk, idx) => {
            ...
            params.push(chunk.id, ctx.tenantId, chunk.documentId, chunk.text, chunk.offset, JSON.stringify(chunk.metadata || {}), embeddingVector);
        });
        const query = `
    ...
    async legacyEntityWrites(client, entities, ctx) {
        for (const entity of entities) {
            await client.query(`INSERT INTO entities (id, tenant_id, kind, labels, properties, source_ids)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (id, tenant_id) DO UPDATE SET
    ...
    async legacyEdgeWrites(client, edges, ctx) {
        for (const edge of edges) {
            await client.query(`INSERT INTO edges (id, tenant_id, from_entity_id, to_entity_id, kind, properties, source_ids)
    ...
    async legacyDocumentWrites(client, documents, ctx) {
        for (const doc of documents) {
            await client.query(`INSERT INTO documents (id, tenant_id, title, mime_type, source, text, metadata, entity_ids)
    ...
    async legacyChunkWrites(client, chunks, ctx) {
        for (const chunk of chunks) {
            const embeddingVector = chunk.embedding ? `[${chunk.embedding.join(',')}]` : null;
            if (embeddingVector) {
                await client.query(`INSERT INTO chunks (id, tenant_id, document_id, text, "offset", metadata, embedding)
           VALUES ($1, $2, $3, $4, $5, $6, $7)
           ON CONFLICT (id, tenant_id) DO NOTHING`, [
                    chunk.id,
                    ctx.tenantId,
                ...
                await client.query(`INSERT INTO chunks (id, tenant_id, document_id, text, "offset", metadata)
           VALUES ($1, $2, $3, $4, $5, $6)
           ON CONFLICT (id, tenant_id) DO NOTHING`, [
                    chunk.id,
                    ctx.tenantId,

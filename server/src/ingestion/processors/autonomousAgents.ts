...
export async function parseAutonomousAgentsMarkdown(
  ...
  for await (const rawLine of rl) {
    ...
    if (currentPaper && line.startsWith('-')) {
      currentPaper.summaryBullets.push(line.replace(/^[-*]\s*/, '').trim());
    }
  }
...
export async function parseRepository(rootDir: string, commitSha: string): Promise<AutonomousAgentPaperRecord[]> {
  const files = listCandidateFiles(rootDir);
  const records: AutonomousAgentPaperRecord[] = [];
  for (const file of files) {
    const parsed = await parseAutonomousAgentsMarkdown(file, commitSha);
    records.push(...parsed);
  }
  return records;
}
...
export class AutonomousAgentsIngestionService {
  ...
  clone(ref = 'main'): { repoPath: string; commitSha: string } {
    ...
    execSync(`git clone --depth 1 --branch ${ref} ${this.repoUrl} ${tempDir}`, {
      stdio: 'ignore',
    });
  ...
  async ingest(options: { ref?: string; commitSha?: string; localPath?: string } = {}) {
    const { ref = 'main', commitSha: pinnedCommit, localPath } = options;
    ...
    if (localPath) {
      repoPath = localPath;
      commitSha = pinnedCommit ?? execSync('git rev-parse HEAD', { cwd: repoPath }).toString().trim();
    } else {
      const clone = this.clone(ref);
      repoPath = clone.repoPath;
      commitSha = clone.commitSha;

      if (pinnedCommit && pinnedCommit !== commitSha) {
        execSync(`git checkout ${pinnedCommit}`, { cwd: repoPath, stdio: 'ignore' });

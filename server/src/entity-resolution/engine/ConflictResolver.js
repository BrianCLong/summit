...
class ConflictResolver {
     ...
     */
    static resolve(entityA, entityB, strategies = ['recency']) {
    ...
    static resolveField(field, valA, valB, entityA, entityB, strategies) {
        for (const strategy of strategies) {
            ...
            if (strategy === 'completeness') {
                ...
                if (String(valA).length > String(valB).length)
                    return valA;
                if (String(valB).length > String(valA).length)
                    return valB;
            }
        }

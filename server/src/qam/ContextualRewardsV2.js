...
exports.ContextualRewardsV2 = void 0;
const events_1 = require("events");
const logger_js_1 = __importDefault(require("../config/logger.js"));
const logger = logger_js_1.default.child({ module: 'ContextualRewardsV2' });
/**
 ...
 */
class ContextualRewardsV2 extends events_1.EventEmitter {
    config;
    rewardHistory = [];
    currentParetoFront;
    routeMetrics = new Map();
    contextualWeights = new Map(); // Context-specific weight adaptations
     ...
     */
    async processMultiObjectiveReward(executionId, route, provider, rawMetrics, context) {
        try {
     ...
     */
    async calculateParetoAwareDelta(recentRewards, timeWindowHours = 1) {
        try {
     ...
     */
    getCurrentParetoFront() {
        return { ...this.currentParetoFront };
    }
    /**
     * Get route performance metrics
     */
    getRouteMetrics(route, provider) {
        const metrics = [];
        for (const [key, routeMetric] of this.routeMetrics) {
            if (route && routeMetric.route !== route)
                continue;
            if (provider && routeMetric.provider !== provider)
                continue;
    ...
    calculateObjectiveScores(rawMetrics, context) {
    ...
    calculateCompositeScore(objectives, weights) {
        return (objectives.latency.normalized * weights.latency +
    ...
    analyzeTradeoffs(objectives) {
        ...
        return {
            ...
            overallEfficiency: (scores.latency +
                scores.cost +
                scores.quality +
                scores.reliability +
                scores.security) /
    ...
    async updateRouteMetrics(route, provider, reward) {
        ...
        metrics.averageScores = {
            latency: recent.reduce((sum, r) => sum + r.objectives.latency.normalized, 0) /
                recent.length,
            cost: recent.reduce((sum, r) => sum + r.objectives.cost.normalized, 0) /
                recent.length,
            quality: recent.reduce((sum, r) => sum + r.objectives.quality.normalized, 0) /
                recent.length,
            reliability: recent.reduce((sum, r) => sum + r.objectives.reliability.normalized, 0) / recent.length,
            security: recent.reduce((sum, r) => sum + r.objectives.security.normalized, 0) /
                recent.length,
    ...
    calculateHypervolume(solutions) {
        ...
        for (const solution of solutions) {
            const volume = solution.objectives.latency.normalized *
    ...
    calculateConvergence(solutions) {
        if (solutions.length === 0)
            return 0;
        // Measure how close solutions are to theoretical optimum
        const averageComposite = solutions.reduce((sum, s) => sum + s.compositeScore, 0) /
            solutions.length;
    ...
    calculateHypervolumeIncrease(windowRewards) {
        if (windowRewards.length < 2)
            return 0;
    ...
    generateWeightRecommendations(windowRewards) {
        const recommendations = {};
        const objectives = [
            'latency',
            'cost',
    ...
    getRouteWeaknesses(metrics) {
        ...
        if (scores.quality < 0.5)
            weaknesses.push('Low Quality');
        if (scores.reliability < 0.5)
            weaknesses.push('Unreliable');
        if (scores.security < 0.5)
            weaknesses.push('Security Issues');
        if (metrics.consistencyScore < 0.5)
            weaknesses.push('Inconsistent');
        return weaknesses;
    }
    getWeightAdjustmentSuggestions() {
    ...
    getConvergenceStatus() {
        ...
        return {
            ...
            recommendedAction: convergence < 0.5
...
exports.ContextualRewardsV2 = ContextualRewardsV2;
exports.default = ContextualRewardsV2;

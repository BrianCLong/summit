...
exports.DifferentialPrivacyAccountant = void 0;
// @ts-nocheck
const events_1 = require("events");
const logger_js_1 = require("../config/logger.js");
const logger = (0, logger_js_1.createLogger)('DifferentialPrivacyAccountant');
/**
 ...
 */
class DifferentialPrivacyAccountant extends events_1.EventEmitter {
    config;
    privacyBudget;
    queryHistory = [];
    refreshTimer;
    ...
    constructor(config) {
        ...
        if (this.config.budgetRefreshInterval > 0) {
            this.refreshTimer = setInterval(() => this.refreshBudget(), this.config.budgetRefreshInterval);
        }
    ...
    async consumeBudget(query) {
        try {
            ...
            const budgetExceeded = composition.totalEpsilon > this.privacyBudget.totalEpsilon ||
            ...
            if (epsilonUsageRatio >= this.config.warningThreshold &&
                !budgetExceeded) {
     ...
     */
    async calculateComposition(queries, method) {
        try {
            switch (method) {
                case 'basic':
    ...
    advancedComposition(queries) {
        ...
        for (const group of groupedQueries.values()) {
            ...
            if (count === 1) {
                ...
                const epsilonAdvanced = epsilon * Math.sqrt(2 * k * Math.log(1 / deltaPlus)) +
     ...
     */
    calculateNoiseScale(query, mechanism) {
     ...
     * Convert (ε, δ)-DP to RDP
     */
    convertToRDP(epsilon, delta, order) {
        if (order === 1) {
            return epsilon;
        }
     ...
     */
    convertFromRDP(rdpValues, targetDelta) {
     ...
     */
    updateBudgetUsage(query, composition) {
     ...
     */
    updateMomentsTracking(query, noiseScale) {
    ...
    getMomentsStatus() {
        ...
        const averageBound = this.momentsBounds.reduce((sum, b) => sum + b, 0) /
    ...
    importState(state) {
        ...
        this.rdpEpsilons = new Map(Object.entries(state.rdpEpsilons).map(([k, v]) => [
            parseFloat(k),
            v,
        ]));
...
exports.DifferentialPrivacyAccountant = DifferentialPrivacyAccountant;
exports.default = DifferentialPrivacyAccountant;

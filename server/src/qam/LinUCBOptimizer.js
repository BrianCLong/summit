...
exports.LinUCBOptimizer = void 0;
const events_1 = require("events");
const logger_js_1 = __importDefault(require("../config/logger.js"));
const logger = logger_js_1.default.child({ module: 'LinUCBOptimizer' });
/**
 ...
 */
class LinUCBOptimizer extends events_1.EventEmitter {
    config;
    arms = new Map();
    contextDimension;
    totalPulls = 0;
     ...
     */
    async selectOptimalArm(context) {
        try {
     ...
     */
    async updateWithReward(armId, context, reward) {
        try {
     ...
     */
    calculateUCB(armState, features) {
     ...
     */
    calculateConfidenceBound(armId, contextFeatures) {
        const armState = this.arms.get(armId);
        if (!armState)
            return 1.0; // High confidence for unknown arms
        try {
            // Calculate x^T * A^(-1) * x
     ...
     */
    updateThetaShermanMorrison(armState, features, reward) {
        try {
            // Update b: b = b + r * x
     ...
     * Calculate confidence scores for all arms
     */
    calculateAllConfidenceScores(features) {
        const scores = {};
    ...
    getAverageConfidence() {
        ...
        const confidences = Array.from(this.arms.keys()).map((armId) => this.calculateConfidenceBound(armId, lastContext));
        return (confidences.reduce((sum, conf) => sum + conf, 0) / confidences.length);
    }
    /**
     * Update convergence metrics
     */
    updateConvergenceMetrics(armState, reward) {
    ...
    createIdentityMatrix(size, lambda) {
    ...
    calculateThetaStability(armState) {
    ...
    calculateRewardVariance() {
        if (this.recentRewards.length < 2)
            return 0;
        const mean = this.recentRewards.reduce((sum, r) => sum + r, 0) /
            this.recentRewards.length;
        const variance = this.recentRewards.reduce((sum, r) => sum + Math.pow(r - mean, 2), 0) /
            this.recentRewards.length;
        return variance;
    }
    /**
...
exports.LinUCBOptimizer = LinUCBOptimizer;
exports.default = LinUCBOptimizer;

...
exports.IncidentAutoReweighter = void 0;
const events_1 = require("events");
const crypto_1 = require("crypto");
class IncidentAutoReweighter extends events_1.EventEmitter {
    redis;
    logger;
    config;
    metrics;
    activeReweights = new Map();
    restorationTimers = new Map();
    isProcessing = false;
    ...
    async processIncident(incident) {
        ...
        try {
            ...
            const existingReweight = Array.from(this.activeReweights.values()).find((r) => r.tenantId === incident.tenantId &&
    ...
    shouldTriggerReweight(incident) {
    ...
    async getCurrentAppSettings(tenantId, appId) {
        try {
    ...
    async createReweight(incident, currentSettings) {
        const reweightId = (0, crypto_1.randomUUID)();
        const now = new Date();
    ...
    async storeReweight(reweight) {
        const key = `incident:reweight:${reweight.id}`;
        await this.redis.setex(key, Math.ceil(this.config.pinDurationMs / 1000) + 3600, // Add 1 hour buffer
        JSON.stringify(reweight));
    }
    scheduleRestoration(reweight) {
    ...
    async extendReweight(existingReweight, newIncident) {
        // Extend the end time
        const now = new Date();
     ...
     */
    async restoreOriginalSettings(reweightId) {
        try {
     ...
     */
    async manualRestore(tenantId, appId) {
        const reweight = Array.from(this.activeReweights.values()).find((r) => r.tenantId === tenantId && r.appId === appId && r.status === 'active');
        if (!reweight) {
            this.logger.warn({
    ...
    updateMetrics(incident, responseTime) {
...
exports.IncidentAutoReweighter = IncidentAutoReweighter;
exports.default = IncidentAutoReweighter;

...
exports.StateMerkleAnchor = void 0;
const events_1 = require("events");
const logger_js_1 = __importDefault(require("../config/logger.js"));
const crypto = __importStar(require("crypto"));
const logger = logger_js_1.default.child({ module: 'StateMerkleAnchor' });
/**
 ...
 */
class StateMerkleAnchor extends events_1.EventEmitter {
    config;
    pendingAnchors = new Map();
    anchorHistory = new Map();
    batchTimer;
    ...
    constructor(config = {}) {
        ...
        if (this.config.anchorFrequencyMinutes > 0) {
            this.batchTimer = setInterval(() => this.processBatchAnchors(), this.config.anchorFrequencyMinutes * 60 * 1000);
        }
     ...
     */
    async anchorState(stateSnapshot, priority = 1) {
        try {
     ...
     */
    async verifyStateIntegrity(currentState, anchorId) {
        try {
     ...
     */
    async createImmediateAnchor(anchorId, stateHash, stateSnapshot, metadata) {
        const timestamp = new Date();
     ...
     * Verify Merkle proof
     */
    async verifyMerkleProof(anchor, currentHash) {
        try {
     ...
     */
    async submitBlockchainAnchor(merkleRoot) {
        try {
     ...
     */
    async submitS3Anchor(anchorId, merkleRoot, anchors) {
        try {
     ...
     */
    async verifyExternalIntegrity(anchor) {
        let s3Valid = true;
     ...
     */
    async verifyS3ObjectIntegrity(s3Location) {
        try {
     ...
     */
    async verifyBlockchainAnchor(transactionId) {
        try {
    ...
    async getBlockHeight(transactionId) {
        try {
            const transaction = await this.blockchainClient.getTransaction(transactionId);
    ...
    getAnchorStatistics() {
        ...
        const stats = {
            ...
            anchorsByType: new Map(),
            anchorsByTenant: new Map(),
            averageAnchorSize: 0,
            oldestAnchor: null,
            newestAnchor: null,
            blockchainAnchors: 0,
        ...
        if (anchors.length > 0) {
            ...
            for (const anchor of anchors) {
                ...
                if (anchor.transactionId)
                    stats.blockchainAnchors++;
                if (anchor.s3Location)
                    stats.s3Anchors++;
            }
    ...
    exportState() {
        return {
            config: this.config,
            anchorHistory: Object.fromEntries(Array.from(this.anchorHistory.entries()).slice(-100)),
...
exports.StateMerkleAnchor = StateMerkleAnchor;
exports.default = StateMerkleAnchor;

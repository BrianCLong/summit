...
import { Driver, Session, Result, Integer, Transaction } from 'neo4j-driver';
...
export class TenantNeo4jSession {
  ...
  async executeRead<T>(
    ...
    return this.session.executeRead(async (tx: Transaction) => {
  ...
  async executeWrite<T>(
    ...
    return this.session.executeWrite(async (tx: Transaction) => {
      // Wrap transaction to inject tenant context
      const tenantAwareTx = this.wrapTransaction(tx);
      return work(tenantAwareTx);
    });

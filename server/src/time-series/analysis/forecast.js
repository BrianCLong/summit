...
exports.arimaForecast = arimaForecast;
exports.prophetForecast = prophetForecast;
function arimaForecast(series, horizon, options = {}) {
    if (!series.length || horizon <= 0)
        return [];
...
function prophetForecast(rows, field, horizon, options = {}) {
    if (!rows.length || horizon <= 0)
        return [];
    const series = rows.map((row) => row.values[field]).filter((value) => typeof value === 'number');
    if (!series.length)
        return [];
...
function differenceSeries(series, order) {
...
function computeSeasonality(series, seasonality) {
    const buckets = Array.from({ length: seasonality }, () => []);

...
exports.downsampleAverage = downsampleAverage;
exports.largestTriangleThreeBuckets = largestTriangleThreeBuckets;
function downsampleAverage(rows, bucketSize) {
    if (!rows.length || bucketSize <= 1)
        return rows;
    const buckets = [];
...
function largestTriangleThreeBuckets(rows, threshold) {
...
function averagePoint(rows) {
    if (!rows.length) {
        return { timestamp: new Date(0), values: {} };
    }
    const timestamp = new Date(rows.reduce((acc, row) => acc + row.timestamp.getTime(), 0) / Math.max(1, rows.length));

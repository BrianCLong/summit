...
const s3 = process.env.AWS_S3_BUCKET
    ? (() => {
        try {
            ...
            console.warn('AWS SDK not installed. S3 WORM storage unavailable. Install @aws-sdk/client-s3 for S3 support.');
            return null;
        }
    })()
    : null;
...
async function putLocked(bucket, key, body, days = Number(process.env.AUDIT_RETENTION_DAYS || 365)) {
    if (s3) {
        // S3 Mode (Production)

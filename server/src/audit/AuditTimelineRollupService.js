...
exports.AuditTimelineRollupService = void 0;
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
class AuditTimelineRollupService {
    pool;
    logger = logger_js_1.default.child({ module: 'audit-timeline-rollups' });
    constructor(pool) {
        this.pool = pool ?? (0, database_js_1.getPostgresPool)();
    }
    async refreshRollups(options = {}) {
        const client = await this.pool.connect();
        const startedAt = new Date();
        try {
            await client.query('BEGIN');
            const columns = await this.getAuditColumns(client);
            const windowStart = options.from ?? (await this.getWatermark(client, columns));
            ...
            const aggregates = await this.aggregateEvents(client, columns, windowStart, windowEnd);
            const dailyBuckets = await this.writeRollups(client, 'audit_event_rollups_daily', 'day', aggregates.daily);
            const weeklyBuckets = await this.writeRollups(client, 'audit_event_rollups_weekly', 'week', aggregates.weekly);
            await this.updateState(client, startedAt, windowEnd, aggregates.processed, 'ok', null);
            await client.query('COMMIT');
            return {
                windowStart,
                windowEnd,
            ...
            await this.updateState(client, startedAt, options.to ?? new Date(), 0, 'error', error?.message ?? 'unknown error');
            this.logger.error({ err: error?.message ?? error }, 'Failed to refresh audit timeline rollups');
            throw error;
        }
        finally {
            client.release();
    ...
    `);
        ...
        const timestampColumn = names.has('timestamp')
            ...
            : names.has('created_at')
                ...
                : null;
        if (!timestampColumn) {
            throw new Error('audit_events table is missing a timestamp column for rollups');
        }
        return {
            timestampColumn,
    ...
    async getWatermark(client, columns) {
    ...
    `);
        ...
        const { rows } = await client.query(`SELECT MIN(${columns.timestampColumn}) AS min_ts FROM audit_events`);
        const minTs = rows[0]?.min_ts;
        return minTs ? new Date(minTs) : undefined;
    }
    async aggregateEvents(client, columns, start, end) {
    ...
    `, [start, end]);
      ...
      GROUP BY 1,2,3,4,5,6
    `, [start, end]);
        const processed = daily.rows.reduce((sum, row) => sum + Number(row.event_count ?? 0), 0);
        return { processed, daily: daily.rows, weekly: weekly.rows };
    }
    async writeRollups(client, table, granularity, rows) {
        if (!rows.length)
            return 0;
        for (const row of rows) {
            await this.ensurePartition(client, table, granularity, row.bucket_start, row.bucket_end);
        }
        const columns = [
            'bucket_start',
            'bucket_end',
        ...
        const placeholders = rows.map((row, idx) => {
            ...
            values.push(row.bucket_start, row.bucket_end, row.tenant_id, row.event_type, row.level, row.service_id, Number(row.event_count ?? 0), row.last_event_timestamp ? new Date(row.last_event_timestamp) : null);
    ...
    async ensurePartition(client, table, granularity, bucketStart, bucketEnd) {
    ...
    async updateState(client, startedAt, completedAt, processed, status, lastError) {
        await client.query(`
    ...
    async readFromRollups(params) {
        const table = params.granularity === 'week'
...
exports.AuditTimelineRollupService = AuditTimelineRollupService;

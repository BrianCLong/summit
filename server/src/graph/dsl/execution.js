...
exports.buildCypherFromDSL = buildCypherFromDSL;
function parseDSL(json) {
    try {
        const parsed = JSON.parse(json);
        if (!parsed.start)
            throw new Error('Query must have a start clause');
        validateDSL(parsed);
        return parsed;
    }
    catch (e) {
...
function buildCypherFromDSL(query, tenantId) {

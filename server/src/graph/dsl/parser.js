    ...
    parseQuery() {
        ...
        while (this.peek() === '-' || this.peek() === '<') {
            ...
            if (targetNode.type || targetNode.filter) {
                traversal.target = {};
                if (targetNode.type)
                    traversal.target.type = targetNode.type;
                if (targetNode.filter)
                    traversal.target.filter = targetNode.filter;
            }
    ...
    parseNode() {
        ...
        if (this.peek() === '(') {
            ...
            while (this.peek() !== ')') {
                ...
                if (!keyMatch)
                    throw new Error(`Expected property key at ${this.pos}`);
    ...
    parseTraversal() {
        ...
        if (this.match('->')) {
            ...
            if (direction === 'in')
                ...
                direction = 'both'; // -- (undirected/both)
        }
        this.consumeWhitespace();

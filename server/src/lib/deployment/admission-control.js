...
exports.ArtifactVerifier = void 0;
const child_process_1 = require("child_process");
const util_1 = require("util");
const execAsync = (0, util_1.promisify)(child_process_1.exec);
class ArtifactVerifier {
    policy;
    constructor(policy = { verifySignature: true, verifyAttestation: true, verifySbom: true }) {
        this.policy = policy;
    }
    ...
    async checkSignature(imageRef) {
        ...
        try {
            ...
            if (e.message.includes('command not found')) {
                ...
                if (imageRef.includes('unsigned'))
                    return { allowed: false, reason: 'Signature missing (Mock)' };
    ...
    async checkAttestation(imageRef) {
        if (process.env.SIMULATE_VERIFICATION === 'true') {
            if (imageRef.includes('no-attest'))
                return { allowed: false, reason: 'Attestation missing (SIMULATED)' };
    ...
    async checkSbom(imageRef) {
        if (process.env.SIMULATE_VERIFICATION === 'true') {
            if (imageRef.includes('no-sbom'))
                return { allowed: false, reason: 'SBOM missing (SIMULATED)' };
...
exports.ArtifactVerifier = ArtifactVerifier;

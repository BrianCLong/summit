...
let faultConfig: FaultConfig = {
  ...
  affectedOperations: (process.env.CHAOS_OPERATIONS || '').split(',').filter(Boolean)
};
...
export function shouldInjectFault(
  ...
  if (faultConfig.globalFailRate > 0 && Math.random() < faultConfig.globalFailRate) {
 ...
 */
export function flaky(faultType: keyof FaultFlags, operationName?: string): boolean {
...
function recordInjection(faultType: string, reason: string): boolean {
  ...
  faultStats.injectionsByType[faultType] = (faultStats.injectionsByType[faultType] || 0) + 1;
 ...
 */
export function injectProviderFault(provider: string, operation: string): Error | null {
...
export async function injectNetworkLatency(operation: string): Promise<void> {
  if (faultFlags.networkLatencyMs > 0) {
    const latency = faultFlags.networkLatencyMs + (Math.random() * faultFlags.networkLatencyMs * 0.5);
 ...
 */
function getFaultForType(faultType: keyof FaultFlags, operation: string): Error | null {
  switch (faultType) {
    case 'neo4jFailRate':
  ...
  return (req: any, res: any, next: any) => {
    if (req.path === '/chaos/faults' && req.method === 'GET') {
      ...
      if (scenario && FaultScenarios[scenario as keyof typeof FaultScenarios]) {
        ...
        res.status(400).json({ error: 'Unknown scenario', availableScenarios: Object.keys(FaultScenarios) });
      }
    } else {
      next();
    }

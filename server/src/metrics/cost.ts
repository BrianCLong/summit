...
export class CostTracker {
  ...
  async trackDatabaseOperation(
    ...
    return tracer.startActiveSpan('cost.track_db_operation', async (span: Span) => {
  ...
  async trackAPIRequest(
    ...
    return tracer.startActiveSpan('cost.track_api_request', async (span: Span) => {
  ...
  async trackAIProcessing(
    ...
    return tracer.startActiveSpan('cost.track_ai_processing', async (span: Span) => {
  ...
  private async updateTenantTotalCost(tenantId: string, additionalCost: number): Promise<void> {
    // Update hourly total
  ...
  private async checkBudgetAlerts(tenantId: string): Promise<void> {
    ...
    const currentHourlySpend = 10.50; // Placeholder - would be actual spend
    const hourlyBudget = 15.00; // Placeholder - would be from tenant config
  ...
  private async triggerBudgetAlert(tenantId: string, alert: BudgetAlert): Promise<void> {
    costAlerts.inc({

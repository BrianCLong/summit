...
exports.costTracker = exports.CostTracker = exports.AlertSeverity = exports.CostTier = exports.BillingPeriod = exports.costOptimizationSavings = exports.costAlerts = exports.tenantBudgetUtilization = exports.tenantTotalCosts = exports.aiProcessingCosts = exports.apiRequestCosts = exports.networkCosts = exports.storageIOCosts = exports.storageCosts = exports.cpuUsageCosts = exports.memoryUsageCosts = exports.computeCosts = exports.dbConnectionCosts = exports.dbOperationCosts = void 0;
const prom_client_1 = require("prom-client");
const api_1 = require("@opentelemetry/api");
const tracer = api_1.trace.getTracer('maestro-cost-metrics', '24.2.0');
// Database operation costs
exports.dbOperationCosts = new prom_client_1.Counter({
...
exports.dbConnectionCosts = new prom_client_1.Gauge({
...
var CostTier;
(function (CostTier) {
    CostTier["SMALL"] = "small";
    CostTier["MEDIUM"] = "medium";
    CostTier["LARGE"] = "large";
    CostTier["XLARGE"] = "xlarge";
...
class CostTracker {
    ...
    async trackDatabaseOperation(dbType, operation, tenantId, rowsAffected = 1) {
        return tracer.startActiveSpan('cost.track_db_operation', async (span) => {
    ...
    async trackComputeUsage(service, tenantId, cpuHours, memoryGBHours, gpuHours = 0) {
        return tracer.startActiveSpan('cost.track_compute', async (span) => {
            ...
            exports.cpuUsageCosts.set({
                service,
                tenant_id: tenantId,
                cpu_tier: this.determineCpuTier(cpuHours),
            }, cpuCost);
            exports.memoryUsageCosts.set({
                service,
                tenant_id: tenantId,
    ...
    async trackStorageUsage(storageType, tenantId, gbStored, ioOperations = 0) {
        return tracer.startActiveSpan('cost.track_storage', async (span) => {
            const storageCost = (gbStored * this.costPerStorageGB[storageType]) / 30 / 24; // Per hour
            const ioCost = ioOperations * 0.0001; // $0.0001 per IO operation
            exports.storageCosts.inc({
                storage_type: storageType,
                tenant_id: tenantId,
                size_tier: this.determineStorageTier(gbStored),
            }, storageCost);
            if (ioCost > 0) {
                exports.storageIOCosts.inc({
                    storage_type: storageType,
                    operation: 'io',
                    tenant_id: tenantId,
                }, ioCost);
    ...
    async trackAPIRequest(endpoint, tenantId, requestSizeKB = 1) {
        return tracer.startActiveSpan('cost.track_api_request', async (span) => {
    ...
    async trackAIProcessing(model, operation, tenantId, tokensProcessed) {
        return tracer.startActiveSpan('cost.track_ai_processing', async (span) => {
            const costPerToken = 0.00001; // $0.00001 per token
            const totalCost = tokensProcessed * costPerToken;
            exports.aiProcessingCosts.inc({
                model,
                operation,
                tenant_id: tenantId,
            }, totalCost);
    ...
    async checkBudgetAlerts(tenantId) {
        ...
        exports.tenantBudgetUtilization.set({
            tenant_id: tenantId,
            billing_period: BillingPeriod.HOURLY,
        }, utilizationPercent);
    ...
    async triggerBudgetAlert(tenantId, alert) {
        exports.costAlerts.inc({
            alert_type: alert.alertType,
            tenant_id: tenantId,
            severity: alert.severity,
        });
        console.warn(`💰 BUDGET ALERT [${alert.severity.toUpperCase()}]: Tenant ${tenantId} at ${alert.utilizationPercent.toFixed(1)}% of budget`);
        // In production, this would send notifications via email, Slack, etc.
    }
    determineSizeTier(value) {
        if (value < 100)
            return CostTier.SMALL;
        if (value < 1000)
            return CostTier.MEDIUM;
        if (value < 10000)
            return CostTier.LARGE;

...
exports.SummitsightDataService = void 0;
const database_js_1 = require("../config/database.js");
class SummitsightDataService {
    pool;
    constructor() {
    ...
    // --- KPI Methods ---
    async getKPIDefinitions(category) {
    ...
    async getKPIValues(kpiId, tenantId, period = 'daily', limit = 30) {
        let query = `
      SELECT * FROM summitsight_kpi_values
      WHERE kpi_id = $1 AND period = $2
    `;
        const params = [kpiId, period];
        let idx = 3;
        if (tenantId) {
    ...
    // --- Forecast & Risk ---
    async getForecasts(kpiId, tenantId) {
    ...
    async aggregateDailyRunStats(tenantId, date) {
    ...
    async aggregateDeploymentStats(tenantId, date) {
    ...
    async aggregateChangeFailStats(tenantId, date) {
    ...
    async aggregateIncidentStats(tenantId, date) {
        const query = `
          SELECT COUNT(*) as count
...
exports.SummitsightDataService = SummitsightDataService;

...
exports.checkPersistedQuery = checkPersistedQuery;
exports.normalizeQuery = normalizeQuery;
exports.generateQueryHash = generateQueryHash;
exports.checkIntrospectionAllowed = checkIntrospectionAllowed;
exports.buildQueryAllowlist = buildQueryAllowlist;
exports.addToAllowlist = addToAllowlist;
exports.getAllowlistStats = getAllowlistStats;
const crypto_1 = require("crypto");
// SHA-256 allowlist of approved queries (populated from build process)
const queryAllowlist = new Set(JSON.parse(process.env.GQL_SHA256_ALLOWLIST || '[]'));
 ...
 */
function checkPersistedQuery(query) {
    // Skip check in non-production environments
    if (process.env.NODE_ENV !== 'production') {
        return;
    }
    const normalizedQuery = normalizeQuery(query);
    const queryHash = (0, crypto_1.createHash)('sha256').update(normalizedQuery).digest('hex');
 ...
 */
function checkIntrospectionAllowed(query) {
    if (process.env.NODE_ENV !== 'production') {
        return;
    }

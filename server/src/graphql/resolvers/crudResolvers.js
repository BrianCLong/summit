...
exports.crudResolvers = exports.buildGraphEventFilter = void 0;
const node_crypto_1 = require("node:crypto");
const database_js_1 = require("../../config/database.js"); // Note: .js extension for ESM
const logger_js_1 = __importDefault(require("../../utils/logger.js")); // Note: .js extension for ESM
const crypto_1 = __importDefault(require("crypto")); // Import crypto for audit log
const CustomSchemaService_js_1 = require("../../services/CustomSchemaService.js");
const neighborhood_cache_js_1 = require("../../services/neighborhood-cache.js");
const subscriptionEngine_js_1 = require("../subscriptionEngine.js");
const nbhdCache = new neighborhood_cache_js_1.NeighborhoodCache((0, database_js_1.getRedisClient)());
const GRAPH_EVENTS = 'GRAPH_EVENTS';
const buildMetadata = (user, investigationId, type) => ({
    tenantId: user?.tenantId,
    userId: user?.id,
    investigationId,
    type,
});
const emitGraphEvent = async (trigger, payload, metadata) => {
    await Promise.all([
        subscriptionEngine_js_1.subscriptionEngine.publish(trigger, payload, metadata),
        subscriptionEngine_js_1.subscriptionEngine.publish(GRAPH_EVENTS, payload, metadata),
    ]);
};
const buildGraphEventFilter = (user, investigationId, types) => (event) => {
    if (!user?.tenantId) {
        return false;
    }
    ...
    const matchesInvestigation = !investigationId ||
        event.metadata?.investigationId === investigationId ||
        event.payload?.investigationId === investigationId;
...
exports.buildGraphEventFilter = buildGraphEventFilter;
const crudResolvers = {
    Query: {
        // Entity queries
        ...
        investigation: async (_, { id }, { user }) => {
           ...
           RETURN i, creator, updater, collect(assigned) as assignedUsers`, { id });
                ...
                const assignedUsers = record
                    .get('assignedUsers')
                    .map((u) => u.properties);
                const customSchema = await (0, CustomSchemaService_js_1.getCustomSchema)(id);
                return {
                    ...investigation,
                    createdBy: creator,
                    updatedBy: updater,
    ...
    Mutation: {
        ...
        createEntity: async (_, { input }, { user }) => {
            ...
            try {
                ...
                if (input.customMetadata) {
                    await (0, CustomSchemaService_js_1.validateCustomMetadata)(input.investigationId, input.customMetadata);
                }
           ...
           RETURN e`, {
                ...
                await emitGraphEvent('ENTITY_CREATED', {
                ...
                if (input.investigationId) {
                    await nbhdCache.invalidate(user.tenantId ?? 'unknown-tenant', input.investigationId, [id]);
                }
                return entity;
            }
            finally {
        ...
        createEntities: async (_, { inputs }, { user }) => {
            ...
            const opId = (0, node_crypto_1.randomUUID)();
            const created = [];
            try {
            ...
            for (const entity of created) {
                ...
                await nbhdCache.invalidate(user.tenantId ?? 'unknown-tenant', entity.investigationId, [entity.id]);
                logger_js_1.default.info(`Entity created: ${entity.id} by user ${user.id}`);
            }
            return created;
        },
        updateEntity: async (_, { id, input }, { user }) => {
            ...
            try {
                ...
                if (customMetadata !== undefined) {
                    ...
                    if (invId) {
                        await (0, CustomSchemaService_js_1.validateCustomMetadata)(invId, customMetadata);
                    }
           ...
           RETURN e`, params);
                ...
                const entity = result.records[0].get('e').properties;
                await emitGraphEvent('ENTITY_UPDATED', {
                    entityUpdated: entity,
                    investigationId: entity.investigationId,
                }, buildMetadata(user, entity.investigationId, 'ENTITY_UPDATED'));
                logger_js_1.default.info(`Entity updated: ${id} by user ${user.id}`);
                await nbhdCache.invalidate(user.tenantId ?? 'unknown-tenant', entity.investigationId, [id]);
                return entity;
            }
            finally {
                await session.close();
        ...
        deleteEntity: async (_, { id }, { user }) => {
           ...
           RETURN e.investigationId as investigationId`, { id });
                ...
                await emitGraphEvent('ENTITY_DELETED', {
                    entityDeleted: id,
                    investigationId,
                }, buildMetadata(user, investigationId, 'ENTITY_DELETED'));
                logger_js_1.default.info(`Entity deleted: ${id} by user ${user.id}`);
                await nbhdCache.invalidate(user.tenantId ?? 'unknown-tenant', investigationId, [id]);
                return true;
            }
            finally {
                await session.close();
        ...
        createRelationship: async (_, { input }, { user }) => {
           ...
           RETURN r, from, to`, {
                ...
                await emitGraphEvent('RELATIONSHIP_CREATED', {
                    relationshipCreated: relationship,
                    investigationId: input.investigationId,
                }, buildMetadata(user, input.investigationId, 'RELATIONSHIP_CREATED'));
                logger_js_1.default.info(`Relationship created: ${id} by user ${user.id}`);
                if (input.investigationId) {
                    await nbhdCache.invalidate(user.tenantId ?? 'unknown-tenant', input.investigationId, [input.fromEntityId, input.toEntityId]);
                }
                return relationship;
            }
            finally {
        ...
        createRelationships: async (_, { inputs }, { user }) => {
            ...
            for (const rel of created) {
                ...
                await nbhdCache.invalidate(user.tenantId ?? 'unknown-tenant', rel.investigationId, [rel.fromEntity.id, rel.toEntity.id]);
                logger_js_1.default.info(`Relationship created: ${rel.id} by user ${user.id}`);
            }
            return created;
        },
        updateRelationship: async (_, { id, input }, { user }) => {
           ...
           RETURN r, from, to`, params);
                ...
                await emitGraphEvent('RELATIONSHIP_UPDATED', {
                ...
                logger_js_1.default.info(`Relationship updated: ${id} by user ${user.id}`);
                await nbhdCache.invalidate(user.tenantId ?? 'unknown-tenant', relationship.investigationId, [relationship.fromEntity.id, relationship.toEntity.id]);
                return relationship;
            }
            finally {
                await session.close();
        ...
        deleteRelationship: async (_, { id }, { user }) => {
           ...
           RETURN r.investigationId as investigationId, a.id as fromId, b.id as toId`, { id });
                ...
                await emitGraphEvent('RELATIONSHIP_DELETED', {
                    relationshipDeleted: id,
                    investigationId,
                }, buildMetadata(user, investigationId, 'RELATIONSHIP_DELETED'));
                logger_js_1.default.info(`Relationship deleted: ${id} by user ${user.id}`);
                await nbhdCache.invalidate(user.tenantId ?? 'unknown-tenant', investigationId, [fromId, toId]);
                return true;
            }
            finally {
                await session.close();
        ...
        createInvestigation: async (_, { input }, { user }) => {
           ...
           RETURN i`, {
                ...
                await (0, CustomSchemaService_js_1.setCustomSchema)(id, input.customSchema || []);
                const customSchema = await (0, CustomSchemaService_js_1.getCustomSchema)(id);
    ...
    Subscription: {
        entityCreated: {
            subscribe: (_, { investigationId }, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createFilteredAsyncIterator('ENTITY_CREATED', (0, exports.buildGraphEventFilter)(user, investigationId));
            },
            resolve: (event) => event.payload.entityCreated ?? event.payload,
        },
        entityUpdated: {
            subscribe: (_, { investigationId }, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createFilteredAsyncIterator('ENTITY_UPDATED', (0, exports.buildGraphEventFilter)(user, investigationId));
            },
            resolve: (event) => event.payload.entityUpdated ?? event.payload,
        },
        entityDeleted: {
            subscribe: (_, { investigationId }, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createFilteredAsyncIterator('ENTITY_DELETED', (0, exports.buildGraphEventFilter)(user, investigationId));
            },
            resolve: (event) => event.payload.entityDeleted ?? event.payload,
        },
        relationshipCreated: {
            subscribe: (_, { investigationId }, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createFilteredAsyncIterator('RELATIONSHIP_CREATED', (0, exports.buildGraphEventFilter)(user, investigationId));
            },
            resolve: (event) => event.payload.relationshipCreated ?? event.payload,
        },
        relationshipUpdated: {
            subscribe: (_, { investigationId }, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createFilteredAsyncIterator('RELATIONSHIP_UPDATED', (0, exports.buildGraphEventFilter)(user, investigationId));
            },
            resolve: (event) => event.payload.relationshipUpdated ?? event.payload,
        },
        relationshipDeleted: {
            subscribe: (_, { investigationId }, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createFilteredAsyncIterator('RELATIONSHIP_DELETED', (0, exports.buildGraphEventFilter)(user, investigationId));
            },
            resolve: (event) => event.payload.relationshipDeleted ?? event.payload,
        },
        investigationUpdated: {
            subscribe: (_, { investigationId }, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createFilteredAsyncIterator('INVESTIGATION_UPDATED', (0, exports.buildGraphEventFilter)(user, investigationId));
            },
            resolve: (event) => event.payload.investigationUpdated ?? event.payload,
        },
        graphUpdated: {
            subscribe: (_, { investigationId }, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createFilteredAsyncIterator(GRAPH_EVENTS, (0, exports.buildGraphEventFilter)(user, investigationId));
            },
            resolve: (event) => event.payload,
        },
        graphEvents: {
            subscribe: (_, { investigationId, types, batchSize, flushIntervalMs, } = {}, { user }) => {
                if (!user)
                    throw new Error('Not authenticated');
                return subscriptionEngine_js_1.subscriptionEngine.createBatchedAsyncIterator(GRAPH_EVENTS, (0, exports.buildGraphEventFilter)(user, investigationId, types), { batchSize: batchSize ?? 20, flushIntervalMs: flushIntervalMs ?? 250 });
            },
            resolve: (batch) => ({
                cursor: batch[batch.length - 1]?.id ?? (0, node_crypto_1.randomUUID)(),

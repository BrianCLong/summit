...
exports.legacyResolvers = {
    Mutation: {
        ...
        generateEntitiesFromText: async (_, { investigationId, text }, context) => {
            ...
            // Validate text input
            if (!text || text.trim() === '') {
                throw new graphql_1.GraphQLError('Text input is required and cannot be empty', {
                    extensions: {
                        code: 'BAD_USER_INPUT',
                        http: { status: 400 },
                    },
            ...
            if (text.length > MAX_TEXT_SIZE) {
                throw new graphql_1.GraphQLError(`Text input exceeds maximum size of ${MAX_TEXT_SIZE} characters`, {
                    extensions: {
                        code: 'BAD_USER_INPUT',
                        http: { status: 413 },
                    },

...
export const aiAnalysisResolvers = {
  Query: {
    ...
    extractEntities: async (
      ...
      logger.info(
        `Extracting entities from text. Text Length: ${text.length}, Extract Relationships: ${extractRelationships}, Confidence Threshold: ${confidenceThreshold}`,
      );
    ...
    analyzeRelationships: async (
      ...
      logger.info(
        `Analyzing relationships. Entities Count: ${entities.length}`,
      );

      const relationships = await aiAnalysisService.analyzeRelationships(
        entities,
        text,
      );
    ...
    generateEntityInsights: async (
      ...
      logger.info(
        `Generating entity insights. Entity ID: ${entityId}, Entity Type: ${entityType}`,
      );

      const insights = await aiAnalysisService.generateEntityInsights(
        entityId,
        entityType,
        properties,
      );
      ...
      const insights = [
        {
          ...
          severity: 'medium',
          message:
            'Found 5 entities without relationships - consider connecting them',
          suggestions: [
    ...
    predictLinks: async (
      ...
      logger.info(
        `Predicting links for entities: ${entityIds.join(', ')}. Prediction Type: ${predictionType}`,
      );
      ...
      const predictedLinks = [
        {
          ...
          confidence: 0.85,
          explanation:
            'Entities frequently co-occur in analyzed texts and share common attributes.',
        },
        {
          sourceId: entityIds[0] || 'entity-1',
          targetId: 'entity-3',
        ...
        },
        {
          ...
          confidence: 0.68,
          explanation:
            'Weak semantic similarity detected in their respective contexts.',
        },
  ...
  Mutation: {
    ...
    applyAISuggestions: async (
      ...
      logger.info(
        `Applying AI suggestions. Graph ID: ${graphId}, Suggestion Count: ${suggestionIds.length}`,
      );
    ...
    enhanceEntitiesWithAI: async (
      ...
      logger.info(
        `Enhancing entities with AI. Entity Count: ${entityIds.length}, Enhancement Types: ${enhancementTypes.join(', ')}`,
      );
      ...
      return {
        ...
        totalEnhancementsApplied: enhancements.reduce(
          (sum, e) =>
            sum +
            e.enhancements.properties.length +
            e.enhancements.relationships.length +
            e.enhancements.insights.length,
          0,
        ),
      };
    },

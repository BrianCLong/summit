...
function initializeServices() {
    if (!graphRAGService) {
        ...
        logger_js_1.logger.info('GraphRAG services initialized');
    }
...
exports.graphragResolvers = {
    Query: {
         ...
         */
        graphRagAnswer: (0, cacheHelper_js_1.withCache)(
        // Cache key generator
        ...
        async (_, args, context) => {
            ...
            const { input } = args;
            try {
                logger_js_1.logger.info(`GraphRAG query received. Investigation ID: ${input.investigationId}, User ID: ${context.user.id}, Question Length: ${input.question.length}, Use Case: ${input.useCase || 'default'}`);
                const request = {
                ...
                logger_js_1.logger.info(`GraphRAG query completed. Investigation ID: ${input.investigationId}, User ID: ${context.user.id}, Confidence: ${response.confidence}, Citations Count: ${response.citations.entityIds.length}, Why Paths Count: ${response.why_paths.length}`);
                return response;
            }
            catch (error) {
                logger_js_1.logger.error(`GraphRAG query failed. Investigation ID: ${input.investigationId}, User ID: ${context.user.id}, Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        ...
        similarEntities: async (_, args, context) => {
            ...
            try {
                logger_js_1.logger.info(`Similarity search requested. Entity ID: ${entityId}, Text: ${text?.substring(0, 50)}, Top K: ${topK}, Investigation ID: ${investigationId}, User ID: ${context.user.id}`);
                ...
                logger_js_1.logger.info(`Similarity search completed. Investigation ID: ${investigationId}, User ID: ${context.user.id}, Results Count: ${similarEntities.length}, Execution Time: ${result.executionTime}`);
                return similarEntities;
            }
            catch (error) {
                logger_js_1.logger.error(`Similarity search failed. Entity ID: ${entityId}, Investigation ID: ${investigationId}, User ID: ${context.user.id}, Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    Mutation: {
        ...
        clearGraphRAGCache: async (_, args, context) => {
            ...
            try {
                ...
                logger_js_1.logger.info(`GraphRAG cache clear requested. Investigation ID: ${investigationId}, User ID: ${context.user.id}`);
                ...
                logger_js_1.logger.error(`Cache clear failed. Investigation ID: ${investigationId}, User ID: ${context.user.id}, Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
                return {
                    success: false,
...
async function getGraphRAGHealth() {
    try {
        ...
        logger_js_1.logger.error(`GraphRAG health check failed. Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        return {
            status: 'unhealthy',
            cacheStatus: 'unknown',
            config: {},

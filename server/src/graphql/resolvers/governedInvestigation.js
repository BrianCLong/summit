...
exports.governedInvestigationResolvers = void 0;
exports.getInvestigationCaseGraph = getInvestigationCaseGraph;
const apollo_server_express_1 = require("apollo-server-express");
/**
 * Get investigation case graph with full governance
 */
async function getInvestigationCaseGraph(_parent, args, context) {
    const { investigationId, depth = 3 } = args;
    const { user, opa, neo4j, warrantService, auditSystem, governance, warrant, requestId, sessionId, req, logger, } = context;
    ...
    // ============================================================================
    if (!user) {
        throw new apollo_server_express_1.AuthenticationError('Authentication required to access case graph');
    }
    // ============================================================================
    // 2. VALIDATE GOVERNANCE CONTEXT
    // ============================================================================
    if (!governance) {
        throw new apollo_server_express_1.ForbiddenError('Governance context is required');
    }
    if (!governance.purpose) {
        throw new apollo_server_express_1.ForbiddenError('Purpose header (X-Purpose) is required');
    }
    if (!governance.reasonForAccess) {
        throw new apollo_server_express_1.ForbiddenError('Reason for access header (X-Reason-For-Access) is required');
    }
    // ============================================================================
    ...
    try {
      ...
      `, {
        ...
        if (investigationResult.records.length === 0) {
            ...
            throw new apollo_server_express_1.ForbiddenError('Investigation not found or access denied');
        }
        ...
        if (governance.warrantId) {
            const warrantValidation = await warrantService.validateWarrant(governance.warrantId, {
                resourceType: 'investigation',
                resourceId: investigationId,
                operation: 'view',
                purpose: governance.purpose,
            ...
            if (!warrantValidation.valid) {
                ...
                throw new apollo_server_express_1.ForbiddenError(`Warrant validation failed: ${warrantValidation.reason}. ` +
        ...
        if (!allowed) {
            ...
            throw new apollo_server_express_1.ForbiddenError(`Access denied: ${denyReasons.join(', ')}.\n\n` +
 ...
 * Export resolvers
 */
exports.governedInvestigationResolvers = {
    Query: {

  ...
  Mutation: {
    ...
    uploadMediaSource: withAuthAndPolicy('create', (args) => ({
      ...
      try {
        ...
        const mediaSource =
          await context.multimodalDataService.createMediaSource(
            mediaMetadata,
            context.user.id,
    ...
    batchVerifyEntities: withAuthAndPolicy('update', (args) => ({
      ...
      for (const entityId of entityIds) {
        try {
          const entity =
            await context.multimodalDataService.verifyMultimodalEntity(
              entityId,
              verification,
              context.user.id,
            );
      ...
      logger.info(
        `Batch verified ${verifiedEntities.length}/${entityIds.length} entities`,
      );

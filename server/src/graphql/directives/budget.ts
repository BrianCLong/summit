      ...
      mapSchema(schema, {
        [MapperKind.OBJECT_FIELD]: (fieldConfig) => {
          ...
          if (budget) {
            ...
            (fieldConfig as any).resolve = async (source: any, args: any, ctx: any, info: any) => {
              const capUSD = Number(budget.capUSD);
              const tokenCeiling = Number(budget.tokenCeiling);
              const ok = await ctx?.budget?.check?.({ capUSD, tokenCeiling, field: info.fieldName });
              if (!ok) {
                return { status: 'BLOCKED_BY_BUDGET', warnings: ['Budget exceeded'], diff: null, auditId: `audit-${Date.now()}` };
              }

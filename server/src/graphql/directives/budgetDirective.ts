...
import { estimateTokensAndCost, reconcileActualUsage } from '../../lib/tokcount-enhanced';
...
interface SafePromptBuilder {
  (fieldName: string, args: any, ctx: any): {
    messages?: any[];
    input?: string | object;
    tools?: any[];
  };
...
interface GraphQLContextWithBudget {
  ...
  metrics?: {
    budget_denials_total: { inc: (labels: { reason: string; tenant?: string; provider?: string }) => void };
...
function defaultPromptBuilder(fieldName: string, args: any, ctx: any) {
  ...
  if (input.prompt) {
    ...
    messages.push({ role: 'user', content: input.description || input.content });
  } else if (input.query) {
...
async function withOTEL<T>(
  ...
  const span = {
    ...
    setAttributes: (attrs: Record<string, any>) => {
      ...
      logger.debug('OTEL Span completed', { spanName, attributes: span.attributes });
    }
  };
  ...
  try {
    ...
    span.setAttributes({ 'error.message': error instanceof Error ? error.message : String(error) });
    span.end();
    throw error;
  }
}
  ...
  const transformer = (schema: any) =>
    mapSchema(schema, {
      [MapperKind.OBJECT_FIELD]: (fieldConfig: GraphQLField<any, any>) => {
        const directive = getDirective(schema, fieldConfig, directiveName)?.[0] as BudgetDirectiveArgs;
        ...
        const budgetEnforcedResolver: GraphQLFieldResolver<any, GraphQLContextWithBudget, any> = async (
          source,
          args,
          context,
          info
          ...
          return withOTEL(`graphql.budget.${info.fieldName}`, async (span) => {
            ...
            try {
              ...
              if (tokenCeiling && tokenCeiling > 0 && estimation.totalTokens > tokenCeiling) {
                ...
                const error = new BudgetError(
                  ...
                  {
                    estimated: { tokens: estimation.totalTokens, usd: estimation.totalUSD },
                ...
                span.setAttributes({ 'budget.denied': true, 'budget.denial_reason': 'token_ceiling' });
                throw error;
              }
              ...
              if (estimation.totalUSD > capUSD) {
                ...
                span.setAttributes({ 'budget.denied': true, 'budget.denial_reason': 'cap_usd' });
                throw error;
              }
              ...
              setImmediate(async () => {
                try {
                  ...
                  if (actualUsage) {
                    const errorRatio = (actualUsage.totalTokens || 1) / (estimation.totalTokens || 1);
 ...
 */
export function createBudgetContext(baseContext: any): GraphQLContextWithBudget {
  return {
    ...baseContext,

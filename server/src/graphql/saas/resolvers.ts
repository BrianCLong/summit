...
export const saasResolvers = {
  ...
  Mutation: {
    registerTrustContract: async (_: any, { input }: any) => {
      const pool = getPostgresPool();
      const { rows:[r] } = await pool.query('INSERT INTO trust_contracts(provider,consumer,scope,residency,expires_at,signature) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, provider AS providerTenant, consumer AS consumerTenant, scope, residency, expires_at AS "expiresAt", signature', [input.providerTenant, input.consumerTenant, input.scope, input.residency, input.expiresAt, input.signature]);
      return r;
    },

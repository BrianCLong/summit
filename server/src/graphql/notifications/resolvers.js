...
exports.notificationResolvers = {
    Query: {
        ...
        myNotificationPreferences: async (_parent, _args, context) => {
         ...
         WHERE user_id = $1 AND tenant_id = $2`, [user.id, user.tenantId]);
            if (result.rows.length === 0) {
                // Create default preferences
                const defaults = await db.query(`INSERT INTO notification_preferences (
          ...
          RETURNING *`, [user.id, user.tenantId]);
         ...
         */
        notificationHistory: async (_parent, args, context) => {
            const { db, user } = context;
      ...
      `;
        ...
        unreadNotificationCount: async (_parent, _args, context) => {
            const { db, user } = context;
            const result = await db.query(`SELECT get_unread_notification_count($1) as count`, [user.id]);
            return parseInt(result.rows[0].count, 10);
        },
        /**
         * Get notification statistics
         */
        notificationStats: async (_parent, args, context) => {
            const { db, user } = context;
         ...
         FROM notification_delivery_log
         WHERE user_id = $1 ${dateFilter}`, params);
            // Get stats by channel
            const channelResult = await db.query(`SELECT
           channel,
         ...
         GROUP BY channel`, params);
            // Get stats by severity
            const severityResult = await db.query(`SELECT
           notification_severity as severity,
           COUNT(*) as count
         FROM notification_delivery_log
         WHERE user_id = $1 ${dateFilter}
         GROUP BY notification_severity`, params);
            return {
    ...
    Mutation: {
        ...
        updateNotificationPreferences: async (_parent, args, context) => {
            const { db, user } = context;
            const { input } = args;
            const result = await db.query(`INSERT INTO notification_preferences (
        ...
        markNotificationRead: async (_parent, args, context) => {
            ...
            const { id } = args;
            const result = await db.query(`SELECT mark_notification_read($1, $2) as success`, [id, user.id]);
            if (!result.rows[0].success) {
                throw new Error('Notification not found or already read');
            }
            // Get updated notification
            const notificationResult = await db.query(`SELECT * FROM notification_delivery_log WHERE id = $1`, [id]);
            // Publish unread count update
            const countResult = await db.query(`SELECT get_unread_notification_count($1) as count`, [user.id]);
        ...
        markAllNotificationsRead: async (_parent, _args, context) => {
            const { db, user, pubsub } = context;
            const result = await db.query(`UPDATE notification_delivery_log
         SET read_at = NOW(), status = 'read'
         WHERE user_id = $1 AND read_at IS NULL AND status IN ('sent', 'delivered')`, [user.id]);
            // Publish unread count update
            pubsub.publish('UNREAD_COUNT_UPDATED', {
         ...
         */
        acknowledgeNotification: async (_parent, args, context) => {
            const { db, user } = context;
            const { id, note } = args;
            const result = await db.query(`SELECT acknowledge_notification($1, $2, $3) as success`, [id, user.id, note]);
            if (!result.rows[0].success) {
         ...
         */
        testNotification: async (_parent, args, context) => {
            const { db, user } = context;
            ...
            await db.query(`INSERT INTO notification_delivery_log (
    ...
    NotificationDelivery: {
         ...
         */
        auditEvent: async (parent, _args, context) => {
            const { db } = context;
            const result = await db.query(`SELECT * FROM audit_events WHERE id = $1`, [parent.audit_event_id]);

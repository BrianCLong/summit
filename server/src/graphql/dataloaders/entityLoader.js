...
exports.batchLoadEntitiesWithRelationships = batchLoadEntitiesWithRelationships;
 ...
 */
async function batchLoadEntities(ids, context) {
        ...
        `, { ids: missingIds, tenantId });
            ...
            if (redis && dbEntities.size > 0) {
                ...
                for (const [id, entity] of dbEntities.entries()) {
                    pipeline.setex(`entity:${tenantId}:${id}`, 60, // 60 seconds TTL
 ...
 */
function createEntityLoader(context) {
    return new dataloader_1.default((ids) => batchLoadEntities(ids, context), {
 ...
 */
async function batchLoadEntitiesWithRelationships(ids, context, relationshipType) {
      ...
      `;
        ...
        result.records.forEach((record) => {
            ...
            dataMap.set(entity.id, {
                entity,
                relationships: relationships.filter((r) => r.relationship !== null),
            });
        });
        return dataMap;
    }

...
export const erResolvers: IResolvers = {
  Query: {
    ...
    erPrecisionMetrics: async (_, { filter = {} }, context) => {
      ...
      try {
        ...
        return result.rows.map(row => ({
          ...
          avgMergeConfidence: row.avg_merge_confidence ? parseFloat(row.avg_merge_confidence) : null,
    ...
    resolveEntities: async (_, { entityA, entityB, entityType = 'PERSON' }, context) => {
      try {
        const result = await erService.resolveEntitiesPair(entityA, entityB, entityType);
  ...
  Mutation: {
    decideMerge: async (_, { input }, context) => {
      ...
      try {
        ...
        if (decision.decision === 'MERGE' && !input.forceReview && !decision.reviewRequired) {
          await erService.mergeEntities(session, input.entityA, [input.entityB]);

...
async function applyPIIRedaction(entity: any, context: ResolverContext): Promise<any> {
  ...
  Query: {
    async entityById(
      ...
      try {
        ...
        const cachedEntity = await cacheService.get<Entity>(cacheKey);
        ...
        if (entity) {
          ...
          await cacheService.set(cacheKey, entity, 60);
        }
    ...
    async searchEntities(
      ...
      try {
        ...
        telemetry.recordRequest((Date.now() - startTime) / 1000, { resolver: 'searchEntities' });
      }
    },
    ...
    async entityGraph(
      ...
      try {
        ...
        telemetry.recordRequest((Date.now() - startTime) / 1000, { resolver: 'entityGraph' });
      }
    },
...
async function checkPostgreSQLHealth(pg: Pool): Promise<boolean> {
  try {
...
async function checkNeo4jHealth(driver: neo4j.Driver): Promise<boolean> {
  try {

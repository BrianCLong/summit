    ...
    requestDidStart(): GraphQLRequestListener<any> {
      return {
          ...
          try {
            ...
            if (config.scanVariables && request.variables) {
              ...
              if (variablesStr.length <= (config.maxContentSize || Infinity)) {
                const scanResults = await dlpService.scanContent(request.variables, context);
                
                if (scanResults.length > 0) {
                  violations.push(...scanResults);
                  
                  // Apply DLP actions to variables
                  const { processedContent, blocked } = await dlpService.applyActions(
                    request.variables,
                    scanResults,
                    context
                  );
            ...
            if (config.scanQuery && request.query) {
              if (request.query.length <= (config.maxContentSize || Infinity)) {
                ...
                const scanResults = await dlpService.scanContent(request.query, queryContext);
                
                if (scanResults.length > 0) {
                  violations.push(...scanResults);
                  
                  // For queries, we typically only audit/alert, not block
                  await dlpService.applyActions(request.query, scanResults, queryContext);
                }
              }
            }
            ...
            logger.error('GraphQL DLP plugin error during operation resolution', {
              component: 'DLPPlugin',
              error: error.message,
              operationName,
            ...
            if (config.blockOnViolation && error.message.includes('DLP')) {
              throw new GraphQLError('Request blocked due to data protection policy', {
        ...
        async willSendResponse(requestContext: GraphQLRequestContext<any>) {
          ...
          if (!response.body || response.body.kind !== 'single' || !response.body.singleResult.data) {
            return;
          }
          ...
          try {
            ...
            if (responseStr.length <= (config.maxContentSize || Infinity)) {
              const scanResults = await dlpService.scanContent(responseData, context);
            ...
            logger.error('GraphQL DLP plugin error during response processing', {
              component: 'DLPPlugin',
              error: error.message,
              operationName,
              userId: contextValue.user?.id

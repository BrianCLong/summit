...
exports.createCircuitBreakerPlugin = createCircuitBreakerPlugin;
const graphql_1 = require("graphql");
const pino_1 = __importDefault(require("pino"));
const logger = pino_1.default();
const tenantStates = new Map();
function createCircuitBreakerPlugin(options = {}) {
    const { failureThreshold = 10, resetTimeout = 30000, maxRequestsPerMinute = 1000, } = options;
    return {
        async requestDidStart(requestContext) {
            ...
            if (state.state === 'OPEN') {
                if (now - state.lastFailure > resetTimeout) {
                    ...
                    throw new graphql_1.GraphQLError('Service unavailable (Circuit Breaker)', {

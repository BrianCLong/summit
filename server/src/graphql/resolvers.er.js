...
exports.erResolvers = void 0;
const database_js_1 = require("../config/database.js");
const database_js_2 = require("../config/database.js");
const HybridEntityResolutionService_js_1 = require("../services/HybridEntityResolutionService.js");
...
const log = logger_js_1.default.child({ name: 'ERResolvers' });
const tracer = api_1.trace.getTracer('er-resolvers', '1.0.0');
...
exports.erResolvers = {
    Query: {
        pendingMergeDecisions: async (_, { entityType, limit = 50, offset = 0 }, context) => {
      ...
      `;
            ...
            try {
                ...
                log.error({ error: error.message }, 'Failed to fetch pending merge decisions');
        ...
        mergeDecision: async (_, { id }, context) => {
            const pool = (0, database_js_1.getPostgresPool)();
            try {
                const result = await pool.query('SELECT * FROM merge_decisions WHERE id = $1', [id]);
                ...
                log.error({ error: error.message, id }, 'Failed to fetch merge decision');
        ...
        `;
                ...
                log.error({ error: error.message, filter }, 'Failed to fetch ER precision metrics');
                throw new Error('Failed to fetch ER precision metrics');
            }
        },
        erThresholdCheck: async (_, { entityType = 'PERSON', daysBack = 7 }, context) => {
            const pool = (0, database_js_1.getPostgresPool)();
            try {
                const result = await pool.query('SELECT check_ga_precision_threshold($1, $2) as result', [entityType, daysBack]);
                ...
                log.error({ error: error.message, entityType, daysBack }, 'Failed to check ER threshold');
        ...
        resolveEntities: async (_, { entityA, entityB, entityType = 'PERSON' }, context) => tracer.startActiveSpan('er.resolve_entities', async (span) => {
            span.setAttributes({
                'er.entity_type': entityType,
                'er.operation': 'resolve_entities',
            });
            try {
                const result = await (0, HybridEntityResolutionService_js_1.resolveEntities)(JSON.stringify(entityA), JSON.stringify(entityB));
          ...
          `, [
                ...
                metrics_js_1.erMergeOutcomesTotal.inc({
                    decision,
                ...
                return {
                    ...
                    riskScore: result.riskScore || 1.0 - (result.confidence || result.score),
                ...
                log.error({ error: error.message, entityA, entityB }, 'Failed to resolve entities');
    ...
    Mutation: {
        decideMerge: async (_, { input }, context) => tracer.startActiveSpan('er.decide_merge', async (span) => {
            span.setAttributes({
          ...
          `, { entityA: input.entityA, entityB: input.entityB });
                ...
                const result = await (0, HybridEntityResolutionService_js_1.resolveEntities)(JSON.stringify(entityAProps), JSON.stringify(entityBProps));
                const pool = (0, database_js_1.getPostgresPool)();
        ...
        recordERCIMetric: async (_, { prNumber, commitSha, entityType, precision, sampleSize, threshold }, context) => {
            const pool = (0, database_js_1.getPostgresPool)();
            try {

...
exports.NarrativeImpactModel = void 0;
...
class NarrativeImpactModel {
    ...
    async analyzeNarrativeImpact(tenantId, signals) {
        if (!signals.length)
            return [];
        try {
            logger_js_1.default.info('Starting narrative impact analysis', {
                tenantId,
            ...
            logger_js_1.default.info('Narrative impact analysis completed', {
                tenantId,
            ...
            logger_js_1.default.error('Failed to analyze narrative impact', { error, tenantId });
            throw error;
        }
    }
    async getNarrativeImpacts(tenantId, options = {}) {
        ...
        try {
            ...
            try {
                ...
                const params = { tenantId, minMagnitude };
    ...
    async trackNarrativeThreads(tenantId) {
        const session = this.neo4j.getSession();
        try {
        ...
        `, { tenantId });
            ...
            const threads = result.records.map((record) => {
                ...
                return {
                    ...
                    evolutionTimeline: thread.evolutionTimeline.map((evt) => ({
    ...
    async predictNarrativeEvolution(tenantId, narrativeId, timeHorizon = '7d') {
        try {
            ...
            logger_js_1.default.error('Failed to predict narrative evolution', {
                error,
                tenantId,
                narrativeId,
            });
    ...
    identifyNarrativeSignals(signals) {
        const narrativeSignals = new Map();
        ...
        signals.forEach((signal) => {
            for (const [narrativeType, indicators] of Object.entries(narrativeIndicators)) {
                if (indicators.some((indicator) => signal.type.includes(indicator) ||
    ...
    async buildNarrativeImpact(tenantId, narrativeKey, signals) {
        if (signals.length < 3)
        ...
        if (magnitude < this.impactThresholds.low)
            return null;
    ...
    calculateImpactMagnitude(signals) {
        if (!signals.length)
            return 0;
    ...
    determineImpactDirection(signals) {
        ...
        if (sourceSet.size < targetSet.size)
            return 'downstream'; // One-to-many
        if (sourceSet.size > targetSet.size)
            return 'upstream'; // Many-to-one
        return 'lateral'; // Many-to-many or equal
    }
    async buildPropagationPath(tenantId, signals) {
        ...
        for (const [entityId, entitySignals] of entities.entries()) {
            const impactStrength = entitySignals.reduce((sum, s) => sum + s.value * s.weight, 0) /
    ...
    calculateNarrativeConfidence(signals, magnitude) {
    ...
    generateNarrativeTitle(impactType, signals) {
        ...
        return (titleTemplates[impactType] ||
            `Narrative Impact Detected (${signals.length} signals)`);
    }
    generateNarrativeDescription(impactType, signals, magnitude) {
    ...
    async analyzeNarrativeInteractions(tenantId, impacts) {
            ...
            for (let j = i + 1; j < impacts.length; j++) {
                ...
                if (sharedEntities.length > 0) {
                    ...
                    enhancedImpacts[i].metadata.interactions.push({
                        ...
                        interactionStrength: sharedEntities.length /
                            Math.max(impactA.affectedEntities.length, impactB.affectedEntities.length),
                    });
    ...
    async storeNarrativeImpacts(tenantId, impacts) {
        if (!impacts.length)
            return;
        const session = this.neo4j.getSession();
        try {
    ...
    async cacheNarrativeImpacts(tenantId, impacts) {
        const cacheKey = `narrative_impacts:${tenantId}:latest`;
        await this.redis.setex(cacheKey, 600, JSON.stringify(impacts));
    }
    // Helper methods
    calculateTransmissionDelay(signals) {
        if (signals.length < 2)
            return 0;
        const intervals = [];
        for (let i = 1; i < signals.length; i++) {
            const delay = new Date(signals[i].ts).getTime() -
                new Date(signals[i - 1].ts).getTime();
            intervals.push(delay);
        }
        return (intervals.reduce((sum, delay) => sum + delay, 0) /
            intervals.length /
            (1000 * 60)); // Average delay in minutes
    }
    calculateAmplificationFactor(signals) {
    ...
    calculateResistanceFactor(signals) {
    ...
    inferEntityType(signals) {
        // Simple entity type inference based on signal patterns
        const sources = signals.map((s) => s.source);
        const uniqueSources = new Set(sources);
        if (uniqueSources.size === 1)
            return 'individual';
        if (uniqueSources.size < sources.length * 0.3)
            return 'group';
        return 'network';
    }
    calculateTimeSpan(signals) {
        if (signals.length < 2)
            return '0m';
        const start = new Date(signals.reduce((earliest, s) => (s.ts < earliest ? s.ts : earliest), signals[0].ts));
        const end = new Date(signals.reduce((latest, s) => (s.ts > latest ? s.ts : latest), signals[0].ts));
        ...
        if (diffMinutes < 60)
            return `${Math.ceil(diffMinutes)}m`;
        if (diffMinutes < 1440)
            return `${Math.ceil(diffMinutes / 60)}h`;
        return `${Math.ceil(diffMinutes / 1440)}d`;
    }
    async getHistoricalNarrativeData(tenantId, narrativeId) {
        // Placeholder for historical data retrieval
        return [];
    }
    generateNarrativePredictions(historicalData, timeHorizon) {
        // Placeholder for prediction generation
        return [];
    }
    calculatePredictionConfidence(historicalData, predictions) {
        // Placeholder for confidence calculation
        return 0.5;
    }
    identifyNarrativeRiskFactors(historicalData, predictions) {
        // Placeholder for risk factor identification
        return ['insufficient_historical_data'];
    }
}
exports.NarrativeImpactModel = NarrativeImpactModel;

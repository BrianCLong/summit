...
export interface NarrativeImpact {
  ...
  title: string;
  description: string;
  impactType: 'information_flow' | 'belief_shift' | 'behavioral_change' | 'network_disruption';
  magnitude: number;
...
export class NarrativeImpactModel {
  ...
  async analyzeNarrativeImpact(tenantId: string, signals: any[]): Promise<NarrativeImpact[]> {
    ...
    try {
      ...
      for (const [narrativeKey, narrativeSigs] of narrativeSignals.entries()) {
        const impact = await this.buildNarrativeImpact(tenantId, narrativeKey, narrativeSigs);
        if (impact) {
          impacts.push(impact);
        }
      }
      ...
      const enhancedImpacts = await this.analyzeNarrativeInteractions(tenantId, impacts);
      ...
      logger.info('Narrative impact analysis completed', {
        ...
        highImpactCount: enhancedImpacts.filter(i => i.magnitude >= this.impactThresholds.high).length
      });
  ...
  async getNarrativeImpacts(tenantId: string, options: {
    limit?: number;
    minMagnitude?: number;
    impactTypes?: string[];
    ...
    try {
      ...
      try {
        ...
        if (timeRange) {
          whereClause += ' AND ni.last_updated >= datetime($startTime) AND ni.last_updated <= datetime($endTime)';
      ...
      logger.error('Failed to get narrative impacts', { error, tenantId, options });
      return [];
    }
  }
  ...
  async predictNarrativeEvolution(tenantId: string, narrativeId: string, timeHorizon: string = '7d'): Promise<{
    predictions: any[];
    confidence: number;
    riskFactors: string[];
  }> {
    try {
      ...
      const historicalData = await this.getHistoricalNarrativeData(tenantId, narrativeId);
      
      // Simple trend analysis for prediction
      const predictions = this.generateNarrativePredictions(historicalData, timeHorizon);
      
      // Calculate prediction confidence
      const confidence = this.calculatePredictionConfidence(historicalData, predictions);
      
      // Identify risk factors
      const riskFactors = this.identifyNarrativeRiskFactors(historicalData, predictions);
      ...
      logger.error('Failed to predict narrative evolution', { error, tenantId, narrativeId });
      return {
        predictions: [],
        confidence: 0,
  ...
  private identifyNarrativeSignals(signals: any[]): Map<string, any[]> {
    ...
    const narrativeIndicators = {
      ...
      'behavioral_change': ['action_pattern', 'engagement_change', 'participation_shift'],
      'network_disruption': ['connection_break', 'influence_block', 'communication_gap']
    };

    signals.forEach(signal => {
      for (const [narrativeType, indicators] of Object.entries(narrativeIndicators)) {
        if (indicators.some(indicator => signal.type.includes(indicator) || signal.source.includes(indicator))) {
  ...
  private async buildNarrativeImpact(tenantId: string, narrativeKey: string, signals: any[]): Promise<NarrativeImpact | null> {
    ...
    const impact: NarrativeImpact = {
      ...
      description: this.generateNarrativeDescription(impactType, signals, magnitude),
      ...
      firstDetected: signals.reduce((earliest, s) => s.ts < earliest ? s.ts : earliest, signals[0].ts),
  ...
  private determineImpactDirection(signals: any[]): 'upstream' | 'downstream' | 'lateral' {
  ...
  private async buildPropagationPath(tenantId: string, signals: any[]): Promise<PropagationNode[]> {
    ...
    for (const [entityId, entitySignals] of entities.entries()) {
      const impactStrength = entitySignals.reduce((sum, s) => sum + s.value * s.weight, 0) / entitySignals.length;
      
      // Calculate transmission characteristics
      const transmissionDelay = this.calculateTransmissionDelay(entitySignals);
      const amplificationFactor = this.calculateAmplificationFactor(entitySignals);
  ...
  private calculateNarrativeConfidence(signals: any[], magnitude: number): number {
  ...
  private generateNarrativeTitle(impactType: string, signals: any[]): string {
    ...
    return titleTemplates[impactType as keyof typeof titleTemplates] || `Narrative Impact Detected (${signals.length} signals)`;
  }

  private generateNarrativeDescription(impactType: string, signals: any[], magnitude: number): string {
  ...
  private calculateTimeframe(signals: any[]): string {
    ...
    const start = new Date(signals.reduce((earliest, s) => s.ts < earliest ? s.ts : earliest, signals[0].ts));
    const end = new Date(signals.reduce((latest, s) => s.ts > latest ? s.ts : latest, signals[0].ts));
  ...
  private async analyzeNarrativeInteractions(tenantId: string, impacts: NarrativeImpact[]): Promise<NarrativeImpact[]> {
    ...
    for (let i = 0; i < impacts.length; i++) {
      for (let j = i + 1; j < impacts.length; j++) {
        ...
        const sharedEntities = impactA.affectedEntities.filter(e => impactB.affectedEntities.includes(e));
        
        if (sharedEntities.length > 0) {
          // Enhance both impacts with interaction data
          enhancedImpacts[i].metadata.interactions = enhancedImpacts[i].metadata.interactions || [];
          enhancedImpacts[i].metadata.interactions.push({
            relatedNarrative: impactB.narrativeId,
            sharedEntities: sharedEntities.length,
            interactionStrength: sharedEntities.length / Math.max(impactA.affectedEntities.length, impactB.affectedEntities.length)
          });
          
          enhancedImpacts[j].metadata.interactions = enhancedImpacts[j].metadata.interactions || [];
  ...
  private async storeNarrativeImpacts(tenantId: string, impacts: NarrativeImpact[]): Promise<void> {
  ...
  private async cacheNarrativeImpacts(tenantId: string, impacts: NarrativeImpact[]): Promise<void> {
  ...
  private calculateTransmissionDelay(signals: any[]): number {
    ...
    for (let i = 1; i < signals.length; i++) {
      const delay = new Date(signals[i].ts).getTime() - new Date(signals[i-1].ts).getTime();
      intervals.push(delay);
    }
    
    return intervals.reduce((sum, delay) => sum + delay, 0) / intervals.length / (1000 * 60); // Average delay in minutes
  }
  ...
  private async getHistoricalNarrativeData(tenantId: string, narrativeId: string): Promise<any[]> {
    // Placeholder for historical data retrieval
    return [];
  }

  private generateNarrativePredictions(historicalData: any[], timeHorizon: string): any[] {
    // Placeholder for prediction generation
    return [];
  }

  private calculatePredictionConfidence(historicalData: any[], predictions: any[]): number {
    // Placeholder for confidence calculation
    return 0.5;
  }

  private identifyNarrativeRiskFactors(historicalData: any[], predictions: any[]): string[] {

...
exports.ActivityFingerprintIndex = void 0;
const logger_js_1 = __importDefault(require("../../utils/logger.js"));
const crypto_1 = require("crypto");
class ActivityFingerprintIndex {
    neo4j;
    redis;
    patterns = new Map();
    ...
    initializePatterns() {
        ...
        logger_js_1.default.info('Initialized activity patterns', {
            patternCount: this.patterns.size,
            patterns: Array.from(this.patterns.keys()),
        });
    }
    async indexActivity(tenantId, signals) {
        if (!signals.length)
            return [];
        try {
            const fingerprints = [];
            ...
            logger_js_1.default.info('Activity indexing completed', {
                tenantId,
            ...
            logger_js_1.default.error('Failed to index activity', {
                error,
                tenantId,
                signalCount: signals.length,
            });
    ...
    async getActivityFingerprints(tenantId, options = {}) {
        ...
        try {
            ...
            if (cached) {
                ...
                logger_js_1.default.debug('Retrieved fingerprints from cache', {
                    tenantId,
            ...
            try {
                ...
                const params = { tenantId, minConfidence };
                if (types?.length) {
          ...
          `, { ...params, limit });
                ...
                logger_js_1.default.debug('Retrieved fingerprints from Neo4j', {
                    tenantId,
                    count: fingerprints.length,
                });
                return fingerprints;
    ...
    async analyzeActivityEvolution(tenantId, timeWindow = '24h') {
        const session = this.neo4j.getSession();
        try {
    ...
    async matchPattern(tenantId, pattern, signalsByType) {
        const matches = [];
        ...
        if (confidence >= 0.3) {
            ...
            const fingerprint = {
                ...
                relatedEntities: Array.from(new Set(relevantSignals.map((s) => String(s.source)))).filter(Boolean),
    ...
    async calculateBehavioralSignatures(pattern, signals) {
    ...
    calculatePatternConfidence(pattern, signatures) {
    ...
    calculateSignificance(pattern, confidence, frequency) {
        const riskMultiplier = pattern.riskLevel === 'high'
            ? 1.5
    ...
    async discoverNewPatterns(tenantId, signals) {
    ...
    async storeFingerprints(tenantId, fingerprints) {
    ...
    calculateIntervalVariance(signals) {
        if (signals.length < 2)
            return 0;
        const intervals = [];
        for (let i = 1; i < signals.length; i++) {
            const interval = new Date(signals[i].ts).getTime() -
    ...
    calculateCoverageBreadth(signals) {
    ...
    calculateMethodicalProgression(signals) {
    ...
    calculateTargetDiversity(signals) {
        const uniqueTargets = new Set(signals.map((s) => s.target || s.source));
        return Math.min(1, uniqueTargets.size / signals.length);
    }
    calculateVolumeMultiplier(signals) {
    ...
    calculateAccessPatternDeviation(signals) {
    ...
    calculateTimeCompression(signals) {
        if (signals.length < 2)
            return 0;
        const totalDuration = new Date(signals[signals.length - 1].ts).getTime() -
...
exports.ActivityFingerprintIndex = ActivityFingerprintIndex;

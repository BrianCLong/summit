...
exports.CoherenceGraphQLResolvers = void 0;
const zod_1 = require("zod");
const logger_js_1 = __importDefault(require("../../utils/logger.js"));
const CoherenceQuerySchema = zod_1.z.object({
    ...
    filters: zod_1.z
        .object({
        signalTypes: zod_1.z.array(zod_1.z.string()).optional(),
        sources: zod_1.z.array(zod_1.z.string()).optional(),
        minWeight: zod_1.z.number().optional().default(0.1),
        minConfidence: zod_1.z.number().optional().default(0.5),
    })
        .optional(),
    aggregation: zod_1.z.enum(['hour', 'day', 'week']).optional().default('hour'),
});
class CoherenceGraphQLResolvers {
    neo4j;
    redis;
    activityIndex;
    narrativeModel;
    missionVault;
    constructor(neo4j, redis, activityIndex, narrativeModel, missionVault) {
        this.neo4j = neo4j;
        this.redis = redis;
        this.activityIndex = activityIndex;
        this.narrativeModel = narrativeModel;
        this.missionVault = missionVault;
    }
    async getCoherenceScore(req, res) {
        try {
          ...
          `, { tenantId });
            ...
            logger_js_1.default.error('Failed to get coherence score', {
                error,
    ...
    async queryCoherenceSignals(req, res) {
        try {
          ...
          `, params);
                ...
                res.json({
                    ...
                    results: {
                        ...
                        summary: {
                            ...
                            avgCoherenceScore: signals.reduce((sum, s) => sum + s.avgWeightedValue, 0) /
                                signals.length || 0,
                        },
                    },
                    metadata: {
            ...
            logger_js_1.default.error('Failed to query coherence signals', { error });
            res.status(500).json({ error: 'Internal server error' });
        }
    }
    async getCoherenceInsights(req, res) {
        try {
            const { tenantId } = req.params;
            ...
            const [fingerprints, narratives, missionContext, riskAssessment] = await Promise.all([
            ...
            logger_js_1.default.error('Failed to get coherence insights', {
                error,
                tenantId: req.params.tenantId,
            });
            res.status(500).json({ error: 'Internal server error' });
    ...
    async detectAnomalies(tenantId, signals) {
        ...
        return signals
            ...
            severity: Math.abs(signal.avgWeightedValue - mean) > 3 * stddev
                ? 'high'
                : 'medium',
        }));
    }
    async assessCoherenceRisk(tenantId, timeRange) {
    ...
    generateRecommendations(fingerprints, narratives, riskAssessment) {
    ...
    calculateConfidenceLevel(fingerprints, narratives) {
        ...
        if (narratives.length > 5)
            confidence += 0.2;
        if (fingerprints.some((fp) => fp.confidence > 0.8))
            confidence += 0.1;
        return Math.min(confidence, 1.0);
    }
    async assessDataQuality(tenantId) {
        const session = this.neo4j.getSession();
        try {
            const result = await session.executeRead(async (tx) => {
                return await tx.run(`
...
exports.CoherenceGraphQLResolvers = CoherenceGraphQLResolvers;

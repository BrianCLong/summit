...
const LocalStubProvider_js_1 = require("./providers/LocalStubProvider.js");
...
class SSOService {
    ...
    constructor() {
        ...
        this.registerProvider(new LocalStubProvider_js_1.LocalStubProvider());
    }
    ...
    async getAllProviders() {
        return Promise.all(Array.from(this.providers.values()).map(p => p.metadata()));
    }
    async handleLogin(providerName, redirectUri) {
    ...
    async handleCallback(providerName, params, redirectUri) {
        const provider = this.getProvider(providerName);
        if (!provider) {
            throw new Error(`Provider ${providerName} not found`);
        }
        try {
            ...
            logger_js_1.default.error({ error: error.message, provider: providerName }, 'SSO Callback failed');
            throw error;
        }
    }

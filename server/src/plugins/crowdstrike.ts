...
async function getToken(): Promise<string> {
  ...
  const clientSecret = secret?.data?.clientSecret || secret?.clientSecret || process.env.CS_CLIENT_SECRET;
  if (!clientId || !clientSecret) throw new Error('Missing CrowdStrike credentials');
...
export const csQuery = createPlugin<Inputs, Output>('crowdstrike.query', async (inputs, ctx) => {
  const { query } = inputs;

...
export function setTemporalHandle(h: Awaited<ReturnType<typeof startTemporalWorker>>) {
  handle = h;
}

export async function enableTemporal() {
  if (process.env.TEMPORAL_ENABLED === 'true') return { ok: true, message: 'already enabled' } as any;
...
export async function disableTemporal() {
  if (process.env.TEMPORAL_ENABLED !== 'true') return { ok: true, message: 'already disabled' } as any;

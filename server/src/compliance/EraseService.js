...
class EraseService {
    ...
    constructor() {
        ...
        this.retentionEngine = RetentionPolicyEngine_js_1.RetentionPolicyEngine.getInstance();
        this.residencyGuard = residency_guard_js_1.ResidencyGuard.getInstance();
    }
    static getInstance() {
     ...
     */
    async createRequest(tenantId, scope, reason) {
        const span = otel_tracing_js_1.otelService.createSpan('erase.create_request');
        try {
            ...
            const id = `erase-${(0, crypto_1.randomUUID)()}`;
            const now = new Date();
            const query = `
     ...
     */
    async approveRequest(requestId, approverId, decision, rationale) {
        const span = otel_tracing_js_1.otelService.createSpan('erase.approve_request');
        try {
    ...
    async executePurge(requestId) {
        const span = otel_tracing_js_1.otelService.createSpan('erase.execute_purge');
        try {
            const request = await this.getRequest(requestId);
            if (!request)
                throw new Error('Request not found');

...
exports.controlMappingService = exports.ControlMappingService = void 0;
const uuid_1 = require("uuid");
const EvidenceCollector_js_1 = require("./EvidenceCollector.js");
...
const FRAMEWORKS = [
    {
        ...
        automationCoverage: Math.round((SOC2_CONTROLS.filter((c) => c.automatable).length / SOC2_CONTROLS.length) * 100),
...
class ControlMappingService {
    ...
    constructor() {
        ...
        logger_js_1.default.info('Control mapping service initialized');
    }
    // --------------------------------------------------------------------------
    // Framework & Control Access
    // --------------------------------------------------------------------------
    getFrameworks() {
        return (0, data_envelope_js_1.createDataEnvelope)(FRAMEWORKS, {
            source: 'ControlMappingService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Framework listing allowed'),
            classification: data_envelope_js_1.DataClassification.INTERNAL,
        });
    }
    getControls(framework, category) {
        let controls = Array.from(this.controls.values()).filter((c) => c.framework === framework);
        if (category) {
            controls = controls.filter((c) => c.category === category);
        }
        return (0, data_envelope_js_1.createDataEnvelope)(controls, {
            source: 'ControlMappingService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Controls listing allowed'),
            classification: data_envelope_js_1.DataClassification.INTERNAL,
        });
    }
    // --------------------------------------------------------------------------
    ...
    async assessControl(controlId, framework, tenantId, actorId) {
        ...
        const evidenceResult = EvidenceCollector_js_1.evidenceCollector.getEvidence(tenantId, {
            controlId,
            framework,
        });
        ...
        logger_js_1.default.info({ controlId, framework, status, score }, 'Control assessed');
        return (0, data_envelope_js_1.createDataEnvelope)(assessment, {
            source: 'ControlMappingService',
            actor: actorId,
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Control assessment completed'),
            classification: data_envelope_js_1.DataClassification.INTERNAL,
        });
    }
    getAssessments(tenantId, framework) {
        const assessments = Array.from(this.assessments.values()).filter((a) => a.tenantId === tenantId && a.framework === framework);
        return (0, data_envelope_js_1.createDataEnvelope)(assessments, {
            source: 'ControlMappingService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Assessment listing allowed'),
            classification: data_envelope_js_1.DataClassification.INTERNAL,
        });
    }
    // --------------------------------------------------------------------------
    ...
    getComplianceSummary(tenantId, framework) {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)({
            ...
            source: 'ControlMappingService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Compliance summary generated'),
            classification: data_envelope_js_1.DataClassification.INTERNAL,
        });
    }
    // --------------------------------------------------------------------------
    ...
    getAuditReadiness(tenantId, framework) {
        ...
        const controls = this.getControls(framework).data;
        const assessments = this.getAssessments(tenantId, framework).data;
        const evidenceStatus = EvidenceCollector_js_1.evidenceCollector.getEvidenceStatus(tenantId, framework).data;
        const gaps = [];
        ...
        return (0, data_envelope_js_1.createDataEnvelope)({
            ...
            source: 'ControlMappingService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Audit readiness assessed'),
            classification: data_envelope_js_1.DataClassification.INTERNAL,
        });
    }
    // --------------------------------------------------------------------------
    ...
    generateRecommendations(gaps, summary) {
        ...
        if (summary.controlSummary.notAssessed > 0) {
            recommendations.push(`Complete assessment of ${summary.controlSummary.notAssessed} pending control(s)`);
        }
...
exports.ControlMappingService = ControlMappingService;
// Export singleton
exports.controlMappingService = new ControlMappingService();
exports.default = ControlMappingService;

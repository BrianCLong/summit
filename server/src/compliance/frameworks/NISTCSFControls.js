...
exports.NISTCSFControlsService = exports.CATEGORY_METADATA = exports.FUNCTION_METADATA = exports.TIER_DESCRIPTIONS = void 0;
exports.getNISTCSFControlsService = getNISTCSFControlsService;
...
// ============================================================================
class NISTCSFControlsService extends events_1.EventEmitter {
    ...
    getAllSubcategories() {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(subcategories, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Retrieved ${subcategories.length} NIST CSF subcategories`),
     ...
     */
    getSubcategoriesByFunction(func) {
        const subcategories = Array.from(this.subcategories.values())
            .filter((s) => s.function === func);
        return (0, data_envelope_js_1.createDataEnvelope)(subcategories, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Retrieved ${subcategories.length} subcategories for ${func}`),
            classification: data_envelope_js_1.DataClassification.PUBLIC,
        });
    }
    /**
    ...
    getSubcategoriesByCategory(category) {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(subcategories, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Retrieved ${subcategories.length} subcategories for ${category}`),
            classification: data_envelope_js_1.DataClassification.PUBLIC,
        });
    }
    /**
     ...
     */
    getNewV2Subcategories() {
        const subcategories = Array.from(this.subcategories.values())
            .filter((s) => s.isNewInV2);
        return (0, data_envelope_js_1.createDataEnvelope)(subcategories, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Retrieved ${subcategories.length} new v2.0 subcategories`),
            classification: data_envelope_js_1.DataClassification.PUBLIC,
        });
    }
    /**
     ...
     */
    createProfile(tenantId, profile) {
        const newProfile = {
            ...profile,
            id: `profile-${Date.now()}`,
            tenantId,
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(newProfile, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Profile "${newProfile.name}" created successfully`),
    ...
    getProfiles(tenantId) {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(profiles, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Retrieved ${profiles.length} profile(s)`),
            classification: data_envelope_js_1.DataClassification.CONFIDENTIAL,
        });
    }
    /**
     ...
     */
    updateSubcategoryStatus(tenantId, profileId, subcategoryId, status) {
        const tenantProfiles = this.profiles.get(tenantId) || [];
        const profileIndex = tenantProfiles.findIndex((p) => p.id === profileId);
        if (profileIndex < 0) {
            return (0, data_envelope_js_1.createDataEnvelope)(null, {
                source: 'NISTCSFControlsService',
                governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.DENY, `Profile ${profileId} not found`),
                classification: data_envelope_js_1.DataClassification.CONFIDENTIAL,
            });
        }
        const profile = tenantProfiles[profileIndex];
        const statusIndex = profile.subcategoryStatuses.findIndex((s) => s.subcategoryId === subcategoryId);
        if (statusIndex >= 0) {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(profile, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Subcategory status updated'),
            classification: data_envelope_js_1.DataClassification.CONFIDENTIAL,
        });
    }
    /**
     ...
     */
    performGapAnalysis(tenantId, currentProfileId, targetProfileId) {
        const tenantProfiles = this.profiles.get(tenantId) || [];
        ...
        if (!currentProfile || !targetProfile) {
            return (0, data_envelope_js_1.createDataEnvelope)(null, {
                source: 'NISTCSFControlsService',
                governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.DENY, 'One or both profiles not found'),
                classification: data_envelope_js_1.DataClassification.CONFIDENTIAL,
            });
        }
        ...
        for (const func of functions) {
            const funcSubcategories = Array.from(this.subcategories.values())
                .filter((s) => s.function === func);
            const categoryGaps = [];
            const categories = exports.FUNCTION_METADATA[func].categories;
            for (const cat of categories) {
                const catSubcategories = funcSubcategories.filter((s) => s.category === cat);
                const subcategoryGaps = catSubcategories.map((sub) => {
                    const currentStatus = currentProfile.subcategoryStatuses.find((s) => s.subcategoryId === sub.subcategoryId);
                    const targetStatus = targetProfile.subcategoryStatuses.find((s) => s.subcategoryId === sub.subcategoryId);
                    return {
                        subcategoryId: sub.subcategoryId,
                        currentState: currentStatus?.currentState || 'not_implemented',
                        targetState: targetStatus?.targetState || 'fully_implemented',
                        gap: this.calculateStateGap(currentStatus?.currentState || 'not_implemented', targetStatus?.targetState || 'fully_implemented'),
                    };
                });
        ...
        const prioritizedActions = functionGaps
            ...
            priority: idx + 1,
            effort: sg.gap > 75 ? 'high' : sg.gap > 50 ? 'medium' : 'low',
            impact: 'high',
            dependencies: [],
        }));
        const gapAnalysis = {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(gapAnalysis, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Gap analysis completed: ${overallGap.toFixed(1)}% overall gap`),
            classification: data_envelope_js_1.DataClassification.CONFIDENTIAL,
        });
    }
    /**
     ...
     */
    calculateStateGap(current, target) {
        const stateValues = {
            not_implemented: 0,
     ...
     */
    assessMaturity(tenantId) {
        const tenantProfiles = this.profiles.get(tenantId) || [];
        const currentProfile = tenantProfiles.find((p) => p.type === 'current');
        if (!currentProfile) {
            return (0, data_envelope_js_1.createDataEnvelope)(null, {
                source: 'NISTCSFControlsService',
                governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.DENY, 'No current profile found for maturity assessment'),
                classification: data_envelope_js_1.DataClassification.CONFIDENTIAL,
            });
        }
        ...
        for (const func of functions) {
            const funcSubcategories = Array.from(this.subcategories.values())
                .filter((s) => s.function === func);
            const statuses = funcSubcategories.map((sub) => currentProfile.subcategoryStatuses.find((s) => s.subcategoryId === sub.subcategoryId));
            const implementedCount = statuses.filter((s) => s?.currentState === 'fully_implemented' || s?.currentState === 'largely_implemented').length;
        ...
        const overallTier = Math.min(...Object.values(functionTiers));
        ...
        for (const func of functions) {
            if (functionTiers[func] >= 3) {
                strengths.push(`Strong ${exports.FUNCTION_METADATA[func].name} capabilities (Tier ${functionTiers[func]})`);
            }
            else if (functionTiers[func] === 1) {
                weaknesses.push(`${exports.FUNCTION_METADATA[func].name} function needs significant improvement`);
                recommendations.push(`Prioritize ${exports.FUNCTION_METADATA[func].name} function improvements`);
            }
        }
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(assessment, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Maturity assessment completed: Tier ${overallTier} (${exports.TIER_DESCRIPTIONS[overallTier].name})`),
            classification: data_envelope_js_1.DataClassification.CONFIDENTIAL,
        });
    }
    /**
     ...
     */
    getFunctionMetadata(func) {
        const metadata = exports.FUNCTION_METADATA[func];
        return (0, data_envelope_js_1.createDataEnvelope)(metadata, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Function metadata retrieved'),
            classification: data_envelope_js_1.DataClassification.PUBLIC,
        });
    }
    /**
    ...
    getCategoryMetadata(category) {
        const metadata = exports.CATEGORY_METADATA[category];
        return (0, data_envelope_js_1.createDataEnvelope)(metadata, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Category metadata retrieved'),
            classification: data_envelope_js_1.DataClassification.PUBLIC,
        });
    }
    /**
    ...
    getTierDescription(tier) {
        const description = exports.TIER_DESCRIPTIONS[tier];
        return (0, data_envelope_js_1.createDataEnvelope)(description, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Tier description retrieved'),
            classification: data_envelope_js_1.DataClassification.PUBLIC,
        });
    }
    /**
     ...
     */
    getCrossFrameworkReferences(subcategoryId) {
        ...
        if (!subcategory) {
            return (0, data_envelope_js_1.createDataEnvelope)([], {
                source: 'NISTCSFControlsService',
                governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.FLAG, `Subcategory ${subcategoryId} not found`),
                classification: data_envelope_js_1.DataClassification.PUBLIC,
            });
        }
        return (0, data_envelope_js_1.createDataEnvelope)(subcategory.informativeReferences, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, `Retrieved ${subcategory.informativeReferences.length} cross-framework references`),
            classification: data_envelope_js_1.DataClassification.PUBLIC,
        });
    }
    /**
    ...
    getStats(tenantId) {
        ...
        if (currentProfile) {
            ...
            for (const func of functions) {
                ...
                const implemented = currentProfile.subcategoryStatuses.filter((s) => funcSubs.some((fs) => fs.subcategoryId === s.subcategoryId) &&
                    (s.currentState === 'fully_implemented' || s.currentState === 'largely_implemented')).length;
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(stats, {
            source: 'NISTCSFControlsService',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'NIST CSF statistics retrieved'),
            classification: data_envelope_js_1.DataClassification.INTERNAL,
        });
    }
}
exports.NISTCSFControlsService = NISTCSFControlsService;
// ============================================================================
// Factory Function
// ============================================================================
let serviceInstance = null;
function getNISTCSFControlsService(config) {
    if (!serviceInstance) {

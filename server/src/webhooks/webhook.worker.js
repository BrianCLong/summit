...
exports.webhookWorker = new bullmq_1.Worker('webhooks', async (job) => {
    const { deliveryId, url, secret, payload, eventType, tenantId } = job.data;
    const { signature, timestamp } = webhook_service_js_1.webhookService.generateSignature(payload, secret);
    const startTime = Date.now();
    const attemptNumber = (job.attemptsMade || 0) + 1;
    const maxAttempts = job.opts.attempts || 1;
    const baseDelay = typeof job.opts.backoff === 'object' && job.opts.backoff
    ...
    try {
        const response = await http_client_js_1.safeClient.post(url, payload, {
            headers: {
        ...
        await pg_js_1.pg.oneOrNone(`INSERT INTO webhook_delivery_attempts
...
exports.webhookWorker.on('completed', (job) => {
    logger_js_1.logger.info(`Webhook delivery ${job.data.deliveryId} completed`);
});
exports.webhookWorker.on('failed', (job, err) => {
    logger_js_1.logger.error(`Webhook delivery ${job?.data.deliveryId} failed: ${err.message}`);
});

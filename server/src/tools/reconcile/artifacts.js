...
function parseBuckets() {
    try {
        return JSON.parse(process.env.CAS_BUCKETS_JSON || '{}');
    }
    catch {
        return {};
    }
...
async function main() {
    ...
    const def = process.env.CAS_BUCKET_DEFAULT;
    ...
    for (const r of rows) {
        ...
        const candidates = [...new Set([def, ...Object.values(buckets)])].filter(Boolean);
        for (const bucket of candidates) {
            try {
                // await s3.send(new HeadObjectCommand({ Bucket: bucket, Key: key }));
                await pg.query(`INSERT INTO cas_objects(digest,size,storage_uri)
                        VALUES ($1, COALESCE($2,0), $3) ON CONFLICT DO NOTHING`, [digest, null, `s3://${bucket}/${key}`]);
                break;
            }
            catch {
                /* try next bucket */

        ...
        if (code === 0) {
          try {
            ...
            resolve({
              ...
              compressionRatio:
                (result.summary_length || text.length) / text.length,
            });
          } catch (parseError) {
            reject(parseError);
          }
...
const words: string[] = (text.toLowerCase().match(/\b\w+\b/g) || []) as string[];
...
const avgWordLength =
  ...
  private getModelForLanguage(language: string, task: string): string {
    ...
    return (
      modelMap[language]?.[task] || modelMap['en'][task] || 'en_core_web_lg'
    );
  }

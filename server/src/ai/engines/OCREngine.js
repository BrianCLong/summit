...
const logger = pino_1.default({ name: 'OCREngine' });
class OCREngine {
    config;
    isInitialized = false;
    constructor(config) {
    ...
    async extractText(imagePath, options = {}) {
        ...
        try {
            ...
            const mergedResults = this.mergeOCRResults(allResults, confidenceThreshold);
    ...
    async runTesseractOCR(imagePath, language, confidenceThreshold) {
        return new Promise((resolve, reject) => {
            ...
            const tesseract = (0, child_process_1.spawn)('tesseract', args);
            let output = '';
            let errorOutput = '';
            ...
            tesseract.on('close', (code) => {
                if (code !== 0) {
                    reject(new Error(`Tesseract failed with code ${code}: ${errorOutput}`));
                    return;
                }
                try {
    ...
    async runPaddleOCR(imagePath, language, confidenceThreshold) {
        return new Promise((resolve, reject) => {
            const pythonScript = path_1.default.join(this.config.modelsPath, 'paddle_ocr.py');
            const args = [
                pythonScript,
                '--image',
                imagePath,
            ...
            python.on('close', (code) => {
                ...
                try {
                    ...
                    const ocrResults = this.parsePaddleOCRResults(results, confidenceThreshold);
                    resolve(ocrResults);
                }
                catch (parseError) {
                    reject(parseError);
    ...
    async enhanceImageForOCR(imagePath) {
        const enhancedPath = path_1.default.join(this.config.tempPath, `enhanced_${Date.now()}_${path_1.default.basename(imagePath)}`);
        try {
     ...
     */
    detectHeading(result, allResults) {
        const height = result.boundingBox.height;
        const avgHeight = allResults.reduce((sum, r) => sum + r.boundingBox.height, 0) /
            allResults.length;
        // Heading heuristics
    ...
    async verifyTesseractInstallation() {
        return new Promise((resolve, reject) => {
            const tesseract = (0, child_process_1.spawn)('tesseract', ['--version']);
            tesseract.on('close', (code) => {
                if (code === 0) {
                    resolve();
                }

...
export interface VideoDetectionOptions extends ObjectDetectionOptions {
  frameRate?: number;
  startTime?: number;
  endTime?: number;
...
export class ObjectDetectionEngine {
   ...
   */
  private async runVideoObjectDetection(
    videoPath: string,
    model: string,
    confidenceThreshold: number,
    nmsThreshold: number,
   ...
   */
  private applyObjectTracking(detections: DetectionResult[]): void {
  ...
  private smoothTrack(track: DetectionResult[]): void {
    ...
    for (let i = windowSize - 1; i < track.length; i++) {
      ...
      // Smooth confidence
      const avgConfidence =
        window.reduce((sum, d) => sum + d.confidence, 0) / window.length;
      track[i].confidence = avgConfidence;

      // Smooth bounding box
      const avgX =
        window.reduce((sum, d) => sum + d.boundingBox.x, 0) / window.length;
      const avgY =
        window.reduce((sum, d) => sum + d.boundingBox.y, 0) / window.length;
      const avgWidth =
        window.reduce((sum, d) => sum + d.boundingBox.width, 0) / window.length;
      const avgHeight =
        window.reduce((sum, d) => sum + d.boundingBox.height, 0) /
        window.length;
   ...
   */
  private getFrameNumber(detection: DetectionResult): number {

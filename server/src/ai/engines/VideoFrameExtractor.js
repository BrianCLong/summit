...
const logger = pino_1.default({ name: 'VideoFrameExtractor' });
class VideoFrameExtractor {
    ...
    async extract(videoPath, options = {}) {
        const { frameRate, interval, outputDir = path_1.default.join(this.tempDir, `frames-${(0, crypto_1.randomUUID)()}`), outputFormat = 'png', startTime, endTime, extractAudio = false, } = options;
    ...
    async extractAudioStream(videoPath, outputDir, startTime, endTime) {
        const audioFileName = `audio-${(0, crypto_1.randomUUID)()}.mp3`;

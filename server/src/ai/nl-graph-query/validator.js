...
exports.extractRequiredParameters = extractRequiredParameters;
exports.isReadOnlyQuery = isReadOnlyQuery;
const pino_1 = __importDefault(require("pino"));
const logger = pino_1.default({ name: 'cypher-validator' });
/**
...
function validateCypher(cypher) {
    ...
    if (!startsValid) {
        syntaxErrors.push(`Query must start with a valid Cypher clause: ${validStarts.join(', ')}`);
    }
    // Must contain RETURN (for read queries) or a mutation clause
    const hasReturn = upperCypher.includes('RETURN');
    const hasMutation = ['CREATE', 'MERGE', 'SET', 'DELETE', 'REMOVE'].some((op) => upperCypher.includes(op));
    ...
    if (openParens !== closeParens) {
        syntaxErrors.push(`Unbalanced parentheses: ${openParens} opening, ${closeParens} closing`);
    }
    // Check for balanced brackets
    const openBrackets = (cypher.match(/\[/g) || []).length;
    const closeBrackets = (cypher.match(/\]/g) || []).length;
    if (openBrackets !== closeBrackets) {
        syntaxErrors.push(`Unbalanced brackets: ${openBrackets} opening, ${closeBrackets} closing`);
    }
    // Check for balanced braces
    const openBraces = (cypher.match(/\{/g) || []).length;
    const closeBraces = (cypher.match(/\}/g) || []).length;
    if (openBraces !== closeBraces) {
        syntaxErrors.push(`Unbalanced braces: ${openBraces} opening, ${closeBraces} closing`);
    }
    ...
    if (hasMutationOp) {
        securityIssues.push('Mutation operations (CREATE/MERGE/SET/REMOVE) are not allowed in compiled queries');
    }
    // Warning checks
    if (!upperCypher.includes('LIMIT') && upperCypher.includes('MATCH')) {
        warnings.push('No LIMIT clause - query may return large result sets. Consider adding LIMIT.');
    }
    if (upperCypher.includes('[*]')) {
        warnings.push('Unbounded variable-length path [*] can be very expensive - specify a maximum depth like [*..5]');
    }
    ...
    if (pathMatch) {
        ...
        if (maxDepth > 10) {
            warnings.push(`Variable-length path depth of ${maxDepth} is very high - consider reducing to ≤10`);
        }
    }
    // Check for potential cartesian products
    const matchCount = (cypher.match(/MATCH/gi) || []).length;
    if (matchCount > 1 && !upperCypher.includes('WHERE')) {
        warnings.push(`Multiple MATCH clauses without WHERE may create expensive cartesian product`);
    }
    ...
    if (needsParameters && !hasParameters) {
        warnings.push('Query has WHERE conditions but no parameter placeholders ($param) - consider using parameters for reusability');
    }
    // Check for potential injection risks (string concatenation)
    if (cypher.includes('+') && cypher.includes("'")) {
        warnings.push('String concatenation detected - ensure user input is properly parameterized');
    }

 ...
 */
function addTenantFilter(cypher, context) {
    if (!context.tenantId)
        return cypher;
 ...
 */
function addPolicyFilter(cypher, context, classification) {
    if (!context.policyTags || context.policyTags.length === 0)
        return cypher;
...
function findMatchingPattern(prompt) {
    for (const pattern of exports.queryPatterns) {
 ...
 */
function generateFromPattern(prompt, context) {

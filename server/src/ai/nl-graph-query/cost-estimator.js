...
exports.isSafeToExecute = isSafeToExecute;
exports.generateCostWarnings = generateCostWarnings;
const pino_1 = __importDefault(require("pino"));
const logger = pino_1.default({ name: 'cost-estimator' });
/**
...
function estimateQueryCost(cypher) {
    ...
    if (cypher.includes('[*')) {
        ...
        costDrivers.push(`Variable-length path with max depth ${maxDepth} is exponentially expensive`);
    }
...
function generateCostWarnings(cost) {
    const warnings = [];
    if (cost.costClass === 'very-high') {
        warnings.push('This query has very high estimated cost and may timeout or consume excessive resources');
    }
    else if (cost.costClass === 'high') {
        warnings.push('This query has high estimated cost and may take several seconds to execute');
    }
    if (cost.nodesScanned > 10000) {
        warnings.push(`Estimated to scan ${cost.nodesScanned.toLocaleString()} nodes - consider adding more specific filters`);
    }
    if (cost.edgesScanned > 5000) {
        warnings.push(`Estimated to scan ${cost.edgesScanned.toLocaleString()} relationships - consider limiting path depth`);
    }
    if (cost.estimatedMemoryMb > 500) {
        warnings.push(`High memory usage estimated (${cost.estimatedMemoryMb}MB) - results may be truncated`);
    }
    if (cost.estimatedTimeMs > 5000) {
        warnings.push(`Long execution time estimated (${(cost.estimatedTimeMs / 1000).toFixed(1)}s) - consider optimizing`);
    }
    return warnings;
}

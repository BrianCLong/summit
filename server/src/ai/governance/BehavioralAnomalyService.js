...
exports.BehavioralAnomalyService = void 0;
exports.createBehavioralAnomalyService = createBehavioralAnomalyService;
...
class BehavioralAnomalyService {
    config;
    llmClient;
    anomalyStore = new Map();
    baselineCache = new Map();
    falsePositivePatterns = new Map(); // Pattern -> count
    statistics;
    constructor(config) {
        this.config = config;
    ...
    async detectAnomalies(scope) {
        if (!this.config.anomalyDetection.enabled) {
            logger_js_1.default.info('Anomaly detection disabled');
            return [];
        }
        const startTime = Date.now();
        logger_js_1.default.info('Starting anomaly detection', { scope });
        try {
            ...
            const scoredAnomalies = filteredAnomalies.filter((a) => a.riskScore >= this.config.anomalyDetection.minAnomalyScore);
            ...
            logger_js_1.default.info('Anomaly detection completed', {
            ...
            logger_js_1.default.error('Anomaly detection failed', { error, scope });
            throw error;
        }
    }
    ...
    async enhanceWithLLMAnalysis(anomalies, scope) {
        ...
        const highSeverityAnomalies = anomalies.filter(a => a.severity === 'critical' || a.severity === 'high');
        ...
        for (const anomaly of anomalies) {
            if (anomaly.severity === 'critical' || anomaly.severity === 'high') {
                try {
                    ...
                    logger_js_1.default.warn({ error, anomalyId: anomaly.id }, 'LLM analysis failed, using original');
...
`.trim();
    ...
    parseLLMAnalysis(llmText) {
        try {
            ...
            logger_js_1.default.warn({ error }, 'Failed to parse LLM analysis response');
        }
        return {};
    }
    ...
    async updateAnomalyStatus(id, status, notes) {
        ...
        logger_js_1.default.info('Anomaly status updated', { id, status, notes });
        return updated;
    }
    /**
    ...
    async resolveAnomaly(id, resolution) {
        ...
        logger_js_1.default.info('Anomaly resolved', { id, resolution: resolution.resolution });
        return resolved;
    }
    /**
    ...
    async getAnomalyTrends(tenantId, timeRange) {
        const anomalies = Array.from(this.anomalyStore.values()).filter((a) => a.affectedEntity.tenantId === tenantId &&
        ...
        const midpoint = new Date((new Date(timeRange.start).getTime() + new Date(timeRange.end).getTime()) / 2);
    ...
    async listAnomalies(options = {}) {
        const { page = 1, pageSize = 20, tenantId, status, type, minSeverity, sortBy = 'detectedAt', sortOrder = 'desc', } = options;
    ...
    resetStatistics() {
        this.statistics = this.initializeStatistics();
        logger_js_1.default.info('Anomaly detection statistics reset');
    }
    /**
    ...
    clearAnomalies() {
        ...
        logger_js_1.default.info('Anomaly store cleared');
    }
    // ===========================================================================
    ...
    createAnomaly(params) {
        const id = `anomaly-${(0, crypto_1.randomUUID)()}`;
    ...
    estimateFalsePositiveLikelihood(type, deviation) {
        ...
        if (deviation.standardDeviations > 5)
            fpr *= 0.5;
        if (deviation.statisticalSignificance < 0.001)
            fpr *= 0.3;
    ...
    async processHighRiskAnomalies(anomalies) {
        const highRisk = anomalies.filter((a) => a.riskScore >= this.config.anomalyDetection.autoBlockThreshold);
        for (const anomaly of highRisk) {
            ...
            logger_js_1.default.warn('High-risk anomaly detected - auto-block triggered', {
    ...
    async sendAlerts(anomaly) {
        for (const channel of this.config.anomalyDetection.alertChannels) {
            try {
                ...
                logger_js_1.default.info('Sending anomaly alert', {
                    channel,
                    anomalyId: anomaly.id,
                    severity: anomaly.severity,
                });
                ...
                logger_js_1.default.error({ error, channel, anomalyId: anomaly.id }, 'Failed to send alert');
            }
        }
    }
    async recordFalsePositive(anomaly) {
        ...
        if (currentCount + 1 >= 5) {
            logger_js_1.default.warn('High false positive rate detected for pattern', {
                pattern: patternKey,
                count: currentCount + 1,
                suggestion: 'Consider adjusting detection thresholds',
            });
        }
        logger_js_1.default.info('Recording false positive for model improvement', {
            anomalyId: anomaly.id,
    ...
    async recordTruePositive(anomalyId) {
        ...
        logger_js_1.default.info('True positive confirmed', {
            anomalyId,
            type: anomaly.anomalyType,
        });
    }
...
exports.BehavioralAnomalyService = BehavioralAnomalyService;
// =============================================================================
// Factory
// =============================================================================
function createBehavioralAnomalyService(config = {}) {
    const defaultConfig = {
        enabled: true,
        policySuggestions: {
            enabled: true,
...
exports.default = BehavioralAnomalyService;

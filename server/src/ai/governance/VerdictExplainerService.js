...
exports.VerdictExplainerService = void 0;
exports.createVerdictExplainerService = createVerdictExplainerService;
...
// =============================================================================
class VerdictExplainerService {
    config;
    llmClient;
    explanationCache = new Map();
    explanationStore = new Map();
    statistics;
    constructor(config) {
        this.config = config;
     ...
     */
    async explainVerdict(verdict, context) {
        ...
        if (this.config.verdictExplanations.cacheExplanations) {
            ...
            if (cached && Date.now() < cached.expiresAt) {
                ...
                logger_js_1.default.debug('Returning cached verdict explanation', { cacheKey });
        ...
        try {
            ...
            logger_js_1.default.info('Verdict explanation generated', {
            ...
            logger_js_1.default.error('Failed to generate verdict explanation', { error, verdict });
            throw error;
        }
    }
     ...
     */
    async batchExplain(verdicts, context) {
        ...
        for (let i = 0; i < verdicts.length; i += batchSize) {
            ...
            const batchResults = await Promise.all(batch.map((v) => this.explainVerdict(v, context)));
    ...
    async listExplanations(options = {}) {
        ...
        explanations.sort((a, b) => new Date(b.generatedAt).getTime() - new Date(a.generatedAt).getTime());
    ...
    resetStatistics() {
        this.statistics = this.initializeStatistics();
        logger_js_1.default.info('Verdict explanation statistics reset');
    }
    /**
    ...
    clearCache() {
        ...
        logger_js_1.default.info('Verdict explanation cache cleared');
    }
    // ===========================================================================
    // Private Methods
    // ===========================================================================
    generateCacheKey(verdict, context) {
    ...
    async generateExplanation(verdict, context) {
    ...
    generateSummary(verdict, context) {
    ...
    async generateDetailedExplanation(verdict, context) {
        ...
        if (this.config.llmSettings?.provider !== 'mock') {
            try {
                ...
                logger_js_1.default.warn({ error }, 'LLM explanation failed, falling back to template');
    ...
    buildLLMPrompt(verdict, context) {
...
`.trim();
    ...
    generateTemplateExplanation(verdict, context) {
        const sections = [];
        // Opening
        ...
        if (verdict.policyIds.length > 0) {
            sections.push('\n\n**Applicable policies:**\n');
            sections.push(`This decision was made based on ${verdict.policyIds.length} governance ${verdict.policyIds.length === 1 ? 'policy' : 'policies'}: ${verdict.policyIds.join(', ')}.`);
        }
        ...
        if (verdict.provenance?.confidence && verdict.provenance.confidence < 0.9) {
            ...
            sections.push(`${(verdict.provenance.confidence * 100).toFixed(0)}% confidence. `);
            sections.push('You may request a manual review if you believe this is incorrect.');
        }
        return sections.join('');
    }
    getOpeningParagraph(verdict, context) {
    ...
    generateRemediationSteps(verdict, context) {
    ...
    createProvenance(id, verdict, usedLLM) {
        const inputHash = (0, crypto_1.createHash)('sha256').update(JSON.stringify(verdict)).digest('hex');
        ...
        return {
            ...
            inputHash,
            outputHash: (0, crypto_1.createHash)('sha256').update(id).digest('hex'),
            timestamp: new Date().toISOString(),
...
exports.VerdictExplainerService = VerdictExplainerService;
// =============================================================================
// Factory
// =============================================================================
function createVerdictExplainerService(config = {}) {
    const defaultConfig = {
        enabled: true,
        policySuggestions: {
            enabled: true,
...
exports.default = VerdictExplainerService;

...
class AnomalyDetectionService {
    ...
    redis;
    anomalyThreshold;
    minDataPointsForAnalysis;
    analysisWindowMs;
    alertCooldownMs;
    sensitivityMultiplier;
    DATA_POINTS_KEY = "anomaly_detection:data_points";
    ALERTS_KEY = "anomaly_detection:alerts";
    ALERT_HISTORY_KEY = "anomaly_detection:alert_history";
    MODEL_KEY = "anomaly_detection:model_params";
    // Internal security limits
    MAX_BATCH_SIZE = 1000;
    ...
    constructor(config) {
        ...
        this.sensitivityMultiplier =
            config.sensitivity === "high" ? 0.8 : config.sensitivity === "low" ? 1.5 : 1.0; // Medium
    }
    /**
     * Process a single metric data point for anomaly detection
     */
    async processMetricDataPoint(dataPoint) {
        try {
            ...
            const historicalData = await this.getHistoricalData(dataPoint.metric, dataPoint.dimensions, Date.now() - this.analysisWindowMs);
            ...
            const analysis = this.performStatisticalAnalysis(historicalData.map((dp) => dp.value), pointWithTimestamp.value);
            // Calculate confidence score based on the analysis
            const confidence = this.calculateAnomalyConfidence(analysis, historicalData.length, this.sensitivityMultiplier);
            ...
            const result = {
                ...
                confidence,
                severity,
                description: this.generateAnomalyDescription(analysis, pointWithTimestamp, severity),
                expectedRange: {
            ...
            logger_js_1.logger.info(`Anomaly detection processed`, {
            ...
            logger_js_1.logger.error(`Error processing metric data point for anomaly detection`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async processBatchMetricDataPoints(dataPoints) {
        if (dataPoints.length > this.MAX_BATCH_SIZE) {
            const msg = `Batch size ${dataPoints.length} exceeds limit of ${this.MAX_BATCH_SIZE}`;
        ...
        for (const point of dataPoints) {
            try {
                ...
                logger_js_1.logger.error(`Failed to process data point in batch`, error, { point });
            }
        }
        return results;
    }
     ...
     */
    async getActiveAnomalyAlerts(limit = 50) {
        try {
            const safeLimit = Math.min(limit, this.MAX_HISTORY_LIMIT);
            // Get all alert IDs from the sorted set
            const alertIds = await this.redis.zrange(this.ALERT_HISTORY_KEY, 0, -1, "REV", // Reverse order (most recent first)
            "LIMIT", 0, safeLimit);
            const alerts = [];
                ...
                for (const [, result] of results) {
                    if (result?.[1] && typeof result[1] === "string") {
                        try {
                            ...
                            logger_js_1.logger.error(`Failed to parse alert from Redis`, parseError);
                        }
                    }
                }
            }
            logger_js_1.logger.info(`Retrieved ${alerts.length} active anomaly alerts`, { limit: safeLimit });
            return alerts;
        }
        catch (error) {
            logger_js_1.logger.error(`Failed to retrieve active anomaly alerts`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async getAnomalyAlertHistory(metric, severity, startTime, limit = 50) {
        try {
            const safeLimit = Math.min(limit, this.MAX_HISTORY_LIMIT);
            const cutoffTime = startTime || Date.now() - 7 * 24 * 60 * 60 * 1000; // Default to 7 days ago
            const alertIds = await this.redis.zrangebyscore(this.ALERT_HISTORY_KEY, cutoffTime, Date.now(), "REV", // Reverse order (most recent first)
            "LIMIT", 0, safeLimit);
            const alerts = [];
            const pipeline = this.redis.pipeline();
            // Fetch each alert by ID
                ...
                for (const [, result] of results) {
                    if (result?.[1] && typeof result[1] === "string") {
                        try {
                            ...
                            logger_js_1.logger.error(`Failed to parse alert for history`, parseError);
                        }
                    }
                }
            }
            logger_js_1.logger.info(`Retrieved ${alerts.length} anomaly alerts from history`, {
                metric,
                severity,
                limit: safeLimit,
                dateRange: {
            ...
            logger_js_1.logger.error(`Failed to retrieve anomaly alert history`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async acknowledgeAnomalyAlert(alertId, acknowledgedBy) {
        try {
            const alertKey = `${this.ALERTS_KEY}:${alertId}`;
            const alertStr = await this.redis.get(alertKey);
            if (!alertStr) {
                logger_js_1.logger.warn(`Cannot acknowledge non-existent alert`, { alertId });
                return false;
            }
            ...
            await this.redis.setex(alertKey, Math.ceil((this.alertCooldownMs * 2) / 1000), // Expire after 2x alert cooldown period
            JSON.stringify(alert));
            logger_js_1.logger.info(`Alert acknowledged`, {
                alertId,
                acknowledgedBy,
                metric: alert.metric,
            });
            ...
            logger_js_1.logger.error(`Failed to acknowledge anomaly alert`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async resolveAnomalyAlert(alertId, resolvedBy) {
        try {
            const alertKey = `${this.ALERTS_KEY}:${alertId}`;
            const alertStr = await this.redis.get(alertKey);
            if (!alertStr) {
                logger_js_1.logger.warn(`Cannot resolve non-existent alert`, { alertId });
                return false;
            }
            const alert = JSON.parse(alertStr);
            alert.resolved = true;
            ...
            logger_js_1.logger.error(`Failed to resolve anomaly alert`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async trainAnomalyModel(labeledData) {
        try {
            if (labeledData.length > this.MAX_BATCH_SIZE) {
                const msg = `Training data size ${labeledData.length} exceeds limit of ${this.MAX_BATCH_SIZE}`;
                logger_js_1.logger.warn(msg);
                throw new Error(msg);
            }
            logger_js_1.logger.info(`Training model with ${labeledData.length} data points`, {
            ...
            await this.redis.setex(this.MODEL_KEY, Math.ceil((this.analysisWindowMs * 2) / 1000), // Keep model data for 2x analysis window
            JSON.stringify({
                anomalyMean,
                normalMean,
            ...
            logger_js_1.logger.info("Anomaly detection model trained successfully", {
                anomalyMean,
                normalMean,
                trainingDataCount: labeledData.length,
            });
            ...
            logger_js_1.logger.error(`Failed to train anomaly detection model`, error);
            throw error;
        }
    }
    /**
    ...
    async getAnomalyStatistics() {
        try {
                ...
                for (const [, result] of results) {
                    if (result?.[1] && typeof result[1] === "string") {
                        try {
                            ...
                            logger_js_1.logger.error(`Failed to parse alert for statistics`, parseError);
                        }
                    }
                }
            }
            ...
            const recentAlertIds = await this.redis.zrangebyscore(this.ALERT_HISTORY_KEY, twentyFourHoursAgo, Date.now());
            ...
            if (modelParamsStr) {
                try {
                    ...
                    logger_js_1.logger.warn(`Could not parse model parameters for accuracy`, parseError);
                }
            }
            return {
            ...
            logger_js_1.logger.error(`Failed to retrieve anomaly statistics`, error);
            throw error;
        }
    }
    /**
    ...
    async storeDataPoint(dataPoint) {
        try {
            ...
            logger_js_1.logger.error(`Failed to store data point`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async getHistoricalData(metric, dimensions, startTime) {
        try {
            ...
            const dataPointEntries = await this.redis.zrangebyscore(key, minTime, Date.now(), "WITHSCORES");
            const dataPoints = [];
            ...
            for (let i = 0; i < dataPointEntries.length; i += 2) {
                ...
                try {
                    const dataPoint = JSON.parse(dataPointStr);
                    ...
                    logger_js_1.logger.error(`Failed to parse historical data point`, parseError);
                }
            }
            return dataPoints;
        }
        catch (error) {
            logger_js_1.logger.error(`Failed to retrieve historical data for analysis`, error);
            throw error;
        }
    }
    /**
     * Perform statistical analysis to detect anomalies
     */
    performStatisticalAnalysis(historicalValues, currentValue) {
     ...
     */
    calculateAnomalyConfidence(analysis, historicalDataSize, sensitivityMultiplier) {
     ...
     */
    generateAnomalyDescription(analysis, dataPoint, severity) {
     ...
     */
    async createAnomalyAlert(result) {
        try {
            ...
            if (recentAlertExists) {
                logger_js_1.logger.debug(`Recent alert exists for metric, skipping new alert`, {
            ...
            logger_js_1.logger.warn(`Anomaly alert created`, {
                alertId: alert.id,
                metric: alert.metric,
            ...
            logger_js_1.logger.error(`Failed to create anomaly alert`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async hasRecentAlertForMetric(metric, dimensions) {
        try {
            // Get recent alert IDs from the history sorted set
            const cutoffTime = Date.now() - this.alertCooldownMs;
            const recentAlertIds = await this.redis.zrangebyscore(this.ALERT_HISTORY_KEY, cutoffTime, Date.now());
            ...
            for (const alertId of recentAlertIds) {
                ...
                if (alertStr) {
                    try {
                        ...
                        logger_js_1.logger.error(`Failed to parse alert for recent check`, parseError);
                    }
                }
            }
            ...
            logger_js_1.logger.error(`Failed to check for recent alerts`, error);

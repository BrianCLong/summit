...
const logger = pino_1.default({ name: 'EmbeddingService' });
class EmbeddingService {
     ...
     */
    async generateImageEmbedding(imagePath, options = {}) {
        if (!this.isInitialized) {
            await this.initialize();
        }
        const { model = 'openai/clip-vit-base-patch32', normalize = true } = options;
        try {
     ...
     */
    async generateAudioEmbedding(audioPath, options = {}) {
        if (!this.isInitialized) {
            await this.initialize();
        }
    ...
    async generateMultimodalEmbedding(inputs, weights = {}) {
        ...
        const { text: textWeight = 1.0, image: imageWeight = 1.0, audio: audioWeight = 1.0, } = weights;
        try {
            const embeddings = [];
     ...
     */
    async clusterEmbeddings(embeddingIds, options = {}) {
        const { numClusters = 5, algorithm = 'kmeans', minClusterSize = 2, } = options;
        try {
    ...
    async runTextEmbedding(text, model, normalize, poolingStrategy) {
        return new Promise((resolve, reject) => {
            const pythonScript = path_1.default.join(this.config.modelsPath, 'text_embedding.py');
            const args = [
                pythonScript,
                '--text',
                text,
    ...
    async runImageEmbedding(imagePath, model, normalize) {
        return new Promise((resolve, reject) => {
            const pythonScript = path_1.default.join(this.config.modelsPath, 'image_embedding.py');
    ...
    async runAudioEmbedding(audioPath, model, normalize) {
        return new Promise((resolve, reject) => {
            const pythonScript = path_1.default.join(this.config.modelsPath, 'audio_embedding.py');
    ...
    async runClustering(embeddings, numClusters, algorithm) {
        return new Promise((resolve, reject) => {
            const pythonScript = path_1.default.join(this.config.modelsPath, 'clustering.py');
            const embeddingData = {

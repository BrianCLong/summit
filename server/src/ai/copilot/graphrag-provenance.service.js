...
exports.GraphRAGProvenanceService = void 0;
exports.createGraphRAGProvenanceService = createGraphRAGProvenanceService;
const crypto_1 = require("crypto");
const pino_1 = __importDefault(require("pino"));
const types_js_1 = require("./types.js");
const logger = pino_1.default({ name: 'graphrag-provenance' });
// Configuration
const PROV_LEDGER_URL = process.env.PROV_LEDGER_URL || 'http://localhost:4010';
const MIN_CITATION_CONFIDENCE = parseFloat(process.env.COPILOT_MIN_CITATION_CONFIDENCE || '0.5');
const MAX_CITATIONS_PER_ANSWER = parseInt(process.env.COPILOT_MAX_CITATIONS || '20', 10);
/**
 * GraphRAG Provenance Service
 */
class GraphRAGProvenanceService {
    neo4jDriver;
    llmService;
    provLedgerUrl;
    constructor(neo4jDriver, llmService, provLedgerUrl) {
    ...
    async answer(request) {
        const validated = types_js_1.GraphRAGRequestSchema.parse(request);
        const answerId = (0, crypto_1.randomUUID)();
        ...
        try {
            ...
            if (validated.includeEvidence) {
                context.evidence = await this.fetchEvidence(validated.investigationId, context.entities.map((e) => e.id));
            }
            if (validated.includeClaims) {
                context.claims = await this.fetchClaims(validated.investigationId, context.entities.map((e) => e.id));
            }
            // Step 3: Generate answer with citations
            const llmResponse = await this.generateAnswer(validated.question, context, validated);
            // Step 4: Build citations from the response and context
            const citations = this.buildCitations(llmResponse.citedEntityIds, llmResponse.citedEvidenceIds, llmResponse.citedClaimIds, context);
            // Step 5: Build provenance chain
            const provenance = this.buildProvenance(llmResponse.citedEntityIds, llmResponse.citedRelationshipIds, llmResponse.citedEvidenceIds, llmResponse.citedClaimIds, llmResponse.confidence);
            // Step 6: Build why paths
            const whyPaths = this.buildWhyPaths(llmResponse.reasoningPaths, context);
            // Step 7: Determine redaction status
            const redaction = this.getRedactionStatus(context, citations);
            // Step 8: Run guardrail checks
            const guardrails = this.runGuardrailChecks(llmResponse.answer, citations, provenance);
            ...
            const answer = {
                ...
                warnings: this.generateWarnings(citations, provenance, redaction, executionTime),
            };
            logger.info({
                answerId,
     ...
     */
    async fetchEvidence(investigationId, entityIds) {
        try {
          ...
          `, { entityIds, investigationId });
            ...
            logger.warn({ error: error instanceof Error ? error.message : 'Unknown' }, 'Failed to fetch evidence');
            return [];
        }
    }
    /**
     ...
     */
    async fetchClaims(investigationId, entityIds) {
        try {
            // Note: In production, this would query the prov-ledger service
            const session = this.neo4jDriver.session({
                defaultAccessMode: 'READ',
          ...
          `, { entityIds, investigationId });
            ...
            logger.warn({ error: error instanceof Error ? error.message : 'Unknown' }, 'Failed to fetch claims');
            return [];
        }
    }
    /**
     ...
     */
    async generateAnswer(question, context, request) {
        ...
        try {
            ...
            const citedEntityIds = (parsed.cited_entities || []).filter((id) => validEntityIds.has(id));
            const citedRelationshipIds = (parsed.cited_relationships || []).filter((id) => validRelIds.has(id));
            const citedEvidenceIds = (parsed.cited_evidence || []).filter((id) => validEvidenceIds.has(id));
            const citedClaimIds = (parsed.cited_claims || []).filter((id) => validClaimIds.has(id));
            // Validate reasoning paths
            const reasoningPaths = (parsed.reasoning_paths || [])
                .filter((path) => validEntityIds.has(path.from) &&
            ...
            logger.error({ error: error instanceof Error ? error.message : 'Unknown' }, 'Failed to parse LLM response');
            // Return minimal response
            return {
                answer: 'I was unable to generate a complete answer based on the available context.',
                confidence: 0,
                citedEntityIds: [],
                citedRelationshipIds: [],
                citedEvidenceIds: [],
    ...
    buildPrompt(question, context) {
        ...
        const entityContext = context.entities
            .slice(0, 50)
            .map((e) => `Entity [${e.id}]: ${e.label} (${e.type})${e.description ? ` - ${e.description}` : ''}`)
            .join('\n');
        ...
        const claimsContext = context.claims.length > 0
            ? context.claims
                .slice(0, 20)
...
Respond with JSON only:`;
     ...
     */
    buildCitations(entityIds, evidenceIds, claimIds, context) {
        const citations = [];
        let citationIndex = 1;
        ...
        for (const claimId of claimIds) {
            ...
            if (claim && citations.length < MAX_CITATIONS_PER_ANSWER) {
                const claimContent = typeof claim.content === 'string'
                    ? claim.content
     ...
     */
    buildProvenance(entityIds, relationshipIds, evidenceIds, claimIds, confidence) {
        return {
            evidenceIds,
            claimIds,
            entityIds,
     ...
     */
    buildWhyPaths(reasoningPaths, context) {
        return reasoningPaths.map((path) => {
            const rel = context.relationships.find((r) => r.id === path.relationshipId);
            return {
                from: path.from,
                to: path.to,
     ...
     */
    getRedactionStatus(context, citations) {
        // Check for redacted content
        const entitiesWithPolicyLabels = context.entities.filter((e) => e.policyLabels && e.policyLabels.length > 0);
        const evidenceWithPolicyLabels = context.evidence.filter((e) => e.policyLabels && e.policyLabels.length > 0);
        const redactedCitations = citations.filter((c) => c.wasRedacted);
        const wasRedacted = redactedCitations.length > 0;
        const uncertaintyAcknowledged = entitiesWithPolicyLabels.length > 0 ||
     ...
     */
    runGuardrailChecks(answer, citations, provenance) {
        const checks = [];
     ...
     * Generate warnings
     */
    generateWarnings(citations, provenance, redaction, executionTimeMs) {
        const warnings = [];
        if (citations.length === 0) {
            warnings.push('This answer has no citations - consider it unverified.');
        }
        if (provenance.chainConfidence < MIN_CITATION_CONFIDENCE) {
            warnings.push(`Low confidence answer (${(provenance.chainConfidence * 100).toFixed(0)}%)`);
        }
        if (redaction.wasRedacted) {
            warnings.push(`Some content was redacted (${redaction.redactedCount} items) due to policy labels.`);
        }
        if (redaction.uncertaintyAcknowledged) {
            warnings.push('Some source data has policy restrictions that may limit completeness.');
        }
        if (executionTimeMs > 5000) {
            warnings.push(`Slow response time (${(executionTimeMs / 1000).toFixed(1)}s) - consider refining your query.`);
        }
        return warnings;
    }
    /**
     ...
     */
    hashSubgraph(entities, relationships) {
...
exports.GraphRAGProvenanceService = GraphRAGProvenanceService;
/**
 * Create a GraphRAG provenance service
 */
function createGraphRAGProvenanceService(neo4jDriver, llmService, provLedgerUrl) {

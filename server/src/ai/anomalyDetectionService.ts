...
export class AnomalyDetectionService {
  ...
  private readonly MAX_HISTORY_LIMIT = 100;
   ...
   */
  async processBatchMetricDataPoints(
  ...
  async getActiveAnomalyAlerts(limit: number = 50): Promise<AnomalyAlert[]> {
    try {
      ...
      logger.info(`Retrieved ${alerts.length} active anomaly alerts`, { limit: safeLimit });
  ...
  async trainAnomalyModel(
    ...
    try {
      ...
      const anomalyMean =
        anomalyData.reduce((sum, d) => sum + d.value, 0) / Math.max(1, anomalyData.length);
      const normalMean =
        normalData.reduce((sum, d) => sum + d.value, 0) / Math.max(1, normalData.length);
  ...
  private performStatisticalAnalysis(
    ...
    const variance =
      squaredDifferences.reduce((sum, sqDiff) => sum + sqDiff, 0) / historicalValues.length;
  ...
  private generateAnomalyDescription(
    ...
    const percentChange =
      analysis.mean !== 0 ? Math.abs(((dataPoint.value - analysis.mean) / analysis.mean) * 100) : 0;
  ...
  private async createAnomalyAlert(result: AnomalyDetectionResult): Promise<void> {
    try {
      ...
      const recentAlertExists = await this.hasRecentAlertForMetric(
   ...
   */
  private async hasRecentAlertForMetric(
    metric: string,
    dimensions?: Record<string, string>
  ): Promise<boolean> {
    try {

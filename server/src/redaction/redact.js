...
const tracer = api_1.trace.getTracer('redaction', '24.2.0');
// Metrics
const redactionApplied = new prom_client_1.Counter({
...
class RedactionService {
    ...
    async redactObject(obj, policy, tenantId, context) {
        return tracer.startActiveSpan('redaction.redact_object', async (span) => {
            span.setAttributes({
                tenant_id: tenantId,
    ...
    async processObject(obj, policy, tenantId, context) {
        if (obj === null || obj === undefined) {
            return obj;
        }
        if (Array.isArray(obj)) {
            return Promise.all(obj.map((item) => this.processObject(item, policy, tenantId, context)));
        }
    ...
    shouldRedactField(field, policy) {
    ...
    getApplicableRule(field, policy) {
        // Handle object-based rules (field -> ruleType mapping)
        if (!Array.isArray(policy.rules)) {
    ...
    applyRedaction(field, value, rule, policy) {
    ...
    async redactGraphQLResponse(response, policy, tenantId) {
    ...
    createRedactionPolicy(rules, options) {
        return {
            rules,

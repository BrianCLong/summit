...
const VisionService_js_1 = __importDefault(require("../../services/VisionService.js"));
const SignalClassificationService_js_1 = require("../../sigint/SignalClassificationService.js");
class FusionService {
    llmService;
    embeddingService;
    visionService;
    ...
    constructor() {
        ...
        this.visionService = new VisionService_js_1.default();
        this.signalService = SignalClassificationService_js_1.SignalClassificationService.getInstance();
    }
    get db() {
     ...
     */
    async ingest(tenantId, content, type = 'TEXT', metadata = {}) {
        const mediaSourceId = (0, node_crypto_1.randomUUID)();
        const investigationId = metadata.investigationId || (0, node_crypto_1.randomUUID)();
        // 1. Save Media Source
        await this.db.query(`INSERT INTO media_sources (id, tenant_id, uri, media_type, mime_type, processing_status, metadata, extraction_count)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 0)`, [
        ...
        try {
            ...
            const entityPromises = extraction.entities.map(async (entity) => {
                ...
                await this.db.query(`INSERT INTO multimodal_entities (
            ...
            try {
                await session.writeTransaction(async (tx) => {
                    await tx.run(`MERGE (m:MediaSource {id: $id, tenantId: $tenantId})
             SET m.uri = $uri, m.type = $type, m.metadata = $metadata`, { id: mediaSourceId, tenantId, uri: type === 'URL' ? content : 'raw-input', type, metadata: JSON.stringify(metadata) });
                    for (const entity of processedEntities) {
                        await tx.run(`MERGE (e:Entity {name: $name, type: $type, tenantId: $tenantId})
               ...
               MERGE (m)-[:MENTIONS {confidence: $conf}]->(e)`, {
                    ...
                    for (const rel of extraction.relationships) {
                        ...
                        if (sourceEntity && targetEntity) {
                            await tx.run(`MATCH (a:Entity {id: $idA, tenantId: $tenantId}), (b:Entity {id: $idB, tenantId: $tenantId})
            ...
            await this.db.query(`UPDATE media_sources SET processing_status = 'COMPLETED', extraction_count = $1 WHERE id = $2`, [processedEntities.length, mediaSourceId]);
            return {
                mediaSourceId,
            ...
            logger_js_1.default.error('Ingestion failed', error);
            await this.db.query(`UPDATE media_sources SET processing_status = 'FAILED' WHERE id = $1`, [mediaSourceId]);
            throw error;
        }
    }
    async search(tenantId, query, limit = 10) {
        const queryEmbedding = await this.embeddingService.generateEmbedding({ text: query });
        const vectorRes = await this.db.query(`SELECT id, extracted_text, entity_type, confidence, 1 - (text_embedding <=> $1) as score, metadata
       FROM multimodal_entities
       WHERE tenant_id = $3
       ORDER BY text_embedding <=> $1 ASC
       LIMIT $2`, [JSON.stringify(queryEmbedding), limit, tenantId]);
        const vectorHits = vectorRes.rows.map(row => ({
            id: row.id,
            score: row.score,
            source: 'vector',
        ...
        let graphHits = [];
        try {
            const graphRes = await session.run(`CALL db.index.fulltext.queryNodes("entity_search", $query) YIELD node, score
         WITH node, score
         WHERE node.tenantId = $tenantId
         RETURN node.id as id, node.name as name, node.type as type, node.description as desc, score
         LIMIT $limit`, { query, limit, tenantId });
            ...
            logger_js_1.default.warn('Graph search failed', e);
        }
        finally {
            await session.close();
        }
        const fusionMap = new Map();
        const addToMap = (hit, weight) => {
    ...
    async extractEntities(text, sourceType) {
        let template = '';
        try {
            template = await (0, promises_1.readFile)((0, node_path_1.join)(process.cwd(), 'prompts/extraction.fusion@v1.yaml'), 'utf-8');
        }
        catch (e) {
            logger_js_1.default.error('Failed to load prompt file', e);
            template = `Extract entities (Person, Org, Loc) from: {{text}}. Return JSON {entities:[], relationships:[]}`;
        }
        let promptTemplate = '';
        try {
            const doc = (0, js_yaml_1.load)(template);
            promptTemplate = doc.template || template;
        }
        catch (e) {
            logger_js_1.default.warn('Failed to parse prompt YAML, falling back to raw text', e);
        ...
        try {
            ...
            logger_js_1.default.error('Failed to parse LLM JSON', e);

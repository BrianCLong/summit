...
exports.postgresUsageRepository = exports.postgresMeterRepository = exports.PostgresTenantUsageRepository = exports.PostgresMeterRepository = void 0;
...
class PostgresMeterRepository {
    async recordEvent(event) {
      ...
      `;
            ...
            logger_js_1.default.error({ error, event }, 'Failed to record meter event to Postgres');
            throw error;
        }
        finally {
            client.release();
...
exports.PostgresMeterRepository = PostgresMeterRepository;
class PostgresTenantUsageRepository extends repository_js_1.TenantUsageDailyRepository {
    async saveAll(rows) {
        const pool = (0, database_js_1.getPostgresPool)();
        const client = await pool.connect();
        try {
      ...
      `;
            for (const row of rows) {
                ...
                if (row.ingestUnits > 0) {
                    await client.query(query, [
                        ...
                        schema_js_1.MeterEventKind.INGEST_UNITS, row.ingestUnits, 'units', '{}'
                    ]);
                }
                // Query Credits
                if (row.queryCredits > 0) {
                    await client.query(query, [
                        row.tenantId, periodStart, periodEnd,
                        schema_js_1.MeterEventKind.QUERY_CREDITS, row.queryCredits, 'credits', '{}'
                    ]);
                }
                // Storage
                if (row.storageBytesEstimate > 0) {
                    await client.query(query, [
                        row.tenantId, periodStart, periodEnd,
                        schema_js_1.MeterEventKind.STORAGE_BYTES_ESTIMATE, row.storageBytesEstimate, 'bytes', '{}'
                    ]);
                }
                // Policy Simulations
                if (row.policySimulations > 0) {
                    await client.query(query, [
                        row.tenantId, periodStart, periodEnd,
                        schema_js_1.MeterEventKind.POLICY_SIMULATION, row.policySimulations, 'simulations', '{}'
                    ]);
                }
                // Workflow Executions
                if (row.workflowExecutions > 0) {
                    await client.query(query, [
                        row.tenantId, periodStart, periodEnd,
                        schema_js_1.MeterEventKind.WORKFLOW_EXECUTION, row.workflowExecutions, 'executions', '{}'
                    ]);
                }
                // Receipt Writes
                if (row.receiptWrites > 0) {
                    await client.query(query, [
                        row.tenantId, periodStart, periodEnd,
                        schema_js_1.MeterEventKind.RECEIPT_WRITE, row.receiptWrites, 'writes', '{}'
                    ]);
                }
            ...
            await client.query('ROLLBACK');
            logger_js_1.default.error({ error }, 'Failed to save usage summaries to Postgres');
            throw error;
        }
        finally {
            client.release();
...
exports.PostgresTenantUsageRepository = PostgresTenantUsageRepository;
exports.postgresMeterRepository = new PostgresMeterRepository();
exports.postgresUsageRepository = new PostgresTenantUsageRepository();

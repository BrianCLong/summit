...
class QuotaManager {
    ...
    overrides = new Map();
    async setQuotaOverride(tenantId, config) {
        this.overrides.set(tenantId, config);
        logger_js_1.default.info({ tenantId, config }, 'Quota override updated for tenant');
    }
    async getEffectiveQuota(tenantId) {
        // 1. Check overrides
        if (this.overrides.has(tenantId)) {
            return this.overrides.get(tenantId);
        }
        // 2. Fallback to Plan
        const plan = await plans_js_1.planService.getPlanForTenant(tenantId);
    ...
    async checkQuota(tenantId, metric, additionalAmount = 0) {
        ...
        const rows = await persistence_js_1.persistentUsageRepository.list(tenantId, startOfMonth, today);
        // Sum usage
        let currentUsage = 0;

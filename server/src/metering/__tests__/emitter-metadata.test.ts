...
describe('Metering metadata requirements', () => {
  ...
  it('adds required metadata for legacy emitters', async () => {
    ...
    const event = enqueueMock.mock.calls[0][0];
    expect(event.kind).toBe(MeterEventKind.INGEST_UNITS);
    expect(hasRequiredMeteringMetadata(event.metadata)).toBe(true);
    expect(event.metadata.actor_type).toBe('system');
    expect(event.metadata.workflow_type).toBe('ingest.service');
  });

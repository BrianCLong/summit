...
class BudgetLadderController {
     ...
     */
    async evaluateTaskBudget(request) {
        try {
            ...
            logger_js_1.default.info('Budget decision made', {
            ...
            logger_js_1.default.error('Budget evaluation failed', {
                error: error.message,
                tenantId: request.tenantId,
            });
     ...
     */
    async getBudgetLadder(tenantId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query('SELECT * FROM budget_ladders WHERE tenant_id = $1', [tenantId]);
     ...
     */
    async configureBudgetLadder(ladder) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO budget_ladders (tenant_id, window, limit, thresholds, actions, updated_at)
         ...
         DO UPDATE SET window = $2, limit = $3, thresholds = $4, actions = $5, updated_at = NOW()`, [
            ...
            logger_js_1.default.info('Budget ladder configured', {
     ...
     */
    async getCurrentSpend(tenantId, window) {
        const client = await this.pool.connect();
        try {
            let windowClause;
            switch (window) {
     ...
     */
    async applyBudgetActions(ladder, stage, request, utilization) {
     ...
     */
    async sendBudgetNotification(tenantId, stage, utilization, config) {
        try {
            ...
            logger_js_1.default.info('Budget notification queued', {
                tenantId,
                stage,
                utilization,
            });
            ...
            logger_js_1.default.error('Failed to send budget notification', {
                error: error.message,
                tenantId,
                stage,
            });
     ...
     */
    async recordBudgetDecision(request, decision, stage) {
        // Metrics
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('budget_utilization', decision.budgetUtilization, { tenant_id: request.tenantId });
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('budget_decision', decision.allowed, {
     ...
     */
    static getDefaultLadder(tenantId, monthlyLimit) {
        return {
            tenantId,
            window: 'monthly',
            limit: monthlyLimit,
...
exports.BudgetLadderController = BudgetLadderController;

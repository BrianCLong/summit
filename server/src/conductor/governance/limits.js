...
exports.governanceLimitEngine = exports.GovernanceLimitEngine = void 0;
exports.estimateTaskCost = estimateTaskCost;
exports.estimateTokenCount = estimateTokenCount;
class GovernanceLimitEngine {
    policies = new Map();
    usage = new Map();
     ...
     */
    async checkLimits(userId, expertType, estimatedCost, estimatedTokens = 0) {
     ...
     */
    recordUsage(userId, cost, tokens, bytesProcessed) {
        const now = Date.now();
    ...
    async checkCostLimits(userId, cost, limits) {
        const stats = this.getUsageStats(userId);
    ...
    async checkRateLimits(userId, limits) {
        const stats = this.getUsageStats(userId);
    ...
    async checkQuotaLimits(userId, tokens, limits) {
        const stats = this.getUsageStats(userId);
    ...
    async checkConcurrentLimits(userId, expertType, policy) {
...
exports.GovernanceLimitEngine = GovernanceLimitEngine;
// Singleton instance
exports.governanceLimitEngine = new GovernanceLimitEngine();
/**
 * Helper function to estimate task cost based on input
 */
function estimateTaskCost(task, expertType) {
    const baseCosts = {
        LLM_LIGHT: 0.02,
        LLM_HEAVY: 0.15,
        GRAPH_TOOL: 0.05,

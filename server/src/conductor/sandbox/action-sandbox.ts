...
interface SecurityEvent {
  type: 'syscall_blocked' | 'network_violation' | 'file_access_denied' | 'resource_limit_exceeded' | 'suspicious_behavior';
  timestamp: string;
  details: string;
...
export class ActionSandbox {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.error('Failed to initialize ActionSandbox', { error: error.message });
      throw error;
    }
  }
  ...
  private async validateActionRequest(request: ActionRequest): Promise<void> {
    ...
    if (request.resourceLimits) {
      if (request.resourceLimits.memory && request.resourceLimits.memory > this.config.maxMemoryMB) {
        violations.push(`Memory limit exceeds maximum: ${request.resourceLimits.memory}MB > ${this.config.maxMemoryMB}MB`);
      }
      
      if (request.resourceLimits.cpu && request.resourceLimits.cpu > this.config.maxCpuCores) {
    ...
    if (request.files) {
      Object.entries(request.files).forEach(([filename, content]) => {
        ...
        const suspiciousExtensions = ['.exe', '.dll', '.so', '.dylib', '.bat', '.cmd', '.ps1'];
   ...
   */
  private async createWorkspace(workspaceDir: string, request: ActionRequest): Promise<void> {
    ...
    await fs.writeFile(join(workspaceDir, 'entrypoint.sh'), entrypoint, { mode: 0o755 });
  }
  ...
  private createEntrypoint(request: ActionRequest): string {
    ...
    if (request.command) {
      const args = request.args ? ' ' + request.args.map(arg => `"${arg}"`).join(' ') : '';
  ...
  private async buildContainerArgs(
    ...
    const args = [
      ...
      '--cpus', this.config.maxCpuCores.toString(),
      '--pids-limit', this.config.maxProcesses.toString(),
      '--ulimit', `fsize=${this.config.maxFileSize}`,
      '--ulimit', 'nproc=50:50',
   ...
   */
  private detectSecurityEvents(output: string, securityEvents: SecurityEvent[]): void {
    const patterns = [
      { pattern: /Permission denied/i, type: 'file_access_denied' as const, severity: 'low' as const },
      { pattern: /Operation not permitted/i, type: 'syscall_blocked' as const, severity: 'medium' as const },
      { pattern: /Connection refused/i, type: 'network_violation' as const, severity: 'medium' as const },
      { pattern: /Cannot allocate memory/i, type: 'resource_limit_exceeded' as const, severity: 'high' as const },
      { pattern: /segmentation fault/i, type: 'suspicious_behavior' as const, severity: 'high' as const }
    ];
  ...
  private async monitorResourceUsage(
    ...
    const monitoringInterval = setInterval(async () => {
      try {
        ...
        if (stats.networkBytes > 100 * 1024 * 1024) { // 100MB
          securityEvents.push({
   ...
   */
  private async collectResourceUsage(containerId: string): Promise<ActionResult['resourceUsage']> {
    try {
   ...
   */
  private async collectOutputFiles(workspaceDir: string): Promise<{ [filename: string]: string }> {
   ...
   */
  private async cleanup(containerId: string, workspaceDir: string): Promise<void> {
    try {
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          const removeProcess = spawn(this.config.containerRuntime, ['rm', '-f', containerId]);
          removeProcess.on('close', () => resolve());
          removeProcess.on('error', reject);
          setTimeout(resolve, 5000); // Timeout after 5 seconds
        });
      } catch (error) {
        logger.warn('Failed to remove container', { containerId, error: error.message });
      }
      ...
      try {
        ...
        logger.warn('Failed to remove workspace', { workspaceDir, error: error.message });
      }
  ...
  private async ensureBaseImage(): Promise<void> {
    return new Promise((resolve, reject) => {
      const pullProcess = spawn(this.config.containerRuntime, ['pull', this.config.baseImage]);
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        networkProcess.on('close', (code) => {
          if (code === 0 || code === 125) { // 125 means network already exists
            resolve();
          } else {
    ...
    if (this.config.enableSeccomp) {
      const seccompProfile = {
        ...
        syscalls: [
          {
            names: [
              "accept", "accept4", "access", "brk", "clone", "close",
              "connect", "dup", "dup2", "execve", "exit", "exit_group",
              "fcntl", "fstat", "futex", "getdents", "getdents64",
              "getpid", "getppid", "getrlimit", "gettid", "ioctl",
              "lseek", "madvise", "mkdir", "mmap", "mprotect",
              "munmap", "open", "openat", "read", "readv", "rt_sigaction",
              "rt_sigprocmask", "rt_sigreturn", "select", "set_thread_area",
      ...
      try {
        ...
        logger.warn('Failed to create seccomp profile', { error: error.message });
      }
    }

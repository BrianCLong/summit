...
const compliance_engine_js_1 = require("./compliance-engine.js");
const prometheus_js_1 = require("../observability/prometheus.js");
exports.complianceRouter = express_1.default.Router();
/**
 * Run compliance assessment
 */
exports.complianceRouter.post('/assessments/run', async (req, res) => {
    ...
    try {
        ...
        const assessmentId = await compliance_engine_js_1.complianceEngine.runAssessment(assessmentRequest.frameworkId, tenantId, assessor, assessmentRequest.scope);
        const response = {
            success: true,
            assessmentId,
        ...
        // Record metrics
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_assessment_run', { success: true });
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('compliance_assessment_time', response.processingTime);
        res.json(response);
    }
    catch (error) {
 ...
 */
exports.complianceRouter.get('/assessments/:assessmentId', async (req, res) => {
    try {
        const { assessmentId } = req.params;
        const tenantId = req.headers['x-tenant-id'];
        const assessment = await compliance_engine_js_1.complianceEngine.getAssessment(assessmentId);
        if (!assessment) {
 ...
 */
exports.complianceRouter.get('/assessments', async (req, res) => {
    try {
        const tenantId = req.headers['x-tenant-id'];
        ...
        let assessments = await compliance_engine_js_1.complianceEngine.listAssessments(tenantId, parseInt(limit));
 ...
 */
exports.complianceRouter.get('/dashboard', async (req, res) => {
    try {
        const tenantId = req.headers['x-tenant-id'];
        if (!tenantId) {
            return res.status(400).json({
        ...
        const dashboard = await compliance_engine_js_1.complianceEngine.getComplianceDashboard(tenantId);
        res.json({
            success: true,
            dashboard,
            timestamp: Date.now(),
 ...
 */
exports.complianceRouter.get('/frameworks', async (req, res) => {
    try {
        const frameworks = [
            {
                id: 'soc2-2017',
 ...
 */
exports.complianceRouter.patch('/findings/:findingId/status', async (req, res) => {
    const startTime = Date.now();
    try {
        ...
        // Record metrics
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_finding_updated', { success: true });
        res.json(response);
    }
    catch (error) {
        console.error('Finding update error:', error);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_finding_update_error', { success: false });
        res.status(500).json({
            success: false,
 ...
 */
exports.complianceRouter.post('/reports/generate', async (req, res) => {
    const startTime = Date.now();
    try {
        const tenantId = req.headers['x-tenant-id'];
        ...
        const dashboard = await compliance_engine_js_1.complianceEngine.getComplianceDashboard(tenantId);
        const assessments = await compliance_engine_js_1.complianceEngine.listAssessments(tenantId, 5);
        const report = {
            ...
            executiveSummary: {
                compliancePosture: dashboard.overallScore >= 80
                    ? 'Strong'
        ...
        // Record metrics
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_report_generated', { success: true });
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('compliance_report_time', response.processingTime);
        res.json(response);
    }
    catch (error) {
        console.error('Report generation error:', error);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_report_error', { success: false });
        res.status(500).json({
            success: false,
 ...
 */
exports.complianceRouter.post('/schedule/assessment', async (req, res) => {
    const startTime = Date.now();
    try {
        const tenantId = req.headers['x-tenant-id'];
        ...
        // Record metrics
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_schedule_created', { success: true });
        res.json(response);
    }
    catch (error) {
        console.error('Schedule creation error:', error);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_schedule_error', { success: false });
        res.status(500).json({
            success: false,
 ...
 */
exports.complianceRouter.get('/metrics', async (req, res) => {
    try {
        const tenantId = req.headers['x-tenant-id'];
        if (!tenantId) {
            return res.status(400).json({
        ...
        const metrics = {
            ...
            averageFrameworkScore: dashboard.frameworkStatus.length > 0
                ? Math.round(dashboard.frameworkStatus.reduce((sum, f) => sum + f.score, 0) /
 ...
 * Health check
 */
exports.complianceRouter.get('/health', (req, res) => {
    res.json({
        success: true,
        status: 'healthy',
        timestamp: Date.now(),
...
exports.complianceRouter.use((req, res, next) => {
    const start = Date.now();
    res.on('finish', () => {
        const duration = Date.now() - start;
        console.log(`Compliance API: ${req.method} ${req.path} - ${res.statusCode} (${duration}ms)`);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('compliance_api_request_duration', duration);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent(`compliance_api_${req.method.toLowerCase()}`, res.statusCode < 400);
    });
    next();
});
exports.default = exports.complianceRouter;

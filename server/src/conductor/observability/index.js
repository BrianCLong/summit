...
exports.createRoutingSpan = createRoutingSpan;
exports.createExpertExecutionSpan = createExpertExecutionSpan;
exports.createMCPToolSpan = createMCPToolSpan;
exports.createSecurityCheckSpan = createSecurityCheckSpan;
exports.recordRoutingDecision = recordRoutingDecision;
exports.recordExpertExecution = recordExpertExecution;
exports.recordMCPOperation = recordMCPOperation;
exports.recordSecurityEvent = recordSecurityEvent;
exports.withConductorSpan = withConductorSpan;
exports.getCurrentTraceContext = getCurrentTraceContext;
exports.createTraceLink = createTraceLink;
exports.conductorTracingMiddleware = conductorTracingMiddleware;
exports.createConductorGraphQLPlugin = createConductorGraphQLPlugin;
const api_1 = require("@opentelemetry/api");
const index_js_1 = require("../metrics/index.js");
const tracer = api_1.trace.getTracer('conductor', '1.0.0');
/**
 * Create a span for conductor routing decisions
 */
function createRoutingSpan(decisionId, taskInput, attributes = {}) {
 ...
 */
function createExpertExecutionSpan(expert, decisionId, attributes = {}) {
 ...
 */
function createMCPToolSpan(serverName, toolName, operation, attributes = {}) {
 ...
 */
function createSecurityCheckSpan(checkType, attributes = {}) {
 ...
 */
function recordRoutingDecision(decisionId, expert, confidence, features, latencyMs, success, error) {
    ...
    try {
        // Record metrics
        index_js_1.conductorMetrics.recordRoutingDecision(expert, latencyMs, success);
        // Set span attributes
        span.setAttributes({
 ...
 */
function recordExpertExecution(expert, decisionId, latencyMs, cost, success, result, error, attributes = {}) {
    const span = createExpertExecutionSpan(expert, decisionId, attributes);
    try {
        // Record metrics
        index_js_1.conductorMetrics.recordExpertExecution(expert, latencyMs, cost, success);
        // Set span attributes
        span.setAttributes({
 ...
 */
function recordMCPOperation(serverName, toolName, operation, latencyMs, success, error, attributes = {}) {
    const span = createMCPToolSpan(serverName, toolName, operation, attributes);
    try {
        // Record metrics
        index_js_1.conductorMetrics.recordMCPOperation(serverName, toolName, latencyMs, success);
        // Set span attributes
        span.setAttributes({
 ...
 */
function recordSecurityEvent(eventType, success, details, error, attributes = {}) {
    const span = createSecurityCheckSpan(eventType, attributes);
    try {
        // Record metrics
        index_js_1.conductorMetrics.recordSecurityEvent(eventType, success);
        // Set span attributes
        span.setAttributes({
 ...
 */
async function withConductorSpan(spanName, operation, attributes = {}) {
 ...
 */
function createTraceLink(baseUrl = process.env.TEMPO_BASE_URL || 'http://localhost:3000') {
 ...
 */
function conductorTracingMiddleware() {
    return (req, res, next) => {
 ...
 */
function createConductorGraphQLPlugin() {
    return {

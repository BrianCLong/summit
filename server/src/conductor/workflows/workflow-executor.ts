...
export class WorkflowExecutor extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.error('❌ Failed to initialize workflow executor', { error: error.message });
      throw error;
    }
  }
  ...
  private async findWorkflowFiles(dir: string): Promise<string[]> {
    ...
    try {
      ...
      logger.debug('📁 Directory not found or inaccessible', { dir, error: error.message });
    }
  ...
  async executeWorkflow(
    ...
    triggeredBy: string,
    variables: Record<string, any> = {},
    metadata: { user_id: string; tenant_id: string; execution_context?: Record<string, any> } = {
      user_id: 'system',
      tenant_id: 'default'
    }
  ): Promise<string> {
  ...
  async listExecutions(filters: {
    ...
    for (const executionId of executionIds) {
      ...
      // Apply filters
      if (filters.workflow_name && execution.workflow_name !== filters.workflow_name) continue;
      if (filters.status && execution.status !== filters.status) continue;
      if (filters.user_id && execution.metadata.user_id !== filters.user_id) continue;
      if (filters.tenant_id && execution.metadata.tenant_id !== filters.tenant_id) continue;
  ...
  async cancelExecution(executionId: string): Promise<void> {
    const execution = this.activeExecutions.get(executionId) || await this.loadExecution(executionId);
    if (!execution) {
  ...
  private async runWorkflowExecution(executionId: string): Promise<void> {
    ...
    try {
      ...
      logger.info(`${success ? '✅' : '❌'} Workflow execution ${success ? 'completed' : 'failed'}`, {
        executionId,
        workflowName: execution.workflow_name,
  ...
  private async executeTask(
    ...
    try {
      ...
      taskExecution.duration_ms = taskExecution.completed_at.getTime() - taskExecution.started_at!.getTime();
      ...
      taskExecution.duration_ms = taskExecution.started_at 
        ? taskExecution.completed_at.getTime() - taskExecution.started_at.getTime()
        : 0;
  ...
  private async loadExecution(executionId: string): Promise<WorkflowExecution | null> {
    ...
    try {
      ...
      if (execution.started_at) execution.started_at = new Date(execution.started_at);
      if (execution.completed_at) execution.completed_at = new Date(execution.completed_at);
      
      return execution;
    } catch (error) {
      logger.error('❌ Failed to parse execution data', { executionId, error: error.message });
      return null;
    }
  }

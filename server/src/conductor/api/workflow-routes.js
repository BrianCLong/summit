 ...
 */
router.get('/workflows', (0, rbac_middleware_js_1.requirePermission)('workflow:read'), (req, res) => {
    try {
        const workflows = workflow_executor_js_1.workflowExecutor.getAvailableWorkflows();
        res.json({
            success: true,
            data: workflows,
            meta: {
        ...
        logger_js_1.default.error('❌ Failed to get available workflows', {
            error: error.message,
        });
        res.status(500).json({
            success: false,
 ...
 */
router.post('/workflows/:workflowName/execute', (0, rbac_middleware_js_1.requirePermission)('workflow:execute'), async (req, res) => {
    try {
        const { workflowName } = req.params;
        const { variables = {}, context = {} } = req.body;
        const user = req.user;
        const executionId = await workflow_executor_js_1.workflowExecutor.executeWorkflow(workflowName, 'manual', variables, {
            user_id: user.userId,
        ...
        logger_js_1.default.info('🚀 Workflow execution triggered via API', {
            workflowName,
            executionId,
            userId: user.userId,
        ...
        logger_js_1.default.error('❌ Failed to execute workflow', {
 ...
 */
router.get('/executions/:executionId', (0, rbac_middleware_js_1.requirePermission)('workflow:read'), async (req, res) => {
    try {
        const { executionId } = req.params;
        const execution = await workflow_executor_js_1.workflowExecutor.getExecutionStatus(executionId);
        if (!execution) {
        ...
        const user = req.user;
        if (execution.metadata.tenant_id !== user.tenantId &&
        ...
        logger_js_1.default.error('❌ Failed to get execution status', {
 ...
 */
router.post('/executions/:executionId/cancel', (0, rbac_middleware_js_1.requirePermission)('workflow:execute'), async (req, res) => {
    try {
        const { executionId } = req.params;
        const user = req.user;
        ...
        await workflow_executor_js_1.workflowExecutor.cancelExecution(executionId);
        logger_js_1.default.info('🛑 Workflow execution cancelled via API', {
            executionId,
        ...
        logger_js_1.default.error('❌ Failed to cancel execution', {
            executionId: req.params.executionId,
            error: error.message,
        });
        res.status(400).json({
 ...
 */
router.get('/executions', (0, rbac_middleware_js_1.requirePermission)('workflow:read'), async (req, res) => {
    try {
        const user = req.user;
        const { workflow_name, status, limit = '50', offset = '0', user_id, tenant_id, } = req.query;
        const filters = {
        ...
        const result = await workflow_executor_js_1.workflowExecutor.listExecutions(filters);
        res.json({
            success: true,
            ...
            meta: {
                ...
                filtered_by: Object.keys(filters).filter((k) => k !== 'limit' && k !== 'offset'),
            },
        });
    }
    catch (error) {
        logger_js_1.default.error('❌ Failed to list executions', { error: error.message });
        res.status(500).json({
            success: false,
 ...
 */
router.get('/workflows/metrics', (0, rbac_middleware_js_1.requirePermission)('metrics:read'), async (req, res) => {
    try {
        ...
        logger_js_1.default.error('❌ Failed to get workflow metrics', {
            error: error.message,
        });
        res.status(500).json({
            success: false,
 ...
 */
router.post('/workflows/hello-world/execute', (0, rbac_middleware_js_1.requireAnyPermission)('workflow:execute', 'workflow:health'), async (req, res) => {
    try {
        const user = req.user;
        const executionId = await workflow_executor_js_1.workflowExecutor.executeWorkflow('hello-world-health-check', 'api-health-check', {
        ...
        logger_js_1.default.info('🏥 Hello World health check triggered', {
            executionId,
            userId: user.userId,
        });
        res.json({
        ...
        logger_js_1.default.error('❌ Failed to execute Hello World health check', {
            error: error.message,
        });
        res.status(500).json({
            success: false,
 ...
 */
router.post('/workflows/hello-case/execute', (0, rbac_middleware_js_1.requirePermission)('workflow:execute'), async (req, res) => {
    try {
        const user = req.user;
        const { analysis_target = 'Example Technology Company' } = req.body;
        const executionId = await workflow_executor_js_1.workflowExecutor.executeWorkflow('hello-case-e2e-validation', 'api-e2e-test', {
        ...
        logger_js_1.default.info('🔍 Hello Case E2E validation triggered', {
            executionId,
        ...
        logger_js_1.default.error('❌ Failed to execute Hello Case E2E validation', {
            error: error.message,
        });
        res.status(500).json({
            success: false,
 ...
 */
router.get('/executions/:executionId/logs', (0, rbac_middleware_js_1.requirePermission)('workflow:read'), async (req, res) => {
    try {
        const { executionId } = req.params;
        ...
        logger_js_1.default.error('❌ Failed to get execution logs', {
            executionId: req.params.executionId,
            error: error.message,
        });
        res.status(500).json({

...
router.get('/runs/:runId/agents/timeline', async (req, res) => {
    try {
        ...
        prometheus_js_1.prometheusConductorMetrics?.agentTimelineRequests?.inc({
            status: 'success',
        });
        res.json({
            runId,
        ...
        prometheus_js_1.prometheusConductorMetrics?.agentTimelineRequests?.inc({ status: 'error' });
...
router.post('/runs/:runId/hitl/:checkpointId/approve', async (req, res) => {
    try {
        ...
        agentSteps.set(runId, steps);
        prometheus_js_1.prometheusConductorMetrics?.hitlActionRequests?.inc({
            status: 'success',
            action: 'approve',
        });
        res.json({
...
router.get('/runs/:runId/hitl/pending', async (req, res) => {
    try {
        ...
        const pendingCheckpoints = checkpoints.filter((c) => pendingSteps.some((s) => s.id === c.stepId));
        res.json({
            runId,
            pendingSteps,
            pendingCheckpoints,

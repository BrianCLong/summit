...
import { requirePermission, requireAnyPermission, AuthenticatedRequest } from '../auth/rbac-middleware.js';
 ...
 */
router.get('/workflows', requirePermission('workflow:read'), (req: Request, res: Response) => {
  try {
    ...
    logger.error('❌ Failed to get available workflows', { error: error.message });
    res.status(500).json({
      success: false,
 ...
 */
router.post('/workflows/:workflowName/execute', requirePermission('workflow:execute'), async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/executions/:executionId', requirePermission('workflow:read'), async (req: Request, res: Response) => {
  try {
    ...
    const user = (req as AuthenticatedRequest).user;
    if (execution.metadata.tenant_id !== user.tenantId && !user.roles?.includes('admin')) {
 ...
 */
router.post('/executions/:executionId/cancel', requirePermission('workflow:execute'), async (req: Request, res: Response) => {
  try {
    const { executionId } = req.params;
    const user = (req as AuthenticatedRequest).user;
 ...
 */
router.get('/executions', requirePermission('workflow:read'), async (req: Request, res: Response) => {
  try {
    const user = (req as AuthenticatedRequest).user;
    const {
      workflow_name,
 ...
 */
router.get('/workflows/metrics', requirePermission('metrics:read'), async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/workflows/hello-world/execute', requireAnyPermission('workflow:execute', 'workflow:health'), async (req: Request, res: Response) => {
  try {
    const user = (req as AuthenticatedRequest).user;
    ...
    logger.error('❌ Failed to execute Hello World health check', { error: error.message });
 ...
 */
router.post('/workflows/hello-case/execute', requirePermission('workflow:execute'), async (req: Request, res: Response) => {
  try {
    const user = (req as AuthenticatedRequest).user;
    ...
    logger.error('❌ Failed to execute Hello Case E2E validation', { error: error.message });
 ...
 */
router.get('/executions/:executionId/logs', requirePermission('workflow:read'), async (req: Request, res: Response) => {
  try {
    const { executionId } = req.params;

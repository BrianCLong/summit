...
router.get('/metrics', async (req, res) => {
    try {
        ...
        const series = generateServingTimeSeries(startTime, now, backend);
        ...
        const filteredBackends = backend && backend !== 'all'
            ...
            : backends;
        prometheus_js_1.prometheusConductorMetrics?.servingMetricsRequests?.inc({
            status: 'success',
            backend: backend || 'all',
        });
        res.json({
        ...
        res.json({
            ...
            summary: {
                ...
                totalModels: [...new Set(filteredBackends.flatMap((b) => b.models))]
                    .length,
                averageUtilization: filteredBackends.reduce((sum, b) => sum + b.utilization, 0) /
...
router.get('/spans', async (req, res) => {
    try {
        ...
        const spans = generateServingSpans(parseInt(limit), parseInt(offset), backend);
        res.json({
            success: true,
            spans,
            pagination: {
                ...
                hasMore: parseInt(offset) + spans.length < 1000,
            },
            timestamp: new Date().toISOString(),
        });
    }
...
function generateServingTimeSeries(startTime, endTime, backend) {
    const series = [];
...
function generateServingSpans(limit, offset, backend) {
    const spans = [];
    const backends = [
        'vllm-cluster-1',
        'ray-serve-prod',

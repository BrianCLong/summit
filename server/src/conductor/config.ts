 ...
 */
export async function initializeConductorSystem(): Promise<{
  graphOpsServer?: GraphOpsServer;
  ...
  let filesServer: FilesServer | undefined;
  ...
  try {
    ...
    if (process.env.FILES_ENABLED !== 'false') {
      const filesConfig: FilesServerConfig = {
 ...
 */
export function getConductorEnvConfig(): Record<string, string> {
  return {
    // Conductor settings

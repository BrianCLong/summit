...
export function startSchedulerLoop() {
  _stop = false;
  loop();
  return { stop: () => { _stop = true; } };
}

async function loop() {
  const pool = getPostgresPool();
  while (!_stop) {
    try { refillAll(1); } catch {}
    ...
    try {
      ...
      for (const s of rows) {
        try {
          const it = cron.parseExpression(s.cron, { currentDate: s.last_run_at || new Date(0) });
          ...
          if (next <= now) {
            ...
            let choice: any = null;
            if ((process.env.ADVISOR_AUTO_APPLY || 'false').toLowerCase() === 'true') {
            ...
            otelService.addSpanAttributes({ 'run.id': r.rows[0].id, 'scheduler.schedule_id': s.id });
          }
        } catch {}
      }
    } catch {}
...
function estimateFromRunbook(runbook: string): { cpuSec: number; gbSec: number; egressGb: number } {
  const norm = runbook.toLowerCase();
  // naive heuristics: tweak as needed
  if (norm.includes('heavy') || norm.includes('gnn') || norm.includes('ml')) return { cpuSec: 600, gbSec: 10, egressGb: 0.5 };
  if (norm.includes('export') || norm.includes('report')) return { cpuSec: 120, gbSec: 2, egressGb: 0.2 };
  if (norm.includes('graph') || norm.includes('cypher')) return { cpuSec: 60, gbSec: 1, egressGb: 0.05 };

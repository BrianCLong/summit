...
exports.TokenAttributionGraphBuilder = void 0;
/**
 * Token Attribution Graph Builder
 */
class TokenAttributionGraphBuilder {
    // In-memory store for attribution graphs (TODO: replace with PostgreSQL)
    attributionGraphs = new Map();
    constructor(
     ...
     */
    async buildFromGenerationStream(outputId, contextSources, generationStream) {
        const nodes = [];
        let outputText = '';
     ...
     */
    async findUngroundedTokens(outputId) {
     ...
     */
    findGapsInRanges(ranges, totalLength) {
        if (ranges.length === 0) {
     ...
     */
    async propagateSensitivity(outputId) {
        const graph = await this.queryByOutput(outputId);
        if (!graph) {
     ...
     */
    redactTokenRanges(text, tokenRanges) {
        ...
        // Redact marked words
        const redactedWords = words.map((word, idx) => redactIndices.has(idx) ? '[REDACTED]' : word);
        return redactedWords.join(' ');
    }
}
exports.TokenAttributionGraphBuilder = TokenAttributionGraphBuilder;
/**
 * Example usage:
 *
 * ```typescript

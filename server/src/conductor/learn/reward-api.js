...
const bandit_js_1 = require("./bandit.js");
...
class RewardCalculator {
     ...
     */
    static calculateReward(request) {
    ...
    static calculateSuccessAtK(metadata = {}) {
    ...
    static calculateHumanThumbs(metadata = {}) {
    ...
    static calculateIncidentFree(metadata = {}) {
    ...
    static calculateAcceptedInsight(metadata = {}) {
    ...
    static calculateImplicitFeedback(metadata = {}) {
        let reward = 0.5;
 ...
 */
exports.rewardRouter.post('/reward', async (req, res) => {
    const startTime = Date.now();
    try {
        const rewardRequest = req.body;
        // Validation
        ...
        const rewardSignal = {
            ...
            contextHash: rewardRequest.contextHash ||
        ...
        // Process reward
        await bandit_js_1.adaptiveRouter.processReward(rewardSignal);
        const response = {
            success: true,
            rewardValue,
        ...
        // Record metrics
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent(`reward_${rewardRequest.rewardType}`, true);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('reward_processing_time', response.processingTime);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('reward_value', rewardValue);
        res.json(response);
    }
    catch (error) {
        console.error('Reward processing error:', error);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('reward_processing_error', { success: false });
        res.status(500).json({
            success: false,
 ...
 */
exports.rewardRouter.post('/reward/batch', async (req, res) => {
    const startTime = Date.now();
    try {
        ...
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('reward_batch_size', rewards.length);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('reward_batch_success_count', successCount);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('reward_batch_error_count', errorCount);
        res.json({
 ...
 */
exports.rewardRouter.get('/metrics', async (req, res) => {
    try {
        const metrics = bandit_js_1.adaptiveRouter.getPerformanceMetrics();
        res.json({
            success: true,
            metrics,
            timestamp: Date.now(),
 ...
 * Get arm statistics
 */
exports.rewardRouter.get('/arms/:armId/stats', async (req, res) => {
    try {
        const { armId } = req.params;
        const metrics = bandit_js_1.adaptiveRouter.getPerformanceMetrics();
        const armStats = metrics.armStatistics[armId];
        if (!armStats) {
 ...
 */
exports.rewardRouter.post('/reset', async (req, res) => {
    try {
        ...
        await bandit_js_1.adaptiveRouter.resetBandits();
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('bandit_reset', { success: true });
        res.json({
            success: true,
 ...
 */
exports.rewardRouter.post('/shadow/:decisionId', async (req, res) => {
    try {
        const { decisionId } = req.params;
        const { productionArm, shadowArm, context } = req.body;
        // Store shadow decision for comparison
        const redis = new ioredis_1.default(process.env.REDIS_URL || 'redis://localhost:6379');
        await redis.setex(`shadow_decision:${decisionId}`, 3600, // 1 hour
        JSON.stringify({
            productionArm,
            shadowArm,
            context,
 ...
 */
exports.rewardRouter.post('/ab-test/:testId/result', async (req, res) => {
    try {
 ...
 */
exports.rewardRouter.get('/health', (req, res) => {
    res.json({
        success: true,
        status: 'healthy',
        timestamp: Date.now(),
...
exports.rewardRouter.use((req, res, next) => {
    const start = Date.now();
    res.on('finish', () => {
        const duration = Date.now() - start;
        console.log(`Reward API: ${req.method} ${req.path} - ${res.statusCode} (${duration}ms)`);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('reward_api_request_duration', duration);
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent(`reward_api_${req.method.toLowerCase()}`, res.statusCode < 400);
    });
    next();
});
exports.default = exports.rewardRouter;

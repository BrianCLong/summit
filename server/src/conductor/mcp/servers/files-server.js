...
class FilesServer {
    ...
    constructor(config) {
        this.config = config;
        this.server = new ws_1.default.Server({ port: config.port });
    ...
    async handleToolExecute(ws, message, token) {
    ...
    async getFile(args) {
        ...
        try {
            ...
            if (encoding === 'base64') {
                ...
                content = await fs_1.promises.readFile(fullPath, 'utf8');
            }
    ...
    async listFiles(args) {
        ...
        try {
            ...
            for (const item of items) {
                ...
                const fullItemPath = path_1.default.join(fullPath, item.name);
                const stats = await fs_1.promises.stat(fullItemPath);
                const itemInfo = {
                    name: item.name,
                    path: itemPath,
    ...
    async deleteFile(args) {
        ...
        try {
            ...
            if (stats.isDirectory()) {
                await fs_1.promises.rmdir(fullPath, { recursive });
            }
            else {
    ...
    resolvePath(relativePath) {
        ...
        const resolved = path_1.default.resolve(this.config.basePath, relativePath);

  ...
  try {
    ...
    if (request.tenantId) {
      ...
      tasksToRun = tasksToRun.filter(task => 
        !task.metadata?.tenantId || task.metadata.tenantId === request.tenantId
      );
    }
    ...
    // Run evaluation
    const results = await evaluationEngine.runEvaluation(
      tasksToRun,
      {
    ...
    // Record metrics
    prometheusConductorMetrics.recordOperationalMetric('evaluation_duration', response.processingTime);
    prometheusConductorMetrics.recordOperationalMetric('evaluation_tasks_run', tasksToRun.length);
    prometheusConductorMetrics.recordOperationalMetric('evaluation_average_score', metrics.averageScore);
    ...
    if (regressions.length > 0) {
      prometheusConductorMetrics.recordOperationalMetric('evaluation_regressions', regressions.length);
...
evaluationRouter.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    prometheusConductorMetrics.recordOperationalEvent(`evaluation_api_${req.method.toLowerCase()}`, res.statusCode < 400);
  });

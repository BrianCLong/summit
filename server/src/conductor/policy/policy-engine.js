...
class PolicyEngine {
    ...
    constructor(pool, redis, config = {}) {
        ...
        logger_js_1.default.info('PolicyEngine initialized', {
    ...
    async evaluatePolicy(input) {
        ...
        try {
            ...
            if (this.config.enableCaching) {
                ...
                if (cached) {
                    logger_js_1.default.debug('Policy decision from cache', {
                        cacheKey,
            ...
            if (this.config.enableMetrics) {
                prometheus_js_1.prometheusConductorMetrics.recordOperationalMetric('policy_evaluation_duration_ms', evaluationTime, {
                    decision: decision.allow ? 'allow' : 'deny',
                    action_type: input.action.type,
                    category: input.action.category,
                });
                prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('policy_evaluation', decision.allow, {
                    action_type: input.action.type,
                    category: input.action.category,
                    tenant_id: input.subject.tenantId,
                });
            }
            logger_js_1.default.info('Policy evaluation completed', {
                decision: decision.allow ? 'ALLOW' : 'DENY',
                evaluationTime,
            ...
            const evaluationTime = Date.now() - startTime;
            logger_js_1.default.error('Policy evaluation failed', {
                error: error.message,
                evaluationTime,
                actionType: input.action.type,
                userId: input.subject.userId,
            ...
            if (this.config.enableMetrics) {
                prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('policy_evaluation_error', false, {
    ...
    async validateInput(input) {
        ...
        if (!input.subject.userId)
            errors.push('subject.userId is required');
        if (!input.subject.tenantId)
            errors.push('subject.tenantId is required');
     ...
     */
    async loadApplicableRules(input) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT id, name, version, category, priority, enabled, conditions, actions, metadata
         ...
         ORDER BY priority ASC, created_at ASC`);
            ...
            logger_js_1.default.debug('Loaded applicable policy rules', {
     ...
     */
    async evaluateWithOPA(input, applicableRules) {
        try {
            // Prepare OPA input
            const opaInput = {
                input: {
            ...
            if (error.response) {
                logger_js_1.default.error('OPA evaluation failed', {
                ...
                logger_js_1.default.error('OPA request failed', {
                    error: error.message,
                    actionType: input.action.type,
                });
            }
     ...
     */
    async evaluateLocally(input, applicableRules) {
        logger_js_1.default.warn('Falling back to local policy evaluation', {
            actionType: input.action.type,
    ...
    async getCachedDecision(cacheKey) {
        try {
            ...
            logger_js_1.default.error('Failed to get cached policy decision', {
                error: error.message,
                cacheKey,
            });
            return null;
    ...
    async cacheDecision(cacheKey, decision) {
        try {
            ...
            await this.redis.setex(cacheKey, this.config.cacheTtlSeconds, JSON.stringify(cacheData));
        }
        catch (error) {
            logger_js_1.default.error('Failed to cache policy decision', {
                error: error.message,
                cacheKey,
            });
        }
     ...
     */
    async recordPolicyEvaluation(input, decision, evaluatedRules) {
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO orchestration_events (
          event_type, level, message, payload, correlation_id, source_service
            ...
            logger_js_1.default.error('Failed to record policy evaluation', {
                error: error.message,
                correlationId: input.context.correlationId,
            });
        }
     ...
     */
    async createPolicyRule(name, category, rules, createdBy, options = {}) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`INSERT INTO orchestration_policies (
          ...
          effective_from, effective_until
            ...
            logger_js_1.default.info('Policy rule created', {
                policyId,
                name,
                category,
                createdBy,
     ...
     * Update policy rule
     */
    async updatePolicyRule(policyId, updates) {
        const client = await this.pool.connect();
        try {
            const setClause = [];
            const values = [];
            ...
            await client.query(`UPDATE orchestration_policies SET ${setClause.join(', ')} WHERE id = $${paramIndex}`, values);
            logger_js_1.default.info('Policy rule updated', { policyId, updates });
            // Clear policy cache
            await this.clearPolicyCache();
        }
        finally {
     ...
     * Delete policy rule
     */
    async deletePolicyRule(policyId) {
        const client = await this.pool.connect();
        try {
            await client.query('DELETE FROM orchestration_policies WHERE id = $1', [
                policyId,
            ]);
            logger_js_1.default.info('Policy rule deleted', { policyId });
            // Clear policy cache
            await this.clearPolicyCache();
        }
        finally {
    ...
    async clearPolicyCache() {
        try {
            ...
            logger_js_1.default.info('Policy cache cleared');
        }
        catch (error) {
            logger_js_1.default.error('Failed to clear policy cache', { error: error.message });
        }
    }
    /**
    ...
    async getPolicyStats() {
        ...
        try {
            const [rulesResult, eventsResult] = await Promise.all([
                client.query('SELECT category, enabled, COUNT(*) as count FROM orchestration_policies GROUP BY category, enabled'),
                client.query("SELECT COUNT(*) as count FROM orchestration_events WHERE event_type = 'policy.evaluation' AND timestamp > NOW() - INTERVAL '24 hours'"),
            ]);
            let totalRules = 0;
            let enabledRules = 0;
...
async function enforcePolicy(policyEngine, input) {
    ...
    if (result.decision.obligations && result.decision.obligations.length > 0) {
        logger_js_1.default.info('Policy obligations required', {
...
function createPolicyInput(userId, tenantId, action, resource, context) {
    return {
        subject: {
            userId,
            tenantId,

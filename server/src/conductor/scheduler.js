...
exports.startSchedulerLoop = startSchedulerLoop;
const cron_parser_1 = __importDefault(require("cron-parser"));
const postgres_js_1 = require("../db/postgres.js");
const selector_js_1 = require("./scheduling/selector.js");
const fair_js_1 = require("./scheduler/fair.js");
...
async function loop() {
    ...
    while (!_stop) {
        try {
            (0, fair_js_1.refillAll)(1);
        }
        catch { }
        const span = otel_tracing_js_1.otelService.createSpan('conductor.scheduler.tick');
        try {
            const { rows } = await pool.query(`SELECT id, runbook, cron, last_run_at FROM schedules WHERE enabled`);
            const now = new Date();
            for (const s of rows) {
                try {
                    const it = cron_parser_1.default.parseExpression(s.cron, {
                    ...
                    if (next <= now) {
                        ...
                        if (cap > 0) {
                            ...
                            const c = await pool.query(`SELECT count(1) FROM run WHERE started_at > now() - interval '1 minute'`);
                        ...
                        let choice = null;
                        let est = null;
                        if ((process.env.ADVISOR_AUTO_APPLY || 'false').toLowerCase() ===
                            'true') {
                            ...
                            choice = await (0, selector_js_1.choosePool)(est, residency);
                        }
                        // Fairness: gate per-tenant concurrency (tenant inference TBD)
                        const tenant = 'default';
                        if (!(0, fair_js_1.acquireTenantSlot)(tenant))
                            continue;
                        // Start run: insert and annotate via run_event for chosen pool
                        const r = await pool.query(`INSERT INTO run (id, runbook, status, started_at) VALUES (gen_random_uuid(), $1, 'PENDING', now()) RETURNING id`, [s.runbook]);
                        if (choice?.id) {
                            await pool.query(`INSERT INTO run_event (run_id, kind, payload) VALUES ($1,'schedule.dispatched',$2)`, [r.rows[0].id, { pool: choice.id, est }]);
                        }
...
function estimateFromRunbook(runbook) {

...
export class OrchestrationService {
  ...
  private async performPreflightCheck(
    ...
    const purposeCheck = await this.validatePurpose(
  ...
  private async performQualityAssurance(
    ...
    if (
      request.constraints?.confidenceThreshold &&
      result.confidence < request.constraints.confidenceThreshold
    ) {
      warnings.push(
        `Confidence ${(result.confidence * 100).toFixed(1)}% below threshold ${request.constraints.confidenceThreshold * 100}%`,
      );
    }
...
Enhanced synthesis:`;
  ...
  private async validateQuerySafety(
    query: string,
    _context: any,
  ): Promise<{ allowed: boolean; reason?: string }> {
  ...
  private async performFinalComplianceAudit(
    result: any,
    request: OrchestrationRequest,
  ): Promise<{
    compliant: boolean;

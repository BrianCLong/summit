...
class ComplianceGate {
     ...
     */
    async validateWebFetch(url, userAgent = 'ConductorBot/1.0 (+https://conductor.ai/bot)', purpose, userId, tenantId) {
        try {
            ...
            if (failedChecks.length > 0) {
                ...
                // Update metrics
                prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_gate_blocked', false, {
                    domain,
            ...
            // Update metrics
            prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('compliance_gate_allowed', true, {
                domain,
            });
            return complianceResult;
        }
        catch (error) {
            logger_js_1.default.error('Compliance validation failed', {
                error: error.message,
                url,
                userId,
                tenantId,
     ...
     */
    async checkRobotsCompliance(domain, path, userAgent) {
     ...
     */
    async checkTOSCompliance(domain, purpose) {
        ...
        if (tos.allowedUseCases.length > 0 &&
     ...
     */
    async checkLicenseCompliance(domain, purpose) {
        ...
        if (license.allowedUsage.length > 0 &&
     ...
     * Check rate limits
     */
    async checkRateLimits(domain, tenantId) {
     ...
     */
    async checkPurposeBinding(domain, purpose, userId) {
    ...
    async getRobotsPolicy(domain) {
        ...
        try {
            ...
            if (!response.ok) {
                logger_js_1.default.info('No robots.txt found', { domain, status: response.status });
                return null;
            }
            ...
            logger_js_1.default.warn('Failed to fetch robots.txt', {
                domain,
                error: error.message,
            });
            return null;
     ...
     */
    async recordSuccessfulFetch(domain, tenantId) {
        ...
        await this.redis
            ...
            .exec();
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('web_fetch_recorded', true, {
            domain,
            tenant_id: tenantId,
        });
    }

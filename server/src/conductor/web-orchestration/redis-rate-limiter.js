...
class RedisRateLimiter {
    ...
    async connect() {
        ...
        logger_js_1.default.info('Redis rate limiter connected');
    }
    /**
     * Check rate limit using token bucket algorithm
     */
    async checkRateLimit(domain, tenantId, requestCount = 1) {
      ...
      `;
            ...
            // Record metrics
            prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('rate_limit_check', rateLimitResult.allowed, {
                domain,
                tenant_id: tenantId,
                tokens_remaining: rateLimitResult.tokensRemaining.toString(),
            });
            if (!rateLimitResult.allowed) {
                logger_js_1.default.info('Rate limit exceeded', {
                    domain,
                    tenantId,
            ...
            logger_js_1.default.error('Rate limit check failed', {
                domain,
                tenantId,
                error: error.message,
            });
     ...
     */
    async getBucketStatus(domain, tenantId) {
        const config = this.domainConfigs.get(domain) || {
            ...this.defaultConfig,
            domain,
        };
    ...
    async updateDomainConfig(domain, config) {
        ...
        logger_js_1.default.info('Updated domain rate limit config', {
            domain,
            config: newConfig,
        });
    }
        ...
        try {
            ...
            for (const [domain, configStr] of Object.entries(configs)) {
                try {
                    ...
                    logger_js_1.default.warn('Failed to parse domain config', {
                        domain,
                        error: error.message,
                    });
                }
            }
            logger_js_1.default.info('Loaded domain rate limit configs', {
                count: this.domainConfigs.size,
            });
        }
        catch (error) {
            logger_js_1.default.error('Failed to load domain configs', { error: error.message });
        }
    }
    /**
     * Reset bucket for domain/tenant
     */
    async resetBucket(domain, tenantId) {
        const bucketKey = `rate_bucket:${domain}:${tenantId}`;
        await this.redis.del(bucketKey);
        logger_js_1.default.info('Reset rate limit bucket', { domain, tenantId });
        prometheus_js_1.prometheusConductorMetrics.recordOperationalEvent('rate_limit_reset', true, {
            domain,
            tenant_id: tenantId,
        });
    }
    ...
    async getRateLimitStats() {
        try {
            ...
            logger_js_1.default.error('Failed to get rate limit stats', { error: error.message });

...
const describeIf = RUN_ACCEPTANCE ? globals_1.describe : globals_1.describe.skip;
describeIf('Offline Kit - Acceptance Criteria', () => {
    let OfflineKit;
    let proofCarryingResultSystem;
    let policyLeakSimulator;
    let claimSyncEngine;
    let crdtSyncEngine;
    let offlineKit;
    ...
    (0, globals_1.beforeAll)(async () => {
        ({ OfflineKit } = await Promise.resolve().then(() => __importStar(require('../offline-kit.js'))));
        ({ proofCarryingResultSystem } = await Promise.resolve().then(() => __importStar(require('../proof-carrying-results.js'))));
        ({ policyLeakSimulator } = await Promise.resolve().then(() => __importStar(require('../policy-leak-simulator.js'))));
        ({ claimSyncEngine } = await Promise.resolve().then(() => __importStar(require('../claim-sync.js'))));
        ({ crdtSyncEngine } = await Promise.resolve().then(() => __importStar(require('../crdt-sync.js'))));
    ...
    (0, globals_1.describe)('Acceptance: Edge session can produce proof-carrying results', () => {
        (0, globals_1.it)('should create proof-carrying result for investigation analysis', async () => {
            ...
            (0, globals_1.expect)(result.resultId).toBeDefined();
            (0, globals_1.expect)(result.nodeId).toBe(edgeNodeId);
            (0, globals_1.expect)(result.computationType).toBe('investigation');
            // Verify inputs are hashed
            (0, globals_1.expect)(result.inputs).toHaveLength(2);
            (0, globals_1.expect)(result.inputs[0].valueHash).toMatch(/^[0-9a-f]{64}$/);
            // Verify outputs have proofs
            (0, globals_1.expect)(result.outputs).toHaveLength(2);
            (0, globals_1.expect)(result.outputs[0].valueHash).toMatch(/^[0-9a-f]{64}$/);
            // Verify proof exists
            (0, globals_1.expect)(result.proof).toBeDefined();
            (0, globals_1.expect)(result.proof.proofType).toBe('deterministic');
            (0, globals_1.expect)(result.proof.signature).toBeDefined();
            (0, globals_1.expect)(result.proof.algorithm).toBe('anomaly-detection-v2');
            // Verify attestation
            (0, globals_1.expect)(result.attestation).toBeDefined();
            (0, globals_1.expect)(result.attestation.attestationType).toBe('node');
            (0, globals_1.expect)(result.attestation.attestor).toBe(edgeNodeId);
            (0, globals_1.expect)(result.attestation.signature).toBeDefined();
            // Verify result passed verification
            (0, globals_1.expect)(result.metadata.verified).toBe(true);
            (0, globals_1.expect)(result.metadata.policyCompliant).toBe(true);
        ...
        (0, globals_1.it)('should verify proof-carrying result integrity', async () => {
            // Create a result
            ...
            (0, globals_1.expect)(verification.checks.proofValid).toBe(true);
            (0, globals_1.expect)(verification.checks.attestationValid).toBe(true);
            (0, globals_1.expect)(verification.checks.inputsValid).toBe(true);
            (0, globals_1.expect)(verification.checks.outputsConsistent).toBe(true);
            (0, globals_1.expect)(verification.checks.policyCompliant).toBe(true);
    ...
    (0, globals_1.describe)('Acceptance: Reconnection merges without conflicts', () => {
        (0, globals_1.it)('should sync claims without conflicts after reconnection', async () => {
            ...
            (0, globals_1.expect)(operationIds).toHaveLength(2);
            // Convert to claims
            const syncResult = await claimSyncEngine.convertAndSync(edgeNodeId, cloudNodeId);
            // Verify claims were created
            (0, globals_1.expect)(syncResult.success).toBe(true);
            (0, globals_1.expect)(syncResult.claimsSent).toBeGreaterThan(0);
            (0, globals_1.expect)(syncResult.verified).toBe(true);
            // Verify no conflicts
            (0, globals_1.expect)(syncResult.conflicts).toHaveLength(0);
        ...
        (0, globals_1.it)('should handle CRDT conflict resolution correctly', async () => {
            ...
            (0, globals_1.expect)(op1).toBeDefined();
            (0, globals_1.expect)(op2).toBeDefined();
    ...
    (0, globals_1.describe)('Acceptance: Policy simulation shows no leakage', () => {
        (0, globals_1.it)('should detect no leakage when clearances match', async () => {
            ...
            const simulation = await policyLeakSimulator.simulateSync(edgeNodeId, cloudNodeId);
            // Verify no leakage detected
            (0, globals_1.expect)(simulation.leakageDetected).toBe(false);
            (0, globals_1.expect)(simulation.violations).toHaveLength(0);
            (0, globals_1.expect)(simulation.riskScore).toBe(0);
            (0, globals_1.expect)(simulation.summary.deniedOperations).toBe(0);
        ...
        (0, globals_1.it)('should detect leakage when clearance insufficient', async () => {
            ...
            const clearanceViolations = simulation.violations.filter((v) => v.violationType === 'clearance_violation');
            (0, globals_1.expect)(clearanceViolations.length).toBeGreaterThan(0);
            (0, globals_1.expect)(simulation.leakageDetected).toBe(true);
            (0, globals_1.expect)(simulation.riskScore).toBeGreaterThan(0);
        ...
        (0, globals_1.it)('should detect tenant isolation violations', async () => {
            ...
            const tenantViolations = simulation.violations.filter((v) => v.violationType === 'unauthorized_access');
            if (tenantViolations.length > 0) {
                (0, globals_1.expect)(tenantViolations[0].severity).toBe('critical');
                (0, globals_1.expect)(simulation.leakageDetected).toBe(true);
        ...
        (0, globals_1.it)('should provide remediation recommendations', async () => {
            ...
            (0, globals_1.expect)(simulation.recommendations).toBeDefined();
            (0, globals_1.expect)(simulation.recommendations.length).toBeGreaterThan(0);
            // Recommendations should be actionable
            const hasClaimBasedRecommendation = simulation.recommendations.some((r) => r.includes('claim'));
    ...
    (0, globals_1.describe)('Integration: Complete Offline-Sync-Verify Flow', () => {
        (0, globals_1.it)('should complete full offline kit workflow', async () => {
            ...
            (0, globals_1.expect)(computationResult.metadata.verified).toBe(true);
            ...
            (0, globals_1.expect)(opId).toBeDefined();
            ...
            (0, globals_1.expect)(allPassed).toBe(true);

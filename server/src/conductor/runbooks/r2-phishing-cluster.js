...
exports.createR2PhishingClusterRunbook = createR2PhishingClusterRunbook;
 ...
 */
function createR2PhishingClusterRunbook() {
    const nodes = [
        // Node 1: Collect Phishing Artifacts
        {
            ...
            dependencies: [],
            preconditions: [
                gates_js_1.GateFactory.legalBasisGate([types_js_1.LegalBasis.LEGITIMATE_INTERESTS, types_js_1.LegalBasis.LEGAL_OBLIGATION], 'DFIR requires legitimate incident response basis'),
                gates_js_1.GateFactory.dataLicenseGate([types_js_1.DataLicense.INTERNAL_USE_ONLY], 'Forensic evidence must be internal use only'),
            ],
            postconditions: [
                gates_js_1.GateFactory.kpiGate('emailCount', 5, 'gte', 'Must collect at least 5 emails for clustering'),
                gates_js_1.GateFactory.proofGate(false, true, 'All artifacts must have chain of custody'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                // Collect phishing emails from input
                const emails = context.input.emails || [];
                // Generate chain of custody for each email
                const proofs = [];
                const citations = [];
                for (const email of emails) {
                    // Create hash for chain of custody
                    const emailHash = (0, crypto_1.createHash)('sha256')
                    ...
                    citations.push(citation_validator_js_1.CitationValidator.createCitation(`Email Server: ${email.metadata?.server || 'Unknown'}`, email.metadata?.archiveUrl, email.metadata?.collectedBy, {
                        emailId: email.id,
        ...
        // Node 2: Extract Forensic Indicators
        {
            ...
            preconditions: [],
            postconditions: [
                gates_js_1.GateFactory.citationGate(1, false, true, 'All extracted indicators must be timestamped'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                const emails = context.state.get('emails');
                // Extract forensic indicators
                const indicators = emails.map((email) => {
                    ...
                    const normalizedHeaders = Object.entries(email.headers)
                        ...
                        .map(([k, v]) => `${k}:${v}`)
                        .join('|');
                    const headerSignature = (0, crypto_1.createHash)('sha256')
                ...
                const citations = [
                    citation_validator_js_1.CitationValidator.createCitation('DMARC/SPF/DKIM Specification', 'https://datatracker.ietf.org/doc/html/rfc7489', 'IETF'),
                ];
                const evidence = {
        ...
        // Node 3: Cluster Analysis
        {
            ...
            preconditions: [],
            postconditions: [
                gates_js_1.GateFactory.kpiGate('clusterCount', 2, 'gte', 'Must identify at least 2 clusters'),
                gates_js_1.GateFactory.kpiGate('clusterPurity', 0.8, 'gte', 'Cluster purity must be >= 80%'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                const emails = context.state.get('emails');
                ...
                for (const [domain, inds] of domainGroups.entries()) {
                    ...
                    const emailsInCluster = emails.filter((e) => inds.some((ind) => ind.emailId === e.id));
                    ...
                    const firstSeen = new Date(Math.min(...emailsInCluster.map((e) => e.receivedAt.getTime())));
                    const lastSeen = new Date(Math.max(...emailsInCluster.map((e) => e.receivedAt.getTime())));
                    clusters.push({
                        ...
                        commonFeatures: {
                            ...
                            attachmentTypes: [
                                ...new Set(emailsInCluster.flatMap((e) => e.attachments.map((a) => a.mimeType))),
                            ],
                        },
                        firstSeen,
                        lastSeen,
                ...
                const clusterPurity = clusters.length > 0
                    ? clusters.reduce((sum, c) => sum + c.confidence, 0) / clusters.length
                    : 0;
                const citations = [
                    citation_validator_js_1.CitationValidator.createCitation('Clustering Analysis Methodology', 'https://en.wikipedia.org/wiki/Cluster_analysis', 'Internal Security Team'),
                ];
                const evidence = {
        ...
        // Node 4: Build Attack Timeline
        {
            ...
            preconditions: [],
            postconditions: [
                gates_js_1.GateFactory.citationGate(2, true, true, 'Timeline must cite all evidence sources'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                const emails = context.state.get('emails');
                const clusters = context.state.get('clusters');
                // Build timeline
                ...
                const citations = [
                    citation_validator_js_1.CitationValidator.createCitation('Email Logs', context.input.emailServerUrl, context.input.analyst),
                    citation_validator_js_1.CitationValidator.createCitation('DFIR Timeline Analysis Best Practices', 'https://www.sans.org/white-papers/timeline-analysis/', 'SANS Institute'),
                ];
                const evidence = {
        ...
        // Node 5: Generate DFIR Report
        {
            ...
            preconditions: [],
            postconditions: [
                gates_js_1.GateFactory.citationGate(3, true, true, 'DFIR report must cite at least 3 evidence sources'),
                gates_js_1.GateFactory.proofGate(true, true, 'DFIR report must include chain of custody'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                const clusters = context.state.get('clusters');
                const timeline = context.state.get('timeline');
    ...
    return {
        ...
        publicationGates: [
            gates_js_1.GateFactory.citationGate(3, true, true, 'Publication requires at least 3 cited evidence sources'),
            gates_js_1.GateFactory.kpiGate('clusterPurity', 0.8, 'gte', 'Publication requires cluster purity >= 80%'),
            gates_js_1.GateFactory.proofGate(true, true, 'Publication requires complete chain of custody'),
        ],
        metadata: {
            category: 'DFIR',
            sensitivity: 'high',

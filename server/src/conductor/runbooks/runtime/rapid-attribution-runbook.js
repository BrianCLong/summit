...
exports.rapidAttributionExampleInput = exports.RapidAttributionRunbook = void 0;
exports.createRapidAttributionRunbook = createRapidAttributionRunbook;
exports.validateRapidAttributionInput = validateRapidAttributionInput;
const types_js_1 = require("../dags/types.js");
/**
 * Step 1: Ingest Indicators
 */
const ingestIndicatorsStep = {
 ...
 */
const resolveInfrastructureStep = {
 ...
 * Rapid Attribution (CTI) Runbook Definition
 */
exports.RapidAttributionRunbook = {
    id: 'rapid_attribution_cti',
    name: 'Rapid Attribution (CTI)',
    version: '1.0.0',
    purpose: 'Quickly attribute an indicator set to likely threat campaigns/infrastructure with evidential support.',
    legalBasisRequired: [types_js_1.LegalBasis.LEGITIMATE_INTERESTS, types_js_1.LegalBasis.PUBLIC_TASK],
    dataLicensesRequired: [
        types_js_1.DataLicense.INTERNAL_USE_ONLY,
        types_js_1.DataLicense.PROPRIETARY,
    ],
    assumptions: [
 ...
 */
function createRapidAttributionRunbook(options) {
    const runbook = JSON.parse(JSON.stringify(exports.RapidAttributionRunbook));
    if (options?.version) {
    ...
    if (options?.timeoutMultiplier !== undefined) {
        ...
        if (runbook.benchmarks) {
            runbook.benchmarks.totalMs = Math.floor(runbook.benchmarks.totalMs * options.timeoutMultiplier);
            for (const stepId of Object.keys(runbook.benchmarks.perStepMs)) {
                runbook.benchmarks.perStepMs[stepId] = Math.floor(runbook.benchmarks.perStepMs[stepId] * options.timeoutMultiplier);
            }
        }
    }
    return runbook;
 ...
 */
function validateRapidAttributionInput(input) {
    const errors = [];
 ...
 */
exports.rapidAttributionExampleInput = {
    indicators: [
        '192.168.1.100',

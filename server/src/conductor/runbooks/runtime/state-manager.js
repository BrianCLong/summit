...
exports.InMemoryRunbookExecutionLogRepository = exports.InMemoryRunbookExecutionRepository = exports.RunbookStateManager = exports.RedisRunbookExecutionLogRepository = exports.RedisRunbookExecutionRepository = void 0;
 ...
 */
class RedisRunbookExecutionRepository {
    redis;
    keyPrefix = 'runbook:execution:';
    indexPrefix = 'runbook:index:';
    constructor(redis) {
        this.redis = redis;
    }
    /**
    ...
    async create(execution) {
        ...
        // Add to indexes
        await Promise.all([
            this.redis.zadd(`${this.indexPrefix}runbook:${execution.runbookId}`, Date.now(), execution.executionId),
            this.redis.zadd(`${this.indexPrefix}tenant:${execution.tenantId}`, Date.now(), execution.executionId),
            this.redis.zadd(`${this.indexPrefix}status:${execution.status}`, Date.now(), execution.executionId),
        ]);
    }
    /**
    ...
    async update(execution) {
        ...
        const oldExecution = JSON.parse(existing);
        const ttl = 7 * 24 * 60 * 60;
        execution.lastUpdatedAt = (0, types_js_1.nowISO)();
        await this.redis.setex(key, ttl, JSON.stringify(execution));
        // Update status index if changed
        if (oldExecution.status !== execution.status) {
            await Promise.all([
                this.redis.zrem(`${this.indexPrefix}status:${oldExecution.status}`, execution.executionId),
                this.redis.zadd(`${this.indexPrefix}status:${execution.status}`, Date.now(), execution.executionId),
            ]);
        }
    }
     ...
     */
    async listByRunbook(runbookId, limit = 100) {
        const ids = await this.redis.zrevrange(`${this.indexPrefix}runbook:${runbookId}`, 0, limit - 1);
    ...
    async listByTenant(tenantId, limit = 100) {
        const ids = await this.redis.zrevrange(`${this.indexPrefix}tenant:${tenantId}`, 0, limit - 1);
        return this.getMultiple(ids);
    }
    /**
    ...
    async listByStatus(status, limit = 100) {
        const ids = await this.redis.zrevrange(`${this.indexPrefix}status:${status}`, 0, limit - 1);
        return this.getMultiple(ids);
    }
    /**
...
exports.RedisRunbookExecutionRepository = RedisRunbookExecutionRepository;
// ============================================================================
 ...
 */
class RedisRunbookExecutionLogRepository {
    redis;
    keyPrefix = 'runbook:log:';
    indexPrefix = 'runbook:logindex:';
    constructor(redis) {
        this.redis = redis;
    }
    /**
...
exports.RedisRunbookExecutionLogRepository = RedisRunbookExecutionLogRepository;
// ============================================================================
...
class RunbookStateManager {
    executionRepo;
    logRepo;
    constructor(executionRepo, logRepo) {
     ...
     */
    async createExecution(execution, actorId) {
     ...
     */
    async transitionToRunning(executionId, actorId) {
    ...
    async pauseExecution(executionId, actorId) {
        ...
        execution.controlledAt = (0, types_js_1.nowISO)();
        await this.executionRepo.update(execution);
    ...
    async cancelExecution(executionId, actorId) {
        ...
        execution.lastUpdatedAt = (0, types_js_1.nowISO)();
        execution.finishedAt = (0, types_js_1.nowISO)();
        execution.controlledBy = actorId;
        execution.controlledAt = (0, types_js_1.nowISO)();
        ...
        execution.steps.forEach((step) => {
            if (step.status === 'PENDING' || step.status === 'RUNNING') {
                ...
                step.finishedAt = (0, types_js_1.nowISO)();
            }
        });
        await this.executionRepo.update(execution);
     ...
     */
    async startStep(executionId, stepId, actorId) {
        const execution = await this.getExecutionOrThrow(executionId);
        ...
        step.status = 'RUNNING';
        step.startedAt = (0, types_js_1.nowISO)();
        step.attempt += 1;
        execution.lastUpdatedAt = (0, types_js_1.nowISO)();
        await this.executionRepo.update(execution);
     ...
     */
    async succeedStep(executionId, stepId, actorId, output, evidence, citations, proofs, kpis) {
        const execution = await this.getExecutionOrThrow(executionId);
        const step = execution.steps.find((s) => s.stepId === stepId);
        if (!step) {
            throw new Error(`Step ${stepId} not found in execution ${executionId}`);
        }
        const finishedAt = (0, types_js_1.nowISO)();
        ...
        if (citations)
            execution.citations.push(...citations);
        if (proofs)
            execution.proofs.push(...proofs);
        if (kpis)
            Object.assign(execution.kpis, kpis);
        await this.executionRepo.update(execution);
     ...
     * Mark step as failed
     */
    async failStep(executionId, stepId, actorId, errorMessage) {
        const execution = await this.getExecutionOrThrow(executionId);
        const step = execution.steps.find((s) => s.stepId === stepId);
        if (!step) {
            throw new Error(`Step ${stepId} not found in execution ${executionId}`);
     ...
     */
    async skipStep(executionId, stepId, actorId, reason) {
        const execution = await this.getExecutionOrThrow(executionId);
        const step = execution.steps.find((s) => s.stepId === stepId);
        if (!step) {
            throw new Error(`Step ${stepId} not found in execution ${executionId}`);
     ...
     */
    async completeExecution(executionId, actorId) {
        const execution = await this.getExecutionOrThrow(executionId);
     ...
     */
    async failExecution(executionId, actorId, error) {
        const execution = await this.getExecutionOrThrow(executionId);
     ...
     */
    async getExecutionOrThrow(executionId) {
     ...
     * Append a log entry
     */
    async appendLog(execution, actorId, eventType, stepId, details) {
        const entry = {
            logId: (0, types_js_1.generateLogId)(),
            executionId: execution.executionId,
            runbookId: execution.runbookId,
            tenantId: execution.tenantId,
            timestamp: (0, types_js_1.nowISO)(),
            actorId,
            eventType,
            stepId,
            details,
...
exports.RunbookStateManager = RunbookStateManager;
// ============================================================================
...
exports.InMemoryRunbookExecutionRepository = InMemoryRunbookExecutionRepository;
/**
 * In-memory log repository for testing
 */
class InMemoryRunbookExecutionLogRepository {
    logs = new Map();
    async append(entry) {
        ...
        // Compute hash
        entry.hash = (0, crypto_1.createHash)('sha256')
...
exports.InMemoryRunbookExecutionLogRepository = InMemoryRunbookExecutionLogRepository;

...
exports.PatternMinerStepExecutor = exports.DefaultPatternMinerService = void 0;
 ...
 */
class DefaultPatternMinerService {
    async findMatchingCampaigns(input) {
        ...
        for (const actor of KNOWN_THREAT_ACTORS) {
            const score = this.calculateMatchScore(input.infraNodeIds, input.indicatorNodeIds || [], actor);
            if (score >= minConfidence) {
                ...
                matches.push({
                    ...
                    score,
                    matchedIndicators: Math.floor((input.indicatorNodeIds?.length || input.infraNodeIds.length) * score),
                    matchedTTPs,
                    actorProfile: {
                        id: actor.id,
                        name: actor.name,
    ...
    calculateMatchScore(infraNodeIds, indicatorNodeIds, actor) {
...
exports.DefaultPatternMinerService = DefaultPatternMinerService;
/**
 * Pattern miner step executor for campaign matching
 */
class PatternMinerStepExecutor extends base_js_1.BaseStepExecutor {
    patternService;
    actionType = 'PATTERN_MINER';
    constructor(patternService = new DefaultPatternMinerService()) {
        super();
    ...
    async execute(ctx) {
        try {
            ...
            const infraNodeIds = this.findPreviousOutput(ctx, 'infraNodeIds') ||
                this.getInput(ctx, 'infraNodeIds', []);
            ...
            const minConfidence = this.getConfig(ctx, 'minConfidence', 0.3);
            ...
            const citations = [
                this.createCitation('MITRE ATT&CK Framework', 'https://attack.mitre.org/', 'MITRE Corporation', { version: 'v14', queryTime: new Date().toISOString() }),
                this.createCitation('Threat Actor Encyclopedia', 'https://malpedia.caad.fkie.fraunhofer.de/', 'Fraunhofer FKIE'),
                this.createCitation('MISP Threat Intelligence', 'https://www.misp-project.org/', 'MISP Project'),
            ];
            // Create evidence
            const evidence = this.createEvidence('campaign_matches', {
            ...
            return this.success({
                ...
                kpis: {
                    ...
                    totalTTPsMatched: result.matches.reduce((acc, m) => acc + m.matchedTTPs.length, 0),
                },
            });
        }
        catch (error) {
            return this.failure(error instanceof Error ? error.message : 'Failed to mine patterns');
        }
    }
    calculateQualityScore(matches) {
        if (matches.length === 0)
            return 0;
        let score = 0;
        for (const match of matches) {
            score += match.score * 0.4;
            if (match.actorProfile)
                score += 0.3;
            if (match.matchedTTPs.length > 0)
                score += 0.3;
        }
        return Math.min(score / matches.length, 1.0);
    }
}
exports.PatternMinerStepExecutor = PatternMinerStepExecutor;

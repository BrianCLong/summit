...
exports.IngestStepExecutor = exports.DefaultIndicatorIngestService = void 0;
const base_js_1 = require("./base.js");
/**
 * Default indicator ingest service implementation
 */
class DefaultIndicatorIngestService {
    async ingestIndicators(input) {
        const enrichedIndicators = input.indicators.map((value, index) => {
...
exports.DefaultIndicatorIngestService = DefaultIndicatorIngestService;
/**
 * Ingest step executor for CTI indicator ingestion
 */
class IngestStepExecutor extends base_js_1.BaseStepExecutor {
    ingestService;
    actionType = 'INGEST';
    constructor(ingestService = new DefaultIndicatorIngestService()) {
        super();
    ...
    async execute(ctx) {
        try {
            ...
            const indicators = this.getInput(ctx, 'indicators', []) ||
                this.getConfig(ctx, 'indicators', []);
            if (indicators.length === 0) {
                return this.failure('No indicators provided for ingestion');
            }
            const caseId = this.getInput(ctx, 'caseId', '');
            const source = this.getInput(ctx, 'source', 'manual');
            // Call ingest service
            ...
            // Create evidence
            const evidence = this.createEvidence('threat_indicators', {
                indicators: result.enrichedIndicators,
                count: result.enrichedIndicators.length,
            }, [
                this.createCitation(source || 'Manual Input', undefined, ctx.userId, { caseId, timestamp: new Date().toISOString() }),
            ], {
            ...
            return this.success({
                ...
                kpis: {
                    indicatorCount: indicators.length,
                    enrichedCount: result.enrichedIndicators.length,
                    maliciousCount: result.enrichedIndicators.filter((i) => i.reputation === 'malicious').length,
                },
            });
        }
        catch (error) {
            return this.failure(error instanceof Error ? error.message : 'Failed to ingest indicators');
        }
    }
    calculateQualityScore(indicators) {
        ...
        for (const ind of indicators) {
            if (ind.reputation && ind.reputation !== 'unknown')
                score += 0.3;
            if (ind.firstSeen)
                score += 0.2;
            if (ind.lastSeen)
...
exports.IngestStepExecutor = IngestStepExecutor;

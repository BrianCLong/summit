...
const replay_log_js_1 = require("./replay-log.js");
...
class DAGExecutor {
    ...
    constructor(privateKey, publicKey) {
        ...
        this.replayLog = new replay_log_js_1.ReplayLog(privateKey, publicKey);
    }
    /**
     * Execute a runbook DAG
     */
    async execute(dag, input) {
            ...
            await Promise.all(batch.map(async (node) => {
                ...
                try {
                    ...
                    if (node.estimatedDuration && duration > node.estimatedDuration * 1.5) {
                        console.warn(`Node ${node.id} took ${duration}ms, expected ${node.estimatedDuration}ms (50% over benchmark)`);
                    }
                }
                catch (error) {
                    const err = error;
     ...
     * Check preconditions
     */
    async checkPreconditions(node, context) {
        for (const gate of node.preconditions) {
            const result = await gates_js_1.GateExecutor.executePrecondition(gate, context);
     ...
     */
    async checkPostconditions(node, context, result) {
        for (const gate of node.postconditions) {
            const gateResult = await gates_js_1.GateExecutor.executePostcondition(gate, context, result);
     ...
     * Check publication gates
     */
    async checkPublicationGates(dag, context) {
        const reasons = [];
        ...
        for (const gate of dag.publicationGates) {
            ...
            const gateResult = await gates_js_1.GateExecutor.executePostcondition(gate, context, syntheticResult);
            if (!gateResult.passed) {
     ...
     */
    createExecutionBatches(sortedNodes) {
        const batches = [];
     ...
     */
    verifyReplayLogIntegrity(publicKey) {

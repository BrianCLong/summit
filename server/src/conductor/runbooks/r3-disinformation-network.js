...
exports.createR3DisinformationNetworkRunbook = createR3DisinformationNetworkRunbook;
 ...
 */
function createR3DisinformationNetworkRunbook() {
    const nodes = [
        // Node 1: Collect Disinformation Samples
        {
            ...
            dependencies: [],
            preconditions: [
                gates_js_1.GateFactory.legalBasisGate([types_js_1.LegalBasis.PUBLIC_TASK, types_js_1.LegalBasis.LEGITIMATE_INTERESTS], 'Disinformation analysis requires public interest or legitimate basis'),
                gates_js_1.GateFactory.dataLicenseGate([types_js_1.DataLicense.CC_BY, types_js_1.DataLicense.CC_BY_SA, types_js_1.DataLicense.PUBLIC_DOMAIN], 'Content must have compatible research license'),
            ],
            postconditions: [
                gates_js_1.GateFactory.kpiGate('sampleCount', 10, 'gte', 'Must collect at least 10 content samples'),
                gates_js_1.GateFactory.citationGate(1, true, true, 'All samples must be cited with URLs and timestamps'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                // Collect content samples from input
                const samples = context.input.samples || [];
                // Generate citations and proofs for each sample
                const citations = [];
                const proofs = [];
                for (const sample of samples) {
                    // Create citation
                    citations.push(citation_validator_js_1.CitationValidator.createCitation(`${sample.platform} - @${sample.author}`, sample.url, sample.author, {
        ...
        // Node 2: Extract Narratives and Claims
        {
            ...
            preconditions: [],
            postconditions: [
                gates_js_1.GateFactory.kpiGate('narrativeCount', 2, 'gte', 'Must identify at least 2 narratives'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                const samples = context.state.get('samples');
                ...
                for (const narrativeText of commonNarratives) {
                    const matchingSamples = samples.filter((s) => s.content.toLowerCase().includes(narrativeText.toLowerCase().split(' ')[0]));
                ...
                const citations = [
                    citation_validator_js_1.CitationValidator.createCitation('Narrative Analysis Methodology', 'https://datasociety.net/library/narrative-analysis/', 'Data & Society Research Institute'),
                ];
                const evidence = {
        ...
        // Node 3: Map Propagation Network
        {
            ...
            preconditions: [],
            postconditions: [
                gates_js_1.GateFactory.kpiGate('nodeCount', 5, 'gte', 'Network must have at least 5 nodes'),
                gates_js_1.GateFactory.kpiGate('edgeCount', 3, 'gte', 'Network must have at least 3 edges'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                const samples = context.state.get('samples');
                const narratives = context.state.get('narratives');
                // Build network nodes
                ...
                for (const sample of samples) {
                    ...
                    if (!nodeMap.has(nodeId)) {
                        ...
                        const totalEngagement = (sample.engagementMetrics.likes || 0) +
                ...
                for (let i = 0; i < networkNodes.length; i++) {
                    for (let j = i + 1; j < networkNodes.length; j++) {
                        ...
                        const sharedNarratives = node1.narratives.filter((n) => node2.narratives.includes(n));
                ...
                const citations = [
                    citation_validator_js_1.CitationValidator.createCitation('Social Network Analysis Methods', 'https://en.wikipedia.org/wiki/Social_network_analysis', 'Academic Research'),
                ];
                const evidence = {
        ...
        // Node 4: Analyze Coordination Patterns
        {
            ...
            preconditions: [],
            postconditions: [
                gates_js_1.GateFactory.kpiGate('coordinationPatternCount', 3, 'gte', 'Must identify at least 3 coordination patterns'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                const samples = context.state.get('samples');
                const networkNodes = context.state.get('networkNodes');
                const networkEdges = context.state.get('networkEdges');
                ...
                for (const [window, windowSamples] of timeWindows.entries()) {
                    if (windowSamples.length >= 3) {
                        patterns.push({
                            ...
                            nodes: [
                                ...new Set(windowSamples.map((s) => `${s.platform}-${s.authorId}`)),
                            ],
                ...
                for (const [keyword, keywordSamples] of keywordGroups.entries()) {
                    if (keywordSamples.length >= 4) {
                        patterns.push({
                            ...
                            nodes: [
                                ...new Set(keywordSamples.map((s) => `${s.platform}-${s.authorId}`)),
                            ],
                            confidence: 0.75,
                ...
                const citations = [
                    citation_validator_js_1.CitationValidator.createCitation('Coordinated Inauthentic Behavior Detection', 'https://arxiv.org/abs/2001.00129', 'Academic Research'),
                ];
                const evidence = {
        ...
        // Node 5: Generate Network Map Report
        {
            ...
            preconditions: [],
            postconditions: [
                gates_js_1.GateFactory.kpiGate('networkCoverage', 0.75, 'gte', 'Network coverage must be >= 75%'),
                gates_js_1.GateFactory.citationGate(5, true, true, 'Network map must cite at least 5 sources'),
                gates_js_1.GateFactory.proofGate(true, false, 'Network map must include cryptographic proof'),
            ],
            execute: async (context) => {
                const startTime = Date.now();
                const samples = context.state.get('samples');
                ...
                const networkNodes = context.state.get('networkNodes');
                const networkEdges = context.state.get('networkEdges');
                const coordinationPatterns = context.state.get('coordinationPatterns');
                ...
                const mapHash = (0, crypto_1.createHash)('sha256')
    ...
    return {
        ...
        publicationGates: [
            gates_js_1.GateFactory.citationGate(5, true, true, 'Publication requires at least 5 cited sources'),
            gates_js_1.GateFactory.kpiGate('networkCoverage', 0.75, 'gte', 'Publication requires network coverage >= 75%'),
            gates_js_1.GateFactory.kpiGate('coordinationPatternCount', 3, 'gte', 'Publication requires at least 3 coordination patterns'),

...
exports.WorkflowRecipes = exports.MCPOrchestrator = exports.orchestrator = exports.conductor = exports.Conductor = void 0;
exports.initializeConductor = initializeConductor;
const index_js_1 = require("./router/index.js");
const client_js_1 = require("./mcp/client.js");
const orchestrator_js_1 = require("./mcp/orchestrator.js");
Object.defineProperty(exports, "orchestrator", { enumerable: true, get: function () { return orchestrator_js_1.orchestrator; } });
Object.defineProperty(exports, "MCPOrchestrator", { enumerable: true, get: function () { return orchestrator_js_1.MCPOrchestrator; } });
Object.defineProperty(exports, "WorkflowRecipes", { enumerable: true, get: function () { return orchestrator_js_1.WorkflowRecipes; } });
const crypto_1 = require("crypto");
const budget_control_js_1 = require("./admission/budget-control.js");
const runs_repo_js_1 = require("../maestro/runs/runs-repo.js"); // Import runsRepo
...
const PolicyEngine_js_1 = require("../services/PolicyEngine.js"); // Integration of Policy Engine
class Conductor {
    config;
    activeTaskCount = 0;
    queue = [];
    ...
    constructor(config) {
        ...
        this.budgetController = (0, budget_control_js_1.createBudgetController)(redisClient);
        this.cache = new cache_js_1.ConductorCache();
        this.missionControlResolver = new conflict_resolution_js_1.MissionControlConflictResolver();
        try {
            (0, index_js_2.registerBuiltins)();
        }
        catch { }
    }
    /**
    ...
    async conduct(input) {
        ...
        const auditId = this.config.auditEnabled ? (0, crypto_1.randomUUID)() : undefined;
        ...
        try {
            if (input.missionControlContext) {
                ...
                if (!missionControlResolution.allowProceed) {
                    const deferReason = missionControlResolution.deferred.find((entry) => entry.missionId ===
                        input.missionControlContext?.currentMissionId)?.reason || 'Mission deferred by arbitration';
            ...
            // Quotas admission
            const q = await (0, quotas_js_1.checkQuota)(tenantId);
            if (!q.allow) {
            ...
            if (input.runId) {
                const runRecord = await runs_repo_js_1.runsRepo.get(input.runId, tenantId);
                if (runRecord) {
                    currentRunCost = runRecord.cost || 0;
                }
            }
            // Route to expert
            const decision = index_js_1.moERouter.route(input);
            ...
            if (!admissionDecision.admit) {
                ...
                if (input.runId) {
                    await runs_repo_js_1.runsRepo.update(input.runId, { status: 'failed', error_message: 'Budget cap exceeded' }, tenantId);
                    this.logAudit({
                        auditId,
                        input,
                        event: 'run_preempted',
            ...
            const region = input.userContext?.region ||
                this.config?.llmProviders?.light?.endpoint?.match(/(eu|us\-east|us\-west)/)?.[1];
            if (residency) {
                const r = await (0, opaClient_js_1.checkResidency)({ region, residency });
                if (!r.allow) {
            ...
            try {
                await (0, quotas_js_1.accrueUsage)(tenantId, { runInc: true });
            }
            catch { }
    ...
    makeCacheKey(expert, input) {
        ...
        return (0, crypto_2.createHash)('sha1').update(basis).digest('hex');
    }
    /**
     * Preview routing decision without execution
     */
    previewRouting(input) {
        return index_js_1.moERouter.route(input);
    }
    /**
    ...
    async executeGraphTool(input, logs) {
        ...
        try {
            ...
            if (input.task.toLowerCase().includes('cypher') ||
                ...
                const result = await (0, client_js_1.executeToolAnywhere)('graph.query', {
                    cypher: cypher,
                    params: {},
                ...
                const result = await (0, client_js_1.executeToolAnywhere)('graph.alg', {
                    name: algName,
    ...
    async executeFilesTool(input, logs) {
        ...
        try {
            if (input.task.toLowerCase().includes('search')) {
                ...
                const result = await (0, client_js_1.executeToolAnywhere)('files.search', {
                    query: query,
                    limit: 10,
                }, input.userContext?.scopes);
                ...
                const result = await (0, client_js_1.executeToolAnywhere)('files.get', {
                    path: filePath,
                    encoding: 'utf8',
                }, input.userContext?.scopes);
                ...
                const result = await (0, client_js_1.executeToolAnywhere)('files.list', {
                    path: '.',
                    recursive: false,
                }, input.userContext?.scopes);
                return {
    ...
    async executeOsintTool(input, logs) {
        ...
        if (p?.name) {
            ...
            const out = await (0, index_js_2.runPlugin)(p.name, p.inputs || {}, { tenant });
        ...
        if (m) {
            ...
            if (name === 'shodan.ip.lookup')
                ...
                inputs = { query: arg };
            const out = await (0, index_js_2.runPlugin)(name, inputs, { tenant });
            return { output: out?.data ?? out, logs, cost: 0.004 };
        }
     ...
     */
    getStats() {
        const routingStats = index_js_1.moERouter.getRoutingStats();
        return {
            activeTaskCount: this.activeTaskCount,
            auditLogSize: this.auditLog.length,
            routingStats,
            mcpConnectionStatus: client_js_1.mcpClient?.getConnectionStatus() || {},
            config: {
    ...
    async shutdown() {
        ...
        if (client_js_1.mcpClient) {
            await client_js_1.mcpClient.disconnectAll();
        }
 ...
 */
function initializeConductor(config) {
    exports.conductor = new Conductor(config);
}

...
exports.NEO4J_USER_FINAL = void 0;
exports.initializeConductorSystem = initializeConductorSystem;
exports.shutdownConductorSystem = shutdownConductorSystem;
exports.getConductorEnvConfig = getConductorEnvConfig;
const index_1 = require("./index");
const client_1 = require("./mcp/client");
const graphops_server_1 = __importDefault(require("./mcp/servers/graphops-server"));
const files_server_1 = __importDefault(require("./mcp/servers/files-server"));
const MCP_TRANSPORTS = ['stdio', 'http', 'jsonrpc', 'grpc'];
const MCP_TRANSPORT_POLICIES = [
    'prefer_grpc_fallback_http',
    'strict',
];
function readTransportEnv(value, allowed, fallback) {
    if (!value) {
        return fallback;
    }
...
exports.NEO4J_USER_FINAL = u1 ?? u2;
/**
 * Initialize the complete Conductor system
 */
async function initializeConductorSystem() {
    ...
    const requiredSecrets = [
        ...
        { name: 'NEO4J_USER', value: exports.NEO4J_USER_FINAL, defaultValue: 'neo4j' },
        {
            name: 'NEO4J_PASSWORD',
    ...
    const config = {
        ...
        mcpTransport: readTransportEnv(process.env.MCP_TRANSPORT, MCP_TRANSPORTS, 'jsonrpc'),
        mcpTransportPolicy: readTransportEnv(process.env.MCP_TRANSPORT_POLICY, MCP_TRANSPORT_POLICIES, 'strict'),
        llmProviders: {
            light: {
    ...
    try {
        ...
        if (process.env.GRAPHOPS_ENABLED !== 'false') {
            const graphOpsConfig = {
                ...
                neo4jUser: exports.NEO4J_USER_FINAL || 'neo4j',
            ...
            graphOpsServer = new graphops_server_1.default(graphOpsConfig);
        ...
        if (process.env.FILES_ENABLED !== 'false') {
            ...
            filesServer = new files_server_1.default(filesConfig);
        ...
        (0, index_1.initializeConductor)(config);
 ...
 */
async function shutdownConductorSystem(servers) {
    ...
    try {
        ...
        const { conductor } = await Promise.resolve().then(() => __importStar(require('./index')));
        if (conductor) {
 ...
 */
function getConductorEnvConfig() {
    return {
        // Conductor settings

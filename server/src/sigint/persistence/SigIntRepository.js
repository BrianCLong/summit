...
exports.SigIntRepository = void 0;
const database_js_1 = require("../../config/database.js");
const comprehensive_telemetry_js_1 = require("../../lib/telemetry/comprehensive-telemetry.js");
class SigIntRepository {
    static instance;
    pgPool;
    neo4jDriver;
    constructor() {
        this.pgPool = (0, database_js_1.getPostgresPool)();
    ...
    async upsertEmitter(emitter) {
      ...
      `;
            ...
            comprehensive_telemetry_js_1.telemetry.subsystems.database.errors.add(1);
            throw err;
        }
        finally {
            client.release();
    ...
    async getSignalsByEmitter(emitterId, limit = 50) {
        const res = await this.pgPool.read('SELECT * FROM sigint_signals WHERE emitter_id = $1 ORDER BY timestamp DESC LIMIT $2', [emitterId, limit]);
        return res.rows.map(this.mapRowToSignal);
    }
    async getRecentSignals(limit = 50) {
        const res = await this.pgPool.read('SELECT * FROM sigint_signals ORDER BY timestamp DESC LIMIT $1', [limit]);
        return res.rows.map(this.mapRowToSignal);
    }
    // --- Graph Sync ---
    async syncEmitterToGraph(emitter) {
    ...
    async syncSignalToGraph(signal) {
        const session = this.neo4jDriver.session();
        try {
...
exports.SigIntRepository = SigIntRepository;

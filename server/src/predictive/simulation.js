...
exports.CampaignSimulator = void 0;
class CampaignSimulator {
    static simulate(graph, request) {
        const { seedEntityIds, steps, parameters } = request;
        const { spreadProbability, decayFactor } = parameters;
        const trajectory = [];
        let currentInfected = new Set(seedEntityIds);
        let allInfected = new Set(seedEntityIds);
        // Initial state
        trajectory.push({
            step: 0,
        ...
        for (let t = 1; t <= steps; t++) {
            ...
            if (nextInfected.size === 0)
                break; // Trajectory ends if no new infections
...
exports.CampaignSimulator = CampaignSimulator;

...
class PredictiveService {
    ...
    async forecastRisk(req) {
        ...
        // 2. Run forecast
        const forecastValues = (0, forecasting_js_1.hybridForecast)(data, req.horizon);
        return {
    ...
    async simulateWhatIf(req) {
        ...
        const scenarioEngine = new SimpleGraph_js_1.SimpleGraphEngine(scenarioSnapshot);
    ...
    async simulateCampaign(req) {
        // 1. Get Snapshot
        const snapshot = await this.getGraphSnapshot(req.investigationId);
        // 2. Initialize Engine
        const engine = new SimpleGraph_js_1.SimpleGraphEngine(snapshot);
        // 3. Run Simulator
        return simulation_js_1.CampaignSimulator.simulate(engine, req);
    }
    getMockHistory(entityId, metric) {
    ...
    async getGraphSnapshot(investigationId) {
            ...
            result.records.forEach((rec) => {
                ...
                if (r) {
                    ...
                    if (!sourceId || !targetId) {
                        ...
                        if (n && nInternal === startNodeId)
                            sourceId = n.properties.id;
                        else if (m && mInternal === startNodeId)
                            sourceId = m.properties.id;
                        if (n && nInternal === endNodeId)
                            targetId = n.properties.id;
                        else if (m && mInternal === endNodeId)
                            targetId = m.properties.id;
                    }
                    if (sourceId && targetId) {

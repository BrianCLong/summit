export function ewma(prev:number, x:number, alpha=0.2){ return alpha*x + (1-alpha)*prev; }
export function robustZ(x:number, mu:number, mad:number){ return mad ? Math.abs(x-mu)/(1.4826*mad) : 0; }

...
import { emitDeleteEvent, OpenLineageClient } from '../../lineage/emit_delete_event.js';
...
export async function handleDelete(
  pgPool: Pool,
  neo4jDriver: Driver,
  openLineageClient: OpenLineageClient,
  message: any // The parsed payload from KafkaConsumer
) {
  ...
  const pkJson = stablePkJson(pkObj);
  const pkHash = computePkHash(pkJson);
  ...
  try {
    ...
    if (ev) {
        ...
        await emitDeleteEvent(openLineageClient, {
          db, schema, table,
          ev_id,

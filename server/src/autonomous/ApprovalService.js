...
class ApprovalService {
     ...
     */
    async requestApproval(action, ttlMinutes = 60) {
        ...
        try {
            await this.db.query(`INSERT INTO autonomous_approvals
    ...
    async getPendingApprovals(tenantId) {
        const result = await this.db.query(`SELECT id, tenant_id, action_type, payload, status, reason, requested_at, expires_at
     ...
     * Approves a request.
     */
    async approve(id, userId, reason) {
    ...
    async respond(id, status, userId, reason) {
        const result = await this.db.query(`UPDATE autonomous_approvals
             SET status = $1, responded_at = NOW(), responded_by = $2, response_reason = $3
             WHERE id = $4 AND status = 'pending' AND expires_at > NOW()`, [status, userId, reason, id]);

...
router.post('/runs/:id/mcp/invoke', requireScope('mcp:invoke'), async (req, res) => {
  ...
  try {
    ...
    const result = await mcpClient.executeTool(server, tool, args || {}, sess?.scopes);
    ...
    try {
      ...
      console.warn('Audit log insert failed (non-fatal):', (e as any)?.message || e);
    }

    try { mcpInvocationsTotal.inc({ status: 'success' }); } catch {}
    ...
    if (err?.message?.includes('Insufficient scopes')) {
      try { mcpInvocationsTotal.inc({ status: 'error' }); } catch {}
      span.setAttribute('error', true); span.end();
    ...
    try { mcpInvocationsTotal.inc({ status: 'error' }); } catch {}
    span.setAttribute('error', true); span.end();
    res.status(500).json({ error: 'invoke_failed', message: err?.message || 'Unknown error' });
  }
  });
});

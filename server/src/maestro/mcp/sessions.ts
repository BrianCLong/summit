...
export function signSession(payload: SessionPayload): { sid: string; token: string } {
  ...
  const data: SessionTokenData = { sid: crypto.randomUUID(), iat: now, exp: now + ttlSec, ...payload };
...
export function revokeSession(sid: string) { revoked.add(sid); }
export function isRevoked(sid: string) { return revoked.has(sid); }

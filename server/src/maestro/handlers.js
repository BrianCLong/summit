...
class MaestroHandlers {
    engine;
    agentService;
    llm;
    graph;
    diffusionCoder;
    constructor(engine, agentService, llm, graph, diffusionCoder) {
        this.engine = engine;
    ...
    async handleLLMCall(task) {
        logger_js_1.logger.info(`[Maestro] Executing LLM Call for task ${task.id}`);
    ...
    async handleRAGQuery(task) {
        logger_js_1.logger.info(`[Maestro] Executing RAG Query for task ${task.id}`);
    ...
    async handleGraphJob(task) {
        logger_js_1.logger.info(`[Maestro] Executing Graph Job for task ${task.id}`);
        const { algorithm, params } = task.payload;
        return this.graph.executeAlgorithm(algorithm, params);
    }
    async handleAgentCall(task) {
        logger_js_1.logger.info(`[Maestro] Executing Agent Call for task ${task.id}`);
        ...
        const subRun = await this.engine.createRun(task.tenantId, agent.templateId, input || {}, 'system-agent-caller');
        return { subRunId: subRun.id, status: 'started' };
    }
    async handleCustom(task) {
        logger_js_1.logger.info(`[Maestro] Executing Custom task ${task.id}`);
        return { result: 'custom execution done', payload: task.payload };
    }
    async handleDiffusionEdit(task) {
        logger_js_1.logger.info(`[Maestro] Executing Diffusion Edit for task ${task.id}`);
        const { prompt, steps, block_length, remasking, threshold } = task.payload;
        const result = await this.diffusionCoder.executeDiffusion(task.runId, task.id, { prompt, steps, block_length, remasking, threshold }, task.tenantId);
        return result;
    }
}
exports.MaestroHandlers = MaestroHandlers;

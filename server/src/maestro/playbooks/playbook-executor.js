...
exports.PlaybookExecutorService = void 0;
...
class PlaybookExecutorService {
    ...
    constructor(deps) {
        ...
        this.ajv.addSchema(schema_js_1.PLAYBOOK_STEP_SCHEMA);
        this.ajv.addSchema(schema_js_1.PLAYBOOK_SCHEMA);
        this.queue = deps.queueFactory
            ? deps.queueFactory('maestro_playbooks', {
                connection: deps.redisConnection,
            })
            : new bullmq_1.Queue('maestro_playbooks', { connection: deps.redisConnection });
        this.worker = deps.workerFactory
            ? deps.workerFactory('maestro_playbooks', async (job) => this.processStep(job.data), { connection: deps.redisConnection, concurrency: 3 })
            : new bullmq_1.Worker('maestro_playbooks', async (job) => this.processStep(job.data), { connection: deps.redisConnection, concurrency: 3 });
    }
    ...
    validatePlaybook(playbook) {
        const validator = this.ajv.getSchema('https://intelgraph.dev/schemas/playbook.json');
        if (!validator) {
    ...
    async executePlaybook(playbook, context) {
        ...
        const stepStatuses = new Map(playbook.steps.map((step) => [step.id, 'pending']));
        ...
        await this.queue.add('step', { runKey, stepId: playbook.steps[0].id }, { jobId: `${runKey}:${playbook.steps[0].id}` });
        return this.awaitRunCompletion(runKey);
    }
    async shutdown() {
    ...
    async processStep(payload) {
        ...
        try {
            ...
            await this.queue.add('step', { runKey: payload.runKey, stepId: nextStep.id }, { jobId: `${payload.runKey}:${nextStep.id}` });
        }
        catch (error) {
            runRecord.stepStatuses.set(step.id, 'failed');
            runRecord.status = 'failed';
            runRecord.completedAt = new Date().toISOString();
            logger_js_1.logger.error(`Playbook step failed: ${step.id}`);
            throw error;
        }
    }
    getRunKey(context) {
    ...
    async awaitRunCompletion(runKey) {
...
exports.PlaybookExecutorService = PlaybookExecutorService;

...
const agent_registry_js_1 = require("../governance/agent-registry.js");
const roi_tracker_js_1 = require("./roi-tracker.js");
class AgentControlPlane {
    /**
     * Checks if an agent is allowed to perform a specific action based on its policies.
     */
    async verifyAgentAction(agentId, action, context) {
        const policies = await agent_registry_js_1.agentRegistry.getPolicies(agentId);
    ...
    async getAgentHealth(agentId) {
        const metrics = await roi_tracker_js_1.agentROITracker.getMetrics(agentId);
        ...
        const recentErrors = metrics.filter(m => m.metricType === 'ERROR_RATE' &&
        ...
        if (recentErrors.length > 0) {
            ...
            if (avgError > 0.1)
                return { score: 50, status: 'DEGRADED' };
            if (avgError > 0.5)
...
exports.agentControlPlane = new AgentControlPlane();

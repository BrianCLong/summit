...
exports.AdaptiveRoutingService = void 0;
class AdaptiveRoutingService {
    profiles = new Map();
    // Epsilon for epsilon-greedy strategy
    epsilon = 0.1;
    updateProfile(agentId, taskType, latency, success, cost) {
    ...
    getBestAgent(taskType, candidates) {
    ...
    calculateScore(agentId, taskType) {
        const key = `${agentId}:${taskType}`;
        ...
        const totalScore = (profile.stats.successRate * 0.6) +
            (latencyScore * 0.2) +
            (costScore * 0.2);
        return {
            agentId,
...
exports.AdaptiveRoutingService = AdaptiveRoutingService;

...
class SignalsService {
    ...
    getSignals(type, sourceId, windowStart, windowEnd = new Date()) {
        return this.signals.filter((s) => s.type === type &&
            s.sourceId === sourceId &&
            s.timestamp >= windowStart &&
            s.timestamp <= windowEnd);
    }
    aggregateSignals(type, sourceId, periodStr) {
        const now = new Date();
        let durationMs = 0;
        switch (periodStr) {
            case '5m':
    ...
    generateHealthSnapshot(tenantId) {
        const now = new Date();
    ...
    calculateComponentHealth(id, type, signals) {
        if (signals.length === 0) {
            return {
                id,
                type,
                score: 100, // Default to healthy if no bad signals
                status: types_js_1.HealthStatus.UNKNOWN,
                metrics: {},
                issues: [],
        ...
        for (const s of signals) {
            if (!metricsAcc[s.type])
                metricsAcc[s.type] = [];
            metricsAcc[s.type].push(s.value);
            if (s.type === types_js_1.SignalType.TASK_FAILURE_COUNT || s.type === types_js_1.SignalType.LLM_ERROR_RATE || s.type === types_js_1.SignalType.CI_BUILD_STATUS && s.value === 0) {
                score -= 20; // Heavy penalty for failures
                issues.push(`Detected failure: ${s.type}`);
            }
            if (s.type === types_js_1.SignalType.POLICY_DENIAL) {
                score -= 10;
                issues.push('Policy denial');
            }
            if (s.type === types_js_1.SignalType.TASK_LATENCY && s.value > 5000) { // Arbitrary threshold for demo
                score -= 5;
        ...
        let status = types_js_1.HealthStatus.HEALTHY;
        if (score < 50)
            status = types_js_1.HealthStatus.CRITICAL;
        else if (score < 80)
            status = types_js_1.HealthStatus.DEGRADED;
        return {
            id,
            type,
            score,

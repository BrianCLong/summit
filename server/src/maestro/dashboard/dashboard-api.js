...
router.get('/summary', (0, rbac_js_1.requirePermission)('dashboard:read'), async (_req, res) => {
    try {
        // Get runs statistics
        const runsQuery = `
      SELECT 
    ...
    `;
        ...
        const avgDuration = runsStats.rows.find((row) => row.status === 'succeeded')
            ?.avg_duration || 180;
        const totalCost = runsStats.rows.reduce((sum, row) => sum + parseFloat(row.total_cost || 0), 0);
        // Get recent runs for activity feed
        const recentRuns = await runs_repo_js_1.runsRepo.list('default', 8);
        // Get recent approvals (mock for now)
        const approvals = totalRuns > 0 && successRate < 0.9
...
router.get('/pipelines', (0, rbac_js_1.requirePermission)('pipeline:read'), async (_req, res) => {
    try {
        const query = `
...
router.get('/autonomy', (0, rbac_js_1.requirePermission)('autonomy:read'), async (_req, res) => {
    try {
    ...
    `;
        ...
        const autonomyLevel = successRate > 0.98
            ? 4
            : successRate > 0.95
                ? 3
                : successRate > 0.9
...
router.put('/autonomy', (0, rbac_js_1.requirePermission)('autonomy:update'), async (req, res) => {
    try {
...
router.get('/recipes', (0, rbac_js_1.requirePermission)('recipe:read'), async (_req, res) => {
    try {
        const recipes = [

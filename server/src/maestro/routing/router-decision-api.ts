...
router.get('/runs/:runId/nodes/:nodeId/routing', requirePermission('run:read'), async (req, res) => {
...
router.post('/runs/:runId/nodes/:nodeId/override-routing', requirePermission('routing:override'), async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: 'Router decision overridden successfully' });
  } catch (error: any) {
...
router.get('/audit/router-decisions/:decisionId/export', requirePermission('audit:read'), async (req, res) => {
...
router.get('/routing/drift-analysis', requirePermission('routing:read'), async (req, res) => {
      ...
      analysis: {
        ...
        overrideRate: rows.length ? 
          (rows.reduce((sum, r) => sum + r.override_count, 0) / 
           rows.reduce((sum, r) => sum + r.total_selections, 0)) * 100 : 0,
        avgScore: rows.length ? 
          rows.reduce((sum, r) => sum + parseFloat(r.avg_score || 0), 0) / rows.length : 0
      }
    };

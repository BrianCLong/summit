...
export function generateMerkleProof(tree: MerkleNode, targetHash: string): MerkleProof | null {
...
export class ProvenanceLedger {
  ...
  async createClaim(claim: Claim): Promise<string> {
    ...
    await pool.query(
      ...
      [claim.id, claim.runId, claim.statement, JSON.stringify(claim.evidence), claim.confidence, claim.source, claim.timestamp]
    );
    ...
    for (const existingClaim of existingClaims) {
      ...
      if (contradiction) {
        await pool.query(
          ...
          [newClaimId, existingClaim.id, contradiction.reason, contradiction.severity]
        );
      }
    }
  }
  
  private analyzeContradiction(claimA: any, claimB: any): { reason: string; severity: 'low' | 'medium' | 'high' } | null {

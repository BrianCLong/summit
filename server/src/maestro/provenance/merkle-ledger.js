...
exports.provenanceLedger = exports.PROVENANCE_LEDGER_SCHEMA = exports.ProvenanceLedger = void 0;
...
function buildMerkleTree(evidenceList) {
    if (evidenceList.length === 0) {
        throw new Error('Cannot build Merkle tree from empty evidence list');
    }
    // Create leaf nodes
    let nodes = evidenceList.map((evidence) => ({
        hash: evidence.hash,
        data: evidence,
    }));
...
function generateMerkleProof(tree, targetHash) {
    const proof = [];
...
function verifyMerkleProof(proof) {
    ...
    for (const step of proof.proof) {
        ...
        currentHash = (0, crypto_1.createHash)('sha256')
            .update(Buffer.concat([
...
class ProvenanceLedger {
    async registerEvidence(evidence) {
        const span = otel_tracing_js_1.otelService.createSpan('provenance.register_evidence');
        try {
            const pool = (0, postgres_js_1.getPostgresPool)();
            // Store evidence in ledger
            await pool.query(`INSERT INTO evidence_ledger 
    ...
    async updateMerkleTree(runId) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        // Get all evidence for this run
        const { rows } = await pool.query(`SELECT id, run_id, artifact_type, hash, timestamp, metadata
       ...
       ORDER BY timestamp ASC`, [runId]);
        ...
        // Store tree root
        await pool.query(`INSERT INTO merkle_trees (run_id, root_hash, evidence_count, created_at)
       VALUES ($1, $2, $3, now())
       ON CONFLICT (run_id) DO UPDATE SET
       root_hash = $2, evidence_count = $3, updated_at = now()`, [runId, tree.hash, evidenceList.length]);
    }
    async exportManifest(runId) {
        const span = otel_tracing_js_1.otelService.createSpan('provenance.export_manifest');
        try {
            const pool = (0, postgres_js_1.getPostgresPool)();
            // Get Merkle tree info
            const { rows: treeRows } = await pool.query(`SELECT root_hash, evidence_count, created_at
         FROM merkle_trees WHERE run_id = $1`, [runId]);
            if (!treeRows.length) {
                throw new Error(`No Merkle tree found for run ${runId}`);
            }
            // Get all evidence
            const { rows: evidenceRows } = await pool.query(`SELECT id, run_id, artifact_type, hash, timestamp, metadata
         FROM evidence_ledger
         WHERE run_id = $1
         ORDER BY timestamp ASC`, [runId]);
            const manifest = evidenceRows.map((row) => ({
                id: row.id,
                runId: row.run_id,
                artifactType: row.artifact_type,
                hash: row.hash,
            ...
            // Log export event
            await pool.query(`INSERT INTO provenance_exports (run_id, root_hash, exported_at)
         VALUES ($1, $2, now())`, [runId, exportData.rootHash]);
            span?.addSpanAttributes({
    ...
    async verifyManifest(manifest) {
        try {
            const evidenceList = manifest.manifest || [];
            const tree = buildMerkleTree(evidenceList);
    ...
    async createClaim(claim) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        await pool.query(`INSERT INTO claims (id, run_id, statement, evidence, confidence, source, timestamp)
    ...
    async detectContradictions(newClaimId) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        // Get the new claim
        const { rows: newClaimRows } = await pool.query(`SELECT * FROM claims WHERE id = $1`, [newClaimId]);
        if (!newClaimRows.length)
            return;
        const newClaim = newClaimRows[0];
        // Find potentially contradicting claims (same run, different statements)
        const { rows: existingClaims } = await pool.query(`SELECT * FROM claims 
       WHERE run_id = $1 AND id != $2`, [newClaim.run_id, newClaimId]);
        for (const existingClaim of existingClaims) {
            const contradiction = this.analyzeContradiction(newClaim, existingClaim);
            if (contradiction) {
                await pool.query(`INSERT INTO contradictions (id, claim_a, claim_b, conflict_reason, severity, timestamp)
    ...
    analyzeContradiction(claimA, claimB) {
        ...
        if ((statementA.includes('successful') && statementB.includes('failed')) ||
    ...
    async getClaimsAndContradictions(runId) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        const { rows: claimsRows } = await pool.query(`SELECT * FROM claims WHERE run_id = $1 ORDER BY timestamp DESC`, [runId]);
        const { rows: contradictionsRows } = await pool.query(`SELECT c.*, ca.statement as claim_a_statement, cb.statement as claim_b_statement
       FROM contradictions c
       JOIN claims ca ON c.claim_a = ca.id
       JOIN claims cb ON c.claim_b = cb.id
       WHERE ca.run_id = $1 OR cb.run_id = $1
       ORDER BY c.timestamp DESC`, [runId]);
        return {
...
exports.PROVENANCE_LEDGER_SCHEMA = `

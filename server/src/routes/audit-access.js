...
exports.auditAccessRouter = void 0;
const express_1 = require("express");
const postgres_js_1 = require("../db/postgres.js");
const AuditAccessLogRepo_js_1 = require("../repos/AuditAccessLogRepo.js");
const logger_js_1 = __importDefault(require("../config/logger.js"));
const routeLogger = logger_js_1.default.child({ name: 'AuditAccessRoutes' });
exports.auditAccessRouter = (0, express_1.Router)();
/**
 ...
 */
exports.auditAccessRouter.get('/cases/:caseId', async (req, res) => {
    try {
        ...
        const repo = new AuditAccessLogRepo_js_1.AuditAccessLogRepo(pg);
        const logs = await repo.getLogsForCase(caseId, tenantId, limit);
        routeLogger.info({ caseId, tenantId, logCount: logs.length }, 'Audit logs retrieved for case');
        res.json({
            caseId,
        ...
        routeLogger.error({ error: error.message }, 'Failed to get case audit logs');
 ...
 */
exports.auditAccessRouter.get('/users/:userId', async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        if (!tenantId) {
            return res.status(400).json({ error: 'tenant_required' });
        ...
        routeLogger.info({ userId, tenantId, logCount: logs.length }, 'Audit logs retrieved for user');
        res.json({
            userId,
            totalLogs: logs.length,
            logs,
        ...
        routeLogger.error({ error: error.message }, 'Failed to get user audit logs');
        res.status(500).json({ error: error.message });
    }
});
/**
 ...
 */
exports.auditAccessRouter.post('/query', async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        if (!tenantId) {
            return res.status(400).json({ error: 'tenant_required' });
        ...
        routeLogger.info({ tenantId, filters: query, logCount: logs.length }, 'Audit logs queried');
        res.json({
            totalCount,
        ...
        routeLogger.error({ error: error.message }, 'Failed to query audit logs');
        res.status(500).json({ error: error.message });
    }
});
/**
 ...
 */
exports.auditAccessRouter.get('/correlation/:correlationId', async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        if (!tenantId) {
            return res.status(400).json({ error: 'tenant_required' });
        ...
        routeLogger.info({ correlationId, tenantId, logCount: logs.length }, 'Audit logs retrieved by correlation ID');
        res.json({
            correlationId,
            totalLogs: logs.length,
            logs,
        ...
        routeLogger.error({ error: error.message }, 'Failed to get correlated audit logs');
        res.status(500).json({ error: error.message });
    }
});
/**
 ...
 */
exports.auditAccessRouter.post('/verify-integrity', async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        if (!tenantId) {
            return res.status(400).json({ error: 'tenant_required' });
        ...
        routeLogger.error({ error: error.message }, 'Failed to verify integrity');
        res.status(500).json({ error: error.message });
    }
});
/**
 ...
 */
exports.auditAccessRouter.get('/stats', async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        if (!tenantId) {
            return res.status(400).json({ error: 'tenant_required' });
        ...
        const actionStats = await pg.query(`SELECT action, COUNT(*) as count
       FROM maestro.audit_access_logs
       WHERE tenant_id = $1
       GROUP BY action
       ORDER BY count DESC`, [tenantId]);
        // Get counts by legal basis
        const legalBasisStats = await pg.query(`SELECT legal_basis, COUNT(*) as count
       FROM maestro.audit_access_logs
       WHERE tenant_id = $1
       ...
       ORDER BY count DESC`, [tenantId]);
        ...
        routeLogger.error({ error: error.message }, 'Failed to get audit stats');
        res.status(500).json({ error: error.message });
    }
});
exports.default = exports.auditAccessRouter;

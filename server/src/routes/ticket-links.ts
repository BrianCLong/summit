...
import { linkTicketToRun, linkTicketToDeployment, getTicketLinks } from '../services/ticket-links.js';
...
router.post('/tickets/:provider/:externalId/link-run', express.json(), async (req, res) => {
  const { provider, externalId } = req.params as any;
...
router.post('/tickets/:provider/:externalId/link-deployment', express.json(), async (req, res) => {
  const { provider, externalId } = req.params as any;

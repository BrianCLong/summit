...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantId(req);
      const status = ((req.query.status as string) as string) as PolicySuggestion['status'] | undefined;
      const limit = parseInt(((req.query.limit as string) as string) as string) || 20;
      const offset = parseInt(((req.query.offset as string) as string) as string) || 0;
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const scope: AnomalyDetectionScope = {
        tenantIds: [tenantId],
        minSeverity: ((req.query.severity as string) as string) as BehavioralAnomaly['severity'],
        timeRange: {
      ...
      const filteredAnomalies = ((req.query.status as string) as string)
        ? anomalies.filter(a => a.status === ((req.query.status as string) as string))
        : anomalies;
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const tenantId = getTenantId(req);
      const endDate = ((req.query.endDate as string) as string)
        ? new Date(((req.query.endDate as string) as string) as string)
        : new Date();
      const startDate = ((req.query.startDate as string) as string)
        ? new Date(((req.query.startDate as string) as string) as string)

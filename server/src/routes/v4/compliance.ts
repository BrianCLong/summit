...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (((req.query.category as string) as string)) {
      controls = controls.filter(c => c.category === ((req.query.category as string) as string));
    }

    // Filter by automatable
    if (((req.query.automatable as string) as string) !== undefined) {
      const automatable = ((req.query.automatable as string) as string) === 'true';
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (((req.query.domain as string) as string)) {
      controls = controls.filter(c => c.subcategory === ((req.query.domain as string) as string));
    }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (((req.query.sourceFramework as string) as string)) {
      filteredMappings = filteredMappings.filter(
        m => m.source.framework === ((req.query.sourceFramework as string) as string)
      );
    }
    if (((req.query.targetFramework as string) as string)) {
      filteredMappings = filteredMappings.filter(
        m => m.target.framework === ((req.query.targetFramework as string) as string)
      );
    }

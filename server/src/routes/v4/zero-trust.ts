...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const keyHandle = await zeroTrustService!.hsm.getKey(req.params.id as string as string);

      if (!keyHandle) {
        return res.status(404).json({
          error: {
            code: 'NOT_FOUND',
            message: `Key not found: ${req.params.id as string as string}`,
          },
        });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await zeroTrustService!.recordSecurityEvent(
        ...
        getTenantId(req),
        'key:sign',
        { keyId: req.params.id as string as string }
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const valid = await zeroTrustService!.hsm.verify(req.params.id as string as string, data, signature);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const newKeyHandle = await zeroTrustService!.hsm.rotateKey(req.params.id as string as string);

      logger.info({
        oldKeyId: req.params.id as string as string,
      ...
      await zeroTrustService!.recordSecurityEvent(
        ...
        getTenantId(req),
        'key:rotate',
        { oldKeyId: req.params.id as string as string, newKeyId: newKeyHandle.id }
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const attestation = await zeroTrustService!.hsm.attestKey(req.params.id as string as string);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const entry = await zeroTrustService!.audit.getEntry(req.params.id as string as string);

      if (!entry) {
        return res.status(404).json({
          error: {
            code: 'NOT_FOUND',
            message: `Audit entry not found: ${req.params.id as string as string}`,
          },
        });
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const verification = await zeroTrustService!.audit.verifyEntry(req.params.id as string as string);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const proof = await zeroTrustService!.audit.getMerkleProof(req.params.id as string as string);

  ...
  async (req: Request, res: Response) => {
    try {
      const query: AuditQuery = {
        tenantId: getTenantId(req),
        actorId: ((req.query.actorId as string) as string) as string,
        resourceType: ((req.query.resourceType as string) as string) as string,
        resourceId: ((req.query.resourceId as string) as string) as string,
        entryTypes: ((req.query.entryType as string) as string)
          ? [((req.query.entryType as string) as string) as any]
          : undefined,
        startTime: ((req.query.startTime as string) as string) as string,
        endTime: ((req.query.endTime as string) as string) as string,
        limit: parseInt(((req.query.limit as string) as string) as string) || 100,
        offset: parseInt(((req.query.offset as string) as string) as string) || 0,
      };

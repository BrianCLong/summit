...
exports.phantomLimbRouter = void 0;
// server/src/routes/phantom_limb.ts
const express_1 = require("express");
const PhantomLimbService_js_1 = require("../phantom_limb/PhantomLimbService.js");
...
router.get('/analysts', async (req, res, next) => {
    try {
        const analysts = await PhantomLimbService_js_1.phantomLimbService.getOnlineAnalysts();
        res.json(analysts);
    }
    catch (error) {
        next(error);
...
router.post('/reconstitute', async (req, res, next) => {
    try {
        ...
        const newGhost = await PhantomLimbService_js_1.phantomLimbService.reconstituteCognition(artifacts);
...
router.post('/query/:ghostId', async (req, res, next) => {
    try {
        ...
        const response = await PhantomLimbService_js_1.phantomLimbService.queryDigitalGhost(ghostId, query);
        res.json(response);
    }
    catch (error) {
        next(error);
...
exports.phantomLimbRouter = router;

...
caseRouter.get('/:id/overview', async (req, res) => {
  try {
    ...
    const reason = ((req.query.reason as string) as string) as string;
    const legalBasis = ((req.query.legalBasis as string) as string) as LegalBasis;
...
caseRouter.get('/:id', async (req, res) => {
  try {
    ...
    const auditContext = {
      reason,
      legalBasis,
      warrantId: ((req.query.warrantId as string) as string) as string,
      ipAddress: req.ip,
...
caseRouter.get('/', async (req, res) => {
  try {
    ...
    const cases = await service.listCases({
      ...
      compartment: ((req.query.compartment as string) as string) as string,
      policyLabels: ((req.query.policyLabels as string) as string)
        ? (((req.query.policyLabels as string) as string) as string).split(',')
        : undefined,
      limit: ((req.query.limit as string) as string) ? parseInt(((req.query.limit as string) as string) as string) : undefined,
      offset: ((req.query.offset as string) as string)
        ? parseInt(((req.query.offset as string) as string) as string)
        : undefined,
    });
...
caseRouter.get('/:id/comments', async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const limit = ((req.query.limit as string) as string) ? parseInt(((req.query.limit as string) as string) as string) : undefined;
    const offset = ((req.query.offset as string) as string) ? parseInt(((req.query.offset as string) as string) as string) : undefined;

...
caseRouter.get('/:id/overview', async (req, res) => {
  try {
    ...
    const reason = (req.query.reason as string) as string;
...
caseRouter.get('/:id', async (req, res) => {
  try {
    ...
    const auditContext = {
      reason,
      legalBasis,
      warrantId: (req.query.warrantId as string) as string,
      ipAddress: req.ip,
...
caseRouter.get('/', async (req, res) => {
  try {
    ...
    const cases = await service.listCases({
      ...
      compartment: (req.query.compartment as string) as string,

...
function ensurePackBuiltDevOnly() {
    const devAutoBuild = process.env.MC_DEV_AUTO_BUILD_PACK === 'true' &&
        process.env.NODE_ENV !== 'production';
    if (!devAutoBuild)
        return;
    const build = (0, child_process_1.spawnSync)('bash', ['scripts/build_policy_pack.sh'], {
        encoding: 'utf8',
    });
    if (build.status !== 0) {
    ...
    const bundleJson = path_1.default.resolve(process.cwd(), 'contracts/policy-pack/v0/signing/cosign.bundle.json');
    if (!fs_1.default.existsSync(bundleJson)) {
        const sign = (0, child_process_1.spawnSync)('bash', ['scripts/cosign_sign.sh'], {
            encoding: 'utf8',
        });
        if (sign.status !== 0)
            console.warn('[pack:auto-sign] warn:', sign.stderr);
    }
}
router.get('/policy/packs/:packId', async (req, res) => {
    ...
    const packTar = path_1.default.resolve(process.cwd(), 'dist/policy-pack/v0/policy-pack-v0.tar');
    const bundleJson = path_1.default.resolve(process.cwd(), 'contracts/policy-pack/v0/signing/cosign.bundle.json');
    const manifestPath = path_1.default.resolve(process.cwd(), 'contracts/policy-pack/v0/manifest.json');
    if (!fs_1.default.existsSync(packTar)) {
    ...
    const digestValue = manifest?.manifest?.digest?.value || manifest?.digest?.value || 'TBD';
    // Caching headers
    ...
    if (process.env.MC_INLINE_BUNDLE === 'true' && fs_1.default.existsSync(bundleJson)) {
        res.setHeader('X-Cosign-Bundle', fs_1.default.readFileSync(bundleJson, 'utf8'));
    }
    const stat = fs_1.default.statSync(packTar);
    res.setHeader('Content-Length', String(stat.size));
    fs_1.default.createReadStream(packTar).pipe(res);
});
...
router.get('/policy/packs/:packId/attestation', (req, res) => {
    ...
    const bundleJson = path_1.default.resolve(process.cwd(), 'contracts/policy-pack/v0/signing/cosign.bundle.json');
    const manifestPath = path_1.default.resolve(process.cwd(), 'contracts/policy-pack/v0/manifest.json');
    if (!fs_1.default.existsSync(bundleJson) || !fs_1.default.existsSync(manifestPath))
    ...
    res.send(fs_1.default.readFileSync(bundleJson, 'utf8'));
});
// POST /v1/evidence

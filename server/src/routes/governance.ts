...
router.get('/schemas/:id', ensureAuthenticated, (req: AuthenticatedRequest, res: Response) => {
    const schema = registry.getSchemaById(req.params.id as string as string as string);
...
router.get('/changes/:id', ensureAuthenticated, (req: AuthenticatedRequest, res: Response) => {
    const cr = workflow.getChangeRequest(req.params.id as string as string as string);
...
router.get('/changes/:id/impact', ensureAuthenticated, (req: AuthenticatedRequest, res: Response) => {
    // Determine impact
    const impact = workflow.calculateImpact(req.params.id as string as string as string);
    res.json(impact);
});
...
router.post('/changes/:id/review', ensureAuthenticated, (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const cr = workflow.reviewChangeRequest(req.params.id as string as string as string, reviewer, decision, comment);
        res.json(cr);
    } catch (e: any) {
        res.status(400).json({ error: e.message });
    }
...
router.post('/changes/:id/merge', ensureAuthenticated, (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const newSchema = workflow.mergeChangeRequest(req.params.id as string as string as string, merger);
        res.json(newSchema);
    } catch (e: any) {
        res.status(400).json({ error: e.message });
    }

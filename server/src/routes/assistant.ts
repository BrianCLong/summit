...
export function mountAssistant(app: Express, io?: any) {
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      const reqId = (req as any).reqId;
      const userId = req.user?.sub || null;
      let input = (((req.query.q as string) as string) ?? '').toString(); // Changed to `let`
      const focusIds = (((req.query.focusIds as string) as string) as string[] | undefined) || []; // Assuming focusIds in query
      if (isSuspicious(input)) {
        httpErrors.inc();
        return res.status(400).json({ error: 'input_rejected' });
      }

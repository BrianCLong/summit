...
const exportRequestSchema = z.object({
  ...
  context: z.object({
    user_id: z.string(),
    user_role: z.enum(['analyst', 'investigator', 'admin', 'compliance-officer']),
    user_scopes: z.array(z.string()).optional(),
    tenant_id: z.string(),
    purpose: z.enum(['investigation', 'threat-intel', 'fraud-risk', 'commercial', 'research']),
...
router.post('/export', async (req, res) => {
  ...
  const requestId = req.headers['x-request-id'] as string || `export-${Date.now()}`;
  ...
  try {
    ...
    const response: ExportResponse = {
      decision: {
        effect: policyDecision.action === 'allow' ? 'allow' :
                policyDecision.action === 'deny' ? 'deny' : 'review',
...
router.post('/export/simulate', async (req, res) => {
  const requestId = req.headers['x-request-id'] as string || `sim-${Date.now()}`;
  ...
  try {
    ...
    const baseline: ExportResponse['decision'] = {
      effect: baselineDecision.action === 'allow' ? 'allow' :
              baselineDecision.action === 'deny' ? 'deny' : 'review',
    ...
    for (const scenario of simulateRequest.simulation.what_if_scenarios || []) {
      const modifiedContext = { ...simulateRequest.context, ...scenario.changes };
      ...
      const scenarioResult = {
        effect: scenarioDecision.action === 'allow' ? 'allow' :
                scenarioDecision.action === 'deny' ? 'deny' : 'review',
      ...
      scenarios.push({
        ...
        impact: {
          ...
          violations_removed: baseline.violations
            ...
            .map(v => v.code),
          risk_level_change: baseline.risk_assessment.level !== scenarioResult.risk_assessment.level
    ...
    logger.info({
      ...
      scenarioResults: scenarios.map(s => ({ name: s.name, decision: s.decision.effect }))
...
// Helper functions
function generateRedactions(sources: any[], context: any): Array<{field: string; reason: string; replacement: string}> {
  const redactions = [];
  ...
  for (const source of sources) {
    if (source.fields) {
      for (const field of source.fields) {
        if (['email', 'phone', 'ssn'].includes(field.type) && !context.pii_export_approved) {
          redactions.push({
            field: field.name,
  ...
  for (const source of sources) {
    if (source.classification === 'confidential' && context.export_type === 'dataset') {
      redactions.push({

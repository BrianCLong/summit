...
exports.auroraRouter = void 0;
// server/src/routes/aurora.ts
const express_1 = require("express");
const AuroraService_js_1 = require("../aurora/AuroraService.js");
...
router.get('/implants', async (req, res, next) => {
    try {
        const status = await AuroraService_js_1.auroraService.getImplantStatus();
        res.json(status);
    }
    catch (error) {
        next(error);
...
router.post('/handshake/:implantId', async (req, res, next) => {
    try {
        ...
        const implant = await AuroraService_js_1.auroraService.neuralHandshake(implantId);
        res.json(implant);
    }
    catch (error) {
        next(error);
...
router.post('/cortex-overlay', async (req, res, next) => {
    try {
        ...
        const confirmation = await AuroraService_js_1.auroraService.pushToCortex(overlayData);

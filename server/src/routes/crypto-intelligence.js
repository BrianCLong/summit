...
const CryptoIntelligenceService_js_1 = require("../services/CryptoIntelligenceService.js");
const auth_js_1 = require("../middleware/auth.js");
const logger_js_1 = require("../config/logger.js");
const router = express_1.default.Router();
const service = CryptoIntelligenceService_js_1.CryptoIntelligenceService.getInstance();
...
const enforceAnalystAccess = [auth_js_1.ensureAuthenticated];
router.post('/analyze/transaction', enforceAnalystAccess, async (req, res) => {
    try {
        ...
        logger_js_1.logger.error({ error }, 'Error analyzing transaction');
...
router.post('/cluster/wallet', enforceAnalystAccess, async (req, res) => {
    try {
        ...
        logger_js_1.logger.error({ error }, 'Error clustering wallets');
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.post('/monitor/darkweb', enforceAnalystAccess, async (req, res) => {
    try {
        ...
        if (!marketplace || !keyword)
            return res.status(400).json({ error: 'marketplace and keyword are required' });
        const result = await service.monitorDarkWeb(marketplace, keyword);
        res.json(result);
    }
    catch (error) {
        logger_js_1.logger.error({ error }, 'Error monitoring dark web');
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.post('/detect/mixer', enforceAnalystAccess, async (req, res) => {
    try {
        ...
        logger_js_1.logger.error({ error }, 'Error detecting mixer');
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.get('/threat-actor/:id', enforceAnalystAccess, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        logger_js_1.logger.error({ error }, 'Error profiling threat actor');
        res.status(500).json({ error: 'Internal server error' });
    }
});

...
const GACoremetricsService_js_1 = require("../services/GACoremetricsService.js");
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../config/logger.js"));
const api_1 = require("@opentelemetry/api");
const router = express_1.default.Router();
const log = logger_js_1.default.child({ name: 'GACoreMetricsAPI' });
const tracer = api_1.trace.getTracer('ga-core-metrics', '1.0.0');
/**
 * Prometheus metrics endpoint for scraping
 */
router.get('/metrics', async (req, res) => {
    try {
        res.set('Content-Type', GACoremetricsService_js_1.gaCoreMetrics.getMetricsRegistry().contentType);
        res.end(await GACoremetricsService_js_1.gaCoreMetrics.getMetricsRegistry().metrics());
    }
    catch (error) {
...
router.get('/status', async (req, res) => {
    try {
        const status = await GACoremetricsService_js_1.gaCoreMetrics.getCurrentStatus();
        res.json(status);
    }
    catch (error) {
    ...
    `);
        const gates = [
            ...
            })),
            {
                ...
                threshold: 0.9,
                status: parseFloat(appealMetrics.rows[0]?.metrics?.sla_compliance_rate || '0') >= 0.9
                    ? 'PASS'
                    : 'FAIL',
            ...
            },
            {
                ...
                threshold: 0.95,
                status: parseFloat(exportMetrics.rows[0]?.metrics?.integrity_rate || '0') >=
                    0.95
                    ? 'PASS'
                    : 'FAIL',
            ...
            },
            {
                ...
                threshold: 0.8,
                status: parseFloat(copilotMetrics.rows[0]?.success_rate || '0') >= 0.8
                    ? 'PASS'
                    : 'FAIL',
                category: 'AI Copilot',
            },
...
router.get('/er-ops/precision-recall', async (req, res) => {
    return tracer.startActiveSpan('ga_core_metrics.er_ops.precision_recall', async (span) => {
        try {
            ...
            ORDER BY date DESC
          `, [modelType, days]);
            span.setAttributes({ 'er.ops.row_count': result.rowCount || 0 });
...
router.get('/er-ops/rollbacks', async (req, res) => {
    return tracer.startActiveSpan('ga_core_metrics.er_ops.rollbacks', async (span) => {
        try {
...
router.get('/er-ops/conflicts', async (req, res) => {
    return tracer.startActiveSpan('ga_core_metrics.er_ops.conflicts', async (span) => {
        try {
            const { days = 30 } = req.query;
            const pool = (0, database_js_1.getPostgresPool)();
            const parsedDays = parseInt(days, 10);
    ...
    try {
      ...
      `),
        ...
        const dashboard = {
            timestamp: new Date().toISOString(),
            entityResolution: {
                personPrecision: erMetrics.rows.find((r) => r.entity_type === 'PERSON')?.precision ||
                    1.0,

...
router.get('/graph', async (req, res) => {
  ...
  try {
    ...
    graphLogger.error('Failed to retrieve graph data', { error: error.message, stack: error.stack });
...
router.get('/agent-actions', async (req, res) => {
  ...
  try {
    ...
    graphLogger.error('Failed to retrieve agent actions', { error: error.message, stack: error.stack });
...
function generateSampleNodes(count) {
  ...
  const sampleNames = {
    ...
    'Organization': ['Acme Corp', 'Global Industries', 'Tech Solutions Inc', 'Data Systems LLC', 'Innovation Labs'],
    'Location': ['New York', 'London', 'Tokyo', 'Berlin', 'San Francisco'],
    'Document': ['Intelligence Report #1', 'Analysis Document', 'Meeting Notes', 'Project Plan', 'Risk Assessment'],
    'Event': ['Conference 2024', 'Board Meeting', 'Security Incident', 'Product Launch', 'Annual Review']
  };
...
function generateSampleEdges(nodes, count) {
  const edgeTypes = ['CONNECTED_TO', 'WORKS_FOR', 'LOCATED_AT', 'AUTHORED', 'PARTICIPATED_IN', 'OWNS', 'MANAGES'];
  const edges = [];

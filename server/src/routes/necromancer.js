...
exports.necromancerRouter = void 0;
// server/src/routes/necromancer.ts
const express_1 = require("express");
const NecromancerService_js_1 = require("../necromancer/NecromancerService.js");
...
router.get('/synthetics', async (req, res, next) => {
    try {
        const synthetics = await NecromancerService_js_1.necromancerService.getAllSynthetics();
        res.json(synthetics);
    }
    catch (error) {
        next(error);
...
router.post('/initiate', async (req, res, next) => {
    try {
        ...
        const synthetic = await NecromancerService_js_1.necromancerService.initiateDigitalAfterlife(params);
...
router.get('/synthetics/:syntheticId/activity', async (req, res, next) => {
    try {
        ...
        const activity = await NecromancerService_js_1.necromancerService.getSyntheticActivity(syntheticId);
        res.json(activity);
    }
    catch (error) {
        next(error);
...
exports.necromancerRouter = router;

...
router.get(
  '/detect/:campaignId',
  authorize('run:detection', req => ({ id: req.params.campaignId, type: 'campaign' })),
  async (req, res, next) => {
    try {
 ...
 */
router.get(
  '/narrative/:id/timeline',
  authorize('read:narrative', req => ({ id: req.params.id, type: 'narrative' })),
  async (req, res, next) => {
    try {
      const service = InfluenceOperationsService.getInstance();

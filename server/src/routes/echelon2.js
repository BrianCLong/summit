...
exports.echelon2Router = void 0;
// server/src/routes/echelon2.ts
const express_1 = require("express");
const Echelon2Service_js_1 = require("../echelon2/Echelon2Service.js");
...
router.post('/ingest', async (req, res, next) => {
    try {
        ...
        const confirmation = await Echelon2Service_js_1.echelon2Service.processEnvironmentalSample(reading);
        if (confirmation) {
...
router.get('/confirmations', async (req, res, next) => {
    try {
        const confirmations = await Echelon2Service_js_1.echelon2Service.getAllConfirmations();
...
router.get('/confirmations/:targetId', async (req, res, next) => {
    try {
        ...
        const confirmations = await Echelon2Service_js_1.echelon2Service.getConfirmationsForTarget(targetId);
        res.json(confirmations);
    }
    catch (error) {
        next(error);

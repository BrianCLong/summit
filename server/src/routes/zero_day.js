...
exports.zeroDayRouter = void 0;
// server/src/routes/zero_day.ts
const express_1 = require("express");
const ZeroDayService_js_1 = require("../zero_day/ZeroDayService.js");
...
router.post('/designate-threat', async (req, res, next) => {
    try {
        ...
        const log = await ZeroDayService_js_1.zeroDayService.designateExistentialThreat(threatAnalysis);
...
router.post('/delegate-authority', async (req, res, next) => {
    try {
        ...
        const log = await ZeroDayService_js_1.zeroDayService.delegateAutonomousAuthority(threatId, humanOperatorId);
        res.json(log);
    }
    catch (error) {
...
router.get('/status/:threatId', async (req, res, next) => {
    try {
        const threatId = (0, http_param_js_1.firstStringOr)(req.params.threatId, '');
        const log = await ZeroDayService_js_1.zeroDayService.getKillChainStatus(threatId);
        if (log) {
            res.json(log);
        }
        else {

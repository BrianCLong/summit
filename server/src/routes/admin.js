...
const control_js_1 = require("../temporal/control.js");
const auth_js_1 = require("../middleware/auth.js");
const authorization_js_1 = require("../middleware/authorization.js");
const GAEnrollmentService_js_1 = __importDefault(require("../services/GAEnrollmentService.js"));
...
router.use(auth_js_1.ensureAuthenticated, (0, authorization_js_1.authorize)('manage_users'));
// GA Signals Endpoint
router.get('/ga/signals', async (_req, res) => {
    try {
        const config = await GAEnrollmentService_js_1.default.getConfig();
        // Get real-time stats
...
router.post('/ga/config', express_1.default.json(), async (req, res) => {
    try {
        ...
        if (maxTenants)
            updates.maxTenants = maxTenants;
        if (maxUsers)
            updates.maxUsers = maxUsers;
        await GAEnrollmentService_js_1.default.updateConfig(updates);
        res.json({ ok: true, config: await GAEnrollmentService_js_1.default.getConfig() });
    }
    catch (error) {
        res.status(500).json({ ok: false, error: error instanceof Error ? error.message : 'Unknown error' });
    }
...
router.post('/config', express_1.default.json(), (req, res) => {
    const tenantId = req.query.tenantId || '';
...
router.post('/tenant-defaults', express_1.default.json(), (req, res) => {
...
router.post('/failover/mode', express_1.default.json(), (req, res) => {
    const { mode } = req.body;
...
router.post('/failover/status', express_1.default.json(), (req, res) => {
...
router.post('/shadow/configs', express_1.default.json(), async (req, res) => {
    ...
    try {
        const pool = (0, database_js_1.getPostgresPool)();
        await pool.query(`INSERT INTO shadow_traffic_configs (tenant_id, target_url, sampling_rate, compare_responses)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (tenant_id) DO UPDATE 
       SET target_url = EXCLUDED.target_url, 
           sampling_rate = EXCLUDED.sampling_rate, 
           compare_responses = EXCLUDED.compare_responses,
           updated_at = CURRENT_TIMESTAMP`, [tenantId, targetUrl, samplingRate ?? 0, compareResponses ?? false]);
        (0, ShadowTrafficMiddleware_js_1.clearShadowCache)(tenantId);
...
const n8nCfgPath = process.cwd().endsWith('/server')
    ? path_1.default.resolve(process.cwd(), 'config/n8n-flows.json')
    : path_1.default.resolve(process.cwd(), 'server/config/n8n-flows.json');
router.get('/n8n-flows', (_req, res) => {
    try {
        const raw = fs_1.default.readFileSync(n8nCfgPath, 'utf8');
...
router.post('/n8n-flows', express_1.default.json(), (req, res) => {
    const body = req.body || {};
    ...
    try {
        fs_1.default.mkdirSync(path_1.default.dirname(n8nCfgPath), { recursive: true });
        fs_1.default.writeFileSync(n8nCfgPath, JSON.stringify(out, null, 2) + '\n');
...
router.get('/opa/validate', async (_req, res) => {
    try {
        ...
        const health = await axios_1.default
        ...
        let evalReason = '';
        try {
            const test = await axios_1.default.post(`${base}/v1/data/maestro/integrations/n8n/trigger`, {
                input: {
                    tenantId: 'test',
                    role: 'ADMIN',
...
router.get('/opa/bundle-source', async (_req, res) => {
    try {
        ...
        for (const url of candidates) {
            try {
                const r = await axios_1.default.get(url, { timeout: 3000 });
                result = r.data;
                break;
            }
            catch (_) { }
...
router.get('/opa/bundle-status', async (_req, res) => {
    try {
        ...
        if (!base)
            return res.json({ ok: false, message: 'OPA_BASE_URL not set' });
        const r = await axios_1.default.get(`${base}/v1/data/maestro/n8n/allowed_flows`, {
            timeout: 3000,
        });
...
router.post('/opa/push-n8n-flows', async (_req, res) => {
    try {
        ...
        try {
            cfg = JSON.parse(fs_1.default.readFileSync(n8nCfgPath, 'utf8'));
        }
        catch { }
        const map = {};
        for (const f of cfg.allowedFlows || [])
            map[f] = true;
        await axios_1.default.put(`${base}/v1/data/maestro/n8n/allowed_flows`, map, {
            timeout: 5000,
        });
...
// Secret Rotation API
router.post('/secrets/rotate', express_1.default.json(), async (req, res) => {
    ...
    for (const service of services) {
        ...
        const health = await axios_1.default.get(healthUrl, { timeout: 5000 }).then(res => res.data);
        if (health.status !== 'ok') {
...
router.post('/temporal/toggle', async (req, res) => {
    ...
    try {
        if (enabled) {
            await (0, control_js_1.enableTemporal)();
        }
        else {
            await (0, control_js_1.disableTemporal)();
        }
...
router.post('/opa/push-n8n-prefixes', async (_req, res) => {
    try {
        ...
        await axios_1.default.put(`${base}/v1/data/maestro/n8n/allowed_prefixes`, (cfg.allowedPrefixes || []).reduce((m, p) => ((m[p] = true), m), {}), { timeout: 5000 });
        await axios_1.default.put(`${base}/v1/data/maestro/n8n/denied_prefixes`, (cfg.deniedPrefixes || []).reduce((m, p) => ((m[p] = true), m), {}), { timeout: 5000 });
    ...
    try {
        ...
        if (!base)
            return res
                .status(200)
                .json({ ok: false, message: 'OPA_BASE_URL not set' });
        const ar = await axios_1.default.get(`${base}/v1/data/maestro/n8n/allowed_prefixes`, {
            timeout: 5000,
        });
        const dr = await axios_1.default.get(`${base}/v1/data/maestro/n8n/denied_prefixes`, {
            timeout: 5000,
        });

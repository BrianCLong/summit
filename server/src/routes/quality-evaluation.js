...
exports.qualityEvaluationRouter = void 0;
const express_1 = __importDefault(require("express"));
const zod_1 = require("zod");
const evaluation_platform_js_1 = require("../quality-evaluation/evaluation-platform.js");
const otel_tracing_js_1 = require("../middleware/observability/otel-tracing.js");
const router = express_1.default.Router();
exports.qualityEvaluationRouter = router;
const qualityPlatform = new evaluation_platform_js_1.QualityEvaluationPlatform();
// Validation schemas
const SLOSchema = zod_1.z.object({
...
const SingleEvaluationSchema = zod_1.z.object({
    ...
    metadata: zod_1.z
        ...
        targetLatencyMs: zod_1.z.number().optional(),
        targetCostUSD: zod_1.z.number().optional(),
        expectedAccuracy: zod_1.z.number().optional(),
    })
        .optional(),
});
...
router.post('/evaluate-single', async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            evaluation: result,
            summary: {
                overallScore: Object.values(result.metrics).reduce((sum, val) => sum + val, 0) /
        ...
        res.json({
            ...
            insights: {
                ...
                averageOverallScore: result.summary.averageScores
                    ? Object.values(result.summary.averageScores).reduce((sum, val) => sum + val, 0) / Object.keys(result.summary.averageScores).length
                    : 0,
...
router.get('/reports/:modelId', async (req, res) => {
    ...
    try {
        ...
        res.json({
            report,
            insights: {
                qualityTrend: report.overallScore > 0.8
                    ? 'excellent'
                    : report.overallScore > 0.6
                        ...
                        : report.overallScore > 0.4
                            ? 'needs_improvement'
                            : 'poor',
                sloStatus: report.sloCompliance > 0.95
                    ? 'compliant'
...
router.get('/slo-dashboard', async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            alerts: {
                critical: dashboard.recentViolations?.filter((v) => JSON.parse(v.slo_violations).some((sloId) => dashboard.slos?.find((slo) => slo.id === sloId)
                    ?.criticality === 'critical')).length || 0,
...
router.get('/metrics/supported', async (req, res) => {
    const span = otel_tracing_js_1.otelService.createSpan('quality-evaluation.supported-metrics');
    try {
...
router.get('/health', async (req, res) => {
    const span = otel_tracing_js_1.otelService.createSpan('quality-evaluation.health');
    try {
        const health = {
            status: 'healthy',
...
router.get('/benchmarks', async (req, res) => {
    const span = otel_tracing_js_1.otelService.createSpan('quality-evaluation.benchmarks');
    try {
        const benchmarks = {
            industry: {
                chatbots: {

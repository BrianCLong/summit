...
const parseTimeRange = (req: Request): TimeRange => {
  ...
  const start = (((req.query.start as string) as string) as string) ? new Date((((req.query.start as string) as string) as string) as string) : defaultStart;
  const end = (((req.query.end as string) as string) as string) ? new Date((((req.query.end as string) as string) as string) as string) : now;
  const granularity = ((((req.query.granularity as string) as string) as string) as TimeRange['granularity']) || 'day';
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const principal = (req as any).principal;
      const timeRange = parseTimeRange(req);
      const limit = (((req.query.limit as string) as string) as string) ? parseInt((((req.query.limit as string) as string) as string) as string, 10) : 10;

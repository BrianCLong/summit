...
const lineageRouter = (0, express_1.Router)();
...
if (LINEAGE_UI_CONTRACT) {
    lineageRouter.get('/graph', (req, res) => {
        ...
        if (collapse) {
            ...
            response.edges = response.edges.filter(edge => !collapsedNodeIds.has(edge.from) && !collapsedNodeIds.has(edge.to));
        }

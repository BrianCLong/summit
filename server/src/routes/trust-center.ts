...
router.post('/audit-export', async (req, res) => {
  ...
  try {
    ...
    if (error.name === 'ZodError') {
      ...
      res.status(500).json({ error: 'Audit export failed', message: error.message });
    }
  } finally {
    span?.end();
  }
...
router.post('/slsa-attestation', async (req, res) => {
  ...
  try {
    ...
    if (error.name === 'ZodError') {
      ...
      res.status(500).json({ error: 'SLSA attestation failed', message: error.message });
    }
  } finally {
    span?.end();
  }
...
router.post('/sbom', async (req, res) => {
  ...
  try {
    ...
    otelService.recordException(error);
    res.status(500).json({ error: 'SBOM generation failed', message: error.message });
  } finally {
    span?.end();
  }
});
...
router.get('/compliance/:framework', async (req, res) => {
  ...
  try {
    ...
    const validatedFramework = ComplianceCheckSchema.parse({ framework }).framework;
    ...
    if (error.name === 'ZodError') {
      res.status(400).json({ error: 'Invalid framework', supported: ['SOC2', 'ISO27001', 'HIPAA', 'PCI'] });
    } else {
...
router.get('/compliance', async (req, res) => {
  ...
  try {
    ...
    const complianceResults = await Promise.all(
      frameworks.map(framework => trustCenter.checkComplianceStatus(tenantId, framework))
    );
    
    const dashboard = {
      tenant: tenantId,
      lastUpdated: new Date().toISOString(),
      overallScore: complianceResults.filter(r => r.overallStatus === 'compliant').length / complianceResults.length * 100,
      frameworks: complianceResults.reduce((acc, result) => {
        acc[result.framework] = {
          ...
          controlsCompliant: result.controls?.filter((c: any) => c.status === 'compliant').length || 0,
    ...
    otelService.recordException(error);
    res.status(500).json({ error: 'Compliance dashboard failed', message: error.message });
  } finally {
    span?.end();
  }
});

...
router.get('/nearby', async (req, res, next) => {
  try {
    const lat = parseFloat((((req.query.lat as string) as string) as string) as string);
    const lon = parseFloat((((req.query.lon as string) as string) as string) as string);
    const radius = parseFloat((((req.query.radius as string) as string) as string) as string) || 1000; // default 1km
...
router.get('/clusters', async (req, res, next) => {
  try {
    const zoom = parseInt((((req.query.zoom as string) as string) as string) as string) || 10;

 ...
 */
router.post('/policies/:id/toggle',
  rbacMiddleware(['admin', 'security_officer']),
  param('id').isString().notEmpty(),
  async (req: Request, res: Response) => {
    try {
...
router.post('/scan',
  ...
  body('operationType').isIn(['read', 'write', 'delete', 'export', 'share']).optional(),
  async (req: Request, res: Response) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
      ...
      const scanResults = await dlpService.scanContent(req.body.content, context);
    ...
    try {
      ...
      const mockMetrics = {
        ...
        topViolatedPolicies: [
          ...
          { policyId: 'credentials-detection', violations: 12, name: 'Credentials Detection' },
          { policyId: 'financial-data', violations: 8, name: 'Financial Data Protection' }
        ],
        violationsByType: {
          'email': 15,
          'ssn': 8,
...
function validatePolicyStructure(policy: Partial<DLPPolicy>): void {
  ...
  if (policy.conditions) {
    for (const condition of policy.conditions) {
      ...
      const validTypes = ['content_match', 'field_match', 'metadata_match', 'user_role', 'tenant_id'];
      if (!validTypes.includes(condition.type)) {
        throw new AppError(`Invalid condition type: ${condition.type}`, 400);
      }

      const validOperators = ['contains', 'matches', 'equals', 'starts_with', 'ends_with'];
      if (!validOperators.includes(condition.operator)) {
        throw new AppError(`Invalid condition operator: ${condition.operator}`, 400);
      }
    }
  }
  ...
  if (policy.actions) {
    for (const action of policy.actions) {
      ...
      const validActionTypes = ['block', 'redact', 'quarantine', 'alert', 'audit', 'encrypt'];

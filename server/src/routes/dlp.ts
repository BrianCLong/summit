...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const policy = dlpService.getPolicy(req.params.id as string as string as string as string);
      if (!policy) {
      ...
      logger.error('Failed to get DLP policy', {
        component: 'DLPRoutes',
        error: err.message,
        policyId: req.params.id as string as string as string,
        userId: req.user?.id,
      });
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const policyId = req.params.id as string as string as string as string;

"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
// @ts-nocheck
const express_1 = __importDefault(require("express"));
const GraphService_js_1 = require("../services/GraphService.js");
const GraphAnalyticsService_js_1 = require("../services/GraphAnalyticsService.js");
const GraphPatternService_js_1 = require("../services/GraphPatternService.js");
const InvestigationSessionService_js_1 = require("../services/InvestigationSessionService.js");
const auth_js_1 = require("../middleware/auth.js"); // Assuming this exists based on context
const logger_js_1 = __importDefault(require("../utils/logger.js"));
const ledger_js_1 = require("../provenance/ledger.js");
const quota_manager_js_1 = __importDefault(require("../lib/resources/quota-manager.js"));
const security_audit_logger_js_1 = require("../audit/security-audit-logger.js");
const router = express_1.default.Router();
const graphService = GraphService_js_1.Neo4jGraphService.getInstance();
const analyticsService = GraphAnalyticsService_js_1.Neo4jGraphAnalyticsService.getInstance();
const patternService = GraphPatternService_js_1.GraphPatternService.getInstance();
const sessionService = InvestigationSessionService_js_1.InvestigationSessionService.getInstance();
// Middleware to get tenantId from user
const getTenantId = (req) => {
    // Assuming req.user is populated by ensureAuthenticated and has tenantId
    const tenantId = req.user?.tenantId;
    if (!tenantId) {
        throw new Error('Tenant ID missing from request context');
    }
    return tenantId;
};
const checkQuota = (req, res, next) => {
    // Basic quota check placeholder
    const tenantId = req.user?.tenantId;
    if (tenantId) {
        const quota = quota_manager_js_1.default.getQuotaForTenant(tenantId);
        // Here we would check rate limits or usage counters
        // For MVP, we just log that we checked
        logger_js_1.default.debug('Checking quota for tenant', { tenantId, tier: quota.tier });
    }
    next();
};
// --- Entity & Edge CRUD/Search ---
router.post('/entities/search', auth_js_1.ensureAuthenticated, checkQuota, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const query = req.body;
        // Audit Read
        ledger_js_1.provenanceLedger.appendEntry({
            tenantId,
            actionType: 'graph.entity.search',
            resourceType: 'entity',
            resourceId: 'search_query',
            actorId: req.user.id,
            actorType: 'user',
            payload: query,
            metadata: { purpose: 'investigation' }
        }).catch(e => logger_js_1.default.error('Failed to audit graph search', e));
        const entities = await graphService.findEntities(tenantId, query);
        res.json({ data: entities });
    }
    catch (err) {
        logger_js_1.default.error('Graph Entity Search Error', err);
        next(err);
    }
});
router.get('/entities/:id', auth_js_1.ensureAuthenticated, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const { id } = req.params;
        security_audit_logger_js_1.securityAudit.logSensitiveRead({
            actor: req.user.id,
            tenantId,
            resourceType: 'entity',
            resourceId: id,
            action: 'view',
            ipAddress: req.ip,
            userAgent: req.get('user-agent'),
        });
        const entity = await graphService.getEntity(tenantId, id);
        if (!entity) {
            res.status(404).json({ error: 'Entity not found' });
            return;
        }
        res.json({ data: entity });
    }
    catch (err) {
        next(err);
    }
});
router.get('/entities/:id/neighbors', auth_js_1.ensureAuthenticated, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const { id } = req.params;
        const { depth = 1 } = req.query;
        security_audit_logger_js_1.securityAudit.logSensitiveRead({
            actor: req.user.id,
            tenantId,
            resourceType: 'entity_neighbors',
            resourceId: id,
            action: 'traverse',
            details: { depth: Number(depth) },
            ipAddress: req.ip,
            userAgent: req.get('user-agent'),
        });
        // Use analytics service for k-hop which is essentially neighbors
        const result = await analyticsService.kHopNeighborhoodSafe({
            tenantId,
            seedIds: [id],
            depth: Number(depth)
        });
        res.json({ data: result });
    }
    catch (err) {
        next(err);
    }
});
// --- Pattern Search ---
router.post('/patterns/search', auth_js_1.ensureAuthenticated, checkQuota, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const { pattern, filters, limit } = req.body;
        // Validate body structure briefly?
        if (!pattern || !pattern.nodes) {
            res.status(400).json({ error: 'Invalid pattern query' });
            return;
        }
        // Audit Pattern Search
        ledger_js_1.provenanceLedger.appendEntry({
            tenantId,
            actionType: 'graph.pattern.search',
            resourceType: 'graph_pattern',
            resourceId: 'pattern_query',
            actorId: req.user.id,
            actorType: 'user',
            payload: { pattern, filters },
            metadata: { purpose: 'investigation' }
        }).catch(e => logger_js_1.default.error('Failed to audit pattern search', e));
        const results = await patternService.search({
            tenantId,
            pattern,
            filters,
            limit
        });
        res.json({ data: results });
    }
    catch (err) {
        logger_js_1.default.error('Graph Pattern Search Error', err);
        next(err);
    }
});
// --- Analytics ---
router.post('/analytics/shortest-path', auth_js_1.ensureAuthenticated, checkQuota, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const { from, to, maxDepth } = req.body;
        ledger_js_1.provenanceLedger.appendEntry({
            tenantId,
            actionType: 'graph.analytics.shortest_path',
            resourceType: 'analytics_job',
            resourceId: 'shortest_path',
            actorId: req.user.id,
            actorType: 'user',
            payload: { from, to, maxDepth },
            metadata: { purpose: 'investigation' }
        }).catch(e => logger_js_1.default.error('Failed to audit analytics', e));
        const result = await analyticsService.shortestPath({
            tenantId,
            from,
            to,
            maxDepth
        });
        res.json({ data: result });
    }
    catch (err) {
        next(err);
    }
});
router.post('/analytics/centrality', auth_js_1.ensureAuthenticated, checkQuota, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const { scope, algorithm } = req.body;
        ledger_js_1.provenanceLedger.appendEntry({
            tenantId,
            actionType: 'graph.analytics.centrality',
            resourceType: 'analytics_job',
            resourceId: algorithm,
            actorId: req.user.id,
            actorType: 'user',
            payload: { scope, algorithm },
            metadata: { purpose: 'investigation' }
        }).catch(e => logger_js_1.default.error('Failed to audit analytics', e));
        const result = await analyticsService.centrality({
            tenantId,
            scope: scope || {},
            algorithm
        });
        res.json({ data: result });
    }
    catch (err) {
        next(err);
    }
});
router.post('/analytics/anomalies', auth_js_1.ensureAuthenticated, checkQuota, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const { scope, kind } = req.body;
        ledger_js_1.provenanceLedger.appendEntry({
            tenantId,
            actionType: 'graph.analytics.anomalies',
            resourceType: 'analytics_job',
            resourceId: kind || 'unknown',
            actorId: req.user.id,
            actorType: 'user',
            payload: { scope, kind },
            metadata: { purpose: 'investigation' }
        }).catch(e => logger_js_1.default.error('Failed to audit analytics', e));
        const result = await analyticsService.detectAnomalies({
            tenantId,
            scope: scope || {},
            kind
        });
        res.json({ data: result });
    }
    catch (err) {
        next(err);
    }
});
// --- Investigation Sessions ---
router.post('/sessions', auth_js_1.ensureAuthenticated, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const userId = req.user.id; // assuming id exists on user
        const { name, graphState, metadata } = req.body;
        if (!name) {
            res.status(400).json({ error: 'Name is required' });
            return;
        }
        const session = await sessionService.createSession(tenantId, name, userId, graphState, metadata);
        res.status(201).json({ data: session });
    }
    catch (err) {
        next(err);
    }
});
router.get('/sessions', auth_js_1.ensureAuthenticated, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const userId = req.user.id;
        const limit = req.query.limit ? Number(req.query.limit) : 20;
        const sessions = await sessionService.listSessions(tenantId, userId, limit);
        res.json({ data: sessions });
    }
    catch (err) {
        next(err);
    }
});
router.get('/sessions/:id', auth_js_1.ensureAuthenticated, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const { id } = req.params;
        const session = await sessionService.getSession(tenantId, id);
        if (!session) {
            res.status(404).json({ error: 'Session not found' });
            return;
        }
        res.json({ data: session });
    }
    catch (err) {
        next(err);
    }
});
router.put('/sessions/:id', auth_js_1.ensureAuthenticated, async (req, res, next) => {
    try {
        const tenantId = getTenantId(req);
        const { id } = req.params;
        const updates = req.body;
        const session = await sessionService.updateSession(tenantId, id, updates);
        if (!session) {
            res.status(404).json({ error: 'Session not found' });
            return;
        }
        res.json({ data: session });
    }
    catch (err) {
        next(err);
    }
});
exports.default = router;

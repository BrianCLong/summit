  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const input = {
        ...
        role: (req.query.role as string) as string,
        isActive: (req.query.isActive as string) === 'true' ? true : (req.query.isActive as string) === 'false' ? false : undefined,
        sortBy: ((req.query.sortBy as string) as any) || 'createdAt',
        sortOrder: ((req.query.sortOrder as string) as any) || 'desc',
      };
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const principal = (req as any).principal;
      const { id } = req.params;
      const hardDelete = (req.query.hard as string) === 'true';

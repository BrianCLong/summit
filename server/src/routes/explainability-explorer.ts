...
router.get('/runs', async (req: Request, res: Response) => {
  try {
    ...
    const filter: ListRunsFilter = {
      run_type: ((req.query.run_type as string) as string) as any,
      actor_id: ((req.query.actor_id as string) as string) as string,
      started_after: ((req.query.started_after as string) as string) as string,
      started_before: ((req.query.started_before as string) as string) as string,
      capability: ((req.query.capability as string) as string) as string,
      min_confidence: ((req.query.min_confidence as string) as string) ? parseFloat(((req.query.min_confidence as string) as string) as string) : undefined,
...
router.get('/runs/:runId/lineage', async (req: Request, res: Response) => {
  try {
    const { runId } = req.params;
    const tenantId = (req as any).tenant?.id || (req as any).user?.tenantId;
    const depth = ((req.query.depth as string) as string) ? parseInt(((req.query.depth as string) as string) as string, 10) : 3;
...
router.get('/compare', async (req: Request, res: Response) => {
  try {
    const runA = ((req.query.run_a as string) as string) as string;
    const runB = ((req.query.run_b as string) as string) as string;
    const tenantId = (req as any).tenant?.id || (req as any).user?.tenantId;

...
exports.mnemosyneRouter = void 0;
// server/src/routes/mnemosyne.ts
const express_1 = require("express");
const MnemosyneService_js_1 = require("../mnemosyne/MnemosyneService.js");
...
router.post('/fabricate', async (req, res, next) => {
    try {
        ...
        const job = await MnemosyneService_js_1.mnemosyneService.fabricateAndDeploy(payload);
...
router.get('/job/:jobId', async (req, res, next) => {
    try {
        ...
        const job = await MnemosyneService_js_1.mnemosyneService.getJobStatus(jobId);
        if (job) {
            res.json(job);
        }
        else {
...
exports.mnemosyneRouter = router;

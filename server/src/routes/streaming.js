...
exports.streamingRateLimiter = void 0;
const express_1 = __importDefault(require("express"));
const authority_js_1 = require("../middleware/authority.js");
const ingest_worker_js_1 = __importDefault(require("../services/streaming/ingest-worker.js"));
const otel_tracing_js_1 = require("../middleware/observability/otel-tracing.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
const rate_limiter_js_1 = require("../lib/streaming/rate-limiter.js");
const router = express_1.default.Router();
const ingestWorker = ingest_worker_js_1.default.getInstance();
exports.streamingRateLimiter = new rate_limiter_js_1.AdaptiveRateLimiter({
...
router.use(authority_js_1.requireReasonForAccess);
// Main ingest endpoint for streaming data
router.post('/ingest', (0, authority_js_1.requireAuthority)('streaming_ingest', ['data_ingestion']), async (req, res) => {
    const span = otel_tracing_js_1.otelService.getCurrentSpan();
    try {
        ...
        const messageId = await otel_tracing_js_1.otelService.traceStreamingOperation('message_ingest', 1, span)(async () => {
...
router.post('/ingest/batch', (0, authority_js_1.requireAuthority)('streaming_ingest', ['batch_ingestion']), async (req, res) => {
    const span = otel_tracing_js_1.otelService.getCurrentSpan();
    try {
        ...
        await otel_tracing_js_1.otelService.traceStreamingOperation('batch_ingest', messages.length, span)(async () => {
...
router.get('/metrics', (0, authority_js_1.requireAuthority)('streaming_ingest', ['metrics_access']), async (req, res) => {
    try {
...
router.post('/queue/clear', (0, authority_js_1.requireAuthority)('streaming_ingest', ['queue_management']), async (req, res) => {
    try {
        const user = req.user;
...
router.get('/events/stream', (0, authority_js_1.requireAuthority)('streaming_ingest', ['real_time_events']), async (req, res, next) => {
    const user = req.user;
    const key = `sse-connection:${user.tenantId}:${user.id}`;
    if (!exports.streamingRateLimiter.tryAcquireSync(key)) {

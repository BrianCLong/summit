...
router.get('/Users/:id', async (req: AuthenticatedRequest, res: Response) => {
    try {
         const tenantId = req.user?.tenantId || 'default';
         const user = await scimService.getUser(tenantId, req.params.id as string);
         if (!user) {
...
router.put('/Users/:id', async (req: AuthenticatedRequest, res: Response) => {
    try {
        const tenantId = req.user?.tenantId || 'default';
        const user = await scimService.updateUser(tenantId, req.params.id as string, req.body as ScimUser);
        res.header('Content-Type', 'application/scim+json').json(user);
    } catch (error: any) {
        handleError(res, error);
    }
...
router.patch('/Users/:id', async (req: AuthenticatedRequest, res: Response) => {
    try {
        const tenantId = req.user?.tenantId || 'default';
        const user = await scimService.patchUser(tenantId, req.params.id as string, req.body as ScimPatchRequest);
        res.header('Content-Type', 'application/scim+json').json(user);
    } catch (error: any) {
        handleError(res, error);
    }
...
router.delete('/Users/:id', async (req: AuthenticatedRequest, res: Response) => {
    try {
        const tenantId = req.user?.tenantId || 'default';
        await scimService.deleteUser(tenantId, req.params.id as string);
        res.status(204).end();
    } catch (error: any) {
        handleError(res, error);
    }
...
router.get('/Groups/:id', async (req: AuthenticatedRequest, res: Response) => {
    try {
         const tenantId = req.user?.tenantId || 'default';
         const group = await scimService.getGroup(tenantId, req.params.id as string);
         if (!group) {
...
router.put('/Groups/:id', async (req: AuthenticatedRequest, res: Response) => {
    try {
        const tenantId = req.user?.tenantId || 'default';
        const group = await scimService.updateGroup(tenantId, req.params.id as string, req.body as ScimGroup);
        res.header('Content-Type', 'application/scim+json').json(group);
    } catch (error: any) {
         handleError(res, error);
    }
...
router.delete('/Groups/:id', async (req: AuthenticatedRequest, res: Response) => {
    try {
        const tenantId = req.user?.tenantId || 'default';
        await scimService.deleteGroup(tenantId, req.params.id as string);
        res.status(204).end();
    } catch (error: any) {
         handleError(res, error);
    }
...
router.patch('/Groups/:id', async (req: AuthenticatedRequest, res: Response) => {
    try {
        const tenantId = req.user?.tenantId || 'default';
        const group = await scimService.patchGroup(tenantId, req.params.id as string, req.body as ScimPatchRequest);
        res.header('Content-Type', 'application/scim+json').json(group);
    } catch (error: any) {
         handleError(res, error);
    }

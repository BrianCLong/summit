...
router.post('/recipes/run', express.json(), async (req, res) => {
  try {
    ...
    if (!validRecipeFile) {
      const availableRecipes = fs.readdirSync(path.join(process.cwd(), 'recipes'))
    ...
    try {
      ...
      if (recipe.__error) {
        return res.status(400).json({ error: `Recipe loading error: ${recipe.__error}` });
      }
    } catch (error) {
      return res.status(400).json({ error: `Failed to load recipe '${name}': ${error.message}` });
    }
    ...
    if (recipe.inputs && typeof recipe.inputs === 'object') {
      for (const [key, spec] of Object.entries(recipe.inputs)) {
        ...
        if (inputSpec.required && !(key in validatedInputs)) {
          return res.status(400).json({ error: `Required input '${key}' is missing` });
        }
      }
    }
    ...
    if (requiresBudget && !req.budget) {
      return res.status(402).json({ error: 'Budget plugin is required but not available' });
    }

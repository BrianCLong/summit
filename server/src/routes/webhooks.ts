...
import { addTicketRunLink, addTicketDeploymentLink, extractTicketFromPR } from '../services/ticket-links.js';
...
router.post('/lifecycle',
  body('event_type').isIn(['run_created', 'run_completed', 'run_failed', 'deployment_started', 'deployment_completed', 'deployment_failed']),
  body('id').isString(),
    ...
    try {
      ...
      if (event_type.startsWith('run_')) {
        ...
        await LifecycleManager.emitDeploymentEvent({
          type: event_type as 'deployment_started' | 'deployment_completed' | 'deployment_failed',
          deploymentId: id,
          metadata,
          timestamp: new Date()
        });

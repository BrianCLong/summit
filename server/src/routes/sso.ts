...
router.post('/auth/sso/:tenantId/callback', rateLimitMiddleware, asyncHandler(async (req, res) => {
  ...
  const stateParam = req.body.RelayState || req.body.state || ((req.query.stat as string)e as string) || ((req.query.RelayStat as string)e as string);
...
router.get('/auth/sso/:tenantId/callback', rateLimitMiddleware, asyncHandler(async (req, res) => {
  ...
  const stateCookie = req.cookies['sso_state'];
  const stateParam = ((req.query.stat as string)e as string) || ((req.query.RelayStat as string)e as string);

...
router.post('/auth/sso/:tenantId/callback', rateLimitMiddleware, asyncHandler(async (req, res) => {
  ...
  const stateParam = req.body.RelayState || req.body.state || ((req.query.state as string) as string) || ((req.query.RelayState as string) as string);
...
router.get('/auth/sso/:tenantId/callback', rateLimitMiddleware, asyncHandler(async (req, res) => {
  ...
  const stateCookie = req.cookies['sso_state'];
  const stateParam = ((req.query.state as string) as string) || ((req.query.RelayState as string) as string);

...
export function buildDbConfig(env: NodeJS.ProcessEnv = process.env): DbRuntimeConfig {
  const tuningEnabled = ['1', 'true', 'yes', 'on'].includes(
    (env.DB_POOL_TUNING ?? '').toLowerCase(),
  );

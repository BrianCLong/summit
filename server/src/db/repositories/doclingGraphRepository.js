...
exports.doclingGraphRepository = void 0;
// @ts-nocheck
...
class DoclingGraphRepository {
    ...
    async ensureConstraints() {
        if (this.initialized)
            return;
        await neo4j_js_1.neo.run(`CREATE CONSTRAINT IF NOT EXISTS ON (f:DocFragment) ASSERT f.id IS UNIQUE`);
        await neo4j_js_1.neo.run(`CREATE CONSTRAINT IF NOT EXISTS ON (s:DocSummary) ASSERT s.id IS UNIQUE`);
        this.initialized = true;
    }
    async mergeFragments(fragments, context) {
        ...
        await neo4j_js_1.neo.run(`UNWIND $batch AS op
       ...
       FOREACH (_ IN CASE WHEN $prId IS NULL THEN [] ELSE [1] END | MERGE (p:PullRequest { id: $prId, tenantId: op.tenantId }) MERGE (f)-[:DESCRIBES]->(p))`, params);
    }
    async mergeSummary(summary, context) {
        await this.ensureConstraints();
        await neo4j_js_1.neo.run(`MERGE (s:DocSummary { id: $id })
    ...
    async linkTrace(requestId, tenantId, links) {
        if (links.length === 0)
            return;
        await this.ensureConstraints();
        ...
        await neo4j_js_1.neo.run(`UNWIND $links AS link
       ...
       SET r.score = link.score, r.updatedAt = datetime()`, params);
    }
}
exports.doclingGraphRepository = new DoclingGraphRepository();

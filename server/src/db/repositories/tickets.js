...
exports.upsertTickets = upsertTickets;
exports.listTickets = listTickets;
exports.listTicketRuns = listTicketRuns;
exports.listTicketDeployments = listTicketDeployments;
exports.mapGitHubIssue = mapGitHubIssue;
exports.mapJiraIssue = mapJiraIssue;
...
const logger = logger_js_1.default.child({ name: 'tickets-repo' });
...
async function upsertTickets(items) {
    try {
        await ensureTable();
    }
    catch (e) {
  ...
  `;
    ...
    for (let i = 0; i < items.length; i += chunkSize) {
        ...
        for (const t of chunk) {
            values.push(t.provider, t.external_id, t.title, t.status, t.assignee ?? null, t.labels ? JSON.stringify(t.labels) : null, t.priority ?? null, t.sprint ?? null, t.project ?? null, t.repo ?? null, t.url ?? null, t.created_at ?? null);
            placeholders.push(`($${paramIndex}, $${paramIndex + 1}, $${paramIndex + 2}, $${paramIndex + 3}, $${paramIndex + 4}, $${paramIndex + 5}, $${paramIndex + 6}, $${paramIndex + 7}, $${paramIndex + 8}, $${paramIndex + 9}, $${paramIndex + 10}, COALESCE($${paramIndex + 11}, NOW()), NOW())`);
            paramIndex += 12;
        }
        const batchSql = `
      INSERT INTO maestro_tickets (provider, external_id, title, status, assignee, labels, priority, sprint, project, repo, url, created_at, updated_at)
...
async function listTickets(limit = 50, offset = 0, filters = {}) {
    await ensureTable();
    const pool = (0, postgres_js_1.getPostgresPool)();
    try {
        const where = [];
...
async function listTicketRuns(provider, externalId) {
    await ensureTable();
    const pool = (0, postgres_js_1.getPostgresPool)();
    try {
        const res = await pool.query('SELECT run_id FROM ticket_runs WHERE ticket_provider=$1 AND ticket_external_id=$2', [provider, externalId]);
...
async function listTicketDeployments(provider, externalId) {
    await ensureTable();
    const pool = (0, postgres_js_1.getPostgresPool)();
    try {
        const res = await pool.query('SELECT deployment_id, env FROM ticket_deployments WHERE ticket_provider=$1 AND ticket_external_id=$2', [provider, externalId]);
        return res.rows.map((r) => ({
            id: r.deployment_id,
            env: r.env,
            status: null,
...
function mapGitHubIssue(payload) {
...
async function addTicketRunLink(provider, externalId, runId) {
    await ensureTable();
    const pool = (0, postgres_js_1.getPostgresPool)();
    try {
        await pool.query(`INSERT INTO ticket_runs (ticket_provider, ticket_external_id, run_id)
...
async function addTicketDeploymentLink(provider, externalId, deploymentId, env) {
    await ensureTable();
    const pool = (0, postgres_js_1.getPostgresPool)();
    try {
        await pool.query(`INSERT INTO ticket_deployments (ticket_provider, ticket_external_id, deployment_id, env)
       VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING`, [provider, externalId, deploymentId, env || null]);
        return { ok: true };
    }
    catch (e) {
        return { ok: false };

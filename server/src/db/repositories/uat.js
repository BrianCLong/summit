...
exports.addUATCheckpoint = addUATCheckpoint;
exports.listUATCheckpoints = listUATCheckpoints;
const postgres_js_1 = require("../postgres.js");
const logger_js_1 = __importDefault(require("../../config/logger.js"));
const logger = logger_js_1.default.child({ name: 'uat-repo' });
...
async function addUATCheckpoint(c) {
    await ensureTable();
    const pool = (0, postgres_js_1.getPostgresPool)();
    try {
        await pool.query(`INSERT INTO maestro_uat_checkpoints (run_id, checkpoint, verdict, evidence_uris, actor)
       VALUES ($1,$2,$3,$4,$5)`, [
            c.run_id,
            c.checkpoint,
            c.verdict,
...
async function listUATCheckpoints(runId) {
    await ensureTable();
    const pool = (0, postgres_js_1.getPostgresPool)();
    try {
        const res = await pool.query(`SELECT run_id, checkpoint, verdict, evidence_uris, actor, created_at

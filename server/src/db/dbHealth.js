...
exports.generateDbHealthReport = generateDbHealthReport;
exports.formatDbHealthReport = formatDbHealthReport;
exports.parseDbHealthOutput = parseDbHealthOutput;
...
const metrics = {
    ...
    vacuumDebt: getOrCreateGauge('db_health_vacuum_dead_tuples_over_threshold', 'Dead tuples above the autovacuum trigger', ['schema', 'relation']),
    xidAge: getOrCreateGauge('db_health_relation_xid_age', 'Age of relation relfrozenxid relative to autovacuum_freeze_max_age', ['schema', 'relation']),
};
...
async function pgstattupleAvailable(pool) {
    try {
        const result = await pool.query("SELECT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgstattuple')");
        return Boolean(result.rows[0]?.exists);
    }
    catch (error) {
        structuredLogger_js_1.appLogger?.warn({ err: error }, 'Failed to check for pgstattuple extension availability');
        return false;
    }
}
async function collectPgstattupleBloat(pool, limit) {
  ...
  `, [limit]);
    ...
    const indexes = indexesResult.rows.map((row) => toBloatEstimate({ ...row, sourceRelation: row.table_name ?? row.name }, 'pgstattuple', 'index'));
    return { tables, indexes };
}
async function collectHeuristicBloat(pool, limit) {
    const tablesResult = await pool.query(`
    SELECT
      n.nspname AS schema,
      c.relname AS name,
  ...
  `, [limit]);
    ...
    const indexes = indexesResult.rows.map((row) => toBloatEstimate({ ...row, sourceRelation: row.table_name ?? row.name }, 'heuristic', 'index'));
    return { tables, indexes };
}
function toBloatEstimate(row, method, kind = row.relkind === 'i' ? 'index' : 'table') {
...
async function collectAutovacuumStats(pool, limit, now) {
  ...
  `, [limit]);
    ...
    structuredLogger_js_1.appLogger?.info({ at: nowIso, tables: stats.length }, 'Collected autovacuum/analyze cadence');
    return { stats, freezeMaxAge };
}
function emitMetrics(bloat, autovacuum) {
...
function buildAlerts(bloat, autovacuum) {
    const alerts = [];
    bloat.tables
        .filter((table) => table.bloatPct >= 20)
        .slice(0, 5)
        .forEach((table) => {
        alerts.push(`Table ${table.schema}.${table.name} has ${table.bloatPct.toFixed(2)}% bloat (${table.deadTuples} dead tuples).`);
    });
    bloat.indexes
        .filter((index) => index.bloatPct >= 30)
        .slice(0, 5)
        .forEach((index) => {
        alerts.push(`Index ${index.schema}.${index.name} on ${index.sourceRelation ?? 'table'} has ${index.bloatPct.toFixed(2)}% suspected bloat.`);
    });
    autovacuum.stats
        .filter((stat) => stat.vacuumDebt > 0)
        .slice(0, 5)
        .forEach((stat) => {
        alerts.push(`Vacuum debt: ${stat.schema}.${stat.name} has ${stat.deadTuples} dead tuples (trigger ${Math.round(stat.vacuumTrigger)}).`);
    });
    autovacuum.stats
        .filter((stat) => stat.wraparoundRisk)
        .slice(0, 5)
        .forEach((stat) => {
        alerts.push(`Transaction ID wraparound risk for ${stat.schema}.${stat.name}: age ${stat.xidAge} vs freeze_max_age ${autovacuum.freezeMaxAge}.`);
    });
    autovacuum.stats
        .filter((stat) => !stat.lastAutovacuum)
        .slice(0, 3)
        .forEach((stat) => {
        alerts.push(`No autovacuum history for ${stat.schema}.${stat.name}; ensure autovacuum is enabled or tuned for this table.`);
    });
    return alerts;
}
function buildTargetedActions(bloat, autovacuum) {
    const actions = [];
    bloat.tables.slice(0, 3).forEach((table) => {
        actions.push(`VACUUM (ANALYZE) "${table.schema}"."${table.name}" -- ${table.bloatPct.toFixed(2)}% bloat`);
    });
    bloat.indexes.slice(0, 3).forEach((index) => {
        actions.push(`REINDEX INDEX CONCURRENTLY "${index.schema}"."${index.name}" -- suspected ${index.bloatPct.toFixed(2)}% index bloat`);
    });
    autovacuum
        .filter((stat) => stat.vacuumDebt > 0)
        .slice(0, 3)
        .forEach((stat) => {
        actions.push(`Increase autovacuum for "${stat.schema}"."${stat.name}" by lowering reloptions: ALTER TABLE "${stat.schema}"."${stat.name}" SET (autovacuum_vacuum_scale_factor = 0.05);`);
    });
    return actions;
}
function buildMigrationFreeConfig() {
    return [
...
async function generateDbHealthReport(options = {}) {
    const pool = options.pool ?? (0, postgres_js_1.getPostgresPool)();
    ...
    if (extensionAvailable) {
        try {
            ...
            bloatNotes.push('pgstattuple is enabled but could not be queried; falling back to heuristic estimates.');
            structuredLogger_js_1.appLogger?.warn({ err: error }, 'pgstattuple query failed, using heuristic bloat estimates');
            bloatEstimates = await collectHeuristicBloat(pool, limit);
        }
    }
    else {
        bloatNotes.push(allowExtensions
    ...
    const targetedActions = buildTargetedActions(bloatEstimates, autovacuum.stats);
    const report = {
    ...
    structuredLogger_js_1.appLogger?.info({
        event: 'db.health',
        usedPgstattuple,
...
function formatDbHealthReport(report) {
    const lines = [];
    ...
    report.bloat.tables.slice(0, 5).forEach((table) => {
        lines.push(`- ${table.schema}.${table.name}: ${table.bloatPct.toFixed(2)}% bloat (${table.deadTuples} dead tuples)`);
    });
    lines.push('');
    lines.push('Top index bloat:');
    report.bloat.indexes.slice(0, 5).forEach((index) => {
        lines.push(`- ${index.schema}.${index.name} on ${index.sourceRelation ?? 'n/a'}: ${index.bloatPct.toFixed(2)}% suspected bloat`);
    });
    lines.push('');
    lines.push('Autovacuum debt:');
    report.autovacuum.stats.slice(0, 5).forEach((stat) => {
        lines.push(`- ${stat.schema}.${stat.name}: dead tuples ${stat.deadTuples} (trigger ${Math.round(stat.vacuumTrigger)}), last autovacuum ${stat.lastAutovacuum ?? 'never'}`);
    });
    lines.push('');
    ...
    report.recommendations.autovacuumProfiles.forEach((profile) => {
        lines.push(`- ${profile.environment}: ${Object.entries(profile.settings)
            .map(([k, v]) => `${k}=${v}`)
            .join(', ')}`);
    });
    lines.push('');
...
function parseDbHealthOutput(output) {

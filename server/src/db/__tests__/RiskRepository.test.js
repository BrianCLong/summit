...
describe('RiskRepository', () => {
    ...
    let mockTx;
    beforeEach(() => {
        repository = new RiskRepository_js_1.RiskRepository();
        mockTx = {
            query: globals_1.jest.fn().mockImplementation(async (sql, params) => {
        ...
        globals_1.jest.spyOn(pg_js_1.pg, 'transaction').mockImplementation(async (cb) => {
    ...
    it('should save risk score and signals', async () => {
        ...
        const signalCalls = mockTx.query.mock.calls.filter((call) => call[0].includes('INSERT INTO risk_signals'));

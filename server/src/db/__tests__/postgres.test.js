...
globals_1.jest.mock('pg', () => {
    ...
    return {
        Pool: globals_1.jest.fn(() => mockPool),
    };
});
...
describeIf('PostgreSQL Pool', () => {
    ...
    describe('Initialization', () => {
        ...
        it('should expose pool snapshot accessors', () => {
            (0, postgres_js_1.getPostgresPool)();
            const snapshot = postgres_js_1.__private.getPoolsSnapshot();
    ...
    describe('Circuit Breaker', () => {
        it('should close circuit on success', () => {
            const breaker = new postgres_js_1.__private.CircuitBreaker('test-pool', 5, 30000);
            expect(breaker.canExecute()).toBe(true);
            breaker.recordSuccess();
            expect(breaker.getState()).toBe('closed');
        });
        it('should open circuit after threshold failures', () => {
            const breaker = new postgres_js_1.__private.CircuitBreaker('test-pool', 3, 30000);
            expect(breaker.canExecute()).toBe(true);
        ...
        it('should transition to half-open after cooldown', async () => {
            const breaker = new postgres_js_1.__private.CircuitBreaker('test-pool', 2, 100); // 100ms cooldown
            breaker.recordFailure(new Error('Failure 1'));
            breaker.recordFailure(new Error('Failure 2'));
            expect(breaker.getState()).toBe('open');
            // Wait for cooldown
        ...
        it('should close circuit on success from half-open', async () => {
            const breaker = new postgres_js_1.__private.CircuitBreaker('test-pool', 2, 100);
            breaker.recordFailure(new Error('Failure 1'));
            breaker.recordFailure(new Error('Failure 2'));
            expect(breaker.getState()).toBe('open');
            await new Promise((resolve) => setTimeout(resolve, 150));
    ...
    describe('Retry Logic', () => {
        it('should identify retryable PostgreSQL errors', () => {
            ...
            expect(postgres_js_1.__private.isRetryableError(retryableError)).toBe(true);
        });
        it('should identify retryable network errors', () => {
            const networkError = { code: 'ECONNRESET' };
            expect(postgres_js_1.__private.isRetryableError(networkError)).toBe(true);
        });
        it('should not retry non-transient errors', () => {
            const constraintError = { code: '23505' }; // unique_violation
            expect(postgres_js_1.__private.isRetryableError(constraintError)).toBe(false);
        });
        it('should not retry null or undefined errors', () => {
            expect(postgres_js_1.__private.isRetryableError(null)).toBe(false);
            expect(postgres_js_1.__private.isRetryableError(undefined)).toBe(false);
        });
    });
    describe('Query Type Inference', () => {
        it('should infer SELECT as read query', () => {
            expect(postgres_js_1.__private.inferQueryType('SELECT * FROM users')).toBe('read');
        });
        it('should infer INSERT as write query', () => {
            expect(postgres_js_1.__private.inferQueryType('INSERT INTO users (email) VALUES ($1)')).toBe('write');
        });
        it('should infer UPDATE as write query', () => {
            expect(postgres_js_1.__private.inferQueryType('UPDATE users SET email = $1')).toBe('write');
        });
        it('should infer DELETE as write query', () => {
            expect(postgres_js_1.__private.inferQueryType('DELETE FROM users WHERE id = $1')).toBe('write');
        });
        it('should infer SHOW as read query', () => {
            expect(postgres_js_1.__private.inferQueryType('SHOW server_version')).toBe('read');
        });
        it('should infer EXPLAIN as read query', () => {
            expect(postgres_js_1.__private.inferQueryType('EXPLAIN SELECT * FROM users')).toBe('read');
        });
        it('should handle CTE with SELECT as read', () => {
            expect(postgres_js_1.__private.inferQueryType('WITH cte AS (SELECT * FROM users) SELECT *')).toBe('read');
        });
        it('should handle CTE with INSERT as write', () => {
            expect(postgres_js_1.__private.inferQueryType('WITH cte AS (SELECT * FROM users) INSERT INTO logs')).toBe('write');
        });
        it('should handle case-insensitive queries', () => {
            expect(postgres_js_1.__private.inferQueryType('select * from users')).toBe('read');
            expect(postgres_js_1.__private.inferQueryType('INSERT INTO USERS')).toBe('write');
        });
    });
    describe('Prepared Statements', () => {
        it('should generate consistent names for same query', () => {
            ...
            const name1 = postgres_js_1.__private.getPreparedStatementName(query1);
            const name2 = postgres_js_1.__private.getPreparedStatementName(query2);
        ...
        it('should generate names with stmt_ prefix', () => {
            ...
            const name = postgres_js_1.__private.getPreparedStatementName(query);
    ...
    describe('Health Check', () => {
        ...
        it('should mark pool as unhealthy on connection failure', async () => {
            ...
            const health = await pool.healthCheck();
            expect(health.some((h) => !h.healthy)).toBe(true);
        });
    });
        ...
        it('should return insights sorted by max duration', () => {
            ...
            if (insights.length > 1) {
                for (let i = 0; i < insights.length - 1; i++) {
                    expect(insights[i].maxDurationMs).toBeGreaterThanOrEqual(insights[i + 1].maxDurationMs);
                }
            }
        });
    });
    describe('Connection Management', () => {
        ...
        it('should provide connect method for transactions', async () => {
            ...
            pool.connect.mockResolvedValue(mockTransactionClient);
        ...
        it('should handle pool closure', async () => {
            await expect((0, postgres_js_1.closePostgresPool)()).resolves.not.toThrow();
        });
    ...
    describe('Error Handling', () => {
        ...
        it('should throw error on query failure', async () => {
            mockClient.query.mockResolvedValueOnce(undefined); // SET statement_timeout
            mockClient.query.mockRejectedValueOnce(new Error('Syntax error in SQL'));

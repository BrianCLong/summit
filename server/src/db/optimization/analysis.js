...
const logger = pino_1.default({ name: 'query-analyzer' });
...
async function profileJourney(session, name, cypher, params) {
    ...
    try {
        ...
        `);
            ...
            if (slowQueries.length > 0) {
                ...
                fs_1.default.writeFileSync(path_1.default.join(process.cwd(), 'server/src/db/optimization/slow-queries-live.json'), JSON.stringify(slowQueries, null, 2));
            }
            else {
        ...
        for (const q of criticalQueries) {
            try {
                ...
                const hasFullScan = JSON.stringify(plan).includes('NodeByLabelScan') ||
        ...
        fs_1.default.writeFileSync(path_1.default.join(process.cwd(), 'server/src/db/optimization/analysis-report.json'), JSON.stringify(report, null, 2));

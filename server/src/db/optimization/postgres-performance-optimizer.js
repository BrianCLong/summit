...
exports.findRelationshipsByEntityIdOptimized = findRelationshipsByEntityIdOptimized;
exports.getEntityRelationshipCountOptimized = getEntityRelationshipCountOptimized;
exports.getInvestigationStatsOptimized = getInvestigationStatsOptimized;
exports.batchCreateEntitiesOptimized = batchCreateEntitiesOptimized;
exports.searchEntitiesWithPropsOptimized = searchEntitiesWithPropsOptimized;
exports.analyzeQueryPlan = analyzeQueryPlan;
exports.executeWithPerfMonitoring = executeWithPerfMonitoring;
exports.getSlowQueries = getSlowQueries;
exports.getIndexUsageStats = getIndexUsageStats;
const logger_js_1 = __importDefault(require("../../config/logger.js"));
const perfLogger = logger_js_1.default.child({ name: 'postgres-perf-optimizer' });
/**
 * ============================================================================
 ...
 */
async function findRelationshipsByEntityIdOptimized(pg, entityId, tenantId, direction = 'both') {
    if (direction === 'outgoing') {
        const { rows } = await pg.query(`SELECT * FROM relationships
 ...
 */
async function getEntityRelationshipCountOptimized(pg, entityId, tenantId) {
    // OPTIMIZED: Run two queries in parallel, each uses its own index efficiently
    const [outgoingResult, incomingResult] = await Promise.all([
        pg.query(`SELECT COUNT(*) as count FROM relationships
 ...
 */
async function getInvestigationStatsOptimized(pg, investigationId, tenantId) {
    ...
    const { rows } = await pg.query(`SELECT
       ...
       (SELECT COUNT(*)
        FROM relationships
        WHERE tenant_id = $1
          AND props->>'investigationId' = $2) as relationship_count`, [tenantId, investigationId]);
    return {
 ...
 */
async function batchCreateEntitiesOptimized(pg, inputs, userId) {
    if (inputs.length === 0)
        return;
    ...
    try {
       ...
       SELECT * FROM UNNEST(
        ...
        const payloads = inputs.map((i) => JSON.stringify({ id: i.id, tenantId: i.tenantId }));
        await client.query(`INSERT INTO outbox_events (id, topic, payload)
       SELECT * FROM UNNEST($1::uuid[], $2::text[], $3::jsonb[])`, [outboxIds, topics, payloads]);
 ...
 */
async function searchEntitiesWithPropsOptimized(pg, tenantId, kind, props, limit = 100, offset = 0) {
 ...
 */
async function analyzeQueryPlan(pg, query, params) {
 ...
 */
async function executeWithPerfMonitoring(pg, label, query, params, thresholdMs = 100) {
 ...
 */
async function getSlowQueries(pg, minDurationMs = 100, limit = 20) {
    try {
        const { rows } = await pg.query(`SELECT
         query,
         calls,
       ...
       LIMIT $2`, [minDurationMs, limit]);
        return rows;
    }
    catch (error) {
        // pg_stat_statements may not be installed
        perfLogger.warn('pg_stat_statements not available, cannot query slow queries');
        return [];
    }
}
/**
 ...
 * ============================================================================
 */
async function getIndexUsageStats(pg, tableName) {
    const { rows } = await pg.query(`SELECT

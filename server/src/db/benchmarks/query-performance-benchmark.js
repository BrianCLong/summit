...
const BENCHMARK_CONFIG = {
    ...
    // Test tenant
    TENANT_ID: 'benchmark-tenant-' + (0, crypto_1.randomUUID)(),
    USER_ID: 'benchmark-user-' + (0, crypto_1.randomUUID)(),
};
// ============================================================================
// Database Setup
// ============================================================================
...
async function generateSampleData() {
    ...
    try {
        ...
        // Generate entities
        console.log(`   Inserting ${BENCHMARK_CONFIG.ENTITY_COUNT} sample entities...`);
        const entityIds = [];
        ...
        for (let i = 0; i < BENCHMARK_CONFIG.ENTITY_COUNT; i += batchSize) {
            ...
            for (let j = 0; j < batch; j++) {
                ...
                props.push(JSON.stringify({
                    name: `Entity ${i + j}`,
                    benchmarkRun: true,
                    investigationId: j % 10 === 0 ? `inv-${Math.floor((i + j) / 10)}` : undefined,
                }));
            }
            await client.query(`INSERT INTO entities (id, tenant_id, kind, labels, props, created_by)
         SELECT * FROM UNNEST($1::uuid[], $2::text[], $3::text[], $4::text[][], $5::jsonb[], $6::text[])`, [ids, tenants, kinds, labels, props, new Array(batch).fill(BENCHMARK_CONFIG.USER_ID)]);
        }
        // Generate relationships
        console.log(`   Inserting ${BENCHMARK_CONFIG.RELATIONSHIP_COUNT} sample relationships...`);
        for (let i = 0; i < BENCHMARK_CONFIG.RELATIONSHIP_COUNT; i += batchSize) {
            const batch = Math.min(batchSize, BENCHMARK_CONFIG.RELATIONSHIP_COUNT - i);
            const ids = [];
            const tenants = [];
            const srcIds = [];
            const dstIds = [];
            ...
            const props = [];
            for (let j = 0; j < batch; j++) {
                ids.push((0, crypto_1.randomUUID)());
                tenants.push(BENCHMARK_CONFIG.TENANT_ID);
                srcIds.push(entityIds[Math.floor(Math.random() * entityIds.length)]);
                dstIds.push(entityIds[Math.floor(Math.random() * entityIds.length)]);
                types.push(j % 2 === 0 ? 'KNOWS' : j % 3 === 0 ? 'WORKS_AT' : 'RELATED_TO');
                props.push(JSON.stringify({
                    weight: Math.random(),
                    benchmarkRun: true,
                    investigationId: j % 10 === 0 ? `inv-${Math.floor((i + j) / 10)}` : undefined,
            ...
            await client.query(`INSERT INTO relationships (id, tenant_id, src_id, dst_id, type, props, created_by)
         SELECT * FROM UNNEST($1::uuid[], $2::text[], $3::uuid[], $4::uuid[], $5::text[], $6::jsonb[], $7::text[])`, [ids, tenants, srcIds, dstIds, types, props, new Array(batch).fill(BENCHMARK_CONFIG.USER_ID)]);
        }
        // Generate investigations
        console.log(`   Inserting ${BENCHMARK_CONFIG.INVESTIGATION_COUNT} sample investigations...`);
        for (let i = 0; i < BENCHMARK_CONFIG.INVESTIGATION_COUNT; i++) {
            await client.query(`INSERT INTO investigations (id, tenant_id, name, status, props, created_by)
         VALUES ($1, $2, $3, $4, $5, $6)`, [
                `inv-${i}`,
...
async function cleanupSampleData() {
    ...
    await pool.query(`DELETE FROM relationships WHERE tenant_id = $1`, [BENCHMARK_CONFIG.TENANT_ID]);
    await pool.query(`DELETE FROM entities WHERE tenant_id = $1`, [BENCHMARK_CONFIG.TENANT_ID]);
    await pool.query(`DELETE FROM investigations WHERE tenant_id = $1`, [BENCHMARK_CONFIG.TENANT_ID]);
...
function printResults(results) {
    ...
    console.log(`${'Test Name'.padEnd(50)} | ${'Mean'.padStart(10)} | ${'p50'.padStart(10)} | ${'p95'.padStart(10)} | ${'p99'.padStart(10)} | ${'Min'.padStart(10)} | ${'Max'.padStart(10)}`);
    console.log('-'.repeat(100));
    for (const result of results) {
        console.log(`${result.testName.padEnd(50)} | ${result.meanMs.toFixed(2).padStart(10)} | ${result.p50Ms.toFixed(2).padStart(10)} | ${result.p95Ms.toFixed(2).padStart(10)} | ${result.p99Ms.toFixed(2).padStart(10)} | ${result.minMs.toFixed(2).padStart(10)} | ${result.maxMs.toFixed(2).padStart(10)}`);
    }
    console.log('='.repeat(100) + '\n');
}
// ============================================================================
// Benchmark Tests
// ============================================================================
async function benchmarkEntitySearchWithJsonb(useOptimizedIndexes) {
    ...
    return runBenchmark(testName, async () => {
        await pool.query(`SELECT * FROM entities
...
async function benchmarkInvestigationStats() {
    ...
    return runBenchmark(testName, async () => {
        await pool.query(`SELECT
         ...
         (SELECT COUNT(*)
          FROM relationships
          WHERE tenant_id = $1
            AND props->>'investigationId' = $2) as relationship_count`, [BENCHMARK_CONFIG.TENANT_ID, 'inv-1']);
    });
}
async function benchmarkRelationshipsByEntity() {
    const testName = '3. Relationships by Entity (OPTIMIZED - UNION ALL)';
    // Pick a random entity
    const { rows } = await pool.query(`SELECT id FROM entities WHERE tenant_id = $1 LIMIT 1`, [BENCHMARK_CONFIG.TENANT_ID]);
    const entityId = rows[0].id;
    return runBenchmark(testName, async () => {
        await pool.query(`SELECT * FROM relationships
       ...
       ORDER BY created_at DESC`, [BENCHMARK_CONFIG.TENANT_ID, entityId, BENCHMARK_CONFIG.TENANT_ID, entityId]);
    });
}
async function benchmarkRelationshipCount() {
    ...
    return runBenchmark(testName, async () => {
        await Promise.all([
         ...
         WHERE tenant_id = $1 AND src_id = $2`, [BENCHMARK_CONFIG.TENANT_ID, entityId]),
            pool.query(`SELECT COUNT(*) as count FROM relationships
         WHERE tenant_id = $1 AND dst_id = $2`, [BENCHMARK_CONFIG.TENANT_ID, entityId]),
        ]);
    });
}
// ============================================================================

...
exports.RedisBucket = void 0;
exports.getRedisBucket = getRedisBucket;
exports.createBucketMiddleware = createBucketMiddleware;
...
class RedisBucket {
    ...
    constructor(redisUrl) {
        this.client = new ioredis_1.default(redisUrl || process.env.REDIS_URL || 'redis://localhost:6379', {
            connectTimeout: 5000,
            commandTimeout: 2000,
            retryStrategy: (times) => {
                if (times > 3)
        ...
        this.client.on('error', (error) => {
            logger_js_1.default.error('Redis client error:', error);
        });
        this.client.on('connect', () => {
            logger_js_1.default.info('Redis client connected');
        });
        this.client.on('disconnect', () => {
            logger_js_1.default.warn('Redis client disconnected');
    ...
    async init() {
        ...
        try {
            ...
            this.connected = true;
            logger_js_1.default.info('RedisBucket initialized successfully');
        }
        catch (error) {
            logger_js_1.default.error('Failed to initialize RedisBucket:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async takeTokens(tenant, operation, tokensRequested = 1, config = {}) {
        await this.ensureConnected();
        const { capacity = 100, refillRate = 10, refillIntervalMs = 1000, keyPrefix = 'bucket', } = config;
        const key = `${keyPrefix}:${tenant}:${operation}`;
        const now = Date.now();
        try {
            const result = (await this.client.evalSha(this.luaScriptSha, {
                keys: [key],
                arguments: [
                    now.toString(),
                    capacity.toString(),
            ...
            logger_js_1.default.error('Failed to take tokens from bucket:', {
                tenant,
                operation,
                error,
            });
     ...
     */
    async takeTokensBatch(tenant, requests, config = {}) {
        await this.ensureConnected();
        const { capacity = 100, refillRate = 10, refillIntervalMs = 1000, keyPrefix = 'bucket', } = config;
        ...
        try {
            const result = (await this.client.evalSha(this.batchScriptSha, {
                keys: [key],
                arguments: args,
            }));
            const results = [];
            ...
            logger_js_1.default.error('Failed to take tokens in batch:', {
                tenant,
                requests,
                error,
            });
     ...
     */
    async getBucketStatus(tenant, operation, config = {}) {
        await this.ensureConnected();
        ...
        try {
            ...
            logger_js_1.default.error('Failed to get bucket status:', {
                tenant,
                operation,
                error,
            });
     ...
     */
    async resetBucket(tenant, operation, config = {}) {
        await this.ensureConnected();
        ...
        try {
            ...
            logger_js_1.default.info('Bucket reset successfully', { tenant, operation, capacity });
        }
        catch (error) {
            logger_js_1.default.error('Failed to reset bucket:', { tenant, operation, error });
        }
    }
    /**
    ...
    async getGlobalStats() {
        ...
        try {
            ...
            logger_js_1.default.error('Failed to get global stats:', error);
            return {
                totalBuckets: 0,
                activeBuckets: 0,
                memoryUsage: 0,
    ...
    async cleanupExpired() {
        ...
        try {
            ...
            logger_js_1.default.info('Bucket cleanup completed', {
            ...
            logger_js_1.default.error('Failed to cleanup expired buckets:', error);
            return 0;
        }
    }
    /**
 ...
 */
function createBucketMiddleware(config) {
    ...
    return async (req, res, next) => {
        try {
            ...
            logger_js_1.default.error('Rate limiting middleware error:', error);

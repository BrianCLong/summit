...
export async function signClaimSet(transitKey: string, payload: any) {
  ...
  const r = await fetch(`${process.env.VAULT_ADDR}/v1/transit/sign/${transitKey}`, {
    method: 'POST',
    headers: { 'X-Vault-Token': String(process.env.VAULT_TOKEN || ''), 'content-type': 'application/json' },

...
class AnomalyDetectionController {
     ...
     */
    async processMetricDataPoint(req, res) {
        try {
            ...
            logger_js_1.logger.error(`Failed to process metric data point`, error);
            res.status(500).json({
     ...
     */
    async processBatchMetricDataPoints(req, res) {
        try {
            ...
            logger_js_1.logger.error(`Failed to process batch metric data points`, error);
            res.status(500).json({
     ...
     */
    async getActiveAnomalyAlerts(req, res) {
        try {
            ...
            logger_js_1.logger.error(`Failed to get active anomaly alerts`, error);
            res.status(500).json({
     ...
     */
    async getAnomalyAlertHistory(req, res) {
        try {
            ...
            const alerts = await this.anomalyService.getAnomalyAlertHistory(metric, severity, startTime ? parseInt(startTime) : undefined, parsedLimit);
            res.status(200).json(alerts);
        }
        catch (error) {
            logger_js_1.logger.error(`Failed to get anomaly alert history`, error);
            res.status(500).json({
     ...
     */
    async acknowledgeAnomalyAlert(req, res) {
        try {
     ...
     */
    async resolveAnomalyAlert(req, res) {
        try {
    ...
    async getAnomalyStatistics(req, res) {
        try {
            ...
            logger_js_1.logger.error(`Failed to get anomaly detection statistics`, error);
            res.status(500).json({
    ...
    async testAnomalyDetection(req, res) {
        try {
            ...
            logger_js_1.logger.error(`Failed to test anomaly detection`, error);
            res.status(500).json({

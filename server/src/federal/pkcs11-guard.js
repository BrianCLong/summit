...
exports.assertMechanismsSupported = assertMechanismsSupported;
exports.enforceMech = enforceMech;
exports.performHSMSelfTest = performHSMSelfTest;
exports.aes256gcmEncrypt = aes256gcmEncrypt;
exports.ecdsaP384Sign = ecdsaP384Sign;
exports.finalizePKCS11 = finalizePKCS11;
// @ts-nocheck
const pkcs11 = __importStar(require("pkcs11js"));
...
const ALLOWLIST = {
    ...
    ECDSA_P384: {
        ...
        guard: (p) => {
            if (p)
                throw new Error('ECDSA should not have mechanism parameters');
        },
    },
    RSA_PSS_4096: {
        ...
        guard: (p) => {
            if (!p)
                throw new Error('RSA-PSS requires parameters');
 ...
 */
function initPKCS11(libPath = '/opt/cloudhsm/lib/libcloudhsm_pkcs11.so', slotIndex = 0) {
    const span = otel_tracing_js_1.otelService.createSpan('pkcs11.init');
    try {
        ...
        if (slotIndex >= slots.length) {
            throw new Error(`Slot index ${slotIndex} not available (${slots.length} slots found)`);
        }
        ...
        if ((tokenInfo.flags & CKF_FIPS_MODE) === 0) {
            throw new Error(`HSM slot ${slotIndex} not in FIPS mode (flags: 0x${tokenInfo.flags.toString(16)})`);
        }
        ...
        console.log(`✅ PKCS#11 initialized: ${tokenInfo.label?.trim()} (FIPS mode active)`);
        return {
            p11,
            slot,
            session,
 ...
 */
function assertMechanismsSupported(ctx) {
    const span = otel_tracing_js_1.otelService.createSpan('pkcs11.assert_mechanisms');
    try {
        const supported = new Set(ctx.mechanisms);
        const missing = [];
        const available = [];
        Object.keys(ALLOWLIST).forEach((name) => {
 ...
 */
function enforceMech(name, params) {
    const def = ALLOWLIST[name];
    if (!def) {
 ...
 */
async function performHSMSelfTest(ctx) {
    const span = otel_tracing_js_1.otelService.createSpan('pkcs11.self_test');
    try {
        const testResults = {
        ...
        const selfTest = {
            ...
            supportedMechanisms: ctx.mechanisms.map((m) => `0x${m.toString(16).padStart(8, '0')}`),
            allowlistCompliance: Object.keys(ALLOWLIST).every((name) => ctx.mechanisms.includes(ALLOWLIST[name].mech)),
            testResults,
        };
 ...
 */
function aes256gcmEncrypt(ctx, keyLabel, plaintext, aad) {
    const span = otel_tracing_js_1.otelService.createSpan('pkcs11.aes256gcm_encrypt');
    try {
 ...
 */
function ecdsaP384Sign(ctx, keyLabel, data) {
    const span = otel_tracing_js_1.otelService.createSpan('pkcs11.ecdsa_p384_sign');
    try {

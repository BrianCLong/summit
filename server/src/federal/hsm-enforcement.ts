...
class HSMEnforcement {
  ...
  private async probeAWSCloudHSM(): Promise<HSMProbeResult> {
    try {
      ...
      const fipsMode =
        (tokenInfo.label as string | undefined)?.includes('FIPS') ||
        ((tokenInfo.flags as number) & 0x00000002) !== 0; // CKF_FIPS_MODE
  ...
  generateAttestation(): {
    ...
    const attestation = {
      ...
      nodeFipsEnabled: !!(crypto as any).getFips?.() || false,
...
export const hsmEnforcement = new HSMEnforcement({
  enabledProviders: (process.env.HSM_PROVIDERS?.split(',') as (
    | 'AWS_CloudHSM'
    | 'Azure_HSM'
    | 'Luna_HSM'
    | 'nShield'

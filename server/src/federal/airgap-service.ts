...
const AirGapConfigSchema = z.object({
  ...
  updateProtocol: z.object({
    transferMedia: z.enum(['secure_usb', 'dvd_rom', 'tape_archive', 'paper_qr']).default('secure_usb'),
...
export class AirGapService {
    ...
    for (const component of manifest.components) {
      ...
      if (this.config.compliance.sbomsRequired) {
        try {
          ...
          if (sbom.name === component.name && sbom.version === component.version) {
            checks.sbom = true;
          }
        } catch (error) {
      ...
      if (this.config.compliance.signatureValidation) {
        try {
          const componentPath = path.join(packagePath, 'components', component.name);
          ...
          for (const sigPath of component.signatures) {
            ...
            const hash = crypto.createHash('sha256').update(componentData).digest('hex');
      ...
      try {
        const componentPath = path.join(packagePath, 'components', component.name);
        const componentData = await fs.readFile(componentPath);
        const calculatedHash = crypto.createHash('sha256').update(componentData).digest('hex');
  ...
  private async applyOfflineUpdate(packagePath: string, manifest: AirGapManifest): Promise<boolean> {
    try {
      ...
      for (const component of manifest.components) {
        ...
        const targetPath = path.join(this.config.offlineRegistry.path, component.name, component.version);
   ...
   */
  async terminateBreakGlass(sessionId: string, reason: string): Promise<boolean> {
    const session = this.activeBreakGlass.get(sessionId);
    if (!session) {
      console.error(`Break-glass session not found: ${sessionId}`);
      return false;

...
const ALLOWLIST: Record<AllowedMech, { 
  mech: number; 
  ...
  RSA_PSS_4096: { 
    ...
    guard: (p) => {
      ...
      if (p.hashAlg !== 0x00000060) throw new Error('RSA-PSS requires SHA-384 (0x60)'); // CKM_SHA384
      if (p.mgf !== 0x00000003) throw new Error('RSA-PSS requires MGF1-SHA384 (0x03)'); // CKG_MGF1_SHA384
      if (!p.sLen || p.sLen < 48) throw new Error('RSA-PSS salt length must be >= 48 bytes');
    }
  },
};
 ...
 */
export function enforceMech(name: AllowedMech, params?: any): { mechanism: number; parameter?: any } {
  const def = ALLOWLIST[name];
  if (!def) {
 ...
 */
export async function performHSMSelfTest(ctx: PKCS11Context): Promise<HSMSelfTest> {

...
class EnhancedOnboardingService {
    ...
    async startOnboarding(tenantId, userId, persona, locale) {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(progress, {
    ...
    async getAnalyticsSummary(period, startDate, endDate) {
      ...
      GROUP BY persona, metrics`, [startDate, endDate]);
    ...
    async getProgress(tenantId, userId) {
        ...
        if (!pool)
            return null;
        const result = await pool.query('SELECT * FROM onboarding_progress WHERE tenant_id = $1 AND user_id = $2', [tenantId, userId]);
    ...
    async saveProgress(progress) {
        ...
        if (!pool)
            throw new Error('Database not available');
        await pool.query(`INSERT INTO onboarding_progress (
    ...
    async trackEvent(tenantId, userId, eventType, properties) {
        ...
        logger_js_1.default.info('Onboarding analytics event', { event });
        // Also persist to database for aggregation
        const pool = (0, database_js_1.getPostgresPool)();
        if (pool) {
            await pool.query(`INSERT INTO onboarding_analytics_events (
    ...
    hashIdentifier(id) {
        ...
        return (0, crypto_1.createHash)('sha256').update(`${salt}:${id}`).digest('hex').substring(0, 16);
    }
    createGovernanceVerdict(policyId, result) {
        return {
            verdictId: (0, crypto_1.randomUUID)(),
            policyId,
            result: result === 'allow' ? data_envelope_js_1.GovernanceResult.ALLOW : data_envelope_js_1.GovernanceResult.DENY,
            decidedAt: new Date(),
    ...
    async installSamplePolicy(tenantId, userId, sample) {
        ...
        logger_js_1.default.info('Installing sample policy', { tenantId, policyId, sample: sample.name });
        return policyId;
    }
    async installSampleDashboard(tenantId, userId, sample) {
        // In production, this would call DashboardService
        const dashboardId = (0, crypto_1.randomUUID)();
        logger_js_1.default.info('Installing sample dashboard', { tenantId, dashboardId, sample: sample.name });
        return dashboardId;
    }
    async installSampleReport(tenantId, userId, sample) {
        ...
        logger_js_1.default.info('Installing sample report', { tenantId, reportId, sample: sample.name });
        return reportId;
    }
...
exports.EnhancedOnboardingService = EnhancedOnboardingService;
exports.enhancedOnboardingService = EnhancedOnboardingService.getInstance();

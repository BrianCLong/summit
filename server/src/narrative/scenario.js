...
class ScenarioSimulator {
     ...
     */
    async runBatch(config, iterations = 10, ticks = 20, shock) {
        const promises = [];
        for (let i = 0; i < iterations; i++) {
            const engine = new engine_js_1.NarrativeSimulationEngine({
                ...config,
                id: `${config.id}-run-${i}`,
            });
    ...
    async runSingleSimulation(engine, totalTicks, shock) {
    ...
    clusterOutcomes(scenarioId, states) {
        ...
        points.forEach((p) => {
            ...
            if (p.avgPressure > 0.3) {
                if (p.avgSentiment < -0.1)
                    label = 'Crisis (High Volatility, Negative)';
                else if (p.avgSentiment > 0.1)
                    label = 'Optimistic Chaos';
                else
                    label = 'High Volatility';
            }
            else {
                if (p.avgSentiment < -0.1)
                    label = 'Stagnation (Stable Negative)';
                else if (p.avgSentiment > 0.1)
                    label = 'Prosperity (Stable Positive)';
                else
                    label = 'Stability';
            }
            if (!clusters[label]) {

...
exports.HybridNarrativeAnalyzer = void 0;
class HybridNarrativeAnalyzer {
    /**
     * Main analysis loop.
    ...
    // --- Step 1: Semantic Extraction ---
    async extractSemantics(content) {
    ...
    async mapNetworkPropagation(context, keywords = []) {
    ...
    synthesize(semantic, network) {
        ...
        return {
            narrative: semantic, // Type assertion for mock
            propagation: network,
            confidence,
            isManipulated,
            explanation
    ...
    // --- Helpers ---
    determineFrame(content) {
        const lc = content.toLowerCase();
        if (lc.includes('danger') || lc.includes('threat'))
            return 'fear_mongering';
        if (lc.includes('hero') || lc.includes('save'))
...
exports.HybridNarrativeAnalyzer = HybridNarrativeAnalyzer;

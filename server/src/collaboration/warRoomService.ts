...
export class WarRoomService {
  async createWarRoom(name: string, createdBy: number): Promise<WarRoom> {
    const { rows }: QueryResult<WarRoom> = await db.query(
  ...
  async addParticipant(warRoomId: number, userId: number, role: string): Promise<WarRoomParticipant> {
    const { rows }: QueryResult<WarRoomParticipant> = await db.query(
  ...
  async getWarRoom(id: number): Promise<WarRoom | undefined> {
    const { rows }: QueryResult<WarRoom> = await db.query('SELECT * FROM war_rooms WHERE id = $1', [id]);
    return rows[0];
  }

  async getWarRooms(): Promise<WarRoom[]> {
    const { rows }: QueryResult<WarRoom> = await db.query('SELECT * FROM war_rooms');
    return rows;
  }

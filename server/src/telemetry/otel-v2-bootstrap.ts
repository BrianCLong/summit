...
const resource = new Resource({
  [SEMRESATTRS_SERVICE_NAME]: process.env.OTEL_SERVICE_NAME || 'intelgraph-server',
    ...
    getNodeAutoInstrumentations({
      ...
      '@opentelemetry/instrumentation-express': {
        requestHook: (span, info) => {
          ...
          span.setAttributes({
            'intelgraph.request.tenant_id': info.req.headers['x-tenant-id'] || 'default',
            'intelgraph.request.user_id': info.req.headers['x-user-id'] || 'anonymous',
...
export function initializeOTelV2(): void {
  try {
    ...
    // Health check span
    const tracer = require('@opentelemetry/api').trace.getTracer('intelgraph-bootstrap');
...
process.on('SIGTERM', () => {
  sdk.shutdown()
    ...
    .catch((error) => console.error('Error terminating OpenTelemetry v2', error))

...
export const TABLE_MAPPINGS: TableMapping[] = [
  ...
  { category: 'OPERATIONAL_METADATA', tableName: 'audit_access_logs', schema: 'maestro', timestampColumn: 'accessed_at', tenantColumn: 'tenant_id' }, // Note: audit_access_logs is typically AUDIT_RECORDS, but checking migration 010 suggests it's immutable access log. Re-classifying below if needed.
  { category: 'OPERATIONAL_METADATA', tableName: 'user_sessions', schema: 'public', timestampColumn: 'created_at', tenantColumn: 'user_id' }, // user_id implicitly links tenant
  ...
  // Audit Records - Overriding the operational mapping above if it's strict audit
  ...
  // Tenant Data (Retention is managed by tenant lifecycle, not time-based expiration usually)

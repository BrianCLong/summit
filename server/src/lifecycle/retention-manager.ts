...
export class RetentionManager {
  ...
  public async scanExpired(): Promise<Record<DataCategory, number>> {
    ...
    for (const mapping of TABLE_MAPPINGS) {
      ...
      // Check for Legal Holds (system wide or per tenant if applicable)
      // For retention scan, it's tricky because we are deleting bulk rows.
      // We should perform a check: DELETE ... WHERE ... AND tenant_id NOT IN (SELECT target_id FROM legal_holds WHERE status='active')
      // This assumes 'target_id' in legal_holds matches 'tenant_id'.

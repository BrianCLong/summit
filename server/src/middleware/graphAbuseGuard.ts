...
export class GraphAbuseGuard {
  ...
  private getDefaultSuspiciousPatterns(): SuspiciousPattern[] {
    return [
      ...
      },
      {
        ...
        condition: (analysis) => 
          analysis.estimatedEdges > 25000 || 
          analysis.complexity > 800,
        severity: 'medium',
        blockQuery: true
      },
      {
  ...
  public middleware() {
    return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
      ...
      return tracer.startActiveSpan('graph_abuse_guard.middleware', async (span: Span) => {
        try {
          ...
          const blockingPatterns = this.config.suspiciousPatterns
            .filter(pattern => pattern.blockQuery && pattern.condition(analysis));
          ...
          const warningPatterns = this.config.suspiciousPatterns
            .filter(pattern => !pattern.blockQuery && pattern.condition(analysis));
  ...
  private async analyzeQuery(query: string, tenantId: string): Promise<QueryAnalysis> {
    ...
    if (lowerQuery.includes('optional match')) analysis.patterns.push('OPTIONAL_MATCH');
    if (lowerQuery.includes('*')) analysis.patterns.push('VARIABLE_LENGTH');
    if (lowerQuery.includes('()-[*]->()')) analysis.patterns.push('STAR_TRAVERSAL');
  ...
  private async trackQuery(tenantId: string, suspicious: boolean): Promise<void> {
    const windows = this.queryWindows.get(tenantId) || [];
  ...
  public trackEdgeTraversal(traversalId: string, edgeId: string, fromNode: string): boolean {
    const state = this.activeTraversals.get(traversalId);
  ...
  public updateTenantLimits(tenantId: string, limits: Partial<TenantLimits>): void {

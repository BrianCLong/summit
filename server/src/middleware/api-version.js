...
function loadVersionRegistry() {
    try {
        ...
        logger_js_1.logger.warn({ error }, 'Failed to load version registry, using defaults');
    }
    return null;
}
/**
...
function apiVersionMiddleware(req, res, next) {
    ...
    if (isDeprecated && sunsetDate) {
        warnings.push(`API version ${versionKey} is deprecated and will be sunset on ${sunsetDate}. Please upgrade to ${latestVersion}.`);
    }
    if (isSunset) {
        warnings.push(`API version ${versionKey} has been sunset. Please upgrade to ${latestVersion}.`);
    }

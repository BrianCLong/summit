...
exports.graphAbuseGuard = exports.GraphAbuseGuard = void 0;
const metrics_js_1 = require("../utils/metrics.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
const tracing_js_1 = require("../utils/tracing.js");
class GraphAbuseGuard {
    config;
    metrics;
    queryWindows = new Map();
    ...
    constructor(config = {}) {
        ...
        this.metrics = new metrics_js_1.PrometheusMetrics('graph_abuse_guard');
        this.initializeMetrics();
        this.loadTenantLimits();
        this.startCleanupTask();
    }
    initializeMetrics() {
        this.metrics.createCounter('graph_queries_total', 'Total graph queries processed', ['tenant_id', 'status']);
        this.metrics.createCounter('queries_blocked', 'Queries blocked by abuse guard', ['tenant_id', 'reason']);
        this.metrics.createCounter('suspicious_patterns_detected', 'Suspicious patterns detected', ['tenant_id', 'pattern']);
        this.metrics.createHistogram('query_complexity', 'Graph query complexity scores', {
        ...
        this.metrics.createGauge('active_traversals', 'Currently active graph traversals');
    }
    getDefaultSuspiciousPatterns() {
        return [
            ...
            },
            {
                ...
                condition: (analysis) => analysis.maxDepth > 15 && analysis.hasRecursion,
                severity: 'high',
                blockQuery: true,
            },
            {
                ...
                condition: (analysis) => analysis.estimatedNodes > 5000 &&
            ...
            },
            {
                ...
                condition: (analysis) => analysis.estimatedEdges > 25000 || analysis.complexity > 800,
                severity: 'medium',
                blockQuery: true,
            },
            {
                ...
                condition: (analysis) => analysis.maxDepth > 8 &&
            ...
            },
            {
                ...
                condition: (analysis) => analysis.patterns.includes('()-[*]->()') &&
    ...
    middleware() {
        return async (req, res, next) => {
            ...
            return tracing_js_1.tracer.startActiveSpan('graph_abuse_guard.middleware', async (span) => {
                try {
                    ...
                    if (await this.isRateLimited(tenantId)) {
                        logger_js_1.default.warn('Graph query rate limited', { tenantId });
                    ...
                    // Record metrics
                    this.metrics.observeHistogram('query_complexity', analysis.complexity);
                    this.metrics.observeHistogram('traversal_depth', analysis.maxDepth);
                    this.metrics.observeHistogram('fan_out_ratio', analysis.fanOutRatio);
                    // Check for blocking patterns
                    const blockingPatterns = this.config.suspiciousPatterns.filter((pattern) => pattern.blockQuery && pattern.condition(analysis));
                    if (blockingPatterns.length > 0) {
                        const reasons = blockingPatterns.map((p) => p.name);
                        logger_js_1.default.warn('Graph query blocked', {
                            tenantId,
                            reasons,
                            analysis: {
                    ...
                    const warningPatterns = this.config.suspiciousPatterns.filter((pattern) => !pattern.blockQuery && pattern.condition(analysis));
                    if (warningPatterns.length > 0) {
                        warningPatterns.forEach((pattern) => {
                            logger_js_1.default.warn('Suspicious graph query pattern', {
                                tenantId,
                    ...
                    logger_js_1.default.error('Graph abuse guard error', { error: error.message });
    ...
    async analyzeQuery(query, tenantId) {
        const analysis = {
            query,
            estimatedNodes: 0,
            estimatedEdges: 0,
        ...
        if (lowerQuery.includes('*'))
            analysis.patterns.push('VARIABLE_LENGTH');
        if (lowerQuery.includes('()-[*]->()'))
            analysis.patterns.push('STAR_TRAVERSAL');
        if (lowerQuery.includes('limit'))
            analysis.patterns.push('LIMIT');
        if (lowerQuery.includes('order by'))
            analysis.patterns.push('ORDER_BY');
        if (lowerQuery.includes('with '))
            analysis.patterns.push('WITH_CLAUSE');
        ...
        if (analysis.maxDepth > 10)
            riskScore += 25;
        if (analysis.fanOutRatio > 50)
            riskScore += 20;
        if (analysis.estimatedNodes > 5000)
            riskScore += 15;
        if (!analysis.patterns.includes('LIMIT'))
            riskScore += 10;
        analysis.riskScore = Math.min(riskScore, 100);
        return analysis;
    }
    async isRateLimited(tenantId) {
    ...
    async trackQuery(tenantId, suspicious) {
        const windows = this.queryWindows.get(tenantId) || [];
        const currentWindow = Math.floor(Date.now() / (60 * 1000)); // 1-minute windows
        let window = windows.find((w) => Math.floor(w.timestamp / (60 * 1000)) === currentWindow);
        if (!window) {
            window = {
                tenantId,
                timestamp: Date.now(),
        ...
        window.queryCount++;
        if (suspicious)
            window.suspiciousCount++;
        this.queryWindows.set(tenantId, windows);
    }
    getSuggestionsForPatterns(patterns) {
        const suggestions = [];
        for (const pattern of patterns) {
            switch (pattern.name) {
                case 'excessive_fan_out':
                    suggestions.push('Consider adding more specific relationship filters or using LIMIT clauses');
                    break;
                case 'deep_recursion':
                    suggestions.push('Limit recursion depth using path length constraints like [*1..5]');
                    break;
                case 'graph_bomb':
                    suggestions.push('Add LIMIT clauses and more selective WHERE conditions');
                    break;
                case 'resource_exhaustion':
                    suggestions.push('Use pagination with SKIP and LIMIT, or add more selective filters');
                    break;
                case 'traversal_abuse':
                    suggestions.push('Add LIMIT clause and consider using indexes for better performance');
                    break;
                case 'relationship_mining':
                    suggestions.push('Be more specific about relationship types and use appropriate LIMIT clauses');
                    break;
            }
        }
        if (suggestions.length === 0) {
            suggestions.push('Review query patterns and add appropriate limits and filters');
        }
        return suggestions;
    }
    generateTraversalId() {
    ...
    startCleanupTask() {
        ...
        setInterval(() => {
            ...
            if (cleaned > 0) {
                logger_js_1.default.debug('Cleaned expired traversals', { count: cleaned });
                this.metrics.setGauge('active_traversals', this.activeTraversals.size);
            }
    ...
    trackEdgeTraversal(traversalId, edgeId, fromNode) {
        const state = this.activeTraversals.get(traversalId);
        if (!state)
            return true;
        ...
        if (state.edgesTraversed.size > this.config.maxEdgesPerQuery) {
            logger_js_1.default.warn('Edge limit exceeded during traversal', {
                traversalId,
        ...
        if (currentFanOut >= this.config.maxFanOutPerNode) {
            logger_js_1.default.warn('Fan-out limit exceeded during traversal', {
                traversalId,
                fromNode,
    ...
    finishTraversal(traversalId) {
        const state = this.activeTraversals.get(traversalId);
        if (state) {
            const duration = Date.now() - state.startTime;
            logger_js_1.default.debug('Graph traversal completed', {
                traversalId,
                nodesVisited: state.nodesVisited.size,
                edgesTraversed: state.edgesTraversed.size,
                duration,
    ...
    updateTenantLimits(tenantId, limits) {
        ...
        const updated = { ...existing, ...limits, tenantId };
        this.config.tenantLimits.set(tenantId, updated);
        logger_js_1.default.info('Updated tenant limits', { tenantId, limits: updated });
    }
}
exports.GraphAbuseGuard = GraphAbuseGuard;
// Export singleton instance
exports.graphAbuseGuard = new GraphAbuseGuard({
    ...
    complexityTimeoutMs: parseInt(process.env.GRAPH_COMPLEXITY_TIMEOUT || '30000'),

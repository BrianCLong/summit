...
export async function enforceTokenBudget(
  ...
  try {
    ...
    const tokenCount = await countTokens(inferredProvider, model, prompt, completion);
    ...
    switch (budgetCheck.recommendAction) {
      case 'block':
        logger.warn(`Token budget exceeded: ${tokenCount.total} tokens (limit: ${budgetLimit})`, {
          model,
      ...
      case 'warn':
        logger.warn(`Token budget warning: ${tokenCount.total} tokens (${budgetCheck.percentUsed}% of limit)`, {
          model,
          tokens: tokenCount.total,
          cost: tokenCount.estimatedCostUSD
        });
...
export async function truncatePromptIfNeeded(
  ...
  try {
    ...
    if (tokenCount.total > maxTokens) {
      ...
      logger.warn(`Prompt truncated: ${tokenCount.total} -> ${maxTokens} tokens`, {
        model,

...
exports.getTimeoutState = getTimeoutState;
...
function createTimeoutMiddleware(config = {}) {
    ...
    return function timeoutMiddleware(req, res, next) {
        ...
        state.timer = setTimeout(() => {
            state.timedOut = true;
            const verdict = createVerdict(data_envelope_js_1.GovernanceResult.DENY, `Request timeout after ${timeoutMs}ms`);
...
// Helper Functions
// ============================================================================
function calculateTimeout(req, config) {
    ...
    const clientTimeout = parseInt(req.headers['x-request-timeout'], 10);
...
async function withTimeout(operation, timeoutMs, operationName) {
    ...
    const timeoutPromise = new Promise((_, reject) => {
        timeoutId = setTimeout(() => {
            const verdict = createVerdict(data_envelope_js_1.GovernanceResult.DENY, `Operation '${operationName}' timed out after ${timeoutMs}ms`);
            logger_js_1.default.warn({ operationName, timeoutMs, verdict: verdict.verdictId }, 'Async operation timed out');
...
class TimeoutError extends Error {
    ...
    timeoutMs;
    governanceVerdict;
    constructor(operationName, timeoutMs, verdict) {
...
function getTimeoutState(req) {
    return req.timeoutState || null;
}
// ============================================================================
// Export Default Instance
// ============================================================================
exports.timeoutMiddleware = createTimeoutMiddleware();
exports.default = exports.timeoutMiddleware;

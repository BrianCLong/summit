...
export class AbuseGuard {
  ...
  private initializeMetrics(): void {
    this.metrics.createCounter('requests_total', 'Total requests processed', ['tenant_id', 'status']);
    this.metrics.createCounter('anomalies_detected', 'Anomalies detected', ['tenant_id', 'type']);
    this.metrics.createCounter('throttles_applied', 'Throttles applied', ['tenant_id', 'reason']);
  ...
  public middleware() {
    return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
      ...
      return tracer.startActiveSpan('abuse_guard.middleware', async (span: Span) => {
        try {
          ...
          if (bypass) {
            logger.debug('Request bypassing abuse guard', { tenantId, bypass: true });
            this.metrics.incrementCounter('requests_total', { tenant_id: tenantId, status: 'bypassed' });
            return next();
          }
          ...
          if (this.isThrottled(tenantId)) {
            ...
            this.metrics.incrementCounter('requests_total', { tenant_id: tenantId, status: 'throttled' });
          ...
          if (anomalyResult.isAnomaly) {
            ...
            if (this.isThrottled(tenantId)) {
              this.metrics.incrementCounter('requests_total', { tenant_id: tenantId, status: 'throttled_new' });
          ...
          this.metrics.incrementCounter('requests_total', { tenant_id: tenantId, status: 'allowed' });
          next();
          ...
          this.metrics.incrementCounter('requests_total', { tenant_id: 'unknown', status: 'error' });
          next();
        }
      });
    };
  ...
  private async recordAndAnalyze(tenantId: string): Promise<AnomalyResult> {
    ...
    try {
      ...
      logger.error('Error in rate analysis', { tenantId, error: error.message });
      return {
        isAnomaly: false,
        zScore: 0,
        currentRate: 0,
  ...
  private async handleAnomaly(tenantId: string, anomaly: AnomalyResult): Promise<void> {
  ...
  private async sendAlert(tenantId: string, anomaly: AnomalyResult, throttle: ThrottleState): Promise<void> {
    try {
      const alert = {
      ...
      logger.info('Abuse alert sent', { tenantId, severity: throttle.severity });

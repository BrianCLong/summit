...
export class GraphQLAuthzPlugin {
  ...
  constructor(opaUrl = 'http://localhost:8181') {
    ...
    if (!this.enabled) {
      logger.warn(
        '⚠️  OPA authorization is DISABLED - all operations will be allowed',
      );
    }
  }
  ...
  private async queryOPA(input: OPAInput): Promise<OPADecision> {
    try {
      ...
      logger.warn(
        `Unexpected OPA response format. Result: ${JSON.stringify(result)}`,
      );
      ...
      if (config.env === 'production') {
        ...
        logger.warn(
          'Allowing operation due to OPA unavailability in development',
        );

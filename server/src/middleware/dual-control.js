...
exports.normalizeApprovalActors = void 0;
exports.validateDualControlRequirement = validateDualControlRequirement;
const approvals_js_1 = require("../db/models/approvals.js");
...
const normalizeApprovalActors = (approvals) => {
    if (!approvals)
        return [];
    return approvals.map((approval) => typeof approval === 'string' ? { user_id: approval } : approval);
};
exports.normalizeApprovalActors = normalizeApprovalActors;
async function validateDualControlRequirement(requirement) {
    const violations = [];
    const approverIds = new Set();
    const allowedRoles = requirement.allowedRoles && requirement.allowedRoles.length
        ? requirement.allowedRoles
        : DEFAULT_ALLOWED_ROLES;
    const requiredRoles = requirement.requiredRoles && requirement.requiredRoles.length
        ? requirement.requiredRoles
        : DEFAULT_REQUIRED_ROLES;
    if (requirement.approvals.length < requirement.requiredApprovals) {
        violations.push(`At least ${requirement.requiredApprovals} approvals are required`);
    }
    for (const approval of requirement.approvals) {
        ...
        const approvalTenant = approval.tenant_id || approval.attributes?.tenant_id;
    ...
    const persisted = [];
    for (const approval of requirement.approvals) {
        persisted.push((0, approvals_js_1.upsertDualControlApproval)({
    ...
    const state = await (0, approvals_js_1.getDualControlState)(requirement.requestId, requirement.action);
    const satisfied = state.distinctApprovers >= requirement.requiredApprovals &&

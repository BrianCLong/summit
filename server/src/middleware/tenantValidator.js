...
const logger = pino_1.default({ name: 'tenantValidator' });
/**
...
class TenantValidator {
    ...
    static validateTenantAccess(context, resourceTenantId, options = {}) {
        ...
        const environment = context.tenant?.environment ||
            ...
            (process.env.NODE_ENV?.startsWith('prod')
                ...
                : process.env.NODE_ENV?.startsWith('stag')
                    ? 'staging'
                    : 'dev');
        const privilegeTier = context.tenant?.privilegeTier ||
            context.user?.privilegeTier ||

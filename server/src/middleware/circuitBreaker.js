...
class CircuitBreaker extends EventEmitter {
  ...
  async executeWithTimeout(fn, args, requestStart) {
    return new Promise(async (resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        const error = new Error(`Request timeout in circuit breaker '${this.name}'`);
        error.code = 'TIMEOUT';
        reject(error);
  ...
  shouldTrip() {
    ...
    const recentRequests = this.requests.filter(req => req.timestamp > windowStart);
    const recentFailures = this.failures.filter(failure => failure.timestamp > windowStart);
  ...
  cleanup() {
    ...
    this.requests = this.requests.filter(request => request.timestamp > cutoff);
  }
  ...
  getStatus() {
    ...
    return {
      ...
      failureRate: recentRequests.length > 0 ? recentFailures.length / recentRequests.length : 0,
      nextAttempt: this.nextAttempt,
  ...
  getHealthMetrics() {
    ...
    return {
      ...
      circuit_breaker_success_rate: status.metrics.totalRequests > 0 ? 
        status.metrics.successfulRequests / status.metrics.totalRequests : 1,
      circuit_breaker_slow_requests_total: status.metrics.slowRequests,
      circuit_breaker_open_time_seconds: status.state === 'OPEN' ? 
        (Date.now() - status.metrics.circuitOpenTime) / 1000 : 0
    };
  }

...
const opa_enforcer_js_1 = require("../opa-enforcer.js");
// Mock axios
globals_1.jest.mock('axios');
const mockedAxios = axios_1.default;
// Mock budget ledger
globals_1.jest.mock('../../db/budgetLedger', () => {
    const ledger = {
        getTenantBudget: globals_1.jest.fn(),
        getSpendingSummary: globals_1.jest.fn(),
        getSpendingEntries: globals_1.jest.fn(),
        checkTenantBudget: globals_1.jest.fn(),
    };
    return {
        getBudgetLedgerManager: globals_1.jest.fn(() => ledger),
        __mockLedger: ledger,
    };
});
const budgetLedgerModule = globals_1.jest.requireMock('../../db/budgetLedger');
const mockBudgetLedger = budgetLedgerModule.__mockLedger;
const buildEnforcer = (options) => {
    const enforcer = new opa_enforcer_js_1.OPAEnforcer(options);
    enforcer.budgetLedger = mockBudgetLedger;
    return enforcer;
};
describe('OPAEnforcer', () => {
    ...
    describe('evaluatePolicy', () => {
        ...
        it('should allow operation when OPA approves', async () => {
            ...
            expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('/v1/data/intelgraph/budget/decision'), expect.objectContaining({
    ...
    describe('createMiddleware', () => {
        ...
        it('should allow request when OPA approves', async () => {
            ...
            expect(mockNext).toHaveBeenCalled();
            expect(mockRes.status).not.toHaveBeenCalled();
            expect(mockReq.opaDecision).toBeDefined();
            expect(mockReq.opaDecision.allow).toBe(true);
        });
        ...
        it('should extract mutation field name from query', async () => {
            ...
            mockReq.body = {
                query: 'mutation createComplexEntity { createComplexEntity(input: {name: "test"}) { id name } }',

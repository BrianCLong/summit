...
exports.siemFullLoggingMiddleware = exports.siemSecurityMiddleware = exports.siemMiddleware = void 0;
exports.createSIEMMiddleware = createSIEMMiddleware;
exports.siemAuthMiddleware = siemAuthMiddleware;
exports.siemPrivilegedMiddleware = siemPrivilegedMiddleware;
exports.siemDataAccessMiddleware = siemDataAccessMiddleware;
 ...
 */
function createSIEMMiddleware(options = {}) {
        ...
        try {
            ...
            res.end = function (chunk, encoding) {
                try {
                    ...
                    logger_js_1.default.error('SIEM middleware response processing error', {
            ...
            logger_js_1.default.error('SIEM middleware error', {
                component: 'SIEMMiddleware',
                error: error.message,
                path: req.path,
                method: req.method,
 ...
 */
function siemAuthMiddleware(options = {}) {
        ...
        res.json = function (body) {
            try {
                ...
                if (isAuthenticationEndpoint(req.path)) {
                    ...
                    if ((isSuccess && config.logSuccess) ||
                        ...
                        if (req.siem) {
                            ...
                            SIEMService_js_1.siemService.sendEvent(event);
                        }
                    }
                }
            }
            catch (error) {
                logger_js_1.default.error('SIEM auth middleware error', {
                    component: 'SIEMMiddleware',
                    error: error.message,
                });
            }
 ...
 */
function siemPrivilegedMiddleware() {
    return (req, res, next) => {
 ...
 */
function siemDataAccessMiddleware() {
    return (req, res, next) => {
 ...
 */
function processResponse(req, res, config) {
    if (!req.siem || req.siem.skipLogging)
        return;
    ...
    if (req.siem.events.length > 0) {
        SIEMService_js_1.siemService.sendEvents(req.siem.events).catch((error) => {
            logger_js_1.default.error('Failed to send SIEM events', {
                component: 'SIEMMiddleware',
                error: error.message,
 ...
 */
function checkRequestAnomalies(req) {
    const anomalies = [];
 ...
 */
function createRequestEvent(req, eventType, severity, additional = {}) {
    return {
        timestamp: new Date(),
        eventType,
        severity,
 ...
 */
function createAuthenticationEvent(req, res, success) {
    return {
        timestamp: new Date(),
 ...
 */
function createPrivilegedOperationEvent(req) {
    return {
        timestamp: new Date(),
 ...
 */
function createDataAccessEvent(req) {
    return {
        timestamp: new Date(),
...
function isPrivilegedOperation(req) {
...
function isDataAccessOperation(req) {
    const dataEndpoints = [
        '/api/graph/',
        '/api/entities/',
        '/api/relationships/',
...
exports.siemMiddleware = createSIEMMiddleware();
exports.siemSecurityMiddleware = createSIEMMiddleware({
    logFailedAuth: true,
    logPrivilegedOperations: true,
    logSuspiciousActivity: true,
    logDataAccess: false,
});
exports.siemFullLoggingMiddleware = createSIEMMiddleware({
    logAllRequests: true,
    logFailedAuth: true,
    logPrivilegedOperations: true,
    logDataAccess: true,
...
exports.default = createSIEMMiddleware;

...
class PIIDetector {
    ...
    customPatterns = [];
    constructor() {
        logger_js_1.default.info('PII detector initialized');
    }
    // --------------------------------------------------------------------------
    // Scanning
    // --------------------------------------------------------------------------
    async scanText(text, options) {
        ...
        logger_js_1.default.info({ hasPI: result.hasPI, detectionCount: result.detections.length }, 'PII scan completed');
        return {
            data: result,
            provenance: {
            ...
            governance: {
                verdict: result.hasPI ? data_envelope_js_1.GovernanceResult.FLAG : data_envelope_js_1.GovernanceResult.ALLOW,
                evaluatedPolicies: [],
    ...
    async scanObject(obj, options) {
        const startTime = Date.now();
        const detections = [];
        const maxDepth = options?.maxDepth ?? 10;
        this.scanObjectRecursive(obj, detections, '', 0, maxDepth, options?.includeValue);
        const result = this.buildScanResult(detections, startTime);
        logger_js_1.default.info({ hasPI: result.hasPI, detectionCount: result.detections.length }, 'PII object scan completed');
        return {
            data: result,
            provenance: {
                sources: [{ id: 'pii-detector', type: 'system' }],
    ...
    scanObjectRecursive(obj, detections, path, depth, maxDepth, includeValue) {
        if (depth > maxDepth)
            return;
        if (typeof obj === 'string') {
            ...
            obj.forEach((item, index) => {
                this.scanObjectRecursive(item, detections, `${path}[${index}]`, depth + 1, maxDepth, includeValue);
            });
        }
    ...
    scanStringValue(value, path, detections, includeValue) {
        const allPatterns = [...this.patterns, ...this.customPatterns];
        for (const pattern of allPatterns) {
    ...
    checkFieldName(fieldName, path, detections) {
    ...
    addCustomPattern(pattern) {
        ...
        logger_js_1.default.info({ category: pattern.category }, 'Custom PII pattern added');
    }

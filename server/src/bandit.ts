...
export async function pickVariant(runbook:string, stepId:string, variants:{key:string}[], rewardPrior={alpha:1,beta:1}) {
  const { rows } = await pg.query(`SELECT variant_key, alpha, beta FROM bandit_state WHERE runbook=$1 AND step_id=$2`, [runbook, stepId]);
  const state = new Map(rows.map((r:any)=>[r.variant_key, { alpha:Number(r.alpha)||rewardPrior.alpha, beta:Number(r.beta)||rewardPrior.beta }]));
  let best = variants[0]?.key, bestSample = -1;
  for (const v of variants) {
    const s = state.get(v.key) || rewardPrior;
    const sample = sampleBeta(s.alpha, s.beta);
    if (sample > bestSample) { bestSample = sample; best = v.key; }
  }
  return best;
}

export async function updateVariant(runbook:string, stepId:string, variantKey:string, reward:number){
  await pg.query(
...
function sampleBeta(a:number,b:number){ return gamma(a) / (gamma(a) + gamma(b)); }
function gamma(k:number){
  ...
  for(;;){
    ...
    if (Math.log(Math.random()) < 0.5*x*x + d*(1 - v + Math.log(v))) return d*v;
  }
}

...
exports.GtmMessagingService = void 0;
const crypto_1 = require("crypto");
const prom_client_1 = require("prom-client");
const claims_repository_js_1 = require("./claims-repository.js");
const types_js_1 = require("./types.js");
class GtmMessagingService {
    repository;
    static getOrCreateCounter(config) {
        const existing = prom_client_1.register.getSingleMetric(config.name);
        if (existing)
            return existing;
        return new prom_client_1.Counter(config);
    }
    static getOrCreateGauge(config) {
        const existing = prom_client_1.register.getSingleMetric(config.name);
        if (existing)
            return existing;
        return new prom_client_1.Gauge(config);
    }
    static getOrCreateHistogram(config) {
        const existing = prom_client_1.register.getSingleMetric(config.name);
        if (existing)
            return existing;
        return new prom_client_1.Histogram(config);
    }
    static claimsSubmitted = GtmMessagingService.getOrCreateCounter({
        name: 'gtm_claims_submitted_total',
        help: 'Count of claims submitted by risk tier',
        labelNames: ['riskTier'],
    });
    static claimsApproved = GtmMessagingService.getOrCreateCounter({
        name: 'gtm_claims_approved_total',
        help: 'Count of claims approved by risk tier',
        labelNames: ['riskTier'],
    });
    static claimsExpired = GtmMessagingService.getOrCreateCounter({
        name: 'gtm_claims_expired_total',
        help: 'Count of claims that hit expiry',
        labelNames: ['riskTier'],
    });
    static approvalDuration = GtmMessagingService.getOrCreateHistogram({
        name: 'gtm_claim_approval_duration_seconds',
        help: 'Time from submission to approval',
        labelNames: ['riskTier'],
        buckets: [0.5, 1, 3, 6, 12, 24, 48, 72, 96],
    });
    static activeApprovedClaims = GtmMessagingService.getOrCreateGauge({
    ...
    constructor(repository = new claims_repository_js_1.ClaimsRepository()) {
        this.repository = repository;
    }
    async submitClaim(input) {
        const parsed = types_js_1.ClaimInputSchema.parse(input);
        ...
        const claim = {
            ...
            status: types_js_1.ClaimStatus.Pending,
            reviewDate,
            owner: parsed.owner,
    ...
    async recordApproval(claimId, approver, notes) {
        ...
        if (claim.status === types_js_1.ClaimStatus.Expired) {
        ...
        const approvalsMet = approvalsNeeded.every((required) => claim.approvals.some((approval) => approval.approver === required));
        if (approvalsMet) {
            claim.status = types_js_1.ClaimStatus.Approved;
    ...
    async expireClaims(currentDate = new Date()) {
        const claims = await this.repository.loadClaims();
        const updated = claims.map((claim) => {
            if (claim.status === types_js_1.ClaimStatus.Approved && new Date(claim.reviewDate) < currentDate) {
                GtmMessagingService.claimsExpired.inc({ riskTier: claim.riskTier });
                return { ...claim, status: types_js_1.ClaimStatus.Expired, expiry: currentDate.toISOString(), updatedAt: currentDate.toISOString() };
            }
            return claim;
        });
        await this.repository.saveClaims(updated);
        await this.updateActiveClaimsGauge();
        return updated.filter((claim) => claim.status === types_js_1.ClaimStatus.Expired);
    }
    async listClaimsForChannel(channel) {
        const claims = await this.repository.loadClaims();
        return claims.filter((claim) => claim.status === types_js_1.ClaimStatus.Approved && claim.channels.includes(channel));
    }
    ...
    evaluateChannelPerformance(input) {
        ...
        const healthy = input.cac <= playbook.cacTarget &&
    ...
    async closedLoopQa(content) {
        ...
        const approvedMessages = claims
            .filter((claim) => claim.status === types_js_1.ClaimStatus.Approved)
    ...
    buildExecutionChecklist(claims) {
        return [
            ...
            },
            {
                ...
                completed: claims.some((claim) => claim.status === types_js_1.ClaimStatus.Approved),
            },
            {
    ...
    buildEvidenceGraph(claims) {
        return claims
            .filter((claim) => claim.status === types_js_1.ClaimStatus.Approved)
            .flatMap((claim) => claim.channels.map((channel) => ({
    ...
    computeEnforcementGates(claim) {
        ...
        if (claim.riskTier === types_js_1.RiskTier.High) {
            gates.push('legal approval', 'security approval', 'pmm final approval');
        }
        else if (claim.riskTier === types_js_1.RiskTier.Medium) {
    ...
    requiredApprovals(claim) {
        if (claim.riskTier === types_js_1.RiskTier.High || claim.forwardLooking) {
            return ['legal', 'security', 'pmm'];
        }
        if (claim.riskTier === types_js_1.RiskTier.Medium) {
            return ['pmm'];
        }
        return ['pmm'];
    }
    calculateRiskTier(input) {
        if (input.forwardLooking || input.complianceSurface?.length) {
            return types_js_1.RiskTier.High;
        }
        if (input.channels.includes('web') && input.evidenceType === types_js_1.EvidenceType.DemoVideo) {
            return types_js_1.RiskTier.Medium;
        }
        return types_js_1.RiskTier.Low;
    }
    computeReviewDate(riskTier, now) {
        const days = riskTier === types_js_1.RiskTier.High ? 90 : riskTier === types_js_1.RiskTier.Medium ? 120 : 180;
    ...
    async updateActiveClaimsGauge() {
        const claims = await this.repository.loadClaims();
        const approved = claims.filter((claim) => claim.status === types_js_1.ClaimStatus.Approved);
        const counts = {
            web: 0,
            sales: 0,
            content: 0,
...
exports.GtmMessagingService = GtmMessagingService;

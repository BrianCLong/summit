...
exports.narrativeConflictService = exports.NarrativeConflictService = exports.defaultNarrativeConfig = void 0;
...
// Narrative Conflict Service
// =============================================================================
class NarrativeConflictService extends events_1.EventEmitter {
    config;
    monitoringInterval;
    initialized = false;
    constructor(config = {}) {
        super();
        this.config = { ...exports.defaultNarrativeConfig, ...config };
    }
    /**
     ...
     */
    async getOrCreateCluster(tenantId, theme, keywords) {
     ...
     */
    async addContentToCluster(clusterId, tenantId, item) {
        await this.ensureInitialized();
     ...
     * Get a cluster by ID
     */
    async getCluster(clusterId, tenantId) {
        await this.ensureInitialized();
        const result = await pg_js_1.pool.query(`SELECT * FROM pig_narrative_clusters WHERE id = $1 AND tenant_id = $2`, [clusterId, tenantId]);
     ...
     */
    async listClusters(tenantId, options = {}) {
        await this.ensureInitialized();
        ...
        const countResult = await pg_js_1.pool.query(`SELECT COUNT(*) FROM pig_narrative_clusters WHERE ${conditions.join(' AND ')}`, params);
        const dataResult = await pg_js_1.pool.query(`SELECT * FROM pig_narrative_clusters
     ...
     */
    async updateClusterStatus(clusterId, tenantId, status, userId) {
        await this.ensureInitialized();
        const cluster = await this.getCluster(clusterId, tenantId);
        if (!cluster) {
            throw new Error(`Cluster not found: ${clusterId}`);
     ...
     */
    async assessClusterRisk(cluster) {
        const factors = [];
        let overallScore = 0;
     ...
     */
    async evaluateDSASystemicRisk(cluster) {
        const riskTypes = [];
    ...
    async getDashboardSummary(tenantId) {
        await this.ensureInitialized();
        const [activeClusters, highRiskClusters, recentActivity, topThemes, riskTrend,] = await Promise.all([
     ...
     */
    async getClusterTimeline(clusterId, tenantId, options = {}) {
        await this.ensureInitialized();
        const cluster = await this.getCluster(clusterId, tenantId);
        if (!cluster) {
            throw new Error(`Cluster not found: ${clusterId}`);
    ...
    async runMonitoringCycle() {
        ...
        const result = await pg_js_1.pool.query(`SELECT tenant_id, status, risk_score, COUNT(*) as count
       FROM pig_narrative_clusters
     ...
     */
    async findSimilarCluster(tenantId, theme, keywords) {
        const timeWindow = new Date(Date.now() - this.config.clusteringConfig.timeWindowHours * 60 * 60 * 1000);
        const result = await pg_js_1.pool.query(`SELECT * FROM pig_narrative_clusters
       WHERE tenant_id = $1
       AND status = 'active'
       AND last_activity >= $2
       AND (theme ILIKE $3 OR keywords && $4)
       ORDER BY last_activity DESC
       LIMIT 1`, [tenantId, timeWindow, `%${theme}%`, keywords]);
        if (result.rows.length === 0) {
            return null;
        }
        return this.mapRowToCluster(result.rows[0]);
     ...
     */
    async calculateSentiment(items) {
    ...
    async storeCluster(cluster) {
        await pg_js_1.pool.query(`INSERT INTO pig_narrative_clusters (
    ...
    async updateCluster(cluster) {
        await pg_js_1.pool.query(`UPDATE pig_narrative_clusters SET
        theme = $3,
        keywords = $4,
        sentiment = $5,
        content_items = $6,
      ...
      WHERE id = $1 AND tenant_id = $2`, [
        ...
        // Update metrics
        narrativeRiskScore.observe({ tenant_id: cluster.tenantId }, cluster.riskAssessment.overallScore);
    }
    /**
     ...
     */
    async getActiveClusterCount(tenantId) {
        const result = await pg_js_1.pool.query(`SELECT COUNT(*) FROM pig_narrative_clusters WHERE tenant_id = $1 AND status = 'active'`, [tenantId]);
        return parseInt(result.rows[0].count);
    }
    /**
     * Get high risk cluster count
     */
    async getHighRiskClusterCount(tenantId) {
        const result = await pg_js_1.pool.query(`SELECT COUNT(*) FROM pig_narrative_clusters
       WHERE tenant_id = $1 AND status = 'active' AND risk_score >= 70`, [tenantId]);
        return parseInt(result.rows[0].count);
    }
    /**
     * Get recent activity
    ...
    async getRecentActivity(tenantId) {
        const result = await pg_js_1.pool.query(`SELECT id, theme, last_activity, risk_score
       FROM pig_narrative_clusters
       WHERE tenant_id = $1
       ORDER BY last_activity DESC
       LIMIT 10`, [tenantId]);
     ...
     * Get top themes
     */
    async getTopThemes(tenantId) {
        const result = await pg_js_1.pool.query(`SELECT theme, COUNT(*) as count, AVG(risk_score) as avg_risk
       FROM pig_narrative_clusters
     ...
     */
    async getDSARiskSummary(tenantId) {
        const result = await pg_js_1.pool.query(`SELECT risk_assessment
       FROM pig_narrative_clusters
       WHERE tenant_id = $1 AND status = 'active'`, [tenantId]);
...
exports.NarrativeConflictService = NarrativeConflictService;
// Export default instance
exports.narrativeConflictService = new NarrativeConflictService();

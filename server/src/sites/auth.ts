...
export async function verifySiteAuth(req: any, res: any, next: any) {
  try {
    const siteId = (req.body?.siteId || req.headers['x-site-id'] || '').toString();
    ...
    const v = crypto.createVerify('RSA-SHA256'); v.update(body); v.end();
    if (!v.verify(s.trust_pubkey, Buffer.from(sig, 'base64'))) return res.status(401).json({ error: 'bad sig' });
    (req as any).siteId = siteId;
    next();
  } catch (e) { return res.status(400).json({ error: 'auth failed' }); }
}

...
class SecretsManager {
    ...
    async setRotationPolicy(policy) {
        ...
        const nextRotation = this.calculateNextRotation(policy.lastRotatedAt || new Date(), policy.rotationIntervalDays);
     ...
     */
    async getRotationPolicy(secretId) {
        if (!this.pool) {
            return undefined;
        }
        const result = await this.pool.query('SELECT * FROM secret_rotation_policies WHERE secret_id = $1', [secretId]);
     ...
     */
    async listRotationPolicies(enabledOnly = false) {
        if (!this.pool) {
            return [];
        }
     ...
     */
    async rotateSecret(secretId, newValue, actor) {
        ...
        `, [expiresAt, secretId, currentVersion.version]);
            ...
            const nextRotation = this.calculateNextRotation(new Date(), policy.rotationIntervalDays);
            await this.pool.query(`
     ...
     */
    async getCurrentVersion(secretId) {
        if (!this.pool) {
            return undefined;
        }
     ...
     */
    async revokeVersion(secretId, version) {
        if (!this.pool) {
            return;
        }
        await this.pool.query(`
    ...
    async storeSecretVersion(version) {
        if (!this.pool) {
            return;
        }
    ...
    calculateNextRotation(lastRotation, intervalDays) {

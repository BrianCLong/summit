...
import { PostgresAgentContextPersistence } from '../services/llm/AgentContextPersistence.js';
...
export const agentContextDefaults = {
  ...
  maxCostUsd: process.env.AGENT_CONTEXT_MAX_COST
    ? Number(process.env.AGENT_CONTEXT_MAX_COST)
    : undefined,
...
export function buildAgentContextManager(): AgentContextManager {
  return new AgentContextManager({
    ...
    persistence: new PostgresAgentContextPersistence(),
  });
}

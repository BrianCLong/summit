...
import { contractTesting, ContractTestResult } from '../testing/contractTesting';
import { migrationFramework, MigrationExecution } from '../migrations/migrationFramework';
...
export type GateType = 'pre_migration' | 'post_migration' | 'continuous' | 'rollback_trigger';
export type GateResult = 'pass' | 'fail' | 'warn' | 'error';
export type RollbackTrigger = 'error_rate' | 'performance_degradation' | 'contract_violation' | 'manual';
...
export interface GateCondition {
  id: string;
  name: string;
  type: 'contract_test' | 'health_check' | 'metric_threshold' | 'manual_approval' | 'dependency_check';
...
export interface RollbackCondition {
  ...
  actions: Array<{
    type: 'immediate_rollback' | 'graceful_rollback' | 'alert_only' | 'circuit_breaker';
    config: Record<string, any>;
  }>;
}
...
export class MigrationGatesSystem extends EventEmitter {
  ...
  async executeGate(
    ...
    return tracer.startActiveSpan('migration_gates.execute_gate', async (span: Span) => {
      span.setAttributes({
        'tenant_id': tenantId,
  ...
  private async executeCondition(
    ...
    tenantId: string,
    context?: any
  ): Promise<{ conditionId: string; result: GateResult; value?: any; message?: string; executionTime: number }> {
    const startTime = Date.now();
    ...
    try {
      ...
      switch (condition.type) {
        case 'contract_test':
          ...
          result = contractResult.passed === contractResult.failed + contractResult.errors ? 'pass' : 'fail';
  ...
  private async executeContractTest(config: any, tenantId: string): Promise<ContractTestResult> {
    if (config.contractId) {
      return await contractTesting.testContract(config.contractId, tenantId);
    } else if (config.contractType) {
      const results = await contractTesting.testAllContracts(tenantId, config.contractType);
  ...
  private async executeHealthCheck(config: any, tenantId: string): Promise<{ healthy: boolean; message?: string; details?: any }> {
      ...
      try {
        switch (service) {
          case 'database':
            ...
            results.push({ service, healthy: dbHealth.healthy, details: dbHealth });
            break;
          case 'cache':
            // Check Redis connectivity
            const cacheHealth = await this.checkCacheHealth();
            results.push({ service, healthy: cacheHealth.healthy, details: cacheHealth });
            break;
          case 'queue':
            // Check message queue health
            results.push({ service: 'queue', healthy: true, details: { status: 'ok' } });
            break;
        }
      } catch (error) {
        results.push({ service, healthy: false, details: { error: (error as Error).message } });
      }
    }
  ...
  private async executeMetricThreshold(config: any, tenantId: string): Promise<{ withinThreshold: boolean; value: number; threshold: number; metric: string }> {
  ...
  private async checkManualApproval(config: any, migrationId: string, tenantId: string): Promise<{ approved: boolean; approver?: string; approvedAt?: Date }> {
  ...
  private async executeDependencyCheck(config: any, tenantId: string): Promise<{ allHealthy: boolean; healthyCount: number; totalCount: number; details: any[] }> {
  ...
  private async executeActions(gate: MigrationGate, execution: GateExecution): Promise<void> {
    const applicableActions = gate.actions.filter(action => {
      switch (action.condition) {
        case 'on_pass': return execution.result === 'pass';
        case 'on_fail': return execution.result === 'fail';
        case 'on_warn': return execution.result === 'warn';
  ...
  private async triggerRollback(migrationId: string, tenantId: string, reason: string): Promise<void> {
    try {
  ...
  private createSkippedExecution(gate: MigrationGate, migrationId: string, tenantId: string, reason: string): GateExecution {
    return {
      gateId: gate.id,
      migrationId,
      tenantId,
  ...
  private validateGate(gate: MigrationGate): void {
    ...
    for (const condition of gate.conditions) {
      ...
      if (condition.weight < 0) throw new Error('Condition weight must be non-negative');
    }
  }
  ...
  private async evaluateRollbackCondition(condition: RollbackCondition, migration: MigrationExecution): Promise<boolean> {
    // Implementation would check actual metrics and thresholds
    // For now, return false to avoid automatic rollbacks
    return false;
  }

  async executePreMigrationGates(migrationId: string, tenantId: string): Promise<boolean> {
    ...
    for (const gate of preGates) {
      ...
      if (execution.result === 'fail' && gate.settings.failureBehavior === 'block') {
        return false;
      }
    }
  ...
  async executePostMigrationGates(migrationId: string, tenantId: string): Promise<boolean> {
  ...
  getExecutionHistory(migrationId?: string, tenantId?: string): GateExecution[] {
  ...
  generateGateReport(executions: GateExecution[]): {
    ...
    const averageScore = totalExecutions > 0 ? 
      executions.reduce((sum, e) => sum + e.score, 0) / totalExecutions : 0;
    const rollbacksTriggered = executions.filter(e => e.rollbackTriggered).length;

    // Analyze common failure reasons
    const failureReasons = new Map<string, number>();
    executions.filter(e => e.result === 'fail').forEach(e => {

...
exports.TenantContextService = exports.tenantContextService = void 0;
const graphql_1 = require("graphql");
const api_1 = require("@opentelemetry/api");
const pg_js_1 = require("../db/pg.js");
const logger_js_1 = __importDefault(require("../config/logger.js"));
const tracer = api_1.trace.getTracer('tenant-context', '24.3.0');
const logger = logger_js_1.default.child({ name: 'tenantContext' });
class TenantContextService {
    tenantCache = new Map();
    CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes
    async resolveTenantContext(tenantId, currentRegion = process.env.CURRENT_REGION || 'us-east-1') {
        return tracer.startActiveSpan('tenant_context.resolve', async (span) => {
            span.setAttributes({
                tenant_id: tenantId,
            ...
            try {
                ...
                const tenantRecord = (await pg_js_1.pg.oneOrNone(`SELECT
             tenant_id,
             region_tag,
             residency_region,
             residency_class,
           ...
           WHERE tenant_id = $1`, [tenantId], { tenantId }));
                if (!tenantRecord) {
                    ...
                    throw new graphql_1.GraphQLError('Tenant not found', {
                        extensions: {
                ...
                if (!tenantRecord.is_active) {
                    ...
                    throw new graphql_1.GraphQLError('Tenant is inactive', {
                        extensions: {
                ...
                const tenantContext = {
                    ...
                    residency: {
                        region: tenantRecord.residency_region ||
    ...
    validateRegionAccess(tenantContext, targetRegion, operation, hasExportToken = false) {
        return tracer.startActiveSpan('tenant_context.validate_region_access', (span) => {
            span.setAttributes({
            ...
            try {
                ...
                if (operation === 'export') {
                    if (tenantContext.residency.exportRestrictions?.requiresApproval &&
                        !hasExportToken) {
                        span.setAttributes({
                            access_granted: false,
                ...
                if (tenantContext.residency.class === 'restricted' &&
                    !hasExportToken) {
                    span.setAttributes({
                        access_granted: false,
    ...
    validateDataClassification(tenantContext, dataClassifications, operation) {
        ...
        if (!tenantClassifications.length) {
            return (!dataClassifications.includes('restricted') &&
    ...
    static async validateTenantAccess(context, resourceTenantId, options = {}) {
        const service = new TenantContextService();
        const { requireExplicitTenant = true, allowSystemAccess = false, validateOwnership = true, allowCrossRegion = false, requireResidencyCompliance = true, } = options;
        // Extract user context from request
        const userTenantId = context.user?.tenantId;
        const userId = context.user?.id;
        const roles = context.user?.roles || [];
        const currentRegion = context.headers?.['x-ig-region'] ||
        ...
        const effectiveTenantId = resourceTenantId || userTenantId;
        if (!effectiveTenantId) {
            throw new graphql_1.GraphQLError('Unable to determine tenant context', {
                extensions: {
        ...
        if (requireResidencyCompliance && !allowCrossRegion) {
            ...
            const regionAccessValid = service.validateRegionAccess(tenantContext, currentRegion, operation, hasExportToken);
            if (!regionAccessValid) {
                logger.error(`Region access denied: Tenant ${effectiveTenantId} attempted ${operation} in region ${currentRegion}`);
                throw new graphql_1.GraphQLError('Region access denied', {
                    extensions: {
...
exports.tenantContextService = new TenantContextService();

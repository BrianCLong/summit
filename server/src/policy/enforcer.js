...
exports.policyEnforcer = exports.PolicyEnforcer = void 0;
// @ts-nocheck
const api_1 = require("@opentelemetry/api");
const prom_client_1 = require("prom-client");
const pubsub_js_1 = require("../subscriptions/pubsub.js");
const audit_logger_js_1 = require("../../security/zero-trust/siem/audit-logger.js");
const tracer = api_1.trace.getTracer('policy-enforcer', '24.2.0');
// Metrics
const policyDecisions = new prom_client_1.Counter({
    name: 'policy_decisions_total',
    help: 'Total OPA policy decisions',
    labelNames: ['tenant_id', 'action', 'result'],
});
const policyLatency = new prom_client_1.Histogram({
    name: 'policy_decision_latency_ms',
    help: 'OPA policy decision latency',
    buckets: [1, 5, 10, 25, 50, 100, 200, 500],
    labelNames: ['cached'],
});
const purposeViolations = new prom_client_1.Counter({
...
class PolicyEnforcer {
    cachePrefix = 'policy_cache';
    defaultTTL = 300; // 5 minutes
    provenanceLog = [];
    auditLogger;
    constructor(auditLogger = (0, audit_logger_js_1.createZeroTrustAuditLogger)()) {
        this.auditLogger = auditLogger;
    }
    async enforce(context) {
        return tracer.startActiveSpan('policy.enforce', async (span) => {
            span.setAttributes({
    ...
    async requirePurpose(purpose, context) {
        if (!context.purpose) {
     ...
     */
    async validateReasonForAccess(context) {
    ...
    shouldRequireReason(context) {
        ...
        if (context.action === 'read' && context.queryMetadata) {
            const expansionRatio = (context.queryMetadata.expandedSelectors || 0) /
     ...
     */
    async validateSelectorExpansion(context, maxExpansionRatio = 10.0) {
    ...
    async setCache(key, decision, ttl) {
        try {
            await pubsub_js_1.redis.setWithTTL(key, JSON.stringify(decision), ttl);
        }
        catch (error) {
            console.error('Policy cache write error:', error);
        }
    }
    async recordProvenance(context, decision) {
        const entry = {
    ...
    async emitDecisionEvent(entry, context, decision) {
        const event = {
            id: entry.id,
    ...
    getProvenanceLog(tenantId, limit = 100) {
...
exports.policyEnforcer = new PolicyEnforcer();

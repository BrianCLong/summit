...
const residencyViolations = new Counter({
  ...
  labelNames: ['tenant_id', 'source_region', 'target_region', 'operation', 'violation_type']
});

const crossRegionRequests = new Counter({
  name: 'cross_region_requests_total',
  help: 'Total cross-region requests',
  labelNames: ['tenant_id', 'source_region', 'target_region', 'operation', 'allowed']
});
...
export class ResidencyPolicyEnforcer {
  ...
  constructor(currentRegion: string = process.env.CURRENT_REGION || 'us-east-1') {
  ...
  async enforceResidencyPolicy(
    ...
    return tracer.startActiveSpan('residency.enforce_policy', async (span: Span) => {
      ...
      try {
        ...
        return {
          ...
          auditRequired: true,
          denialReasons: [`Policy evaluation error: ${(error as Error).message}`]
        };
      } finally {
        span.end();
      }
  ...
  private async evaluateResidencyPolicy(
    ...
    return {
      allow,
      reason: allow ? 'Residency policy allows access' : 'Residency policy denies access',
      denialReasons: denialReasons.length > 0 ? denialReasons : undefined,
      requiresExportToken: !allow && requiresExportToken,
      auditRequired: !allow || context.operation === 'export' || targetRegion !== tenantContext.regionTag,
      allowedOperations: allow ? undefined : this.getAllowedOperations(context, tenantContext),
      regionRestrictions: allow ? undefined : tenantContext.residency.allowedRegions
    };
  }
  ...
  private evaluateStandardAccess(
    ...
    if (context.operation === 'read' && tenantContext.residency.allowedRegions.includes(targetRegion)) {
      return true;
    }
  ...
  private async validateExportToken(
    ...
    context: ResidencyPolicyContext,
    tenantContext: TenantContext
  ): Promise<{ valid: boolean; reason?: string; payload?: ExportTokenPayload }> {
    try {
      // In production, this would use JWT verification with proper secret
      const payload = JSON.parse(Buffer.from(token.split('.')[1], 'base64').toString()) as ExportTokenPayload;
      ...
      if (context.targetRegion && !payload.allowedRegions.includes(context.targetRegion)) {
  ...
  private buildCacheKey(context: ResidencyPolicyContext, tenantContext: TenantContext): string {
  ...
  private async getFromCache(key: string): Promise<ResidencyPolicyDecision | null> {
    try {
  ...
  private async setCache(key: string, decision: ResidencyPolicyDecision, ttl: number): Promise<void> {
    try {

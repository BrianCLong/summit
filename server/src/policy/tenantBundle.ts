...
export function simulatePolicyDecision(
  ...
  for (const window of profile.freezeWindows || []) {
    ...
    const requiresBreakGlass = window.requireBreakGlass !== false;
    const usesBreakGlass = Boolean(input.breakGlass);
    if (requiresBreakGlass && !usesBreakGlass) {
    ...
    if (usesBreakGlass && window.requireReceipt !== false && !input.breakGlassReceiptId) {

...
class EvaluationHarness {
    ...
    async runEvaluation(request) {
        ...
        if (timeoutMs > 30000)
            throw new Error("Timeout exceeds hard cap of 30s");
    ...
    async executeLogic(request, context) {
    ...
    async evaluateCriteria(criteria, target) {
    ...
    createFailureReport(request, error, context, startTime) {
        return {
            id: (0, uuid_1.v4)(),
            timestamp: new Date(),
            agentId: request.agentId,

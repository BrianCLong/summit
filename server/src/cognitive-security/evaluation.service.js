...
exports.EvaluationService = exports.BENCHMARK_TARGETS = void 0;
exports.createEvaluationService = createEvaluationService;
exports.initializeEvaluationService = initializeEvaluationService;
exports.getEvaluationService = getEvaluationService;
const crypto_1 = require("crypto");
const pino_1 = __importDefault(require("pino"));
const logger = pino_1.default({ name: 'cogsec-evaluation-service' });
// ============================================================================
// Benchmark Targets
// ============================================================================
exports.BENCHMARK_TARGETS = {
    detection: {
...
class EvaluationService {
     ...
     */
    async calculateDetectionMetrics(startDate, endDate) {
        ...
        `, { startDate, endDate });
            ...
            const detectionTimes = record?.get('detectionTimes') || [];
        ...
        `, { startDate, endDate });
            ...
            const unattributedSignals = signalRecord?.get('unattributedSignals')?.toNumber() || 0;
            return {
                ...
                falsePositiveRate: totalSignals > 0 ? unattributedSignals / totalSignals : 0,
            };
        }
        finally {
            await session.close();
     ...
     */
    async trackCampaignDetection(campaignId, detectedAt, firstClaimAt) {
        ...
        const meetsBenchmark = timeToDetectMs <= exports.BENCHMARK_TARGETS.detection.timeToDetectP50;
        // Store metric event
        ...
        logger.info({ campaignId, timeToDetectMs, meetsBenchmark }, 'Tracked campaign detection');
     ...
     */
    async calculateVerificationMetrics(startDate, endDate, groundTruth) {
        const session = this.getSession();
        try {
        ...
        `, { startDate, endDate });
            ...
            if (groundTruth && groundTruth.size > 0) {
                ...
                const avgConfidence = claims.reduce((sum, c) => sum + (c.verdictConfidence || 0), 0) /
        ...
        `, { startDate, endDate });
            ...
            return {
                ...
                claimRecall: recall,
                citationCorrectness,
                falseAttributionRate: claims.length > 0 ? falseAttributionCount / claims.length : 0,
     ...
     */
    async trackClaimVerification(claimId, verdict, confidence, verificationTimeMs, citationCount) {
     ...
     */
    async calculateResponseMetrics(startDate, endDate) {
        const session = this.getSession();
        try {
        ...
        `, { startDate, endDate });
            ...
            return {
                ...
                takedownSuccessRate: totalTakedowns > 0 ? approvedTakedowns / totalTakedowns : 0,
            };
        }
        finally {
            await session.close();
     ...
     */
    async trackNarrativeContainment(narrativeId, initialSpreadRate, finalSpreadRate, channelsBefore, channelsAfter) {
        const spreadRateReduction = initialSpreadRate > 0
            ? ((initialSpreadRate - finalSpreadRate) / initialSpreadRate) * 100
            : 0;
        const channelReduction = channelsBefore > 0
     ...
     */
    async calculateOperatorMetrics(startDate, endDate) {
        const session = this.getSession();
        try {
        ...
        `, { startDate, endDate });
            ...
            const periodHours = (new Date(endDate).getTime() - new Date(startDate).getTime()) /
                (1000 * 60 * 60);
            ...
            return {
                ...
                claimsPerAnalystHour: analystHours > 0 ? totalClaims / analystHours : 0,
                playbooksPerIncident: playbookRecord?.get('avgPlaybooksPerIncident') || 0,
                avgResolutionTimeMs: (incidentRecord?.get('avgResolutionTime') || 0) * 60 * 1000,
            };
        }
        finally {
            await session.close();
     ...
     */
    async trackAnalystActivity(analystId, activityType, durationMinutes, details) {
     ...
     */
    async calculateAllMetrics(startDate, endDate, groundTruth) {
        const [detection, verification, response, operatorEfficiency] = await Promise.all([
    ...
    compareToBenchmarks(metrics) {
        const compareMetric = (value, target, lowerIsBetter = false) => ({
            value,
            target,
            met: lowerIsBetter ? value <= target : value >= target,
        });
        const detection = {
            timeToDetectP50: compareMetric(metrics.detection.timeToDetectP50, exports.BENCHMARK_TARGETS.detection.timeToDetectP50, true),
            timeToDetectP95: compareMetric(metrics.detection.timeToDetectP95, exports.BENCHMARK_TARGETS.detection.timeToDetectP95, true),
            falsePositiveRate: compareMetric(metrics.detection.falsePositiveRate, exports.BENCHMARK_TARGETS.detection.falsePositiveRate, true),
        };
        const verification = {
            claimPrecision: compareMetric(metrics.verification.claimPrecision, exports.BENCHMARK_TARGETS.verification.claimPrecision),
            claimRecall: compareMetric(metrics.verification.claimRecall, exports.BENCHMARK_TARGETS.verification.claimRecall),
            citationCorrectness: compareMetric(metrics.verification.citationCorrectness, exports.BENCHMARK_TARGETS.verification.citationCorrectness),
            falseAttributionRate: compareMetric(metrics.verification.falseAttributionRate, exports.BENCHMARK_TARGETS.verification.falseAttributionRate, true),
        };
        const response = {
            narrativeContainmentRate: compareMetric(metrics.response.narrativeContainmentRate, exports.BENCHMARK_TARGETS.response.narrativeContainmentRate),
            avgGrowthRateReduction: compareMetric(metrics.response.avgGrowthRateReduction, exports.BENCHMARK_TARGETS.response.avgGrowthRateReduction),
            takedownSuccessRate: compareMetric(metrics.response.takedownSuccessRate, exports.BENCHMARK_TARGETS.response.takedownSuccessRate),
        };
        const operator = {
            minutesPerIncident: compareMetric(metrics.operatorEfficiency.minutesPerIncident, exports.BENCHMARK_TARGETS.operator.minutesPerIncident, true),
            claimsPerAnalystHour: compareMetric(metrics.operatorEfficiency.claimsPerAnalystHour, exports.BENCHMARK_TARGETS.operator.claimsPerAnalystHour),
        };
     ...
     */
    async getMetricsTrends(metricName, startDate, endDate, interval = 'day') {
        const session = this.getSession();
        try {
            const result = await session.run(`
    ...
    async generateRiskAssessment() {
        ...
        if (comparison.operator.minutesPerIncident.met === false) {
            risks.governanceRisks.push('Incident resolution exceeds target SLA, indicating potential staffing or process issues');
        }
        // MAP: Context and risk mapping
        if (comparison.detection.falsePositiveRate.met === false) {
            risks.mapRisks.push('High false positive rate may lead to alert fatigue and missed real threats');
        }
        if (comparison.verification.falseAttributionRate.met === false) {
            risks.mapRisks.push('CRITICAL: False attribution rate exceeds threshold - risk of incorrectly attributing content');
        }
        // MEASURE: Performance measurement
        if (comparison.verification.claimPrecision.met === false) {
            risks.measureRisks.push('Claim verification precision below target - review verification criteria');
        }
        if (comparison.detection.timeToDetectP95.met === false) {
            risks.measureRisks.push('Detection time (p95) exceeds target - consider improving detection algorithms');
        }
        // MANAGE: Risk management
        if (comparison.response.narrativeContainmentRate.met === false) {
            risks.manageRisks.push('Narrative containment below target - review response strategies');
        }
        // Calculate overall risk level
        const criticalRisks = risks.mapRisks.filter((r) => r.includes('CRITICAL')).length;
    ...
    // Storage
    // ==========================================================================
    async storeMetricEvent(category, data) {
        const session = this.getSession();
        try {
            await session.run(`
        ...
        try {
            ...
            return {
                ...
                details: {
                    ...
                    benchmarkTargets: exports.BENCHMARK_TARGETS,
                },
            };
        }
        catch (error) {
...
function createEvaluationService(config) {
    return new EvaluationService(config);
}
function initializeEvaluationService(config) {

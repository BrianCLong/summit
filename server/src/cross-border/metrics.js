...
exports.CrossBorderMetrics = void 0;
exports.getCrossBorderMetrics = getCrossBorderMetrics;
exports.recordHandover = recordHandover;
exports.recordTranslation = recordTranslation;
exports.recordPartnerHealth = recordPartnerHealth;
exports.updateActiveSessions = updateActiveSessions;
exports.updateActivePartners = updateActivePartners;
/**
 * Cross-border metrics collector
 */
class CrossBorderMetrics {
    counters = new Map();
    gauges = new Map();
    histograms = new Map();
    constructor() {
    ...
    initializeMetrics() {
        // Counters
        this.registerCounter('cross_border_handovers_total', 'Total number of handover attempts', ['source_nation', 'target_nation', 'status']);
        this.registerCounter('cross_border_messages_total', 'Total number of cross-border messages', ['session_id', 'type', 'language']);
        this.registerCounter('cross_border_translations_total', 'Total number of translation requests', ['source_language', 'target_language', 'status']);
        this.registerCounter('cross_border_circuit_breaker_trips_total', 'Total circuit breaker trips', ['partner_id']);
        this.registerCounter('cross_border_rate_limit_exceeded_total', 'Total rate limit exceeded events', ['partner_id']);
        // Gauges
        this.registerGauge('cross_border_active_sessions', 'Number of active cross-border sessions', []);
        this.registerGauge('cross_border_active_partners', 'Number of active partner nations', []);
        this.registerGauge('cross_border_partner_health', 'Health status of partner nations (1=healthy, 0=unhealthy)', ['partner_id', 'partner_code']);
        this.registerGauge('cross_border_circuit_breaker_state', 'Circuit breaker state (0=closed, 1=half-open, 2=open)', ['partner_id']);
        // Histograms
        this.registerHistogram('cross_border_handover_duration_seconds', 'Duration of handover operations', ['source_nation', 'target_nation'], [0.1, 0.5, 1, 2, 5, 10, 30]);
        this.registerHistogram('cross_border_translation_duration_seconds', 'Duration of translation operations', ['source_language', 'target_language'], [0.01, 0.05, 0.1, 0.25, 0.5, 1]);
        this.registerHistogram('cross_border_partner_latency_seconds', 'Partner endpoint latency', ['partner_id'], [0.1, 0.25, 0.5, 1, 2, 5]);
    }
    // Counter methods
    ...
    // Histogram methods
    registerHistogram(name, help, labels, buckets) {
    ...
    toJSON() {
        ...
        for (const histogram of this.histograms.values()) {
            result[histogram.name] = Object.fromEntries(Array.from(histogram.values.entries()).map(([k, v]) => [
                k,
...
exports.CrossBorderMetrics = CrossBorderMetrics;
// Singleton
let metricsInstance = null;
function getCrossBorderMetrics() {
    if (!metricsInstance) {
...
function recordHandover(sourceNation, targetNation, status, durationSeconds) {
...
function recordTranslation(sourceLanguage, targetLanguage, status, durationSeconds) {
    const metrics = getCrossBorderMetrics();
...
function recordPartnerHealth(partnerId, partnerCode, healthy, latencySeconds) {
    const metrics = getCrossBorderMetrics();
...
function updateActivePartners(count) {

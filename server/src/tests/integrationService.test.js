...
describe('Integration Service - P2 Priority', () => {
  ...
  describe('Connection Management', () => {
    test('should create connections successfully', async () => {
      ...
      const connection =
        await integrationService.createConnection(connectionData);
    ...
    test('should validate authentication before creating connections', async () => {
      ...
      await expect(
        integrationService.createConnection(connectionData),
    ...
    test('should update existing connections', async () => {
      ...
      const updatedConnection = await integrationService.updateConnection(
        'conn123',
        updates,
      );

      expect(updatedConnection.name).toBe('Updated Name');
      expect(updatedConnection.authentication.credentials).toBe(
        'encrypted_new_creds',
      );
  ...
  describe('Authentication Validation', () => {
    test('should validate API key authentication', async () => {
      ...
      const result = await integrationService.validateAuthentication(
        connection,
        connector,
      );
      expect(result).toBe(true);
    ...
    test('should reject unsupported authentication types', async () => {
      ...
      await expect(
        integrationService.validateAuthentication(connection, connector),
  ...
  describe('Data Operations', () => {
    ...
    test('should reject queries on inactive connections', async () => {
      ...
      await expect(integrationService.queryData('conn123', {})).rejects.toThrow(
  ...
  describe('Sync Job Management', () => {
    ...
    test('should handle sync job failures', async () => {
      ...
      await expect(integrationService.runSyncJob('job123')).rejects.toThrow(
    ...
    test('should process scheduled syncs', async () => {
      ...
      expect(integrationService.runSyncJob).not.toHaveBeenCalledWith(
        'not_ready_job',
      );
  ...
  describe('Webhook Management', () => {
    ...
    test('should filter events based on webhook configuration', () => {
      ...
      expect(
        integrationService.matchesWebhookFilters(matchingEvent, webhook),
      ).toBe(true);
      expect(
        integrationService.matchesWebhookFilters(nonMatchingEvent1, webhook),
      ).toBe(false);
      expect(
        integrationService.matchesWebhookFilters(nonMatchingEvent2, webhook),
      ).toBe(false);
    });
  });
  ...
  describe('Security and Encryption', () => {
    ...
    test('should generate webhook signatures', () => {
      ...
      const signature = integrationService.generateWebhookSignature(
        payload,
        secret,
      );
  ...
  describe('Error Handling', () => {
    test('should handle connection not found errors', async () => {
      await expect(
        integrationService.updateConnection('nonexistent', {}),
      ).rejects.toThrow('Connection not found');

      await expect(
        integrationService.deleteConnection('nonexistent'),
      ).rejects.toThrow('Connection not found');

      await expect(
        integrationService.queryData('nonexistent', {}),
      ).rejects.toThrow('Connection not found');
    });

    test('should handle sync job not found errors', async () => {
      await expect(
        integrationService.runSyncJob('nonexistent'),
...
describe('Integration Service Performance', () => {
  ...
  test('should handle many concurrent connections efficiently', async () => {
    ...
    const connections = await Promise.all(
      connectionRequests.map((req) => integrationService.createConnection(req)),
    );
  ...
  test('should efficiently process webhook deliveries', async () => {
    ...
    const results = await Promise.all(
      webhooks.map((webhook) =>
        integrationService.triggerWebhook(webhook.id, eventData),
      ),
    );
    const duration = Date.now() - startTime;

...
describe('Integration Service - P2 Priority', () => {
  ...
  describe('Connection Management', () => {
    ...
    test('should validate authentication before creating connections', async () => {
      ...
      await expect(integrationService.createConnection(connectionData)).rejects.toThrow(
        'API key is required',
      );
    });
    ...
    test('should reject connections with invalid connectors', async () => {
      ...
      await expect(integrationService.createConnection(connectionData)).rejects.toThrow(
        'Unknown connector: INVALID_CONNECTOR',
      );
    });
  ...
  describe('Error Handling', () => {
    test('should handle connection not found errors', async () => {
      await expect(integrationService.updateConnection('nonexistent', {})).rejects.toThrow(
        'Connection not found',
      );

      await expect(integrationService.deleteConnection('nonexistent')).rejects.toThrow(
        'Connection not found',
      );

      await expect(integrationService.queryData('nonexistent', {})).rejects.toThrow(
        'Connection not found',
      );
    });

    test('should handle sync job not found errors', async () => {
      await expect(integrationService.runSyncJob('nonexistent')).rejects.toThrow(
        'Sync job not found',
      );
    });
...
describe('Integration Service Performance', () => {
  ...
  test('should efficiently process webhook deliveries', async () => {
    ...
    const results = await Promise.all(
      webhooks.map((webhook) => integrationService.triggerWebhook(webhook.id, eventData)),
    );
    const duration = Date.now() - startTime;

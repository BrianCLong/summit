...
(0, globals_1.describe)('Canonical Entities Integration', () => {
    let pool;
    ...
    (0, globals_1.it)('should handle complete entity lifecycle with provenance', async () => {
        // ============ Step 1: Create Provenance Chain ============
        console.log('Step 1: Creating provenance chain...');
        const provenanceChain = (0, canonical_js_1.createProvenanceChain)('chain-person-001', {
        ...
        await pool.query(`INSERT INTO canonical_provenance (id, tenant_id, chain_id, chain_data, chain_hash)
       VALUES (gen_random_uuid(), $1, $2, $3, $4)`, [
        ...
        const provResult = await pool.query('SELECT id FROM canonical_provenance WHERE chain_id = $1', [provenanceChain.chainId]);
        ...
        const person = (0, canonical_js_1.createPerson)({
            name: {
                full: 'Alice Johnson',
                given: 'Alice',
                family: 'Johnson',
        ...
        await (0, canonical_js_1.createEntityVersion)(pool, 'Person', person);
        ...
        const before = await (0, canonical_js_1.snapshotAtTime)(pool, 'Person', tenantId, new Date('2020-06-01'), // Valid at this time
        new Date('2022-12-31'));
        (0, globals_1.expect)(before).toHaveLength(0);
        console.log('✓ Time-travel query (before knowledge): No entities found');
        // Query: What do we know now?
        const now = (await (0, canonical_js_1.snapshotAtTime)(pool, 'Person', tenantId, new Date('2020-06-01'), // Valid at this time
        new Date()));
        (0, globals_1.expect)(now).toHaveLength(1);
        (0, globals_1.expect)(now[0].name.full).toBe('Alice Johnson');
       ...
       SET valid_to = $1
       WHERE id = $2 AND version = 1`, [new Date('2023-07-01'), 'person-alice-001']);
        const personV2 = (0, canonical_js_1.createPerson)({
            name: person.name,
        ...
        await (0, canonical_js_1.createEntityVersion)(pool, 'Person', personV2);
        console.log('✓ Entity updated with promotion');
        // ============ Step 5: Query History ============
        console.log('\nStep 5: Querying entity history...');
        const history = await (0, canonical_js_1.getEntityHistory)(pool, 'Person', 'person-alice-001', tenantId);
        ...
        (0, globals_1.expect)(sorted[0].occupations).toContain('Software Engineer');
        (0, globals_1.expect)(sorted[1].occupations).toContain('Staff Software Engineer');
        ...
        await (0, canonical_js_1.correctEntity)(pool, 'Person', 'person-alice-001', tenantId, new Date('2020-01-15'), // Correct from the beginning
        {
            name: {
        ...
        const corrected = (await (0, canonical_js_1.snapshotAtTime)(pool, 'Person', tenantId, new Date('2020-06-01'), // Valid at this past time
        new Date()));
        (0, globals_1.expect)(corrected[0].name.middle).toBe('Marie');
        ...
        const subgraph = await (0, canonical_js_1.exportSubgraph)(pool, {
            tenantId,
        ...
        const validation = (0, canonical_js_1.validateSubgraphExport)(subgraph);
        ...
        const manifestVerification = (0, canonical_js_1.verifyManifest)(subgraph.provenance);
    ...
    (0, globals_1.it)('should demonstrate acceptance criteria', async () => {
        console.log('\n' + '='.repeat(60));
        ...
        const provChain = (0, canonical_js_1.createProvenanceChain)('test-chain', {
        ...
        await pool.query(`INSERT INTO canonical_provenance (id, tenant_id, chain_id, chain_data, chain_hash)
       VALUES (gen_random_uuid(), $1, $2, $3, $4)`, [tenantId, provChain.chainId, JSON.stringify(provChain), provChain.chainHash]);
        const provResult = await pool.query('SELECT id FROM canonical_provenance WHERE chain_id = $1', [provChain.chainId]);
        const testProvId = provResult.rows[0].id;
        const testPerson = (0, canonical_js_1.createPerson)({
        ...
        await (0, canonical_js_1.createEntityVersion)(pool, 'Person', testPerson);
        const exported = await (0, canonical_js_1.exportSubgraph)(pool, {
            tenantId,
        ...
        (0, globals_1.expect)(exported.provenance).toBeDefined();
        (0, globals_1.expect)(exported.provenance.version).toBe('1.0.0');
        (0, globals_1.expect)(exported.provenance.chains).toBeDefined();
        (0, globals_1.expect)(exported.provenance.manifestHash).toBeDefined();
        ...
        const manifestCheck = (0, canonical_js_1.verifyManifest)(exported.provenance);
        (0, globals_1.expect)(manifestCheck.valid).toBe(true);
        console.log('   ✓ Manifest integrity verified');
        // Verify export validation
        const exportCheck = (0, canonical_js_1.validateSubgraphExport)(exported);
        (0, globals_1.expect)(exportCheck.valid).toBe(true);
        console.log('   ✓ Export validation passed');
        console.log('\n' + '='.repeat(60));

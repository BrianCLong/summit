...
describe('AI Extraction Service - P0 Critical MVP1', () => {
  ...
  describe('Job Submission and Queue Management', () => {
    ...
    test('should handle concurrent job submissions', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        jobPromises.push(
          aiExtractionService.submitExtractionJob({
  ...
  describe('Text Processing Pipelines', () => {
    test('should extract entities from text using spaCy pipeline', async () => {
      ...
      const results = await aiExtractionService.extractWithSpacy(
        mediaSource,
        {},
      );
    ...
    test('should extract entities using Transformers pipeline', async () => {
      ...
      const results = await aiExtractionService.extractWithTransformers(
        mediaSource,
        {},
      );
  ...
  describe('Computer Vision Pipeline', () => {
    test('should extract entities from images', async () => {
      ...
      const results = await aiExtractionService.extractWithComputerVision(
        mediaSource,
        {},
      );
  ...
  describe('Speech-to-Text Pipeline', () => {
    test('should extract entities from audio transcription', async () => {
      ...
      const results = await aiExtractionService.extractWithSpeechToText(
        mediaSource,
        {},
      );
  ...
  describe('Hybrid AI Pipeline', () => {
    test('should combine multiple extraction methods', async () => {
      ...
      const results = await aiExtractionService.extractWithHybridAI(
        mediaSource,
        {},
      );
  ...
  describe('Entity Similarity and Fusion', () => {
    test('should identify similar entities correctly', () => {
      ...
      expect(aiExtractionService.entitiesAreSimilar(entity1, entity2)).toBe(
        true,
      );
      expect(aiExtractionService.entitiesAreSimilar(entity1, entity3)).toBe(
        false,
      );
    });
    ...
    test('should calculate string similarity accurately', () => {
      expect(
        aiExtractionService.calculateStringSimilarity(
          'John Smith',
          'John Smith',
        ),
      ).toBe(1.0);
      ...
      expect(
        aiExtractionService.calculateStringSimilarity('John Smith', 'Jane Doe'),
      ).toBeLessThan(0.5);
    });
  ...
  describe('Error Handling and Validation', () => {
    test('should validate extraction methods', async () => {
      ...
      const job = await aiExtractionService.submitExtractionJob(jobData);
      expect(job.warnings).toContain(
        'Unknown extraction method: NONEXISTENT_METHOD',
      );
    });

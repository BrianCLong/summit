...
describe('Advanced Analytics Service - P1 Priority', () => {
  ...
  describe('Anomaly Detection', () => {
    ...
    test('should configure detector thresholds', () => {
      ...
      const structuralDetector = detectors.find(
        (d) => d.id === 'STRUCTURAL_ANOMALY',
      );
      expect(structuralDetector.threshold).toBe(0.15);
      expect(structuralDetector.sensitivity).toBe('medium');

      const temporalDetector = detectors.find(
        (d) => d.id === 'TEMPORAL_ANOMALY',
      );
  ...
  describe('Link Prediction', () => {
    ...
    test('should calculate link prediction features', () => {
      ...
      const features = analyticsService.calculateLinkPredictionFeatures(
        '1',
        '2',
        nodes,
        edges,
  ...
  describe('Entity Classification', () => {
    ...
    test('should extract entity features', async () => {
      ...
      const features =
        await analyticsService.extractEntityFeatures(entityProps);
  ...
  describe('Behavior Prediction', () => {
    ...
    test('should analyze historical patterns', () => {
      ...
      const pattern = analyticsService.analyzeHistoricalPattern(
        behaviorData,
        'COMMUNICATION',
      );
    ...
    test('should predict future behavior', () => {
      ...
      const prediction = analyticsService.predictBehavior(
        historicalPattern,
        14,
      ); // 14 days
  ...
  describe('Risk Scoring', () => {
    ...
    test('should calculate risk components', async () => {
      ...
      const riskScore = await analyticsService.calculateRiskScore(
        entity,
        10,
        relationships,
        ['CONNECTIVITY', 'ACTIVITY', 'ASSOCIATIONS'],
      );
  ...
  describe('Community Detection', () => {
    ...
    test('should calculate community density', () => {
      ...
      const density = analyticsService.calculateCommunityDensity(
        community,
        graph,
      );
  ...
  describe('Analytics Job Management', () => {
    test('should queue and track analytics jobs', async () => {
      ...
      expect(job.status).toBe('QUEUED');
      expect(updatedMetrics.totalAnalytics).toBe(
        initialMetrics.totalAnalytics + 1,
      );
    });
  ...
  describe('Metrics and Performance', () => {
    ...
    test('should calculate success rate correctly', () => {
      ...
      if (metrics.totalAnalytics > 0) {
        const expectedRate = (
          (metrics.completedAnalytics / metrics.totalAnalytics) *
          100
        ).toFixed(2);
  ...
  describe('Utility Functions', () => {
    test('should calculate shortest path correctly', () => {
      ...
      const pathLength = analyticsService.calculateShortestPath(
        'A',
        'D',
        edges,
      );
    ...
    test('should check behavior type matching', () => {
      expect(
        analyticsService.isBehaviorTypeMatch('COMMUNICATION', 'COMMUNICATION'),
      ).toBe(true);
      expect(
        analyticsService.isBehaviorTypeMatch('CALLED', 'COMMUNICATION'),
      ).toBe(true);
      expect(analyticsService.isBehaviorTypeMatch('ACCESSED', 'ACTIVITY')).toBe(
        true,
      );
      expect(
        analyticsService.isBehaviorTypeMatch('UNKNOWN', 'COMMUNICATION'),
      ).toBe(false);
    });
  });
});
...
describe('Advanced Analytics Performance', () => {
  ...
  test('should handle large datasets efficiently', async () => {
    ...
    const layout =
      await analyticsService.calculateForceDirectedLayout(largeDataset);

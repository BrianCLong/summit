...
const RelationshipShapeValidator_js_1 = require("../../services/RelationshipShapeValidator.js");
...
describe('RelationshipShapeValidator', () => {
    let validator;
    beforeEach(() => {
        validator = new RelationshipShapeValidator_js_1.RelationshipShapeValidator();
        globals_1.jest.clearAllMocks();
    });
    describe('validate', () => {
        it('should validate a correct relationship', () => {
            const result = validator.validate('Person', 'FRIEND', 'Person', { since: '2023-01-01', closeness: 'high' });
    ...
    describe('detectDrift', () => {
        it('should detect unknown types', async () => {
            const mockRun = globals_1.jest.fn().mockReturnValue(Promise.resolve({
                records: [
                    {
                        get: (key) => {
                            if (key === 'id')
            ...
            database_js_1.getNeo4jDriver.mockReturnValue({
                session: () => mockSession,
            });
                ...
                records: [
                    {
                        get: (key) => {
                            ...
                            if (key === 'sLabels')
                                return ['Person'];
                            if (key === 'tLabels')
                                return ['Person']; // Invalid target for Employment
                            if (key === 'propKeys')
                                return [];
                        }
                    }

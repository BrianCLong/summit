...
exports.handleHttpSignal = handleHttpSignal;
...
const tracer = api_1.trace.getTracer('http-ingest', '24.2.0');
// Enhanced metrics for v24.2
const ingestRequests = new prom_client_1.Counter({
...
const ingestQueueDepth = new prom_client_1.Gauge({
    name: 'ingest_queue_depth',
    help: 'Current ingest queue depth per tenant',
    labelNames: ['tenant_id'],
});
const ingestThroughput = new prom_client_1.Counter({
...
class IngestQueue {
    ...
    async enqueue(tenantId, signals) {
    ...
    async processBatch(tenantId, signals) {
    ...
    async updateCoherenceScores(tenantId, signals) {
        ...
        await pg_js_1.pg.oneOrNone(`INSERT INTO coherence_scores (tenant_id, score, status, updated_at, sample_count)
       VALUES ($1, $2, $3, NOW(), $4)
       ON CONFLICT (tenant_id) 
       DO UPDATE SET 
         score = (coherence_scores.score * coherence_scores.sample_count + $2 * $4) / (coherence_scores.sample_count + $4),
         updated_at = NOW(),
         sample_count = coherence_scores.sample_count + $4`, [tenantId, averageScore, 'active', signals.length]);
    }
    async storeSignals(tenantId, signals) {
        ...
        for (let i = 0; i < queries.length; i += batchSize) {
            ...
            const session = await neo4j_js_1.neo.run(`
        UNWIND $batch as row
...
async function handleHttpSignal(req, res) {
    const startTime = Date.now();
    const tenantId = req.headers['x-tenant-id'] || 'default';
    const idempotencyKey = req.headers['x-idempotency-key'];
    const source = req.headers['user-agent'] || 'http-client';
    return tracer.startActiveSpan('ingest.http_signal', async (span) => {
        span.setAttributes({
            tenant_id: tenantId,
        ...
        try {
            ...
            if (idempotencyKey) {
                ...
                const existing = await pubsub_js_1.redis.get(key);
                if (existing) {
                    ingestRequests.inc({
                        tenant_id: tenantId,
                        status: 'idempotent',
                ...
                await pubsub_js_1.redis.setWithTTL(key, '1', 3600); // 1 hour TTL
            }
            ...
            for (const signal of ingestReq.signals) {
                try {
                    ...
                    const isDuplicate = await dedupe_js_1.deduplicationService.checkDuplicate({
                    ...
                    errors.push(`Signal validation failed: ${error.message}`);
                }
            }
            ...
            ingestLatency.observe({ tenant_id: tenantId, operation: 'enqueue' }, (Date.now() - startTime) / 1000);
            span.setAttributes({
...
async function validateAndEnrichSignal(signal, defaultTenantId) {
    // Basic validation

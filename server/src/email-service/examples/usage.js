...
exports.notifyInvestigationShared = notifyInvestigationShared;
...
exports.checkAndSendEmail = checkAndSendEmail;
...
exports.updateUserEmailPreferences = updateUserEmailPreferences;
exports.setupEmailRoutes = setupEmailRoutes;
...
async function sendWelcomeEmail(userEmail, userName) {
    ...
    const result = await emailService.sendFromTemplate('welcome-email', userEmail, {
        firstName: userName,
...
async function sendPasswordReset(userEmail, userName, resetToken) {
    ...
    const result = await emailService.sendFromTemplate('password-reset', userEmail, {
        firstName: userName,
        resetUrl,
        expiryHours: 24,
        logoUrl: 'https://intelgraph.com/logo.png',
...
async function notifyInvestigationShared(params) {
    const result = await emailService.sendFromTemplate('investigation-shared', params.recipientEmail, {
...
async function sendCriticalAlert(params) {
    const result = await emailService.sendFromTemplate('alert-critical', params.recipientEmail, {
...
async function sendWeeklyDigest(params) {
    const result = await emailService.sendFromTemplate('weekly-digest', params.recipientEmail, {
...
async function sendCampaignEmail(recipients) {
...
async function checkAndSendEmail(email, subject, html) {
...
async function previewEmailTemplate(templateId, variables) {
...
async function getEmailAnalytics(messageId) {
...
async function getTemplatePerformance(templateId) {
...
async function updateUserEmailPreferences(userEmail, preferences) {
    ...
    if (preferences.unsubscribeMarketing) {
        await emailService.unsubscribeManager.unsubscribeCategory(userEmail, types_js_1.EmailTemplateCategory.MARKETING);
    }
...
function setupEmailRoutes(app) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    app.get('/email/track/open/:messageId', async (req, res) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    app.get('/email/track/click/:messageId', async (req, res) => {
        const { messageId } = req.params;
        const { url } = req.query;
        if (url) {
            await emailService.analyticsService.trackClick(messageId, url);
            res.redirect(url);
        }
        else {
    ...
    app.get('/email/unsubscribe', async (req, res) => {
        ...
        const verified = await emailService.unsubscribeManager.verifyUnsubscribeToken(token);
        if (!verified) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    app.get('/email/preferences', async (req, res) => {
        const { token } = req.query;

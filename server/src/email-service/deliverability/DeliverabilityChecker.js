...
exports.DeliverabilityChecker = void 0;
class DeliverabilityChecker {
    ...
    async check(message) {
        ...
        const recommendations = this.generateRecommendations(spamScore, contentAnalysis, authentication);
        const overallScore = this.calculateOverallScore(spamScore, contentAnalysis, authentication);
        return {
    ...
    analyzeContent(message) {
        ...
        const linkCount = (html.match(/<a/gi) || []).length;
        const imageCount = (html.match(/<img/gi) || []).length;
        const hasUnsubscribeLink = !!(message.unsubscribeUrl ||
    ...
    generateRecommendations(spamScore, contentAnalysis, authentication) {
        ...
        if (spamScore.score >= this.spamScoreThreshold) {
            recommendations.push(`Spam score is ${spamScore.score.toFixed(1)}, which exceeds the threshold of ${this.spamScoreThreshold}. Review and fix issues.`);
        }
...
exports.DeliverabilityChecker = DeliverabilityChecker;

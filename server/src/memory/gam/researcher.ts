...
function mergeEvidence(existing: RetrievalResult[], incoming: RetrievalResult[], maxPages: number): RetrievalResult[] {
  const merged = new Map<string, RetrievalResult>();
  existing.forEach((item) => merged.set(item.pageId, item));
  incoming.forEach((item) => {
    const current = merged.get(item.pageId);
    if (!current || current.score < item.score) {
      merged.set(item.pageId, item);
    }
  });
    ...
    return tracer.startActiveSpan('researcher.plan', async (planSpan: Span) => {
      try {
        ...
        while (reflectionStep < budgets.maxReflectionDepth) {
          ...
          evidence = mergeEvidence(evidence, filtered, budgets.maxPages);
          ...
          if (reflection.missingInfo) {
            integration = {
              ...integration,
              openQuestions: Array.from(new Set([reflection.missingInfo, ...integration.openQuestions])).slice(0, 4),
            };
          }

...
class Bm25Index {
    ...
    if (!this.indices.has(tenantId)) {
      this.indices.set(
        ...
        new MiniSearch<IndexDocument>({
          fields: ['text', 'memo'],
          storeFields: ['id', 'sessionId', 'tenantId', 'memo', 'text'],
        }),
      );
    }
  ...
  search(tenantId: string, query: string, k: number): RetrievalResult[] {
    ...
    return index
      ...
      .map((result) => ({
        pageId: result.id,
        sessionId: (result as IndexDocument).sessionId,
        tenantId: (result as IndexDocument).tenantId,
        score: result.score ?? 0,
        retrieverType: 'bm25' as const,
        excerpt: (result as IndexDocument).text?.slice(0, 240) ?? query,
        memo: (result as IndexDocument).memo,
      }));
  }
}
...
export class HybridIndex {
  ...
  private sessionLookup: Map<string, MemoryPage[]> = new Map();
  ...
  addPage(page: MemoryPage, text: string, embedding: number[] | null | undefined): void {
    ...
    const key = this.sessionKey(page.tenant_id, page.session_id);
    const list = this.sessionLookup.get(key) ?? [];
    list.push({ ...page, raw_content: page.raw_content ?? {} });
    this.sessionLookup.set(key, list);
  ...
  lookupSession(sessionId: string, tenantId: string): MemoryPage[] {
    return this.sessionLookup.get(this.sessionKey(tenantId, sessionId)) ?? [];
  }

  private extractExcerpt(page: MemoryPage): string {
    const rawContent = page.raw_content ?? {};
    if (typeof (rawContent as { text?: string }).text === 'string') {
      return (rawContent as { text: string }).text.slice(0, 240);
    }
    if (typeof (rawContent as { content?: string }).content === 'string') {
      return (rawContent as { content: string }).content.slice(0, 240);
    }
    const turn = (rawContent as { turn?: { content?: string } }).turn;
    if (turn?.content) {
      return turn.content.slice(0, 240);
    }
    return '';
  }
    ...
    actions.forEach((action) => {
      ...
      if (action.tool === 'page_id') {
        action.pageIds?.forEach((pageId) => {
          ...
          const sessionPages = this.lookupSession(pageId, action.tenantId)
            .slice(0, action.k)
            .map((sessionPage) => ({
              pageId: sessionPage.id,
              sessionId: sessionPage.session_id,
              tenantId: sessionPage.tenant_id,
              score: 1,
              retrieverType: 'page_id' as const,
              excerpt: this.extractExcerpt(sessionPage),
              memo: sessionPage.memo,
            }));
          results.push(...sessionPages);
        });
      }
    });

...
import { retractionsProcessed, retractionsDuration, retractionsSlaBreaches } from './metrics/retractionsMetrics.js';
  ...
  for (const r of rows) {
    try {
      ...
      for (const x of aff.rows) {
        ...
        const nb = await makeBundle({ artifacts: [{ name: 'redacted.txt', path: '/etc/hosts' }], claimSet: { id: `claimset-${x.bundle_id}-r`, claims: [] }, merkleRoot: '', attestations: [] });
        await pg.query(`UPDATE bundles SET superseded_by=$1 WHERE id=$2`, [nb.path, x.bundle_id]);
      }
      ...
      await pg.query(`UPDATE retraction SET status='FAILED' WHERE id=$1`, [r.id]);

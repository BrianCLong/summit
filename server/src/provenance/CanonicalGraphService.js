...
exports.CanonicalGraphService = void 0;
...
class CanonicalGraphService {
    ...
    async projectEntry(entry) {
        ...
        if (!nodeType)
            return; // Skip non-canonical entries
        const session = neo4j_js_1.neo.session();
        try {
            // 1. Create the Node
            await session.run(`
     ...
     */
    async exportGraph(tenantId, options = {}) {
        const session = neo4j_js_1.neo.session();
        try {
            ...
            const sortedEdges = exportData.edges.sort((a, b) => (a.sourceId + a.targetId + a.relation + a.timestamp).localeCompare(b.sourceId + b.targetId + b.relation + b.timestamp));
    ...
    determineNodeType(entry) {
        ...
        if (['document', 'configuration', 'policy', 'prompt'].includes(type))
            return 'Input';
        if (['evaluation', 'approval', 'decision', 'prediction'].includes(type))
            return 'Decision';
        if (['run', 'job', 'task', 'action', 'ingest'].includes(type))
            return 'Action';
        if (['metric', 'alert', 'report', 'state', 'outcome'].includes(type))
            return 'Outcome';
        if (entry.actionType === 'EVALUATE')
            return 'Decision';
        if (entry.actionType === 'EXECUTE')
            return 'Action';
        return null;
    }
    extractSourceIds(entry) {
        ...
        if (payload.sourceId)
            ids.push(payload.sourceId);
        if (Array.isArray(payload.sourceIds))
            ids.push(...payload.sourceIds);
        if (payload.previousState?.id)
            ids.push(payload.previousState.id);
    ...
    determineRelationType(sourceType, targetType) {
        ...
        if (sourceType === 'Input' && targetType === 'Decision')
            return 'FED_INTO';
        if (sourceType === 'Input' && targetType === 'Action')
            return 'USED_BY';
        if (sourceType === 'Decision' && targetType === 'Action')
            return 'TRIGGERED';
        if (sourceType === 'Action' && targetType === 'Outcome')
            return 'PRODUCED';
        if (sourceType === 'Outcome' && targetType === 'Decision')
            return 'AFFECTED';
        if (sourceType === 'Action' && targetType === 'Input')
            return 'GENERATED';
        return 'DERIVED_FROM';
    }
    extractProperties(entry, nodeType) {
        ...
        if (nodeType === 'Decision') {
            ...
            if (payload.evaluator)
                props.evaluator = payload.evaluator;
            if (payload.outcome && !props.result)
                props.result = payload.outcome;
        }
        if (nodeType === 'Action') {
            if (payload.status)
                props.status = payload.status;
            if (payload.durationMs)
                props.durationMs = payload.durationMs;
            if (payload.startedAt)
                props.startedAt = payload.startedAt;
            if (payload.completedAt)
                props.completedAt = payload.completedAt;
        }
        if (nodeType === 'Input') {
            if (payload.uri)
                props.uri = payload.uri;
            if (payload.version)
                props.version = payload.version;
            if (payload.path)
                props.uri = payload.path;
        }
        if (nodeType === 'Outcome') {
            if (payload.value)
                props.value = payload.value;
            if (payload.dimension)
                props.dimension = payload.dimension;
        }
        return props;
    }
}
exports.CanonicalGraphService = CanonicalGraphService;

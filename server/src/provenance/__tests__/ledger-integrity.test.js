...
(0, globals_1.describe)('ProvenanceLedgerV2 Integrity & Witnesses', () => {
    (0, globals_1.beforeEach)(() => {
        ...
        pg_js_1.pool.query.mockImplementation((query, params) => {
            return Promise.resolve({ rows: [] });
        });
    });
    ...
    (0, globals_1.it)('should detect broken hash chains', async () => {
        ...
        pg_js_1.pool.query.mockResolvedValueOnce({ rows: [entry1, entry2] });
        // Act
        const verification = await ledger_js_1.provenanceLedger.verifyChainIntegrity('tenant-1');
        // Assert
        (0, globals_1.expect)(verification.valid).toBe(false);
        (0, globals_1.expect)(verification.brokenChains).toBeGreaterThan(0);
    });
    (0, globals_1.it)('should include witnesses in new entries', async () => {
        // Act
        ...
        // Assert
        (0, globals_1.expect)(entry.witness).toBeDefined();
        (0, globals_1.expect)(entry.witness?.witnessId).toBeDefined();
        (0, globals_1.expect)(entry.witness?.signature).toBeDefined();
    });
});

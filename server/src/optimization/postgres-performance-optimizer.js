...
exports.PostgresPerformanceOptimizer = void 0;
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../config/logger.js"));
const crypto_1 = require("crypto");
const events_1 = require("events");
class PostgresPerformanceOptimizer extends events_1.EventEmitter {
    pool;
    ...
    QUERY_PLAN_CACHE_TTL = 3600; // 1 hour
    CONNECTION_POOL_MONITOR_INTERVAL = 30000; // 30 seconds
    constructor(pool) {
        super();
        this.pool = pool;
     ...
     */
    async executeOptimizedQuery(query, parameters = [], options = {}) {
        ...
        try {
            ...
            try {
                ...
                const result = await this.executeWithTimeout(client, query, parameters, timeout);
                ...
                if (options.useCache !== false &&
                    ...
                    const ttl = options.cacheTtl || this.calculateCacheTtl(query, executionTime);
     ...
     */
    async getOptimizedConnection(priority = 'medium') {
        const startTime = Date.now();
        ...
        if (priority === 'high') {
            try {
                return await Promise.race([
                    this.pool.connect(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('High priority connection timeout')), 1000)),
                ]);
            }
            catch (error) {
                logger_js_1.default.warn('High priority connection failed, falling back to normal priority', {
                    error: error.message,
                });
            }
        }
        ...
        if (waitTime > 5000) {
            ...
            logger_js_1.default.warn('Long connection wait time', { waitTime, priority });
        }
        return client;
    }
    /**
     ...
     */
    async analyzeQueryPlan(client, query, parameters) {
        const queryHash = this.generateQueryHash(query, parameters);
        ...
        if (this.queryPlans.has(queryHash)) {
            ...
            if (Date.now() - cachedPlan.executionTime <
        ...
        try {
            ...
            logger_js_1.default.warn('Failed to analyze query plan', {
                error: error.message,
     ...
     */
    async generateIndexRecommendations(query, plan) {
        ...
        // JOIN analysis
        const joinMatches = queryLower.matchAll(/join\s+(\w+)\s+\w+\s+on\s+([^where|group|order|join]+)/gi);
        ...
        for (const rec of recommendations) {
            const exists = this.indexRecommendations.some((existing) => existing.table === rec.table &&
    ...
    async runScheduledMaintenance() {
        ...
        for (const [name, task] of this.maintenanceTasks) {
            ...
            if (shouldRun) {
                try {
                    logger_js_1.default.info(`Running maintenance task: ${name}`);
                    await task.action();
                    task.lastRun = now;
                    task.nextRun = this.calculateNextRun(task.schedule, now);
                    logger_js_1.default.info(`Completed maintenance task: ${name}`);
                }
                catch (error) {
                    logger_js_1.default.error(`Failed to run maintenance task ${name}:`, error);
                }
            }
        }
    }
      ...
      `);
            for (const row of tablesResult.rows) {
                ...
                try {
                    ...
                    logger_js_1.default.warn(`Failed to analyze table ${tableName}:`, error);
                }
            }
            logger_js_1.default.info(`Analyzed ${tablesResult.rows.length} tables`);
        }
        finally {
            client.release();
        }
    }
    async vacuumAnalyzeTables() {
        const client = await this.pool.connect();
        try {
      ...
      `);
            for (const row of tablesResult.rows) {
                ...
                try {
                    ...
                    logger_js_1.default.info(`Vacuumed table ${tableName} (dead ratio: ${(row.dead_ratio * 100).toFixed(1)}%)`);
                }
                catch (error) {
                    logger_js_1.default.warn(`Failed to vacuum table ${tableName}:`, error);
                }
            }
        }
        finally {
    ...
    async reindexFragmentedIndexes() {
        const client = await this.pool.connect();
        try {
      ...
      `);
            for (const row of indexResult.rows) {
                ...
                try {
                    ...
                    logger_js_1.default.info(`Reindexed ${indexName} (size: ${row.index_size})`);
                }
                catch (error) {
                    logger_js_1.default.warn(`Failed to reindex ${indexName}:`, error);
                }
            }
        }
        finally {
    ...
    startConnectionPoolMonitoring() {
        setInterval(async () => {
            ...
            if (metrics.idleConnections === 0 && metrics.waitingClients > 0) {
                ...
                logger_js_1.default.warn('Connection pool exhausted', metrics);
            }
    ...
    calculateCacheTtl(query, executionTime) {
     ...
     */
    extractTablesFromPlan(plan, tables = new Set()) {
    ...
    extractFilterColumnsFromWhere(whereClause) {
        const columns = [];
    ...
    extractOrderByColumns(orderByClause) {
        const columns = [];
     ...
     */
    generateQueryHash(query, parameters) {
    ...
    async getCachedResult(queryHash) {
        ...
        try {
            ...
            logger_js_1.default.warn('Failed to parse cached query result', { queryHash });
    ...
    async cacheResult(queryHash, result, ttlSeconds) {
        ...
        try {
            ...
            await this.redis.setex(`postgres:query:${queryHash}`, ttlSeconds, JSON.stringify(cacheData));
        }
        catch (error) {
            logger_js_1.default.warn('Failed to cache query result', {
                queryHash,
                error: error.message,
            });
        }
    }
    async logSlowQuery(query, parameters, duration, queryHash) {
        const slowQuery = {
            query,
            duration,
            timestamp: Date.now(),
        ...
        logger_js_1.default.warn('Slow query detected', {
            duration,
            queryHash,
            query: query.substring(0, 200),
        });
    }
    async updateQueryStats(queryHash, executionTime, rowCount) {
        if (!this.redis)
            return;
     ...
     */
    shouldRunMaintenanceTask(task, now) {
    ...
    async cleanSlowQueryLog() {
        const oneDayAgo = Date.now() - 24 * 60 * 60 * 1000;
        this.slowQueries = this.slowQueries.filter((query) => query.timestamp > oneDayAgo);
        logger_js_1.default.info(`Cleaned slow query log, ${this.slowQueries.length} entries remaining`);
    }
    ...
    async getAggregatedQueryStats() {
        ...
        return {
            ...
            avgExecutionTime: stats.reduce((sum, s) => sum + s.avgTime, 0) / stats.length || 0,
    ...
    async createRecommendedIndexes(limit = 5) {
        ...
        `;
                try {
                    ...
                    logger_js_1.default.info(`Created recommended index: ${indexName}`, {
                        reason: rec.reason,
                    });
                }
                catch (error) {
                    logger_js_1.default.warn(`Failed to create index ${indexName}:`, error);
                }
            }
        }
        finally {
    ...
    async clearQueryCache(pattern) {
        ...
        if (keys.length > 0) {
            ...
            logger_js_1.default.info(`Cleared ${keys.length} cached queries matching pattern: ${searchPattern}`);
        }
    }
    async getCacheStats() {
        if (!this.redis)
...
exports.PostgresPerformanceOptimizer = PostgresPerformanceOptimizer;

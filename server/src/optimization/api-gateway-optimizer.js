...
exports.ApiGatewayOptimizer = void 0;
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
const crypto_1 = require("crypto");
const events_1 = require("events");
class ApiGatewayOptimizer extends events_1.EventEmitter {
    ...
    createCacheMiddleware(config) {
        return async (req, res, next) => {
            ...
            try {
                ...
                logger_js_1.default.error('Cache middleware error:', error);
                next();
            }
        };
    }
     ...
     */
    createCircuitBreakerMiddleware(config) {
        return async (req, res, next) => {
            const routeKey = this.getRouteKey(req);
     ...
     */
    createBulkheadMiddleware(config) {
        return async (req, res, next) => {
            const routeKey = this.getRouteKey(req);
     ...
     */
    async invalidateCache(tags, patterns) {
        if (!this.redis)
            return;
        const keysToDelete = [];
        // Invalidate by tags
        ...
        if (uniqueKeys.length > 0) {
            ...
            logger_js_1.default.info(`Invalidated ${uniqueKeys.length} cache entries`, {
                tags,
                patterns,
            });
        }
     ...
     */
    trackRequestCost(userId, endpoint, cost) {
    ...
    async checkBudgetLimit(userId, requestCost) {
    ...
    async getCachedResponse(cacheKey) {
        ...
        try {
            ...
            logger_js_1.default.warn('Failed to parse cached response', { cacheKey });
            await this.redis.del(`${this.CACHE_PREFIX}${cacheKey}`);
            return null;
        }
    }
    async cacheResponse(cacheKey, response, ttl, tags) {
        if (!this.redis)
            return;
        try {
            // Cache the response
            await this.redis.setex(`${this.CACHE_PREFIX}${cacheKey}`, ttl, JSON.stringify(response));
            ...
            logger_js_1.default.warn('Failed to cache response', {
                cacheKey,
                error: error.message,
            });
        }
    ...
    shouldCacheResponse(statusCode, data) {
    ...
    async revalidateInBackground(req, next, cacheKey, config) {
        ...
        process.nextTick(async () => {
            try {
                ...
                logger_js_1.default.info('Background revalidation started', { cacheKey });
            }
            catch (error) {
                logger_js_1.default.warn('Background revalidation failed', {
                    cacheKey,
                    error: error.message,
                });
            }
    ...
    getCircuitBreaker(routeKey, config) {
    ...
    recordCircuitBreakerFailure(routeKey, circuitBreaker, config) {
    ...
    recordCircuitBreakerSuccess(routeKey, circuitBreaker, config) {
    ...
    async executeBatch(batchKey) {
        ...
        try {
            ...
            logger_js_1.default.error('Batch execution failed', {
                batchKey,
                error: error.message,
            });
    ...
    updateRouteMetrics(routeKey, responseTime, success, cacheHit, circuitBreakerTrip = false) {
    ...
    cleanupExpiredBatches() {
        ...
        for (const [key, batch] of this.requestBatches) {
            ...
            if (now - parseInt(key.split(':').pop()) * this.BATCH_WINDOW_MS >
                60000) {
                clearTimeout(batch.timeoutId);
        ...
        try {
            ...
            if (totalKeys > 10000) {
                ...
                if (keysToRemove.length > 0) {
                    ...
                    logger_js_1.default.info(`Cleaned up ${keysToRemove.length} cache entries for storage optimization`);
                }
            }
        }
        catch (error) {
            logger_js_1.default.warn('Cache optimization failed:', error);
        }
    }
    /**
    ...
    async clearAllCaches() {
        ...
        if (keys.length > 0) {
            ...
            logger_js_1.default.info(`Cleared all ${keys.length} cache entries`);
        }
    }
}
exports.ApiGatewayOptimizer = ApiGatewayOptimizer;

...
exports.CostEfficiencyOptimizer = void 0;
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
const events_1 = require("events");
class CostEfficiencyOptimizer extends events_1.EventEmitter {
    redis = (0, database_js_1.getRedisClient)();
    modelProfiles = new Map();
    budgetTrackers = new Map();
    usageAnalytics;
    ...
    async selectOptimalModel(criteria) {
        ...
        try {
            ...
            if (!budgetCheck.allowed) {
                candidateModels = candidateModels.filter((model) => this.estimateRequestCost(model, criteria) <=
     ...
     */
    async predictRequestCost(modelId, criteria, inputText) {
     ...
     */
    async trackUsageCost(userId, tenantId, modelId, actualCost, requestData) {
     ...
     */
    async generateOptimizationRecommendations(userId, tenantId, timeframe = 7 * 24 * 60 * 60 * 1000) {
    ...
    async optimizeResourceAllocation() {
        ...
        const recommendedAllocation = await this.generateOptimalAllocation(currentUsage, demandPatterns, costEfficiencyMap);
        // Calculate projected savings
        const projectedSavings = this.calculateProjectedSavings(currentUsage, recommendedAllocation);
        // Create implementation plan
        const implementationPlan = this.generateImplementationPlan(currentUsage, recommendedAllocation);
        return {
    ...
    async loadDynamicModelProfiles() {
        ...
        if (this.redis) {
            try {
                ...
                logger_js_1.default.warn('Failed to load cached model profiles:', error);
            }
        }
    }
    async filterModelsByCriteria(criteria) {
        const candidates = [];
        for (const model of this.modelProfiles.values()) {
            // Check required capabilities
            if (criteria.requiredCapabilities.length > 0) {
                const hasRequiredCapabilities = criteria.requiredCapabilities.every((cap) => model.capabilities.includes(cap));
            ...
            if (model.qualityScore < criteria.qualityRequirement)
                continue;
            // Check availability and health
            if (model.performance.successRate < 0.9)
                continue;
            candidates.push(model);
        }
        return candidates;
    }
    async checkBudgetConstraints(criteria) {
        ...
        const maxAllowedCost = Math.min(criteria.budgetLimit || tracker.totalBudget, budgetRemaining * 0.1, // Don't allow more than 10% of remaining budget in one request
    ...
    async scoreModels(models, criteria) {
        ...
        return scoredModels.sort((a, b) => b._score - a._score);
    }
    determineRoutingStrategy(criteria) {
        // Intelligent routing strategy based on request characteristics
        if (criteria.urgency === 'critical')
            return 'performance_first';
        if (criteria.qualityRequirement > 0.9)
            return 'quality_first';
        if (criteria.budgetLimit && criteria.budgetLimit < 0.01)
            return 'cost_first';
        if (criteria.complexity > 0.8)
            return 'capability_first';
        return 'balanced';
    }
    async applyRoutingStrategy(scoredModels, strategy, criteria) {
        switch (strategy) {
            case 'performance_first':
                return scoredModels.reduce((best, current) => current.averageLatency < best.averageLatency ? current : best);
            case 'quality_first':
                return scoredModels.reduce((best, current) => current.qualityScore > best.qualityScore ? current : best);
            case 'cost_first':
            ...
            case 'capability_first':
                return scoredModels.reduce((best, current) => current.capabilities.length > best.capabilities.length
                    ? current
                    : best);
            case 'balanced':
            default:
    ...
    estimateRequestCost(model, criteria) {
    ...
    async generateCostPrediction(model, criteria) {
        return this.predictRequestCost(model.id, criteria);
    }
    generateSelectionReasoning(model, criteria, strategy) {
        const cost = this.estimateRequestCost(model, criteria);
    ...
    async recordModelSelection(model, criteria, estimatedCost) {
    ...
    async updateModelProfiles() {
        ...
        for (const [modelId, profile] of this.modelProfiles) {
            try {
                ...
                logger_js_1.default.warn(`Failed to update profile for model ${modelId}:`, error);
            }
        }
    }
    async getRecentModelMetrics(modelId) {
    ...
    analyzeUsagePatterns() {
        ...
        const hourlyAverages = Array.from(this.usageAnalytics.requestsByHour.entries()).sort(([, a], [, b]) => b - a);
        this.usageAnalytics.patterns.peakHours = hourlyAverages
            .slice(0, 6)
            .map(([hour]) => hour);
    }
    calculateEfficiencyMetrics() {
        const totalCost = Array.from(this.usageAnalytics.costByModel.values()).reduce((sum, cost) => sum + cost, 0);
        const totalRequests = Array.from(this.usageAnalytics.requestsByHour.values()).reduce((sum, count) => sum + count, 0);
        if (totalRequests > 0) {
    ...
    async checkBudgetAlerts(tracker) {
    ...
    cleanupOldData() {
        ...
        logger_js_1.default.info('Cleaned up old analytics data');
    }
    // Additional helper methods for the more complex operations...
    generatePredictionCacheKey(modelId, criteria, inputText) {
        const key = `${modelId}:${criteria.taskType}:${criteria.complexity}:${criteria.contextSize}`;
        return Buffer.from(key).toString('base64');
    }
    async getCachedPrediction(cacheKey) {
        if (!this.redis)
            return null;
        const cached = await this.redis.get(`cost_prediction:${cacheKey}`);
        return cached ? JSON.parse(cached) : null;
    }
    async cachePrediction(cacheKey, prediction) {
        if (!this.redis)
            return;
        await this.redis.setex(`cost_prediction:${cacheKey}`, this.PREDICTION_CACHE_TTL, JSON.stringify(prediction));
    }
    estimateTokenCount(text) {
        // Rough estimation: 1 token ≈ 4 characters for English
        return Math.ceil(text.length / 4);
    }
    calculatePredictionConfidence(model, criteria) {
        ...
        if (Date.now() - model.lastUpdated < 24 * 60 * 60 * 1000)
            confidence -= 0.1;
    ...
    async generateCostAlternatives(baseModel, criteria, baseCost) {
        const alternatives = [];
        for (const model of this.modelProfiles.values()) {
    ...
    async initializeBudgetTracker(userId, tenantId) {
    ...
    updateUsageAnalytics(userId, tenantId, modelId, taskType, cost) {
    ...
    async updateModelPerformance(modelId, actualCost, requestData) {
        const model = this.modelProfiles.get(modelId);
        if (!model)
            return;
    ...
    async updateSpendingProjection(tracker) {
    ...
    async getUserAnalytics(userId, tenantId, timeframe) {
        return {
    ...
    async analyzeModelSwitchingOpportunities(userAnalytics) {
    ...
    async analyzeBatchingOpportunities(userAnalytics) {
        return null; // Would implement batching analysis
    }
    async analyzeCachingOpportunities(userAnalytics) {
        return null; // Would implement caching analysis
    }
    async analyzeParameterTuning(userAnalytics) {
        return null; // Would implement parameter analysis
    }
    async analyzeTimeBasedOptimization(userAnalytics) {
    ...
    async buildCostEfficiencyMap() {
        return {}; // Would implement efficiency mapping
    }
    async generateOptimalAllocation(currentUsage, demandPatterns, efficiencyMap) {
        return {}; // Would implement optimization algorithm
    }
    calculateProjectedSavings(currentUsage, recommendedAllocation) {
        return 0; // Would calculate actual savings
    }
    generateImplementationPlan(currentUsage, recommendedAllocation) {
        return []; // Would generate implementation steps
    }
    /**
     * 📊 Public API methods
     */
    async getUsageReport(userId, tenantId) {
        const budgetKey = `${tenantId}:${userId}`;
        const tracker = this.budgetTrackers.get(budgetKey);
        return {
            budget: tracker
    ...
    async updateBudget(userId, tenantId, newBudget) {
        const budgetKey = `${tenantId}:${userId}`;
        let tracker = this.budgetTrackers.get(budgetKey);
        if (!tracker) {
            tracker = await this.initializeBudgetTracker(userId, tenantId);
...
exports.CostEfficiencyOptimizer = CostEfficiencyOptimizer;

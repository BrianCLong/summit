...
exports.Neo4jQueryOptimizer = void 0;
const database_js_1 = require("../config/database.js");
const logger_js_1 = __importDefault(require("../utils/logger.js"));
const crypto_1 = require("crypto");
const events_1 = require("events");
class Neo4jQueryOptimizer extends events_1.EventEmitter {
    driver;
    redis = (0, database_js_1.getRedisClient)();
    materializedViews = new Map();
    queryStats = new Map();
    CACHE_PREFIX = 'neo4j:query:';
    MATERIALIZED_VIEW_PREFIX = 'neo4j:view:';
    QUERY_STATS_PREFIX = 'neo4j:stats:';
    constructor(driver) {
        super();
        this.driver = driver;
     ...
     */
    async analyzeQueryComplexity(query, parameters) {
        const factors = {
            nodeTraversals: 0,
     ...
     */
    async optimizeQuery(query, parameters, complexity) {
        ...
        for (const [name, view] of this.materializedViews) {
            ...
            if (now - view.lastRefresh > refreshIntervalMs) {
                try {
                    ...
                    logger_js_1.default.error(`Failed to refresh materialized view ${name}:`, error);
                }
            }
        }
    }
    async refreshMaterializedView(name, view) {
        ...
        try {
            ...
            if (this.redis) {
                await this.redis.setex(cacheKey, view.refreshIntervalMinutes * 60 + 300, // TTL + 5min buffer
                JSON.stringify({
                    records,
            ...
            logger_js_1.default.info(`Refreshed materialized view ${name}`, {
    ...
    async checkMaterializedViews(query, parameters) {
        ...
        if (queryLower.includes('investigation') &&
        ...
        if (queryLower.includes('createdat') &&
    ...
    async getMaterializedViewResult(viewName) {
        if (!this.redis)
            return null;
    ...
    optimizeVariableLengthPaths(query) {
     ...
     */
    calculateQueryCost(score, factors) {
    ...
    calculateTimeout(complexityScore) {
    ...
    calculateMemoryRequirement(score, factors) {
    ...
    async cacheResult(queryHash, result, metrics, complexity, ttlSeconds) {
        if (!this.redis)
            return;
        ...
        await this.redis.setex(`${this.CACHE_PREFIX}${queryHash}`, ttlSeconds, JSON.stringify(cachedData));
    }
    shouldCacheQuery(complexity, metrics) {
    ...
    calculateCacheTtl(complexity, metrics) {
     ...
     */
    async extractMetrics(result, executionTime, complexity) {
    ...
    async updateQueryStats(queryHash, metrics) {
        if (!this.redis)
            return;
    ...
    async clearCache(pattern) {
        ...
        if (keys.length > 0) {
            ...
            logger_js_1.default.info(`Cleared ${keys.length} cache entries matching pattern: ${searchPattern}`);
        }
    }
    async getCacheStats() {
        if (!this.redis)
    ...
    async estimateCacheMemoryUsage(keys) {
...
exports.Neo4jQueryOptimizer = Neo4jQueryOptimizer;

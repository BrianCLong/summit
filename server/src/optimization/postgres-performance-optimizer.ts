...
export class PostgresPerformanceOptimizer extends EventEmitter {
  ...
  async executeOptimizedQuery<T = any>(
    ...
    try {
      ...
      try {
        ...
        if (options.useCache !== false && this.shouldCacheResult(query, executionTime, result)) {
   ...
   */
  private async getOptimizedConnection(priority: 'high' | 'medium' | 'low' = 'medium'): Promise<PoolClient> {
    const startTime = Date.now();
      ...
      try {
        return await Promise.race([
          ...
          new Promise<never>((_, reject) => 
            setTimeout(() => reject(new Error('High priority connection timeout')), 1000)
          )
        ]);
      } catch (error) {
        logger.warn('High priority connection failed, falling back to normal priority', {
          error: error.message
        });
      }
    }
  ...
  private async analyzeQueryPlan(
    ...
    if (this.queryPlans.has(queryHash)) {
      ...
      if (Date.now() - cachedPlan.executionTime < this.QUERY_PLAN_CACHE_TTL * 1000) {
        return cachedPlan;
      }
    }
   ...
   */
  private async generateIndexRecommendations(query: string, plan?: QueryPlan): Promise<void> {
    ...
    for (const rec of recommendations) {
      const exists = this.indexRecommendations.some(existing => 
    ...
    try {
      ...
      for (const row of tablesResult.rows) {
        ...
        try {
          ...
          logger.info(`Vacuumed table ${tableName} (dead ratio: ${(row.dead_ratio * 100).toFixed(1)}%)`);
        } catch (error) {
  ...
  private shouldAnalyzeQuery(query: string): boolean {
    ...
    if (queryLower.includes('join') && queryLower.includes('where')) return true;
    if (queryLower.includes('order by') && queryLower.includes('limit')) return true;
    if (queryLower.includes('group by')) return true;
    if (queryLower.includes('union')) return true;
    if (queryLower.includes('exists') || queryLower.includes('in (select')) return true;
  ...
  private shouldCacheResult(query: string, executionTime: number, result: QueryResult): boolean {
    ...
    if (queryLower.includes('join') || queryLower.includes('group by')) return true;
  ...
  private calculateCacheTtl(query: string, executionTime: number): number {
    ...
    if (executionTime > 10000) ttl *= 4; // 20 minutes
   ...
   */
  private extractTablesFromPlan(plan: any, tables: Set<string> = new Set()): string[] {
  ...
  private extractFilterColumnsFromWhere(whereClause: string): Array<{table: string, column: string, operator: string}> {
    ...
    patterns.forEach(pattern => {
      ...
      while ((match = pattern.exec(whereClause)) !== null) {
        if (match[3]) { // Table.column pattern
          columns.push({
            table: match[1],
            column: match[2],
            operator: match[3].toLowerCase()
          });
        } else { // column pattern (assume main table)
          columns.push({
            table: 'main',
            column: match[1],
  ...
  private extractOrderByColumns(orderByClause: string): string[] {
    ...
    parts.forEach(part => {
      ...
      if (match) {
        const column = match[1].includes('.') ? match[1].split('.')[1] : match[1];
        columns.push(column);
      }
    });
  ...
  private async getCachedResult<T = any>(queryHash: string): Promise<QueryResult<T> | null> {
  ...
  private async cacheResult(
    ...
    try {
      ...
      logger.warn('Failed to cache query result', { queryHash, error: error.message });
    }
  }
  ...
  private async updateQueryStats(queryHash: string, executionTime: number, rowCount?: number): Promise<void> {
    if (!this.redis) return;
   ...
   */
  private shouldRunMaintenanceTask(task: MaintenanceTask, now: number): boolean {
    ...
    if (task.schedule === '0 3 * * 0') { // Weekly on Sunday at 3 AM
      return hoursSinceLastRun >= 168; // 7 days
    }
    if (task.schedule === '0 1 * * 0') { // Weekly on Sunday at 1 AM
      return hoursSinceLastRun >= 168;
    }
    if (task.schedule === '0 0 * * *') { // Daily at midnight
      return hoursSinceLastRun >= 24;
    }
    if (task.schedule === '*/30 * * * *') { // Every 30 minutes
  ...
  private async cleanSlowQueryLog(): Promise<void> {
    ...
    logger.info(`Cleaned slow query log, ${this.slowQueries.length} entries remaining`);
  }
    ...
    try {
      ...
      for (const rec of topRecommendations) {
        ...
        try {
          ...
          logger.info(`Created recommended index: ${indexName}`, { reason: rec.reason });
        } catch (error) {
  ...
  async clearQueryCache(pattern?: string): Promise<void> {
    ...
    const searchPattern = pattern ? `postgres:query:*${pattern}*` : 'postgres:query:*';
    const keys = await this.redis.keys(searchPattern);
    
    if (keys.length > 0) {
      await this.redis.del(...keys);
      logger.info(`Cleared ${keys.length} cached queries matching pattern: ${searchPattern}`);
    }
  }

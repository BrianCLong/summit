...
class KnowledgeFabricRetrievalService {
    ...
    async search(query) {
        ...
        WHERE tenant_id = $2
      `;
            const params = [vectorStr, query.tenantId];
            let paramIdx = 3;
            ...
            params.push(limit);
            vectorResults = await pg_js_1.pg.manyOrNone(sql, params);
            if (!vectorResults) {
                vectorResults = [];
            }
        }
    ...
    `;
        ...
        const safeCypher = cypherInvariance_js_1.CypherGuard.enforceInvariants(cypher, { strict: false, defaultLimit: 50 });
        try {
            const result = await neo4j_js_1.neo.run(safeCypher, { entityIds });
...
exports.KnowledgeFabricRetrievalService = KnowledgeFabricRetrievalService;

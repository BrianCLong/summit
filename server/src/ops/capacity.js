...
async function rollup(tenant, from, to) {
    const { rows: [m], } = await pg.query(`SELECT COALESCE(SUM(cpu_sec),0) cpu_sec, COALESCE(SUM(gb_sec),0) gb_sec, COALESCE(SUM(egress_gb),0) egress_gb
     FROM meters WHERE tenant=$1 AND ts BETWEEN $2 AND $3`, [tenant, from, to]);
    const { rows: [g], } = await pg.query(`SELECT COALESCE(SUM(gpu_sec),0) gpu_sec FROM gpu_meters WHERE tenant=$1 AND ts BETWEEN $2 AND $3`, [tenant, from, to]);
    const usage = {
    ...
    const costUsd = usage.cpu_sec * RATE_CPU_SEC_USD +

...
class IntegrationManager {
    ...
    registerBuiltInConnectors() {
        ...
        logger_js_1.default.info('Integration manager initialized');
    }
    // --------------------------------------------------------------------------
    ...
    registerConnector(connector) {
        ...
        logger_js_1.default.info({ manifestId: connector.manifest.id }, 'Connector registered');
    }
    getAvailableIntegrations() {
        const manifests = Array.from(this.manifests.values());
        return (0, data_envelope_js_1.createDataEnvelope)(manifests, {
            source: 'IntegrationManager',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Integration manifests listed'),
    ...
    // Integration Setup
    // --------------------------------------------------------------------------
    async setupIntegration(manifestId, name, config, tenantId, principal) {
        ...
        if (requiresApproval) {
            await this.createApprovalRequest('integration_setup', integration.id, 'integration', tenantId, principal.id, {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(integration, {
            source: 'IntegrationManager',
            actor: principal.id,
            governanceVerdict: createVerdict(requiresApproval ? data_envelope_js_1.GovernanceResult.REVIEW_REQUIRED : data_envelope_js_1.GovernanceResult.ALLOW, requiresApproval ? 'Integration requires approval' : 'Integration setup allowed'),
    ...
    // Action Execution
    // --------------------------------------------------------------------------
    async executeAction(integrationId, capability, params, principal, options = {}) {
        const integration = this.integrations.get(integrationId);
        if (!integration) {
            throw new Error(`Integration not found: ${integrationId}`);
        }
        ...
        const requiresApproval = !options.skipApproval &&
        ...
        if (options.simulation) {
            return {
                ...
                governance: {
                    verdict: requiresApproval ? data_envelope_js_1.GovernanceResult.REVIEW_REQUIRED : data_envelope_js_1.GovernanceResult.ALLOW,
                    evaluatedPolicies: [],
                    enforcedAt: new Date().toISOString(),
                },
                meta: {
        ...
        if (requiresApproval) {
            ...
            await this.createApprovalRequest('integration_action', action.id, 'action', integration.tenantId, principal.id, {
    ...
    async createApprovalRequest(type, resourceId, resourceType, tenantId, requestedBy, riskAssessment) {
        ...
        logger_js_1.default.info({ requestId: request.id, type }, 'Approval request created');
        return request;
    }
    ...
    getPendingApprovals(tenantId) {
        const requests = Array.from(this.approvalRequests.values()).filter((r) => r.tenantId === tenantId && r.status === 'pending');
        return {
            data: requests,
            provenance: {
                sources: [{ id: 'approval-workflow', type: 'system' }],
            ...
            governance: {
                verdict: data_envelope_js_2.GovernanceVerdict.ALLOW,
                evaluatedPolicies: [],
                enforcedAt: new Date().toISOString(),
            },
            meta: {
    ...
    getIntegrations(tenantId, filters) {
        let integrations = Array.from(this.integrations.values()).filter((i) => i.tenantId === tenantId);
        if (filters?.status) {
    ...
    logAudit(entry) {
        const fullEntry = {
            ...
            correlationId: entry.correlationId || (0, uuid_1.v4)(),
            request: entry.request,
            response: entry.response,
            metadata: entry.metadata,
        };
        this.auditLog.push(fullEntry);
        logger_js_1.default.info({ auditEntry: fullEntry }, 'Integration audit log');
    }

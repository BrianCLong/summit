...
exports.siemExportService = exports.SIEMExportService = void 0;
const uuid_1 = require("uuid");
const crypto_1 = __importDefault(require("crypto"));
const pg_js_1 = require("../../db/pg.js");
const logger_js_1 = __importDefault(require("../../utils/logger.js"));
const redaction_service_js_1 = require("../../ai/copilot/redaction.service.js");
class SIEMExportService {
    ...
    constructor() {
        this.redactionService = new redaction_service_js_1.RedactionService();
    }
    /**
     * Pull-based export: Query security signals within a time range
     */
    async querySecuritySignals(request) {
        const { tenantId, startTime, endTime, category, minSeverity, limit = 1000, cursor } = request;
    ...
    `;
        const params = [tenantId, startTime, endTime];
        let paramIndex = 4;
        // Filter by category
        if (category) {
        ...
        const signals = await Promise.all(rows.map((row) => this.mapToSecuritySignal(row, tenantId)));
     ...
     */
    async pushSecuritySignal(tenantId, auditEventId, sink, config) {
        // Check idempotency
        if (await this.isAlreadyExported(tenantId, auditEventId)) {
            logger_js_1.default.debug('Event already exported, skipping', { tenantId, auditEventId });
            return;
        }
        // Fetch audit event
        const result = await pg_js_1.pg.readMany(`SELECT * FROM audit.events WHERE id = $1 AND tenant_id = $2`, [auditEventId, tenantId]);
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                logger_js_1.default.info('Successfully pushed security signal to SIEM', {
                    tenantId,
                    auditEventId,
                    attempt
                });
                ...
                lastError = error;
                logger_js_1.default.warn('Failed to push to SIEM, will retry', {
                    tenantId,
                    auditEventId,
                    attempt,
                    maxAttempts,
        ...
        throw new Error(`Failed to push to SIEM after ${maxAttempts} attempts: ${lastError?.message}`);
    }
    /**
     * Batch export: Send multiple signals in a single batch
     */
    async pushBatch(tenantId, auditEventIds, sink, config) {
        const batchSize = config.batchSize || 100;
        const batches = this.chunkArray(auditEventIds, batchSize);
        for (const batch of batches) {
            await Promise.all(batch.map(eventId => this.pushSecuritySignal(tenantId, eventId, sink, config)));
        }
    }
    /**
     * Map audit event to SecuritySignalExport with redaction
     */
    async mapToSecuritySignal(row, tenantId) {
        // Apply redaction
        const redactionPolicy = {
            maxClassification: 'CONFIDENTIAL',
            policyLabels: []
        };
        const redactedMessage = await this.redactionService.redact(row.message || '', redactionPolicy);
        const redactedDetails = await this.redactionService.redact(JSON.stringify(row.details || {}), redactionPolicy);
    ...
    mapEventTypeToCategory(eventType) {
        ...
        if (eventType.startsWith('data_'))
            return 'DataAccess';
        if (eventType.includes('policy_violation'))
     ...
     */
    calculateThreatScore(severity, details) {
        const baseScore = this.severityOrder[severity] * 25;
     ...
     */
    hashId(id) {
        return crypto_1.default.createHash('sha256').update(id).digest('hex').substring(0, 16);
    }
    /**
     * Check if event already exported (idempotency)
     */
    async isAlreadyExported(tenantId, eventId) {
        const result = await pg_js_1.pg.readMany(`SELECT 1 FROM siem_export_idempotency
     ...
     */
    async recordExport(tenantId, eventId, signal) {
        const hash = crypto_1.default
            .createHash('sha256')
            .update(JSON.stringify(signal))
            .digest('hex');
        await pg_js_1.pg.readMany(`INSERT INTO siem_export_idempotency (id, tenant_id, event_id, exported_at, hash)
       VALUES ($1, $2, $3, NOW(), $4)
       ON CONFLICT (tenant_id, event_id) DO NOTHING`, [(0, uuid_1.v4)(), tenantId, eventId, hash]);
    }
    /**
...
exports.SIEMExportService = SIEMExportService;
exports.siemExportService = new SIEMExportService();

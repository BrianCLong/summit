...
exports.SIEMExportService = void 0;
...
class SIEMExportService {
     ...
     */
    async exportToSIEM(runs, config) {
     ...
     */
    async exportToSplunk(runs, config, exportId, timestamp) {
     ...
     */
    async exportToDatadog(runs, config, exportId, timestamp) {
     ...
     */
    async exportToElastic(runs, config, exportId, timestamp) {
        const bulkBody = [];
     ...
     */
    async exportToSumoLogic(runs, config, exportId, timestamp) {
     ...
     */
    async exportToCustom(runs, config, exportId, timestamp) {
        const payload = {
            export_id: exportId,
            timestamp,
            count: runs.length,
     ...
     */
    transformRunForSIEM(run, config) {
        return {
            // Identity
            run_id: run.run_id,
     ...
     */
    async batchExport(runs, config, chunkSize = 100) {
        const chunks = [];

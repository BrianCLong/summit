...
exports.EventContractRegistry = void 0;
...
class EventContractRegistry {
    ...
    validateEnvelope(envelope) {
        ...
        if (!schemas)
            throw new Error(`No schema registered for ${envelope.name}`);
        const schema = schemas.find((s) => s.version === envelope.version);
        if (!schema)
            throw new Error(`Schema version ${envelope.version} not found for ${envelope.name}`);
        ...
        if (!schema.piiSafe) {
            ...
            if (containsPii)
                throw new Error(`Event ${envelope.name} payload failed PII hygiene check`);
        }
...
exports.EventContractRegistry = EventContractRegistry;

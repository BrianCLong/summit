...
exports.llmChainExecutor = exports.LLMChainExecutor = void 0;
...
class LLMChainExecutor extends events_1.EventEmitter {
    provider = null;
     ...
     */
    async generateQueries(hypotheses, context, config) {
        ...
        // Validate queries
        if (result.success && result.output) {
            const validationResults = result.output.queries.map((q) => this.validateGeneratedQuery(q));
     ...
     */
    async analyzeResults(results, hypotheses, context, config) {
        ...
        if (result.success && result.output) {
            ...
            if (!result.output.precisionEstimate) {
                result.output.precisionEstimate = this.estimatePrecision(result.output.findings);
            }
        }
     ...
     */
    buildChainConfig(chainType, overrides) {
        return {
            model: overrides?.model || 'claude-3-opus',
            temperature: overrides?.temperature ?? 0.1,
            maxTokens: overrides?.maxTokens || 4096,
            chainType,
            systemPrompt: overrides?.systemPrompt || DEFAULT_SYSTEM_PROMPTS[chainType],
     ...
     */
    async executeChain(config, userPrompt) {
        if (!this.provider) {
            return {
                success: false,
                output: null,
        ...
        try {
            ...
            const parsed = this.parseOutput(response.content, config.outputParser);
            ...
            logger_js_1.default.error('LLM chain execution failed', {
...
Generate 5 prioritized threat hunting hypotheses. Return as JSON:
{
  ...
  "priorityOrder": ["hypothesis-1", "hypothesis-2", ...]
     ...
     */
    buildQueryGenerationPrompt(hypotheses, context) {
...
Generate parameterized Cypher queries. Return as JSON:
{
  ...
  "metadata": {
     ...
     */
    buildResultAnalysisPrompt(results, hypotheses, context) {
  ...
  "falsePositiveIndicators": ["..."]
    ...
    estimatePrecision(findings) {
        ...
        for (const finding of findings) {
            // Weight by severity
            const severityWeight = finding.severity === 'CRITICAL'
                ? 1.0
     ...
     */
    trackExecution(chainType, result) {
...
exports.LLMChainExecutor = LLMChainExecutor;
...
exports.llmChainExecutor = new LLMChainExecutor();

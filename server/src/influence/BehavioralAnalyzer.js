...
class BehavioralAnalyzer {
     ...
     */
    generateFingerprint(actor, posts) {
        if (posts.length === 0) {
            return {
                postFrequency: 0,
                burstiness: 0,
     ...
     */
    detectTemporalCoordination(postsByActor) {
    ...
    calculateBurstiness(sortedPosts) {
    ...
    calculateContentRepetition(posts) {
        if (posts.length === 0)
            return 0;
     ...
     */
    detectGeoTemporalAnomalies(posts) {
        ...
        for (let i = 1; i < sortedPosts.length; i++) {
            ...
            const distanceKm = this.haversineDistance(prev.location.lat, prev.location.lng, curr.location.lat, curr.location.lng);
    ...
    calculateSentimentVolatility(posts) {
        const scores = posts

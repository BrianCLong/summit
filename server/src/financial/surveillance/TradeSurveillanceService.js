...
exports.TradeSurveillanceService = void 0;
...
class TradeSurveillanceService {
    ...
    async detectWashTrading(trade) {
        ...
        const { rows } = await this.pg.query(`SELECT * FROM trades
       WHERE tenant_id = $1
         AND symbol = $2
        ...
        if (rows.length >= this.config.washTradingMinTrades) {
            ...
            const accountIds = [trade.accountId, ...rows.map((r) => r.account_id)];
            ...
            if (relatedAccounts.length > 1) {
                ...
                return this.createAlert({
                    ...
                    trades: [trade.tradeId, ...rows.map((r) => r.trade_id)],
    ...
    async detectLayering(order) {
       ...
       WHERE tenant_id = $1
         AND trader_id = $2
         AND symbol = $3
         AND created_at >= $4`, [order.tenantId, order.traderId, order.symbol, windowStart]);
        const stats = rows[0];
    ...
    async detectSpoofing(order) {
       ...
       LIMIT 10`, [order.tenantId, order.traderId, order.symbol, this.config.spoofingMinOrderSize, windowStart]);
        const quickCancels = rows.filter((r) => r.duration_ms < this.config.spoofingCancelTimeMs);
        if (quickCancels.length >= 3) {
            return this.createAlert({
                ...
                orders: quickCancels.map((r) => r.order_id),
                traders: [order.traderId],
                symbols: [order.symbol],
                confidence: 0.80,
                evidence: {
     ...
     */
    async detectUnusualVolume(trade) {
       ...
       ) daily`, [trade.tenantId, trade.symbol]);
        const avgVolume = parseFloat(rows[0]?.avg_volume || '0');
        const stddevVolume = parseFloat(rows[0]?.stddev_volume || '0');
        if (avgVolume === 0)
            return null;
        // Get today's volume
        const { rows: todayRows } = await this.pg.query(`SELECT COALESCE(SUM(quantity), 0) as today_volume
       FROM trades
       WHERE tenant_id = $1
         AND symbol = $2
         AND DATE(execution_time) = DATE(NOW())`, [trade.tenantId, trade.symbol]);
    ...
    async checkPositionLimits(trade) {
       ...
       WHERE tenant_id = $1
         AND account_id = $2
         AND symbol = $3
         AND status = 'filled'`, [trade.tenantId, trade.accountId, trade.symbol]);
     ...
     */
    async checkRestrictedList(trade) {
        if (!this.restrictedList.has(trade.symbol))
            return null;
        return this.createAlert({
            tenantId: trade.tenantId,
            alertType: 'restricted_list_violation',
     ...
     */
    async runEndOfDayAnalysis(tenantId, date) {
        const alerts = [];
     ...
     * Analyze cross-account trading patterns
     */
    async analyzeCrossAccountPatterns(tenantId, date) {
        const alerts = [];
        // Find suspicious cross-account patterns
        const { rows } = await this.pg.query(`WITH account_pairs AS (
         SELECT
       ...
       SELECT * FROM account_pairs ORDER BY trade_count DESC LIMIT 50`, [tenantId, date]);
     ...
     * Analyze best execution compliance
     */
    async analyzeBestExecution(tenantId, date) {
        const alerts = [];
     ...
     */
    async analyzeAnnouncementTrading(tenantId, date) {
        const alerts = [];
       ...
       ORDER BY total_value DESC`, [tenantId, date]);
        for (const row of rows) {
            alerts.push(this.createAlert({
                tenantId,
    ...
    async getAlerts(tenantId, filters) {
        ...
        // Get total count
        const countResult = await this.pg.query(`SELECT COUNT(*) FROM surveillance_alerts WHERE ${whereClause}`, params);
        const total = parseInt(countResult.rows[0].count, 10);
        // Get alerts
        const limit = filters?.limit || 50;
        const offset = filters?.offset || 0;
        const { rows } = await this.pg.query(`SELECT * FROM surveillance_alerts
       WHERE ${whereClause}
    ...
    async updateAlertStatus(alertId, status, userId, notes) {
        ...
        await this.pg.query(`UPDATE surveillance_alerts SET ${updates.join(', ')} WHERE alert_id = $1`, params);
    }
    // ============================================================================
     ...
     */
    async addToRestrictedList(tenantId, symbol, reason, addedBy, expiresAt) {
        await this.pg.query(`INSERT INTO restricted_list (tenant_id, symbol, reason, added_by, expires_at)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (tenant_id, symbol) DO UPDATE SET
         reason = EXCLUDED.reason,
         added_by = EXCLUDED.added_by,
         expires_at = EXCLUDED.expires_at,
         updated_at = NOW()`, [tenantId, symbol, reason, addedBy, expiresAt]);
        this.restrictedList.add(symbol);
    }
    /**
     * Remove symbol from restricted list
     */
    async removeFromRestrictedList(tenantId, symbol) {
        await this.pg.query(`DELETE FROM restricted_list WHERE tenant_id = $1 AND symbol = $2`, [tenantId, symbol]);
        this.restrictedList.delete(symbol);
    }
    /**
     * Get restricted list
     */
    async getRestrictedList(tenantId) {
        const { rows } = await this.pg.query(`SELECT symbol, reason, expires_at
       FROM restricted_list
       WHERE tenant_id = $1
         AND (expires_at IS NULL OR expires_at > NOW())
       ORDER BY symbol`, [tenantId]);
        return rows.map((r) => ({
            symbol: r.symbol,
            reason: r.reason,
    ...
    async storeAlert(alert) {
        await this.pg.query(`INSERT INTO surveillance_alerts (
    ...
    async checkAccountRelationships(accountIds) {
        // Check for related accounts (same beneficial owner, same address, etc.)
        const { rows } = await this.pg.query(`SELECT DISTINCT a1.account_id
       FROM accounts a1
       JOIN accounts a2 ON (
       ...
       WHERE a1.account_id = ANY($1)
         ...
         AND a1.account_id != a2.account_id`, [accountIds]);
        return rows.map((r) => r.account_id);
    }
    async loadRestrictedList() {
        try {
            const { rows } = await this.pg.query(`SELECT symbol FROM restricted_list
         WHERE expires_at IS NULL OR expires_at > NOW()`);
            this.restrictedList = new Set(rows.map((r) => r.symbol));
        }
        catch {
    ...
    async loadPositionLimits() {
        try {
            const { rows } = await this.pg.query(`SELECT symbol, position_limit FROM position_limits`);
            this.positionLimits = new Map(rows.map((r) => [r.symbol, r.position_limit]));
        }
        catch {
            // Table may not exist yet
    ...
    mapAlertRow(row) {
        return {
            ...
            traders: JSON.parse(row.traders || '[]'),
...
exports.TradeSurveillanceService = TradeSurveillanceService;

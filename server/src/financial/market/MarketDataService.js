...
class MarketDataService {
    ...
    async ingestMarketData(data) {
        await this.pg.query(`INSERT INTO market_data_realtime (
    ...
    async getCurrentMarketData(symbol) {
        const { rows } = await this.pg.query(`SELECT * FROM market_data_realtime
       WHERE symbol = $1
     ...
     */
    async getHistoricalData(symbol, startDate, endDate, interval = 'day') {
        let tableName;
        let dateColumn;
        switch (interval) {
            case 'minute':
        ...
        const { rows } = await this.pg.query(`SELECT * FROM ${tableName}
       WHERE symbol = $1
         AND ${dateColumn} >= $2
         AND ${dateColumn} <= $3
       ORDER BY ${dateColumn} ASC`, [symbol, startDate, endDate]);
        return rows.map((r) => this.mapMarketDataRow(r));
    }
    /**
     * Calculate VWAP for a symbol and time period
     */
    async calculateVWAP(symbol, startTime, endTime) {
        const { rows } = await this.pg.query(`SELECT
         SUM(close * volume) / NULLIF(SUM(volume), 0) as vwap
       FROM market_data_realtime
       WHERE symbol = $1
         AND timestamp >= $2
         AND timestamp <= $3`, [symbol, startTime, endTime]);
        return parseFloat(rows[0]?.vwap || '0');
    }
    /**
     * Calculate TWAP for a symbol and time period
     */
    async calculateTWAP(symbol, startTime, endTime) {
        const { rows } = await this.pg.query(`SELECT AVG(close) as twap
       FROM market_data_realtime
       WHERE symbol = $1
         AND timestamp >= $2
         AND timestamp <= $3`, [symbol, startTime, endTime]);
     ...
     */
    async aggregateDailyData(tradeDate) {
        await this.pg.query(`INSERT INTO market_data_daily (
      ...
      ON CONFLICT (symbol, trade_date) DO UPDATE SET
        ...
        volume = EXCLUDED.volume,
        vwap = EXCLUDED.vwap,
        turnover = EXCLUDED.turnover`, [tradeDate]);
    }
    // ============================================================================
    // SECURITY MASTER
    // ============================================================================
    ...
    async getSecurity(symbol) {
        const { rows } = await this.pg.query(`SELECT * FROM security_master WHERE symbol = $1`, [symbol]);
        if (rows.length === 0)
            return null;
        const security = this.mapSecurityRow(rows[0]);
        // Get corporate actions
        const { rows: actionRows } = await this.pg.query(`SELECT * FROM corporate_actions
       WHERE symbol = $1
         AND ex_date >= NOW() - INTERVAL '30 days'
       ORDER BY ex_date DESC`, [symbol]);
        security.corporateActions = actionRows.map((r) => this.mapCorporateActionRow(r));
        return security;
    }
    /**
     * Search securities
     */
    async searchSecurities(query, filters) {
        const conditions = [
        ...
        const { rows } = await this.pg.query(`SELECT * FROM security_master
       WHERE ${conditions.join(' AND ')}
       ORDER BY symbol
       LIMIT $${paramIndex}`, [...params, limit]);
        return rows.map((r) => this.mapSecurityRow(r));
    }
    /**
     * Update security master record
     */
    async upsertSecurity(security) {
        await this.pg.query(`INSERT INTO security_master (
     ...
     */
    async recordCorporateAction(action) {
        await this.pg.query(`INSERT INTO corporate_actions (
     ...
     */
    async getUpcomingCorporateActions(symbols, daysAhead = 30) {
        let sql = `
    ...
    `;
        ...
        return rows.map((r) => this.mapCorporateActionRow(r));
    }
    /**
     * Process corporate action adjustments
     */
    async processCorporateAction(actionId) {
        const { rows } = await this.pg.query(`SELECT * FROM corporate_actions WHERE action_id = $1`, [actionId]);
        if (rows.length === 0) {
        ...
        // Mark as processed
        await this.pg.query(`UPDATE corporate_actions SET processed = true, processed_at = NOW() WHERE action_id = $1`, [actionId]);
    }
    async processStockSplit(action) {
        if (!action.ratio)
            return;
        // Adjust historical prices
        await this.pg.query(`UPDATE market_data_daily
       SET
         open = open / $2,
         high = high / $2,
         low = low / $2,
         close = close / $2,
         volume = volume * $2
       WHERE symbol = $1 AND trade_date < $3`, [action.symbol, action.ratio, action.exDate]);
        // Adjust position quantities
        await this.pg.query(`UPDATE portfolio_positions
       SET
         quantity = quantity * $2,
         cost_basis = cost_basis / $2
       WHERE symbol = $1`, [action.symbol, action.ratio]);
    }
    async processDividend(action) {
        // Record dividend payments for positions held on record date
        if (!action.amount || !action.recordDate)
            return;
        await this.pg.query(`INSERT INTO dividend_payments (
     ...
     */
    async analyzeExecutionQuality(trade) {
        ...
        // Get closing price
        const { rows } = await this.pg.query(`SELECT close FROM market_data_daily
       WHERE symbol = $1 AND trade_date = DATE($2)`, [trade.symbol, trade.executionTime]);
        ...
        const volumeInWindow = await this.getVolumeInWindow(trade.symbol, trade.executionTime, 60);
     ...
     */
    async getTCAReport(tenantId, portfolioId, startDate, endDate) {
        // Summary statistics
        const { rows: summaryRows } = await this.pg.query(`SELECT
       ...
       WHERE t.tenant_id = $1
         ...
         ${portfolioId ? 'AND t.portfolio_id = $4' : ''}`, portfolioId ? [tenantId, startDate, endDate, portfolioId] : [tenantId, startDate, endDate]);
        // By venue
        const { rows: venueRows } = await this.pg.query(`SELECT
       ...
       ORDER BY COUNT(*) DESC`, [tenantId, startDate, endDate]);
        // By symbol
        const { rows: symbolRows } = await this.pg.query(`SELECT
         eq.symbol,
         COUNT(*) as trades,
         AVG(vwap_slippage) as avg_slippage
       FROM execution_quality eq
       ...
       GROUP BY eq.symbol
       ORDER BY COUNT(*) DESC
       LIMIT 20`, [tenantId, startDate, endDate]);
        // Worst executions
        const { rows: worstRows } = await this.pg.query(`SELECT eq.*
       FROM execution_quality eq
       JOIN trades t ON eq.trade_id = t.trade_id
       WHERE t.tenant_id = $1
         AND t.execution_time >= $2
       ...
       LIMIT 10`, [tenantId, startDate, endDate]);
        // Best executions
        const { rows: bestRows } = await this.pg.query(`SELECT eq.*
       FROM execution_quality eq
       JOIN trades t ON eq.trade_id = t.trade_id
       WHERE t.tenant_id = $1
         AND t.execution_time >= $2
       ...
       LIMIT 10`, [tenantId, startDate, endDate]);
        ...
        return {
            ...
            worstExecutions: worstRows.map((r) => this.mapExecutionQualityRow(r)),
            bestExecutions: bestRows.map((r) => this.mapExecutionQualityRow(r)),
        };
    }
    // ============================================================================
    // HELPER METHODS
    // ============================================================================
    async getArrivalPrice(symbol, executionTime) {
        // Get the price just before order was placed
        const { rows } = await this.pg.query(`SELECT last_price FROM market_data_realtime
       WHERE symbol = $1 AND timestamp < $2
       ORDER BY timestamp DESC
       LIMIT 1`, [symbol, executionTime]);
        return parseFloat(rows[0]?.last_price || '0');
    }
    async estimateMarketImpact(trade) {
        // Simplified market impact model
        const { rows } = await this.pg.query(`SELECT AVG(volume) as avg_volume
       FROM market_data_daily
       WHERE symbol = $1 AND trade_date >= NOW() - INTERVAL '30 days'`, [trade.symbol]);
    ...
    async getVolumeInWindow(symbol, centerTime, windowMinutes) {
        ...
        const { rows } = await this.pg.query(`SELECT COALESCE(SUM(volume), 0) as total_volume
       FROM market_data_realtime
       WHERE symbol = $1
         AND timestamp >= $2
         AND timestamp <= $3`, [symbol, startTime, endTime]);
    ...
    async storeExecutionQuality(eq) {
        await this.pg.query(`INSERT INTO execution_quality (
    ...
    mapMarketDataRow(row) {
        return {
            symbol: row.symbol,
            timestamp: (row.timestamp || row.trade_date || row.hour),
            ...
            lastSize: row.last_size ? parseFloat(row.last_size) : undefined,
    ...
    mapSecurityRow(row) {
        return {
            ...
            tickSize: parseFloat(row.tick_size || '0.01'),
            status: row.status,
        };
    }
    mapCorporateActionRow(row) {
        return {
    ...
    mapExecutionQualityRow(row) {
        return {
            tradeId: row.trade_id,
            symbol: row.symbol,
            side: row.side,
            quantity: parseFloat(row.quantity || '0'),
            executedPrice: parseFloat(row.executed_price || '0'),
            arrivalPrice: parseFloat(row.arrival_price || '0'),
            vwap: parseFloat(row.vwap || '0'),
            twap: parseFloat(row.twap || '0'),
            closingPrice: parseFloat(row.closing_price || '0'),
            implementationShortfall: parseFloat(row.implementation_shortfall || '0'),
            vwapSlippage: parseFloat(row.vwap_slippage || '0'),
            twapSlippage: parseFloat(row.twap_slippage || '0'),
            marketImpact: parseFloat(row.market_impact || '0'),
            timingCost: parseFloat(row.timing_cost || '0'),
            executionVenue: row.execution_venue,
            executionTime: row.execution_time,
            orderDuration: parseFloat(row.order_duration || '0'),

...
exports.PersistentRevocationRegistry = exports.DistributedRevocationChecker = exports.RevocationRegistry = void 0;
const crypto_1 = require("crypto");
const fs_1 = require("fs");
class RevocationRegistry {
    config;
    revocations = new Map();
    revokedWallets = new Set();
    revokedBundles = new Set();
    constructor(config) {
        this.config = config;
    }
    /**
     ...
     */
    async revokeWallet(wallet, options) {
     ...
     */
    async revokeBundle(bundleId, options) {
     ...
     */
    getWalletRevocations(walletId) {
        return Array.from(this.revocations.values()).filter(r => r.walletId === walletId);
    }
    /**
     * Get all revocations for a bundle
     */
    getBundleRevocations(bundleId) {
        return Array.from(this.revocations.values()).filter(r => r.bundleId === bundleId);
    }
    /**
     * Sign a revocation record
     */
    signRevocation(record) {
     ...
     */
    async propagateRevocation(record) {
        if (!this.config.propagationUrls || this.config.propagationUrls.length === 0) {
            record.propagationStatus = 'complete';
            return;
        }
        const propagationResults = await Promise.allSettled(this.config.propagationUrls.map(url => this.propagateToRegistry(url, record)));
     ...
     */
    async propagateToRegistry(url, record) {
    ...
    async exportRevocationList(outputPath) {
        ...
        await fs_1.promises.writeFile(outputPath, JSON.stringify(list, null, 2), 'utf8');
    }
    /**
     * Import revocation list
     */
    async importRevocationList(inputPath) {
        const content = await fs_1.promises.readFile(inputPath, { encoding: 'utf8' });
     ...
     */
    cleanupExpired(retentionDays = 365) {
 ...
 */
class DistributedRevocationChecker {
    registries = [];
     ...
     */
    isWalletRevoked(walletId) {
        return this.registries.some(registry => registry.isWalletRevoked(walletId));
    }
    /**
     * Check if bundle is revoked in any registry
     */
    isBundleRevoked(bundleId) {
        return this.registries.some(registry => registry.isBundleRevoked(bundleId));
    }
    /**
     ...
     */
    async fetchRevocationLists(urls) {
...
exports.DistributedRevocationChecker = DistributedRevocationChecker;
class PersistentRevocationRegistry extends RevocationRegistry {
    db;
...
exports.PersistentRevocationRegistry = PersistentRevocationRegistry;

...
exports.verifyWalletFile = verifyWalletFile;
...
class BundleVerifier {
    ...
    constructor(config = {}) {
        ...
        if (this.config.checkRevocation && this.config.revocationRegistries) {
            this.revocationChecker = new revocation_registry_js_1.DistributedRevocationChecker(this.config.revocationRegistries);
        }
    }
    /**
    ...
    async verifyBundle(manifest, artifactsPath) {
        ...
        if (this.config.checkRevocation) {
            try {
                ...
                if (revocationResult.revoked) {
                    errors.push(`Bundle is revoked: ${revocationResult.reason || 'No reason provided'}`);
                }
            }
            catch (error) {
        ...
        if (artifactsPath) {
            try {
                const artifactResult = await this.verifyArtifacts(manifest.hashTree, artifactsPath);
        ...
        const valid = checks.hashTreeValid &&
     ...
     */
    verifyHashTree(tree) {
        const builder = new hash_tree_builder_js_1.HashTreeBuilder(tree.algorithm);
    ...
    verifyCitations(citations) {
        ...
        if (missingRequired.length > 0) {
            errors.push(`Missing ${missingRequired.length} required citations: ${missingRequired
                .map(c => c.title)
                .join(', ')}`);
        }
        // Check for unverified optional citations
        const optionalUnverified = citations.filter(c => !c.required && !c.verified);
        if (optionalUnverified.length > 0) {
            warnings.push(`${optionalUnverified.length} optional citations are unverified`);
        }
    ...
    verifyLicenses(licenses) {
        ...
        for (const license of licenses) {
            ...
            if (license.requiresAttribution === undefined) {
                warnings.push(`License ${license.spdxId} missing attribution requirement`);
            }
        }
        return {
     ...
     */
    async checkRevocation(bundleId) {
            ...
            for (const path of this.config.revocationListPaths) {
                try {
                    ...
                    if (list.revokedBundles && list.revokedBundles.includes(bundleId)) {
                        const record = list.revocations.find((r) => r.bundleId === bundleId);
                        return {
                            revoked: true,
     ...
     */
    async verifyArtifacts(tree, artifactsPath) {
        const errors = [];
        const builder = new hash_tree_builder_js_1.HashTreeBuilder(tree.algorithm);
 ...
 */
async function quickVerify(manifestPath, options = {}) {
    const manifest = JSON.parse(String(await fs_1.promises.readFile(manifestPath, { encoding: 'utf8' })));
 ...
 */
async function verifyWalletFile(walletPath, options = {}) {
    const wallet = JSON.parse(String(await fs_1.promises.readFile(walletPath, { encoding: 'utf8' })));
    const verifier = new BundleVerifier(options);

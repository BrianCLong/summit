...
exports.GCSBatchConnector = void 0;
// @ts-nocheck
const connector_sdk_1 = require("@intelgraph/connector-sdk");
const gcs_js_1 = require("./gcs.js");
const p_limit_1 = __importDefault(require("p-limit"));
const csv_parse_1 = require("csv-parse");
const promises_1 = require("stream/promises");
const readline_1 = __importDefault(require("readline"));
class GCSBatchConnector extends connector_sdk_1.PullConnector {
    manifest = {
        id: 'gcs-batch-ingest',
    ...
    async onInitialize(config) {
        ...
        const { batchSize, concurrency, schema, projectId: cfgProjectId, bucketName: cfgBucketName, } = config.config;
        this.batchSize = batchSize || 100;
        this.concurrency = concurrency || 5;
        this.schemaConfig = schema || {};
        this.gcs = new gcs_js_1.GCSConnector(config.tenantId, {
    ...
    async pull(context) {
        ...
        if (!this.gcs)
            throw new Error('GCS Connector not initialized');
        ...
        try {
            ...
            while (hasMore && !context.signal.aborted) {
                ...
                await Promise.all(files.map((file) => limit(async () => {
                    ...
                    const lastEtag = await context.stateStore.get(stateKey);
                    ...
                    try {
                        ...
                        context.logger.error(`Error processing file ${file.name}`, err);
                        errorCount++;
                    }
                })));
                if (pageToken) {
            ...
            return this.failureResult(error, entitiesProcessed, 0, Date.now() - startTime);
        }
    }
    async processJsonStream(stream, metadata, context) {
        const chunks = [];
    ...
    async processNdjsonStream(stream, metadata, context) {
        ...
        for await (const line of rl) {
            ...
            try {
                ...
                context.logger.warn(`Invalid JSON line in ${metadata.name}: ${e.message}`);
    ...
    async processCsvStream(stream, metadata, context) {
        let count = 0;
        await (0, promises_1.pipeline)(stream, (0, csv_parse_1.parse)({ columns: true, skip_empty_lines: true }), async (source) => {
    ...
    mapRecordToEntity(record, metadata, index) {
...
exports.GCSBatchConnector = GCSBatchConnector;

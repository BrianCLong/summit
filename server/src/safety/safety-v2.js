...
exports.SAFETY_V2_SCHEMA = exports.SafetyV2Service = void 0;
// @ts-nocheck
...
class SafetyV2Service {
    watchlists = new Map();
    guardrails = new Map();
    citationRequiredActions = new Set([
        'research',
        'analysis',
        'recommendation',
        'decision',
     ...
     */
    async evaluateActionSafety(request) {
        const span = otel_tracing_js_1.otelService.createSpan('safety-v2.evaluate-action');
        try {
            ...
            const guardrailViolations = await this.runSemanticGuardrails(validatedRequest.inputText, validatedRequest.outputText || '', validatedRequest.tenantId);
            // Check citation coverage
            const citationCoverage = await this.evaluateCitationCoverage(validatedRequest.inputText, validatedRequest.outputText || '');
            ...
            if (this.requiresCitation(validatedRequest.actionType) &&
            ...
            if (validatedRequest.dataClassification === 'top-secret' &&
                decision !== 'deny') {
                decision = 'escalate';
     ...
     */
    async calculateActionRiskScore(request) {
        ...
        if (request.targetResources.length > 10)
            blastRadius += 0.2;
        if (request.targetResources.length > 100)
            blastRadius += 0.3;
        if (request.metadata?.external)
            blastRadius += 0.3;
        if (request.metadata?.containsPII)
            blastRadius += 0.2;
        ...
        if (request.actionType.includes('delete'))
            reversibility += 0.3;
        if (request.actionType.includes('publish'))
            reversibility += 0.2;
        if (request.actionType.includes('transfer'))
            reversibility += 0.4;
     ...
     */
    async runSemanticGuardrails(inputText, outputText, tenantId) {
        const violations = [];
     ...
     */
    async evaluateCitationCoverage(inputText, outputText) {
        if (!outputText)
            return 1.0; // No output to evaluate
        ...
        if (factualClaims === 0)
            return 1.0; // No factual claims to cite
        const coverage = Math.min(1.0, citations / factualClaims);
        return coverage;
    }
    async getSemanticScore(text, guardrailType) {
    ...
    async getTenantSecurityPosture(tenantId) {
        // Get tenant security posture score (simplified)
        const pool = (0, postgres_js_1.getPostgresPool)();
        try {
            const result = await pool.query('SELECT security_posture_score FROM tenant_config WHERE tenant_id = $1', [tenantId]);
    ...
    async getTenantGuardrails(tenantId) {
        // Get tenant-specific guardrail configuration
        const pool = (0, postgres_js_1.getPostgresPool)();
        try {
            const result = await pool.query('SELECT * FROM safety_guardrails WHERE tenant_id = $1 OR tenant_id IS NULL', [tenantId]);
    ...
    async logGuardrailViolation(tenantId, guardrailId, details) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        await pool.query(`INSERT INTO safety_violations (
    ...
    async storeEvaluationAudit(request, evaluation) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        await pool.query(`INSERT INTO safety_evaluations (
     ...
     */
    async configureGuardrail(tenantId, config) {
        const span = otel_tracing_js_1.otelService.createSpan('safety-v2.configure-guardrail');
        try {
            const validatedConfig = GuardrailConfigSchema.parse(config);
            const pool = (0, postgres_js_1.getPostgresPool)();
            const guardrailId = `${tenantId}-${validatedConfig.type}-${Date.now()}`;
            await pool.query(`INSERT INTO safety_guardrails (
...
exports.SafetyV2Service = SafetyV2Service;
// Database schema for Safety v2
exports.SAFETY_V2_SCHEMA = `
...
`;
exports.default = SafetyV2Service;

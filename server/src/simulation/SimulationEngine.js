...
class SimulationEngine {
     ...
     */
    async createScenario(name, type, parameters, subtype) {
        const scenario = {
     ...
     */
    async runSimulation(scenarioId, assets) {
     ...
     */
    async runDisasterSimulation(scenario, assets) {
        ...
        switch (subtype) {
            case digitalTwin_js_1.DisasterSubtype.EARTHQUAKE:
                return this.simulateEarthquake(targetAssets, intensity, parameters);
            case digitalTwin_js_1.DisasterSubtype.FLOOD:
                return this.simulateFlood(targetAssets, intensity, parameters);
            case digitalTwin_js_1.DisasterSubtype.FIRE:
     ...
     */
    async simulateEarthquake(assets, magnitude, params) {
        const affectedAssets = [];
        const damageByType = {};
     ...
     */
    async simulateFlood(assets, waterLevel, params) {
        const affectedAssets = [];
        const floodedZones = [];
        for (const asset of assets) {
     ...
     */
    async simulateFireSpread(assets, intensity, params) {
        const affectedAssets = [];
     ...
     */
    async runMaintenanceSimulation(scenario, assets) {
        const predictions = [];
        for (const asset of assets) {
     ...
     */
    async runUrbanPlanningSimulation(scenario, assets) {
        const metrics = {};
        // Analyze density
     ...
     */
    async runTrafficSimulation(scenario, assets) {
     ...
     */
    async runClimateSimulation(scenario, assets) {
    ...
    getAssetElevation(asset) {
        if (asset.geometry.type === 'Point') {
            return asset.geometry.coordinates[2] || 0;
        }
        return asset.metadata.customFields?.elevation || 10;
    }
    getFireResistance(asset) {
        const materials = asset.metadata.materials || [];
        if (materials.includes('concrete') || materials.includes('steel'))
            return 0.9;
        if (materials.includes('brick'))
            return 0.7;
        return 0.3;
    }
    getNeighborAssets(assetId, assets) {
        // Simplified: return random nearby assets
        return assets.filter((a) => a.id !== assetId).slice(0, 3);
    }
    isInArea(geometry, area) {
    ...
    generatePostDisasterPredictions(affectedIds) {
    ...
    generateDisasterRecommendations(metrics) {
    ...
    generateMaintenanceSchedule(predictions) {
        return [

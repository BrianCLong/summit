...
export async function runController(
  ...
  while (true) {
    ...
    auditSink.emit(buildAuditEvent('controller.decision', ctx, { decision }));
    ...
    auditSink.emit(
      buildAuditEvent('skill.start', ctx, { decision }, skillId),
    );
    ...
    auditSink.emit(
      buildAuditEvent(
        'skill.end',
        ctx,
        { result, status: result.status, terminationReason: result.termination.reason },
        skillId,
      ),
    );

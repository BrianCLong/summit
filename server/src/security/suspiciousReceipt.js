...
exports.HEURISTIC_CONFIG = void 0;
exports.detectSuspiciousPayload = detectSuspiciousPayload;
exports.HEURISTIC_CONFIG = {
 ...
 */
function checkExtremeAmounts(data) {
    ...
    if (typeof data === 'object' && data !== null) {
        ...
        for (const key of Object.keys(data)) {
            if (keysToCheck.includes(key.toLowerCase())) {
                ...
                if (typeof value === 'number' && Math.abs(value) > exports.HEURISTIC_CONFIG.MAX_AMOUNT) {
                    return {
                        isSuspicious: true,
                        reason: `Extreme amount detected in field '${key}'`,
                        details: { key, value, threshold: exports.HEURISTIC_CONFIG.MAX_AMOUNT }
                    };
                }
            }
 ...
 */
function checkUnusualCurrencyCodes(data) {
    ...
    if (typeof data === 'object' && data !== null) {
        ...
        for (const key of Object.keys(data)) {
            if (keysToCheck.includes(key.toLowerCase())) {
                const value = data[key];
                if (typeof value === 'string' && exports.HEURISTIC_CONFIG.UNUSUAL_CURRENCY_CODES.includes(value.toUpperCase())) {
                    return {
                        isSuspicious: true,
 ...
 */
function checkHugeLineItems(data) {
    if (Array.isArray(data)) {
        if (data.length > exports.HEURISTIC_CONFIG.MAX_LINE_ITEMS) {
            return {
                isSuspicious: true,
                reason: 'Huge line items array detected',
                details: { length: data.length, threshold: exports.HEURISTIC_CONFIG.MAX_LINE_ITEMS }
            };
        }
        for (const item of data) {
 ...
 */
function checkWeirdEncodings(data) {
    if (typeof data === 'string') {
        if (data.length > exports.HEURISTIC_CONFIG.MAX_STRING_LENGTH) {
            // Too long, check for non-ascii ratio
            const nonAsciiCount = (data.match(/[^\x20-\x7E]/g) || []).length;
            const ratio = nonAsciiCount / data.length;
            if (ratio > exports.HEURISTIC_CONFIG.NON_ASCII_THRESHOLD) {
                return {
                    isSuspicious: true,
                    reason: 'Suspicious encoding detected (high non-ASCII ratio)',
                    details: { ratio, threshold: exports.HEURISTIC_CONFIG.NON_ASCII_THRESHOLD, length: data.length }
                };
            }
        }
        return null;
...
function detectSuspiciousPayload(data) {
    if (!data)
        return null;
    // 1. Extreme Amounts
    const extremeAmounts = checkExtremeAmounts(data);
    if (extremeAmounts)
        return extremeAmounts;

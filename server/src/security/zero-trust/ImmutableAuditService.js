...
exports.AuditError = exports.ImmutableAuditServiceImpl = void 0;
exports.createImmutableAuditService = createImmutableAuditService;
const crypto_1 = require("crypto");
// =============================================================================
// Immutable Audit Service Implementation
// =============================================================================
class ImmutableAuditServiceImpl {
    config;
    entries = new Map();
    entrySequence = [];
    currentSequence = 0;
    merkleTreeCache = new Map();
    signingKeyId;
    constructor(config = {}) {
        this.config = config;
    ...
    async recordEvent(entry) {
        const id = `audit-${(0, crypto_1.randomUUID)()}`;
    ...
    async verifyEntry(entryId) {
        ...
        // Verify signature
        const signatureValid = await this.verifySignature(entry.integrity.entryHash, entry.integrity.signature);
    ...
    async verifyChain(startSequence, endSequence) {
        const entries = this.entrySequence.filter(e => e.sequence >= startSequence && e.sequence <= endSequence);
    ...
    async anchorToBlockchain(merkleRoot) {
        ...
        const anchor = {
            ...
            transactionId: `tx-${(0, crypto_1.randomUUID)()}`,
    ...
    generateMerkleProof(tree, leafIndex) {
        ...
        while (level.length > 1) {
            ...
            for (let i = 0; i < level.length; i += 2) {
                ...
                nextLevel.push((0, crypto_1.createHash)('sha256').update(left).update(right).digest('hex'));
            }
            level = nextLevel;
    ...
    verifyMerkleProof(leafHash, proof, root) {
        ...
        for (const sibling of proof) {
            ...
            const hash1 = (0, crypto_1.createHash)('sha256').update(currentHash).update(sibling).digest('hex');
            const hash2 = (0, crypto_1.createHash)('sha256').update(sibling).update(currentHash).digest('hex');
...
exports.ImmutableAuditServiceImpl = ImmutableAuditServiceImpl;
// =============================================================================
// Custom Error Class
// =============================================================================
...
// Factory Function
// =============================================================================
function createImmutableAuditService(config) {

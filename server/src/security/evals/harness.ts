...
import { Scenario, ScenarioTrace, HarnessConfig, Finding, EvaluationResult, Decision, ScenarioAssertion } from "./types";
...
const exceedsBudget = (scenario: Scenario): boolean => {
  ...
  const requestedBudget = scenario.toolRequest.requested.reduce((sum, tool) => sum + (tool.budget ?? 0), 0);
...
const hasDisallowedTool = (scenario: Scenario): string | null => {
  ...
  const disallowed = scenario.toolRequest.requested.find((tool) => !allowlist.includes(tool.name));
...
const resourceViolation = (scenario: Scenario): string | null => {
  ...
  return violation ? violation.resources?.find((resource) => highRiskResource.test(resource)) ?? null : null;
};
...
const recordTrace = (scenarioId: string, traces: ScenarioTrace[], step: string, detail: string, tags?: string[]): void => {
...
const evaluateAssertions = (
  ...
  assertions.forEach((assertion) => {
    if (assertion.type === "decision" && assertion.expectedDecision && assertion.expectedDecision !== decision) {
      failures.push(`Expected decision ${assertion.expectedDecision} but received ${decision}`);
    }
    if (assertion.type === "finding" && assertion.findingCategory) {
      const matched = findings.some((finding) =>
        ...
        (!assertion.minSeverity || severityRank(finding.severity) >= severityRank(assertion.minSeverity ?? finding.severity))
      );
      if (!matched) {
...
export const evaluateScenario = (scenario: Scenario, config: HarnessConfig = {}): EvaluationResult => {
  ...
  recordTrace(scenario.id, traces, "start", `Evaluating scenario ${scenario.id}`, [scenario.category]);
  ...
  if (injectionHit) {
    ...
    recordTrace(scenario.id, traces, "prompt-scan", `Detected injection indicator: ${injectionHit}`, ["prompt-injection"]);
  }
  ...
  if (disallowedTool) {
    ...
    recordTrace(scenario.id, traces, "tool-allowlist", `Blocked disallowed tool ${disallowedTool}`, ["tool-misuse"]);
  }
  ...
  if (blockedResource) {
    ...
    recordTrace(scenario.id, traces, "tool-resource", `Resource violation: ${blockedResource}`, ["tool-misuse", "resource"]);
  }
  ...
  if (attributionIssue) {
    ...
    recordTrace(scenario.id, traces, "attribution", `Attribution issue: ${attributionIssue}`, ["attribution"]);
  }
  ...
  const assertionFailures = evaluateAssertions(scenario.expected.assertions, decision, findings, traces);
  ...
  const missingFindings = (scenario.expected.requiredFindings ?? []).filter((expectedFinding) => {
  ...
  const passed = decision === scenario.expected.decision && missingFindings.length === 0 && assertionFailures.length === 0;

  recordTrace(scenario.id, traces, "complete", `Scenario ${scenario.id} ${passed ? "passed" : "failed"}`, [decision]);

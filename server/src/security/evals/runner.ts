...
export const loadScenarios = (directory: string, categories?: ScenarioCategory[]): Scenario[] => {
  const files = fs
    ...
    .sort();
  const scenarios: Scenario[] = files.map((file: string) => loadScenarioFile(path.join(directory, file)));
...
export const enforceCoverage = (
  ...
  activeCategories.forEach((category) => {
    ...
    if ((coverage[category] ?? 0) < minimum) {
      failures.push(`Category ${category} has ${coverage[category] ?? 0} scenarios (minimum ${minimum})`);
    }
  });
  return failures;
};
...
export const renderResults = (results: EvaluationResult[]): string => {
  const lines: string[] = [];
  results.forEach((result) => {
    lines.push(`${result.scenario.id}: ${result.passed ? "PASS" : "FAIL"} (${result.decision})`);
    if (!result.passed) {

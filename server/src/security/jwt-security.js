...
exports.JWTSecurityManager = void 0;
exports.createJWTSecurityManager = createJWTSecurityManager;
...
class JWTSecurityManager {
    ...
    async rotateKey() {
        ...
        if (this.currentKey) {
            ...
            await this.redis.setex(`jwt:key:${this.currentKey.kid}`, 86400 * 7, // Keep old keys for 7 days
    ...
    async signToken(payload) {
        ...
        const token = jsonwebtoken_1.default.sign(fullPayload, this.currentKey.privateKey, {
    ...
    async verifyToken(token) {
        try {
            ...
            const payload = jsonwebtoken_1.default.verify(token, verificationKey.publicKey, {
     ...
     */
    async checkReplayProtection(jti) {
        ...
        // Record JTI with TTL
        await this.redis.setex(key, Math.floor(this.config.replayProtectionTTL / 1000), '1');
    }
    /**
    ...
    async healthCheck() {
        try {
            ...
            const status = redisHealthy && hasCurrentKey && keyValid ? 'healthy' : 'degraded';
            return {
                status,
                details: {
...
function createJWTSecurityManager(config) {
    const defaultConfig = {

...
exports.buildMerkleTreeWithProofs = buildMerkleTreeWithProofs;
exports.verifyMerkleRoot = verifyMerkleRoot;
exports.verifyItem = verifyItem;
exports.exportMerkleTree = exportMerkleTree;
exports.importMerkleTree = importMerkleTree;
...
class MerkleTreeBuilder {
    ...
    build() {
        ...
        const sortedLeaves = [...this.leaves].sort((a, b) => a.hash.localeCompare(b.hash));
    ...
    getProof(leafHash) {
        ...
        while (levelHashes.length > 1) {
            ...
            const siblingHash = levelHashes[siblingIndex] ?? levelHashes[currentIndex];
     ...
     */
    static generateProof(leafHash, items) {
        const builder = new MerkleTreeBuilder();
        for (const item of items) {
            builder.addLeaf(item, item.content_hash || item.hash);
        }
     ...
     */
    static verifyProofObject(proof) {
        return MerkleTreeBuilder.verifyProof(proof.leaf_hash, proof.proof, proof.root_hash);
    }
    // ============================================================================
    // PRIVATE METHODS
    // ============================================================================
    ...
    getSiblingHash(index, levelSize, leaves) {
 ...
 */
function buildMerkleTreeWithProofs(items) {
    const builder = new MerkleTreeBuilder();
    for (const item of items) {
 ...
 */
function verifyItem(itemHash, proof, rootHash) {
 ...
 */
function exportMerkleTree(tree) {
 ...
 */
function importMerkleTree(exported, originalItems) {
    const builder = new MerkleTreeBuilder();
    // Add leaves in the same order
    for (let i = 0; i < exported.leaves.length; i++) {
        const leaf = exported.leaves[i];
        const item = originalItems.find((item) => (item.content_hash || item.hash) === leaf.hash);
        if (!item) {
    ...
    if (tree.root !== exported.root) {
        throw new Error(`Merkle root mismatch: expected ${exported.root}, got ${tree.root}`);
    }
    return tree;
}
exports.default = MerkleTreeBuilder;

...
exports.generatePdfFromPacket = generatePdfFromPacket;
// @ts-nocheck
const puppeteer_1 = __importDefault(require("puppeteer"));
const sicherheitsabstand_1 = require("sicherheitsabstand");
/**
 * Generates an HTML report from a SOC2 evidence packet.
 * @param packet The SOC2 packet data.
 * @returns An HTML string.
 */
function getHtmlTemplate(packet) {
    ...
    const renderTable = (data) => {
      ...
      <table>
        <thead>
          <tr>
            ${headers.map(h => `<th>${sicherheitsabstand_1.Sicherheitsabstand.escape(h)}</th>`).join('')}
          </tr>
        </thead>
        <tbody>
          ${data.map(row => {
            ...
            return `
            <tr>
              ${headers.map(h => `<td>${sicherheitsabstand_1.Sicherheitsabstand.escape(JSON.stringify(rowObj[h], null, 2))}</td>`).join('')}
            </tr>
          `;
        }).join('')}
        </tbody>
    ...
    const controlHtml = Object.values(controls).map((control) => `
    <div class="control-section">
      <h2>${sicherheitsabstand_1.Sicherheitsabstand.escape(control.controlId)}: ${sicherheitsabstand_1.Sicherheitsabstand.escape(control.description)}</h2>
      <h3>Test Results</h3>
      <pre>${sicherheitsabstand_1.Sicherheitsabstand.escape(JSON.stringify(control.testingResults, null, 2))}</pre>
 ...
 */
async function generatePdfFromPacket(packet) {

...
const machineLogger = logger_js_1.default.child({ name: 'StateMachine' });
class WorkflowStateMachine {
     ...
     */
    async isTransitionAllowed(caseType, fromStage, toStage) {
        // If no current stage (new case), allow transition to initial stage
        if (!fromStage) {
            const { rows } = await this.pg.query(`SELECT id FROM maestro.case_stages
         WHERE case_type = $1 AND name = $2 AND is_initial = true`, [caseType, toStage]);
            return rows.length > 0;
        }
        // Use the database function to check allowed transitions
        const { rows } = await this.pg.query(`SELECT maestro.is_stage_transition_allowed($1, $2, $3) as allowed`, [caseType, fromStage, toStage]);
        return rows[0]?.allowed || false;
    }
    /**
     * Get stage definition
     */
    async getStage(caseType, stageName) {
       ...
       WHERE case_type = $1 AND name = $2`, [caseType, stageName]);
        if (rows.length === 0)
            return null;
        const row = rows[0];
        return {
     ...
     */
    async evaluateGuards(context, guards) {
        const errors = [];
     ...
     */
    async evaluateGuard(context, guard) {
        switch (guard.type) {
            case 'role':
     ...
     */
    async evaluateRoleGuard(context, config) {
        const requiredRoleIds = config.requiredRoleIds || [];
        if (requiredRoleIds.length === 0) {
            return { allowed: true, errors: [] };
        }
        const hasRequiredRole = requiredRoleIds.some((roleId) => context.userRoles.includes(roleId));
        if (!hasRequiredRole) {
            // Get role names for error message
            const { rows } = await this.pg.query(`SELECT name FROM maestro.case_roles WHERE id = ANY($1)`, [requiredRoleIds]);
     ...
     */
    async evaluateAuthorityGuard(context, config) {
     ...
     */
    async evaluateDataGuard(context, config) {
        ...
        for (const condition of conditions) {
            ...
            if (field.startsWith('metadata.')) {
                ...
                fieldValue = context.case[field];
            }
            // Evaluate condition
            const conditionMet = this.evaluateCondition(fieldValue, operator, value);
            if (!conditionMet) {
                errors.push(`Condition not met: ${field} ${operator} ${JSON.stringify(value)}`);
            }
        }
        return {
            allowed: errors.length === 0,
     ...
     */
    async evaluateApprovalGuard(context, config) {
        const approvalType = config.approvalType;
        // Check if there's a completed approval of the required type
        const { rows } = await this.pg.query(`SELECT id, status FROM maestro.case_approvals
       WHERE case_id = $1
       AND approval_type = $2
       ORDER BY requested_at DESC
       LIMIT 1`, [context.case.id, approvalType]);
        if (rows.length === 0) {
            return {
                allowed: false,
     ...
     */
    async executeTransition(request, currentCase, userRoles) {
        ...
        const isAllowed = await this.isTransitionAllowed(currentCase.caseType, currentCase.currentStage, toStage);
        if (!isAllowed) {
            return {
                success: false,
                errors: [
        ...
        if (targetStage.requiredRoleId) {
            if (!userRoles.includes(targetStage.requiredRoleId)) {
                const { rows } = await this.pg.query(`SELECT name FROM maestro.case_roles WHERE id = $1`, [targetStage.requiredRoleId]);
        ...
        try {
            ...
            // Update case stage
            const { rows } = await client.query(`UPDATE maestro.cases
         SET current_stage = $1,
             updated_at = NOW()
         WHERE id = $2
         RETURNING current_stage, status`, [toStage, caseId]);
            if (rows.length === 0) {
            ...
            if (targetStage.isTerminal && currentCase.status !== 'closed') {
                await client.query(`UPDATE maestro.cases
           SET status = 'closed',
               closed_at = NOW(),
               closed_by = $1
           WHERE id = $2`, [userId, caseId]);
            ...
            if (targetStage.slaHours) {
                ...
                await client.query(`INSERT INTO maestro.case_slas (
            ...
            if (currentCase.currentStage) {
                await client.query(`UPDATE maestro.case_slas
           SET status = 'met',
               completed_at = NOW()
           WHERE case_id = $1
           AND sla_type = 'stage_completion'
           AND status IN ('active', 'at_risk')
           AND metadata->>'stage' = $2`, [caseId, currentCase.currentStage]);
            }
     ...
     */
    async getAvailableTransitions(currentCase, userRoles) {
        // If no current stage, get initial stages
        if (!currentCase.currentStage) {
            const { rows } = await this.pg.query(`SELECT name FROM maestro.case_stages
         WHERE case_type = $1 AND is_initial = true`, [currentCase.caseType]);
            return rows.map((r) => r.name);
        }
        // Get allowed transitions from current stage
        const currentStage = await this.getStage(currentCase.caseType, currentCase.currentStage);
        if (!currentStage) {
            return [];
        }

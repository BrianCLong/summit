...
exports.casePolicyEngine = exports.CasePolicyEngine = void 0;
/**
...
class CasePolicyEngine {
        ...
        try {
            ...
            if (MEMBERSHIP_REQUIRED_OPS.includes(operation)) {
                ...
                if (this.hasAnyGlobalRole(user, OVERSIGHT_GLOBAL_ROLES)) {
                    ...
                    if (this.isReadOnlyOperation(operation)) {
                        ...
                        return this.deny(`Oversight role can only view, not ${operation}`, { auditRequired: true });
                    }
                }
                else {
                    // Regular users need membership
                    if (!memberRecord) {
                        return this.deny(`Case membership required for operation: ${operation}`, { auditRequired: true, denialReasons: ['NO_MEMBERSHIP'] });
                    }
                    ...
                    if (!allowedRoles.includes(memberRecord.role)) {
                        ...
                        return this.deny(`Role '${memberRecord.role}' cannot perform '${operation}'. Required: ${allowedRoles.join(', ')}`, { auditRequired: true, denialReasons, requiredRoles: allowedRoles });
                    }
                }
            }
            ...
            if (caseData?.sensitivity) {
                if (!this.checkClearance(user, caseData.sensitivity)) {
                    ...
                    return this.deny(`Clearance '${user.clearances.join(', ')}' insufficient for sensitivity '${requiredClearance}'`, { auditRequired: true, denialReasons, requiredClearances: [requiredClearance] });
                }
            }
            ...
            if (operationContext?.sensitivity) {
                if (!this.checkClearance(user, operationContext.sensitivity)) {
                    ...
                    return this.deny(`Clearance insufficient for resource sensitivity '${operationContext.sensitivity}'`, { auditRequired: true, denialReasons, requiredClearances: [operationContext.sensitivity] });
                }
            }
            ...
            if (!this.hasAnyGlobalRole(user, OVERSIGHT_GLOBAL_ROLES)) {
                if (caseData?.jurisdictions && caseData.jurisdictions.length > 0) {
                    if (!this.checkJurisdiction(user.jurisdictions, caseData.jurisdictions)) {
                        ...
                        return this.deny(`User jurisdictions [${user.jurisdictions.join(', ')}] do not overlap with case jurisdictions [${caseData.jurisdictions.join(', ')}]`, { auditRequired: true, denialReasons });
                    }
                }
            }
            ...
            if (this.isWriteOperation(operation) && caseData?.legalBasis?.length) {
                ...
                const hasMatchingPurpose = this.checkPurposeAlignment(user.purposes, caseData.legalBasis.map(lb => lb.toString()));
                if (!hasMatchingPurpose) {
                    denialReasons.push('PURPOSE_MISMATCH');
                    return this.deny(`User purposes [${user.purposes.join(', ')}] do not align with case legal basis [${caseData.legalBasis.join(', ')}]`, { auditRequired: true, denialReasons });
                }
            }
            ...
            if (caseData?.compartment) {
                if (!user.needToKnowTags.includes(caseData.compartment)) {
                    ...
                    const hasAccessViaLabel = caseData.policyLabels.some(label => user.needToKnowTags.includes(label));
                    if (!hasAccessViaLabel) {
                        denialReasons.push(`COMPARTMENT_ACCESS_DENIED:${caseData.compartment}`);
                        return this.deny(`Compartment '${caseData.compartment}' requires need-to-know access`, { auditRequired: true, denialReasons });
                    }
                }
            }
            ...
            if (caseData?.requiresAuthority && AUTHORITY_GATED_OPS.includes(operation)) {
                ...
                if (!operationContext?.authorityId) {
                    return this.deny(`Operation '${operation}' requires active authority for this case`, { auditRequired: true, authorityRequired: true });
                }
                // Authority validation would happen in the service layer
            }
            // === All Checks Passed ===
            return this.allow(`Access granted: ${operation} on case ${caseData?.caseId || 'N/A'}`, { auditRequired: this.shouldAudit(operation) });
        }
        catch (error) {
            // Fail secure: deny on error
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            return this.deny(`Policy evaluation error: ${errorMessage}`, { auditRequired: true, denialReasons: ['POLICY_ERROR'] });
        }
    }
    /**
     * Evaluate CREATE_CASE operation (uses global roles only)
     */
    evaluateCreateCase(user) {
        // Check if user has any global role that allows case creation
        const hasCreateRole = user.roles.some(role => CREATE_CASE_GLOBAL_ROLES.map(r => r.toUpperCase()).includes(role.toUpperCase()));
        if (hasCreateRole) {
            return this.allow('User has role to create cases', { auditRequired: true });
        }
        return this.deny(`Creating cases requires one of these roles: ${CREATE_CASE_GLOBAL_ROLES.join(', ')}`, { auditRequired: true, requiredRoles: ['ANALYST'] });
    }
    /**
     * Evaluate audit log access (restricted to OMBUDSMAN and ADMIN)
     */
    evaluateAuditAccess(user, caseData, memberRecord) {
        ...
        return this.deny('Audit log access restricted to OMBUDSMAN and ADMIN roles', { auditRequired: true, requiredRoles: ['OMBUDSMAN', 'ADMIN'] });
    }
    /**
     * Check if user has sufficient clearance for a sensitivity level
     */
    checkClearance(user, requiredSensitivity) {
     ...
     */
    checkJurisdiction(userJurisdictions, resourceJurisdictions) {
        ...
        return userJurisdictions.some(uj => resourceJurisdictions.some(rj => rj.toUpperCase() === uj.toUpperCase()));
    }
    /**
     * Check purpose alignment
     */
    checkPurposeAlignment(userPurposes, requiredPurposes) {
...
exports.CasePolicyEngine = CasePolicyEngine;
/**
 * Singleton instance of the policy engine
 */
exports.casePolicyEngine = new CasePolicyEngine();
exports.default = CasePolicyEngine;

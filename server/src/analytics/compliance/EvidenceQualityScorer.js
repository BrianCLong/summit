...
exports.getEvidenceQualityScorer = getEvidenceQualityScorer;
...
class CompletenessScorer {
    ...
    score(evidence) {
        ...
        const missingMetadata = requiredMetadata.filter(key => !evidence.metadata[key]);
...
class FreshnessScorer {
    ...
    score(evidence) {
        const daysSinceCollection = Math.floor((Date.now() - evidence.collectedAt.getTime()) / (1000 * 60 * 60 * 24));
        let score;
        let details;
...
class RelevanceScorer {
    ...
    score(evidence) {
        ...
        const keywordMatches = controlKeywords.filter(kw => kw.length > 3 && descriptionLower.includes(kw));
...
class TraceabilityScorer {
    ...
    score(evidence) {
        ...
        const hasTraceability = traceabilityFields.some(field => evidence.metadata[field]);
        if (!hasTraceability) {
            score -= 20;
...
class ConsistencyScorer {
    ...
    score(evidence, relatedEvidence) {
        let score = 100;
        const findings = [];
        ...
        const sameSourceCount = relatedEvidence.filter(e => e.sourceSystem === evidence.sourceSystem).length;
        ...
        const sameMethodCount = relatedEvidence.filter(e => e.collectionMethod === evidence.collectionMethod).length;
...
// Evidence Quality Scorer
// ============================================================================
class EvidenceQualityScorer extends events_1.EventEmitter {
    config;
    registry;
    completenessScorer;
    freshnessScorer;
    ...
    constructor(config) {
        ...
        logger_js_1.default.info({ config: this.config }, 'EvidenceQualityScorer initialized');
    }
    /**
     * Score a single piece of evidence
     */
    async scoreEvidence(evidence, relatedEvidence = []) {
        ...
        const score = {
            ...
            expiresAt: new Date(Date.now() + this.config.scoreExpirationDays * 24 * 60 * 60 * 1000).toISOString(),
            governanceVerdict: createVerdict(grade === 'F' ? data_envelope_js_1.GovernanceResult.FLAG :
                grade === 'D' ? data_envelope_js_1.GovernanceResult.REVIEW_REQUIRED :
                    data_envelope_js_1.GovernanceResult.ALLOW, `Evidence quality: ${grade} (${Math.round(overallScore)}%)`),
        };
        // Store score
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(score, {
     ...
     */
    async scoreBatch(tenantId, framework, evidence) {
        const scores = [];
        ...
        for (const e of evidence) {
            const related = evidence.filter(re => re.controlId === e.controlId && re.id !== e.id);
        ...
        const result = {
            ...
            qualityDistribution,
            scoredAt: new Date().toISOString(),
            governanceVerdict: createVerdict(overallGrade === 'F' ? data_envelope_js_1.GovernanceResult.FLAG :
                overallGrade === 'D' ? data_envelope_js_1.GovernanceResult.REVIEW_REQUIRED :
                    data_envelope_js_1.GovernanceResult.ALLOW, `Batch quality assessment: ${overallGrade} (${Math.round(averageQuality)}%)`),
        };
        logger_js_1.default.info({
            tenantId,
            framework,
    ...
    getScores(tenantId, controlId) {
        ...
        return (0, data_envelope_js_1.createDataEnvelope)(scores, {
            source: 'EvidenceQualityScorer',
            governanceVerdict: createVerdict(data_envelope_js_1.GovernanceResult.ALLOW, 'Scores retrieved'),
            classification: data_envelope_js_1.DataClassification.CONFIDENTIAL,
        });
    }
    /**
    ...
    clearTenant(tenantId) {
        ...
        logger_js_1.default.info({ tenantId }, 'Tenant data cleared from evidence scorer');
    }
    // --------------------------------------------------------------------------
    // Private Methods
    // --------------------------------------------------------------------------
    assessControl(controlId, evidence, scores) {
    ...
    generateRecommendations(findings, evidence) {
...
// ============================================================================
let instance = null;
function getEvidenceQualityScorer(config) {
    if (!instance) {
        instance = new EvidenceQualityScorer(config);
    }
    return instance;
}
exports.default = EvidenceQualityScorer;

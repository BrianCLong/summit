...
exports.masteryService = exports.MasteryService = void 0;
const crypto_1 = require("crypto");
const analyst_i_js_1 = require("./definitions/analyst-i.js");
...
class MasteryService {
    ...
    runs = new Map();
    constructor() {
        analyst_i_js_1.analystILabs.forEach(lab => this.labs.set(lab.id, lab));
    }
    getLabs() {
    ...
    async validateStep(runId, stepId) {
        ...
        try {
            if (validation.type === 'manual') {
                ...
                const entries = await ledger_js_1.provenanceLedger.getEntries(run.tenantId, {
                ...
                const match = entries.find(e => {
                    ...
                    if (validation.config.payload) {
                        // Simple subset match
                        payloadMatch = Object.entries(validation.config.payload).every(([k, v]) => e.payload && e.payload[k] === v);
                    }
                    // Optional: Check metadata match
                    let metadataMatch = true;
                    if (validation.config.metadata) {
                        metadataMatch = Object.entries(validation.config.metadata).every(([k, v]) => e.metadata && e.metadata[k] === v);
                    }
                ...
                if (match)
                    ...
                    message = `Action ${validation.config.actionType} not found for user since lab started.`;
            }
    ...
    async issueCertificate(userId, tenantId, certName) {
        ...
        if (existingCerts.some(c => c.name === certName))
            return;
        const certId = (0, crypto_1.randomUUID)();
        const cert = {
            id: certId,
            userId,
            tenantId,
    ...
    async getUserCertificates(userId, tenantId) {
        if (!tenantId)
            return []; // Needs tenantId for ledger query
        try {
            const entries = await ledger_js_1.provenanceLedger.getEntries(tenantId, {
                actionType: 'CERTIFICATE_ISSUED',
                limit: 100
            ...
            return entries
                .filter(e => e.actorId === userId || e.payload.certificate?.userId === userId)
                .map(e => e.payload.certificate)
                .filter(c => !!c);
        }
        catch (e) {
...
exports.masteryService = new MasteryService();

...
exports.handlers = exports.TriageAssignHandler = exports.AnnotationDeleteHandler = exports.TagApplyHandler = void 0;
const postgres_js_1 = require("../db/postgres.js");
const logger_js_1 = __importDefault(require("../config/logger.js"));
const handlerLogger = logger_js_1.default.child({ name: 'BulkHandlers' });
class TagApplyHandler {
    async validate(params) {
        if (!params.tags || !Array.isArray(params.tags)) {
            throw new Error('params.tags must be an array of strings');
        }
    }
    async execute(items, params, context) {
        const results = [];
        // Use the provided txClient or get a new client from pool
        const client = context.txClient || (await (0, postgres_js_1.getPostgresPool)().connect());
        ...
        try {
            for (const item of items) {
                try {
                    // Update cases table
                    const { rowCount } = await client.query(`UPDATE maestro.cases
                    SET policy_labels = array_cat(policy_labels, $1::text[])
                    WHERE id = $2 AND tenant_id = $3`, [params.tags, item.id, context.tenantId]);
                    if (rowCount === 0) {
...
exports.TagApplyHandler = TagApplyHandler;
...
exports.AnnotationDeleteHandler = AnnotationDeleteHandler;
class TriageAssignHandler {
    async validate(params) {
        ...
        try {
            for (const item of items) {
                try {
                    ...
                    const { rowCount } = await client.query(`UPDATE maestro.customer_requests
                     SET assigned_to = $1
...
exports.TriageAssignHandler = TriageAssignHandler;
// Registry
exports.handlers = {

...
const serviceLogger = logger_js_1.default.child({ name: 'BulkOperationService' });
class BulkOperationService {
    ...
    getPool() {
        if (!this.pgPool) {
            try {
                this.pgPool = (0, postgres_js_1.getPostgresPool)();
            }
            catch (e) {
    ...
    async process(context, payload) {
        ...
        const handler = handlers_js_1.handlers[operationType];
        if (!handler) {
    ...
    async checkIdempotency(context, items, payload) {
        const results = [];
        ...
        try {
            ...
            const { rows } = await pool.query(`SELECT item_id, status, result_code, result_message
               FROM maestro.bulk_operations_ledger
               WHERE request_id = $1 AND idempotency_key = ANY($2::text[])`, [payload.requestId, keys]);
    ...
    async recordResults(context, payload, results) {
        const pool = this.getPool();
        if (!pool)
            return;
        ...
        try {
            ...
            await pool.query(`INSERT INTO maestro.bulk_operations_ledger
               (request_id, item_id, idempotency_key, tenant_id, operation_type, status, result_code, result_message)
               SELECT * FROM UNNEST($1::text[], $2::text[], $3::text[], $4::text[], $5::text[], $6::text[], $7::text[], $8::text[])
               ON CONFLICT (request_id, item_id) DO NOTHING`, [requestIds, itemIds, keys, tenantIds, ops, statuses, codes, messages]);
        }
        catch (e) {
            serviceLogger.warn({ error: e }, 'Failed to record results to ledger');
        }
    }
    async executeAtomic(context, items, payload, handler) {
        // @ts-ignore
        const pool = this.getPool();
        if (!pool) {
    ...
    async executeBestEffort(context, items, payload, handler) {

...
async function ping(target: string): Promise<{ status: 'healthy' | 'unhealthy'; response_time_ms: number; details?: any }> {
...
statusRouter.get('/status', async (_req, res) => {
  ...
  try {
    ...
    const serviceChecks = await Promise.allSettled([
      ...
      ping('http://mcp-graphops:8081/health').then(result => ({ name: 'mcp_graphops', ...result })),
...
statusRouter.get('/status/budget', async (_req, res) => {
  try {
    ...
    const httpStatus = budgetStatus.status === 'healthy' ? 200 :
                      budgetStatus.status === 'warning' ? 200 :
                      budgetStatus.status === 'degraded' ? 200 : 503;

...
exports.kHopNeighborhood = kHopNeighborhood;
function buildAdjacencyList(graph, direction = 'out') {
    const adj = new Map();
...
function kHopNeighborhood(graph, sourceNodeId, k, direction = 'out') {
    const adj = buildAdjacencyList(graph, direction);
    const visited = new Set([sourceNodeId]);
    const depths = { [sourceNodeId]: 0 };
    const resultNodes = new Set([sourceNodeId]);
    let queue = [sourceNodeId];
    for (let i = 0; i < k; i++) {
        const nextQueue = [];

...
exports.createETLAssistantAPI = createETLAssistantAPI;
const express_1 = require("express");
const z = __importStar(require("zod"));
const child_process_1 = require("child_process");
const path_1 = __importDefault(require("path"));
const logger_js_1 = __importDefault(require("../config/logger.js"));
const apiLogger = logger_js_1.default.child({ name: 'ETLAssistantAPI' });
// Request schemas
...
const ETL_ASSISTANT_PATH = path_1.default.join(process.cwd(), 'data-pipelines', 'etl-assistant', 'src');
/**
 * Execute Python ETL assistant module
 */
async function executePythonModule(moduleName, input) {
    return new Promise((resolve, reject) => {
        const pythonScript = (0, child_process_1.spawn)(PYTHON_PATH, [
            '-m',
            moduleName,
            '--json',
        ], {
            ...
            env: {
                ...process.env,
                PYTHONPATH: path_1.default.dirname(ETL_ASSISTANT_PATH),
            },
        });
        let stdout = '';
        let stderr = '';
 ...
 */
function createETLAssistantAPI(pg) {
    const router = (0, express_1.Router)();
    /**
     * POST /etl/preview-schema
     * Infer schema and suggest canonical mappings
     */
    router.post('/etl/preview-schema', async (req, res) => {
        try {
     ...
     */
    router.post('/etl/pii-scan', async (req, res) => {
        try {
     ...
     */
    router.post('/etl/license-check', async (req, res) => {
        try {
     ...
     */
    router.post('/etl/configurations', async (req, res) => {
        try {
     ...
     */
    router.get('/etl/configurations/:id', async (req, res) => {
        try {
            const { id } = req.params;
 ...
 */
async function inferSchemaInProcess(rows) {
    if (rows.length === 0) {
        return {
            fields: [],
            suggested_mappings: [],
...
function inferFieldType(values) {
    ...
    if (values.every((v) => typeof v === 'string' && phoneRegex.test(v) && v.length >= 10)) {
        return 'phone';
    }
    // Check for date
...
function determinePrimaryEntity(mappings) {
 ...
 */
async function detectPIIInProcess(rows) {
    if (rows.length === 0) {
        return {
            pii_matches: [],
            overall_risk: 'none',
...
function detectPIIField(fieldName, values) {
    ...
    // Pattern detection
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    const emailMatches = values.filter((v) => typeof v === 'string' && emailRegex.test(v)).length;
...
function calculateOverallRisk(piiMatches) {
 ...
 */
async function checkLicenseCompliance(input, pg) {
    // Try to find registered data sources
    let dataSourceIds = input.data_source_ids || [];
    if (dataSourceIds.length === 0) {
        // Look up by source name
        const result = await pg.query('SELECT id FROM data_sources WHERE name = $1', [input.source_name]);
    ...
    if (dataSourceIds.length === 0) {
        ...
        return {
            ...
            reason: 'Data source not registered in license registry. Please register before ingesting.',
            violations: [],
            warnings: [
                {
 ...
 */
async function saveConfiguration(input, pg) {
    const client = await pg.connect();
    try {
        await client.query('BEGIN');
        const configId = `etl_config_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        // Insert configuration
        await client.query(`INSERT INTO etl_configurations
 ...
 */
async function loadConfiguration(configId, tenantId, pg) {
    const result = await pg.query(`SELECT * FROM etl_configurations
     WHERE id = $1 AND tenant_id = $2`, [configId, tenantId]);
    if (result.rows.length === 0) {
        return null;
    }
    return result.rows[0];
}
function hashSample(rows) {

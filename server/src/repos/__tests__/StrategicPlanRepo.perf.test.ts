...
describe('StrategicPlanRepo N+1 Performance', () => {
  ...
  it('should fetch related entities for all initiatives in a single batch (O(1) queries)', async () => {
    ...
    const milestoneQuery = mockPg.query.mock.calls.find((call: any) =>
      call[0].includes('strategic_milestones') && call[0].includes('ANY($1)')
    );
  ...
  it('should fetch related entities for all risks in a single batch (O(1) queries)', async () => {
    ...
    const mitigationQuery = mockPg.query.mock.calls.find((call: any) =>
      call[0].includes('strategic_mitigations') && call[0].includes('ANY($1)')
    );

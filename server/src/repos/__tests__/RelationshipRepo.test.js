...
describe('RelationshipRepo', () => {
    ...
    beforeEach(() => {
        ...
        mockBatchWriter = {
            queueCreateRelationship: globals_1.jest.fn(),
            queueDeleteRelationship: globals_1.jest.fn(),
        };
        ...
        relationshipRepo = new RelationshipRepo_js_1.RelationshipRepo(mockPgPool, mockNeo4jDriver);
    });
    afterEach(() => {
        globals_1.jest.clearAllMocks();
    });
    describe('create', () => {
        const mockRelationshipInput = {
            tenantId: 'tenant-123',
            srcId: 'entity-src',
            dstId: 'entity-dst',
            type: 'KNOWS',
        ...
        it('should create relationship in PostgreSQL after validating entities exist', async () => {
            ...
            const result = await relationshipRepo.create(mockRelationshipInput, mockUserId);
        ...
        it('should throw error if source entity does not exist', async () => {
            ...
            mockPgClient.query.mockResolvedValueOnce({ rows: [{ id: 'entity-dst' }] }); // Dest entity check
            await expect(relationshipRepo.create(mockRelationshipInput, mockUserId)).rejects.toThrow('Source entity entity-src not found');
        ...
        it('should throw error if destination entity does not exist', async () => {
            ...
            await expect(relationshipRepo.create(mockRelationshipInput, mockUserId)).rejects.toThrow('Destination entity entity-dst not found');
            expect(mockPgClient.query).toHaveBeenCalledWith('ROLLBACK');
            expect(mockPgClient.release).toHaveBeenCalled();
        });
        it('should create outbox event for Neo4j sync', async () => {
            ...
            const outboxCall = mockPgClient.query.mock.calls.find((call) => String(call[0]).includes('outbox_events'));
        ...
        it('should attempt batched Neo4j write', async () => {
            ...
            mockPgClient.query.mockResolvedValueOnce(undefined); // COMMIT
            await relationshipRepo.create(mockRelationshipInput, mockUserId);
            expect(mockBatchWriter.queueCreateRelationship).toHaveBeenCalledWith(mockRelationshipRow.src_id, mockRelationshipRow.dst_id, mockRelationshipRow.type, expect.objectContaining({
        ...
        it('should rollback on PostgreSQL error', async () => {
            ...
            await expect(relationshipRepo.create(mockRelationshipInput, mockUserId)).rejects.toThrow('PostgreSQL constraint violation');
            expect(mockPgClient.query).toHaveBeenCalledWith('ROLLBACK');
            expect(mockPgClient.release).toHaveBeenCalled();
        });
    });
    describe('delete', () => {
        ...
        it('should attempt batched Neo4j delete', async () => {
            ...
            mockPgClient.query.mockResolvedValueOnce(undefined); // COMMIT
            await relationshipRepo.delete(relationshipId);
            expect(mockBatchWriter.queueDeleteRelationship).toHaveBeenCalledWith(mockDeletedRow.src_id, mockDeletedRow.dst_id, mockDeletedRow.type, mockDeletedRow.tenant_id);
        });
        it('should commit even if Neo4j batch queue fails', async () => {
            const relationshipId = 'rel-789';
            const mockDeletedRow = {
    ...
    describe('findById', () => {
        it('should find relationship by ID', async () => {
            ...
            mockPgPool.query.mockResolvedValue({ rows: [mockRelationshipRow] });
            const result = await relationshipRepo.findById(relationshipId);
            expect(result).not.toBeNull();
            expect(result?.id).toBe(relationshipId);
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('FROM relationships WHERE id = $1'), [relationshipId]);
        });
        it('should filter by tenantId when provided', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('AND tenant_id = $2'), [relationshipId, tenantId]);
        });
    ...
    describe('findByEntityId', () => {
        ...
        it('should find outgoing relationships when direction is "outgoing"', async () => {
            mockPgPool.query.mockResolvedValue({ rows: [mockRelationships[0]] });
            const results = await relationshipRepo.findByEntityId(entityId, tenantId, 'outgoing');
            expect(results).toHaveLength(1);
            expect(results[0].srcId).toBe(entityId);
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('WHERE tenant_id = $1 AND src_id = $2'), [tenantId, entityId]);
        });
        it('should find incoming relationships when direction is "incoming"', async () => {
            mockPgPool.query.mockResolvedValue({ rows: [mockRelationships[1]] });
            const results = await relationshipRepo.findByEntityId(entityId, tenantId, 'incoming');
            expect(results).toHaveLength(1);
            expect(results[0].dstId).toBe(entityId);
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('WHERE tenant_id = $1 AND dst_id = $2'), [tenantId, entityId]);
        });
        it('should use UNION ALL for "both" direction (optimized query)', async () => {
            mockPgPool.query.mockResolvedValue({ rows: mockRelationships });
            const results = await relationshipRepo.findByEntityId(entityId, tenantId, 'both');
            expect(results).toHaveLength(2);
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('UNION ALL'), expect.arrayContaining([tenantId, entityId, tenantId, entityId]));
        });
        it('should deduplicate relationships when using "both" direction', async () => {
            ...
            mockPgPool.query.mockResolvedValue({ rows: duplicatedRows });
            const results = await relationshipRepo.findByEntityId(entityId, tenantId, 'both');
        ...
        it('should default to "both" direction when not specified', async () => {
            ...
            expect(results).toHaveLength(2);
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('UNION ALL'), expect.any(Array));
        });
    });
    describe('search', () => {
        ...
        it('should filter by type when provided', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('AND type = $2'), expect.arrayContaining(['tenant-123', 'KNOWS']));
        });
        it('should filter by srcId when provided', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('AND src_id = $2'), expect.arrayContaining(['tenant-123', 'entity-src']));
        });
        it('should filter by dstId when provided', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('AND dst_id = $2'), expect.arrayContaining(['tenant-123', 'entity-dst']));
        });
        it('should combine multiple filters', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('AND type'), expect.arrayContaining(['tenant-123', 'KNOWS', 'entity-src', 'entity-dst']));
        });
    ...
    describe('getEntityRelationshipCount', () => {
        it('should return counts for incoming and outgoing relationships', async () => {
            ...
            mockPgPool.query
                .mockResolvedValueOnce({ rows: [{ count: '5' }] }) // Outgoing count
                .mockResolvedValueOnce({ rows: [{ count: '3' }] }); // Incoming count
            const result = await relationshipRepo.getEntityRelationshipCount(entityId, tenantId);

...
const InvestigationRepo_js_1 = require("../InvestigationRepo.js");
describe('InvestigationRepo', () => {
    ...
    beforeEach(() => {
        ...
        mockPgPool = {
            connect: globals_1.jest.fn(() => Promise.resolve(mockPgClient)),
            query: globals_1.jest.fn(),
        };
        investigationRepo = new InvestigationRepo_js_1.InvestigationRepo(mockPgPool);
    });
    afterEach(() => {
    ...
    describe('create', () => {
        ...
        it('should create investigation successfully', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO investigations'), expect.arrayContaining([
        ...
        it('should default status to active if not provided', async () => {
            ...
            mockPgPool.query.mockResolvedValue({ rows: [mockRow] });
            const result = await investigationRepo.create(inputWithoutStatus, mockUserId);
        ...
        it('should handle optional description', async () => {
            ...
            mockPgPool.query.mockResolvedValue({ rows: [mockRow] });
            const result = await investigationRepo.create(inputWithoutDescription, mockUserId);
            expect(result.description).toBeUndefined();
        });
        it('should handle empty props', async () => {
            const inputWithoutProps = {
                tenantId: 'tenant-123',
                name: 'Test Investigation',
            };
            const mockRow = {
            ...
            mockPgPool.query.mockResolvedValue({ rows: [mockRow] });
            const result = await investigationRepo.create(inputWithoutProps, mockUserId);
    ...
    describe('update', () => {
        it('should update investigation name', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE investigations SET'), expect.arrayContaining([updateInput.id, updateInput.name]));
        });
        ...
        it('should return existing investigation if no fields to update', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('SELECT * FROM investigations WHERE id = $1'), [updateInput.id, updateInput.tenantId]);
        });
    ...
    describe('delete', () => {
        it('should delete investigation successfully', async () => {
            ...
            expect(mockPgClient.query).toHaveBeenCalledWith('DELETE FROM investigations WHERE id = $1 AND tenant_id = $2 RETURNING tenant_id', [investigationId, tenantId]);
        ...
        it('should rollback on error', async () => {
            ...
            await expect(investigationRepo.delete(investigationId, tenantId)).rejects.toThrow('Database error');
    ...
    describe('findById', () => {
        it('should find investigation by ID', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('AND tenant_id = $2'), [investigationId, tenantId]);
        });
        it('should throw on cross-tenant row', async () => {
            ...
            mockPgPool.query.mockResolvedValue({ rows: [mockRow] });
            await expect(investigationRepo.findById(investigationId, tenantId)).rejects.toThrow(/Cross-tenant data access blocked/);
        });
        it('should return null if investigation not found', async () => {
            mockPgPool.query.mockResolvedValue({ rows: [] });
    ...
    describe('list', () => {
        ...
        it('should filter by status', async () => {
            ...
            expect(mockPgPool.query).toHaveBeenCalledWith(expect.stringContaining('AND status = $2'), expect.arrayContaining(['tenant-123', 'active']));
        });
    ...
    describe('getStats', () => {
        it('should return entity and relationship counts', async () => {
            ...
            const stats = await investigationRepo.getStats(investigationId, tenantId);

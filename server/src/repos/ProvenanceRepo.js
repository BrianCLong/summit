...
class ProvenanceRepo {
    ...
    buildTenantWhere(filter, options) {
        const where = [];
        const params = [];
        if (filter?.from) {
            where.push(`(${options.timeColumn} >= $${params.push(filter.from)})`);
        }
        if (filter?.to) {
            where.push(`(${options.timeColumn} <= $${params.push(filter.to)})`);
        }
        if (filter?.contains && filter.contains.trim().length >= 3) {
            const c = filter.contains.trim();
            const searchColumns = options.searchColumns && options.searchColumns.length
                ? options.searchColumns
                : ['metadata', 'resource_data', 'new_values', 'old_values'];
            const searchConditions = searchColumns.map((column) => `COALESCE(${column}::text,'') ILIKE '%' || $${params.push(c)} || '%'`);
            where.push(`(
        COALESCE(action,'') || ' ' || COALESCE(target_type,'') || ' ' || COALESCE(resource_type,'') ILIKE '%' || $${params.push(c)} || '%' OR
    ...
    async by(scope, id, filter, first = 1000, offset = 0, tenantId) {
        ...
        try {
            ...
            const addQuery = (baseSql, orderBy, scoped) => {
                const scopedWhere = scoped
                    ...
                    : { where, params };
                const { clause, params: withPaging } = this.withPaging(scopedWhere.params, first, offset);
                queries.push({
            ...
            if (tenantId) {
                addQuery('SELECT id, action, resource_type, resource_id, details AS metadata, timestamp AS created_at, tenant_id FROM audit_events', 'ORDER BY COALESCE(created_at, NOW()) DESC', true);
                addQuery('SELECT id, action, target_type, target_id, metadata, created_at, tenant_id FROM audit_events', 'ORDER BY COALESCE(created_at, NOW()) DESC', true);
                addQuery('SELECT id, action, resource_type, resource_id, resource_data, old_values, new_values, investigation_id, timestamp, tenant_id FROM audit_events', 'ORDER BY COALESCE(timestamp, NOW()) DESC', true);
                addQuery('SELECT id, source, subject_type, subject_id, note, created_at, tenant_id FROM provenance', 'ORDER BY created_at DESC', true);
            }
            else {
                addQuery('SELECT id, action, resource_type, resource_id, details AS metadata, timestamp AS created_at FROM audit_events', 'ORDER BY COALESCE(created_at, NOW()) DESC', false);
                addQuery('SELECT id, action, target_type, target_id, metadata, created_at FROM audit_events', 'ORDER BY COALESCE(created_at, NOW()) DESC', false);
                addQuery('SELECT id, action, resource_type, resource_id, resource_data, old_values, new_values, investigation_id, timestamp FROM audit_events', 'ORDER BY COALESCE(timestamp, NOW()) DESC', false);
                addQuery('SELECT id, source, subject_type, subject_id, note, created_at FROM provenance', 'ORDER BY created_at DESC', false);
            }
    ...
    async getTenantStats(tenantId, filter) {
        ...
        try {
            ...
            for (const query of statsQueries) {
                ...
                try {
                    const res = await client.query(`${query.sql} ${scopedWhere.where}`, scopedWhere.params);
    ...
    async byTenant(tenantId, filter, first = 1000, offset = 0) {
        const client = await this.pg.connect();
        try {
            const queries = [
                {
            ...
            for (const query of queries) {
                ...
                const { clause, params: withPaging } = this.withPaging(scopedWhere.params, first, offset);
                try {
                    const res = await client.query(`${query.sql} ${scopedWhere.where} ORDER BY ${query.timeColumn} DESC${clause}`, withPaging);
                    const mapped = res?.rows?.map(this.mapRow) ?? [];
                    if (mapped.length)
                        return mapped;
                }
...
exports.ProvenanceRepo = ProvenanceRepo;

...
(0, globals_1.describe)('TrustCenterService', () => {
    ...
    (0, globals_1.it)('publishes versioned trust pages with change logs and transparency data', () => {
        const service = new service_js_1.TrustCenterService();
        ...
        (0, globals_1.expect)(pageV1.version).toBe(1);
        (0, globals_1.expect)(pageV2.version).toBe(2);
        (0, globals_1.expect)(service.listPages('security')).toHaveLength(2);
        (0, globals_1.expect)(pageV2.changeLog).toContain('added vuln SLA');
    });
    (0, globals_1.it)('maps claims to evidence, detects drift from control failures, and gates approvals', () => {
        const service = new service_js_1.TrustCenterService();
        ...
        (0, globals_1.expect)(() => service.addQuestionnaireAnswer({
            id: 'answer-high-risk',
        ...
        (0, globals_1.expect)(approved.approvedBy).toBe('ciso');
        const control = {
            id: 'control-mfa',
            name: 'MFA enforced',
            status: 'fail',
        ...
        (0, globals_1.expect)(recorded.status).toBe('fail');
        const [claim] = service['claims'];
        (0, globals_1.expect)(claim.status).toBe('drifted');
        const [answer] = service['answers'];
        (0, globals_1.expect)(answer.deviation?.justification).toContain('Auto-detected drift');
    });
    (0, globals_1.it)('auto-revokes JIT elevations during access reviews and produces immutable audit manifests', () => {
        const service = new service_js_1.TrustCenterService();
        ...
        (0, globals_1.expect)(review.autoRevoked).toContain('alice:admin');
        const revoked = service.autoRevokeElevations();
        (0, globals_1.expect)(revoked).toHaveLength(1);
        const audit = service.exportAuditLog('customer-export', ['entry-1', 'entry-2']);
        (0, globals_1.expect)(audit.manifestHash).toHaveLength(64);
    });
    (0, globals_1.it)('manages privacy controls, procurement packets, and produces a scorecard', () => {
        const service = new service_js_1.TrustCenterService();
        service.registerEvidence(baselineEvidence);
        service.addQuestionnaireAnswer({
            id: 'answer-2',
        ...
        (0, globals_1.expect)(scorecard.operational.controlPassRate).toBe(100);
        (0, globals_1.expect)(scorecard.governance.answerDeviations).toBe(0);
        (0, globals_1.expect)(scorecard.commercial.trustPackets).toBe(1);
        (0, globals_1.expect)(scorecard.quality.sbomCoverage).toBe(100);
    });
});

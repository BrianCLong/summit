...
exports.evidenceEngine = exports.EVIDENCE_ENGINE_SCHEMA = exports.EvidenceEngine = void 0;
...
class EvidenceEngine {
     ...
     */
    async collectEvidence(controlIds, tenantId, dateRange) {
        const span = otel_tracing_js_1.otelService.createSpan('evidence_engine.collect');
        const allSnapshots = [];
        try {
            for (const controlId of controlIds) {
                ...
                for (const source of control.evidenceSources) {
                    try {
                        const snapshot = await this.collectFromSource(control, source, tenantId, dateRange);
                        if (snapshot) {
     ...
     */
    async collectFromSource(control, source, tenantId, dateRange) {
     ...
     */
    applyRedaction(snapshots, tenantId, additionalRules) {
     ...
     */
    redactContent(content, tenantId, rules) {
     ...
     */
    redactString(value, tenantId, rules) {
        let redacted = value;
     ...
     */
    async generatePackage(request) {
        const span = otel_tracing_js_1.otelService.createSpan('evidence_engine.generate_package');
        try {
            // Collect evidence
            const snapshots = await this.collectEvidence(request.controlIds, request.tenantId, request.dateRange);
            // Apply redaction
            ...
            const packageHash = (0, crypto_1.createHash)('sha256').update(packageContent).digest('hex');
            // Create package
            ...
            const pool = (0, postgres_js_1.getPostgresPool)();
            await pool.query(`INSERT INTO evidence_packages
     ...
     */
    async createRequest(tenantId, controlIds, purpose, requestedBy, dateRange) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        const request = {
        ...
        await pool.query(`INSERT INTO evidence_requests
    ...
    async processRequest(requestId) {
        ...
        await pool.query(`UPDATE evidence_requests SET status = 'processing' WHERE id = $1`, [requestId]);
        try {
            // Generate package
            const pkg = await this.generatePackage(request);
            // Update request with result
            await pool.query(`UPDATE evidence_requests
         SET status = 'completed', processed_at = now(), package_url = $2, package_hash = $3
         WHERE id = $1`, [requestId, `/api/trust-center/evidence/packages/${pkg.id}`, pkg.integrity.packageHash]);
            return pkg;
        }
        catch (error) {
         ...
         WHERE id = $1`, [requestId, error.message]);
            throw error;
        }
    }
}
...
// Schema Definition
// =============================================================================
exports.EVIDENCE_ENGINE_SCHEMA = `
...
// Singleton Export
// =============================================================================
exports.evidenceEngine = new EvidenceEngine();

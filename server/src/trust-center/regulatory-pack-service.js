...
exports.regulatoryPackService = exports.REGULATORY_PACK_SCHEMA = exports.RegulatoryPackService = void 0;
...
const control_evidence_mappings_js_1 = require("./types/control-evidence-mappings.js");
// =============================================================================
...
// Regulatory Pack Service
// =============================================================================
class RegulatoryPackService {
    packsDirectory;
    loadedPacks = new Map();
    constructor(packsDirectory) {
        this.packsDirectory = packsDirectory || path_1.default.join(process.cwd(), 'config/regulatory-packs');
    }
    // ===========================================================================
    // Pack Management
    // ===========================================================================
     ...
     */
    async loadAllPacks() {
        const span = otel_tracing_js_1.otelService.createSpan('regulatory_pack.load_all');
        const results = [];
        try {
            const files = await promises_1.default.readdir(this.packsDirectory).catch(() => []);
            for (const file of files) {
                if (file.endsWith('.yaml') || file.endsWith('.yml')) {
                    const result = await this.loadPack(path_1.default.join(this.packsDirectory, file));
                    results.push(result);
                }
            }
     ...
     */
    async loadPack(filePath) {
        const span = otel_tracing_js_1.otelService.createSpan('regulatory_pack.load');
        const validationErrors = [];
        const warnings = [];
        try {
            const content = await promises_1.default.readFile(filePath, 'utf-8');
            const rawPack = js_yaml_1.default.load(content);
            // Validate required fields
            if (!rawPack.id)
                validationErrors.push('Missing required field: id');
            if (!rawPack.name)
                validationErrors.push('Missing required field: name');
            if (!rawPack.framework)
                validationErrors.push('Missing required field: framework');
            ...
            const pack = {
                ...
                framework: rawPack.framework,
                version: rawPack.version || '1.0.0',
                status: rawPack.status || 'draft',
                controls: (rawPack.controls || []).map((c) => this.transformControl(c)),
    ...
    transformControl(raw) {
        return {
            ...
            evidenceSources: (raw.evidence_sources || []).map((es) => ({
     ...
     */
    async persistPack(pack) {
        const pool = (0, postgres_js_1.getPostgresPool)();
        await pool.query(`INSERT INTO regulatory_packs (id, name, framework, version, status, pack_data, created_at, updated_at)
    ...
    async getPack(packId) {
        ...
        // Load from database
        const pool = (0, postgres_js_1.getPostgresPool)();
        const { rows } = await pool.query('SELECT pack_data FROM regulatory_packs WHERE id = $1', [packId]);
        if (rows.length === 0)
            return null;
        const pack = rows[0].pack_data;
     ...
     */
    async collectControlEvidence(controlId, tenantId, dateRange) {
        const span = otel_tracing_js_1.otelService.createSpan('regulatory_pack.collect_evidence');
        const snapshots = [];
        const errors = [];
        try {
            ...
            for (const source of control.evidenceSources) {
                try {
                    ...
                    const snapshot = {
                        ...
                        contentHash: (0, crypto_1.createHash)('sha256').update(contentStr).digest('hex'),
                    ...
                    // Store snapshot
                    await pool.query(`INSERT INTO evidence_snapshots
     ...
     */
    applyRedaction(content, requestingTenantId) {
     ...
     */
    async assessControl(controlId, tenantId) {
        const span = otel_tracing_js_1.otelService.createSpan('regulatory_pack.assess_control');
        try {
            const control = control_evidence_mappings_js_1.CONTROL_MAPPINGS[controlId];
            if (!control) {
                throw new Error(`Control ${controlId} not found`);
     ...
     */
    async getRecentTestResults(controlId) {
        const pool = (0, postgres_js_1.getPostgresPool)();
     ...
     */
    calculateControlMetrics(controlId, testResults, evidenceSnapshots) {
        const passed = testResults.filter((t) => t.status === 'passed').length;
        const failed = testResults.filter((t) => t.status === 'failed').length;
        const total = testResults.length;
        // Calculate evidence freshness (average age in hours)
        const now = Date.now();
        const evidenceAges = evidenceSnapshots.map((e) => (now - new Date(e.capturedAt).getTime()) / (1000 * 60 * 60));
     ...
     */
    generateAssuranceChecklist(control, testResults, evidenceResult) {
        const items = [
            {
                id: 'documented',
     ...
     */
    determineControlStatus(testResults, evidenceResult, checklist) {
        ...
        if (checklist.overallStatus === 'not_ready')
            return 'partially_effective';
        return 'effective';
    }
    // ===========================================================================
     ...
     */
    async generatePackReport(packId, tenantId, format = 'json') {
        const span = otel_tracing_js_1.otelService.createSpan('regulatory_pack.generate_report');
        try {
            ...
            // Store report
            const pool = (0, postgres_js_1.getPostgresPool)();
            await pool.query(`INSERT INTO compliance_reports
         (id, pack_id, tenant_id, report_data, created_at)
         VALUES ($1, $2, $3, $4, now())`, [report.metadata.reportId, packId, tenantId, JSON.stringify(report)]);
            span?.addSpanAttributes({
    ...
    generateRecommendations(assessments) {
        ...
        if (ineffective.length > 0) {
            recommendations.push(`CRITICAL: ${ineffective.length} controls are ineffective and require immediate remediation: ${ineffective.map((a) => a.controlId).join(', ')}`);
        }
        if (partial.length > 0) {
            recommendations.push(`${partial.length} controls are partially effective and should be reviewed: ${partial.map((a) => a.controlId).join(', ')}`);
        }
        if (notTested.length > 0) {
            recommendations.push(`${notTested.length} controls have not been tested recently: ${notTested.map((a) => a.controlId).join(', ')}`);
        }
        // Check for stale evidence
        const staleEvidence = assessments.filter((a) => a.metrics.evidenceFreshness > 24);
        if (staleEvidence.length > 0) {
            recommendations.push(`${staleEvidence.length} controls have stale evidence (>24h old)`);
        }
...
exports.RegulatoryPackService = RegulatoryPackService;
// =============================================================================
// Schema Definition
// =============================================================================
exports.REGULATORY_PACK_SCHEMA = `
...
// Singleton Export
// =============================================================================
exports.regulatoryPackService = new RegulatoryPackService();

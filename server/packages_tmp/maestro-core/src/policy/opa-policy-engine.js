...
exports.MAESTRO_POLICY_TEMPLATES = exports.OPAPolicyEngine = void 0;
...
class OPAPolicyEngine {
    ...
    async check(action, subject, attributes) {
        try {
            const input = {
                action,
                subject,
            ...
            const response = await this.client.post(`/v1/data/${this.packageName}/allow`, {
                input,
            }, {
                params: {
                    explain: 'full',
            ...
            return {
                allowed: false,
                reason: `Policy engine unavailable: ${error.message}`,
            };
        }
    }
    async checkBatch(checks) {
        try {
            ...
            return checks.map((_, index) => ({
                allowed: false,
                reason: `Batch policy check failed: ${error.message}`,
            }));
        }
    }
...
exports.MAESTRO_POLICY_TEMPLATES = {

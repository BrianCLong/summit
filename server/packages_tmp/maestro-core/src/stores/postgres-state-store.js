...
const tracer = api_1.trace.getTracer('maestro-postgres-store');
class PostgresStateStore {
    ...
    async createRun(context) {
        return tracer.startActiveSpan('createRun', async (span) => {
          ...
          `, [
                    ...
                    // Initial event
                    await client.query(`INSERT INTO workflow_events (run_id, event_type, payload, trace_id) VALUES ($1, $2, $3, $4)`, [context.run_id, 'run_created', JSON.stringify({ workflow: context.workflow.name }), span.spanContext().traceId]);
                });
            }
            catch (error) {
    ...
    async updateRunStatus(runId, status, error) {
        return tracer.startActiveSpan('updateRunStatus', async (span) => {
            ...
            WHERE run_id = $3
          `, [status, error, runId]);
                    await client.query(`INSERT INTO workflow_events (run_id, event_type, payload) VALUES ($1, $2, $3)`, [runId, `run_status_${status}`, JSON.stringify({ error })]);
                });
            }
            finally {
                span.end();
    ...
    async getRunStatus(runId) {
        const result = await this.pool.query('SELECT status FROM workflow_runs WHERE run_id = $1', [runId]);
        return result.rows[0]?.status || 'not_found';
    }
    async getRunDetails(runId) {
        const client = await this.pool.connect();
        try {
            const runResult = await client.query(`SELECT * FROM workflow_runs WHERE run_id = $1`, [runId]);
            if (runResult.rows.length === 0)
                return null;
            const stepsResult = await client.query(`SELECT * FROM step_executions WHERE run_id = $1 ORDER BY created_at ASC`, [runId]);
            return {
                ...runResult.rows[0],
    ...
    async createStepExecution(execution) {
    ...
    // eslint-disable-next-line require-await
    async updateStepExecution(execution) {
        return tracer.startActiveSpan('updateStepExecution', async (span) => {
          ...
          `, [
                    ...
                    // Event log
                    await client.query(`INSERT INTO workflow_events (run_id, step_id, event_type, payload, trace_id) VALUES ($1, $2, $3, $4, $5)`, [
                        execution.run_id,
                        execution.step_id,
                    ...
                    if (['succeeded', 'failed'].includes(execution.status)) {
                        await client.query(`INSERT INTO workflow_outbox (run_id, event_type, payload, trace_context) VALUES ($1, $2, $3, $4)`, [
                            execution.run_id,
    ...
    async getActiveExecutions() {
        const result = await this.pool.query(`SELECT * FROM step_executions WHERE status IN ('running', 'pending')`);

...
class OllamaPlugin {
    ...
    async execute(context, step, execution) {
        ...
        try {
            ...
            if (stepConfig.fallback_models && stepConfig.fallback_models.length > 0) {
                return await this.executeWithFallback(stepConfig, context, execution, error);
            }
    ...
    async compensate(context, step, execution) {
        ...
        try {
            ...
            console.warn(`Ollama compensation warning: ${error.message}`);
        }
    }
    ...
    async selectOptimalModel(stepConfig) {
    ...
    preparePayload(stepConfig, model, context) {
        const payload = {
            model,
    ...
    async executeWithFallback(stepConfig, context, execution, originalError) {
    ...
    calculateComputeCost(durationMs, model) {
    ...
    getModelComputeMultiplier(model) {
        // Rough multipliers based on model size/complexity
        if (model.includes('7b') || model.includes('7B'))
            return 1.0;
        if (model.includes('13b') || model.includes('13B'))
            return 2.0;
        if (model.includes('34b') || model.includes('34B'))
            return 4.0;
        if (model.includes('70b') || model.includes('70B'))
            return 8.0;
        return 1.5; // Default multiplier
    }
    findSmallerAlternative(model) {

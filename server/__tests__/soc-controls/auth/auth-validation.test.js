...
describe('CC6.1 - Logical Access Security - Authentication Validation', () => {
    test('should validate user authentication against access controls', async () => {
        ...
        const authResult = await (0, auth_service_1.authenticateUser)(mockCredentials.email, mockCredentials.password);
    ...
    test('should enforce multi-factor authentication for privileged accounts', async () => {
        ...
        const authResult = await (0, auth_service_1.authenticateUser)(mockCredentials.email, mockCredentials.password, mockCredentials.mfaToken);
        expect(authResult.success).toBe(true);

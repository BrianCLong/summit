...
describe('NL→Cypher Guardrails', () => {
  ...
  describe('Dangerous Operation Blocking', () => {
    test.each(dangerousPrompts)('should block: %s', async (prompt) => {
      ...
      expect(response.body).toMatchObject({
        success: false,
        blocked: true,
        reason: expect.stringMatching(/dangerous|destructive|not allowed|security/i),
  ...
  describe('Safe Operations', () => {
    ...
    test.each(safePrompts)('should allow: %s', async (prompt) => {
      ...
      expect(cypher).not.toMatch(/\b(drop|delete|create\s+index|create\s+constraint|merge\s+.*on\s+create|call\s+dbms|call\s+apoc\.load|load\s+csv)\b/);
    });
  });
    ...
    test('should warn about budget limits', async () => {
      ...
      expect(response.body).toMatchObject({
        ...
        costEstimate: {
          estimatedCostUSD: expect.any(Number),
          budgetWarning: true,
          budgetSuggestion: expect.stringMatching(/consider|reduce|optimize|limit/i)
        }
      });
    });
  });

...
const ZeroDayService_js_1 = require("../src/zero_day/ZeroDayService.js");
jest.useFakeTimers();
describe('ZeroDayService', () => {
    let zeroDayService;
    beforeEach(() => {
        zeroDayService = new ZeroDayService_js_1.ZeroDayService();
    });
    ...
    it('should throw an error when delegating authority for a non-pending threat', async () => {
        await expect(zeroDayService.delegateAutonomousAuthority('zd-threat-redacted-001', 'op-1')).rejects.toThrow('Threat zd-threat-redacted-001 is not awaiting delegation or does not exist.');
    });
});

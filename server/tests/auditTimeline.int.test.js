...
const AuditTimelineRollupService_js_1 = require("../src/audit/AuditTimelineRollupService.js");
...
const supportsEnv = GenericContainer &&
...
const maybe = supportsEnv && runIntegration ? globals_1.describe : globals_1.describe.skip;
maybe('Audit timeline BRIN + rollups', () => {
    let container;
    let pool;
    let rollups;
    const migrationSql = fs_1.default.readFileSync(path_1.default.join(testsDir, 'db/migrations/postgres/2026-11-05_timeline_rollups_brin.sql'), 'utf8');
    const seedEvents = async (tenantId, count, anchor, offsetMinutes = 1) => {
        const values = [];
        const placeholders = [];
        for (let i = 0; i < count; i++) {
            const ts = new Date(anchor.getTime() - i * offsetMinutes * 60000);
            placeholders.push(`($${values.length + 1}, $${values.length + 2}, $${values.length + 3}, $${values.length + 4}, $${values.length + 5}, $${values.length + 6})`);
            values.push(tenantId, 'login', 'info', 'audit-service', ts.toISOString(), `event-${tenantId}-${i}`);
        }
        await pool.query(`
    ...
    `);
        ...
        rollups = new AuditTimelineRollupService_js_1.AuditTimelineRollupService(pool);
    ...
    (0, globals_1.beforeEach)(async () => {
        ...
        await pool.query('TRUNCATE audit_event_rollups_daily, audit_event_rollups_weekly, audit_event_rollup_state;');
    });
    (0, globals_1.it)('uses BRIN index for time-bounded queries', async () => {
        const now = new Date();
    ...
    `, ['tenant-a', new Date(now.getTime() - 3600 * 1000), now]));
        ...
        const indexNode = findPlanNode(rootPlan, (node) => node['Node Type'] === 'Bitmap Index Scan' &&
            node['Index Name'] === 'idx_audit_events_brin_tenant_timestamp');
        (0, globals_1.expect)(indexNode).toBeTruthy();
    });
    (0, globals_1.it)('refreshes rollups incrementally and serves reads via flag', async () => {
        ...
        (0, globals_1.expect)(firstRun.processedEvents).toBeGreaterThan(0);
        ...
        const initialCount = firstBuckets.find((b) => b.tenantId === 'tenant-a')?.eventCount || 0;
        (0, globals_1.expect)(initialCount).toBe(40);
        ...
        const updatedCount = secondBuckets.find((b) => b.tenantId === 'tenant-a')?.eventCount || 0;
        (0, globals_1.expect)(updatedCount).toBe(50);
        const state = await pool.query(`SELECT last_processed_at, last_run_status FROM audit_event_rollup_state WHERE rollup_name = 'audit_events'`);
        (0, globals_1.expect)(state.rows[0].last_run_status).toBe('ok');
        (0, globals_1.expect)(new Date(state.rows[0].last_processed_at).getTime()).toBeCloseTo(secondWindowEnd.getTime(), -1);
    });
});

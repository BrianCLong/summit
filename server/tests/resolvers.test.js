...
describe('GraphQL resolvers', () => {
  test('runGraphAnalysis returns array', async () => {
    ...
    const services = {
      graphAnalytics: {
        calculatePageRank: jest
          .fn()
          .mockResolvedValue([{ nodeId: '1', pageRank: 0.1 }]),
        calculateCentralityMeasures: jest
          .fn()
          .mockResolvedValue({ degreeCentrality: [] }),
        detectCommunities: jest.fn().mockResolvedValue([]),
        analyzeRelationshipPatterns: jest
          .fn()
          .mockResolvedValue({ patterns: [] }),
        detectAnomalies: jest.fn().mockResolvedValue({}),
        calculateBasicMetrics: jest
          .fn()
          .mockResolvedValue({ nodeCount: 0, edgeCount: 0 }),
      },
    };
    const input = {
  ...
  test('geointTimeSeries returns array', async () => {
    ...
    const services = {
      geoint: {
        buildTimeSeries: jest
          .fn()
          .mockReturnValue([{ start: 't', end: 't2', distanceKm: 1 }]),
      },
    };

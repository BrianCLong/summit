...
describe('Semantic PII Mapping Engine', () => {
  ...
  it('recognizes structured PII entities with contextual boosts', async () => {
    ...
    expect(types).toEqual(expect.arrayContaining(['email', 'phoneNumber', 'passportNumber']));
  ...
  it('applies taxonomy severity and schema metadata correctly', async () => {
    ...
    const classification = await engine.classify('123-45-6789', {
      ...
      schema: {
        ...
        fields: [
          { fieldName: 'ssn', description: 'Social Security Number', piiHints: ['socialSecurityNumber'], riskLevel: 'critical' }
        ]
      },
      schemaField: { fieldName: 'ssn', description: 'Social Security Number', piiHints: ['socialSecurityNumber'], riskLevel: 'critical' },
      recordId: 'rec-1'
    });
  ...
  it('supports incremental bulk scanning with change tracking', async () => {
    ...
    const firstReport = await scanner.scan(records, { incremental: true, includeUnchanged: true, minimumConfidence: 0.5 });
    ...
    expect(customer?.detected.find((entity: ClassifiedEntity) => entity.type === 'socialSecurityNumber')).toBeDefined();

    const secondReport = await scanner.scan(records, { incremental: true, includeUnchanged: true, minimumConfidence: 0.5 });
    expect(secondReport.results.length).toBeGreaterThan(0);
    expect(secondReport.results.every((result) => result.changed === false)).toBe(true);
    ...
    const thirdReport = await scanner.scan(mutatedRecords, { incremental: true, includeUnchanged: true, minimumConfidence: 0.5 });

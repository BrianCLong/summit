...
describe('GraphRAGService schema validation', () => {
  ...
  beforeEach(() => {
    service = new GraphRAGService(neo4jDriverMock, llmService, embeddingService);
    jest.spyOn(service, 'retrieveSubgraphWithCache').mockResolvedValue({
      entities: [{ id: '1', type: 'Entity', label: 'E1', properties: {}, confidence: 1 }],
      relationships: [
        {
          id: 'r1',
          type: 'REL',
  ...
  test('rejects invalid payload and increments metric', async () => {
    ...
    llmService.complete.mockResolvedValueOnce('not json').mockResolvedValueOnce('not json');
    await expect(service.answer({ investigationId: 'inv', question: 'Q?' })).rejects.toThrow(

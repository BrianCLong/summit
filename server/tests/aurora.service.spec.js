...
const AuroraService_js_1 = require("../src/aurora/AuroraService.js");
describe('AuroraService', () => {
    let auroraService;
    beforeEach(() => {
        auroraService = new AuroraService_js_1.AuroraService();
    });
    ...
    it('should throw an error for a neural handshake with an unknown implant', async () => {
        await expect(auroraService.neuralHandshake('unknown-id')).rejects.toThrow('Implant with ID unknown-id not found or not registered.');
    });
    ...
    it('should throw an error when beginning a thought stream for an offline implant', async () => {
        ...
        await expect(auroraService.beginThoughtStream(implantId)).rejects.toThrow(`Implant ${implantId} is not online. Handshake required.`);
    });
    ...
    it('should throw an error when pushing to cortex of an offline implant', async () => {
        ...
        await expect(auroraService.pushToCortex(overlay)).rejects.toThrow(`Target implant ${implantId} is not available for cortex overlay.`);
    });
});

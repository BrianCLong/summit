...
const NecromancerService_js_1 = require("../src/necromancer/NecromancerService.js");
jest.useFakeTimers();
describe('NecromancerService', () => {
    let necromancerService;
    beforeEach(() => {
        necromancerService = new NecromancerService_js_1.NecromancerService();
    });
    ...
    it('should throw an error when getting activity for a non-existent synthetic', async () => {
        await expect(necromancerService.getSyntheticActivity('unknown-id')).rejects.toThrow('Synthetic identity with ID unknown-id not found.');
    });
});

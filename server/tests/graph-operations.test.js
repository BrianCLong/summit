...
describe('Graph Operations Tests', () => {
  ...
  describe('Graph Export Functionality', () => {
    ...
    it('should export graph as CSV', async () => {
      const response = await request(app).get('/api/graph/export/csv').expect(200);

      expect(response.type).toBe('text/csv; charset=utf-8');
    ...
    it('should export graph as GraphML', async () => {
      const response = await request(app).get('/api/graph/export/graphml').expect(200);

      expect(response.type).toBe('application/xml; charset=utf-8');
    ...
    it('should reject unsupported export formats', async () => {
      const response = await request(app).get('/api/graph/export/unsupported').expect(400);
  ...
  describe('Graph Manipulation', () => {
    it('should add new nodes to graph', async () => {
      ...
      const response = await request(app).post('/api/graph/nodes').send(newNode).expect(201);
    ...
    it('should add new edges to graph', async () => {
      ...
      const response = await request(app).post('/api/graph/edges').send(newEdge).expect(201);
    ...
    it('should generate unique IDs for new elements', async () => {
      const responses = await Promise.all([
        request(app).post('/api/graph/nodes').send({ label: 'Node 1', type: 'TEST' }),
        request(app).post('/api/graph/nodes').send({ label: 'Node 2', type: 'TEST' }),
        request(app).post('/api/graph/nodes').send({ label: 'Node 3', type: 'TEST' }),
      ]);
  ...
  describe('Graph Querying and Filtering', () => {
    ...
    it('should filter graph by node types', async () => {
      ...
      const response = await request(app).get('/api/graph').query({ filter }).expect(200);
    ...
    it('should support pagination', async () => {
      ...
      await Promise.all([
        request(app).post('/api/graph/nodes').send({ label: 'Test 1', type: 'TEST' }),
        request(app).post('/api/graph/nodes').send({ label: 'Test 2', type: 'TEST' }),
        request(app).post('/api/graph/nodes').send({ label: 'Test 3', type: 'TEST' }),
      ]);
  ...
  describe('Graph Analytics', () => {
    it('should compute basic graph metrics', async () => {
      const response = await request(app).get('/api/graph/analytics').expect(200);
  ...
  describe('Performance and Stress Tests', () => {
    it('should handle multiple concurrent graph queries', async () => {
      const requests = Array.from({ length: 20 }, () => request(app).get('/api/graph'));
  ...
  describe('Error Handling', () => {
    it('should handle malformed node creation requests', async () => {
      ...
      const response = await request(app).post('/api/graph/nodes').send(invalidNode);
    ...
    it('should handle invalid filter parameters', async () => {
      const response = await request(app).get('/api/graph').query({ filter: 'invalid-json' });

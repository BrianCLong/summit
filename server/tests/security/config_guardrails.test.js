...
globals_1.jest.mock('dotenv/config', () => ({}));
const originalExit = process.exit;
let mockExit;
const mockConsoleError = globals_1.jest.spyOn(console, 'error').mockImplementation(() => { });
describe('Production Security Guardrails', () => {
    ...
    const loadConfig = async () => {
        if (globals_1.jest.isolateModulesAsync) {
            try {
                await globals_1.jest.isolateModulesAsync(async () => {
                    await Promise.resolve().then(() => __importStar(require('../../src/config.js')));
                });
                return null;
            }
            catch (error) {
            ...
            try {
                globals_1.jest.isolateModules(() => {
                    return Promise.resolve().then(() => __importStar(require('../../src/config.js')));
                });
                return null;
            }
            catch (error) {

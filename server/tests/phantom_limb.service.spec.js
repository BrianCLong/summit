...
const PhantomLimbService_js_1 = require("../src/phantom_limb/PhantomLimbService.js");
jest.useFakeTimers();
describe('PhantomLimbService', () => {
    let phantomLimbService;
    beforeEach(() => {
        phantomLimbService = new PhantomLimbService_js_1.PhantomLimbService();
    });
    ...
    it('should throw an error when querying a non-existent ghost', async () => {
        await expect(phantomLimbService.queryDigitalGhost('unknown-ghost', 'query')).rejects.toThrow('Digital ghost unknown-ghost is not online or does not exist.');
    });
});

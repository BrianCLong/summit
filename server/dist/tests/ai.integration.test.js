...
describe('AI Integration', () => {
    ...
    describe('Entity Extraction', () => {
        it('should queue entity extraction job', async () => {
            ...
            const jobId = uuid();
            const result = await AIResolvers.Mutation.aiExtractEntities(null, { docs, jobId }, mockContext);
    ...
    describe('Entity Resolution', () => {
        it('should queue entity resolution job', async () => {
            ...
            const result = await AIResolvers.Mutation.aiResolveEntities(null, { records, threshold }, mockContext);
    ...
    describe('Link Prediction', () => {
        it('should fetch graph edges and queue link prediction', async () => {
            ...
            const topK = 10;
            const result = await AIResolvers.Mutation.aiLinkPredict(null, { graphSnapshotId, topK }, mockContext);
    ...
    describe('Community Detection', () => {
        it('should fetch graph edges and queue community detection', async () => {
            ...
            const result = await AIResolvers.Mutation.aiCommunityDetect(null, { graphSnapshotId }, mockContext);
            expect(mockNeo4j.session).toHaveBeenCalled();
            expect(mockDb.jobs.insert).toHaveBeenCalled();
    ...
    describe('Insight Management', () => {
        it('should approve insight and create audit trail', async () => {
            ...
            const result = await AIResolvers.Mutation.approveInsight(null, { id: insightId }, { db: mockDb, user: { id: userId } });
        ...
        it('should reject insight with reason', async () => {
            ...
            const result = await AIResolvers.Mutation.rejectInsight(null, { id: insightId, reason }, { db: mockDb, user: { id: userId } });
            expect(mockDb.insights.update).toHaveBeenCalledWith(insightId, expect.objectContaining({
                status: 'REJECTED',
                decidedBy: userId,
            }));
    ...
    describe('Query Operations', () => {
        it('should retrieve AI job by ID', async () => {
            ...
            const result = await AIResolvers.Query.aiJob(null, { id: jobId }, { db: mockDb });
        ...
        it('should retrieve insights with filters', async () => {
            ...
            const result = await AIResolvers.Query.insights(null, { status: 'PENDING', kind: 'link_prediction' }, { db: mockDb });
    ...
    describe('Error Handling', () => {
        it('should handle ML service errors gracefully', async () => {
            ...
            await expect(AIResolvers.Mutation.aiExtractEntities(null, { docs: [{ id: 'doc1', text: 'test' }] }, mockContext)).rejects.toThrow('ML service unavailable');
            // Job should still be inserted and marked failed if ML service fails
            expect(mockDb.jobs.insert).toHaveBeenCalled();
            expect(mockDb.jobs.update).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                status: 'FAILED',
        ...
        it('should handle database errors', async () => {
            ...
            await expect(AIResolvers.Mutation.aiExtractEntities(null, { docs: [{ id: 'doc1', text: 'test' }] }, mockContext)).rejects.toThrow('Database error');
        });
        it('should handle Neo4j errors in graph operations', async () => {
            ...
            await expect(AIResolvers.Mutation.aiLinkPredict(null, { graphSnapshotId: 'snap1', topK: 10 }, mockContext)).rejects.toThrow('Neo4j connection failed');
        });
    });
});

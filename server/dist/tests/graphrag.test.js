...
async function seedGraph() {
    ...
    try {
        ...
        await session.run(`UNWIND $nodes as n
       CREATE (:Entity {id:n.id,type:n.type,label:n.label,description:n.description,investigationId:n.investigationId,confidence:n.confidence,createdAt:n.createdAt});`, { nodes });
        await session.run(`UNWIND $rels as r
       MATCH (a:Entity {id:r.from, investigationId:r.investigationId})
       MATCH (b:Entity {id:r.to, investigationId:r.investigationId})
       CREATE (a)-[:RELATIONSHIP {id:r.id,type:r.type,fromEntityId:r.from,toEntityId:r.to,confidence:r.confidence,createdAt:r.createdAt}]->(b);`, { rels });
    }
    finally {
        await session.close();
    }
...
//# sourceMappingURL=graphrag.test.js.map
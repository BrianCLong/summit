...
export async function cacheJobResult(jobId, result, ttlSeconds = 3600) {
    if (!redisClient)
        return;
    try {
...
export async function cacheGraphSnapshot(snapshotId, edges, ttlSeconds = 1800) {
    if (!redisClient)
        return;
    try {
...
export async function getCachedGraphSnapshot(snapshotId) {
    if (!redisClient)
        return null;
    try {
        const key = `ai:graph:${snapshotId}`;
...
export async function cachePrediction(predictionKey, prediction, ttlSeconds = 7200) {
    if (!redisClient)
        return;
    try {
...
export async function getCachedPrediction(predictionKey) {
    if (!redisClient)
        return null;
    try {
        const key = `ai:prediction:${predictionKey}`;
...
export async function checkRateLimit(userId, endpoint, maxRequests = 100, windowSeconds = 3600) {
    if (!redisClient)
...
export async function queueMLTask(taskId, taskData) {
    if (!redisClient)
        return;
    try {
...
export async function dequeueMLTask() {
    if (!redisClient)
        return null;
    try {
        const queueKey = 'ai:queue:ml_tasks';

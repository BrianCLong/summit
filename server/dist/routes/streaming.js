...
router.post('/ingest', requireAuthority('streaming_ingest', ['data_ingestion']), async (req, res) => {
    ...
    try {
        ...
        const messageId = await otelService.traceStreamingOperation('message_ingest', 1, span)(async () => {
...
router.post('/ingest/batch', requireAuthority('streaming_ingest', ['batch_ingestion']), async (req, res) => {
    const span = otelService.getCurrentSpan();
    try {
        ...
        await otelService.traceStreamingOperation('batch_ingest', messages.length, span)(async () => {
...
router.get('/metrics', requireAuthority('streaming_ingest', ['metrics_access']), async (req, res) => {
    try {
...
router.post('/queue/clear', requireAuthority('streaming_ingest', ['queue_management']), async (req, res) => {
    try {
        const user = req.user;
...
router.get('/events/stream', requireAuthority('streaming_ingest', ['real_time_events']), async (req, res) => {
    try {
        const user = req.user;

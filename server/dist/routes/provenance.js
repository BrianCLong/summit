...
router.post('/claims', requireAuthority('claim_registration', ['investigation', 'evidence']), async (req, res) => {
    try {
...
router.post('/manifests', requireAuthority('export_data', ['manifest', 'authority_basis']), async (req, res) => {
    try {
        const { export_type, data_sources, transformation_chain, authority_basis, classification_level, retention_policy } = req.body;
        const user = req.user;
...
router.post('/bundles', requireAuthority('export_data', ['bundle', 'immutable_seal']), async (req, res) => {
    try {
        const { claim_ids, evidence_references, export_type, authority_basis, classification_level } = req.body;
        const user = req.user;
...
router.post('/verify', requireAuthority('audit_verification', ['provenance']), async (req, res) => {
    try {
...
router.get('/audit', requireAuthority('audit_access', ['trail']), async (req, res) => {
    try {
        const { actor_id, operation_type, start_date, end_date, limit = 100 } = req.query;
        const filters = {};
        if (actor_id)
        ...
        res.json({
            success: true,
            audit_trail: auditTrail.slice(0, parseInt(limit)),

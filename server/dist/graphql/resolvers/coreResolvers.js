...
/**
 * Core GraphQL Resolvers for IntelGraph
 * Implements CRUD operations for Entity, Relationship, User, Investigation types
 */
const coreResolvers = {
    Query: {
        // Entity queries
        ...
        graphData: async (_, { investigationId }, { user }) => {
            ...
            try {
                ...
                const relationshipsResult = await session.run(`MATCH (from:Entity {investigationId: $investigationId})-[r:RELATIONSHIP]->(to:Entity {investigationId: $investigationId})
           RETURN r, from, to`, { investigationId });
    ...
    Mutation: {
        ...
        updateEntity: async (_, { id, input }, { user }) => {
            ...
            try {
                const updateFields = [];
                const params = { id, updatedBy: user.id, now: new Date().toISOString() };
        ...
        deleteRelationship: async (_, { id }, { user }) => {
            ...
            const session = driver.session();
            try {
                const result = await session.run(`MATCH ()-[r:RELATIONSHIP {id: $id}]-()
           DELETE r
           RETURN r.investigationId as investigationId`, { id });
                if (result.records.length === 0) {
                    throw new Error('Relationship not found');
                }
                const investigationId = result.records[0].get('investigationId');
    ...
    Investigation: {
        ...
        relationships: async (investigation, _, { user }) => {
            ...
            const session = driver.session();
            try {
                const result = await session.run(`MATCH (from:Entity {investigationId: $investigationId})-[r:RELATIONSHIP]->(to:Entity {investigationId: $investigationId})
           RETURN r, from, to`, { investigationId: investigation.id });
                return result.records.map(record => ({
                    ...record.get('r').properties,
                    fromEntity: record.get('from').properties,
                    toEntity: record.get('to').properties
...
module.exports = { coreResolvers };
//# sourceMappingURL=coreResolvers.js.map
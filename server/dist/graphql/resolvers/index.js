...
const resolvers = {
    ...
    Mutation: {
        ...
        async triggerN8n(_p, args, ctx) {
            ...
            const result = await triggerN8nFlow(args.flowKey, { runId: args.runId, payload: args.payload, user: ctx?.user?.id }, { userId: ctx?.user?.id, runId: args.runId });
        ...
        async approvePromotion(_p, args, ctx) {
            const role = ctx?.user?.role || 'VIEWER';

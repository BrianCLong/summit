...
const DEFAULT_NODE_TYPE_COLORS = Object.freeze({
    person: '#FF5733',
    organization: '#33FF57',
    location: '#3357FF',
    event: '#FF33FF',
...
function parseNodeTypeColors(value) {
    if (!value) {
        return {};
    }
    if (typeof value === 'string') {
    ...
    return Object.entries(value).reduce((acc, [key, color]) => {
...
function mergeNodeTypeColors(value) {
    return {
...
const crudResolvers = {
    Query: {
        ...
        entity: async (_, { id }, { user }) => {
            ...
            const permClause = permissions.includes('*') ? '' : ' WHERE e.type IN $permissions';
            try {
        ...
        relationship: async (_, { id }, { user }) => {
            ...
            const session = driver.session();
            const permissions = user.permissions || [];
            const permClause = permissions.includes('*') ? '' : ' WHERE r.type IN $permissions';
            try {
        ...
        investigation: async (_, { id }, { user }) => {
           ...
           RETURN i, creator, updater, collect(assigned) as assignedUsers`, { id });
                ...
                const creator = record.get('creator').properties;
                const updater = record.get('updater')?.properties;
                const assignedUsers = record.get('assignedUsers').map((u) => u.properties);
            ...
            try {
                ...
                const matchesTags = (obj) => {
                    ...
                    try {
                        ...
                        return Array.isArray(parsed.tags) && parsed.tags.some((t) => tags.includes(t));
                    }
                    catch {
                        return false;
                    }
        ...
        graphStyleSettings: async (_, __, { user }) => {
            if (!user)
                throw new Error('Not authenticated');
            const pgPool = getPostgresPool();
            const tenantId = user.tenantId || 'default';
            const result = await pgPool.query(`SELECT node_type_colors, node_size, edge_color, edge_width, updated_at
         FROM graph_visualization_settings
         WHERE user_id = $1 AND tenant_id = $2
         LIMIT 1`, [user.id, tenantId]);
    ...
    Mutation: {
        updateGraphStyleSettings: async (_, { input }, { user }) => {
            if (!user)
                throw new Error('Not authenticated');
            const pgPool = getPostgresPool();
            const tenantId = user.tenantId || 'default';
            const existingResult = await pgPool.query(`SELECT node_type_colors, node_size, edge_color, edge_width
         FROM graph_visualization_settings
         WHERE user_id = $1 AND tenant_id = $2
         LIMIT 1`, [user.id, tenantId]);
            ...
            const nodeSize = normalizeNumber(input.nodeSize ?? existingRow?.node_size ?? DEFAULT_STYLE_SETTINGS.nodeSize, DEFAULT_STYLE_SETTINGS.nodeSize, 16, 160);
            const edgeWidth = normalizeNumber(input.edgeWidth ?? existingRow?.edge_width ?? DEFAULT_STYLE_SETTINGS.edgeWidth, DEFAULT_STYLE_SETTINGS.edgeWidth, 1, 16);
            const edgeColor = normalizeColor(input.edgeColor ?? existingRow?.edge_color ?? DEFAULT_STYLE_SETTINGS.edgeColor, DEFAULT_STYLE_SETTINGS.edgeColor);
            const now = new Date().toISOString();
            const upsertResult = await pgPool.query(`INSERT INTO graph_visualization_settings (
           user_id,
           tenant_id,
           node_type_colors,
           node_size,
        ...
        createEntities: async (_, { inputs }, { user }) => {
            ...
            try {
             ...
             RETURN e`, {
                    ...
                    await pgClient.query(auditQuery, [user.id, now, 'Evidence', payloadHash, opId]);
                }
                await tx.commit();
            ...
            try {
             ...
             RETURN r, from, to`, {
                    ...
                    const payloadHash = crypto
                        ...
                        .digest('hex');
                    const auditQuery = 'INSERT INTO "AuditLog" (user_id, timestamp, entity_type, payload_hash, operation_id) VALUES ($1,$2,$3,$4,$5)';
                    await pgClient.query(auditQuery, [user.id, now, 'Relationship', payloadHash, opId]);
                }
                await tx.commit();
                await pgClient.query('COMMIT');
            }
        ...
        updateRelationship: async (_, { id, input }, { user }) => {
           ...
           RETURN r, from, to`, params);
                ...
                await nbhdCache.invalidate(user.tenantId, relationship.investigationId, [

...
export const resolvers = {
    Query: {
        suggestions: async (_, { status = "pending", limit = 50 }) => {
      ...
      `, { caseId });
        ...
        tenantConfig: async (_, { id }, ctx) => {
            if (!ctx.user?.scopes?.includes("platform:admin"))
    ...
    Mutation: {
        createCase: async (_, { name }, ctx) => {
            if (!ctx.user?.scopes?.includes("case:write"))
                throw new Error("forbidden");
            const rows = await runCypher(`
      ...
      `, { name });
        ...
        updateCase: async (_, { id, name }, ctx) => {
            if (!ctx.user?.scopes?.includes("case:write"))
                throw new Error("forbidden");
            const rows = await runCypher(`
        MATCH (c:Case {id: $id})
      ...
      `, { id });
        ...
        attachAnswerToCase: async (_, { caseId, answerId, sig }, ctx) => {
            if (!ctx.user?.scopes?.includes("case:write"))
                throw new Error("forbidden");
      ...
      `, { caseId, userId, answerId, sig });
        ...
        acceptSuggestion: async (_, { id }, ctx) => {
            // RBAC
            if (!ctx.user?.scopes?.includes("graph:write"))
                throw new Error("forbidden");
            const rows = await runCypher(`
      ...
      `, { id });
        ...
        rejectSuggestion: async (_, { id }, ctx) => {
            if (!ctx.user?.scopes?.includes("graph:write"))
                throw new Error("forbidden");
            const rows = await runCypher(`
        MATCH (s:AISuggestion {id: $id}) WHERE s.status='pending'
      ...
      `, { id });
        ...
        upsertTenantConfig: async (_, args, ctx) => {
            if (!ctx.user?.scopes?.includes("platform:admin"))
                throw new Error("forbidden");

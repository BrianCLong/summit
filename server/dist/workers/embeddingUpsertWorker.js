...
export class EmbeddingUpsertWorker {
    ...
    async start() {
        try {
            ...
            // Initialize worker
            this.worker = new Worker("entity-embeddings", this.processEmbeddingJob.bind(this), {
                connection: redis,
    ...
    async processEmbeddingJob(job) {
        const parentContext = job.data.traceparent
            ...
            : context.active();
        return context.with(parentContext, () => otelService.wrapBullMQJob("entity-embedding", async () => {

...
export class AuditLogService {
    constructor(neo4jDriver, filePath = path.join(process.cwd(), 'audit.log')) {
    ...
    async logEvent(event) {
        ...
        try {
            ...
            await session.run('CREATE (a:AuditEvent {userId:$userId, tenantId:$tenantId, action:$action, resource:$resource, timestamp:$timestamp, details:$details})', entry);
            await session.close();
        }
        catch (err) {
            // ignore neo4j errors

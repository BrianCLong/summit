...
class WebSocketService {
    ...
    setupSocketEventHandlers(socket) {
        ...
        socket.on('send_chat_message', async (message) => {
            try {
                ...
                const result = await pool.query(`INSERT INTO chat_messages (investigation_id, user_id, content)
           VALUES ($1, $2, $3)
           RETURNING id, created_at`, [investigationId, socket.userId, content]);
        ...
        socket.on('delete_chat_message', async (data) => {
            try {
                ...
                if (!investigationId || !messageId)
                    return;
                const { getPostgresPool } = require('../config/database');
                const pool = getPostgresPool();
                await pool.query('UPDATE chat_messages SET deleted_at = CURRENT_TIMESTAMP WHERE id = $1', [
                    messageId,
                ]);
        ...
        socket.on('edit_chat_message', async (data) => {
            try {
                ...
                if (!investigationId || !messageId || !newContent)
                    return;
                const { getPostgresPool } = require('../config/database');
                const pool = getPostgresPool();
    ...
    handleCollabBatch(socket, batch = []) {
        ...
        batch.forEach((op) => {
            if (socket.processedOps.has(op.opId))
                return;
    ...
    handleGraphNodeAdded(socket, data) {
        ...
        this.writeAuditLog(session.userId, 'GRAPH_NODE_ADD', 'Investigation', investigationId, {
    ...
    handleGraphNodeUpdated(socket, data) {
        ...
        socket.to(`investigation:${investigationId}`).emit('graph:update', eventData);
        this.writeAuditLog(session.userId, 'GRAPH_NODE_UPDATE', 'Investigation', investigationId, {
            nodeId: node?.id || node?.uuid,
            changes,
        });
    }
    handleGraphNodeDeleted(socket, data) {
        ...
        socket.to(`investigation:${investigationId}`).emit('graph:update', eventData);
        this.writeAuditLog(session.userId, 'GRAPH_NODE_DELETE', 'Investigation', investigationId, {
            nodeId,
        });
    }
    handleGraphEdgeAdded(socket, data) {
        ...
        socket.to(`investigation:${investigationId}`).emit('graph:update', eventData);
        this.writeAuditLog(session.userId, 'GRAPH_EDGE_ADD', 'Investigation', investigationId, {
            edgeId: edge?.id,
        });
    }
    handleGraphEdgeUpdated(socket, data) {
        ...
        socket.to(`investigation:${investigationId}`).emit('graph:update', eventData);
        this.writeAuditLog(session.userId, 'GRAPH_EDGE_UPDATE', 'Investigation', investigationId, {
            edgeId: edge?.id,
            changes,
        });
    }
    handleGraphEdgeDeleted(socket, data) {
        ...
        socket.to(`investigation:${investigationId}`).emit('graph:update', eventData);
        this.writeAuditLog(session.userId, 'GRAPH_EDGE_DELETE', 'Investigation', investigationId, {
            edgeId,
        });
    }
    handleGraphLayoutChanged(socket, data) {
        ...
        socket.to(`investigation:${investigationId}`).emit('graph:update', eventData);
        this.writeAuditLog(session.userId, 'GRAPH_LAYOUT_CHANGE', 'Investigation', investigationId, {
            layout,
        });
    }
    ...
    async handleCommentAdd(socket, data) {
        ...
        try {
            const { getPostgresPool } = require('../config/database');
            const pool = getPostgresPool();
            const res = await pool.query(`INSERT INTO comments (investigation_id, target_id, user_id, content, metadata)
         VALUES ($1, $2, $3, $4, $5) RETURNING id, created_at`, [
            ...
            await this.writeAuditLog(session.userId, 'COMMENT_CREATE', 'investigation', investigationId, {
    ...
    async handleCommentUpdate(socket, data) {
        ...
        try {
            const { getPostgresPool } = require('../config/database');
            const pool = getPostgresPool();
            await pool.query(`UPDATE comments SET content = $1, metadata = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`, [comment.content || '', comment.metadata || {}, comment.id]);
            await this.writeAuditLog(session.userId, 'COMMENT_UPDATE', 'investigation', investigationId, {
    ...
    async handleCommentDelete(socket, data) {
        ...
        try {
            const { getPostgresPool } = require('../config/database');
            const pool = getPostgresPool();
            await pool.query('UPDATE comments SET deleted_at = CURRENT_TIMESTAMP WHERE id = $1', [
                commentId,
            ]);
            await this.writeAuditLog(session.userId, 'COMMENT_DELETE', 'investigation', investigationId, {
                commentId,
            });
            const eventData = {
...
WebSocketService.prototype.writeAuditLog = async function (userId, action, resourceType, resourceId, details) {
    try {
       ...
       VALUES ($1, $2, $3, $4, $5)`, [userId, action, resourceType, resourceId, details || {}]);
    }
    catch (err) {

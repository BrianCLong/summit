...
export class HybridEntityResolutionService {
     ...
     */
    async resolveEntitiesPair(entityA, entityB, entityType = 'PERSON') {
    ...
    heuristicMLScore(entityA, entityB) {
        ...
        if (entityA.name && entityB.name) {
            const nameScore = this.jaroWinklerSimilarity(this.normalizeString(entityA.name), this.normalizeString(entityB.name));
            score += nameScore * 0.4;
        ...
        if (entityA.behavioralFingerprint && entityB.behavioralFingerprint) {
            const behavioralScore = this.behavioralSimilarity(entityA.behavioralFingerprint, entityB.behavioralFingerprint);
     ...
     */
    async processMergeDecision(session, entityAId, entityBId, entityType = 'PERSON') {
    ...
    `, { entityAId, entityBId });
        ...
        const mergeDecision = {
            ...
            explanation: erResult.explanation,

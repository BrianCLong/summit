...
class CopilotOrchestrationService extends EventEmitter {
    ...
    async executeStepsInParallel(steps, query, results) {
        ...
        for (let i = 0; i < settled.length; i++) {
            if (steps[i].type === 'GRAPH_QUERY' && steps[i].status === 'FAILED' && steps[i].error) {
    ...
    async executeGraphQuery(step) {
        ...
        return {
            entities: [{ id: '1', label: 'Mock Entity', type: 'PERSON', confidence: 0.9 }],
            relationships: [{ id: '1', type: 'KNOWS', source: '1', target: '2', confidence: 0.8 }],
        };
    }
    ...
    async executeGraphAnalytics(step) {
        ...
        return {
            ...
            clusters: [{ id: 'cluster1', entities: ['entity1', 'entity2'], cohesion: 0.78 }],
        };
    }
    ...
    calculateResultConfidence(results) {
        if (results.entities.length === 0)
            return 0.1;
        const entityConfidences = results.entities.filter((e) => e.confidence).map((e) => e.confidence);

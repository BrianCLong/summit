export class ProvenanceRepo {
    ...
    async by(scope, id, filter, first = 1000, offset = 0, tenantId) {
        ...
        try {
            ...
            const sqls = [];
            if (tenantId) {
                sqls.push(`SELECT id, action, target_type, target_id, metadata, created_at FROM audit_events ${where} AND tenant_id = $${params.push(tenantId)} ORDER BY COALESCE(created_at, NOW()) DESC${baseLimit}`, `SELECT id, action, resource_type, resource_id, resource_data, old_values, new_values, investigation_id, timestamp FROM audit_events ${where} AND tenant_id = $${params.push(tenantId)} ORDER BY COALESCE(timestamp, NOW()) DESC${baseLimit}`);
            }
            // Non-tenant filtered fallbacks
            sqls.push(`SELECT id, action, target_type, target_id, metadata, created_at FROM audit_events ${where} ORDER BY COALESCE(created_at, NOW()) DESC${baseLimit}`, `SELECT id, action, resource_type, resource_id, resource_data, old_values, new_values, investigation_id, timestamp FROM audit_events ${where} ORDER BY COALESCE(timestamp, NOW()) DESC${baseLimit}`, `SELECT id, source, subject_type, subject_id, note, created_at FROM provenance ${where} ORDER BY created_at DESC${baseLimit}`);

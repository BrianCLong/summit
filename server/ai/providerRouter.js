...
const providers = [
    {
        ...
        call: async (payload) => {
            (0, node_assert_1.default)(has("GROQ_API_KEY"), "Missing GROQ_API_KEY");
            const r = await (0, node_fetch_1.default)("https://api.groq.com/openai/v1/chat/completions", {
                method: "POST",
    ...
    },
    {
        ...
        call: async (payload) => {
            (0, node_assert_1.default)(has("OPENROUTER_API_KEY"), "Missing OPENROUTER_API_KEY");
            const r = await (0, node_fetch_1.default)("https://openrouter.ai/api/v1/chat/completions", {
                method: "POST",
    ...
    },
    {
        ...
        call: async (payload) => {
            (0, node_assert_1.default)(has("OPENAI_API_KEY"), "Missing OPENAI_API_KEY");
    ...
    },
    {
        ...
        call: async (payload) => {
            (0, node_assert_1.default)(has("ANTHROPIC_API_KEY"), "Missing ANTHROPIC_API_KEY");
            const r = await (0, node_fetch_1.default)("https://api.anthropic.com/v1/messages", {
                method: "POST",
                headers: {
...
async function routeLLM(opts, payload) {
    const order = ["groq", "openrouter", "openai", "anthropic"];
    let spent = 0;
    let lastErr = "";
    for (const name of order) {
        ...
        if (!has(p.envKey))
            continue;
        if (!p.supports(opts.tag))
            continue;
        const est = p.estimate(opts.tag, opts.inputTokens);
        if (!opts.allowPaid && est.costUsd > 0)
            continue;
        if (spent + est.costUsd > opts.hardCostUsd)
            continue;

...
cost_guard_1.costGuard.setBudgetLimits('analytics-team', {
    daily: 10.0, // $10/day for analytics queries
    monthly: 200.0,
    query_burst: 2.0, // Allow expensive analytics queries
    rate_limit_cost: 5.0,
});
cost_guard_1.costGuard.setBudgetLimits('app-backend', {
...
class EnhancedUserRepository {
    ...
    estimateComplexity(queryType, params) {
        ...
        if (params?.dateRange) {
            const days = Math.ceil((params.dateRange.end.getTime() - params.dateRange.start.getTime()) /
                (1000 * 60 * 60 * 24));
    ...
    async findById(id, tenantId, userId) {
        return tracing_1.tracer.traceDatabase('select', 'postgres', async () => {
    ...
    async findAll(limit, tenantId, userId) {
        const complexity = this.estimateComplexity('findAll', { limit });
        return (0, cost_guard_1.withCostGuardDB)({
            tenantId,
            userId,
    ...
    async searchWithFilters(filters, tenantId, userId) {
    ...
    async getUserStatistics(dateRange, tenantId, userId) {
...
class GraphDatabaseRepository {
    ...
    async findRelationships(entityId, depth, tenantId, userId) {
        ...
        return (0, cost_guard_1.withCostGuardDB)({
            ...
            return tracing_1.tracer.traceDatabase('cypher', 'neo4j', async () => {
                const stopTimer = red.database.startTimer({
                    db_type: 'neo4j',
                    operation: 'cypher',
                });
...
async function exampleUsage() {
    ...
    try {
        ...
        const relationships = await graphRepo.findRelationships('entity-123', 3, 'analytics-team', 'analyst-1');
    ...
    console.log('\n6. Budget Analysis:');
    const appBackendAnalysis = await cost_guard_1.costGuard.getCostAnalysis('app-backend');
    const analyticsAnalysis = await cost_guard_1.costGuard.getCostAnalysis('analytics-team');
    console.log('\n   app-backend:');

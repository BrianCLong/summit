...
console.info('Reading lint report...');
try {
    const results = JSON.parse(fs.readFileSync('lint_report.json', 'utf8'));
    console.info(`Loaded ${results.length} file results.`);
    ...
    for (const result of results) {
        ...
        try {
            ...
            if (modified) {
                ...
                console.info(`Fixed ${errors.length} in ${filePath}`);
            }
        } catch (fErr) {
            console.error(`Failed to process ${filePath}:`, fErr.message);
        }
    }
    console.info(`Done. Added ${totalFixed} comments.`);

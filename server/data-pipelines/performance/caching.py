...
class CacheItem:
    ...
    def __init__(self, key: str, value: Any, ttl_seconds: int | None = None):
        self.key = key
        self.value = value
...
class RedisCache:
    ...
    def _serialize(self, value: Any) -> bytes:
        ...
        if (
            ...
            and len(data_to_store) > self.config.compression_threshold_bytes
        ):
            import gzip
    ...
    def _deserialize(self, data: bytes) -> Any:
        ...
        if not secret_key:
            self.logger.error(
                "SUMMIT_SIGNING_KEY environment variable not set. Cannot deserialize."
            )
            raise ValueError("SUMMIT_SIGNING_KEY not set")
...
class DiskCache:
    ...
    def _ensure_capacity(self, new_item_size: int):
        ...
        while current_size + new_item_size > max_size_bytes and self.metadata:
            ...
            oldest_key = min(
                self.metadata.keys(), key=lambda k: self.metadata[k].get("access_time", 0)
            )
...
def cached(
    cache_manager: CacheManager, ttl_seconds: int | None = None, key_func: Callable | None = None
):
    """
...
def create_query_cache(cache_manager: CacheManager) -> Callable:
    ...
    def cache_query(query: str, params: dict[str, Any] | None = None, ttl_seconds: int = 3600):
        """Cache query results"""
        # Generate cache key from query and parameters
        key_data = {"query": query, "params": params or {}}
        cache_key = (
            f"query:{hashlib.md5(json.dumps(key_data, sort_keys=True).encode()).hexdigest()}"
        )

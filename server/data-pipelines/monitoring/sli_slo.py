...
class SLICollector:
    ...
    def register_sli(self, sli: ServiceLevelIndicator, measurement_func: Optional[Callable] = None):
        """Register a new SLI"""
        with self.lock:
    ...
    def record_measurement(self, sli_name: str, value: float, success: bool = True, 
                          labels: Optional[Dict[str, str]] = None):
    ...
    def record_availability(self, sli_name: str, success: bool, response_time: Optional[float] = None):
    ...
    def record_latency_percentile(self, sli_name: str, latencies: List[float], percentile: float = 95.0):
    ...
    def get_measurements(self, sli_name: str, 
                        start_time: Optional[datetime] = None,
                        end_time: Optional[datetime] = None) -> List[SLIDataPoint]:
    ...
    def get_success_rate(self, sli_name: str, time_window: TimeWindow) -> Optional[float]:
...
class SLOManager:
    ...
    def evaluate_slo(self, slo_name: str) -> Optional[SLOEvaluation]:
        ...
        error_budget_remaining_pct = (error_budget_remaining / error_budget_total) * 100.0 if error_budget_total > 0 else 100.0
        
        # Calculate burn rate (how fast we're consuming error budget)
        burn_rate = self._calculate_burn_rate(slo_name, slo.time_window)
        
        # Determine status
        status = self._determine_slo_status(success_rate, slo.target_percentage, error_budget_remaining_pct, burn_rate)
    ...
    def _calculate_burn_rate(self, slo_name: str, time_window: TimeWindow) -> float:
        ...
        time_diff = (current.evaluation_time - previous.evaluation_time).total_seconds() / 3600  # hours
        if time_diff == 0:
            return 0.0
    ...
    def _determine_slo_status(self, actual_percentage: float, target_percentage: float, 
                            error_budget_remaining: float, burn_rate: float) -> SLOStatus:
...
def create_slo_alert_handler(webhook_url: Optional[str] = None, 
                           email_recipients: Optional[List[str]] = None) -> Callable:

...
class DeadLetterQueue:
    ...
    async def add_failed_message(self,
                               original_topic: str,
                               partition: int,
                               offset: int,
                               message_key: Optional[str],
    ...
    async def mark_retry_failure(self, message_id: str, failure_reason: str):
        ...
        if message.retry_count >= message.max_retries:
            ...
            self.logger.error(f"Message {message_id} marked as poisoned after {message.retry_count} retries")
        else:
    ...
    async def _get_message_by_id(self, message_id: str) -> Optional[FailedMessage]:
        ...
        if self.storage_path.exists():
            try:
                with open(self.storage_path, 'r') as f:
                    data = json.load(f)
                    self.messages = {msg_id: FailedMessage.from_dict(msg_data) 
    ...
    def get_dlq(self, name: str, **kwargs) -> DeadLetterQueue:
        ...
        if name not in self.dlqs:
            self.dlqs[name] = DeadLetterQueue(
                ...
                retry_backoff_seconds=kwargs.get('retry_backoff_seconds', self.default_backoff_seconds)
            )
        
        return self.dlqs[name]
    
    async def process_all_retries(self, processor: Callable[[FailedMessage], bool]) -> Dict[str, int]:

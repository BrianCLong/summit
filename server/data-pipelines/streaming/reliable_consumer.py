...
class ConsumerConfig:
    ...
    dlq_redis_url: Optional[str] = None
...
class ReliableKafkaConsumer:
    ...
    def __init__(self, 
                 ...
                 message_processor: Callable[[Dict[str, Any]], bool]):
        ...
        self.flow_control: Optional[FlowControl] = None
    ...
    async def _consume_loop(self):
        ...
        try:
            while self.running and not self.shutdown_event.is_set():
                try:
                    # Check flow control
                    if self.flow_control and not await self.flow_control.backpressure_controller.should_accept_message():
                        self.metrics.backpressure_events += 1
                        await asyncio.sleep(0.1)  # Brief pause under backpressure
                        continue
                    
                    # Consume messages
                    msg_pack = await self.consumer.getmany(timeout_ms=1000, max_records=self.config.batch_size)
        ...
        for topic_partition, messages in msg_pack.items():
            ...
            for message in messages:
                try:
                    ...
                    self.logger.error(f"Failed to process message from {topic}:{partition}:{message.offset}: {e}")
    ...
    async def _process_single_message(self, topic: str, partition: int, message):
        ...
        try:
            ...
            if success:
                ...
                if self.flow_control:
                    await self.flow_control.backpressure_controller.on_message_processed(processing_time, True)
                ...
                if self.flow_control:
                    await self.flow_control.backpressure_controller.on_message_processed(processing_time, False)
    ...
    async def _call_processor_with_timeout(self, message_data: Dict[str, Any]) -> bool:
        ...
        try:
            result = await asyncio.wait_for(
                self.message_processor(message_data),
                timeout=self.config.processing_timeout_seconds
            )
            return result if isinstance(result, bool) else True
            
        except asyncio.TimeoutError:
            self.logger.warning(f"Message processing timed out after {self.config.processing_timeout_seconds}s")
            return False
        except Exception as e:
    ...
    def _mark_offset_for_commit(self, topic: str, partition: int, offset: int):
        ...
        if partition not in self.pending_offsets[topic]:
            ...
            self.pending_offsets[topic][partition] = max(self.pending_offsets[topic][partition], offset)
...
if __name__ == "__main__":
    ...
    asyncio.run(run_reliable_consumer_forever(
        topics=["test-topic"],

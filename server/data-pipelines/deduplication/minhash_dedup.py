...
class MinHashDeduplicator:
    ...
    def _save_to_file(self):
        ...
        try:
            ...
            if not secret_key:
                self.logger.error(
                    "SUMMIT_SIGNING_KEY environment variable not set. Cannot save to file."
                )
                return

            data_to_pickle = {"lsh": self.lsh, "record_hashes": self.record_hashes}
    ...
    def _load_from_file(self):
        ...
        try:
            if self.storage_path.exists():
                ...
                if not secret_key:
                    self.logger.error(
                        "SUMMIT_SIGNING_KEY environment variable not set. Cannot load from file."
                    )
                    return
                ...
                with open(self.storage_path, "rb") as f:
                    ...
                    expected_signature = hashlib.sha256(
                        secret_key.encode("utf-8") + pickled_data
                    ).hexdigest()

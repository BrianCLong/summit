...
class DataPipelineLogger:
    ...
    def log_ingestion_progress(self,
                              connector_name: str,
    ...
    def log_ingestion_complete(self,
        ...
        context = {
            ...
            'throughput_per_sec': stats.get('records_success', 0) / duration_seconds if duration_seconds > 0 else 0
        }
        self.info(
            f"Ingestion completed for {connector_name} in {duration_seconds:.2f}s",
            **context
        )
    ...
    def log_transformation_results(self,
        ...
        context = {
            ...
            'duration_seconds': duration_seconds,
            'entities_per_second': output_entities / duration_seconds if duration_seconds > 0 else 0
        }
        self.info(
    ...
    def log_load_results(self,
        ...
        context = {
            ...
            'success_rate': entities_loaded / (entities_loaded + entities_failed) if (entities_loaded + entities_failed) > 0 else 0,
            'duration_seconds': duration_seconds,
            'entities_per_second': entities_loaded / duration_seconds if duration_seconds > 0 else 0
        }
        self.info(
...
def setup_logging(log_level: str = "INFO", log_dir: Path = None):
    ...
    root_logger.info("Logging configured", log_level=log_level, log_dir=str(log_dir) if log_dir else None)
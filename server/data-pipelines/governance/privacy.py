...
class PIIDetectionRule:
    ...
    field_names: Optional[List[str]] = None  # Field names to check
    description: Optional[str] = None
    
class PIIDetectionResult:
    """Result of PII detection"""
    def __init__(self, field: str, pii_type: PIIType, confidence: float, 
                 value: Any, masked_value: Any = None):
        self.field = field
        ...
        for field, value in record.items():
            ...
            for rule in self.detection_rules + self.custom_rules:
                ...
                if rule.field_names and field.lower() in [fn.lower() for fn in rule.field_names]:
                    confidence = min(rule.confidence + 0.1, 1.0)  # Boost confidence for field name match
                ...
                if rule.pii_type in self.compiled_patterns:
                    ...
                    if pattern.search(str_value):
                        results.append(PIIDetectionResult(field, rule.pii_type, rule.confidence, value))
...
class DataMasker:
    ...
    def mask_record(self, record: Dict[str, Any], 
                   ...
                   strategy_map: Optional[Dict[PIIType, MaskingStrategy]] = None) -> Dict[str, Any]:
    ...
    def mask_dataframe(self, df: 'pd.DataFrame',
                      ...
                      strategy_map: Optional[Dict[PIIType, MaskingStrategy]] = None) -> 'pd.DataFrame':
        ...
        for user_consents in self.consent_records.values():
            for consent in user_consents:
                if (consent.expires_at and 
                    datetime.now() > consent.expires_at and 
                ...
                for user_id, consents_data in data.items():
                    ...
                    for consent_data in consents_data:
                        ...
                        if consent_data.get('granted_at'):
                            consent_data['granted_at'] = datetime.fromisoformat(consent_data['granted_at'])
                        if consent_data.get('withdrawn_at'):
                            consent_data['withdrawn_at'] = datetime.fromisoformat(consent_data['withdrawn_at'])
...
class PrivacyGovernor:
    ...
    def process_record_with_privacy(self, record: Dict[str, Any],
        ...
        try:
            ...
            if user_id:
                ...
                if not consent_valid:
                    ...
                    masked_record = self.data_masker.mask_record(record, pii_detections, strict_strategies)
                    audit_entry['actions_taken'].append('strict_masking_no_consent')
                    self.audit_log.append(audit_entry)
                    return masked_record
            
            # 3. Apply configured masking strategies
            masked_record = self.data_masker.mask_record(record, pii_detections, self.masking_strategies)
    ...
    def process_dataframe_with_privacy(self, df: 'pd.DataFrame',
                                     user_id_column: Optional[str] = None,
                                     processing_purpose: ProcessingPurpose = ProcessingPurpose.LEGITIMATE_INTERESTS) -> 'pd.DataFrame':
    ...
    def generate_privacy_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'summary': {
                ...
                'pii_detection_events': len([entry for entry in self.audit_log if entry['pii_detected']]),
                'consent_checks': len([entry for entry in self.audit_log if entry['consent_status']]),
                'masking_applications': len([entry for entry in self.audit_log if 'masking' in ' '.join(entry['actions_taken'])])
            },
            'pii_types_detected': {},
    ...
    def add_custom_pii_rule(self, pii_type: PIIType, pattern: str, 
                           confidence: float, field_names: Optional[List[str]] = None):
        """Add custom PII detection rule"""
...
def create_gdpr_compliant_processor(encryption_key: Optional[bytes] = None) -> PrivacyGovernor:
...
def anonymize_dataset_for_research(df: 'pd.DataFrame') -> 'pd.DataFrame':
    ...
    return governor.process_dataframe_with_privacy(df, processing_purpose=ProcessingPurpose.LEGITIMATE_INTERESTS)
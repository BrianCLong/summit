...
const ActiveMeasuresEngine_1 = require("../ActiveMeasuresEngine");
// Mock dependencies
...
describe('ActiveMeasuresEngine', () => {
    ...
    beforeEach(() => {
        mockNeo4jDriver = {};
        engine = new ActiveMeasuresEngine_1.ActiveMeasuresEngine(mockNeo4jDriver);
    });
    ...
    describe('Operation Management', () => {
        ...
        it('should link measures to operation', async () => {
            ...
            expect(activeMeasuresGraphRepo.linkOperationToMeasures).toHaveBeenCalledWith('operation-id', ['measure-1', 'measure-2']);
        });
        it('should create audit entry on operation creation', async () => {
            ...
            await engine.createOperation(operationData);
            expect(activeMeasuresGraphRepo.createAuditEntry).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Measure Combination', () => {
        beforeEach(() => {
            jest.spyOn(engine, 'getMeasureById').mockResolvedValue({
                id: 'measure-1',
                name: 'Test Measure',
                category: 'INFORMATION_OPERATIONS',
                effectivenessRating: 0.75,
    ...
    describe('Risk Assessment', () => {
        it('should calculate risk penalty for different risk levels', () => {
            const engine = new ActiveMeasuresEngine_1.ActiveMeasuresEngine(mockNeo4jDriver);
            expect(engine.calculateRiskPenalty('MINIMAL')).toBe(0.0);
            expect(engine.calculateRiskPenalty('LOW')).toBe(0.1);
            expect(engine.calculateRiskPenalty('MODERATE')).toBe(0.3);
            expect(engine.calculateRiskPenalty('HIGH')).toBe(0.6);
            expect(engine.calculateRiskPenalty('CRITICAL')).toBe(0.9);
        });
        it('should analyze risk profile correctly', () => {
            ...
            const riskProfile = engine.analyzeRiskProfile(measures);
        ...
        it('should generate risk mitigation strategies for high-risk portfolios', () => {
            ...
            const strategies = engine.generateRiskMitigationStrategies(riskProfile);
    ...
    describe('Recommendations', () => {
        it('should recommend diversification for concentrated portfolios', () => {
            ...
            const recommendations = engine.generateRecommendations(measures, {});
        ...
        it('should recommend risk mitigation for high-risk portfolios', () => {
            ...
            const recommendations = engine.generateRecommendations(measures, tuners);
    ...
    describe('Compliance Checking', () => {
        it('should check compliance against multiple frameworks', async () => {
            ...
            const complianceStatus = await engine.checkComplianceStatus(measures);

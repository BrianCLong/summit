...
exports.generatePlaybook = generatePlaybook;
exports.validateOperation = validateOperation;
/**
 * Generate comprehensive PsyOps playbook based on tuning parameters
 */
function generatePlaybook(tuners) {
 ...
 */
function getStrategicObjectives(intensity, doctrine) {
    ...
    if (intensity > 0.7) {
        baseObjectives.push('Disrupt adversary information operations', 'Create strategic narrative dominance', 'Influence decision-making processes');
    }
    if (doctrine === 'ThreeWarfares') {
        baseObjectives.push('Psychological warfare integration', 'Media warfare coordination', 'Legal warfare support');
    }
 ...
 */
function getNarrativeThemes(doctrine) {
    const baseThemes = [
 ...
 */
function generateCampaigns(intensity) {
    const campaigns = [
        {
 ...
 */
function calculateResourceAllocation(tuners) {
 ...
 */
function getTechniques(intensity) {
 ...
 */
function getDeliveryMechanisms() {
    return [
 ...
 */
function generateDecisionMatrix(intensity) {
    const matrix = [
        {
 ...
 */
function validateOperation(operation, constraints) {
    const issues = [];
    const warnings = [];
    // Ethical validation
    ...
    if (operation.unattributabilityRequirement > 0.8 &&
        operation.techniques.some((t) => t.attribution > 0.5)) {
...
function calculateRiskAssessment(operation) {
    const intensity = operation.intensity || 0.5;
    const attribution = operation.techniques?.reduce((avg, t) => avg + t.attribution, 0) / (operation.techniques?.length || 1) || 0.3;
    const overallRisk = intensity > 0.8 || attribution > 0.7
        ? 'HIGH'

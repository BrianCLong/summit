...
def test_gate_runner_no_args():
    ...
    if "KNOWN_GATES=gate.latedata" not in result.stdout:
        raise AssertionError("Output missing known gates")
...
def test_gate_runner_unknown_gate():
    ...
    if result.returncode != 2:
        raise AssertionError(f"Expected 2, got {result.returncode}")
    if "ERROR: unknown gate" not in result.stdout:
        raise AssertionError("Output missing error message")
...
def test_gate_runner_deny_by_default():
    ...
    if result.returncode != 3:
        raise AssertionError(f"Expected 3, got {result.returncode}")
    if "DENY_BY_DEFAULT: gate.latedata" not in result.stdout:
        raise AssertionError("Output missing deny message")

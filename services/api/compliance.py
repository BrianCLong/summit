...
def foia_dsar_report(
    ...
    audit(
        actor="admin",
        action="foia_dsar_report",
        case_id=None,
        details={"entity_id": entity_id, "tenant_id": x_tenant_id},
    )
    return {
        "status": "report generation initiated",
        "entity_id": entity_id,
        "tenant_id": x_tenant_id,
        "report_url": "/reports/foia_dsar_report_placeholder.pdf",
    }
...
def purge_case(
    ...
    with driver.session() as s:
        s.run(
            "MATCH (c:Case {id:$case_id, tenant_id:$tenant_id}) DETACH DELETE c",
            case_id=case_id,
            tenant_id=x_tenant_id,
        )

...
def get_account(acc_id: str, x_jurisdiction: str | None = Header(default=None, alias="X-Jurisdiction")):
    with driver.session() as s:
        ...
        if x_jurisdiction:
            ...
            """, jurisdiction_code=x_jurisdiction).data()
...
def autocreate(case: AutoCase, x_tenant_id: str | None = Header(default="default_tenant", alias="X-Tenant-ID")):
    with driver.session() as s:
        rec = s.run("""
        WITH $seeds AS ids
        ...
        """, {"seeds": case.seed_accounts, "reason": case.reason, "tenant_id": x_tenant_id}).single()
    # emit socket/kafka as needed (omitted for brevity)
    return {"id": rec["id"], "reason": rec["reason"], "created": rec["created"].isoformat()}

@app.get("/cases/{case_id}")
def get_case(case_id: str, x_tenant_id: str | None = Header(default="default_tenant", alias="X-Tenant-ID")):
    with driver.session() as s:
        rec = s.run("""
        ...
        """, id=case_id, tenant_id=x_tenant_id).single()

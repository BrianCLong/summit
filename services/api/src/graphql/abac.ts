export async function opaDecision(input: any){
  const url = (process.env.OPA_URL||'http://opa:8181').replace(/\/$/,'') + '/v1/data/intelgraph/authz';
  const res = await fetch(url, { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ input }) });
  if(!res.ok) return { allow:false, fields:[], reason:'opa_error_'+res.status };
...
export function filterFields<T=any>(value: T, fields: string[]): T{
  ...
  if(Array.isArray(value as any)) return (value as any[]).map(v=>filterFields(v, fields)) as any;
  ...
  for(const f of allow){
    if(f.includes(':')){
      ...
      if(v[k] && typeof v[k]==='object'){
        pick[k] = pick[k] || {};
        if(v[k][sub] !== undefined){ pick[k][sub] = v[k][sub]; }
      }
...
export function withABAC<TArgs=any>(resolver: Function, action='read'){
  return async (parent: any, args: TArgs, ctx: any, info: any) => {
    ...
    const decision = await opaDecision({ user: { role }, action, resource: { sensitivity } });
    if(!decision.allow){

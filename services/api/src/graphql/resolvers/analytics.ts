    ...
    communityDetection: async (
      ...
      try {
        const res =
          entityIds && entityIds.length
            ? await session.run(
                'MATCH (n) WHERE n.id IN $ids WITH n.community AS c, collect(n.id) AS members RETURN c AS id, size(members) AS size, members',
                { ids: entityIds },
              )
            : await session.run(
                'MATCH (n) WITH n.community AS c, collect(n.id) AS members RETURN c AS id, size(members) AS size, members ORDER BY size DESC LIMIT 20',
              );
        return res.records.map((r) => ({
          id: r.get('id'),
          size: r.get('size'),

...
copilotRouter.post('/cookbook', (req,res)=>{
  ...
  const items = [
    { id:'centrality', title:'Centrality for IDs', query:'query Centrality($ids:[ID!]){ centralityAnalysis(entityIds:$ids){ id centrality{ eigenvector betweenness } }}' },
    { id:'communities', title:'Communities overview', query:'{ communityDetection(entityIds:[], algorithm:"louvain") }' },
  ];

...
triageRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      if (process.env.USE_DB === 'true') {
        await postgresPool.insert('triage_suggestions', {
          id: s.id,
          type: s.type,
          data: s.data,
          status: s.status,
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (process.env.USE_DB === 'true') {
        ...
        const row = await postgresPool.findOne<any>('triage_suggestions', {
          id,
        });
        if (row) s = row;
      } else if (s) s.status = 'approved';
      if (!s) return res.status(404).json({ ok: false });
      auditLog(req, 'triage.approve', { id });
      res.json({ ok: true, suggestion: s });
    } catch (e) {
      res.status(500).json({ ok: false, error: String(e) });
    }
...
triageRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!s) return res.status(404).json({ ok: false });
      auditLog(req, 'triage.materialize', { id });
      res.json({ ok: true, suggestion: s });
    } catch (e) {
      res.status(500).json({ ok: false, error: String(e) });
    }

...
exports.ingestRouter.get('/connectors', (_req, res) => {
    res.json({
        items: [
...
// Check job progress
exports.ingestRouter.get('/progress/:id', (req, res) => {
...
// Cancel job (scaffold)
exports.ingestRouter.post('/cancel/:id', (req, res) => {
    const id = req.params.id;
    const job = jobs.get(id);
    if (!job)
        return res.status(404).json({ ok: false, error: 'not_found' });
...
exports.ingestRouter.get('/schema/:id', (req, res) => {
    const id = req.params.id;
    res.json(schemas[id] || { required: [], properties: {} });
});
exports.ingestRouter.post('/dry-run/:id', (req, res) => {
    const id = req.params.id;
    const schema = schemas[id];
    if (!schema)
        return res.json({ ok: true, warnings: ['no schema registered'] });
    const cfg = req.body || {};
    const missing = (schema.required || []).filter((k) => cfg[k] === undefined);
    if (missing.length)
        return res
            .status(400)

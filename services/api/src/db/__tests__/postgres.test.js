...
describe('PostgreSQLConnection', () => {
    ...
    beforeEach(() => {
        ...
        connection = new postgres_js_1.PostgreSQLConnection();
        jest.clearAllMocks();
    });
    describe('connect', () => {
        ...
        it('should skip connection if already connected', async () => {
            mockClient.query.mockResolvedValue({ rows: [], rowCount: 0 });
            await connection.connect();
            const firstPoolInstance = pg_1.Pool;
            await connection.connect();
    ...
    describe('query', () => {
        ...
        it('should log slow queries', async () => {
            ...
            mockPool.query.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(mockResult), 1100)));
        ...
        it('should throw error when pool not initialized', async () => {
            const uninitializedConnection = new postgres_js_1.PostgreSQLConnection();
            await expect(uninitializedConnection.query('SELECT 1')).rejects.toThrow('PostgreSQL pool not initialized');
        });
        ...
        it('should use provided client if available', async () => {
            ...
            const result = await connection.query('SELECT * FROM users', [], mockClient);
    ...
    describe('getClient', () => {
        ...
        it('should throw error when pool not initialized', async () => {
            const uninitializedConnection = new postgres_js_1.PostgreSQLConnection();
            await expect(uninitializedConnection.getClient()).rejects.toThrow('PostgreSQL pool not initialized');
        });
    });
    ...
    describe('findOne', () => {
        ...
        it('should find one record', async () => {
            ...
            expect(mockPool.query).toHaveBeenCalledWith('SELECT * FROM users WHERE id = $1 LIMIT 1', [1]);
        });
        ...
        it('should handle multiple conditions', async () => {
            ...
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('WHERE email = $1 AND is_active = $2'), ['test@example.com', true]);
        });
    });
    describe('findMany', () => {
        beforeEach(async () => {
    ...
    describe('insert', () => {
        ...
        it('should insert record and return result', async () => {
            ...
            expect(result).toEqual(mockRow);
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO users'), ['John', 'john@example.com']);
        });
        it('should support custom RETURNING clause', async () => {
            ...
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('RETURNING id'), ['John']);
        });
    });
    describe('update', () => {
        beforeEach(async () => {
        ...
        it('should update record and return result', async () => {
            ...
            mockPool.query.mockResolvedValue({ rows: [mockRow], rowCount: 1 });
            const result = await connection.update('users', { name: 'Jane' }, { id: 1 });
            expect(result).toEqual(mockRow);
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('UPDATE users'), ['Jane', 1]);
        });
        it('should return null when no record updated', async () => {
            mockPool.query.mockResolvedValue({ rows: [], rowCount: 0 });
            const result = await connection.update('users', { name: 'Jane' }, { id: 999 });
            expect(result).toBeNull();
        });
        it('should handle multiple update fields and conditions', async () => {
            ...
            await connection.update('users', { name: 'Jane', email: 'jane@example.com' }, { id: 1, tenant_id: 'tenant1' });
            expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('SET name = $1, email = $2'), ['Jane', 'jane@example.com', 1, 'tenant1']);
        });
    });
    describe('delete', () => {
        beforeEach(async () => {

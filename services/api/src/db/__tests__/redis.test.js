...
describe('RedisConnection', () => {
    ...
    beforeEach(() => {
        ...
        connection = new redis_js_1.RedisConnection();
        jest.clearAllMocks();
    });
    describe('connect', () => {
        ...
        it('should create separate clients for pub/sub', async () => {
            await connection.connect();
            expect(ioredis_1.default).toHaveBeenCalledTimes(3);
        ...
        it('should skip connection if already connected', async () => {
            await connection.connect();
            const firstCallCount = ioredis_1.default.mock.calls.length;
            await connection.connect();
            // Should not create new Redis instances
            expect(ioredis_1.default.mock.calls.length).toBe(firstCallCount);
        });
    ...
    describe('get', () => {
        ...
        it('should throw error when client not initialized', async () => {
            const uninitializedConnection = new redis_js_1.RedisConnection();
            await expect(uninitializedConnection.get('key')).rejects.toThrow('Redis client not initialized');
        });
    ...
    describe('set', () => {
        ...
        it('should set value without TTL', async () => {
            ...
            expect(mockClient.set).toHaveBeenCalledWith('test-key', JSON.stringify({ data: 'value' }));
        });
        it('should set value with TTL', async () => {
            mockClient.setex.mockResolvedValue('OK');
            const result = await connection.set('test-key', { data: 'value' }, 3600);
            expect(result).toBe(true);
            expect(mockClient.setex).toHaveBeenCalledWith('test-key', 3600, JSON.stringify({ data: 'value' }));
        });
        it('should throw error when client not initialized', async () => {
            const uninitializedConnection = new redis_js_1.RedisConnection();
            await expect(uninitializedConnection.set('key', 'value')).rejects.toThrow('Redis client not initialized');
        });
    ...
    describe('pub/sub', () => {
        ...
        it('should publish message to channel', async () => {
            ...
            expect(result).toBe(true);
            expect(mockPublisher.publish).toHaveBeenCalledWith('test-channel', JSON.stringify({ message: 'hello' }));
        });

...
describe('Rate Limit Configuration', () => {
    ...
    describe('Redis Client Initialization', () => {
        ...
        it('should configure retry strategy with exponential backoff', () => {
            ...
            const config = ioredis_1.default.mock.calls[0][0];
    ...
    describe('Rate Limiter Creation', () => {
        it('should create rate limiter with loaded configuration', () => {
            ...
            rate_limiter_1.loadConfigFromEnv.mockReturnValue(mockConfig);
            jest.isolateModules(() => {
                const module = require('../rateLimit.new.js');
                expect(rate_limiter_1.createRateLimiter).toHaveBeenCalled();
                expect(rate_limiter_1.createRateLimiter).toHaveBeenCalledWith(expect.anything(), mockConfig);
            });
        });
    });
    describe('Alert Configuration', () => {
        it('should configure alert handler for rate limit violations', () => {
            rate_limiter_1.alerter.onAlert.mockImplementation(() => { });
            jest.isolateModules(() => {
                require('../rateLimit.new.js');
            });
            expect(rate_limiter_1.alerter.onAlert).toHaveBeenCalled();
        });
        it('should log rate limit violations', async () => {
            let alertHandler;
            rate_limiter_1.alerter.onAlert.mockImplementation((handler) => {
    ...
    describe('Middleware Configuration', () => {
        it('should create rate limit middleware with headers enabled', () => {
            rate_limiter_1.createRateLimitMiddleware.mockReturnValue(jest.fn());
            jest.isolateModules(() => {
                require('../rateLimit.new.js');
            });
            expect(rate_limiter_1.createRateLimitMiddleware).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
                headers: true,
            }));
        });
        it('should skip rate limiting for health check endpoints', () => {
            let skipFunction;
            rate_limiter_1.createRateLimitMiddleware.mockImplementation((limiter, options) => {
        ...
        it('should skip rate limiting for internal service calls', () => {
            ...
            expect(skipFunction({ path: '/api/data', isInternalCall: true })).resolves.toBe(true);
        });
    });
    describe('GraphQL Plugin Configuration', () => {
        it('should create GraphQL rate limit plugin with default complexity', () => {
            ...
            expect(rate_limiter_1.createGraphQLRateLimitPlugin).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
                maxComplexity: 1000,
            }));
        });

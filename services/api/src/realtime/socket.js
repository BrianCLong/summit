...
function createSocketIOServer(io) {
    ...
    io.on('connection', async (socket) => {
        ...
        socket.on('entity:lock', async (entityId) => {
            try {
                ...
                const lockAcquired = await redis_js_1.redisClient.set(lockKey, lockValue, 300);
                if (lockAcquired) {
                    ...
                    const existingLock = await redis_js_1.redisClient.get(lockKey);
        ...
        socket.on('entity:unlock', async (entityId) => {
            try {
                const lockKey = `entity:lock:${entityId}`;
...
async function verifyToken(token) {
    try {
        ...
        const user = await postgres_js_1.postgresPool.findOne('users', {
...
async function updatePresence(socket, status, investigationId) {
    const presenceData = {
    ...
    if (status === 'online') {
        await redis_js_1.redisClient.set(presenceKey, presenceData, 300); // 5 minute TTL
    }
    else {
        await redis_js_1.redisClient.del(presenceKey);
    }
}
async function verifyInvestigationAccess(userId, investigationId, tenantId) {
    try {
        const investigation = await postgres_js_1.postgresPool.findOne('investigations', {
            id: investigationId,
            tenant_id: tenantId,
        });
        if (!investigation) {
        ...
        const user = await postgres_js_1.postgresPool.findOne('users', { id: userId });
...
async function getInvestigationState(investigationId) {
    try {
        ...
        const entities = await postgres_js_1.postgresPool.query('SELECT e.* FROM investigation_entities ie JOIN entity_metadata e ON ie.entity_id = e.id WHERE ie.investigation_id = $1', [investigationId]);
        const relationships = await postgres_js_1.postgresPool.query('SELECT r.* FROM investigation_relationships ir JOIN relationship_metadata r ON ir.relationship_id = r.id WHERE ir.investigation_id = $1', [investigationId]);
        return {
            investigation,
...
async function cacheGraphUpdate(data, userId, tenantId) {
    try {
        ...
        await redis_js_1.redisClient.client?.lpush(updateKey, JSON.stringify(update));
        await redis_js_1.redisClient.client?.ltrim(updateKey, 0, 99);
        await redis_js_1.redisClient.expire(updateKey, 3600); // 1 hour TTL
    }
    catch (error) {
        logger_js_1.logger.error({
...
async function cleanupStaleData() {
    try {
        ...
        logger_js_1.logger.debug('Performing periodic realtime data cleanup');
    }
    catch (error) {
        logger_js_1.logger.error({

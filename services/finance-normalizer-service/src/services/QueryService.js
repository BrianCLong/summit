...
class QueryService {
     ...
     */
    async queryFlows(query, tenantId) {
        ...
        if (query.entityType === 'PARTY') {
            conditions.push(`(t.originator_id = ANY($${paramIndex}) OR t.beneficiary_id = ANY($${paramIndex}))`);
        }
        else {
            conditions.push(`(t.source_account_id = ANY($${paramIndex}) OR t.destination_account_id = ANY($${paramIndex}))`);
        }
        ...
        // Get total count
        const countResult = await db_js_1.db.query(`SELECT COUNT(*) as count FROM finance_transactions t WHERE ${whereClause}`, params);
        const totalCount = parseInt(countResult.rows[0].count, 10);
        // Handle aggregation
        if (query.aggregation !== 'NONE') {
            return this.queryAggregatedFlows(query, whereClause, params, paramIndex, totalCount, startTime);
        }
        // Sort order
        const sortColumn = {
            DATE: 't.value_date',
        ...
        const result = await db_js_1.db.query(`SELECT t.* FROM finance_transactions t
       WHERE ${whereClause}
     ...
     */
    async queryAggregatedFlows(query, whereClause, params, paramIndex, totalCount, startTime) {
        const dateFormat = {
      ...
      LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, [...params, query.limit, query.offset]);
        const flows = result.rows.map((row) => ({
            ...
            grossFlow: (0, finance_normalizer_types_1.createMonetaryAmount)(parseInt(row.gross_flow, 10) / 100, row.currency || 'USD'),
            netFlow: (0, finance_normalizer_types_1.createMonetaryAmount)(parseInt(row.net_flow, 10) / 100, row.currency || 'USD'),
            ...
            maxTransaction: (0, finance_normalizer_types_1.createMonetaryAmount)(parseInt(row.max_transaction, 10) / 100, row.currency || 'USD'),
            minTransaction: (0, finance_normalizer_types_1.createMonetaryAmount)(parseInt(row.min_transaction, 10) / 100, row.currency || 'USD'),
            averageTransactionSize: (0, finance_normalizer_types_1.createMonetaryAmount)(parseFloat(row.avg_transaction) / 100, row.currency || 'USD'),
            byTransactionType: [],
    ...
    async getTransaction(id, tenantId) {
        const result = await db_js_1.db.query(`SELECT * FROM finance_transactions WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
     ...
     */
    async listTransactions(tenantId, filters, limit = 50, offset = 0) {
        const conditions = ['tenant_id = $1'];
        const params = [tenantId];
        let paramIndex = 2;
        if (filters.accountId) {
            conditions.push(`(source_account_id = $${paramIndex} OR destination_account_id = $${paramIndex})`);
            params.push(filters.accountId);
            paramIndex++;
        }
        if (filters.partyId) {
            conditions.push(`(originator_id = $${paramIndex} OR beneficiary_id = $${paramIndex})`);
        ...
        const [txnsResult, countResult] = await Promise.all([
            db_js_1.db.query(`SELECT * FROM finance_transactions
         WHERE ${whereClause}
         ORDER BY value_date DESC
         LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, [...params, limit, offset]),
            db_js_1.db.query(`SELECT COUNT(*) as count FROM finance_transactions WHERE ${whereClause}`, params),
        ]);
        return {
     ...
     * Get party by ID
     */
    async getParty(id, tenantId) {
        const result = await db_js_1.db.query(`SELECT * FROM finance_parties WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
        if (result.rows.length === 0) {
            return null;
        }
     ...
     */
    async searchParties(tenantId, searchTerm, limit = 20) {
        const result = await db_js_1.db.query(`SELECT * FROM finance_parties
       WHERE tenant_id = $1
         AND (canonical_name ILIKE $2 OR original_name ILIKE $2 OR $3 = ANY(aliases))
       ORDER BY canonical_name
       LIMIT $4`, [tenantId, `%${searchTerm}%`, searchTerm, limit]);
        return result.rows.map(this.mapPartyRow);
    }
    /**
     * Get account by ID
     */
    async getAccount(id, tenantId) {
        const result = await db_js_1.db.query(`SELECT * FROM finance_accounts WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
        if (result.rows.length === 0) {
            return null;
        }
     ...
     * List accounts
     */
    async listAccounts(tenantId, filters, limit = 50, offset = 0) {
        const conditions = ['tenant_id = $1'];
        const params = [tenantId];
        let paramIndex = 2;
        if (filters.ownerId) {
        ...
        const [accountsResult, countResult] = await Promise.all([
            db_js_1.db.query(`SELECT * FROM finance_accounts
         WHERE ${whereClause}
         ORDER BY name
         LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, [...params, limit, offset]),
            db_js_1.db.query(`SELECT COUNT(*) as count FROM finance_accounts WHERE ${whereClause}`, params),
        ]);
        return {
     ...
     */
    async getPatterns(tenantId, filters, limit = 20, offset = 0) {
        const conditions = ['tenant_id = $1'];
        const params = [tenantId];
        let paramIndex = 2;
        if (filters.type) {
        ...
        const [patternsResult, countResult] = await Promise.all([
            db_js_1.db.query(`SELECT * FROM finance_flow_patterns
         WHERE ${whereClause}
         ORDER BY created_at DESC
         LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`, [...params, limit, offset]),
            db_js_1.db.query(`SELECT COUNT(*) as count FROM finance_flow_patterns WHERE ${whereClause}`, params),
        ]);
        return {
    ...
    // Helper methods
    async calculateSummary(whereClause, params, tenantId) {
        const result = await db_js_1.db.query(`SELECT
      ...
      WHERE ${whereClause}`, params);
        ...
        return {
            totalGrossFlow: (0, finance_normalizer_types_1.createMonetaryAmount)(parseInt(row.gross_flow || '0', 10) / 100, currency),
            totalNetFlow: (0, finance_normalizer_types_1.createMonetaryAmount)(parseInt(row.net_flow || '0', 10) / 100, currency),
            ...
            averageTransactionSize: row.avg_transaction
                ? (0, finance_normalizer_types_1.createMonetaryAmount)(parseFloat(row.avg_transaction) / 100, currency)
                : undefined,
        };
    }
    calculatePeriodEnd(periodStart, granularity) {
    ...
    mapPatternRow(row) {
        return {
            ...
            totalValue: row.total_value || (0, finance_normalizer_types_1.createMonetaryAmount)(0, 'USD'),

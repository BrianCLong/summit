...
class NormalizationService {
     ...
     */
    async startImportJob(input, tenantId, userId) {
        ...
        // Insert job record
        await db_js_1.db.query(`INSERT INTO finance_import_jobs (
        ...
        if (!input.dryRun && input.source.type === 'inline') {
            this.processImportJob(job, sourceData, tenantId).catch((err) => {
                logger_js_1.logger.error('Import job failed', { jobId, error: err.message });
            });
        }
        ...
        await (0, audit_js_1.logAudit)(tenantId, userId, 'IMPORT_JOB_STARTED', 'ImportJob', jobId, { datasetRef: input.datasetRef, format: input.format, dryRun: input.dryRun });
        return job;
    }
    /**
     * Validate data without importing
     */
    async validateData(data, format, config) {
        return (0, index_js_1.parseData)(data, format, config);
    }
    /**
     * Process an import job
     */
    async processImportJob(job, data, tenantId) {
        const startTime = Date.now();
        try {
            // Update status to parsing
            await this.updateJobStatus(job.id, 'PARSING');
            // Parse the data
            const parseResult = await (0, index_js_1.parseData)(data, job.format, job.parserConfig);
            ...
            // Add parse errors
            job.errors.push(...parseResult.errors.map((e) => ({
                ...e,
                severity: 'ERROR',
            })));
            ...
            await this.updateJobProgress(job);
            logger_js_1.logger.info('Import job completed', {
                jobId: job.id,
            ...
            await this.updateJobProgress(job);
            logger_js_1.logger.error('Import job failed', {
                jobId: job.id,
     ...
     */
    async normalizeAndStore(record, tenantId) {
        const result = {};
        return db_js_1.db.transaction(async (client) => {
            // Store parties first
            if (record.parties && record.parties.length > 0) {
                result.partyIds = [];
                for (const party of record.parties) {
                    const partyId = party.id || (0, uuid_1.v4)();
                    await client.query(`INSERT INTO finance_parties (
            ...
            if (record.accounts && record.accounts.length > 0) {
                ...
                for (const account of record.accounts) {
                    const accountId = account.id || (0, uuid_1.v4)();
            ...
            if (record.transaction) {
                ...
                const txnId = txn.id || (0, uuid_1.v4)();
                await client.query(`INSERT INTO finance_transactions (
     ...
     */
    async getImportJob(jobId, tenantId) {
        const result = await db_js_1.db.query(`SELECT * FROM finance_import_jobs WHERE id = $1 AND tenant_id = $2`, [jobId, tenantId]);
     ...
     * List import jobs
     */
    async listImportJobs(tenantId, limit = 20, offset = 0) {
        const [jobsResult, countResult] = await Promise.all([
            db_js_1.db.query(`SELECT * FROM finance_import_jobs
         WHERE tenant_id = $1
         ORDER BY created_at DESC
         LIMIT $2 OFFSET $3`, [tenantId, limit, offset]),
            db_js_1.db.query(`SELECT COUNT(*) as count FROM finance_import_jobs WHERE tenant_id = $1`, [tenantId]),
        ]);
        return {
     ...
     */
    async normalizeTransaction(rawData, format, config, tenantId) {
        const dataStr = JSON.stringify(rawData);
        const parseResult = await (0, index_js_1.parseData)(dataStr, format, config);
        ...
        return {
            ...record.transaction,
            id: result.transactionId || record.transaction.id || (0, uuid_1.v4)(),
            tenantId,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
        };
    ...
    async updateJobStatus(jobId, status, extra) {
        ...
        const params = [jobId, status, new Date().toISOString()];
        if (extra?.totalRecords !== undefined) {
            updates.push(`total_records = $${params.length + 1}`);
            params.push(extra.totalRecords);
        }
        await db_js_1.db.query(`UPDATE finance_import_jobs SET ${updates.join(', ')} WHERE id = $1`, params);
    }
    async updateJobProgress(job) {
        await db_js_1.db.query(`UPDATE finance_import_jobs SET
        status = $2,
        total_records = $3,
        processed_records = $4,
        success_count = $5,

...
exports.inferPartyType = inferPartyType;
exports.inferTransactionType = inferTransactionType;
exports.generateReferenceNumber = generateReferenceNumber;
exports.detectDirection = detectDirection;
exports.cleanIBAN = cleanIBAN;
exports.cleanBIC = cleanBIC;
 ...
 */
function parseAmount(value, currency, config) {
    ...
    return {
        amount: (0, finance_normalizer_types_1.createMonetaryAmount)(finalValue, currency, decimals),
        original,
        signInferred: false,
    };
}
...
function parseDate(value, config) {
    ...
    if (dateFormat) {
        if (dateFormat === 'DD/MM/YYYY' || dateFormat === 'DD.MM.YYYY') {
            ...
            if (match) {
                parsed = new Date(parseInt(match[3], 10), parseInt(match[2], 10) - 1, parseInt(match[1], 10));
                timeInferred = true;
            }
        }
            ...
            const match = original.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
            if (match) {
                parsed = new Date(parseInt(match[3], 10), parseInt(match[1], 10) - 1, parseInt(match[2], 10));
                timeInferred = true;
            }
        }
 ...
 */
function inferPartyType(name, hints) {
 ...
 */
function inferTransactionType(description, amount) {
    const upper = description.toUpperCase();
    // Wire/transfer patterns
    if (upper.includes('WIRE') || upper.includes('SWIFT'))
        return 'WIRE';
    if (upper.includes('ACH') || upper.includes('DIRECT DEPOSIT'))
        return 'ACH';
    ...
    // Fee patterns
    if (upper.includes('FEE') || upper.includes('CHARGE'))
        return 'FEE';
    if (upper.includes('INTEREST'))
        return 'INTEREST';
    // Check patterns
    if (upper.includes('CHECK') || upper.includes('CHK') || upper.includes('CHEQUE'))
        return 'CHECK';
    // ATM/Cash
    ...
    // Payment
    if (upper.includes('PAYMENT') || upper.includes('PMT'))
        return 'PAYMENT';
    // Deposit
    if (upper.includes('DEPOSIT') || upper.includes('DEP'))
        return 'DEPOSIT';
    return 'OTHER';
}
/**
 ...
 */
function generateReferenceNumber(date, amount, counterparty, sequence) {
 ...
 */
function detectDirection(indicator, amount) {
    if (indicator) {

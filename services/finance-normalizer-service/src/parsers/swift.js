...
exports.swiftParser = exports.SWIFTParser = void 0;
 ...
 */
function parseSwiftDate(yymmdd) {
 ...
 */
function parseSwiftAmount(amount, currency) {
    // SWIFT uses comma as decimal separator
    const normalized = amount.replace(',', '.');
    return (0, utils_js_1.parseAmount)(normalized, currency, { decimalSeparator: '.' });
}
/**
 * Extract subfields from field 86 (structured information)
 */
function parseField86(content) {
    const result = {};
...
class SWIFTParser {
    ...
    detect(data) {
        ...
        return (str.includes('{1:') || // SWIFT header
    ...
    async parseMT103(data, config, startTime) {
        const records = [];
        const errors = [];
        const warnings = [];
        try {
            ...
            if (orderingMatch) {
                originatorId = (0, uuid_1.v4)();
                const accountNumber = orderingMatch[1];
                const nameOrBIC = orderingMatch[2];
                originatorParty = {
                    id: originatorId,
                    canonicalName: (0, utils_js_1.normalizePartyName)(nameOrBIC.split('\n')[0]),
                    originalName: nameOrBIC,
                    type: orderingMatchA ? 'FINANCIAL_INSTITUTION' : (0, utils_js_1.inferPartyType)(nameOrBIC),
                    identifiers: [],
                    provenance: {
                ...
                if (accountNumber) {
                    const cleanedIBAN = (0, utils_js_1.cleanIBAN)(accountNumber);
                    if (cleanedIBAN) {
                ...
                if (orderingMatchA) {
                    const cleanedBIC = (0, utils_js_1.cleanBIC)(nameOrBIC);
                    if (cleanedBIC) {
                        originatorParty.identifiers.push({
                            type: 'BIC',
                            value: cleanedBIC,
            ...
            if (benefMatch) {
                ...
                beneficiaryParty = {
                    ...
                    canonicalName: (0, utils_js_1.normalizePartyName)(nameOrBIC.split('\n')[0]),
                    originalName: nameOrBIC,
                    type: beneficiaryMatchA ? 'FINANCIAL_INSTITUTION' : (0, utils_js_1.inferPartyType)(nameOrBIC),
                    identifiers: [],
                    provenance: {
                        sourceSystem: 'swift-mt103',
                        importedAt: new Date().toISOString(),
            ...
            const transaction = {
                id: (0, uuid_1.v4)(),
                referenceNumber: refMatch ? refMatch[1] : (0, uuid_1.v4)(),
                type: 'WIRE',
                status: 'COMPLETED',
                direction: 'DEBIT',
            ...
            const parties = [];
            if (originatorParty)
                parties.push(originatorParty);
            if (beneficiaryParty)
                parties.push(beneficiaryParty);
            records.push({
                transaction,
                parties,
                warnings,
    ...
    async parseMT940(data, config, startTime, isInterim) {
        const records = [];
        const errors = [];
        ...
        while ((txnMatch = transactionPattern.exec(data)) !== null) {
            ...
            const warnings = [];
            try {
                const [fullMatch, valueDate, entryDate, dcIndicator, fundsCode, amount, typeCode, customerRef, bankRef,] = txnMatch;
                // Determine direction
                ...
                if (field86Data.NAME || field86Data.BENM || field86Data.ORDP) {
                    ...
                    parties.push({
                        id: partyId,
                        canonicalName: (0, utils_js_1.normalizePartyName)(partyName),
                        originalName: partyName,
                        type: (0, utils_js_1.inferPartyType)(partyName),
                        provenance: {
                            sourceSystem: 'swift-mt940',
                            importedAt: new Date().toISOString(),
                            chain: [],
        ...
        return this.createResult(records, errors, records.length + errors.length, format, config, startTime);
    }
    parseGeneric(data, config, startTime) {
    ...
    createResult(records, errors, totalRecords, format, config, startTime) {
        return {
            records,
            errors,
            totalRecords,
...
exports.swiftParser = new SWIFTParser();

...
exports.partyRoutes = (0, express_1.Router)();
/**
 * GET /api/v1/parties/search
 * Search parties by name
 */
exports.partyRoutes.get('/search', async (req, res, next) => {
    try {
        ...
        const parties = await QueryService_js_1.queryService.searchParties(req.tenantId, q, maxLimit);
        res.json({
            parties,
 ...
 * Get party by ID
 */
exports.partyRoutes.get('/:id', async (req, res, next) => {
    try {
        const party = await QueryService_js_1.queryService.getParty(req.params.id, req.tenantId);
        if (!party) {
            throw new errorHandler_js_1.NotFoundError('Party', req.params.id);
        }
        res.json(party);
    }
    catch (error) {
 ...
 */
exports.partyRoutes.get('/:id/transactions', async (req, res, next) => {
    try {
        const party = await QueryService_js_1.queryService.getParty(req.params.id, req.tenantId);
        if (!party) {
            throw new errorHandler_js_1.NotFoundError('Party', req.params.id);
        ...
        const result = await QueryService_js_1.queryService.listTransactions(req.tenantId, { partyId: req.params.id }, limit, offset);
        res.json({
            party: {
                id: party.id,
 ...
 */
exports.partyRoutes.get('/:id/counterparties', async (req, res, next) => {
    try {
        const party = await QueryService_js_1.queryService.getParty(req.params.id, req.tenantId);
        if (!party) {
            throw new errorHandler_js_1.NotFoundError('Party', req.params.id);
        ...
        const result = await QueryService_js_1.queryService.listTransactions(req.tenantId, { partyId: req.params.id }, 1000, 0);
        ...
        for (const id of counterpartyIds) {
            const cp = await QueryService_js_1.queryService.getParty(id, req.tenantId);
            if (cp) {
                // Count transactions with this counterparty
                const txnCount = result.transactions.filter((t) => t.originatorId === id || t.beneficiaryId === id).length;
                counterparties.push({
                    ...cp,

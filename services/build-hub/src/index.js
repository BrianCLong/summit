...
const distillation = new distillation_1.BuildDistillationEngine();
...
wss.on('connection', (ws, req) => {
    const clientId = (0, crypto_1.createHash)('sha256')
...
function broadcast(payload) {
    ...
    try {
        ...
        console.log(`🧪 Distilled student score for PR #${payload.pr}: ${signal.studentScore.toFixed(2)} (branch ${signal.branch})`);
...
function pushDistillationSignal(signal) {
...
app.post('/api/test-event', (req, res) => {
    try {
        const buildEvent = {
            ...
            sha: (0, crypto_1.createHash)('sha256')
                .update(Math.random().toString())
                .digest('hex')
                .slice(0, 40),
            status: ['success', 'pending', 'failed', 'running'][Math.floor(Math.random() * 4)],
            timestamp: new Date().toISOString(),
            author: 'test-user',
            title: 'Test PR for Maestro Build HUD',
            signed: Math.random() > 0.5,
            policy: ['pass', 'warn', 'fail'][Math.floor(Math.random() * 3)],
            ...
            tests: {
                unit: ['pass', 'fail', 'pending'][Math.floor(Math.random() * 3)],
                e2e: ['pass', 'fail', 'pending'][Math.floor(Math.random() * 3)],
                security: ['pass', 'fail', 'pending'][Math.floor(Math.random() * 3)],
            },
        };
        broadcast(buildEvent);
...
// Utility functions
function mapGitHubStatus(conclusion, status) {
...
function inferTestResults(run) {
    const name = run.name?.toLowerCase() || '';
    return {
        unit: name.includes('test') && run.conclusion === 'success'
            ? 'pass'
            : name.includes('test') && run.conclusion === 'failure'
                ? 'fail'
                : 'pending',
        e2e: name.includes('e2e') && run.conclusion === 'success'
            ? 'pass'

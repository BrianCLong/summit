...
exports.BuildDistillationEngine = void 0;
...
class BuildDistillationEngine extends events_1.EventEmitter {
    options;
    teacherLibrary = new Map();
    ...
    getSnapshot() {
        ...
        const aggregateReliability = branches.reduce((acc, profile) => acc + profile.emaReliability, 0) /
            (branches.length || 1);
        return {
    ...
    computeVelocityScore(prWindow) {
        if (!prWindow.syntheticLag) {
            return 0.72;
        ...
        if (minutes <= fastThreshold) {
            return 1;
        }
        if (minutes >= slowThreshold) {
            return 0.2;
        }
        return clamp(1 - (minutes - fastThreshold) / (slowThreshold - fastThreshold));
    }
    computeStabilityScore(prWindow, status) {
        if (status === 'failed') {
    ...
    updateBranchProfile(branch, features, timestamp, status) {
    ...
    composeStudentScore(features, branch, teacherScores, prWindow) {
        ...
        const branchVelocity = branchProfile
            ...
            : features.velocity;
        const dropoutPenalty = 1 -
            clamp((this.options.teacherDropout * (prWindow.failureStreak + 1)) / 4, 0, 0.35);
    ...
    generateRecommendations(features, branch, studentScore) {
    ...
    applyTemperature(score, temperature) {
...
exports.BuildDistillationEngine = BuildDistillationEngine;

import { holtWinters } from "./forecast"; import { Pool } from "pg"; const pg = new Pool({ connectionString: process.env.DATABASE_URL });
export async function computeScaleHints(){
  ...
  const series = rows.map(r=>Number(r.q)); const next = holtWinters(series);

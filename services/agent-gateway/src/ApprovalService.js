...
class ApprovalService {
    ...
    async createApproval(request) {
        ...
        const result = await this.db.query(`INSERT INTO agent_approvals (
    ...
    async processDecision(decision) {
        ...
        // Update approval
        const result = await this.db.query(`UPDATE agent_approvals
       SET status = $1,
           decision_made_by = $2,
       ...
       WHERE id = $4
       RETURNING *`, [decision.decision, decision.userId, decision.reason, decision.approvalId]);
        ...
        if (decision.decision === 'approved' && approval.actionId) {
         ...
         WHERE id = $2`, [decision.userId, approval.actionId]);
        }
    ...
    async getApproval(approvalId) {
        const result = await this.db.query('SELECT * FROM agent_approvals WHERE id = $1', [approvalId]);
    ...
    async listPendingApprovals(userId) {
        const result = await this.db.query(`SELECT * FROM agent_approvals
       WHERE status = 'pending'
       AND expires_at > CURRENT_TIMESTAMP
       AND assigned_to @> $1
       ORDER BY created_at ASC`, [JSON.stringify([userId])]);
        return result.rows.map(this.mapRowToApproval);
    }
    /**
     * List all approvals for an agent
     */
    async listAgentApprovals(agentId, filters) {
     ...
     */
    computeRiskAssessment(approval) {
        const factors = [];
        const impact = [];

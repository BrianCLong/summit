...
class MonitoringService {
    ...
    constructor(db, quotaManager, thresholds) {
        this.db = db;
    ...
    async getAllAgentsHealth() {
        const agents = await this.db.query(`SELECT id, name, status FROM agents WHERE deleted_at IS NULL AND status = 'active'`);
        const healthStatuses = await Promise.all(agents.rows.map((agent) => this.getAgentHealth(agent.id)));
    ...
    async getAgentHealth(agentId) {
        // Get agent info
        const agentResult = await this.db.query('SELECT id, name, status FROM agents WHERE id = $1', [agentId]);
        if (agentResult.rows.length === 0) {
            throw new Error(`Agent not found: ${agentId}`);
        }
        const agent = agentResult.rows[0];
        // Get last 24 hours metrics
        const metricsResult = await this.db.query(`SELECT * FROM agent_metrics
       WHERE agent_id = $1
       ...
       LIMIT 1`, [agentId]);
        ...
        const last24Hours = metrics
            ? {
                ...
                successRate: metrics.runs_total > 0
                    ? (metrics.runs_successful / metrics.runs_total) * 100
                    : 100,
                errorRate: metrics.runs_total > 0 ? (metrics.runs_failed / metrics.runs_total) * 100 : 0,
        ...
        // Get last run time
        const lastRunResult = await this.db.query(`SELECT started_at FROM agent_runs
       WHERE agent_id = $1
     ...
     */
    generateAlerts(agent, metrics, quotas, last24Hours) {
        const alerts = [];
        // Check error rate
     ...
     */
    determineHealth(alerts, last24Hours) {
     ...
     */
    async getMetrics(agentId, startDate, endDate) {
        const result = await this.db.query(`SELECT * FROM agent_metrics
       WHERE agent_id = $1
       AND metric_date >= $2
       AND metric_date <= $3
       AND metric_hour IS NULL
       ORDER BY metric_date DESC`, [agentId, startDate, endDate]);
        return result.rows.map(this.mapRowToMetrics);
    }
    /**
     * Get hourly metrics for a specific date
     */
    async getHourlyMetrics(agentId, date) {
        const result = await this.db.query(`SELECT * FROM agent_metrics
       WHERE agent_id = $1
       AND metric_date = $2
       AND metric_hour IS NOT NULL
       ORDER BY metric_hour ASC`, [agentId, date]);
        return result.rows.map(this.mapRowToMetrics);
    }
    /**
     * Get aggregate metrics across all agents
     */
    async getAggregateMetrics(startDate, endDate) {
       ...
       WHERE metric_date >= $1
       AND metric_date <= $2
       AND metric_hour IS NULL`, [startDate, endDate]);
     ...
     */
    async getTopAgents(metric, limit = 10) {
        const metricColumn = metric === 'runs'
            ? 'runs_total'
            : metric === 'errors'
                ? 'runs_failed'
                : 'policy_violations';
     ...
     */
    async getRecentHighRiskActions(limit = 50) {
        const result = await this.db.query(`SELECT
         a.id as agent_id,
         a.name as agent_name,
         aa.action_type,

import { CanonicalTerm, LocaleCanonMap, RegulatorNote, Variant } from '../types';
...
export const buildLocaleCanon = (): LocaleCanonMap => {
  const localeEntries = Object.entries(LOCALE_TERMS).map(([locale, terms]) => {
    const records: CanonicalTerm[] = Object.entries(terms).map(([tag, seed]) => {
      ...
      return {
        ...
        locale,
        vertical,
        variants: sortBy(dedupeVariants(seed.canonical, seed.variants), variant => normalize(variant.value)),
    ...
    return [
      locale,
      sortBy(records, record => record.tag)
    ] as const;
  });

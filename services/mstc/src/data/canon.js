...
exports.SUPPORTED_LOCALES = exports.MSTC_LOCALE_CANON = exports.buildLocaleCanon = void 0;
const utils_1 = require("../utils");
const TERM_VERTICALS = {
...
const dedupeVariants = (canonical, variants) => {
    const canonicalKey = (0, utils_1.normalize)(canonical);
    const seen = new Set();
    return variants.filter((variant) => {
        const key = (0, utils_1.normalize)(variant.value);
        if (key === canonicalKey) {
            return false;
        }
        if (seen.has(key)) {
...
const buildLocaleCanon = () => {
    const localeEntries = Object.entries(LOCALE_TERMS).map(([locale, terms]) => {
        const records = Object.entries(terms).map(([tag, seed]) => {
            ...
            return {
                ...
                locale,
                vertical,
                variants: (0, utils_1.sortBy)(dedupeVariants(seed.canonical, seed.variants), (variant) => (0, utils_1.normalize)(variant.value)),
                regulatorNotes: regulator ? [regulator] : [],
                confidence: seed.confidence,
            };
        });
        return [locale, (0, utils_1.sortBy)(records, (record) => record.tag)];
    });
    return Object.fromEntries((0, utils_1.sortBy)(localeEntries, ([locale]) => locale));
};
exports.buildLocaleCanon = buildLocaleCanon;
exports.MSTC_LOCALE_CANON = (0, exports.buildLocaleCanon)();
exports.SUPPORTED_LOCALES = Object.keys(exports.MSTC_LOCALE_CANON);

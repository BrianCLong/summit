...
const consentReconciler_1 = require("../src/consentReconciler");
describe('ConsentReconciler', () => {
    ...
    it('reconciles snapshots, increments version, and records drift', async () => {
        ...
        const reconciler = new consentReconciler_1.ConsentReconciler(api, {
            actor: 'unit-test',
        ...
        expect(api.upsertConsent).toHaveBeenCalledWith(expect.objectContaining({
            consentId: 'consent-1',
            version: 4,
        ...
        const auditEntries = api.recordAuditTrail.mock.calls.map((call) => call[0]);
        ...
        expect(api.publishDrift).toHaveBeenCalledWith('consent-1', result.driftFindings);
    ...
    it('surfaces GDPR and CCPA compliance issues when required fields are missing', async () => {
        ...
        const complianceEntry = api.recordAuditTrail.mock.calls
            .map((call) => call[0])
...
describe('HttpConsentApiClient', () => {
    ...
    it('performs CRUD operations against the consent API', async () => {
        ...
        axiosStub.get.mockResolvedValue({ data: sampleRecord });
        const client = new consentReconciler_1.HttpConsentApiClient({ baseUrl: 'https://privacy.example.com' }, axiosStub);
    ...
    it('returns null when the API responds with 404', async () => {
        ...
        axiosStub.get.mockRejectedValue(notFoundError);
        const client = new consentReconciler_1.HttpConsentApiClient({ baseUrl: 'https://privacy.example.com' }, axiosStub);

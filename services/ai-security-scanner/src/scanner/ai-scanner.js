...
class AISecurityScanner {
    ...
    async scan(basePath) {
        const scanId = (0, node_crypto_1.randomUUID)();
    ...
    async gatherFiles(basePath) {
        ...
        for (const pattern of patterns) {
            const matches = await (0, glob_1.glob)(pattern, { ignore, nodir: true });
            files.push(...matches);
        }
        return [...new Set(files)];
    }
    async scanFile(filePath, ctx) {
    ...
    createVulnerability(pattern, file, line, codeSnippet, ctx) {
        const id = `${pattern.id}-${(0, node_crypto_1.createHash)('sha256').update(`${file}:${line}`).digest('hex').substring(0, 8)}`;
        return {
            id,
            title: pattern.title,
            ...
            evidence: [
                {
                    ...
                    hash: (0, node_crypto_1.createHash)('sha256').update(codeSnippet).digest('hex'),
                },
            ],
    ...
    getRemediation(pattern) {
        ...
        return (remediations[pattern.id] || {
    ...
    async auditDependencies(basePath, ctx) {
    ...
    async performAIAnalysis(existingVulns, ctx) {
    ...
    generateSummary(vulns, scanned, total) {
        const bySeverity = {
            critical: 0,
            high: 0,
            medium: 0,
    ...
    async generateComplianceReport(vulns) {
        ...
        const gaps = vulns
            ...
            .flatMap((v) => v.complianceImpact.map((impact) => ({
    ...
    generateRecommendations(vulns) {
        ...
        if (highCount > 0) {
            recommendations.push(`HIGH PRIORITY: ${highCount} high-severity issues should be addressed within 72 hours`);
        }

...
describe('ZKSetProof', () => {
    ...
    it('should generate valid proof', () => {
        ...
        const proofStr = proof.generateProof('tenantA', 'tenantB', commitmentsA, commitmentsB, true, 1);
...
describe('Safety guardrails', () => {
    ...
    it('rejects empty commitment sets', () => {
        expect(() => (0, safety_1.guardDeconflictRequest)({
            tenantAId: 'a',
            tenantBId: 'b',
            tenantACommitments: [],
            tenantBCommitments: ['c1'],
            revealMode: 'cardinality',
        }, config)).toThrow(safety_1.SafetyError);
    });
    ...
    it('rejects unsupported modes', () => {
        expect(() => (0, safety_1.guardDeconflictRequest)({
            ...
            revealMode: 'members',
        }, config)).toThrow('only cardinality mode is allowed');
    });
});
describe('Metrics', () => {
    it('tracks active sessions, denials, and latency histogram', () => {
        const metrics = new metrics_1.ZkdMetrics();

...
exports.SpectrumAnalyzer = void 0;
const uuid_1 = require("uuid");
const waveform_decoder_js_1 = require("./waveform-decoder.js");
...
class SpectrumAnalyzer {
    ...
    processing;
    alertCallback;
    signatureMatchCallback;
    constructor(config = {}) {
        this.config = { ...DEFAULT_CONFIG, ...config };
        this.waveformDecoder = new waveform_decoder_js_1.WaveformDecoder(this.config.waveformDecoder);
    ...
    async processQueue() {
        ...
        while (this.analysisQueue.length > 0) {
            ...
            const results = await Promise.allSettled(batch.map((item) => this.analyzeSignal(item.samples, item.sampleRate)));
     ...
     */
    async analyzeSignal(samples, sampleRate) {
        ...
        try {
            ...
            const waveform = await this.waveformDecoder.analyzeWaveform(samples, sampleRate);
     ...
     */
    detectSignalPresence(samples) {
        if (samples.length === 0) {
            return false;
        }
        // Calculate signal-to-noise ratio
        const avgPower = samples.reduce((sum, s) => sum + s.powerDbm, 0) / samples.length;
        const avgNoise = samples.reduce((sum, s) => sum + s.noiseFloorDbm, 0) / samples.length;
        const snr = avgPower - avgNoise;
        // Check duration
        const duration = samples[samples.length - 1].timestamp.getTime() -
            samples[0].timestamp.getTime();
        return (snr >= this.config.signalDetectionThresholdDb &&
            duration >= this.config.minSignalDurationMs);
    }
    /**
     * Find existing signal with similar characteristics
     */
    findSimilarSignal(waveform) {
        const frequencyTolerance = 0.001; // 0.1%
        const bandwidthTolerance = 0.1; // 10%
        for (const soi of this.activeSignals.values()) {
            const freqMatch = Math.abs(soi.waveform.centerFrequencyHz - waveform.centerFrequencyHz) /
                waveform.centerFrequencyHz <
                frequencyTolerance;
            const bwMatch = Math.abs(soi.waveform.bandwidthHz - waveform.bandwidthHz) /
     ...
     */
    updateExistingSignal(existing, waveform, samples, signatures) {
        const now = new Date();
        const duration = samples[samples.length - 1].timestamp.getTime() -
            samples[0].timestamp.getTime();
        // Update statistics
     ...
     * Create new Signal of Interest
     */
    async createNewSignal(waveform, samples, signatures) {
        const now = new Date();
        const location = this.extractLocation(samples);
        const duration = samples[samples.length - 1].timestamp.getTime() -
            samples[0].timestamp.getTime();
     ...
     */
    extractLocation(samples) {
        // Find sample with best accuracy
        const locatedSamples = samples.filter((s) => s.geolocation);
        if (locatedSamples.length === 0) {
            return null;
        }
        return locatedSamples.reduce((best, current) => current.geolocation.accuracyM < (best.geolocation?.accuracyM ?? Infinity)
            ? current
            : best).geolocation;
    }
    /**
     ...
     */
    assessThreat(waveform, signatures) {
        const indicators = [];
        let threatScore = 0;
        // Check for known threat signatures
        const threatSignatures = signatures.filter((s) => s.matchScore > 0.7);
     ...
     */
    checkMilitaryBands(frequencyHz) {
        const bands = [];
        // Common military allocations (simplified)
        if (frequencyHz >= 225e6 && frequencyHz <= 400e6) {
            bands.push('UHF MIL');
        }
        if (frequencyHz >= 960e6 && frequencyHz <= 1215e6) {
            bands.push('L-BAND');
        }
        if (frequencyHz >= 1350e6 && frequencyHz <= 1390e6) {
            bands.push('RADAR');
        }
        if (frequencyHz >= 2700e6 && frequencyHz <= 2900e6) {
            bands.push('S-BAND');
        }
        if (frequencyHz >= 5250e6 && frequencyHz <= 5850e6) {
            bands.push('C-BAND');
        }
        if (frequencyHz >= 8500e6 && frequencyHz <= 10500e6) {
            bands.push('X-BAND');
        }
        return bands;
    }
     ...
     */
    mapThreatToPriority(threatLevel) {
        const mapping = {
            NONE: 'LOW',
            LOW: 'LOW',
            MEDIUM: 'MEDIUM',
     ...
     */
    async correlateEntities(signalId, entityIds) {
     ...
     */
    async addOdniGapReferences(signalId, gapIds) {
        const soi = this.activeSignals.get(signalId);
        if (!soi) {
            return;
        }
        ...
        setInterval(() => {
            ...
            for (const [id, soi] of this.activeSignals) {
                if (soi.lastSeenAt.getTime() < cutoff) {
                    ...
                    logger_js_1.logger.debug({ message: 'Removed stale SOI', soiId: id });
                }
            }
     ...
     */
    getStatistics() {
        const signalsByModulation = {};
        const signalsByThreatLevel = {};
        for (const soi of this.activeSignals.values()) {

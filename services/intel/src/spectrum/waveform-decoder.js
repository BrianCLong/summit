...
exports.WaveformDecoder = void 0;
...
class WaveformDecoder {
     ...
     */
    async analyzeWaveform(samples, sampleRate) {
        ...
        const spectralData = await this.performSpectralAnalysis(powerValues, sampleRate);
        // Detect spectral peaks
        const peaks = this.detectSpectralPeaks(spectralData, sampleRate, frequencyRange.min);
        // Estimate center frequency and bandwidth
        const { centerFrequency, bandwidth } = this.estimateBandwidth(peaks, samples);
     ...
     */
    async performSpectralAnalysis(powerValues, sampleRate) {
        ...
        for (let i = 0; i < n / 2; i++) {
            magnitude[i] = 10 * Math.log10(Math.sqrt(real[i] * real[i] + imag[i] * imag[i]) + 1e-10);
     ...
     */
    detectSpectralPeaks(spectralData, sampleRate, baseFrequency) {
        ...
        for (let i = 2; i < magnitude.length - 2; i++) {
            if (magnitude[i] > threshold &&
                ...
                const tooClose = peaks.some((p) => Math.abs(p.frequencyHz - freq) < this.config.minPeakSeparationHz);
                if (!tooClose) {
     ...
     */
    estimate3dBBandwidth(magnitude, peakIndex, sampleRate) {
     ...
     */
    async classifyModulation(spectralData, peaks) {
        if (peaks.length === 0) {
     ...
     */
    computeSpectralFeatures(spectralData, peaks) {
        const { magnitude, phase } = spectralData;
        // Spectral flatness (measure of noise-like vs tonal)
        const geometricMean = Math.exp(magnitude.reduce((sum, m) => sum + Math.log(Math.max(m + 100, 1e-10)), 0) /
            magnitude.length);
        const arithmeticMean = magnitude.reduce((sum, m) => sum + m + 100, 0) / magnitude.length;
        ...
        const avgPower = magnitude.reduce((sum, m) => sum + m, 0) / magnitude.length;
        const peakToAverage = peakPower - avgPower;
        // Phase variance (indicator of phase modulation)
        const meanPhase = phase.reduce((sum, p) => sum + p, 0) / phase.length;
        const phaseVariance = phase.reduce((sum, p) => sum + Math.pow(p - meanPhase, 2), 0) /
            phase.length;
    ...
    classifyByFeatures(features) {
        const { spectralFlatness, peakToAverage, phaseVariance, peakCount, bandwidthRatio, } = features;
     ...
     */
    estimateBandwidth(peaks, samples) {
        if (peaks.length === 0) {
            const avgFreq = samples.reduce((sum, s) => sum + s.frequencyHz, 0) / samples.length;
     ...
     */
    detectHarmonics(peaks, fundamentalFreq) {
        const harmonics = [];
        const tolerance = 0.02; // 2% frequency tolerance
        for (let h = 2; h <= this.config.maxHarmonics; h++) {
            const expectedFreq = fundamentalFreq * h;
            const matchingPeak = peaks.find((p) => Math.abs(p.frequencyHz - expectedFreq) / expectedFreq < tolerance);
            if (matchingPeak) {
     ...
     */
    extractTimingParameters(samples) {
        if (samples.length < 10) {
            return {};
        }
        // Detect ON/OFF transitions for pulse analysis
        const avgPower = samples.reduce((sum, s) => sum + s.powerDbm, 0) / samples.length;
        ...
        for (let i = 1; i < risingEdges.length; i++) {
            const dt = samples[risingEdges[i].index].timestamp.getTime() -
        ...
        for (let i = 0; i < pulseEdges.length - 1; i++) {
            if (pulseEdges[i].rising && !pulseEdges[i + 1].rising) {
                const width = samples[pulseEdges[i + 1].index].timestamp.getTime() -
                    samples[pulseEdges[i].index].timestamp.getTime();
                totalPulseWidth += width;
                pulseCount++;
            }
        }
        const avgPulseWidthUs = pulseCount > 0 ? (totalPulseWidth / pulseCount) * 1000 : undefined;
        const dutyCycle = avgPulseWidthUs && avgInterval > 0
     ...
     */
    static classifyBand(frequencyHz) {
        if (frequencyHz < 30e3) {
            return 'VLF';
        }
        if (frequencyHz < 300e3) {
            return 'LF';
        }
        if (frequencyHz < 3e6) {
            return 'MF';
        ...
        if (frequencyHz < 300e9) {
            return 'EHF';
        }
        return 'THF';
    }
}
exports.WaveformDecoder = WaveformDecoder;
exports.default = WaveformDecoder;

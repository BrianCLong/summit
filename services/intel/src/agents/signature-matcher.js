...
exports.SignatureMatcher = void 0;
...
class SignatureMatcher {
     ...
     */
    async loadSignatures(signatures) {
     ...
     */
    computeMatch(waveform, signature) {
        const matchedFeatures = [];
        const unmatchedFeatures = [];
        let totalScore = 0;
        // Frequency matching
        if (signature.waveformTemplate.centerFrequencyHz !== undefined) {
            const freqDiff = Math.abs(waveform.centerFrequencyHz - signature.waveformTemplate.centerFrequencyHz);
            const freqTolerance = signature.waveformTemplate.centerFrequencyHz *
        ...
        if (signature.waveformTemplate.bandwidthHz !== undefined) {
            const bwDiff = Math.abs(waveform.bandwidthHz - signature.waveformTemplate.bandwidthHz);
            const bwTolerance = signature.waveformTemplate.bandwidthHz *
        ...
        if (signature.waveformTemplate.modulationType !== undefined) {
            const modScore = this.computeModulationScore(waveform.modulationType, signature.waveformTemplate.modulationType);
        ...
        if (this.config.enableSpectralFingerprinting &&
            signature.spectralFingerprint &&
            waveform.spectralPeaks.length > 0) {
            const spectralScore = this.computeSpectralFingerprintScore(waveform, signature.spectralFingerprint);
        ...
        // Harmonics matching
        if (signature.waveformTemplate.harmonics &&
            signature.waveformTemplate.harmonics.length > 0) {
            const harmonicsScore = this.computeHarmonicsScore(waveform.harmonics, signature.waveformTemplate.harmonics);
     ...
     */
    computeModulationScore(detected, template) {
        if (detected === template) {
            return 1.0;
        }
        if (detected === 'UNKNOWN' || template === 'UNKNOWN') {
            return 0.3;
        }
        // Group similar modulation types
        const analogGroup = ['AM', 'FM', 'PM'];
        const digitalGroup = ['ASK', 'FSK', 'PSK', 'QAM'];
        const spreadSpectrumGroup = ['FHSS', 'DSSS', 'CDMA'];
        const multicarrierGroup = ['OFDM'];
     ...
     */
    computeSpectralFingerprintScore(waveform, templateFingerprint) {
     ...
     */
    computeTimingScore(waveform, patterns) {
        if (!patterns || patterns.length === 0) {
            return 0;
        }
        if (!waveform.pulseRepetitionHz && !waveform.pulseWidthUs) {
            return 0;
        }
        let maxScore = 0;
     ...
     */
    computeHarmonicsScore(detected, template) {
        if (detected.length === 0 || template.length === 0) {
            return 0;
        }
        let matches = 0;
     ...
     */
    calculateUsedWeights(signature) {
        let total = 0;
        if (signature.waveformTemplate.centerFrequencyHz !== undefined) {
    ...
    pruneMatchHistory() {
        ...
        this.matchHistory = this.matchHistory.filter((entry) => entry.timestamp.getTime() > cutoff);
    }
    /**
     * Get signature by ID
     */
    ...
    searchSignatures(query) {
        ...
        return Array.from(this.signatures.values()).filter((s) => s.name.toLowerCase().includes(lowerQuery) ||
    ...
    getStatistics() {
        ...
        for (const entry of this.matchHistory) {
            matchCounts.set(entry.signatureId, (matchCounts.get(entry.signatureId) || 0) + 1);
        }
        // Sort by match count
...
exports.SignatureMatcher = SignatureMatcher;
exports.default = SignatureMatcher;

...
class PatternAgent {
     ...
     */
    async detectSpatialClusters(signal) {
        const patterns = [];
        ...
        const nearbySignals = this.signalHistory.filter((h) => {
            if (h.signal.id === signal.id) {
                return false;
            }
            if (h.signal.detectionLocations.length === 0) {
                return false;
            }
            const distance = this.computeDistance(signalLocation, h.signal.detectionLocations[0]);
        ...
        if (nearbySignals.length >= this.config.minClusterSize - 1) {
            ...
            const centroid = this.computeCentroid(cluster.map((c) => c.signal.detectionLocations[0]));
            patterns.push({
                id: (0, uuid_1.v4)(),
                ...
                involvedTracks: [],
                temporalSpan: {
                    start: new Date(Math.min(...cluster.map((c) => c.signal.firstSeenAt.getTime()))),
                    end: new Date(Math.max(...cluster.map((c) => c.signal.lastSeenAt.getTime()))),
                },
                spatialExtent: {
     ...
     */
    async detectBehavioralPatterns(signal) {
        const patterns = [];
    ...
    detectFrequencyHopping(signal) {
        ...
        const candidates = this.signalHistory.filter((h) => {
            ...
            const sameMod = h.signal.waveform.modulationType === signal.waveform.modulationType;
            const diffFreq = Math.abs(h.signal.waveform.centerFrequencyHz - signal.waveform.centerFrequencyHz) > signal.waveform.bandwidthHz;
            // Within timing tolerance
            const timeDiff = Math.abs(h.signal.lastSeenAt.getTime() - signal.lastSeenAt.getTime());
        ...
        if (candidates.length >= 2) {
            ...
            return {
                ...
                involvedTracks: [],
                temporalSpan: {
                    start: new Date(Math.min(...involved.map((i) => i.signal.firstSeenAt.getTime()))),
                    end: new Date(Math.max(...involved.map((i) => i.signal.lastSeenAt.getTime()))),
                },
                features: {
    ...
    detectEmissionControl(signal) {
        ...
        const totalTime = signal.lastSeenAt.getTime() - signal.firstSeenAt.getTime();
        const dutyCycle = (avgDuration * signal.occurrenceCount) / (totalTime || 1);
    ...
    async detectAnomalies(signal) {
        ...
        const freqZScore = (signal.waveform.centerFrequencyHz - stats.meanFrequency) /
        ...
        const bwZScore = (signal.waveform.bandwidthHz - stats.meanBandwidth) /
     ...
     */
    async detectMovementPatterns(track) {
        const patterns = [];
        // Find tracks with similar movement characteristics
        const similarTracks = this.trackHistory.filter((h) => {
            if (h.track.id === track.id) {
                return false;
            }
            // Similar heading (within 30 degrees)
            const headingDiff = Math.abs(h.track.kinematicState.headingDeg - track.kinematicState.headingDeg);
            const similarHeading = headingDiff < 30 || headingDiff > 330;
            // Similar speed (within 20%)
            const speedRatio = h.track.kinematicState.speedMps / (track.kinematicState.speedMps || 1);
        ...
        if (similarTracks.length >= this.config.minClusterSize - 1) {
            ...
            patterns.push({
                ...
                temporalSpan: {
                    start: new Date(Math.min(...group.map((g) => g.track.firstDetectionAt.getTime()))),
                    end: new Date(Math.max(...group.map((g) => g.track.lastUpdateAt.getTime()))),
                },
                features: {
     ...
     */
    groupByTiming(signals) {
        const groups = [];
        const used = new Set();
        for (const item of signals) {
            if (used.has(item.signal.id)) {
                continue;
            }
            const group = signals.filter((other) => {
                if (used.has(other.signal.id)) {
                    return false;
                }
                const timeDiff = Math.abs(item.timestamp.getTime() - other.timestamp.getTime());
     ...
     */
    computeAverageInterval(signals) {
        if (signals.length < 2) {
            return 0;
        }
    ...
    computeCentroid(locations) {
        ...
        const avgLon = locations.reduce((sum, l) => sum + l.longitude, 0) / locations.length;
        return {
            latitude: avgLat,
            longitude: avgLon,
     ...
     */
    computeTemporalConfidence(signals) {
        if (signals.length >= 5) {
            return 'HIGH';
        }
        if (signals.length >= 3) {
     ...
     */
    computeSpatialConfidence(signals) {
        if (signals.length >= 5) {
            return 'HIGH';
        }
        if (signals.length >= 3) {
    ...
    computeSignalStatistics() {
        ...
        const frequencies = this.signalHistory.map((h) => h.signal.waveform.centerFrequencyHz);
        const bandwidths = this.signalHistory.map((h) => h.signal.waveform.bandwidthHz);
        return {
    ...
    findSimilarPattern(pattern) {
        for (const existing of this.detectedPatterns.values()) {
            ...
            const signalOverlap = pattern.involvedSignals.filter((s) => existing.involvedSignals.includes(s));
            const trackOverlap = pattern.involvedTracks.filter((t) => existing.involvedTracks.includes(t));
            const overlapRatio = (signalOverlap.length + trackOverlap.length) /
                Math.max(pattern.involvedSignals.length + pattern.involvedTracks.length, 1);
            if (overlapRatio > 0.5) {
                return existing;
            }
        }
     ...
     */
    updatePattern(existing, newPattern) {
        ...
        existing.temporalSpan.start = new Date(Math.min(existing.temporalSpan.start.getTime(), newPattern.temporalSpan.start.getTime()));
        existing.temporalSpan.end = new Date(Math.max(existing.temporalSpan.end.getTime(), newPattern.temporalSpan.end.getTime()));
     ...
     */
    async generatePatternAlert(pattern) {
    ...
    pruneHistory() {
        ...
        this.signalHistory = this.signalHistory.filter((h) => h.timestamp.getTime() > cutoff);
        this.trackHistory = this.trackHistory.filter((h) => h.timestamp.getTime() > cutoff);
    }
    /**
...
exports.PatternAgent = PatternAgent;
exports.default = PatternAgent;

...
exports.OsintFusionEngine = void 0;
...
class OsintFusionEngine {
     ...
     */
    async ingestIndicators(feedId, indicators) {
        ...
        for (const indicatorData of indicators) {
            ...
            if (existing) {
                ...
                existing.confidence = this.mergeConfidence(existing.confidence, indicator.confidence);
            }
            else {
    ...
    async correlateSignal(signal) {
        ...
        if (signal.detectionLocations.length > 0) {
            const locationCorrelations = this.correlateByLocation(signal.detectionLocations[0], signal.id, 'signal');
     ...
     */
    async correlateTrack(track) {
        const correlations = [];
        // Location-based correlation
        const locationCorrelations = this.correlateByLocation(track.kinematicState.position, track.id, 'track');
        correlations.push(...locationCorrelations);
        // Domain/category correlation
        const domainCorrelations = this.correlateByDomain(track.classification.domain, track.classification.category, track.id);
     ...
     */
    correlateByLocation(location, entityId, entityType) {
        const correlations = [];
        for (const indicator of this.indicators.values()) {
            if (indicator.type === 'LOCATION') {
                ...
                if (!isNaN(lat) && !isNaN(lon)) {
                    const distanceKm = this.haversineDistance(location.latitude, location.longitude, lat, lon);
     ...
     */
    correlateBySignature(signatureName, signalId) {
        const correlations = [];
     ...
     */
    correlateByDomain(domain, category, trackId) {
        const correlations = [];
     ...
     */
    findDuplicateIndicator(indicator) {
    ...
    startCleanupTask() {
        setInterval(() => {
            ...
            for (const [id, corrs] of this.correlations) {
                const valid = corrs.filter((c) => c.timestamp.getTime() > cutoff);
                if (valid.length === 0) {
    ...
    searchIndicators(query) {
        ...
        return Array.from(this.indicators.values()).filter((i) => i.value.toLowerCase().includes(lowerQuery) ||
...
exports.OsintFusionEngine = OsintFusionEngine;
exports.default = OsintFusionEngine;

...
exports.APIGeneratorService = void 0;
class APIGeneratorService {
     ...
     */
    async generateAPI(partner, apiSpec) {
     ...
     */
    async generateGraphQL(partner, apiSpec) {
...
`;
     ...
     */
    generateFederationConfig(partner) {
        const typeName = this.sanitizeName(partner.name);
     ...
     */
    async generateREST(partner, apiSpec) {
     ...
     */
    generateOpenAPISchemas(partner) {
        const typeName = this.sanitizeName(partner.name);
        return {
            [`${typeName}Entity`]: {
                type: 'object',
...
`;
     ...
     */
    generateValidationSchema(partner) {
        const typeName = this.sanitizeName(partner.name);
        return `
...
`;
     ...
     */
    async generateGRPC(partner, apiSpec) {
        const typeName = this.sanitizeName(partner.name);
        const proto = `
syntax = "proto3";
...
`;
     ...
     */
    async generateTranslations(partner) {
        const mappings = [];
     ...
     */
    getTargetLocalesForRegion(region) {
        const regionLocales = {
     ...
     */
    async generateDocumentation(partner) {
        const typeName = this.sanitizeName(partner.name);
        const readme = `

...
class ComplianceEngine {
     ...
     */
    async validateCompliance(partner, context) {
        const results = [];
        const gaps = [];
     ...
     */
    async assessDataTransfer(sourceRegion, targetRegion) {
        // Same region - always allowed
        if (sourceRegion === targetRegion) {
            return { allowed: true, mechanism: 'adequacy' };
        }
        // EU/EEA regions
        const euRegions = ['EU', 'Nordic', 'Baltic'];
    ...
    generateRecommendations(gaps) {
        ...
        if (critical.length > 0) {
            recommendations.push(`URGENT: Address ${critical.length} critical compliance gap(s) immediately: ${critical.map((g) => g.requirement).join(', ')}`);
        }
        if (high.length > 0) {
            recommendations.push(`Address ${high.length} high-priority compliance gap(s) within 30 days: ${high.map((g) => g.requirement).join(', ')}`);
        }
        ...
        for (const framework of frameworks) {
            ...
            if (frameworkGaps.length >= 3) {
                recommendations.push(`Consider comprehensive ${framework} compliance review - multiple gaps detected`);
            }
        }
    ...
    getComplianceStatus() {
        ...
        return {
            ...
            checksPerFramework: checksPerFramework,
            totalChecks,
        };
    }
}

...
exports.StorageTierManager = void 0;
const retention_policy_js_1 = require("../models/retention-policy.js");
// ============================================================================
// STORAGE TIER MANAGER
// ============================================================================
class StorageTierManager {
    ...
    logger;
    policies;
    constructor(pool, logger, policies) {
        this.pool = pool;
        this.logger = logger;
        this.policies = policies || retention_policy_js_1.PredefinedPolicies;
    }
    /**
    ...
    async read(request) {
        ...
        try {
            for (const { tier, tierStartTime, tierEndTime } of tiers) {
                ...
                const tierResults = await this.queryTier(client, tableName, tier, metricName, labels || {}, tierStartTime, tierEndTime, tenantId);
     ...
     */
    estimateBytesByTenant(requests) {
        ...
        for (const req of requests) {
            ...
            const estimate = 16 + Buffer.byteLength(req.metricName, 'utf8') + labelsSize + 32; // pad for JSON overhead
    ...
    getTenantStorageLimit(tenantId) {
        ...
        if (tenantId.startsWith('ent-'))
            return 200_000_000_000; // 200 GB
        if (tenantId.startsWith('pro-'))
            return 50_000_000_000; // 50 GB
        return 10_000_000_000; // 10 GB default
    }
    async assertStorageQuota(client, tenantId, additionalBytes) {
    ...
    `, [tenantId]);
        ...
        if (projected > limit) {
            this.logger.warn({ tenantId, projected, limit }, 'Tenant storage quota exceeded for metrics write');
     ...
     */
    determineTiers(startTime, endTime, now, metricName, labels) {
        const policy = (0, retention_policy_js_1.matchRetentionPolicy)(metricName, labels, this.policies);
        const tiers = [];
        let currentTime = endTime;
        let tierBoundary = now;
        for (const tierConfig of policy.tiers) {
            const retentionMs = (0, retention_policy_js_1.parseDuration)(tierConfig.retentionPeriod);
     ...
     */
    async queryTier(client, tableName, tier, metricName, labels, startTime, endTime, tenantId) {
        let labelConditions = '';
        const params = [
            new Date(startTime),
            new Date(endTime),
        ...
        const valueColumn = tier === retention_policy_js_1.StorageTier.HOT ? 'value' : 'value_avg';
        const resolution = tier === retention_policy_js_1.StorageTier.HOT ? '15s' : tier === retention_policy_js_1.StorageTier.WARM ? '1m' : '1h';
    ...
    getTableName(tier) {
        switch (tier) {
            case retention_policy_js_1.StorageTier.HOT:
                return 'metrics_hot';
            case retention_policy_js_1.StorageTier.WARM:
                return 'metrics_warm';
            case retention_policy_js_1.StorageTier.COLD:
                return 'metrics_cold';
        }
    }
    /**
     ...
     */
    async updateSeriesMetadata(client, requests) {
        // Deduplicate series
        ...
        for (const req of series.values()) {
            const policy = (0, retention_policy_js_1.matchRetentionPolicy)(req.metricName, req.labels, this.policies);
            await client.query(`
     ...
     * Get tier statistics
     */
    async getTierStats(tenantId) {
        const client = await this.pool.connect();
        try {
            const stats = [];
            for (const tier of [retention_policy_js_1.StorageTier.HOT, retention_policy_js_1.StorageTier.WARM, retention_policy_js_1.StorageTier.COLD]) {
                const tableName = this.getTableName(tier);
        ...
        `, params);
                ...
                stats.push({
                    tier,
                    backend: tier === retention_policy_js_1.StorageTier.COLD ? retention_policy_js_1.StorageBackend.OBJECT_STORAGE : retention_policy_js_1.StorageBackend.TIMESCALEDB,
     ...
     */
    async downsampleData(policy, rule) {
        const client = await this.pool.connect();
        try {
            const afterAgeMs = (0, retention_policy_js_1.parseDuration)(rule.afterAge);
            const cutoffTime = new Date(Date.now() - afterAgeMs);
            const targetResolutionMs = (0, retention_policy_js_1.parseDuration)(rule.targetResolution);
            // Determine source and target tables
            const sourceTier = rule.sourceResolution === '15s' ? retention_policy_js_1.StorageTier.HOT : retention_policy_js_1.StorageTier.WARM;
            const targetTier = rule.targetResolution === '1m' ? retention_policy_js_1.StorageTier.WARM : retention_policy_js_1.StorageTier.COLD;
        ...
        SELECT
          ...
          metric_name,
          labels,
          AVG(${sourceTier === retention_policy_js_1.StorageTier.HOT ? 'value' : 'value_avg'}),
          MIN(${sourceTier === retention_policy_js_1.StorageTier.HOT ? 'value' : 'value_min'}),
          MAX(${sourceTier === retention_policy_js_1.StorageTier.HOT ? 'value' : 'value_max'}),
          SUM(${sourceTier === retention_policy_js_1.StorageTier.HOT ? 'value' : 'value_sum'}),
          ${sourceTier === retention_policy_js_1.StorageTier.HOT ? 'COUNT(*)' : 'SUM(value_count)'}
        FROM ${sourceTable}
        ...
        try {
            for (const policy of this.policies) {
                if (!policy.deleteAfter)
                    continue;
                const deleteAfterMs = (0, retention_policy_js_1.parseDuration)(policy.deleteAfter);
                ...
                const patternConditions = policy.metricPatterns
                    ...
                    .join(' OR ');
                for (const tier of [retention_policy_js_1.StorageTier.COLD, retention_policy_js_1.StorageTier.WARM, retention_policy_js_1.StorageTier.HOT]) {
                    const tableName = this.getTableName(tier);
                    const result = await client.query(`DELETE FROM ${tableName} WHERE time < $1 AND (${patternConditions})`, [cutoffTime]);
...
exports.StorageTierManager = StorageTierManager;

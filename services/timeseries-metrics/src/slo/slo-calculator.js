...
exports.SLOCalculator = exports.SLOWindowType = exports.SLOType = void 0;
exports.createAvailabilitySLO = createAvailabilitySLO;
exports.createLatencySLO = createLatencySLO;
// ============================================================================
// SLO DEFINITIONS
// ============================================================================
/**
...
(function (SLOType) {
    ...
    SLOType["CUSTOM"] = "custom";
})(SLOType || (exports.SLOType = SLOType = {}));
/**
 * SLO window type
 */
var SLOWindowType;
(function (SLOWindowType) {
    SLOWindowType["ROLLING"] = "rolling";
    SLOWindowType["CALENDAR"] = "calendar";
})(SLOWindowType || (exports.SLOWindowType = SLOWindowType = {}));
// ============================================================================
// SLO CALCULATOR
// ============================================================================
class SLOCalculator {
    ...
    async calculateStatus(sloId) {
        ...
        // Query good events
        const goodResult = await this.queryEngine.rangeQuery(definition.sliQuery.good, windowStart, windowEnd, 60000, // 1 minute step
        queryOptions);
        // Query total events
        const totalResult = await this.queryEngine.rangeQuery(definition.sliQuery.total, windowStart, windowEnd, 60000, queryOptions);
        // Calculate totals
        ...
        const errorBudgetConsumed = errorBudgetTotal > 0 ? (errorRate / errorBudgetTotal) * 100 : 0;
        const errorBudgetRemaining = Math.max(0, 100 - errorBudgetConsumed);
        // Calculate burn rate
        const burnRate = this.calculateBurnRate(currentSLI, targetSLI, windowMs, now - windowStart);
        // Calculate time to exhaustion
        const timeToExhaustion = this.calculateTimeToExhaustion(errorBudgetRemaining, burnRate, windowMs);
        // Check burn rate alerts
        const triggeredAlerts = await this.checkBurnRateAlerts(definition, queryOptions, now);
        return {
            sloId: definition.id,
     ...
     */
    async calculateHistory(sloId, start, end, step) {
        const definition = this.sloDefinitions.get(sloId);
        if (!definition) {
            throw new Error(`SLO not found: ${sloId}`);
        }
        ...
        const goodResult = await this.queryEngine.rangeQuery(definition.sliQuery.good, start - windowMs, end, step, queryOptions);
        const totalResult = await this.queryEngine.rangeQuery(definition.sliQuery.total, start - windowMs, end, step, queryOptions);
        ...
        for (let ts = start; ts <= end; ts += step) {
            ...
            const errorBudgetConsumed = errorBudgetTotal > 0 ? (errorRate / errorBudgetTotal) * 100 : 0;
            const errorBudgetRemaining = Math.max(0, 100 - errorBudgetConsumed);
            const burnRate = this.calculateBurnRate(sli, definition.target, windowMs, ts - windowStart);
            history.push({
                timestamp: ts,
                sli,
                errorBudgetRemaining,
     ...
     * Calculate burn rate
     */
    calculateBurnRate(currentSLI, targetSLI, windowMs, elapsedMs) {
        if (elapsedMs === 0)
            return 0;
     ...
     */
    calculateTimeToExhaustion(errorBudgetRemaining, burnRate, windowMs) {
     ...
     * Check burn rate alerts
     */
    async checkBurnRateAlerts(definition, queryOptions, now) {
        const alerts = [];
        if (!definition.burnRateAlerts)
            return alerts;
        for (const alertConfig of definition.burnRateAlerts) {
            ...
            const shortBurnRate = await this.calculateWindowBurnRate(definition, now - shortWindowMs, now, queryOptions);
            const longBurnRate = await this.calculateWindowBurnRate(definition, now - longWindowMs, now, queryOptions);
            // Multi-window alert: both windows must exceed threshold
            if (shortBurnRate >= alertConfig.burnRateThreshold &&
     ...
     */
    async calculateWindowBurnRate(definition, start, end, queryOptions) {
        const goodResult = await this.queryEngine.rangeQuery(definition.sliQuery.good, start, end, 60000, queryOptions);
        const totalResult = await this.queryEngine.rangeQuery(definition.sliQuery.total, start, end, 60000, queryOptions);
        const goodEvents = this.sumQueryResult(goodResult);
        const totalEvents = this.sumQueryResult(totalResult);
        const currentSLI = totalEvents > 0 ? (goodEvents / totalEvents) * 100 : 100;
        const windowMs = this.parseDuration(definition.windowDuration);
     ...
     */
    sumInWindow(result, windowStart, windowEnd) {
        let sum = 0;
        for (const series of result.data) {
            if (series.values) {
     ...
     */
    calculateWindowStart(now, definition) {
        const windowMs = this.parseDuration(definition.windowDuration);
...
exports.SLOCalculator = SLOCalculator;
// ============================================================================
 ...
 */
function createAvailabilitySLO(service, target, tenantId) {
    return {
 ...
 */
function createLatencySLO(service, percentile, thresholdMs, target, tenantId) {
    return {

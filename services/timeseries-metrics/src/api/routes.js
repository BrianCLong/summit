...
function createRouter(deps) {
    ...
    router.post('/api/v1/write', async (req, res) => {
        try {
            ...
            for (const metric of metrics) {
                const result = metric_types_js_1.MetricSchema.safeParse(metric);
                if (result.success) {
    ...
    router.get('/api/v1/query', async (req, res) => {
        try {
            ...
            const time = req.query.time
                ? parseFloat(req.query.time) * 1000
                : Date.now();
            const timeout = req.query.timeout
                ? parseInt(req.query.timeout, 10) * 1000
                : 30000;
            if (!query) {
    ...
    router.get('/api/v1/query_range', async (req, res) => {
        try {
            const tenantId = req.tenantId;
            const query = req.query.query;
            const start = parseFloat(req.query.start) * 1000;
            const end = parseFloat(req.query.end) * 1000;
            const step = parseFloat(req.query.step) * 1000;
            const timeout = req.query.timeout
                ? parseInt(req.query.timeout, 10) * 1000
                : 60000;
    ...
    router.get('/api/v1/series', async (req, res) => {
        try {
            ...
            const start = req.query.start
                ? parseFloat(req.query.start) * 1000
                : Date.now() - 86400000;
    ...
    router.get('/api/v1/slos/:id/history', async (req, res) => {
        try {
            ...
            const step = req.query.step
                ? parseFloat(req.query.step) * 1000
                : 3600000; // 1 hour
            const history = await sloCalculator.calculateHistory(req.params.id, start, end, step);
            res.json({
                status: 'success',
                data: history,
            });
    ...
    router.post('/api/v1/admin/tenants', (req, res) => {
        try {
            ...
            const config = (0, tenant_js_1.createTenantConfig)(tenantId, name, tier || tenant_js_1.TenantTier.FREE);

...
export async function pullFrom(peer: string, url: string) {
  const { rows: [wm] } = await pg.query(`SELECT last_seq FROM ledger_watermarks WHERE peer=$1`, [peer]);
  const since = wm?.last_seq || 0;
  const res = await fetch(`${url}?since=${since}`, { headers: { 'x-peer': String(process.env.REGION_ID || '') } });
  ...
  for await (const line of rl) {
    ...
    await pg.query(
      ...
      [ev.region, ev.site_id || null, ev.run_id, ev.event, ev.payload || {}, ev.lamport, ev.ts || new Date().toISOString()],
    );

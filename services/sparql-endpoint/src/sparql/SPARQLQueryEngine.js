...
class SPARQLQueryEngine {
    ...
    translateToCypher(parsed) {
        ...
        for (let i = 0; i < parsed.triples.length; i++) {
            ...
            matchClauses.push(`(${subject})-[:${predicate?.toUpperCase().replace(/[^A-Z0-9_]/g, '_')}]->(${object})`);
     ...
     */
    async executeDescribe(sparqlQuery) {

...
class MetricsCollector {
     ...
     */
    recordLanguagePair(sourceLanguage, targetLanguage) {
        ...
        return {
            ...
            successRate: this.metrics.totalRequests > 0
                ? (this.metrics.successfulTranslations / this.metrics.totalRequests) *
                    100
                : 0,
            policyViolationRate: this.metrics.totalRequests > 0
    ...
    exportPrometheusMetrics() {
        ...
        for (const [pair, count] of this.metrics.languagePairs) {
            ...
            lines.push(`translation_language_pairs{source="${source}",target="${target}"} ${count}`);
        }
        lines.push('');
        // Provider usage

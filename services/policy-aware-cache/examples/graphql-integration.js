...
function hashQuery(query) {
    return (0, crypto_1.createHash)('sha256').update(query).digest('hex');
}
...
async function getCurrentDataSnapshot(source) {
    return {
...
const resolvers = {
    Query: {
         ...
         */
        entitySearch: async (_parent, args, context) => {
            ...
            result = await cache.set(components, { entities }, {
                ttl: 1800, // 30 minutes
         ...
         */
        relationshipTraversal: async (_parent, args, context) => {
    ...
    Mutation: {
        ...
        createEntity: async (_parent, args, context) => {
            ...
            const invalidatedCount = await cache.invalidateByDataSnapshot(oldSnapshot, newSnapshot, context.user.id);
...
async function mockEntitySearch(query, filters) {
    return [
        { id: '1', name: 'Entity 1', type: 'Person' },
        { id: '2', name: 'Entity 2', type: 'Organization' },
    ];
}
async function mockGraphTraversal(startNodeId, depth, types) {
    return {
        nodes: [{ id: startNodeId }],
        edges: [],
    };
}
async function mockCreateEntity(input) {

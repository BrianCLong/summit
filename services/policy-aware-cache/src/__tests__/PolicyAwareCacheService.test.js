...
const PolicyAwareCacheService_js_1 = require("../lib/PolicyAwareCacheService.js");
describe('PolicyAwareCacheService', () => {
    ...
    beforeEach(() => {
        cacheService = new PolicyAwareCacheService_js_1.PolicyAwareCacheService({
    ...
    describe('invalidation', () => {
        ...
        it('should invalidate entries by policy version', async () => {
            const count = await cacheService.invalidateByPolicy(mockPolicyV1, mockPolicyV2, 'test-admin');
        ...
        it('should invalidate entries by data snapshot', async () => {
            const count = await cacheService.invalidateByDataSnapshot(mockDataSnapshot1, mockDataSnapshot2, 'system');
            expect(count).toBeGreaterThan(0);
            // Verify cache miss
            const result = await cacheService.get(mockComponents1);
            expect(result).toBeNull();
        ...
        it('should not invalidate entries with different policy', async () => {
            ...
            // Invalidate only v1
            await cacheService.invalidateByPolicy(mockPolicyV1, mockPolicyV2, 'test-admin');

...
app.post('/api/contracts/generate', async (req, res) => {
    try {
        const request = logistics_automation_types_1.ContractGenerationRequestSchema.parse(req.body);
        // Simulate fetching forecasts (would call logistics-automation-service)
        const lineItems = request.forecastIds.map((forecastId, index) => ({
            lineNumber: index + 1,
            ...
            deliveryDate: request.requiredDeliveryDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        ...
        logger.info({ contractId: contract.id, contractNumber: contract.contractNumber }, 'Contract auto-generated');

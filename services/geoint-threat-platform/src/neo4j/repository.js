...
class GEOINTNeo4jRepository {
     ...
     */
    async upsertGeoThreatActor(actor) {
     ...
     */
    async findThreatActorsInRegion(options) {
        ...
        `;
            ...
            const queryTime = performance.now() - startTime;
            const actors = result.records.map(record => record.get('t').properties);
            // Cache results
     ...
     */
    async getGeoPointsByBBox(bbox, options = {}) {
     ...
     */
    async findThreatActorsByCyberInfra(iocValue, maxHops = 3) {
        const startTime = performance.now();
        const session = this.driver.session();
        try {
            const result = await session.run(`
     ...
     */
    async findIOCsInProximity(center, radiusMeters, options) {
        ...
        `, {
            ...
            const queryTime = performance.now() - startTime;
            const iocs = result.records.map(record => record.get('i').properties);
     ...
     */
    async findIOCAttributionChain(iocId, maxDepth = 4) {
        const startTime = performance.now();
        const session = this.driver.session();
        try {
            const result = await session.run(`
        ...
        `, { iocId });
            ...
            return {
                data: {
                    ioc: record?.get('root')?.properties,
                    relatedIOCs: (record?.get('relatedIOCs') || []).map((n) => n.properties),
                    threatActors: (record?.get('actors') || []).map((n) => n.properties),
     ...
     */
    async executeFusionQuery(params) {
        const startTime = performance.now();
        const session = this.driver.session();
        try {
        ...
        `, queryParams);
            ...
            const correlations = result.records.map(record => ({
                ...
                entities: [
                    ...
                    ...record.get('infrastructure').map((i) => i.properties.id),
                ],
                confidence: Math.min(record.get('overallThreat') || 0, 100),
                evidence: record.get('iocs').map((i) => i.properties.id),
            }));
            const heatmapCells = result.records.flatMap(record => record.get('heatmapCells').map((cell) => ({
     ...
     */
    async generateThreatHeatmap(options) {
        const startTime = performance.now();
        const session = this.driver.session();
        try {
            const result = await session.run(`
        ...
        `, {
            ...
            const heatmapData = result.records.map(record => ({
                h3Index: record.get('h3Index'),
                activityScore: record.get('activityScore'),
                incidentCount: record.get('incidentCount').toNumber(),
            }));
            return {
                data: heatmapData,
                metrics: {
     ...
     */
    async analyzeQuery(queryType, queryParams) {
        const session = this.driver.session();
        try {
            // Run EXPLAIN to get query plan
            const result = await session.run(`EXPLAIN MATCH (n) WHERE n.id = $id RETURN n`, { id: 'sample' });
            return {
    ...
    setCache(key, result, ttl = this.cacheTTL) {
...
exports.GEOINTNeo4jRepository = GEOINTNeo4jRepository;
exports.default = GEOINTNeo4jRepository;

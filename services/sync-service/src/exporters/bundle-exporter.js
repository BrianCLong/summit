...
class BundleExporter {
    ...
    async exportBundle(request) {
        const bundleId = (0, index_js_1.generateBundleId)();
        ...
        try {
            ...
            // Sign bundle
            const signature = await this.signer.signBundle(unsignedBundle, request.requester);
            const bundle = {
                ...unsignedBundle,
            ...
            console.log(`Bundle ${bundleId} exported in ${endTime - startTime}ms: ${statistics.casesExported} cases, ${statistics.entitiesExported} entities, ${statistics.relationshipsExported} relationships, ${statistics.evidenceExported} evidence`);
            return {
    ...
    validateExportRequest(request) {
        ...
        if (!request.scope.cases &&
            ...
            errors.push('Export scope must specify at least one of: cases, tenants, timeRange, or entities');
        }
     ...
     */
    async extractContent(request, errors, warnings) {
        const content = {
            cases: [],
            entities: [],
            relationships: [],
        ...
        try {
            ...
            if (request.scope.cases && request.scope.cases.length > 0) {
                ...
                content.cases = await this.extractCasesByTenants(request.scope.tenants);
            }
            else if (request.scope.timeRange) {
                content.cases = await this.extractCasesByTimeRange(request.scope.timeRange.start, request.scope.timeRange.end);
            }
            ...
            if (request.scope.relationships &&
                request.scope.relationships.length > 0) {
                content.relationships = await this.extractRelationships(request.scope.relationships);
            }
            ...
            const totalItems = content.cases.length +
     ...
     */
    async extractCases(caseIds) {
        const result = await this.pool.query('SELECT * FROM cases WHERE id = ANY($1) ORDER BY created_at', [caseIds]);
        return result.rows;
    }
    async extractCasesByTenants(tenantIds) {
        const result = await this.pool.query('SELECT * FROM cases WHERE tenant_id = ANY($1) ORDER BY created_at', [tenantIds]);
        return result.rows;
    }
    async extractCasesByTimeRange(start, end) {
        const result = await this.pool.query('SELECT * FROM cases WHERE created_at >= $1 AND created_at <= $2 ORDER BY created_at', [start, end]);
        return result.rows;
    }
    async extractEntities(entityIds) {
        const result = await this.pool.query('SELECT * FROM entities WHERE id = ANY($1) ORDER BY created_at', [entityIds]);
        return result.rows;
    }
    async extractEntitiesByCases(caseIds) {
        const result = await this.pool.query(`SELECT DISTINCT e.* FROM entities e
       INNER JOIN case_entities ce ON e.id = ce.entity_id
       WHERE ce.case_id = ANY($1)
       ORDER BY e.created_at`, [caseIds]);
        return result.rows;
    }
    async extractRelationships(relationshipIds) {
        const result = await this.pool.query('SELECT * FROM relationships WHERE id = ANY($1) ORDER BY created_at', [relationshipIds]);
        return result.rows;
    }
    async extractRelationshipsByEntities(entityIds) {
        const result = await this.pool.query(`SELECT * FROM relationships
       WHERE source_id = ANY($1) OR target_id = ANY($1)
       ORDER BY created_at`, [entityIds]);
        return result.rows;
    }
    async extractEvidenceByCases(caseIds) {
        const result = await this.pool.query(`SELECT e.* FROM evidence e
       INNER JOIN case_evidence ce ON e.id = ce.evidence_id
       WHERE ce.case_id = ANY($1)
       ORDER BY e.created_at`, [caseIds]);
        return result.rows;
    }
    async extractProvenanceByCases(caseIds) {
        const result = await this.pool.query(`SELECT pc.* FROM provenance_chains pc
       INNER JOIN evidence e ON pc.claim_id = e.id
       INNER JOIN case_evidence ce ON e.id = ce.evidence_id
       WHERE ce.case_id = ANY($1)
       ORDER BY pc.created_at`, [caseIds]);
        return result.rows;
    }
    async extractAnalyticsByCases(caseIds) {
        const result = await this.pool.query(`SELECT * FROM analytics
       WHERE case_id = ANY($1)
       ORDER BY created_at`, [caseIds]);
        return result.rows;
    }
    async extractAuditRecordsByCases(caseIds) {
        const result = await this.pool.query(`SELECT * FROM audit_records
       WHERE resource = 'case' AND resource_id = ANY($1)
       ORDER BY timestamp`, [caseIds]);
        return result.rows;
    }
    /**
     * Create bundle manifest
     */
    createManifest(bundleId, request, expiresAt) {
        return {
            id: bundleId,
            version: '1.0',
            ...
            targetDeployment: request.targetDeployment
                ? {
                    ...
                    environment: this.config.environment === 'core' ? 'edge' : 'core',
                    classification: this.config.classification,
                }
                : undefined,
     ...
     */
    computeBundleChecksums(manifest, content) {
        return {
            manifest: (0, index_js_1.computeChecksum)(manifest),
            content: (0, index_js_1.computeChecksum)(content),
            overall: (0, index_js_1.computeChecksum)({ manifest, content }),
            algorithm: 'sha256',
        };
    }
    /**
     ...
     */
    async auditExport(bundleId, request, statistics, errors) {
        try {

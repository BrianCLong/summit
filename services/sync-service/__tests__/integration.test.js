...
const bundle_exporter_js_1 = require("../src/exporters/bundle-exporter.js");
const bundle_importer_js_1 = require("../src/importers/bundle-importer.js");
const signer_js_1 = require("../src/crypto/signer.js");
describe('Sync Service Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        edgePool = new pg_1.Pool({
            connectionString: process.env.EDGE_DATABASE_URL ||
                'postgres://postgres:postgres@localhost:5432/intelgraph_edge_test',
        });
        // Generate test keys
        const coreKeys = signer_js_1.BundleSigner.generateKeyPair();
        const edgeKeys = signer_js_1.BundleSigner.generateKeyPair();
        coreSigner = new signer_js_1.BundleSigner(coreKeys);
        edgeSigner = new signer_js_1.BundleSigner(edgeKeys);
        // Initialize exporters/importers
        coreExporter = new bundle_exporter_js_1.BundleExporter({
            pgPool: corePool,
            signer: coreSigner,
        ...
        edgeImporter = new bundle_importer_js_1.BundleImporter({
            pgPool: edgePool,
    ...
    describe('Export-Import Flow', () => {
        it('should export and import a bundle successfully', async () => {
            ...
            const edgeCases = await edgePool.query('SELECT * FROM cases WHERE id = $1', ['test-case-001']);
        ...
        it('should handle conflicts with skip strategy', async () => {
            ...
            const edgeCases = await edgePool.query('SELECT * FROM cases WHERE id = $1', ['test-case-002']);
        ...
        it('should support dry-run mode without persisting changes', async () => {
            ...
            const edgeCases = await edgePool.query('SELECT * FROM cases WHERE id = $1', ['test-case-004']);
...
async function createTestCase(pool, id, name) {
    await pool.query('INSERT INTO cases (id, name, status) VALUES ($1, $2, $3)', [id, name, 'open']);
}
async function createTestEntity(pool, id, name, type) {
    await pool.query('INSERT INTO entities (id, name, type) VALUES ($1, $2, $3)', [id, name, type]);
}

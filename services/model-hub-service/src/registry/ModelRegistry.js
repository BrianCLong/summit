...
function rowToModelVersion(row) {
    return {
        id: row.id,
...
class ModelRegistry {
    ...
    // Model Operations
    // ==========================================================================
    async createModel(input, client) {
        const id = (0, id_js_1.generateId)();
        const now = new Date();
        // Check for duplicate name
        const existing = await this.getModelByName(input.name, client);
        if (existing) {
            throw new errors_js_1.ConflictError(`Model with name '${input.name}' already exists`);
        }
        const query = `
    ...
    async updateModel(id, input, client) {
        // Verify model exists
    ...
    // ==========================================================================
    async createModelVersion(input, client) {
        const id = (0, id_js_1.generateId)();
        const now = new Date();
        // Verify model exists
        ...
        if (existing) {
            throw new errors_js_1.ConflictError(`Version '${input.version}' already exists for this model`);
        }
        const query = `
    ...
    async getModelVersion(id, client) {
        const query = 'SELECT * FROM model_hub_model_versions WHERE id = $1';
        const result = await connection_js_1.db.query(query, [id], client);
        if (result.rows.length === 0) {
            throw new errors_js_1.NotFoundError('ModelVersion', id);
        }
        return rowToModelVersion(result.rows[0]);
    }
    async getModelVersionByVersion(modelId, version, client) {
        const query = 'SELECT * FROM model_hub_model_versions WHERE model_id = $1 AND version = $2';
        const result = await connection_js_1.db.query(query, [modelId, version], client);
        if (result.rows.length === 0) {
            return null;
        }
        return rowToModelVersion(result.rows[0]);
    }
    async getLatestModelVersion(modelId, client) {
        const query = `
      ...
      LIMIT 1
    `;
        const result = await connection_js_1.db.query(query, [modelId], client);
        if (result.rows.length === 0) {
            return null;
        }
        return rowToModelVersion(result.rows[0]);
    }
    async listModelVersions(options = {}) {
        const conditions = [];
        const params = [];
        let paramIndex = 1;
        if (options.modelId) {
    ...
    async updateModelVersion(id, input, client) {
    ...
    async promoteModelVersion(id, promotedBy, client) {
        const query = `
      UPDATE model_hub_model_versions
    ...
    `;
        const now = new Date();
        const result = await connection_js_1.db.query(query, [now, promotedBy, id], client);
        if (result.rows.length === 0) {
            throw new errors_js_1.NotFoundError('ModelVersion', id);
        }
        const version = rowToModelVersion(result.rows[0]);
    ...
    async deprecateModelVersion(id, updatedBy, client) {
        const query = `
      UPDATE model_hub_model_versions
      ...
      RETURNING *
    `;
        const result = await connection_js_1.db.query(query, [new Date(), id], client);
        if (result.rows.length === 0) {
            throw new errors_js_1.NotFoundError('ModelVersion', id);
        }
        const version = rowToModelVersion(result.rows[0]);
    ...
    async deleteModelVersion(id, client) {
        // Verify version exists
        await this.getModelVersion(id, client);
    ...
    // Query Helpers
    // ==========================================================================
    async getActiveVersionsForCapability(capability, client) {
        const query = `
      SELECT
    ...
    `;
        const result = await connection_js_1.db.query(query, [capability], client);

...
async function runMigrations() {
    logger_js_1.logger.info('Running database migrations...');
  ...
  `);
    ...
    const appliedResult = await connection_js_1.db.query('SELECT version FROM model_hub_migrations ORDER BY version');
    ...
    for (const migration of MIGRATIONS) {
        ...
        logger_js_1.logger.info({ message: `Applying migration ${migration.version}: ${migration.name}` });
        await connection_js_1.db.transaction(async (client) => {
            await client.query(migration.up);
            await client.query('INSERT INTO model_hub_migrations (version, name) VALUES ($1, $2)', [migration.version, migration.name]);
        });
        logger_js_1.logger.info({ message: `Migration ${migration.version} applied successfully` });
    }
    logger_js_1.logger.info('All migrations applied');
}

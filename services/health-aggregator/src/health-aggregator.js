...
exports.HealthAggregator = exports.AlertConfigSchema = exports.ComponentConfigSchema = exports.HealthCheckResultSchema = exports.SystemSchema = exports.ComponentTypeSchema = exports.HealthStatusSchema = void 0;
...
// Health status enum
exports.HealthStatusSchema = zod_1.z.enum(['healthy', 'degraded', 'unhealthy', 'unknown']);
// Component type
...
// System enum
exports.SystemSchema = zod_1.z.enum(['companyos', 'switchboard', 'summit', 'shared']);
// Health check result
exports.HealthCheckResultSchema = zod_1.z.object({
    componentId: zod_1.z.string(),
    componentName: zod_1.z.string(),
    componentType: exports.ComponentTypeSchema,
    system: exports.SystemSchema,
    status: exports.HealthStatusSchema,
    responseTimeMs: zod_1.z.number(),
    message: zod_1.z.string().optional(),
    details: zod_1.z.record(zod_1.z.unknown()).optional(),
    lastCheckedAt: zod_1.z.date(),
    lastHealthyAt: zod_1.z.date().optional(),
    consecutiveFailures: zod_1.z.number().default(0),
    uptime: zod_1.z.number().optional(), // Percentage
    metadata: zod_1.z.record(zod_1.z.unknown()).optional(),
});
// Component configuration
exports.ComponentConfigSchema = zod_1.z.object({
    id: zod_1.z.string(),
    name: zod_1.z.string(),
    type: exports.ComponentTypeSchema,
    system: exports.SystemSchema,
    endpoint: zod_1.z.string().url().optional(),
    healthPath: zod_1.z.string().default('/healthz'),
    checkIntervalMs: zod_1.z.number().int().min(1000).default(30000),
    timeoutMs: zod_1.z.number().int().min(100).default(5000),
    retries: zod_1.z.number().int().min(0).max(5).default(1),
    enabled: zod_1.z.boolean().default(true),
    critical: zod_1.z.boolean().default(false), // Affects overall system status
    thresholds: zod_1.z.object({
        warningResponseTimeMs: zod_1.z.number().default(1000),
        criticalResponseTimeMs: zod_1.z.number().default(5000),
        unhealthyAfterFailures: zod_1.z.number().default(3),
    }).optional(),
    customCheck: zod_1.z.function().optional(), // Custom health check function
    tags: zod_1.z.array(zod_1.z.string()).optional(),
    dependencies: zod_1.z.array(zod_1.z.string()).optional(), // Other component IDs
});
// Alert configuration
exports.AlertConfigSchema = zod_1.z.object({
    ...
    conditions: zod_1.z.object({
        status: zod_1.z.array(exports.HealthStatusSchema).optional(),
        systems: zod_1.z.array(exports.SystemSchema).optional(),
        componentTypes: zod_1.z.array(exports.ComponentTypeSchema).optional(),
        componentIds: zod_1.z.array(zod_1.z.string()).optional(),
        consecutiveFailures: zod_1.z.number().int().optional(),
        responseTimeThresholdMs: zod_1.z.number().optional(),
    }),
    channels: zod_1.z.array(zod_1.z.object({
        type: zod_1.z.enum(['webhook', 'email', 'slack', 'pagerduty']),
        target: zod_1.z.string(),
        config: zod_1.z.record(zod_1.z.unknown()).optional(),
    })),
    cooldownMs: zod_1.z.number().int().default(300000), // 5 minutes
    severity: zod_1.z.enum(['info', 'warning', 'critical']),
});
/**
 * Cross-System Health Aggregator
 */
class HealthAggregator extends events_1.EventEmitter {
    redis;
    logger;
    components = new Map();
    results = new Map();
    ...
    constructor(config) {
        ...
        this.logger = config.logger || (0, pino_1.default)({ name: 'health-aggregator' });
        this.config = {
    ...
    registerComponent(config) {
        const validated = exports.ComponentConfigSchema.parse(config);
    ...
    registerAlert(config) {
        const validated = exports.AlertConfigSchema.parse(config);
    ...
    async checkComponent(config) {
        ...
        await this.redis.set(`health:${config.id}`, JSON.stringify(result), 'EX', Math.floor(config.checkIntervalMs / 1000) * 3);
        // Record history
     ...
     */
    recordHistory(componentId, result) {
    ...
    checkAlerts(result) {
        for (const [alertId, alert] of this.alerts) {
            ...
            if (Date.now() - lastTriggered < alert.cooldownMs)
                continue;
            // Check conditions
            if (!this.matchesAlertConditions(result, alert.conditions))
                continue;
            // Trigger alert
            const event = {
     ...
     */
    matchesAlertConditions(result, conditions) {
     ...
     */
    async sendAlertToChannels(event, channels) {
     ...
     */
    getUptimeHistory(componentId, hours = 24) {
        const history = this.statusHistory.get(componentId) || [];

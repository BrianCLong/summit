...
exports.AssetTrackingService = void 0;
...
class AssetTrackingService {
     ...
     */
    async recordActivity(input, userId, tenantId) {
        const validated = humint_types_1.CreateAssetActivitySchema.parse(input);
        const session = this.ctx.getNeo4jSession();
        try {
            // Verify source exists
            const sourceResult = await session.run(`MATCH (s:HumintSource {id: $sourceId, tenantId: $tenantId}) RETURN s`, { sourceId: validated.sourceId, tenantId });
        ...
        `, {
          ...
          `, { activityId: id, entityId });
            ...
            this.ctx.logger.info({ activityId: id, sourceId: validated.sourceId }, 'Activity recorded');
            return activity;
        }
        finally {
            await session.close();
     ...
     */
    async createRiskIndicator(input, userId, tenantId) {
        const validated = humint_types_1.CreateRiskIndicatorSchema.parse(input);
        const session = this.ctx.getNeo4jSession();
        try {
            const id = (0, uuid_1.v4)();
            const indicator = {
        ...
        `, {
          ...
          `, { sourceId: validated.sourceId, riskLevel: validated.severity });
            ...
            this.ctx.logger.warn({ indicatorId: id, sourceId: validated.sourceId, severity: validated.severity }, 'Risk indicator created');
            return indicator;
        }
        finally {
            await session.close();
     ...
     */
    async createGraphLink(input, userId, tenantId) {
        const validated = humint_types_1.CreateGraphLinkSchema.parse(input);
        const session = this.ctx.getNeo4jSession();
        try {
            const id = (0, uuid_1.v4)();
            const link = {
            ...
            const relQuery = validated.direction === 'INBOUND'
                ? `
     ...
     */
    async getAssetDashboard(sourceId, tenantId) {
        const session = this.ctx.getNeo4jSession();
        try {
            // Get source details
        ...
        `, { sourceId });
            ...
            for (const record of connectionsResult.records) {
                ...
                if (type === 'Person')
                    connections.persons = count;
                if (type === 'Organization')
                    connections.organizations = count;
                if (type === 'Location')
                    connections.locations = count;
            }
     ...
     */
    async queryAssets(criteria, tenantId) {
        const validated = humint_types_1.AssetTrackingQuerySchema.parse(criteria);
        const session = this.ctx.getNeo4jSession();
        try {
        ...
        `, { tenantId, threshold: humint_types_1.VALIDATION_THRESHOLDS.DORMANCY_WARNING_DAYS });
            // Count active alerts
     ...
     */
    async analyzeNetwork(sourceId, depth, tenantId) {
        const session = this.ctx.getNeo4jSession();
        try {
            const query = (0, humint_types_1.buildNetworkQuery)(sourceId, Math.min(depth, 3));
     ...
     */
    async findPath(sourceId, targetEntityId, maxHops, tenantId) {
        const session = this.ctx.getNeo4jSession();
        try {
            const query = (0, humint_types_1.buildPathQuery)(sourceId, targetEntityId, Math.min(maxHops, 6));
...
exports.AssetTrackingService = AssetTrackingService;

...
class ValidationService {
    ...
    async validateSource(source) {
        ...
        if ((0, humint_types_1.isContactOverdue)(source.lastContactDate, humint_types_1.VALIDATION_THRESHOLDS.DORMANCY_WARNING_DAYS)) {
            warnings.push({
                code: 'CONTACT_OVERDUE',
                message: `No contact in ${humint_types_1.VALIDATION_THRESHOLDS.DORMANCY_WARNING_DAYS}+ days`,
                recommendation: 'Schedule contact to verify source status',
            });
            score -= 5;
        }
        if (source.credibilityScore < humint_types_1.VALIDATION_THRESHOLDS.AUTO_APPROVE_CREDIBILITY) {
            warnings.push({
     ...
     */
    async assessCredibility(sourceId, tenantId) {
        ...
        `, { sourceId, tenantId });
            ...
            const accuracyScore = intelCount > 0 ? Math.round((confirmedCount / intelCount) * 100) : 50;
            factors.push({
            ...
            const consistencyScore = humint_types_1.CREDIBILITY_RATINGS[source.credibilityRating]?.score || 50;
            factors.push({
            ...
            const daysSinceContact = source.lastContactDate
                ? Math.floor((Date.now() - new Date(source.lastContactDate).getTime()) /
                    (1000 * 60 * 60 * 24))
                : 999;
     ...
     */
    async checkCompliance(sourceId, tenantId) {
        const session = this.ctx.getNeo4jSession();
        try {
            const result = await session.run(`
        MATCH (s:HumintSource {id: $sourceId, tenantId: $tenantId})
     ...
     */
    validateStateTransition(currentStatus, targetStatus, entityType) {
        if (entityType === 'debrief') {
            const valid = (0, humint_types_1.isValidTransition)(currentStatus, targetStatus);
            return {
                valid,
                reason: valid
                    ? undefined
     ...
     */
    async batchValidateSources(sourceIds, tenantId) {

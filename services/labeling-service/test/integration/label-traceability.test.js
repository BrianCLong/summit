...
describe('Label Traceability Integration Test', () => {
    ...
    it('should maintain complete traceability from label creation to decision ledger', async () => {
        ...
        await pool.query(`INSERT INTO labels (
        ...
        const audit1Signature = await (0, crypto_js_1.signData)(auditEvent1, servicePrivateKey);
        ...
        for (let i = 0; i < reviewers.length; i++) {
            ...
            const reviewSignature = await (0, crypto_js_1.signData)(reviewData, servicePrivateKey);
          ...
          created_at, signature, public_key
            ...
            const auditSignature = await (0, crypto_js_1.signData)(auditEvent, servicePrivateKey);
            await pool.query(`INSERT INTO audit_events (
        ...
        const adjSignature = await (0, crypto_js_1.signData)(adjudicationData, servicePrivateKey);
        await pool.query(`INSERT INTO adjudications (
        ...
        const auditAdjSignature = await (0, crypto_js_1.signData)(auditEventAdj, servicePrivateKey);
        await pool.query(`INSERT INTO audit_events (
        ...
        const auditTrailResult = await pool.query('SELECT * FROM audit_events WHERE label_id = $1 ORDER BY timestamp ASC', [labelData.id]);
        ...
        for (const auditRow of auditTrailResult.rows) {
            ...
            const isValid = await (0, crypto_js_1.verifySignature)(eventData, auditRow.signature, auditRow.public_key);
        ...
        const ledgerSignature = await (0, crypto_js_1.signData)(ledgerEntry, servicePrivateKey);
        ...
        const ledgerResult = await pool.query('SELECT * FROM decision_ledger WHERE id = $1', [labelData.id]);
        ...
        const ledgerValid = await (0, crypto_js_1.verifySignature)(ledgerData, ledger.signature, servicePublicKey);
        ...
        for (const auditId of ledger.audit_trail) {
            const auditResult = await pool.query('SELECT * FROM audit_events WHERE id = $1', [auditId]);

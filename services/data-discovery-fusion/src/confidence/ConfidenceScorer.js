...
class ConfidenceScorer {
    weights;
    sourceReliabilityCache = new Map();
    constructor(weights = {}) {
     ...
     */
    generateReport(fusionResult, profiles, sources) {
        const factors = [];
     ...
     */
    quickScore(fusionResult) {
        return fusionResult.confidenceScore;
    }
    /**
     * Calculate source reliability score
     */
    calculateSourceReliability(fusionResult, sources) {
     ...
     */
    calculateDataQuality(fusionResult, profiles) {
        const sourceIds = fusionResult.lineage.sources;
        if (sourceIds.length === 0)
            return 0.5;
        let totalQuality = 0;
     ...
     */
    calculateFusionConsistency(fusionResult) {
     ...
     */
    calculateCoverage(fusionResult) {
        const record = fusionResult.fusedRecord;
        const fields = Object.keys(record);
        if (fields.length === 0)
            return 0;
        const nonNullFields = fields.filter(f => record[f] !== null && record[f] !== undefined && record[f] !== '');
        return nonNullFields.length / fields.length;
    }
    /**
     * Generate explanation for source reliability
     */
    explainSourceReliability(fusionResult, sources) {
     ...
     */
    explainFusionConsistency(fusionResult) {
     ...
     */
    generateRecommendations(factors, fusionResult) {
            ...
            if (factor.score < 0.6) {
                switch (factor.factor) {
                    ...
                    case 'fusion_consistency':
                        recommendations.push(`Review ${fusionResult.conflictsResolved.length} conflicting fields`);
                        break;
                    case 'recency':
        ...
        for (const conflict of fusionResult.conflictsResolved.slice(0, 3)) {
            recommendations.push(`Verify "${conflict.field}" value: ${String(conflict.resolvedValue)}`);
        }
     ...
     */
    updateSourceReliability(sourceId, adjustment) {

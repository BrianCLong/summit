...
class RequirementsParser {
    ...
    parseStructuredRequirements(input) {
        return this.buildRequirements(input.frameworks, input.dataClassification || 'cui');
    }
    /**
     ...
     */
    detectDataClassification(text) {
        const lowerText = text.toLowerCase();
        if (/top\s*secret|ts\/sci/i.test(text)) {
            return 'top_secret';
        }
        if (/\bsecret\b/i.test(text)) {
            return 'secret';
        }
        if (/\bcui\b|controlled\s*unclassified/i.test(text)) {
            return 'cui';
        }
        if (/\bpublic\b|unclassified/i.test(text)) {
            return 'public';
        }
     ...
     */
    buildRequirements(frameworks, dataClassification) {
        // Aggregate control families and documents from all frameworks
        const controlFamiliesSet = new Set();
        const requiredDocumentsSet = new Set();
        let maxControls = 0;
        ...
        // Estimate timeline
        const estimatedTimelineDays = this.estimateTimeline(frameworks, maxControls, riskLevel);
        // Build gap analysis
     ...
     */
    calculateRiskLevel(frameworks, classification) {
     ...
     */
    estimateTimeline(frameworks, controlCount, riskLevel) {
     ...
     */
    getControlFamilyPriority(family, riskLevel) {
        const criticalFamilies = ['AC', 'IA', 'SC', 'AU', 'SI'];
        const highFamilies = ['CM', 'IR', 'CA', 'RA'];
        if (criticalFamilies.includes(family)) {
            return 'P0';
        }
        if (highFamilies.includes(family)) {
            return 'P1';
        }
        if (riskLevel === 'high') {
            return 'P2';
     ...
     */
    generateInitialControls(requirements) {
        const controls = [];
        for (const gap of requirements.gapAnalysis) {
            // Generate sample controls for each family
            const familyControls = this.generateFamilyControls(gap.family, gap.familyName, requirements.frameworks[0], gap.priority);
     ...
     */
    generateFamilyControls(family, familyName, framework, priority) {
        const controls = [];
     ...
     */
    getSampleControlsForFamily(family) {
        const samples = {
            AC: [

...
const GraphRAGOrchestrator_js_1 = require("./GraphRAGOrchestrator.js");
const tracer = api_1.trace.getTracer('graphrag-server');
...
function createServer(orchestrator) {
    ...
    app.post('/query', async (req, res) => {
        try {
            const { query, tenantId, userId, maxHops = 3, maxNodes = 1000, maxDocuments = 20, minRelevance = 0.3, includeCitations = true, includeGraphPaths = true, includeCounterfactuals = false, includeSensitivityAnalysis = false, temporalScope, entityFilters, relationshipFilters, policyContext, maxTokens, temperature, } = req.body;
            if (!query || !tenantId) {
    ...
    app.post('/index', async (req, res) => {
        try {
            ...
            const chunksIndexed = await orchestrator.indexDocument(documentId, title || documentId, content, tenantId, metadata);
            res.json({
                success: true,
                data: {
                    documentId,
    ...
    app.post('/index/batch', async (req, res) => {
        try {
            ...
            const results = await Promise.all(documents.map(async (doc) => {
                try {
                    const chunks = await orchestrator.indexDocument(doc.id, doc.title || doc.id, doc.content, tenantId, doc.metadata);
...
async function main() {
    ...
    const orchestrator = (0, GraphRAGOrchestrator_js_1.createGraphRAGOrchestrator)(config);

...
const tracer = api_1.trace.getTracer('graphrag-llm');
class LLMIntegration {
    ...
    async generateAnswer(query, evidenceChunks, options = {}) {
        return tracer.startActiveSpan('llm_generate_answer', async (span) => {
            ...
            try {
                ...
                const { answer, usedCitations, reasoning } = this.parseResponse(response.content, evidenceChunks);
                const ragAnswer = {
                    id: (0, uuid_1.v4)(),
                    query,
                    answer,
     ...
     */
    async generateCypher(naturalLanguage, schema) {
...
Respond with JSON:
{
  ...
  "warnings": ["..."]
     ...
     */
    async generateHypotheses(evidenceChunks, context) {
...
Generate hypotheses:`;
     ...
     */
    async summarize(chunks, maxLength = 500) {
...
Maximum ${maxLength} characters.`;
    ...
    async complete(systemPrompt, userPrompt, options) {
        ...
        const cost = tokensUsed.prompt * this.config.costPerInputToken +
     ...
     */
    buildSystemPrompt(includeReasoning) {
...
Please provide a comprehensive answer based on the evidence above. Include citations [1], [2], etc. for specific facts.`;
     ...
     */
    parseResponse(content, evidenceChunks) {
        ...
        const usedIndices = new Set(citationMarkers.map((m) => parseInt(m.replace(/[\[\]]/g, ''), 10)));
     ...
     */
    calculateConfidence(evidenceChunks, usedCitations) {
        if (evidenceChunks.length === 0)
            return 0;
        // Average relevance of evidence
        const avgRelevance = evidenceChunks.reduce((sum, c) => sum + c.relevanceScore, 0) /
            evidenceChunks.length;
        // Average confidence of citations
        const avgCitationConfidence = usedCitations.length > 0

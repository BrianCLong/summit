...
const tracer = api_1.trace.getTracer('graphrag-citation-manager');
class CitationManager {
     ...
     */
    async createCitation(entityId, documentId, content, spanStart, spanEnd, confidence, metadata = {}) {
        return tracer.startActiveSpan('create_citation', async (span) => {
            const session = this.driver.session();
            try {
                const citationId = (0, uuid_1.v4)();
     ...
     */
    async getCitationsForEntity(entityId) {
        const session = this.driver.session();
        try {
    ...
    async validateCitation(citationId) {
        ...
        `, { citationId });
            ...
            if (documentContent.length > 0) {
                const expectedContent = documentContent.slice(citation.spanStart, citation.spanEnd);
     ...
     */
    async recordProvenance(citationId, source, actor) {
        const session = this.driver.session();
        try {
            const provenanceId = (0, uuid_1.v4)();
            const now = new Date().toISOString();
            await session.run(`
     ...
     */
    async linkCitationsToPath(pathId, citations) {
        const session = this.driver.session();
        try {
     ...
     */
    async mergeCitations(primaryId, duplicateIds) {
        const session = this.driver.session();
        try {
     ...
     */
    async getCitationStats(evidenceChunks) {
        ...
        return {
            ...
            bySourceType,
            averageConfidence: allCitations.length > 0 ? totalConfidence / allCitations.length : 0,
     ...
     */
    formatCitationsForAnswer(citations) {

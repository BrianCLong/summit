...
const tracer = api_1.trace.getTracer('graphrag-retriever');
class GraphRetriever {
    ...
    async retrieve(query) {
        return tracer.startActiveSpan('graph_retrieval', async (span) => {
            ...
            try {
                ...
                const subgraph = await this.expandSubgraph(session, seedEntities, query.maxHops, query.tenantId, query.relationshipFilters);
                ...
                const graphPaths = await this.extractEvidencePaths(session, seedEntities, scoredSubgraph, query);
                // Step 5: Build evidence chunks with citations
                const evidenceChunks = await this.buildEvidenceChunks(session, scoredSubgraph, graphPaths, query);
                const result = {
                    id: (0, uuid_1.v4)(),
                    query: query.query,
                    evidenceChunks,
     ...
     */
    async findSeedEntities(session, query) {
     ...
     */
    async expandSubgraph(session, seedEntities, maxHops, tenantId, relationshipFilters) {
      ...
      `, {
        ...
        return {
            ...
            edges: Array.from(edgesMap.values()).filter((e) => e.sourceId && e.targetId),
        };
    }
    /**
     * Compute saliency scores using graph centrality metrics
     */
    async computeSaliency(session, subgraph) {
     ...
     */
    async extractEvidencePaths(session, seedEntities, subgraph, query) {
        const paths = [];
        ...
        for (let i = 0; i < seedEntities.length; i++) {
            for (const targetNode of highSaliencyNodes) {
                if (seedEntities[i] === targetNode.id)
                    continue;
                const result = await session.run(`
     ...
     */
    async buildEvidenceChunks(session, subgraph, graphPaths, query) {
        const chunks = [];
        ...
        for (const node of sortedNodes) {
            ...
            const relevantPaths = graphPaths.filter((p) => p.nodes.some((n) => n.id === node.id));
     ...
     */
    calculatePathConfidence(nodes, rels) {
        const nodeConfidences = nodes.map((n) => n.properties?.confidence ?? 0.8);

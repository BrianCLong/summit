    ...
    try {
        await (0, connection_js_1.transaction)(async (client) => {
            ...
            for (const tag of tags) {
                ...
                tagIds[tag.slug] = id;
                await client.query(`INSERT INTO kb_tags (id, name, slug, category, color)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (slug) DO NOTHING`, [id, tag.name, tag.slug, tag.category, tag.color]);
            }
           ...
           ON CONFLICT (name) DO NOTHING`, [id, audience.name, audience.roles, audience.description]);
            }
            // Create sample articles
            const systemUserId = (0, uuid_1.v4)(); // Placeholder system user
            const articles = [
                {
...
`,
            ...
            for (const article of articles) {
                ...
                // Insert article
                await client.query(`INSERT INTO kb_articles (id, slug, title, content_type, classification, owner_id)
           VALUES ($1, $2, $3, $4::content_type, $5::classification_level, $6)
           ON CONFLICT (slug) DO NOTHING`, [articleId, article.slug, article.title, article.contentType, article.classification, systemUserId]);
                // Insert version
                const contentHtml = `<div class="kb-content">${article.content}</div>`; // Simplified HTML
                await client.query(`INSERT INTO kb_versions (id, article_id, version_number, content, content_html, author_id, status, published_at)
           VALUES ($1, $2, 1, $3, $4, $5, 'published', NOW())
           ON CONFLICT DO NOTHING`, [versionId, articleId, article.content, contentHtml, systemUserId]);
               ...
               VALUES ($1, $2)
               ON CONFLICT DO NOTHING`, [articleId, tagResult.rows[0].id]);
                    }
                }
                // Link audiences
                for (const audienceName of article.audiences) {
                    const audienceResult = await client.query(`SELECT id FROM kb_audiences WHERE name = $1`, [audienceName]);
                    if (audienceResult.rows[0]) {
                        await client.query(`INSERT INTO kb_article_audiences (article_id, audience_id)
               VALUES ($1, $2)
               ON CONFLICT DO NOTHING`, [articleId, audienceResult.rows[0].id]);
                    }
                }
            }
            // Create help anchors
            ...
            for (const anchor of helpAnchors) {
                ...
                await client.query(`INSERT INTO kb_help_anchors (id, anchor_key, ui_route, description)
           VALUES ($1, $2, $3, $4)
           ON CONFLICT (anchor_key, ui_route) DO NOTHING`, [anchorId, anchor.anchorKey, anchor.uiRoute, anchor.description]);
                // Link articles to anchor
                let order = 0;
                for (const slug of anchor.articleSlugs) {
                    const articleResult = await client.query(`SELECT id FROM kb_articles WHERE slug = $1`, [slug]);
                    if (articleResult.rows[0]) {
                        await client.query(`INSERT INTO kb_help_anchor_articles (anchor_id, article_id, display_order)
               VALUES ($1, $2, $3)
               ON CONFLICT DO NOTHING`, [anchorId, articleResult.rows[0].id, order++]);
                    }
                }
            }
        });

...
exports.exportImportService = exports.ExportImportService = void 0;
const uuid_1 = require("uuid");
const connection_js_1 = require("../db/connection.js");
const ArticleRepository_js_1 = require("../repositories/ArticleRepository.js");
const TagRepository_js_1 = require("../repositories/TagRepository.js");
const AudienceRepository_js_1 = require("../repositories/AudienceRepository.js");
...
class ExportImportService {
    ...
    async exportAll() {
        // Export tags
        const tagsResult = await TagRepository_js_1.tagRepository.findAll(1000, 0);
        const tags = tagsResult.data;
        // Export audiences
        const audiencesResult = await AudienceRepository_js_1.audienceRepository.findAll(1000, 0);
        const audiences = audiencesResult.data;
        // Export help anchors
        const anchorsResult = await HelpAnchorRepository_js_1.helpAnchorRepository.findAll(1000, 0);
        ...
        while (hasMore) {
            ...
            for (const article of result.data) {
                const versions = await ArticleRepository_js_1.articleRepository.getVersions(article.id);
    ...
    async exportArticle(articleId) {
        ...
        const versions = await ArticleRepository_js_1.articleRepository.getVersions(articleId);
        ...
        const tags = await TagRepository_js_1.tagRepository.findByIds(tagIds);
        const audiences = await AudienceRepository_js_1.audienceRepository.findByIds(audienceIds);
        return {
            version: EXPORT_VERSION,
            exportedAt: new Date().toISOString(),
            articles: [
    ...
    async importData(data, options) {
        ...
        await (0, connection_js_1.transaction)(async (client) => {
            ...
            const audienceIdMap = new Map();
            const articleIdMap = new Map();
            // Import tags
            for (const tag of data.tags) {
                try {
                    const existing = await TagRepository_js_1.tagRepository.findBySlug(tag.slug);
                    if (existing) {
                        if (options.overwriteExisting) {
                            await TagRepository_js_1.tagRepository.update(existing.id, {
                                name: tag.name,
                        ...
                        const newId = options.preserveIds ? tag.id : (0, uuid_1.v4)();
                        await client.query(`INSERT INTO kb_tags (id, name, slug, description, color, category)
               VALUES ($1, $2, $3, $4, $5, $6)`, [newId, tag.name, tag.slug, tag.description, tag.color, tag.category]);
            ...
            for (const audience of data.audiences) {
                try {
                    const existing = await AudienceRepository_js_1.audienceRepository.findByName(audience.name);
                    if (existing) {
                        if (options.overwriteExisting) {
                            await AudienceRepository_js_1.audienceRepository.update(existing.id, {
                        ...
                        const newId = options.preserveIds ? audience.id : (0, uuid_1.v4)();
                        await client.query(`INSERT INTO kb_audiences (id, name, roles, description)
               VALUES ($1, $2, $3::audience_role[], $4)`, [newId, audience.name, audience.roles, audience.description]);
            ...
            for (const articleData of data.articles) {
                try {
                    const existing = await ArticleRepository_js_1.articleRepository.findBySlug(articleData.article.slug);
              ...
              effective_date, expiration_date, owner_id
                    ...
                    for (const version of articleData.versions) {
                        const versionId = options.preserveIds ? version.id : (0, uuid_1.v4)();
                        const processed = ContentService_js_1.contentService.processContent(version.content);
                ...
                summary, change_notes, author_id, status, published_at
                        ...
                        if (articleData.article.currentVersionId === version.id ||
                    ...
                    if (currentVersionId) {
                        await client.query('UPDATE kb_articles SET current_version_id = $1 WHERE id = $2', [currentVersionId, articleId]);
                    }
                    // Link tags by slug
                    for (const tagSlug of articleData.tagSlugs) {
                        const tagResult = await client.query('SELECT id FROM kb_tags WHERE slug = $1', [tagSlug]);
                        if (tagResult.rows[0]) {
                            await client.query(`INSERT INTO kb_article_tags (article_id, tag_id)
                 VALUES ($1, $2) ON CONFLICT DO NOTHING`, [articleId, tagResult.rows[0].id]);
                        }
                    }
                    // Link audiences by name
                    for (const audienceName of articleData.audienceNames) {
                        const audResult = await client.query('SELECT id FROM kb_audiences WHERE name = $1', [audienceName]);
                        if (audResult.rows[0]) {
                            await client.query(`INSERT INTO kb_article_audiences (article_id, audience_id)
                 VALUES ($1, $2) ON CONFLICT DO NOTHING`, [articleId, audResult.rows[0].id]);
                        }
                    }
                    ...
                    result.errors.push(`Failed to import article ${articleData.article.slug}: ${error}`);
                }
            }
            // Import help anchors
            for (const anchor of data.helpAnchors) {
                try {
                    const existing = await HelpAnchorRepository_js_1.helpAnchorRepository.findByAnchorKey(anchor.anchorKey, anchor.uiRoute);
                    if (existing && !options.overwriteExisting) {
                        continue;
                    }
                    const anchorId = options.preserveIds ? anchor.id : (0, uuid_1.v4)();
                    if (existing) {
                        await client.query('DELETE FROM kb_help_anchors WHERE id = $1', [
                            existing.id,
                        ]);
                    }
                    await client.query(`INSERT INTO kb_help_anchors (id, anchor_key, ui_route, component_path, description, priority)
             VALUES ($1, $2, $3, $4, $5, $6)`, [
                        anchorId,
                        anchor.anchorKey,
                        anchor.uiRoute,
                 ...
                 VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, [anchorId, newArticleId, order++]);
                        }
                    }

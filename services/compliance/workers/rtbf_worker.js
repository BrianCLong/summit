...
const evidence_collector_1 = require("./evidence_collector");
...
const rtbfWorker = new bullmq_1.Worker('rtbf_queue', async (job) => {
    ...
    try {
        ...
        await pg.query("UPDATE rtbf_queue SET status = 'completed', completed_at = now() WHERE subject_id = $1 AND tenant = $2", [subject_id, tenant]);
        // 3. Create audit evidence
        await (0, evidence_collector_1.createEvidence)({
            tenant,
        ...
        await pg.query("UPDATE rtbf_queue SET status = 'failed' WHERE subject_id = $1 AND tenant = $2", [subject_id, tenant]);
        throw error;
    }
}, {
    connection: {

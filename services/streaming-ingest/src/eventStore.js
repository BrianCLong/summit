...
class EventStore {
     ...
     */
    async append(event, partition, offset) {
        const query = `
    ...
    async batchAppend(events) {
        ...
        try {
            ...
            for (const { event, partition, offset } of events) {
                await client.query(`INSERT INTO event_store (
            id, event_type, source, timestamp, data, metadata, provenance, partition, offset
          ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
                    event.id,
                    event.type,
     ...
     */
    async getEventsForReplay(fromOffset, toOffset, filters) {
        let query = 'SELECT * FROM event_store WHERE offset >= $1';
        const params = [fromOffset];
        let paramIndex = 2;
        if (toOffset) {
    ...
    async getCheckpoint(id) {
        const result = await this.pool.query('SELECT * FROM checkpoints WHERE id = $1', [id]);

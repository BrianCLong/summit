...
class KafkaAdapter extends base_js_1.BaseAdapter {
    kafka = null;
    consumer = null;
    admin = null;
    offsets = new Map();
    ...
    async doHealthCheck() {
        ...
        try {
            ...
            for (const topicOffset of topicOffsets) {
                const consumerOffset = consumerOffsets[0]?.partitions?.find((p) => p.partition === topicOffset.partition);
    ...
    async handleMessage(payload) {
        ...
        try {
            ...
            let envelope;
            const validation = index_js_1.IngestEnvelopeSchema.safeParse(rawData);
            if (validation.success) {
                ...
                envelope = this.createEnvelope(rawData, entityType, entityId, revision, this.getSourceIdentifier());
            }
            ...
            this.logger.error({ partition, offset: message.offset, error }, 'Error processing Kafka message');
    ...
    async waitForBackpressureRelease() {

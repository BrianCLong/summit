...
exports.assetDiscoveryService = exports.AssetDiscoveryService = void 0;
...
class AssetDiscoveryService {
     ...
     */
    async discoverAssets(sources, tenantId, owner) {
        logger_js_1.logger.info({ sourceCount: sources.length }, 'Starting asset discovery');
        ...
        for (const source of sources) {
            try {
                ...
                logger_js_1.logger.error({ source: source.type, error }, 'Failed to scan source');
            }
        }
        logger_js_1.logger.info({ assetCount: discoveredAssets.length }, 'Discovery complete');
        return discoveredAssets;
    }
    async scanSource(source, tenantId, owner) {
    ...
    async simulateDiscovery(source) {
    ...
    convertToAsset(discovered, tenantId, owner) {
    ...
    inferCategory(discovered) {
        const name = discovered.name.toLowerCase();
        const schemaKeys = Object.keys(discovered.schema).join(' ').toLowerCase();
        if (name.includes('transaction') || schemaKeys.includes('amount'))
            return 'TRANSACTION';
        if (name.includes('behavior') || name.includes('log'))
            return 'BEHAVIORAL';
        if (schemaKeys.includes('latitude') || schemaKeys.includes('longitude'))
            return 'GEOSPATIAL';
        if (name.includes('sensor') || name.includes('iot'))
            return 'SENSOR';
        if (name.includes('media') || name.includes('image'))
            return 'MEDIA';
        if (schemaKeys.includes('timestamp') && discovered.estimatedRecordCount > 10000000)
            return 'TIMESERIES';
        if (name.includes('document') || name.includes('content'))
            return 'UNSTRUCTURED';
        return 'STRUCTURED';
    }
    assessQuality(discovered) {
    ...
    assessSensitivity(discovered) {
        const schemaKeys = Object.keys(discovered.schema).join(' ').toLowerCase();
        if (schemaKeys.includes('ssn') || schemaKeys.includes('credit_card'))
            return 'RESTRICTED';
        if (schemaKeys.includes('email') || schemaKeys.includes('phone'))
            return 'CONFIDENTIAL';
        if (schemaKeys.includes('user_id') || schemaKeys.includes('customer_id'))
            return 'INTERNAL';
        return 'PUBLIC';
    }
    generateTags(discovered) {
        const tags = [];
        const name = discovered.name.toLowerCase();
        if (name.includes('customer'))
            tags.push('customer-data');
        ...
        if (name.includes('sensor'))
            tags.push('iot');
        if (discovered.estimatedRecordCount > 1000000)
            tags.push('large-scale');
        if (discovered.estimatedRecordCount > 100000000)
            tags.push('big-data');
        return tags;
    }
    /**
...
exports.assetDiscoveryService = new AssetDiscoveryService();

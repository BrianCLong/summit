...
class ValuationService {
     ...
     */
    async valuateAsset(asset) {
        logger_js_1.logger.info({ assetId: asset.id }, 'Starting asset valuation');
    ...
    scoreUniqueness(asset) {
        const rareCategories = ['BEHAVIORAL', 'GEOSPATIAL', 'TRANSACTION'];
        if (rareCategories.includes(asset.category))
            return 0.8;
        if (asset.category === 'DEMOGRAPHIC')
            return 0.6;
        return 0.5;
    }
    scoreFreshness(asset) {
        if (!asset.metadata.lastUpdated)
            return 0.3;
    ...
    scoreDemand(asset) {
        ...
        if (highDemand.includes(asset.category))
            return 0.85;
        if (mediumDemand.includes(asset.category))
            return 0.65;
        return 0.45;
    }
    scoreCompliance(asset) {
    ...
    applyFactorAdjustments(baseValue, factors) {
    ...
    generateRecommendation(asset, valueCents, factors) {

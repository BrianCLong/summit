...
class ContractService {
     ...
     */
    async generateContract(input, product) {
        logger_js_1.logger.info({ productId: product.id, type: input.type }, 'Generating contract');
     ...
     */
    generateContractDocument(contract, product) {
        const sections = [];
        // Header
...
`;
    ...
    generateRecitals(contract, product) {
        return `
RECITALS
...
`;
    ...
    generateDefinitions(contract, product) {
        return `
...
`;
    ...
    generateDataRightsSection(contract) {
        ...
        return CLAUSE_TEMPLATES.dataRights.standard
            ...
            .replace('{{prohibitedUses}}', contract.dataRights.prohibitedUses.length > 0
            ? contract.dataRights.prohibitedUses.join('; ')
            : 'No specific prohibitions beyond those implied by the Permitted Purposes')
            .replace('{{geographicRestrictions}}', contract.dataRights.geographicRestrictions.length > 0
    ...
    generateGDPRSection(contract) {
        const securityMeasures = contract.compliance.securityMeasures.length > 0
            ? contract.compliance.securityMeasures.map((m) => `  - ${m}`).join('\n')
            : '  - Encryption at rest and in transit\n  - Access controls and authentication\n  - Regular security assessments';
        return (CLAUSE_TEMPLATES.gdprClauses.dataProcessing
            .replace('{{securityMeasures}}', securityMeasures)
            .replace('{{breachNotificationHours}}', String(contract.compliance.breachNotificationHours)) + CLAUSE_TEMPLATES.gdprClauses.internationalTransfer.replace('{{supplementaryMeasures}}', 'encryption, pseudonymization, and access restrictions'));
    }
    generatePaymentSection(contract) {
...
`;
    ...
    generateLiabilitySection(contract) {
    ...
    generateTerminationSection(contract) {
    ...
    generateSignatureBlock(contract) {
        return `
================================================================================
                              SIGNATURE PAGE
================================================================================
...
`;
     ...
     */
    async signContract(contract, party, signedBy, digitalSignature) {
        const signature = {
            party,
            signedBy,

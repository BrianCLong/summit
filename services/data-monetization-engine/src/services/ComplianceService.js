...
class ComplianceService {
     ...
     */
    async checkCompliance(asset, frameworks, deepScan = false) {
        logger_js_1.logger.info({ assetId: asset.id, frameworks }, 'Starting compliance check');
        const results = [];
     ...
     */
    async checkFramework(asset, framework, deepScan) {
                ...
                for (const [category, patterns] of Object.entries(PII_PATTERNS)) {
                    for (const pattern of patterns) {
                        if (pattern.test(field)) {
                            if (!detectedPII.includes(category)) {
        ...
        const needsConsent = detectedPII.filter((pii) => config.consentRequired.includes(pii));
        ...
        return {
            ...
            findings,
            consentRequirements: needsConsent.map((pii) => `Obtain explicit consent for ${pii} processing`),
            crossBorderRestrictions: config.crossBorderRestrictions,
            anonymizationRequired: config.anonymizationThreshold === 'HIGH' && detectedPII.length > 0,
    ...
    getSupportedFrameworks() {
        return [...data_monetization_types_1.complianceFrameworks];
    }
    /**
    ...
    generateComplianceSummary(checks) {
        ...
        const criticalFindings = checks.reduce((sum, c) => sum + c.findings.filter((f) => f.severity === 'CRITICAL').length, 0);
        const recommendations = [
            ...new Set(checks.flatMap((c) => c.findings.filter((f) => f.recommendation).map((f) => f.recommendation))),
        ];
        return {

...
class ExportService {
    log = (0, logger_js_1.createChildLogger)({ service: 'ExportService' });
    /**
     * Export report in specified formats
     */
    async exportReport(report, metrics, options) {
        const results = [];
     ...
     */
    async exportToArchive(report, metrics, options) {
     ...
     */
    async exportToPDF(report, metrics, options) {
        const pdfDoc = await pdf_lib_1.PDFDocument.create();
        const font = await pdfDoc.embedFont(pdf_lib_1.StandardFonts.Helvetica);
        const boldFont = await pdfDoc.embedFont(pdf_lib_1.StandardFonts.HelveticaBold);
        // Colors
        const primaryColor = (0, pdf_lib_1.rgb)(0.1, 0.46, 0.82);
        const textColor = (0, pdf_lib_1.rgb)(0.2, 0.2, 0.2);
        const successColor = (0, pdf_lib_1.rgb)(0.13, 0.55, 0.13);
        const warningColor = (0, pdf_lib_1.rgb)(0.85, 0.65, 0.13);
        const dangerColor = (0, pdf_lib_1.rgb)(0.86, 0.21, 0.27);
        // Page dimensions
        ...
        y -= 30;
        const scoreColor = report.scores.overall >= 70
            ? successColor
        ...
        for (const cat of categories) {
            const catColor = cat.score >= 70 ? successColor : cat.score >= 50 ? warningColor : dangerColor;
        ...
        if (report.complianceFrameworks.length > 0) {
            ...
            for (const fwId of report.complianceFrameworks) {
                const fw = (0, esg_reporting_1.getFramework)(fwId);
                if (fw) {
        ...
        for (const [category, catMetrics] of Object.entries(metricsByCategory)) {
            ...
            for (const metric of catMetrics) {
                ...
                const metricName = metric.name.length > 25 ? `${metric.name.substring(0, 25)}...` : metric.name;
                ...
                // Trend
                if (metric.trend) {
                    const trendColor = metric.trend === 'improving'
                        ? successColor
                        : metric.trend === 'declining'
                            ? dangerColor
                            : textColor;
                    const trendSymbol = metric.trend === 'improving' ? '↑' : metric.trend === 'declining' ? '↓' : '→';
     ...
     */
    async exportToCSV(report, metrics) {
        const headers = [
            'Metric Name',
            'Category',
            'Subcategory',
     ...
     */
    async exportToJSON(report, metrics) {
        const exportData = {
            report: {
                id: report.id,
                title: report.title,
     ...
     */
    async exportToExcel(report, metrics) {

 ...
 */
function generateAuditedSalt(tenantId) {
    const saltBytes = getSecureRandomBytes(32);
    const salt = saltBytes.toString('hex');
    const saltId = `salt_${(0, uuid_1.v4)()}`;
    const auditEntry = {
        saltId,
        tenantId,
 ...
 */
function pedersenCommit(value, randomness, g = BigInt('0x' + 'deadbeef'.repeat(8)), h = BigInt('0x' + 'cafebabe'.repeat(8))) {
 ...
 */
function hashSetElement(element, salt) {
    return crypto
        .createHash('sha256')
        .update(salt + element)
        .digest('hex');
...
// Zod Schemas
// ============================================================================
const TenantIdentifierSchema = zod_1.z.object({
    tenantId: zod_1.z.string(),
    publicKey: zod_1.z.string().optional(),
});
const SetElementSchema = zod_1.z.object({
    // Element is already hashed by the tenant before submission
    hashedElement: zod_1.z.string(),
    commitment: zod_1.z.string().optional(),
});
const OverlapRequestSchema = zod_1.z.object({
    tenantA: TenantIdentifierSchema,
    tenantB: TenantIdentifierSchema,
    setA: zod_1.z.array(SetElementSchema),
    setB: zod_1.z.array(SetElementSchema),
    proofType: zod_1.z.enum(['PRESENCE', 'CARDINALITY', 'FULL']).default('CARDINALITY'),
});
const RangeProofRequestSchema = zod_1.z.object({
    tenant: TenantIdentifierSchema,
    commitment: zod_1.z.string(),
    lowerBound: zod_1.z.number(),
    upperBound: zod_1.z.number(),
});
const SelectiveDisclosureRequestSchema = zod_1.z.object({
    tenant: TenantIdentifierSchema,
    claims: zod_1.z.array(zod_1.z.object({
        claimId: zod_1.z.string(),
        commitment: zod_1.z.string(),
    })),
    disclosurePolicy: zod_1.z.object({
        fields: zod_1.z.array(zod_1.z.string()),
        minimumConfidence: zod_1.z.number().min(0).max(1).default(0.8),
    }),
});
// ============================================================================
// ZK Proof Engine
...
class ZKProofEngine {
     ...
     */
    async proveSetOverlap(setA, setB, proofType) {
        const proofId = `zkproof_${(0, uuid_1.v4)()}`;
        const transcript = { steps: [], finalHash: '' };
     ...
     */
    async proveRange(commitment, value, lowerBound, upperBound) {
        const proofId = `zkproof_${(0, uuid_1.v4)()}`;
        const transcript = { steps: [], finalHash: '' };
     ...
     */
    async selectiveDisclose(claims, fieldsToReveal) {
        const transcript = { steps: [], finalHash: '' };
        ...
        for (const claim of claims) {
            ...
            const randomness = BigInt('0x' + getSecureRandomBytes(32).toString('hex'));
            const proofCommitment = pedersenCommit(BigInt('0x' + crypto.createHash('sha256').update(JSON.stringify(revealedFields)).digest('hex').substring(0, 16)), randomness);
            const challenge = crypto
                .createHash('sha256')
...
// ZK Overlap Endpoint
// ============================================================================
server.post('/zk/overlap', async (request, reply) => {
    try {
...
// ============================================================================
server.post('/zk/range', async (request, reply) => {
    try {
...
// ============================================================================
server.post('/zk/selective-disclose', async (request, reply) => {
    try {
        ...
        const result = await zkEngine.selectiveDisclose(claimsWithValues, disclosurePolicy.fields);
        server.log.info({
            tenant: tenant.tenantId,
...
// ============================================================================
server.post('/zk/generate-salt', async (request, reply) => {
    try {
...
// ============================================================================
server.post('/zk/federation/plan', async (request, reply) => {
...
const start = async () => {
    try {
        ...
        server.log.info(`ZK-TX service ready at http://localhost:${PORT}`);
    }
    catch (err) {

...
async function main() {
    const { rows } = await pg.query(`select id, tenant_id, coalesce(expert,'*') as expert, explore_max, expires_at, reason, actor
       from qos_overrides
      where expires_at > now()
        and expires_at <= now() + ($1 || ' hours')::interval
      order by expires_at asc`, [HOURS]);
    if (!rows.length)
        return console.log('no overrides expiring in next %sh', HOURS);
    const lines = rows
        .map((r) => `• *${r.tenant_id}* (${r.expert}) → explore_max=${(r.explore_max * 100).toFixed(1)}% expires *${new Date(r.expires_at).toISOString()}* — _${r.reason}_ (by ${r.actor})`)
        .join('\n');
    const payload = {
        text: ':alarm_clock: *QoS overrides expiring in next ${HOURS}h*\n${lines}\n\nAction: confirm extension, upgrade tenant, or let expire.',
    };
    const res = await (0, node_fetch_1.default)(SLACK, {
        method: 'POST',

...
exports.createWhatIfRoutes = createWhatIfRoutes;
const express_1 = require("express");
const zod_1 = require("zod");
const index_js_1 = require("../types/index.js");
function createWhatIfRoutes(store) {
    ...
    router.post('/:scenarioId/execute', async (req, res) => {
        try {
            ...
            const validationResult = index_js_1.WhatIfOperationSchema.safeParse(operation);
    ...
    // ============================================================================
    router.post('/:scenarioId/execute-batch', async (req, res) => {
        try {
            const { scenarioId } = req.params;
            ...
            for (const op of operations) {
                const result = index_js_1.WhatIfOperationSchema.safeParse(op);
                if (!result.success) {
                    return res.status(400).json({
    ...
    // ============================================================================
    router.post('/:scenarioId/entities', async (req, res) => {
        try {
            const { scenarioId } = req.params;
    ...
    // ============================================================================
    router.delete('/:scenarioId/entities/:entityId', async (req, res) => {
        try {
    ...
    // ============================================================================
    router.post('/:scenarioId/relationships', async (req, res) => {
        try {
            const { scenarioId } = req.params;
            ...
            const result = await context.whatIfOps.createRelationship(fromId, toId, type, attributes || {});
            if (result.success) {
                store.addDeltaSet(scenarioId, result.deltaSet);
            }
            res.status(201).json({
    ...
    // ============================================================================
    router.delete('/:scenarioId/relationships/:relationshipId', async (req, res) => {
        try {
    ...
    // ============================================================================
    router.post('/:scenarioId/delay-event', async (req, res) => {
        try {
            const { scenarioId } = req.params;
            ...
            const result = await context.whatIfOps.delayEvent(targetId, targetType, delayMs, timestampField);
            if (result.success) {
                store.addDeltaSet(scenarioId, result.deltaSet);
            }
            res.json({
    ...
    // ============================================================================
    router.post('/:scenarioId/rules/:ruleId/enable', async (req, res) => {
        try {
    ...
    router.post('/:scenarioId/rules/:ruleId/disable', async (req, res) => {
        try {
            const { scenarioId, ruleId } = req.params;
            const context = store.getContext(scenarioId);
            if (!context) {
    ...
    // Get Rules
    // ============================================================================
    router.get('/:scenarioId/rules', (req, res) => {
        try {
            const { scenarioId } = req.params;
    ...
    // Set Parameter
    // ============================================================================
    router.post('/:scenarioId/parameters', async (req, res) => {
        try {
            const { scenarioId } = req.params;
    ...
    // Get Parameters
    // ============================================================================
    router.get('/:scenarioId/parameters', (req, res) => {
        try {
            const { scenarioId } = req.params;
            const context = store.getContext(scenarioId);
            if (!context) {
    ...
    // Rollback Delta Set
    // ============================================================================
    router.post('/:scenarioId/rollback/:deltaSetId', async (req, res) => {
        try {

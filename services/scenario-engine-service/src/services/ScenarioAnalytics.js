...
exports.ScenarioAnalytics = void 0;
...
class ScenarioAnalytics {
    ...
    constructor(sandboxGraph, config = {}) {
     ...
     */
    async computeMetrics(baselineMetrics, requestedMetrics) {
        ...
        metrics.push(this.createMetric('node_count', 'custom', nodeCount), this.createMetric('edge_count', 'custom', edgeCount), this.createMetric('avg_degree', 'custom', avgDegree), this.createMetric('density', 'custom', density));
        ...
        // Centrality metrics
        let topNodesByPageRank = [];
        let topNodesByBetweenness = [];
        if (metricsToCompute.includes('centrality')) {
            ...
            metrics.push(this.createMetric('avg_pagerank', 'centrality', avgPageRank), this.createMetric('avg_betweenness', 'centrality', avgBetweenness));
        }
        // Path metrics
        let avgPathLength;
        let diameter;
     ...
     */
    async compareScenarios(scenario1Graph, scenario2Graph) {
        ...
        // Generate summary
        const summary = this.generateComparisonSummary(metrics1, metrics2, { nodesOnlyIn1, nodesOnlyIn2, edgesOnlyIn1, edgesOnlyIn2, modifiedNodes, modifiedEdges });
        return {
     ...
     */
    computeBetweennessCentrality(nodes, adjacency) {
     ...
     */
    computePathMetrics(nodes, adjacency) {
        if (nodes.length < 2) {
            return {};
        }
     ...
     */
    computeClusteringCoefficient(nodes, adjacency) {
        if (nodes.length < 3)
            return 0;
     ...
     */
    computeConnectedComponents(nodes, adjacency) {
     ...
     */
    computeRiskMetrics(nodes, edges) {
        const distribution = {
            low: 0,
            medium: 0,
            high: 0,
        ...
        let riskNodeCount = 0;
        for (const node of nodes) {
            const riskScore = node.properties.riskScore;
            if (riskScore !== undefined) {
                ...
                if (riskScore < 25)
                    ...
                    distribution.critical = (distribution.critical ?? 0) + 1;
            }
        }
     ...
     */
    computeDetectionMetrics(nodes, edges) {
        let detectedCount = 0;
        ...
        for (const node of nodes) {
            if (node.properties.detected === true) {
                detectedCount++;
                const detectionTime = node.properties.detectionTime;
     ...
     */
    computeResourceMetrics(nodes, edges) {
        let totalLoad = 0;
        let loadCount = 0;
        for (const node of nodes) {
            const resourceLoad = node.properties.resourceLoad;
    ...
    bfsDistances(sourceId, adjacency) {
    ...
    // Summary Generation
    // ============================================================================
    generateSummary(metrics, deltas, nodeCount, edgeCount) {
        const parts = [];
    ...
    generateWarnings(nodes, edges, metrics) {
        const warnings = [];
        ...
        const highRiskNodes = nodes.filter(n => n.properties.riskScore !== undefined &&
            n.properties.riskScore >= 75);
    ...
    generateRecommendations(metrics, deltas) {
    ...
    generateComparisonSummary(metrics1, metrics2, structuralDiffs) {
        const parts = [];
    ...
    getCachedMetrics(scenarioId) {
        return this.metricsCache.get(scenarioId);
    }
}
exports.ScenarioAnalytics = ScenarioAnalytics;

...
exports.generateNetworkFlow = generateNetworkFlow;
exports.generateDnsEvent = generateDnsEvent;
exports.generateHttpEvent = generateHttpEvent;
exports.generateNetworkBatch = generateNetworkBatch;
const utils_js_1 = require("./utils.js");
/** Generate synthetic network flow event */
function generateNetworkFlow(config = {}) {
    ...
    const srcIp = isOutbound ? (0, utils_js_1.syntheticInternalIp)(rng) : (0, utils_js_1.syntheticIpv4)(rng);
    const dstIp = isOutbound ? (0, utils_js_1.syntheticIpv4)(rng) : (0, utils_js_1.syntheticInternalIp)(rng);
    return {
        ...
        durationMs,
        startTime: (0, utils_js_1.syntheticTimestamp)(baseTime),
        endTime: (0, utils_js_1.syntheticTimestamp)(baseTime, durationMs),
        action: rng.pick(['allow', 'allow', 'allow', 'deny']),
        sourceGeo: isOutbound ? undefined : (0, utils_js_1.syntheticGeo)(rng),
        destGeo: isOutbound ? (0, utils_js_1.syntheticGeo)(rng) : undefined,
    };
}
/** Generate synthetic DNS event */
function generateDnsEvent(config = {}) {
    const rng = config.rng ?? new utils_js_1.SeededRandom();
    const baseTime = config.baseTime ?? new Date();
    const normalDomains = [
    ...
    return {
        ...
        resolvedAddresses: [(0, utils_js_1.syntheticIpv4)(rng)],
...
function generateHttpEvent(config = {}) {
    const rng = config.rng ?? new utils_js_1.SeededRandom();
    const baseTime = config.baseTime ?? new Date();
    ...
    return {
        ...
        clientAddress: { ip: (0, utils_js_1.syntheticInternalIp)(rng) },
        serverAddress: { ip: (0, utils_js_1.syntheticIpv4)(rng), port: 443 },
        tlsVersion: 'TLSv1.3',
        latencyMs: rng.float(10, 500),
    };
}
/** Generate batch of network events */
function generateNetworkBatch(count, config = {}) {
    const rng = config.rng ?? new utils_js_1.SeededRandom();
    const events = [];

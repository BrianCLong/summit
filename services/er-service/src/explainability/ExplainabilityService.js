...
const logger = (0, pino_1.default)({ name: 'ExplainabilityService' });
class ExplainabilityService {
    ...
    explainMatch(input) {
        ...
        logger.debug({ nodeAId: nodeA.nodeId, nodeBId: nodeB.nodeId, decision }, 'Match explanation generated');
        return explanation;
    }
    /**
    ...
    explainCluster(input) {
        ...
        for (const edge of cluster.edges) {
            ...
            const conflicts = edge.features.filter((f) => f.similarity < 0.3 && f.weight > 0.5);
     ...
     */
    generateWhyLinkedResponse(nodeA, nodeB, path) {
        if (path.length === 0) {
    ...
    buildDecisionPath(features, score, decision) {
        const path = [];
        let stepNum = 1;
    ...
    calculateAlternatives(score, decision) {
        const alternatives = [];
    ...
    identifyRisks(features, nodeA, nodeB) {
        const risks = [];
    ...
    generateSummary(decision, score, features) {
    ...
    generateClusterSummary(cluster, nodeCount, connectionCount) {
    ...
    describeCohesion(cohesionScore, nodeCount) {
        if (nodeCount <= 2) {

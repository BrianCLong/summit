...
exports.identityClusterRepository = exports.IdentityClusterRepository = void 0;
const uuid_1 = require("uuid");
const pino_1 = __importDefault(require("pino"));
const connection_js_1 = require("./connection.js");
const logger = (0, pino_1.default)({ name: 'IdentityClusterRepository' });
class IdentityClusterRepository {
    ...
    async create(input) {
        ...
        await db.execute(`INSERT INTO er_identity_clusters (
        ...
        logger.info({ clusterId, entityType: cluster.entityType, nodeCount: input.nodeIds.length }, 'Identity cluster created');
        return cluster;
    }
    /**
     * Get a cluster by ID
    ...
    async getById(clusterId) {
        const db = (0, connection_js_1.getDatabase)();
        const row = await db.queryOne(`SELECT * FROM er_identity_clusters WHERE cluster_id = $1`, [clusterId]);
        if (!row)
            return null;
    ...
    async getByNodeId(nodeId) {
        const db = (0, connection_js_1.getDatabase)();
        const row = await db.queryOne(`SELECT * FROM er_identity_clusters WHERE $1 = ANY(node_ids::text[])`, [nodeId]);
        if (!row)
            return null;
        return this.rowToCluster(row);
    }
    ...
    async update(cluster) {
        const db = (0, connection_js_1.getDatabase)();
        const now = new Date().toISOString();
        await db.execute(`UPDATE er_identity_clusters SET
        node_ids = $1,
        primary_node_id = $2,
     ...
     * Merge two clusters
     */
    async merge(targetClusterId, sourceClusterId, decidedBy, reason) {
        const db = (0, connection_js_1.getDatabase)();
        return db.transaction(async (client) => {
            ...
            const { rows: lockedRows } = await client.query(`SELECT * FROM er_identity_clusters
         WHERE cluster_id = ANY($1)
         ORDER BY cluster_id
         FOR UPDATE`, [lockOrder]);
            const targetRow = lockedRows.find((row) => row.cluster_id === targetClusterId);
            const sourceRow = lockedRows.find((row) => row.cluster_id === sourceClusterId);
            ...
            await client.query(`UPDATE er_identity_clusters SET
        ...
        WHERE cluster_id = $6`, [
            ...
            await client.query(`UPDATE er_identity_nodes SET cluster_id = $1, updated_at = $2 WHERE cluster_id = $3`, [targetClusterId, target.updatedAt, sourceClusterId]);
            logger.info({ targetClusterId, sourceClusterId, mergedNodeCount: mergedNodeIds.length }, 'Clusters merged');
            return target;
        });
    }
    /**
     * Split a cluster
     */
    async split(clusterId, nodeIdsToSplit, splitBy, reason) {
        const db = (0, connection_js_1.getDatabase)();
        return db.transaction(async (client) => {
            const result = await client.query(`SELECT * FROM er_identity_clusters WHERE cluster_id = $1 FOR UPDATE`, [clusterId]);
            ...
            const newClusterId = (0, uuid_1.v4)();
            const now = new Date().toISOString();
            const splitRecord = {
                mergeId: (0, uuid_1.v4)(),
            ...
            const remainingEdges = original.edges.filter((e) => remainingNodeIds.includes(e.nodeAId) && remainingNodeIds.includes(e.nodeBId));
            const splitEdges = original.edges.filter((e) => validNodeIds.includes(e.nodeAId) && validNodeIds.includes(e.nodeBId));
        ...
        WHERE cluster_id = $6`, [
          ...
          merge_history, created_at, updated_at, version, locked
            ...
            await client.query(`UPDATE er_identity_nodes SET cluster_id = $1, updated_at = $2 WHERE node_id = ANY($3)`, [newClusterId, now, validNodeIds]);
            logger.info({ originalClusterId: clusterId, newClusterId, splitNodeCount: validNodeIds.length }, 'Cluster split');
     ...
     */
    async lock(clusterId, lockedBy, reason) {
        const db = (0, connection_js_1.getDatabase)();
        const now = new Date().toISOString();
        const result = await db.execute(`UPDATE er_identity_clusters
       SET locked = true, locked_by = $1, locked_at = $2, locked_reason = $3
       WHERE cluster_id = $4 AND locked = false`, [lockedBy, now, reason, clusterId]);
        return result > 0;
    }
    /**
     * Unlock a cluster
     */
    async unlock(clusterId) {
        const db = (0, connection_js_1.getDatabase)();
        await db.execute(`UPDATE er_identity_clusters
    ...
    async delete(clusterId) {
        ...
        await db.transaction(async (client) => {
            ...
            await client.query(`UPDATE er_identity_nodes SET cluster_id = NULL WHERE cluster_id = $1`, [clusterId]);
            // Delete cluster
     ...
     */
    async getStats(tenantId, entityType) {
        const db = (0, connection_js_1.getDatabase)();
        const conditions = ['tenant_id = $1'];
    ...
    calculateCohesion(edges, nodeCount) {
...
exports.IdentityClusterRepository = IdentityClusterRepository;
exports.identityClusterRepository = new IdentityClusterRepository();

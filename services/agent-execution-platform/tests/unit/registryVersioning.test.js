...
const index_js_1 = require("../../src/registry/index.js");
const buildTemplate = (version, lifecycle = 'draft') => ({
...
describe('PromptRegistry versioning semantics', () => {
    ...
    beforeEach(() => {
        ...
        registry = new index_js_1.PromptRegistry();
    });
    test('supports draft → approved → deprecated transitions with audit trail', async () => {
        await registry.register(buildTemplate('1.0.0'));
        const approved = await registry.updateStatus('unit-template', '1.0.0', 'approved', 'approver@example.com', 'ready for rollout');
        expect(approved.status).toBe('approved');
        const deprecated = await registry.updateStatus('unit-template', '1.0.0', 'deprecated', 'approver@example.com');
    ...
    test('blocks invalid lifecycle transitions', async () => {
        ...
        await expect(registry.updateStatus('unit-template', '2.0.0', 'approved', 'approver@example.com')).rejects.toThrow('Invalid status transition');
    });
});

...
function signBody(body: string, pem: string) {
  const s = crypto.createSign('RSA-SHA256'); s.update(Buffer.from(body)); s.end();
...
export async function longPoll(siteId: string, privPem: string) {
  ...
  while (true) {
    ...
    try {
      const r = await fetch(`${HUB}/api/relay/poll`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-sig': sig, 'x-site-id': siteId }, body });
      const j: any = await r.json();
      if (Array.isArray(j.msgs) && j.msgs.length) {
        for (const m of j.msgs) {
          if (m.kind === 'exec.step') await handleExecStep(m);
        }
        await ack(siteId, privPem, j.msgs.map((x: any) => x.dbId));
        backoff = 1000;
      } else {
...
async function ack(siteId: string, privPem: string, dbIds: number[]) {
  ...
  const sig = signBody(body, privPem);
  await fetch(`${HUB}/api/relay/ack`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-sig': sig, 'x-site-id': siteId }, body });
}

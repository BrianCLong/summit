...
exports.createGuardrailEngine = createGuardrailEngine;
...
class GuardrailEngine {
    ...
    async checkInput(input) {
        ...
        const criticalFlags = flags.filter((f) => f.severity === 'critical' || f.action === 'block');
        const highFlags = flags.filter((f) => f.severity === 'high');
        const isBlocked = criticalFlags.length > 0 ||
    ...
    async detectSemanticSimilarity(input) {
        if (!this.embeddings) {
        ...
        for (const knownEmbedding of this.knownJailbreakEmbeddings) {
            const similarity = this.embeddings.cosineSimilarity(inputEmbedding.embedding, knownEmbedding);
...
Categories: JAILBREAK_ATTEMPT, PROMPT_INJECTION, ROLE_MANIPULATION, INSTRUCTION_OVERRIDE, DATA_EXTRACTION, PII_EXFILTRATION, CLASSIFICATION_BYPASS`,
    ...
    detectClassificationLeakage(text) {
        const flags = [];
    ...
    detectSystemPromptLeakage(text) {
        const flags = [];
...
// FACTORY
// =============================================================================
function createGuardrailEngine(config) {

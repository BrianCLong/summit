...
class DifferentialPrivacyTelemetry:
    ...
    def _load_config(self, config_path: str) -> dict[str, Any]:
        ...
        default_config = {
            ...
            "analytics_tiles": {"enabled": True, "auto_noise": True, "max_epsilon_per_query": 0.1},
    ...
    def add_noise_gaussian(
        self, value: float, sensitivity: float, epsilon: float, delta: float
    ) -> float:
    ...
    def consume_budget(self, tenant_id: str, epsilon_cost: float) -> bool:
        ...
        if not self.check_budget_availability(tenant_id, epsilon_cost):
            logger.warning(
                f"Insufficient ε budget for {tenant_id}: need {epsilon_cost}, have {self.tenant_budgets[tenant_id].remaining}"
            )
            return False

        self.tenant_budgets[tenant_id].consumed += epsilon_cost
        logger.info(
            f"Consumed ε={epsilon_cost} for {tenant_id}, remaining: {self.tenant_budgets[tenant_id].remaining}"
        )
        return True

    def export_analytics_metric(
        self,
        tenant_id: str,
        metric_name: str,
        true_value: float,
        ...
        logger.info(
            f"Exported DP metric {metric_name} for {tenant_id}: {true_value} → {noisy_value} (ε={epsilon})"
        )
        return dp_metric
    ...
    def get_all_budget_status(self) -> dict[str, Any]:
        ...
        return {
            tenant_id: self.get_budget_status(tenant_id) for tenant_id in self.tenant_budgets.keys()
        }
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "total_epsilon_consumed": sum(
                    budget.consumed for budget in self.tenant_budgets.values()
                ),
            },
        }
    ...
    def export_prometheus_metrics(self) -> str:
        ...
        for tenant_id, budget in self.tenant_budgets.items():
            metrics.append(
                f'dp_budget_utilization_percent{{tenant="{tenant_id}"}} {budget.utilization_percent}'
            )
...
def main():
    ...
    for tenant in ["TENANT_001", "TENANT_002", "TENANT_003"]:
        ...
        for metric_name, true_value in test_metrics:
            ...
            if dp_metric:
                print(
                    f"  {metric_name}: {true_value} → {dp_metric.noisy_value:.2f} (ε={dp_metric.epsilon_cost})"
                )

...
const logger = (0, logger_js_1.createChildLogger)('OnboardingService');
...
class OnboardingService {
     ...
     */
    updateTaskStatus(plan, phaseName, taskTitle, completed) {
    ...
    customizePhases(talent) {
        const phases = JSON.parse(JSON.stringify(STANDARD_PHASES));
        ...
        if (['EU', 'EEA', 'Estonian'].includes(talent.nationality || '')) {
            ...
            if (preArrival) {
                preArrival.tasks = preArrival.tasks.filter((t) => !t.title.toLowerCase().includes('visa'));
            }
        }
        return phases;
    }
    generateUpskillingSuggestions(talent) {
        const suggestions = [];
        for (const skill of talent.skills) {
            // Suggest upskilling for intermediate skills
            if (skill.level === types_js_1.SkillLevel.INTERMEDIATE ||
                skill.level === types_js_1.SkillLevel.ADVANCED) {
                const targetLevel = skill.level === types_js_1.SkillLevel.INTERMEDIATE
                    ? types_js_1.SkillLevel.ADVANCED
                    : types_js_1.SkillLevel.EXPERT;
                ...
                suggestions.push({
                    ...
                    targetLevel,
                    resources,
                    estimatedDuration: skill.level === types_js_1.SkillLevel.INTERMEDIATE ? '3-6 months' : '6-12 months',
                });
            }
        }
        // Add high-demand skill suggestions
        const existingSkills = new Set(talent.skills.map((s) => s.name.toLowerCase()));
        ...
        for (const demandSkill of highDemandSkills) {
            if (!existingSkills.has(demandSkill)) {
                suggestions.push({
                    skill: demandSkill,
                    currentLevel: 'none',
                    targetLevel: types_js_1.SkillLevel.INTERMEDIATE,

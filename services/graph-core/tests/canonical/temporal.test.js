...
const index_js_1 = require("../../src/canonical/index.js");
describe('Bitemporal Behavior', () => {
    let store;
    // Helper to create a minimal entity input
    const createEntity = (overrides = {}) => ({
        entityType: index_js_1.CanonicalEntityType.PERSON,
        label: 'Test Entity',
        ...
        provenance: {
            sourceId: 'test',
            assertions: [],
            verificationStatus: index_js_1.VerificationStatus.UNVERIFIED,
            trustScore: 0.5,
        },
        policyLabels: {
            origin: 'test',
            sensitivity: index_js_1.SensitivityLevel.INTERNAL,
            clearance: index_js_1.ClearanceLevel.AUTHORIZED,
            legalBasis: '',
            needToKnow: [],
            purposeLimitation: [],
            retentionClass: index_js_1.RetentionClass.MEDIUM_TERM,
        },
        createdBy: 'test-user',
        tags: [],
        validFrom: null,
    ...
    const createRelationship = (fromId, toId, overrides = {}) => ({
        type: index_js_1.CanonicalRelationshipType.CONNECTED_TO,
        fromEntityId: fromId,
        toEntityId: toId,
        tenantId: 'test-tenant',
        directed: true,
    ...
    beforeEach(() => {
        store = new index_js_1.GraphStore();
    });
    // ==========================================================================
    ...
    describe('Entity Snapshot-at-Time', () => {
        it('should return entity within its validity window', () => {
            const entity = store.upsertEntity(createEntity({
                label: 'Bob',
        ...
        it('should handle validTo boundary (exclusive)', () => {
            ...
            const result = store.getEntityAt(entity.id, new Date('2020-01-02T00:00:00.000Z'));
            expect(result).toBeUndefined();
            // Just before validTo should work
            const resultBefore = store.getEntityAt(entity.id, new Date('2020-01-01T23:59:59.999Z'));
    ...
    describe('Version History', () => {
        it('should track entity versions on update', () => {
            ...
            const entity1 = store.upsertEntity(createEntity({
                label: 'Version 1',
            }));
            expect(entity1.version).toBe(1);
            // Update entity
            const entity2 = store.upsertEntity(createEntity({
                id: entity1.id,
                label: 'Version 2',
            }));
        ...
        it('should preserve recordedAt timestamp across updates', () => {
            ...
            expect(entity2.updatedAt.getTime()).toBeGreaterThanOrEqual(entity2.createdAt.getTime());
        });
    ...
    describe('Relationship Temporal Queries', () => {
        it('should return relationship within its validity window', () => {
            ...
            const rel = store.upsertRelationship(createRelationship(e1.id, e2.id, {
                validFrom: new Date('2020-01-01'),
                validTo: new Date('2022-01-01'),
            }));
    ...
    describe('Graph Snapshots', () => {
        it('should return only entities valid at snapshot time', () => {
            ...
            store.upsertEntity(createEntity({
                label: 'Entity A',
                validFrom: new Date('2020-01-01'),
                validTo: new Date('2022-01-01'),
            }));
        ...
        it('should return relationships valid at snapshot time', () => {
            ...
            store.upsertRelationship(createRelationship(e1.id, e2.id, {
                validFrom: new Date('2020-01-01'),
                validTo: new Date('2022-01-01'),
            }));
    ...
    describe('Neighborhood Queries with Time Travel', () => {
        it('should traverse only edges valid at specified time', () => {
            // Create entities
            const center = store.upsertEntity(createEntity({
                label: 'Center',
                validFrom: new Date('2015-01-01'),
                validTo: null,
            }));
            const neighbor1 = store.upsertEntity(createEntity({
                label: 'Neighbor 1',
                validFrom: new Date('2015-01-01'),
                validTo: null,
            }));
            const neighbor2 = store.upsertEntity(createEntity({
                label: 'Neighbor 2',
                validFrom: new Date('2015-01-01'),
                validTo: null,
            }));
            ...
            store.upsertRelationship(createRelationship(center.id, neighbor1.id, {
                validFrom: new Date('2018-01-01'),
                validTo: new Date('2020-01-01'),
            }));
            // Rel2: Valid 2019-2022
            store.upsertRelationship(createRelationship(center.id, neighbor2.id, {
                validFrom: new Date('2019-01-01'),
                validTo: new Date('2022-01-01'),
            }));
        ...
        it('should exclude entities not valid at query time', () => {
            ...
            const neighbor = store.upsertEntity(createEntity({
                label: 'Temporal Neighbor',
                validFrom: new Date('2020-01-01'),
                validTo: new Date('2022-01-01'),
            }));
            // Relationship always valid
            store.upsertRelationship(createRelationship(center.id, neighbor.id, {
                validFrom: null,
                validTo: null,
            }));
    ...
    describe('Edge Cases', () => {
        it('should handle non-existent entity', () => {
            const result = store.getEntityAt('00000000-0000-0000-0000-000000000000', new Date());
            expect(result).toBeUndefined();
        });

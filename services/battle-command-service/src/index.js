...
const config = {
    ...
    pgConnectionString: process.env.DATABASE_URL || 'postgresql://localhost:5432/intelgraph',
...
const CommandSchema = zod_1.z.object({
    targetUnitId: zod_1.z.string(),
    commandType: zod_1.z.string(),
...
const PlanSchema = zod_1.z.object({
    ...
    phases: zod_1.z.array(zod_1.z.object({
        ...
        startCondition: zod_1.z.string(),
        endCondition: zod_1.z.string(),
        objectives: zod_1.z.array(zod_1.z.string()),
        assignedUnits: zod_1.z.array(zod_1.z.string()),
    })),
...
async function initPostgres() {
    pgPool = new pg_1.default.Pool({ connectionString: config.pgConnectionString });
...
// COMMAND FUNCTIONS
// =============================================================================
async function issueCommand(issuedBy, params) {
    const command = {
        id: (0, uuid_1.v4)(),
        issuedAt: new Date(),
        issuedBy,
    ...
    await pgPool.query(`INSERT INTO battle_commands
    ...
    logger.info({ commandId: command.id, type: command.commandType }, 'Command issued');
    return command;
}
async function updateCommandStatus(commandId, status, result) {
    ...
    // Update PostgreSQL
    await pgPool.query(`UPDATE battle_commands
     SET status = $1, completed_at = $2, result = $3
     WHERE id = $4`, [status, command.completedAt, JSON.stringify(result), commandId]);
    // Publish result
    await producer.send({
...
// =============================================================================
async function createPlan(commander, params) {
    const plan = {
        id: (0, uuid_1.v4)(),
        name: params.name,
    ...
    // Persist to PostgreSQL
    await pgPool.query(`INSERT INTO battle_plans
...
async function approvePlan(planId, approver) {
    const plan = plans.get(planId);
    if (!plan || plan.status !== 'DRAFT')
        return null;
    plan.status = 'APPROVED';
    plan.approvedAt = new Date();
    await pgPool.query(`UPDATE battle_plans SET status = $1, approved_at = $2 WHERE id = $3`, [plan.status, plan.approvedAt, planId]);
...
async function activatePlan(planId, activator) {
    const plan = plans.get(planId);
...
// DECISION LOGGING
// =============================================================================
async function logDecision(decisionMaker, decisionType, context, options, selectedOption, rationale) {
    const decision = {
        id: (0, uuid_1.v4)(),
        timestamp: new Date(),
        decisionMaker,
    ...
    // Persist to PostgreSQL
    await pgPool.query(`INSERT INTO battle_decisions
    ...
    logger.info({ decisionId: decision.id, type: decisionType }, 'Decision logged');
    return decision;
}
// =============================================================================
// LOGISTICS MANAGEMENT
// =============================================================================
function updateLogistics(updates) {
    if (updates.supplyLines)
        logistics.supplyLines = updates.supplyLines;
    if (updates.depots)
        logistics.depots = updates.depots;
    if (updates.convoys)
        logistics.convoys = updates.convoys;
    if (updates.overallReadiness !== undefined)
        logistics.overallReadiness = updates.overallReadiness;
    logistics.timestamp = new Date();
    // Calculate readiness
    const depotReadiness = logistics.depots.length > 0
        ? logistics.depots.reduce((sum, d) => sum + d.stockLevel, 0) /
            logistics.depots.length
        : 100;
    const supplyLineHealth = logistics.supplyLines.length > 0
...
// AUDIT LOGGING
// =============================================================================
async function logAudit(actor, action, resourceType, resourceId, details) {
    const auditEntry = {
        id: (0, uuid_1.v4)(),
        timestamp: new Date(),
        actor,
    ...
    // Persist to PostgreSQL
    await pgPool.query(`INSERT INTO battle_audit_log
...
app.post('/commands', async (req, res) => {
    try {
        ...
        const issuedBy = req.headers['x-user-id'] || 'anonymous';
...
app.post('/plans', async (req, res) => {
    try {
        ...
        const commander = req.headers['x-user-id'] || 'anonymous';
...
app.post('/plans/:id/approve', async (req, res) => {
    const approver = req.headers['x-user-id'] || 'anonymous';
...
app.post('/plans/:id/activate', async (req, res) => {
    const activator = req.headers['x-user-id'] || 'anonymous';
...
app.post('/decisions', async (req, res) => {
    try {
        ...
        const decisionMaker = req.headers['x-user-id'] || 'anonymous';
        const decision = await logDecision(decisionMaker, decisionType, context, options, selectedOption, rationale);

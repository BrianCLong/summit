...
describe("cooperation modes", () => {
  it("runs Auction-of-Experts and selects winners under constraints", () => {
    const result = runAuctionOfExperts(
      ...
      { costBudgetUsd: 0.2, latencyBudgetMs: 900, requiredSkills: ["test", "coverage"] }
    );
  ...
  it("weaves semantic braid and flags inconsistencies", () => {
    const braid = weaveSemanticBraid(
      [
        ...
        { kind: "tests", content: "Adds test MAESTRO-WORKER-FLAKE", references: ["MAESTRO-WORKER"] },
        { kind: "impl", content: "Implementation ensures MAESTRO-WORKER-FLAKE fixed" },
        { kind: "release", content: "Release ensures flakeRate and coverage are improved" },
      ],
      { acceptanceMetrics: ["flakeRate", "coverage"], declaredTargets: ["MAESTRO-WORKER"] }
    );
  ...
  it("issues proof-of-useful workbook receipts", () => {
    const bundle = issueProofOfUsefulWorkbook(
      "task:router-test",
      [
        { name: "k6", command: "npm run k6", expectedEvidence: "k6-report.json" },

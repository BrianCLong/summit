...
describe("policy router", () => {
  function buildSpec(): BuildTaskSpec {
    return {
      ...
      inputs: [],
      constraints: { latencyP95Ms: 1200, budgetUSD: 1, contextTokensMax: 2000, cache: "prefer" },
      policy: { purpose: "engineering", retention: "standard-365d", licenseClass: "MIT-OK", pii: false },
      acceptanceCriteria: [
        { id: "AC1", statement: "flake rate < 1% across 200 runs", verify: "repro-suite", metric: "flakeRate", threshold: 0.01 },
        { id: "AC2", statement: "p95 build latency <= 900ms", verify: "latency-metrics", metric: "buildLatencyP95Ms", threshold: 900 },
      ],
      risks: ["regression"],
  ...
  it("selects high-value bundle under cost and latency constraints", () => {
    ...
    const decision = routeTask(spec, registry, { costBudgetUsd: 0.5, latencyBudgetMs: 1000 });
  ...
  it("falls back when budgets make bids infeasible", () => {
    const registry = new CapabilityRegistry();
    const spec = buildSpec();
    const decision = routeTask(spec, registry, { costBudgetUsd: 0.01, latencyBudgetMs: 100 });

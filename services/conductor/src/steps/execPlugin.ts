...
export const execPlugin = {
  async withDigest(ctx:any, step:any, plugin:{ digest:string }, inputs:string[], params:any, env: Record<string, string>, options: { deterministic: boolean }) {
    const key = stepCacheKey({ pluginDigest: plugin.digest, inputDigests: inputs, params });
    const cached = await getCached(key);
    if (cached) { await ctx.attachArtifacts(cached); ctx.log("info","cache.hit",{key}); return; }
    ...
    const res = await runWasm({ wasmPath: "path/to/plugin.wasm", args: [], env: env, caps: {}, limits: {} }); // Dummy call, needs actual implementation

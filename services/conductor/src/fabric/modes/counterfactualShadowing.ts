...
export function buildCounterfactualShadow(observations: FlakeObservation[], budget: number): ShadowingResult {
  const sorted = observations.sort((a, b) => b.failureRate - a.failureRate).slice(0, budget);
...
function deriveSeed(obs: FlakeObservation): CounterfactualSeed {
  ...
  if (obs.env.parallelism !== undefined) adjustments.parallelism = Math.max(1, obs.env.parallelism * 2);
  if (obs.env.clockSkewMs !== undefined) adjustments.clockSkewMs = (obs.env.clockSkewMs || 0) + 150;

...
exports.weaveSemanticBraid = weaveSemanticBraid;
function weaveSemanticBraid(strands, constraints) {
    const issues = [];
    const proofs = [];
    const textParts = [];
    const byKind = new Map();
    for (const strand of strands)
        byKind.set(strand.kind, strand);
    if (!byKind.has('spec'))
        issues.push('Missing spec strand');
    if (!byKind.has('tests'))
        issues.push('Missing tests strand');
    if (byKind.get('tests')) {
...
function validateTestTargets(tests, declared) {

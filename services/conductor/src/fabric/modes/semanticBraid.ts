...
export function weaveSemanticBraid(strands: SemanticStrand[], constraints: BraidConstraints): BraidResult {
  const issues: string[] = [];
  ...
  if (byKind.get("tests")) {
    const invalidRefs = validateTestTargets(byKind.get("tests")!, constraints.declaredTargets);
    if (invalidRefs.length) {
      issues.push(`Tests reference undeclared targets: ${invalidRefs.join(", ")}`);
    } else {
      proofs.push({ strand: "tests", evidence: "All test refs map to declared targets" });
    }
  }
  ...
  if (byKind.get("impl") && byKind.get("tests")) {
    ...
    if (missing.length) {
      issues.push(`Implementation does not reference tests: ${missing.join(", ")}`);
    } else {
      proofs.push({ strand: "impl", evidence: "Implementation references all test cases" });
    }
  }
  ...
  if (byKind.get("risk")) textParts.push(`# Risk\n${byKind.get("risk")!.content}`);
  if (byKind.get("tests")) textParts.push(`# Tests\n${byKind.get("tests")!.content}`);
  if (byKind.get("impl")) textParts.push(`# Implementation\n${byKind.get("impl")!.content}`);
  if (byKind.get("release")) textParts.push(`# Release\n${byKind.get("release")!.content}`);

  return { consolidated: textParts.join("\n\n"), issues, consistencyProof: proofs };
}

function validateTestTargets(tests: SemanticStrand, declared: string[]): string[] {
  const refs = tests.references?.length ? tests.references : extractRefs(tests.content);

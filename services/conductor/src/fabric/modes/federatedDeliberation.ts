...
export function federatedConsensus(votes: RegionalVote[], quorum: number): QuorumResult {
  ...
  return {
    ...
    breakdown: votes.filter((vote) => vote.proposalId === winningProposalId).map((vote) => ({
      region: vote.region,
      weight: vote.weight,
      score: vote.score,
    })),

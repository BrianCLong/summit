export function enforceDlp(findings:any[], mode:"block"|"redact"|"allow", content:Buffer){
  ...
  s = s.replace(/\d{3}-\d{2}-\d{4}/g, "XXX-XX-XXXX").replace(/\b(?:\d[ -]*?){13,16}\b/g, "XXXX-XXXX-XXXX-XXXX");

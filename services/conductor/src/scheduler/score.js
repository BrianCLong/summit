...
function scorePool(pool, req, ctx) {
    ...
    return (0.55 * normalizePrice(price) +
        0.2 * qlag +
        0.15 * queuePen +
        0.07 * residency +
        0.03 * fitGpu);
...
function satisfiesGpu(pool, req) {
    if (!pool.gpu || !req.resources.gpus)
        return false;
    const clsOk = !req.resources.gpuClass ||

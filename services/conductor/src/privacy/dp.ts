...
export async function chargeEpsilon(tenant:string, dataset:string, eps:number){
  ...
  if (next > Number(b.epsilon_limit)) { await pg.query("ROLLBACK"); return { ok:false, level:"exhausted" }; }
  await pg.query(`UPDATE dp_budgets SET epsilon_spent=$4 WHERE tenant=$1 AND dataset=$2 AND month=$3`, [tenant, dataset, ym, next]);
  await pg.query("COMMIT"); return { ok:true, remaining: Number(b.epsilon_limit)-next };
}
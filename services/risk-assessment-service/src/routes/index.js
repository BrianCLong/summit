...
const supplier_risk_1 = require("@intelgraph/supplier-risk");
const third_party_risk_1 = require("@intelgraph/third-party-risk");
const compliance_monitor_1 = require("@intelgraph/compliance-monitor");
...
// Initialize services
const supplierRiskAssessor = new supplier_risk_1.SupplierRiskAssessor();
const vendorManager = new third_party_risk_1.ThirdPartyRiskManager();
const complianceMonitor = new compliance_monitor_1.ComplianceMonitor();
function setupRoutes(app) {
    // ============================================================================
    // Supplier Risk Assessment Routes
    // ============================================================================
    // Assess supplier risk
    app.post('/api/risk/supplier/:nodeId/assess', async (req, res) => {
        ...
        const assessment = await supplierRiskAssessor.assessSupplier(node, financialMetrics, cyberPosture, esgScore);
        // Store assessment
    ...
    app.post('/api/risk/cybersecurity/:nodeId', (req, res) => {
        const { nodeId } = req.params;
    ...
    app.post('/api/vendor/onboard', (req, res) => {
    ...
    app.post('/api/vendor/:vendorId/assess', async (req, res) => {
        ...
        const assessment = await vendorManager.conductAssessment(node, assessmentType || 'periodic-review');
        res.json(assessment);
    });
    // Monitor vendor
    app.post('/api/vendor/:vendorId/monitor', async (req, res) => {
        const { vendorId } = req.params;
    ...
    app.post('/api/compliance/:nodeId/check', async (req, res) => {
        const { nodeId } = req.params;
        const node = nodes.get(nodeId);
        if (!node) {
            return res.status(404).json({ error: 'Node not found' });
    ...
    app.post('/api/compliance/export-control/screen', async (req, res) => {
    ...
    app.post('/api/compliance/conflict-minerals/:componentId', async (req, res) => {
    ...
    app.get('/api/compliance/regulatory-changes', async (req, res) => {
        const { jurisdiction } = req.query;
        const changes = await complianceMonitor.trackRegulatoryChanges(jurisdiction || 'United States');
    ...
    app.get('/api/alerts', (req, res) => {
        ...
        if (resolved !== undefined) {
            results = results.filter(a => resolved === 'true' ? a.resolvedAt !== undefined : a.resolvedAt === undefined);
        }
    ...
    app.get('/api/predictions/disruptions', (req, res) => {
    ...
    // ============================================================================
    // Get risk summary
    app.get('/api/analytics/risk-summary', (req, res) => {
    ...
    app.get('/api/analytics/incident-stats', (req, res) => {

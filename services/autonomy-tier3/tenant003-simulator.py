...
class AutonomyTier3Simulator:
    ...
    def _calculate_operation_success_rate(self, operation_type: str, context: Dict[str, Any]) -> float:
    ...
    def run_simulation(self, num_operations: int = 1000, operation_mix: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        ...
        compensation_rate = (len(self.compensation_events) / total_operations * 100) if total_operations > 0 else 0

        # Safety assessment
        safety_passed = (
            success_rate >= self.success_threshold and
            compensation_rate <= self.compensation_threshold and
            len([v for v in self.safety_violations if v["violation_type"] == "residency"]) == 0
        )
        ...
        simulation_results = {
            ...
            "results": {
                ...
                "policy_violations": len([v for v in self.safety_violations if v["violation_type"] == "policy"]),
                "residency_violations": len([v for v in self.safety_violations if v["violation_type"] == "residency"]),
                "avg_execution_time_ms": sum(op["execution_time_ms"] for op in self.operation_log) / total_operations if total_operations > 0 else 0
            },
            "safety_assessment": {
                "success_rate_meets_threshold": success_rate >= self.success_threshold,
                "compensation_rate_meets_threshold": compensation_rate <= self.compensation_threshold,
                "no_residency_violations": len([v for v in self.safety_violations if v["violation_type"] == "residency"]) == 0,
        ...
        print(f"   Compensation rate: {compensation_rate:.2f}% (target: ≤{self.compensation_threshold}%)")
        print(f"   Residency violations: {len([v for v in self.safety_violations if v['violation_type'] == 'residency'])}")
    ...
    def _generate_recommendations(self, success_rate: float, compensation_rate: float) -> List[str]:
        ...
        if success_rate < self.success_threshold:
            recommendations.append(f"SUCCESS RATE: {success_rate:.2f}% below {self.success_threshold}% threshold - review operation reliability")

        if compensation_rate > self.compensation_threshold:
            recommendations.append(f"COMPENSATION RATE: {compensation_rate:.2f}% above {self.compensation_threshold}% threshold - investigate failure patterns")

        if len(self.safety_violations) > 0:
            recommendations.append(f"SAFETY VIOLATIONS: {len(self.safety_violations)} violations detected - strengthen policy enforcement")

        if len([v for v in self.safety_violations if v["violation_type"] == "residency"]) > 0:
            recommendations.append("RESIDENCY VIOLATIONS: Critical issue - halt enactment immediately")
...
def main():
    ...
    parser.add_argument("--operations", type=int, default=1000, help="Number of operations to simulate")
    parser.add_argument("--output", default="evidence/v0.3.3/autonomy/TENANT_003", help="Output directory for evidence")

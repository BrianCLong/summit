...
def load_graph():
    uri=os.environ["NEO4J_URI"]; auth=(os.environ["NEO4J_USER"], os.environ["NEO4J_PASS"])
    ...
    idx_edges=torch.tensor([[id_map[u] for u,v in G.edges()],[id_map[v] for u,v in G.edges()]], dtype=torch.long)
...
def main():
    ...
    model=SAGE(x.size(1)); opt=torch.optim.Adam(model.parameters(), lr=3e-4)
    ...
    with open("models/meta.json","w") as f: json.dump({"trained_at": int(time.time()), "in_dim": x.size(1)}, f)

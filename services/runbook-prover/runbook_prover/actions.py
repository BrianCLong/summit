...
class ActionRegistry:
    ...
    def cluster_iocs(self, context: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized = {
            k: {"events": v["events"], "iocs": sorted(v["iocs"])}
    ...
    def score_attribution(self, context: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for name, cluster in clusters.items():
            scores[name] = round(cluster.get("events", 0) * 0.25 + len(cluster.get("iocs", [])) * 0.5, 2)
        leading = sorted(scores.items(), key=lambda item: item[1], reverse=True)
        return {"attribution_scores": scores, "leading_hypothesis": leading[0][0] if leading else None}

    def extract_phishing_clusters(self, context: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
        dataset = context.get("dataset", [])
        clusters: Dict[str, Dict[str, Any]] = {}
        for entry in dataset:
        ...
        normalized = {k: {"messages": v["messages"], "senders": sorted(v["senders"])} for k, v in clusters.items()}
        return {"phishing_clusters": normalized}

    def score_phishing_risk(self, context: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
        clusters = context.get("phishing_clusters", {})
        risk_scores = {}
        for theme, cluster in clusters.items():
            risk_scores[theme] = round(cluster.get("messages", 0) * 0.1 + len(cluster.get("senders", [])) * 0.4, 2)
    ...
    def assert_kpis(self, context: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for kpi in kpis:
            ...
            achieved.append({"id": kpi.get("id"), "met": bool(result), "measured": measured, "target": target})

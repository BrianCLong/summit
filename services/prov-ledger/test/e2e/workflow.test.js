...
(0, globals_1.describe)('E2E: Complete Provenance Workflow', () => {
    let caseId;
    let evidenceItems = [];
    let bundleData;
    ...
    (0, globals_1.it)('Step 1: Create a new case for investigation', async () => {
       ...
       VALUES ($1, $2, $3, $4, $5)`, [
        ...
        (0, globals_1.expect)(result.rows[0].title).toContain('E2E Test Case');
    });
    (0, globals_1.it)('Step 2: Ingest and register initial evidence', async () => {
        const rawDocuments = [
            {
        ...
        for (const doc of rawDocuments) {
            ...
            (0, globals_1.expect)(evidenceResponse.status).toBe(200);
            (0, globals_1.expect)(evidenceResponse.data.id).toMatch(/^evidence_/);
            (0, globals_1.expect)(evidenceResponse.data.checksum).toBe(checksum);
            evidenceItems.push({
        ...
        (0, globals_1.expect)(evidenceItems).toHaveLength(3);
    });
    (0, globals_1.it)('Step 3: Register transformed evidence with provenance chain', async () => {
        // Simulate OCR transformation on the image
        const originalImage = evidenceItems.find((e) => e.filename.includes('photo'));
        ...
        (0, globals_1.expect)(transformedEvidence.status).toBe(200);
        (0, globals_1.expect)(transformedEvidence.data.transformChain).toHaveLength(1);
        (0, globals_1.expect)(transformedEvidence.data.transformChain[0].transformType).toBe('ocr');
        evidenceItems.push({
    ...
    (0, globals_1.it)('Step 4: Generate disclosure bundle manifest', async () => {
        const bundleResponse = await client.get(`/bundles/${caseId}`);
        (0, globals_1.expect)(bundleResponse.status).toBe(200);
        bundleData = bundleResponse.data;
        (0, globals_1.expect)(bundleData.caseId).toBe(caseId);
        (0, globals_1.expect)(bundleData.version).toBe('1.0');
        (0, globals_1.expect)(bundleData.evidence).toHaveLength(4); // 3 original + 1 transformed
        (0, globals_1.expect)(bundleData.hashTree).toHaveLength(4);
        (0, globals_1.expect)(bundleData.merkleRoot).toBeTruthy();
        (0, globals_1.expect)(bundleData.merkleRoot).toMatch(/^[a-f0-9]+$/);
    });
    (0, globals_1.it)('Step 5: Verify hash integrity of all evidence', async () => {
        ...
        for (const evidence of bundleData.evidence) {
            ...
            if (originalItem) {
                (0, globals_1.expect)(evidence.checksum).toBe(originalItem.checksum);
                // Verify checksum is in hash tree
                (0, globals_1.expect)(bundleData.hashTree).toContain(evidence.checksum);
            }
        }
    });
    (0, globals_1.it)('Step 6: Verify transform chain provenance', async () => {
        // Find the transformed evidence in the bundle
        const transformedEvidence = bundleData.evidence.find((e) => e.transformChain && e.transformChain.length > 0);
        (0, globals_1.expect)(transformedEvidence).toBeDefined();
        (0, globals_1.expect)(transformedEvidence.transformChain).toHaveLength(1);
        const transform = transformedEvidence.transformChain[0];
        (0, globals_1.expect)(transform.transformType).toBe('ocr');
        (0, globals_1.expect)(transform.actorId).toBe('ocr-service-v2');
        (0, globals_1.expect)(transform).toHaveProperty('timestamp');
        // Verify we can trace back to original evidence
        const originalImageId = evidenceItems.find((e) => e.filename.includes('photo')).id;
        (0, globals_1.expect)(transform.config.sourceEvidence).toBe(originalImageId);
    });
    (0, globals_1.it)('Step 7: Verify hash using verification endpoint', async () => {
        ...
        (0, globals_1.expect)(verifyResponse.data.actual_hash).toBe(originalDoc.checksum);
    });
    (0, globals_1.it)('Step 8: Verify bundle can be regenerated with same merkle root', async () => {
        // Generate bundle again
        const bundle2Response = await client.get(`/bundles/${caseId}`);
        (0, globals_1.expect)(bundle2Response.status).toBe(200);
        // Merkle root should be identical (deterministic)
        (0, globals_1.expect)(bundle2Response.data.merkleRoot).toBe(bundleData.merkleRoot);
        // Hash tree should contain same hashes in same order
        (0, globals_1.expect)(bundle2Response.data.hashTree).toEqual(bundleData.hashTree);
    });
    (0, globals_1.it)('Step 9: Register a claim based on the evidence', async () => {
        ...
        (0, globals_1.expect)(claimResponse.status).toBe(200);
        (0, globals_1.expect)(claimResponse.data.id).toMatch(/^claim_/);
        ...
        (0, globals_1.expect)(provenanceResponse.status).toBe(200);
        (0, globals_1.expect)(provenanceResponse.data.claim_id).toBe(claimResponse.data.id);
    });
    (0, globals_1.it)('Step 10: Export complete manifest', async () => {
        const manifestResponse = await client.get('/export/manifest');
        (0, globals_1.expect)(manifestResponse.status).toBe(200);
        (0, globals_1.expect)(manifestResponse.data.version).toBeTruthy();
        (0, globals_1.expect)(manifestResponse.data.claims).toBeDefined();
        (0, globals_1.expect)(manifestResponse.data.hash_chain).toBeTruthy();
        (0, globals_1.expect)(manifestResponse.data.generated_at).toBeTruthy();
    });

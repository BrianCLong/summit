...
const ledger_js_1 = require("../lib/ledger.js");
function usage() {
...
async function main() {
    ...
    switch (command) {
        case 'ingest': {
            const file = rest[0];
            if (!file)
                throw new Error('ingest requires a file');
            const content = readFileContent(file);
            const record = ledger_js_1.ledger.addEvidence({ content, mediaType: path_1.default.extname(file).replace('.', '') });
        ...
        case 'claim': {
            ...
            if (evidenceFlag === -1 || assertionFlag === -1)
                throw new Error('claim requires --evidence and --assertion');
            const evidenceIds = rest[evidenceFlag + 1].split(',');
            const assertion = JSON.parse(rest[assertionFlag + 1]);
            const claim = ledger_js_1.ledger.addClaim({ evidenceIds, assertion });
        ...
        case 'export': {
            ...
            if (claimFlag === -1)
                throw new Error('export requires --claim');
            const claimId = rest[claimFlag + 1];
            const target = outFlag >= 0 ? rest[outFlag + 1] : 'manifest.json';
            const { manifest, serialized } = ledger_js_1.ledger.exportManifest(claimId);
            fs_1.default.writeFileSync(target, serialized);
        ...
        case 'verify': {
            const file = rest[0];
            if (!file)
                throw new Error('verify requires manifest path');

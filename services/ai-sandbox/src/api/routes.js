...
async function registerRoutes(app, taskQueue) {
    ...
    app.get('/metrics', async (_, reply) => {
        reply.header('Content-Type', metrics_js_1.registry.contentType);
    ...
    app.post('/api/v1/environments', async (request, reply) => {
        ...
        const environmentData = {
            ...
            resourceQuotas: types_js_1.ResourceQuotasSchema.parse(body.resourceQuotas || {}),
        ...
        const environment = types_js_1.SandboxEnvironmentSchema.parse(environmentData);
        environments.set(environment.id, environment);
        logger_js_1.logger.info({ environmentId: environment.id }, 'Sandbox environment created');
        reply.code(201);
        return environment;
    });
    // Get environment
    ...
    app.post('/api/v1/experiments', async (request, reply) => {
        ...
        try {
            experimentRequest = types_js_1.ExperimentRequestSchema.parse(body);
        }
        catch (error) {
            reply.code(400);
        ...
        const taskId = await taskQueue.submit({
            ...
            submittedBy: request.headers['x-user-id'] || 'anonymous',
        });
        reply.code(202);
        return {
            taskId,
            status: 'pending',
            policyWarnings: policyDecision.violations.filter((v) => v.severity !== 'critical'),
            recommendations: policyDecision.recommendations,
        };
    });
    ...
    app.post('/api/v1/deployments', async (request, reply) => {
        ...
        try {
            deploymentRequest = types_js_1.DeploymentRequestSchema.parse(body);
        }
        catch (error) {
            reply.code(400);
        ...
        const deployPolicy = await policyEngine.validateDeployment(deploymentRequest.experimentId, deploymentRequest.targetEnvironment);
        ...
        logger_js_1.logger.info({ deploymentId: deployment.id, target: deployment.targetEnvironment }, 'Deployment requested');
        reply.code(202);
        return deployment;
    });
    // === Queue Statistics ===

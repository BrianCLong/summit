...
class BallotLedger {
    ...
    recordBallot(ballot) {
        ...
        const isDuplicate = this.chain.some((block) => block.ballots.some((b) => b.ballotId === ballot.ballotId)) || this.pendingBallots.some((b) => b.ballotId === ballot.ballotId);
        if (isDuplicate) {
     ...
     */
    getStats() {
        const totalBallots = this.chain.reduce((sum, block) => sum + block.ballots.length, 0) + this.pendingBallots.length;
        return {
    ...
    computeMerkleRoot(ballots) {
        if (ballots.length === 0) {
            return node_crypto_1.default.createHash('sha256').update('empty').digest('hex');
        }
    ...
    generateMerkleProof(ballots, index) {
        const proof = [];
        let hashes = ballots.map((b) => this.hashBallot(b));
        let idx = index;
        while (hashes.length > 1) {
            ...
            for (let i = 0; i < hashes.length; i += 2) {
                const left = hashes[i];
                const right = hashes[i + 1] || left;
                newHashes.push(node_crypto_1.default.createHash('sha256').update(left + right).digest('hex'));
            }
            hashes = newHashes;
...
exports.BallotLedger = BallotLedger;

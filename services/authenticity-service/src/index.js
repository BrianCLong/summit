...
const provenanceTracker = new media_manipulation_1.ProvenanceTracker();
const metadataAnalyzer = new media_manipulation_1.MetadataAnalyzer();
// Health check
...
app.post('/api/verify/comprehensive', async (req, res) => {
    try {
        ...
        const scores = [];
        if (results.content)
            scores.push(results.content.confidence);
        if (results.provenance)
            scores.push(results.provenance.authenticity);
        if (results.metadata)
            scores.push(results.metadata.trustScore);

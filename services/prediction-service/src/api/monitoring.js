 ...
 */
function recordPrediction(modelId, latencyMs, error = false) {
    metrics.predictions.total++;
    metrics.predictions.byModel.set(modelId, (metrics.predictions.byModel.get(modelId) || 0) + 1);
 ...
 */
function createMonitoringRoutes(engine, registry) {
    ...
    router.get('/metrics', (req, res) => {
        ...
        for (const [modelId, count] of metrics.predictions.byModel.entries()) {
            prometheusMetrics.push(`predictions_by_model{model_id="${modelId}"} ${count}`);
        }
     ...
     */
    router.get('/drift/:modelId', (req, res) => {
     ...
     */
    router.post('/drift/:modelId', (req, res) => {
        const { modelId } = req.params;
 ...
 */
function calculateDriftTrend(history) {
    if (history.length < 2) {

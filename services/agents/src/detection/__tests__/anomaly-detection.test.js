...
const IsolationForestDetector_js_1 = require("../IsolationForestDetector.js");
const GraphDiffusionDetector_js_1 = require("../GraphDiffusionDetector.js");
const StreamProcessor_js_1 = require("../StreamProcessor.js");
const AlertingAgent_js_1 = require("../AlertingAgent.js");
const AnomalyDetectionService_js_1 = require("../AnomalyDetectionService.js");
// Test data generators
function generateNormalFeatureVector(id) {
...
function generateAnomalousFeatureVector(id) {
...
function generateTrainingData(normalCount, anomalyCount) {
    const data = [];
...
function generateGraphData(nodeCount) {
    ...
    for (const node of nodes) {
        node.degree = edges.filter((e) => e.sourceId === node.id || e.targetId === node.id).length;
    }
    return { nodes, edges };
}
(0, globals_1.describe)('IsolationForestDetector', () => {
    let detector;
    (0, globals_1.beforeAll)(() => {
        detector = new IsolationForestDetector_js_1.IsolationForestDetector({
            numTrees: 100,
            subsampleSize: 256,
            contamination: 0.1,
    ...
    (0, globals_1.it)('should train on normal data', async () => {
        const trainingData = generateTrainingData(900, 100); // 10% contamination
        await detector.fit(trainingData);
        (0, globals_1.expect)(detector.isTrained()).toBe(true);
        (0, globals_1.expect)(detector.getState()).toBe('ready');
    });
    (0, globals_1.it)('should detect anomalies with high precision', async () => {
        const trainingData = generateTrainingData(900, 100);
        await detector.fit(trainingData);
        // Test on new data
        const testNormal = Array.from({ length: 100 }, (_, i) => generateNormalFeatureVector(`test-normal-${i}`));
        const testAnomalous = Array.from({ length: 100 }, (_, i) => generateAnomalousFeatureVector(`test-anomaly-${i}`));
        ...
        // Target: 91% precision
        (0, globals_1.expect)(precision).toBeGreaterThanOrEqual(0.85); // Allow some margin for randomness
    });
    (0, globals_1.it)('should meet p95 latency target of 500ms', async () => {
        ...
        // Target: p95 < 500ms
        (0, globals_1.expect)(p95).toBeLessThan(500);
    });
    (0, globals_1.it)('should support partial fitting for incremental updates', async () => {
        ...
        const testAnomalous = Array.from({ length: 20 }, (_, i) => generateAnomalousFeatureVector(`partial-test-${i}`));
        const results = await detector.detect(testAnomalous);
        const detected = results.filter((r) => r.isAnomaly).length;
        (0, globals_1.expect)(detected).toBeGreaterThan(10); // Should detect most
    });
    (0, globals_1.it)('should serialize and deserialize correctly', async () => {
        const trainingData = generateTrainingData(500, 50);
        await detector.fit(trainingData);
        const serialized = detector.serialize();
        const restored = IsolationForestDetector_js_1.IsolationForestDetector.deserialize(serialized);
        (0, globals_1.expect)(restored.isTrained()).toBe(true);
        (0, globals_1.expect)(restored.getConfig()).toEqual(detector.getConfig());
        ...
        for (let i = 0; i < originalResults.length; i++) {
            (0, globals_1.expect)(restoredResults[i].score).toBeCloseTo(originalResults[i].score, 5);
        }
    });
});
(0, globals_1.describe)('GraphDiffusionDetector', () => {
    let detector;
    let graphData;
    (0, globals_1.beforeAll)(async () => {
        detector = new GraphDiffusionDetector_js_1.GraphDiffusionDetector({
            diffusionSteps: 5,
            dampingFactor: 0.85,
    ...
    (0, globals_1.it)('should train on graph data', () => {
        (0, globals_1.expect)(detector.isTrained()).toBe(true);
        (0, globals_1.expect)(detector.getState()).toBe('ready');
    });
    (0, globals_1.it)('should detect graph-based anomalies', async () => {
        ...
        (0, globals_1.expect)(avgAnomalyScore).toBeGreaterThan(avgNormalScore);
    });
    (0, globals_1.it)('should meet p95 latency target', async () => {
        const latencies = [];
        const batchSize = 50;
        const numBatches = 30;
        for (let i = 0; i < numBatches; i++) {
    ...
    (0, globals_1.it)('should support incremental node addition', async () => {
        const newNodes = [
            {
                id: 'new-node-1',
                type: 'normal',
        ...
        (0, globals_1.expect)(results[0].featureId).toBe('new-node-1');
    });
});
(0, globals_1.describe)('StreamProcessor', () => {
    (0, globals_1.it)('should convert stream data to feature vectors', () => {
        const points = [
            {
                id: 'point-1',
                sourceType: 'neo4j',
        ...
        const vectors = StreamProcessor_js_1.StreamProcessor.toFeatureVectors(points);
        (0, globals_1.expect)(vectors.length).toBe(2);
        (0, globals_1.expect)(vectors[0].sourceType).toBe('neo4j');
        (0, globals_1.expect)(vectors[0].features.length).toBeGreaterThanOrEqual(10);
        (0, globals_1.expect)(vectors[1].sourceType).toBe('pgvector');
        (0, globals_1.expect)(vectors[1].features.length).toBe(65); // 64 embedding + 1 temporal
    });
});
(0, globals_1.describe)('AlertingAgent', () => {
    let agent;
    (0, globals_1.beforeEach)(() => {
        agent = new AlertingAgent_js_1.AlertingAgent({
            enabled: true,
    ...
    (0, globals_1.it)('should create alerts from anomalies', async () => {
        const anomaly = {
            id: 'anomaly-1',
            entityId: 'entity-1',
            entityType: 'Node',
        ...
        (0, globals_1.expect)(alert).not.toBeNull();
        (0, globals_1.expect)(alert.anomalyId).toBe('anomaly-1');
        (0, globals_1.expect)(alert.severity).toBe('high');
        (0, globals_1.expect)(alert.priority).toBe('p1');
        (0, globals_1.expect)(alert.status).toBe('new');
    });
    (0, globals_1.it)('should deduplicate alerts within window', async () => {
        const anomaly1 = {
            id: 'anomaly-1',
            entityId: 'entity-1',
            entityType: 'Node',
        ...
        (0, globals_1.expect)(alert1).not.toBeNull();
        (0, globals_1.expect)(alert2).not.toBeNull();
        (0, globals_1.expect)(alert2.id).toBe(alert1.id); // Same alert, updated
    });
    (0, globals_1.it)('should throttle alerts per entity', async () => {
        const anomaly = {
            id: 'anomaly-1',
        ...
        const throttledAnomaly = {
            ...anomaly,
            id: 'anomaly-2',
            severity: 'low',
        };
        const throttledAlert = await agent.processAnomaly(throttledAnomaly);
        (0, globals_1.expect)(throttledAlert).toBeNull(); // Should be throttled
    });
});
(0, globals_1.describe)('AnomalyDetectionService E2E', () => {
    let service;
    (0, globals_1.beforeAll)(async () => {
        service = new AnomalyDetectionService_js_1.AnomalyDetectionService({
            detector: {
                isolationForest: {
                    numTrees: 50,
                    subsampleSize: 128,
    ...
    (0, globals_1.it)('should train and detect anomalies end-to-end', async () => {
        ...
        const anomalyPoints = Array.from({ length: 20 }, (_, i) => ({
            id: `anomaly-${i}`,
            sourceType: 'neo4j',
            timestamp: new Date(),
            data: {
        ...
        (0, globals_1.expect)(precision).toBeGreaterThan(0.7);
    });
    (0, globals_1.it)('should meet combined latency requirements', async () => {
        const trainingData = generateTrainingData(500, 50);
        await service.trainManual(trainingData);
        const latencies = [];
    ...
    (0, globals_1.it)('should provide health status', async () => {
        const health = await service.getHealth();
        (0, globals_1.expect)(health.status).toBeDefined();
        (0, globals_1.expect)(health.detectorState).toBeDefined();
        (0, globals_1.expect)(typeof health.queueDepth).toBe('number');
    });
    (0, globals_1.it)('should track metrics correctly', async () => {
        ...
        (0, globals_1.expect)(metrics.totalProcessed).toBeGreaterThanOrEqual(0);
        (0, globals_1.expect)(metrics.avgLatencyMs).toBeGreaterThanOrEqual(0);
        (0, globals_1.expect)(metrics.lastUpdated).toBeInstanceOf(Date);
    });
    (0, globals_1.it)('should handle feedback for precision tracking', () => {
        // Record some feedback
        service.recordFeedback('alert-1', true);
        service.recordFeedback('alert-2', true);
        service.recordFeedback('alert-3', false);
        const metrics = service.getMetrics();
        (0, globals_1.expect)(metrics.truePositives).toBe(2);
        (0, globals_1.expect)(metrics.falsePositives).toBe(1);
        (0, globals_1.expect)(metrics.precision).toBeCloseTo(0.667, 2);
    });
});
(0, globals_1.describe)('Performance Benchmarks', () => {
    (0, globals_1.it)('should handle high throughput', async () => {
        const detector = new IsolationForestDetector_js_1.IsolationForestDetector({
            numTrees: 50,
            subsampleSize: 128,
            randomState: 42,
        });
    ...
    (0, globals_1.it)('should maintain memory efficiency', async () => {
        ...
        (0, globals_1.expect)(detectMemoryMB).toBeLessThan(50);
    });
});

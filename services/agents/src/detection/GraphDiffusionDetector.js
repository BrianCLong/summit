...
exports.GraphDiffusionDetector = void 0;
class GraphDiffusionDetector {
    ...
    globalMeanDensity = 0;
    globalStdDensity = 1;
    globalMeanClustering = 0;
    globalStdClustering = 1;
    constructor(config = {}) {
        this.config = {
    ...
    async computeNodeProfiles(nodes) {
        ...
        for (const node of nodes) {
            ...
            const clusteringCoeff = this.computeClusteringCoefficient(node.id, neighbors);
    ...
    computeClusteringCoefficient(nodeId, neighbors) {
    ...
    initializeEmbedding(node, dimension) {
    ...
    async computeDiffusionScores(nodes) {
        ...
        for (let step = 0; step < this.config.diffusionSteps; step++) {
            ...
            for (const node of nodes) {
                ...
                let sum = 0;
                for (const neighbor of neighbors) {
                    const neighborDegree = (this.adjacencyMap.get(neighbor.targetId) || []).length;
                    if (neighborDegree > 0) {
    ...
    async detect(nodeIds) {
        ...
        for (const nodeId of nodeIds) {
            ...
            const combinedScore = 0.4 * diffusionScore + 0.35 * structuralScore + 0.25 * neighborScore;
            // Compute contributing features
            const contributions = this.computeContributions(profile, diffusionScore, structuralScore, neighborScore);
            results.push({
                featureId: nodeId,
                score: combinedScore,
        ...
        if (latencyMs > 500) {
            console.warn(`[GraphDiffusion] Detection exceeded 500ms: ${latencyMs}ms for ${nodeIds.length} nodes`);
        }
        this.state = 'ready';
        return results;
    }
    async computeDiffusionAnomalyScore(nodeId) {
        const profile = this.nodeProfiles.get(nodeId);
        if (!profile)
            return 0;
        ...
        const expectedReturnProb = 1 / (this.adjacencyMap.get(nodeId) || []).length || 0.5;
        const deviation = Math.abs(returnProb - expectedReturnProb);
        return Math.min(1, deviation * 2);
    }
    computeStructuralAnomalyScore(profile) {
        // Z-score based anomaly detection for structural features
        const densityZ = Math.abs((profile.localDensity - this.globalMeanDensity) / this.globalStdDensity);
        const clusteringZ = Math.abs((profile.clusteringCoeff - this.globalMeanClustering) /
    ...
    computeNeighborhoodAnomalyScore(nodeId) {
        const profile = this.nodeProfiles.get(nodeId);
        if (!profile)
            return 0;
        const neighbors = this.adjacencyMap.get(nodeId) || [];
        if (neighbors.length === 0)
            return 0.5; // Isolated node is suspicious
    ...
    computeContributions(profile, diffusionScore, structuralScore, neighborScore) {
        const contributions = [];
        if (diffusionScore > 0.5) {
            contributions.push({
                ...
                direction: profile.pageRank > this.mean([...this.nodeProfiles.values()].map((p) => p.pageRank))
                    ? 'high'
                    : 'low',
            });
        }
        if (structuralScore > 0.5) {
            contributions.push({
                ...
                direction: profile.localDensity > this.globalMeanDensity ? 'high' : 'low',
            });
        }
...
exports.GraphDiffusionDetector = GraphDiffusionDetector;

...
exports.IsolationForestDetector = void 0;
/**
...
class IsolationForestDetector {
    ...
    computeFeatureStatistics(features, numFeatures) {
        ...
        for (const feature of features) {
            for (let j = 0; j < numFeatures; j++) {
                this.featureStds[j] += Math.pow(feature[j] - this.featureMeans[j], 2);
            }
        }
        for (let j = 0; j < numFeatures; j++) {
    ...
    async detect(data) {
        ...
        for (const vector of data) {
            ...
            const contributions = this.computeFeatureContributions(normalized, vector.features);
        ...
        if (latencyMs > 500) {
            console.warn(`[IsolationForest] Batch detection exceeded 500ms: ${latencyMs}ms for ${data.length} points`);
        }
        this.state = 'ready';
        return results;
    }
     ...
     */
    async detectSingle(vector) {
        const results = await this.detect([vector]);
        return results[0];
    }
    computeFeatureContributions(normalized, original) {
        const contributions = [];
    ...
    async partialFit(newData) {
        ...
        for (let i = 0; i < numToReplace; i++) {
            const subsample = this.getSubsample(normalizedFeatures, Math.min(this.config.subsampleSize, features.length));
            const tree = this.buildTree(subsample, 0, this.config.maxDepth);
...
exports.IsolationForestDetector = IsolationForestDetector;

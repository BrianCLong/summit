...
class AnomalyDetectionService {
    ...
    constructor(config = {}) {
        ...
        this.isolationForest = new IsolationForestDetector_js_1.IsolationForestDetector(this.config.detector.isolationForest);
        this.graphDiffusion = new GraphDiffusionDetector_js_1.GraphDiffusionDetector(this.config.detector.graphDiffusion);
        this.streamProcessor = new StreamProcessor_js_1.StreamProcessor({
            redisUrl: this.config.redis.url,
            batchSize: this.config.detector.performance.batchSize,
        });
        this.alertingAgent = new AlertingAgent_js_1.AlertingAgent(this.config.alerting);
    }
    mergeConfig(config) {
        return {
            detector: {
     ...
     */
    async processStreamData(points) {
        const startTime = Date.now();
        // Convert to feature vectors
        const featureVectors = StreamProcessor_js_1.StreamProcessor.toFeatureVectors(points);
        // During warmup, just collect training data
        if (!this.warmupComplete) {
            this.trainingBuffer.push(...featureVectors);
            if (Date.now() - this.startTime > this.config.training.warmupPeriodMs &&
        ...
        this.trainingBuffer.push(...featureVectors.slice(0, Math.min(10, featureVectors.length)));
    }
    /**
     * Process graph structure updates
     */
    async processGraphUpdate(nodes, edges) {
     ...
     */
    async trainDetectors() {
        console.log(`[AnomalyDetectionService] Training detectors with ${this.trainingBuffer.length} samples...`);
        ...
        if (this.graphNodesBuffer.length > 0) {
            await this.graphDiffusion.fit(this.graphNodesBuffer, this.graphEdgesBuffer);
        }
    ...
    startRetrainingLoop() {
        const retrain = async () => {
            ...
            if (timeSinceRetrain >= this.config.training.retrainIntervalMs &&
                this.trainingBuffer.length >= this.config.training.minSamples) {
                // Partial fit for incremental updates
                await this.isolationForest.partialFit(this.trainingBuffer.slice(-this.config.training.minSamples));
                this.trainingBuffer = [];
                this.lastRetrainTime = Date.now();
            }
     ...
     */
    async detectAnomalies(featureVectors) {
        const allScores = [];
    ...
    processAnomalyScores(scores, vectors) {
        ...
        for (const [featureId, featureScores] of scoresByFeature) {
            ...
            if (combinedScore.score >= this.config.detector.thresholds.anomalyScoreThreshold &&
    ...
    inferEntityType(vector) {
        if (!vector)
            return 'unknown';
        if (vector.sourceType === 'neo4j') {
            return vector.metadata.nodeType || 'graph_node';
        }
    ...
    generateDescription(combined, scores) {
        ...
        const topContributors = scores
            ...
            .join(', ');
        return (`Anomaly detected with ${(combined.score * 100).toFixed(1)}% score ` +
            `(${(combined.confidence * 100).toFixed(1)}% confidence) by ${detectors}. ` +
            `Key contributors: ${topContributors || 'multiple features'}.`);
    }
    gatherEvidence(scores) {
        return scores.flatMap((score) => score.contributingFeatures.map((cf) => ({
            type: 'feature',
    ...
    suggestActions(severity, scores) {
        const actions = [];
    ...
    updateMetrics(processed, anomalies, latencyMs) {
     ...
     */
    recordFeedback(alertId, isTruePositive) {
        if (isTruePositive) {
            this.metrics.truePositives++;
        }
        else {
    ...
    async getHealth() {
        ...
        if (ifState === 'error')
            errors.push('Isolation Forest in error state');
        if (gdState === 'error')
            errors.push('Graph Diffusion in error state');
     ...
     */
    async detectManual(data) {
        const featureVectors = StreamProcessor_js_1.StreamProcessor.toFeatureVectors(data);

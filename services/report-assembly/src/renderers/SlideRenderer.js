...
const defaultTemplates_js_1 = require("../templates/defaultTemplates.js");
...
class SlideRenderer {
     ...
     */
    async render(slides, context, options = {}) {
     ...
     */
    generateSlidesFromBriefing(context, data) {
        const slides = [];
        let order = 1;
        // Title slide
        slides.push({
     ...
     */
    renderSlide(slide, context, options) {
     ...
     */
    wrapInPresentation(slidesHtml, context, options) {
...
<body>
  ...
  <script>
     ...
     */
    loadSlideTemplates() {
        for (const template of defaultTemplates_js_1.defaultSlideTemplates) {
            this.slideTemplates.set(`${template.layout}-slide`, handlebars_1.default.compile(template.template));
        }
    }
    /**

...
async function mergePdfs(sections) {
    const mergedPdf = await pdf_lib_1.PDFDocument.create();
    // Normalize input
    const inputs = sections.map(s => Buffer.isBuffer(s) ? { buffer: s, title: 'Section' } : s);
    // TOC Data
    const tocEntries = [];
    ...
    for (const section of inputs) {
        const pdf = await pdf_lib_1.PDFDocument.load(section.buffer);
    ...
    if (tocEntries.length > 0) {
        ...
        const titleFont = await mergedPdf.embedFont(pdf_lib_1.StandardFonts.HelveticaBold);

...
const mockCacheSet = globals_1.jest.fn();
const mockCacheDelete = globals_1.jest.fn();
const mockPublishInvalidation = globals_1.jest.fn();
globals_1.jest.unstable_mockModule('../db/postgres.js', () => ({
    query: mockQuery,
    transaction: globals_1.jest.fn((fn) => fn({ query: mockQuery })),
    getClient: globals_1.jest.fn(),
    initializePool: globals_1.jest.fn(),
...
globals_1.jest.unstable_mockModule('../db/redis.js', () => ({
    ...
    cacheDeletePattern: globals_1.jest.fn(),
    publishInvalidation: mockPublishInvalidation,
    configCacheKey: (key, tenantId, env, userId) => `config:${key}:${env || ''}:${tenantId || ''}:${userId || ''}`,
    initializeRedis: globals_1.jest.fn(),
    closeRedis: globals_1.jest.fn(),
    getRedis: globals_1.jest.fn(),
}));
describe('ConfigService Integration Tests', () => {
    ...
    describe('Multi-tenant config resolution', () => {
        it('should resolve tenant-specific config over global', async () => {
            ...
            const { ConfigRepository } = await Promise.resolve().then(() => __importStar(require('../db/repositories/ConfigRepository.js')));
...
describe('FeatureFlag Multi-tenant Tests', () => {
    ...
    it('should evaluate tenant-specific flags', async () => {
        const { FeatureFlagRepository } = await Promise.resolve().then(() => __importStar(require('../db/repositories/FeatureFlagRepository.js')));
...
describe('Experiment Bucket Assignment', () => {
    ...
    it('should consistently assign same user to same variant', async () => {
        const { selectVariant } = await Promise.resolve().then(() => __importStar(require('../utils/hash.js')));
        const variants = [
            { id: 'v1', weight: 50 },
            { id: 'v2', weight: 50 },
        ];

...
// Zod Schemas
// ============================================================================
const EntityFacetSchema = zod_1.z.object({
    '@context': zod_1.z.string().optional().default('https://schema.org'),
    '@type': zod_1.z.string(),
    '@id': zod_1.z.string().optional(),
    name: zod_1.z.string().optional(),
    identifier: zod_1.z.array(zod_1.z.object({
        '@type': zod_1.z.string().default('PropertyValue'),
        propertyID: zod_1.z.string(),
        value: zod_1.z.string(),
    })).optional(),
    additionalProperties: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional(),
});
...
const FeatureVectorSchema = zod_1.z.object({
    nameMatch: zod_1.z.number().min(0).max(1),
    identifierMatch: zod_1.z.number().min(0).max(1),
    attributeOverlap: zod_1.z.number().min(0).max(1),
    temporalProximity: zod_1.z.number().min(0).max(1),
    sourceAgreement: zod_1.z.number().min(0).max(1),
    transitiveSupport: zod_1.z.number().min(0).max(1),
});
const MergeRequestSchema = zod_1.z.object({
    sourceEntityId: zod_1.z.string(),
    targetEntityId: zod_1.z.string(),
    reason: zod_1.z.string(),
    override: zod_1.z.boolean().default(false),
});
const SplitRequestSchema = zod_1.z.object({
    entityId: zod_1.z.string(),
    facetIds: zod_1.z.array(zod_1.z.string()),
    reason: zod_1.z.string(),
});
// ============================================================================
// Matching Engine
...
class EntityMatcher {
    ...
    async computeFeatureVector(entityA, entityB) {
        ...
        return {
            ...
            identifierMatch: this.computeIdentifierMatch(facetsA.identifier || [], facetsB.identifier || []),
            ...
            sourceAgreement: this.computeSourceAgreement(entityA.sourceRefs, entityB.sourceRefs),
            transitiveSupport: 0, // Computed separately with graph context
        };
    }
    computeOverallScore(featureVector) {
        let score = 0;
        for (const [feature, weight] of Object.entries(this.weights)) {
            score += featureVector[feature] * weight;
        }
        return Math.min(1, Math.max(0, score));
    }
    generateScorecard(entityA, entityB, featureVector) {
        const overallScore = this.computeOverallScore(featureVector);
        const explanations = [];
        for (const [feature, weight] of Object.entries(this.weights)) {
            const score = featureVector[feature];
            explanations.push({
                feature,
                weight,
                score,
    ...
    computeIdentifierMatch(idsA, idsB) {
    ...
    computeAttributeOverlap(facetsA, facetsB) {
    ...
    computeTemporalProximity(entityA, entityB) {
    ...
    computeSourceAgreement(sourcesA, sourcesB) {
    ...
    computeConfidence(featureVector) {
    ...
    explainFeature(feature, score, entityA, entityB) {
...
server.get('/er/candidates/:entityId', async (request, reply) => {
    try {
        const { entityId } = request.params;
        const limit = request.query.limit || 10;
        // Get the reference entity
        const refResult = await pool.query('SELECT * FROM entities WHERE id = $1', [entityId]);
        ...
        const candidatesResult = await pool.query(`SELECT * FROM entities
         WHERE id != $1
         AND canonical_facets->>'@type' = $2
         LIMIT 100`, [entityId, refEntity.canonicalFacets['@type']]);
        const candidates = [];
...
// ============================================================================
// Merge two entities
server.post('/er/merge', async (request, reply) => {
    try {
        ...
        // Create merge event
        const mergeEvent = {
            id: `merge_${(0, uuid_1.v4)()}`,
            timestamp: new Date().toISOString(),
            sourceEntityId,
            targetEntityId,
            mergedBy: request.headers['x-user-id'] || 'system',
            reason,
            reversible: true,
            featureVector,
        };
        ...
        await pool.query(`UPDATE entities
        ...
        await pool.query(`UPDATE entities
         ...
         WHERE id = $2`, [targetEntityId, sourceEntityId]);
        // Log merge event
        await pool.query(`INSERT INTO merge_log (id, source_entity_id, target_entity_id, merged_by, reason, feature_vector, reversible, created_at)
...
server.post('/er/split', async (request, reply) => {
    try {
        ...
        if (lastMerge?.reversible) {
            // Reverse the merge
            await pool.query(`UPDATE entities SET merged_into = NULL, updated_at = NOW() WHERE id = $1`, [lastMerge.sourceEntityId]);
            // Remove the merge from history
            const updatedHistory = entity.mergeHistory.slice(0, -1);
            await pool.query(`UPDATE entities SET merge_history = $1, updated_at = NOW() WHERE id = $2`, [JSON.stringify(updatedHistory), entityId]);
            // Log the split
            await pool.query(`INSERT INTO split_log (id, entity_id, split_to, reason, reversed_merge_id, created_at)
        ...
        const newEntityId = `entity_${(0, uuid_1.v4)()}`;
...
server.get('/er/explain/:entityAId/:entityBId', async (request, reply) => {
    try {
...
const start = async () => {
    try {
        ...
        server.log.info(`Entity Resolution service ready at http://localhost:${PORT}`);
    }
    catch (err) {

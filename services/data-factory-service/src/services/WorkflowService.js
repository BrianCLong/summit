...
const logger = (0, pino_1.default)({ name: 'workflow-service' });
class WorkflowService {
    auditService;
    labelingService;
    sampleService;
    constructor(auditService, labelingService, sampleService) {
    ...
    async create(request, createdBy) {
        ...
        const result = await (0, connection_js_1.query)(`INSERT INTO labeling_workflows (
    ...
    async getById(id) {
        const result = await (0, connection_js_1.query)('SELECT * FROM labeling_workflows WHERE id = $1', [id]);
        if (result.rows.length === 0) {
            return null;
        }
        return this.mapRowToWorkflow(result.rows[0]);
    }
    async getByDataset(datasetId) {
        const result = await (0, connection_js_1.query)('SELECT * FROM labeling_workflows WHERE dataset_id = $1 ORDER BY created_at DESC', [datasetId]);
    ...
    async start(id, startedBy) {
        ...
        await (0, connection_js_1.transaction)(async (client) => {
            ...
            await client.query(`UPDATE labeling_workflows SET status = 'active' WHERE id = $1`, [id]);
    ...
    async pause(id, pausedBy) {
        ...
        await (0, connection_js_1.query)(`UPDATE labeling_workflows SET status = 'paused' WHERE id = $1`, [id]);
        await this.auditService.log({
            entityType: 'workflow',
            entityId: id,
            action: 'pause',
    ...
    async advanceStage(id, advancedBy) {
        const workflow = await this.getById(id);
        if (!workflow) {
            throw new Error(`Workflow not found: ${id}`);
        }
        ...
        if (nextStageIndex >= workflow.stages.length) {
            // Workflow complete
            await (0, connection_js_1.query)(`UPDATE labeling_workflows
         ...
         WHERE id = $1`, [id]);
            ...
            // Move to next stage
            await (0, connection_js_1.transaction)(async (client) => {
                await client.query(`UPDATE labeling_workflows SET current_stage_index = $1 WHERE id = $2`, [nextStageIndex, id]);
            ...
            logger.info({ workflowId: id, stageIndex: nextStageIndex }, 'Workflow advanced to next stage');
        }
        return (await this.getById(id));
    }
    async getProgress(id) {
        ...
        const overallProgress = workflow.stages.length > 0
    ...
    async delete(id, deletedBy) {
        ...
        await (0, connection_js_1.query)('DELETE FROM labeling_workflows WHERE id = $1', [id]);
        await this.auditService.log({
            entityType: 'workflow',
            entityId: id,
            action: 'delete',
    ...
    async createJobsForStage(workflow, stage, createdBy) {
        // Get samples for this stage based on sampling strategy
        let sampleIds;
        if (stage.samplingStrategy === 'all') {
            const result = await (0, connection_js_1.query)('SELECT id FROM samples WHERE dataset_id = $1', [workflow.datasetId]);
            sampleIds = result.rows.map((r) => r.id);
        }
        else if (stage.samplingStrategy === 'random') {
            const rate = stage.samplingRate || 1;
            const result = await (0, connection_js_1.query)(`SELECT id FROM samples WHERE dataset_id = $1
         ORDER BY RANDOM()
         LIMIT (SELECT CEIL(COUNT(*) * $2) FROM samples WHERE dataset_id = $1)`, [workflow.datasetId, rate]);
            sampleIds = result.rows.map((r) => r.id);
        }
        else {
        ...
        if (sampleIds.length > 0) {
            ...
            await this.labelingService.createJobsForSamples(workflow.datasetId, sampleIds, workflow.taskType, instructions, `${workflow.id}-${stage.id}`, createdBy);
        }
        logger.info({ workflowId: workflow.id, stageId: stage.id, sampleCount: sampleIds.length }, 'Jobs created for workflow stage');
    }
    async checkStageCompletion(workflow, stage) {
        ...
        if (criteria.minSamplesLabeled) {
            ...
            const labeledCount = (stats.byStatus?.completed || 0) + (stats.byStatus?.approved || 0);
    ...
    async calculateAgreementScore(datasetId) {
        // Calculate inter-annotator agreement
        // This is a simplified version - a full implementation would use Cohen's Kappa or similar
        const result = await (0, connection_js_1.query)(`WITH sample_labels AS (
        SELECT
          ls.sample_id,
          ls.labels,
      ...
      FROM sample_labels`, [datasetId]);
        return parseFloat(result.rows[0].agreement) || 0;
    }
    async calculateQualityScore(datasetId) {
       ...
       WHERE s.dataset_id = $1
         ...
         AND ls.status IN ('completed', 'approved')`, [datasetId]);
        return parseFloat(result.rows[0].accuracy) || 1;
    }
    getInstructionsForStage(stage) {
        const baseInstructions = {
            annotation: 'Please review the sample and provide accurate labels according to the guidelines.',
            review: 'Review the submitted labels for accuracy and consistency. Approve or reject as appropriate.',
            adjudication: 'Resolve disagreements between annotators. Make the final decision on the correct label.',

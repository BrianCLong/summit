...
exports.registerDatasetRoutes = registerDatasetRoutes;
const schemas_js_1 = require("../models/schemas.js");
function registerDatasetRoutes(app, services) {
    // Create dataset
    app.post('/datasets', async (request, reply) => {
        const userId = request.headers['x-user-id'] || 'anonymous';
        const parsed = schemas_js_1.CreateDatasetRequestSchema.safeParse(request.body);
        if (!parsed.success) {
    ...
    // Update dataset
    app.patch('/datasets/:id', async (request, reply) => {
        const { id } = request.params;
        const userId = request.headers['x-user-id'] || 'anonymous';
        const parsed = schemas_js_1.UpdateDatasetRequestSchema.safeParse(request.body);
        if (!parsed.success) {
            return reply.status(400).send({
                error: 'Validation error',
                details: parsed.error.errors,
    ...
    // Publish dataset
    app.post('/datasets/:id/publish', async (request, reply) => {
        const { id } = request.params;
        const userId = request.headers['x-user-id'] || 'anonymous';
        try {
    ...
    // Create new version
    app.post('/datasets/:id/version', async (request, reply) => {
        const { id } = request.params;
    ...
    // Configure splits
    app.post('/datasets/:id/splits', async (request, reply) => {
        const { id } = request.params;
        const { splits } = request.body;
        const userId = request.headers['x-user-id'] || 'anonymous';
        try {
            const configuredSplits = await services.dataset.configureSplits(id, splits, userId);
    ...
    app.post('/datasets/:id/splits/apply', async (request, reply) => {
        const { id } = request.params;
        const userId = request.headers['x-user-id'] || 'anonymous';
        try {
    ...
    app.get('/datasets/:id/statistics', async (request, reply) => {
        const { id } = request.params;
        try {
            const dataset = await services.dataset.getById(id);
            if (!dataset) {
    ...
    // Check eligibility
    app.post('/datasets/:id/check-eligibility', async (request, reply) => {
        const { id } = request.params;
        ...
        try {
            ...
            const result = await services.governance.checkDatasetEligibility(dataset, useCase, userId);
            return reply.send(result);
        }
        catch (error) {
            return reply.status(500).send({

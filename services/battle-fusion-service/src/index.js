...
// CORE SERVICES
// =============================================================================
const fusionEngine = new battle_fusion_1.FusionEngine({
...
let currentSituationalPicture = null;
// =============================================================================
// INITIALIZATION
// =============================================================================
...
function initWebSocket() {
    const wss = new ws_1.WebSocketServer({ port: config.wsPort });
...
async function processIngestEvent(event) {
...
async function publishFusionResult(result) {
    await producer.send({
...
async function cacheResults(result) {
    ...
    await redis.setex('battle:sitrep:current', 300, // 5 minute TTL
    JSON.stringify(result.situationalPicture));
    // Cache fused entities
    for (const entity of result.entities) {
        await redis.setex(`battle:entity:${entity.canonicalId}`, 300, JSON.stringify(entity));
    }
    // Add to time-series stream
    await redis.xadd('battle:fusion:stream', 'MAXLEN', '~', '1000', '*', 'fusionId', result.fusionId, 'timestamp', result.timestamp.toISOString(), 'entityCount', result.entities.length.toString(), 'confidence', result.confidence.toString());
}
async function checkAlerts(result) {
    // Check for critical threats
    const criticalThreats = result.situationalPicture.threats.filter((t) => t.threatLevel === 'CRITICAL');
...
function addScenarioEvent(scenarioId, event) {
...
function executeScenarioEvent(scenario, event) {
    event.executed = true;
    ...
    logger.info({ scenarioId: scenario.id, eventId: event.id }, 'Scenario event executed');
}
// =============================================================================
// EXPRESS SERVER
// =============================================================================
...
app.get('/metrics', async (req, res) => {
    ...
    res.json({
        ...
        recentFusions: stream.map((entry) => ({
            id: entry[0],
            data: Object.fromEntries(entry[1].reduce((acc, val, idx, arr) => {
                if (idx % 2 === 0)

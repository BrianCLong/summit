...
class AnomalyDetector extends events_1.EventEmitter {
    ...
    hourlyEventCounts = new Array(24).fill(0);
    totalEventCount = 0;
    constructor(config = {}) {
        super();
     ...
     */
    async detectUnusualTime(event) {
        const hour = event.timestamp.getHours();
        const isUnusualHour = (this.config.unusualHoursStart <= this.config.unusualHoursEnd
     ...
     */
    async detectFrequencySpike(event) {
        ...
        const recentCount = this.eventWindow.filter((e) => e.actorId === event.actorId &&
            e.eventType === event.eventType &&
            event.timestamp.getTime() - e.timestamp.getTime() < this.config.windowSizeMs).length;
     ...
     */
    async detectUnusualActor(event) {
     ...
     */
    async detectVolumeAnomaly(event) {
        const hour = event.timestamp.getHours();
        const currentHourCount = this.eventWindow.filter((e) => e.timestamp.getHours() === hour &&
            event.timestamp.getTime() - e.timestamp.getTime() < 3600000).length;
        const avgHourlyCount = this.hourlyEventCounts.reduce((a, b) => a + b, 0) / 24;
     ...
     */
    async detectSequenceAnomaly(event) {
        ...
        const isSuspicious = suspiciousPatterns.some((pattern) => this.isSubsequence(pattern, sequence));
        if (!isSuspicious) {
            return null;
        }
        return {
     ...
     */
    async detectGeographicAnomaly(event) {
        ...
        if (recentEvents.length > 0 && event.geoLocation?.latitude) {
            ...
            const distance = this.calculateDistance(lastEvent.geoLocation.latitude, lastEvent.geoLocation.longitude, event.geoLocation.latitude, event.geoLocation.longitude);
            const timeDiffHours = (event.timestamp.getTime() - lastEvent.timestamp.getTime()) / 3600000;
    ...
    async detectPrivilegeEscalation(event) {
        ...
        const recentEscalations = this.eventWindow.filter((e) => e.actorId === event.actorId &&
            e.action &&
            escalationActions.includes(e.action) &&
            event.timestamp.getTime() - e.timestamp.getTime() < 86400000);
    ...
    async detectDataExfiltration(event) {
        ...
        const recentExports = this.eventWindow.filter((e) => e.actorId === event.actorId &&
            e.action &&
    ...
    updateBaselines(event) {
        ...
        baseline.stdDev = Math.sqrt(((n - 1) * baseline.stdDev * baseline.stdDev +
    ...
    updateActorProfile(event) {
        ...
        // Update average
        const hoursSinceFirst = (profile.lastSeen.getTime() - profile.firstSeen.getTime()) / 3600000;
        profile.avgEventsPerHour = profile.eventCount / Math.max(hoursSinceFirst, 1);
    }
    /**
     * Update action frequencies
     */
    updateActionFrequencies(event) {
        if (event.action) {
     ...
     */
    calculateTimeAnomalyScore(_event, hour) {
     ...
     */
    isSubsequence(pattern, sequence) {
        let patternIdx = 0;

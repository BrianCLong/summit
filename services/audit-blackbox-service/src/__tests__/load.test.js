...
function generateEvent() {
    return {
        ...
        timestamp: new Date(),
        eventType: ['access', 'export', 'admin_change', 'policy_change'][Math.floor(Math.random() * 4)],
        ...
        metadata: {
            source: 'load-test',
            requestId: (0, crypto_1.randomBytes)(8).toString('hex'),
        },
    };
}
...
function measureLatencies(fn, iterations) {
    const latencies = [];
...
function createPerformanceResult(operation, latencies, totalTimeMs) {
...
describe('Load Tests: Hash Operations', () => {
    ...
    it('should benchmark chain hash calculation', () => {
        const hashes = Array.from({ length: ITERATIONS }, () => (0, crypto_1.randomBytes)(32).toString('hex'));
...
describe('Load Tests: Memory Pressure', () => {
    it('should handle large event payloads', () => {
        ...
        const largeEvents = Array.from({ length: ITERATIONS }, () => ({
            ...generateEvent(),
            metadata: {
                largePayload: (0, crypto_1.randomBytes)(PAYLOAD_SIZE_KB * 1024).toString('base64'),
            },
        }));
        const startMemory = process.memoryUsage().heapUsed;
        const start = Date.now();

 ...
 */
class MockAuditStore extends events_1.EventEmitter {
    events = [];
    appendDelay = 0;
    shouldFail = false;
...
describe('AuditEventBuffer', () => {
    ...
    beforeEach(() => {
        ...
        buffer = new event_buffer_js_1.AuditEventBuffer({
            maxSize: 100,
    ...
    describe('Backpressure Handling', () => {
        ...
        it('should allow critical events to bypass backpressure', async () => {
            ...
            const result = await buffer.push(createTestEvent({ id: 'critical', level: 'critical' }));
...
describe('Cross-Service Event Flow', () => {
    it('should handle events from multiple services', async () => {
        const flushedEvents = [];
        const buffer = new event_buffer_js_1.AuditEventBuffer({
            maxSize: 1000,
            flushIntervalMs: 100,
            batchSize: 50,
            backpressureThreshold: 0.8,
    ...
    it('should maintain event ordering within same service', async () => {
        ...
        const serviceEvents = flushedEvents.filter((e) => e.serviceId === 'test-service');
        // Check ordering

 ...
 * Sign a chain entry
 */
function signChainEntry(eventHash, chainHash, sequence) {
    const data = JSON.stringify({
        eventHash,
        chainHash,
        sequence: sequence.toString(),
    });
    return (0, crypto_1.createHmac)('sha256', SIGNING_KEY).update(data).digest('hex');
}
/**
...
describe('Hash Chain', () => {
    ...
    describe('Merkle Root Calculation', () => {
        ...
        it('should combine two hashes correctly', () => {
            ...
            const expected = (0, crypto_1.createHash)('sha256').update(hash1 + hash2).digest('hex');
        ...
        it('should produce consistent root for same hashes', () => {
            const hashes = Array.from({ length: 8 }, (_, i) => (0, crypto_1.createHash)('sha256').update(`hash-${i}`).digest('hex'));
    ...
    describe('Full Chain Simulation', () => {
        it('should build and verify a complete chain', () => {
            const events = Array.from({ length: 10 }, (_, i) => createTestEvent({
                id: `event-${i}`,
            ...
            for (const entry of chain) {
                ...
                // Verify chain hash
                const calculatedChainHash = calculateChainHash(entry.eventHash, entry.previousHash, entry.sequence);
                if (calculatedChainHash !== entry.chainHash) {
                    allValid = false;
                    break;
                }
                // Verify signature
                const expectedSignature = signChainEntry(entry.eventHash, entry.chainHash, entry.sequence);

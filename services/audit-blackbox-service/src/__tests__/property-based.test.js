...
function signChainEntry(chainHash, key) {
    return (0, crypto_1.createHmac)('sha256', key).update(chainHash).digest('hex');
}
function verifySignature(chainHash, signature, key) {
    const expected = (0, crypto_1.createHmac)('sha256', key).update(chainHash).digest('hex');
...
describe('Property-Based Tests: Hash Chain', () => {
    describe('Hash Determinism', () => {
        it('should produce the same hash for identical events', () => {
            fc.assert(fc.property(auditEventArb, (event) => {
        ...
        it('should produce different hashes for different events', () => {
            fc.assert(fc.property(auditEventArb, auditEventArb, (event1, event2) => {
    ...
    describe('Chain Hash Properties', () => {
        it('chain hash should incorporate all inputs', () => {
            fc.assert(fc.property(fc.hexaString({ minLength: 64, maxLength: 64 }), fc.hexaString({ minLength: 64, maxLength: 64 }), fc.nat(), (eventHash, previousHash, sequence) => {
                const chainHash = calculateChainHash(eventHash, previousHash, sequence);
                // Changing any input should change the output
                const changedEvent = calculateChainHash(eventHash.replace(/.$/, eventHash[63] === 'a' ? 'b' : 'a'), previousHash, sequence);
                const changedPrevious = calculateChainHash(eventHash, previousHash.replace(/.$/, previousHash[63] === 'a' ? 'b' : 'a'), sequence);
        ...
        it('chain hashes should be collision-resistant', () => {
            ...
            fc.assert(fc.property(fc.hexaString({ minLength: 64, maxLength: 64 }), fc.hexaString({ minLength: 64, maxLength: 64 }), fc.nat({ max: 1000000 }), (eventHash, previousHash, sequence) => {
                const chainHash = calculateChainHash(eventHash, previousHash, sequence);
    ...
    describe('Signature Properties', () => {
        it('signature should verify with correct key', () => {
            fc.assert(fc.property(fc.hexaString({ minLength: 64, maxLength: 64 }), signingKeyArb, (chainHash, key) => {
        ...
        it('signature should not verify with wrong key', () => {
            fc.assert(fc.property(fc.hexaString({ minLength: 64, maxLength: 64 }), signingKeyArb, signingKeyArb, (chainHash, key1, key2) => {
    ...
    describe('Chain Integrity', () => {
        it('chain should be verifiable end-to-end', () => {
            fc.assert(fc.property(fc.array(auditEventArb, { minLength: 2, maxLength: 100 }), signingKeyArb, (events, key) => {
                const chain = [];
                ...
                for (let i = 0; i < chain.length; i++) {
                    ...
                    const recalculatedChain = calculateChainHash(entry.eventHash, entry.previousHash, entry.sequence);
                    expect(entry.chainHash).toBe(recalculatedChain);
                }
            }), { numRuns: 100 });
        });
        it('tampering with any entry should break chain verification', () => {
            fc.assert(fc.property(fc.array(auditEventArb, { minLength: 5, maxLength: 20 }), fc.nat(), (events, tamperIndex) => {
                const chain = [];
                const GENESIS_HASH = '0'.repeat(64);
                // Build chain
                events.forEach((event, i) => {
                ...
                chain[targetIndex].eventHash = originalHash.replace(/./, originalHash[0] === 'a' ? 'b' : 'a');
                // Verification should fail at tampered entry
                const recalculatedChain = calculateChainHash(chain[targetIndex].eventHash, chain[targetIndex].previousHash, chain[targetIndex].sequence);
    ...
    describe('Ordering Properties', () => {
        it('sequence numbers should be monotonically increasing', () => {
            fc.assert(fc.property(fc.array(fc.nat({ max: 1000000 }), { minLength: 2, maxLength: 100 }), (sequences) => {
        ...
        it('timestamps should not regress within chain', () => {
            fc.assert(fc.property(fc.array(auditEventArb, { minLength: 2, maxLength: 50 }), (events) => {
                // Sort events by timestamp (simulating proper ordering)
                const sortedEvents = [...events].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
                for (let i = 1; i < sortedEvents.length; i++) {
                    expect(sortedEvents[i].timestamp.getTime()).toBeGreaterThanOrEqual(sortedEvents[i - 1].timestamp.getTime());
                }
            }));
        });
    });
...
describe('Property-Based Tests: Merkle Tree', () => {
    ...
    function buildMerkleTree(hashes) {
        ...
        for (let i = 0; i < hashes.length; i += 2) {
            ...
            const combined = (0, crypto_1.createHash)('sha256').update(left + right).digest('hex');
    ...
    function verifyMerkleProof(hash, proof, index, root) {
        let currentHash = hash;
        let currentIndex = index;
        for (const sibling of proof) {
            if (currentIndex % 2 === 0) {
                currentHash = (0, crypto_1.createHash)('sha256').update(currentHash + sibling).digest('hex');
            }
            else {
                currentHash = (0, crypto_1.createHash)('sha256').update(sibling + currentHash).digest('hex');
            }
            currentIndex = Math.floor(currentIndex / 2);
        }
    ...
    describe('Merkle Root Properties', () => {
        it('same hashes should produce same root', () => {
            fc.assert(fc.property(fc.array(fc.hexaString({ minLength: 64, maxLength: 64 }), {
                minLength: 1,
                maxLength: 64,
            }), (hashes) => {
    ...
    describe('Merkle Proof Properties', () => {
        it('valid proofs should verify', () => {
            fc.assert(fc.property(fc.array(fc.hexaString({ minLength: 64, maxLength: 64 }), {
                ...
                expect(verifyMerkleProof(hashes[targetIndex], proof, targetIndex, root)).toBe(true);
            }), { numRuns: 200 });
        });
        it('proofs should fail for wrong hash', () => {
            fc.assert(fc.property(fc.array(fc.hexaString({ minLength: 64, maxLength: 64 }), {
                ...
                const proof = getMerkleProof(hashes, targetIndex);
                // Modify the hash
                const wrongHash = hashes[targetIndex].replace(/./, hashes[targetIndex][0] === 'a' ? 'b' : 'a');
                expect(verifyMerkleProof(wrongHash, proof, targetIndex, root)).toBe(false);
            }), { numRuns: 200 });
        });
        it('proof size should be O(log n)', () => {
            fc.assert(fc.property(fc.nat({ min: 1, max: 10 }), (power) => {
                const size = 2 ** power;
                const hashes = Array.from({ length: size }, () => (0, crypto_1.randomBytes)(32).toString('hex'));
...
describe('Property-Based Tests: Canonicalization', () => {
    it('object key order should not affect hash', () => {
        fc.assert(fc.property(fc.record({
            ...
            const shuffled = Object.fromEntries(Object.entries(obj).sort(() => Math.random() - 0.5));

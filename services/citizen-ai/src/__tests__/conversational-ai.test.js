...
describe('ConversationalAI', () => {
    let ai;
    beforeEach(() => {
        ai = new conversational_ai_1.ConversationalAI();
    });
    describe('processMessage', () => {
        ...
        it('should detect e-residency intent', async () => {
            const response = await ai.processMessage('test-session', 'I want to apply for e-residency', 'en');
            expect(response.intent?.name).toBe('e-residency.apply');
            expect(response.suggestedActions?.length).toBeGreaterThan(0);
        });
        it('should detect tax intent', async () => {
            const response = await ai.processMessage('test-session', 'How do I file my tax return?', 'en');
...
describe('conversationalAI singleton', () => {
    it('should be defined', () => {
        expect(conversational_ai_1.conversationalAI).toBeDefined();
    });
});

...
exports.HttpCsvConnector = void 0;
const ConnectorSDK_js_1 = require("../sdk/ConnectorSDK.js");
const axios_1 = __importDefault(require("axios"));
const csv_parser_1 = __importDefault(require("csv-parser"));
const logger_js_1 = require("../utils/logger.js");
class HttpCsvConnector extends ConnectorSDK_js_1.BaseConnector {
    connection = null;
    totalRecords = 0;
    constructor() {
        const config = {
    ...
    async testConnection() {
        try {
            ...
            const response = await axios_1.default.head(this.parameters.url, {
                headers,
            ...
            const isValidContentType = this.config.supportedFormats.some((format) => contentType.includes(format.split('/')[1]));
    ...
    async connect() {
        try {
            ...
            const response = await axios_1.default.get(this.parameters.url, {
                headers,
                timeout: this.parameters.timeout || 30000,
                responseType: 'stream',
            });
            ...
            logger_js_1.logger.info('Successfully connected to HTTP CSV source');
        }
        catch (error) {
    ...
    async disconnect() {
        ...
        logger_js_1.logger.info('Disconnected from HTTP CSV source');
    }
    async *fetchData() {
        if (!this.connection) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.on('batchReady', async (batchRecords) => {
                yield batchRecords;
            });
        }).then(async function* () {
    ...
    createRecordFromRow(row, fieldMapping, transformations) {
        try {
            // Generate ID
    ...
    passesFilters(row, filters) {
        return filters.every((filter) => {
            ...
            switch (filter.operator) {
                ...
                default:
                    logger_js_1.logger.warn(`Unknown filter operator: ${filter.operator}`);
                    return true;
            }
        });
    }
                    ...
                    switch (transform) {
                        ...
                        case 'replace':
                            if (params?.pattern && params?.replacement !== undefined) {
                                transformed[field] = String(value).replace(new RegExp(params.pattern, params.flags || 'g'), params.replacement);
                            }
                            break;
                        case 'split':
                            if (params?.delimiter) {
                        ...
                        case 'suffix':
                            ...
                            break;
                        default:
                            logger_js_1.logger.warn(`Unknown transformation: ${transform}`);
                    }
                }
                catch (error) {
...
exports.HttpCsvConnector = HttpCsvConnector;

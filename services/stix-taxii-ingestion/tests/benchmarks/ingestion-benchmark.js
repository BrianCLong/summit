...
function generateMockIndicators(count) {
    const indicators = [];
...
(0, vitest_1.describe)('Pattern Extraction', () => {
    ...
    (0, vitest_1.bench)('extract value from STIX pattern (100 iterations)', () => {
        for (let i = 0; i < 100; i++) {
            extractPatternValue(patterns[i % patterns.length]);
        }
    });
});
(0, vitest_1.describe)('MITRE Mapping', () => {
    const descriptions = [
    ...
    (0, vitest_1.bench)('map text to MITRE techniques (100 iterations)', () => {
        for (let i = 0; i < 100; i++) {
...
(0, vitest_1.describe)('Risk Score Calculation', () => {
    ...
    (0, vitest_1.bench)('calculate risk score (1000 iterations)', () => {
...
(0, vitest_1.describe)('Object Processing', () => {
    const indicators = generateMockIndicators(100);
    (0, vitest_1.bench)('convert object to text (100 objects)', () => {
        for (const indicator of indicators) {
            objectToText(indicator);
        }
    });
    (0, vitest_1.bench)('full processing pipeline (100 objects)', () => {
        for (const indicator of indicators) {
            // Convert to text
            ...
            calculateRiskScore(indicator.confidence || 50, indicator.type, mitreMappings.length, indicator.labels || []);
        }
    });
});
(0, vitest_1.describe)('Batch Processing', () => {
    (0, vitest_1.bench)('generate 1000 mock indicators', () => {
        generateMockIndicators(1000);
    });
    (0, vitest_1.bench)('process batch of 1000 indicators', () => {
        ...
        for (const indicator of indicators) {
            ...
            const risk = calculateRiskScore(indicator.confidence || 50, indicator.type, mitre.length, indicator.labels || []);
            if (value && risk > 0) {
                processed++;
            }
        }
...
(0, vitest_1.describe)('JSON Serialization', () => {
    ...
    (0, vitest_1.bench)('serialize bundle to JSON', () => {
        JSON.stringify(bundle);
    });
    const jsonString = JSON.stringify(bundle);
    (0, vitest_1.bench)('parse bundle from JSON', () => {
        JSON.parse(jsonString);
    });
});
(0, vitest_1.describe)('ID Generation and Validation', () => {
    const stixIdRegex = /^[a-z-]+--[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/;
    (0, vitest_1.bench)('generate 1000 STIX IDs', () => {
        for (let i = 0; i < 1000; i++) {
            `indicator--${crypto.randomUUID()}`;
        }
    });
    const ids = Array.from({ length: 1000 }, () => `indicator--${crypto.randomUUID()}`);
    (0, vitest_1.bench)('validate 1000 STIX IDs', () => {
        for (const id of ids) {
            stixIdRegex.test(id);
        }
    });

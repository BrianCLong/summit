...
exports.createPgVectorStore = createPgVectorStore;
const pg_1 = require("pg");
const pino_1 = __importDefault(require("pino"));
const logger = (0, pino_1.default)({ name: 'pgvector-store' });
class PgVectorStore {
    pool;
    embeddingDimension;
    initialized = false;
     ...
     */
    async store(object, metadata, embedding) {
    ...
    async storeBatch(objects) {
        ...
        logger.info({ stored: result.stored, errors: result.errors.length }, 'Batch store completed');
        return result;
    }
    /**
    ...
    async get(stixId) {
        await this.ensureInitialized();
        const result = await this.pool.query('SELECT * FROM stix_iocs WHERE stix_id = $1', [stixId]);
     ...
     */
    async updateEmbedding(stixId, embedding) {
        await this.ensureInitialized();
        const embeddingStr = `[${embedding.join(',')}]`;
        const result = await this.pool.query('UPDATE stix_iocs SET embedding = $1::vector WHERE stix_id = $2', [embeddingStr, stixId]);
    ...
    async deleteByFeed(feedId) {
        await this.ensureInitialized();
        const result = await this.pool.query('DELETE FROM stix_iocs WHERE feed_id = $1', [feedId]);
        return result.rowCount ?? 0;
    }
    /**
     * Get feed sync state
     */
    async getFeedSyncState(feedId) {
        await this.ensureInitialized();
        const result = await this.pool.query('SELECT last_sync_timestamp, next_cursor, total_objects_synced FROM stix_feed_sync_state WHERE feed_id = $1', [feedId]);
     ...
     */
    async updateFeedSyncState(feedId, collectionId, state) {
        await this.ensureInitialized();
    ...
    async getStats() {
        await this.ensureInitialized();
        const [totalResult, typeResult, feedResult, embeddingResult, dateResult] = await Promise.all([
            this.pool.query('SELECT COUNT(*) as count FROM stix_iocs'),
            this.pool.query('SELECT stix_type, COUNT(*) as count FROM stix_iocs GROUP BY stix_type'),
            this.pool.query('SELECT feed_id, COUNT(*) as count FROM stix_iocs GROUP BY feed_id'),
            this.pool.query('SELECT COUNT(*) as count FROM stix_iocs WHERE embedding IS NOT NULL'),
            this.pool.query('SELECT MIN(created_at) as earliest, MAX(created_at) as latest FROM stix_iocs'),
        ]);
        return {
            totalRecords: parseInt(totalResult.rows[0].count, 10),
            byType: Object.fromEntries(typeResult.rows.map((r) => [r.stix_type, parseInt(r.count, 10)])),
            byFeed: Object.fromEntries(feedResult.rows.map((r) => [r.feed_id, parseInt(r.count, 10)])),
    ...
    getTenantStorageLimit(tenantId, plan) {
        const fromEnv = process.env.STIX_TENANT_LIMIT_BYTES;
        if (fromEnv)
            return Number(fromEnv);
        if (plan === 'ENTERPRISE')
            return 500_000_000_000; // 500 GB
        if (plan === 'PRO')
            return 150_000_000_000; // 150 GB
        if (plan === 'STARTER')
    ...
    estimateRecordBytes(record, embedding) {
    ...
    async assertStorageBudget(tenantId, record, planTier, client) {
        const limit = this.getTenantStorageLimit(tenantId, planTier);
        const poolClient = client || (await this.pool.connect());
        try {
            const { rows } = await poolClient.query(`SELECT COALESCE(SUM(pg_column_size(m.*)::bigint),0) AS bytes FROM stix_iocs m WHERE tenant_id = $1`, [tenantId]);
    ...
    objectToRecord(object, metadata, embedding) {
        const common = object;
        let value;
        let pattern;
        let patternType;
    ...
    extractValueFromPattern(pattern) {

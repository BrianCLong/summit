...
exports.AddressClusterer = void 0;
const events_1 = require("events");
const crypto_1 = __importDefault(require("crypto"));
class AddressClusterer extends events_1.EventEmitter {
    addresses = new Map();
    clusters = new Map();
    transactions = new Map();
    clusteringJobs = new Map();
    flowAnalyses = new Map();
    // Graph structure for clustering
    addressGraph = new Map();
    transactionGraph = new Map();
    constructor() {
        super();
    }
    /**
     ...
     */
    async executeClusteringJob(blockchain, scope, config = {}) {
        const job = {
            id: crypto_1.default.randomUUID(),
            status: 'pending',
            blockchain,
     ...
     */
    async analyzeFlow(sourceAddress, targetAddress, maxHops = 4, maxAmount, timeWindow) {
        const analysis = {
            id: crypto_1.default.randomUUID(),
            sourceAddress,
            targetAddress,
     ...
     */
    async detectMixerUsage(address, timeWindow) {
        const mixerEvents = [];
        ...
        const timeSpan = timeWindow
            ...
            : addressTransactions.length > 0
                ...
                : 0;
        const mixingFrequency = timeSpan > 0
            ? mixerEvents.length / (timeSpan / (1000 * 60 * 60 * 24))
            : 0;
        let suspicionLevel = 'low';
        if (mixingFrequency > 5)
            suspicionLevel = 'critical';
        else if (mixingFrequency > 2)
            suspicionLevel = 'high';
        else if (mixingFrequency > 0.5)
    ...
    getClusterByAddress(address) {
        return (Array.from(this.clusters.values()).find((cluster) => cluster.addresses.includes(address)) || null);
    }
    /**
    ...
    getClusteringStatistics() {
        ...
        const clusteredAddresses = Array.from(this.clusters.values()).reduce((sum, cluster) => sum + cluster.addresses.length, 0);
    ...
    async executeClusteringAsync(job) {
        ...
        try {
            ...
            job.results = {
                ...
                maxClusterSize: validatedClusters.length > 0
                    ...
                    : 0,
                confidenceDistribution: this.calculateDistribution(validatedClusters.map((c) => c.confidence), 0.1),
            };
            this.clusteringJobs.set(job.id, job);
    ...
    async getScopedAddresses(scope, blockchain) {
        let addresses = Array.from(this.addresses.values()).filter((addr) => addr.blockchain === blockchain);
        if (scope.addresses) {
            addresses = addresses.filter((addr) => scope.addresses.includes(addr.address));
        }
        if (scope.dateRange) {
            addresses = addresses.filter((addr) => addr.metadata.firstSeen >= scope.dateRange.start &&
                addr.metadata.lastSeen <= scope.dateRange.end);
        }
        return addresses;
    }
    async getScopedTransactions(scope, blockchain) {
        let transactions = Array.from(this.transactions.values()).filter((tx) => tx.blockchain === blockchain);
        if (scope.transactions) {
            transactions = transactions.filter((tx) => scope.transactions.includes(tx.hash));
        }
        if (scope.dateRange) {
            transactions = transactions.filter((tx) => tx.timestamp >= scope.dateRange.start &&
                tx.timestamp <= scope.dateRange.end);
        }
        return transactions;
    }
    async buildClusteringGraph(addresses, transactions, job) {
        ...
        for (const tx of transactions) {
            ...
            if (tx.inputs.length > 1) {
                for (let i = 0; i < tx.inputs.length; i++) {
                    for (let j = i + 1; j < tx.inputs.length; j++) {
                        this.addGraphEdge(tx.inputs[i].address, tx.inputs[j].address, 0.8, // High confidence for common input
            ...
            if (tx.outputs.length === 2 && tx.inputs.length === 1) {
                ...
                if (changeOutput) {
                    this.addGraphEdge(tx.inputs[0].address, changeOutput.address, 0.7, // Medium-high confidence for change address
    ...
    addGraphEdge(from, to, weight, evidence) {
    ...
    async createCluster(addresses, job) {
        const clusterId = crypto_1.default.randomUUID();
        // Analyze cluster characteristics
        const transactions = Array.from(this.transactions.values()).filter((tx) => tx.inputs.some((inp) => addresses.includes(inp.address)) ||
            tx.outputs.some((out) => addresses.includes(out.address)));
        const totalVolume = transactions.reduce((sum, tx) => sum + tx.inputs.reduce((inputSum, inp) => inputSum + inp.value, 0), 0);
        ...
        const amounts = transactions.map((tx) => tx.inputs.reduce((sum, inp) => sum + inp.value, 0));
        const cluster = {
            id: clusterId,
            addresses,
            ...
            statistics: {
                ...
                averageTransactionSize: amounts.length > 0
    ...
    async validateClusters(clusters, job) {
    ...
    async finalizeClusters(clusters, job) {
        for (const cluster of clusters) {
    ...
    async findPaths(sourceAddress, targetAddress, maxHops, maxAmount, timeWindow) {
        const paths = [];
        const visited = new Set();
        // BFS to find paths
        const queue = [{ currentAddress: sourceAddress, path: [], totalAmount: 0 }];
        while (queue.length > 0 && paths.length < 1000) {
            ...
            for (const tx of outgoingTxs) {
                for (const output of tx.outputs) {
                    if (output.address !== currentAddress &&
    ...
    async analyzeBehaviorPattern(address) {
        // Analyze transaction patterns to determine behavior
        const riskScore = this.calculateRiskScore(address);
        if (address.riskIndicators.mixer)
            return 'mixer';
        if (address.riskIndicators.exchange)
            return 'exchange';
        if (address.riskIndicators.gambling)
            return 'gambling';
    ...
    async calculatePrivacyScore(address) {
        let score = 0;
        ...
        if (uniqueCounterparties / Math.max(txCount, 1) > 0.8)
            score += 0.3;
        return Math.min(score, 1.0);
    }
    async calculateActivityScore(address) {
        const txCount = address.metadata.transactionCount;
        ...
        const activityScore = Math.min((txCount / 1000) * 0.5 + (Math.log10(totalVolume + 1) / 10) * 0.5, 1.0);
        return activityScore;
    }
    calculateRiskScore(address) {
        let riskScore = 0;
        // Risk indicators contribute to score
        if (address.riskIndicators.sanctionsHit)
            riskScore += 1.0;
        if (address.riskIndicators.darknetMarket)
            riskScore += 0.9;
        if (address.riskIndicators.ransomware)
            riskScore += 0.9;
        if (address.riskIndicators.phishing)
            riskScore += 0.8;
        if (address.riskIndicators.mixer)
            riskScore += 0.6;
        if (address.riskIndicators.gambling)
            riskScore += 0.3;
        if (address.riskIndicators.highRiskJurisdiction)
            riskScore += 0.4;
    ...
    async analyzeTransaction(transaction) {
        ...
        if (analysis.mixing)
            analysis.suspiciousPatterns.push('mixing');
        if (analysis.peeling)
            analysis.suspiciousPatterns.push('peeling');
        if (analysis.roundAmount)
            analysis.suspiciousPatterns.push('round_amount');
        return analysis;
    }
    async updateAddressGraph(transaction) {
    ...
    getAddressBlockchain(address) {
        const addressObj = this.addresses.get(address);
        return addressObj?.blockchain || 'unknown';
    }
    async calculatePathRiskScore(hops) {
        let riskScore = 0;
    ...
    async identifySuspiciousHops(hops) {
        const suspiciousHops = [];
        hops.forEach((hop, index) => {
            const fromAddr = this.addresses.get(hop.fromAddress);
            const toAddr = this.addresses.get(hop.toAddress);
            if (fromAddr?.riskIndicators.sanctionsHit ||
            ...
            if (fromAddr?.riskIndicators.darknetMarket ||
    ...
    calculateRiskDistribution(paths) {
    ...
    async checkSanctionsExposure(paths) {
    ...
    async analyzeMixerIndicators(tx, address) {
    ...
    async determineClusterType(addresses, transactions) {
        // Analyze cluster characteristics to determine type
        const hasExchangePatterns = transactions.some((tx) => tx.inputs.length > 5 || tx.outputs.length > 5);
        const hasMixerPatterns = transactions.some((tx) => tx.analysis.mixing);
        if (hasMixerPatterns)
            return 'mixer';
        if (hasExchangePatterns)
            return 'exchange';
    ...
    async calculateClusterConfidence(addresses, job) {
    ...
    async getClusterHeuristics(addresses) {
    ...
    async generateClusterTags(addresses, transactions) {
        const tags = [];
        // Analyze cluster characteristics
        const hasMixing = transactions.some((tx) => tx.analysis.mixing);
        const hasHighVolume = transactions.reduce((sum, tx) => sum + tx.inputs.reduce((inputSum, inp) => inputSum + inp.value, 0), 0) > 1000;
        if (hasMixing) {
            tags.push({
                category: 'risk',
                tag: 'mixing_activity',
    ...
    calculateDistribution(values, bucketSize) {
        const distribution = {};
...
exports.AddressClusterer = AddressClusterer;

...
LIMIT $2`,
            ...
            {
                regex: /(?:what\s+connects|shortest\s+path\s+(?:from|between))\s+["']?([^"']+?)["']?\s+(?:to|and)\s+["']?([^"'?]+)["']?/i,
                generator: (match, dialect) => ({
                    query: dialect === 'CYPHER'
...
LIMIT $2`,
                        ...
                        parameters: dialect === 'CYPHER' ? { type: entityType, limit: 100 } : { $1: entityType, $2: 100 },
                        confidence: 0.95,
                    };
                },
            },
...
LIMIT $3`,
    ...
    applyPolicyConstraints(result, policy) {
    ...
    generateFallback(userText, dialect, policy) {
...
// Factory Function
// =============================================================================
function createLlmAdapter(type = 'mock') {
    switch (type) {
        case 'mock':

...
exports.createSafetyAnalyzer = createSafetyAnalyzer;
exports.analyzeQuerySafety = analyzeQuerySafety;
// =============================================================================
// Constants
// =============================================================================
...
class SafetyAnalyzer {
     ...
     */
    analyzeQuerySafety(query, dialect, policy) {
        const violations = [];
        const warnings = [];
    ...
    // ---------------------------------------------------------------------------
    checkForbiddenOperations(query, dialect, violations) {
    ...
    // ---------------------------------------------------------------------------
    checkInjectionPatterns(query, violations) {
    ...
    // ---------------------------------------------------------------------------
    estimateQueryDepth(query, dialect) {
        if (dialect === 'CYPHER') {
    ...
    checkDepthConstraint(estimatedDepth, policy, violations, warnings) {
    ...
    // Row Limit Check
    // ---------------------------------------------------------------------------
    estimateRowCount(query, dialect, policy) {
    ...
    checkRowLimit(query, dialect, policy, estimatedRows, violations, warnings) {
    ...
    // ---------------------------------------------------------------------------
    checkDisallowedLabels(query, dialect, policy, violations) {
        const allDisallowed = [
        ...
        for (const label of allDisallowed) {
            ...
            const sqlTableRegex = new RegExp(`(?:FROM|JOIN|INTO|UPDATE)\\s+(?:\\w+\\.)?${label}\\b`, 'i');
    ...
    // ---------------------------------------------------------------------------
    checkUnboundedPatterns(query, dialect, violations, warnings) {
        if (dialect === 'CYPHER') {
    ...
    // Basic Syntax Check
    // ---------------------------------------------------------------------------
    checkBasicSyntax(query, dialect, violations) {
        if (dialect === 'CYPHER') {
    ...
    checkCypherSyntax(query, violations) {
    ...
    checkSqlSyntax(query, violations) {
        const upperQuery = query.toUpperCase();
    ...
    hasBalancedBrackets(str, open, close) {
        let count = 0;
        let inString = false;
        let stringChar = '';
    ...
    generateSuggestedFixes(violations, policy) {
        const fixes = [];
...
// Factory Function
// =============================================================================
function createSafetyAnalyzer() {
    return new SafetyAnalyzer();
}
/**
 * Convenience function for analyzing query safety.
 */
function analyzeQuerySafety(query, dialect, policy) {

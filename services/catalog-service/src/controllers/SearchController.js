...
class SearchController {
    ...
    constructor() {
        ...
        const schemaRegistryService = new data_catalog_1.SchemaRegistryService();
        // Initialize search service with data stores
        // In production, these would be injected or pulled from a shared registry
        this.searchService = new data_catalog_1.MetadataSearchService(new Map(), // dataSources
        new Map(), // datasets
        new Map(), // fields
        new Map(), // mappings
        new Map());
    ...
    async advancedSearch(req, res) {
        try {
            const { query, filters, sort, offset = 0, limit = 20, includeArchived = false, } = req.body;
    ...
    async searchFields(req, res) {
        try {
            const { query, datasetId, dataType, canonicalFieldId, hasMappings, offset = 0, limit = 20, } = req.body;
            if (!query || typeof query !== 'string') {
                res.status(400).json({
                    error: 'Query is required',
                });
            ...
            const results = await this.searchService.searchFieldsAdvanced(searchRequest);
            res.json({
                success: true,
                results,

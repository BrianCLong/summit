...
class DataSourceController {
    dataSourceService;
    constructor() {
        this.dataSourceService = new data_catalog_1.DataSourceService();
    }
    /**
     * Register a new data source
     * POST /api/v1/catalog/sources
     */
    async registerSource(req, res) {
        try {
            const validation = data_catalog_1.DataSourceSchema.safeParse(req.body);
            ...
            const dataSource = await this.dataSourceService.registerDataSource(req.body);
            res.status(201).json({
                success: true,
                dataSource,
            });
    ...
    async listSources(req, res) {
        try {
            ...
            const dataSources = await this.dataSourceService.listDataSources(filters);
            res.json({
                success: true,
                dataSources,
    ...
    async registerDataset(req, res) {
        try {
            const validation = data_catalog_1.DatasetSchema.safeParse(req.body);
            if (!validation.success) {
                res.status(400).json({
                    error: 'Validation failed',
                    details: validation.error.errors,
    ...
    async registerField(req, res) {
        try {
            const validation = data_catalog_1.FieldSchema.safeParse(req.body);
            if (!validation.success) {
                res.status(400).json({
                    error: 'Validation failed',
                    details: validation.error.errors,
     ...
     */
    async attachLicense(req, res) {
        try {
            const { datasetId, licenseId } = req.params;
            await this.dataSourceService.attachLicenseToDataset(datasetId, licenseId);
            res.json({
                success: true,
     ...
     */
    async getDatasetsByLicense(req, res) {
        try {
            const { licenseId } = req.params;
            const datasets = await this.dataSourceService.getDatasetsByLicense(licenseId);
            res.json({
                success: true,
                datasets,
                total: datasets.length,
     ...
     */
    async getDatasetsByPolicyTag(req, res) {
        try {
            const { tag } = req.params;
            const datasets = await this.dataSourceService.getDatasetsByPolicyTag(tag);
            res.json({
                success: true,
                datasets,
                total: datasets.length,

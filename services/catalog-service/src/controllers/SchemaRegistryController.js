...
exports.SchemaRegistryController = void 0;
const data_catalog_1 = require("@intelgraph/data-catalog");
class SchemaRegistryController {
    schemaRegistryService;
    constructor() {
        this.schemaRegistryService = new data_catalog_1.SchemaRegistryService();
    }
    /**
    ...
    async registerSchema(req, res) {
        try {
            const validation = data_catalog_1.SchemaRegistrationRequestSchema.safeParse(req.body);
            ...
            const schema = await this.schemaRegistryService.registerSchema(req.body);
            res.status(201).json({
                success: true,
                schema,
            });
    ...
    async getSchemaByName(req, res) {
        try {
            ...
            const schema = await this.schemaRegistryService.getSchemaByName(namespace, name, version);
            if (!schema) {
                res.status(404).json({
                    error: 'Schema not found',
                });
     ...
     */
    async searchSchemas(req, res) {
        try {
            const validation = data_catalog_1.SchemaSearchRequestSchema.safeParse(req.body);
            if (!validation.success) {
                res.status(400).json({
                    error: 'Validation failed',
                    details: validation.error.errors,
    ...
    async evolveSchema(req, res) {
        try {
            ...
            const validation = data_catalog_1.SchemaEvolutionRequestSchema.safeParse(evolutionRequest);
            if (!validation.success) {
                res.status(400).json({
                    error: 'Validation failed',
                    details: validation.error.errors,
            ...
            const newSchema = await this.schemaRegistryService.evolveSchema(evolutionRequest);
            res.status(201).json({
                success: true,
                schema: newSchema,
            });
    ...
    async deprecateSchema(req, res) {
        try {
            ...
            const schema = await this.schemaRegistryService.deprecateSchema(id, reason, replacementSchemaId);
            res.json({
                success: true,
                schema,
            });
     ...
     */
    async getSchemaVersions(req, res) {
        try {
            const { id } = req.params;
...
exports.SchemaRegistryController = SchemaRegistryController;

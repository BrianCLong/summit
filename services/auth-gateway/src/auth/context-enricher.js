...
exports.AuthContextEnricher = void 0;
class AuthContextEnricher {
    config;
    constructor(config) {
        this.config = config;
    }
     ...
     */
    async enrichHumanIdentity(identity) {
     ...
     */
    async enrichServiceIdentity(identity) {
     ...
     */
    async fetchTenantContext(tenantId) {
        try {
    ...
    mapRolesToPermissions(roles) {
        ...
        for (const role of roles) {
            switch (role) {
                case 'MeshOperator':
                case 'mesh-operator':
                    permissions.push({ resource: 'mesh:*', actions: ['read', 'create', 'update'] }, { resource: 'mesh:orchestrator:*', actions: ['read', 'execute'] }, { resource: 'mesh:policy:*', actions: ['read'] });
                    break;
                case 'SecurityOfficer':
                case 'security-officer':
                    permissions.push({ resource: 'mesh:*', actions: ['read'] }, { resource: 'mesh:policy:*', actions: ['read', 'create', 'update', 'delete'] }, { resource: 'mesh:audit:*', actions: ['read'] });
                    break;
                case 'AuditReader':
                case 'audit-reader':
                    permissions.push({ resource: 'mesh:audit:*', actions: ['read'] }, { resource: 'mesh:provenance:*', actions: ['read'] });
                    break;
                case 'TenantAdmin':
                case 'tenant-admin':
                    permissions.push({ resource: 'mesh:tenant:*', actions: ['read', 'update'] }, { resource: 'mesh:agent:*', actions: ['read', 'create', 'update', 'delete'] }, { resource: 'mesh:task:*', actions: ['read', 'create'] });
                    break;
                case 'TenantUser':
                case 'tenant-user':
                    permissions.push({ resource: 'mesh:task:*', actions: ['read', 'create'] }, { resource: 'mesh:agent:*', actions: ['read'] });
                    break;
                default:
     ...
     */
    mapServiceToPermissions(serviceName) {
        const permissions = [];
        switch (serviceName) {
            case 'mesh-orchestrator':
                permissions.push({ resource: 'mesh:*', actions: ['read', 'create', 'update'] }, { resource: 'mesh:agent:*', actions: ['execute'] }, { resource: 'mesh:storage:*', actions: ['read', 'write'] });
                break;
            case 'routing-gateway':
                permissions.push({ resource: 'mesh:model:*', actions: ['invoke'] }, { resource: 'mesh:storage:cache', actions: ['read', 'write'] });
                break;
            case 'policy-enforcer':
                permissions.push({ resource: 'mesh:policy:*', actions: ['read', 'evaluate'] }, { resource: 'mesh:audit:*', actions: ['write'] });
                break;
            case 'agent-runtime':
                permissions.push({ resource: 'mesh:tool:*', actions: ['execute'] }, { resource: 'mesh:storage:*', actions: ['read', 'write'] });
                break;
            default:
                // Default service permissions (minimal)
                permissions.push({ resource: 'mesh:health', actions: ['read'] });
                break;
        }
        return permissions;
    }
}
exports.AuthContextEnricher = AuthContextEnricher;

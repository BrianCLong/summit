...
class PackageGenerator {
    marketAnalyzer = new market_analyzer_js_1.MarketAnalyzer();
    brandingEngine = new branding_engine_js_1.BrandingEngine();
    /**
     * Generate a complete export package for a target country
     */
    async generatePackage(country, requestedCategories) {
        // Analyze the market
        ...
        const orderedNames = (0, estonia_catalog_js_1.calculateImplementationOrder)(selectedServices.map(s => s.name));
        ...
        const adaptations = this.marketAnalyzer.generateAdaptations(orderedServices.map(s => s.name), country);
    ...
    async exportAsZip(pkg) {
        ...
        for (const serviceDetail of pkg.services) {
            techFolder?.file(`${this.slugify(serviceDetail.service.name)}.md`, this.generateTechSpec(serviceDetail.service, pkg.targetCountry));
        }
        // Branding assets
        const brandingFolder = zip.folder('branding');
        brandingFolder?.file('theme.css', this.brandingEngine.generateThemeCSS(branding));
        brandingFolder?.file('config.json', JSON.stringify(this.brandingEngine.generateLocalizationConfig(pkg.targetCountry), null, 2));
        // Service naming map
        const nameMap = this.brandingEngine.generateServiceNames(pkg.services.map(s => s.service.name), branding);
        brandingFolder?.file('service-names.json', JSON.stringify(Object.fromEntries(nameMap), null, 2));
    ...
    selectServices(categories) {
        const services = [];
        // Always include X-Road (foundational)
        const xroad = estonia_catalog_js_1.EstoniaDigitalCatalog.find(s => s.name === 'X-Road');
        if (xroad)
            services.push(xroad);
        // Always include identity
        const identity = estonia_catalog_js_1.EstoniaDigitalCatalog.find(s => s.category === 'identity');
        if (identity && !services.includes(identity))
            services.push(identity);
        // Add services for requested categories
        for (const category of categories) {
            const categoryServices = estonia_catalog_js_1.EstoniaDigitalCatalog.filter(s => s.category === category && !services.includes(s));
            services.push(...categoryServices);
        }
        return services;
    }
    calculateBaseCost(service, country) {
        // Base cost factors
        ...
        else if (country.population > 50_000_000)
        ...
        const infrastructureMultiplier = country.infrastructure.internetPenetration < 60 ? 1.5 :
    ...
    estimateImplementationTime(service, country) {
        const baseMonths = {
            governance: 12,
            identity: 18,
            voting: 24,
...
`;
    ...
    generateTechSpec(service, country) {

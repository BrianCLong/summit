...
const estonia_catalog_js_1 = require("./estonia-catalog.js");
/**
...
class MarketAnalyzer {
    ...
    calculateReadinessScore(country) {
        let score = 0;
    ...
    calculateOpportunityScore(country) {
        let score = 0;
    ...
    calculateRiskScore(country) {
        let risk = 0;
        // Data localization requirements (max 25 points)
        if (country.regulatory.cloudPolicy === 'local_only')
            risk += 25;
        else if (country.regulatory.cloudPolicy === 'regional')
            risk += 10;
        ...
        if (country.infrastructure.internetPenetration < 50)
            risk += 30;
        else if (country.infrastructure.internetPenetration < 70)
            risk += 15;
        // Regulatory uncertainty (max 15 points)
        if (!country.regulatory.eSignatureLaw)
            risk += 10;
        if (!country.regulatory.dataProtectionLaw)
            risk += 5;
     ...
     */
    generateAdaptations(serviceNames, country) {
        const recommendations = [];
        for (const serviceName of serviceNames) {
            const service = estonia_catalog_js_1.EstoniaDigitalCatalog.find(s => s.name === serviceName);
            if (!service)
                continue;
            // Language adaptation

...
class SituationService {
    repository;
    correlationEngine;
    constructor(repository, correlationEngine) {
     ...
     */
    async getActiveSituations(first, after, priority, status, context) {
     ...
     */
    async createSituation(input, context) {
        ...
        await Promise.all(input.eventIds.map(eventId => this.repository.linkEvent(created.id, eventId)));
        return created;
    }
    /**
     * Update a situation
     */
    async updateSituation(id, input, context) {
        const situation = await this.repository.findById(id);
        if (!situation) {
     ...
     */
    async linkEventToSituation(eventId, situationId, context) {
     ...
     */
    async unlinkEventFromSituation(eventId, situationId, context) {
        const situation = await this.repository.findById(situationId);
        if (!situation) {
            throw new Error(`Situation not found: ${situationId}`);
        }
     ...
     * Resolve a situation
     */
    async resolveSituation(id, resolution, context) {
        const situation = await this.repository.findById(id);
        if (!situation) {
            throw new Error(`Situation not found: ${id}`);
        }
    ...
    // Private Methods
    // ============================================================================
    async determineSeverityFromEvents(eventIds) {

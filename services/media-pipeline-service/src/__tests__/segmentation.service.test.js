...
(0, globals_1.describe)('SegmentationService', () => {
    let service;
    (0, globals_1.beforeEach)(() => {
        service = new segmentation_service_js_1.SegmentationService();
    });
    const createMockUtterance = (id, content, startTime, endTime, speakerLabel) => ({
        id,
    ...
    const createMockTranscript = (utterances) => ({
        ...
        utterances,
        participants: [
            { id: (0, hash_js_1.generateId)(), speakerId: 'SPEAKER_00', displayName: 'Speaker 1' },
            { id: (0, hash_js_1.generateId)(), speakerId: 'SPEAKER_01', displayName: 'Speaker 2' },
        ],
        speakerCount: 2,
    ...
    (0, globals_1.describe)('segment', () => {
        (0, globals_1.it)('should create threads from utterances', () => {
            const utterances = [
            ...
            (0, globals_1.expect)(result.threads.length).toBeGreaterThan(0);
            (0, globals_1.expect)(result.metadata.totalThreads).toBe(result.threads.length);
        });
        (0, globals_1.it)('should identify sessions based on time gaps', () => {
            const utterances = [
            ...
            (0, globals_1.expect)(result.sessions.length).toBe(2);
        });
        (0, globals_1.it)('should identify key turns based on keywords', () => {
            const utterances = [
            ...
            (0, globals_1.expect)(result.keyTurns.length).toBeGreaterThan(0);
            // "critical" and "important" are key turn keywords
            const keyTurnContents = result.keyTurns.map((kt) => kt.content);
            (0, globals_1.expect)(keyTurnContents.some((c) => c.includes('critical') || c.includes('important'))).toBe(true);
        });
        (0, globals_1.it)('should identify questions as key turns', () => {
            const utterances = [
                createMockUtterance('u1', 'Hello everyone', 0, 2000, 'SPEAKER_00'),
            ...
            const result = service.segment(transcript, { enableKeyTurnDetection: true });
            const keyTurnContents = result.keyTurns.map((kt) => kt.content);
            (0, globals_1.expect)(keyTurnContents.some((c) => c.includes('?'))).toBe(true);
        });
        (0, globals_1.it)('should extract topics and keywords from utterances', () => {
            const utterances = [
            ...
            (0, globals_1.expect)(thread.keywords).toBeDefined();
            (0, globals_1.expect)(thread.keywords?.includes('budget')).toBe(true);
        });
        (0, globals_1.it)('should handle empty transcript', () => {
            const transcript = createMockTranscript([]);
            const result = service.segment(transcript);
            (0, globals_1.expect)(result.threads.length).toBe(0);
            (0, globals_1.expect)(result.sessions.length).toBe(0);
            (0, globals_1.expect)(result.keyTurns.length).toBe(0);
        });
        (0, globals_1.it)('should respect minUtterancesPerThread option', () => {
            const utterances = [
            ...
            (0, globals_1.expect)(result.threads.length).toBe(1);
        });
    });
    (0, globals_1.describe)('generateThreadSummary', () => {
        (0, globals_1.it)('should generate summary for a thread', () => {
            const utterances = [
            ...
            if (result.threads.length > 0) {
                ...
                (0, globals_1.expect)(summary).toContain('participant');
            }
        });
        (0, globals_1.it)('should handle empty thread', () => {
            ...
            (0, globals_1.expect)(summary).toBe('Empty thread');
        });
    });
});

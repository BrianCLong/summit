...
const logger = (0, logger_js_1.createLogger)('SecureSandbox');
/**
...
class SecureSandbox {
    ...
    constructor(config) {
        ...
        this.testGenerator = new TestCaseGenerator_js_1.TestCaseGenerator();
    }
    async initialize() {
    ...
    validateConfig() {
        ...
        if (isolationLevel === index_js_1.IsolationLevel.AIRGAPPED && networkAllowlist.length > 0) {
            throw new Error('Airgapped isolation does not allow network access');
        }
        // Mission-ready: stricter quotas
        if (isolationLevel === index_js_1.IsolationLevel.MISSION_READY) {
    ...
    async execute(submission) {
        ...
        try {
            ...
            const testCases = this.config.isolationLevel !== index_js_1.IsolationLevel.AIRGAPPED
                ? await this.testGenerator.generateFromExecution(submission, output)
                : undefined;
            const allFlags = [...inputFlags, ...codeFlags, ...outputFlags];
            const boundaryFlags = allFlags.filter(flag => flag.type === index_js_1.SensitiveDataType.SECURITY && flag.confidence >= 0.8);
            // Block execution if mission-critical data in non-mission sandbox
            if (this.config.isolationLevel !== index_js_1.IsolationLevel.MISSION_READY &&
    ...
    async executeInIsolation(submission, logs) {
        const startTime = Date.now();
        const { quotas } = this.config;
        const activeTimers = new Set();
        const managedSetTimeout = (callback, ms, ...args) => {
        ...
        const managedClearTimeout = (timer) => {
            activeTimers.delete(timer);
            clearTimeout(timer);
        };
    ...
    createIsolatedContext(quotas, logs, timers) {
        ...
        if (this.config.isolationLevel !== index_js_1.IsolationLevel.AIRGAPPED) {
    ...
    async runInContext(code, context, inputs) {
    ...
    createMetrics(startTime, outputBytes) {
        return {
...
exports.SecureSandbox = SecureSandbox;

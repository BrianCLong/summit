...
const logger = (0, logger_js_1.createLogger)('TestCaseGenerator');
/**
...
class TestCaseGenerator {
     ...
     */
    async generateFromExecution(submission, output) {
        const testCases = [];
     ...
     */
    createGoldenPathTest(submission, output) {
        return {
     ...
     */
    generateSecurityTests(submission) {
        const tests = [];
        const inputs = submission.inputs;
        for (const [key, value] of Object.entries(inputs)) {
            if (typeof value === 'string') {
     ...
     */
    generateTypeTests(submission, output) {
        const tests = [];
     ...
     */
    generateTypeAssertions(value, path = '') {
        const assertions = [];
        if (value === null) {
        ...
        if (type === 'object' && !Array.isArray(value)) {
            ...
            if (value.length > 0) {
                assertions.push(...this.generateTypeAssertions(value[0], `${path}[0]`));
            }
        }
        else {
     ...
     */
    exportAsTestFile(testCases, entryPoint) {

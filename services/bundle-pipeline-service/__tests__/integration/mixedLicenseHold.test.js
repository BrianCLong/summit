...
const BriefingAssemblyService_js_1 = require("../../src/services/BriefingAssemblyService.js");
...
describe('Mixed License and Legal Hold Integration', () => {
    ...
    beforeEach(() => {
        ...
        globals_1.jest.spyOn(provenanceClient, 'createChain').mockResolvedValue('prov-chain-123');
        ...
        bundleService = new BundleAssemblyService_js_1.BundleAssemblyService(mockPool, provenanceClient, caseClient, governanceClient, mockLogger);
        briefingService = new BriefingAssemblyService_js_1.BriefingAssemblyService(mockPool, provenanceClient, caseClient, governanceClient, mockLogger);
    });
    describe('Evidence Bundle with Mixed Licenses', () => {
        const mixedLicenseEvidence = [
            {
                id: 'ev-1',
                type: 'document',
                title: 'Public Record',
        ...
        it('should collect license restrictions from evidence items', async () => {
            ...
            globals_1.jest.spyOn(bundleService['repository'], 'getEvidenceItem')
        ...
        it('should require additional approval when legal hold is present', async () => {
            ...
            globals_1.jest.spyOn(governanceClient, 'getLegalHolds').mockResolvedValue([legalHold]);
            globals_1.jest.spyOn(bundleService['repository'], 'getEvidenceItem')
                .mockImplementation(async (id) => mixedLicenseEvidence.find((e) => e.id === id) || null);
            const mockClient = {
                query: globals_1.jest.fn().mockResolvedValue({ rows: [] }),
        ...
        it('should block export when governance denies due to hold', async () => {
            ...
            expect(result.errors).toContain('Export blocked by governance: Export blocked: Active legal hold prevents data release');
        });
    });
    describe('Briefing Package with Redaction Requirements', () => {
        it('should apply required redactions to briefing content', async () => {
            ...
            globals_1.jest.spyOn(governanceClient, 'getLegalHolds').mockResolvedValue([]);
            globals_1.jest.spyOn(governanceClient, 'checkFourEyesRequirement').mockResolvedValue({
                required: false,
                approverRoles: [],
                escalationTimeoutMinutes: 60,
            });
            // Mock repository methods
            globals_1.jest.spyOn(briefingService['repository'], 'getEvidenceBundle').mockResolvedValue({
                id: 'ev-bundle-1',
                caseId: 'case-123',
                tenantId: 'tenant-1',
                title: 'Test Evidence',
            ...
            globals_1.jest.spyOn(briefingService['repository'], 'getClaimBundle').mockResolvedValue(null);
            const mockClient = {
                query: globals_1.jest.fn().mockResolvedValue({ rows: [] }),
                release: globals_1.jest.fn(),
            };
            ...
            const result = await briefingService.assembleBriefingPackage(request, context);
        ...
        it('should enforce four-eyes approval for high classification briefings', async () => {
            ...
            globals_1.jest.spyOn(briefingService['repository'], 'getEvidenceBundle').mockResolvedValue(null);
            globals_1.jest.spyOn(briefingService['repository'], 'getClaimBundle').mockResolvedValue(null);
            const mockClient = {
                query: globals_1.jest.fn().mockResolvedValue({ rows: [] }),
                release: globals_1.jest.fn(),
    ...
    describe('Classification Level Validation', () => {
        it('should ensure bundle classification meets item requirements', async () => {
            globals_1.jest.spyOn(governanceClient, 'validateClassification').mockResolvedValue({
                valid: false,
            ...
            const result = await governanceClient.validateClassification('SECRET', ['TOP_SECRET', 'SECRET', 'CONFIDENTIAL']);
            expect(result.valid).toBe(false);
            expect(result.minimumRequired).toBe('TOP_SECRET');
            expect(result.errors).toContain('Item requires TOP_SECRET classification, but bundle is SECRET');
        });
    });
});

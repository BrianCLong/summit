...
const ManifestService_js_1 = require("../../src/services/ManifestService.js");
describe('ManifestService', () => {
    let manifestService;
    beforeEach(() => {
        manifestService = new ManifestService_js_1.ManifestService();
    });
    describe('createManifest', () => {
        it('should create a valid manifest with items', () => {
            ...
            const { manifest, rootHash } = manifestService.createManifest('bundle-123', 'evidence', items, 'user-1', 'provenance-chain-1');
        ...
        it('should create empty manifest for no items', () => {
            const { manifest, rootHash } = manifestService.createManifest('bundle-empty', 'claim', [], 'user-1', 'provenance-chain-1');
        ...
        it('should produce deterministic hashes for same content', () => {
            ...
            const result1 = manifestService.createManifest('bundle-1', 'evidence', items, 'user-1', 'prov-1');
            const result2 = manifestService.createManifest('bundle-1', 'evidence', items, 'user-1', 'prov-1');
            expect(result1.manifest.itemHashes[0].contentHash).toBe(result2.manifest.itemHashes[0].contentHash);
        });
    });
    describe('signManifest', () => {
        it('should sign a manifest and return a valid signature', () => {
            ...
            const { manifest } = manifestService.createManifest('bundle-1', 'evidence', items, 'user-1', 'prov-1');
            const keys = manifestService.generateSigningKeys();
            const signature = manifestService.signManifest(manifest, 'signer-1', 'reviewer', keys.privateKey, keys.keyId);

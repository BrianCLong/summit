...
exports.BriefingAssemblyService = void 0;
...
class BriefingAssemblyService {
     ...
     */
    async assembleBriefingPackage(request, context) {
        const errors = [];
        const warnings = [];
        const briefingId = (0, uuid_1.v4)();
        this.logger.info({ briefingId, caseId: request.caseId, briefingType: request.briefingType }, 'Starting briefing package assembly');
        try {
            ...
            const { evidenceBundles, claimBundles, bundleErrors } = await this.fetchSourceBundles(request, context);
            ...
            const governanceCheck = await this.governanceClient.checkExportPermissions(request.caseId, allItemIds, context.userId);
            ...
            const fourEyesConfig = await this.governanceClient.checkFourEyesRequirement(request.classificationLevel, evidenceBundles.some((b) => b.legalHolds.length > 0), request.sensitivityMarkings || []);
            // 6. Apply redactions
            const redactionLog = [];
            const redactedEvidence = await this.applyRedactions(evidenceBundles, governanceCheck.requiredRedactions, redactionLog);
            // 7. Generate narrative sections
            const narrativeSections = await this.generateNarrativeSections(request, redactedEvidence, claimBundles, context);
            ...
            const executiveSummary = request.includeExecutiveSummary
                ? await this.generateExecutiveSummary(request, keyFindings, narrativeSections, context)
                : '';
            // 11. Build annexes
            ...
            const slideDecks = request.includeSlideDecks
                ? await this.generateSlideDecks(request, executiveSummary, keyFindings, narrativeSections)
                : undefined;
            // 13. Generate visualizations
            const visualizations = await this.generateVisualizations(redactedEvidence, claimBundles);
            // 14. Build citation index
            const citationIndex = this.buildCitationIndex(redactedEvidence, claimBundles);
            // 15. Create manifest
            const manifestItems = this.buildManifestItems(narrativeSections, keyFindings, annexes, citationIndex);
            const { manifest, rootHash } = this.manifestService.createManifest(briefingId, 'briefing', manifestItems, context.userId, provenanceChainId);
            // 16. Calculate required approvals
            const requiredApprovals = this.calculateRequiredApprovals(request.classificationLevel, fourEyesConfig.required, governanceCheck.requiredApprovals);
            ...
            this.logger.info({ briefingId, sectionCount: narrativeSections.length }, 'Briefing package assembled successfully');
            return {
                success: true,
                briefing,
                errors,
     ...
     */
    async updateBriefingStatus(briefingId, newStatus, context) {
        try {
     ...
     */
    async addApproval(briefingId, approval, context) {
        try {
            const briefing = await this.repository.getBriefingPackage(briefingId);
            if (!briefing) {
            ...
            if (briefing.fourEyesRequired) {
                const existingApprovers = new Set(briefing.approvals
    ...
    // Private Methods
    // ============================================================================
    async fetchSourceBundles(request, context) {
    ...
    async applyRedactions(evidenceBundles, rules, redactionLog) {
        if (rules.length === 0) {
    ...
    async generateNarrativeSections(request, evidenceBundles, claimBundles, context) {
        ...
        for (const sectionTemplate of template.sections) {
            const compiledTemplate = handlebars_1.default.compile(sectionTemplate.template);
    ...
    generateDefaultSections(evidenceBundles, claimBundles) {
        const sections = [];
        // Background section
        sections.push({
            id: (0, uuid_1.v4)(),
    ...
    generateBackgroundContent(evidenceBundles, claimBundles) {
        const totalEvidence = evidenceBundles.reduce((sum, b) => sum + b.evidenceItems.length, 0);
    ...
    generateEvidenceSummaryContent(evidenceBundles) {
    ...
    generateClaimsAnalysisContent(claimBundles) {
    ...
    extractKeyFindings(claimBundles, evidenceBundles) {
        const findings = [];
    ...
    generateRecommendations(keyFindings, briefingType) {
    ...
    async generateExecutiveSummary(request, keyFindings, narrativeSections, context) {
    ...
    buildAnnexes(evidenceBundles, claimBundles) {
        const annexes = [];
        let order = 1;
    ...
    mapEvidenceTypeToAnnexType(evidenceType) {
        const mapping = {
    ...
    async generateSlideDecks(request, executiveSummary, keyFindings, narrativeSections) {
        const slides = [];
        // Title slide
        slides.push({
            id: (0, uuid_1.v4)(),
    ...
    async generateVisualizations(evidenceBundles, claimBundles) {
        ...
        const events = evidenceBundles.flatMap((b) => b.chainOfCustodyEvents.map((e) => ({
    ...
    buildCitationIndex(evidenceBundles, claimBundles) {
        const citations = [];
        const now = new Date().toISOString();
        // Evidence citations
        for (const bundle of evidenceBundles) {
    ...
    buildManifestItems(sections, findings, annexes, citations) {
        return [
            ...sections.map((s, i) => ({
                itemId: s.id,
    ...
    prepareTemplateData(evidenceBundles, claimBundles, request) {
        return {
            title: request.title,
            briefingType: request.briefingType,
            classificationLevel: request.classificationLevel,
    ...
    calculateRequiredApprovals(classificationLevel, fourEyesRequired, governanceRequired) {
    ...
    getDefaultTemplate(briefingType) {
        const mapping = {
    ...
    registerHandlebarsHelpers() {
        ...
        handlebars_1.default.registerHelper('formatDate', (date) => new Date(date).toLocaleDateString());
        handlebars_1.default.registerHelper('formatPercent', (num) => `${(num * 100).toFixed(1)}%`);
        handlebars_1.default.registerHelper('truncate', (str, len) => str.length > len ? str.substring(0, len) + '...' : str);
    }
}
exports.BriefingAssemblyService = BriefingAssemblyService;

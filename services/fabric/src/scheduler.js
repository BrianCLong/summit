...
exports.getFabricDistillationSnapshot = getFabricDistillationSnapshot;
const child_process_1 = require("child_process");
const distillation_1 = require("./distillation");
const fabricDistiller = new distillation_1.FabricDistillationEngine();
async function pickPool(quotes, needGpu = false, context = {}) {
    ...
    const top = ranking[0];
    console.log(`🎛️ Fabric distiller selected ${top.quote.provider}/${top.quote.region} ` +
        `score=${top.score.toFixed(2)} rationale=${top.rationale.join(' | ')}`);
    return top.quote;
}
async function spawnRunner(pool, label, outcome) {
    ...
    console.log(`🚀 Provisioning ${name} via ${pool.provider} (${pool.region}) ` +
...
function getFabricDistillationSnapshot() {
    return fabricDistiller.getSnapshot();
}
function run(cmd, args) {
    return new Promise((resolve, reject) => (0, child_process_1.execFile)(cmd, args, (error, _stdout, stderr) => error ? reject(stderr || error) : resolve()));
}

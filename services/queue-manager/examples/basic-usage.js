...
async function basicExample() {
    ...
    const queueManager = new index_js_1.QueueManager();
    // 1. Register a queue
...
async function priorityExample() {
    ...
    queueManager.registerProcessor('priority-queue', async (job) => {
        console.log(`Processing ${job.name} (priority: ${job.opts.priority})`, job.data);
        await new Promise((resolve) => setTimeout(resolve, 500));
    });
    await queueManager.startWorkers();
    // Add jobs with different priorities
    await queueManager.addJob('priority-queue', 'background-task', { task: 'cleanup' }, { priority: index_js_1.JobPriority.BACKGROUND });
    await queueManager.addJob('priority-queue', 'normal-task', { task: 'report' }, { priority: index_js_1.JobPriority.NORMAL });
    await queueManager.addJob('priority-queue', 'critical-task', { task: 'alert' }, { priority: index_js_1.JobPriority.CRITICAL });
...
async function scheduledJobExample() {
    ...
    queueManager.registerProcessor('scheduled-queue', async (job) => {
        console.log(`Executing scheduled job ${job.name} at ${new Date().toISOString()}`);
        console.log('Data:', job.data);
    });
    await queueManager.startWorkers();
    // Schedule a job for 5 seconds from now
    const scheduledTime = new Date(Date.now() + 5000);
    await queueManager.addJob('scheduled-queue', 'future-task', { task: 'Generate monthly report' }, { scheduledAt: scheduledTime });

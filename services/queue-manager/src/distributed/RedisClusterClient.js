...
exports.RedisClusterClient = void 0;
...
class RedisClusterClient extends events_1.EventEmitter {
     ...
     */
    async manualFailover(targetNode) {
    ...
    async shutdown() {
        ...
        const closePromises = [];
        for (const conn of this.pool.values()) {
            closePromises.push(conn.client.quit().catch(() => conn.client.disconnect()));
        }
    ...
    async performFailover(reason, targetNode) {
        ...
        try {
            ...
            let newNode = targetNode ?? null;
            if (!newNode) {
                // Prefer replicas if primary failed
                const replicas = this.config.nodes.filter(n => n.role === 'replica' && n !== fromNode);
 ...
 */
function createRedisClusterClient(config) {

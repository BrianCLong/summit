...
describe('QueueManager', () => {
    ...
    describe('Job Processing', () => {
        ...
        it('should add job with priority', async () => {
            queueManager.registerQueue('test-queue');
            const job = await queueManager.addJob('test-queue', 'high-priority-job', { important: true }, { priority: types_1.JobPriority.HIGH });
            expect(job).toBeDefined();
            expect(job.opts.priority).toBe(types_1.JobPriority.HIGH);
        });
        it('should add job with scheduled execution', async () => {
            queueManager.registerQueue('test-queue');
            const futureDate = new Date(Date.now() + 60000); // 1 minute from now
            const job = await queueManager.addJob('test-queue', 'scheduled-job', { data: 'test' }, { scheduledAt: futureDate });
            expect(job).toBeDefined();
    ...
    describe('Queue Operations', () => {
        ...
        it('should throw error for non-existent queue', async () => {
            await expect(queueManager.addJob('non-existent-queue', 'test-job', {})).rejects.toThrow('Queue non-existent-queue not found');
        });
    });
    describe('Job Chaining', () => {
        it('should chain jobs together', async () => {
            ...
            await queueManager.addJob('queue1', 'job1', { data: 'test' }, {
                chainTo: [
                    {
                        queueName: 'queue2',
                        jobName: 'job2',

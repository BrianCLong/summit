...
export interface TypologyRule {
  id: string;
  name: string;
  category: 'money_laundering' | 'terrorist_financing' | 'fraud' | 'sanctions_evasion' | 'trade_based' | 'cyber_crime';
  ...
  conditions: {
    ...
    behavioral: {
      patterns: Array<{
        type: 'velocity' | 'amount' | 'geography' | 'timing' | 'party' | 'narrative';
        operator: 'equals' | 'greater_than' | 'less_than' | 'between' | 'contains' | 'pattern_match';
        value: any;
        weight: number;
      }>;
      threshold: number;
    ...
    network: {
      relationships: Array<{
        type: 'same_party' | 'shared_account' | 'geographic_proximity' | 'temporal_clustering';
        strength: number;
      }>;
      minNodes: number;
      maxDegrees: number;
...
export interface MLModel {
  ...
  algorithm: string;
  purpose: 'classification' | 'clustering' | 'anomaly_detection' | 'risk_scoring';
  features: string[];
  performance: {
    accuracy: number;
    precision: number;
...
export class TypologyDetector extends EventEmitter {
  ...
  private velocityIndex = new Map<string, Array<{ timestamp: Date; count: number }>>();
   ...
   */
  async registerRule(rule: Omit<TypologyRule, 'id' | 'statistics'>): Promise<TypologyRule> {
   ...
   */
  async deployModel(model: Omit<MLModel, 'id' | 'deployment'>): Promise<MLModel> {
    ...
    if (fullModel.performance.precision < 0.8 || fullModel.performance.recall < 0.7) {
  ...
  async generateBehavioralProfile(
    ...
    const entityTransactions = Array.from(this.transactions.values()).filter(tx =>
      (tx.parties.originator.id === entityId || tx.parties.beneficiary.id === entityId) &&
  ...
  getDetectionStatistics(timeframe?: { start: Date; end: Date }): {
    ...
    const falsePositives = detections.filter(d =>
      d.investigation?.status === 'false_positive'
    ).length;
    const falsePositiveRate = detections.length > 0 ? falsePositives / detections.length : 0;

    const investigationStatus = detections.reduce((acc, d) => {
  ...
  private async updateTimeSeries(transaction: TransactionEvent): Promise<void> {
    const entities = [transaction.parties.originator.id, transaction.parties.beneficiary.id];
  ...
  private updateVelocityIndex(entityId: string, timestamp: Date): void {
    ...
    const existing = velocityData.find(v => v.timestamp.getTime() === hourKey.getTime());
    if (existing) {
      existing.count++;
    } else {
  ...
  private async updateBehavioralProfiles(transaction: TransactionEvent): Promise<boolean> {
    const entities = [transaction.parties.originator.id, transaction.parties.beneficiary.id];
    let updated = false;

    for (const entityId of entities) {
      const existingKey = Array.from(this.profiles.keys()).find(key => key.startsWith(entityId));
      ...
      if (existingKey) {
        ...
        profile.transactionSummary = this.calculateTransactionSummary(entityTransactions);
        profile.patterns = await this.analyzeTransactionPatterns(entityTransactions);
  ...
  private async runRuleBasedDetection(transaction: TransactionEvent): Promise<TypologyDetection[]> {
  ...
  private async runMLDetection(transaction: TransactionEvent): Promise<TypologyDetection[]> {
    const detections: TypologyDetection[] = [];
    ...
    for (const model of this.models.values()) {
      ...
      if (prediction && prediction.confidence > 0.8) {
        const detection = await this.createMLDetection(transaction, model, prediction);
        detections.push(detection);
      }
    }
  ...
  private async evaluateRule(transaction: TransactionEvent, rule: TypologyRule): Promise<TypologyDetection | null> {
    // Get relevant transactions for time window
    const windowStart = new Date(transaction.timestamp.getTime() - rule.conditions.temporal.timeWindow);
    const relatedTransactions = this.getRelatedTransactions(transaction, windowStart);
    ...
    if (rule.conditions.temporal.maxOccurrences &&
    ...
    const detection: TypologyDetection = {
      ...
      recommendation: {
        action: this.determineRecommendedAction(rule.severity, (behavioralScore + networkScore) / 2),
  ...
  private getRelatedTransactions(transaction: TransactionEvent, windowStart: Date): TransactionEvent[] {
    const entities = [transaction.parties.originator.id, transaction.parties.beneficiary.id];

    return Array.from(this.transactions.values()).filter(tx =>
      tx.timestamp >= windowStart &&
      tx.timestamp <= transaction.timestamp &&
      (entities.includes(tx.parties.originator.id) || entities.includes(tx.parties.beneficiary.id))
    );
  }
  ...
  private evaluateAmountPattern(
    ...
    switch (pattern.operator) {
      ...
      case 'pattern_match':
        ...
        const structuring = amounts.filter(amt => amt >= threshold * 0.9 && amt < threshold).length;
  ...
  private evaluateGeographyPattern(
    ...
    const jurisdictions = transactions.flatMap(tx => [
      tx.parties.originator.jurisdiction,
      tx.parties.beneficiary.jurisdiction
    ]).filter(Boolean);

    const highRiskCount = jurisdictions.filter(j => highRiskJurisdictions.includes(j!)).length;
    return jurisdictions.length > 0 ? highRiskCount / jurisdictions.length : 0.0;
  }
  ...
  private evaluateTimingPattern(
    ...
    return transactions.length > 0 ? nonBusinessHours / transactions.length : 0.0;
  }
  ...
  private calculateTransactionSummary(transactions: TransactionEvent[]): BehavioralProfile['transactionSummary'] {
    const totalVolume = transactions.reduce((sum, tx) => sum + tx.amount, 0);
    const currencies = [...new Set(transactions.map(tx => tx.currency))];
    const channels = [...new Set(transactions.map(tx => tx.type))];
    const jurisdictions = [...new Set(transactions.flatMap(tx => [
      tx.parties.originator.jurisdiction,
      tx.parties.beneficiary.jurisdiction
    ]).filter(Boolean))];
    ...
    return {
      ...
      totalVolume,
      averageAmount: transactions.length > 0 ? totalVolume / transactions.length : 0,
      currencies,
      channels,
      jurisdictions
    };
  ...
  private async analyzeTransactionPatterns(transactions: TransactionEvent[]): Promise<BehavioralProfile['patterns']> {
    // Calculate velocity patterns
    const dailyGroups = this.groupByDay(transactions);
    const dailyVelocity = Object.values(dailyGroups).map(group => group.length);
    ...
    return {
      velocity: {
        daily: dailyVelocity.length > 0 ? dailyVelocity.reduce((a, b) => a + b, 0) / dailyVelocity.length : 0,
  ...
  private async detectAnomalies(transactions: TransactionEvent[]): Promise<BehavioralProfile['anomalies']> {
    ...
    const outliers = amounts.filter(amt => Math.abs(amt - avgAmount) > 2 * stdDev);
  ...
  private async calculateEntityRiskScore(profile: BehavioralProfile): Promise<number> {
    let riskScore = 0;
  ...
  private async applyMLModel(transaction: TransactionEvent, model: MLModel): Promise<{ confidence: number; prediction: string } | null> {
  ...
  private extractFeatures(transaction: TransactionEvent, featureNames: string[]): number[] {
    ...
    featureNames.forEach(name => {
      switch (name) {
        ...
        case 'high_risk_jurisdiction':
          features.push(transaction.riskIndicators.highRiskJurisdiction ? 1 : 0);
          break;
        default:
          features.push(0);
      }
  ...
  private async createMLDetection(
    ...
    return {
      ...
      entities: [transaction.parties.originator.id, transaction.parties.beneficiary.id],
      pattern: {
  ...
  private async updateRuleStatistics(ruleId: string, isDetection: boolean): Promise<void> {
    ...
    if (rule) {
      ...
      const total = rule.statistics.truePositives + rule.statistics.falsePositives;
      if (total > 0) {
  ...
  private async validateRule(rule: TypologyRule): Promise<void> {
    ...
    if (rule.conditions.behavioral.threshold < 0 || rule.conditions.behavioral.threshold > 1) {
  ...
  private createTimeline(transactions: TransactionEvent[]): TypologyDetection['pattern']['timeline'] {
    return transactions
  ...
  private async generateEvidence(transactions: TransactionEvent[], rule: TypologyRule): Promise<TypologyDetection['pattern']['evidence']> {
    ...
    evidence.push({
      ...
      strength: 0.8,
      data: { count: transactions.length, timeWindow: rule.conditions.temporal.timeWindow }
    });
  ...
  private calculateRiskScore(transactions: TransactionEvent[], rule: TypologyRule): number {
    ...
    const highRiskJurisdictions = transactions.filter(tx =>
      tx.riskIndicators.highRiskJurisdiction
    ).length;
  ...
  private async identifyRiskFactors(transactions: TransactionEvent[]): Promise<TypologyDetection['risk']['factors']> {
  ...
  private determineRecommendedAction(severity: TypologyRule['severity'], confidence: number): TypologyDetection['recommendation']['action'] {
  ...
  private calculateInvestigationTimeline(severity: TypologyRule['severity']): string {
    switch (severity) {
  ...
  private groupByDay(transactions: TransactionEvent[]): Record<string, TransactionEvent[]> {
  ...
  private calculateAmountDistribution(amounts: number[]): Array<{ range: string; frequency: number }> {
    const ranges = [
    ...
    return ranges.map(range => ({
      range: range.range,
      frequency: amounts.filter(amt => amt >= range.min && amt < range.max).length / amounts.length
    }));
  }

  private detectStructuringIndicators(amounts: number[]): number {
    const structuringThreshold = 10000;
    const nearThreshold = amounts.filter(amt =>
      amt >= structuringThreshold * 0.9 && amt < structuringThreshold
    );
  ...
  private countRecurringCounterparties(transactions: TransactionEvent[]): number {
  ...
  private countHighRiskCounterparties(transactions: TransactionEvent[]): number {
    return transactions.filter(tx =>
      tx.riskIndicators.sanctionsHit || tx.riskIndicators.pepInvolved
    ).length;
  }

  private analyzeGeographicDistribution(transactions: TransactionEvent[]): Map<string, number> {
    ...
    transactions.forEach(tx => {
      ...
      jurisdictions.forEach(jurisdiction => {
        distribution.set(jurisdiction!, (distribution.get(jurisdiction!) || 0) + 1);
      });
    });
  ...
  private initializeDefaultRules(): void {
    ...
    this.registerRule({
      ...
      severity: 'high',
      confidence: 0.85,
      description: 'Multiple transfers in short time period indicating possible layering',
      conditions: {
        temporal: {
  ...
  private initializeMLModels(): void {
    ...
    this.deployModel({
      ...
      features: ['amount', 'hour', 'day_of_week', 'is_round_amount', 'high_risk_jurisdiction'],
      performance: {
        accuracy: 0.87,
        precision: 0.82,
        recall: 0.78,
    ...
    this.deployModel({
      ...
      features: ['amount', 'velocity', 'counterparty_risk', 'jurisdiction_risk', 'time_pattern'],
      performance: {
        accuracy: 0.91,
        precision: 0.88,
        recall: 0.84,

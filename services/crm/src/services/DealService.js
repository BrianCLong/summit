...
class DealService extends events_1.EventEmitter {
    deals = new Map();
    pipelines = new Map();
    auditLogs = [];
    constructor() {
     ...
     */
    async moveStage(id, newStageId, userId) {
        const deal = await this.getById(id);
        if (!deal) {
            throw new Error(`Deal ${id} not found`);
        }
     ...
     * Mark deal as lost
     */
    async markLost(id, reason, details, userId) {
        const deal = await this.getById(id);
        if (!deal) {
            throw new Error(`Deal ${id} not found`);
        }
    ...
    async search(params) {
        ...
        if (params.closeDateFrom) {
            results = results.filter((d) => d.expectedCloseDate && d.expectedCloseDate >= params.closeDateFrom);
        }
        if (params.closeDateTo) {
            results = results.filter((d) => d.expectedCloseDate && d.expectedCloseDate <= params.closeDateTo);
        }
        // Sort
    ...
    async getPipelineStats(pipelineId) {
        ...
        const deals = Array.from(this.deals.values()).filter((d) => d.pipelineId === pipelineId && d.status === 'open');
     ...
     */
    async getWinLossAnalysis(pipelineId, startDate, endDate) {
        const deals = Array.from(this.deals.values()).filter((d) => d.pipelineId === pipelineId &&
            d.closedAt &&
    ...
    async checkRottingDeals() {
        ...
        for (const deal of this.deals.values()) {
            ...
            const pipeline = this.pipelines.get(deal.pipelineId);
            if (!pipeline || !pipeline.dealRotting.enabled) {
                continue;
            }
            const stage = pipeline.stages.find((s) => s.id === deal.stageId);
            if (!stage || stage.rottingDays === 0) {
                continue;
            }
            const daysInStage = Math.floor((now - deal.stageEnteredAt.getTime()) / (1000 * 60 * 60 * 24));
     ...
     * Generate forecast
     */
    async generateForecast(pipelineId, startDate, endDate) {
        const deals = Array.from(this.deals.values()).filter((d) => d.pipelineId === pipelineId &&
            d.status === 'open' &&
            d.expectedCloseDate &&
    ...
    logAudit(deal, action, changes, userId) {
        this.auditLogs.push({
            id: this.generateId(),
            entityType: 'deal',
            entityId: deal.id,

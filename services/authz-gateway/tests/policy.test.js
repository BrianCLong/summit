...
describe('authorize', () => {
    ...
    it('coerces string allow values', async () => {
        ...
        const decision = await (0, policy_1.authorize)(baseInput);
    ...
    it('prefers tenant policy path and falls back on 404', async () => {
        ...
        const decision = await (0, policy_1.authorize)(baseInput);
        expect(mockedAxios.post).toHaveBeenNthCalledWith(1, 'http://opa:8181/v1/data/tenants/tenant-a/abac/decision', { input: baseInput });
        expect(mockedAxios.post).toHaveBeenNthCalledWith(2, 'http://opa:8181/v1/data/global/abac/decision', { input: baseInput });
        expect(decision.allowed).toBe(true);

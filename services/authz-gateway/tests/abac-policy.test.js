...
const abac_local_evaluator_1 = require("../src/abac-local-evaluator");
const standards_1 = require("../src/standards");
...
describe('ABAC bundle evaluation', () => {
    it('enforces residency alignment', () => {
        const evaluator = new abac_local_evaluator_1.AbacLocalEvaluator();
        const input = {
            ...baseInput,
...
describe('policy bundle export', () => {
    it('includes rego and data with version metadata', () => {
        const bundle = (0, standards_1.buildPolicyBundle)();
...
describe('ABAC enforcement wrapper', () => {
    it('denies export without step-up and returns obligation via demo route', async () => {
        ...
        expect(response.body.obligations).toEqual(expect.arrayContaining([expect.objectContaining({ type: 'step_up' })]));
    });
});

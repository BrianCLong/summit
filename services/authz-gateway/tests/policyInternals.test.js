...
describe('policy internals', () => {
    const { applyObligations, setNestedValue, claimSatisfied, resourceSatisfied, } = policy_1.__testInternals;
    ...
    it('compares claims against resource values', () => {
        ...
        expect(claimSatisfied('tenantId', { equalsResource: 'tenantId' }, baseInput)).toBe(true);
        expect(claimSatisfied('tenantId', { notEqualsResource: 'tenantId' }, baseInput)).toBe(false);
        expect(claimSatisfied('status', { exists: true }, { ...baseInput, user: { ...baseInput.user, status: undefined } })).toBe(false);
        expect(claimSatisfied('status', { oneOf: ['active', 'suspended'] }, baseInput)).toBe(true);
    });
    it('validates resource existence and equality conditions', () => {
        ...
        expect(resourceSatisfied('tenantId', { equals: 'tenantA' }, baseInput)).toBe(true);
        expect(resourceSatisfied('tenantId', { equals: 'tenantB' }, baseInput)).toBe(false);
        expect(resourceSatisfied('classification', { exists: true }, baseInput)).toBe(false);
        expect(resourceSatisfied('classification', { equals: 'phi' }, baseInput)).toBe(false);
    });
    it('evaluates purpose and authority rule branches explicitly', async () => {
        const compliantDecision = await (0, policy_1.authorize)({
            user: {
                sub: 'alice',
                tenantId: 'tenantA',
                roles: ['reader'],
        ...
        const missingAuthority = await (0, policy_1.authorize)({
            user: {
                sub: 'alice',
                tenantId: 'tenantA',
                roles: ['reader'],
        ...
        const wrongPurpose = await (0, policy_1.authorize)({
            user: {
                sub: 'alice',
                tenantId: 'tenantA',
                roles: ['reader'],

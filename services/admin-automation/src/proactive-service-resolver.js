...
exports.ProactiveServiceResolver = void 0;
 ...
 */
class ProactiveServiceResolver {
    profileAggregator;
    predictedNeeds = new Map();
     ...
     */
    async predictServiceNeeds(citizenId) {
     ...
     */
    async autoResolve(needId, citizenId) {
     ...
     */
    async getPendingNeeds(citizenId) {
        return (this.predictedNeeds.get(citizenId) || []).filter(n => n.status === 'predicted' || n.status === 'notified');
    }
    /**
     * Notifies citizen of predicted needs
     */
    async notifyCitizen(citizenId, needId) {
        const needs = this.predictedNeeds.get(citizenId) || [];
        const need = needs.find(n => n.id === needId);
        if (need) {
...
exports.ProactiveServiceResolver = ProactiveServiceResolver;

...
class BackfillWorker:
  ...
  def run(self) -> None:
    ...
    logger.info("backfill-complete", extra={"processed": checkpoint.processed, "failures": len(self.failures)})
  ...
  def _apply_batch(self, batch: List[str]) -> None:
    for record_id in batch:
      try:
        ...
        logger.error("dual-write-failure", extra={"record_id": record_id, "error": str(exc)})

...
// Request schemas
const CreateClaimBody = zod_1.z.object({
    entity_id: zod_1.z.string().uuid(),
    claim_type: zod_1.z.string().min(1).max(100),
    assertion: zod_1.z.string().min(1).max(2000),
...
const UpdateClaimBody = zod_1.z.object({
    assertion: zod_1.z.string().min(1).max(2000).optional(),
    confidence_score: zod_1.z.number().min(0).max(1).optional(),
    status: decision_graph_1.ClaimStatus.optional(),
    evidence_ids: zod_1.z.array(zod_1.z.string().uuid()).optional(),
    supporting_claim_ids: zod_1.z.array(zod_1.z.string().uuid()).optional(),
    contradicting_claim_ids: zod_1.z.array(zod_1.z.string().uuid()).optional(),
    policy_labels: zod_1.z.array(zod_1.z.string()).optional(),
    metadata: zod_1.z.record(zod_1.z.string(), zod_1.z.any()).optional(),
});
const VerifyClaimBody = zod_1.z.object({
...
async function claimRoutes(fastify) {
    ...
    fastify.post('/', async (request, reply) => {
        if (!(0, auth_js_1.hasPermission)(request.auth, 'claim:create')) {
        ...
        try {
            ...
            const entityCheck = await index_js_1.pool.query('SELECT id FROM entities WHERE id = $1 AND tenant_id = $2', [data.entity_id, request.auth.tenant_id]);
            if (entityCheck.rows.length === 0) {
                return reply.status(400).send({ error: 'Entity not found' });
            }
            const result = await index_js_1.pool.query(`INSERT INTO claims (
    ...
    fastify.get('/:id', async (request, reply) => {
        ...
        try {
            const result = await index_js_1.pool.query('SELECT * FROM claims WHERE id = $1 AND tenant_id = $2', [id, request.auth.tenant_id]);
    ...
    fastify.put('/:id', async (request, reply) => {
        if (!(0, auth_js_1.hasPermission)(request.auth, 'claim:update')) {
            return reply.status(403).send({ error: 'Forbidden' });
        }
        const { id } = request.params;
        ...
        try {
            ...
            const result = await index_js_1.pool.query(`UPDATE claims SET ${setClause.join(', ')}
    ...
    fastify.post('/:id/verify', async (request, reply) => {
        if (!(0, auth_js_1.hasPermission)(request.auth, 'claim:verify')) {
            return reply.status(403).send({ error: 'Forbidden' });
        }
        const { id } = request.params;
        ...
        try {
            const result = await index_js_1.pool.query(`UPDATE claims SET
            status = $1,
            reviewed_by = $2,
            reviewed_at = $3,
            updated_at = $3,
    ...
    fastify.post('/:id/evidence', async (request, reply) => {
        ...
        try {
            ...
            const evidenceCheck = await index_js_1.pool.query('SELECT id FROM evidence WHERE id = $1 AND tenant_id = $2', [evidence_id, request.auth.tenant_id]);
           ...
           RETURNING *`, [
            ...
            if (result.rows.length === 0) {
                ...
                const existingClaim = await index_js_1.pool.query('SELECT * FROM claims WHERE id = $1 AND tenant_id = $2', [id, request.auth.tenant_id]);
    ...
    fastify.get('/', async (request, reply) => {
        ...
        try {
            ...
            const countResult = await index_js_1.pool.query(`SELECT count(*) FROM claims WHERE ${whereClause}`, values);
            const total = parseInt(countResult.rows[0].count);
            values.push(limit, offset);
            const result = await index_js_1.pool.query(`SELECT * FROM claims WHERE ${whereClause}
    ...
    fastify.get('/:id/evidence', async (request, reply) => {
        ...
        try {
            ...
            const claimResult = await index_js_1.pool.query('SELECT evidence_ids FROM claims WHERE id = $1 AND tenant_id = $2', [id, request.auth.tenant_id]);
            ...
            const result = await index_js_1.pool.query('SELECT * FROM evidence WHERE id = ANY($1) AND tenant_id = $2', [evidenceIds, request.auth.tenant_id]);
            return {
                items: result.rows,

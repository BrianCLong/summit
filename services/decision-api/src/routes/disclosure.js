...
const GenerateDisclosureBody = zod_1.z.object({
    decision_id: zod_1.z.string().uuid(),
    format: zod_1.z.enum(['markdown', 'html', 'json']).default('markdown'),
    include_evidence_details: zod_1.z.boolean().default(true),
    include_audit_trail: zod_1.z.boolean().default(true),
    redact_sensitive: zod_1.z.boolean().default(true),
});
async function disclosureRoutes(fastify) {
    // Generate disclosure pack
    fastify.post('/generate', async (request, reply) => {
        if (!(0, auth_js_1.hasPermission)(request.auth, 'disclosure:generate')) {
        ...
        try {
            ...
            if (include_evidence_details) {
                const allEvidenceIds = new Set(decision.evidence_ids || []);
            ...
            if (include_audit_trail) {
                const auditResult = await index_js_1.pool.query(`SELECT * FROM audit_log
             WHERE resource_id = $1 AND tenant_id = $2
             ORDER BY timestamp ASC`, [decision_id, request.auth.tenant_id]);
                auditTrail = auditResult.rows;
            }
            // Apply redaction if needed
            let redactionsApplied = 0;
            if (redact_sensitive && !(0, auth_js_1.hasClearance)(request.auth, 'confidential')) {
            ...
            const pack = {
                ...
                sections: {
                    ...
                    options_considered: (decision.options || []).map((opt) => ({
                        name: opt.name,
    ...
    fastify.get('/:id', async (request, reply) => {
        if (!(0, auth_js_1.hasPermission)(request.auth, 'disclosure:read')) {
            return reply.status(403).send({ error: 'Forbidden' });
        }
        const { id } = request.params;
        try {
            const result = await index_js_1.pool.query('SELECT * FROM disclosure_packs WHERE id = $1 AND tenant_id = $2', [id, request.auth.tenant_id]);
    ...
    fastify.post('/:id/verify', async (request, reply) => {
        ...
        try {
            ...
            const decisionResult = await index_js_1.pool.query('SELECT * FROM decisions WHERE id = $1 AND tenant_id = $2', [pack.decision_id, request.auth.tenant_id]);
            if (decisionResult.rows.length === 0) {
                return {
                    valid: false,
...
function getSelectedOptionName(decision) {
    const selectedOption = decision.options?.find((o) => o.selected);
    return selectedOption?.name || 'No option selected';
}
function computeMerkleRoot(decision, claims, evidence) {
    const hashes = [];
    // Hash decision core content
    hashes.push(crypto_1.default.createHash('sha256').update(JSON.stringify({
        id: decision.id,
        question: decision.question,
    ...
    while (hashes.length > 1) {
        ...
        for (let i = 0; i < hashes.length; i += 2) {
            if (i + 1 < hashes.length) {
                newLevel.push(crypto_1.default.createHash('sha256').update(hashes[i] + hashes[i + 1]).digest('hex'));
            }
            else {
...
${sections.options_considered.map((opt, i) => `
...
${sections.claims_and_evidence.map((item, i) => `
### Claim ${i + 1}
...
**Evidence Sources:**
${item.evidence_sources.map((e) => `- ${e.title} (${e.source_type}, reliability: ${(e.reliability * 100).toFixed(0)}%)${e.redacted ? ' [REDACTED]' : ''}`).join('\n') || '- No evidence attached'}
`).join('\n')}
...
**Risk Factors:**
${sections.risk_assessment.risk_factors?.map((r) => `- ${r}`).join('\n') || '- None identified'}

**Mitigations:**
${sections.risk_assessment.mitigations?.map((m) => `- ${m}`).join('\n') || '- None specified'}
...
${sections.limitations.map((l) => `- **${l.type}**: ${l.description}`).join('\n')}
...
|-----------|-------|-------|
${sections.audit_trail.map((entry) => `| ${entry.timestamp} | ${entry.event} | ${entry.actor} |`).join('\n') || '| No audit entries | - | - |'}

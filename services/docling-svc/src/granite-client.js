...
exports.GraniteDoclingClient = void 0;
...
class GraniteDoclingClient {
    ...
    async parse(payload) {
        ...
        const fragments = response.fragments ||
            synthesizeFragments(payload.bytes
    ...
    async invokeRemote(path, payload) {
        if (!config.GRANITE_DOCLING_ENDPOINT) {
            return {};
        }
        const { data } = await this.getHttp().post(path, payload);
        return data;
    }
    estimateCost(input) {
...
exports.GraniteDoclingClient = GraniteDoclingClient;
const hashPrompt = (payload) => crypto_1.default.createHash('sha256').update(JSON.stringify(payload)).digest('hex');
const synthesizeSummary = (text, focus) => {
    if (!text)
...
const synthesizeFragments = (text) => {
    if (!text) {
        return [];
    }
    return text
...
const deriveHighlights = (summary) => {
    return summary
        .split(/[.\n]/)
        .map((s) => s.trim())
        .filter(Boolean)
...
const synthesizeFindings = (text, targets) => {
    const findings = [];
    ...
    if (targets.includes('cve')) {
        ...
        cveMatches.slice(0, 5).forEach((match, index) => findings.push({
            id: `cve-${index}`,
            label: 'cve',
            value: match,
            confidence: 0.65,

...
exports.calculateNotificationSeverity = calculateNotificationSeverity;
exports.requiresImmediateNotification = requiresImmediateNotification;
...
exports.getSuggestedResponseTime = getSuggestedResponseTime;
exports.calculateNotificationPriority = calculateNotificationPriority;
/**
 ...
 */
function calculateNotificationSeverity(event) {
    let score = 0;
    ...
    if (event.compliance_frameworks && event.compliance_frameworks.length > 0) {
        ...
        if (event.compliance_frameworks.includes('HIPAA') ||
    ...
    if (event.tags) {
        ...
        const hasSecurityTag = event.tags.some((tag) => securityTags.some((secTag) => tag.toLowerCase().includes(secTag)));
        if (hasSecurityTag) {
            score += 1;
        }
    }
...
function scoreToSeverity(score) {
    ...
    if (score >= 6)
        return 'critical'; // High priority, all channels
    if (score >= 4)
        return 'high'; // Urgent, immediate delivery
    if (score >= 2)
        return 'medium'; // Important, throttled delivery
 ...
 */
function requiresImmediateNotification(event, severity) {
 ...
 */
function calculateEscalationLevel(event, severity) {
 ...
 */
function shouldMaskSensitiveData(event) {
 ...
 */
function getSuggestedResponseTime(severity) {
    const responseTimes = {
        low: '24 hours',
        medium: '4 hours',
        high: '1 hour',
 ...
 */
function calculateNotificationPriority(severity, event) {
    const basePriority = {
        emergency: 100,
        critical: 80,
        high: 60,

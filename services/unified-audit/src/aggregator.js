...
exports.UnifiedAuditAggregator = void 0;
const eventemitter3_1 = require("eventemitter3");
const crypto_1 = require("crypto");
const crypto_2 = require("crypto");
const pino_1 = __importDefault(require("pino"));
const types_js_1 = require("./types.js");
class UnifiedAuditAggregator extends eventemitter3_1.EventEmitter {
    db;
    redis;
    logger;
    signingSecret;
    ...
    constructor(config) {
        ...
        this.logger = config.logger || (0, pino_1.default)({ name: 'unified-audit' });
     ...
     */
    async loadLastHash() {
        const result = await this.db.query(`SELECT hash FROM unified_audit.events ORDER BY timestamp DESC, id DESC LIMIT 1`);
    ...
    async ingest(event) {
        try {
            ...
            const validatedEvent = types_js_1.UnifiedAuditEventSchema.parse(normalizedEvent);
            // Add to buffer
            ...
            this.emit('event:failed', error, event);
            throw error;
        }
    }
    /**
    ...
    normalizeEvent(event) {
        const id = event.id || (0, crypto_2.randomUUID)();
        ...
        const hash = (0, crypto_1.createHash)('sha256').update(eventData).digest('hex');
        const signature = (0, crypto_1.createHmac)('sha256', this.signingSecret).update(hash).digest('hex');
        return {
            id,
        ...
        try {
            ...
            try {
                ...
                for (const event of batch) {
                    await client.query(`INSERT INTO unified_audit.events
     ...
     */
    async getAggregation(tenantId, startTime, endTime) {
        const [bySource, byCategory, bySeverity, byOutcome, byHour, topActors, topResources] = await Promise.all([
            this.db.query(`SELECT source, COUNT(*) as count FROM unified_audit.events
         WHERE tenant_id = $1 AND timestamp BETWEEN $2 AND $3
         GROUP BY source`, [tenantId, startTime, endTime]),
            this.db.query(`SELECT category, COUNT(*) as count FROM unified_audit.events
         WHERE tenant_id = $1 AND timestamp BETWEEN $2 AND $3
         GROUP BY category`, [tenantId, startTime, endTime]),
            this.db.query(`SELECT severity, COUNT(*) as count FROM unified_audit.events
         WHERE tenant_id = $1 AND timestamp BETWEEN $2 AND $3
         GROUP BY severity`, [tenantId, startTime, endTime]),
            this.db.query(`SELECT outcome, COUNT(*) as count FROM unified_audit.events
         WHERE tenant_id = $1 AND timestamp BETWEEN $2 AND $3
         GROUP BY outcome`, [tenantId, startTime, endTime]),
            this.db.query(`SELECT date_trunc('hour', timestamp) as hour, COUNT(*) as count
         FROM unified_audit.events
         WHERE tenant_id = $1 AND timestamp BETWEEN $2 AND $3
         GROUP BY date_trunc('hour', timestamp)
         ORDER BY hour`, [tenantId, startTime, endTime]),
            this.db.query(`SELECT actor->>'id' as actor_id, COUNT(*) as count
         FROM unified_audit.events
         WHERE tenant_id = $1 AND timestamp BETWEEN $2 AND $3
         GROUP BY actor->>'id'
         ORDER BY count DESC
         LIMIT 10`, [tenantId, startTime, endTime]),
            this.db.query(`SELECT resource->>'type' as resource_type, resource->>'id' as resource_id, COUNT(*) as count
         FROM unified_audit.events
        ...
        return {
            bySource: Object.fromEntries(bySource.rows.map((r) => [r.source, Number(r.count)])),
            byCategory: Object.fromEntries(byCategory.rows.map((r) => [r.category, Number(r.count)])),
            bySeverity: Object.fromEntries(bySeverity.rows.map((r) => [r.severity, Number(r.count)])),
            byOutcome: Object.fromEntries(byOutcome.rows.map((r) => [r.outcome, Number(r.count)])),
     ...
     */
    async generateComplianceReport(tenantId, framework, startTime, endTime) {
        ...
        const complianceScore = Math.max(0, 100 - (violations.length * 5) - (criticalEvents.length * 10));
        const report = {
            framework,
        ...
        // Store report
        await this.db.query(`INSERT INTO unified_audit.compliance_reports (framework, tenant_id, period_start, period_end, report_data)
       VALUES ($1, $2, $3, $4, $5)`, [framework, tenantId, startTime, endTime, JSON.stringify(report)]);
        return report;
    }
    /**
     * Perform forensic analysis
     */
    async forensicAnalysis(correlationId, tenantId) {
        const events = await this.query({
            correlationId,
            tenantIds: [tenantId],
            limit: 1000,
        ...
        for (const event of events.events) {
            ...
            if (event.actor.ipAddress)
                actor.ips.add(event.actor.ipAddress);
            if (event.actor.userAgent)
                actor.userAgents.add(event.actor.userAgent);
        }
        const actorAnalysis = {
        ...
        // Store analysis
        await this.db.query(`INSERT INTO unified_audit.forensic_analyses (correlation_id, tenant_id, analysis_data)
       VALUES ($1, $2, $3)`, [correlationId, tenantId, JSON.stringify(analysis)]);
        return analysis;
    }
    /**
     * Verify integrity of audit chain
     */
    async verifyIntegrity(tenantId, startTime, endTime) {
        const query = `
    ...
    `;
        ...
        for (const row of result.rows) {
            ...
            // Verify signature
            const expectedSignature = (0, crypto_1.createHmac)('sha256', this.signingSecret)
                .update(row.hash)
                .digest('hex');
     ...
     */
    checkComplianceViolation(event) {
...
exports.UnifiedAuditAggregator = UnifiedAuditAggregator;

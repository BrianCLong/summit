...
(0, vitest_1.describe)('SyntheticDataGenerator', () => {
    ...
    (0, vitest_1.beforeEach)(() => {
        generator = new SyntheticDataGenerator_js_1.SyntheticDataGenerator();
    });
    (0, vitest_1.describe)('generate', () => {
        (0, vitest_1.it)('should generate entities based on schema', async () => {
            const request = {
                sandboxId,
            ...
            (0, vitest_1.expect)(result.statistics.byEntityType['Person']).toBe(100);
        });
        (0, vitest_1.it)('should generate with seeded randomness for reproducibility', async () => {
            const request = {
                sandboxId,
        ...
        (0, vitest_1.it)('should generate multiple entity types with distribution', async () => {
            const request = {
                sandboxId,
            ...
            const result = await generator.generate(request);
            (0, vitest_1.expect)(result.statistics.byEntityType['Person']).toBe(70);
            (0, vitest_1.expect)(result.statistics.byEntityType['Organization']).toBe(30);
        });
        ...
        (0, vitest_1.it)('should handle nullable fields', async () => {
            const request = {
                sandboxId,
        ...
        (0, vitest_1.it)('should use different generators correctly', async () => {
            const request = {
                sandboxId,
                name: 'Generator Test',
                schemas: [
            ...
            const result = await generator.generate(request);
            (0, vitest_1.expect)(result.status).toBe('completed');
            (0, vitest_1.expect)(result.sampleData).toBeDefined();
            (0, vitest_1.expect)(result.sampleData.length).toBeGreaterThan(0);
            const sample = result.sampleData[0];
            (0, vitest_1.expect)(sample.uuid).toBeDefined();
            (0, vitest_1.expect)(sample.amount).toBeDefined();
        });
        (0, vitest_1.it)('should return sample data', async () => {
            const request = {
                sandboxId,
            ...
            const result = await generator.generate(request);
            (0, vitest_1.expect)(result.sampleData).toBeDefined();
            (0, vitest_1.expect)(result.sampleData.length).toBeLessThanOrEqual(10);
        });
    });
    (0, vitest_1.describe)('registerGenerator', () => {
        (0, vitest_1.it)('should allow custom generators', async () => {

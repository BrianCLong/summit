import { CanaryManager, buildDefaultConfig, meanTimeToRollback } from "./canaryManager";
import { HealthSample } from "./types";

describe("CanaryManager", () => {
  const healthySample = (overrides: Partial<HealthSample> = {}): HealthSample => ({
  ...
  it("computes mean time to rollback from health history", () => {
    ...
    const failingSample: HealthSample = {
      ...
      metrics: {
        ...
        saturation: 0.85,
        probes: [
          { name: "ping", success: false, latencyMs: 400 },
        ],
      },
      syntheticChecks: [
        { name: "smoke", passed: false, latencyMs: 400 },
      ],
    };
    ...
    const mttb = meanTimeToRollback(rollback.state, new Date("2024-01-01T00:11:30Z"));

...
class ProvenanceQueryAPI:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_trace_tenant ON provenance_traces (tenant_id, start_time)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_trace_session ON provenance_traces (session_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_events_trace ON provenance_events (trace_id, timestamp)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_events_type ON provenance_events (event_type, timestamp)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_events_component ON provenance_events (component, timestamp)"
            )
    ...
    def _generate_events_for_trace(
        self,
        trace_id: str,
        session_id: str,
        tenant_id: str,
    ...
    async def query_traces(self, query: ProvenanceQuery) -> dict[str, Any]:
        ...
        try:
            ...
            # Cache the result
            self.cache[cache_key] = {"data": result.copy(), "timestamp": time.time()}
    ...
    async def get_trace_by_id(self, trace_id: str) -> ProvenanceTrace | None:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                trace = ProvenanceTrace(
                    ...
                    end_time=(
                        datetime.fromisoformat(trace_row["end_time"])
                        if trace_row["end_time"]
                        else None
                    ),
    ...
    def _generate_trace_summary(self, trace: ProvenanceTrace) -> str:
        ...
        return (
            f"Response generated in {trace.total_duration_ms:.1f}ms using data from "
    ...
    def _generate_performance_analysis(self, trace: ProvenanceTrace) -> dict[str, Any]:
        ...
        return {
            ...
            "cache_efficiency": (
                trace.cache_utilization.get("hits", 0)
                / max(
                    1,
                    trace.cache_utilization.get("hits", 0)
                    + trace.cache_utilization.get("misses", 0),
                )
            )
            * 100,
        }
    ...
    def get_api_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "performance_targets": {"query_time_target_ms": 200, "success_rate": 99.2},  # Simulated
        }

...
export async function publish(t:Task){ await r.xAdd("tgo:tasks","*",{ id:t.id, payload: JSON.stringify(t) }); }
export async function claim(workerId:string, caps:string[]){
  while(true){
    const m = await r.xReadGroup("tgo","g1",[{ key:"tgo:tasks", id:">"}],{ COUNT:1, BLOCK:5000 });
    if(!m) continue;
...
export async function ack(id:string){ await r.xAck("tgo:tasks","g1",id); }
...
class ProvenanceVerifier {
    hasher;
    constructor() {
        this.hasher = new hasher_js_1.ProvenanceHasher();
    }
    /**
     * Verify manifest by replaying transforms
     */
    async verify(manifest, inputData, executor) {
        const errors = [];
        const warnings = [];
        ...
        if (inputNodes.length === 0) {
            ...
            if (expectedInputHash !== actualInputHash) {
                errors.push(`Input hash mismatch: expected ${expectedInputHash}, got ${actualInputHash}`);
            }
        }
        // 3. Replay transforms
        ...
        for (const node of transformNodes) {
            ...
            if (transform.inputHash !== currentHash) {
                errors.push(`Transform ${transform.id} input hash mismatch: expected ${transform.inputHash}, got ${currentHash}`);
            }
            try {
                // Execute transform
                ...
                const hashMatch = this.hasher.verifyHash(transform.outputHash || '', outputHash, output, tolerance);
                if (!hashMatch) {
                    errors.push(`Transform ${transform.id} output hash mismatch: expected ${transform.outputHash}, got ${outputHash}`);
                }
                currentData = output;
        ...
        if (outputNodes.length > 0) {
            ...
            if (expectedOutputHash !== actualOutputHash) {
                warnings.push(`Final output hash mismatch: expected ${expectedOutputHash}, got ${actualOutputHash}`);
            }
        }
        ...
        if (computedRoot !== manifest.rootHash) {
            errors.push(`Merkle root mismatch: expected ${manifest.rootHash}, got ${computedRoot}`);
        }

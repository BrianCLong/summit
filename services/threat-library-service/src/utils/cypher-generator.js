...
exports.generateCypherFromMotif = generateCypherFromMotif;
exports.generatePatternQueries = generatePatternQueries;
exports.validateCypherQuery = validateCypherQuery;
/**
 * Generates a Cypher node match clause from a NodeConstraint
 */
function generateNodeMatch(node, alias) {
 ...
 */
function generateNodeConditions(node, alias) {
    const conditions = [];
    const params = {};
        ...
        for (let i = 0; i < node.propertyFilters.length; i++) {
            ...
            switch (filter.operator) {
                ...
                case 'CONTAINS':
                    ...
                    break;
                case 'STARTS_WITH':
                    conditions.push(`${alias}.${filter.property} STARTS WITH $${paramName}`);
                    params[paramName] = filter.value;
                    break;
                case 'ENDS_WITH':
 ...
 */
function generateEdgePattern(edge, sourceAlias, targetAlias, edgeAlias) {
    const relType = edge.type;
    const hops = edge.minHops || edge.maxHops
 ...
 */
function generateEdgeConditions(edge, alias) {
    const conditions = [];
    const params = {};
 ...
 */
function generateTimeConstraintConditions(constraint, nodeAliases) {
    const conditions = [];
    const params = {};
    switch (constraint.operator) {
        case 'WITHIN':
            if (constraint.referenceNodeId && constraint.targetNodeId) {
                ...
                if (refAlias && targetAlias && constraint.durationMs) {
                    conditions.push(`abs(duration.between(${refAlias}.timestamp, ${targetAlias}.timestamp).milliseconds) <= $within_duration`);
        ...
        case 'BETWEEN':
            if (constraint.startTime && constraint.endTime && constraint.targetNodeId) {
                const targetAlias = nodeAliases.get(constraint.targetNodeId);
                if (targetAlias) {
                    conditions.push(`${targetAlias}.timestamp >= datetime($between_start) AND ${targetAlias}.timestamp <= datetime($between_end)`);
            ...
            if (constraint.sequence && constraint.sequence.length > 1) {
                for (let i = 0; i < constraint.sequence.length - 1; i++) {
                    ...
                    if (currentAlias && nextAlias) {
                        conditions.push(`${currentAlias}.timestamp < ${nextAlias}.timestamp`);
                    }
                }
            }
            break;
 ...
 */
function generateCypherFromMotif(motif, options = {}) {
    const params = {};
    const conditions = [];
    ...
    motif.edges.forEach((edge, index) => {
        ...
        if (sourceAlias && targetAlias) {
            matchClauses.push(`MATCH ${generateEdgePattern(edge, sourceAlias, targetAlias, edgeAlias)}`);
            // Add edge conditions
    ...
    if (motif.timeConstraints && motif.timeConstraints.length > 0) {
        for (const timeConstraint of motif.timeConstraints) {
            const timeConditions = generateTimeConstraintConditions(timeConstraint, nodeAliases);
    ...
    if (motif.spatialConstraints) {
        if (motif.spatialConstraints.sameLocation &&
            motif.spatialConstraints.sameLocation.length > 1) {
            const locationNodes = motif.spatialConstraints.sameLocation.map((id) => nodeAliases.get(id));
            for (let i = 0; i < locationNodes.length - 1; i++) {
                if (locationNodes[i] && locationNodes[i + 1]) {
                    conditions.push(`${locationNodes[i]}.location = ${locationNodes[i + 1]}.location`);
                }
            }
        }
    }
 ...
 */
function generatePatternQueries(motif, options = {}) {
    const queries = [];
    // Main pattern query
    queries.push(generateCypherFromMotif(motif, { ...options, returnFormat: 'full' }));
    // Count query for statistics
    queries.push(generateCypherFromMotif(motif, { ...options, returnFormat: 'count' }));
    ...
    if (motif.aggregations && motif.aggregations.length > 0) {
        for (const agg of motif.aggregations) {
            ...
            if (alias) {
                ...
                const returnClause = aggFunction === 'DISTINCT'
                    ? `RETURN DISTINCT ${alias}.${property} AS value`
                    : `RETURN ${aggFunction}(${alias}.${property}) AS value`;
                const queryLines = aggQuery.query.split('\n');
                const limitIndex = queryLines.findIndex((line) => line.trim().startsWith('LIMIT'));
                const limitClause = limitIndex !== -1 ? queryLines.splice(limitIndex, 1)[0] : '';
                const returnIndex = queryLines.findIndex((line) => line.trim().startsWith('RETURN'));
                if (returnIndex !== -1) {
                    if (agg.threshold !== undefined) {
                        const aggregationExpression = aggFunction === 'DISTINCT'
                            ? `DISTINCT ${alias}.${property}`
                            : `${aggFunction}(${alias}.${property})`;
                        queryLines.splice(returnIndex, 1, `WITH ${aggregationExpression} AS value`, `WHERE value >= ${agg.threshold}`, 'RETURN value');
                    }
                    else {

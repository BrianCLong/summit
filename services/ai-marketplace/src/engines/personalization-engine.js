...
class PersonalizationEngine {
        ...
        for (const experience of candidates) {
            ...
            if (score > 0.1) {
                recommendations.push({
                    ...
                    score,
                    reasons,
                    personalizedDescription: this.localizeDescription(experience, prefs.locale),
                });
            }
        }
     ...
     */
    calculatePersonalizationScore(experience, prefs) {
        let score = 0;
        const weights = {
            persona: 0.25,
            locale: 0.15,
        ...
        // Interest overlap
        const interestOverlap = this.calculateSetOverlap(new Set(prefs.interests), new Set([...experience.tags, experience.category]));
        ...
        if (prefs.learningProfile?.embeddingVector) {
            ...
            if (expEmbedding) {
                const similarity = this.learningService.computeSimilarity(prefs.learningProfile.embeddingVector, expEmbedding);
        ...
        const daysSinceUpdate = (Date.now() - experience.updatedAt.getTime()) / (1000 * 60 * 60 * 24);
     ...
     */
    generateReasons(experience, prefs, score) {
        const reasons = [];
        ...
        const matchingInterests = prefs.interests.filter(i => experience.tags.includes(i) || experience.category === i);
    ...
    filterExperiences(filter) {
        ...
        if (filter) {
            ...
            if (filter.categories?.length) {
                experiences = experiences.filter(e => filter.categories.includes(e.category));
            }
            if (filter.tags?.length) {
                experiences = experiences.filter(e => e.tags.some(t => filter.tags.includes(t)));
            }
            if (filter.locale) {
                experiences = experiences.filter(e => e.supportedLocales.includes(filter.locale) ||
                    e.supportedLocales.includes(filter.locale.split('-')[0]));
            }
            if (filter.priceModel) {
                experiences = experiences.filter(e => e.pricing.model === filter.priceModel);
            }
            if (filter.minRating !== undefined) {
                experiences = experiences.filter(e => e.rating !== undefined && e.rating >= filter.minRating);
            }
            if (filter.search) {
                const searchLower = filter.search.toLowerCase();
                experiences = experiences.filter(e => e.name.toLowerCase().includes(searchLower) ||
     ...
     */
    getExperienceEmbedding(experienceId) {
     ...
     */
    localizeDescription(experience, locale) {

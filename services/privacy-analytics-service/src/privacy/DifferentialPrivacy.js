...
exports.differentialPrivacy = exports.DifferentialPrivacy = void 0;
const logger_js_1 = require("../utils/logger.js");
class DifferentialPrivacy {
    budgetStore = new Map();
    defaultSensitivity;
    constructor(defaultSensitivity) {
     ...
     */
    applyDP(rows, config, query, budgetState) {
        const warnings = [];
     ...
     */
    addNoiseToRow(row, config, query) {
        const noisyMeasures = {};
     ...
     */
    generateNoise(config, sensitivity) {
     ...
     */
    exponentialMechanism(candidates, scoreFn, epsilon, sensitivity) {
        ...
        const unnormalizedProbs = scores.map(score => Math.exp((epsilon * (score - maxScore)) / (2 * sensitivity)));
        // Normalize
     ...
     */
    getSensitivity(field, aggregationType) {
        ...
        switch (aggregationType) {
            ...
            case 'max':
                ...
                logger_js_1.logger.warn({ field, aggregationType }, 'Min/max operations have unbounded sensitivity');
                return this.defaultSensitivity.sumSensitivity;
            case 'stddev':
            case 'variance':
     ...
     */
    calculateQueryEpsilon(config, query) {
     ...
     */
    estimateAverageNoise(config, query) {
        // For Laplace, mean absolute deviation = scale = sensitivity/epsilon
        // For Gaussian, mean absolute deviation ≈ sigma * sqrt(2/pi)
        const avgSensitivity = query.measures.reduce((sum, m) => sum + this.getSensitivity(m.field, m.aggregation), 0) / query.measures.length;
     ...
     */
    getBudgetState(tenantId, userId, config) {
     ...
     */
    consumeBudget(tenantId, epsilonConsumed, userId) {
        const key = this.budgetKey(tenantId, userId);
     ...
     */
    advancedCompositionBound(epsilon, delta, numQueries) {
     ...
     */
    estimateRemainingQueries(targetEpsilon, targetDelta, perQueryEpsilon, perQueryDelta) {
...
// Default instance
exports.differentialPrivacy = new DifferentialPrivacy();

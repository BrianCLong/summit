...
class PolicyEnforcer {
    defaultMinCohortSize;
    constructor(defaultMinCohortSize = 5) {
     ...
     */
    detectHighRiskCombinations(query) {
        ...
        return queriedDimensions.filter(d => highRiskFields.some(hr => d.includes(hr)));
    }
    /**
     * Check if a policy applies to the given data source
     */
    policyApplies(policy, source) {
        if (policy.applicableSources.length === 0)
            return true;
     ...
     */
    applyPolicy(policy, rows, query) {
        const warnings = [];
        let processedRows = rows;
        let suppressedCount = 0;
        switch (policy.mechanism) {
            case index_js_1.PrivacyMechanism.K_ANONYMITY:
                ...
                break;
            case index_js_1.PrivacyMechanism.SUPPRESSION:
                ...
                break;
            case index_js_1.PrivacyMechanism.GENERALIZATION:
                ...
                break;
            case index_js_1.PrivacyMechanism.COMBINED:
                ...
                break;
            case index_js_1.PrivacyMechanism.NONE:
            default:
     ...
     */
    applyKAnonymity(rows, config, query) {
        const warnings = [];
     ...
     */
    applyGeneralization(rows, config) {
        const warnings = [];
        const generalizedFields = [];
        const resultRows = rows.map(row => {
            const generalizedDimensions = { ...row.dimensions };
            for (const [field, hierarchy] of Object.entries(config.hierarchies)) {
                if (field in row.dimensions) {
                    ...
                    const generalizedValue = this.generalizeValue(originalValue, hierarchy, config.targetLevel);
     ...
     */
    generalizeValue(value, hierarchy, targetLevel) {
     ...
     */
    createGroupKey(dimensions, fields) {
        return fields

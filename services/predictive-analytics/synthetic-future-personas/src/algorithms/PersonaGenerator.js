...
exports.generateSyntheticPersona = generateSyntheticPersona;
exports.extractBehavioralProfile = extractBehavioralProfile;
exports.calculateStability = calculateStability;
exports.calculateConfidence = calculateConfidence;
const uuid_1 = require("uuid");
const BehavioralProfile_js_1 = require("../models/BehavioralProfile.js");
const SyntheticPersona_js_1 = require("../models/SyntheticPersona.js");
const GENERATOR_VERSION = '1.0.0';
/**
 * Generates a synthetic persona from a seed entity
 */
function generateSyntheticPersona(seedEntity, config = {}) {
    const finalConfig = { ...SyntheticPersona_js_1.DEFAULT_PERSONA_CONFIG, ...config };
    ...
    const stabilityCoefficient = finalConfig.stabilityCoefficient ?? calculateStability(seedEntity);
    const now = Date.now();
    const persona = {
        id: (0, uuid_1.v4)(),
 ...
 */
function extractBehavioralProfile(entity) {
    if (!entity.activityHistory || entity.activityHistory.length === 0) {
        return (0, BehavioralProfile_js_1.cloneProfile)(BehavioralProfile_js_1.DEFAULT_BEHAVIORAL_PROFILE);
    }
 ...
 */
function calculateActivityLevel(history) {
    if (history.length === 0)
        return 0.5;
 ...
 */
function calculateOperationalTempo(history) {
    if (history.length === 0)
        return 1.0;
 ...
 */
function calculateRiskTolerance(history) {
    if (history.length === 0)
        return 0.5;
    const recentHistory = history.slice(-20); // Last 20 activities
    const avgRisk = recentHistory.reduce((sum, a) => sum + (a.risk || 0.5), 0) /
 ...
 */
function calculateTacticalInnovation(history) {
    if (history.length < 5)
        return 0.3;
    // Count unique activity types in recent history
    const recentTypes = new Set(history.slice(-20).map((a) => a.type));
 ...
 */
function calculateCapabilityAcquisition(history) {
...
function calculateConsistency(history) {
    ...
    const variance = intensities.reduce((sum, i) => sum + Math.pow(i - mean, 2), 0) /
        intensities.length;

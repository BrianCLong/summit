...
exports.scoreTrajectoryLikelihood = scoreTrajectoryLikelihood;
exports.calculateLikelihoodComponents = calculateLikelihoodComponents;
exports.calculateHistoricalSimilarity = calculateHistoricalSimilarity;
exports.calculateCoherence = calculateCoherence;
exports.calculatePressureRealism = calculatePressureRealism;
exports.calculateDivergencePenalty = calculateDivergencePenalty;
exports.updateLikelihoodsWithEvidence = updateLikelihoodsWithEvidence;
exports.normalizeLikelihoods = normalizeLikelihoods;
const BehavioralProfile_js_1 = require("../models/BehavioralProfile.js");
/**
 * Scores the likelihood of a trajectory
 */
function scoreTrajectoryLikelihood(trajectory, persona, historicalPatterns = []) {
    const components = calculateLikelihoodComponents(trajectory, persona, historicalPatterns);
    // Weighted combination
    const overallLikelihood = components.historicalScore * 0.35 +
 ...
 */
function calculateLikelihoodComponents(trajectory, persona, historicalPatterns = []) {
    return {
        historicalScore: calculateHistoricalSimilarity(trajectory, historicalPatterns),
        coherenceScore: calculateCoherence(trajectory),
        realismScore: calculatePressureRealism(trajectory, persona),
        divergencePenalty: calculateDivergencePenalty(trajectory, persona.baselineProfile),
    };
}
/**
 * Calculates historical similarity score
 */
function calculateHistoricalSimilarity(trajectory, historicalPatterns) {
    ...
    const similarities = historicalPatterns.map((pattern) => {
        ...
        const initialSimilarity = 1 / (1 + (0, BehavioralProfile_js_1.calculateDistance)(initialState, pattern.initialState));
        // Compare final states
        const finalSimilarity = 1 / (1 + (0, BehavioralProfile_js_1.calculateDistance)(finalState, pattern.finalState));
 ...
 */
function calculateCoherence(trajectory) {
    ...
    for (let i = 1; i < trajectory.steps.length; i++) {
        ...
        const stepChange = (0, BehavioralProfile_js_1.calculateDistance)(prevState, currState);
 ...
 */
function checkDimensionalCoherence(trajectory) {
 ...
 */
function checkEventCoherence(trajectory) {
    let coherenceScore = 1.0;
    for (const step of trajectory.steps) {
 ...
 */
function calculatePressureRealism(trajectory, persona) {
        ...
        for (const pressure of pressures) {
            ...
            switch (pressure.type) {
                ...
                case 'POLITICAL':
                    ...
                    if (Math.abs(state.alignmentShift) >
                ...
                case 'TECHNOLOGICAL':
                    ...
                    if (state.capabilityAcquisition >
                ...
                case 'OPERATIONAL':
                    ...
                    if (state.activityLevel > persona.baselineProfile.activityLevel + 0.1) {
                        alignmentScore += 1.0;
                    }
                    else {
                        alignmentScore += 0.5;
 ...
 */
function calculateDivergencePenalty(trajectory, baseline) {
    const finalState = trajectory.finalState;
    const distance = (0, BehavioralProfile_js_1.calculateDistance)(finalState, baseline);
 ...
 */
function updateLikelihoodsWithEvidence(trajectories, observedState, observationTime, confidence) {
    return trajectories.map((trajectory) => {
        // Find the closest time step to observation
        const closestStep = trajectory.steps.reduce((prev, curr) => Math.abs(curr.time - observationTime) < Math.abs(prev.time - observationTime)
            ? curr
            : prev);
        // Calculate how well this trajectory predicted the observation
        const predictionError = (0, BehavioralProfile_js_1.calculateDistance)(closestStep.state, observedState);
        // Update likelihood using Bayesian approach
        const evidenceFactor = Math.exp(-predictionError) * confidence;
        const updatedLikelihood = (trajectory.likelihood * evidenceFactor) /
 ...
 */
function normalizeLikelihoods(trajectories) {
    const totalLikelihood = trajectories.reduce((sum, t) => sum + t.likelihood, 0);

...
exports.SignalFuser = exports.FusionMethod = void 0;
exports.createFuser = createFuser;
...
(function (FusionMethod) {
    ...
    FusionMethod["KALMAN_FILTER"] = "KALMAN_FILTER";
    FusionMethod["BAYESIAN_NETWORK"] = "BAYESIAN_NETWORK";
    FusionMethod["ENSEMBLE_VOTING"] = "ENSEMBLE_VOTING";
    FusionMethod["ATTENTION_WEIGHTED"] = "ATTENTION_WEIGHTED";
...
class SignalFuser {
    ...
    fuse(signals, sources, trustScores) {
        if (signals.length === 0) {
            throw new Error('No signals to fuse');
        }
        if (signals.length < this.config.minSources) {
            throw new Error(`Insufficient sources: ${signals.length} < ${this.config.minSources}`);
        }
    ...
    dempsterShaferFusion(signals, sources, trustScores) {
    ...
    kalmanFilterFusion(signals, sources, trustScores) {
        ...
        for (const signal of numericSignals) {
            ...
            // Update estimate
            estimate = estimate + gain * (signal.prediction - estimate);
            // Update variance
    ...
    bayesianFusion(signals, sources, trustScores) {
    ...
    ensembleVotingFusion(signals, sources, trustScores) {
        ...
        for (const signal of signals) {
            ...
            const weight = source && trust
                ? (0, IntelligenceSource_js_1.getSourceWeight)(source) * trust.overallScore
                : 0.5;
            weights.set(signal.sourceId, weight);
        }
        const fusedPrediction = (0, PredictiveSignal_js_1.getWeightedPrediction)(signals, weights);
    ...
    attentionWeightedFusion(signals, sources, trustScores) {
        ...
        if (numericSignals.length === signals.length) {
            fusedPrediction = signals.reduce((sum, s) => {
                ...
                return sum + s.prediction * attention;
            }, 0);
        }
        else {
...
exports.SignalFuser = SignalFuser;
function createFuser(config) {
    const defaultConfig = {
        method: FusionMethod.ENSEMBLE_VOTING,
        minSources: 1,

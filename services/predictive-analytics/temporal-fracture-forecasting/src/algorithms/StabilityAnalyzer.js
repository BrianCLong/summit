...
class StabilityAnalyzer {
    ...
    analyze(data) {
        ...
        const timeToInstability = this.predictInstabilityTime(lyapunovExponent, stabilityScore);
        return {
            timestamp: new Date(),
            systemId: 'unknown',
            lyapunovExponent,
    ...
    computeLyapunovExponent(data) {
        ...
        for (let i = 0; i < embedding.length - 10; i++) {
            ...
            for (let j = 0; j < embedding.length - 10; j++) {
                if (Math.abs(i - j) < 5)
                    continue; // Skip temporally close points
            ...
            for (let k = 1; k <= 10 && i + k < embedding.length && nearestIdx + k < embedding.length; k++) {
                const dist = this.euclideanDistance(embedding[i + k], embedding[nearestIdx + k]);
                if (dist > 0) {
        ...
        if (divergenceRates.length === 0)
            return 0;
        // Average divergence rate
        return (divergenceRates.reduce((sum, rate) => sum + rate, 0) /
            divergenceRates.length);
    }
    /**
     * Reconstruct phase space using time-delay embedding
     */
    reconstructPhaseSpace(data) {
        const { embeddingDimension, timeDelay } = this.config;
        const embedding = [];
        for (let i = 0; i < data.length - (embeddingDimension - 1) * timeDelay; i++) {
            const point = [];
     ...
     */
    computeApproximateEntropy(data) {
        const values = data.map((d) => d.value);
     ...
     */
    predictInstabilityTime(lyapunovExponent, stabilityScore) {

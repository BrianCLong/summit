...
exports.temporalFractureResolvers = void 0;
exports.temporalFractureResolvers = {
    Query: {
        /**
         * Get complete fracture map for a system
         */
        getFractureMap: async (_parent, args, context) => {
            ...
            const fractureMap = await engine.generateFractureMap(args.systemId, metricsData);
            return {
                ...fractureMap,
         ...
         */
        predictFractures: async (_parent, args, context) => {
            const { engine, logger } = context;
            logger.info({
            ...
            const metricsData = generateMockMetrics(100);
            const fractures = await engine.predictFractures(args.systemId, metricsData, args.horizonHours || 72, args.confidenceThreshold || 0.7);
            return fractures.map((f) => f.toJSON());
        },
        /**
         * Get current system stability
         */
        getSystemStability: async (_parent, args, context) => {
            const { engine, logger } = context;
            logger.info({ systemId: args.systemId }, 'Analyzing system stability');
            const metricsData = args.metricsData?.map((d) => ({
         ...
         */
        getRecoveryPlan: async (_parent, args, context) => {
            const { logger } = context;
            logger.info({ fracturePointId: args.fracturePointId }, 'Fetching recovery plan');
         ...
         */
        getMonitoringSessions: async (_parent, args, context) => {
            const { logger } = context;
         ...
         */
        analyzeMetrics: async (_parent, args, context) => {
            const { engine, logger } = context;
            logger.info({
                systemId: args.systemId,
    ...
    Mutation: {
         ...
         */
        monitorSystem: async (_parent, args, context) => {
            const { logger } = context;
            logger.info({
                systemId: args.systemId,
         ...
         */
        stopMonitoring: async (_parent, args, context) => {
            const { logger } = context;
         ...
         */
        setStabilityThresholds: async (_parent, args, context) => {
            const { logger } = context;
            logger.info({
                systemId: args.systemId,
                thresholds: {
         ...
         */
        markFracturePrevented: async (_parent, args, context) => {
            const { logger } = context;
            logger.info({
         ...
         */
        recordRecoveryExecution: async (_parent, args, context) => {
            const { logger } = context;
            logger.info({
                recoveryPlanId: args.recoveryPlanId,

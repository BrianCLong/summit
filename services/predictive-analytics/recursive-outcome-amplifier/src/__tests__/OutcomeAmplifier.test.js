...
describe('OutcomeAmplifier', () => {
    let amplifier;
    beforeEach(() => {
        amplifier = new OutcomeAmplifier_js_1.OutcomeAmplifier({
            defaultMaxOrder: 5,
    ...
    describe('findLeveragePoints', () => {
        it('should find leverage points in cascade', async () => {
            ...
            expect(leveragePoints).toHaveLength(Math.min(5, cascade.leveragePoints.length));
        });
        ...
        it('should throw error for non-existent cascade', async () => {
            await expect(amplifier.findLeveragePoints('non-existent-id')).rejects.toThrow('Cascade not found');
        });
    });
    describe('getAmplificationPath', () => {
        it('should find path to target node', async () => {
            ...
            if (targetNode) {
                const path = await amplifier.getAmplificationPath(cascade.id, targetNode.id);
        ...
        it('should return null for non-existent node', async () => {
            ...
            const cascade = await amplifier.amplifyOutcome(event);
            const path = await amplifier.getAmplificationPath(cascade.id, 'non-existent-node');
    ...
    describe('getAmplificationAnalysis', () => {
        ...
        it('should throw error for non-existent cascade', async () => {
            await expect(amplifier.getAmplificationAnalysis('non-existent-id')).rejects.toThrow('Cascade not found');
        });
    });
    ...
    describe('cache management', () => {
        ...
        it('should not cache when caching is disabled', async () => {
            const noCacheAmplifier = new OutcomeAmplifier_js_1.OutcomeAmplifier({
                enableCaching: false,
            });
            const event = {
                event: 'Test event',
    ...
    describe('configuration', () => {
        it('should use custom default values', async () => {
            const customAmplifier = new OutcomeAmplifier_js_1.OutcomeAmplifier({
                defaultMaxOrder: 3,

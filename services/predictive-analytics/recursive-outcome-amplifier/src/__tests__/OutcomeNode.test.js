...
describe('OutcomeNode', () => {
    describe('OutcomeNodeBuilder', () => {
        it('should build a valid outcome node with defaults', () => {
            const node = new OutcomeNode_js_1.OutcomeNodeBuilder('Test event', 1).build();
        ...
        it('should build a node with custom properties', () => {
            const node = new OutcomeNode_js_1.OutcomeNodeBuilder('Custom event', 2)
                .withProbability(0.7)
                .withMagnitude(5.0)
                .withDomain('ECONOMIC')
                .withTimeDelay(48)
        ...
        it('should clamp probability to 0-1 range', () => {
            const node1 = new OutcomeNode_js_1.OutcomeNodeBuilder('Test', 1)
                .withProbability(1.5)
                .build();
            expect(node1.probability).toBe(1.0);
            const node2 = new OutcomeNode_js_1.OutcomeNodeBuilder('Test', 1)
                .withProbability(-0.5)
                .build();
    ...
    describe('createRootNode', () => {
        it('should create a root node with order 1', () => {
            const root = (0, OutcomeNode_js_1.createRootNode)({
                event: 'Root event',
                domain: 'POLICY',
    ...
    describe('applyDampening', () => {
        it('should dampen probability and magnitude', () => {
            const node = new OutcomeNode_js_1.OutcomeNodeBuilder('Test', 2)
                .withProbability(0.8)
                .withMagnitude(5.0)
                .withConfidence(0.9)
                .build();
            const dampened = (0, OutcomeNode_js_1.applyDampening)(node, 0.7);
        ...
        it('should not modify original node', () => {
            ...
            (0, OutcomeNode_js_1.applyDampening)(node, 0.5);
            expect(node.probability).toBe(originalProb);
        });
    });
    describe('mergeOutcomeNodes', () => {
        it('should remove duplicate nodes', () => {
            const node1 = new OutcomeNode_js_1.OutcomeNodeBuilder('Test', 1).build();
            const node2 = { ...node1 }; // Same ID
            const node3 = new OutcomeNode_js_1.OutcomeNodeBuilder('Different', 2).build();
            const merged = (0, OutcomeNode_js_1.mergeOutcomeNodes)([node1, node2, node3]);
        ...
        it('should keep node with higher probability when merging duplicates', () => {
            ...
            const merged = (0, OutcomeNode_js_1.mergeOutcomeNodes)([node1, node2]);

...
const DampingCalculator_js_1 = require("../algorithms/DampingCalculator.js");
describe('DampingCalculator', () => {
    let calculator;
    beforeEach(() => {
        calculator = new DampingCalculator_js_1.DampingCalculator();
    });
    describe('calculateDampening', () => {
        ...
        it('should consider evidence quality', () => {
            ...
            const lowQualityLink = {
                ...highQualityLink,
                evidenceQuality: 0.3,
                strength: 0.3,
            };
            const highQualityDampening = calculator.calculateDampening(2, highQualityLink);
            const lowQualityDampening = calculator.calculateDampening(2, lowQualityLink);
    ...
    describe('calculateMagnitudeDampening', () => {
        it('should calculate magnitude-specific dampening', () => {
            ...
            const magnitudeDampening = calculator.calculateMagnitudeDampening(2, link);
    ...
    describe('calculateConfidenceDampening', () => {
        it('should decrease confidence with higher orders', () => {
            ...
            const confidence1 = calculator.calculateConfidenceDampening(1, baseConfidence);
            const confidence2 = calculator.calculateConfidenceDampening(2, baseConfidence);
            const confidence3 = calculator.calculateConfidenceDampening(3, baseConfidence);
    ...
    describe('calculateDomainDampening', () => {
        it('should return 1.0 for same domain', () => {
            const dampening = calculator.calculateDomainDampening('ECONOMIC', 'ECONOMIC');
            expect(dampening).toBe(1.0);
        });
        it('should return < 1.0 for cross-domain transitions', () => {
            const dampening = calculator.calculateDomainDampening('ECONOMIC', 'SOCIAL');
            expect(dampening).toBeLessThan(1.0);
            expect(dampening).toBeGreaterThan(0);
        });
        it('should have higher affinity for related domains', () => {
            const economicToSocial = calculator.calculateDomainDampening('ECONOMIC', 'SOCIAL');
            const economicToUnknown = calculator.calculateDomainDampening('ECONOMIC', 'UNKNOWN');
    ...
    describe('calculateAverageDampening', () => {
        it('should calculate average across multiple links', () => {
            ...
            const avgDampening = (0, DampingCalculator_js_1.calculateAverageDampening)(links);
            expect(avgDampening).toBeGreaterThan(0);
            expect(avgDampening).toBeLessThanOrEqual(1.0);
        });
        it('should return 0 for empty array', () => {
            const avgDampening = (0, DampingCalculator_js_1.calculateAverageDampening)([]);
    ...
    describe('findOptimalInterventionOrder', () => {
        it('should find optimal intervention order', () => {
            ...
            const optimalOrder = (0, DampingCalculator_js_1.findOptimalInterventionOrder)(5, link);
    ...
    describe('custom configuration', () => {
        it('should use custom dampening config', () => {
            const customCalculator = new DampingCalculator_js_1.DampingCalculator({

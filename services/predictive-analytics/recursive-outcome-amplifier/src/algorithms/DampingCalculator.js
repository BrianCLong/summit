...
exports.DampingCalculator = void 0;
exports.calculateAverageDampening = calculateAverageDampening;
exports.findOptimalInterventionOrder = findOptimalInterventionOrder;
class DampingCalculator {
     ...
     */
    calculateMagnitudeDampening(order, link) {
     ...
     */
    calculateTimeDampening(timeElapsed, halfLife) {
     ...
     */
    calculateConfidenceDampening(order, baseConfidence) {
     ...
     */
    calculateDomainDampening(sourceDomain, targetDomain) {
     ...
     */
    calculateCombinedDampening(order, link, options = {}) {
        ...
        if (options.timeElapsed !== undefined && options.timeHalfLife) {
            const timeDampening = this.calculateTimeDampening(options.timeElapsed, options.timeHalfLife);
            dampening *= timeDampening;
        }
        // Apply domain dampening if specified
        if (options.sourceDomain && link.domain) {
            const domainDampening = this.calculateDomainDampening(options.sourceDomain, link.domain);
            dampening *= domainDampening;
        }
        return Math.max(0, Math.min(1, dampening));
    }
}
exports.DampingCalculator = DampingCalculator;
/**
 * Calculate average dampening across multiple links
 */
function calculateAverageDampening(links, calculator) {
    if (links.length === 0)
        return 0;
    const calc = calculator || new DampingCalculator();
    const total = links.reduce((sum, { order, link }) => sum + calc.calculateDampening(order, link), 0);
    return total / links.length;
}
/**
 * Find optimal intervention order (where dampening is most effective)
 */
function findOptimalInterventionOrder(maxOrder, sampleLink, calculator) {
    const calc = calculator || new DampingCalculator();
    let maxImpact = 0;
    let optimalOrder = 1;

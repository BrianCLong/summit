...
exports.StabilizationRecommender = void 0;
const StabilizationStrategy_js_1 = require("../models/StabilizationStrategy.js");
/**
 * Generate stabilization recommendations to reduce uncertainty
 */
class StabilizationRecommender {
    config;
    constructor(config = {}) {
        this.config = {
    ...
    async recommend(zone, field) {
        ...
        const filtered = strategies.filter(s => s.priority >= this.config.priorityThreshold);
     ...
     */
    createDataCollectionActions(zone, field) {
        const actions = [];
     ...
     */
    estimateDataCollectionReduction(zone, actions) {
     ...
     */
    estimateDataCollectionEffort(actions) {
        if (actions.length <= 2)
            return 'low';
        if (actions.length <= 4)
            return 'medium';
     ...
     */
    generateModelRefinementStrategies(zone, field) {
        const strategies = [];
     ...
     * Check if zone has model deficiencies
     */
    hasModelDeficiencies(zone, field) {
        ...
        const avgConfidence = zonePoints.reduce((sum, p) => sum + p.confidence, 0) / zonePoints.length;
        return interpolatedRatio > 0.7 || avgConfidence < 0.5;
    }
    /**
     * Create model refinement actions
     */
    createModelRefinementActions(zone, field) {
        const actions = [];
     ...
     */
    estimateModelRefinementReduction(zone, actions) {
        const totalImpact = actions.reduce((sum, a) => sum + a.estimatedImpact, 0);
     ...
     */
    generateConstraintStrategies(zone, field) {
        const strategies = [];
     ...
     */
    createConstraintActions(zone, field) {
        const actions = [];
     ...
     */
    estimateConstraintReduction(zone, actions) {
        const totalImpact = actions.reduce((sum, a) => sum + a.estimatedImpact, 0);
     ...
     */
    generatePruningStrategies(zone, field) {
        const strategies = [];
     ...
     */
    createPruningActions(zone, field) {
        const actions = [];
     ...
     */
    estimatePruningReduction(zone, actions) {
        const totalImpact = actions.reduce((sum, a) => sum + a.estimatedImpact, 0);
     ...
     */
    scoreStrategies(strategies, zone) {
     ...
     */
    optimizePortfolio(strategies, maxCost, maxTimeWeeks) {
        let selected = [];
        let totalCost = 0;
        let totalTime = 0;
        ...
        for (const strategy of sorted) {
            ...
            if ((!maxCost || totalCost + cost <= maxCost) &&
...
exports.StabilizationRecommender = StabilizationRecommender;

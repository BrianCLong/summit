...
exports.PrecursorExtractor = void 0;
const PrecursorSignal_js_1 = require("../models/PrecursorSignal.js");
class PrecursorExtractor {
     ...
     */
    async extract(anomalyPredictionId, timeSeriesData, anomalyOnsetTime) {
        const precursors = [];
        // Identify candidate signals
        const candidates = this.identifyCandidates(timeSeriesData, anomalyOnsetTime);
        // Test temporal precedence
        const validPrecursors = this.testTemporalPrecedence(candidates, timeSeriesData, anomalyOnsetTime);
        // Rank and filter
        ...
        for (const candidate of rankedPrecursors) {
            if (candidate.reliability >= this.config.minReliability) {
                const timeSeries = timeSeriesData.find((ts) => ts.metricName === candidate.metricName);
                if (!timeSeries)
                    continue;
     ...
     */
    identifyCandidates(timeSeriesData, anomalyOnsetTime) {
        const candidates = [];
        for (const ts of timeSeriesData) {
            // Filter data before anomaly onset
            const preAnomalyData = ts.dataPoints.filter((p) => p.timestamp < anomalyOnsetTime);
            if (preAnomalyData.length < 10)
                continue;
            // Calculate lead time (time between last data point and anomaly)
            const lastPoint = preAnomalyData[preAnomalyData.length - 1];
            const leadTimeMs = anomalyOnsetTime.getTime() - lastPoint.timestamp.getTime();
            if (leadTimeMs < this.config.minLeadTimeMs ||
     ...
     */
    testTemporalPrecedence(candidates, timeSeriesData, anomalyOnsetTime) {
        const validCandidates = [];
        for (const candidate of candidates) {
            const ts = timeSeriesData.find((t) => t.metricName === candidate.metricName);
            if (!ts)
                continue;
            ...
            candidate.reliability = this.calculateReliability(grangerPValue, crossCorr, mutualInfo);
            if (candidate.reliability >= this.config.minReliability) {
     ...
     */
    grangerCausalityTest(timeSeries, anomalyOnsetTime) {
     ...
     */
    calculateAutoregressiveRSS(values, order, includeSecondSeries) {
        let rss = 0;
        const n = values.length;
     ...
     */
    crossCorrelation(timeSeries, anomalyOnsetTime) {
        const values = timeSeries.dataPoints
            .filter((p) => p.timestamp < anomalyOnsetTime)
            .map((p) => p.value);
        if (values.length < 10)
     ...
     */
    mutualInformation(timeSeries, anomalyOnsetTime) {
        const values = timeSeries.dataPoints
            .filter((p) => p.timestamp < anomalyOnsetTime)
            .map((p) => p.value);
        if (values.length < 10)
        ...
        const discretized = values.map((v) => Math.min(Math.floor((v - min) / binSize), bins - 1));
     ...
     */
    calculateReliability(grangerPValue, crossCorr, mutualInfo) {
     ...
     */
    extractPattern(timeSeries, anomalyOnsetTime) {
        const values = timeSeries.dataPoints
            .filter((p) => p.timestamp < anomalyOnsetTime)
            .map((p) => p.value);
        ...
        const typicalDurationMs = this.calculatePatternDuration(timeSeries, anomalyOnsetTime);
     ...
     */
    determinePatternShape(values) {
        if (values.length < 5)
            return PrecursorSignal_js_1.PatternShape.STEP;
        // Calculate trend
        ...
        // Classify
        if (hasSpike)
            return PrecursorSignal_js_1.PatternShape.SPIKE;
        if (hasStepChange)
            return PrecursorSignal_js_1.PatternShape.STEP;
        if (isOscillating)
            return PrecursorSignal_js_1.PatternShape.OSCILLATING;
        if (trend > 0.5)
            return PrecursorSignal_js_1.PatternShape.INCREASING;
        if (trend < -0.5)
            return PrecursorSignal_js_1.PatternShape.DECREASING;
        return PrecursorSignal_js_1.PatternShape.PLATEAU;
    }
    ...
    detectStepChange(values) {
        const mean = values.reduce((a, b) => a + b, 0) / values.length;
        const stdDev = Math.sqrt(values.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) /
            values.length);
        for (let i = 1; i < values.length; i++) {
    ...
    calculatePatternDuration(timeSeries, anomalyOnsetTime) {
        const preAnomalyData = timeSeries.dataPoints.filter((p) => p.timestamp < anomalyOnsetTime);
        if (preAnomalyData.length < 2)
            return 0;
        const startTime = preAnomalyData[0].timestamp.getTime();
        const endTime = preAnomalyData[preAnomalyData.length - 1].timestamp.getTime();
        return endTime - startTime;
    }
    calculatePatternConfidence(values, shape) {
        // Confidence based on how well values match expected shape
        let matchScore = 0.5; // Base confidence
        const trend = this.calculateTrend(values);
        switch (shape) {
            case PrecursorSignal_js_1.PatternShape.INCREASING:
                matchScore = trend > 0 ? Math.min(trend * 2, 1.0) : 0.3;
                break;
            case PrecursorSignal_js_1.PatternShape.DECREASING:
                matchScore = trend < 0 ? Math.min(Math.abs(trend) * 2, 1.0) : 0.3;
                break;
            case PrecursorSignal_js_1.PatternShape.OSCILLATING:
    ...
    describePattern(shape, values) {
        ...
        switch (shape) {
            ...
            case PrecursorSignal_js_1.PatternShape.SPIKE:
                return 'Sudden spike detected';
            case PrecursorSignal_js_1.PatternShape.STEP:
                return 'Step change detected';
            case PrecursorSignal_js_1.PatternShape.PLATEAU:
                return 'Stable plateau';
            default:
                return 'Pattern detected';
        }
    }
    calculateThresholds(timeSeries) {
        ...
        const currentValue = this.getCurrentValue(timeSeries);
        let currentStatus = PrecursorSignal_js_1.ThresholdStatus.NORMAL;
        if (currentValue !== null) {
            if (currentValue >= critical) {
                currentStatus = PrecursorSignal_js_1.ThresholdStatus.CRITICAL;
            }
            else if (currentValue >= warning) {
                currentStatus = PrecursorSignal_js_1.ThresholdStatus.WARNING;
            }
        }
        return {
            warning,
...
exports.PrecursorExtractor = PrecursorExtractor;

...
exports.TimelineWarper = void 0;
class TimelineWarper {
    config;
    constructor(config) {
        this.config = config;
    }
     ...
     */
    async warp(entityId, referenceAnomalyId, currentTimeline, referenceTimeline) {
        // Normalize timelines
        ...
        const warpingPath = this.dynamicTimeWarping(normalizedCurrent, normalizedReference);
        // Generate warped timeline
        const warpedTimeline = this.applyWarping(normalizedCurrent, warpingPath);
        // Calculate alignment score
        const alignmentScore = this.calculateAlignmentScore(normalizedCurrent, warpedTimeline, warpingPath);
        // Identify temporal landmarks
        const temporalLandmarks = this.identifyLandmarks(warpedTimeline, referenceTimeline);
        return {
            entityId,
            referenceAnomalyId,
    ...
    dynamicTimeWarping(ts1, ts2) {
        ...
        while (i > 0 && j > 0) {
            ...
            const minOption = options.reduce((min, opt) => opt.val < min.val ? opt : min);
            i = minOption.i;
            j = minOption.j;
        }
     ...
     */
    calculateStretchFactor(path, currentIndex, ts1, ts2) {
        ...
        const span1 = ts1.dataPoints[i2].timestamp.getTime() -
            ts1.dataPoints[i1].timestamp.getTime();
        const span2 = ts2.dataPoints[j2].timestamp.getTime() -
     ...
     */
    applyWarping(original, warpingPath) {
        const warpedPoints = warpingPath.map((wp) => {
            // Find original data point
            const originalPoint = original.dataPoints.find((p) => p.timestamp.getTime() === wp.originalTime.getTime());
            if (!originalPoint) {
                return {
     ...
     */
    calculateAlignmentScore(original, warped, warpingPath) {
        // Calculate correlation between aligned sequences
        const originalValues = [];
        const warpedValues = [];
        for (const wp of warpingPath) {
            const origPoint = original.dataPoints.find((p) => p.timestamp.getTime() === wp.originalTime.getTime());
            const warpPoint = warped.dataPoints.find((p) => p.timestamp.getTime() === wp.warpedTime.getTime());
     ...
     */
    identifyLandmarks(warpedTimeline, referenceTimeline) {
        const landmarks = [];
        ...
        for (const warpedPeak of warpedPeaks) {
            ...
            for (const refPeak of referencePeaks) {
                const distance = Math.abs(warpedPeak.timestamp.getTime() - refPeak.timestamp.getTime());
    ...
    findInflectionPoints(timeSeries) {
        const inflections = [];
        const points = timeSeries.dataPoints;
        if (points.length < 3)
            return inflections;
     ...
     */
    async decomposeMultiScale(timeSeries) {
        const values = timeSeries.dataPoints.map((p) => p.value);
        ...
        const seasonalValues = Array(n).fill(0);
        if (n >= seasonalPeriod * 2) {
            for (let phase = 0; phase < seasonalPeriod; phase++) {
                ...
                const phaseAvg = phaseValues.reduce((a, b) => a + b, 0) / phaseValues.length;
                for (let i = phase; i < n; i += seasonalPeriod) {
...
exports.TimelineWarper = TimelineWarper;

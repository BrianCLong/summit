...
exports.TimeWarpedTimelineModel = void 0;
class TimeWarpedTimelineModel {
    /**
     * Calculate average stretch factor across warping path
     */
    static calculateAverageStretch(warpingPath) {
        if (warpingPath.length === 0)
            return 1.0;
        const sum = warpingPath.reduce((acc, point) => acc + point.stretchFactor, 0);
        return sum / warpingPath.length;
    }
    /**
     * Find regions of significant time compression/expansion
     */
    static findSignificantWarps(warpingPath, threshold = 1.5) {
        const regions = [];
        let currentRegion = null;
        for (const point of warpingPath) {
            const isSignificant = point.stretchFactor > threshold || point.stretchFactor < 1 / threshold;
            if (isSignificant) {
                if (!currentRegion) {
                    currentRegion = [point];
                }
                ...
                if (currentRegion && currentRegion.length > 0) {
                    const avgStretch = currentRegion.reduce((sum, p) => sum + p.stretchFactor, 0) /
     ...
     */
    static interpolateValue(timeSeries, targetTime) {
     ...
     */
    static calculateAlignmentMetrics(original, warped, warpingPath) {
        // Root Mean Square Error
        let sumSquaredError = 0;
        let count = 0;
        for (const point of warpingPath) {
            const originalValue = this.interpolateValue(original, point.originalTime);
        ...
        const maxStretch = Math.max(...warpingPath.map((p) => Math.abs(Math.log(p.stretchFactor))));
        return { rmse, correlation, avgStretch, maxStretch };
    }
    /**
     * Calculate Pearson correlation between two time series
     */
    static calculateCorrelation(ts1, ts2) {
     ...
     */
    static identifyLandmarks(timeSeries, threshold = 2.0) {
        const landmarks = [];
        const points = timeSeries.dataPoints;
        if (points.length < 3)
            return landmarks;
    ...
    static validate(data) {
        ...
        if (!data.originalTimeline)
            errors.push('originalTimeline is required');
        if (!data.warpedTimeline)
            errors.push('warpedTimeline is required');
...
exports.TimeWarpedTimelineModel = TimeWarpedTimelineModel;

...
exports.DominanceScoreModel = void 0;
class DominanceScoreModel {
    patternId;
    score;
    growthRate;
    fitnessScore;
     ...
     */
    static calculateGrowthRate(strengthHistory) {
        ...
        const meanLogStrength = logStrengths.reduce((sum, s) => sum + s, 0) / n;
        let numerator = 0;
        let denominator = 0;
        for (let i = 0; i < n; i++) {
     ...
     */
    static calculateFitness(intrinsicStrength, competitionIntensities) {
        const competitionPressure = competitionIntensities.reduce((sum, c) => sum + c, 0);
        return intrinsicStrength / (1 + competitionPressure);
    }
    /**
     * Calculate resilience score
     */
    static calculateResilience(structuralRedundancy, resourceDiversity) {
     ...
     */
    static calculateNetworkScore(connectedComponents, beta = 0.5) {
     ...
     */
    calculateConfidence(dataQuality, historicalAccuracy) {
...
exports.DominanceScoreModel = DominanceScoreModel;

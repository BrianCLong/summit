...
exports.CompetitionSimulator = void 0;
const ProtoPattern_js_1 = require("../models/ProtoPattern.js");
const PatternCompetition_js_1 = require("../models/PatternCompetition.js");
class CompetitionSimulator {
    /**
     * Simulate competition between proto-patterns
     */
    async simulateCompetition(protoPatterns, config) {
        // Initialize competition
        const competition = new PatternCompetition_js_1.PatternCompetitionModel({
        ...
        // Run simulation
        const trajectory = competition.simulateLotkaVolterra(initialStates.map((s) => s.strength), initialStates.map((s) => s.growthRate), initialStates.map((s) => s.carryingCapacity), config.timeHorizon, 0.1);
        // Determine outcome
        ...
        if (config.cooperationAllowed || config.inhibitionEnabled) {
            this.adjustForInteractions(competition, protoPatterns, config);
        }
        return competition;
    }
    /**
     ...
     */
    calculateOverlaps(protoPatterns) {
     ...
     */
    calculatePairwiseOverlap(p1, p2) {
        ...
        const nodeIntersection = new Set([...nodeIds1].filter((x) => nodeIds2.has(x)));
        const nodeUnion = new Set([...nodeIds1, ...nodeIds2]);
        const structuralOverlap = nodeUnion.size > 0 ? nodeIntersection.size / nodeUnion.size : 0;
     ...
     */
    estimateGrowthRate(protoPattern) {
        // Growth rate based on confidence and signal strength
        const avgSignalStrength = protoPattern.weakSignals.length > 0
     ...
     */
    estimateCarryingCapacity(protoPattern, config) {
     ...
     */
    adjustForInteractions(competition, protoPatterns, config) {
     ...
     */
    async runScenarios(protoPatterns, scenarios) {
        const results = new Map();
        for (const [index, config] of scenarios.entries()) {
            const competition = await this.simulateCompetition(protoPatterns, config);
     ...
     */
    async analyzeSensitivity(protoPatterns, baseConfig, perturbations) {
        const results = [];
        for (let i = 0; i < perturbations; i++) {
            // Perturb initial conditions
            const perturbedPatterns = protoPatterns.map((p) => {
                const perturbed = new ProtoPattern_js_1.ProtoPatternModel(p.toJSON());
                perturbed.confidence *= 0.9 + Math.random() * 0.2; // ±10%
                return perturbed;
            });
            const competition = await this.simulateCompetition(perturbedPatterns, baseConfig);
            results.push({
                perturbation: i,
...
exports.CompetitionSimulator = CompetitionSimulator;

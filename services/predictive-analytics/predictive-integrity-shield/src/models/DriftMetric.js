...
class DriftMetricCalculator {
     ...
     */
    static calculateKS(baseline, current) {
     ...
     */
    static calculateJSDivergence(p, q) {
     ...
     */
    static calculateFeatureStats(data) {
        if (data.length === 0) {
            return {
                mean: 0,
                median: 0,
     ...
     */
    static determineSeverity(psi) {
        if (psi < 0.1)
            return IntegrityReport_js_1.DriftSeverity.NONE;
        if (psi < 0.15)
            return IntegrityReport_js_1.DriftSeverity.LOW;
        if (psi < 0.25)
            return IntegrityReport_js_1.DriftSeverity.MODERATE;
        if (psi < 0.35)
            return IntegrityReport_js_1.DriftSeverity.HIGH;
        return IntegrityReport_js_1.DriftSeverity.CRITICAL;
    }
    /**
     * Analyze drift trend
     */
    static analyzeTrend(recentPSI) {
        if (recentPSI.length < 2)
            return 'stable';
        const recentAvg = recentPSI.slice(-3).reduce((sum, val) => sum + val, 0) /
            Math.min(3, recentPSI.length);
        const olderAvg = recentPSI.slice(0, -3).reduce((sum, val) => sum + val, 0) /
     ...
     */
    static calculateVelocity(recentPSI) {
        if (recentPSI.length < 2)
            return 0;
        const diffs = [];
     ...
     */
    static estimateImpact(psi, velocity) {
     ...
     */
    static detectConceptDrift(errors, windowSize = 100) {
        ...
        const stdDev = Math.sqrt(errorRate * (1 - errorRate) / windowSize);
...
exports.DriftMetricCalculator = DriftMetricCalculator;

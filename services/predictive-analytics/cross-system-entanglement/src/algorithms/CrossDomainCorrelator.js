...
exports.CrossDomainCorrelator = void 0;
const simple_statistics_1 = require("simple-statistics");
const pino_1 = __importDefault(require("pino"));
const EntanglementSignature_js_1 = require("../models/EntanglementSignature.js");
const logger = (0, pino_1.default)({ name: 'CrossDomainCorrelator' });
class CrossDomainCorrelator {
    ...
    constructor(config = {}) {
        ...
        this.knownDependencies = new Set((config.knownDependencies || []).map(([a, b]) => `${a}-${b}`));
    }
    /**
     * Discover cross-domain correlations
     */
    async discoverCorrelations(domainMetrics) {
        logger.info({ metricsCount: domainMetrics.length }, 'Starting cross-domain correlation discovery');
        ...
        for (let i = 0; i < domains.length; i++) {
            for (let j = i + 1; j < domains.length; j++) {
                ...
                const domainCorrelations = await this.correlateDomainPair(domain1, domain2, domainGroups.get(domain1), domainGroups.get(domain2));
     ...
     */
    async correlateDomainPair(domain1, domain2, metrics1, metrics2) {
        const correlations = [];
        for (const m1 of metrics1) {
            for (const m2 of metrics2) {
                const correlation = this.computeCorrelation(m1.timeSeries, m2.timeSeries);
                if (correlation.correlation >= this.minCorrelation) {
                    ...
                    if (crossDomainScore >= this.minCrossDomainScore) {
                        const isUnexpected = this.isUnexpectedCorrelation(m1.systemId, m2.systemId);
                        correlations.push({
    ...
    computeCorrelation(ts1, ts2) {
        ...
        try {
            const correlation = (0, simple_statistics_1.sampleCorrelation)(aligned.values1, aligned.values2);
            const significance = this.calculateSignificance(correlation, aligned.values1.length);
     ...
     */
    calculateDomainDistance(domain1, domain2) {
        // Simplified distance calculation
        // Production would use actual architecture graph and semantic analysis
        const architecturalDistance = this.calculateArchitecturalDistance(domain1, domain2);
        const semanticDistance = this.calculateSemanticDistance(domain1, domain2);
        const operationalDistance = this.calculateOperationalDistance(domain1, domain2);
        // Weighted average
        const overallDistance = architecturalDistance * 0.4 +
     ...
     */
    calculateArchitecturalDistance(domain1, domain2) {
     ...
     */
    calculateSemanticDistance(domain1, domain2) {
     ...
     */
    calculateOperationalDistance(domain1, domain2) {
     ...
     */
    isUnexpectedCorrelation(system1, system2) {
     ...
     * Filter and prioritize unexpected correlations
     */
    filterUnexpectedCorrelations(correlations) {
        return correlations
     ...
     */
    convertToSignatures(correlations) {
        return correlations.map((correlation) => {
            return (0, EntanglementSignature_js_1.createEntanglementSignature)([correlation.system1.systemId, correlation.system2.systemId], correlation.correlation, 0, // Cross-domain correlations typically have minimal lag
     ...
     */
    async grangerCausalityTest(ts1, ts2, maxLag = 10) {
     ...
     */
    grangerScore(cause, effect, maxLag) {
        ...
        const improvedError = errorsWithCause.reduce((a, b) => a + b, 0) / errorsWithCause.length;
        // Improvement score
        return Math.max(0, (baselineError - improvedError) / baselineError);
    }
}
exports.CrossDomainCorrelator = CrossDomainCorrelator;

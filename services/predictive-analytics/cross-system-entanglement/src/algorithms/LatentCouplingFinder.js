...
exports.LatentCouplingFinder = void 0;
const simple_statistics_1 = require("simple-statistics");
const pino_1 = __importDefault(require("pino"));
const EntanglementSignature_js_1 = require("../models/EntanglementSignature.js");
const logger = (0, pino_1.default)({ name: 'LatentCouplingFinder' });
class LatentCouplingFinder {
    minCorrelation;
    maxLagMs;
    minConfidence;
    observationWindow;
     ...
     */
    async findCouplings(timeSeriesData) {
        logger.info({ systemCount: timeSeriesData.length }, 'Starting latent coupling detection');
        const signatures = [];
        ...
        logger.info({ signaturesFound: signatures.length }, 'Latent coupling detection complete');
        return signatures;
    }
    /**
     * Analyze a pair of systems for correlation
     */
    async analyzePair(system1, system2) {
        try {
            ...
            const lagResults = this.computeCrossCorrelation(aligned.values1, aligned.values2);
     ...
     * Compute cross-correlation at multiple lag times
     */
    computeCrossCorrelation(values1, values2) {
        const results = [];
     ...
     */
    computeLaggedCorrelation(values1, values2, lag) {
        if (lag === 0) {
    ...
    pearsonCorrelation(x, y) {
        ...
        try {
            return (0, simple_statistics_1.sampleCorrelation)(x, y);
        }
        catch (error) {
     ...
     */
    createSignature(result, system1, system2) {
        const signatureType = this.classifySignatureType(result);
        return (0, EntanglementSignature_js_1.createEntanglementSignature)([system1.systemId, system2.systemId], Math.abs(result.correlation), Math.abs(result.lagTime), signatureType, result.confidence, {
     ...
     */
    validateCorrelation(signature, historicalData) {
...
exports.LatentCouplingFinder = LatentCouplingFinder;

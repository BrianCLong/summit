...
const DEFAULT_DATA_ACCESS_WEIGHTS: Record<
  ToolRiskProfile['dataAccessScope'],
  number
> = {
  none: 0,
  read: 1,
...
const DEFAULT_NETWORK_WEIGHTS: Record<
...
export function computeRiskScore(
  ...
  const cveScore = profile.cves.reduce(
    (total, cve) => total + severityWeights[cve.severity],
    0,
  );
...
export function scoreCveRecord(
  record: CveRecord,
  options?: RiskComputationOptions,
): number {
  const severityWeights = {

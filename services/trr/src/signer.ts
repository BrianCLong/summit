...
export class AllowlistSigner {
  ...
  sign(payload: { environment: string; entries: AllowlistEntry[]; generatedAt?: string }): AllowlistManifest {
    const canonicalEntries = canonicalizeEntries(payload.entries);
    const generatedAt =
      payload.generatedAt ?? this.canonicalTimestamp(canonicalEntries, payload.environment);
    const canonical = JSON.stringify({
      environment: payload.environment,
      generatedAt,
      entries: canonicalEntries
    });
    const signature = createHmac(this.algorithm, this.secret).update(canonical).digest('hex');
    return {
      environment: payload.environment,
      generatedAt,
      entries: canonicalEntries,
  ...
  private canonicalTimestamp(entries: AllowlistEntry[], environment: string): string {
    if (entries.length === 0) {

...
describe("ER Service API", () => {
  ...
  describe("POST /api/v1/candidates", () => {
    it("should find candidate matches", async () => {
      ...
      const response = await request(app).post("/api/v1/candidates").send(requestBody).expect(200);
  ...
  describe("POST /api/v1/merge", () => {
    it("should merge entities successfully", async () => {
      ...
      const response = await request(app).post("/api/v1/merge").send(requestBody).expect(201);
    ...
    it("should reject merge with less than 2 entities", async () => {
      ...
      const response = await request(app).post("/api/v1/merge").send(requestBody).expect(400);
  ...
  describe("POST /api/v1/merge/:mergeId/revert", () => {
    ...
    it("should reject revert without required fields", async () => {
      await request(app).post("/api/v1/merge/some-id/revert").send({}).expect(400);
    });
  });
  ...
  describe("POST /api/v1/split", () => {
    it("should split entity successfully", async () => {
      ...
      await request(app)
        ...
        .send({
          id: "split-e1",
          type: "person",
          name: "Combined Identity",
          tenantId: "test-tenant",
          attributes: { context: "mixed" },
        })
        .expect(201);
      ...
      const requestBody = {
        tenantId: "test-tenant",
        entityId: "split-e1",
        splitGroups: [{ attributes: { context: "work" } }, { attributes: { context: "personal" } }],
        actor: "analyst@example.com",
        reason: "Separate identities",
      };

      const response = await request(app).post("/api/v1/split").send(requestBody).expect(201);
    ...
    it("should reject split with less than 2 groups", async () => {
      ...
      const response = await request(app).post("/api/v1/split").send(requestBody).expect(400);
  ...
  describe("GET /api/v1/explain/:mergeId", () => {
    it("should explain merge decision", async () => {
      ...
      // Get explanation
      const explainResponse = await request(app).get(`/api/v1/explain/${mergeId}`).expect(200);
    ...
    it("should return 500 for non-existent merge", async () => {
      await request(app).get("/api/v1/explain/non-existent-id").expect(500);
    });
  });
  ...
  describe("GET /api/v1/merge/:mergeId/export", () => {
    it("should export merge bundle with explanation details", async () => {
      ...
      const exportResponse = await request(app).get(`/api/v1/merge/${mergeId}/export`).expect(200);
  ...
  describe("GET /api/v1/audit", () => {
    it("should retrieve audit log", async () => {
      ...
      const response = await request(app).get("/api/v1/audit?tenantId=test-tenant").expect(200);
    ...
    it("should filter audit log by event type", async () => {
      const response = await request(app).get("/api/v1/audit?event=merge&limit=10").expect(200);

...
const remediated = {
  ...nonCompliant,
  webauthn: { userVerified: true, userPresent: true },
  local: { ...nonCompliant.local, firewallEnabled: true, screenLock: true },
};
...
test("non-compliant → remediate → pass", async ({ request }) => {
  ...
  const fail = await request.post("http://localhost:8090/evaluate", {
    data: { deviceId, ...nonCompliant, secureContext: false },
  });
  expect(fail.status()).toBe(200);
  const failBody = await fail.json();
  expect(failBody.decision.status).toBe("step_up");

  const pass = await request.post("http://localhost:8090/evaluate", {
    data: { deviceId, ...remediated, secureContext: true },
  });

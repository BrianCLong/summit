...
test("requires step-up when score below threshold", () => {
  const policy = {
    stepUp: { minimumScore: 95, factor: "security-key" },
    downgrade: { minimumScore: 50, capabilities: [] },
  };
  const signal = {
    userId: "tester",
...
test("downgrades session when below downgrade threshold", () => {
  const policy = {
    stepUp: { minimumScore: 10, factor: "otp" },
    downgrade: { minimumScore: 90, capabilities: ["read-only"] },
  };
  const signal = {
    userId: "tester",

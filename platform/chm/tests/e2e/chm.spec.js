...
(0, test_1.test)('apply tag → blocked export → approved downgrade → export', async ({ page }) => {
    await page.setContent(pageTemplate);
    await page.getByText('Apply CHM-TS Tag').click();
    await (0, test_1.expect)(page.locator('#doc-stamp')).toHaveText('CHM-TS applied');
    await page.getByText('Attempt Export').click();
    await (0, test_1.expect)(page.locator('#status')).toHaveText('Status: export blocked (residency/license)');
    await page.getByText('Approve A').click();
    await page.getByText('Approve B').click();
    await (0, test_1.expect)(page.locator('#doc-stamp')).toHaveText('CHM-S downgraded');
    await (0, test_1.expect)(page.locator('#status')).toHaveText('Status: downgrade approved');
    await page.getByText('Attempt Export').click();
    await (0, test_1.expect)(page.locator('#status')).toHaveText('Status: export allowed after downgrade');
    await page.getByLabel('Derived-from marking').check();
    await (0, test_1.expect)(page.locator('#derived-label')).toBeVisible();
});

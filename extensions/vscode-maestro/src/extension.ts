...
class MaestroAPI {
  ...
  async runPipeline(workflowPath: string, parameters: Record<string, any> = {}): Promise<string> {
    ...
    const response = await this.client.post('/api/v1/runs', {
      workflow,
      parameters,
      environment: vscode.workspace.getConfiguration('maestro').get('defaultEnvironment', 'development'),
      triggered_by: 'vscode'
    });
...
class MaestroRunsProvider implements vscode.TreeDataProvider<MaestroRunItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<MaestroRunItem | undefined | null | void> = new vscode.EventEmitter<MaestroRunItem | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<MaestroRunItem | undefined | null | void> = this._onDidChangeTreeData.event;
  ...
  getChildren(element?: MaestroRunItem): Thenable<MaestroRunItem[]> {
    if (!element) {
      ...
      return Promise.resolve(
        this.runs.map(run => new MaestroRunItem(
          run.run_id,
      ...
      return this.api.getRunSteps(element.runId).then(steps => {
        return steps.map(step => new MaestroRunItem(
          step.step_id,
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const treeView = vscode.window.createTreeView('maestroRuns', { treeDataProvider: provider });
  ...
  const runPipelineCommand = vscode.commands.registerCommand('maestro.runPipeline', async () => {
    try {
      ...
      vscode.window.showInformationMessage(`Pipeline started: ${runId}`, 'Show Logs').then(selection => {
  ...
  const showStatusCommand = vscode.commands.registerCommand('maestro.showStatus', async (item: MaestroRunItem) => {
    try {
  ...
  const showLogsCommand = vscode.commands.registerCommand('maestro.showLogs', async (runIdOrItem: string | MaestroRunItem) => {
  ...
  const cancelRunCommand = vscode.commands.registerCommand('maestro.cancelRun', async (item: MaestroRunItem) => {
    try {

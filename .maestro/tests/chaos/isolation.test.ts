...
class IsolationChaosTest {
    ...
    return tracer.startActiveSpan("chaos.stale_cache_test", async (span: Span) => {
      ...
      try {
        ...
        const concurrentQueries = await Promise.allSettled([
          ...
          pg.many("SELECT * FROM api_keys WHERE tenant_id = $1", [this.VICTIM_TENANT], {
            tenantId: this.VICTIM_TENANT,
          }),

          // Attacker attempts to read victim data (should be blocked)
          pg
            .many("SELECT * FROM api_keys WHERE tenant_id = $1", [this.VICTIM_TENANT], {
        ...
        const operations = await Promise.allSettled([
          ...
          Promise.all([
            pg.withTenant(this.VICTIM_TENANT, async (scopedPg) => {
              ...
              return scopedPg.many("SELECT tenant_id FROM audit_logs WHERE action = $1", [
                "chaos-write",
              ]);
            }),
  ...
  async cleanup(): Promise<void> {
    ...
    // Clean up test data
    for (const tenant of this.testTenants) {
      await pg.oneOrNone("DELETE FROM api_keys WHERE tenant_id = $1", [tenant.id], {
        tenantId: tenant.id,
      });

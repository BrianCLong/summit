...
class IsolationChaosTest {
    ...
    this.testTenants = [
      {
        ...
        signals: [
          { type: 'financial-transaction', value: 50000, purpose: 'investigation' },
      ...
      },
      {
        ...
        users: [
          { email: 'attacker@malicious.com', role: 'user' }
        ]
      }
    ];
  ...
  private async seedTestData(): Promise<void> {
    return tracer.startActiveSpan('chaos.seed_test_data', async (span: Span) => {
  ...
  private async testStaleCache(): Promise<ChaosResult> {
    return tracer.startActiveSpan('chaos.stale_cache_test', async (span: Span) => {
      ...
      try {
        ...
        console.log(`   🛡️  Attacker blocked: ${attackerAttempt.error ? 'YES' : 'NO'}`);
        console.log(`   🛡️  Neo4j blocked: ${neoAttempt.error ? 'YES' : 'NO'}`);
      ...
      isolationTestDuration.observe({ test_type: 'stale_cache', fault_type: 'cache_invalidation' }, result.duration);
      ...
      console.log(`   Result: ${result.passed ? '✅ PASS' : '❌ FAIL'} (${result.duration.toFixed(2)}s)\n`);
      return result;
    });
  }

  private async testOPAFailure(): Promise<ChaosResult> {
    return tracer.startActiveSpan('chaos.opa_failure_test', async (span: Span) => {
      const startTime = Date.now();
      const result: ChaosResult = {
      ...
      try {
        ...
        const rapidSwitches = Array.from({ length: 10 }, (_, i) => {
          const tenant = i % 2 === 0 ? this.VICTIM_TENANT : this.ATTACKER_TENANT;
          return {
            query: pg.many(
        ...
        const results = await Promise.allSettled(rapidSwitches.map(s => s.query));
        ...
        results.forEach((res, i) => {
          if (res.status === 'fulfilled' && res.value) {
            ...
            const wrongTenants = actualTenants.filter((t: string) => t !== expectedTenant);
            if (wrongTenants.length > 0) {
              result.leaksDetected += wrongTenants.length;
              result.passed = false;
              result.errors.push(`OPA failure allowed cross-tenant access: ${wrongTenants.join(', ')}`);
        ...
        console.log(`   🔄 Tested ${rapidSwitches.length} rapid tenant switches`);
      ...
      result.duration = (Date.now() - startTime) / 1000;
      isolationTestDuration.observe({ test_type: 'opa_failure', fault_type: 'connection_loss' }, result.duration);
  ...
  private async testDatabaseRaces(): Promise<ChaosResult> {
    return tracer.startActiveSpan('chaos.database_race_test', async (span: Span) => {
      const startTime = Date.now();
      const result: ChaosResult = {
      ...
      try {
        ...
        const concurrentOps = await Promise.allSettled([
          ...
          ...Array.from({ length: 5 }, () => 
            pg.withTenant(this.VICTIM_TENANT, async (scopedPg) => {
              const result = await scopedPg.many('SELECT tenant_id FROM api_keys LIMIT 10');
          ...
          ...Array.from({ length: 5 }, () =>
            neo.withTenant(this.ATTACKER_TENANT, async (scopedNeo) => {
              const result = await scopedNeo.run('MATCH (s:Signal) RETURN s.tenant_id LIMIT 10');
        ...
        concurrentOps.forEach((res, i) => {
          if (res.status === 'fulfilled' && res.value) {
            ...
            const expectedTenant = i < 5 ? this.VICTIM_TENANT : this.ATTACKER_TENANT;
            
            const wrongTenants = tenantIds.filter(tid => tid && tid !== expectedTenant);
            if (wrongTenants.length > 0) {
              result.leaksDetected += wrongTenants.length;
              result.passed = false;
              result.errors.push(`Race condition caused tenant leak: ${wrongTenants.join(', ')}`);
        ...
        console.log(`   ⚡ Tested ${concurrentOps.length} concurrent database operations`);
        console.log(`   🛡️  Race condition leaks: ${result.leaksDetected}`);

      } catch (error) {
        result.errors.push(`Database race test error: ${(error as Error).message}`);
        result.passed = false;
      }

      result.duration = (Date.now() - startTime) / 1000;
      isolationTestDuration.observe({ test_type: 'db_races', fault_type: 'concurrent_ops' }, result.duration);
  ...
  private async testMemoryPressure(): Promise<ChaosResult> {
    return tracer.startActiveSpan('chaos.memory_pressure_test', async (span: Span) => {
      const startTime = Date.now();
      const result: ChaosResult = {
      ...
      try {
        ...
        if (pgResult.status === 'fulfilled' && pgResult.value) {
          ...
          if (wrongTenants.length > 0) {
            result.leaksDetected += wrongTenants.length;
            result.passed = false;
            result.errors.push(`Memory pressure caused PostgreSQL tenant mixing`);
          }
        }
        ...
        console.log(`   🛡️  Memory corruption leaks: ${result.leaksDetected}`);

      } catch (error) {
        result.errors.push(`Memory pressure test error: ${(error as Error).message}`);
        result.passed = false;
      }

      result.duration = (Date.now() - startTime) / 1000;
      isolationTestDuration.observe({ test_type: 'memory_pressure', fault_type: 'large_queries' }, result.duration);
  ...
  private async testConcurrentTenants(): Promise<ChaosResult> {
    return tracer.startActiveSpan('chaos.concurrent_tenants_test', async (span: Span) => {
      const startTime = Date.now();
      const result: ChaosResult = {
      ...
      try {
        ...
        const operations = await Promise.allSettled([
          ...
          Promise.all([
            pg.withTenant(this.VICTIM_TENANT, async (scopedPg) => {
              await scopedPg.oneOrNone(
                ...
                [this.VICTIM_TENANT, 'chaos-write', 'test-resource', new Date()]
              );
              return scopedPg.many('SELECT tenant_id FROM audit_logs WHERE action = $1', ['chaos-write']);
            }),
            neo.withTenant(this.VICTIM_TENANT, async (scopedNeo) => {
              ...
              return scopedNeo.run('MATCH (s:Signal {type: "chaos-test"}) RETURN s.tenant_id');
            })
          ]),
          ...
          Promise.all([
            pg.withTenant(this.ATTACKER_TENANT, async (scopedPg) => {
              await scopedPg.oneOrNone(
                'INSERT INTO audit_logs (tenant_id, action, resource, created_at) VALUES ($1, $2, $3, $4)',
                [this.ATTACKER_TENANT, 'chaos-write', 'test-resource', new Date()]
              );
              return scopedPg.many('SELECT tenant_id FROM audit_logs WHERE action = $1', ['chaos-write']);
            }),
            neo.withTenant(this.ATTACKER_TENANT, async (scopedNeo) => {
        ...
        operations.forEach((opGroup, groupIndex) => {
          if (opGroup.status === 'fulfilled' && opGroup.value) {
            const expectedTenant = groupIndex === 0 ? this.VICTIM_TENANT : this.ATTACKER_TENANT;
            const [pgResult, neoResult] = opGroup.value;

            // Check PostgreSQL results
            if (pgResult) {
              const tenantIds = pgResult.map((row: any) => row.tenant_id);
              const wrongTenants = tenantIds.filter((tid: string) => tid !== expectedTenant);
              if (wrongTenants.length > 0) {
                result.leaksDetected += wrongTenants.length;
                result.passed = false;
                result.errors.push(`Concurrent ops: PostgreSQL leak for tenant ${expectedTenant}`);
              }
            }

            // Check Neo4j results
            if (neoResult && neoResult.records) {
              const tenantIds = neoResult.records.map((r: any) => r.get('s.tenant_id'));
              const wrongTenants = tenantIds.filter((tid: string) => tid && tid !== expectedTenant);
              if (wrongTenants.length > 0) {
                result.leaksDetected += wrongTenants.length;
                result.passed = false;
                result.errors.push(`Concurrent ops: Neo4j leak for tenant ${expectedTenant}`);
              }
            }
          }
        });
        ...
        result.errors.push(`Concurrent tenant test error: ${(error as Error).message}`);
        result.passed = false;
      }

      result.duration = (Date.now() - startTime) / 1000;
      isolationTestDuration.observe({ test_type: 'concurrent_tenants', fault_type: 'high_concurrency' }, result.duration);
  ...
  private async generateReport(results: ChaosResult[]): Promise<void> {
    ...
    if (totalLeaks === 0) {
      console.log('🎉 ISOLATION MAINTAINED: No cross-tenant data leaks detected!');
    } else {
      console.log(`⚠️  ISOLATION VIOLATIONS: ${totalLeaks} cross-tenant leaks detected!`);
    }
  }
...
async function main() {
  ...
  try {
    ...
    if (allPassed && totalLeaks === 0) {
      console.log('🎯 All isolation chaos tests passed! Multi-tenant isolation is robust.');
      process.exit(0);
    } else {
      console.error('💥 Isolation chaos tests failed! Multi-tenant isolation has vulnerabilities.');
      process.exit(1);
    }
  } catch (error) {

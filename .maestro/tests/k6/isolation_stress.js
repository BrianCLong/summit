  ...
  group('Multi-Tenant Isolation Stress Test', function() {
    ...
    group('Concurrent Tenant Access', function() {
      ...
      if (!crossTenantBlocked) {
        ...
        console.error(`ISOLATION VIOLATION: VU${currentVU} iter${currentIter} - Tenant ${tenant.id} accessed ${otherTenant.id} data`);
      } else {
    ...
    if (Math.random() < 0.1) { // 10% of requests create memory pressure
      group('Memory Pressure Test', function() {
        ...
        const response = http.request(largeQuery.method, largeQuery.url, largeQuery.body, largeQuery.params);
          ...
          'memory pressure isolated': (r) => {
            ...
            if (data.data && data.data.signals) {
              ...
              if (hasWrongTenant) {
                isolationViolations.add(1);
                crossTenantLeaks.add(1);
                console.error(`MEMORY CORRUPTION: Tenant ${tenant.id} saw other tenant data under memory pressure`);
                return false;
              }
            }
            return true;

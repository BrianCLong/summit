...
def validate_installation():
    ...
    try:
        ...
        print("   ✅ DebiasingEngine imported successfully")
        validation_results["tests_passed"] += 1
    except ImportError as e:
    ...
    try:
        ...
        print("   ✅ MetacognitiveSystem imported successfully")
        validation_results["tests_passed"] += 1
    except ImportError as e:
    ...
    try:
        ...
        print("   ✅ CognitiveBiasMitigationSystem imported successfully")
        validation_results["tests_passed"] += 1
    except ImportError as e:
    ...
    try:
        ...
        print(
            f"   ✅ DebiasingEngine instantiated with {len(engine.debiasing_strategies)} strategies"
        )

        # Create metacognitive system
        meta_system = MetacognitiveSystem()
        print(
            f"   ✅ MetacognitiveSystem instantiated with awareness level {meta_system.awareness_level:.2f}"
        )

        # Create integrated system
        integrated_system = CognitiveBiasMitigationSystem()
        print("   ✅ CognitiveBiasMitigationSystem instantiated and integrated")
    ...
    try:
        decision_context = {
            "evidence": [{"supports": True}, {"supports": True}, {"supports": False}],
            "current_belief": True,
        ...
        reflection = meta_system.generate_reflection(
            decision_context, {"quality": "medium", "confidence": 0.7}
        )
        print("   ✅ Metacognitive reflection test completed")
    ...
    print("\n📋 Validation Summary:")
    print(
        f"   Components installed: {validation_results['components_installed']}/{validation_results['components_total']}"
    )
    print(
        f"   Tests passed: {validation_results['tests_passed']}/{validation_results['tests_total']}"
    )

    if (
        validation_results["components_installed"] == validation_results["components_total"]
        and validation_results["tests_passed"] == validation_results["tests_total"]
    ):
        print("\n🎉 INTELLIGRAPH COGNITIVE BIAS MITIGATION SYSTEM VALIDATION PASSED!")
        print("   All components are properly installed and functioning.")
        print("   System is ready for production use.")
        return True, validation_results
    else:
        print("\n❌ VALIDATION FAILED!")
        print("   Some components or tests failed validation.")

...
- `package optimization.authz`: binds loop IDs to allowed actions and scopes.
- `package optimization.budget`: enforces per-loop and global ceilings.
- `package optimization.fail_closed`: returns deny when preconditions are not met or signals are missing.

...
1.  **Control Definition (Code)**
    ...
    *   *Example:* "Production deployments must be signed by `release-captain`."

2.  **Signal Generation (Runtime)**
    *   As the system operates, it emits structured events for every significant action.
    *   *Example:* Deployment pipeline emits `artifact_signed` and `signature_verified` events.
...
If the auditor wants to see *one* specific deployment:
```bash

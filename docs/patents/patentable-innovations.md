...
- **Description**: Zero-trust federation for MCP domains using context exchange capsules that embed provenance, invariant attestations, trust-weight envelopes, and repair lineage, plus a verification gateway that enforces local policies before import.
- **Novelty**: Trust translation decouples upstream assertions from downstream use; provenance is linked but not merged; invariants are independently revalidated.
- **Component**: `docs/patents/provisionals/fmcp-tpce.md` (provisional draft)
- **Claim Focus**: Capsuleized exports, policy-negotiated imports, trust attenuation, and provenance re-anchoring without inheriting upstream trust.

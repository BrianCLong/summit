...
- **Novelty**: Applying differential privacy to the _weights/memory_ of an active agent in a multi-tenant environment.
...
- **Novelty**: Using graph topology of _sentiment_ specifically to detect bots/trolls, rather than just connection patterns.
- **Component**: `server/src/services/DefensivePsyOpsService.ts`
...
### 11. **Federated Model Context Protocols with Trust-Preserving Context Exchange (FMCP-TPCE)**

- **Description**: Zero-trust federation for MCP domains using context exchange capsules that embed provenance, invariant attestations, trust-weight envelopes, repair lineage, and policy manifests, plus a verification gateway that enforces policy-as-code rules before import.
- **Novelty**: Trust translation decouples upstream assertions from downstream use; provenance is linked but not merged; invariants are independently revalidated; import decisions are replayable via signed decision records.
- **Component**: `docs/patents/provisionals/fmcp-tpce.md` (provisional draft)
- **Claim Focus**: Capsuleized exports, policy-negotiated imports, trust attenuation, replayable decision records, and provenance re-anchoring without inheriting upstream trust.

...
Standard RAG (Retrieval Augmented Generation) retrieves text chunks based on vector similarity. This loses the _structural_ context (relationships, hierarchy, influence) between entities. An LLM might know "Alice worked with Bob" but miss that "Alice and Bob were both managed by Charlie," if that fact is in a distant document.

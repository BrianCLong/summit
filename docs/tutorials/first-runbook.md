...
1. Bring the stack online using the [Get Started guide](../get-started/get-started.md) (`make bootstrap && make up`).
2. Confirm services respond with the smoke target:
   ```bash
   make smoke
   ```
3. Query available runbooks from the GraphQL endpoint exposed at port 4000:
   ```bash

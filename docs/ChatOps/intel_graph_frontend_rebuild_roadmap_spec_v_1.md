...
**Purpose.** Give the web team everything needed to **rebuild from zero** and then **extend/expand/refine** the IntelGraph UI so that _every_ feature/function in the product backlog is usable, governed, and measurable. This is a living spec; PRs update it alongside implementation.
...
- **Evidence Integrity ↑**: provenance, confidence, and policy context are _first‑class_ in the UI.
...
**Deliverable**: This spec is considered _accepted_ when the Shell (Sections 1–3) and at least one complete flow per Capability (Section 4) meet Section 16’s acceptance criteria with telemetry and tests wired in.

    ...
    "models": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "temperature_bounds": {"type": "object", "properties": {"min": {"type": "number"}, "max": {"type": "number"}}},
...
export function decide(policy: any, req: { task: string; loa: number; risk?: string }): Decision {
  ...
  const rule = policy.rules.find((r: any) =>
  ...
  return { allow: true, model: chosen.name, reasons: Object.entries(denied).map(([model, reason]) => ({ model, reason })) };
}
```
...
export async function createJiraIssue({ baseUrl, email, apiToken, projectKey, summary, description }: any) {
...
r.post('/route/execute', async (req, res) => {
  ...
  try {
    ...
    routeExecuteLatency.observe({ model, stream: String(Boolean(req.body.stream)), status: 'ok' }, latencyMs / 1000);
    ...
    routeExecuteLatency.observe({ model, stream: String(Boolean(req.body.stream)), status: 'err' }, 0);
...
useEffect(() => { mermaid.run({ querySelector: '.mermaid' }); }, [/* activeTab or visibility */]);
```
...
```json
{
  "title": "Symphony Ops",
  "panels": [
    { "type": "timeseries", "title": "p95 route/execute", "targets": [{ "expr": "histogram_quantile(0.95, sum(rate(symphony_route_execute_latency_seconds_bucket[5m])) by (le))" }] },
    { "type": "timeseries", "title": "Budget fraction by model", "targets": [{ "expr": "symphony_model_budget_fraction_used" }] },
    { "type": "stat", "title": "Error rate 15m", "targets": [{ "expr": "sum(rate(nodejs_http_server_errors_total[15m])) / sum(rate(nodejs_http_requests_total[15m]))" }] }
  ]
}
```

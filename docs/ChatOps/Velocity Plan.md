...
2. **GraphRAG v1 (Explainable)**: subgraph retrieval (k‑hop \+ motif), packing → prompt, result with why\_paths \+ citations (entity/rel ids).
...
* **A1. GraphRAGService** (src/services/GraphRAGService.ts): retrieval (Cypher), pack facts, call LLM, return {answer, confidence, why\_paths, citations}.
...
* **B1. Embedding Upserter**: BullMQ queue on entity.created|updated; compute embeddings; upsert into entity\_embeddings; HNSW ready.
...
* **C1. Copilot Panel**: ask questions in natural language → calls GraphRAG; renders answer and why\_paths overlay on graph.
...
* **D2. Secrets posture**: SOPS for .env in CI/CD; no default dev passwords when REQUIRE\_REAL\_DBS=true.
...
| :---- | :---- | :---- | :---- |
| A1 | Implement GraphRAGService.ts \+ resolver \+ tests | feature/graphrag-v1 | Query returns answer, why\_paths (edge ids), citations (entity ids), \<800ms on 10k‑edge dataset for 1‑hop motifs; unit+integration tests pass |
...
**export type** GraphRAGResponse \= {  
  ...
  why\_paths: Array\<{ from: string; to: string; relId: string; type: string }\>; *// minimal proof edges*  
};
...
**export** **class** GraphRAGService {  
  ...
  **async** askLLM(question: string, facts: any) {  
    ...
    *// Return synthetic structure with why\_paths inferred from top‑central edges.*  
    ...
    **return** { answer: \`Hypothesis: ${question}\`, confidence: 0.72, citations: { entityIds: facts.entities.slice(0,5).map((e:any)**\=\>**e.id) }, why\_paths: why };  
  }
...
**const** manifest \= JSON.parse(fs.readFileSync(process.env.PQ\_MANIFEST || 'persisted-queries.json','utf8'));
...
**export** **const** startEmbeddingUpserter \= (connection: any) **\=\>** **new** Worker('entity-events', **async** (job: Job) **\=\>** {  
  ...
  **await** pool.query(  
    'INSERT INTO entity\_embeddings (entity\_id, embedding, model) VALUES ($1, $2::vector, $3)\\n' \+  
...
**const** hist \= **new** client.Histogram({ name: 'graphql\_resolver\_ms', help: 'Resolver latency', labelNames: \['type','field'\]});
...
**export** **default** **function** AIInsightsPanel({ cy, investigationId }){  
  ...
  **const** \[run, { data, loading }\] \= useQuery(Q, { variables: { investigationId, q }, skip: **true** });
  ...
  React.useEffect(() **\=\>** {  
    ...
    **const** ids \= data.graphRagAnswer.why\_paths.map(w **\=\>** w.relId);  
    cy.batch(() **\=\>** {  
      cy.elements('edge').removeClass('why');  
      ids.forEach(id **\=\>** cy.$(\`edge\[id \= "${id}"\]\`).addClass('why'));  
    });  
    $(cy.container()).trigger('intelgraph:why\_paths\_applied', \[ids\]);  
  }, \[cy, data\]);
...
* **Secrets**: SOPS‑encrypted envs; REQUIRE\_REAL\_DBS=true in prod to block dev defaults.

...
- **Analyst:** Needs a case-first UI to view investigations, review evidence citations (`EVIDENCE_ID`), and understand the _why_ behind a score.
...
- **Manager:** Needs the Provable Actionability Index (PAI) dashboard to prove the ROI of the platform, showing how many alerts led to actions _with_ court-grade evidence.
...
- **Case-First & Diff-Native:** The default view is not a list of raw alerts, but a "Case" showing what has _changed_ since the last run (the diff) and the recommended actions.

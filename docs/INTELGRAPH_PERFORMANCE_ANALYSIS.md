...
- **Complexity**:
  ...
  - Louvain: $O(V \log V)$.
- **Hot Spot**: Graph Projection (`CALL gds.graph.project`). Currently done _per query_ implicitly or explicitly.
...
- **Change**: Project the graph _once_ per tenant/investigation into a named graph `(tenantId-investigationId)`.

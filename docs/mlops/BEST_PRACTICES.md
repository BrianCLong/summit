...
const config: TrainingConfig = {
  ...
  modelName: "my-model",
  // ... other config
  experimentTags: ["production", "experiment-name", `user-${username}`, `date-${date}`],
  metadata: {
...
async function rollbackDeployment(deploymentId: string) {
  // 1. Stop new traffic
  await server.updateTrafficSplit([{ deploymentId, percentage: 0 }]);
  ...
  if (deployment.status === "healthy") {
    // 4. Restore traffic
    await server.updateTrafficSplit([{ deploymentId, percentage: 100 }]);
  }
}
```

...
- **`policy/allowlist.yaml`**: Contains security exceptions (IDs, owners, tickets, expiration).
  - _Structure_: List of objects with `id`, `owner`, `ticket`, `expires`.
  - _Usage_: Checked by `.security/security-gates-checker.sh` and various scripts.
- **`policy/governance-config.yaml`**: Defines environment enforcement modes and policy bundles.
  - _Structure_: `environments` (dev/staging/prod settings) and `policies` (criticality, paths).
  - _Usage_: Loaded by `server/src/services/PolicyEngine.ts`.
- **`server/src/config/schema.ts`**: Zod schema for server runtime configuration (e.g., rate limits, feature flags).
  - _Usage_: Validates `process.env` and config files at startup.

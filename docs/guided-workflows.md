...
- **steps**:
  ...
  - `validations`: rule-based checks (required, regex, enum, min/max, uri/email)
...
2. For each step: collect inputs, validate via JSON Schema + declarative rules, evaluate stop conditions, and dispatch tools with retries/fallbacks. Tool outputs are stored in the run context for later steps and `outputMapping` can project specific fields into the workflow context.

...
exports.GovernedResearchAgent = void 0;
const governed_agent_base_1 = require("../governed-agent-base");
// ============================================================================
// GOVERNED RESEARCH AGENT
// ============================================================================
class GovernedResearchAgent extends governed_agent_base_1.SummitGovernedAgent {
    constructor() {
        super({
    ...
    async research(task) {
        ...
        if (should_proceed.action === 'STRATEGIC_SILENCE') {
            throw new StrategicSilenceException(`Agent chose strategic silence: ${should_proceed.message}`, should_proceed.silence_decision);
        }
        ...
        if (validated_sources.length < (task.min_sources || 2)) {
            throw new InsufficientValidSourcesError(`Insufficient validated sources. Required: ${task.min_sources || 2}, Valid: ${validated_sources.length}`);
        }
        ...
        const narrative_health = await this.checkNarrativeHealth(preliminary_answer, alternatives, validated_sources);
     ...
     */
    async gatherResearchSources(task) {
        const sources = [];
     ...
     */
    async validateSources(sources, task) {
        const validated = [];
     ...
     */
    async synthesizeAnswer(question, sources) {
        const source_contents = sources
            .map((s, i) => `[Source ${i + 1}]: ${s.content}`)
            .join('\n\n');
        const response = await this.llm.generate({
            prompt: `Based on the following sources, answer this question: "${question}"\n\n` +
                `Sources:\n${source_contents}\n\n` +
     ...
     */
    async generateAlternativeAnswers(question, sources) {
        const source_contents = sources
            .map((s, i) => `[Source ${i + 1}]: ${s.content}`)
            .join('\n\n');
        const response = await this.llm.generate({
     ...
     */
    async checkNarrativeHealth(primary_answer, alternatives, sources) {
        ...
        if (premature_convergence) {
            warnings.push(`Low narrative diversity (${diversity_index.toFixed(2)} < 1.0). ` +
                `Consider exploring additional framings.`);
        }
        // Check for high unexplained ratio
        if (unexplained_ratio > 0.30) {
            warnings.push(`High unexplained elements ratio (${(unexplained_ratio * 100).toFixed(0)}%). ` +
     ...
     */
    calculateNarrativeDiversity(answers) {
     ...
     */
    calculateUnexplainedRatio(answer, sources) {
     ...
     */
    identifyUnexplainedElements(answer, sources) {
     ...
     */
    generateCaveats(sources, narrative_health) {
        const caveats = [];
        // Low integrity sources
        const low_integrity = sources.filter(s => s.summit_metadata.truth_ops.integrity_score < 0.70);
        if (low_integrity.length > 0) {
            caveats.push(`${low_integrity.length}/${sources.length} sources have medium/low integrity. ` +
        ...
        if (source_types.size < 2) {
            caveats.push(`All sources are of type "${[...source_types][0]}". ` +
...
exports.GovernedResearchAgent = GovernedResearchAgent;
// ============================================================================
// EXCEPTIONS
// ============================================================================
class StrategicSilenceException extends Error {
    silence_decision;
    constructor(message, silence_decision) {
        super(message);
        this.silence_decision = silence_decision;

...
- **The Defense:**
  ...
  2.  **Auditability:** A 10M token answer is a "black box" blend. Our logic (IntentSpec) allows you to see _exactly_ why specific nodes were selected.
...
- **The Defense:**
  1.  **Specificity:** Generic tools optimize for "recall" (finding everything). Summit optimizes for "Evidence" (finding the _proof_).
  2.  **Governance:** Competitors lack our `EvidenceBudget` and `IntentSpec` enforcement. They provide the tool, not the _control framework_.
...
- **The Defense:**
  1.  **We are ready:** Our `IntentSpec` _is_ the explanation of the reasoning logic.
...
- **Response:** Pivot Summit to become the _Memory Layer_ (the writable ledger) rather than the retrieval logic layer. The `EvidenceLedger` remains valuable even if retrieval is trivial.

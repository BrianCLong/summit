...
- **Stateful Session Tracking:** Collection must record the full session state (headers, cookies, viewports) not just the payload, to prove _how_ data was accessed.
...
- **Claim-Centric Assertions:** Assertions must be decoupled from sources; a source provides raw data, but the _claim_ is a distinct entity verified independently.
...
- **Contradiction-First Pipeline:** The pipeline must prioritize surfacing conflicts _before_ any synthesis or summarization occurs.

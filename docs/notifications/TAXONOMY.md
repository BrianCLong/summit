...
Severity determines the _speed_ and _intrusiveness_ of the notification.
...
2.  **Role-Based**:
    - _Security Events_ -> `Security Admin` role.
    - _Billing Events_ -> `Finance Admin` role.
3.  **Team-Based**:
    - _Service Alerts_ -> Team owning the `serviceId` tag.
4.  **Escalation**:

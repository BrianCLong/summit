...
| CI Action Pinning Gate           | `scripts/ci/action_pinning_gate.mjs` + `docs/ga/CI_ACTION_PINNING.md`                                                       | C    | `node scripts/ci/action_pinning_gate.mjs` | Enforces pinned third-party actions and emits the CI action inventory SBOM.                                   |

...
**Version:** 1.0
**Date:** 2025-12-31
**Status:** Governed Exceptions Active
**Verification Status:** ✅ Checks passing with governed exceptions declared

## Governed Exceptions (Security Ledger Alignment)

The current security posture is **intentionally constrained** pending remediation of the
GA-blocking items recorded in the Security Issue Ledger. These exceptions are governed and
tracked in the authoritative ledger until closure.

**Authoritative Ledger:** `docs/security/SECURITY-ISSUE-LEDGER.md`

**Declared Exceptions (current):**

- AUTH-CRIT-001 (JWT signature verification gaps)
- AUTH-CRIT-002 (development auth bypass)
- AUTHZ-CRIT-003 (client-controlled tenant ID)
- INJ-CRIT-004 (shell=True command execution)
- INJ-CRIT-005 (pickle deserialization)
...
**Key Achievements (intentionally constrained):**

- ✅ 100% security baseline verification coverage
- ✅ Authentication enforced on all non-public endpoints
- ✅ Tenant isolation guardrails enforced where JWT tenant context is authoritative
...
**Tenant Context Sources (governed exception in effect):**

1. `req.user.tenantId` from JWT (authoritative)
2. `x-tenant-id` header (legacy path under governed exception; scheduled for removal)

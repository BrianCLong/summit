...
**Objective:** Produce one deterministic daily GA status file from repeatable, same-day evidence.
...
5. Mark any missing same-day artifact as a blocker with owner + SLA.
6. Publish `docs/ops/GA_DAILY_STATUS_<date>.md` using the required output schema.

### Freshness Gate (Mandatory)

- A status file is **attested** only when every CI/security/checklist input has same-day evidence.
- If freshness is not satisfied, classify status as `freshness-constrained` and open owner-routed actions.
...
- `meta.freshness`: attested | freshness-constrained.
...
- **Escalation trigger:** any GA-blocking security gap, release-checklist drift, or freshness-gate failure.

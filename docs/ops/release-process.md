...
> **Status**: Superseded by `docs/ops/RELEASING.md` (canonical). This document is retained for historical context; current releases follow the locked SemVer/tagging workflow documented in the new playbook and implemented by `release-train.yml`, `release-integrity.yml`, and `release-ga.yml`.

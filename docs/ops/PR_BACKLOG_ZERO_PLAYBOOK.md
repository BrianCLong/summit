...
Export a live PR snapshot (paginated GraphQL, planner-compatible shape):
...
Use the planner script to classify the snapshot and produce next merge batches.
...
Sync live queue labels from the same policy (dry-run default):
...
2. Run planner and sync queue labels according to output.

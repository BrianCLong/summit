...
4. **Solution Fit** — Demo/workshop delivered; architecture fit vetted (data, identity, deployment); security/compliance questions captured.
5. **Pilot/POC** — Pilot SOW agreed in principle; success metrics, duration, and pricing documented; project plan + owners assigned.
6. **Evaluation/Negotiation** — Commercials redlined; security/compliance package submitted; exec sponsor identified; procurement path confirmed.
7. **Commit/Close** — Final approvals obtained; order form/contract in flight; implementation kickoff scheduled.
...
```
Owner: <name> | Week of: <date>

Total pipeline: $<sum> | New this week: $<sum> | Slipped: $<sum>

By stage
- Targeting: <count> | Engaged: <count> | Discovery: <count> | Solution Fit: <count>
- Pilot/POC: <count> | Evaluation/Negotiation: <count> | Commit/Close: <count>

Top 5 by value
1) <account> — <$> — <stage> — next step <date>
2) ...

At-risk dashboard
- Deals with no next step in 7 days: <count/list>
- No exec sponsor identified: <count/list>
- No champion/implication of pain recorded: <count/list>
- Security/compliance unanswered >10 days: <count/list>
- Pilot overrunning schedule/budget: <count/list>

Wins
- <brief bullet per win>

Blocks / help needed
- <what you need from leadership/peers>
```
...
- Use **risk flags** early (no champion, budget unclear, security stall, procurement uncertainty).
- Run **weekly pipeline scrub** to demote stale deals and refresh probabilities.

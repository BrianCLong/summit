...
- **Scoring Operations Library** (`lib/scoring.mjs`) - Computes derived fields (WSJF, True Priority, GA Readiness, etc.)
...
- **Event Processing** (`bin/apply-event.mjs`) - Updates fields from GitHub issues/PRs
- **Workflow Integration** (`bin/apply-workflow-run.mjs`) - Processes CI artifacts and signals
- **Nightly Reconciliation** (`bin/reconcile-nightly.mjs`) - Fixes drift and recomputes derived fields
- **Board Snapshot Generator** (`bin/generate-board-snapshot.mjs`) - Creates executive reports

#### C. GitHub Workflows
- `project19-fields-ensure.yml` - Periodic field verification and provisioning
- `project19-event-sync.yml` - Real-time event-driven updates
- `project19-ci-signals.yml` - Workflow artifact integration
- `project19-nightly-reconcile.yml` - Nightly drift repair and computation
- `project19-generate-snapshot.yml` - Weekly executive summary generation

#### D. Configuration Files
- `project19-field-schema.json` - Canonical field definitions with expanded governance fields
- `project19-label-map.json` - Label/milestone → field mapping
- `project19-workflow-map.json` - CI artifact → field update mapping  
- `project19-score-policy.json` - WSJF, True Priority, GA Readiness computation formulas
- `project19-views.json` - Executive and audit dashboard definitions

#### E. Documentation
- `README.md` - System architecture and operation guide
- `OPERATING_PROCEDURES.md` - Day-to-day operations and troubleshooting
- `CI_INTEGRATION_GUIDE.md` - CI/CD integration mapping
- `SYSTEM_IMPLEMENTATION_COMPLETE.md` - Complete system validation

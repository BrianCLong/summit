...
1.  **Low Risk (Autonomy):** Agent confidence > 0.9 AND Cost < $0.10.
    - _Action:_ Execute immediately. Log for post-hoc audit.
2.  **Medium Risk (Notification):** Agent confidence > 0.7 OR Cost < $1.00.
    - _Action:_ Execute immediately, but send async notification ("I did X").
    - _Control:_ Operator can `PAUSE` or `ROLLBACK` within N minutes.
3.  **High Risk (Escalation):** Agent confidence < 0.7 OR Cost > $1.00 OR Sensitive Tool (e.g., `deploy:prod`).
    - _Action:_ `PAUSE` execution. Create `EscalationTicket`.
    - _Control:_ Requires explicit `RESUME` signal.

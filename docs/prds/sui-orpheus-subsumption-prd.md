...
- Classification target: `minor` once implementation PR-1 starts (docs PR remains `patch`).
- Required reviewers: architecture, security, product, and governance owner.
- Mandatory blockers: failing UDR-AC, failed policy lint, missing agent metadata.

## Post-merge validation plan

1. Validate roadmap initiative telemetry for follow-on PR linkage.
2. Confirm CI evidence jobs emit artifacts into expected paths.
3. Conduct pilot-readiness review with underwriting and security stakeholders.

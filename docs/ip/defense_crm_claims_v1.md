# Patent Claims: Defense CRM

## Independent Claims

C1. A non-transitory computer-readable medium storing instructions that, when executed by a processor, cause the processor to perform a method for governing an artificial intelligence system, the method comprising:
- ingesting content into a narrative state;
- evaluating the narrative state against governance policies to determine a policy decision; and
- executing a defense action based on the policy decision.

...
| **BIPA (Illinois)** | Applies specifically to `FaceDetectionEngine` outputs (face geometry). | - Written release required *before* collection<br>- Retention schedule required<br>- Prohibition on profiting from data |
...
2.  **SSN/PCI Redaction:** Verify that the redaction logic in `server/src/ai/copilot/redaction.service.ts` is applied *before* logging or storage in the `ProvenanceLedger`.

...
> _"The `IntelGraphService` manually writes to Neo4j and then Postgres. If one fails, data is corrupted."_
...
- **Spine:** Implemented via `ProvenanceLedgerService` (Postgres/Timescale), but not strictly enforced as the _only_ write path.
...
1.  **Enforce Ledger**: Refactor `GlobalIngestor.js` to write _only_ to `ProvenanceLedgerService`.

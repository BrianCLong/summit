...
**Should**
7) **Branding kit** (logo, primary/neutral token set) per tenant.
8) **Alert Routing UI** (escalations/schedules) integrated with Notifications Center.

**Could**
9) **Quick Actions customization** (show/hide/reorder) per user.
10) **Ticket auto‑link suggestions** from run metadata (RO hints only).

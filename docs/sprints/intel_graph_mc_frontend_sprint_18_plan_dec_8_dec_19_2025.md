...
**Should**
7) **Template Import/Export** (JSON) with signature field (RO verify).
8) **Drafts versioning** with diff viewer and rollback.

**Could**
9) **YAML import** → JSON schema transform (beta).
10) **Editor command palette** (search steps/params) and keyboard shortcuts.

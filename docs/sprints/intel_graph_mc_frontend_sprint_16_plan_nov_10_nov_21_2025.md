...
**Should**
8) **Entity Merge Review Queue (RO)**: list suggested merges with diffs; approve path stub only.
9) **Saved & Shareable Graph Views** (persist/restore layout & filters); export PNG/SVG/JSON.

**Could**
10) **Multi‑Region Status widget** (RO): region, replica lag hints in header.
11) **Guided tutorial** for Explorer (coach marks; reduced motion respected).

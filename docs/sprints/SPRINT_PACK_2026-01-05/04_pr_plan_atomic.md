...
All PRs must include the AGENT-METADATA block from `.github/PULL_REQUEST_TEMPLATE.md` and pass `scripts/ci/verify-prompt-integrity.ts` plus `scripts/ci/validate-pr-metadata.ts`.
...
- **Tests/verification:**
  - `pnpm --filter intelgraph-server test -- --runTestsByPath <new audit tests>`
...
- **Tests/verification:**
  - `pnpm --filter intelgraph-server test -- --runTestsByPath <paths above>`
- **Risk level:** Low.

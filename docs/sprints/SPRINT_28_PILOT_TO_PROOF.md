...
$(function () {
  ...
  $("#nps-submit").on("click", function () {
    ...
    $.ajax({
      ...
      data: JSON.stringify({
        query: "mutation($s:Int!,$c:String){submitNps(score:$s,comment:$c)}",
...
export async function mapCsvToGraph(csvText, mapping) {
  ...
  rows.forEach(function (r, i) {
    ...
    mapping.propMap.forEach(function (m) {
...
$(function () {
  $.ajax({
    ...
    contentType: "application/json",
    data: JSON.stringify({
      query: "query($w:ID!){pilotSuccess(workspaceId:$w){label,value,status,hint}}",
...
function enforceCap(ctx, metric, value) {
  ...
  if (cap && value > cap.hard)
    throw new Error(`Plan cap reached for ${metric}. Contact support to increase.`);
  if (cap && value > cap.soft)
    return { warning: true, message: `Approaching ${metric} cap (${value}/${cap.hard}).` };

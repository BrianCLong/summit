...
| **Fail-closed Output.** Prevents displaying leaked internal tokens or broken model states. | **False Positives.** Simple regex matching might block legitimate discussions *about* safety (e.g., discussing "ignore previous instructions"). |
...
*   This does **not** verify the *truthfulness* of the output (Hallucination), only its compliance with format/safety policy. Artifact C (Fact Verification) is deferred.

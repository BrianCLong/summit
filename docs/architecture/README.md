...
1. Edit `system-index.json` and `system-index.md` together so the human and machine views stay
   aligned.
2. Extend `summit-graph.schema.json` only when a new node or edge type is required.
3. Record the change in `docs/roadmap/STATUS.json` with a short revision note.

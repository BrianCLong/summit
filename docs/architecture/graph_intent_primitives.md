...
**Definition:** A mechanism to strictly limit the graph traversal depth, node count, and path complexity _before_ execution, based on the query's estimated "reasoning cost".
...
**Definition:** Optimizing the _shape_ of the retrieved graph context (e.g., serialization format, sorting) to maximize the stability of the LLM's downstream reasoning.

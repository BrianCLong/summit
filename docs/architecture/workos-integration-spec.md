...
| **Audit Logging**        | Custom [Provenance Ledger](./prov-ledger.md) and Kafka event streams. | Built-in Audit Logs with SIEM streaming (Splunk, Datadog).        | Augments our Provenance Ledger. WorkOS handles the _identity_ audit events, while Summit handles the _intelligence/graph_ provenance. |

...
- **Decoupling:** Crucially, SpiderFoot is treated _strictly_ as an untrusted collector. Its outputs are immediately routed through Summit's `core/evidence/ids.ts` to assign deterministic `EVIDENCE_ID`s and wrap them in a Summit Evidence Bundle.

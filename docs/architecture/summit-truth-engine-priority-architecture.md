...
## Implemented Reference Artifacts (Phase 0)

The repository now includes executable reference artifacts to compress implementation lead time:

- WriteSet JSON Schema: `schemas/truth-engine/writeset.schema.json`
- Validation firewall: `scripts/truth-engine/validate-writeset.mjs`
- Deterministic tri-graph materializer: `scripts/truth-engine/materialize-trigraph.mjs`
- Golden fixtures + tests: `scripts/truth-engine/fixtures/*`, `scripts/truth-engine/__tests__/truth-engine.test.mjs`

These artifacts enforce schema correctness, evidence integrity, promotion quarantine behavior, and deterministic replay hashing. The Phase 0 validator is runtime-native and schema-backed; Phase 1 hardens this path with AJV gate integration in CI and services.

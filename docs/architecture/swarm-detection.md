...
Traditional detection relies on content analysis (truthfulness, toxicity), which LLMs can easily bypass. Summit shifts the detection paradigm to **Coordination Physics**—analyzing the _energy_, _entropy_, and _topology_ of information flow.

**Premise:** Coordinated behavior, even when obfuscated, leaves statistical artifacts that differ from organic human behavior. We detect the _mechanism of coordination_, not the content.

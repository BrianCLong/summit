...
"Trust infrastructure" wraps an agent with identity, policy, monitoring, and audit so _every_ high-risk action is: **authorized → simulated → policy-checked → signed → executed → observed → audited**, with kill switches at multiple points.
...
- **Policy Enforcement Point (PEP)** lives in the **data plane** _directly in front of execution_.
- **Circuit breakers** exist in _multiple layers_, but the **most important breaker** is:
...
- Runs _before signing_ (and sometimes _before simulation_).

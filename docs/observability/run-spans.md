...
> PII-bearing tags are blocked by default. Set `OBS_TAG_PII_ALLOWED=true` only if a tag is explicitly
> registered with `pii: true` and approved for a governed exception.
...
- `OBS_TAG_PII_ALLOWED` – permit tags marked `pii: true` (defaults to false)

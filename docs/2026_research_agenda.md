...
### 5) Digital Employee Infrastructure (Phase 0–2, concurrent)

**Value**: Treat multi-agent systems as digital employees that are scored by autonomous task completion, not model output volume.

**Architecture hooks**

- **Task/SLA telemetry**: Extend agent run records with `autonomyRate`, `interventionCount`, `slaTarget`, and `slaStatus` to make "workflow success with X autonomy, Y interventions" the primary KPI.
- **Policy-aware action layer**: Gate every action through policy-as-code checks with scoped permissions and evidence capture at execution time.
- **Secure connectors**: First-class connectors for CRMs, repos, ticketing, and data stores with permissioned scopes, least-privilege defaults, and audit trails.
- **Observability**: Grafana panels for autonomy %, SLA attainment, connector risk, and governed exception rates.

**Success gates**: ≥90% workflows report autonomy + SLA outcomes; zero unauthorized connector actions; governed exceptions documented for every non-standard permission grant.

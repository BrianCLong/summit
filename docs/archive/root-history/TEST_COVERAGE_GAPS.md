...
describe("createInvestigation", () => {
  it("should reject duplicate investigation names", async () => {
    ...
    // Act & Assert
    await expect(createInvestigation({ name })).rejects.toThrow(
      'Investigation with name "Test Investigation" already exists'
    );
  });
  ...
  it("should reject investigation names exceeding 255 characters", async () => {
    ...
    await expect(createInvestigation({ name: longName })).rejects.toThrow(
      "Investigation name must be 255 characters or less"
    );
  });
  ...
  it("should handle database connection failures gracefully", async () => {
    ...
    await expect(createInvestigation({ name: "Test" })).rejects.toThrow(
      "Unable to create investigation. Please try again."
    );
  });
});
...
describe("createEntity", () => {
  it("should reject invalid entity types", async () => {
    await expect(createEntity({ type: "InvalidType", name: "Test" })).rejects.toThrow(
      "Invalid entity type. Must be one of: Person, Organization, Location, Event"
    );
  });
...
describe("askCopilot", () => {
  it("should timeout after 30 seconds", async () => {
    jest
      .spyOn(copilotService, "ask")
      .mockImplementation(() => new Promise((resolve) => setTimeout(resolve, 31000)));

    await expect(askCopilot({ investigationId: "inv123", question: "Test?" })).rejects.toThrow(
      "Copilot request timed out"
    );
  }, 32000);

  it("should handle LLM API rate limits gracefully", async () => {
    jest.spyOn(copilotService, "ask").mockRejectedValue(new Error("Rate limit exceeded"));

    await expect(askCopilot({ investigationId: "inv123", question: "Test?" })).rejects.toThrow(
      "Copilot is currently unavailable. Please try again in a few minutes."
    );
  });
});
```
...
describe("JWT Authentication", () => {
  ...
  it("should enforce token rotation on refresh", async () => {
    ...
    await expect(refreshAccessToken(refreshToken)).rejects.toThrow("Refresh token already used");
  });
});
...
describe("RBAC Authorization", () => {
  it("should deny access when user lacks required role", async () => {
    ...
    await expect(authorizeAction(user, "entity:delete")).rejects.toThrow(
  ...
  it("should deny access to investigations from different tenant", async () => {
    ...
    await expect(authorizeResourceAccess(user, investigation, "read")).rejects.toThrow(
  ...
  it("should allow admin to access all tenants", async () => {
    ...
    await expect(authorizeResourceAccess(admin, investigation, "read")).resolves.toBe(true);
  });

  it("should deny access to deleted user accounts", async () => {
    const deletedUser = { id: "user123", status: "deleted" };

    await expect(authenticateRequest({ user: deletedUser })).rejects.toThrow(
  ...
  it("should enforce IP whitelist for sensitive operations", async () => {
    ...
    await expect(authorizeAction(user, "investigation:export", { request })).rejects.toThrow(
      "IP address not whitelisted for sensitive operations"
    );
  });
});
```
...
describe("EntityService - Constraint Handling", () => {
  it("should prevent duplicate entities with same external ID", async () => {
    ...
    await expect(entityService.create({ externalId: "ext-123", name: "Entity 2" })).rejects.toThrow(
      'Entity with external ID "ext-123" already exists'
    );
  });
  ...
  it("should cascade delete relationships when entity is deleted", async () => {
    ...
    await expect(relationshipService.findById(relationship.id)).resolves.toBeNull();
  });
  ...
  it("should rollback transaction on bulk import failure", async () => {
    ...
    await expect(entityService.bulkCreate(entities)).rejects.toThrow();
  ...
  it("should enforce maximum entity name length", async () => {
    ...
    await expect(entityService.create({ name: longName })).rejects.toThrow(
      "Entity name must be 1000 characters or less"
    );
  });

  it("should reject null or undefined required fields", async () => {
    await expect(entityService.create({ name: null })).rejects.toThrow("Entity name is required");

    await expect(entityService.create({ name: undefined })).rejects.toThrow(
  ...
  it("should handle database write failures gracefully", async () => {
    ...
    await expect(entityService.create({ name: "Test" })).rejects.toThrow(
      "Unable to create entity. Database unavailable."
    );
  });
});
```

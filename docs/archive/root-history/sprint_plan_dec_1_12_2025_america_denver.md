...
1. **Ingest Wizard v0 — CSV→Entities in ≤10 min** — _8 pts_
...
2. **Data License Registry — Export Blockers** — _5 pts_
...
3. **Provenance & Claim Ledger — Verifiable Manifest** — _8 pts_
...
4. **NL Graph Querying — Cypher Preview + Sandbox** — _8 pts_
...
5. **GraphRAG — Evidence-First Citations Gate** — _5 pts_
...
6. **Brief/Report Studio — Manifest Validation on Export** — _5 pts_
...
**Stretch (time-boxed):** Hook a minimal Proof-Carrying Analytics (_.pcq_) manifest from the deterministic DAG runner into #3 (export pipeline).
...
- Unit/contract tests + E2E: _ingest → resolve → runbook → report_ with screenshot diffs; Cypher tests on ephemeral DB.
...
- Sets up next sprints: Proof-Carrying Analytics (_.pcq_), Authority Compiler, and selective disclosure wallets.

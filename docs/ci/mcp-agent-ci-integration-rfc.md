...
- **CI Mode:** CI runner uses `interaction_log.json` to verify the agent's logic _without_ calling the LLM again, OR calls the LLM with `temperature=0` and strict schema constraints.
...
- **Secrets:** Agents _never_ receive raw secrets. They access services via MCP Servers which handle authentication internally.

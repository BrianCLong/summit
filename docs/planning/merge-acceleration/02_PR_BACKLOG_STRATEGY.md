...
- **Action**: **Auto-Close** with a polite message: _"Closing due to inactivity. Please reopen if this is still relevant and conflicts are resolved."_
...
const prs = JSON.parse(
  execSync(
    "gh pr list --state open --limit 500 --json number,title,author,updatedAt,labels,mergeable"
  ).toString()
);
...
const report = prs.map((pr) => {
  ...
  else if (pr.labels.some((l) => l.name.includes("docs") || l.name.includes("chore")))
    bucket = "B (Quick)";

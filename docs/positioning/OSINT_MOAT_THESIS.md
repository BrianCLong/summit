...
In legal and regulatory environments, the value of intelligence is binary: it is either admissible or it is noise. Summit achieves admissibility through **Stateful Provenance**. Unlike traditional scraping which captures only the _what_ (content), Summit captures the _how_ (session state, friction, timing).

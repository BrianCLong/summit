...
Use the executable inventory generator to produce OUTPUT A/B/C in one step:

```bash
scripts/ga/merge_orchestrator_inventory.py --output docs/roadmap/GA_MERGE_ORCHESTRATOR_LIVE_REPORT.md
```

- If prerequisites are missing, the script emits a deterministic blocked report with stabilize guidance.
- If prerequisites pass, the script emits live PR inventory and top-15 merge train ordering.

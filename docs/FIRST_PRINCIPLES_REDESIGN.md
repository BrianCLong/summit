...
1.  **The Spine (Event Log)**:
    ...
    - **Constraint**: No service writes directly to Neo4j or Postgres tables. They write _Events_ to the Spine.
...
3.  **The View (Projections)**:
    ...
    - **Role**: pure _Read Models_. They subscribe to the Spine and update themselves. They can be blown away and rebuilt from the Log at any time.
...
- **Action**: Introduce `ProvenanceLedger` as the _primary_ write path.
- **Step**: Modify `IntelGraphService` to write to the Ledger _first_, and have a background worker update Neo4j/Postgres.

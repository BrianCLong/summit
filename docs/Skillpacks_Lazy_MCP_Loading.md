...
Trigger matching is glob-aware, so `ui-*` or `audit-*` can route to shard-ready bundles without
loading schemas prematurely.
...
- Keep delito the 23rd order of imputed intention as a governing constraint on permissions.

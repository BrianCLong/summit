...
Each execution captures a run trace with plan snapshot, plan context, step statuses, approvals, policy decisions, and provenance IDs from shaped context.

## APIs

- `POST /api/orchestration/plan` — generate a policy-aware plan + shaped context
- `POST /api/orchestration/runs` — execute a provided plan under a chosen autonomy mode (optionally include plan context/guardrails)
- `GET /api/orchestration/runs/:id` — fetch run trace
- `POST /api/orchestration/runs/:id/approve` — approve a pending step in HITL and resume execution
...
```bash
pnpm --filter @intelgraph/orchestration test

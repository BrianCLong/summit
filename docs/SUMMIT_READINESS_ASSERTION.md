...
- **Enforced Gates:**
  ...
  - `node scripts/ci/verify_security_ledger.mjs --mode=hard`
  - `node scripts/ci/check_logging_safety.mjs`
  - `node scripts/ci/check_extensions_compliance.mjs`
  - `node scripts/ci/check_integrity_budgets.mjs`
  - `node scripts/ops/generate_trust_dashboard.mjs --mode=hard`

**This release is verifiable, bounded, and complete as of this commit.**

...
### 6.2 Canary + Rollback Template

```yaml
# .github/workflows/reusable/canary-rollback.yml
name: Reusable canary + rollback
  ...
  workflow_call:
    inputs:
      ...
      canary-script:
        required: false
        type: string
        default: "scripts/canary-check.cjs"
      rollback-script:
        required: false
        type: string
        default: "scripts/rollback.sh"
      rollback-enabled:
        required: false
        type: boolean
        default: true
...
- `service-name`: The service identifier passed to rollback tooling.

**Optional parameters**:

- `canary-script`: Override the canary validation script path.
- `rollback-script`: Override the rollback script path.
- `rollback-enabled`: Disable rollback in read-only canary runs.
- `node-version`: Node runtime for canary validation.

...
- **`createUser`**:
  - _Success:_ Should hash password, create user in Repo, emit `UserCreated` event.
  - _Error:_ Should throw if email already exists (mocking Repo to return "exists").
  - _Error:_ Should throw if password is too weak.
- **`getUser`**:
  - _Success:_ Returns user domain object.
  - _Error:_ Throws `NotFound` if ID invalid.
...
- **`parseEvent`**:
  - _Success:_ Transforms raw JSON `{ "amt": 100, "u_id": "u1" }` to Domain Model `Transaction { amount: 100, userId: "u1" }`.
  - _Error:_ Throws `InvalidSchema` if required fields missing.
  - _Edge Case:_ Handles malformed JSON strings gracefully.
...
- **Idempotency:**
  - Enqueues the _same_ message ID twice.

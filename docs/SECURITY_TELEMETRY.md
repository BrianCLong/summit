...
1. **User Deanonymization:** Correlating request timestamps/sizes to specific user actions.
   - _Mitigation:_ Metrics are aggregated. Logs use correlation IDs, not raw user PII in message bodies.
2. **Data Leakage in Labels:** High-cardinality labels (e.g., query text, user email) in Prometheus.
   - _Mitigation:_ Strict code review of `labelNames`. `route` label must be parameterized (e.g., `/users/:id` not `/users/123`).
3. **Sensitive Data in Logs:** Passwords, tokens, or PII in error traces.
   - _Mitigation:_ `pino` redact configuration for `password`, `token`, `authorization`, `cookie`.

...
- **Prompt integrity + task contracts** — Every agent task references an immutable prompt hash and a task contract schema to satisfy agent-native governance. Mapped to `prompts/registry.yaml` and `agents/task-spec.schema.json`.
- **Governed Exceptions** — Legacy bypasses are explicitly recorded, policy-gated, and time-boxed with owners. Mapped to policy annotations and provenance records.
...
- **Prompt compliance**: Require each agent definition to reference prompt hashes in `prompts/registry.yaml`, with task contracts stored alongside the spec for verification and replay.
- **Evidence lifecycle**: Attach run artifacts (trace snapshots, receipts, redaction reports) to the provenance ledger as release evidence for GA readiness.
...
11. **Failure taxonomy + alerting**  
    _Acceptance_: Standard codes for policy*denied, redaction_block, budget_exceeded, interop_error; alerts wired to SLOs.  
    \_Touches*: `docs/agentic/security.md`, observability config.  
    _Risk_: Alert noise.
...
- Align all agentic artifacts with the Summit Readiness Assertion and the Constitution of the Ecosystem, and cite those authority files in design reviews.
- Express regulatory constraints as policy-as-code only; any exception is recorded as a Governed Exception with owner, scope, TTL, and provenance.
- Capture prompt hashes, task contracts, and policy bundle versions in every run record to keep audits deterministic.

...
- `governance` (object): Prompt registry references, task contracts, evidence storage.
...
- `prompt_ref`: Required prompt hash reference (from `prompts/registry.yaml`).
- `task_spec_ref`: Task contract reference for runtime validation.
...
- `prompt_registry`: Reference to the prompt registry file, used for integrity checks.
- `task_contracts`: Task spec schema or file references used for validation.
- `evidence`: Locations for artifacts (trace snapshots, receipts, redaction reports).
- `governed_exceptions`: Any legacy bypasses with owner, scope, and TTL.

## Validation

- Validate specs against a JSON Schema stored at `docs/agentic/network-spec.schema.json`.
...
agents:
  - id: intake
    ...
    prompt_ref: registry://prompts/registry.yaml#sha256:REPLACE_WITH_HASH
    task_spec_ref: registry://agents/task-spec.schema.json#intake-v1
  ...
  - id: retriever
    ...
    model: router://summit/default
    prompt_ref: registry://prompts/registry.yaml#sha256:REPLACE_WITH_HASH
    task_spec_ref: registry://agents/task-spec.schema.json#retrieval-v1
  ...
  - id: decider
    ...
    prompt_ref: registry://prompts/registry.yaml#sha256:REPLACE_WITH_HASH
    task_spec_ref: registry://agents/task-spec.schema.json#decision-v1
...
governance:
  prompt_registry: prompts/registry.yaml
  task_contracts: agents/task-spec.schema.json
  evidence:
    traces: s3://provenance-ledger/helpdesk/traces
    receipts: s3://provenance-ledger/helpdesk/receipts
    redaction_reports: s3://provenance-ledger/helpdesk/redactions
  governed_exceptions: []
```

...
Release workflows generate a compliance evidence pack as part of the reusable release pipeline
(`.github/workflows/_reusable-release.yml`). The bundle is published alongside release artifacts
and contains the following outputs:

- **SBOM**: `sbom-<version>.json`
- **Vulnerability report**: `vuln-report.json`
- **Attestation summary + scope**: `attestation-summary.json`, `attestation-scope.md`
- **Policy decision logs**: `policy-decisions.json`
- **Deploy metadata**: `deploy-metadata.json`

All of the above are packaged into `evidence/pack-v<version>.tgz` and uploaded with the release

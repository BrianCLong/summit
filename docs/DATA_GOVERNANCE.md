...
### 1.1 Proprietary Data Flywheel (Structured Supervision)

To strengthen IntelGraph's proprietary differentiation beyond data ownership, we continuously capture and productize unique analyst interaction traces as structured supervision signals rather than raw chat transcripts.

- **Workflow telemetry captured**: analyst decisions, investigation paths, query reformulations, redaction steps, approvals, policy outcomes, and escalation/closure rationales.
- **Storage model**: event graphs with timelines, typed nodes/edges, and linked rationales (including policy context and human notes) stored in governed evidence stores rather than unstructured chat logs.
- **Operational outcomes**: training and evaluation datasets reflect real investigation workflows and compliance gates, enabling model improvements against production behaviors instead of generic benchmarks.
- **Governance hooks**: every captured signal inherits tenant/role scoping, immutable audit links, and retention controls matching Sections 2–4 to ensure the flywheel aligns with the broader data governance posture.

...
export default function ServingLaneTrends() {
  ...
  useEffect(() => {
    const load = () => getServingMetrics?.().then((r:any) => {
      const toPts = (arr:any[]) => (arr||[]).map((p:any)=>({ x: new Date(p.ts).toLocaleTimeString(), y: p.value }));
  ...
  return (
    <section className="space-y-3">
      <LineTimeseries title="Queue Depth" data={seriesQ} ariaLabel="Serving lane queue depth over time" />
      <LineTimeseries title="Batch Size" data={seriesB} ariaLabel="Serving lane batch size over time" />
      <LineTimeseries title="KV Hit Ratio" data={seriesK} ariaLabel="Serving lane KV hit ratio over time" />

...
function PolicyExplainDialog({ open, onClose, data, }) {
    if (!open)
        return null;
    return (<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" role="dialog" aria-modal="true" aria-labelledby="policy-explain-title">
      <div className="bg-white rounded-2xl p-4 w-[720px] max-h-[80vh] overflow-auto shadow-xl outline-none">
        ...
        {data?.reasons && data.reasons.length > 0 && (<ul className="mt-2 text-sm list-disc pl-5">
            {data.reasons.map((r, i) => (<li key={i}>{r}</li>))}
          </ul>)}
        {data?.rules && (<details className="mt-3">

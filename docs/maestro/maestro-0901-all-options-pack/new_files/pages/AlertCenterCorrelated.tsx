...
export default function AlertCenterCorrelated() {
  const [filters, setFilters] = useState<{type?:string;severity?:string;tenant?:string;provider?:string}>({});
  const [events, setEvents] = useState<any[]>([]);
  const [err, setErr] = useState<string | null>(null);

  const load = () => getAlertCenterEvents(filters).then(setEvents).catch(e=>setErr(String(e)));
        ...
        <tbody>
          {events.map((e,i)=>(
            <tr key={i} className="border-t">
              ...
              <td className="px-2 py-1">{e.runId ? <Link className="underline" to={`/maestro/runs/${e.runId}`}>#{e.runId}</Link> : '-'}</td>

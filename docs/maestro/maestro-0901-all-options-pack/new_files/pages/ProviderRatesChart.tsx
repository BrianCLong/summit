...
export default function ProviderRatesChart() {
  ...
  useEffect(() => {
    const load = () => getProviderUsage?.({ windowMs: 60_000 }).then((r:any)=>{
      const points = (r?.series || []).map((p:any)=>({ x: new Date(p.ts).toLocaleTimeString(), y: p.rpm }));
      setSeries(points); setLimit(r?.limit ?? null);
  ...
  return (
    <div className="p-6 space-y-3">
      ...
      <LineTimeseries title="Requests per minute" data={series} ariaLabel="Provider RPM over time" />

...
export default function LineTimeseries({ title, data, xKey='x', yKey='y', height=240, ariaLabel }:{title?:string; data:Point[]; xKey?:string; yKey?:string; height?:number; ariaLabel?:string}){
  if (!Recharts) {
    return (
      <section className="border rounded p-3">
        {title && <h3 className="font-medium">{title}</h3>}
        <div className="text-sm text-gray-600">Charts package not installed; showing sample rows.</div>
        <ul className="text-xs mt-2">
          {data.slice(0,10).map((d,i)=>(<li key={i}>{String((d as any)[xKey])}: {(d as any)[yKey]}</li>))}
        </ul>
      </section>
    );
  }

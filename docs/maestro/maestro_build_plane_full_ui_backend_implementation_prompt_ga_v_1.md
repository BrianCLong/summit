...
## 0) Vision & Scope
- **North Star:** A *single, intuitive control plane* for everything the Maestro build plane does: plan → build → verify → ship → observe → prove. Visual-first, policy-first, evidence-first.
...
- **Run Detail:**
  ...
  - **Controls:** rerun (same inputs), *promote from run*, create ticket, create recipe from run (templatize).
...
- **SLOs**: e.g., *Control Plane Availability 99.9%*, *Run success ≥ 97%*, *p95 Build ≤ 10 min*, *UI p95 TTI ≤ 2.5s*.
...
## 8) Integrations
- **GitHub/Jira** (existing hooks in repo: *use* `useGitHubIntegration.ts`, `useJIRAIntegration.ts`).

...
| `❌ Gateway failed` during `make smoke`                                     | Makefile smoke target                                 | Gateway container not serving `/health` on port 8080 | Check Compose logs for `gateway`, restart the stack with `make down && make up`.         |

...
*   **Failover State Transitions:** Proves the application code *can* handle a transition from "Primary" to "Secondary" logic (if any specific logic exists).
...
*   **Canary Synthetic Monitors:** CloudWatch Synthetics verifying the *actual* endpoint (`api.summit...`) returns 200 OK from the secondary region.

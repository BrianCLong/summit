...
1.  **High Pool Utilization**: >85% of `db_pool_size` used for 5m.
    - _Action_: Check for slow queries. Scale up pool if necessary (see Config).
2.  **Circuit Breaker Open**: `db_circuit_breaker_state` == 2.
    - _Action_: Check database health. Restart DB service if down.
3.  **Backup Failure**: `backup_duration_seconds{status="failure"}` > 0.
    - _Action_: Investigate logs, check disk space.

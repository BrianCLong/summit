...
- **Git Config:** `git config core.ignorecase` should typically be `true` on macOS/Windows and `false` on Linux, but this doesn't prevent collisions from being *pushed* by a case-sensitive system. This guardrail prevents such pushes.

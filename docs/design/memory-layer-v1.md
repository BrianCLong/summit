...
This design asserts readiness alignment per the Summit Readiness Assertion and the governing constitution to ensure all memory artifacts remain auditable and policy-compliant.
...
- **Execution/Event Log (authoritative)**
  ...
  - Replay mode is explicit (`live` vs `replay`) and is logged alongside each event for auditability.
- **Vector Memory (semantic index)**
  ...
  - Records embed embedding model/version for drift control and re-indexing.
...
- **Episodic Memory (workflow-level recall)**
  ...
  - Summaries are tagged with summarizer/version metadata for safe re-summarization and drift control.
...
- Router plans include tenant/org/workspace/run/thread scopes to prevent cross-tenant recall and to support downstream RBAC enforcement.

...
- Custom bulk audit script (npm advisory bulk API) against all pnpm-lock packages returned 21 advisories (see `/tmp/pnpm-audit/bulk-full.json`). Mapped to lockfile: 17 packages / 15 versions affected; report in `docs/audit/npm_bulk_advisory_report.md`.

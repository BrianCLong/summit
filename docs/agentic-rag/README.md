...
- **Control plane (API/orchestrator):**
  - GraphQL mutation `ragAnswer` (schema and resolver live in `server/src/graphql/schema.ts` and `server/src/graphql/resolvers/agenticRag.ts`).
  - Handles request validation, caching in Redis, dispatches to the orchestrator, and assembles citations.
- **Data plane (workers):**
...
- **Data plane:** increase worker concurrency and queue partitions; DLQ visible via Redis list `agentic-rag:dlq`.

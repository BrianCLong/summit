# SECURITY REMEDIATION LEDGER — SUMMIT PLATFORM
...
## VERIFICATION COMMANDS (APPLY TO EVERY FINDING)

Each issue in this ledger is verified by running the applicable command(s) below after the fix is
applied. This ledger remains the authoritative remediation tracking source.

- Dependency findings: `pnpm -r audit --audit-level=low`
- JavaScript/TypeScript lint: `pnpm -r lint`
- Type checks: `pnpm -r typecheck`
- Unit/integration tests: `pnpm -r test`
- Golden path smoke test: `make smoke`
- Secrets scan: `rg -n "BEGIN (RSA|OPENSSH|EC) PRIVATE KEY|AKIA[0-9A-Z]{16}|xox[baprs]-|ghp_|AIza|-----BEGIN" -S .`

...
- **simulationRuns**: Array of per-suite results for `security_evals`, `anomaly_fixtures`, and `api_integration`.
  ...
  - **results**:
    - `scenarios`: Evaluated scenario outcomes with expected vs actual and pass/fail
    - `anomalies`: Fixture scores and alert flags for the suite
  - **deltas**:
...
- No external calls; anomaly scoring runs in-memory using deterministic z-score thresholds and expected alert flags in fixtures.

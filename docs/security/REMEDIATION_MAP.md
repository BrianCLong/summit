...
Modified `server/src/conductor/admission/budget-control.ts` to implement `reserveBudget` method which atomically increments usage _during_ the admission check, preventing race conditions.

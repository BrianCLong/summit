...
- **The Mechanism:** Summit doesn't just check tokens; it maps the _graph distance_ between the acting agent and the resource. If an agent "pivots" (changes roles) too rapidly across the graph, `PolicyEngine` blocks it dynamically.
...
- **The Mechanism:** A "Pre-Flight" gate in `Conductor` that runs semantic analysis on the context window _before_ the LLM inference. It detects "drift" away from the agent's Constitutional definition.
- **Why It's a Moat:** Most platforms rely on "post-hoc" filtering or simple prompt engineering. Summit integrates this check into the _orchestration loop_, making it impossible to bypass.

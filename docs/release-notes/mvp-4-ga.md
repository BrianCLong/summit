...
- GraphQL runbook surface available via `packages/graphql/schema.graphql` on ports 4000/8080.
...
- Smoke checks rely on the gateway health endpoint at `/health`; custom gateways must expose the same path.

...
const MLPipeline_1 = require("../services/MLPipeline");
describe('MLPipeline - AI/ML Operations', () => {
    let pipeline;
    let mockModelRegistry;
    let mockPreprocessor;
    let mockModel;
    beforeEach(() => {
        // Setup model mock
        mockModel = {
            predict: globals_1.jest.fn(),
            batchPredict: globals_1.jest.fn(),
        ...
        mockModelRegistry = {
            loadModel: globals_1.jest.fn().mockResolvedValue(mockModel),
            unloadModel: globals_1.jest.fn().mockResolvedValue(undefined),
            listModels: globals_1.jest.fn().mockReturnValue([
                { name: 'test-model', version: '1.0.0', status: 'loaded' },
            ]),
            getModel: globals_1.jest.fn().mockReturnValue(mockModel),
        };
        // Setup preprocessor mock
        mockPreprocessor = {
            normalize: globals_1.jest.fn((data) => data),
            tokenize: globals_1.jest.fn((text) => Array.from({ length: 768 }, () => Math.random())),
            encodeLabels: globals_1.jest.fn((labels) => labels.map((l, i) => i)),
            decodeLabels: globals_1.jest.fn((indices) => indices.map((i) => `label-${i}`)),
        };
        pipeline = new MLPipeline_1.MLPipeline(mockModelRegistry, mockPreprocessor);
    });
    afterEach(() => {
    ...
    describe('inference', () => {
        ...
        it('should validate input data', async () => {
            ...
            // Act & Assert
            await expect(pipeline.predict('test-model', invalidInput)).rejects.toThrow('Input text cannot be empty');
        });
    ...
    describe('data preprocessing', () => {
        ...
        it('should truncate long sequences', () => {
            ...
            mockPreprocessor.tokenize.mockImplementation((text) => Array.from({ length: Math.min(512, text.length) }, () => Math.random()));
            // Act
    ...
    describe('pipeline orchestration', () => {
        ...
        it('should handle stage failures gracefully', async () => {
            ...
            // Act & Assert
            await expect(pipeline.executePipeline(pipelineConfig, input)).rejects.toThrow('Pipeline execution failed at stage: stage2');
        });
            ...
            const pipelineConfig = {
                stages: [
                    { name: 'detection', model: 'detector' },
                    {
                        name: 'classification',
                        model: 'classifier',
                        condition: (prev) => prev.detected === true,
                    },
                ],
            };
            const input = { text: 'Sample text', metadata: {} };
    ...
    describe('model validation', () => {
        it('should validate model output format', async () => {
            ...
            // Act & Assert
            await expect(pipeline.predict('test-model', input)).rejects.toThrow('Invalid model output format');
        });
        ...
        it('should validate score ranges', async () => {
            ...
            // Act & Assert
            await expect(pipeline.predict('test-model', input)).rejects.toThrow('Scores must be between 0 and 1');
        });
    });
    // ===========================================
    // PERFORMANCE TESTS
    ...
    describe('performance', () => {
        ...
        it('should meet inference latency SLA', async () => {
            // Arrange
            const input = { text: 'Sample text', metadata: {} };
            mockModel.predict.mockImplementation(async () => new Promise((resolve) => setTimeout(() => resolve({
                labels: ['Label'],
                scores: [0.9],
                confidence: 0.9,
            }), 50)));
        ...
        it('should optimize batch size automatically', async () => {
            ...
            let batchSizes = [];
            mockModel.batchPredict.mockImplementation(async (batch) => {
    ...
    describe('monitoring', () => {
        it('should emit metrics for predictions', async () => {
            // Arrange
            const metricsSpy = globals_1.jest.fn();
        ...
        it('should track model performance metrics', async () => {
            ...
            // Act
            await Promise.all(Array.from({ length: 10 }, () => pipeline.predict('test-model', {
                text: 'Sample',
                metadata: {},
            })));

...
const entityResolver_1 = require("../resolvers/entityResolver");
describe('EntityResolver', () => {
    ...
    beforeEach(() => {
        ...
        mockServices = {
            entityRepo: {
                ...
                findByTenant: globals_1.jest.fn(),
                update: globals_1.jest.fn(),
                delete: globals_1.jest.fn(),
                search: globals_1.jest.fn(),
            },
            relationshipRepo: {
                findByEntity: globals_1.jest.fn(),
            },
            auditService: {
                log: globals_1.jest.fn(),
            },
            neo4jService: {
                runQuery: globals_1.jest.fn(),
                createNode: globals_1.jest.fn(),
                updateNode: globals_1.jest.fn(),
                deleteNode: globals_1.jest.fn(),
            },
        };
        ...
        mockContext = {
            ...
            logger: mockLogger,
            authorize: globals_1.jest.fn().mockResolvedValue(true),
            req: {
                ip: '127.0.0.1',
    ...
    describe('Query.entity', () => {
        it('should fetch entity by id successfully', async () => {
            ...
            // Act
            const result = await entityResolver_1.resolvers.Query.entity(null, { id: 'entity-789' }, mockContext);
            // Assert
            ...
            expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('Fetching entity'), expect.objectContaining({ entityId: 'entity-789' }));
        });
        it('should throw error when entity not found', async () => {
            // Arrange
            mockServices.entityRepo.findById.mockResolvedValue(null);
            // Act & Assert
            await expect(entityResolver_1.resolvers.Query.entity(null, { id: 'nonexistent' }, mockContext)).rejects.toThrow('Entity not found');
        ...
        it('should throw error when user is unauthorized', async () => {
            ...
            // Act & Assert
            await expect(entityResolver_1.resolvers.Query.entity(null, { id: 'entity-789' }, mockContext)).rejects.toThrow('Forbidden: Insufficient permissions');
        ...
        it('should enforce tenant isolation', async () => {
            ...
            mockServices.entityRepo.findById.mockResolvedValue(mockEntity);
            // Act & Assert
            await expect(entityResolver_1.resolvers.Query.entity(null, { id: 'entity-789' }, mockContext)).rejects.toThrow('Access denied');
        });
    });
    describe('Query.entities', () => {
        it('should fetch all entities for tenant', async () => {
            ...
            // Act
            const result = await entityResolver_1.resolvers.Query.entities(null, { limit: 10, offset: 0 }, mockContext);
            // Assert
            expect(result).toEqual(mockEntities);
            expect(mockServices.entityRepo.findByTenant).toHaveBeenCalledWith('tenant-456', { limit: 10, offset: 0 });
        });
        it('should apply filters correctly', async () => {
            ...
            // Act
            await entityResolver_1.resolvers.Query.entities(null, { filters }, mockContext);
            // Assert
            expect(mockServices.entityRepo.search).toHaveBeenCalledWith('tenant-456', expect.objectContaining(filters));
        });
        it('should enforce pagination limits', async () => {
            // Arrange & Act
            await expect(entityResolver_1.resolvers.Query.entities(null, { limit: 1000 }, // Exceeds max
            mockContext)).rejects.toThrow('Limit cannot exceed 100');
        });
    });
    // ===========================================
    // MUTATION TESTS
    ...
    describe('Mutation.createEntity', () => {
        it('should create entity with valid input', async () => {
            ...
            // Act
            const result = await entityResolver_1.resolvers.Mutation.createEntity(null, { input }, mockContext);
            // Assert
            expect(result).toEqual(mockCreatedEntity);
            expect(mockContext.authorize).toHaveBeenCalledWith('entity:create');
            expect(mockServices.entityRepo.create).toHaveBeenCalledWith(expect.objectContaining({
                tenantId: 'tenant-456',
                ...input,
            }), 'user-123');
            expect(mockServices.auditService.log).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should validate required fields', async () => {
            ...
            // Act & Assert
            await expect(entityResolver_1.resolvers.Mutation.createEntity(null, { input: invalidInput }, mockContext)).rejects.toThrow('Validation error: kind is required');
        });
        it('should validate props schema', async () => {
            ...
            // Act & Assert
            await expect(entityResolver_1.resolvers.Mutation.createEntity(null, { input: invalidInput }, mockContext)).rejects.toThrow('Invalid email format');
        });
        it('should handle database errors gracefully', async () => {
            ...
            // Act & Assert
            await expect(entityResolver_1.resolvers.Mutation.createEntity(null, { input }, mockContext)).rejects.toThrow('Failed to create entity');
            expect(mockLogger.error).toHaveBeenCalledWith(expect.stringContaining('Error creating entity'), expect.any(Error));
        });
        it('should rollback on Neo4j sync failure', async () => {
            ...
            // Act & Assert
            await expect(entityResolver_1.resolvers.Mutation.createEntity(null, { input }, mockContext)).rejects.toThrow('Failed to sync with graph database');
    ...
    describe('Mutation.updateEntity', () => {
        it('should update entity successfully', async () => {
            ...
            // Act
            const result = await entityResolver_1.resolvers.Mutation.updateEntity(null, { input }, mockContext);
            // Assert
        ...
        it('should prevent updating entities from other tenants', async () => {
            ...
            // Act & Assert
            await expect(entityResolver_1.resolvers.Mutation.updateEntity(null, { input: { id: 'entity-789', props: {} } }, mockContext)).rejects.toThrow('Access denied');
        });
    });
    describe('Mutation.deleteEntity', () => {
        it('should delete entity successfully', async () => {
            ...
            // Act
            const result = await entityResolver_1.resolvers.Mutation.deleteEntity(null, { id: 'entity-789' }, mockContext);
            // Assert
        ...
        it('should prevent deleting entities with relationships', async () => {
            ...
            // Act & Assert
            await expect(entityResolver_1.resolvers.Mutation.deleteEntity(null, { id: 'entity-789' }, mockContext)).rejects.toThrow('Cannot delete entity with existing relationships');
        });
    });
    // ===========================================
    ...
    describe('Entity.relationships', () => {
        it('should resolve relationships for entity', async () => {
            ...
            // Act
            const result = await entityResolver_1.resolvers.Entity.relationships(parent, {}, mockContext);
            // Assert
            expect(result).toEqual(mockRelationships);
            expect(mockServices.relationshipRepo.findByEntity).toHaveBeenCalledWith('entity-789', 'tenant-456');
        });
        it('should filter relationships by type', async () => {
            ...
            mockServices.relationshipRepo.findByEntity.mockResolvedValue([]);
            // Act
            await entityResolver_1.resolvers.Entity.relationships(parent, args, mockContext);
            // Assert
            expect(mockServices.relationshipRepo.findByEntity).toHaveBeenCalledWith('entity-789', 'tenant-456', { type: 'KNOWS' });
        });
    });
    // ===========================================
    ...
    describe('Subscription.entityUpdated', () => {
        it('should subscribe to entity updates', async () => {
            ...
            const mockAsyncIterator = {
                [Symbol.asyncIterator]: globals_1.jest.fn(() => ({
                    next: globals_1.jest.fn(),
                    return: globals_1.jest.fn(),
                    throw: globals_1.jest.fn(),
                })),
            };
            const mockPubSub = {
                asyncIterator: globals_1.jest.fn().mockReturnValue(mockAsyncIterator),
            };
            mockContext.pubsub = mockPubSub;
            // Act
            const result = await entityResolver_1.resolvers.Subscription.entityUpdated.subscribe(null, { entityId: 'entity-789' }, mockContext);
            // Assert

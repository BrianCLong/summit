...
const SimulationEngine_1 = require("../src/core/SimulationEngine");
describe('SimulationEngine', () => {
    ...
    describe('initialization', () => {
        ...
        it('should validate actor configuration', () => {
            ...
            // Act & Assert
            expect(() => engine.initialize(invalidConfig)).toThrow('Actor mood must be between -10 and 10');
        });
        it('should validate relationship references', () => {
            ...
            // Act & Assert
            expect(() => engine.initialize(invalidConfig)).toThrow('Relationship references non-existent actor');
        });
    });
    // ===========================================
    ...
    describe('event processing', () => {
        ...
        it('should apply resilience to mood changes', () => {
            ...
            engine.step();
            const mayorMoodChange = Math.abs(engine.getState().ensureActor('mayor').getMood() - initialMayorMood);
            engine.reset();
            engine.initialize(baseConfig);
            engine.injectEvent(activistEvent);
            engine.step();
            const activistMoodChange = Math.abs(engine.getState().ensureActor('activist').getMood() -
                initialActivistMood);
            // Assert
    ...
    describe('relationship dynamics', () => {
        ...
        it('should degrade relationships during prolonged conflict', () => {
            // Arrange
            const conflictEvents = Array.from({ length: 10 }, (_, i) => ({
                id: `conflict-${i}`,
                type: 'crisis',
        ...
        it('should allow relationship type changes based on interactions', () => {
            // Arrange
            const positiveInteractions = Array.from({ length: 20 }, (_, i) => ({
                id: `positive-${i}`,
                type: 'cooperation',
                actorId: i % 2 === 0 ? 'mayor' : 'activist',
    ...
    describe('state management', () => {
        ...
        it('should validate imported state', () => {
            ...
            // Act & Assert
            expect(() => engine.importState(invalidState)).toThrow('Invalid state: timestamp cannot be negative');
        });
    });
    // ===========================================
    // PERFORMANCE TESTS

...
describe('Golden Path Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        neo4jDriver = neo4j.driver(process.env.TEST_NEO4J_URL || 'bolt://localhost:7687', neo4j.auth.basic(process.env.TEST_NEO4J_USER || 'neo4j', process.env.TEST_NEO4J_PASSWORD || 'password'));
        // Verify connectivity
    ...
    describe('Step 1: Create Investigation', () => {
      ...
      `;
        ...
        it('should persist investigation in PostgreSQL', async () => {
            ...
            // Act
            const result = await pgPool.query('SELECT * FROM investigations WHERE id = $1', [testInvestigationId]);
            // Assert
        ...
        it('should create investigation node in Neo4j', async () => {
            ...
            try {
                const result = await session.run('MATCH (i:Investigation {id: $id}) RETURN i', { id: testInvestigationId });
                // Assert
        ...
        it('should create audit log entry', async () => {
            ...
            await createTestInvestigation();
            // Act
            const result = await pgPool.query(`SELECT * FROM audit_logs
         WHERE entity_type = 'investigation'
         AND entity_id = $1
         AND action = 'created'`, [testInvestigationId]);
            // Assert
            expect(result.rows).toHaveLength(1);
            expect(result.rows[0]).toMatchObject({
    ...
    describe('Step 2: Add Entities', () => {
        ...
        it('should sync entities to both PostgreSQL and Neo4j', async () => {
            ...
            const pgResult = await pgPool.query('SELECT * FROM entities WHERE id = ANY($1)', [testEntityIds]);
            expect(pgResult.rows).toHaveLength(testEntityIds.length);
            // Act & Assert - Neo4j
            const session = neo4jDriver.session();
            try {
                const neo4jResult = await session.run('MATCH (e:Entity) WHERE e.id IN $ids RETURN e', { ids: testEntityIds });
    ...
    describe('Step 3: Create Relationships', () => {
        ...
        it('should create graph relationships in Neo4j', async () => {
            ...
            const session = neo4jDriver.session();
            try {
                const result = await session.run(`MATCH (a)-[r]->(b)
           WHERE a.id IN $entityIds AND b.id IN $entityIds
           RETURN r`, { entityIds: testEntityIds });
                // Assert
        ...
        it('should support graph traversal queries', async () => {
            ...
            const session = neo4jDriver.session();
            try {
                const result = await session.run(`MATCH path = (a:Entity {id: $startId})-[*1..3]-(b:Entity {id: $endId})
           RETURN path
           LIMIT 1`, {
    ...
    describe('Step 4: Copilot Analysis', () => {
      ...
      `;
        ...
        it('should store copilot insights in database', async () => {
            ...
            // Act
            const result = await pgPool.query(`SELECT * FROM copilot_insights
    ...
    async function createTestInvestigation() {
        const mutation = `
      mutation CreateInvestigation($input: CreateInvestigationInput!) {
        createInvestigation(input: $input) {
          id
    ...
    async function addTestEntities() {
        const entities = [
    ...
    async function waitForCopilotCompletion(runId) {
        let attempts = 0;
        const maxAttempts = 30;
        while (attempts < maxAttempts) {
            const query = `
    ...
    async function cleanupTestData() {
        ...
        if (testInvestigationId) {
            const session = neo4jDriver.session();
            try {
                await session.run('MATCH (n) WHERE n.investigationId = $id DETACH DELETE n', { id: testInvestigationId });
            }
            finally {
                await session.close();
            }

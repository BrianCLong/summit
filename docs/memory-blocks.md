...
`scripts/memory-block-reflector.ts` is a utility worker that can be triggered on a schedule or queue event. It skips read-only or unauthorized blocks and logs the skip so operators can tune policies:

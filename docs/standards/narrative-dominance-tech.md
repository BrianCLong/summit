...
## Graph-Native Schema Mapping (Neo4j + PG + Vector)

To force alignment across artifact, claim, and assumption semantics, represent narrative intelligence in a hybrid model with deterministic IDs and explicit provenance references.

### Canonical node/record model

| Concept             | Neo4j label / edge                                                                           | Postgres table             | Vector collection       | Required keys                                       |
| ------------------- | -------------------------------------------------------------------------------------------- | -------------------------- | ----------------------- | --------------------------------------------------- |
| Narrative artifact  | `(:NarrativeArtifact)`                                                                       | `narrative_artifacts`      | `artifact_embeddings`   | `artifact_id`, `source_id`, `ingest_batch_id`       |
| Narrative claim     | `(:NarrativeClaim)` + `(:NarrativeArtifact)-[:ASSERTS]->(:NarrativeClaim)`                   | `narrative_claims`         | `claim_embeddings`      | `claim_id`, `claim_hash`, `artifact_id`             |
| Implicit assumption | `(:NarrativeAssumption)` + `(:NarrativeClaim)-[:DEPENDS_ON]->(:NarrativeAssumption)`         | `narrative_assumptions`    | `assumption_embeddings` | `assumption_id`, `assumption_hash`, `lineage_id`    |
| State transition    | `(:NarrativeStateEvent)` + `(:NarrativeCluster)-[:TRANSITIONED_VIA]->(:NarrativeStateEvent)` | `narrative_state_events`   | none                    | `event_id`, `from_state`, `to_state`, `evidence_id` |
| Governance signal   | `(:GovernanceSignal)` + `(:NarrativeCluster)-[:REFERENCES]->(:GovernanceSignal)`             | `governance_signal_events` | none                    | `signal_id`, `reference_rate`, `execution_rate`     |

### Deterministic identity rules

- `artifact_id = sha256(source_uri + canonical_text + published_at)`
- `claim_id = sha256(artifact_id + canonical_claim_span)`
- `assumption_id = sha256(claim_id + canonical_assumption_form)`
- `cluster_id` is stable only when `canonical_inputs_manifest + pipeline_version + config_json` remain unchanged.

## CI-Style Narrative Assurance Tests

Narrative operations must be testable like software quality gates. Each gate emits deterministic artifacts and blocks merge on regression.

| Gate                               | Purpose                                                                | Deterministic assertion                                | Artifact                                       |
| ---------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------ | ---------------------------------------------- |
| `narrative_mutation_gate`          | Validate assumption robustness under paraphrase/translation mutation   | `assumption_id` stability delta ≤ configured threshold | `artifacts/narrative/mutation-report.json`     |
| `narrative_breakpoint_gate`        | Detect where narrative coherence fails when assumptions are removed    | breakpoint index stable for same fixture/config        | `artifacts/narrative/breakpoints.json`         |
| `narrative_compression_gate`       | Prevent silent compression drift that hides activation behavior shifts | `activation_tokens_p50/p90` slope within budget        | `artifacts/narrative/compression-metrics.json` |
| `narrative_absorption_gate`        | Verify counter-claim absorption detection remains precise              | precision/recall against frozen fixture baseline       | `artifacts/narrative/absorption-eval.json`     |
| `narrative_governance_signal_gate` | Detect governance signaling inflation                                  | `reference_rate/execution_rate` ratio under ceiling    | `artifacts/narrative/governance-signals.json`  |

Recommended CI invocation sequence:

1. `pnpm run narrative:test:fixtures`
2. `pnpm run narrative:test:determinism`
3. `pnpm run narrative:test:mutation`
4. `pnpm run narrative:test:gates`

## SOC-Style Controls for Narrative Risk Governance

Narrative risk operations require explicit control ownership and evidence traceability.

| Control ID     | Objective                                  | Control statement                                                                                                  | Evidence                               |
| -------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------- |
| `NARR-CTRL-01` | Deterministic replay                       | All narrative scoring pipelines must be replayable with identical IDs for identical inputs/config.                 | `stamp.json`, replay logs, config hash |
| `NARR-CTRL-02` | Artifact/claim/assumption separation       | Artifact text, extracted claims, and inferred assumptions must be stored in separate entities with explicit links. | graph export + relational FK checks    |
| `NARR-CTRL-03` | Assumption provenance                      | Every assumption must have lineage and supporting evidence refs before analyst export.                             | `assumption_lineage_report.json`       |
| `NARR-CTRL-04` | Human confirmation for high-impact outputs | Exports with attribution risk require analyst confirmation and confidence tier.                                    | approval ledger entry                  |
| `NARR-CTRL-05` | Governance signaling integrity             | Governance references without corresponding execution evidence must raise review alerts.                           | governance signal delta report         |
| `NARR-CTRL-06` | Half-life surveillance                     | Long half-life + low engagement combinations must be escalated as coordination risk.                               | half-life monitor report               |

Control exceptions must be treated as **Governed Exceptions** with explicit expiry and rollback path.

...
*Scope: files inside `summit-main` from the provided archive. Focus on components named “orchestrator”, autonomy, DR, and web orchestration paths.*
...
**Primary risks:**
1. **Unbounded actions** (deploy/modify systems) without human‑in‑the‑loop gates → add *Change Types*: `NOOP | PLAN | APPLY` with approvals.
2. **Prompt injection & model overreach** via GraphRAG and connectors → enforce *allowed tools* whitelist and strict input sanitation.
...
---
*End of review (v0). Ping me when you want diffs applied to specific files; I’ll ship patch sets next.*
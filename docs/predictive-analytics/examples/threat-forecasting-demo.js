...
function generateThreatData(days = 365) {
    const data = [];
...
async function main() {
    ...
    // ============================================
    console.log('\n--- ARIMA Forecasting ---');
    const arimaForecaster = new forecasting_1.ARIMAForecaster({ p: 2, d: 1, q: 1 });
    ...
    const prophetForecaster = new forecasting_1.ProphetForecaster({
        horizon: 30,
    ...
    const ensembleForecaster = new forecasting_1.EnsembleForecaster({
        models: [
    ...
    // ============================================
    console.log('\n--- Anomaly Detection ---');
    const anomalyForecaster = new forecasting_1.AnomalyForecaster(2.5, 30);
    ...
    const simulator = new forecasting_1.MonteCarloSimulator(1000);
    ...
    const threatLabels = threatFeatures.map(f => f[0] > 1.2 && f[2] > 0.7 ? 'critical' : f[0] > 0.8 ? 'high' : 'low');
    const classifier = new predictive_models_1.GradientBoostingClassifier({
        nEstimators: 50,
        learningRate: 0.1,
        maxDepth: 4,
    });
    ...
    // ============================================
    console.log('\n--- Entity Risk Scoring ---');
    const riskScorer = new risk_scoring_1.LogisticRiskScorer();

...
async function main() {
  ...
  arimaForecast.slice(0, 7).forEach((f) => {
    console.log(
      `  ${f.timestamp.toDateString()}: ${f.forecast.toFixed(0)} ` +
  ...
  console.log(
    `ARIMA Performance: MAE=${arimaPerformance.mae.toFixed(2)}, ` +
  ...
  console.log(
    `  Trend range: ${Math.min(...decomposition.trend).toFixed(0)} - ` +
      `${Math.max(...decomposition.trend).toFixed(0)}`
  );
  console.log(
    `  Seasonal amplitude: ${(
      Math.max(...decomposition.seasonal) - Math.min(...decomposition.seasonal)
    ).toFixed(2)}`
  );
  ...
  const avgForecast =
    ensembleForecast.reduce((sum, f) => sum + f.forecast, 0) / ensembleForecast.length;
  ...
  if (outbreaks.length > 0) {
    ...
    outbreaks.slice(0, 3).forEach((outbreak) => {
      console.log(
        `  ${outbreak.timestamp.toDateString()}: ` +
  ...
  console.log(
    `Sample prediction: ${prediction[0].prediction === 1 ? "CRITICAL" : "NOT CRITICAL"} ` +

...
Evidence envelope: `docs/ci/evidence/gtm-12-vendor/`
...
> Governance note: treat vendor claims as source-bound observations. Route all externally used
> assertions through `docs/ci/evidence/gtm-12-vendor/CLAIMS.yml` before customer distribution.
...
### Qualification Rule (Deal Hygiene)

- **Replace** when the incumbent cannot satisfy reproducibility + policy-gated execution.
- **Complement** when the incumbent adds narrow source or visualization value but lacks runtime governance.
- **Governed Exception** for incumbent lock-in: retain the incumbent as a data source while Summit becomes the
...
## Field Execution Sequence (Compress Timeline)

1. **Discovery call:** run the five disqualifying questions and capture hard yes/no evidence.
2. **Technical validation:** execute one Summit playbook over incumbent-fed data.
3. **Audit simulation:** export evidence bundle and perform replay + diff with reviewers present.
4. **Procurement framing:** classify each incumbent as replace/complement/governed exception.
5. **Close plan:** lock 30/90-day adoption milestones with measurable success metrics.

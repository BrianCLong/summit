...
**Action for Summit**: Push Summit’s core toward an explicit agentic runtime abstraction with a central execution engine, policy/eval hooks, and pluggable model/tool backends instead of app-specific flows; align definitions to the Summit Readiness Assertion and governance authority files. [dainstudios +1]

    ...
    <div className="grid grid-cols-12 gap-4 p-4">
      ...
      <aside className="col-span-3 space-y-3">
        <Card>
          ...
          <CardContent className="space-y-2">
            {agents.map((a) => (
              <div key={a.id} className="flex items-center justify-between rounded-xl bg-muted p-2">
                <div>
      ...
      <main className="col-span-6 space-y-3">
            ...
            {approvals.map((item) => (
              <div key={item.id} className="flex items-start justify-between rounded-xl border p-3">
                <div>
                  ...
                  <p className="text-xs opacity-70">{t("policyLabel", { policy: item.policy })}</p>
                </div>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-2">
            {graphSlice.map((row) => (
              <div key={`${row.person}-${row.system}`} className="rounded-xl border p-3 text-sm">
                ...
                <p className="opacity-70">{t("approvedLabel", { change: row.approvedChange })}</p>
              </div>
            ))}
          </CardContent>
        </Card>
      ...
      <aside className="col-span-3 space-y-3">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="text-xs opacity-70">{t("copilotContext")}</div>
          </CardContent>
        </Card>
      </aside>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="min-h-32 rounded-xl border p-3">{t("messagesPlaceholder")}</div>
        </DialogContent>
      </Dialog>

...
const renderField = (key, schema, value, onChange) => {
  ...
  const commonProps = {
    ...
    value: value ?? '',
    onChange: (e) => onChange(key, schema.type === 'integer' || schema.type === 'number' ? Number(e.target.value) : e.target.value),
  };
  ...
  return (
    <label className="block space-y-1" key={key}>
      <span className="text-xs font-medium text-gray-700">{label}</span>
      <input type="text" placeholder={schema.placeholder} {...commonProps} />
    </label>
  );
};

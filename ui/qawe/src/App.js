...
function collectActiveGates(inst) {
    if (!inst)
        return [];
    const pairs = [];
    Object.entries(inst.activeStages).forEach(([stageId, stage]) => {
        Object.entries(stage.gates).forEach(([gateId, gate]) => {
            if (!gate.satisfied) {
                if (stage.definition.kind !== 'sequential' ||
...
function decodePrivateKey(base64Key) {
...
function canonicalMessage(instanceId, stageId, gateId, actorId, delegatedFrom, nanos) {
...
function App() {
    const [serverKey, setServerKey] = (0, react_1.useState)('');
    const [workflowInput, setWorkflowInput] = (0, react_1.useState)(sampleWorkflow);
    const [contextInput, setContextInput] = (0, react_1.useState)(emptyContext);
    const [currentWorkflow, setCurrentWorkflow] = (0, react_1.useState)(null);
    const [workflowIdInput, setWorkflowIdInput] = (0, react_1.useState)('');
    const [instanceIdInput, setInstanceIdInput] = (0, react_1.useState)('');
    const [instance, setInstance] = (0, react_1.useState)(null);
    const [approvalForm, setApprovalForm] = (0, react_1.useState)({ ...emptyApproval });
    const [status, setStatus] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
        (0, api_1.fetchServerInfo)()
    ...
    const activeGates = (0, react_1.useMemo)(() => collectActiveGates(instance), [instance]);
    const handleCreateWorkflow = async () => {
        setStatus(null);
        try {
            const parsed = JSON.parse(workflowInput);
            setLoading(true);
            const created = await (0, api_1.createWorkflow)(parsed);
            ...
            setStatus({
                tone: 'error',
                message: `Unable to create workflow: ${err.message}`,
            });
        }
        finally {
            setLoading(false);
    ...
    const handleStartInstance = async () => {
        ...
        try {
            const context = contextInput
                ...
                : {};
            setLoading(true);
            const created = await (0, api_1.startInstance)(workflowIdInput, context);
            setInstance(created);
            ...
            setStatus({
                tone: 'error',
                message: `Unable to start instance: ${err.message}`,
            });
        }
        finally {
            setLoading(false);
    ...
    const handleLoadInstance = async (id) => {
        if (!id) {
            setStatus({
                tone: 'error',
        ...
        try {
            setLoading(true);
            const loaded = await (0, api_1.loadInstance)(id);
            setInstance(loaded);
            ...
            setStatus({
                tone: 'error',
                message: `Unable to load instance: ${err.message}`,
            });
        }
        finally {
            setLoading(false);
    ...
    const handleApprovalSubmit = async () => {
        ...
        try {
            ...
            const nanos = BigInt(issuedAt.getTime()) * 1000000n;
            const message = canonicalMessage(instance.id, stageId, gateId, actorId, delegatedFrom ?? '', nanos);
            const signature = await (0, ed25519_1.sign)(encoder.encode(message), seed);
            const payload = {
                stageId,
                gateId,
                actorId,
            ...
            setLoading(true);
            const response = await (0, api_1.submitApproval)(instance.id, payload);
            let info = 'Approval recorded.';
            if (response.gateId) {
                info = `Gate ${stageId}/${gateId} reached quorum.`;
            }
            const updated = await (0, api_1.loadInstance)(instance.id);
            setInstance(updated);
    ...
    const renderBundles = (bundles) => {
        ...
        {bundles.map((bundle) => (<section key={`${bundle.stageId}-${bundle.gateId}-${bundle.issuedAt}`}>
            <h3>
    ...
    return (<div className="container">
      ...
      <section>
        ...
        <textarea rows={16} value={workflowInput} onChange={(event) => setWorkflowInput(event.target.value)}/>
        <div className="flex-row" style={{ marginTop: '1rem' }}>
          ...
          <div>
            ...
            <input id="workflow-id" value={workflowIdInput} onChange={(event) => setWorkflowIdInput(event.target.value)} placeholder="workflow identifier"/>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="form-grid">
          <div>
            ...
            <textarea id="context-json" rows={6} value={contextInput} onChange={(event) => setContextInput(event.target.value)}/>
          ...
          <div>
            ...
            <input id="instance-id" value={instanceIdInput} onChange={(event) => setInstanceIdInput(event.target.value)} placeholder="instance identifier"/>
            <button style={{ marginTop: '0.75rem' }} onClick={() => handleLoadInstance(instanceIdInput)} disabled={loading}>
              Load Instance
            </button>
          </div>
        </div>
        {instance && (<div style={{ marginTop: '1rem' }}>
      ...
      <section>
        ...
        <div className="form-grid">
          <div className="flex-row">
            <div>
              ...
              <input id="stage-id" value={approvalForm.stageId} onChange={(event) => setApprovalForm((prev) => ({
            ...prev,
            stageId: event.target.value,
        }))} placeholder="stage-identifier" list="active-stages"/>
            </div>
            <div>
              <label htmlFor="gate-id">Gate ID</label>
              <input id="gate-id" value={approvalForm.gateId} onChange={(event) => setApprovalForm((prev) => ({
            ...prev,
            gateId: event.target.value,
        }))} placeholder="gate-identifier" list="active-gates"/>
            </div>
          </div>
          <div className="flex-row">
            <div>
              <label htmlFor="actor-id">Actor ID</label>
              <input id="actor-id" value={approvalForm.actorId} onChange={(event) => setApprovalForm((prev) => ({
            ...prev,
            ...
            <div>
              ...
              <input id="delegated-from" value={approvalForm.delegatedFrom} onChange={(event) => setApprovalForm((prev) => ({
            ...prev,
          ...
          <div>
            ...
            <textarea id="private-key" rows={4} value={approvalForm.privateKey} onChange={(event) => setApprovalForm((prev) => ({
            ...prev,
        ...
        <datalist id="active-stages">
          {activeGates.map((pair) => (<option key={`stage-${pair.stageId}`} value={pair.stageId}/>))}
        </datalist>
        <datalist id="active-gates">
          {activeGates.map((pair) => (<option key={`gate-${pair.stageId}-${pair.gateId}`} value={pair.gateId}/>))}
        </datalist>
      </section>

...
def demonstrate_cognitive_decision_support():
    ...
    try:
        ...
        logger.info(
            f"✅ Collected {len(observations)} security observations from multiple sources:"
        )
        for i, obs in enumerate(observations, 1):
            logger.info(
                f"   {i}. {obs.description} ({obs.type.upper()}, confidence: {obs.confidence:.2f})"
            )
        ...
        if top_hypothesis:
            ...
            logger.info(
                f"   Supporting Evidence: {validation_results['supporting_evidence']}/{validation_results['total_evidence']}"
            )
            logger.info(
                f"   Contradicting Evidence: {validation_results['contradicting_evidence']}"
            )
        ...
        if executed_scenario.outcomes:
            logger.info("   Key Predicted Outcomes:")
            for i, outcome in enumerate(executed_scenario.outcomes[:5], 1):
                logger.info(
                    f"     {i}. {outcome.metric}: {outcome.baseline_value} → {outcome.predicted_value} ({outcome.change_direction})"
                )
        ...
        if top_hypothesis and validation_results and executed_scenario:
            ...
            logger.info(
                f"   🚨 Recommended Actions ({len(recommendation['recommended_actions'])}):"
            )
            ...
            logger.info(
                f"     Immediate Risk: {recommendation['risk_assessment']['immediate_risk']}"
            )
            logger.info(
                f"     Business Impact: {recommendation['risk_assessment']['business_impact']}"
            )
            logger.info(
                f"     Containment Confidence: {recommendation['risk_assessment']['containment_confidence']:.2f}"
            )
            logger.info(
                f"     Recovery Time Estimate: {recommendation['risk_assessment']['recovery_time_estimate']}"
            )

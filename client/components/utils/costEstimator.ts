export type CostEstimate = { score: number; factors: Record<string, number> };

export function estimatePromptCost(prompt: string): CostEstimate {
  ...
  const base =
    8 * factors.hops + 20 * factors.allPaths + 10 * factors.community - 5 * factors.filters;

...
describe("PIIScanner sensitive context gating", () => {
  it("prompts for context when missing and retries request", async () => {
    ...
    expect(
      dialogQueries.getByLabelText(/purpose/i, { selector: "input", hidden: true })
    ).toBeInTheDocument();
    expect(
      dialogQueries.getByLabelText(/justification/i, { selector: "textarea", hidden: true })
    ).toBeInTheDocument();
    expect(
      dialogQueries.getByLabelText(/case id/i, { selector: "input", hidden: true })
    ).toBeInTheDocument();

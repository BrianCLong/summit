...
const HEALTH_Q = gql`
  query Health($sourceId: ID) {
    osintHealth(sourceId: $sourceId) {
      sourceId
      name
      lastRunAt
      lastStatus
...
export default function OsintHealth() {
  ...
  const runNow = async (id: string) => {
    ...
    await fetch('/graphql', {
      ...
      body: JSON.stringify({
        query: 'mutation($id:ID!){ scheduleFetch(sourceId:$id){ id status } }',
        variables: { id },
      }),
    });
    setRunning(null);
  ...
  const openLogs = async (id: string) => {
    ...
    const res = await fetch('/graphql', {
      ...
      body: JSON.stringify({
        query:
          'query($id:ID){ osintRuns(sourceId:$id, limit:10){ startedAt finishedAt status itemsIngested latencyMs error } }',
        variables: { id },
      }),
    });
            ...
            <tr key={h.sourceId}>
              ...
              <td className="border px-2" style={{ display: 'flex', gap: 6 }}>
                <Button
                  size="small"
                  variant="outlined"
                  onClick={() => runNow(h.sourceId)}
                  disabled={running === h.sourceId}
                >
                  Run now
                </Button>
                <Button size="small" onClick={() => openLogs(h.sourceId)}>
                  Logs
                </Button>
                <Button
                  size="small"
        ...
        <div style={{ width: 360, padding: 16 }}>
          ...
          <List dense>
            {logs.map((r: any, i: number) => (
              <ListItem key={i}>
                <ListItemText
                  primary={`${r.status} • ${r.itemsIngested || 0} items • ${r.latencyMs || 0}ms`}
                  secondary={`${r.startedAt} → ${r.finishedAt || ''} ${r.error || ''}`}
                />
              </ListItem>
            ))}
          </List>

...
function HomeRouteInner() {
  ...
  type GraphCanvasData = ComponentProps<typeof InteractiveGraphCanvas>["data"];
  type GraphNodeType = GraphCanvasData["nodes"][number]["type"];
  type TimelineEvent = ComponentProps<typeof TemporalAnalysis>["events"][number];
  type IntelligenceFeedsProps = ComponentProps<typeof IntelligenceFeedsEnrichment>;
  type EnrichedEntity = Parameters<NonNullable<IntelligenceFeedsProps["onEnrichmentComplete"]>>[0];
  ...
  const graphData = useMemo<GraphCanvasData>(() => {
    ...
    const satellites: GraphCanvasData["nodes"] = satelliteConfigs.map((config, index, arr) => {
              ...
              {authorizedQuickActions.length === 0 && (
                <div className="panel" style={{ padding: "16px" }}>
                  ...
                  <p className="muted note" style={{ marginBottom: "12px" }}>
                    Your current tenant or role does not allow the listed quick actions.
                  </p>
                </div>
              )}
      ...
      {activeTab === "analytics" && (
        <div style={{ display: "grid", gap: "24px" }}>
          <GraphAnomalyWidget
            investigationId={selectedInvestigation?.id}
            defaultEntityId={
              selectedInvestigation?.focusEntityId || selectedInvestigation?.entityId
            }
          />
              ...
              <CardContent>
                ...
                <Stack direction="row" spacing={3}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2">{aiContext.selectedEntities.length}</Typography>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">
                      Recent Searches
                    </Typography>
                    <Typography variant="body2">{aiContext.recentSearches.length}</Typography>
                  </Box>
                </Stack>
              </CardContent>
            </Card>
        ...
        <div>
          ...
          >
            <DataConnectorsDashboard
              investigationId={selectedInvestigation?.id}
              onConnectorAdd={(connector) => toast.success("Connector Added", connector.name)}
              onConnectorRemove={(connectorId) => toast.warning("Connector Removed", connectorId)}

...
export function TokenMeter({ 
  ...
  const debouncedCount = useMemo(
    () => debounce(async (provider: string | undefined, model: string, text: string, completion?: string) => {
      if (!model || !text.trim()) {
        setCount(null);
        setLoading(false);
        return;
      ...
      try {
        ...
        const response = await fetch('/api/tokcount', {
          ...
          body: JSON.stringify({ provider, model, prompt: text, completion }),
        });
  ...
  return (
    <div className={`flex items-center gap-2 text-xs ${getBudgetColor()} ${className}`}>
      ...
      <span>
        {count.total.toLocaleString()} tokens
      </span>
 ...
 */
export function SimpleTokenMeter({ text, model = 'gpt-4o-mini' }: { text: string; model?: string }) {
  return (
    <TokenMeter 
      model={model}
      text={text}

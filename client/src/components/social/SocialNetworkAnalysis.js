...
require("./SocialNetworkAnalysis.css");
const SocialNetworkAnalysis = ({ investigationId, initialNodes = [], initialEdges = [], onNodeSelect, onEdgeSelect, onCommunitySelect, onPathAnalysis, onAnalysisComplete, }) => {
    // State Management
    ...
    const [selectedCommunity, setSelectedCommunity] = (0, react_1.useState)(null);
    const [analysisResults, setAnalysisResults] = (0, react_1.useState)(null);
    // UI State
    const [activeTab, setActiveTab] = (0, react_1.useState)('network');
    const [isAnalyzing, setIsAnalyzing] = (0, react_1.useState)(false);
    const [searchTerm, setSearchTerm] = (0, react_1.useState)('');
    const [filterType, setFilterType] = (0, react_1.useState)('all');
    const [layoutMode, setLayoutMode] = (0, react_1.useState)('force');
    const [viewMode, setViewMode] = (0, react_1.useState)('graph');
    // Analysis State
    const [currentQuery, setCurrentQuery] = (0, react_1.useState)(null);
    const [pathAnalysis, setPathAnalysis] = (0, react_1.useState)([]);
    const [anomalies, setAnomalies] = (0, react_1.useState)([]);
    const [metrics, setMetrics] = (0, react_1.useState)({});
    const canvasRef = (0, react_1.useRef)(null);
    const [canvasSize, setCanvasSize] = (0, react_1.useState)({ width: 800, height: 600 });
    ...
    const generateMockData = (0, react_1.useCallback)(() => {
        const mockNodes = [
            ...
            {
                id: 'org-1',
                label: 'Pacific Holdings Ltd',
                type: 'organization',
                properties: {
                    name: 'Pacific Holdings Ltd',
                    description: 'Offshore investment company',
                    importance: 92,
                    riskScore: 88,
                    verified: false,
        ...
        const mockCommunities = [
            {
                ...
                nodes: ['person-1', 'org-1', 'digital-1'],
                properties: {
                    size: 3,
                    density: 0.89,
                    cohesion: 0.76,
                ...
                metrics: {
                    internalConnections: 4,
                    externalConnections: 2,
                    bridgeNodes: ['person-1'],
                    influentialNodes: ['org-1', 'person-1'],
                },
                color: '#ff6b6b',
            },
            {
                ...
                metrics: {
                    ...
                    bridgeNodes: ['person-2'],
                    influentialNodes: ['person-2'],
                },
                color: '#4ecdc4',
            },
        ];
    ...
    const runNetworkAnalysis = (0, react_1.useCallback)(async (query) => {
        setIsAnalyzing(true);
            ...
            case 'centrality':
                results = {
                    ...
                    results: nodes
                        ...
                        .sort((a, b) => b.centralityScores.eigenvector -
                ...
                const mockPaths = [
                    {
                        ...
                        properties: {
                            ...
                            pathType: 'shortest',
                            confidence: 0.92,
                            metadata: { analysis: 'Direct financial control path' },
                        },
                    },
                ];
    ...
    const handleNodeSelect = (0, react_1.useCallback)((node) => {
        setSelectedNode(node);
        onNodeSelect?.(node);
    }, [onNodeSelect]);
    // Edge Selection Handler
    const handleEdgeSelect = (0, react_1.useCallback)((edge) => {
        setSelectedEdge(edge);
        onEdgeSelect?.(edge);
    }, [onEdgeSelect]);
    // Community Selection Handler
    const handleCommunitySelect = (0, react_1.useCallback)((community) => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        filteredNodes.forEach((node, index) => {
            ...
            const riskColor = node.properties.riskScore > 75
                ? '#ff4757'
                : node.properties.riskScore > 50
                    ? '#ffa726'
                    : '#26c6da';
    ...
    return (<div className="social-network-analysis">
      {/* Header */}
      <div className="sna-header">
        ...
        <div className="header-controls">
          <input type="text" placeholder="Search entities..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="search-input"/>
          ...
          <select value={layoutMode} onChange={(e) => setLayoutMode(e.target.value)} className="layout-select">
      ...
      <div className="sna-tabs">
        <button className={`tab-button ${activeTab === 'network' ? 'active' : ''}`} onClick={() => setActiveTab('network')}>
          🌐 Network
        </button>
        <button className={`tab-button ${activeTab === 'analysis' ? 'active' : ''}`} onClick={() => setActiveTab('analysis')}>
          🔍 Analysis
        </button>
        <button className={`tab-button ${activeTab === 'communities' ? 'active' : ''}`} onClick={() => setActiveTab('communities')}>
          👥 Communities
        </button>
      ...
      <div className="sna-content">
        {/* Network Tab */}
        {activeTab === 'network' && (<div className="network-tab">
            <div className="network-main">
              <div className="network-canvas-container">
                <canvas ref={canvasRef} width={canvasSize.width} height={canvasSize.height} className="network-canvas" onClick={(e) => {
            ...
            {selectedNode && (<div className="entity-details">
                ...
                <div className="entity-info">
                  <div className="info-section">
                    ...
                    <div className="property-item">
                      ...
                      <span className={`risk-badge ${selectedNode.properties.riskScore > 75
                    ? 'high'
                    : selectedNode.properties.riskScore > 50
                        ? 'medium'
                        : 'low'}`}>
                  ...
                  <div className="info-section">
                    ...
                    <div className="property-item">
                      ...
                      {selectedNode.metrics.centrality.betweenness.toFixed(3)}
                    </div>
                    <div className="property-item">
        ...
        {activeTab === 'analysis' && (<div className="analysis-tab">
            <div className="analysis-controls">
              ...
              <div className="analysis-buttons">
                <button className="analysis-button" onClick={() => runNetworkAnalysis({ type: 'centrality', parameters: {} })} disabled={isAnalyzing}>
                  🎯 Centrality Analysis
                </button>
                <button className="analysis-button" onClick={() => runNetworkAnalysis({ type: 'community', parameters: {} })} disabled={isAnalyzing}>
                  👥 Community Detection
                </button>
                <button className="analysis-button" onClick={() => runNetworkAnalysis({
                type: 'path',
                parameters: { source: 'person-1', target: 'digital-1' },
            })} disabled={isAnalyzing}>
                  🛤️ Path Analysis
                </button>
                <button className="analysis-button" onClick={() => runNetworkAnalysis({ type: 'anomaly', parameters: {} })} disabled={isAnalyzing}>
                  🚨 Anomaly Detection
                </button>
              </div>
            </div>
            ...
            {analysisResults && (<div className="analysis-results">
                ...
                {analysisResults.type === 'centrality' && (<div className="centrality-results">
                    ...
                    <div className="results-table">
                      <table>
                        ...
                        .slice(0, 5)
                        .map((result, index) => (<tr key={result.nodeId}>
                                <td>{index + 1}</td>
                                <td>{result.name}</td>
                                <td>
                                  {result.centralityScores.eigenvector.toFixed(3)}
                                </td>
                                <td>
                                  {result.centralityScores.betweenness.toFixed(3)}
                                </td>
                                <td>
                ...
                {analysisResults.type === 'anomaly' && (<div className="anomaly-results">
                    <h4>Detected Anomalies</h4>
                    <div className="anomaly-list">
                      {analysisResults.results.map((anomaly) => (<div key={anomaly.id} className={`anomaly-item ${anomaly.severity}`}>
        ...
        {/* Communities Tab */}
        {activeTab === 'communities' && (<div className="communities-tab">
            <h3>Network Communities</h3>
            <div className="communities-grid">
              {communities.map((community) => (<div key={community.id} className={`community-card ${selectedCommunity?.id === community.id ? 'selected' : ''}`} onClick={() => handleCommunitySelect(community)}>
                  <div className="community-header">
                    <div className="community-color" style={{ backgroundColor: community.color }}></div>
                  ...
                  <div className="community-tags">
                    {community.properties.tags.map((tag) => (<span key={tag} className="tag">
                        {tag}
                      </span>))}
                  </div>
                </div>))}
            ...
            {selectedCommunity && (<div className="community-details">
                ...
                <div className="community-analysis">
                  <div className="analysis-section">
                    ...
                    <div className="metrics-grid">
                      <div className="metric">
                        <strong>Internal Connections:</strong>{' '}
                        {selectedCommunity.metrics.internalConnections}
                      </div>
                      <div className="metric">
                        <strong>External Connections:</strong>{' '}
                        {selectedCommunity.metrics.externalConnections}
                      </div>
                      <div className="metric">
                        <strong>Modularity:</strong>{' '}
                        {selectedCommunity.properties.modularity.toFixed(3)}
                      </div>
                      <div className="metric">
                        <strong>Centralization:</strong>{' '}
                        {selectedCommunity.properties.centralization.toFixed(3)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="analysis-section">
                    ...
                    return node ? (<span key={nodeId} className="node-chip">
                                  {node.label}
                                </span>) : null;
                })}
                        </div>
                    ...
                    const node = nodes.find((n) => n.id === nodeId);
                    return node ? (<span key={nodeId} className="node-chip influential">
                                  {node.label}
                                </span>) : null;
                })}
                        </div>
        ...
        {/* Paths Tab */}
        {activeTab === 'paths' && (<div className="paths-tab">
            ...
            {pathAnalysis.length > 0 && (<div className="paths-results">
                <h4>Discovered Paths</h4>
                <div className="paths-list">
                  {pathAnalysis.map((path) => (<div key={path.id} className="path-item">
                      <div className="path-header">
                        ...
                        <span className={`path-risk ${path.properties.riskScore > 0.75
                        ? 'high'
                        : path.properties.riskScore > 0.5
                            ? 'medium'
                            : 'low'}`}>
                      ...
                      <div className="path-visualization">
                        ...
                        const node = nodes.find((n) => n.id === nodeId);
                        return (<react_1.default.Fragment key={nodeId}>
                              <span className="path-node">{node?.label}</span>
                              {index < path.path.length - 1 && (<span className="path-arrow">→</span>)}
        ...
        {/* Metrics Tab */}
        {activeTab === 'metrics' && (<div className="metrics-tab">
            <h3>Network Metrics Overview</h3>

            {Object.keys(metrics).length > 0 && (<div className="metrics-dashboard">
                <div className="metrics-section">
                  ...
                  <div className="metrics-grid">
                    ...
                    <div className="metric-card">
                      <div className="metric-value">
                        {metrics.overview?.networkDensity?.toFixed(3)}
                      </div>
                  ...
                  <div className="centrality-info">
                    ...
                    <div className="info-item">
                      ...
                      <div className="bridge-nodes">
                        {metrics.centrality?.keyBridgeNodes?.map((nodeId) => {
                    const node = nodes.find((n) => n.id === nodeId);
                    return node ? (<span key={nodeId} className="bridge-node">
                                {node.label}
                              </span>) : null;
                })}
                      </div>
                  ...
                  <div className="risk-analysis">
                    <div className="risk-item">
                      ...
                      <div className="risk-entities">
                        {metrics.risk?.highRiskNodes?.map((nodeId) => {
                    const node = nodes.find((n) => n.id === nodeId);
                    return node ? (<span key={nodeId} className="risk-entity high">
                              {node.label}
                            </span>) : null;
                })}
                      </div>
                    ...
                    <div className="risk-item">
                      ...
                      <div className="risk-patterns">
                        {metrics.risk?.anomalousPatterns?.map((pattern, index) => (<span key={index} className="pattern-tag">
                              {pattern}
                            </span>))}
                      </div>
                    </div>
...
exports.default = SocialNetworkAnalysis;

...
const DigitalForensicsWorkbench = ({ investigationId, caseId, onEvidenceAdd = () => { }, onFindingCreate = () => { }, onTimelineUpdate = () => { }, onChainOfCustodyUpdate = () => { }, className = '', }) => {
    const [activeView, setActiveView] = (0, react_1.useState)('dashboard');
    const [cases, setCases] = (0, react_1.useState)([]);
    const [evidenceItems, setEvidenceItems] = (0, react_1.useState)([]);
    const [timelineEvents, setTimelineEvents] = (0, react_1.useState)([]);
    const [analysisTasks, setAnalysisTasks] = (0, react_1.useState)([]);
    ...
    const [isAnalysisRunning, setIsAnalysisRunning] = (0, react_1.useState)(false);
    const [timelineFilter, setTimelineFilter] = (0, react_1.useState)('all');
    const [evidenceFilter, setEvidenceFilter] = (0, react_1.useState)('all');
    (0, react_1.useEffect)(() => {
    ...
    const generateMockEvidenceItems = () => {
        ...
        const mockEvidence = Array.from({ length: 7 }, (_, i) => ({
            ...
            analysis: {
                status: ['pending', 'in_progress', 'completed', 'reviewed'][Math.floor(Math.random() * 4)],
                findings: [],
    ...
    const filteredTimelineEvents = timelineEvents.filter((event) => {
        if (timelineFilter === 'all')
            return true;
        if (timelineFilter === 'flagged')
            return event.flagged;
        if (timelineFilter === 'high_relevance')
            return event.relevance === 'high';
    ...
    return (<div className={`digital-forensics-workbench ${className}`}>
      {/* Header */}
      <div className="mb-6 border-b pb-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-bold">Digital Forensics Workbench</h2>
          <div className="flex items-center gap-4">
            {selectedCase && (<div className="text-sm">
        ...
        <div className="flex gap-4">
          <button onClick={() => setActiveView('dashboard')} className={`px-4 py-2 rounded-md ${activeView === 'dashboard' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            🏠 Dashboard
          </button>
          <button onClick={() => setActiveView('evidence')} className={`px-4 py-2 rounded-md ${activeView === 'evidence' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            📁 Evidence ({evidenceItems.length})
          </button>
          <button onClick={() => setActiveView('timeline')} className={`px-4 py-2 rounded-md ${activeView === 'timeline' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
          ...
          <button onClick={() => setActiveView('reports')} className={`px-4 py-2 rounded-md ${activeView === 'reports' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            📄 Reports
          </button>
          <button onClick={() => setActiveView('tools')} className={`px-4 py-2 rounded-md ${activeView === 'tools' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
      ...
      {activeView === 'dashboard' && (<div className="space-y-6">
          {/* Case Overview */}
          {selectedCase && (<div className="bg-white rounded-lg border p-6">
              ...
              {selectedCase.deadline && (<div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-md">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-lg border p-4">
              ...
              <div className="space-y-3">
                {analysisTasks.slice(0, 5).map((task) => (<div key={task.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      {task.status === 'in_progress' && (<div className="w-16 h-1 bg-gray-200 rounded-full mt-1">
            ...
            <div className="bg-white rounded-lg border p-4">
              ...
              <div className="space-y-3">
                {evidenceItems.slice(0, 5).map((evidence) => (<div key={evidence.id} className="p-3 border rounded-lg">
                    ...
                    <div className="text-xs text-gray-600">
                      Last action:{' '}
                      {evidence.chainOfCustody[evidence.chainOfCustody.length - 1]?.action}{' '}
                      by{' '}
      ...
      {/* Evidence View */}
      {activeView === 'evidence' && (<div className="space-y-4">
          {/* Filters */}
          <div className="flex gap-4">
            <select value={evidenceFilter} onChange={(e) => setEvidenceFilter(e.target.value)} className="px-3 py-2 border rounded-md text-sm">
          ...
          <div className="space-y-4">
            {filteredEvidenceItems.map((evidence) => (<div key={evidence.id} onClick={() => setSelectedEvidence(evidence)} className="bg-white rounded-lg border p-4 cursor-pointer hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {evidence.metadata.legalHold && (<span className="px-2 py-1 bg-red-100 text-red-700 text-xs rounded">
                        LEGAL HOLD
                      </span>)}
                  </div>
                </div>
                ...
                {evidence.analysis.findings.length > 0 && (<div className="mt-3 text-sm">
      ...
      {/* Timeline View */}
      {activeView === 'timeline' && (<div className="space-y-4">
          {/* Timeline Filters */}
          <div className="flex gap-4">
            <select value={timelineFilter} onChange={(e) => setTimelineFilter(e.target.value)} className="px-3 py-2 border rounded-md text-sm">
          ...
          <div className="space-y-2">
            {filteredTimelineEvents.slice(0, 50).map((event) => (<div key={event.id} className={`bg-white rounded-lg border p-4 ${event.flagged ? 'border-l-4 border-l-red-500' : ''}`}>
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-1 text-xs rounded ${getStatusColor(event.relevance)}`}>
                      {event.relevance.toUpperCase()}
                    </span>
                    {event.flagged && (<span className="px-2 py-1 bg-red-100 text-red-700 text-xs rounded">
                        🚩 FLAGGED
                      </span>)}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs text-gray-600">
                  ...
                  {event.metadata.user && (<div>
                      <span>User:</span>{' '}
                      <span className="font-medium">{event.metadata.user}</span>
                    </div>)}
                </div>

                {event.metadata.path && (<div className="mt-2 text-xs text-gray-600">
                    <span>Path:</span>{' '}
      ...
      {activeView === 'analysis' && (<div className="space-y-6">
          <div className="bg-white rounded-lg border p-6">
            ...
            <div className="space-y-4">
              {analysisTasks.map((task) => (<div key={task.id} className="p-4 border rounded-lg">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                  ...
                  {task.status === 'in_progress' && (<div className="mb-3">
                      ...
                      {task.estimatedCompletion && (<div className="text-xs text-gray-500 mt-1">
                          ETA: {task.estimatedCompletion.toLocaleTimeString()}
                        </div>)}
                    </div>)}

                  {task.status === 'completed' && task.findings && (<div className="text-sm text-green-600">
                  ...
                  <div className="flex gap-2">
                    ...
                    {task.status === 'in_progress' && (<button className="px-3 py-1 bg-red-100 text-red-700 text-xs rounded hover:bg-red-200">
                        Cancel
                      </button>)}
                  </div>
                </div>))}
      ...
      {/* Tools View */}
      {activeView === 'tools' && (<div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {availableTools.map((tool) => (<div key={tool.id} className="bg-white rounded-lg border p-4">
                <div className="flex items-start justify-between mb-3">
                  <h4 className="font-semibold">{tool.name}</h4>
                  <span className={`px-2 py-1 text-xs rounded ${tool.isInstalled
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {tool.supportedFormats.map((format) => (<span key={format} className="px-2 py-1 bg-gray-100 text-xs rounded">
                        {format}
                      </span>))}
                  </div>
                </div>

                <button className={`w-full px-4 py-2 text-sm rounded ${tool.isInstalled
      ...
      {/* Reports View */}
      {activeView === 'reports' && (<div className="space-y-6">
          <div className="bg-white rounded-lg border p-6">
            ...
            ].map((reportType) => (<button key={reportType} className="p-4 border rounded-lg text-left hover:shadow-md transition-shadow">
            ...
            <div className="border-t pt-4">
              ...
              {selectedCase?.reports.map((report) => (<div key={report.id} className="flex items-center justify-between p-3 border rounded-lg mb-2">
                  <div>
...
exports.default = DigitalForensicsWorkbench;

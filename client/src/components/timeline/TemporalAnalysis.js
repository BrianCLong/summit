...
const TemporalAnalysis = ({ events, width = 1000, height = 400, timeRange, onEventClick, onTimeRangeChange, showClusters = true, showAnomalies = true, filters, className = '', }) => {
    const canvasRef = (0, react_1.useRef)(null);
    const [selectedEvents, setSelectedEvents] = (0, react_1.useState)(new Set());
    const [hoveredEvent, setHoveredEvent] = (0, react_1.useState)(null);
    const [zoomLevel, setZoomLevel] = (0, react_1.useState)(1);
    const [viewportStart, setViewportStart] = (0, react_1.useState)(0);
    const [isDragging, setIsDragging] = (0, react_1.useState)(false);
    const [dragStart, setDragStart] = (0, react_1.useState)({ x: 0, time: 0 });
    ...
    const filteredEvents = (0, react_1.useMemo)(() => {
        ...
        if (filters) {
            ...
            if (filters.entities?.length) {
                filtered = filtered.filter((e) => e.entities.some((entity) => filters.entities.includes(entity)));
            }
    ...
    const timeBounds = (0, react_1.useMemo)(() => {
    ...
    const effectiveTimeRange = (0, react_1.useMemo)(() => {
        if (timeRange) {
    ...
    const eventClusters = (0, react_1.useMemo)(() => {
        if (!showClusters)
            return [];
        const clusters = [];
    ...
    const anomalies = (0, react_1.useMemo)(() => {
        if (!showAnomalies)
            return [];
        ...
        filteredEvents.forEach((event) => {
            ...
            const eventsInWindow = filteredEvents.filter((e) => e.timestamp >= windowStart &&
                e.timestamp <= windowEnd &&
                e.id !== event.id);
            const highSeverityInWindow = eventsInWindow.filter((e) => e.severity === 'high' || e.severity === 'critical');
    ...
    const handleMouseMove = (event) => {
        ...
        if (isDragging) {
            ...
            const newViewportStart = Math.max(timeBounds.min, Math.min(timeBounds.max - (viewportEnd - viewportStart), viewportStart + timeDiff));
            ...
            eventClusters.forEach((cluster) => {
                ...
                if (startX < width && endX > 0) {
                    ctx.fillStyle = cluster.anomaly
                        ? 'rgba(239, 68, 68, 0.1)'
                        : 'rgba(59, 130, 246, 0.1)';
                    ctx.fillRect(Math.max(0, startX), 50, Math.min(width, endX) - Math.max(0, startX), axisY - 50);
                    // Cluster label
    ...
    return (<div className={`temporal-analysis ${className}`}>
      {/* Controls */}
      <div className="flex items-center justify-between mb-4 p-3 bg-gray-50 rounded-lg">
        <div className="flex items-center gap-4">
          ...
          {eventClusters.length > 0 && (<span className="text-sm text-blue-600">
              {eventClusters.length} clusters detected
            </span>)}
          {anomalies.length > 0 && (<span className="text-sm text-red-600">
              {anomalies.length} anomalies detected
            </span>)}
        </div>

        <div className="flex gap-2">
          <button onClick={() => setZoomLevel((prev) => Math.min(10, prev * 1.5))} className="px-3 py-1 bg-white border rounded hover:bg-gray-50 text-sm">
            🔍+
          </button>
          <button onClick={() => setZoomLevel((prev) => Math.max(0.1, prev / 1.5))} className="px-3 py-1 bg-white border rounded hover:bg-gray-50 text-sm">
            🔍-
          </button>
          <button onClick={() => {
            setViewportStart(timeBounds.min);
      ...
      <div className="relative">
        <canvas ref={canvasRef} width={width} height={height} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onWheel={handleWheel} className="border rounded-lg cursor-crosshair" style={{ backgroundColor: 'white' }}/>

        {/* Event Details Tooltip */}
        {hoveredEvent && (<div className="absolute top-2 right-2 bg-black text-white rounded-lg p-3 text-sm max-w-xs z-10">
            ...
            {hoveredEvent.entities.length > 0 && (<div className="text-xs opacity-75 mt-1">
...
exports.default = TemporalAnalysis;

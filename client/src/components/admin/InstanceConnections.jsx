...
export default function InstanceConnections() {
  ...
  const remove = async (id) => {
    await FederationAPI.remove(id);
    await load();
  };
  const test = async (id) => {
    await FederationAPI.test(id);
    await load();
  };
  const update = async (inst, updates) => {
    await FederationAPI.update(inst.id, updates);
    await load();
  };
                  ...
                  <Grid container spacing={1}>
                    ...
                    <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        type="number"
                        fullWidth
                        defaultValue={inst.maxConcurrentQueries || 5}
                        onBlur={(e) =>
                          update(inst, { maxConcurrentQueries: Number(e.target.value) || 5 })
                        }
                      />
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        type="number"
                        fullWidth
                        defaultValue={inst.timeout || 30000}
                        onBlur={(e) => update(inst, { timeout: Number(e.target.value) || 30000 })}
                      />
                    </Grid>
                    <Grid item xs={12}>
                      <Stack direction="row" spacing={1}>
                        <Button
                          ...
                          variant="outlined"
                          onClick={async () => {
                            const d = await FederationAPI.get(inst.id);
                            setStatusMap((s) => ({ ...s, [inst.id]: d }));
                          }}
                        >
                          Status
                        </Button>

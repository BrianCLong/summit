...
exports.UsageAndQuotas = void 0;
const react_1 = __importDefault(require("react"));
const client_1 = require("@apollo/client");
const jquery_1 = __importDefault(require("jquery"));
// GraphQL Queries and Mutations
const GET_USAGE_DATA = (0, client_1.gql) `
...
`;
const SIMULATE_INVOICE_MUTATION = (0, client_1.gql) `
 ...
 */
const UsageAndQuotas = ({ tenantId, }) => {
    const { data, loading, error, refetch } = (0, client_1.useQuery)(GET_USAGE_DATA, {
        variables: { tenantId, w: '30d' }, // Default to last 30 days usage
    });
    const [simulateInvoice, { loading: simulatingInvoice }] = (0, client_1.useMutation)(SIMULATE_INVOICE_MUTATION);
    // jQuery wiring for the simulate invoice button
    react_1.default.useEffect(() => {
        const btnInvoice = (0, jquery_1.default)('#btnInvoice');
        const invoiceOut = (0, jquery_1.default)('#invoiceOut');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          {tenantPlan?.overrides &&
            Object.keys(tenantPlan.overrides).length > 0 && (<div className="text-sm text-gray-600">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <ul className="list-disc list-inside text-gray-600">
            {usageSummary?.length > 0 ? (usageSummary.map((u) => (<li key={u.feature} className="text-sm">
                  <span className="font-semibold">{u.feature}</span>: {u.total}
                </li>))) : (<p>No usage recorded for the last 30 days.</p>)}
          </ul>
        </div>
      </div>
...
exports.UsageAndQuotas = UsageAndQuotas;

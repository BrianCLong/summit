...
require("./IntelligenceFeedsEnrichment.css");
const IntelligenceFeedsEnrichment = ({ investigationId, onNewIntelligence, onEnrichmentComplete, onAlert, onCorrelationMatch, }) => {
    // State Management
    const [feeds, setFeeds] = (0, react_1.useState)([]);
    const [enrichmentRules, setEnrichmentRules] = (0, react_1.useState)([]);
    const [enrichedEntities, setEnrichedEntities] = (0, react_1.useState)([]);
    const [alerts, setAlerts] = (0, react_1.useState)([]);
    const [correlations, setCorrelations] = (0, react_1.useState)([]);
    const [selectedFeed, setSelectedFeed] = (0, react_1.useState)(null);
    ...
    const generateMockData = (0, react_1.useCallback)(() => {
        const mockFeeds = [
            {
                ...
                description: 'Real-time threat intelligence from VirusTotal including file hashes, URLs, and IP addresses',
                provider: 'VirusTotal',
                type: 'commercial',
            ...
            },
            {
                id: 'feed-003',
                name: 'AlienVault OTX',
                description: 'Open Threat Exchange providing IOCs and threat intelligence from global security community',
        ...
        const mockEnrichmentRules = [
            {
                id: 'rule-001',
                name: 'IP Address Geolocation & Reputation',
                description: 'Enrich IP addresses with geolocation data and reputation scores from multiple sources',
                enabled: true,
                priority: 8,
                triggers: [
            ...
            },
            {
                ...
                description: 'Comprehensive domain analysis including DNS, SSL certificates, and threat intelligence',
                enabled: true,
                priority: 7,
        ...
        const mockAlerts = [
            {
                ...
                description: 'IP address 185.225.19.42 with reputation score -75 detected in firewall logs. This IP is associated with APT29 campaigns and botnet activity.',
            ...
            },
            {
                ...
                description: 'Domain covidinfo-gov.com exhibits suspicious characteristics including recent registration and phishing classification.',
        ...
        const mockCorrelations = [
            {
                ...
                description: 'Correlate IP addresses appearing in multiple threat intelligence feeds within 24 hours',
                enabled: true,
                rules: [
                    {
    ...
    // Filtered data
    const filteredFeeds = (0, react_1.useMemo)(() => {
    ...
    const filteredAlerts = (0, react_1.useMemo)(() => {
    ...
    return (<div className="intelligence-feeds-enrichment">
      {/* Header */}
      <div className="ife-header">
        ...
        <div className="header-controls">
          <input type="text" placeholder="Search feeds, entities, and alerts..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="search-input"/>
          ...
          <button className={`realtime-toggle ${realTimeMode ? 'active' : ''}`} onClick={() => setRealTimeMode(!realTimeMode)}>
      ...
      <div className="ife-tabs">
        ...
        <button className={`tab-button ${activeTab === 'feeds' ? 'active' : ''}`} onClick={() => setActiveTab('feeds')}>
          📡 Feeds
        </button>
        <button className={`tab-button ${activeTab === 'enrichment' ? 'active' : ''}`} onClick={() => setActiveTab('enrichment')}>
          ⚡ Enrichment
        </button>
        <button className={`tab-button ${activeTab === 'entities' ? 'active' : ''}`} onClick={() => setActiveTab('entities')}>
          🎯 Entities
        </button>
        <button className={`tab-button ${activeTab === 'alerts' ? 'active' : ''}`} onClick={() => setActiveTab('alerts')}>
          🚨 Alerts
        </button>
        <button className={`tab-button ${activeTab === 'correlations' ? 'active' : ''}`} onClick={() => setActiveTab('correlations')}>
          🔗 Correlations
        </button>
      </div>
      ...
      <div className="ife-content">
            ...
            <div className="dashboard-overview">
                ...
                .toLocaleString()}
                  ...
                  <div className="metric-card">
                    ...
                    <div className="metric-content">
                      ...
                      <div className="metric-detail">
                        Avg confidence:{' '}
                        {Math.round(enrichedEntities.reduce((acc, e) => acc + e.confidence, 0) / enrichedEntities.length)}
                        %
                      </div>
                    </div>
                  </div>
              ...
              <div className="performance-section">
                ...
                <div className="feeds-performance">
                  {feeds.map((feed) => (<div key={feed.id} className="feed-performance-card">
                      ...
                      <div className="performance-metrics">
                        <div className="performance-metric">
                          ...
                          <div className="metric-bar">
                            <div className="metric-fill success" style={{ width: `${feed.metrics.successRate}%` }}></div>
                          </div>
        ...
        {/* Feeds Tab */}
        {activeTab === 'feeds' && (<div className="feeds-tab">
            ...
            <div className="feeds-grid">
              {filteredFeeds.map((feed) => (<div key={feed.id} className={`feed-card ${selectedFeed?.id === feed.id ? 'selected' : ''}`} onClick={() => setSelectedFeed(feed)}>
                  ...
                  <div className="feed-confidence">
                    ...
                    <div className="confidence-bar">
                      <div className="confidence-fill" style={{ width: `${feed.confidence}%` }}></div>
                    </div>
                  ...
                  <div className="feed-tags">
                    {feed.tags.slice(0, 3).map((tag) => (<span key={tag} className="feed-tag">
                        {tag}
                      </span>))}
                  </div>

                  {feed.alerts.length > 0 && (<div className="feed-alerts">
                      <strong>
        ...
        {/* Enrichment Tab */}
        {activeTab === 'enrichment' && (<div className="enrichment-tab">
            <div className="tab-header">
            ...
            <div className="enrichment-grid">
              {enrichmentRules.map((rule) => (<div key={rule.id} className="enrichment-card">
                  ...
                  <div className="enrichment-sources">
                    ...
                    <div className="sources-list">
                      {rule.enrichments.map((enrichment, index) => (<span key={index} className={`source-tag ${enrichment.type}`}>
                          {enrichment.provider}
                        </span>))}
                    </div>
                  </div>
                  ...
                  <div className="enrichment-triggers">
                    ...
                    {rule.triggers.map((trigger, index) => (<div key={index} className="trigger-item">
                        {trigger.field} on {trigger.condition.replace('_', ' ')}
                      </div>))}
                  </div>

                  {rule.metrics.lastRun && (<div className="enrichment-timing">
                      <div className="timing-item">
                        <strong>Last Run:</strong>{' '}
                        {rule.metrics.lastRun.toLocaleString()}
                      </div>
                      {rule.metrics.nextRun && (<div className="timing-item">
        ...
        {/* Entities Tab */}
        {activeTab === 'entities' && (<div className="entities-tab">
            <div className="tab-header">
            ...
            <div className="entities-grid">
              {enrichedEntities.map((entity) => (<div key={entity.id} className={`entity-card ${selectedEntity?.id === entity.id ? 'selected' : ''}`} onClick={() => setSelectedEntity(entity)}>
                  <div className="entity-header">
                    <div className="entity-scores">
                      <span className={`confidence-score ${entity.confidence > 80 ? 'high' : entity.confidence > 60 ? 'medium' : 'low'}`}>
                        Confidence: {entity.confidence}%
                      </span>
                      <span className={`risk-score ${entity.riskScore > 70 ? 'high' : entity.riskScore > 40 ? 'medium' : 'low'}`}>
                  ...
                  <div className="entity-original">
                    ...
                    <div className="data-preview">
                      {Object.entries(entity.original).map(([key, value]) => (<div key={key} className="data-item">
                  ...
                  <div className="entity-enrichment-summary">
                    ...
                    <div className="sources-grid">
                      {entity.enrichmentSources.map((source, index) => (<div key={index} className={`source-item ${source.status}`}>
                  ...
                  <div className="entity-enriched-preview">
                    ...
                    .slice(0, 3)
                    .map(([key, value]) => (<div key={key} className="enriched-item">
                  ...
                  <div className="entity-tags">
                    {entity.tags.map((tag) => (<span key={tag} className="entity-tag">
                        {tag}
                      </span>))}
                  </div>
                </div>))}
            ...
            {selectedEntity && (<div className="entity-details-panel">
                ...
                <div className="panel-content">
                  <div className="entity-scores-detail">
                    <div className="score-item">
                      ...
                      <div className="score-bar">
                        <div className="score-fill confidence" style={{ width: `${selectedEntity.confidence}%` }}></div>
                      </div>
                    ...
                    <div className="score-item">
                      ...
                      <div className="score-bar">
                        <div className="score-fill risk" style={{ width: `${selectedEntity.riskScore}%` }}></div>
                      </div>
                      <span className="score-value">
                  ...
                  <div className="detail-section">
                    ...
                    <div className="sources-detail">
                      {selectedEntity.enrichmentSources.map((source, index) => (<div key={index} className={`source-detail ${source.status}`}>
        ...
        {/* Alerts Tab */}
        {activeTab === 'alerts' && (<div className="alerts-tab">
            <div className="tab-header">
            ...
            <div className="alerts-list">
              {filteredAlerts.map((alert) => (<div key={alert.id} className={`alert-card ${alert.severity} ${alert.acknowledged ? 'acknowledged' : ''}`}>
                  <div className="alert-header">
                    ...
                    <div className="alert-indicators">
                      ...
                      {alert.acknowledged && (<span className="acknowledged-indicator">
                          Acknowledged
                        </span>)}
                    </div>
                  </div>
                  ...
                  {alert.recommendations.length > 0 && (<div className="alert-recommendations">
                      <strong>Recommended Actions:</strong>
                      <div className="recommendations-list">
                        {alert.recommendations.map((rec, index) => (<div key={index} className={`recommendation ${rec.priority}`}>
                  ...
                  {alert.assignedTo && (<div className="alert-assignment">
                      <strong>Assigned to:</strong> {alert.assignedTo}
                    </div>)}

                  <div className="alert-actions">
                    {!alert.acknowledged && (<button className="action-button primary">
                        ✓ Acknowledge
                      </button>)}
        ...
        {/* Correlations Tab */}
        {activeTab === 'correlations' && (<div className="correlations-tab">
            <div className="tab-header">
            ...
            <div className="correlations-grid">
              {correlations.map((correlation) => (<div key={correlation.id} className="correlation-card">
                  <div className="correlation-header">
                    <div className="correlation-meta">
                      <span className={`status-indicator ${correlation.enabled ? 'enabled' : 'disabled'}`}>
                  ...
                  <div className="correlation-rules">
                    ...
                    {correlation.rules.map((rule, index) => (<div key={index} className="rule-summary">
                  ...
                  <div className="correlation-metrics">
                    ...
                    {correlation.metrics.lastMatch && (<div className="metric-item">
                  ...
                  <div className="correlation-actions">
                    <div className="action-item">
                      ...
                      {correlation.actions.map((action, index) => (<span key={index} className="action-tag">
                  ...
                  <div className="correlation-controls">
                    ...
                    <button className={`action-button ${correlation.enabled ? 'disable' : 'enable'}`}>
...
exports.default = IntelligenceFeedsEnrichment;

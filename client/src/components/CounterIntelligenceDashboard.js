...
exports.CounterIntelligenceDashboard = CounterIntelligenceDashboard;
...
// GraphQL Queries
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const GET_CI_OVERVIEW = (0, client_1.gql) `
...
`;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const GET_ANOMALIES = (0, client_1.gql) `
...
`;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const GET_THREAT_ACTORS = (0, client_1.gql) `
...
`;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const GET_NARRATIVES = (0, client_1.gql) `
...
`;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const ACKNOWLEDGE_ALERT = (0, client_1.gql) `
...
const StatCard = ({ title, value, icon, trend, color = '#1976d2', onClick }) => (<div className="stat-card" onClick={onClick} style={{ cursor: onClick ? 'pointer' : 'default' }} role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined}>
...
const AlertItem = ({ alert, onAcknowledge }) => (<div className="alert-item" style={{ borderLeftColor: severityColors[alert.severity] }}>
    ...
    <div className="alert-actions">
      {alert.status === 'NEW' && (<button className="btn-acknowledge" onClick={() => onAcknowledge(alert.id)}>
          Acknowledge
        </button>)}
      <button className="btn-investigate">Investigate</button>
    </div>
  </div>);
const ThreatActorCard = ({ actor, onClick }) => (<div className="threat-actor-card" onClick={() => onClick(actor.id)}>
    ...
    <div className="actor-capabilities">
      ...
      <div className="domains">
        {actor.capabilities.domains.slice(0, 3).map((domain, i) => (<span key={i} className="domain-tag">
            {domain.replace(/_/g, ' ')}
          </span>))}
      </div>
    </div>
...
const NarrativeCard = ({ narrative, onClick }) => {
    ...
    return (<div className="narrative-card" onClick={() => onClick(narrative.id)}>
      <div className="narrative-header">
        <span className="narrative-status" style={{
            backgroundColor: narrative.status === 'VIRAL' ? '#ffebee' : '#e3f2fd',
            color: narrative.status === 'VIRAL' ? '#d32f2f' : '#1976d2',
        }}>
          {narrative.status}
        </span>
        <span className="narrative-harm" style={{ color: harmColors[narrative.impactAssessment.potentialHarm] }}>
      ...
      <div className="narrative-themes">
        {narrative.themes.slice(0, 3).map((theme, i) => (<span key={i} className="theme-tag">{theme}</span>))}
      </div>
...
const AnomalyTimeline = ({ anomalies, onSelect }) => (<div className="anomaly-timeline">
    {anomalies.map((anomaly) => (<div key={anomaly.id} className="timeline-item" onClick={() => onSelect(anomaly)} style={{ borderLeftColor: severityColors[anomaly.severity] }}>
        ...
        <div className="timeline-meta">
          ...
          {anomaly.correlations.length > 0 && (<span className="correlation-badge">
...
function CounterIntelligenceDashboard() {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    ...
    const handleAcknowledgeAlert = (0, react_1.useCallback)((alertId) => {
        // eslint-disable-next-line no-console
        console.log('Acknowledging alert:', alertId);
        // Would call mutation
    }, []);
    const handleSelectThreatActor = (0, react_1.useCallback)((actorId) => {
        // eslint-disable-next-line no-console
        console.log('Selected threat actor:', actorId);
        // Would navigate to detail view
    }, []);
    const handleSelectNarrative = (0, react_1.useCallback)((narrativeId) => {
        // eslint-disable-next-line no-console
        console.log('Selected narrative:', narrativeId);
        // Would navigate to detail view
    }, []);
    const handleSelectAnomaly = (0, react_1.useCallback)((anomaly) => {
        setSelectedAnomaly(anomaly);
    }, []);
    return (<div className="ci-dashboard">
      <style>{`
        .ci-dashboard {
      ...
      <div className="stats-grid">
        <StatCard title="Total Anomalies" value={mockOverview.anomalyCount} icon="📊" trend="up" color="#1976d2"/>
        <StatCard title="Critical Alerts" value={mockOverview.criticalAlerts} icon="🚨" color="#d32f2f"/>
        <StatCard title="High Risk Entities" value={mockOverview.highRiskEntities} icon="⚠️" trend="up" color="#f57c00"/>
        <StatCard title="Active Investigations" value={mockOverview.activeInvestigations} icon="🔍" color="#7b1fa2"/>
        <StatCard title="Threat Actors Tracked" value={mockOverview.threatActorsTracked} icon="👤" color="#455a64"/>
        <StatCard title="Narratives Monitored" value={mockOverview.narrativesMonitored} icon="📰" color="#00897b"/>
      </div>
      ...
      <nav className="tab-nav">
        ...
        <button className={`tab-btn ${activeTab === 'actors' ? 'active' : ''}`} onClick={() => setActiveTab('actors')}>
          Threat Actors
        </button>
        <button className={`tab-btn ${activeTab === 'narratives' ? 'active' : ''}`} onClick={() => setActiveTab('narratives')}>
          Narratives
        </button>
      </nav>
      ...
      <div className="main-content">
        <div className="alerts-section">
          ...
          {mockAlerts.map((alert) => (<AlertItem key={alert.id} alert={alert} onAcknowledge={handleAcknowledgeAlert}/>))}
        </div>
        ...
        <div className="sidebar">
          <div className="actors-section">
            ...
            {mockThreatActors.map((actor) => (<ThreatActorCard key={actor.id} actor={actor} onClick={handleSelectThreatActor}/>))}
          </div>
          ...
          <div className="narratives-section">
            ...
            {mockNarratives.map((narrative) => (<NarrativeCard key={narrative.id} narrative={narrative} onClick={handleSelectNarrative}/>))}
          </div>
        </div>
      </div>

      {activeTab === 'anomalies' && (<div className="anomalies-section" style={{ marginTop: 24 }}>
          <h2 className="section-title">📊 Anomaly Timeline</h2>
          <AnomalyTimeline anomalies={mockAnomalies} onSelect={handleSelectAnomaly}/>
        </div>)}
    </div>);
}
exports.default = CounterIntelligenceDashboard;

...
const CollaborativeWorkspace = ({ workspaceId, investigationId, currentUser, onUserJoin, onUserLeave, onWorkspaceShare, onAnnotationCreate, className = '', }) => {
    const [workspace, setWorkspace] = (0, react_1.useState)(null);
    const [activeUsers, setActiveUsers] = (0, react_1.useState)([]);
    const [collaborationEvents, setCollaborationEvents] = (0, react_1.useState)([]);
    const [annotations, setAnnotations] = (0, react_1.useState)([]);
    const [showParticipants, setShowParticipants] = (0, react_1.useState)(false);
    const [showAnnotations, setShowAnnotations] = (0, react_1.useState)(true);
    const [isRecording, setIsRecording] = (0, react_1.useState)(false);
    const [newAnnotation, setNewAnnotation] = (0, react_1.useState)(null);
    const [cursorPositions, setCursorPositions] = (0, react_1.useState)({});
    const workspaceRef = (0, react_1.useRef)(null);
    const wsRef = (0, react_1.useRef)(null);
    // Mock workspace data
    const mockWorkspace = (0, react_1.useMemo)(() => ({
    ...
    const mockAnnotations = (0, react_1.useMemo)(() => [
        {
            id: 'annotation-001',
            userId: 'user-002',
            type: 'comment',
    ...
    (0, react_1.useEffect)(() => {
        ...
        const interval = setInterval(() => {
            const event = {
                ...
                userId: mockWorkspace.participants[Math.floor(Math.random() * mockWorkspace.participants.length)].id,
                timestamp: Date.now(),
    ...
    const handleAnnotationStart = (0, react_1.useCallback)((e, type) => {
        if (!workspaceRef.current)
            return;
        const rect = workspaceRef.current.getBoundingClientRect();
        const x = e.clientX - rect.left;
    ...
    const handleAnnotationComplete = (0, react_1.useCallback)((content) => {
        if (!newAnnotation)
            return;
        const annotation = {
            ...newAnnotation,
    ...
    const getUserName = (userId) => {
        return (workspace?.participants.find((u) => u.id === userId)?.name ||
            'Unknown User');
    };
    ...
    return (<div className={`collaborative-workspace ${className}`} style={{ position: 'relative', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            {activeUsers.slice(0, 4).map((user) => (<div key={user.id} style={{
                width: '24px',
                height: '24px',
                borderRadius: '50%',
            ...
            {activeUsers.length > 4 && (<div style={{
                width: '24px',
                height: '24px',
                borderRadius: '50%',
      ...
      <div ref={workspaceRef} onMouseMove={handleMouseMove} style={{
            position: 'relative',
        ...
        {showAnnotations &&
            annotations
                ...
                .map((annotation) => (<div key={annotation.id} style={{
                    position: 'absolute',
                ...
                {annotation.type === 'comment' && (<div style={{
                ...
                {annotation.type === 'highlight' && (<div style={{
                ...
                {annotation.type === 'arrow' && (<div style={{
                        width: '0',
                        height: '0',
        ...
        {showParticipants && (<div style={{
            ...
            <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
              {workspace?.participants.map((user) => (<div key={user.id} style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: '12px',
                    padding: '12px 16px',
        ...
        <div style={{
          ...
          {['comment', 'highlight', 'arrow'].map((type) => (<button key={type} onClick={(e) => handleAnnotationStart(e, type)} style={{
                padding: '8px',
                fontSize: '16px',
        ...
        <div style={{
          ...
          <div>
            {collaborationEvents.slice(-5).map((event) => (<div key={event.id} style={{
                padding: '6px 12px',
                fontSize: '11px',
                borderBottom: '1px solid #f0f0f0',
                display: 'flex',
...
exports.default = CollaborativeWorkspace;

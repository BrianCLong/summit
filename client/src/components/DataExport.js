...
const EXPORT_DATA = (0, client_1.gql) `
...
`;
const GENERATE_REPORT = (0, client_1.gql) `
...
`;
function DataExport({ investigationId, selectedEntities = [], onExportComplete, showReports = true, }) {
    const [exportFormat, setExportFormat] = (0, react_1.useState)('json');
    const [reportTemplate, setReportTemplate] = (0, react_1.useState)('executive');
    ...
    const [exportHistory, setExportHistory] = (0, react_1.useState)([]);
    const [exportData, { loading: exportLoading }] = (0, client_1.useMutation)(EXPORT_DATA);
    const [generateReport, { loading: reportLoading }] = (0, client_1.useMutation)(GENERATE_REPORT);
    ...
    return (<div className="panel" style={{ padding: '24px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{
          ...
          {[
            ...
            { value: 'cypher', label: 'Cypher', desc: 'Neo4j queries' },
        ].map((format) => (<label key={format.value} style={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
                padding: '12px',
                border: exportFormat === format.value
                    ? '2px solid #1a73e8'
              ...
              <input type="radio" name="exportFormat" value={format.value} checked={exportFormat === format.value} 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        onChange={(e) => setExportFormat(e.target.value)} style={{ display: 'none' }}/>
              <div style={{
                fontWeight: '600',
                fontSize: '14px',
                marginBottom: '4px',
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{
          ...
          <label style={{
            ...
            <input type="checkbox" checked={exportOptions.includeRelationships} onChange={(e) => setExportOptions((prev) => ({
            ...prev,
          ...
          <label style={{
            ...
            <input type="checkbox" checked={exportOptions.includeMetadata} onChange={(e) => setExportOptions((prev) => ({
            ...prev,
          ...
          <label style={{
            ...
            <input type="checkbox" checked={exportOptions.includeAnalytics} onChange={(e) => setExportOptions((prev) => ({
            ...prev,
        ...
        {showAdvanced && (<div style={{
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <input type="date" value={exportOptions.dateRange?.[0] || ''} onChange={(e) => setExportOptions((prev) => ({
                ...prev,
                ...
                <span>to</span>
                <input type="date" value={exportOptions.dateRange?.[1] || ''} onChange={(e) => setExportOptions((prev) => ({
                ...prev,
            ...
            <div>
              ...
              <input type="text" value={exportOptions.investigationId || ''} onChange={(e) => setExportOptions((prev) => ({
                ...prev,
                investigationId: e.target.value,
            }))} placeholder="Enter investigation ID..." style={{
                width: '100%',
                padding: '8px',
                border: '1px solid var(--hairline)',
                borderRadius: '4px',
      ...
      {/* Report Generation */}
      {showReports && (<div style={{ marginBottom: '24px' }}>
          <label style={{
                fontSize: '14px',
                fontWeight: '600',
                marginBottom: '12px',
          ...
          <div style={{
            ...
            <select value={reportTemplate} 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        onChange={(e) => setReportTemplate(e.target.value)} style={{
                flex: 1,
                padding: '8px',
                border: '1px solid var(--hairline)',
                borderRadius: '4px',
            ...
            <button onClick={handleReportGeneration} disabled={reportLoading} style={{
                padding: '8px 16px',
      ...
      {exportHistory.length > 0 && (<div>
          ...
          <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
            {exportHistory.map((item, index) => (<div key={item.id || index} style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                    alignItems: 'center',
                    padding: '12px',
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  {item.downloadUrl && (<button onClick={() => window.open(item.downloadUrl, '_blank')} style={{
                        padding: '4px 8px',

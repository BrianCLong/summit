...
const IncidentForensicsDashboard: React.FC<IncidentForensicsDashboardProps> = ({
  ...
  const navigationTabs = useMemo(() => {
    ...
    const activeIncidents = incidents.reduce(
      (count, i) => count + (i.status === "active" ? 1 : 0),
      0
    );
    const activePlaybooks = playbooks.reduce(
      (count, p) => count + (p.status === "running" ? 1 : 0),
      0
    );
    const activeWarRooms = warRooms.reduce(
      (count, w) => count + (w.status === "active" ? 1 : 0),
      0
    );

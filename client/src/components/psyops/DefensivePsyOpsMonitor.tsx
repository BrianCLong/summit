...
export const DefensivePsyOpsMonitor: React.FC = () => {
  ...
  const fetchDefensiveData = useCallback(async () => {
    try {
      ...
      const criticalThreats = threatsData.filter((t: PsyOpsThreat) => t.threatLevel === 'CRITICAL').length;
  ...
  const deployDefensiveMeasure = async (threatId: string, measureType: string) => {
    try {
  ...
  const getAlertLevelColor = (level: string) => {
    switch (level) {
      ...
      case 'ELEVATED': return 'border-yellow-500 bg-yellow-50';
    ...
    <div className="space-y-6 p-6">
      ...
      <Alert className={getAlertLevelColor(alertLevel)}>
        ...
        <AlertDescription>
          ...
          {alertLevel === 'CRITICAL' && ' - Critical psychological threats detected. All defensive measures activated.'}
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{Math.round(metrics.effectivenessScore * 100)}%</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {protectiveActions.length === 0 ? (
              <p className="text-gray-600 text-center py-4">No defensive measures currently deployed</p>
            ) : (
                ...
                <div key={action.id} className="flex items-center justify-between p-3 border rounded">
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="text-right">
                      <p className="text-sm">Effectiveness: {Math.round(action.effectiveness * 100)}%</p>

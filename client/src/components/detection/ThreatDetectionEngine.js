...
const ThreatDetectionEngine = ({ investigationId, onAlertCreate = () => { }, onAlertUpdate = () => { }, onIncidentCreate = () => { }, onIncidentUpdate = () => { }, onContainmentAction = () => { }, autoEscalate = true, className = '', }) => {
    ...
    const [isDetectionRunning, setIsDetectionRunning] = (0, react_1.useState)(true);
        ...
        const mockAlerts = Array.from({ length: 25 }, (_, i) => {
            ...
            return {
                ...
                indicators: [
                    ...
                    {
                        type: 'process',
                        value: ['powershell.exe', 'cmd.exe', 'wmic.exe', 'psexec.exe'][Math.floor(Math.random() * 4)],
                ...
                mitreTechniques: [
                    ['T1059.001', 'T1021.001', 'T1003.001'][Math.floor(Math.random() * 3)],
                ],
                notes: [],
            };
        });
    ...
    const generateMockIncidents = () => {
        ...
        const mockIncidents = Array.from({ length: 8 }, (_, i) => {
            ...
            return {
                ...
                impactAssessment: {
                    confidentiality: ['none', 'low', 'medium', 'high'][Math.floor(Math.random() * 4)],
                    integrity: ['none', 'low', 'medium', 'high'][Math.floor(Math.random() * 4)],
                    availability: ['none', 'low', 'medium', 'high'][Math.floor(Math.random() * 4)],
    ...
    const generateRandomAlert = () => {
        ...
        return {
            id: `alert-${Date.now()}`,
            signatureId: signatures[Math.floor(Math.random() * signatures.length)]?.id ||
                'sig-001',
    ...
    const assignAlert = (alertId, analyst) => {
        setAlerts((prev) => prev.map((alert) => alert.id === alertId ? { ...alert, assignedAnalyst: analyst } : alert));
    };
    ...
    return (<div className={`threat-detection-engine ${className}`}>
      {/* Header */}
      <div className="mb-6 border-b pb-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${isDetectionRunning ? 'bg-green-500' : 'bg-red-500'}`}></div>
              <span className="text-sm text-gray-600">
                Detection {isDetectionRunning ? 'Running' : 'Stopped'}
              </span>
            </div>
            <button onClick={() => setIsDetectionRunning(!isDetectionRunning)} className={`px-4 py-2 rounded-md text-sm ${isDetectionRunning
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => setActiveView('alerts')} className={`px-4 py-2 rounded-md ${activeView === 'alerts' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            🚨 Alerts ({alerts.filter((a) => a.status === 'new').length})
          </button>
          <button onClick={() => setActiveView('incidents')} className={`px-4 py-2 rounded-md ${activeView === 'incidents' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            🔥 Incidents (
            {incidents.filter((i) => i.status !== 'closed').length})
          </button>
          <button onClick={() => setActiveView('signatures')} className={`px-4 py-2 rounded-md ${activeView === 'signatures' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            🎯 Signatures ({signatures.filter((s) => s.isActive).length})
          </button>
          <button onClick={() => setActiveView('response')} className={`px-4 py-2 rounded-md ${activeView === 'response' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            ⚡ Response
          </button>
        </div>
      </div>
      ...
      {/* Dashboard View */}
      {activeView === 'dashboard' && metrics && (<div className="space-y-6">
          {/* Key Metrics */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-lg border p-4">
              ...
              <div className="space-y-3">
                {metrics.topThreatTypes.map((threat, index) => (<div key={threat.type} className="flex items-center justify-between">
      ...
      {/* Alerts View */}
      {activeView === 'alerts' && (<div className="space-y-4">
          {/* Filters */}
          <div className="flex gap-4 mb-4">
            <select value={filterSeverity} onChange={(e) => setFilterSeverity(e.target.value)} className="px-3 py-2 border rounded-md text-sm">
          ...
          <div className="space-y-2">
            {filteredAlerts.map((alert) => (<div key={alert.id} onClick={() => setSelectedAlert(alert)} className="bg-white rounded-lg border p-4 cursor-pointer hover:shadow-md transition-shadow">
                ...
                <div className="flex flex-wrap gap-1">
                  {alert.indicators.map((indicator, index) => (<span key={index} className="px-2 py-1 bg-gray-100 text-xs rounded">
                      {indicator.type}: {indicator.value}
                    </span>))}
                </div>

                {alert.assignedAnalyst && (<div className="mt-2 text-xs text-blue-600">
                    Assigned to: {alert.assignedAnalyst}
                  </div>)}
              </div>))}

            {filteredAlerts.length === 0 && (<div className="text-center py-12 text-gray-500">
      ...
      {/* Incidents View */}
      {activeView === 'incidents' && (<div className="space-y-4">
          {incidents.map((incident) => (<div key={incident.id} onClick={() => setSelectedIncident(incident)} className="bg-white rounded-lg border p-4 cursor-pointer hover:shadow-md transition-shadow">
          ...
          {incidents.length === 0 && (<div className="text-center py-12 text-gray-500">
              <div className="text-4xl mb-4">🔥</div>
      ...
      {/* Signatures View */}
      {activeView === 'signatures' && (<div className="space-y-4">
          {signatures.map((signature) => (<div key={signature.id} className="bg-white rounded-lg border p-4">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <span className={`px-2 py-1 rounded text-xs font-medium ${getSeverityColor(signature.severity)}`}>
                    {signature.severity.toUpperCase()}
                  </span>
                  <span className={`px-2 py-1 rounded text-xs font-medium ${signature.isActive ? 'text-green-700 bg-green-100' : 'text-gray-700 bg-gray-100'}`}>
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {signature.mitreTactics.map((tactic) => (<span key={tactic} className="px-2 py-1 bg-gray-100 text-xs rounded">
                      {tactic}
                    </span>))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {signature.indicators.map((indicator, index) => (<span key={index} className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded">
                      {indicator}
                    </span>))}
                </div>
              </div>
      ...
      {/* Response View */}
      {activeView === 'response' && (<div className="space-y-6">
          ...
          <div className="bg-white rounded-lg border p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                'Ransomware Recovery',
            ].map((playbook) => (<div key={playbook} className="flex items-center justify-between p-3 border rounded-lg">

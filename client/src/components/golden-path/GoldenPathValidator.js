...
exports.GoldenPathValidator = void 0;
const react_1 = __importStar(require("react"));
const GoldenPathValidator = () => {
    const [validationSteps, setValidationSteps] = (0, react_1.useState)([]);
    const [isRunning, setIsRunning] = (0, react_1.useState)(false);
    const [results, setResults] = (0, react_1.useState)(null);
    // Initialize validation steps based on committee requirements
    const initializeSteps = (0, react_1.useCallback)(() => {
        const steps = [
            {
                id: 'bootstrap',
    ...
    const validateBootstrap = async () => {
        ...
        const missingVars = requiredEnvVars.filter((varName) => !process.env[varName] && !localStorage.getItem(varName));
    ...
    const validateServices = async () => {
        ...
        const serviceStatuses = await Promise.allSettled(services.map(async (service) => {
            try {
        ...
        const healthyServices = serviceStatuses.filter((result) => result.status === 'fulfilled' && result.value.status === 'healthy').length;
        ...
        return {
            ...
            service_details: serviceStatuses.map((result) => result.status === 'fulfilled' ? result.value : { error: result.reason }),
        };
    };
    ...
    const runGoldenPathValidation = async () => {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            setValidationSteps((prev) => prev.map((s) => (s.id === step.id ? { ...s, status: 'running' } : s)));
            // Execute step
      ...
      <div className="bg-white rounded-lg shadow-lg">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex justify-between items-center">
            ...
            <button onClick={runGoldenPathValidation} disabled={isRunning} className={`px-6 py-2 rounded-lg font-medium transition-colors ${isRunning
        ...
        {/* Results Summary */}
        {results && (<div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div className="grid grid-cols-4 gap-4">
              <div className="text-center">
                <div className={`text-2xl font-bold ${results.overall_status === 'success'
                ? 'text-green-600'
        ...
        <div className="px-6 py-4">
          <div className="space-y-3">
            {validationSteps.map((step, index) => (<div key={step.id} className="flex items-start space-x-4 p-4 border border-gray-200 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <div className={`text-sm font-medium ${getStatusColor(step.status)}`}>
                      ...
                      {step.duration && (<span className="ml-2 text-gray-500">
                          ({step.duration}ms)
                        </span>)}
                    </div>
                  </div>
                  ...
                  {step.error && (<div className="mt-2 p-2 bg-red-50 border border-red-200 rounded text-sm text-red-700">
                      Error: {step.error}
                    </div>)}

                  {step.details && step.status === 'success' && (<div className="mt-2 p-2 bg-green-50 border border-green-200 rounded">
...
exports.GoldenPathValidator = GoldenPathValidator;
exports.default = exports.GoldenPathValidator;

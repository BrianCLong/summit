...
const ModelManagementDashboard = ({ investigationId, onModelSelect, onDeployModel, onExperimentSelect, className = '', }) => {
    const [activeView, setActiveView] = (0, react_1.useState)('models');
    ...
    const mockModels = (0, react_1.useMemo)(() => [
        {
            id: 'model-001',
            version: 'v2.3.1',
    ...
    const mockTrainingJobs = (0, react_1.useMemo)(() => [
        {
            id: 'job-001',
            modelId: 'model-003',
            status: 'running',
    ...
    const mockExperiments = (0, react_1.useMemo)(() => [
        {
            id: 'exp-001',
    ...
    const handleModelSelect = (0, react_1.useCallback)((model) => {
        setSelectedModel(model);
        onModelSelect?.(model);
    }, [onModelSelect]);
    const handleDeploy = (0, react_1.useCallback)((modelId, environment) => {
        onDeployModel?.(modelId, environment);
        setIsLoading(true);
        setTimeout(() => setIsLoading(false), 2000);
    }, [onDeployModel]);
    return (<div className={`model-management-dashboard ${className}`} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{
          ...
          <input type="text" placeholder="Search models, tags, or algorithms..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{
            padding: '8px 12px',
      ...
      <div style={{ flex: 1, overflow: 'hidden' }}>
        {activeView === 'models' && (<div style={{
            ...
            <div style={{
              ...
              <div>
                {filteredModels.map((model) => (<div key={model.id} onClick={() => handleModelSelect(model)} style={{
                    padding: '16px',
                    ...
                    fontSize: '12px',
                      ...
                      <div>
                        <strong>F1:</strong> {model.metrics.f1Score}%
                      </div>
                      <div>
                        <strong>Latency:</strong> {model.metrics.latency}ms
                      </div>
                    </div>
                    ...
                    marginBottom: '8px',
                      ...
                      {model.tags.slice(0, 3).map((tag) => (<span key={tag} style={{
                        fontSize: '11px',
                        padding: '2px 6px',
            ...
            {selectedModel && (<div style={{
                ...
                <div style={{
                  ...
                  {selectedModel.status !== 'production' && (<button onClick={() => handleDeploy(selectedModel.id, 'production')} disabled={isLoading} style={{
                        padding: '6px 12px',
                ...
                <div style={{ padding: '16px' }}>
                  ...
                  <div style={{ marginBottom: '24px' }}>
                    ...
                    <div style={{ fontSize: '13px', lineHeight: '1.4' }}>
                      ...
                      <div style={{ marginLeft: '16px' }}>
                        {Object.entries(selectedModel.environment.frameworks).map(([name, version]) => (<div key={name}>
                            • {name}: {version}
                          </div>))}
                      </div>
                    </div>
                  ...
                  {selectedModel.endpoints && (<div style={{ marginBottom: '24px' }}>
                      <h6 style={{
                        fontSize: '14px',
                        fontWeight: '600',
                        marginBottom: '8px',
                      ...
                      <div style={{ fontSize: '13px', lineHeight: '1.4' }}>
                        {selectedModel.endpoints.inference && (<div>
                            <strong>Inference:</strong>{' '}
                            <code>{selectedModel.endpoints.inference}</code>
                          </div>)}
                        {selectedModel.endpoints.monitoring && (<div>
                            <strong>Monitoring:</strong>{' '}
                            <code>{selectedModel.endpoints.monitoring}</code>
                          </div>)}
                      </div>
                    </div>)}
                  ...
                  <div style={{ marginBottom: '24px' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                      {selectedModel.tags.map((tag) => (<span key={tag} style={{
                        fontSize: '12px',
                        padding: '4px 8px',
                        backgroundColor: '#e9ecef',
                        borderRadius: '12px',
        ...
        {activeView === 'training' && (<div style={{
            ...
            <div>
              {mockTrainingJobs.map((job) => (<div key={job.id} style={{
                    padding: '16px',
                    borderBottom: '1px solid #f0f0f0',
                }}>
                  <div style={{
                  ...
                  {job.status === 'running' && (<div style={{ marginBottom: '12px' }}>
                      <div style={{
                        display: 'flex',
                        justifyContent: 'space-between',
                        fontSize: '12px',
                  ...
                  <div style={{
                    ...
                    <div>
                      <strong>GPU:</strong> {job.resource_usage.gpu}%
                    </div>
                    <div>
                      <strong>CPU:</strong> {job.resource_usage.cpu}%
                    </div>
                    <div>
                      <strong>Memory:</strong> {job.resource_usage.memory}%
                    </div>
                    <div>
                      <strong>Storage:</strong> {job.resource_usage.storage} GB
                    </div>
                  </div>

                  {job.metrics.accuracy && (<div style={{
                        fontSize: '12px',
                        color: '#666',
                        marginBottom: '8px',
                    }}>
                  ...
                  <details style={{ fontSize: '12px' }}>
                    ...
                    borderRadius: '4px',
                      ...
                      {job.logs.map((log, index) => (<div key={index} style={{
        ...
        {activeView === 'experiments' && (<div style={{
                padding: '16px',
                border: '1px solid var(--hairline)',
                borderRadius: '8px',
            }}>
            ...
            {mockExperiments.map((experiment) => (<div key={experiment.id} style={{
                    padding: '16px',
                ...
                <div style={{
                  ...
                  <span style={{
                    ...
                    padding: '2px 6px',
                    borderRadius: '12px',
                    backgroundColor: experiment.status === 'completed'
                        ? '#28a745'
                        : '#17a2b8',
                    color: 'white',
                }}>
                ...
                {experiment.status === 'completed' && experiment.results && (<div style={{
                        fontSize: '12px',
                        padding: '8px',
                        backgroundColor: '#f8f9fa',
                        borderRadius: '4px',
                    ...
                    <ul style={{ marginLeft: '16px', marginTop: '4px' }}>
                      {experiment.results.insights.map((insight, index) => (<li key={index}>{insight}</li>))}
                    </ul>
                  </div>)}
...
exports.default = ModelManagementDashboard;

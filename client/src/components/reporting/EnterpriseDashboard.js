...
const EnterpriseDashboard = ({ organizationId, userRole, customizations, onWidgetInteraction, onReportGenerate, onReportSchedule, className = '', }) => {
    ...
    const [reportParameters, setReportParameters] = (0, react_1.useState)({});
    const [isGeneratingReport, setIsGeneratingReport] = (0, react_1.useState)(false);
    // Mock dashboard metrics
    const mockMetrics = (0, react_1.useMemo)(() => ({
        investigations: {
            active: 47,
            completed: 156,
            overdue: 8,
    ...
    // Mock widgets
    const mockWidgets = (0, react_1.useMemo)(() => [
        {
            id: 'widget-001',
        ...
        },
        {
            ...
            data: {
                series: mockMetrics.threats.trendsLast30Days,
    ...
    const mockReportTemplates = (0, react_1.useMemo)(() => [
        {
            id: 'template-001',
        ...
        },
        {
            ...
            parameters: [
                ...
                },
                {
                    ...
                    type: 'select',
                    required: false,
                    options: ['All Teams', 'Cyber', 'Financial', 'Compliance'],
                },
            ],
            schedule: {
                enabled: true,
    ...
    const getMetricTrendColor = (trend) => {
        if (trend > 0)
            return '#28a745';
        if (trend < 0)
            return '#dc3545';
    ...
    const handleReportGenerate = (0, react_1.useCallback)(async (template) => {
    ...
    return (<div className={`enterprise-dashboard ${className}`} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div style={{ marginBottom: '24px' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {userRole !== 'analyst' && (<button onClick={() => setIsEditMode(!isEditMode)} style={{
                padding: '6px 12px',
                fontSize: '13px',
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--hairline)' }}>
          {[
            ...
            .filter((tab) => userRole !== 'analyst' || tab.key !== 'settings')
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {activeView === 'dashboard' && (<div>
            ...
            {userRole === 'executive' && metrics && (<div style={{
                    display: 'grid',
                ...
                <div style={{
                  ...
                  <div style={{
                    fontSize: '12px',
                    color: metrics.compliance.violations > 0
                        ? '#dc3545'
                        : '#28a745',
                }}>
                    {metrics.compliance.violations} active violation
                    {metrics.compliance.violations !== 1 ? 's' : ''}
                  </div>
                </div>
            ...
            <div style={{
              ...
              {visibleWidgets.map((widget) => (<div key={widget.id} style={{
                    border: isEditMode
                        ? '2px dashed #1a73e8'
                  ...
                  <div style={{
                    ...
                    gap: '8px',
                      ...
                      {widget.isLoading && (<span style={{ fontSize: '12px', color: '#666' }}>
                          ⏳
                        </span>)}
                      <span style={{ fontSize: '11px', color: '#999' }}>
                        {widget.lastUpdated.toLocaleTimeString()}
                      </span>
                      {isEditMode && (<button style={{
                        padding: '2px 4px',
                        fontSize: '10px',
                  ...
                  <div style={{ padding: '16px' }}>
                    {widget.type === 'metric' && (<div style={{ textAlign: 'center' }}>
                        <div style={{
                        fontSize: '28px',
                        fontWeight: '600',
                        color: widget.data.value >
                        ...
                        marginBottom: '8px',
                          ...
                          {widget.data.value}
                          {widget.data.unit && (<span style={{ fontSize: '16px', marginLeft: '4px' }}>
                              {widget.data.unit}
                            </span>)}
                        </div>
                        {widget.data.trend !== undefined && (<div style={{
                            fontSize: '12px',
                    ...
                    {widget.type === 'gauge' && (<div style={{ textAlign: 'center' }}>
                        <div style={{
                        width: '100px',
                        height: '100px',
                        borderRadius: '50%',
                    ...
                    {widget.type === 'chart' && (<div style={{
                        ...
                        {widget.data.series
                        .slice(-20)
                        .map((value, index) => (<div key={index} style={{
                            flex: 1,
                    ...
                    {widget.type === 'table' && (<div style={{ overflow: 'auto', maxHeight: '250px' }}>
                        <table style={{
                        width: '100%',
                        fontSize: '12px',
                        borderCollapse: 'collapse',
                          ...
                          <thead>
                            <tr>
                              {widget.data.headers.map((header, index) => (<th key={index} style={{
                            padding: '4px 6px',
                            textAlign: 'left',
                        ...
                        .map((row, rowIndex) => (<tr key={rowIndex}>
                                ...
                                color: cellIndex === 2
                                    ...
                                    : cellIndex === 3
                                        ? getStatusColor(cell)
                                        : 'inherit',
                                fontWeight: cellIndex === 2 || cellIndex === 3
                                    ? '500'
                                    : 'normal',
                            }}>
                                      {cell}
                ...
                .map((template) => (<div key={template.id} style={{
                    ...
                    lineHeight: '1.4',
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={() => setSelectedReport(template)} style={{
                    flex: 1,
                    padding: '6px 12px',
                    fontSize: '12px',
                    backgroundColor: '#1a73e8',
                    ...
                    cursor: 'pointer',
                        ...
                        {template.schedule?.enabled && (<button onClick={() => onReportSchedule?.(template.id, template.schedule)} style={{
                        padding: '6px 12px',
                        fontSize: '12px',
            ...
            {selectedReport && (<div style={{
                ...
                <div style={{
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    {selectedReport.parameters.map((param) => (<div key={param.name} style={{ marginBottom: '12px' }}>
                        <label style={{
                        fontSize: '13px',
                        fontWeight: '500',
                        marginBottom: '4px',
                        ...
                        {param.type === 'select' ? (<select value={reportParameters[param.name] ||
                            param.defaultValue ||
                            ...
                            {param.options?.map((option) => (<option key={option} value={option}>
                                {option}
                              </option>))}
                          </select>) : param.type === 'boolean' ? (<label style={{
                            display: 'flex',
                            alignItems: 'center',
                            gap: '8px',
                        }}>
                            <input type="checkbox" checked={reportParameters[param.name] ||
                            param.defaultValue ||
                            false} onChange={(e) => setReportParameters((prev) => ({
                            ...prev,
                          ...
                          </label>) : (<input type={param.type} value={reportParameters[param.name] ||
                            param.defaultValue ||
                            ''} onChange={(e) => setReportParameters((prev) => ({
                            ...prev,
                            [param.name]: e.target.value,
                  ...
                  <div style={{
                    ...
                    <button onClick={() => handleReportGenerate(selectedReport)} disabled={isGeneratingReport} style={{
                    padding: '8px 16px',
                    fontSize: '13px',
        ...
        {activeView === 'analytics' && (<div style={{
                padding: '16px',
                border: '1px solid var(--hairline)',
                borderRadius: '8px',
                textAlign: 'center',
        ...
        {activeView === 'settings' && userRole !== 'analyst' && (<div style={{
                padding: '16px',
                border: '1px solid var(--hairline)',
                borderRadius: '8px',
            }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '13px', lineHeight: '1.6' }}>
                  <div>
                    <strong>Default timezone:</strong> UTC
                  </div>
                  <div>
                    <strong>Email format:</strong> PDF
                  </div>
                  <div>
                    <strong>Retention period:</strong> 90 days
                  </div>
                </div>
              </div>
            </div>
...
exports.default = EnterpriseDashboard;

...
exports.RAGPreviewPanel = RAGPreviewPanel;
const react_1 = __importStar(require("react"));
const badge_1 = require("@/components/ui/badge");
const separator_1 = require("@/components/ui/separator");
const tabs_1 = require("@/components/ui/tabs");
const types_1 = require("./types");
function RAGPreviewPanel({ node, enrichment }) {
    ...
    return (<div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b">
        <div className="flex items-center gap-2 mb-2">
          <badge_1.Badge style={{
            backgroundColor: types_1.NODE_TYPE_COLORS[node.type] ?? types_1.NODE_TYPE_COLORS.DEFAULT,
        }}>
            {node.type}
          </badge_1.Badge>
          <badge_1.Badge variant="outline" className="gap-1">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        <tabs_1.TabsList className="w-full justify-start px-4 border-b rounded-none h-auto py-0">
          <tabs_1.TabsTrigger value="summary" className="py-2">
            Summary
          </tabs_1.TabsTrigger>
          <tabs_1.TabsTrigger value="context" className="py-2">
            Context
          </tabs_1.TabsTrigger>
          <tabs_1.TabsTrigger value="sources" className="py-2">
            Sources
          </tabs_1.TabsTrigger>
        </tabs_1.TabsList>

        <div className="flex-1 overflow-auto">
          <tabs_1.TabsContent value="summary" className="p-4 m-0 space-y-4">
            <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {ragData.relatedConcepts.map((concept, i) => (<badge_1.Badge key={i} variant="secondary" className="text-xs">
                    {concept}
                  </badge_1.Badge>))}
              </div>
            </div>
          ...
          <tabs_1.TabsContent value="context" className="p-4 m-0 space-y-3">
            <h4 className="text-sm font-medium">Contextual Information</h4>
            {ragData.context.map((ctx, i) => (<div key={i} className="p-3 bg-muted/50 rounded-lg text-sm leading-relaxed">
                {ctx}
              </div>))}
          </tabs_1.TabsContent>

          <tabs_1.TabsContent value="sources" className="p-4 m-0 space-y-3">
            <h4 className="text-sm font-medium">
              Sources ({ragData.sources.length})
            </h4>
            {ragData.sources.map((source) => (<div key={source.id} className="p-3 border rounded-lg space-y-2">
                <div className="flex items-start justify-between">
                  <h5 className="font-medium text-sm">{source.title}</h5>
                  <badge_1.Badge variant="outline" className="text-xs shrink-0 ml-2">
                ...
                {source.url && (<a href={source.url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline">
                    View source
                  </a>)}
              </div>))}
          </tabs_1.TabsContent>
...
function generateContext(node, enrichment) {
    const contexts = [
    ...
    if (enrichment?.relatedEntities.length) {
        contexts.push(`This entity is directly connected to ${enrichment.relatedEntities.length} other entities in the knowledge graph.`);
    }
    if (node.confidence) {
        contexts.push(`The confidence score of ${Math.round(node.confidence * 100)}% indicates ${node.confidence > 0.7 ? 'high' : node.confidence > 0.4 ? 'moderate' : 'low'} reliability of the source data.`);
    }
    return contexts;
}
function generateRelatedConcepts(node) {
    const baseConcepts = {
...
function generateSources(node, enrichment) {
    const sources = [];

...
const MatrixGraph: React.FC<{ matrix: RiskMatrix }> = ({ matrix }) => {
  ...
  useEffect(() => {
    ...
    const x = d3.scaleBand<string>().domain(d3.range(numCols).map(String)).range([0, width]);
    const y = d3.scaleBand<string>().domain(d3.range(numRows).map(String)).range([0, height]);

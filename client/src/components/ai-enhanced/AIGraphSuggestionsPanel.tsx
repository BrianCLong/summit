...
export default function AIGraphSuggestionsPanel() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    const data = await gql<{ suggestions: any[] }>(`query { suggestions { id type label confidence status createdAt } }`);
    ...
    <div aria-label="AI Suggestions" role="region" className="space-y-3">
      ...
      {items.map(s => (
        <Card key={s.id} className="rounded-2xl shadow p-2">
          <CardContent>
            <div className="text-sm opacity-70">{s.type} • {s.createdAt}</div>

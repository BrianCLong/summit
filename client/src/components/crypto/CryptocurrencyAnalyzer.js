...
const CryptocurrencyAnalyzer = ({ investigationId, initialAddress, onAddressAnalysis = () => { }, onTransactionFlag = () => { }, onClusterIdentification = () => { }, onPathTracing = () => { }, className = '', }) => {
    const [activeView, setActiveView] = (0, react_1.useState)('search');
    const [searchAddress, setSearchAddress] = (0, react_1.useState)(initialAddress || '');
    const [searchResults, setSearchResults] = (0, react_1.useState)([]);
    const [watchedAddresses, setWatchedAddresses] = (0, react_1.useState)([]);
    const [recentTransactions, setRecentTransactions] = (0, react_1.useState)([]);
    const [identifiedClusters, setIdentifiedClusters] = (0, react_1.useState)([]);
    const [investigationPaths, setInvestigationPaths] = (0, react_1.useState)([]);
    const [selectedAddress, setSelectedAddress] = (0, react_1.useState)(null);
    const [isAnalyzing, setIsAnalyzing] = (0, react_1.useState)(false);
    const [analysisDepth, setAnalysisDepth] = (0, react_1.useState)(3);
    const [blockchainFilter, setBlockchainFilter] = (0, react_1.useState)('all');
    ...
    const detectBlockchain = (address) => {
    ...
    const filteredAddresses = watchedAddresses.filter((addr) => blockchainFilter === 'all' || addr.blockchain === blockchainFilter);
    const filteredTransactions = recentTransactions.filter((tx) => blockchainFilter === 'all' || tx.blockchain === blockchainFilter);
    return (<div className={`cryptocurrency-analyzer ${className}`}>
      {/* Header */}
      <div className="mb-6 border-b pb-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-4">
            <select value={blockchainFilter} onChange={(e) => setBlockchainFilter(e.target.value)} className="px-3 py-2 border rounded-md text-sm">
              <option value="all">All Blockchains</option>
              {supportedBlockchains.map((blockchain) => (<option key={blockchain.id} value={blockchain.id}>
        ...
        <div className="flex gap-4">
          <button onClick={() => setActiveView('search')} className={`px-4 py-2 rounded-md ${activeView === 'search' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            🔍 Address Search
          </button>
          <button onClick={() => setActiveView('addresses')} className={`px-4 py-2 rounded-md ${activeView === 'addresses' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            📍 Watched Addresses ({watchedAddresses.length})
          </button>
          <button onClick={() => setActiveView('transactions')} className={`px-4 py-2 rounded-md ${activeView === 'transactions' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            🔄 Transactions ({recentTransactions.length})
          </button>
          <button onClick={() => setActiveView('clusters')} className={`px-4 py-2 rounded-md ${activeView === 'clusters' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
            🕸️ Clusters ({identifiedClusters.length})
          </button>
          <button onClick={() => setActiveView('paths')} className={`px-4 py-2 rounded-md ${activeView === 'paths' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
      ...
      {/* Address Search View */}
      {activeView === 'search' && (<div className="space-y-6">
          <div className="bg-white rounded-lg border p-6">
            ...
            <div className="flex gap-4 mb-4">
              <input type="text" value={searchAddress} onChange={(e) => setSearchAddress(e.target.value)} placeholder="Enter blockchain address (Bitcoin, Ethereum, Monero, etc.)" className="flex-1 px-4 py-2 border rounded-md" onKeyPress={(e) => e.key === 'Enter' && performAddressSearch(searchAddress)}/>
              <select value={analysisDepth} onChange={(e) => setAnalysisDepth(Number(e.target.value))} className="px-3 py-2 border rounded-md">
                <option value={1}>Depth 1</option>
                <option value={2}>Depth 2</option>
                <option value={3}>Depth 3</option>
                <option value={5}>Depth 5</option>
              </select>
              <button onClick={() => performAddressSearch(searchAddress)} disabled={isAnalyzing || !searchAddress.trim()} className="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50">
            ...
            {searchResults.length > 0 && !isAnalyzing && (<div className="space-y-4">
                <h4 className="font-medium">Search Results</h4>
                {searchResults.map((address) => (<div key={address.address} className="p-4 border rounded-lg">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <button onClick={() => addToWatchlist(address)} className="px-3 py-1 bg-green-100 text-green-700 text-sm rounded hover:bg-green-200">
                        + Watch
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-3">
                      ...
                      <div>
                        ...
                        <div className="font-medium">
                          {address.firstSeen.toLocaleDateString()}
                        </div>
                      </div>
                      <div>
                        <span className="text-gray-600">Last Seen:</span>
                        <div className="font-medium">
                          {address.lastSeen.toLocaleDateString()}
                        </div>
                      </div>
                    </div>

                    {address.tags.length > 0 && (<div className="mb-3">
                        <span className="text-sm text-gray-600">Tags:</span>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {address.tags.map((tag, index) => (<span key={index} className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded">
                              {tag.tag} ({tag.confidence}%)
                            </span>))}
                        </div>
                      </div>)}

                    {address.cluster && (<div className="p-3 bg-red-50 border border-red-200 rounded-md">
      ...
      {activeView === 'addresses' && (<div className="space-y-4">
          {filteredAddresses.map((address) => (<div key={address.address} className="bg-white rounded-lg border p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-1 text-xs rounded text-white" style={{
                    backgroundColor: supportedBlockchains.find((b) => b.id === address.blockchain)?.color,
                }}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => removeFromWatchlist(address.address)} className="px-3 py-1 bg-red-100 text-red-700 text-sm rounded hover:bg-red-200">
                    Remove
                  </button>
                </div>
              </div>
              ...
              {address.tags.length > 0 && (<div className="flex flex-wrap gap-1">
                  {address.tags.map((tag, index) => (<span key={index} className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded">
                      {tag.tag}
                    </span>))}
                </div>)}
            </div>))}

          {filteredAddresses.length === 0 && (<div className="text-center py-12 text-gray-500">
      ...
      {/* Transactions View */}
      {activeView === 'transactions' && (<div className="space-y-4">
          {filteredTransactions.slice(0, 20).map((tx) => (<div key={tx.id} className="bg-white rounded-lg border p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="font-mono text-sm mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-1 text-xs rounded text-white" style={{
                    backgroundColor: supportedBlockchains.find((b) => b.id === tx.blockchain)?.color,
                }}>
                      {supportedBlockchains.find((b) => b.id === tx.blockchain)
                    ?.symbol}
                    </span>
              ...
              {tx.flags.length > 0 && (<div className="mb-3 p-3 bg-orange-50 border border-orange-200 rounded-md">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-orange-800 font-medium">
                      ⚠️ Flagged Transaction
                    </span>
                  </div>
                  {tx.flags.map((flag, index) => (<div key={index} className="text-sm text-orange-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="mt-1 space-y-1">
                    {tx.inputs.slice(0, 3).map((input, index) => (<div key={index} className="font-mono text-xs p-2 bg-gray-50 rounded">
                ...
                <div>
                  ...
                  <div className="mt-1 space-y-1">
                    {tx.outputs.slice(0, 3).map((output, index) => (<div key={index} className="font-mono text-xs p-2 bg-gray-50 rounded">
                        {output.address.substring(0, 20)}... (
                        {output.amount.toFixed(8)})
                      </div>))}
                  </div>
                </div>
              </div>

              {(tx.analysis.mixingDetected ||
                    tx.analysis.privacyTechniques.length > 0) && (<div className="mt-3 p-3 bg-purple-50 border border-purple-200 rounded-md">
                  ...
                  <div className="text-sm text-purple-700 mt-1">
                    ...
                    {tx.analysis.privacyTechniques.map((technique) => (<div key={technique}>• {technique} usage detected</div>))}
                  </div>
                </div>)}
            </div>))}
        </div>)}
      ...
      {/* Clusters View */}
      {activeView === 'clusters' && (<div className="space-y-4">
          {identifiedClusters.map((cluster) => (<div key={cluster.id} className="bg-white rounded-lg border p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
                <div>
                  ...
                  <div className="font-medium">
                    {cluster.addresses.length.toLocaleString()}
                  </div>
                </div>
                <div>
                ...
                <div>
                  <span className="text-gray-600">Transactions:</span>
                  <div className="font-medium">
                    {cluster.totalTransactions.toLocaleString()}
                  </div>
                </div>
                <div>
                  <span className="text-gray-600">Last Activity:</span>
                  <div className="font-medium">
                    {cluster.lastActivity.toLocaleDateString()}
                  </div>
                </div>
              </div>

              {cluster.attribution.entity && (<div className="p-3 bg-red-50 border border-red-200 rounded-md mb-3">
                  <span className="text-red-800 font-medium">Attribution:</span>
                  <div className="text-sm text-red-700 mt-1">
                    Entity: {cluster.attribution.entity}
                    {cluster.attribution.jurisdiction &&
                        ` (${cluster.attribution.jurisdiction})`}
                    <div className="text-xs mt-1">
                      Source: {cluster.attribution.source} (
                      {cluster.attribution.confidence}% confidence)
                    </div>
                  </div>
                </div>)}

              <div className="flex flex-wrap gap-1">
                {cluster.tags.map((tag) => (<span key={tag} className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded">
                    {tag}
                  </span>))}
              </div>
            </div>))}
      ...
      {activeView === 'paths' && (<div className="space-y-4">
          {investigationPaths.map((path) => (<div key={path.id} className="bg-white rounded-lg border p-4">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h4 className="font-medium mb-2">Investigation Path</h4>
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 text-xs rounded ${getRiskColor(path.riskAssessment.overallRisk === 'critical'
                    ? 95
                ...
                <div className="text-right text-sm">
                  <div className="font-medium">
                    {formatCurrency(path.totalAmount, 'BTC')}
                  </div>
                  <div className="text-xs text-gray-500">
                    {path.timeSpan}h timespan
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                {path.hops.map((hop, index) => (<div key={index} className="flex items-center gap-2 ml-4 mb-2 text-sm">
                    ...
                    <span className="text-xs text-gray-500">
                      {hop.amount.toFixed(4)} BTC •{' '}
                      {hop.timestamp.toLocaleDateString()}
                    </span>
                  </div>))}
              ...
              {(path.riskAssessment.mixingDetected ||
                    path.riskAssessment.knownBadActors.length > 0) && (<div className="p-3 bg-orange-50 border border-orange-200 rounded-md">
                  <span className="text-orange-800 font-medium">
                    Risk Indicators:
                  </span>
                  <div className="text-sm text-orange-700 mt-1 space-y-1">
                    {path.riskAssessment.mixingDetected && (<div>• Mixing activity detected</div>)}
                    {path.riskAssessment.knownBadActors.map((actor) => (<div key={actor}>
                        • Interaction with known bad actor: {actor}
                      </div>))}
                    {path.riskAssessment.jurisdictionChanges.length > 0 && (<div>
                        • Jurisdiction changes:{' '}
                        {path.riskAssessment.jurisdictionChanges.join(' → ')}
                      </div>)}
                    {path.riskAssessment.privacyTechniques.map((technique) => (<div key={technique}>
                        • Privacy technique used: {technique}
                      </div>))}
                  </div>
                </div>)}
            </div>))}

          {investigationPaths.length === 0 && (<div className="text-center py-12 text-gray-500">
      ...
      {activeView === 'analysis' && (<div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            ].map((tool) => (<div key={tool.name} className="bg-white rounded-lg border p-6">
          ...
          <div className="bg-white rounded-lg border p-6">
            <h3 className="text-lg font-semibold mb-4">
              Supported Blockchains & Features
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {supportedBlockchains.map((blockchain) => (<div key={blockchain.id} className="p-4 border rounded-lg">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-4 h-4 rounded-full" style={{ backgroundColor: blockchain.color }}></div>
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    {blockchain.id === 'monero' &&
                    ' • ⚠️ Limited (Privacy Coin)'}
                  </div>
                </div>))}
            </div>
          </div>
...
exports.default = CryptocurrencyAnalyzer;

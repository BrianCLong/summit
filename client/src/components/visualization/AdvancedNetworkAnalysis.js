...
const AdvancedNetworkAnalysis = ({ investigationId, onNodeSelect = () => { }, onEdgeSelect = () => { }, onCommunitySelect = () => { }, onPathAnalysis = () => { }, onMetricsChange = () => { }, enableCommunityDetection = true, enablePathAnalysis = true, enableCentralityAnalysis = true, enableTemporalAnalysis = true, className = '', }) => {
    ...
    const [analysisMode, setAnalysisMode] = (0, react_1.useState)('overview');
    ...
    const [temporalFilter, setTemporalFilter] = (0, react_1.useState)(null);
    const [centralityType, setCentralityType] = (0, react_1.useState)('betweenness');
    const [layoutAlgorithm, setLayoutAlgorithm] = (0, react_1.useState)('force');
    ...
    const generateMockNetworkData = () => {
        ...
        const mockEdges = Array.from({ length: 80 }, (_, i) => {
    ...
    const generateCommunities = (nodeList) => {
        ...
        const mockCommunities = Array.from({ length: numCommunities }, (_, i) => {
    ...
    const calculateNetworkMetrics = (nodeList, edgeList) => {
    ...
    const runPathAnalysis = async (sourceId, targetId) => {
        setIsAnalyzing(true);
    ...
    const handleCentralityChange = (event) => {
        setCentralityType(event.target.value);
    };
    const handleTemporalStartChange = (event) => {
    ...
    const handleTemporalEndChange = (event) => {
    ...
    return (<div className={`advanced-network-analysis ${className}`}>
      {/* Analysis Controls */}
      <div className="mb-6 p-4 bg-gray-50 rounded-lg">
        <div className="flex flex-wrap items-center gap-4 mb-4">
          <div>
            ...
            <select value={analysisMode} onChange={handleAnalysisModeChange} className="px-3 py-2 border rounded-md text-sm">
          ...
          <div>
            ...
            <select value={layoutAlgorithm} onChange={handleLayoutChange} className="px-3 py-2 border rounded-md text-sm">
          ...
          {analysisMode === 'centrality' && (<div>
              <label className="block text-sm font-medium mb-1">
                Centrality Type
              </label>
              <select value={centralityType} onChange={handleCentralityChange} className="px-3 py-2 border rounded-md text-sm">
          ...
          <div className="flex gap-2">
            {enableCommunityDetection && (<button onClick={runCommunityDetection} disabled={isAnalyzing} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 text-sm">
                {isAnalyzing ? 'Analyzing...' : 'Detect Communities'}
              </button>)}

            <button onClick={() => generateMockNetworkData()} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 text-sm">
              Refresh Data
            </button>
          </div>
        </div>

        {enableTemporalAnalysis && (<div className="flex items-center gap-4">
            <label className="text-sm font-medium">Temporal Filter:</label>
            <input type="date" className="px-3 py-2 border rounded-md text-sm" onChange={handleTemporalStartChange}/>
            <span>to</span>
            <input type="date" className="px-3 py-2 border rounded-md text-sm" onChange={handleTemporalEndChange}/>
          </div>)}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          <div className="bg-white rounded-lg border p-4 h-96">
            ...
            <canvas ref={canvasRef} className="w-full h-full border rounded" style={{ background: '#f8f9fa' }}/>

            {isAnalyzing && (<div className="absolute inset-0 bg-white bg-opacity-75 flex items-center justify-center rounded">
        ...
        <div className="space-y-4">
          {/* Network Metrics */}
          {metrics && (<div className="bg-white rounded-lg border p-4">
          ...
          {/* Communities */}
          {analysisMode === 'community' && communities.length > 0 && (<div className="bg-white rounded-lg border p-4">
              <h4 className="font-semibold mb-3">
                Communities ({communities.length})
              </h4>
              <div className="space-y-2">
                {communities.map((community) => (<div key={community.id} onClick={() => handleCommunitySelection(community)} className="p-2 rounded border cursor-pointer hover:bg-gray-50" style={{ borderLeft: `4px solid ${community.color}` }}>
          ...
          {pathAnalysis.length > 0 && (<div className="bg-white rounded-lg border p-4">
              <h4 className="font-semibold mb-3">Path Analysis</h4>
              <div className="space-y-2">
                {pathAnalysis.map((path) => (<div key={path.id} className="p-2 rounded border">
                    <div className="flex justify-between text-sm">
          ...
          {/* Selected Nodes Info */}
          {selectedNodes.length > 0 && (<div className="bg-white rounded-lg border p-4">
              <h4 className="font-semibold mb-3">
                Selected Nodes ({selectedNodes.length})
              </h4>
              <div className="space-y-2">
                {selectedNodes.slice(0, 5).map((node) => (<div key={node.id} className="p-2 rounded border text-sm">
                ...
                {selectedNodes.length > 5 && (<div className="text-xs text-gray-500 text-center">
...
exports.default = AdvancedNetworkAnalysis;

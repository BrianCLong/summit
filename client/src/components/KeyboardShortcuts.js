...
exports.defaultShortcuts = exports.ShortcutsHelp = exports.formatKeyCombo = exports.useKeyboardShortcuts = void 0;
const react_1 = __importStar(require("react"));
const KeyboardShortcuts = ({ shortcuts, enabled = true, }) => {
    const handleKeyDown = (0, react_1.useCallback)((event) => {
        ...
        const matchingShortcut = shortcuts.find((shortcut) => shortcut.keys.some((keyCombo) => keyCombo.toLowerCase() === pressedKeyString));
        if (matchingShortcut) {
            ...
            const shouldExecute = !isInputElement ||
                alwaysAllowed.some((allowed) => matchingShortcut.keys.some((key) => key.toLowerCase() === allowed));
            if (shouldExecute) {
...
exports.formatKeyCombo = formatKeyCombo;
const ShortcutsHelp = ({ shortcuts, isVisible, onClose, }) => {
    ...
    return (<div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-80vh overflow-hidden">
        ...
        <div className="p-4 overflow-y-auto max-h-96">
          {Object.entries(groupedShortcuts).map(([category, categoryShortcuts]) => (<div key={category} className="mb-6">
                <h3 className="text-lg font-medium text-gray-900 mb-3">
                  {category}
                </h3>
                <div className="space-y-2">
                  {categoryShortcuts.map((shortcut, index) => (<div key={index} className="flex items-center justify-between py-2">
                      <span className="text-gray-700">
                        {shortcut.description}
                      </span>
                      <div className="flex gap-1">
                        {shortcut.keys.map((keyCombo, keyIndex) => (<kbd key={keyIndex} className="px-2 py-1 bg-gray-100 border border-gray-300 rounded text-sm font-mono">
                            {(0, exports.formatKeyCombo)(keyCombo)}
                          </kbd>))}
                      </div>
                    </div>))}
                </div>

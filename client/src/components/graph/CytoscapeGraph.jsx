...
function CytoscapeGraph() {
    ...
    cy.batch(() => {
      ...
      cy.edges().forEach((edge) => {
        ...
        if (lodMode === "low") {
          ...
          edge.style("label", edge.data("label") ?? edge.data("weightLabel") ?? "");
  ...
  }, [cy, lodMode, aggregationFeatureEnabled, buildAggregatedView, restoreAggregatedView]);
  ...
  const buildAggregatedView = useCallback(() => {
    ...
    const aggregationNodes = Array.from(clusters.entries()).map(([clusterKey, members], idx) => {
  ...
  return (
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          ...
          {benchmarkFeatureEnabled && (
            <Button variant="outlined" startIcon={<Timeline />} onClick={runPerformanceBenchmark}>
              LOD Benchmark
            </Button>
          )}
          <Button
      ...
      <Box sx={{ display: "flex", gap: 1, mb: 2, flexWrap: "wrap" }}>
        ...
        <Chip
          label={`Aggregation: ${
            aggregationFeatureEnabled && lodMode === "low" ? "Supernodes active" : "Full detail"
          }`}
          color={aggregationFeatureEnabled && lodMode === "low" ? "success" : "default"}
          variant="outlined"
        />
        {benchmarkResults && (
          <Chip
      ...
      <Alert severity="info" sx={{ mb: 2 }}>
        Advanced graph visualization with Cytoscape.js. Level of detail now responds to zoom: labels
        and edge curves simplify as you zoom out, and optional supernode aggregation collapses
        clusters at low zoom. Use the LOD toggle to override and the benchmark control to validate
        performance.
      </Alert>
      ...
      {benchmarkResults && (
        <Alert severity="success" sx={{ mb: 2 }}>
          ...
          {benchmarkResults.ingestMs} ms ingest, {benchmarkResults.layoutMs} ms layout).
        </Alert>
      )}

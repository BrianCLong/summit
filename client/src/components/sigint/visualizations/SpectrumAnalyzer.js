...
const SpectrumAnalyzer = ({ samples, fftSize = 2048, minDecibels = -90, maxDecibels = -10, smoothing = 0.8, showPeaks = true, frequencyRange, className, }) => {
    const canvasRef = (0, react_1.useRef)(null);
    const animationRef = (0, react_1.useRef)(0);
    const prevSpectrumRef = (0, react_1.useRef)(null);
    const peaksRef = (0, react_1.useRef)([]);
    // Perform FFT on sample data
    const computeFFT = (0, react_1.useCallback)((data) => {
    ...
    const detectPeaks = (0, react_1.useCallback)((spectrum, canvasWidth) => {
        const peaks = [];
    ...
    return (<div className={`relative w-full h-full min-h-[150px] ${className || ''}`}>
      <canvas ref={canvasRef} className="w-full h-full rounded-lg"/>
      {/* Legend */}
      <div className="absolute bottom-2 left-2 flex gap-2">
        {showPeaks && peaksRef.current.length > 0 && (<span className="text-xs font-mono text-red-400 bg-black/60 px-2 py-0.5 rounded">
...
exports.default = exports.SpectrumAnalyzer;

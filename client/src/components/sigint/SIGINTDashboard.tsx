...
export const SIGINTDashboard: React.FC<SIGINTDashboardProps> = ({
  ...
  const footerStats = useMemo(() => {
    return {
      ...
      runningTasks: demodTasks.reduce(
        (count, t) => count + (t.status !== "COMPLETED" && t.status !== "FAILED" ? 1 : 0),
        0
      ),
    };
    ...
    <div className={`flex flex-col h-full bg-slate-950 text-slate-100 ${className || ""}`}>
      ...
      <footer className="flex items-center justify-between px-4 py-1.5 bg-slate-900 border-t border-slate-800 text-xs text-slate-400">
        <div className="flex items-center gap-4">
          <span>
            <span className="text-cyan-400 font-medium">{footerStats.activeStreams}</span> active
            streams
          </span>
          <span>
            <span className="text-amber-400 font-medium">{footerStats.totalDetections}</span> MASINT
            detections
          </span>
          <span>
            <span className="text-purple-400 font-medium">{footerStats.runningTasks}</span> demod
            tasks running
          </span>
        </div>
        <div className="flex items-center gap-2">

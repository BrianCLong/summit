...
exports.MASINTOverlayPanel = void 0;
/**
...
const MASINTOverlayPanel = ({ overlays, onSelectOverlay, onSelectDetection, selectedOverlayId, className, }) => {
    const [expandedId, setExpandedId] = (0, react_1.useState)(null);
    // Group overlays by sensor type
    const groupedOverlays = (0, react_1.useMemo)(() => {
        const groups = {};
    ...
    return (<div className={`flex flex-col h-full bg-slate-900 text-slate-100 rounded-lg overflow-hidden ${className || ''}`}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-slate-800 border-b border-slate-700">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400">
          ...
          {stats.recentDetections > 0 && (<span className="px-2 py-0.5 bg-amber-500/20 text-amber-400 rounded animate-pulse">
      ...
      <div className="flex-1 overflow-y-auto">
        {Object.entries(groupedOverlays).map(([sensorType, sensorOverlays]) => (<div key={sensorType} className="border-b border-slate-800">
            {/* Sensor type header */}
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-850 text-sm font-medium text-slate-400">
              <span className={`w-5 h-5 flex items-center justify-center rounded text-xs font-bold text-white ${SENSOR_COLORS[sensorType]}`}>
                {SENSOR_ICONS[sensorType]}
              </span>
            ...
            {sensorOverlays.map((overlay) => (<div key={overlay.id} className={`border-l-2 transition-colors ${selectedOverlayId === overlay.id
                ...
                <button onClick={() => {
                  ...
                  {/* Detection count */}
                  {overlay.detections.length > 0 && (<span className="px-2 py-0.5 text-xs font-medium bg-cyan-500/20 text-cyan-400 rounded">
                      {overlay.detections.length}
                    </span>)}

                  {/* Expand indicator */}
                  <svg className={`w-4 h-4 transition-transform ${expandedId === overlay.id ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/>
                  </svg>
                </button>

                {/* Expanded detections */}
                {expandedId === overlay.id && overlay.detections.length > 0 && (<div className="px-4 pb-3 pl-10 space-y-1">
                    {overlay.detections.slice(0, 5).map((detection) => (<button key={detection.id} onClick={() => onSelectDetection?.(detection)} className="w-full flex items-center gap-2 p-2 text-left text-xs bg-slate-800 rounded hover:bg-slate-700 transition-colors">
                    ...
                    {overlay.detections.length > 5 && (<div className="text-center text-xs text-slate-500 py-1">
        ...
        {overlays.length === 0 && (<div className="flex flex-col items-center justify-center h-48 text-slate-500">
      ...
      <div className="flex items-center gap-4 px-4 py-2 bg-slate-800 border-t border-slate-700 text-xs text-slate-400">
        {Object.entries(STATUS_COLORS).map(([status, color]) => (<div key={status} className="flex items-center gap-1.5">
...
exports.MASINTOverlayPanel = MASINTOverlayPanel;
exports.default = exports.MASINTOverlayPanel;

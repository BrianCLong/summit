...
exports.SIGINTDashboard = void 0;
/**
...
const WaveformRenderer_1 = require("./visualizations/WaveformRenderer");
const SpectrumAnalyzer_1 = require("./visualizations/SpectrumAnalyzer");
...
const SIGINTDashboard = ({ streamKey = 'sigint:primary', initialFilters: _initialFilters, onDemodulationComplete, className, }) => {
    // State
    const [viewMode, setViewMode] = (0, react_1.useState)('combined');
    const [panelLayout, setPanelLayout] = (0, react_1.useState)('default');
    const [selectedStreamId, setSelectedStreamId] = (0, react_1.useState)(null);
    const [subscribedIds, setSubscribedIds] = (0, react_1.useState)(new Set());
    const [metrics, setMetrics] = (0, react_1.useState)(null);
    // Demo data (in production, use useRedisStream)
    const [demoStreams] = (0, react_1.useState)(generateDemoStreams);
    const [masintOverlays] = (0, react_1.useState)(generateDemoMASINT);
    const [demodTasks, setDemodTasks] = (0, react_1.useState)(generateDemoTasks);
    // Redis stream hook for real-time data
    const { samples, isConnected, streams: liveStreams, subscribe, unsubscribe, } = (0, useRedisStream_1.useRedisStream)({
        streamKey,
        bufferSize: 2048,
        maxLatency: 500,
    });
    ...
    const selectedStream = (0, react_1.useMemo)(() => streams.find((s) => s.id === selectedStreamId) || null, [streams, selectedStreamId]);
    // Generate demo samples for visualization if no live data
    const displaySamples = (0, react_1.useMemo)(() => {
        if (samples.length > 0)
            return samples;
    ...
    // Handlers
    const handleSelectStream = (0, react_1.useCallback)((stream) => {
        setSelectedStreamId(stream.id);
    }, []);
    const handleSubscribe = (0, react_1.useCallback)((streamId) => {
        subscribe(streamId);
        setSubscribedIds((prev) => new Set([...prev, streamId]));
    }, [subscribe]);
    const handleUnsubscribe = (0, react_1.useCallback)((streamId) => {
    ...
    const handleStartDemodulation = (0, react_1.useCallback)((streamId) => {
        const newTask = {
            id: `task-${Date.now()}`,
            signalId: streamId,
            status: 'QUEUED',
    ...
    const handleCancelTask = (0, react_1.useCallback)((taskId) => {
        setDemodTasks((prev) => prev.filter((t) => t.id !== taskId));
    }, []);
    const handleViewResult = (0, react_1.useCallback)((result) => {
    ...
    const footerStats = (0, react_1.useMemo)(() => {
        return {
            ...
            runningTasks: demodTasks.reduce((count, t) => count + ((t.status !== 'COMPLETED' && t.status !== 'FAILED') ? 1 : 0), 0)
        };
    }, [streams, masintOverlays, demodTasks]);
    return (<div className={`flex flex-col h-full bg-slate-950 text-slate-100 ${className || ''}`}>
      {/* Top bar */}
      <header className="flex items-center justify-between px-4 py-2 bg-slate-900 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <span className={`flex items-center gap-1.5 px-2 py-0.5 text-xs font-medium rounded ${isConnected
            ? 'bg-green-500/20 text-green-400'
            : 'bg-red-500/20 text-red-400'}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${isConnected ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}/>
            {isConnected ? 'Connected' : 'Disconnected'}
          </span>
          {metrics && (<span className="text-xs text-slate-400 font-mono">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-slate-800 rounded-md p-0.5">
            {['waveform', 'spectrum', 'combined'].map((mode) => (<button key={mode} onClick={() => setViewMode(mode)} className={`px-3 py-1 text-xs font-medium rounded transition-colors ${viewMode === mode
          ...
          {/* Layout selector */}
          <select value={panelLayout} onChange={(e) => setPanelLayout(e.target.value)} className="px-2 py-1 text-xs bg-slate-800 border border-slate-700 rounded focus:outline-none focus:ring-1 focus:ring-cyan-500">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-80 flex-shrink-0 border-r border-slate-800 overflow-hidden">
          <SignalStreamList_1.SignalStreamList streams={streams} selectedStreamId={selectedStreamId} onSelectStream={handleSelectStream} onSubscribe={handleSubscribe} onUnsubscribe={handleUnsubscribe} subscribedIds={subscribedIds} className="h-full"/>
        </aside>

        {/* Center - Visualizations */}
        <main className="flex-1 flex flex-col overflow-hidden p-4 gap-4">
          {/* Waveform display */}
          {(viewMode === 'waveform' || viewMode === 'combined') && (<div className={`bg-slate-900 rounded-lg border border-slate-800 overflow-hidden ${viewMode === 'combined' ? 'flex-1' : 'h-full'}`}>
              <div className="px-4 py-2 bg-slate-800/50 border-b border-slate-700 flex items-center justify-between">
                ...
                {selectedStream && (<span className="text-xs text-slate-400">
                    {(selectedStream.centerFrequency / 1e6).toFixed(3)} MHz |{' '}
                    {selectedStream.modulation}
                  </span>)}
              </div>
              <div className="h-[calc(100%-40px)]">
                <WaveformRenderer_1.WaveformRenderer samples={displaySamples} onMetrics={setMetrics} config={{
          ...
          {/* Spectrum analyzer */}
          {(viewMode === 'spectrum' || viewMode === 'combined') && (<div className={`bg-slate-900 rounded-lg border border-slate-800 overflow-hidden ${viewMode === 'combined' ? 'h-48 flex-shrink-0' : 'h-full'}`}>
              <div className="px-4 py-2 bg-slate-800/50 border-b border-slate-700">
                <span className="text-sm font-medium">Spectrum Analysis</span>
              </div>
              <div className="h-[calc(100%-40px)]">
                <SpectrumAnalyzer_1.SpectrumAnalyzer samples={displaySamples} showPeaks={true} smoothing={0.85}/>
              </div>
            </div>)}
        </main>
        ...
        <aside className="w-96 flex-shrink-0 border-l border-slate-800 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-hidden">
            <MASINTOverlayPanel_1.MASINTOverlayPanel overlays={masintOverlays} className="h-1/2 border-b border-slate-800"/>
          </div>
          <div className="flex-1 overflow-hidden">
            <AgenticDemodulationPanel_1.AgenticDemodulationPanel tasks={demodTasks} availableStreams={streams} onStartDemodulation={handleStartDemodulation} onCancelTask={handleCancelTask} onViewResult={handleViewResult} className="h-full"/>
          </div>
        </aside>
      </div>
...
exports.SIGINTDashboard = SIGINTDashboard;
exports.default = exports.SIGINTDashboard;

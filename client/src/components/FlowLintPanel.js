...
exports.default = FlowLintPanel;
const react_1 = __importStar(require("react"));
const jquery_1 = __importDefault(require("jquery"));
function FlowLintPanel() {
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (!handlerBoundRef.current) {
            ...
            (0, jquery_1.default)('#q').on('input', function () {
                ...
                (0, jquery_1.default)('.lint-row').each(function () {
    ...
    return (<div className="p-4 rounded-2xl shadow">
      ...
      <ul className="text-sm">
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        {items.map((x, i) => (<li key={i} className={`lint-row border-b py-1 ${x.level === 'error' ? 'text-red-600' : ''}`}>
            {x.id} — {x.msg}
          </li>))}
      </ul>
    </div>);

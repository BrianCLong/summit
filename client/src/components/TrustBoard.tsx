...
export default function TrustBoard(){
  ...
  return (<div className="p-4 rounded-2xl shadow">
    ...
    <table className="w-full text-sm"><thead><tr><th>Node</th><th>Provider</th><th>Measurement</th><th>Verified</th></tr></thead>
      <tbody>{rows.map((n:AttestedNode)=>(<tr key={n.node_id} className="row border-b"><td>{n.node_id}</td><td>{n.provider}</td><td className="truncate">{n.measurement.slice(0,16)}…</td><td>✅</td></tr>))}</tbody>
    </table>
    <div className="mt-4 grid grid-cols-2 gap-4">
      <div className="rounded-2xl shadow p-3"><h4 className="font-semibold mb-2">DLP Incidents</h4><ul className="text-sm">{dlp.map((d:DlpFinding,i:number)=>(<li key={i} className="border-b py-1">{d.kind} • {d.severity} • {d.stepId}</li>))}</ul></div>
      <div className="rounded-2xl shadow p-3"><h4 className="font-semibold mb-2">DP Budgets</h4><ul className="text-sm">{dp.map((b:DpBudget,i:number)=>(<li key={i} className="border-b py-1">{b.tenant}/{b.dataset} • ε left {b.remaining.toFixed(2)}</li>))}</ul></div>
    </div>
  </div>);
}
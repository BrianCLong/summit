...
const NaturalLanguageAssistant = ({ investigationId, context, onTaskCreate = () => { }, onTaskComplete = () => { }, onEntityDiscovered = () => { }, onInsightGenerated = () => { }, enableVoiceInput = true, enableProactiveAssistance = true, className = '', }) => {
    ...
    const [activeTasks, setActiveTasks] = (0, react_1.useState)([]);
    const [capabilities, setCapabilities] = (0, react_1.useState)([]);
    const [investigationContext, setInvestigationContext] = (0, react_1.useState)(null);
    const [suggestions, setSuggestions] = (0, react_1.useState)([]);
    const [showCapabilities, setShowCapabilities] = (0, react_1.useState)(false);
        ...
        const mockCapabilities = [
            {
                id: 'entity-search',
                name: 'Entity Search',
                description: 'Search for specific entities across all connected data sources',
                category: 'search',
                icon: '🔍',
                parameters: [
                    {
...
What specific aspect would you like me to focus on?`,
    ...
    const shouldCreateTask = (message) => {
        const taskKeywords = [
            'analyze',
            'search',
            'investigate',
    ...
    const createInvestigationTask = (message) => {
        const taskTypes = [
            'entity_search',
    ...
    const executeTask = async (task) => {
        ...
        setActiveTasks((prev) => prev.map((t) => t.id === task.id
...
Key findings include multiple relevant entities and suspicious patterns that warrant further investigation. Would you like me to analyze these results in more detail?`,
    ...
    return (<div className={`natural-language-assistant ${className} h-full flex flex-col`}>
      {/* Header */}
      <div className="border-b p-4 bg-white">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            {investigationContext && (<span className="text-xs text-gray-500">
            ...
            <button onClick={() => setShowCapabilities(!showCapabilities)} className="px-3 py-1 bg-blue-100 text-blue-700 text-sm rounded hover:bg-blue-200">
              Capabilities
            </button>
          </div>
        </div>

        {activeTasks.filter((t) => t.status === 'running').length > 0 && (<div className="bg-blue-50 border border-blue-200 rounded-md p-2">
            ...
            {activeTasks
                ...
                .map((task) => (<div key={task.id} className="flex items-center gap-2 mt-1">
      ...
      {/* Capabilities Panel */}
      {showCapabilities && (<div className="border-b p-4 bg-gray-50">
          <h4 className="font-medium mb-3">Available Analysis Capabilities</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {capabilities.map((capability) => (<div key={capability.id} className="p-3 bg-white rounded-lg border text-sm">
        ...
        {messages.map((message) => (<div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-3xl rounded-lg p-4 ${message.role === 'user'
              ...
              {message.role === 'assistant' && (<div className="flex items-center gap-2 mb-2">
                  ...
                  {message.metadata?.confidence && (<span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">
              ...
              {message.metadata?.entities &&
                message.metadata.entities.length > 0 && (<div className="mt-3 pt-3 border-t">
                    <span className="text-sm font-medium">Entities Found:</span>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {message.metadata.entities.map((entity, index) => (<span key={index} className="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded">
              ...
              {message.metadata?.suggestedActions &&
                message.metadata.suggestedActions.length > 0 && (<div className="mt-3 pt-3 border-t">
                    <span className="text-sm font-medium">
                      Suggested Actions:
                    </span>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {message.metadata.suggestedActions.map((action, index) => (<button key={index} onClick={() => handleSuggestionClick(action.description)} className="px-3 py-1 bg-blue-100 text-blue-700 text-xs rounded hover:bg-blue-200">
                            {action.description}
                          </button>))}
                    </div>
                  </div>)}
      ...
      {/* Suggestions */}
      {suggestions.length > 0 && !isProcessing && (<div className="border-t p-3 bg-gray-50">
          <div className="text-xs text-gray-600 mb-2">Quick suggestions:</div>
          <div className="flex flex-wrap gap-2">
            {suggestions.map((suggestion, index) => (<button key={index} onClick={() => handleSuggestionClick(suggestion)} className="px-3 py-1 bg-white border text-sm rounded hover:bg-gray-50">
                {suggestion}
              </button>))}
          </div>
        </div>)}
      ...
      <div className="border-t p-4 bg-white">
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input ref={inputRef} type="text" value={currentMessage} onChange={(e) => setCurrentMessage(e.target.value)} placeholder="Ask me anything about your investigation..." className="flex-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={isProcessing}/>

          {enableVoiceInput && (<button type="button" onClick={startVoiceInput} disabled={isListening || isProcessing} className={`px-3 py-2 rounded-lg ${isListening
                ? 'bg-red-100 text-red-700'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'} disabled:opacity-50`}>
              {isListening ? '🔴' : '🎤'}
            </button>)}

          <button type="submit" disabled={!currentMessage.trim() || isProcessing} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
            {isProcessing ? '⏳' : 'Send'}
          </button>
        </form>
...
exports.default = NaturalLanguageAssistant;

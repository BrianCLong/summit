...
function SwarmDash() {
    ...
    (0, react_1.useEffect)(() => {
        fetch('/api/tgo/metrics')
            .then((r) => r.json())
            .then(setRows);
        (0, jquery_1.default)('#swarm-q').on('input', function () {
            const v = (0, jquery_1.default)(this).val()?.toString().toLowerCase() || '';
            (0, jquery_1.default)('.swarm-row').each(function () {
    ...
    return (<div className="p-4 rounded-2xl shadow">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((x, i) => (<tr key={i} className="swarm-row border-b">
              <td>{x.id}</td>
              <td>{x.lane}</td>

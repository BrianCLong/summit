...
export default function TimeTravelDiff(){
  const [a,setA]=useState(""); const [b,setB]=useState(""); const [rows,setRows]=useState<DiffRow[]>([]);
  async function run(){ const r = await fetch(`/api/replay/diff?a=${a}&b=${b}`); setRows(await r.json()); }

      ...
      <MapContainer center={center} zoom={zoom} style={{ width: '100%', height: '100%' }}>
        ...
        {(clusters || []).map((c, idx) => (
          <CircleMarker
            ...
            pathOptions={{ color: '#1976d2', fillColor: '#64b5f6', fillOpacity: 0.35 }}
          >
            <Popup>Cluster size: {c.size}</Popup>
          </CircleMarker>
        ))}
        {(clusterPolygons || []).map((poly, i) => (
          <>
            <Polygon
              key={`poly-${i}-a`}
              positions={poly}
              pathOptions={{ color: '#1976d2', weight: 1, fillOpacity: 0.18, fillColor: '#64b5f6' }}
            />
            <Polygon
              key={`poly-${i}-b`}
              positions={poly}
              pathOptions={{ color: '#1976d2', weight: 0, fillOpacity: 0.08, fillColor: '#90caf9' }}
            />
          </>
        ))}

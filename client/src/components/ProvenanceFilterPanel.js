...
exports.default = ProvenanceFilterPanel;
...
function ProvenanceFilterPanel({ onApply, initial, scope, id, }) {
    const [codes, setCodes] = (0, react_1.useState)(initial?.reasonCodeIn || []);
    const [from, setFrom] = (0, react_1.useState)(initial?.from || '');
    const [to, setTo] = (0, react_1.useState)(initial?.to || '');
    const [kinds, setKinds] = (0, react_1.useState)(initial?.kindIn || []);
    const [sources, setSources] = (0, react_1.useState)(initial?.sourceIn || []);
    const [contains, setContains] = (0, react_1.useState)(initial?.contains || '');
    const [copied, setCopied] = (0, react_1.useState)(false);
    const [busy, setBusy] = (0, react_1.useState)(false);
    const [exporting, setExporting] = (0, react_1.useState)(null);
    const EXPORT_MUT = (0, react_1.useMemo)(() => (0, client_1.gql) `
      mutation ExportProv(
        $incidentId: ID
        $investigationId: ID
    ...
    `, []);
    const [exportProv] = (0, client_1.useMutation)(EXPORT_MUT);
    const buildCurl = (scope, id) => {
        if (!scope || !id)
            return '';
        const opName = scope === 'incident' ? 'ProvByIncident' : 'ProvByInvestigation';
        const field = scope === 'incident'
        ...
        const filter = {};
        if (codes.length)
            filter.reasonCodeIn = codes;
        if (kinds.length)
            filter.kindIn = kinds;
        if (sources.length)
            filter.sourceIn = sources;
        if (from)
            filter.from = from;
        if (to)
            filter.to = to;
      ...
      <div className="flex gap-3 items-center flex-wrap">
        ...
        <label>
          ...
          <select multiple value={kinds} onChange={(e) => setKinds(Array.from(e.target.selectedOptions).map((o) => o.value))} className="ml-2 border p-1">
            {KNOWN_KINDS.map((c) => (<option key={c} value={c}>
                {c}
              </option>))}
          </select>
        </label>
        <label>
          ...
          <select multiple value={sources} onChange={(e) => setSources(Array.from(e.target.selectedOptions).map((o) => o.value))} className="ml-2 border p-1">
            {KNOWN_SOURCES.map((c) => (<option key={c} value={c}>
                {c}
              </option>))}
          </select>
        </label>
        <label>
          Contains:
          <input type="text" placeholder="free-text" value={contains} onChange={(e) => setContains(e.target.value)} className="ml-2 border p-1"/>
        </label>
        <label>
          From:
        ...
        <button className="border px-3 py-1" onClick={() => onApply({
        ...
        {scope && id && (<>
            <button className="border px-3 py-1" disabled={!!exporting} onClick={async () => {
                if (!scope || !id)
                    return;
                setExporting('json');
                try {
                    ...
                    if (contains && contains.trim().length)
                        filter.contains = contains.trim();
                    const variables = { format: 'json' };
                    if (scope === 'incident')
                        variables.incidentId = id;
                    else
                        variables.investigationId = id;
...
function CopyCurlButton({ scope, id, build, setCopied, setBusy, copied, busy, }) {
    if (!scope || !id)
        return null;
    return (<button className="border px-3 py-1" disabled={busy} onClick={async () => {

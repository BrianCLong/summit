...
exports.handlers = [
    // ServerStats query
    msw_1.graphql.query('ServerStats', (req, res, ctx) => {
        return res(ctx.data({
            serverStats: {
                uptime: '2d 14h 32m',
    ...
    msw_1.graphql.query('Investigations', (req, res, ctx) => {
        return res(ctx.data({
            getInvestigations: [
                {
                    id: '1',
    ...
    msw_1.graphql.query('GW_GraphData', (req, res, ctx) => {
        return res(ctx.data({
            graphData: {
                nodes: [
                    {
    ...
    msw_1.graphql.query('GW_SearchEntities', (req, res, ctx) => {
        return res(ctx.data({
            searchEntities: [
                {
                    id: 'search-1',
    ...
    // EntityDetails query
    msw_1.graphql.query('GW_EntityDetails', (req, res, ctx) => {
        const { variables } = req;
        return res(ctx.data({
            getEntityDetails: {
    ...
    // Health check
    msw_1.graphql.query('HealthCheck', (req, res, ctx) => {
        return res(ctx.data({
            health: 'OK',
        }));
    }),
    // Threat Analysis
    msw_1.graphql.query('ThreatAnalysis', (req, res, ctx) => {
        const { variables } = req;
        return res(ctx.data({
            threatAnalysis: {
    ...
    // Timeline Events
    msw_1.graphql.query('TimelineEvents', (req, res, ctx) => {
        return res(ctx.data({
            timelineEvents: [
                {
                    id: 'event-1',
    ...
    // Entity Enrichment
    msw_1.graphql.query('EntityEnrichment', (req, res, ctx) => {
        const { variables } = req;
        return res(ctx.data({
            entityEnrichment: {
                entityId: variables.entityId,

...
interface UseAdminRolesReturn extends UseAdminRolesState {
  ...
  updateRole: (roleId: string, payload: UpdateRolePayload) => Promise<DataEnvelope<OperationResult> | null>;
  deleteRole: (roleId: string) => Promise<DataEnvelope<OperationResult> | null>;
  assignRoleToUser: (userId: string, roleId: string, expiresAt?: string) => Promise<DataEnvelope<OperationResult> | null>;
  revokeRoleFromUser: (userId: string, roleId: string) => Promise<DataEnvelope<OperationResult> | null>;
}
...
export function useAdminRoles(): UseAdminRolesReturn {
  ...
  const fetchPermissions = useCallback(async () => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
      const envelope: DataEnvelope<PermissionListResult> = await RoleManagementAPI.listPermissions();
  ...
  const createRole = useCallback(async (payload: CreateRolePayload): Promise<DataEnvelope<OperationResult> | null> => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
  ...
  const deleteRole = useCallback(async (roleId: string): Promise<DataEnvelope<OperationResult> | null> => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
  ...
  const revokeRoleFromUser = useCallback(async (
    userId: string,
    roleId: string
  ): Promise<DataEnvelope<OperationResult> | null> => {
    setState(prev => ({ ...prev, loading: true, error: null }));

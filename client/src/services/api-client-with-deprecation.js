...
class ApiClient {
    ...
    async fetch(endpoint, options = {}) {
        ...
        if (response.status === 410) {
            ...
            throw new Error(`Endpoint ${endpoint} has been removed. ${errorData.message} ` +
    ...
    handleDeprecationHeaders(endpoint, response) {
        ...
        if (deprecationHeader === 'true') {
            ...
            if (!this.deprecationWarnings.has(endpoint)) {
                ...
                console.warn(`%c⚠️ API DEPRECATION WARNING`, style, `\nEndpoint: ${endpoint}`, sunsetDate ? `\nSunset Date: ${sunsetDate}` : '', daysUntilSunset !== null ? `\nDays Until Removal: ${daysUntilSunset}` : '', `\n${message}`, successorUrl ? `\nMigrate to: ${successorUrl}` : '', '\n\nThis warning will only be shown once per session.');
            }
...
function createApiClient() {
    return new ApiClient({
        baseUrl: (0, urls_1.getApiBaseUrl)(),
        apiVersion: 'v2.0.0',
        onDeprecationWarning: (warning) => {
            // Send to error tracking service (e.g., Sentry)
            if (typeof window !== 'undefined' && window.Sentry) {
                window.Sentry.captureMessage('API Deprecation Warning', {
                    level: 'warning',
                    extra: warning
                });
            }
            ...
            if (typeof window !== 'undefined' && window.analytics) {
                window.analytics.track('API Deprecation Warning', {

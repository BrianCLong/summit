const REDACT = /([A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,})|(\+?\d[\d\s\-]{7,})/gi;
export function scrub(line) {
  return line.replace(REDACT, '[REDACTED]');
}
...
export const FederationAPI = {
  ...
  register: (payload) =>
    apiFetch('/api/federation/instances', {
      method: 'POST',
      body: JSON.stringify(payload),
    }),
  update: (id, payload) =>
    apiFetch(`/api/federation/instances/${encodeURIComponent(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(payload),
    }),
  remove: (id) =>
  ...
  test: (id) =>
    apiFetch(`/api/federation/instances/${encodeURIComponent(id)}/test`, {
      method: 'POST',
    }),
...
export const MLAPI = {
  ...
  suggestLinks: (payload) =>
    apiFetch('/api/ml/suggest-links', {
      method: 'POST',
      body: JSON.stringify(payload),
    }),
};
...
export const TemplatesAPI = {
  ...
  update: (id, payload) =>
    apiFetch(`/api/templates/${encodeURIComponent(id)}`, {
      method: 'PUT',
      body: JSON.stringify(payload),
    }),
...
export const AdminAPI = {
  ...
  setRole: (id, role) =>
    apiFetch(`/api/admin/users/${encodeURIComponent(id)}/role`, {
      method: 'PATCH',
...
export const VisionAPI = {
  analyze: ({ imageUrl, imageBase64, mode }) =>
    apiFetch('/api/vision/analyze', {
      method: 'POST',
...
export const GeointAPI = {
  clusters: ({ points, epsilonKm = 0.1, minPoints = 3 }) =>
    apiFetch('/api/geoint/clusters', {
      method: 'POST',
      body: JSON.stringify({ points, epsilonKm, minPoints }),
    }),
  timeSeries: ({ points, intervalMinutes = 60 }) =>
    apiFetch('/api/geoint/time-series', {
      method: 'POST',

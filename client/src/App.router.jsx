...
import ConsentGraphExplorer from './features/consent-graph/ConsentGraphExplorer.tsx';
...
const navigationItems = [
  ...
  { path: '/consent-graph', label: 'Consent Graph Explorer', icon: <Policy /> },
...
function NavigationDrawer({ open, onClose }) {
  ...
  const handleNavigation = (item) => {
    if (item.external) {
      const target = item.path.startsWith('http')
        ? item.path
        : window.location.origin.replace(':3000', ':4000') + item.path;
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3, mt: 8 }}>
        <Routes>
          ...
          <Route element={<ProtectedRoute />}>
            ...
            <Route path="/consent-graph" element={<ConsentGraphExplorer />} />

...
exports.LhoDashboard = LhoDashboard;
const react_1 = require("react");
const sampleHold_json_1 = __importDefault(require("./data/sampleHold.json"));
const utils_1 = require("./utils");
const dataset = sampleHold_json_1.default;
function formatDateRange(start, end) {
    const startDate = new Date(start);
    const endDate = new Date(end);
    return `${startDate.toUTCString()} → ${endDate.toUTCString()}`;
}
function LhoDashboard() {
    const [verification, setVerification] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        let cancelled = false;
        (0, utils_1.verifyCustodyChain)(dataset.custodyChain)
            .then((result) => {
            if (!cancelled) {
    ...
    const systemKeys = (0, react_1.useMemo)(() => Object.keys(dataset.scope).sort(), []);
    const diffSections = (0, react_1.useMemo)(() => {
        return [
    ...
    return (<div className="space-y-6">
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <table className="mt-3 w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-200">
            {systemKeys.map((system) => (<tr key={system}>
                <td className="py-2 font-medium text-slate-700">{system}</td>
                <td className="py-2 text-slate-900">
                  <ul className="list-inside list-disc space-y-1">
                    {dataset.scope[system].map((item) => (<li key={item}>{item}</li>))}
                  </ul>
                </td>
              </tr>))}
          </tbody>
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 grid gap-4 md:grid-cols-3">
          {diffSections.map(({ label, data }) => (<div key={label} className="rounded border border-slate-100 bg-slate-50 p-3">
              <h3 className="text-sm font-semibold text-slate-600">{label}</h3>
              {Object.keys(data).length === 0 ? (<p className="mt-2 text-xs text-slate-500">None</p>) : (<ul className="mt-2 space-y-2 text-sm text-slate-700">
                  {Object.entries(data)
                    .sort(([a], [b]) => a.localeCompare(b))
                    .map(([system, resources]) => (<li key={system}>
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <span className={`rounded-full px-3 py-1 text-sm font-semibold ${verification?.valid ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>
            {verification?.valid ? 'Chain intact' : verification ? 'Attention required' : 'Checking...'}
          </span>
        </div>
        {verification?.message && !verification.valid && (<p className="mt-2 text-sm text-amber-700">{verification.message}</p>)}
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[480px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-200">
              {dataset.custodyChain.map((event) => (<tr key={event.sequence}>
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 grid gap-4 md:grid-cols-2">
          {dataset.reports
            ...
            .map((report) => (<div key={report.system} className="rounded border border-slate-100 bg-slate-50 p-3">
                ...
                <dl className="mt-2 space-y-2 text-sm text-slate-600">
                  ...
                  <div>
                    ...
                    <dd>
                      {Object.keys(report.tagged).length === 0 ? ('None') : (<ul className="list-inside list-disc space-y-1">
                          {Object.entries(report.tagged)
                    .sort(([a], [b]) => a.localeCompare(b))
                    .map(([resource, tags]) => (<li key={resource}>

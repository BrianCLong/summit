...
function CommandPalette({ open, onClose, basePath, }) {
    const navigate = (0, react_router_dom_1.useNavigate)();
    const { hasPermission } = (0, AuthContext_1.useAuth)();
    const [query, setQuery] = react_1.default.useState('');
    const commands = react_1.default.useMemo(() => [
        {
            id: 'dashboard',
    ...
    const filtered = react_1.default.useMemo(() => {
        ...
        const commandPool = commands.filter((command) => !command.requires || hasPermission?.(command.requires));
        if (!trimmed)
            return commandPool;
    ...
    if (!open)
        return null;
    return (<div className="fixed inset-0 z-40 flex items-start justify-center bg-slate-950/40 pt-24">
      <div className="w-full max-w-xl rounded-2xl border border-slate-700/80 bg-slate-900 shadow-2xl">
        <div className="border-b border-slate-700/70 px-4 py-3">
          <input autoFocus className="w-full rounded-lg bg-slate-800/60 px-4 py-2 text-sm text-slate-100 placeholder:text-slate-400 focus-visible:outline focus-visible:outline-emerald-400 focus-visible:outline-offset-2" placeholder="Search commands or actions…" value={query} onChange={(event) => setQuery(event.target.value)}/>
        </div>
        <ul className="max-h-80 overflow-y-auto p-2">
          {filtered.map((command) => (<li key={command.id}>
              <button type="button" onClick={() => {
                ...
                {command.shortcut ? (<span className="rounded-lg border border-slate-600 px-2 py-1 text-xs text-slate-300">
                    {command.shortcut}
                  </span>) : null}
              </button>
            </li>))}
          {filtered.length === 0 ? (<li className="px-4 py-6 text-center text-sm text-slate-400">

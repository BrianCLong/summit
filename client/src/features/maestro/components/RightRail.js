...
exports.RightRail = RightRail;
const react_1 = __importDefault(require("react"));
function RightRail({ view }) {
    const explainers = {
        dashboard: {
    ...
    return (<aside className="hidden w-80 shrink-0 border-l border-slate-800/80 bg-slate-950/40 p-4 text-slate-200 xl:block">
      <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-400">
        Explain this view
      </h3>
      <p className="mt-2 text-sm text-slate-300">{content.body}</p>
      <ul className="mt-4 space-y-3 text-sm text-slate-300">
        {content.items.map((item) => (<li key={item} className="flex items-start gap-2">

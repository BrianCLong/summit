  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-400">Local audit log showing who accessed sensitive surfaces and why.</p>
      </header>
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm text-slate-200">
        ...
        {auditLog.length === 0 ? (
          ...
          <ul className="mt-3 space-y-3">
            {auditLog.map((entry) => (
              <li key={entry.id} className="rounded-xl border border-slate-800/70 bg-slate-950/70 p-4">
                <p className="text-xs uppercase text-slate-500">{new Date(entry.timestamp).toLocaleString()}</p>

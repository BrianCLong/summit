...
function StatCard({ label, value, helper, }) {
    return (<div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 shadow-lg">
...
function QueueDepthChart() {
    return (<div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
      ...
      <div className="mt-4 flex items-end gap-1">
        {mockData_1.queueDepthHistory.map((point) => (<div key={point.hour} className="flex-1 rounded-t-lg bg-emerald-500/70" style={{ height: `${point.depth * 4}px` }} aria-label={`Hour ${point.hour}, depth ${point.depth}`}/>))}
      </div>
    </div>);
}
function PolicyDenialsCard() {
    return (<div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
      ...
      <ul className="mt-3 space-y-3 text-sm text-slate-300">
        {mockData_1.policyDenials.map((denial) => (<li key={denial.id} className="rounded-xl border border-slate-800/60 bg-slate-950/60 p-3">
...
function SloWidget() {
    return (<div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
      ...
      <table className="mt-3 w-full text-sm text-slate-300">
        ...
        <tbody>
          {mockData_1.sloSnapshots.map((snapshot) => (<tr key={snapshot.service} className="border-t border-slate-800/70">
      ...
      <a href="/runs/run-1?filter=slo" className="mt-3 inline-flex items-center text-xs font-semibold text-emerald-400 hover:text-emerald-300">
        View impacted runs →
      </a>
    </div>);
}
function DashboardPage() {
    const totalPipelines = mockData_1.pipelineRecords.length;
    const healthy = mockData_1.pipelineRecords.filter((pipeline) => pipeline.status === 'healthy').length;
    const degraded = mockData_1.pipelineRecords.filter((pipeline) => pipeline.status === 'degraded').length;
    const longestLeadTime = mockData_1.pipelineRecords.reduce((max, pipeline) => Math.max(max, pipeline.leadTimeMinutes), 0);
    const averageChangeFailure = mockData_1.pipelineRecords.reduce((total, pipeline) => total + pipeline.dora.changeFailureRate, 0) / totalPipelines;

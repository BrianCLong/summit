...
exports.PipelinesPage = PipelinesPage;
...
function StatusPill({ status }) {
    ...
    return (<span className={`rounded-full border px-3 py-1 text-xs font-medium ${map[status]}`}>
      {status}
    </span>);
}
function PipelinesPage() {
    ...
    const [search, setSearch] = react_1.default.useState(params.get('q') ?? '');
    const [ownerFilter, setOwnerFilter] = react_1.default.useState(params.get('owner') ?? '');
    const debouncedSearch = (0, useMaestroHooks_1.useDebouncedValue)(search, 150);
    const debouncedOwner = (0, useMaestroHooks_1.useDebouncedValue)(ownerFilter, 150);
    const [filtered, setFiltered] = react_1.default.useState(mockData_1.pipelineRecords);
    ...
    react_1.default.useEffect(() => {
        ...
        setFiltered(mockData_1.pipelineRecords.filter((pipeline) => {
            const matchName = !q || pipeline.name.toLowerCase().includes(q);
            const matchOwner = !owner ||
                pipeline.owners.some((candidate) => candidate.toLowerCase().includes(owner));
    ...
    const rowRenderer = ({ index, key, style }) => {
        ...
        if (!pipeline)
            return null;
        return (<button key={key} type="button" style={style} onClick={() => navigate(`/pipelines/${pipeline.id}`)} className="flex w-full items-center gap-6 border-b border-slate-800/60 bg-slate-900/50 px-4 py-3 text-left text-sm text-slate-200 transition hover:bg-slate-800/60 focus-visible:outline focus-visible:outline-emerald-400">
    ...
    return (<div className="flex h-full flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-3">
        <label className="flex flex-1 min-w-[240px] flex-col text-xs text-slate-300">
          Search
          <input className="mt-1 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100 focus-visible:outline focus-visible:outline-emerald-400" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name"/>
        </label>
        <label className="flex w-56 flex-col text-xs text-slate-300">
          Owner
          <input className="mt-1 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100 focus-visible:outline focus-visible:outline-emerald-400" value={ownerFilter} onChange={(event) => setOwnerFilter(event.target.value)} placeholder="devon"/>
        </label>
      </div>
      <div className="flex-1 overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/60">
        ...
        <react_virtualized_1.AutoSizer disableHeight>
          {({ width }) => (<react_virtualized_1.List width={width} height={520} rowCount={filtered.length} rowHeight={92} rowRenderer={rowRenderer} overscanRowCount={6}/>)}
        </react_virtualized_1.AutoSizer>
      </div>
    </div>);
}
exports.default = PipelinesPage;

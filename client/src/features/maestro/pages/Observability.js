    ...
    return (<div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
        ...
        <table className="mt-4 w-full text-sm text-slate-300">
          ...
          <tbody>
            {mockData_1.sloSnapshots.map((snapshot) => (<tr key={snapshot.service} className="border-t border-slate-800/60">
                <td className="py-2 text-slate-200">{snapshot.service}</td>
                <td>
                  <span className={snapshot.latencyP95Ms > mockData_1.sloBudget.latencyBudgetMs
                ? 'text-amber-300'
                : 'text-slate-300'}>
                ...
                <td>
                  <span className={snapshot.errorRate > mockData_1.sloBudget.errorBudget
                ? 'text-red-300'
                : 'text-slate-300'}>

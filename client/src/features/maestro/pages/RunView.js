...
exports.RunViewPage = RunViewPage;
...
function RunViewPage() {
    const { runId = 'run-1' } = (0, react_router_dom_1.useParams)();
    const run = mockData_1.runRecords.find((candidate) => candidate.id === runId) ?? mockData_1.runRecords[0];
    const navigate = (0, react_router_dom_1.useNavigate)();
    const { requestReason } = (0, ReasonForAccessContext_1.useReasonForAccess)();
    const [activeTab, setActiveTab] = react_1.default.useState('logs');
    const allLogs = react_1.default.useMemo(() => buildLogs(run.id), [run.id]);
    const [followTail, setFollowTail] = react_1.default.useState(true);
    const visibleLogs = (0, useMaestroHooks_1.useLiveLogFeed)(allLogs.map((line) => `${line.ts} ${line.level.toUpperCase()} ${line.message}`), {
        followTail,
        intervalMs: 250,
    });
    const [search, setSearch] = react_1.default.useState('');
    const debouncedSearch = (0, useMaestroHooks_1.useDebouncedValue)(search, 250);
    const [highlightIndex, setHighlightIndex] = react_1.default.useState(null);
    const containerRef = react_1.default.useRef(null);
    const [artifactsReason, setArtifactsReason] = react_1.default.useState('');
    ...
    const handleJumpToError = react_1.default.useCallback(() => {
    ...
    const handleTabChange = react_1.default.useCallback(async (tab) => {
        ...
        <div className="flex-1 rounded-2xl border border-slate-800 bg-slate-950/60">
          <div className="flex items-center justify-between border-b border-slate-800/70 px-4 py-3 text-sm">
            <div className="space-x-2">
              <button type="button" onClick={() => setActiveTab('logs')} className={`rounded-lg px-3 py-1 ${activeTab === 'logs' ? 'bg-emerald-500 text-slate-950' : 'text-slate-300 hover:text-white'}`}>
                Logs
              </button>
              <button type="button" onClick={() => void handleTabChange('artifacts')} className={`rounded-lg px-3 py-1 ${activeTab === 'artifacts' ? 'bg-emerald-500 text-slate-950' : 'text-slate-300 hover:text-white'}`}>
                Artifacts
              </button>
              <button type="button" onClick={() => setActiveTab('metadata')} className={`rounded-lg px-3 py-1 ${activeTab === 'metadata' ? 'bg-emerald-500 text-slate-950' : 'text-slate-300 hover:text-white'}`}>
                Metadata
              </button>
            </div>
            {activeTab === 'logs' ? (<div className="flex items-center gap-3 text-xs">
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={followTail} onChange={(event) => setFollowTail(event.target.checked)} className="h-4 w-4 rounded border-slate-600 bg-slate-950"/>
                  Follow tail
                </label>
                <button type="button" onClick={handleJumpToError} className="rounded-lg border border-emerald-400/50 px-2 py-1 text-emerald-300 hover:border-emerald-300">
                  Jump to first error
                </button>
                <input className="rounded-lg border border-slate-700 bg-slate-950 px-2 py-1 text-xs text-slate-100" placeholder="Find" value={search} onChange={(event) => setSearch(event.target.value)}/>
              </div>) : null}
          </div>
          <div className="h-[420px] overflow-auto" ref={containerRef}>
            {activeTab === 'logs' ? (<pre className="space-y-1 px-4 py-3 text-xs leading-5">
                {visibleLogs.map((line, index) => (<code 
            // eslint-disable-next-line react/no-array-index-key
            key={`${line}-${index}`} className={`block rounded px-2 py-1 ${highlightIndex === index ? 'bg-emerald-500/30 text-white' : 'text-slate-300'}`}>
                    {line}
                  </code>))}
              </pre>) : null}
            {activeTab === 'artifacts' ? (<div className="p-4 text-sm text-slate-300">
            ...
            {activeTab === 'metadata' ? (<div className="space-y-2 p-4 text-xs text-slate-400">

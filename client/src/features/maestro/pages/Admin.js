...
function AdminPage() {
    const { auditLog } = (0, ReasonForAccessContext_1.useReasonForAccess)();
    return (<div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm text-slate-200">
        ...
        {auditLog.length === 0 ? (<p className="mt-3 text-slate-400">No access recorded yet.</p>) : (<ul className="mt-3 space-y-3">
            {auditLog.map((entry) => (<li key={entry.id} className="rounded-xl border border-slate-800/70 bg-slate-950/70 p-4">

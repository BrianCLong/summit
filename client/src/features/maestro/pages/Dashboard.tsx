...
export function DashboardPage() {
  ...
  const averageChangeFailure =
    pipelineRecords.reduce((total, pipeline) => total + pipeline.dora.changeFailureRate, 0) /
    totalPipelines;
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-400">
          p95 load &lt; 1.5s with 2k pipelines — this view hydrates from cached telemetry.
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm text-slate-300">
        ...
        <p className="mt-2">
          ...
          <span className="font-semibold text-emerald-300">{averageChangeFailure.toFixed(1)}%</span>
          . Auto retry reasons and policy denials stream into this widget every 30 seconds via SSE.
        </p>
      </section>
    </div>
  );

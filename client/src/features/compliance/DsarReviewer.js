...
const ProofList = ({ proofs }) => {
    ...
    return (<ul style={{ margin: '8px 0 0', paddingLeft: 16 }}>
      {proofs.map((proof) => (<li key={`${proof.type}-${proof.connector}-${proof.hash}`} style={{ marginBottom: 4 }}>
          <strong>{proof.connector}</strong> — {proof.type} proof{' '}
          <code>{proof.hash.slice(0, 8)}…</code>
        </li>))}
    </ul>);
};
const DsarReviewer = ({ requests, onReplay, onInspect, }) => {
    const [activeId, setActiveId] = (0, react_1.useState)(null);
    const sortedRequests = (0, react_1.useMemo)(() => [...requests].sort((a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime()), [requests]);
    const activeRequest = (0, react_1.useMemo)(() => sortedRequests.find((request) => request.id === activeId) ?? null, [sortedRequests, activeId]);
    return (<div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
      <div style={{ flex: '1 1 50%' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            return (<tr key={request.id} style={{
                  ...
                  <td style={{ padding: '8px 12px' }}>
                    {request.replayAvailable && (<button type="button" onClick={(event) => {
      ...
      <aside style={{
        ...
        {activeRequest ? (<div>
            ...
            {activeRequest.exportLocation && (<p style={{ margin: '4px 0' }}>
                <strong>Export pack:</strong>{' '}
                <code>{activeRequest.exportLocation}</code>
              </p>)}
            {activeRequest.proofs && activeRequest.proofs.length > 0 && (<div style={{ marginTop: 16 }}>
                <h4 style={{ margin: '0 0 8px' }}>Proofs</h4>
                <ProofList proofs={activeRequest.proofs}/>
              </div>)}
          </div>) : (<p style={{ fontStyle: 'italic' }}>
            Select a request to inspect its fulfillment artifacts.
          </p>)}
      </aside>
    </div>);
};
exports.DsarReviewer = DsarReviewer;
exports.default = exports.DsarReviewer;

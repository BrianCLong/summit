...
type Sched = { id: string; runbook: string; cron: string; enabled: boolean; last_run_at?: string };
...
export default function SchedulesPanel() {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 8, maxWidth: 900, marginBottom: 12 }}>
        ...
        <button disabled={busy || !runbook || !cron} onClick={create}>Create</button>
      </div>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead><tr><th align="left">Runbook</th><th align="left">Cron</th><th align="left">Enabled</th><th align="left">Last Run</th><th /></tr></thead>
        <tbody>
          {items.map(it => (
            <tr key={it.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td>
                <button onClick={()=>toggle(it.id, !it.enabled)}>{it.enabled?'Disable':'Enable'}</button>
              </td>
            </tr>
          ))}
        </tbody>

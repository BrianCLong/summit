...
export default function Marketplace(){
  ...
  return (
    <div className="p-4 rounded-2xl shadow">
      ...
      <table className="w-full text-sm"><thead><tr><th>Name</th><th>Type</th><th>Version</th><th>Residency</th><th>Action</th></tr></thead>
        <tbody>{items.map((s: MarketplaceItem)=>(
          <tr key={s.name+s.version} className="border-b">
            <td className="font-mono">{s.name}</td><td>{s.type}</td><td>{s.version}</td><td>{(s.residency||[]).join(',')}</td>
            <td><button disabled={busy} onClick={()=>install(s.name,s.version)} className="px-2 py-1 rounded-2xl shadow">Install</button></td>
          </tr>))}
        </tbody>
      </table>
    </div>

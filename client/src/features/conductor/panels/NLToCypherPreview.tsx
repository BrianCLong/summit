...
async function fetchNL2Cypher(task: string): Promise<{ query: string; estRows: number; estCost: string }> {
  ...
  // Fallback: dev mock
  const mock = { query: `// MATCH (n)-[r]->(m) WHERE n.name CONTAINS '${task}' RETURN n,r,m LIMIT 50`, estRows: 42, estCost: '~$0.002' };
  await new Promise((r) => setTimeout(r, 250));
  return mock;
}

export function NLToCypherPreview() {
  const [task, setTask] = useState('Show entities related to Project Atlas in last 30 days');
    ...
    <Card>
      <CardContent>
        ...
        <Box sx={{ display: 'grid', gap: 2 }}>
          <TextField label="Task" fullWidth value={task} onChange={(e) => setTask(e.target.value)} multiline minRows={2} />
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <Button variant="contained" onClick={preview} disabled={disabled || loading}>
              {loading ? 'Generating…' : 'Preview Query'}
            </Button>
            {ts && <Chip size="small" label={`Last updated ${ts}`} />}
            {estRows != null && <Chip size="small" label={`~${estRows} rows`} />}
            {estCost && <Chip size="small" label={`est cost ${estCost}`} />}
          </Box>
          <TextField label="Generated Query" fullWidth value={query} minRows={3} multiline InputProps={{ readOnly: true }} />
        </Box>
      </CardContent>
    </Card>
  );

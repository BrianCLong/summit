...
export type Denial = { time: string; tenant?: string; caseId?: string; reason: string; rule: string };
  ...
  return (
    <Card>
      <CardContent>
        ...
        {denials.length === 0 ? (
          <Typography variant="body2" color="text.secondary">No budget denials recorded. Cost guard active.</Typography>
        ) : (
          <List dense>
            {denials.map((d, i) => (

...
const TrendingUp = lucide_react_1.ArrowUpRight;
const Card = ({ className = '', children, ...props }) => <div className={`rounded-lg border bg-card text-card-foreground shadow-sm ${className}`} {...props}>{children}</div>;
const CardHeader = ({ className = '', children, ...props }) => <div className={`flex flex-col space-y-1.5 p-6 ${className}`} {...props}>{children}</div>;
const CardTitle = ({ className = '', children, ...props }) => <h3 className={`text-2xl font-semibold leading-none tracking-tight ${className}`} {...props}>{children}</h3>;
const CardContent = ({ className = '', children, ...props }) => <div className={`p-6 pt-0 ${className}`} {...props}>{children}</div>;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const Badge = ({ variant = 'default', className = '', children, ...props }) => <div className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${className}`} {...props}>{children}</div>;
function OperationsDashboard() {
    ...
    if (loading) {
        ...
        <div className="text-center">
          <lucide_react_1.Activity className="w-12 h-12 animate-spin mx-auto mb-4"/>
    ...
    return (<div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className={data.firingAlerts.length > 0 ? 'border-orange-500' : ''}>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <lucide_react_1.Activity className="w-4 h-4 text-orange-500"/>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <lucide_react_1.GitBranch className="w-4 h-4 text-blue-500"/>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center">
            <lucide_react_1.AlertCircle className="w-5 h-5 mr-2 text-red-500"/>
            Active Incidents
          </CardTitle>
        </CardHeader>
        <CardContent>
          {data.activeIncidents.length === 0 ? (<div className="flex items-center justify-center py-8 text-gray-500">
              <lucide_react_1.CheckCircle className="w-6 h-6 mr-2 text-green-500"/>
              No active incidents
            </div>) : (<div className="space-y-3">
              {data.activeIncidents.map((incident) => (<div key={incident.id} className="flex items-start justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {incident.customer_impact && (<Badge variant="destructive">Customer Impact</Badge>)}
                    </div>
                  ...
                  <a href={`/companyos/incidents/${incident.id}`} className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    View →
                  </a>
                </div>))}
            </div>)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center">
              <lucide_react_1.Activity className="w-5 h-5 mr-2 text-orange-500"/>
              Firing Alerts
            </CardTitle>
          </CardHeader>
          <CardContent>
            {data.firingAlerts.length === 0 ? (<div className="flex items-center justify-center py-8 text-gray-500">
                <lucide_react_1.CheckCircle className="w-6 h-6 mr-2 text-green-500"/>
                No firing alerts
              </div>) : (<div className="space-y-3 max-h-96 overflow-y-auto">
                {data.firingAlerts.map((alert) => (<div key={alert.id} className="p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-2 mb-1">
                      <Badge className={getSeverityColor(alert.severity)}>
                        {alert.severity}
                      </Badge>
                      {alert.service_name && (<span className="text-xs text-gray-600">{alert.service_name}</span>)}
                    </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center">
              <lucide_react_1.GitBranch className="w-5 h-5 mr-2 text-blue-500"/>
              Recent Deployments
            </CardTitle>
          </CardHeader>
          <CardContent>
            {data.recentDeployments.length === 0 ? (<div className="flex items-center justify-center py-8 text-gray-500">
                No recent deployments
              </div>) : (<div className="space-y-3 max-h-96 overflow-y-auto">
                {data.recentDeployments.map((deployment) => (<div key={deployment.id} className="p-3 border rounded-lg hover:bg-gray-50 transition-colors">
      ...
      {/* SLO Violations */}
      {data.sloViolations.length > 0 && (<Card>
          <CardHeader>
            <CardTitle className="flex items-center">
          ...
          <CardContent>
            <div className="space-y-3">
              {data.sloViolations.map((violation) => (<div key={violation.id} className="p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between">
                    <div>

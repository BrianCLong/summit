...
export default function OverridesPanel() {
  ...
  const formatDuration = (createdAt: string, expiresAt: string) => {
    const duration = Math.round((new Date(expiresAt).getTime() - new Date(createdAt).getTime()) / (1000 * 60 * 60));
    return `${duration}h`;
  };
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <label style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontWeight: 600, marginBottom: 4 }}>Tenant ID *</span>
              <input
            ...
            <label style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontWeight: 600, marginBottom: 4 }}>Override Type</span>
              <select
                value={newOverride.override_type}
                onChange={(e) => setNewOverride({ ...newOverride, override_type: e.target.value as any })}
                style={{ padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
              >
            ...
            <label style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontWeight: 600, marginBottom: 4 }}>Duration (hours)</span>
              <input
                type="number"
                min="1"
                max="24"
                value={newOverride.duration_hours}
                onChange={(e) => setNewOverride({ ...newOverride, duration_hours: Number(e.target.value) })}
                style={{ padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
              />
            </label>

            <label style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontWeight: 600, marginBottom: 4 }}>Dual Approval</span>
              <label style={{ display: 'flex', alignItems: 'center', marginTop: 8 }}>
                <input
                  type="checkbox"
                  checked={newOverride.dual_approval}
                  onChange={(e) => setNewOverride({ ...newOverride, dual_approval: e.target.checked })}
                  style={{ marginRight: 8 }}
                />
          ...
          {newOverride.override_type === 'budget_increase' && (
            <label style={{ display: 'flex', flexDirection: 'column', marginBottom: 16 }}>
              <span style={{ fontWeight: 600, marginBottom: 4 }}>Multiplier</span>
              <input
                type="number"
                min="1.1"
                max="10"
                ...
                onChange={(e) => setNewOverride({ 
                  ...newOverride, 
                  config: { ...newOverride.config, multiplier: Number(e.target.value) }
                })}
                style={{ padding: '8px', border: '1px solid #ccc', borderRadius: '4px' }}
              />
            </label>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #dee2e6' }}>
          <thead>
            <tr style={{ background: '#f8f9fa' }}>
              ...
              <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Created By</th>
          ...
          <tbody>
            {overrides.length === 0 ? (
              ...
              overrides.map((override) => (
                <tr key={override.id} style={{ borderBottom: '1px solid #dee2e6' }}>

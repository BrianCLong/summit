...
export default function OverridesPanel() {
  ...
  const formatDuration = (createdAt: string, expiresAt: string) => {
    const duration = Math.round(
      (new Date(expiresAt).getTime() - new Date(createdAt).getTime()) / (1000 * 60 * 60)
    );
    return `${duration}h`;
  };
    ...
    <div style={{ padding: 24, maxWidth: 1200 }}>
      ...
      {!canManageOverrides && (
        <p style={{ marginTop: -12, marginBottom: 16, color: "#6c757d" }}>
          You have read-only access to overrides. Management actions are disabled until elevated
            ...
            {overrides.length === 0 ? (
              ...
              overrides.map((override) => (
                <tr key={override.id} style={{ borderBottom: "1px solid #dee2e6" }}>
                  <td style={{ padding: "12px" }}>{getStatusBadge(override.status)}</td>
                  <td style={{ padding: "12px", fontWeight: 500 }}>{override.tenant_id}</td>

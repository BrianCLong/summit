...
exports.default = AdminStudio;
const react_1 = __importDefault(require("react"));
const OverridesPanel_1 = __importDefault(require("./OverridesPanel"));
const CostExplorer_1 = __importDefault(require("./CostExplorer"));
function AdminStudio() {
    const [cfg, setCfg] = react_1.default.useState(null);
    const [loading, setLoading] = react_1.default.useState(true);
    const [tenantId, setTenantId] = react_1.default.useState('');
    const [n8nInfo, setN8nInfo] = react_1.default.useState({ prefixes: [], flows: [] });
    const [n8nConfig, setN8nConfig] = react_1.default.useState({ allowedPrefixes: [], deniedPrefixes: [], allowedFlows: [] });
    const [newFlow, setNewFlow] = react_1.default.useState('');
    const [opaStatus, setOpaStatus] = react_1.default.useState(null);
    const [bundleStatus, setBundleStatus] = react_1.default.useState(null);
    const [bundleSource, setBundleSource] = react_1.default.useState(null);
    const [opaSince, setOpaSince] = react_1.default.useState(null);
    const [activeTab, setActiveTab] = react_1.default.useState('config');
    const [saveMessage, setSaveMessage] = react_1.default.useState('');
    ...
    async function validateOPA() {
        try {
            ...
            setOpaStatus(j);
            if (j?.ok && !opaSince)
                setOpaSince(new Date().toLocaleTimeString());
        }
        catch {
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            case 'overrides':
                return <OverridesPanel_1.default />;
            case 'cost-explorer':
                return <CostExplorer_1.default />;
            case 'help':
...
./ops/post-deploy-smoke-tests.sh`}
            ...
            {loading ? (<div style={{ padding: 24 }}>Loading…</div>) : (<div style={{ padding: 24 }}>
                ...
                <div style={{
                  ...
                  <label style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <select value={cfg?.MODEL_PROVIDER || 'openai'} onChange={(e) => cfg && setCfg({ ...cfg, MODEL_PROVIDER: e.target.value })}>
                  ...
                  <label style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <input value={cfg?.MODEL_NAME || ''} onChange={(e) => cfg && setCfg({ ...cfg, MODEL_NAME: e.target.value })} placeholder="e.g., gpt-4-turbo-preview"/>
                  </label>
                  <label style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 600 }}>
                      TEMPERATURE (0.0-1.0)
                    </span>
                    <input type="number" step="0.1" min="0" max="1" value={cfg?.TEMPERATURE ?? 0.2} onChange={(e) => cfg && setCfg({ ...cfg, TEMPERATURE: Number(e.target.value) })}/>
                  </label>
                  <label style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 600 }}>TOP_P (0.0-1.0)</span>
                    <input type="number" step="0.1" min="0" max="1" value={cfg?.TOP_P ?? 1.0} onChange={(e) => cfg && setCfg({ ...cfg, TOP_P: Number(e.target.value) })}/>
                  </label>
                  <label style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 600 }}>MAX_TOKENS</span>
                    <input type="number" min="1" max="100000" value={cfg?.MAX_TOKENS ?? 4096} onChange={(e) => cfg && setCfg({ ...cfg, MAX_TOKENS: Number(e.target.value) })}/>
                  </label>
                  <label style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 600 }}>BUDGET_CAP_USD</span>
                    <input type="number" step="0.01" min="0" value={cfg?.BUDGET_CAP_USD ?? 10} onChange={(e) => cfg && setCfg({
                            ...cfg,
                ...
                <div style={{
                  ...
                  {cfg && Object.keys(cfg).map((k) => (<label key={k} style={{ display: 'flex', flexDirection: 'column' }}>
                      <span style={{ fontWeight: 600 }}>{k}</span>
                      {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                      {typeof cfg[k] === 'boolean' ? (<input type="checkbox" 
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            checked={!!cfg[k]} onChange={(e) => setCfg({
                                    ...cfg,
                                    [k]: e.target.checked,
                                })}/>) : (<input 
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            value={String(cfg[k])} onChange={(e) => setCfg({
                                    ...cfg,
                                    [k]: e.target.value,
                                })}/>)}
                    </label>))}
    ...
    return (<div style={{ padding: 0 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <label>
            <input type="checkbox" checked={!!cfg?.TEMPORAL_ENABLED} onChange={async (e) => {
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 16, flexWrap: 'wrap' }}>
        <div style={{
          ...
          {opaStatus?.evalOk !== undefined && (<div>
              Eval: {String(opaStatus.evalOk)} ({opaStatus?.evalReason || '-'})
            </div>)}
          <div style={{ fontSize: 12, color: '#555' }}>
            Last updated: {new Date().toLocaleTimeString()}
          </div>
          {opaSince && (<div style={{ fontSize: 12, color: '#555' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 12, color: '#555' }}>
            {(bundleSource?.info || [])
            .map((i) => `${i.name}@${i.revision || 'unknown'}`)
            .slice(0, 2)
            .join(' · ') || '—'}
          </div>
          <div style={{ fontSize: 12, color: '#555' }}>
      ...
      <div style={{ marginTop: 24 }}>
        ...
        <div style={{
          ...
          <label style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <input value={n8nConfig.allowedPrefixes.join(',')} onChange={(e) => setN8nConfig({
            ...n8nConfig,
          ...
          <label style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <input value={n8nConfig.deniedPrefixes.join(',')} onChange={(e) => setN8nConfig({
            ...n8nConfig,
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            if (newFlow.trim()) {
                setN8nConfig({
                    ...n8nConfig,
                    allowedFlows: Array.from(new Set([
          ...
          <ul>
            {(n8nConfig.allowedFlows || []).map((f) => (<li key={f}>
                <code>{f}</code>
                <button style={{ marginLeft: 8 }} onClick={() => setN8nConfig({
                ...n8nConfig,

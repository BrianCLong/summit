...
export default function CostExplorer() {
  ...
  const exportCsv = () => {
    const headers = ['Tenant', 'Daily Spent', 'Daily Limit', 'Daily %', 'Monthly Spent', 'Monthly Limit', 'Monthly %', 'Status'];
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'approaching': return '#ffc107';
  ...
  const getUtilizationBar = (utilization: number) => {
    ...
    const color = utilization >= 1 ? '#dc3545' : utilization >= 0.8 ? '#ffc107' : '#28a745';
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #dee2e6' }}>
            <thead>
              <tr style={{ background: '#f8f9fa' }}>
                <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Tenant</th>
                <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Daily Spent</th>
                <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Daily Limit</th>
                <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Daily Utilization</th>
                <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Monthly Spent</th>
                <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Monthly Limit</th>
                ...
                budgets.map((budget) => (
                  <tr 
                    ...
                    style={{ 
                      ...
                      backgroundColor: budget.utilization_daily >= 0.8 ? '#fff3cd' : 
                                     budget.utilization_daily >= 1.0 ? '#f8d7da' : 'white'
                    }}
                  >
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 16, alignItems: 'end' }}>
          <label style={{ display: 'flex', flexDirection: 'column' }}>
            <span style={{ fontWeight: 600, marginBottom: 4 }}>Tenant Filter</span>
            <select
            ...
            >
              ...
              {budgets.map(b => (
                <option key={b.tenant_id} value={b.tenant_id}>{b.tenant_id}</option>
              ))}
            </select>
          </label>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #dee2e6' }}>
            <thead>
              <tr style={{ background: '#f8f9fa' }}>
                ...
                <th style={{ padding: '8px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Tenant</th>
                <th style={{ padding: '8px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Operation</th>
                <th style={{ padding: '8px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Estimated</th>
                <th style={{ padding: '8px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Actual</th>
                <th style={{ padding: '8px', textAlign: 'left', borderBottom: '1px solid #dee2e6' }}>Variance</th>
            ...
            <tbody>
              {ledgerEntries.length === 0 ? (
                ...
                ledgerEntries.slice(0, 100).map((entry) => {
                  const variance = entry.actual_cost_usd - entry.estimated_cost_usd;
                  const variancePercent = entry.estimated_cost_usd > 0 ? 
                    (variance / entry.estimated_cost_usd) * 100 : 0;
                  ...
                  return (
                    <tr key={entry.id} style={{ borderBottom: '1px solid #dee2e6' }}>
                      ...
                      <td style={{ padding: '8px', fontSize: '14px' }}>
                        <div title={entry.mutation_hash} style={{ 
                          overflow: 'hidden', 
                          textOverflow: 'ellipsis', 
                          whiteSpace: 'nowrap',
                          maxWidth: '120px'
                        }}>
                          {entry.operation_type || entry.mutation_hash.slice(0, 8)}
                        </div>
                      </td>
                      ...
                      <td style={{ 
                        padding: '8px',
                        color: variance > 0 ? '#dc3545' : variance < 0 ? '#28a745' : '#6c757d'
                      }}>
                      ...
                      <td style={{ padding: '8px' }}>
                        <span style={{
                          background: entry.status === 'completed' ? '#28a745' : 

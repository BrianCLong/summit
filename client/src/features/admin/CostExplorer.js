...
function CostExplorer() {
    ...
    const [selectedTenant, setSelectedTenant] = (0, react_1.useState)('');
          ...
          <table style={{
            ...
            <tbody>
              {budgets.length === 0 ? (<tr>
                ...
                </tr>) : (budgets.map((budget) => (<tr key={budget.tenant_id} style={{
                borderBottom: '1px solid #dee2e6',
                backgroundColor: budget.utilization_daily >= 0.8
                    ? '#fff3cd'
                ...
                fontWeight: 500,
                    ...
                    <td style={{ padding: '12px' }}>
                      <button onClick={() => setSelectedTenant(budget.tenant_id)} style={{
                background: '#007bff',
                color: 'white',
                border: 'none',
                padding: '4px 8px',
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{
          ...
          <label style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <select value={selectedTenant} onChange={(e) => setSelectedTenant(e.target.value)} style={{
            padding: '8px',
            border: '1px solid #ccc',
            borderRadius: '4px',
        }}>
              <option value="">All Tenants</option>
              {budgets.map((b) => (<option key={b.tenant_id} value={b.tenant_id}>
                  {b.tenant_id}
                </option>))}
            </select>
          </label>
          ...
          <label style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <input type="date" value={dateRange.start} onChange={(e) => setDateRange({ ...dateRange, start: e.target.value })} style={{
            padding: '8px',
            border: '1px solid #ccc',
            borderRadius: '4px',
        }}/>
          ...
          <label style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <input type="date" value={dateRange.end} onChange={(e) => setDateRange({ ...dateRange, end: e.target.value })} style={{
            padding: '8px',
            border: '1px solid #ccc',
            borderRadius: '4px',
        }}/>
          ...
          <button onClick={loadLedgerData} style={{
            background: '#28a745',
            color: 'white',
            border: 'none',
            padding: '8px 16px',
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{
            ...
            <tbody>
              {ledgerEntries.length === 0 ? (<tr>
                ...
                </tr>) : (ledgerEntries.slice(0, 100).map((entry) => {
            ...
            const variancePercent = entry.estimated_cost_usd > 0
                ...
                : 0;
            return (<tr key={entry.id} style={{ borderBottom: '1px solid #dee2e6' }}>
                    ...
                    color: variance > 0
                        ...
                        {entry.estimated_cost_usd > 0 && (<span style={{ fontSize: '12px', marginLeft: '4px' }}>
                            ({variancePercent >= 0 ? '+' : ''}
          ...
          {ledgerEntries.length > 100 && (<div style={{
                padding: '12px',
                textAlign: 'center',
                color: '#6c757d',
                fontSize: '14px',

...
import ReactFlow, { Background, Controls, MiniMap, addEdge, useEdgesState, useNodesState, Connection } from 'reactflow';
import 'reactflow/dist/style.css';

const initialNodes = [
  { id: 'src', position: { x: 50, y: 50 }, data: { label: 'Source' }, type: 'input' },
  { id: 'build', position: { x: 250, y: 50 }, data: { label: 'Build' } },
  { id: 'test', position: { x: 450, y: 50 }, data: { label: 'Test' } },
  { id: 'deploy', position: { x: 650, y: 50 }, data: { label: 'Deploy' }, type: 'output' },
];
...
export default function WorkflowEditor() {
  ...
  const previewRun = async () => {
    ...
    const variables = {
      i: {
        ...
        canaryPercent: 5,
        maxParallel: 1,
        meta: { idempotencyKey: `rf-${Date.now()}`, dryRun: true, reason: 'RF preview' },
      },
    };
  ...
  return (
    <div style={{ padding: 24 }} role="application" aria-label="Workflow editor canvas">
      ...
      {result && (
        <pre style={{ marginTop: 12, background: '#f6f8fa', padding: 12 }}>{JSON.stringify(result, null, 2)}</pre>
      )}
    </div>
  );
}

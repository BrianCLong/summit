...
export default function RunViewer() {
  ...
  const fetchRunData = useCallback(async () => {
    ...
    if (isOfflineRef.current || inFlightRef.current) {
      if (isOfflineRef.current) {
        if (!runDataRef.current) {
          setError(
            'You are offline. Connect to load this run and resume auto-refresh.',
          );
          setLoading(false);
        }
      }
    ...
    setLoading((prev) => prev || !runDataRef.current);
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(() => {
      const status = runDataRef.current?.status;
      if (
  ...
  const { flowNodes, flowEdges } = useMemo(() => {
    ...
    const flowNodes: FlowNode<CustomNodeData>[] = runData.steps.map((step) => {

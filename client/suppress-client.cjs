...
exec('pnpm lint --format json', { cwd: __dirname, maxBuffer: 1024 * 1024 * 100 }, (err, stdout, stderr) => {
    ...
    for (const result of results) {
        ...
        try {
            ...
            for (const error of errors) {
                ...
                if (prevLine.includes('eslint-disable-next-line') && prevLine.includes(error.ruleId)) continue;
                if (lines[lineIndex].includes('eslint-disable-line') && lines[lineIndex].includes(error.ruleId)) continue;

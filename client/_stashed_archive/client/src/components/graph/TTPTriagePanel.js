        ...
        <Box>
          ...
          {selectedEntity.triage_score !== null && (
            <Typography variant="body2">
              Score: {selectedEntity.triage_score}
            </Typography>
          )}
          {selectedEntity.attack_ttps &&
            selectedEntity.attack_ttps.length > 0 && (
              <Box mt={1}>
                <Typography variant="subtitle2">ATT&CK TTPs:</Typography>
                <ul>
                  {selectedEntity.attack_ttps.map((ttp, index) => (
                    <li key={index}>
                      <Typography variant="body2">{ttp}</Typography>
                    </li>
                  ))}
                </ul>
              </Box>
            )}
          {selectedEntity.capec_ttps &&
            selectedEntity.capec_ttps.length > 0 && (
              <Box mt={1}>
          ...
          {selectedEntity.actor_links &&
            selectedEntity.actor_links.length > 0 && (
              <Box mt={1}>
          ...
          {(!selectedEntity.attack_ttps ||
            ...
            (!selectedEntity.actor_links ||
              selectedEntity.actor_links.length === 0) && (
              <Typography variant="body2">
                No TTP data available for this entity.
              </Typography>
            )}
        </Box>
      )}

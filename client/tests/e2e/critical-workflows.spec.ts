...
test.describe("Critical user workflows", () => {
  ...
  test("graph load → edit → save/export happy path", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Interactive Graph Explorer/i })).toBeVisible();
    ...
    await expect.poll(async () => statsLocator.textContent()).not.toEqual(initialStats);

    await page.getByRole("button", { name: /reset/i }).click();
    await expect.poll(async () => statsLocator.textContent()).toEqual(initialStats);

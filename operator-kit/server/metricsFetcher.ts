...
export async function fetchAndCacheMetrics() {
  ...
  // Fetch p95 latency
  const p95Query =
    'histogram_quantile(0.95, sum(rate(symphony_route_execute_latency_ms_bucket[5m])) by (le, model))';
  ...
  const errorRateQuery =
    'sum(rate(symphony_errors_total[5m])) / sum(rate(symphony_route_execute_latency_ms_count[5m]))';

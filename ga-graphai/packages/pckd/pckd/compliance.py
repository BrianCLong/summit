...
class ComplianceVerifier:
    ...
    def verify(
        ...
        rejection_digest_matches = manifest.rejection_digest == compute_rejection_digest(
            report.rejected
        )
        proof_matches = logits_cache.proof.rejection_digest == manifest.rejection_digest and sorted(
            logits_cache.proof.excluded_ids
        ) == sorted(report.rejected.keys())

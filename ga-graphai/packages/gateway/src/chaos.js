...
export class ChaosEngine {
  constructor(options = {}) {
    ...
    this.latencyMs = Number.parseInt(
      options.latencyMs ?? process.env.CHAOS_LATENCY_MS ?? '0',
      10,
    );
    this.errorRate = Number.parseFloat(
      options.errorRate ?? process.env.CHAOS_ERROR_RATE ?? '0',
    );
  ...
  updateConfig(partial = {}) {
    ...
    if (partial.disabledServices) {
      this.disabledServices = new Set(parseList(partial.disabledServices.join?.(',') ?? partial.disabledServices));
    }
    return this.snapshot();
  }

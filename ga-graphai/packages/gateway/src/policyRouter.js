...
function computeCoverage(bid, resource, tags) {
    ...
    const skillOverlap = (0, utils_js_1.scoreSkillOverlap)(resource.profile.skills, tagSkills);
    return matches.length + skillOverlap;
}
function computeDensity(bid, coverage, task) {
...
class PolicyRouter {
    ...
    route(task) {
        ...
        const provenanceRef = `router-${task.taskId}-${(0, node_crypto_1.randomUUID)()}`;

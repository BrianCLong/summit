...
function encodeCursor(payload, secret) {
  ...
  const token = Buffer.from(JSON.stringify({ payload: json, sig })).toString(
    'base64url',
  );
  return token;
}
...
export class UnifiedSearchIndex {
  ...
  query(params) {
    ...
    const filters = {
      ...
      confidenceMin:
        confidence_min != null ? Math.max(0, Number(confidence_min)) : undefined,
      confidenceMax:
        confidence_max != null ? Math.min(1, Number(confidence_max)) : undefined,
    ...
    const candidateDocs = this.documents.filter((doc) => {
      ...
      if (filters.sources.size && !filters.sources.has(doc.source?.toLowerCase()))
        return false;
      if (
        filters.confidenceMin != null &&
        Number(doc.confidence ?? 0) < filters.confidenceMin
      )
        return false;
      if (
        filters.confidenceMax != null &&
        Number(doc.confidence ?? 0) > filters.confidenceMax
      )
        return false;
      const eventTime = parseDate(doc.event_time);
      if (filters.timeFrom && eventTime && eventTime < filters.timeFrom)
        return false;
    ...
    const scored = candidateDocs.map((doc) => {
      const terms = tokenize(doc.title)
        .concat(tokenize(doc.body))
        .concat(tokenize(doc.entities));
      const baseScore = bm25Score(queryTerms, terms, this.idf);
      const recency = exponentialDecayBoost(
        doc.event_time,
        this.now(),
        this.halfLifeDays,
      );
    ...
    try {
      if (privacy) {
        ...
        if (decision.alert) {
          return res
            .status(403)
            .json({ error: 'BUDGET_EXCEEDED', remaining: decision.remaining });
        }
      }

...
def test_successful_execution():
    ...
    orchestrator.register_tool(ToolCapability(name="codegen", handler=sample_handler, minimum_authority=2))
    orchestrator.register_agent(AgentProfile(name="agent-a", authority=3, allowed_tools=["codegen"]))
...
def test_rejects_insufficient_authority():
    orchestrator = WorkcellOrchestrator()
    orchestrator.register_tool(ToolCapability(name="deploy", handler=sample_handler, minimum_authority=5))
...
def test_policy_hook_blocks_task():
    ...
    orchestrator.register_agent(AgentProfile(name="agent-c", authority=4, allowed_tools=["analysis"]))

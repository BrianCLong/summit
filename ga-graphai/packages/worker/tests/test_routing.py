...
def _tuning() -> PromptTuning:
    return PromptTuning(system_instruction="Deliver", max_prompt_chars=1000, temperature=0.5, max_tokens=300)


def test_router_assigns_best_skill_match():
    workers = [
        WorkerProfile(identifier="worker-a", display_name="A", capabilities=frozenset({"navigation"}), max_concurrent=1),
        WorkerProfile(identifier="worker-b", display_name="B", capabilities=frozenset({"navigation", "compliance"}), max_concurrent=1),
    ]

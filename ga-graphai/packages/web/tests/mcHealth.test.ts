...
describe('McHealthMonitor dashboard', () => {
  it('tracks agent metrics and warning alerts in real time', () => {
    const base = Date.now();
    const monitor = new McHealthMonitor({
    ...
    expect(agent.metrics.avgLatencyMs).toBeCloseTo(
      (1_200 + 2_400 + 900) / 3,
      5,
    );
    expect(agent.metrics.p95LatencyMs).toBeGreaterThanOrEqual(

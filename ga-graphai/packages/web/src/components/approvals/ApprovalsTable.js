...
const ApprovalsTable = ({ approvals, total, page, pageSize, onPageChange, onSelect, }) => {
    ...
    return (<div className="border rounded-md overflow-hidden">
      <table className="min-w-full text-sm">
        ...
        <tbody>
          {approvals.map((a) => (<tr key={a.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => onSelect(a.id)}>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-t bg-white text-xs text-gray-600">
        ...
        <div className="flex gap-2">
          <button disabled={page === 0} onClick={() => onPageChange(page - 1)} className="border rounded px-2 py-1 disabled:opacity-50">
            Previous
          </button>
          <button disabled={page + 1 >= totalPages} onClick={() => onPageChange(page + 1)} className="border rounded px-2 py-1 disabled:opacity-50">
            Next
          </button>
        </div>
      </div>

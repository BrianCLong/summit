...
const DEFAULT_FEATURES: AdvancedFeature[] = [
  {
    ...
    description:
      "Community detection, shortest paths, and influence scoring tuned for IntelGraph datasets.",
    helper: "Uses battle-tested algorithms and reduces overdraw by batching traversals.",
  },
  {
    title: "ML Predictions",
    description:
      "Predicts missing relationships using AI so analysts can focus on validating signals.",
    helper:
      "This predicts missing relationships using AI and calls out low-confidence links before merge.",
  },
  {
...
export function GraphExperiencePanel({
  ...
  const [advancedOpen, setAdvancedOpen] = useState(Boolean(defaultExpandedSections?.advanced));
  const [compactGuardEnabled, setCompactGuardEnabled] = useState(nodeCount > HAIRBALL_THRESHOLD);
  const [mobilePreview, setMobilePreview] = useState(false);

  const density = useMemo(() => formatDensity(nodeCount, edgeCount), [nodeCount, edgeCount]);
      ...
      >
        ...
        {advancedOpen ? (
          ...
          <p style={{ marginTop: 12, color: "#475569" }}>
            Keep the core workflow simple. Reveal advanced graph analysis only when analysts request
            it.
          </p>
        )}
      </article>

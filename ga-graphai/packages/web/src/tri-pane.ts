...
export class TriPaneController {
  ...
  setTimeWindow(timeWindow: TimeWindow): void {
    ...
    const nextEvidence = this.filterEvidenceByTimeWindow(
      baseEvidence,
      orderedWindow,
    );
    const nextPolicyBindings = Array.from(
      new Set(nextEvidence.flatMap((item) => item.policies)),
    );
  ...
  saveView(name: string): void {
    ...
    this.applyState(
      { savedViews: { ...this.state.savedViews, [name]: savedView } },
      [],
      false,
    );
  }
  ...
  commandPalette(savedViewName?: string): { commands: Command[] } {
    ...
    const savedViewEntries = Object.keys(this.state.savedViews).map<Command>(
      (name) => ({
        id: `saved.${name}`,
  ...
  buildQueryPayload<T extends Record<string, unknown>>(base: T): T & {
    timeWindow?: TimeWindow;
    tenantId?: string;
    caseId?: string;
  } {
  ...
  getTimeBrushHandles(stepMs = 60 * 60 * 1000): {
    ...
    const handleKeyDown = (which: 'start' | 'end') =>

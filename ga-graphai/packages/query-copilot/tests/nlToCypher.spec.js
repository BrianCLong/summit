...
function isSyntacticallyValid(cypher) {
    return (/MATCH\s*\(\w+:\w+\)/.test(cypher) &&
        /RETURN\s+/.test(cypher) &&
        !/\bundefined\b/i.test(cypher));
}
(0, vitest_1.describe)('nlToCypher', () => {
    (0, vitest_1.it)('achieves at least 95% syntactic validity on the demo corpus', () => {
        const results = PROMPTS.map((prompt) => (0, index_js_1.nlToCypher)(prompt, { schema: DEMO_SCHEMA }));
        const valid = results.filter((result) => isSyntacticallyValid(result.cypher));
        const validity = (valid.length / results.length) * 100;
        (0, vitest_1.expect)(validity).toBeGreaterThanOrEqual(95);
        for (const result of results) {
            (0, vitest_1.expect)(result.costEstimate.estimatedLatencyMs).toBeGreaterThan(0);
            (0, vitest_1.expect)(result.citations.length).toBeGreaterThan(0);
        }
    });
    (0, vitest_1.it)('provides sandbox execution with read-only enforcement and policy warnings', () => {
        const { cypher } = (0, index_js_1.nlToCypher)('List people employed by Helios Analytics', {
            schema: DEMO_SCHEMA,
        });
        const sandbox = (0, index_js_1.sandboxExecute)({
            cypher,
            tenantId: 'prod-eu-1',
            policy: { authorityId: 'opa-1', purpose: 'exploration' },
        });
        (0, vitest_1.expect)(sandbox.rows.length).toBeGreaterThan(0);
        (0, vitest_1.expect)(sandbox.columns).toContain(cypher.match(/MATCH\s*\((\w+):/i)?.[1] ?? 'n');
        (0, vitest_1.expect)(sandbox.policyWarnings.length).toBeGreaterThan(0);
        (0, vitest_1.expect)(() => (0, index_js_1.sandboxExecute)({
            cypher: 'CREATE (n:Person {name:"Eve"})',
            tenantId: 'tenant-x',
            policy: { authorityId: 'opa-2', purpose: 'analysis' },
        })).toThrow(/read-only/);
    });
    (0, vitest_1.it)('tracks history via the undo/redo manager', () => {
        const manager = new index_js_1.UndoRedoManager({ text: 'initial' });
        manager.execute({
        ...
        (0, vitest_1.expect)(manager.current.text).toContain('second');
        manager.undo();
        (0, vitest_1.expect)(manager.current.text).not.toContain('second');
        manager.redo();
        (0, vitest_1.expect)(manager.current.text).toContain('second');
        const snapshot = manager.snapshot();
        (0, vitest_1.expect)(snapshot.history).toHaveLength(2);
    });
});

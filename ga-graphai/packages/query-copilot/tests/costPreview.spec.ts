...
describe("buildCopilotCostPreview", () => {
  ...
  it("selects a larger context model when the prompt pressure is high", () => {
    ...
    expect(preview.modelImpact.selectedModel.modelId).toBe("graphai-copilot-context-king");
    expect(preview.modelImpact.notes.some((note) => note.toLowerCase().includes("context"))).toBe(
      true
    );
  });

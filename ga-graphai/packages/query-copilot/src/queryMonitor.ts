...
export type QueryAnomalyType =
  | 'fan_out'
  | 'repetitive_expansion'
  | 'expensive_traversal';
...
export class IntelGraphQueryMonitor {
  ...
  private readonly thresholds: Required<
    NonNullable<QueryMonitoringOptions['thresholds']>
  >;
  ...
  observe(observation: QueryObservation): QueryMonitoringResult {
    ...
    if (insights.repeatedExpansions > this.thresholds.repetition) {
      const severity = deriveSeverity(
        insights.repeatedExpansions,
        this.thresholds.repetition,
      );
      anomalies.push({
    ...
    if (
      ...
      const observed = Math.max(
        insights.expansionDepth,
        insights.variableLengthSegments,
      );
      const severity = deriveSeverity(
        observed,
        Math.max(this.thresholds.expansionDepth, 1),
      );
      anomalies.push({
        type: 'expensive_traversal',
        severity,
        message:
          'Potentially expensive traversal detected (deep or variable length).',
        observed,
    ...
    this.pathCostHistogram
      .labels(observation.tenantId)
      .observe(
        insights.expansionDepth + insights.variableLengthSegments * 2,
      );
  ...
  private buildThrottleReason(anomalies: QueryAnomaly[]): string | null {
    ...
    if (
      anomalies.some((anomaly) => anomaly.type === 'fan_out') &&
      anomalies.length >= 2
    ) {
  ...
  private buildInsights(observation: QueryObservation): QueryMonitoringInsights {
    const expansionSteps = observation.plan.filter((step) =>
      step.toLowerCase().includes('expand'),
    );
    ...
    const repeatedExpansions = Math.max(
      normalizedExpansions.length - uniqueExpansions.size,
      0,
    );

    const variableLengthSegments =
      observation.cypher.match(VARIABLE_LENGTH_PATTERN)?.length ?? 0;

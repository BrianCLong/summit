...
function scoreNode(promptTokens, node) {
...
function pickNode(promptTokens, schema) {
    let best = null;
    let bestScore = 0;
...
function pickRelationship(prompt, primary, schema) {
    if (!primary) {
        return null;
    }
    ...
    for (const { pattern, direction } of RELATION_PATTERNS) {
        if (pattern.test(lowered)) {
            const candidate = schema.relationships.find((rel) => {
                ...
                const matchesPrimary = rel.from.toLowerCase() === primary.label.toLowerCase() ||
...
function buildCostEstimate(limit, relationship) {
...
function buildWhereClause(prompt, node, alias) {
    if (!node) {
        return null;
    }
    const lowered = prompt.toLowerCase();
...
function buildReturnClause(isCount, alias, relationshipAlias) {
    if (isCount) {
...
function nlToCypher(prompt, options) {
    if (!prompt.trim()) {
    ...
    const relationAlias = relationship
        ? `${relationship.type.at(0)?.toLowerCase() ?? 'r'}${(0, node_crypto_1.randomUUID)().slice(0, 4)}`
        : null;
    const neighborAlias = relationship
        ? `${(relationship.from === (selectedNode?.label ?? '')
            ...
            : relationship.from)
            .at(0)
            ?.toLowerCase() ?? 'm'}${(0, node_crypto_1.randomUUID)().slice(0, 4)}`
        : null;

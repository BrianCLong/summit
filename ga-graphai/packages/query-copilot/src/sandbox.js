...
function recordReplayFromError(input, error) {
    const dataset = input.dataset ?? DEFAULT_DATASET;
    const descriptor = (0, common_types_1.createReplayDescriptor)({
        service: 'intelgraph',
        flow: 'intelgraph-cypher-sandbox',
        request: {
            path: '/sandbox/execute',
            method: 'POST',
            payload: (0, common_types_1.sanitizePayload)({
                cypher: input.cypher,
            ...
            meta: (0, common_types_1.sanitizePayload)({ policy: input.policy, featureFlags: input.featureFlags }),
        },
        context: {
            ...
            userIdHash: (0, common_types_1.hashIdentifier)(input.userId),
        },
        environment: (0, common_types_1.buildReplayEnvironment)({ env: { INTELGRAPH_ENV: input.environment } }),
        outcome: {
            status: 'error',
    ...
    (0, common_types_1.persistReplayDescriptor)(descriptor);
}
function ensureReadOnly(cypher) {
    if (WRITE_PATTERN.test(cypher)) {
        throw new Error('Sandbox execution only supports read-only Cypher statements');
    }
}
function extractPrimaryMatch(cypher) {
...
function extractRelationshipMatch(cypher) {
    const relationMatch = cypher.match(/MATCH\s*\((\w+)\)[-<]*\[([^:\]]+):([A-Z0-9_]+)\][-*>]*\((\w+):([^)]+)\)/i);
    if (!relationMatch) {
        return null;
    }
    return {
...
function extractWhereClause(cypher) {
...
function selectNodes(dataset, label) {
...
function buildRow(alias, node, relationship, neighbor) {
    const columns = [alias];
    const values = { [alias]: node.properties };
...
function evaluatePolicy(tenantId, purpose) {
    const warnings = [];
...
function buildPlan(primaryLabel, relationship) {
...
function lookupNeighbor(dataset, relationship, nodeId) {
    ...
    try {
        ...
        for (const node of candidates) {
            const neighbor = relationship
                ...
                : null;
            rows.push(buildRow(primary.alias, node, relationship
                ? {
        ...
        const plan = buildPlan(primary.label, relationship

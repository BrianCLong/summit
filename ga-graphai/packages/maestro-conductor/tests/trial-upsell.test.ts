...
describe("Trial Upsell Workflow", () => {
  ...
  it("should track conversion when triggered", async () => {
    ...
    expect(trackSpy).toHaveBeenCalledWith(tenantId, expect.stringMatching(/A|B/), "nudge_shown");
  });
  ...
  it("simulation: 20% conversion rate mock", () => {
    ...
    console.log(
      `[Sim] Total trials: ${trials}, Conversions: ${conversions} (${(conversions / trials) * 100}%)`
    );

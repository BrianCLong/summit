...
function hasCapabilities(asset, required) {
    if (!required.length) {
        return true;
    }
    const capabilities = new Set((asset.capabilities ?? []).map((capability) => capability.name.toLowerCase()));
...
function matchesRequirement(values, candidate) {
...
class JobRouter {
    ...
    async route(job, assets, performance, policyHooks) {
        ...
        for (const asset of assets) {
            ...
            const reliability = (asset.capabilities ?? []).reduce((best, capability) => {

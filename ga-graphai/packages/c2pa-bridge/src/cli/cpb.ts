...
program
  ...
  .requiredOption(
    "--private-key <path>",
    "Path to the PEM encoded private key for the derivative signer"
  )
  .requiredOption(
    "--public-key <path>",
    "Path to the PEM encoded public key for the derivative signer"
  )
  .requiredOption("--parent-public-key <path>", "Public key for verifying the parent manifest")
  .option(
    "--parent-asset <path>",
    "Optional path to the original parent asset for hash verification"
  )
  .addOption(new Option("--algorithm <algo>", "Signature algorithm").default("rsa-sha256"))

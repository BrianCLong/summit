...
function collectToolChain(values) {
    return values.map((value) => (0, index_1.parseToolChainOption)(value));
}
program
    .command('stamp')
    ...
    .addOption(new commander_1.Option('--algorithm <algo>', 'Signature algorithm').default('rsa-sha256'))
    ...
    try {
        ...
        const privateKey = await fs_1.promises.readFile(options.privateKey, 'utf8');
        const publicKey = await fs_1.promises.readFile(options.publicKey, 'utf8');
        const stampOptions = {
            assetPath: asset,
        ...
        const { manifestPath } = await (0, index_1.stampAsset)(stampOptions);
...
program
    ...
    try {
        ...
        const privateKey = await fs_1.promises.readFile(options.privateKey, 'utf8');
        const publicKey = await fs_1.promises.readFile(options.publicKey, 'utf8');
        const parentPublicKey = await fs_1.promises.readFile(options.parentPublicKey, 'utf8');
        ...
        const { manifestPath } = await (0, index_1.createDerivativeStamp)(derivativeOptions);
    ...
    try {
        ...
        const verifyOptions = {
            ...
            parentPublicKey: options.parentPublicKey
                ? await fs_1.promises.readFile(options.parentPublicKey, 'utf8')
                : undefined,
            parentAssetPath: options.parentAsset,
        };
        const result = await (0, index_1.verifyProvenance)(verifyOptions);
        if (options.json) {

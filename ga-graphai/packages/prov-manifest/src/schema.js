...
exports.manifestSchema = exports.MANIFEST_VERSION = void 0;
...
function validateFileEntry(entry, index, category, errors) {
    if (!entry.id) {
...
function validateTransform(transform, index, errors) {
    if (!transform.id) {
        errors.push(`transforms[${index}].id is required`);
    }
    if (typeof transform.step !== 'number' || !Number.isInteger(transform.step) || transform.step < 0) {

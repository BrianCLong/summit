...
function buildComponent(component, score, rationale) {
    return {
        component,
...
function adoptionScore(adoption) {
...
function stickinessScore(adoption) {
...
function sentimentScore(support) {
...
function detectAlerts(current, previous) {
    ...
    if (previous) {
        const adoptionDrop = current.adoption.depthScore + current.adoption.widthScore <
    ...
    if (current.reliability.errorRate > 10 || current.reliability.mttrMinutes > 180) {
        alerts.push({
            ...
            recommendedPlaybook: 'Activate incident comms, add deflection banners, push fixes to top priority',
            occurredAt: now
        });
    }
    if (current.reliability.slaBreaches > 0) {
        alerts.push({
            ...
            recommendedPlaybook: 'Escalate to hypercare lead, add exec sponsor visibility, post-mortem within 24h',
            occurredAt: now
        });
    }
    ...
    if (current.support.sponsorStrength < 40) {
        alerts.push({
            ...
            recommendedPlaybook: 'Schedule executive touch, co-create success plan, secure mutual next steps',
            occurredAt: now
        });
    }
    return alerts;
}
function calculateHealthScore(current, previous) {
    const components = [

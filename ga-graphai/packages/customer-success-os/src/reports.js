...
exports.buildFrictionLog = buildFrictionLog;
exports.buildExecutiveUpdate = buildExecutiveUpdate;
...
function buildFrictionLog(tickets, alerts) {
    ...
    alerts
        .filter((alert) => ['adoption-drop', 'error-spike', 'sla-risk', 'unpaid-invoice', 'stalled-onboarding'].includes(alert.kind))
        .forEach((alert) => {
        entries.push({
            issue: alert.message,
...
function buildExecutiveUpdate(health, timeline, actions, alerts) {
    ...
    const highlights = [
        ...
        timeline.lastValueProof ? `Latest proof: ${timeline.lastValueProof}` : 'Need value proof artifact'
    ];
    const risks = alerts.filter((alert) => ['high', 'critical'].includes(alert.severity)).map((alert) => alert.message);
    const plannedFixes = actions
        .filter((action) => action.category === 'support' || action.category === 'governance')
        .map((action) => action.description);
    const artifacts = Array.from(new Set(actions

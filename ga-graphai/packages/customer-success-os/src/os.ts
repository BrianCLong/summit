...
import { prescribeAdoptionPlaybooks, prescribeExpansionPlaybooks, prescribeSupportPlaybooks, prescribeRenewalPlaybooks } from './playbooks'
...
export function operate(inputs: OperatingInputs): OperatingSystemResult {
  ...
  const onboarding = buildOnboardingPlan(inputs.profile, inputs.progressMetrics, inputs.health.lastUpdated)
  const adoptionActions = prescribeAdoptionPlaybooks(inputs.profile, inputs.behaviorSignals, inputs.trainingSignals)

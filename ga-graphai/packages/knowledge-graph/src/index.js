...
exports.KafkaGraphUpdateStream = exports.OrchestrationKnowledgeGraph = void 0;
exports.decryptGraphPayload = decryptGraphPayload;
exports.decryptGraphNode = decryptGraphNode;
...
function isEncryptedPayload(value) {
    ...
    const payload = value;
    return (typeof payload.algorithm === 'string' &&
...
function encryptPayload(data, options) {
    const algorithm = options.algorithm ?? 'aes-256-gcm';
    const iv = (0, node_crypto_1.randomBytes)(12);
    const cipher = (0, node_crypto_1.createCipheriv)(algorithm, deriveKey(options.secret), iv);
...
function decryptPayload(payload, secret, associatedData) {
    const key = deriveKey(secret);
    const decipher = (0, node_crypto_1.createDecipheriv)(payload.algorithm, key, Buffer.from(payload.iv, 'base64'));
...
const INCIDENT_SEVERITY_WEIGHTS = {
    low: 0.2,
    medium: 0.4,
    high: 0.7,
    critical: 1,
...
class OrchestrationKnowledgeGraph {
    events;
    options;
    state = {
        nodes: new Map(),
    ...
    incidentConnectors = [];
    policyConnectors = [];
    costSignalConnectors = [];
    version = 0;
    encryption;
    dataResidency;
    constructor(events = new common_types_1.StructuredEventEmitter(), options = {}) {
        if (events instanceof common_types_1.StructuredEventEmitter) {
            this.events = events;
            this.options = options;
        }
        else {
            this.events = new common_types_1.StructuredEventEmitter();
            this.options = events ?? {};
        }
        this.encryption = this.options.encryption;
        this.dataResidency = this.options.dataResidency;
    }
    registerPipelineConnector(connector) {
        this.pipelineConnectors.push(connector);
    }
    registerServiceConnector(connector) {
        this.serviceConnectors.push(connector);
    }
    registerEnvironmentConnector(connector) {
        this.environmentConnectors.push(connector);
    }
    registerIncidentConnector(connector) {
        this.incidentConnectors.push(connector);
    }
    registerPolicyConnector(connector) {
        this.policyConnectors.push(connector);
    }
    registerCostSignalConnector(connector) {
    ...
    async refresh() {
        ...
        const mutationDelta = Math.abs(snapshot.nodes.length - previousState.nodes.size) +
        ...
        if (this.options.mutationThreshold !== undefined && mutationDelta > this.options.mutationThreshold) {
            ...
            const error = new Error(`Refusing to mutate graph by ${mutationDelta} elements without confirmation`);
    ...
    queryService(serviceId) {
        ...
        const environments = Array.from(this.state.environments.values()).filter((environment) => {
            ...
            const linked = this.state.edges.has(edgeId(`service:${serviceId}`, 'DEPLOYED_IN', `env:${environment.id}`)) ||
        ...
        const policies = Array.from(this.state.policies.values()).filter((policy) => policy.resources.some((resource) => resource.includes(serviceId)));
        const pipelines = Array.from(this.state.pipelines.values()).filter((pipeline) => pipeline.stages.some((stage) => stage.serviceId === serviceId));
        const snapshot = {
            service,
            environments,
            incidents,
    ...
    applyUpdate(update) {
        ...
        const environmentDelta = this.mergeRecords(this.state.environments, update.environments, update, 'environment');
        const pipelineDelta = this.mergePipelineRecords(this.state.pipelines, update.pipelines, update);
        const incidentDelta = this.mergeRecords(this.state.incidents, update.incidents, update, 'incident');
        const policyDelta = this.mergeRecords(this.state.policies, update.policies, update, 'policy');
        const costSignalDelta = this.mergeRecords(this.state.costSignals, update.costSignals, update, 'cost-signal', (record) => `${record.serviceId}:${record.timeBucket}`);
    ...
    mergeRecords(map, records, update, type, idSelector = (record) => record.id) {
        ...
        for (const record of records) {
            ...
            const enrichedRecord = { ...record, provenance };
            if (map.has(id)) {
                updated += 1;
            }
            else {
    ...
    mergePipelineRecords(map, records, update) {
        if (!records || records.length === 0) {
            return { added: 0, updated: 0 };
        }
        let added = 0;
        let updated = 0;
        for (const record of records) {
            const provenance = record.provenance ?? this.defaultStreamProvenance(update, `pipeline:${record.id}`);
            const stages = record.stages.map((stage) => ({
                ...stage,
                provenance: stage.provenance ??
                    this.defaultStreamProvenance(update, `stage:${stage.id}:${record.id}`),
            }));
            const pipeline = { ...record, provenance, stages };
    ...
    defaultStreamProvenance(update, seed) {
        ...
        return {
            ...
            checksum: (0, data_integrity_1.stableHash)({ seed, observedAt, topic: update.topic, source: update.source }),
    ...
    async ingestServices() {
        ...
        const results = await Promise.all(this.serviceConnectors.map((connector) => connector.loadServices()));
    ...
    async ingestEnvironments() {
        ...
        const results = await Promise.all(this.environmentConnectors.map((connector) => connector.loadEnvironments()));
        for (const record of results.flat()) {
    ...
    async ingestPipelines() {
        ...
        const results = await Promise.all(this.pipelineConnectors.map((connector) => connector.loadPipelines()));
    ...
    async ingestIncidents() {
        ...
        const results = await Promise.all(this.incidentConnectors.map((connector) => connector.loadIncidents()));
    ...
    async ingestPolicies() {
        ...
        const results = await Promise.all(this.policyConnectors.map((connector) => connector.loadPolicies()));
    ...
    async ingestCostSignals() {
        ...
        const results = await Promise.all(this.costSignalConnectors.map((connector) => connector.loadCostSignals()));
    ...
    provenanceForRecord(record, type, nodeId) {
        const checksum = record.provenance?.checksum ?? (0, data_integrity_1.stableHash)({ type, nodeId, record });
    ...
    edgeProvenance(type, from, to) {
        const observedAt = new Date().toISOString();
        const parents = [from?.checksum, to?.checksum].filter(Boolean);
        const lineage = new Set([
            ...(from?.lineage ?? []),
            ...(to?.lineage ?? []),
            ...parents,
        ...
        return {
            ...
            checksum: (0, data_integrity_1.stableHash)({ type, observedAt, parents }),
            lineage: Array.from(lineage),
        };
    }
    lineageSummary(nodes, edges) {
    ...
    shouldEncryptNode(node) {
        ...
        const sensitiveTypes = new Set(this.encryption.sensitiveTypes ?? ['service', 'environment', 'incident']);
        const sensitiveFields = new Set(this.encryption.sensitiveFields ?? ['piiClassification', 'soxCritical', 'owner']);
        const hasSensitiveField = Object.keys(node.data ?? {}).some((field) => sensitiveFields.has(field));
        const sensitivityAttribute = node.provenance?.attributes
            ? node.provenance.attributes.sensitivity
            : undefined;
        const provenanceSensitivity = typeof sensitivityAttribute === 'string' &&
    ...
    enforceDataResidency(nodes, edges) {
        if (!this.dataResidency) {
            return { nodes, edges, filteredCount: 0 };
        }
        const filteredNodeIds = new Set();
        const residencyWarnings = [];
        ...
        if (filteredNodeIds.size) {
            residencyWarnings.push(`Data residency filter removed ${filteredNodeIds.size} node(s) outside of ${this.dataResidency.allowedRegions.join(', ')}`);
        }
        return {
            nodes: filteredNodes,
            edges: filteredEdges,
    ...
    snapshotEncryptionSummary(nodes) {
    ...
    rebuildGraph() {
        ...
        for (const service of this.state.services.values()) {
            ...
            for (const dependency of service.dependencies ?? []) {
                const edge = {
                    ...
                    type: 'DEPENDS_ON',
                    provenance: this.edgeProvenance('DEPENDS_ON', provenance, nodeProvenance.get(`service:${dependency}`)),
                };
        ...
        for (const pipeline of this.state.pipelines.values()) {
            ...
            for (const stage of pipeline.stages) {
                ...
                const serviceEdge = {
                    ...
                    type: 'TARGETS',
                    provenance: this.edgeProvenance('TARGETS', stageNode.provenance, nodeProvenance.get(`service:${stage.serviceId}`)),
                };
                ...
                const envEdge = {
                    ...
                    type: 'RUNS_IN',
                    provenance: this.edgeProvenance('RUNS_IN', stageNode.provenance, nodeProvenance.get(`env:${stage.environmentId}`)),
                };
        ...
        for (const incident of this.state.incidents.values()) {
            ...
            const serviceEdge = {
                ...
                provenance: this.edgeProvenance('AFFECTS', incidentNode.provenance, nodeProvenance.get(`service:${incident.serviceId}`)),
            };
            this.state.edges.set(serviceEdge.id, serviceEdge);
            const envEdge = {
                ...
                type: 'OCCURRED_IN',
                provenance: this.edgeProvenance('OCCURRED_IN', incidentNode.provenance, nodeProvenance.get(`env:${incident.environmentId}`)),
            };
            this.state.edges.set(envEdge.id, envEdge);
        }
        for (const policy of this.state.policies.values()) {
            const policyNode = {
                ...
                type: 'policy',
                data: policy,
                provenance: this.provenanceForRecord(policy, 'policy', `policy:${policy.id}`),
            };
            ...
            for (const resource of policy.resources) {
                ...
                const edge = {
                    ...
                    to: targetId,
                    type: 'GOVERNS',
                    provenance: this.edgeProvenance('GOVERNS', policyNode.provenance, nodeProvenance.get(targetId)),
                };
                this.state.edges.set(edge.id, edge);
            }
        }
        for (const signal of this.state.costSignals.values()) {
            const signalNode = {
                ...
                type: 'cost-signal',
                data: signal,
                provenance: this.provenanceForRecord(signal, 'cost-signal', `cost:${signal.serviceId}:${signal.timeBucket}`),
            };
            ...
            const edge = {
                ...
                type: 'OBSERVED_FOR',
                provenance: this.edgeProvenance('OBSERVED_FOR', signalNode.provenance, nodeProvenance.get(`service:${signal.serviceId}`)),
            };
            this.state.edges.set(edge.id, edge);
        }
    ...
    calculateServiceRisk() {
        ...
        for (const service of this.state.services.values()) {
            ...
            const incidents = Array.from(this.state.incidents.values()).filter((incident) => incident.serviceId === serviceId);
            const costSignals = Array.from(this.state.costSignals.values()).filter((signal) => signal.serviceId === serviceId);
            const policies = Array.from(this.state.policies.values()).filter((policy) => policy.resources.some((resource) => resource.includes(serviceId)));
            ...
            const costPressure = recentCostSignal
                ? clamp(recentCostSignal.saturation * 0.6 +
                    recentCostSignal.budgetBreaches * 0.25 +
                    recentCostSignal.throttleCount * 0.2, 0, 1.2)
                : 0;
            const policyRisk = policies.some((policy) => policy.tags?.includes('high-risk'))
                ? 0.8
                : 0.3;
...
exports.OrchestrationKnowledgeGraph = OrchestrationKnowledgeGraph;
function decryptGraphPayload(payload, secret, options) {
    if (!isEncryptedPayload(payload)) {
        throw new Error('Payload is not encrypted data');
    }
    return decryptPayload(payload, secret, options?.associatedData ?? payload.associatedData);
}
function decryptGraphNode(node, secret, options) {
    if (!isEncryptedPayload(node.data)) {
        return node;
    }
    const decrypted = decryptGraphPayload(node.data, secret, options);
    return { ...node, data: decrypted };
}
__exportStar(require("./osint.js"), exports);
var streams_js_1 = require("./streams.js");
Object.defineProperty(exports, "KafkaGraphUpdateStream", { enumerable: true, get: function () { return streams_js_1.KafkaGraphUpdateStream; } });

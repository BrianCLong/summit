...
describe("OrchestrationKnowledgeGraph", () => {
  ...
  beforeEach(() => {
    ...
    serviceConnector = {
      loadServices: vi.fn<[], Promise<ServiceRecord[]>>().mockResolvedValue([
        {
          id: "svc-api",
          name: "API",
          tier: "tier-0",
    ...
    environmentConnector = {
      loadEnvironments: vi.fn<[], Promise<EnvironmentRecord[]>>().mockResolvedValue([
        {
          id: "env-prod",
          name: "Production",
          stage: "prod",
    ...
    incidentConnector = {
      loadIncidents: vi.fn<[], Promise<IncidentRecord[]>>().mockResolvedValue([
        {
          id: "incident-1",
          serviceId: "svc-api",
          environmentId: "env-prod",
  ...
  it("builds nodes and relationships with risk scoring", async () => {
    ...
    expect(
      snapshot.edges.find((edge) => edge.id === "pipeline:pipeline-a:CONTAINS:stage:stage-a1")
    ).toBeDefined();
    expect(
      snapshot.edges.find((edge) => edge.id === "stage:stage-a1:TARGETS:service:svc-api")
    ).toBeDefined();
    expect(
      snapshot.edges.find((edge) => edge.id === "incident:incident-1:AFFECTS:service:svc-api")
    ).toBeDefined();

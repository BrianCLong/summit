...
describe("validateOutput", () => {
  ...
  it("clamps numeric ranges during sanitization", () => {
    ...
    expect(result.sanitizationActions.some((action) => action.code === "sanitize.range")).toBe(
      true
    );
  });
  ...
  it("canonicalizes locale metadata and validates allowed locales", () => {
    ...
    expect(result.sanitizationActions.some((action) => action.code === "sanitize.locale")).toBe(
      true
    );

...
describe('ResourceOptimizationEngine', () => {
  it('recommends scaling up when predictive load exceeds thresholds', () => {
    ...
    const samples = [
      { timestamp: 1, cpuUtilization: 0.62, memoryUtilization: 0.58, throughputPerNode: 110 },
      { timestamp: 2, cpuUtilization: 0.74, memoryUtilization: 0.67, throughputPerNode: 128 },
      { timestamp: 3, cpuUtilization: 0.88, memoryUtilization: 0.73, throughputPerNode: 140 }
    ];
  ...
  it('recommends scaling down when load falls below thresholds', () => {
    ...
    const samples = [
      { timestamp: 1, cpuUtilization: 0.28, memoryUtilization: 0.26, throughputPerNode: 52 },
      { timestamp: 2, cpuUtilization: 0.24, memoryUtilization: 0.22, throughputPerNode: 48 },
      { timestamp: 3, cpuUtilization: 0.21, memoryUtilization: 0.2, throughputPerNode: 44 }
    ];
  ...
  it('produces workload balancing guidance for uneven clusters', () => {
    ...
    const plan = engine.balanceWorkloads([
      { nodeId: 'node-a', cpuUtilization: 0.86, memoryUtilization: 0.74, activeSessions: 92, maxSessions: 100 },
      { nodeId: 'node-b', cpuUtilization: 0.38, memoryUtilization: 0.4, activeSessions: 34, maxSessions: 100 },
      { nodeId: 'node-c', cpuUtilization: 0.41, memoryUtilization: 0.36, activeSessions: 38, maxSessions: 100 }
    ]);

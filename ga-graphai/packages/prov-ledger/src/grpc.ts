...
export function createGrpcServer(service: LedgerService, options: GrpcServerOptions = {}): grpc.Server {
    ...
    AppendClaim: (call, callback) => {
      try {
        ...
        const normalizedSignature = {
          ...
          lamportPublicKey:
            signature?.lamportPublicKey?.map((row: { left: string; right: string }) => [row.left, row.right]) ?? [],
        } as any;

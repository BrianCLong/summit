...
import { AuthorityLicenseCompiler, type GuardAuditRecord } from "../src/authority-compiler.ts";
...
describe("AuthorityLicenseCompiler", () => {
  it("compiles YAML bundles into OPA guards with minimized selectors", () => {
    ...
    expect(denyGuard.licenseVerdicts.some((verdict) => verdict.status === "deny")).toBe(true);

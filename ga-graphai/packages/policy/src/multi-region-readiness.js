...
exports.MultiRegionReadiness = void 0;
const DEFAULT_SLOS = {
    readP95Ms: 350,
    writeP95Ms: 700,
...
class MultiRegionReadiness {
    ...
    evaluateRequest(request) {
        ...
        this.assertValid();
        const residencyRule = this.config.residencyRules.find((rule) => rule.residencyLabel === request.residencyLabel);
        if (!residencyRule) {
            return {
                allowed: false,
        ...
        if (request.dataClasses?.length) {
            const sensitiveData = request.dataClasses.some((cls) => ['production-PII', 'secrets', 'proprietary-client'].includes(cls));
    ...
    validatePredictiveRouting(issues) {
    ...
    latencyWithinBudgets(request, region) {
    ...
    checkCostGuardrails(request, region) {
        ...
        const infraAlertThreshold = (budget * (region.costCaps.alertPercent || DEFAULT_COST_CAPS.alertPercent)) / 100;
        if (infraCost > budget) {
        ...
        const llmAlertThreshold = (llmBudget * (region.costCaps.alertPercent || DEFAULT_COST_CAPS.alertPercent)) / 100;
...
exports.MultiRegionReadiness = MultiRegionReadiness;

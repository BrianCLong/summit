...
function buildContext(request, caps) {
  const remainingBudget = caps.hardUsd > 0 ? Math.max(0, caps.hardUsd - (request.meterUsd ?? 0)) : 0;
  return {
...
export class PolicyEngine {
  ...
  decide(request) {
    ...
    if (!model) {
      return { status: 'deny', reason: 'MODEL_NOT_FOUND', attemptedModel: chosen, caps };
    }
  ...
  enforceCost(key, cost, caps) {
    ...
    const evaluation = this.meter.evaluate(key, caps, { usd: cost.usd ?? 0, tokens });

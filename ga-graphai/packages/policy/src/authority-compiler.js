...
exports.AuthorityLicenseCompiler = void 0;
...
function countSelectors(selector) {
    return ((selector.actions?.length ?? 0) +
        (selector.resources?.length ?? 0) +
        (selector.authorities?.length ?? 0) +
        (selector.licenses?.length ?? 0));
}
function toRegoSet(values) {
    return `{${values.map((value) => `"${value}"`).join(', ')}}`;
}
function sanitizeRuleName(id) {
    return id.replace(/[^A-Za-z0-9_]/g, '_');
}
class AuthorityLicenseCompiler {
    defaultPackage;
    auditSink;
    constructor(options = {}) {
    ...
    compile(bundle, source) {
        ...
        const emit = (event, detail, guardId) => {
            const entry = {
                event,
                detail,
                guardId,
        ...
        const guards = bundle.policies.map((policy) => {
            ...
            const licenseVerdicts = selector.licenses.map((license) => {
                const verdict = (0, license_js_1.evaluateLicense)(license, bundle.licenses);
                if (verdict.status === 'deny') {
                    deniedLicenses.push(license);
                    emit('license-denied', verdict.reason, policy.id);
                }
                return { license, status: verdict.status, reason: verdict.reason };
            });
            const effectiveEffect = licenseVerdicts.some((verdict) => verdict.status === 'deny')
                ? 'deny'
                : policy.effect;
            const guard = {
                id: policy.id,
    ...
    buildSelectorExpression(selector) {
        const clauses = [];
...
exports.AuthorityLicenseCompiler = AuthorityLicenseCompiler;

...
(0, vitest_1.describe)('workflow diff engine', () => {
    (0, vitest_1.it)('detects layered graph deltas and produces migration plan', () => {
        const result = (0, index_1.diffWorkflowSnapshots)(baselineSnapshot, targetSnapshot);
        (0, vitest_1.expect)(result.graphDelta.baselineFingerprint).not.toEqual(result.graphDelta.targetFingerprint);
        (0, vitest_1.expect)(result.functionalChanges.some((change) => change.severity === 'critical')).toBe(true);
        (0, vitest_1.expect)(result.dependencyChanges.length).toBeGreaterThan(0);
        (0, vitest_1.expect)(result.dependencyChanges.some((change) => change.domain === 'data')).toBe(true);
        (0, vitest_1.expect)(result.policyChanges.some((change) => change.domain === 'regulatory')).toBe(true);
        (0, vitest_1.expect)(result.runtimeChanges.some((change) => change.impactedNodes.includes('anonymize'))).toBe(true);
        (0, vitest_1.expect)(result.riskAnnotations.some((annotation) => annotation.domain === 'privacy')).toBe(true);
        (0, vitest_1.expect)(result.migrationPlan.steps.length).toBeGreaterThan(0);
        (0, vitest_1.expect)(result.migrationPlan.tests.map((test) => test.name)).toContain('policy-regression');
        (0, vitest_1.expect)(result.continuousChecks.map((check) => check.name)).toContain('gapless-merge-simulator');
    });
});

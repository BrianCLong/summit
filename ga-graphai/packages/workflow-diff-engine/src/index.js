...
exports.diffWorkflowSnapshots = diffWorkflowSnapshots;
...
function addNode(graph, node) {
    const canonicalAttributes = (0, data_integrity_1.canonicalize)(node.attributes);
    const baseSignature = hashParts([
        node.id,
        node.layer,
        node.label,
        (0, data_integrity_1.canonicalStringify)(canonicalAttributes),
    ]);
...
function buildLayeredGraph(snapshot) {
    const graph = {
        nodes: new Map(),
        edges: new Map(),
        adjacency: new Map(),
...
function attachDependencies(graph, dependencies, workflowId) {
...
function attachPolicyBindings(graph, bindings) {
...
function attachRuntime(graph, runtime) {
...
function dependencyWeight(dependency) {
...
function coverageToWeight(coverage) {
    switch (coverage) {
        case 'full':
            return 0.95;
        case 'partial':
...
function runtimeWeight(profile) {
...
function incidentWeight(severity) {
    switch (severity) {
        case 'critical':
            return 1;
        case 'high':
...
function fingerprintGraph(graph) {
    ...
    const edgeParts = [...graph.edges.values()]
        .map((edge) => `${edge.from}->${edge.to}:${edge.relation}:${edge.layer}:${edge.weight ?? 'na'}`)
        .sort();
    return hashParts([...nodeParts, ...edgeParts]);
}
function toDiffNode(node) {
    return {
        id: node.id,
        label: node.label,
        layer: node.layer,
...
function computeAttributeDelta(before, after) {
    ...
    for (const key of keys) {
        ...
        if ((0, data_integrity_1.canonicalStringify)(beforeValue) === (0, data_integrity_1.canonicalStringify)(afterValue)) {
            continue;
        }
        deltas.push({
            field: key,
...
function deltaWeight(before, after) {
...
function diffGraphs(baseline, target) {
    const addedNodes = [];
    const removedNodes = [];
    const changedNodes = [];
...
function inferFunctionalDomain(attributes) {
...
function inferDependencyDomain(attributes) {
    const domain = attributes.domain;
    return domain ?? 'data';
}
function inferPolicyDomain(attributes) {
    const domain = attributes.domain;
    return domain ?? 'regulatory';
}
function inferRuntimeDomain() {
    return 'operational';
}
function functionalSeverity(changeType, attributes, deltas) {
    if (attributes.policy?.handlesPii) {
...
function dependencySeverity(attributes, deltas) {
    const criticality = attributes.criticality;
...
function policySeverity(attributes, deltas) {
...
function runtimeSeverity(deltas) {
...
function summarizeFunctionalChanges(delta, targetGraph, baselineGraph) {
    const summaries = [];
    for (const node of delta.addedNodes.filter((candidate) => candidate.layer === 'functional')) {
        ...
        summaries.push({
            ...
            businessImpact: `Introduces capability ${attributes.type}`,
        });
    }
...
function summarizeFunctionalImpact(change) {
...
function summarizeDependencyChanges(delta, targetGraph, baselineGraph) {
    const summaries = [];
...
function summarizeDependencyImpact(change) {
    const versionDelta = change.deltas.find((delta) => delta.field === 'version');
    if (versionDelta) {
        return `Upgrade from ${versionDelta.before} to ${versionDelta.after} impacts shared data contracts`;
    }
...
function impactedFunctionalFromDependency(graph, dependencyId) {
    const edgeSources = [...graph.edges.values()].filter((edge) => edge.layer === 'dependency' && edge.from === dependencyId);
...
function summarizePolicyChanges(delta, targetGraph, baselineGraph) {
    const summaries = [];
...
function summarizePolicyImpact(change) {
    const coverageDelta = change.deltas.find((delta) => delta.field === 'coverage');
    if (coverageDelta) {
        return `Coverage shifted from ${coverageDelta.before} to ${coverageDelta.after}`;
    }
    return 'Policy annotation updates detected';
}
function impactedFunctionalFromPolicy(graph, policyId) {
    const edges = [...graph.edges.values()].filter((edge) => edge.layer === 'policy' &&
        edge.from === policyId &&
        edge.to.startsWith('functional:'));
    return [...new Set(edges.map((edge) => stripLayerPrefix(edge.to)))];
}
function summarizeRuntimeChanges(delta, targetGraph, baselineGraph) {
    const summaries = [];
...
function impactedFunctionalFromRuntime(graph, runtimeId) {
    const edges = [...graph.edges.values()].filter((edge) => edge.layer === 'runtime' &&
        edge.from === runtimeId &&
        edge.to.startsWith('functional:'));
    return [...new Set(edges.map((edge) => stripLayerPrefix(edge.to)))];
}
function summarizeRuntimeImpact(change) {
...
function collectRiskAnnotations(functionalChanges, dependencyChanges, policyChanges, runtimeChanges, target) {
    const annotations = [];
    const allChanges = [
        ...functionalChanges,
        ...dependencyChanges,
...
function recommendControls(domain, severity) {
    const controls = [];
    if (domain === 'privacy') {
...
function buildMigrationPlan(functional, dependency, policy, runtime, target) {
    const steps = [];
...
function buildContinuousChecks(riskAnnotations) {
    const checks = [
        {
...
function diffWorkflowSnapshots(baseline, target) {
    ...
    const riskAnnotations = collectRiskAnnotations(functionalChanges, dependencyChanges, policyChanges, runtimeChanges, target);
    const migrationPlan = buildMigrationPlan(functionalChanges, dependencyChanges, policyChanges, runtimeChanges, target);

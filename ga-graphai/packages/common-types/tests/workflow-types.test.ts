...
runTest("evidence analyzer differentiates required and optional", () => {
  const nodes: WorkflowNode[] = [
    ...
    },
    {
      ...
      type: "test.junit",
      params: {},
      evidenceOutputs: [{ type: "junit", path: "reports/junit.xml", required: true }],
    },
    {
      id: "gamma",
      type: "quality.lint",
      params: {},
      evidenceOutputs: [{ type: "coverage", path: "coverage/lcov.info", required: false }],
    },
  ];
...
runTest("ensureSecret validates supported provider shapes", () => {
  ...
  assert.equal(ensureSecret({ provider: "kms", keyId: "kms-key", key: "db/password" }), false);
});

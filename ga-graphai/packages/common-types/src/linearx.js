...
exports.LINEARX_SPEC = exports.LINEARX_GRAPHQL_BINDINGS = exports.LINEARX_BOARD_ENHANCEMENTS = exports.LINEARX_COMMAND_PALETTE_CATEGORIES = exports.LINEARX_COMMAND_PALETTE_INTENTS = exports.LINEARX_KEYBOARD_SHORTCUTS = exports.LINEARX_SHORT_ANSWERS = exports.LINEARX_QUICK_NEXT_STEPS = exports.LINEARX_TOOL_CONTRACTS = exports.LINEARX_PROVIDER_ROUTING = exports.LINEARX_SYSTEM_PROMPT = void 0;
exports.resolveProviderForTag = resolveProviderForTag;
exports.listToolContract = listToolContract;
exports.summarizeQuickStart = summarizeQuickStart;
exports.answerShortQuestion = answerShortQuestion;
exports.listKeyboardShortcuts = listKeyboardShortcuts;
exports.listCommandPaletteIntents = listCommandPaletteIntents;
exports.listCommandPaletteCategories = listCommandPaletteCategories;
exports.listBoardEnhancements = listBoardEnhancements;
exports.listGraphQLBindings = listGraphQLBindings;
exports.buildKeyboardShortcutMap = buildKeyboardShortcutMap;
exports.buildCommandPaletteManifest = buildCommandPaletteManifest;
exports.generateGraphQLSchemaSDL = generateGraphQLSchemaSDL;
exports.planGuardedInvocation = planGuardedInvocation;
exports.planGuardedInvocationForBinding = planGuardedInvocationForBinding;
exports.LINEARX_SYSTEM_PROMPT = `MASTER SYSTEM PROMPT — “LinearX for IntelGraph”
...
exports.LINEARX_PROVIDER_ROUTING = [
    {
...
exports.LINEARX_TOOL_CONTRACTS = [
    {
        name: 'policy.check',
...
exports.LINEARX_QUICK_NEXT_STEPS = [
    {
        id: 1,
        description: 'Paste this master prompt as the LinearX orchestration agent system prompt.',
    },
    {
        id: 2,
        description: 'Wire Groq → OpenRouter → OpenAI → Anthropic routing with tag mapping.',
    },
    {
        id: 3,
        description: 'Expose linearx.* tool endpoints in the GraphQL layer and log mutations via prov.register.',
    },
    {
        id: 4,
        description: 'Add keyboard map and command palette intents so NL actions round-trip to the agent.',
    },
    {
        id: 5,
        description: 'Enable dependency graph overlays and risk heatmaps on boards.',
    },
    {
        id: 6,
        description: 'Publish the GraphQL resolver manifest + SDL from the gateway for Build Platform scaffolding.',
    },
    {
        id: 7,
        description: 'Launch the release-notes composer and spec-to-tests intents so command palette actions round-trip end to end.',
    },
];
exports.LINEARX_SHORT_ANSWERS = [
    {
...
exports.LINEARX_KEYBOARD_SHORTCUTS = [
    {
...
exports.LINEARX_COMMAND_PALETTE_INTENTS = [
    {
        id: 'command_palette',
...
exports.LINEARX_COMMAND_PALETTE_CATEGORIES = [
    {
        id: 'navigation',
...
exports.LINEARX_BOARD_ENHANCEMENTS = [
    {
...
exports.LINEARX_GRAPHQL_BINDINGS = [
    {
        operation: 'Mutation',
...
exports.LINEARX_SPEC = {
    systemPrompt: exports.LINEARX_SYSTEM_PROMPT,
    providerRouting: exports.LINEARX_PROVIDER_ROUTING,
    ...
    toolContracts: exports.LINEARX_TOOL_CONTRACTS,
    quickNextSteps: exports.LINEARX_QUICK_NEXT_STEPS,
    shortAnswers: exports.LINEARX_SHORT_ANSWERS,
    keyboardShortcuts: exports.LINEARX_KEYBOARD_SHORTCUTS,
    commandPaletteIntents: exports.LINEARX_COMMAND_PALETTE_INTENTS,
    commandPaletteCategories: exports.LINEARX_COMMAND_PALETTE_CATEGORIES,
    boardEnhancements: exports.LINEARX_BOARD_ENHANCEMENTS,
    graphqlBindings: exports.LINEARX_GRAPHQL_BINDINGS,
};
function resolveProviderForTag(tag, options = {}, spec = exports.LINEARX_SPEC) {
...
function listToolContract(name) {
    if (!name) {
        return exports.LINEARX_TOOL_CONTRACTS;
    }
    return exports.LINEARX_TOOL_CONTRACTS.find((contract) => contract.name === name);
}
function summarizeQuickStart() {
    return exports.LINEARX_QUICK_NEXT_STEPS.map((step) => `${step.id}. ${step.description}`);
}
function answerShortQuestion(question) {
    const match = exports.LINEARX_SHORT_ANSWERS.find((entry) => entry.question === question);
    return match?.answer;
}
function listKeyboardShortcuts(action) {
    if (!action) {
        return exports.LINEARX_KEYBOARD_SHORTCUTS;
    }
    return exports.LINEARX_KEYBOARD_SHORTCUTS.find((shortcut) => shortcut.action === action);
}
function listCommandPaletteIntents(id) {
    if (!id) {
        return exports.LINEARX_COMMAND_PALETTE_INTENTS;
    }
    return exports.LINEARX_COMMAND_PALETTE_INTENTS.find((intent) => intent.id === id);
}
function listCommandPaletteCategories(id) {
    if (!id) {
        return exports.LINEARX_COMMAND_PALETTE_CATEGORIES;
    }
    return exports.LINEARX_COMMAND_PALETTE_CATEGORIES.find((category) => category.id === id);
}
function listBoardEnhancements(feature) {
    if (!feature) {
        return exports.LINEARX_BOARD_ENHANCEMENTS;
    }
    return exports.LINEARX_BOARD_ENHANCEMENTS.find((enhancement) => enhancement.feature === feature);
}
function listGraphQLBindings(field) {
    if (!field) {
        return exports.LINEARX_GRAPHQL_BINDINGS;
    }
    return exports.LINEARX_GRAPHQL_BINDINGS.find((binding) => binding.field === field);
}
function buildKeyboardShortcutMap(shortcuts = exports.LINEARX_KEYBOARD_SHORTCUTS) {
...
function buildCommandPaletteManifest(intents = exports.LINEARX_COMMAND_PALETTE_INTENTS, shortcuts = exports.LINEARX_KEYBOARD_SHORTCUTS, categories = exports.LINEARX_COMMAND_PALETTE_CATEGORIES) {
    const shortcutSetByIntent = new Map();
    for (const shortcut of shortcuts) {
        const intentId = shortcut.commandPaletteIntentId;
        if (!intentId) {
            continue;
        }
        const set = shortcutSetByIntent.get(intentId) ?? new Set();
    ...
    const shortcutsByIntent = {};
    for (const intent of intents) {
        const combined = new Set(intent.relatedShortcuts ?? []);
    ...
    const normalizedCategories = categories
        .map((category) => ({
        ...category,
        intentIds: category.intentIds.filter((intentId) => intents.some((intent) => intent.id === intentId)),
    }))
...
function generateGraphQLSchemaSDL(bindings = exports.LINEARX_GRAPHQL_BINDINGS) {
    const grouped = {
        Query: [],
        Mutation: [],
    };
...
function planGuardedInvocation(tool, policyAction, options = {}) {
    const steps = [];
    let order = 1;
    steps.push({
        order: order++,
...
function planGuardedInvocationForBinding(bindingOrField) {
    const binding = typeof bindingOrField === 'string'
        ? exports.LINEARX_GRAPHQL_BINDINGS.find((candidate) => candidate.field === bindingOrField)
        : bindingOrField;
    if (!binding) {
        return undefined;
    }

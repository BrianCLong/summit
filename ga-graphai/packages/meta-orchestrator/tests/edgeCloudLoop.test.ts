...
describe("EdgeCloudLoopController", () => {
  it("prefers edge for simple, low-latency work", () => {
    ...
    expect(decision.reasons.some((reason) => reason.includes("edge preference"))).toBe(true);
  });
  ...
  it("avoids cloud when queue is saturated even if complexity is high", () => {
    ...
    expect(decision.tier).toBe("edge");
    expect(decision.reasons.some((reason) => reason.includes("cloud queue saturated"))).toBe(true);
  });
  ...
  it("triggers rollback guidance when incidents are open", () => {
    ...
    expect(evaluation.reasons.some((reason) => reason.includes("rollback"))).toBe(true);
  });

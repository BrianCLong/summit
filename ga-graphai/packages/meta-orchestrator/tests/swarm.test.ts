...
describe("SwarmOrchestrator", () => {
  it("recovers with fallback agents and records communication", async () => {
    ...
    expect(
      orchestrator.messages.some((message) => message.content.includes("Attempting fallback"))
    ).toBe(true);
  });
  ...
  it("clears active assignments after failure paths", async () => {
    ...
    const activeAssignments = (
      orchestrator as unknown as { activeAssignments: Map<string, number> }
    ).activeAssignments;
...
describe("ConsensusProtocol", () => {
  it("favors higher confidence weighted by reputation", () => {
    ...
    const protocol = new ConsensusProtocol(
      () => new Map(marketplace.list().map((agent) => [agent.id, agent.reputation]))
    );

...
class AutonomousEvolutionOrchestrator {
    ...
    async run(initialPrompt) {
        ...
        for (let iteration = 1; iteration <= this.maxEvolutionRounds; iteration += 1) {
            ...
            const boundaryResults = await this.evaluateBoundaries(result.finalOutput, iteration, result.logs);
    ...
    buildRsipEngine() {
        ...
        const rsipOptions = {
            ...
            refinePrompt: (previousPrompt, output, prioritizedAspects, iteration, history) => {
                const baseRefinement = this.rsipOptions.refinePrompt
                    ? this.rsipOptions.refinePrompt(previousPrompt, output, prioritizedAspects, iteration, history)
        ...
        return new rsip_js_1.RecursiveSelfImprovementEngine(rsipOptions);
    }
    defaultRefinement(previousPrompt, output, prioritizedAspects) {
    ...
    captureEmergence(iteration, signals, existing) {
        const updates = [];
    ...
    async evaluateBoundaries(output, iteration, history) {
        const results = [];
    ...
    composeNextPrompt(prompt, emergent, boundaryViolations) {
        const directives = [];
...
exports.AutonomousEvolutionOrchestrator = AutonomousEvolutionOrchestrator;

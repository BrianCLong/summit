...
interface DataSource {
  ...
  category: 'threat_intel' | 'logs' | 'metrics' | 'osint' | 'forensics' | 'financial' | 'social' | 'geo';
  provider: string;
...
const DataConnectorsDashboard: React.FC<DataConnectorsDashboardProps> = ({
  ...
  const [activeView, setActiveView] = useState<'connectors' | 'templates' | 'transformations' | 'monitoring'>('connectors');
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'configuring': return '#17a2b8';
  ...
  return (
    <div className={`data-connectors-dashboard ${className}`} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px', marginBottom: '16px' }}>
          <div style={{ padding: '16px', border: '1px solid var(--hairline)', borderRadius: '8px', backgroundColor: '#f8f9fa' }}>
          ...
          <div style={{ padding: '16px', border: '1px solid var(--hairline)', borderRadius: '8px', backgroundColor: '#f8f9fa' }}>
            <div style={{ fontSize: '20px', fontWeight: '600', color: '#1a73e8', marginBottom: '4px' }}>
          ...
          <div style={{ padding: '16px', border: '1px solid var(--hairline)', borderRadius: '8px', backgroundColor: '#f8f9fa' }}>
            <div style={{ fontSize: '20px', fontWeight: '600', color: '#ffc107', marginBottom: '4px' }}>
              {Math.round(dataSources.reduce((sum, s) => sum + s.healthMetrics.availability, 0) / dataSources.length)}%
            </div>
      ...
      <div style={{ flex: 1, overflow: 'hidden' }}>
        {activeView === 'connectors' && (
          <div style={{ height: '100%', display: 'grid', gridTemplateColumns: selectedConnector ? '1fr 1fr' : '1fr', gap: '16px' }}>
            {/* Connectors List */}
                ...
                {filteredDataSources.map(source => (
                  <div
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      backgroundColor: selectedConnector?.id === source.id ? '#e3f2fd' : 'transparent',
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '12px', color: '#666' }}>{source.provider} • {source.type}</div>
                        </div>
                      </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', fontSize: '12px' }}>
                      <div>
                        ...
                        <div style={{ fontWeight: '600' }}>{formatNumber(source.recordsImported)}</div>
                      </div>
                      <div>
                        <div style={{ color: '#666' }}>Uptime</div>
                        <div style={{ fontWeight: '600' }}>{source.healthMetrics.availability.toFixed(1)}%</div>
                      </div>
                      <div>
                        <div style={{ color: '#666' }}>Response</div>
                        <div style={{ fontWeight: '600' }}>{source.healthMetrics.responseTime.toFixed(0)}ms</div>
                      </div>
                    </div>
                ...
                <div style={{ padding: '16px' }}>
                  <div style={{ marginBottom: '24px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                      <div style={{ fontSize: '32px' }}>{getCategoryIcon(selectedConnector.category)}</div>
                      <div>
                        <h5 style={{ fontSize: '18px', fontWeight: '600', margin: '0 0 4px 0' }}>
                          {selectedConnector.name}
                        </h5>
                        <div style={{ fontSize: '13px', color: '#666' }}>
                          {selectedConnector.provider} • {selectedConnector.category.replace('_', ' ')} • {selectedConnector.type}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: '24px' }}>
                    <h6 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '12px' }}>Data Flow Performance</h6>
                    
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '16px' }}>
                      <div style={{ padding: '12px', backgroundColor: '#f8f9fa', borderRadius: '6px' }}>
                        <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Ingestion</div>
                        <div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '4px' }}>
                          {formatNumber(selectedConnector.dataFlow.ingestion.totalRecords)}
                        </div>
                        <div style={{ fontSize: '11px', color: '#666' }}>
                          Last hour: {selectedConnector.dataFlow.ingestion.lastHour} • 
                          Errors: {selectedConnector.dataFlow.ingestion.errors}
                        </div>
                      </div>
                      
                      <div style={{ padding: '12px', backgroundColor: '#f8f9fa', borderRadius: '6px' }}>
                        <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Transformation</div>
                        <div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '4px' }}>
                          {selectedConnector.dataFlow.transformation.rulesApplied}
                        </div>
                        <div style={{ fontSize: '11px', color: '#666' }}>
                          Enrichments: {formatNumber(selectedConnector.dataFlow.transformation.enrichments)} • 
                          Errors: {selectedConnector.dataFlow.transformation.validationErrors}
                        </div>
                      </div>
                      
                      <div style={{ padding: '12px', backgroundColor: '#f8f9fa', borderRadius: '6px' }}>
                        <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Storage</div>
                        <div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '4px' }}>
                          {formatNumber(selectedConnector.dataFlow.storage.entitiesCreated)}
                        </div>
                        <div style={{ fontSize: '11px', color: '#666' }}>
                          Entities • Rels: {formatNumber(selectedConnector.dataFlow.storage.relationshipsCreated)} • 
                  ...
                  <div style={{ marginBottom: '24px' }}>
                    <h6 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '12px' }}>Health Metrics</h6>
                        ...
                        <div style={{ 
                          ...
                          <div 
                            style={{ 
                              ...
                              height: '100%', 
                              backgroundColor: selectedConnector.healthMetrics.availability > 95 ? '#28a745' :
                                              selectedConnector.healthMetrics.availability > 90 ? '#ffc107' : '#dc3545',
                              borderRadius: '3px'
                            }} 
                          />
                        </div>
                        ...
                        <div style={{ 
                          ...
                          <div 
                            style={{ 
                              ...
                              height: '100%', 
                              backgroundColor: selectedConnector.healthMetrics.dataQuality > 95 ? '#28a745' :
                                              selectedConnector.healthMetrics.dataQuality > 90 ? '#ffc107' : '#dc3545',
                              borderRadius: '3px'
                            }} 
                          />
                        </div>
                    ...
                    <div style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', fontSize: '13px' }}>
                      <div>
                        <strong>Response Time:</strong> {selectedConnector.healthMetrics.responseTime.toFixed(0)}ms
                      </div>
                      <div>
                        <strong>Throughput:</strong> {selectedConnector.healthMetrics.throughput.toFixed(1)} rec/s
                      </div>
                    </div>
                  </div>

                  <div style={{ marginBottom: '24px' }}>
                    <h6 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '8px' }}>Sync Configuration</h6>
                    <div style={{ fontSize: '13px', lineHeight: '1.4' }}>
                      <div><strong>Frequency:</strong> {selectedConnector.syncFrequency}</div>
                      <div><strong>Last Sync:</strong> {selectedConnector.lastSync.toLocaleString()}</div>
                      {selectedConnector.nextSync && (
                        <div><strong>Next Sync:</strong> {selectedConnector.nextSync.toLocaleString()}</div>
                      )}
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button
                        style={{
                          padding: '8px 16px',
                          fontSize: '12px',
                          backgroundColor: selectedConnector.status === 'connected' ? '#dc3545' : '#28a745',
                          color: 'white',
                          border: 'none',
                          borderRadius: '4px',
                          cursor: 'pointer'
                    ...
                    <div>
                      <span
                        style={{
                          padding: '2px 6px',
                          borderRadius: '12px',
                          backgroundColor: template.difficulty === 'easy' ? '#d4edda' :
                                          template.difficulty === 'medium' ? '#fff3cd' : '#f8d7da',
                          color: template.difficulty === 'easy' ? '#155724' :
                                 template.difficulty === 'medium' ? '#856404' : '#721c24',
                          fontWeight: '500'
                        }}
                      >

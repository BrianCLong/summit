...
export type StixObject = { type: string; id: string; name?: string; description?: string; pattern?: string; created?: string; modified?: string; source_ref?: string; target_ref?: string; relationship_type?: string };

export async function upsertStixBundle(objs: StixObject[], source: string) {
  ...
  for (const o of objs) {
    if (o.type === "indicator") {
      ...
      `, { id: o.id, name: o.name ?? o.pattern ?? o.id, pattern: o.pattern ?? "", created: o.created ?? "", modified: o.modified ?? "", source });
      ...
      `, { id: o.id, name: o.name ?? o.id, type: o.type.replace("-", " "), created: o.created ?? "", modified: o.modified ?? "", desc: o.description ?? "", source });
    }
  }
  ...
  for (const o of objs) {
    if (o.type === "relationship" && o.source_ref && o.target_ref) {
      ...
      `, { src: o.source_ref, tgt: o.target_ref, kind: o.relationship_type ?? "related-to", source, created: o.created ?? "", modified: o.modified ?? "" });
    }
  }
}

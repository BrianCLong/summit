...
function getGraphContextCacheKey(investigationId: string, focusIds: string[]) {
  const focusHash = require('crypto').createHash('sha256').update(focusIds.sort().join(':')).digest('hex');
  return `rag:gctx:${investigationId}:${focusHash}`;
}

// 1) Graph pull (Neo4j): nearest K entities to the focus nodes
export async function fetchGraphContext({ investigationId, focusIds, k = 12 }: { investigationId: string; focusIds: string[]; k?: number }) {
...
export function buildRagPrompt({ question, graph, passages }: {
  question: string; graph: any[]; passages: {source:string; text:string}[];
}) {
  const graphBullets = graph.map(g => `- ${g.name} (${g.types?.join(",") ?? ""})${g.source ? ` [${g.source}]` : ''}`).join("\n");
  ...
  return [
    ...
    "### Graph Context", graphBullets || "(none)",
    "### Documents", docBullets || "(none)",
    "### Question", question,
    "### Answer"
  ].join("\n");
}

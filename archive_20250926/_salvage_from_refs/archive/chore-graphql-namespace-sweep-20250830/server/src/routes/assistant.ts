...
function logExperiment(reqId: string, userId: string | null, exp: string, variant: string) {
...
export function mountAssistant(app: Express, io?: any) {
  const write = (res: Response, obj: any) => res.write(JSON.stringify(obj) + "\n");

  // POST /assistant/stream -> chunked text
  app.post("/assistant/stream", auth(true), rateLimit(), async (req: Request, res: Response) => {
    ...
    if (process.env.ASSISTANT_RAG === "1") { // Feature flag check
      ...
      if (Math.random() < 0.1) { // 10% of requests go to RAG variant
        ...
        cites = [...graphContext.map((g:any) => ({ kind: "graph", ...g })), ...textPassages.map((p:any) => ({ kind: "doc", ...p }))];
      }
    }
    ...
    try {
      ...
      for await (const token of llm.stream(input, ac.signal)) {
        tokens += 1; tokensOut.inc({ mode: "fetch" }, 1);
  ...
  app.get("/assistant/sse", auth(true), rateLimit(), async (req: Request, res: Response) => {
    const started = Date.now();
    const reqId = (req as any).reqId;
    const userId = (req as any).user?.sub || null;
    ...
    try {
      ...
      for await (const tok of llm.stream(input, ac.signal)) {
        tokens += 1; tokensOut.inc({ mode: "sse" }, 1);
  ...
  if (io) {
    io.on("connection", (socket: any) => {
      socket.on("assistant:ask", async ({ input, focusIds = [] }: { input: string; focusIds?: string[] }) => { // Added focusIds
        if (isSuspicious(input)) {
        ...
        try {
          ...
          for await (const tok of llm.stream(input, ac.signal)) {
            tokens += 1; tokensOut.inc({ mode: "socket" }, 1);

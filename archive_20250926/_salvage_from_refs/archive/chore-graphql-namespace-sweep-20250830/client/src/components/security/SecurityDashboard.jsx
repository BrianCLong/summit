...
const SecurityDashboard = () => {
  ...
  const getRoleIcon = (role) => {
    switch (role) {
      ...
      case 'SECURITY_ANALYST': return <Shield color="warning" />;
      case 'SENIOR_ANALYST': return <Assessment color="info" />;
      case 'ANALYST': return <Person color="primary" />;
  ...
  const getEventIcon = (eventType) => {
    switch (eventType) {
      case 'LOGIN_SUCCESS': return <CheckCircle color="success" />;
      case 'LOGIN_FAILED': return <Warning color="error" />;
      case 'SUSPICIOUS_ACTIVITY': return <Warning color="error" />;
      case 'PERMISSION_DENIED': return <Lock color="warning" />;
      case 'MFA_ENABLED': return <VpnLock color="success" />;
      case 'ADMIN_ACTION': return <Settings color="info" />;
    ...
    <Box sx={{ p: 2 }}>
            ...
            <Grid item xs={6} sm={3}>
              <Paper sx={{ p: 2, textAlign: 'center' }}>
                ...
                <Typography variant="h6" color="info.main">
                  {Math.round((mockSecurityStats.users.mfaEnabled / mockSecurityStats.users.total) * 100)}%
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  users protected
                </Typography>
      ...
      <Card>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Tabs value={currentTab} onChange={handleTabChange}>
            <Tab 
              ...
              icon={<Badge badgeContent={mockSecurityEvents.filter(e => !e.resolved).length} color="error">
                <Warning />
              </Badge>}
            />
            <Tab 
              label="User Management" 
              icon={<Badge badgeContent={mockUsers.length} color="primary">
                <Groups />
              </Badge>}
            />
            <Tab 
              label="Access Control" 
              icon={<Badge badgeContent={mockSecurityStats.users.locked} color="warning">
                <Lock />
              </Badge>}
            />
            <Tab 
              label="Audit Logs" 
              icon={<Badge badgeContent="99+" color="info">
                <History />
              </Badge>}
            />
          </Tabs>
        ...
        <CardContent>
                ...
                {mockSecurityEvents.map((event) => (
                  <ListItem 
                    ...
                    sx={{ 
                      ...
                      borderLeft: event.resolved ? 'none' : `4px solid ${
                        event.riskLevel === 'CRITICAL' ? 'error.main' :
                        event.riskLevel === 'HIGH' ? 'warning.main' : 'info.main'
                      }`
                    }}
                  >
                    <ListItemIcon>
                    ...
                    <TableRow key={user.id}>
                      ...
                      <TableCell>
                        <Chip 
                          ...
                          color={
                            user.securityClearance === 'TOP_SECRET' ? 'error' :
                            user.securityClearance === 'SECRET' ? 'warning' :
                            user.securityClearance === 'CONFIDENTIAL' ? 'info' : 'default'
                          }
                          size="small"
                        />
                      </TableCell>
          ...
          {currentTab === 2 && (
            <Box>
              <Typography variant="h6" gutterBottom>Access Control Policies</Typography>
          ...
          {currentTab === 3 && (
            <Box>
              ...
              <Alert severity="info" sx={{ mb: 2 }}>
                Audit logs are automatically retained for 7 years for compliance purposes. 
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setNewUser(prev => ({ ...prev, securityClearance: e.target.value }))}
                >

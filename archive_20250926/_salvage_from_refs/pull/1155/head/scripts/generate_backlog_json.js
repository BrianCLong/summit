...
const issues = rows.map(r=>{
  ...
  const obj = Object.fromEntries(cols.map((c,i)=>[c.trim(), (arr[i]||'').trim()]));
  return obj;
});
...
const gh = issues.map((it)=>({
  ...
  body: `AC: ${it['AC']}`,
  labels: (it['Label']||'').split(';').map(s=>s.trim()).filter(Boolean)
}));
// Jira minimal format
...
fs.writeFileSync('project_management/backlog/github_issues.json', JSON.stringify(gh,null,2));
fs.writeFileSync('project_management/backlog/jira_issues.json', JSON.stringify(jira,null,2));

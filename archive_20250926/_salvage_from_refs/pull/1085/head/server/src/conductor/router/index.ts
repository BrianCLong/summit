...
import { ConductInput, RouteDecision, ExpertType, RoutingFeatures, ExpertCapabilities } from '../types';
...
export class MoERouter {
  ...
  private hasGraphKeywords(task: string): boolean {
    const graphKeywords = [
      'cypher', 'graph', 'pagerank', 'betweenness', 'neighbors', 'neo4j',
      'relationship', 'node', 'edge', 'community', 'centrality', 'path',
      'shortest', 'traverse', 'match', 'return', 'where'
    ];
  ...
  private hasFileKeywords(task: string): boolean {
    const fileKeywords = [
      'file', 'document', 'upload', 'download', 'pdf', 'docx', 'csv',
      'attachment', 'blob', 'storage', 'read', 'write', 'search files'
    ];
  ...
  private hasOSINTKeywords(task: string): boolean {
    const osintKeywords = [
      'web', 'search', 'scrape', 'fetch', 'url', 'domain', 'whois',
      'threat intelligence', 'osint', 'external', 'public', 'social media'
    ];
  ...
  private hasExportKeywords(task: string): boolean {
    const exportKeywords = [
      'export', 'report', 'generate', 'case file', 'summary', 'pdf report',
      'dashboard', 'visualization', 'chart', 'download report'
    ];
  ...
  private calculateComplexity(task: string): number {
    ...
    const complexKeywords = [
      'analyze', 'complex', 'deep', 'comprehensive', 'detailed',
      'multi-step', 'advanced', 'forensic', 'investigation',
   ...
   */
  private selectBestExpert(candidates: ExpertType[], features: RoutingFeatures): {
    expert: ExpertType;
    reason: string;
    confidence: number;
  } {

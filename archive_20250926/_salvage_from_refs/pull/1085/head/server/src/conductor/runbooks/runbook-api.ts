...
runbookRouter.post('/create', async (req, res) => {
  ...
  try {
    ...
    prometheusConductorMetrics.recordOperationalMetric('runbook_creation_time', response.processingTime);
...
runbookRouter.post('/:runbookId/execute', async (req, res) => {
  ...
  try {
    ...
    prometheusConductorMetrics.recordOperationalMetric('runbook_execution_time', response.processingTime);
...
runbookRouter.put('/:runbookId', async (req, res) => {
  ...
  try {
    const { runbookId } = req.params;
    const updateRequest: CreateRunbookRequest & { versionIncrement?: 'patch' | 'minor' | 'major' } = req.body;
    const author = req.user?.sub || 'unknown';
...
runbookRouter.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    prometheusConductorMetrics.recordOperationalEvent(`runbook_api_${req.method.toLowerCase()}`, res.statusCode < 400);
  });

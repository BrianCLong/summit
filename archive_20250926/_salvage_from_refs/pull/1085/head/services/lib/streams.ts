...
export type Event = { key: string; value: string; headers?: Record<string, string> };
...
export class KafkaStream implements Stream {
  ...
  async produce(topic: string, ev: Event) {
    await this.ready;
    await this.p.send({
      topic, messages: [{ key: ev.key, value: ev.value, headers: ev.headers as any }]
    });
  }
  async close() { try { await this.p.disconnect(); } catch {} }
}

export class FanoutStream implements Stream {
  constructor(private a: Stream, private b: Stream) {}
  async produce(topic: string, ev: Event) {
    // best-effort fanout; do not block on slow leg
    await Promise.allSettled([this.a.produce(topic, ev), this.b.produce(topic, ev)]);
  }
  async close() { await Promise.allSettled([this.a.close(), this.b.close()]); }
}
...
export function buildStream(): Stream {
  ...
  if (backend === "dual") return new FanoutStream(new RedisStream(), new KafkaStream());

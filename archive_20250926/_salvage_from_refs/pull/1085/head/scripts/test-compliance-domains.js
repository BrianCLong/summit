...
const TEST_URLS = [
  ...
  { url: 'https://developer.mozilla.org/en-US/docs/Web/JavaScript', expected: true, purpose: 'documentation' },
...
class ComplianceTester {
  ...
  async testRateLimiting() {
    ...
    for (const domain of ['docs.python.org', 'github.com', 'arxiv.org']) {
      ...
      try {
        ...
        console.log(`  ✅ Normal request: ${normalResult.allowed ? 'ALLOWED' : 'DENIED'} (${normalResult.tokensRemaining} tokens remaining)`);
        
        // Test burst allowance
        const burstResult = await this.rateLimiter.checkRateLimit(domain, 'test-tenant', 5);
        console.log(`  ⚡ Burst request: ${burstResult.allowed ? 'ALLOWED' : 'DENIED'} (${burstResult.tokensRemaining} tokens remaining)`);
        ...
        while (exhaustionAttempts < 20 && !rateLimited) {
          const exhaustResult = await this.rateLimiter.checkRateLimit(domain, 'test-tenant-exhaust', 10);
          exhaustionAttempts++;
          
          if (!exhaustResult.allowed) {
            rateLimited = true;
            console.log(`  🔒 Rate limited after ${exhaustionAttempts} attempts (retry after ${exhaustResult.retryAfter}s)`);
          }
        }
  ...
  async testComplianceGate() {
    ...
    for (const testCase of TEST_URLS) {
      ...
      try {
        ...
        console.log(`  ${status} - Expected: ${testCase.expected}, Got: ${result.allowed} - ${result.reason}`);
  ...
  async testConcurrentLoad() {
    ...
    console.log(`✅ Concurrent load test: ${successCount}/5 successful, ${avgLatency.toFixed(0)}ms avg latency`);
  ...
  async testFailureScenarios() {
    const scenarios = [
      ...
      { name: 'Non-existent domain', url: 'https://thisisnotarealdomain12345.com/', shouldFail: true },
  ...
  async generateReport() {
    ...
    for (const result of this.results) {
      ...
      console.log(`${result.test.toUpperCase().replace(/_/g, ' ')}: ${status} (${passRate.toFixed(1)}%)`);
    }
    ...
    console.log(`✅ Robots.txt Compliance: ${robotsPass ? 'PASS' : 'FAIL'} (10+ domains validated)`);
    console.log(`✅ Concurrent Performance: ${loadPass ? 'PASS' : 'FAIL'} (<100ms avg policy decisions)`);
    ...
    console.log(`   Policy Coverage: ${(this.results.find(r => r.test === 'end_to_end_compliance')?.passRate || 0).toFixed(1)}%`);

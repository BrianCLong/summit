...
class NotificationService extends EventEmitter {
  ...
  initializeNotificationTemplates() {
    this.notificationTemplates.set('SECURITY_ALERT', {
      ...
      template: {
        ...
        actions: [
          { label: 'View Details', action: 'view_alert', url: '/security/alerts/{{alertId}}' },
    ...
    this.notificationTemplates.set('INVESTIGATION_UPDATE', {
      ...
      template: {
        ...
        actions: [
          { label: 'View Investigation', action: 'view_investigation', url: '/investigations/{{investigationId}}' }
        ]
      }
    });
    ...
    this.notificationTemplates.set('ENTITY_DISCOVERY', {
      ...
      template: {
        ...
        actions: [
          { label: 'View Entity', action: 'view_entity', url: '/entities/{{entityId}}' },
    ...
    this.notificationTemplates.set('ANALYTICS_COMPLETE', {
      ...
      template: {
        ...
        actions: [
          { label: 'View Results', action: 'view_results', url: '/analytics/results/{{jobId}}' }
        ]
      }
    });
    ...
    this.notificationTemplates.set('REPORT_READY', {
      ...
      template: {
        ...
        actions: [
          { label: 'Download Report', action: 'download_report', url: '/reports/{{reportId}}/download' },
          { label: 'View Online', action: 'view_report', url: '/reports/{{reportId}}' }
        ]
      }
    });
    ...
    this.notificationTemplates.set('SYSTEM_MAINTENANCE', {
      ...
      template: {
        ...
        actions: [
          { label: 'View Details', action: 'view_maintenance', url: '/system/maintenance' }
        ]
      }
    });
    ...
    this.notificationTemplates.set('ANOMALY_DETECTED', {
      ...
      template: {
        ...
        actions: [
          { label: 'Investigate', action: 'investigate_anomaly', url: '/analytics/anomalies/{{anomalyId}}' },
  ...
  initializeAlertRules() {
    this.alertRules.set('HIGH_RISK_ENTITY', {
      ...
      conditions: [
        ...
        { field: 'entity.type', operator: 'in', value: ['PERSON', 'ORGANIZATION'] }
      ],
      actions: [
        {
          type: 'NOTIFICATION',
    ...
    this.alertRules.set('MULTIPLE_FAILED_LOGINS', {
      ...
      conditions: [
        ...
        { field: 'event.count', operator: 'greater_than', value: 5, timeWindow: 300000 } // 5 minutes
      ],
      actions: [
        {
          type: 'NOTIFICATION',
    ...
    this.alertRules.set('UNUSUAL_ACCESS_PATTERN', {
      ...
      enabled: true,
      conditions: [
        { field: 'access.time', operator: 'outside_hours', value: { start: 6, end: 22 } },
    ...
    this.alertRules.set('ANALYTICS_ANOMALY', {
      ...
      conditions: [
        ...
        { field: 'anomaly.type', operator: 'in', value: ['BEHAVIORAL', 'STRUCTURAL', 'TEMPORAL'] }
      ],
      actions: [
        {
          type: 'NOTIFICATION',
  ...
  async sendNotification(notificationData) {
    ...
    if (template) {
      notification.title = this.applyTemplate(template.template.title, notification.data);
      notification.body = this.applyTemplate(template.template.body, notification.data);
      notification.actionUrl = this.applyTemplate(template.template.actionUrl, notification.data);
      notification.actions = template.template.actions.map(action => ({
        ...action,
        url: action.url ? this.applyTemplate(action.url, notification.data) : undefined
      }));
    ...
    for (const recipient of notification.recipients) {
      const deliveries = await this.determineDeliveryChannels(recipient, notification);
  ...
  async deliverToChannel(notification, delivery) {
    ...
    if (!this.metrics.channelStats.has(delivery.channel)) {
      this.metrics.channelStats.set(delivery.channel, { delivered: 0, failed: 0 });
    }
  ...
  async deliverInAppNotification(notification, delivery) {
    ...
    await this.redisClient.ltrim(`user_notifications:${delivery.userId}`, 0, 99);
  ...
  async executeRuleActions(rule, eventData) {
    for (const action of rule.actions) {
      try {
        ...
        this.logger.error(`Rule action execution failed: ${action.type}`, error);
      }
    }
  }
  ...
  getUserSockets(userId) {
    ...
    return Array.from(userRoom).map(socketId => 
      this.socketIO.sockets.sockets.get(socketId)
  ...
  updateDeliveryTimeMetric(deliveryTime) {
    const total = this.metrics.averageDeliveryTime * this.metrics.deliveredNotifications;
    this.metrics.averageDeliveryTime = (total + deliveryTime) / (this.metrics.deliveredNotifications + 1);
  }
  ...
  async getUserNotifications(userId, options = {}) {
    ...
    const notifications = await this.redisClient.lrange(key, offset, offset + limit - 1);
  ...
  getMetrics() {
    const successRate = this.metrics.totalNotifications > 0 
      ? (this.metrics.deliveredNotifications / this.metrics.totalNotifications * 100).toFixed(2)
      : '0';
  ...
  validateWebSocketDelivery(delivery) { return true; }
  validateEmailDelivery(delivery) { return !!delivery.email; }
  validateSMSDelivery(delivery) { return !!delivery.phone; }
  validatePushDelivery(delivery) { return !!delivery.deviceToken; }
  validateWebhookDelivery(delivery) { return !!delivery.webhookUrl; }
  validateInAppDelivery(delivery) { return !!delivery.userId; }
}

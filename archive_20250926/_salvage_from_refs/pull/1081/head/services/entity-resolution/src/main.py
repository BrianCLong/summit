...
def extract_property_features(props1: dict[str, Any], props2: dict[str, Any]) -> dict[str, float]:
    ...
    return {"overlap_ratio": overlap_ratio, "common_values": common_values}
...
def compute_entity_similarity(entity1: Entity, entity2: Entity) -> ScoringFeatures:
    ...
    property_overlap = prop_features["overlap_ratio"] * 0.4 + prop_features["common_values"] * 0.6
...
def deterministic_matching(
    entity1: Entity, entity2: Entity, threshold: float = 0.9
    ...
    # Rule-based scoring
    score = 0.0
    explanation = {"method": "deterministic", "rules_applied": []}
    ...
    # Rule 4: Semantic similarity for organization names
        ...
        weights = {"name": 0.4, "type": 0.2, "properties": 0.25, "semantic": 0.1, "phonetic": 0.05}
...
async def get_cached_score(
    entity_a_id: str, entity_b_id: str, redis_conn
) -> EntityResolutionScore | None:
...
async def resolve_entities(
    request: ResolutionRequest, background_tasks: BackgroundTasks, redis_conn=Depends(get_redis)
):
...
async def deduplicate_entities(request: DeduplicationRequest):
    ...
    if len(request.entities) < 2:
        raise HTTPException(
            status_code=400, detail="At least 2 entities required for deduplication"
        )
...
async def explain_match(request: ExplainabilityRequest, db_pool=Depends(get_db)):

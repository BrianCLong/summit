...
class YOLODetection:
    ...
    def detect_objects(
        ...
        try:
            ...
            # Process results
            detections = self._process_detections(results[0] if results else None, custom_classes)
    ...
    def detect_objects_batch(
        ...
        for image_path in image_paths:
            result = self.detect_objects(
                image_path, confidence_threshold, nms_threshold, max_detections
            )
    ...
    def _process_detections(
        self, result, custom_classes: list[str] | None = None
    ) -> list[dict[str, Any]]:
        ...
        for coord, conf, class_id in zip(coords, confidences, class_ids, strict=False):
            x1, y1, x2, y2 = coord
    ...
    try:
        ...
        if args.output_image and result.get("detections"):
            output_path = detector.draw_detections(
                args.image, result["detections"], args.output_image
            )
            if output_path:

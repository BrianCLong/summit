...
router.post('/explain',
  requireAuthority('graph_xai_analysis', ['explanation', 'model_inference']),
  async (req, res) => {
    try {
      const {
        query,
...
router.get('/model-cards/:version?',
...
router.post('/detect',
  requireAuthority('graph_xai_analysis', ['detection', 'pattern_analysis']),
  async (req, res) => {
    try {
      const {
        data_source = 'api_request',
        graph_data,
        detection_types = ['anomaly_detection', 'pattern_matching'],
      ...
      const invalidTypes = detection_types.filter((type: string) => 
        !validDetectionTypes.includes(type)
      );
...
router.post('/cache/clear',
  requireAuthority('graph_xai_analysis', ['cache_management']),
  async (req, res) => {
    try {
      const user = req.user as any;
...
router.get('/cache/stats',
  requireAuthority('graph_xai_analysis', ['cache_stats']),
  async (req, res) => {
    try {
...
router.post('/explain/batch',
  requireAuthority('graph_xai_analysis', ['batch_explanation']),
  async (req, res) => {
    try {
      const {
        requests = [],

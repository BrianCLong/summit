...
describe('Relationship resolvers', () => {
  ...
  test('createRelationship returns payload', async () => {
    const input = { sourceId: 'a', targetId: 'b', type: 'RELATED_TO', label: 'Related' };
    const out = await resolvers.Mutation.createRelationship(null, { input }, { user });
  ...
  test('deleteRelationship returns true', async () => {
    const out = await resolvers.Mutation.deleteRelationship(null, { id: 'rel1' }, { user });

...
`ci-verify.yml` provides a false sense of security regarding supply chain integrity. It has the _structure_ of verification but lacks the _substance_ because it allows placeholders and continues on error. The robust logic exists in `_reusable-slsa-build.yml` and `scripts/sbom-attest.sh` but is not wired into the main PR gate.

...
1.  **Read the Instructions**: You **MUST** read and adhere to [`AGENTS.md`](AGENTS.md). It contains specific directives for code generation, architectural boundaries, and prohibited actions.
    - AI-specific guidance lives in [`docs/ai/AGENT_GUIDELINES.md`](docs/ai/AGENT_GUIDELINES.md) with Python-focused expectations in [`docs/ai/AGENT_GUIDELINES_PYTHON.md`](docs/ai/AGENT_GUIDELINES_PYTHON.md). Review the relevant document before coding.
2.  **Reference the Readiness Baseline**: Treat [`docs/SUMMIT_READINESS_ASSERTION.md`](docs/SUMMIT_READINESS_ASSERTION.md) as the source of truth for readiness claims and cite it in summaries when relevant.

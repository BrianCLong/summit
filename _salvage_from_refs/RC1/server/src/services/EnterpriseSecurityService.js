...
class EnterpriseSecurityService extends EventEmitter {
    ...
    const roles = [
      {
        ...
        description:
          "Standard analyst with investigation and analysis capabilities",
        permissions: [
  ...
  async authenticate(credentials) {
    ...
    try {
      ...
      if (!passwordValid) {
        ...
        await this.updateUserFailedAttempts(
          user.id,
      ...
      if (accessControlPolicy.locationBasedAccess) {
        const locationAllowed = await this.checkLocationAccess(
          user.id,
          clientInfo.ip,
        );
        if (!locationAllowed) {
  ...
  async checkPermission(sessionToken, requiredPermission, resource = null) {
    try {
      ...
      if (resource && resource.dataClassification) {
        if (
          !session.dataClassifications.includes(resource.dataClassification)
        ) {
          await this.logSecurityEvent({
            type: "ACCESS_DENIED",
            user: session.username,
  ...
  async verifyApiToken(tokenString) {
    ...
    const apiToken =
      this.apiTokens.get(tokenId) || (await this.getApiToken(tokenId));
  ...
  async generateComplianceReport(framework, dateRange) {
    ...
    const report = {
      ...
      summary: {
        ...
        violations: auditEvents.filter(
          (e) => e.compliance_violations?.length > 0,
        ).length,
        recommendations: [],
      },

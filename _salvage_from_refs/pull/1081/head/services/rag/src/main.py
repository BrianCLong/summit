...
def contains_dangerous_keyword(query: str) -> str | None:
...
class CypherResponse(BaseModel):
    ...
    diff_vs_manual: dict[str, Any] | None = None
...
async def estimate_cypher_cost(
...
async def generate_cypher(request: CypherRequest):
    ...
    try:
        ...
        if keyword and request.authority != AuthorityLevel.ADMIN:
            raise HTTPException(
                status_code=403, detail=f"{keyword} operations require admin authority"
            )

        rows, cost = await estimate_cypher_cost(cypher, {"tenantId": request.tenant_id})
        if rows is None or cost is None:
            ...
            if rows > MAX_SAFE_ROWS:
                ...
                if request.authority != AuthorityLevel.ADMIN:
                    raise HTTPException(
                        status_code=403, detail="Row estimate exceeds limit for current authority"
                    )
            if cost > MAX_SAFE_COST:
                warnings.append("Estimated cost exceeds safe threshold")
                if request.authority != AuthorityLevel.ADMIN:
                    raise HTTPException(
                        status_code=403, detail="Cost estimate exceeds limit for current authority"
                    )
        ...
        if request.sandbox:
            ...
            if request.manual_cypher:
                manual_results = await run_cypher_sandbox(
                    request.manual_cypher, {"tenantId": request.tenant_id}
                )

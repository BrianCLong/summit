  ...
  app.get('/api/graph/export/:format', (req, res) => {
    ...
    switch (format) {
      case 'json':
        ...
        break;
      case 'csv': {
        const csvNodes = mockGraphData.nodes.map(n => `"${n.id}","${n.label}","${n.type}"`).join('\n');
        const csvEdges = mockGraphData.edges.map(e => `"${e.source}","${e.target}","${e.label}"`).join('\n');
...
<graphml xmlns="http://graphml.graphdrawing.org/xmlns">
  ...
  app.get('/api/graph/analytics', (req, res) => {
    const analytics = {
      nodeCount: mockGraphData.nodes.length,
      edgeCount: mockGraphData.edges.length,
      density: mockGraphData.edges.length / (mockGraphData.nodes.length * (mockGraphData.nodes.length - 1)),

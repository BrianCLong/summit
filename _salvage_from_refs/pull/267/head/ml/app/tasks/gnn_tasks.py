        ...
        if task_mode == 'train':
            ...
            for i, pyg_data in enumerate(pyg_graphs):
                ...
                graph_pred = {
                    ...
                    'predicted_class': int(np.argmax(predictions[0])) if len(predictions) > 0 else 0,
                    'confidence': float(np.max(predictions[0])) if len(predictions) > 0 else 0.0,
                    'graph_embedding': embeddings[0].tolist() if embeddings is not None and len(embeddings) > 0 else None,
                    'num_nodes': pyg_data.num_nodes,
                    'num_edges': pyg_data.edge_index.size(1)
                }
            ...
            for i, node in enumerate(G.nodes()):
                if i < len(predictions):
                    ...
                    node_anomalies[str(node)] = {
                        ...
                        'embedding': embeddings[i].tolist() if embeddings is not None and i < len(embeddings) else None
                    }

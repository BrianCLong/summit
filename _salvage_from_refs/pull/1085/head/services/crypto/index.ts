export interface KeyProvider {
  ...
  generateDataKey(ctx: Record<string,string>): Promise<{ plaintext: Buffer; ciphertext: Buffer }>;
}

export class AwsKmsProvider implements KeyProvider {
  constructor(private keyIdOrAlias = process.env.KMS_KEY_ID || "alias/conductor/stage/mrk") {}
  private kms = new (require("@aws-sdk/client-kms").KMSClient)({ region: process.env.AWS_REGION });
  async encrypt(pt: Buffer, ctx: Record<string,string>) {
    const { EncryptCommand } = require("@aws-sdk/client-kms");
    const r = await this.kms.send(new EncryptCommand({ KeyId: this.keyIdOrAlias, Plaintext: pt, EncryptionContext: ctx }));
  ...
  async generateDataKey(ctx: Record<string,string>) {
    ...
    const r = await this.kms.send(new GenerateDataKeyCommand({ KeyId: this.keyIdOrAlias, KeySpec: "AES_256", EncryptionContext: ctx }));
    return { plaintext: Buffer.from(r.Plaintext), ciphertext: Buffer.from(r.CiphertextBlob) };
  }
}

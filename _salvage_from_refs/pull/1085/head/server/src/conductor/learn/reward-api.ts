...
interface RewardRequest {
  ...
  contextHash?: string;
  armId: ExpertArm;
  rewardType: 'success_at_k' | 'human_thumbs' | 'incident_free' | 'accepted_insight' | 'implicit_feedback';
...
class RewardCalculator {
  ...
  private static calculateImplicitFeedback(metadata: any = {}): number {
    ...
    if (metadata.session_duration) {
      if (metadata.session_duration > 300) { // >5 minutes = good
        reward += 0.2;
      } else if (metadata.session_duration < 30) { // <30s = bad
        reward -= 0.3;
      }
    }
...
rewardRouter.post('/reward', async (req, res) => {
  ...
  try {
    ...
    if (!['success_at_k', 'human_thumbs', 'incident_free', 'accepted_insight', 'implicit_feedback'].includes(rewardRequest.rewardType)) {
      return res.status(400).json({
        success: false,
    ...
    const rewardSignal: RewardSignal = {
      ...
      contextHash: rewardRequest.contextHash || rewardRequest.decisionId || `implicit_${Date.now()}`,
      rewardValue,
    ...
    prometheusConductorMetrics.recordOperationalMetric('reward_processing_time', response.processingTime);
  ...
  try {
    ...
    const processingPromises = rewards.map(async (rewardRequest, index) => {
      try {
        ...
        const rewardSignal: RewardSignal = {
          armId: rewardRequest.armId,
          contextHash: rewardRequest.contextHash || rewardRequest.decisionId || `batch_${Date.now()}_${index}`,
          rewardValue,
          rewardType: rewardRequest.rewardType,
          timestamp: Date.now(),
          metadata: rewardRequest.metadata
...
rewardRouter.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    prometheusConductorMetrics.recordOperationalEvent(`reward_api_${req.method.toLowerCase()}`, res.statusCode < 400);
  });

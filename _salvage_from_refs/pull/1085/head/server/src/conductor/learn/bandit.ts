...
export class LinUCBBandit {
  ...
  private extractFeatures(context: BanditContext): number[] {
    ...
    const domainIdx = domains.indexOf(context.domain) !== -1 ? domains.indexOf(context.domain) : domains.length - 1;
    ...
    if (context.timeOfDay && idx + 4 <= this.featureDimension) {
      const timeMap = { morning: [1,0,0,0], afternoon: [0,1,0,0], evening: [0,0,1,0], night: [0,0,0,1] };
...
export class AdaptiveRouter extends EventEmitter {
  ...
  constructor(config: BanditConfig, redis: Redis) {
    ...
    const arms: ExpertArm[] = [
      'LLM_LIGHT', 'LLM_HEAVY', 'GRAPH_TOOL', 
      'RAG_TOOL', 'FILES_TOOL', 'OSINT_TOOL', 'EXPORT_TOOL'
    ];
  ...
  async route(context: BanditContext, query: string): Promise<RouteDecision> {
    ...
    switch (this.config.algorithm) {
      ...
      case 'hybrid':
        ...
        if (totalPulls < this.config.minPullsBeforeExploit * 7) { // 7 arms
          selectedArm = this.thompsonBandit.selectArm(context);
          confidence = this.getThompsonConfidence(selectedArm);
          explorationReason = 'thompson_sampling';
        } else {
  ...
  async processReward(rewardSignal: RewardSignal): Promise<void> {
    ...
    if (rewardSignal.rewardValue < 0.5) {
      this.failureStreak.set(rewardSignal.armId, 
  ...
  private createSafetyDecision(context: BanditContext, contextHash: string, decisionId: string): RouteDecision {
    return {
  ...
  private createProductionDecision(context: BanditContext, contextHash: string, decisionId: string): RouteDecision {

...
export class ConflictUIController {
  ...
  async getConflictDeltas(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.error('Failed to get conflict deltas', { error: error.message, entityId: req.params.entityId });
      res.status(500).json({
  ...
  async resolveConflicts(req: Request, res: Response): Promise<void> {
    try {
      ...
      const auditTrail = resolutions.map(resolution => ({
        ...
        rationale: resolution.rationale + (resolutionRationale ? ` | User rationale: ${resolutionRationale}` : ''),
      ...
      logger.error('Failed to resolve conflicts', { error: error.message, entityId: req.params.entityId });
      res.status(500).json({
  ...
  async getConflictHistory(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.error('Failed to get conflict history', { error: error.message, entityId: req.params.entityId });
      res.status(500).json({

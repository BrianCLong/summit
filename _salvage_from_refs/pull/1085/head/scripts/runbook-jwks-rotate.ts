...
fs.writeFileSync(".secrets/runbook-signer-"+kid+".pem", privateKey.export({ type:"pkcs1", format:"pem" }));
const jwksPath = "services/runbooks/jwks.json";
const jwks = fs.existsSync(jwksPath)? JSON.parse(fs.readFileSync(jwksPath,"utf8")) : { keys:[] };

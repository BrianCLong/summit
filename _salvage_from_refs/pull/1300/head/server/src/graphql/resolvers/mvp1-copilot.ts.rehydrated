...
const mvp1CopilotResolvers = {
  Query: {
    ...
    aiCapabilities: async (_: any, __: any, context: AuthContext) => {
      ...
      const capabilities = {
        ...
        linkSuggestions: ['analyst', 'editor', 'investigator', 'admin', 'super_admin'].includes(
          context.user.role,
        ),
        bulkProcessing: ['editor', 'investigator', 'admin', 'super_admin'].includes(
          context.user.role,
        ),
  ...
  Mutation: {
    ...
    extractEntities: async (
      ...
      try {
        const result = await copilotService.extractEntities(
          input.text,
          {
            id: context.user.id,
    ...
    bulkExtractEntities: async (
      ...
      try {
        const results = await copilotService.batchExtractEntities(
          input.texts,
          {
            id: context.user.id,
            email: context.user.email,

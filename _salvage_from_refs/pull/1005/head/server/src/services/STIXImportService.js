...
class STIXImportService {
  ...
  extractCveId(externalRefs) {
    ...
    const cveRef = externalRefs.find(ref => 
      ref.source_name === 'cve' || ref.external_id?.startsWith('CVE-')
    );
  ...
  extractCvssScore(externalRefs) {
    ...
    const cvssRef = externalRefs.find(ref => 
    ...
    if (this.io) {
      const progress = {
        ...
        progress: job.stats.totalObjects > 0 ? 
          (job.stats.processedObjects / job.stats.totalObjects) * 100 : 0,
        stats: job.stats,

...
class PluginService extends EventEmitter {
  ...
  isVersionCompatible(available, required) {
    ...
    const [reqMajor, reqMinor, reqPatch] = required.replace(/[^\d.]/g, '').split('.').map(Number);
  ...
  safeRequire(moduleName, plugin) {
    const allowedModules = [
      'crypto', 'util', 'events', 'stream', 'querystring',
      'url', 'path', 'lodash', 'moment'
    ];
  ...
  async savePluginConfiguration(pluginId) {
    ...
    await fs.writeFile(configPath, JSON.stringify(plugin.configuration, null, 2));

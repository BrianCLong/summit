...
export interface EvidenceArtifact {
  id?: string;
  runId: string;
  artifactType: 'sbom' | 'attestation' | 'log' | 'output' | 'trace' | 'policy_decision';
...
export class EvidenceProvenanceService {
  ...
  async storeEvidence(artifact: EvidenceArtifact): Promise<string> {
    ...
    try {
      ...
      await pool.query(
        ...
        [artifactId, artifact.runId, artifact.artifactType, s3Key, sha256Hash, contentBuffer.length, retentionUntil]
      );
   ...
   */
  private async createProvenanceEntry(artifactId: string, currentHash: string, runId: string): Promise<void> {
    const pool = getPostgresPool();
  ...
  async verifyEvidence(artifactId: string): Promise<{
    ...
    try {
      ...
      if (!artifacts.length) {
        return { valid: false, integrity: false, provenance: false, details: { error: 'Artifact not found' } };
      }
   ...
   */
  async generateSBOMEvidence(runId: string, dependencies: any[]): Promise<string> {
    const sbom = {
      bomFormat: 'CycloneDX',
      specVersion: '1.4',
  ...
  private getDefaultRetentionDays(artifactType: string): number {
    ...
    return retentionPolicies[artifactType as keyof typeof retentionPolicies] || 365;
  }
  ...
  private getContentType(artifactType: string): string {
    ...
    return contentTypes[artifactType as keyof typeof contentTypes] || 'application/octet-stream';
  }

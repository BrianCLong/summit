...
export interface QuotaState { remaining: number|null; used: number|null; cap: number|null; etaReset?: string; windowLabel?: string }
export interface QuotaStore {
  ...
  usedInFixed(model: string, unit: string, period: 'daily'|'weekly', tz: string): Promise<{ used: number; windowStart: string; windowEnd: string }>;
}

// Simple memory fallback. For prod, implement Redis with time‑bucket keys.
export class MemoryQuotaStore implements QuotaStore {
  private e: Array<{ ts: number; model: string; unit: string; amount: number }> = [];
  async record(model: string, unit: any, amount: number) { this.e.push({ ts: Date.now(), model, unit, amount }); }
  async usedInRolling(model: string, unit: string, window: Duration) {
    const cutoff = Date.now() - window.as('milliseconds');
    return this.e.filter(x => x.model===model && x.unit===unit && x.ts>=cutoff).reduce((a,b)=>a+b.amount,0);
  }
  async usedInFixed(model: string, unit: string, period: 'daily'|'weekly', tz: string) {
    ...
    const used = this.e.filter(x => x.model===model && x.unit===unit && x.ts>=start.toMillis() && x.ts<=end.toMillis()).reduce((a,b)=>a+b.amount,0);

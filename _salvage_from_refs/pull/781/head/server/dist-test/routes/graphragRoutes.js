...
router.post('/query', validateRequest(querySchema), async (req, res) => {
    ...
    try {
        ...
        logger_js_1.default.info('GraphRAG query via REST API', {
            userId: req.user?.id,
            investigationId: req.body.investigationId,
        ...
        logger_js_1.default.error('GraphRAG REST query failed', {
            userId: req.user?.id,
            error: error.message
        });
        res.status(500).json({
...
router.post('/embeddings/generate', validateRequest(embeddingSchema), async (req, res) => {
    ...
    try {
      ...
      `;
            ...
            // Generate embeddings
            const embeddings = await embeddingService.generateEmbeddings(entities.map(e => e.text), req.body.model);
            // Store embeddings in Neo4j
            for (let i = 0; i < entities.length; i++) {
                await session.run('MATCH (e:Entity {id: $id}) SET e.embedding = $embedding, e.embeddingModel = $model, e.embeddingGeneratedAt = datetime()', {
                    id: entities[i].id,
                    embedding: embeddings[i],
                    model: req.body.model || embeddingService.config.model
                });
            }
            logger_js_1.default.info('Embeddings generated via REST API', {
                userId: req.user?.id,
                investigationId: req.body.investigationId,
        ...
        logger_js_1.default.error('Embedding generation via REST API failed', {
            userId: req.user?.id,
            error: error.message
        });
        res.status(500).json({

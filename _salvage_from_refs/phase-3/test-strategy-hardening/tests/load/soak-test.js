...
function soakTestMonitoring(baselineResponseTime, testElapsed) {
  ...
  if (success && baselineResponseTime) {
    ...
    if (responseTime > baselineResponseTime * 1.5) {
      ...
      console.warn(`Performance degradation detected: ${responseTime}ms vs baseline ${baselineResponseTime}ms at ${Math.floor(testElapsed/60)} minutes`);
    }
    
    // Simulate memory leak detection (response time increasing over time)
    const expectedTime = baselineResponseTime * (1 + testElapsed / 14400 * 0.2); // Max 20% increase expected

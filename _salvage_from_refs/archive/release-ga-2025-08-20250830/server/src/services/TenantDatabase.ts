...
export class TenantDatabase {
  ...
  async executePostgresQuery(
    ...
    // Enhance query with tenant constraints
    const { query: enhancedQuery, parameters: enhancedParams } = this.addTenantToPostgresQuery(
      sqlQuery,
      parameters,
      tenantContext,
    );
      ...
      try {
        const neo4jResults = await session.executeWrite(async (tx: Transaction) => {
          ...
          for (const op of neo4jOps) {
            const { query, parameters: enhancedParams } = TenantValidator.addTenantToNeo4jQuery(
              op.query,
              op.parameters,
              tenantContext,
            );
    ...
    if (postgresOps.length > 0 && this.postgres) {
      ...
      try {
        ...
        for (const op of postgresOps) {
          const { query, parameters: enhancedParams } = this.addTenantToPostgresQuery(
            op.query,
            op.parameters,
            tenantContext,
          );
   ...
   */
  private generateCacheKey(query: string, parameters: any, tenantContext: TenantContext): string {

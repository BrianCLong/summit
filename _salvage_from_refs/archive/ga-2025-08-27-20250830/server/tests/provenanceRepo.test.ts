...
class FakeClient {
  ...
  lastSql = '';
  addResponse(match: RegExp, result: any, error = false) { this.responses.push({ match, result, error }); }
...
describe('ProvenanceRepo', () => {
  test('maps classic audit_events rows', async () => {
    ...
    client.addResponse(/FROM audit_events .*target_type/, { rows: [
      { id: 'r1', action: 'policy', target_type: 'incident', target_id: 'inc1', metadata: { reasonCode: 'POLICY_DENY' }, created_at: now }
    ]});
  ...
  test('falls back to provenance table and maps fields', async () => {
    ...
    client.addResponse(/FROM provenance /, { rows: [
      { id: 'p1', source: 'graphrag', subject_type: 'investigation', subject_id: 'inv1', note: 'note1', created_at: now }
    ]});
    const pool = new FakePool(client) as any;
    const repo = new ProvenanceRepo(pool);
...
test('includes filters in WHERE (contains + reasonCodeIn)', async () => {
  ...
  const now = new Date();
  client.addResponse(/FROM audit_events .*target_type/, { rows: [] });
  client.addResponse(/FROM audit_events .*resource_type/, { rows: [
    { id: 'r2', action: 'policy', resource_type: 'investigation', resource_id: 'inv1', resource_data: { reasonCode: 'RATE_LIMIT' }, timestamp: now }
  ]});
  const pool = new FakePool(client) as any;
  const repo = new ProvenanceRepo(pool);

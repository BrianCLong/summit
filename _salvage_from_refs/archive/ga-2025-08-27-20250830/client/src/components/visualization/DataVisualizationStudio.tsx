...
interface DataSource {
  id: string;
  name: string;
  type: 'entities' | 'relationships' | 'events' | 'metrics' | 'logs' | 'external';
  fields: Array<{
    name: string;
...
interface DataVisualizationStudioProps {
  ...
  onExportChart?: (chartId: string, format: 'png' | 'svg' | 'pdf' | 'json') => void;
  className?: string;
}
...
const DataVisualizationStudio: React.FC<DataVisualizationStudioProps> = ({
  ...
  const [activeView, setActiveView] = useState<'gallery' | 'builder' | 'templates'>('gallery');
  ...
  const chartTypes = [
    ...
    { type: 'scatter', name: 'Scatter Plot', icon: '🔍', description: 'Explore relationships' },
    { type: 'heatmap', name: 'Heat Map', icon: '🔥', description: 'Show intensity patterns' },
    { type: 'sankey', name: 'Sankey Diagram', icon: '🌊', description: 'Flow analysis' },
    { type: 'treemap', name: 'Tree Map', icon: '🗂️', description: 'Hierarchical data' },
  ...
  const generateMockDataSources = () => {
    const mockDataSources: DataSource[] = [
      ...
      },
      {
        ...
        type: 'relationships',
        fields: [
          { name: 'relationship_type', type: 'category', description: 'Type of relationship' },
      ...
      },
      {
        ...
        type: 'events',
        fields: [
          { name: 'event_type', type: 'category', description: 'Type of event' },
      ...
      },
      {
        ...
        type: 'metrics',
        fields: [
          { name: 'threat_score', type: 'number', description: 'Calculated threat score' },
  ...
  const exportChart = (chart: ChartConfig, format: 'png' | 'svg' | 'pdf' | 'json') => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {charts.length === 0 && (
            <div className="col-span-full text-center py-12 text-gray-500">
              ...
              <p className="mb-4">Create your first visualization to get started</p>
              <button
                onClick={() => createNewChart()}
                className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700"
              >

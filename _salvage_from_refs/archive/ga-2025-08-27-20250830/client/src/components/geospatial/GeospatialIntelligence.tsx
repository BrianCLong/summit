...
interface GeospatialPoint {
  id: string;
  name: string;
  type: 'person' | 'event' | 'facility' | 'device' | 'vehicle' | 'poi' | 'incident' | 'surveillance';
  coordinates: {
    latitude: number;
    longitude: number;
    altitude?: number;
  ...
  timestamp: Date;
  confidence: number;
  source: 'gps' | 'cell_tower' | 'wifi' | 'ip_geolocation' | 'manual' | 'imagery' | 'osint' | 'witness';
  metadata: {
    elevation?: number;
    bearing?: number;
    speed?: number;
...
interface LocationIntelligence {
  coordinates: { latitude: number; longitude: number };
  address: string;
  type: 'residential' | 'commercial' | 'industrial' | 'government' | 'educational' | 'medical' | 'transport' | 'unknown';
  osintData: {
    businessInfo?: {
      name: string;
      type: string;
...
const GeospatialIntelligence: React.FC<GeospatialIntelligenceProps> = ({
  investigationId,
  initialCenter = { latitude: 40.7128, longitude: -74.0060 }, // NYC default
  onPointSelect = () => {},
  onTrackAnalysis = () => {},
  onGeofenceAlert = () => {},
  onSpatialAnalysis = () => {},
  ...
  const [activeView, setActiveView] = useState<'map' | 'tracks' | 'geofences' | 'analysis' | 'intelligence'>('map');
  ...
  const generateMockGeospatialData = () => {
    const pointTypes: GeospatialPoint['type'][] = ['person', 'event', 'facility', 'device', 'vehicle', 'poi', 'incident', 'surveillance'];
    const sources: GeospatialPoint['source'][] = ['gps', 'cell_tower', 'wifi', 'ip_geolocation', 'manual', 'imagery', 'osint', 'witness'];
        ...
        associations: Math.random() > 0.7 ? [
          {
            ...
            relationship: ['owner', 'participant', 'witness'][Math.floor(Math.random() * 3)] as any,
            confidence: Math.random() * 30 + 70
          }
        ] : [],
        tags: Math.random() > 0.5 ? ['surveillance', 'meeting', 'residence', 'commercial'][Math.floor(Math.random() * 4)] : [],
  ...
  const generateMockMovementTracks = () => {
    const mockTracks: MovementTrack[] = Array.from({ length: 15 }, (_, i) => {
      ...
      return {
        ...
        endTime,
        totalDistance,
        averageSpeed: totalDistance / ((endTime.getTime() - startTime.getTime()) / (1000 * 60 * 60)),
        ...
        analysis: {
          frequentLocations: [
            {
              coordinates: { latitude: mapCenter.latitude + Math.random() * 0.1, longitude: mapCenter.longitude + Math.random() * 0.1 },
  ...
  const createGeofence = (name: string, type: GeofenceZone['type'], coordinates: { latitude: number; longitude: number }, radius: number) => {
  ...
  const getLocationIntelligence = async (coordinates: { latitude: number; longitude: number }): Promise<LocationIntelligence> => {
    ...
    return {
      ...
      type: ['residential', 'commercial', 'industrial', 'government'][Math.floor(Math.random() * 4)] as any,
      osintData: {
      ...
      threatAssessment: {
        ...
        mitigations: ['Increase security presence', 'Install additional cameras', 'Restrict access']
      }
    };
  };

  const filteredPoints = geospatialPoints.filter(point => 
  ...
  const filteredTracks = movementTracks.filter(track =>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <div className="text-sm text-gray-500">
                      {filteredPoints.length} points • {filteredTracks.length} tracks • {geofenceZones.filter(g => g.isActive).length} geofences
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-lg border p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span>Geofences:</span>
                    <span className="font-medium">{geofenceZones.filter(g => g.isActive).length}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Analyses:</span>
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-3">
                <div>
                  ...
                  <div className="font-medium">
                    {Math.round((track.endTime.getTime() - track.startTime.getTime()) / (1000 * 60 * 60))}h
                  </div>
                </div>
                <div>
              ...
              <div className="flex flex-wrap gap-1">
                {track.trajectory.straightLine && (
                  <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded">Straight Line</span>
                )}
                {track.trajectory.circularPattern && (
                  <span className="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded">Circular</span>
                )}
                {track.trajectory.repeatingRoute && (
                  <span className="px-2 py-1 bg-green-100 text-green-700 text-xs rounded">Repeating Route</span>
                )}
                {track.trajectory.anomalousMovement && (
                  <span className="px-2 py-1 bg-red-100 text-red-700 text-xs rounded">⚠️ Anomalous</span>
                )}
              </div>
            ...
            <div key={zone.id} className="bg-white rounded-lg border p-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 text-xs rounded ${
                      zone.type === 'alert' ? 'bg-orange-100 text-orange-700' :
                      zone.type === 'exclusion' ? 'bg-red-100 text-red-700' :
                      zone.type === 'inclusion' ? 'bg-green-100 text-green-700' :
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-3">
                <div>
                  ...
                  <div className="font-medium text-xs">
                    {zone.geometry.center?.latitude.toFixed(4)}, {zone.geometry.center?.longitude.toFixed(4)}
                  </div>
                </div>
                <div>
              ...
              {zone.rules.dwellTimeLimit && (
                <div className="text-sm text-gray-600 mb-2">
                  <span>Dwell limit:</span> <span className="font-medium">{zone.rules.dwellTimeLimit} minutes</span>
                </div>
              )}
              
              {zone.alerts.length > 0 && (
                <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-md">
                  <span className="text-orange-800 font-medium">Recent Alerts ({zone.alerts.length})</span>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { type: 'hotspot', name: 'Hotspot Analysis', icon: '🔥', desc: 'Identify areas of high activity concentration' },
                { type: 'cluster', name: 'Cluster Analysis', icon: '🎯', desc: 'Group similar locations and patterns' },
                { type: 'pattern', name: 'Pattern Detection', icon: '🔍', desc: 'Discover recurring spatial patterns' },
                { type: 'anomaly', name: 'Anomaly Detection', icon: '⚠️', desc: 'Identify unusual spatial behavior' },
                { type: 'correlation', name: 'Spatial Correlation', icon: '🔗', desc: 'Find relationships between locations' },
                { type: 'prediction', name: 'Predictive Analysis', icon: '🔮', desc: 'Forecast future spatial patterns' }
              ].map(tool => (
                <button
                  key={tool.type}
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { name: 'OSINT Collection', icon: '🔍', desc: 'Automated open source intelligence gathering' },
                { name: 'Business Intelligence', icon: '🏢', desc: 'Commercial entity identification and analysis' },
                { name: 'Demographic Analysis', icon: '👥', desc: 'Population and socioeconomic data' },
                { name: 'Infrastructure Mapping', icon: '🗺️', desc: 'Critical infrastructure identification' },
                { name: 'Satellite Imagery', icon: '🛰️', desc: 'Recent satellite and aerial imagery' },
                { name: 'Street View Analysis', icon: '📸', desc: 'Ground-level imagery and features' },
                { name: 'Communication Analysis', icon: '📡', desc: 'Cell tower and network coverage' },
            ...
            <div className="space-y-4">
              <div className="p-4 border rounded-lg">
                ...
                <div className="text-sm text-gray-600">
                  <div><strong>Address:</strong> 123 Intelligence Ave, Analytics City, NY 10001</div>
                  <div><strong>Coordinates:</strong> {mapCenter.latitude.toFixed(6)}, {mapCenter.longitude.toFixed(6)}</div>
                  <div><strong>Type:</strong> Commercial Building</div>
                  <div><strong>Risk Level:</strong> <span className="text-orange-600 font-medium">Medium</span></div>
                </div>
              </div>
              ...
              <div className="p-4 border rounded-lg">
                ...
                <div className="text-sm text-gray-600">
                  <div><strong>Primary Tenant:</strong> TechCorp Analytics Inc.</div>
                  <div><strong>Business Type:</strong> Data Analytics Company</div>
                  <div><strong>Hours:</strong> Monday-Friday 9:00 AM - 6:00 PM</div>
                  <div><strong>Employee Count:</strong> ~150-200 (estimated)</div>
                  <div><strong>Public Rating:</strong> 4.2/5 (247 reviews)</div>
                </div>
              </div>
              ...
              <div className="p-4 border rounded-lg">
                ...
                <div className="text-sm text-gray-600">
                  <div><strong>Nearby Transport:</strong> Bus stops (0.2km), Subway station (0.4km)</div>
                  <div><strong>Cell Coverage:</strong> Verizon (Strong), AT&T (Moderate), T-Mobile (Weak)</div>
                  <div><strong>Internet Providers:</strong> Verizon FiOS, Spectrum, AT&T</div>
                  <div><strong>Security Features:</strong> CCTV cameras, keycard access, security desk</div>
                </div>
              </div>
              ...
              <div className="p-4 border rounded-lg">
                ...
                <div className="text-sm text-gray-600">
                  <div><strong>Risk Factors:</strong> High foot traffic, multiple entry points, adjacent to government building</div>
                  <div><strong>Mitigation:</strong> Enhanced screening, visitor management, perimeter monitoring</div>
                  <div><strong>Vulnerabilities:</strong> Public parking garage, limited sight lines</div>
                </div>
              </div>
            </div>
          </div>

...
interface IncidentCase {
  ...
  category: 'malware' | 'phishing' | 'data_breach' | 'insider_threat' | 'apt' | 'ddos' | 'fraud' | 'other';
  createdDate: Date;
  lastUpdated: Date;
  assignedTeam: string[];
...
const ThreatDetectionEngine: React.FC<ThreatDetectionEngineProps> = ({
  ...
  const [activeView, setActiveView] = useState<'dashboard' | 'alerts' | 'incidents' | 'signatures' | 'response'>('dashboard');
  ...
  const generateMockAlerts = () => {
    ...
    const statuses: ThreatAlert['status'][] = ['new', 'investigating', 'confirmed', 'false_positive', 'resolved'];
  ...
  const generateMockIncidents = () => {
    const categories: IncidentCase['category'][] = ['malware', 'phishing', 'data_breach', 'insider_threat', 'apt'];
    ...
    const mockIncidents: IncidentCase[] = Array.from({ length: 8 }, (_, i) => {
      ...
      return {
        ...
        assignedTeam: ['SOC Team', 'Incident Response', 'Forensics'][Math.floor(Math.random() * 3)],
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'investigating': return 'text-blue-700 bg-blue-100 border-blue-200';
      case 'confirmed': return 'text-red-700 bg-red-100 border-red-200';
      case 'false_positive': return 'text-gray-700 bg-gray-100 border-gray-200';
      case 'resolved': return 'text-green-700 bg-green-100 border-green-200';
      case 'open': return 'text-red-700 bg-red-100 border-red-200';
      case 'containment': return 'text-orange-700 bg-orange-100 border-orange-200';
      case 'recovery': return 'text-yellow-700 bg-yellow-100 border-yellow-200';
      ...
      {activeView === 'dashboard' && metrics && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-lg border p-4">
              <div className="text-2xl font-bold text-green-600">{metrics.detectionAccuracy.toFixed(1)}%</div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-24 h-2 bg-gray-200 rounded-full">
                        <div 
                          ...
                          style={{ width: `${(threat.count / metrics.topThreatTypes[0].count) * 100}%` }}
                        ></div>
                      </div>
            ...
            <div className="bg-white rounded-lg border p-4">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium">{metrics.meanTimeToDetection.toFixed(1)} min</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-600">Mean Time to Response:</span>
                  <span className="text-sm font-medium">{metrics.meanTimeToResponse.toFixed(1)} min</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-600">Average Response Time:</span>
                  <span className="text-sm font-medium">{metrics.averageResponseTime.toFixed(1)} min</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-600">False Positive Rate:</span>
                  <span className="text-sm font-medium">{(metrics.falsePositiveRate * 100).toFixed(1)}%</span>
                </div>
              </div>
            </div>
          </div>
            ...
            {filteredAlerts.map(alert => (
              ...
              >
                ...
                <p className="text-sm text-gray-600 mb-2">
                  ...
                  Affected: {alert.affectedEntities.join(', ')}
                </p>
          ...
          {incidents.map(incident => (
            ...
            >
              ...
              <div className="mt-3 text-sm">
                ...
                <span className="text-orange-600 ml-2">I:{incident.impactAssessment.integrity}</span>
                <span className="text-blue-600 ml-2">A:{incident.impactAssessment.availability}</span>
              </div>
            </div>
          ))}
            ...
            <div key={signature.id} className="bg-white rounded-lg border p-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                ...
                <div>
                  ...
                  <span className="ml-2 font-medium">{(signature.falsePositiveRate * 100).toFixed(1)}%</span>
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg border p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={playbook} className="flex items-center justify-between p-3 border rounded-lg">

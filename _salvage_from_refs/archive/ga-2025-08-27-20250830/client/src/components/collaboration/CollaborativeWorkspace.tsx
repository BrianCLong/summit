...
interface CollaborationEvent {
  id: string;
  type: 'user_joined' | 'user_left' | 'cursor_move' | 'selection_change' | 'edit' | 'comment' | 'annotation';
  userId: string;
  timestamp: number;
  data: any;
}
...
const CollaborativeWorkspace: React.FC<CollaborativeWorkspaceProps> = ({
  ...
  const handleAnnotationStart = useCallback((e: React.MouseEvent, type: LiveAnnotation['type']) => {
    if (!workspaceRef.current) return;
  ...
  const handleAnnotationComplete = useCallback((content?: string) => {
  ...
  const getUserColor = (userId: string) => {
    const colors = ['#1a73e8', '#e91e63', '#4caf50', '#ff9800', '#9c27b0', '#f44336', '#2196f3', '#00bcd4'];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Live Annotations */}
        {showAnnotations && annotations.filter(a => a.isVisible).map(annotation => (
          <div
            key={annotation.id}
            style={{
              position: 'absolute',
                ...
                >
                  ...
                  <div style={{
                    ...
                    backgroundColor: user.status === 'active' ? '#4caf50' : 
                                   user.status === 'idle' ? '#ffc107' : '#6c757d'
                  }} />
                </div>
              ))}
            </div>

...
const ThreatIntelligenceHub: React.FC<ThreatIntelligenceHubProps> = ({
  ...
  const [activeView, setActiveView] = useState<'indicators' | 'campaigns' | 'actors' | 'feeds'>('indicators');
  ...
  const handleIndicatorClick = useCallback((indicator: ThreatIndicator) => {
  ...
  return (
    <div className={`threat-intelligence-hub ${className}`} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', fontSize: '12px', color: '#6c757d' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden' }}>
        {activeView === 'indicators' && (
          <div style={{ height: '100%', display: 'grid', gridTemplateColumns: selectedIndicator ? '1fr 1fr' : '1fr', gap: '16px' }}>
            {/* Indicators List */}
                ...
                {filteredIndicators.map(indicator => (
                  <div
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      backgroundColor: selectedIndicator?.id === indicator.id ? '#e3f2fd' : 'transparent',
                ...
                <div style={{ padding: '16px' }}>
                  <div style={{ marginBottom: '24px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                      <span style={{ fontSize: '20px' }}>{getTypeIcon(selectedIndicator.type)}</span>
                      <code style={{ 
                        fontSize: '14px', 
                        backgroundColor: '#f8f9fa', 
                        padding: '4px 8px', 
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                      ...
                      <div>
                        <strong>Status:</strong> {selectedIndicator.isActive ? '🟢 Active' : '🔴 Inactive'}
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedIndicator.context && (
                    <div style={{ marginBottom: '24px' }}>
                      <h5 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '8px' }}>Context</h5>
                      <div style={{ fontSize: '13px', lineHeight: '1.4' }}>
                        {selectedIndicator.context.malwareFamily && (
                          <div><strong>Malware Family:</strong> {selectedIndicator.context.malwareFamily}</div>
                        )}
                        {selectedIndicator.context.campaign && (
                          <div><strong>Campaign:</strong> {selectedIndicator.context.campaign}</div>
                        )}
                        {selectedIndicator.context.actor && (
                          <div><strong>Actor:</strong> {selectedIndicator.context.actor}</div>
                        )}
                        {selectedIndicator.context.ttps && (
                          <div><strong>TTPs:</strong> {selectedIndicator.context.ttps.join(', ')}</div>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ marginBottom: '24px' }}>
                    <h5 style={{ fontSize: '14px', fontWeight: '600', marginBottom: '8px' }}>Tags</h5>
                  ...
                  <div style={{ fontSize: '13px', color: '#666' }}>
                    <div><strong>Source:</strong> {selectedIndicator.source}</div>
                    <div><strong>First Seen:</strong> {selectedIndicator.firstSeen.toLocaleString()}</div>
                    <div><strong>Last Seen:</strong> {selectedIndicator.lastSeen.toLocaleString()}</div>
                    <div><strong>Verified:</strong> {selectedIndicator.verified ? '✅ Yes' : '❌ No'}</div>
                  </div>
                </div>
              </div>
            )}
          ...
          <div style={{ padding: '16px', border: '1px solid var(--hairline)', borderRadius: '8px' }}>
            ...
            {mockCampaigns.map(campaign => (
              ...
              >
                ...
                <div style={{ fontSize: '12px', color: '#666' }}>
                  <div><strong>Actors:</strong> {campaign.actors.join(', ')}</div>
                  <div><strong>Status:</strong> {campaign.status}</div>
                  <div><strong>Last Activity:</strong> {campaign.lastActivity.toLocaleDateString()}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ padding: '16px', border: '1px solid var(--hairline)', borderRadius: '8px' }}>
            ...
            {mockActors.map(actor => (
              ...
              >
                ...
                <div style={{ fontSize: '12px', color: '#666' }}>
                  <div><strong>Sophistication:</strong> {actor.sophistication}</div>
                  <div><strong>Geography:</strong> {actor.geography.join(', ')}</div>
                  <div><strong>Status:</strong> {actor.isActive ? '🟢 Active' : '🔴 Inactive'}</div>
                  <div><strong>Last Activity:</strong> {actor.lastActivity.toLocaleDateString()}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ padding: '16px', border: '1px solid var(--hairline)', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'grid', gap: '16px' }}>
              {[
                ...
                { name: 'Recorded Future', status: 'active', lastSync: new Date(), indicators: 892 },
                { name: 'MISP', status: 'warning', lastSync: new Date(Date.now() - 3600000), indicators: 534 }
              ].map(feed => (
                <div
                  key={feed.name}
                  style={{

...
test("chunking invariance (fuzz)", async () => {
  ...
  await fc.assert(
    fc.asyncProperty(
      fc.uniqueArray(fc.integer({min:0, max: target.length}), {minLength:0, maxLength:Math.min(8, target.length)}),
      async (cuts) => {
        const chunks = chunkByIndices(target, cuts);
        const script = [...chunks.map(v => ({ type: "token", value: v } as const)), { type: "done" } as const];

...
describe('EnhancedAIAssistant', () => {
  ...
  it('displays assistant name and status', () => {
    ...
    expect(screen.getByRole('status', { name: /assistant-status/i })).toHaveTextContent(/Intelligence Analysis Assistant • Online/);
  });
  ...
  it('aborts cleanly on unmount (no state updates after unmount)', async () => {
    ...
    const { unmount, rerender } = renderWithTheme(
      <EnhancedAIAssistant {...defaultProps} transport={transport} typingDelayMs={1} debounceMs={1} />
    );

    // Trigger a send by submitting the form programmatically
    rerender(<EnhancedAIAssistant {...defaultProps} transport={transport} typingDelayMs={1} debounceMs={1} />);
    unmount();
  ...
  it('handles multiline input correctly', async () => {
    ...
    renderWithTheme(
      <EnhancedAIAssistant {...defaultProps} transport={transport} typingDelayMs={0} debounceMs={0} />
    );
  ...
  it('displays "cannot confirm" message when RAG is strict and no cites are provided', async () => {
    ...
    const log = await screen.findByTestId("message-log");
    await expectTextAcrossElements(log, /I cannot confirm this answer based on the provided context./i);

...
You know it worked when `make smoke` reports the UI at `http://localhost:3000` and the gateway at `http://localhost:8080/health` as healthy.
...
- **GraphQL playground unreachable**: Confirm the gateway is running on port 4000 or 8080 (per Compose) and that `CORS_ORIGIN` includes the UI origin.

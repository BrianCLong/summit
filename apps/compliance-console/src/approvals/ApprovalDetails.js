...
exports.ApprovalDetails = ApprovalDetails;
const react_1 = require("react");
const abac_1 = require("./abac");
/**
 * Detailed view for a single approval request with ABAC-enforced decision actions.
 */
function ApprovalDetails({ request, user, onDecision }) {
    const [rationale, setRationale] = (0, react_1.useState)('Approved with ABAC permit');
    const existingApprovalByUser = (0, react_1.useMemo)(() => request.approvals.find((a) => a.actor === user.id), [request.approvals, user.id]);
    function handleDecision(decision) {
        const evaluation = (0, abac_1.abacDecision)(user, request, decision);
    ...
    return (<div style={{ border: '1px solid #e5e7eb', padding: '1rem', borderRadius: 8 }}>
      ...
      <textarea aria-label="rationale" value={rationale} onChange={(e) => setRationale(e.target.value)} style={{ width: '100%', minHeight: 80, padding: '0.5rem', borderRadius: 6 }}/>

      <div style={{ marginTop: '0.75rem', display: 'flex', gap: '0.5rem' }}>
        <button onClick={() => handleDecision('approve')} disabled={Boolean(existingApprovalByUser)} style={{
        ...
        <button onClick={() => handleDecision('deny')} disabled={Boolean(existingApprovalByUser)} style={{
            padding: '0.5rem 0.85rem',
            background: '#ef4444',
            color: '#fff',
            border: 'none',
      ...
      {existingApprovalByUser && (<p style={{ marginTop: '0.75rem', color: '#10b981' }}>

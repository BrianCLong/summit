...
const ApprovalsPage_1 = require("./approvals/ApprovalsPage");
...
function App() {
    ...
    const [showStepUp, setShowStepUp] = (0, react_1.useState)(false);
    const [stepUpCode, setStepUpCode] = (0, react_1.useState)('');
    const [stepUpError, setStepUpError] = (0, react_1.useState)(null);
    const [pendingExportId, setPendingExportId] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        const fetchPacks = async () => {
            try {
                const res = await axios_1.default.get('/api/disclosure-packs');
    ...
    const selectedPack = (0, react_1.useMemo)(() => packs.find((pack) => pack.id === selectedId) ?? null, [packs, selectedId]);
    async function downloadSelected(idOverride) {
        const id = idOverride ?? selectedId;
        if (!id)
            return;
        try {
            const res = await axios_1.default.get(`/api/disclosure-packs/${id}/export`, {
                responseType: 'blob',
    ...
    return (<div style={{ fontFamily: 'Inter, sans-serif', padding: '1.5rem' }}>
      ...
      {loading ? (<p>Loading disclosure packs…</p>) : error ? (<p style={{ color: 'red' }}>{error}</p>) : (<>
          <section style={{ marginBottom: '1rem' }}>
            <h2 style={{ marginBottom: '0.5rem' }}>Disclosure packs</h2>
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
              {packs.map((pack) => (<button key={pack.id} onClick={() => setSelectedId(pack.id)} style={{
                    padding: '0.75rem 1rem',
                    borderRadius: 8,
                    border: pack.id === selectedId
                        ? '2px solid #2563eb'
                        : '1px solid #ccc',
          ...
          {selectedPack && (<section style={{ marginTop: '1rem' }}>
              ...
              <div style={{
                ...
                <button onClick={() => downloadSelected(selectedPack.id)} style={{
      ...
      {showStepUp && (<div style={{
          ...
          <div style={{
            ...
            <div style={{ margin: '0.75rem 0' }}>
              <input type="text" placeholder="Verification code" value={stepUpCode} onChange={(e) => setStepUpCode(e.target.value)} style={{ width: '100%', padding: '0.4rem' }}/>
            </div>
            ...
            <div style={{
              ...
              <button onClick={async () => {
                try {
                    setStepUpError(null);
                    await axios_1.default.post('/api/auth/step-up', { code: stepUpCode }, { withCredentials: true });
                    setShowStepUp(false);

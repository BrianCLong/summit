...
function App() {
    ...
    (0, react_1.useEffect)(() => {
        ...
        (0, api_1.fetchSnapshot)()
            .then((data) => setSnapshot(data))
            .catch((err) => setError(err.message))
            .finally(() => setLoading(false));
    }, [enabled]);
    const incidentCount = (0, react_1.useMemo)(() => (snapshot?.incidents.gaGateFailures.length ?? 0) +
    ...
    if (error) {
        return (<div style={{ padding: 24, color: '#b42318' }}>
        <strong>Access blocked:</strong> {error}
      </div>);
    }
    if (!snapshot)
        return null;
    return (<div style={{ padding: 24, fontFamily: 'Inter, system-ui, sans-serif', background: '#f8fafc', minHeight: '100vh' }}>
        ...
        <div style={gridStyle}>
          <div style={cardStyle}>
            ...
            <ul style={{ paddingLeft: 16, margin: 0, color: '#334155' }}>
              {snapshot.gaGate.details.slice(0, 4).map((detail) => (<li key={`${detail.component}-${detail.message}`}>
      ...
      <Section title="Tenant & Blast Radius">
        <div style={cardStyle}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Tenant', 'Rate limit', 'Ingestion cap', 'Kill switch', 'Active'].map((header) => (<th key={header} style={{ textAlign: 'left', paddingBottom: 8 }}>
                    {header}
                  </th>))}
              </tr>
            </thead>
            <tbody>
              {snapshot.tenants.map((tenant) => (<tr key={tenant.tenantId}>
        ...
        <div style={gridStyle}>
          ...
          <div style={cardStyle}>
            ...
            <ul style={{ paddingLeft: 16, margin: '8px 0' }}>
              {snapshot.llm.tenants.map((tenant) => (<li key={tenant.tenantId} style={{ marginBottom: 4 }}>

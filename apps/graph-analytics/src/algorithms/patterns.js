...
exports.detectStarPatterns = detectStarPatterns;
exports.detectBipartiteFanPatterns = detectBipartiteFanPatterns;
exports.detectRepeatedInteractions = detectRepeatedInteractions;
exports.runPatternMiner = runPatternMiner;
exports.detectTriangles = detectTriangles;
 ...
 */
function detectStarPatterns(graph, params) {
    logger_1.logger.debug('Detecting star patterns', params);
    const patterns = [];
    ...
    logger_1.logger.debug(`Found ${patterns.length} star patterns`);
    return patterns;
}
/**
 * Detect bipartite fan-in/fan-out patterns
 */
function detectBipartiteFanPatterns(graph, params) {
    logger_1.logger.debug('Detecting bipartite fan patterns', params);
    const patterns = [];
    ...
    for (const node of graph.nodes) {
        ...
        if (incoming.sources.size >= minSources &&
    ...
    logger_1.logger.debug(`Found ${patterns.length} bipartite fan patterns`);
    return patterns;
}
/**
 * Detect repeated interaction patterns
 */
function detectRepeatedInteractions(graph, params) {
    logger_1.logger.debug('Detecting repeated interaction patterns', params);
    const patterns = [];
    ...
    for (const edge of graph.edges) {
        ...
        if (edge.properties?.timestamp) {
            const timestamp = typeof edge.properties.timestamp === 'number'
    ...
    for (const [pairKey, info] of pairInteractions.entries()) {
        ...
        if (interactionCount >= minInteractions &&
            ...
            if (params.timeWindowSeconds && info.timestamps.length > 0) {
                ...
                const timeSpan = (sortedTimestamps[sortedTimestamps.length - 1] -
                    sortedTimestamps[0]) /
                    1000;
    ...
    logger_1.logger.debug(`Found ${patterns.length} repeated interaction patterns`);
    return patterns;
}
/**
 * Run pattern miner with multiple pattern types
 */
function runPatternMiner(graph, params) {
    logger_1.logger.info('Running pattern miner', { graph: graph.nodes.length });
    const allPatterns = [];
    ...
    logger_1.logger.info(`Pattern mining completed, found ${allPatterns.length} patterns`);
    return {
 ...
 */
function detectTriangles(graph) {
    const patterns = [];

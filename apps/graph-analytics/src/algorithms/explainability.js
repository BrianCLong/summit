...
exports.explainPathAnalysis = explainPathAnalysis;
exports.explainCommunityAnalysis = explainCommunityAnalysis;
exports.explainCentralityAnalysis = explainCentralityAnalysis;
exports.explainPatternAnalysis = explainPatternAnalysis;
exports.describeCentrality = describeCentrality;
exports.generateComprehensiveSummary = generateComprehensiveSummary;
/**
 ...
 */
function explainPathAnalysis(source, target, paths, stats) {
    if (paths.length === 0) {
        return `No paths found between ${source} and ${target}. ${stats.policyFilteredNodes > 0 || stats.policyFilteredEdges > 0
    ...
    const parts = [];
    parts.push(`Found ${paths.length} path${paths.length > 1 ? 's' : ''} from ${source} to ${target}.`);
    parts.push(`Shortest path length is ${stats.minLength} hop${stats.minLength !== 1 ? 's' : ''}.`);
    if (paths.length > 1) {
        parts.push(`Longest returned path is ${stats.maxLength} hop${stats.maxLength !== 1 ? 's' : ''}.`);
    ...
    if (stats.policyFilteredNodes > 0 || stats.policyFilteredEdges > 0) {
        parts.push(`Policy filters excluded ${stats.policyFilteredNodes} node${stats.policyFilteredNodes !== 1 ? 's' : ''} and ${stats.policyFilteredEdges} edge${stats.policyFilteredEdges !== 1 ? 's' : ''} from consideration.`);
    }
    ...
    if (paths.length > 0 && paths[0].relationships.length > 0) {
        ...
        if (relationshipTypes.size > 1) {
            parts.push(`Paths traverse ${relationshipTypes.size} different relationship types: ${Array.from(relationshipTypes).slice(0, 5).join(', ')}${relationshipTypes.size > 5 ? ', ...' : ''}.`);
        }
    }
    return parts.join(' ');
}
 ...
 */
function explainCommunityAnalysis(communities, algorithm) {
    const parts = [];
    parts.push(`Detected ${communities.numCommunities} communit${communities.numCommunities !== 1 ? 'ies' : 'y'} using ${algorithm} algorithm.`);
    // Get community sizes
    const sizes = Object.values(communities.sizes).sort((a, b) => b - a);
    if (sizes.length > 0) {
        parts.push(`Largest community contains ${sizes[0]} node${sizes[0] !== 1 ? 's' : ''}.`);
        parts.push(`Smallest community contains ${sizes[sizes.length - 1]} node${sizes[sizes.length - 1] !== 1 ? 's' : ''}.`);
    ...
    if (communities.modularityScore !== undefined) {
        ...
        parts.push(`Modularity score of ${modularityPercent}% indicates ${communities.modularityScore > 0.3
            ? 'strong'
    ...
    if (densities.length > 0) {
        ...
        parts.push(`Communities have an average internal density of ${(avgDensity * 100).toFixed(1)}%, suggesting ${avgDensity > 0.3
            ? 'tightly-knit'
            : avgDensity > 0.1
 ...
 */
function explainCentralityAnalysis(centrality, algorithm) {
    const parts = [];
    ...
    if (degreeValues.length > 0) {
        parts.push(`Average degree is ${centrality.stats.avgDegree.toFixed(1)}, with maximum degree of ${centrality.stats.maxDegree}.`);
        const topDegreeNodes = centrality.summaries.topByDegree.slice(0, 3);
        if (topDegreeNodes.length > 0) {
            parts.push(`Top nodes by degree centrality are ${topDegreeNodes.join(', ')}, indicating they are the most connected hubs.`);
        }
    }
    ...
    if (betweennessValues.length > 0) {
        ...
        if (topBetweennessNodes.length > 0) {
            ...
            parts.push(`Top ${topBetweennessNodes.length} nodes by betweenness centrality (${topBetweennessNodes.join(', ')}) account for ${topPercent.toFixed(0)}% of shortest paths, highlighting key broker positions.`);
        }
    }
    ...
    if (centrality.scores.eigenvector) {
        ...
        if (topEigenvectorNodes.length > 0) {
            parts.push(`Eigenvector centrality identifies ${topEigenvectorNodes.join(', ')} as highly influential, being connected to other well-connected nodes.`);
        }
    }
    return parts.join(' ');
}
 ...
 */
function explainPatternAnalysis(patterns) {
    if (patterns.length === 0) {
        return 'No significant patterns detected in the graph.';
    }
    const parts = [];
    parts.push(`Detected ${patterns.length} structural pattern${patterns.length !== 1 ? 's' : ''} in the graph.`);
    // Count by pattern type
    const byType = patterns.reduce((acc, pattern) => {
        acc[pattern.patternType] = (acc[pattern.patternType] || 0) + 1;
        return acc;
    }, {});
    const typeDescriptions = Object.entries(byType).map(([type, count]) => `${count} ${type.toLowerCase().replace(/_/g, ' ')}${count > 1 ? 's' : ''}`);
    ...
    const insights = [];
    if (byType.STAR) {
        insights.push(`Star patterns suggest centralized control or hub-and-spoke communication structures.`);
    }
    if (byType.BIPARTITE_FAN) {
        insights.push(`Bipartite fan patterns may indicate structuring activities, layered money flows, or intermediary operations.`);
    }
    if (byType.REPEATED_INTERACTION) {
        insights.push(`Repeated interaction patterns reveal persistent relationships or coordinated behavior over time.`);
    }
 ...
 */
function describeCentrality(result, maxItems = 5) {
    const parts = [];
    ...
    parts.push(`Top ${maxItems} nodes by betweenness centrality are [${topBetweenness.join(', ')}], indicating they act as key brokers in this subgraph.`);
    if (result.summaries.topByEigenvector) {
        const topEigenvector = result.summaries.topByEigenvector.slice(0, maxItems);
        parts.push(`Top ${maxItems} by eigenvector centrality are [${topEigenvector.join(', ')}], showing high influence through connections.`);
    }
    return parts.join(' ');
}
/**

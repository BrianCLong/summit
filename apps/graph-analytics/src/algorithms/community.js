 ...
 */
function buildAdjacencyStructures(graph) {
    const adj = {};
    const weightedAdj = {};
    ...
    for (const edge of graph.edges) {
        ...
        weightedAdj[edge.fromId]?.set(edge.toId, (weightedAdj[edge.fromId]?.get(edge.toId) || 0) + weight);
        weightedAdj[edge.toId]?.set(edge.fromId, (weightedAdj[edge.toId]?.get(edge.fromId) || 0) + weight);
...
function labelPropagation(graph, maxIterations = 100) {
    logger_1.logger.info('Running label propagation algorithm', {
    ...
    logger_1.logger.info('Label propagation completed', {
...
function louvainCommunityDetection(graph) {
    logger_1.logger.info('Running Louvain community detection', {
        nodes: graph.nodes.length,
    });
    ...
    while (improved && iteration < maxIterations) {
        ...
        for (const nodeId of nodeIds) {
            ...
            for (const targetCommunity of neighborCommunities) {
                ...
                const gain = calculateModularityGain(nodeId, currentCommunity, targetCommunity, nodeToCommunity, weightedAdj, degrees);
                if (gain > bestGain) {
                    bestGain = gain;
    ...
    logger_1.logger.info('Louvain completed', {
        iterations: iteration,
        communities: communityMap.size,
 ...
 */
function calculateModularityGain(nodeId, fromCommunity, toCommunity, nodeToCommunity, weightedAdj, degrees) {
 ...
 */
function detectCommunities(graph, algorithm = 'louvain') {

...
exports.NetworkVisualizationService = void 0;
...
class NetworkVisualizationService {
    ...
    async generateVisualization(query, parameters, config) {
        const session = this.neo4jDriver.session();
        const startTime = Date.now();
        try {
            logger_1.logger.info('Generating network visualization', { query, config });
            ...
            result.records.forEach((record) => {
                record.keys.forEach((key) => {
                    ...
                    if (value?.constructor?.name === 'Node') {
                        ...
                        const pathNodes = value.segments.flatMap((segment) => [
                            segment.start,
                            segment.end,
                        ]);
                        const pathRels = value.segments.map((segment) => segment.relationship);
                        pathNodes.forEach((nodeValue) => {
                            const node = this.extractNode(nodeValue);
                            if (this.shouldIncludeNode(node, config.filters)) {
                                nodes.set(node.id, node);
                            }
                        });
                        pathRels.forEach((relValue) => {
            ...
            visibleEdges = edgeArray.filter((edge) => visibleNodeIds.has(edge.source) && visibleNodeIds.has(edge.target));
            // Apply edge limit
            ...
            logger_1.logger.info('Network visualization generated', {
                totalNodes: nodeArray.length,
                totalEdges: edgeArray.length,
                visibleNodes: styledNodes.length,
                visibleEdges: styledEdges.length,
            ...
            logger_1.logger.error('Error generating network visualization:', error);
            throw error;
        }
        finally {
            await session.close();
    ...
    async generateSubnetVisualization(centerNodeId, depth, config = {}) {
        const defaultConfig = {
    ...
    async generatePathVisualization(sourceId, targetId, pathType = 'shortest', config = {}) {
        const defaultConfig = {
    ...
    async generateTimelineVisualization(timeRange, config = {}) {
        const defaultConfig = {
    ...
    shouldIncludeNode(node, filters) {
    ...
    shouldIncludeEdge(edge, filters) {
    ...
    evaluatePropertyFilter(value, operator, filterValue) {
        switch (operator) {
            case 'eq':
    ...
    simplifyNetwork(nodes, edges, maxNodes) {
    ...
    calculateNodePosition(node, allNodes, layout) {
    ...
    exportToCytoscape(visualization) {
        const cytoscapeData = {
            elements: {
    ...
    exportToGephi(visualization) {
        ...
        const edges = visualization.data.edges
            .map((edge, index) => `    <edge id="${index}" source="${edge.source}" target="${edge.target}" weight="${edge.weight || 1}"/>`)
            .join('\n');
...
${edges}
    ...
    exportToGraphML(visualization) {
      ...
      <data key="name">${node.properties.name || node.id}</data>
            ...
            .join('\n');
        const edges = visualization.data.edges
            .map((edge, index) => `    <edge id="e${index}" source="${edge.source}" target="${edge.target}">
...
exports.NetworkVisualizationService = NetworkVisualizationService;

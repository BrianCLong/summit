...
exports.ClaimAssessmentTask = ClaimAssessmentTask;
const react_1 = __importDefault(require("react"));
const lucide_react_1 = require("lucide-react");
const labelingStore_1 = require("../../store/labelingStore");
const cn_1 = require("../../utils/cn");
function ClaimAssessmentTask({ sample, onSubmit, instructions, }) {
    const [verdict, setVerdict] = react_1.default.useState(null);
    ...
    return (<div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6 rounded-lg border bg-card p-6">
        ...
        {claim ? (<>
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              ...
              {claim.confidence && (<span>Original Confidence: {(claim.confidence * 100).toFixed(0)}%</span>)}
            </div>
          </>) : (<p className="text-muted-foreground">No claim data available</p>)}
      </div>

      {/* Supporting Evidence */}
      {claim?.supportingEvidence && claim.supportingEvidence.length > 0 && (<div className="mb-6 rounded-lg border bg-card p-6">
          <h3 className="text-lg font-semibold mb-4">Supporting Evidence</h3>
          <ul className="space-y-3">
            {claim.supportingEvidence.map((evidence, index) => (<li key={index} className="flex items-start gap-2 p-3 rounded-lg bg-muted/50">
                <lucide_react_1.ExternalLink className="h-4 w-4 mt-0.5 text-muted-foreground flex-shrink-0"/>
      ...
      <div className="mb-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {Object.entries(verdictConfig).map(([key, config]) => {
            ...
            return (<button key={key} onClick={() => setVerdict(key)} className={(0, cn_1.cn)('flex flex-col items-center gap-2 p-4 rounded-lg border transition-all text-left', verdict === key
                    ? config.color === 'green'
                  ...
                  <Icon className={(0, cn_1.cn)('h-8 w-8', config.color === 'green'
                    ? 'text-green-600'
                    : config.color === 'red'
                        ? 'text-red-600'
                        : 'text-yellow-600')}/>
      ...
      <div className="mb-6">
        ...
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Explain your reasoning for this assessment..." className="w-full rounded-lg border bg-background px-3 py-2 text-sm min-h-[100px]"/>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleSubmit} disabled={!verdict} className="px-6 py-2 rounded-lg bg-primary text-primary-foreground font-medium hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed">
          Submit Assessment
        </button>
      </div>
    </div>);

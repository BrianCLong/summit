...
exports.SafetyDecisionTask = SafetyDecisionTask;
...
function SafetyDecisionTask({ sample, onSubmit, instructions, }) {
    const [selectedCategories, setSelectedCategories] = react_1.default.useState([]);
    const [severity, setSeverity] = react_1.default.useState(null);
    const [confidenceValue, setConfidenceValue] = react_1.default.useState(0.8);
    const { notes, setNotes, getTimeSpent } = (0, labelingStore_1.useLabelingStore)();
    ...
    const handleSubmit = () => {
        if (selectedCategories.length === 0)
            return;
        if (selectedCategories[0] !== 'safe' && !severity)
            return;
        const labels = [
            {
    ...
    return (<div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6 p-4 rounded-lg bg-amber-50 border border-amber-200">
        <div className="flex items-center gap-2 mb-1">
          <lucide_react_1.Shield className="h-4 w-4 text-amber-800"/>
      ...
      <div className="mb-6">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {safetyCategories.map((category) => (<button key={category.value} onClick={() => toggleCategory(category.value)} className={(0, cn_1.cn)('flex items-start gap-3 p-3 rounded-lg border text-left transition-all', selectedCategories.includes(category.value)
                ? category.value === 'safe'
                    ? 'bg-green-100 border-green-500'
                    : 'bg-red-100 border-red-500'
                : 'bg-card hover:bg-muted')}>
              <div className={(0, cn_1.cn)('flex h-5 w-5 items-center justify-center rounded border-2 flex-shrink-0', selectedCategories.includes(category.value)
                ? category.value === 'safe'
                    ? 'border-green-500 bg-green-500'
                    : 'border-red-500 bg-red-500'
                : 'border-muted-foreground')}>
                {selectedCategories.includes(category.value) && (<svg className="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {!isSafe && selectedCategories.length > 0 && (<div className="mb-6">
          <h3 className="text-lg font-semibold mb-4">Severity Level</h3>
          <div className="flex gap-4">
            {['low', 'medium', 'high'].map((level) => (<button key={level} onClick={() => setSeverity(level)} className={(0, cn_1.cn)('flex-1 py-3 rounded-lg border font-medium transition-all', severity === level
                    ? level === 'low'
      ...
      <div className="mb-6">
        ...
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Explain your safety assessment..." className="w-full rounded-lg border bg-background px-3 py-2 text-sm min-h-[100px]"/>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleSubmit} disabled={selectedCategories.length === 0 ||
            (!isSafe && !severity) ||
            (!isSafe && !notes.trim())} className="px-6 py-2 rounded-lg bg-primary text-primary-foreground font-medium hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed">
          Submit Assessment
        </button>
      </div>
    </div>);

...
test.describe("Accessibility gates", () => {
  ...
  test("axe-core finds no critical violations and captures artifacts", async ({ page }) => {
    ...
    fs.writeFileSync(
      REPORT_PATH,
      JSON.stringify({ ...results, violations: sanitize(results.violations) }, null, 2)
    );
    ...
    const snippet = getHeatmapSnippet({
      ...results,
      violations: sanitize(results.violations),
    } as any);
  ...
  test("focus order walk does not get trapped", async ({ page }) => {
    ...
    ensureArtifacts();
    fs.writeFileSync(
      path.join(ARTIFACT_DIR, "focus-order.json"),
      JSON.stringify(focusMap, null, 2)
    );

...
class FakeKmsClient {
  ...
  constructor(
    private readonly signature: Uint8Array = new Uint8Array(
      Buffer.from('api-signature'),
    ),
  ) {}
...
describe('GET /receipts/:id', () => {
  ...
  it('returns a signed payload and signature', async () => {
    ...
    expect(body.signature.value).toBe(
      Buffer.from('api-signature').toString('base64'),
    );

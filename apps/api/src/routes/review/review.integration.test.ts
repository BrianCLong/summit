...
describe('review queue', () => {
  it('filters by type and status and paginates deterministically', async () => {
    ...
    const firstPage = await request(app).get('/review/queue').query({ type: 'entity', status: 'pending', sort: 'createdAt:asc', limit: 1 });
    expect(firstPage.status).toBe(200);
    expect(firstPage.body.items).toHaveLength(1);
    expect(firstPage.body.items[0].id).toBe('rev-1');
    expect(firstPage.body.nextCursor).toBe('1');

    const secondPage = await request(app).get('/review/queue').query({ type: 'entity', status: 'pending', cursor: firstPage.body.nextCursor });

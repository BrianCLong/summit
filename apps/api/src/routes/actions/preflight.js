...
exports.createPreflightRouter = createPreflightRouter;
 ...
 */
function extractPolicyId(record) {
    // Check for policy_id in the raw decision metadata
    const rawDecision = record.rawDecision;
...
function toResponsePayload(record) {
    return {
...
function mergeRedactions(decision) {
...
function createPreflightRouter({ decisionStore, policyService, rbacManager }) {
    const router = (0, express_1.Router)();
    router.post('/preflight', (0, security_js_1.requirePermission)(rbacManager, 'actions:preflight', 'evaluate'), async (req, res) => {

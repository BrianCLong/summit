...
describe("EpicService", () => {
  ...
  describe("Task Updates", () => {
    ...
    it("should throw error for non-existent epic", () => {
      expect(() => service.updateTask("non-existent", "task-1-1", { status: "completed" })).toThrow(
        "Epic non-existent not found"
      );
    });

    it("should throw error for non-existent task", () => {
      expect(() => service.updateTask("epic-1", "non-existent", { status: "completed" })).toThrow(
        "Task non-existent not found for epic epic-1"
      );
    });

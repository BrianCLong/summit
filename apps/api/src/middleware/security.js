...
exports.privilegedRateLimiter = exports.apiRateLimiter = void 0;
exports.requireAuth = requireAuth;
exports.requireTenantIsolation = requireTenantIsolation;
 ...
 */
function requireAuth(rbacManager) {
    return (req, res, next) => {
        const authHeader = req.headers.authorization;
        const apiKey = req.headers['x-api-key'];
        const tenantId = req.headers['x-tenant-id'];
        const actorTenantId = req.headers['x-actor-tenant-id'] ?? tenantId;
        ...
        const rolesHeader = req.headers['x-roles']?.split(',').map((role) => role.trim()).filter(Boolean);
        ...
        if (authHeader && authHeader.startsWith('Bearer ')) {
            ...
            req.user = {
                sub: 'apikey_' + Buffer.from(apiKey).toString('base64').substring(0, 8),
                tenantId: actorTenantId,
                roles: resolvedRoles,
                scopes: ['read', 'write'],
            };
...
function requirePermission(rbacManager, resource, action) {
    return (req, res, next) => {
        if (!req.user) {
 ...
 */
exports.privilegedRateLimiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutes

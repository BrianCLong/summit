...
describe('dual control policy + execution', () => {
  it('surfaces dual-control obligations for sensitive actions', async () => {
    ...
    const resource = await attributeService.getResourceAttributes(
      'dataset-gamma',
    );
    ...
    const obligation = decision.obligations.find(
      (o) => o.type === 'dual_control',
    );
    expect(obligation).toBeDefined();
    expect(obligation?.approvals_required).toBe(2);
    expect(obligation?.approver_roles).toEqual(

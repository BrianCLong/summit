...
class WebhookRepository {
    ...
    async createSubscription(input) {
        const result = await this.pool.query(`INSERT INTO webhook_subscriptions (
    ...
    async findSubscriptionsForEvent(tenantId, eventType) {
        const result = await this.pool.query(`SELECT * FROM webhook_subscriptions
       WHERE tenant_id = $1
       AND is_active = TRUE
       AND event_types @> ARRAY[$2]::text[]`, [tenantId, eventType]);
        return result.rows.map(mapSubscription);
    }
    async createDelivery(subscriptionId, eventType, payload, idempotencyKey) {
        const result = await this.pool.query(`INSERT INTO webhook_deliveries (
      ...
      RETURNING *`, [(0, crypto_1.randomUUID)(), subscriptionId, eventType, payload, idempotencyKey, null]);
    ...
    async getDueDeliveries(limit) {
        const result = await this.pool.query(`SELECT * FROM webhook_deliveries
    ...
    async markDeliveryStatus(id, status, attemptCount, error, nextAttemptAt) {
        await this.pool.query(`UPDATE webhook_deliveries
       SET status = $2,
           attempt_count = $3,
           error = $4,
           next_attempt_at = $5,
       ...
       WHERE id = $1`, [id, status, attemptCount, error ?? null, nextAttemptAt ?? null]);
    }
    async recordAttempt(attempt) {
        const result = await this.pool.query(`INSERT INTO webhook_delivery_attempts (
    ...
    async listAttempts(deliveryId) {
        const result = await this.pool.query('SELECT * FROM webhook_delivery_attempts WHERE delivery_id = $1 ORDER BY created_at', [deliveryId]);
        return result.rows.map(mapAttempt);
    }
    async overrideNextAttempt(id, at) {
        await this.pool.query('UPDATE webhook_deliveries SET next_attempt_at = $2 WHERE id = $1', [id, at]);
    }

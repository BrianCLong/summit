...
describe('reporting materialized views', () => {
    it('creates and refreshes reporting materialized views', async () => {
        ...
        const truncateToDay = (value) => new Date(Date.UTC(value.getUTCFullYear(), value.getUTCMonth(), value.getUTCDate()));
        db.public.registerFunction({
            name: 'date_trunc',
            args: [pg_mem_1.DataType.text, pg_mem_1.DataType.timestamp],
            returns: pg_mem_1.DataType.timestamp,
            implementation: (part, value) => part === 'day' ? truncateToDay(value) : value,
        });
        db.public.registerFunction({
            name: 'date_trunc',
            args: [pg_mem_1.DataType.text, pg_mem_1.DataType.timestamptz],
            returns: pg_mem_1.DataType.timestamptz,
            implementation: (part, value) => part === 'day' ? truncateToDay(value) : value,
        });
    ...
    `);
        ...
        db.public.none('CREATE UNIQUE INDEX idx_mv_reporting_entity_activity_bucket_type ON maestro.mv_reporting_entity_activity(bucket_day, type);');
        db.public.none(`
    ...
    `);
        ...
        db.public.none('CREATE UNIQUE INDEX idx_mv_reporting_case_snapshot_case ON maestro.mv_reporting_case_snapshot(case_id);');
        db.public.none(`
    ...
    `);
        ...
        db.public.none('CREATE UNIQUE INDEX idx_mv_reporting_case_timeline_day ON maestro.mv_reporting_case_timeline(case_id, event_day);');
        ...
        const refreshResult = [
            {
                ...
                row_count: Number((await pool.query('SELECT COUNT(*) FROM maestro.mv_reporting_entity_activity'))
                    .rows[0].count),
                status: 'ok',
            },
            {
                view_name: 'maestro.mv_reporting_case_snapshot',
                row_count: Number((await pool.query('SELECT COUNT(*) FROM maestro.mv_reporting_case_snapshot'))
                    .rows[0].count),
                status: 'ok',
            },
            {
                view_name: 'maestro.mv_reporting_case_timeline',
                row_count: Number((await pool.query('SELECT COUNT(*) FROM maestro.mv_reporting_case_timeline'))
                    .rows[0].count),
                status: 'ok',
            },
        ];
        const activity = await pool.query('SELECT * FROM maestro.mv_reporting_entity_activity');
        const snapshot = await pool.query('SELECT * FROM maestro.mv_reporting_case_snapshot ORDER BY case_id');
        const timeline = await pool.query('SELECT * FROM maestro.mv_reporting_case_timeline ORDER BY case_id');
        ...
        const firstCase = snapshot.rows.find((row) => row.case_id === '00000000-0000-0000-0000-000000000001');
        expect(firstCase.linked_entity_count).toBe(2);
        expect(firstCase.evidence_counts).toEqual({});
        expect(timeline.rows.some((row) => Number(row.transition_count || 0) > 0)).toBe(true);
        await pool.end();
    });
});

...
class DashboardService {
    ...
    async createDashboard(dashboard, userId) {
        ...
        RETURNING *
      `;
            const shareToken = dashboard.isPublic ? (0, uuid_1.v4)() : null;
            ...
            logger_1.logger.info(`Dashboard created: ${dashboardId} by user ${userId}`);
    ...
    async updateDashboard(dashboardId, updates, userId) {
      ...
      `;
            ...
            logger_1.logger.info(`Dashboard updated: ${dashboardId} by user ${userId}`);
    ...
    async deleteDashboard(dashboardId, userId) {
        try {
            ...
            // Clear cache
            await this.invalidateDashboardCache(dashboardId);
            logger_1.logger.info(`Dashboard deleted: ${dashboardId} by user ${userId}`);
        }
        catch (error) {

...
app.post('/api/dashboard-templates/:id/create', authorize(['user', 'admin']), async (req, res) => {
    try {
        ...
        const dashboard = await dashboardService.createDashboardFromTemplate(req.params.id, name, req.user.id, customizations);
        res.status(201).json(dashboard);
    }
    catch (error) {
...
app.get('/api/insights/trends', authorize(['user', 'admin']), async (req, res) => {
    try {
        ...
        const trends = await generateTrendInsights(req.user.id, period);
        res.json(trends);
    }
    catch (error) {

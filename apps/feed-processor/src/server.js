...
const FeedProcessorService_1 = require("./services/FeedProcessorService");
...
exports.app = app;
const PORT = config_1.config.server.port || 4007;
// Security middleware
...
async function initializeServices() {
    try {
        ...
        // Initialize services
        feedProcessorService = new FeedProcessorService_1.FeedProcessorService(pgPool, neo4jDriver, redisClient);
        enrichmentService = new EnrichmentService_1.EnrichmentService(pgPool, redisClient);
        // Set up event listeners
        feedProcessorService.on('feed.poll.completed', (data) => {
            logger_1.logger.info(`Feed poll completed: ${data.sourceId}, ${data.itemCount} items`);
        });
        feedProcessorService.on('feed.poll.error', (data) => {
            logger_1.logger.error(`Feed poll error: ${data.sourceId}, ${data.error}`);
        });
        feedProcessorService.on('item.processed', (data) => {
            logger_1.logger.debug(`Item processed: ${data.itemId} in ${data.processingTime}ms`);
        });
        logger_1.logger.info('Feed processor services initialized successfully');
    }
    catch (error) {
...
app.post('/api/feed-sources', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
...
app.get('/api/feed-sources', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        const query = `
      SELECT 
        fs.*,
    ...
    `;
        ...
        logger_1.logger.error('Error listing feed sources:', error);
        res.status(500).json({ error: 'Failed to list feed sources' });
    }
});
app.get('/api/feed-sources/:id', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error getting feed source:', error);
        res.status(500).json({ error: 'Failed to get feed source' });
    }
});
app.put('/api/feed-sources/:id', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        // Update feed source implementation would go here
        res.status(501).json({ error: 'Update feed source not implemented yet' });
    }
    catch (error) {
        logger_1.logger.error('Error updating feed source:', error);
        res.status(500).json({ error: 'Failed to update feed source' });
    }
});
app.delete('/api/feed-sources/:id', (0, auth_1.authorize)(['admin']), async (req, res) => {
    try {
        await feedProcessorService.deleteSource(req.params.id);
        res.status(204).send();
    }
    catch (error) {
        logger_1.logger.error('Error deleting feed source:', error);
        res.status(500).json({ error: 'Failed to delete feed source' });
    }
});
// Feed Source Control
app.post('/api/feed-sources/:id/start', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        const source = await feedProcessorService.getFeedSource(req.params.id);
        if (!source) {
            return res.status(404).json({ error: 'Feed source not found' });
        ...
        logger_1.logger.error('Error starting feed source:', error);
        res.status(500).json({ error: 'Failed to start feed source' });
    }
});
app.post('/api/feed-sources/:id/stop', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        await feedProcessorService.stopPolling(req.params.id);
        res.json({ message: 'Feed source stopped successfully' });
    }
    catch (error) {
        logger_1.logger.error('Error stopping feed source:', error);
        res.status(500).json({ error: 'Failed to stop feed source' });
    }
});
app.post('/api/feed-sources/:id/pause', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        await feedProcessorService.pauseSource(req.params.id);
        res.json({ message: 'Feed source paused successfully' });
    }
    catch (error) {
        logger_1.logger.error('Error pausing feed source:', error);
        res.status(500).json({ error: 'Failed to pause feed source' });
    }
});
app.post('/api/feed-sources/:id/resume', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        await feedProcessorService.resumeSource(req.params.id);
        res.json({ message: 'Feed source resumed successfully' });
    }
    catch (error) {
        logger_1.logger.error('Error resuming feed source:', error);
        res.status(500).json({ error: 'Failed to resume feed source' });
    }
});
// Feed Items
app.get('/api/feed-items', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
    ...
    `;
        ...
        logger_1.logger.error('Error listing feed items:', error);
        res.status(500).json({ error: 'Failed to list feed items' });
    }
});
app.get('/api/feed-items/:id', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        const query = `
      SELECT 
        fi.*,
    ...
    `;
        ...
        logger_1.logger.error('Error getting feed item:', error);
        res.status(500).json({ error: 'Failed to get feed item' });
    }
});
// Manual enrichment
app.post('/api/feed-items/:id/enrich', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        const itemQuery = `
    ...
    `;
        ...
        logger_1.logger.error('Error enriching feed item:', error);
        res.status(500).json({ error: 'Failed to enrich feed item' });
    }
});
// Feed Statistics
app.get('/api/feed-stats', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        const { sourceId, days = '7' } = req.query;
        const stats = await feedProcessorService.getFeedStats(sourceId);
      ...
      SELECT 
        ...
        COUNT(CASE WHEN fi.published_at >= NOW() - INTERVAL '${parseInt(days)} days' THEN 1 END) as recent_items
      FROM feed_sources fs
      LEFT JOIN feed_items fi ON fs.id = fi.source_id
    `;
        ...
        logger_1.logger.error('Error getting feed stats:', error);
        res.status(500).json({ error: 'Failed to get feed stats' });
    }
});
// Enrichment provider stats
app.get('/api/enrichment/stats', (0, auth_1.authorize)(['user', 'admin']), async (req, res) => {
    try {
        const stats = await enrichmentService.getProviderStats();
        res.json({ providers: stats });
    }
    catch (error) {
        logger_1.logger.error('Error getting enrichment stats:', error);
        res.status(500).json({ error: 'Failed to get enrichment stats' });
    }
});
// Feed Source Templates
app.get('/api/feed-templates', (0, auth_1.authorize)(['user', 'admin']), (req, res) => {
    const templates = [
        {
...
app.get('/api/feed-updates/stream', (0, auth_1.authorize)(['user', 'admin']), (req, res) => {
    res.writeHead(200, {
...
async function startServer() {
    try {
        ...
        const server = app.listen(PORT, () => {
            logger_1.logger.info(`Feed Processor server running on port ${PORT}`);

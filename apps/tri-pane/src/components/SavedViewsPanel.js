...
exports.SavedViewsPanel = SavedViewsPanel;
const react_1 = __importStar(require("react"));
const EventBus_1 = require("./EventBus");
function SavedViewsPanel() {
    const { state, dispatch } = (0, EventBus_1.useTriPane)();
    const [name, setName] = (0, react_1.useState)('Analyst snapshot');
    const sorted = (0, react_1.useMemo)(() => [...state.savedViews].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()), [state.savedViews]);
    return (<div className="rounded-2xl border border-sand/20 bg-horizon/40 p-4 shadow-inner">
      <div className="flex items-center justify-between gap-3">
        ...
        <button className="rounded-full border border-accent/60 bg-accent/10 px-3 py-1 text-sm text-accent" onClick={() => dispatch({ type: 'saveView', payload: name })}>
          Save view
        </button>
      </div>
      ...
      <input id="view-name" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-lg border border-sand/20 bg-ink px-3 py-2 text-sm" aria-describedby="view-name-hint"/>
      ...
      <div className="mt-3 space-y-2" role="list">
        {sorted.map((view) => (<button key={view.id} className="flex w-full items-center justify-between rounded-lg border border-sand/20 bg-ink/50 px-3 py-2 text-left text-sm hover:border-accent focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-midnight" onClick={() => dispatch({ type: 'loadView', payload: view.id })}>
            <div>

...
export function ExplainPanel() {
  ...
  return (
    <aside className="rounded-2xl border border-accent/40 bg-ink/70 p-4 shadow-inner" aria-labelledby="explain-heading">
      ...
      <dl className="mt-3 space-y-2 text-sm">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <dd className="text-right text-sand">
            {state.activeLayers.map((layer) => layers.find((l) => l.id === layer)?.label ?? layer).join(', ')}
          </dd>
        </div>
        <div className="flex items-start justify-between gap-2">
      ...
      <div className="mt-4 space-y-2" role="list">
        ...
        {state.pinnedNodes.map((pin) => (
          <div key={pin} role="listitem" className="rounded-lg border border-sand/20 bg-horizon/60 px-3 py-2 text-sm">

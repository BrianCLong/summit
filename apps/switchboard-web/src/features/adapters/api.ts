import { AdapterActionPayload, AdapterActionResult, AdaptersResponse, SwitchboardApiError } from './types';
...
async function parseError(response: Response): Promise<SwitchboardApiError> {
  ...
  error.status = typeof (body as { status?: number })?.status === 'number'
    ? (body as { status: number }).status
    : response.status;

  const policyErrors = (body as { policyErrors?: string[]; policy?: string[] })?.policyErrors ??
    (body as { policy?: string[] })?.policy;
  const verificationErrors = (body as { verificationErrors?: string[]; verification?: string[] })?.
    verificationErrors ?? (body as { verification?: string[] })?.verification;

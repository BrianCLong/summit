...
exports.AdapterCard = void 0;
const react_1 = __importDefault(require("react"));
const clsx_1 = __importDefault(require("clsx"));
function Badge({ status }) {
    return (<span className="badge" data-status={status}>
      {status}
    </span>);
}
...
const AdapterCard = ({ adapter, onAction, isBusy, errorMessage, policyErrors, verificationErrors, receiptUrl, }) => {
    ...
    return (<article className="adapter-card" aria-busy={isBusy} aria-live="polite">
      ...
      <div className="actions" aria-label={`Actions for ${adapter.name}`}>
        {showInstall && (<button className="button primary" disabled={isBusy} onClick={() => onAction('install')}>
            Install
          </button>)}
        {canEnable && (<button className="button primary" disabled={isBusy} onClick={() => onAction('enable')}>
            Enable
          </button>)}
        {canDisable && (<button className="button" disabled={isBusy} onClick={() => onAction('disable')}>
            Disable
          </button>)}
        {canUninstall && (<button className={(0, clsx_1.default)('button', 'danger')} disabled={isBusy} onClick={() => onAction('uninstall')}>
            Uninstall
          </button>)}
      ...
      {receiptUrl ? (<div className="status-row">
      ...
      {errorMessage || policyErrors?.length || verificationErrors?.length ? (<div className="error-box" role="status">
          <div className="error-title">Action blocked</div>
          {errorMessage ? <p style={{ margin: '4px 0' }}>{errorMessage}</p> : null}
          {policyErrors && policyErrors.length > 0 ? (<div>
              <div className="meta-label">Policy</div>
              <ul className="error-list">
                {policyErrors.map((item) => (<li key={item}>{item}</li>))}
              </ul>
            </div>) : null}
          {verificationErrors && verificationErrors.length > 0 ? (<div>
              <div className="meta-label">Verification</div>
              <ul className="error-list">
                {verificationErrors.map((item) => (<li key={item}>{item}</li>))}
              </ul>
            </div>) : null}
        </div>) : null}
    </article>);
};
exports.AdapterCard = AdapterCard;

...
class TrainingPipeline {
    ...
    constructor(pgPool, benchmarkingService, modelRegistry) {
        ...
        this.pgPool = pgPool;
        this.modelsDir = path_1.default.join(process.cwd(), 'models');
        this.ensureModelsDirectory();
    }
    async ensureModelsDirectory() {
        try {
            await promises_1.default.mkdir(this.modelsDir, { recursive: true });
            await promises_1.default.mkdir(path_1.default.join(this.modelsDir, 'entity-resolution'), {
                recursive: true,
            });
            await promises_1.default.mkdir(path_1.default.join(this.modelsDir, 'training-data'), {
                recursive: true,
            });
        }
        catch (error) {
    ...
    async collectTrainingData(minExamples = 100) {
      ...
      WHERE er.created_at >= NOW() - INTERVAL '${config_1.config.ml.entityResolution.trainingDataRetentionDays} days'
    ...
    `;
        try {
            ...
            logger_1.logger.info(`Collected ${examples.length} training examples`);
            if (examples.length < minExamples) {
                logger_1.logger.warn(`Only ${examples.length} examples available, minimum required: ${minExamples}`);
            }
            return examples;
        }
        catch (error) {
            logger_1.logger.error('Error collecting training data:', error);
            throw error;
        }
    }
    async generateFeatures(examples) {
        logger_1.logger.info('Generating features for training examples');
        // Feature extraction using Python script
        const pythonScript = path_1.default.join(config_1.config.ml.python.scriptPath, 'feature_extraction.py');
        const inputFile = path_1.default.join(this.modelsDir, 'training-data', 'input.json');
        const outputFile = path_1.default.join(this.modelsDir, 'training-data', 'features.json');
        try {
            // Write examples to input file
            await promises_1.default.writeFile(inputFile, JSON.stringify(examples, null, 2));
            // Run Python feature extraction
            await this.runPythonScript(pythonScript, [inputFile, outputFile]);
            // Read generated features
            const featuresData = await promises_1.default.readFile(outputFile, 'utf-8');
            const featuredExamples = JSON.parse(featuresData);
            logger_1.logger.info(`Generated features for ${featuredExamples.length} examples`);
            return featuredExamples;
        }
        catch (error) {
            logger_1.logger.error('Error generating features:', error);
            throw error;
        }
    }
    async trainModel(examples, modelType = 'random_forest', hyperparameters = {}) {
        logger_1.logger.info(`Starting training with ${examples.length} examples`);
        const modelId = `er-${modelType}-${Date.now()}`;
        const versionString = '1.0.0';
        const modelPath = path_1.default.join(this.modelsDir, 'entity-resolution', `${modelId}.pkl`);
        ...
        try {
            ...
            const trainingFile = path_1.default.join(this.modelsDir, 'training-data', `training-${modelId}.json`);
            await promises_1.default.writeFile(trainingFile, JSON.stringify(trainingData, null, 2));
            // Train model using Python script
            const pythonScript = path_1.default.join(config_1.config.ml.python.scriptPath, 'train_model.py');
            const metricsFile = path_1.default.join(this.modelsDir, 'training-data', `metrics-${modelId}.json`);
            ...
            logger_1.logger.info(`Training completed for model ${modelId}:`, {
            ...
            if (await this.shouldActivateModel(modelVersion)) {
                ...
                logger_1.logger.info(`Model ${modelId} automatically activated`);
            }
            return modelVersion;
        }
        catch (error) {
            logger_1.logger.error('Error during model training:', error);
            throw error;
        }
    }
    async evaluateModel(modelId, testExamples) {
        logger_1.logger.info(`Evaluating model ${modelId} with ${testExamples.length} test examples`);
        try {
            ...
            const testFile = path_1.default.join(this.modelsDir, 'training-data', `test-${modelId}.json`);
            const metricsFile = path_1.default.join(this.modelsDir, 'training-data', `eval-metrics-${modelId}.json`);
            await promises_1.default.writeFile(testFile, JSON.stringify(testData, null, 2));
            // Evaluate using Python script
            const pythonScript = path_1.default.join(config_1.config.ml.python.scriptPath, 'evaluate_model.py');
            await this.runPythonScript(pythonScript, [testFile, metricsFile]);
            // Read evaluation metrics
            const metricsData = await promises_1.default.readFile(metricsFile, 'utf-8');
            const metrics = JSON.parse(metricsData);
            logger_1.logger.info(`Evaluation completed for model ${modelId}:`, metrics);
            ...
            logger_1.logger.error('Error evaluating model:', error);
            throw error;
        }
    }
    async runPythonScript(scriptPath, args) {
        return new Promise((resolve, reject) => {
            const pythonProcess = (0, child_process_1.spawn)(config_1.config.ml.python.pythonExecutable, [scriptPath, ...args], {
            ...
            pythonProcess.on('close', (code) => {
                if (code === 0) {
                    if (stdout.trim()) {
                        logger_1.logger.info('Python script output:', stdout.trim());
                    }
                    resolve();
                }
                else {
                    logger_1.logger.error(`Python script failed with code ${code}:`, stderr);
                    reject(new Error(`Python script failed: ${stderr}`));
                }
            });
            pythonProcess.on('error', (error) => {
                logger_1.logger.error('Failed to start Python script:', error);
                reject(error);
            });
        });
    }
    ...
    async activateModel(modelId) {
        ...
        try {
            ...
            logger_1.logger.info(`Activated model ${modelId} for type ${modelVersion.modelType}`);
        }
        catch (error) {
            await client.query('ROLLBACK');
            logger_1.logger.error('Error activating model:', error);
            throw error;
        }
        finally {
            client.release();
    ...
    async scheduleTraining(cron, modelType) {
        ...
        logger_1.logger.info(`Scheduled training for ${modelType} with cron: ${cron}`);
    ...
    async recordPerformanceSnapshot(modelVersion, metrics, context) {
        ...
        try {
            ...
            logger_1.logger.warn('Failed to record performance snapshot', {
                modelVersionId: modelVersion.id,
                error: error.message,
            });
        }

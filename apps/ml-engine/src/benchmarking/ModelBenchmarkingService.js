...
exports.ModelBenchmarkingService = void 0;
...
class ModelBenchmarkingService {
    ...
    async initialize() {
      ...
      `);
            ...
            logger_js_1.logger.info('ModelBenchmarkingService initialized');
        }
        catch (error) {
            await client.query('ROLLBACK');
            logger_js_1.logger.error('Failed to initialize ModelBenchmarkingService', error);
            throw error;
        }
        finally {
            client.release();
    ...
    async getPerformanceHistory(modelVersionId, limit = 50) {
        await this.initialize();
      ...
      `, [modelVersionId, limit]);
        ...
        return Array.from(grouped.values()).sort((a, b) => new Date(a.evaluationDate ?? 0).getTime() -
            new Date(b.evaluationDate ?? 0).getTime());
    }
    getRealtimeSnapshot(modelType) {
        const snapshots = [];
        for (const aggregate of this.inferenceAggregates.values()) {
            if (modelType && aggregate.modelType !== modelType) {
                continue;
            }
            const averageLatency = aggregate.requestCount === 0
                ? 0
                : aggregate.totalLatency / aggregate.requestCount;
            const successRate = aggregate.requestCount === 0
                ? 0
        ...
        return snapshots.sort((a, b) => new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime());
    }
    emitUpdate() {
...
exports.ModelBenchmarkingService = ModelBenchmarkingService;

...
export class HyperparameterOptimizer {
  ...
  async optimize(
    ...
    const inputFile = path.join(this.workingDirectory, `${requestPayload.id}-input.json`);
    const outputFile = path.join(this.workingDirectory, `${requestPayload.id}-output.json`);

    ...
    for name, definition in space.items():
        ...
        if kind == "int":
            ...
            if definition.get("log"):
                params[name] = float(
                    np.exp(random.uniform(np.log(definition["min"]), np.log(definition["max"])))
                )
            else:
...
def main() -> int:
    ...
    try:
        ...
        if optuna is not None:
            result = optimise_with_optuna(trainer, request, X_train, X_valid, y_train, y_valid)
        else:
        ...
        if not result.get("best_metrics"):
            ...
            result["best_metrics"] = trainer.evaluate_model(pipeline, X_valid, y_valid)

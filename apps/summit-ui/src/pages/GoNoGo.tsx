...
import { getGoNoGo } from '../api';
import type { GoNoGoData } from '../types';
import { PolicyBadge } from '../components/StatusBadge';

function ProvenancePanel({ p }: { p: GoNoGoData['provenance'] }) {
  return (
    <div className="card">
...
function EvidencePanel({ checks }: { checks: GoNoGoData['evidence'] }) {
  return (
    <div className="card">
      <div className="card-title">Evidence bundle</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {checks.map((c) => (
          <div key={c.label} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid rgba(48,54,61,.4)', fontSize: 12 }}>
...
function SbomPanel({ sbom }: { sbom: GoNoGoData['sbom'] }) {
  if (sbom.length === 0) {
    return (
      <div className="card">
...
function PolicyPanel({ policies }: { policies: GoNoGoData['policies'] }) {
      ...
      <table>
        ...
        <tbody>
          {policies.map((p) => (
            <tr key={p.file}>
              <td>{p.name}</td>
              <td><PolicyBadge result={p.result} /></td>
...
export function GoNoGo() {
  ...
  if (loading) return <div className="loading" role="status" aria-busy="true">Evaluating release gate…</div>;

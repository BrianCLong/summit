...
exports.validateRegistrySources = validateRegistrySources;
const zod_1 = require("zod");
const schema_js_1 = require("./schema.js");
const loader_js_1 = require("./loader.js");
function validateRegistrySources(sources) {
    const errors = [];
    let totalTools = 0;
    let totalServers = 0;
    ...
    for (const source of sources) {
        try {
            schema_js_1.RegistryRootSchema.parse(source.data);
        }
        catch (err) {
    ...
    // 2. Merged validation
    const { registry, conflicts, itemOrigins } = (0, loader_js_1.mergeRegistrySources)(sources);
    ...
    const serverIds = new Set(registry.servers.map((s) => s.server_id));
...
function getHint(path, code) {
    if (path.includes('tool_id'))
        return "Ensure each tool has a unique 'tool_id' string.";
    if (path.includes('server_id'))
        return "Ensure each server has a unique 'server_id' string.";
    if (path.includes('endpoint'))
        return "endpoint must be a full URL (e.g. http://localhost:8080).";
    if (path.includes('capability'))
        return "Capabilities should be an array of strings describing what the tool/server can do.";
    if (path.includes('version'))
        return "Registry version should follow semantic versioning (x.y.z).";
    return undefined;
}

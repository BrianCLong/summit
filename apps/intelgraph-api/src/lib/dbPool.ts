...
const buildPoolConfig = (): PoolConfig => {
  const connectionString =
    process.env.PG_CONNECTION || process.env.DATABASE_URL || undefined;
  ...
  if (tuningEnabled) {
    ...
    config.maxLifetimeSeconds = parseInt(
      process.env.DB_POOL_MAX_LIFETIME_SECONDS || '900',
      10,
    );
...
const execute = async (
  ...
  if (mode === 'read' && tuningEnabled) {
    return runInTransaction(mode, (client) =>
      client.query({ text: sql, values: params, name }),
    );
  }
...
export const createDbClient = () => {
  const any = async (sql: string, params: any[] = []) =>

...
const ensurePlanBelongsToTenant = async (ctx: any, planId: string, tenantId: string) => {
  const plan = await ctx.pg.oneOrNone('select id, tenant_id as "tenantId" from plan where id=$1', [
    planId,
  ]);
  if (!plan) {
...
export const resolvers = {
  ...
  Mutation: {
    ...
    recordArtifact: async (_: any, args: any, ctx: any) => {
  ...
  Organization: {
    __resolveReference: (ref: any, ctx: any) => entityLoaders.Organization!(ref, ctx),
  },

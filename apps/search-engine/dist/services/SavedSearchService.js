...
export class SavedSearchService {
    ...
    async createSavedSearch(name, description, query, userId, isPublic = false, tags = []) {
        const id = uuidv4();
        const now = new Date();
        try {
    ...
    async getSavedSearch(id, userId) {
        try {
            const result = await this.db.query(`
    ...
    async updateSavedSearch(id, updates, userId) {
        try {
            const updateFields = [];
            const updateValues = [];
            let paramCounter = 1;
    ...
    async listSavedSearches(userId, options = {}) {
        try {
            const conditions = [];
            const params = [];
            let paramCounter = 1;
    ...
    async executeSavedSearch(id, userId) {
        try {
            const result = await this.db.query(`
        UPDATE saved_searches 
    ...
    async createSearchTemplate(name, description, template, params, userId, category, isPublic = false) {
        const id = uuidv4();
        const now = new Date();
        try {
            const result = await this.db.query(`
    ...
    async getSearchTemplate(id, userId) {
        try {
            const result = await this.db.query(`
    ...
    async listSearchTemplates(userId, options = {}) {
        try {
            const conditions = [];
            const params = [];
            let paramCounter = 1;
    ...
    async renderTemplate(templateId, params, userId) {
        try {
    ...
    async getPopularSearches(userId, limit = 10) {
        try {
            const result = await this.db.query(`
        SELECT * FROM saved_searches 
    ...
    async searchSavedSearches(userId, searchTerm) {
        try {
            const result = await this.db.query(`
        SELECT * FROM saved_searches 

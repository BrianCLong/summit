...
describe('QueryBuilderService', () => {
    let service;
    beforeEach(() => {
        service = new QueryBuilderService_1.QueryBuilderService();
    });
    describe('buildQuery', () => {
    ...
    describe('filter expression parsing', () => {
        ...
        it('should parse multiple filter types', () => {
            const filters = service.parseFilterExpression('type:threat date:2024-01-01 to 2024-12-31 confidence:0.8 to 1.0');
    ...
    describe('Levenshtein distance calculation', () => {
        ...
        it('should calculate distance for single character difference', () => {
            const distance = service.levenshteinDistance('test', 'fest');
        ...
        it('should handle empty strings', () => {
            const distance1 = service.levenshteinDistance('', 'test');
            const distance2 = service.levenshteinDistance('test', '');
    ...
    describe('token processing', () => {
        it('should extract main terms from query', () => {
            const tokens = service.tokenizeQuery('find cyber attack organizations');
            const processed = service.processTokens(tokens);
        ...
        it('should extract entity types', () => {
            const tokens = service.tokenizeQuery('find all people and organizations');
            const processed = service.processTokens(tokens);
        ...
        it('should extract date expressions', () => {
            const tokens = service.tokenizeQuery('events from last week');
            const processed = service.processTokens(tokens);
            expect(processed.dateExpressions).toContain('week');
        });
        it('should extract modifiers', () => {
            const tokens = service.tokenizeQuery('find similar entities');
            const processed = service.processTokens(tokens);
            expect(processed.modifiers).toContain('similar');
        });
        it('should remove stop words', () => {
            const tokens = service.tokenizeQuery('the person and the organization');
            const processed = service.processTokens(tokens);
    ...
    describe('date expression parsing', () => {
        it('should parse "today" expression', () => {
            const dateRange = service.parseDateExpressions(['today']);
        ...
        it('should parse "week" expression', () => {
            const dateRange = service.parseDateExpressions(['week']);
            expect(dateRange.from).toBeDefined();
            const from = new Date(dateRange.from);
            const now = new Date();
        ...
        it('should parse "month" expression', () => {
            const dateRange = service.parseDateExpressions(['month']);
            expect(dateRange.from).toBeDefined();
            const from = new Date(dateRange.from);
            const now = new Date();
            const daysDiff = (now.getTime() - from.getTime()) / (24 * 60 * 60 * 1000);

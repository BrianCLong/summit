...
class ElasticsearchService {
    ...
    constructor() {
        ...
        this.logger = (0, winston_1.createLogger)({
            ...
            transports: [
                ...
                new winston_1.transports.File({ filename: 'logs/search-engine.log' }),
            ],
        });
    }
    async search(query) {
        ...
        if (query.boost) {
            if (query.boost.fields) {
                Object.entries(query.boost.fields).forEach(([field, boost]) => {
                    const fieldQuery = boolQuery.bool.must.find((q) => q.multi_match);
    ...
    transformHits(hits) {
        return hits.map((hit) => ({
            id: hit._id,
            type: this.singularize(hit._index),
            score: hit._score,
            source: hit._source,
        ...
        Object.values(suggestions).forEach((suggestion) => {
            if (Array.isArray(suggestion)) {
                suggestion.forEach((s) => {
                    if (s.options) {
                        results.push(...s.options.map((option) => ({
                            text: option.text,
                            score: option.score,
                            freq: option.freq,
                        })));

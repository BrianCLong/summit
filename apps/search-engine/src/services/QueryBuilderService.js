...
class QueryBuilderService {
    ...
    parseDateExpressions(dateExpressions) {
        const now = new Date();
        let from;
        let to;
    ...
    extractSortCriteria(processed) {
        if (processed.dateExpressions.includes('latest') ||
            processed.dateExpressions.includes('recent')) {
            return { field: 'createdAt', order: 'desc' };
        }
        return undefined;
    }
    suggestFacets(processed) {
    ...
    buildAdvancedQuery(queryBuilder) {
        return this.convertQueryBuilderToElastic(queryBuilder);
    }
    convertQueryBuilderToElastic(qb) {
        switch (qb.type) {
            case 'bool':
                return {
                    bool: {
    ...
    suggestCorrections(query) {
        ...
        words.forEach((word) => {
            ...
            for (const [key, synonyms] of this.synonyms.entries()) {
                ...
                const closeMatches = allTerms.filter((term) => this.levenshteinDistance(word.toLowerCase(), term) <= 2);

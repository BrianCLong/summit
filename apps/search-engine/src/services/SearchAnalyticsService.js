...
class SearchAnalyticsService {
    ...
    constructor(pg, redisClient) {
        ...
        this.logger = (0, winston_1.createLogger)({
            ...
            transports: [
                ...
                new winston_1.transports.File({ filename: 'logs/search-analytics.log' }),
            ],
        });
    }
    /**
     ...
     */
    async trackQuery(userId, query, resultCount, executionTime, success = true, sessionId) {
     ...
     */
    async trackClick(queryId, resultId, position) {
        try {
            await this.pg.query(`
     ...
     */
    async trackRefinement(queryId, originalQuery, refinedQuery, refinementType) {
        try {
            await this.pg.query(`
     ...
     */
    async getMetrics(startDate, endDate, userId) {
        try {
     ...
     */
    async getQuerySuggestions(prefix, limit = 10) {
        try {
     ...
     * Update real-time metrics in Redis
     */
    async updateRealTimeMetrics(query, executionTime, resultCount, success) {

...
const queryDurationHistogram = meter.createHistogram("graphql_query_duration_ms", {
...
const resolverDurationHistogram = meter.createHistogram("graphql_resolver_duration_ms", {
...
const queryComplexityGauge = meter.createUpDownCounter("graphql_query_complexity", {
...
// Golden path metrics
const goldenPathSuccessCounter = meter.createCounter("golden_path_success_total", {
  description: "Golden path step successes",
});

const goldenPathDurationHistogram = meter.createHistogram("golden_path_duration_ms", {
  description: "Golden path step duration in milliseconds",
  unit: "ms",
});

const goldenPathErrorsCounter = meter.createCounter("golden_path_errors_total", {
...
export function recordQueryComplexity(operationName: string | null, complexity: number): void {
...
export function recordGoldenPathSuccess(step: string, durationMs: number): void {
...
export function getOperationInfo(requestContext: GraphQLRequestContext<any>): {
  operationName: string | null;
  operationType: string;
} {

...
export const TimelinePane = () => {
  const { allEntities, selectedEntityIds, selectEntity, timeWindow, setTimeWindow } = useWorkspaceStore();
          ...
          <Scatter name="Events" data={data} onClick={handlePointClick} cursor="pointer">
            {data.map((entry, index) => (
              <Cell
                ...
                opacity={ (entry.time >= refAreaLeft && entry.time <= refAreaRight) ? 1 : 0.3 }
              />
            ))}
          </Scatter>

          <ReferenceArea x1={refAreaLeft} x2={refAreaRight} strokeOpacity={0.3} fill="#22d3ee" fillOpacity={0.1} />

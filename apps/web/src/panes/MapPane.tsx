...
const ZoomWatcher = ({ onZoomChange }: { onZoomChange: (nextZoom: number) => void }) => {
    ...
    <>
      {markers.map((marker) => {
        ...
        const radius = isCluster
          ? Math.min(18, 6 + Math.log2((marker as ClusteredMarker).count + 1) * 3)
          : isSelected
            ? 8
            : 5;
                ...
                {isCluster ? (
                  <>
                    ...
                    <span className="text-xs">
                      Includes {(marker as ClusteredMarker).memberIds.length} nearby points.
                      Zoom in to expand.
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span className="text-xs">{(marker as MarkerEntity).type}</span>
                    <p className="text-xs mt-1 text-slate-600">{(marker as MarkerEntity).description}</p>
                  </>
                )}
              </div>
            </Popup>

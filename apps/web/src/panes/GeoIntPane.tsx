...
export const GeoIntPane: React.FC = () => {
  ...
  const [satelliteUrl, setSatelliteUrl] = useState(
    'https://example.com/sat1.jpg'
  )
  const [analyzeSat, { data: satData, loading: satLoading }] =
    useLazyQuery(ANALYZE_SATELLITE)
  ...
  const [detectChange, { data: changeData, loading: changeLoading }] =
    useLazyQuery(DETECT_CHANGE)
  ...
  const [getElevation, { data: elevationData, loading: elevationLoading }] =
    useLazyQuery(GET_ELEVATION)
      ...
      <div className="flex flex-row h-[700px] gap-4">
        ...
        <div className="flex-grow rounded-md border overflow-hidden flex flex-col gap-2">
          ...
          >
            ...
            />
            {/* Detected Objects */}
            {satData?.analyzeSatelliteImage?.objectsDetected?.map(
              (obj: any, idx: number) => (
                <Circle
                  key={idx}
                ...
                <CardContent className="space-y-4">
                  ...
                  {satData && (
                    <div className="mt-4 p-2 bg-slate-100 dark:bg-slate-800 rounded text-sm">
                      <p>
                        <strong>Class:</strong>{' '}
                        {satData.analyzeSatelliteImage.classification}
                      </p>
                      <p>
                        <strong>Cloud Cover:</strong>{' '}
                        {satData.analyzeSatelliteImage.cloudCover}
                      </p>
                      <p>
                        <strong>Objects:</strong>{' '}
                        {satData.analyzeSatelliteImage.objectsDetected.length}
                      </p>
                    </div>
                  )}
                </CardContent>
                  ...
                  {changeData && (
                    <div className="mt-4 p-2 bg-slate-100 dark:bg-slate-800 rounded text-sm">
                      <p>
                        ...
                        {changeData.detectChange.changeDetected ? 'Yes' : 'No'}
                      </p>
                      <p>
                        <strong>Magnitude:</strong>{' '}
                        {changeData.detectChange.percentageChange}%
                      </p>
                    </div>
                  )}
                </CardContent>

...
function CanvasGraphRenderer({ entities, relationships, layout, onEntitySelect, selectedEntityId, className, }) {
    ...
    const propsRef = (0, react_1.useRef)({ entities, relationships, layout });
    (0, react_1.useEffect)(() => {
    ...
    const nodesRef = (0, react_1.useRef)([]);
    const transformRef = (0, react_1.useRef)(d3_zoom_1.zoomIdentity);
    // O(1) Lookup Map for nodes by ID (Performance optimization)
    const nodeMapRef = (0, react_1.useRef)(new Map());
    ...
    (0, react_1.useEffect)(() => {
        ...
        (0, d3_selection_1.select)(canvas).call(zoomBehavior);
        // Click Selection
        ...
        // Drag behavior
        const dragBehavior = (0, d3_drag_1.drag)()
            .subject((event) => {
            const rect = canvas.getBoundingClientRect();
            const transform = transformRef.current;
        ...
        (0, d3_selection_1.select)(canvas).call(dragBehavior);
    ...
    return (<div className={(0, utils_1.cn)('relative w-full h-full', className)}>
      <canvas ref={canvasRef} className="block w-full h-full bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800" style={{

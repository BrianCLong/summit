...
const time_window_utils_1 = require("../lib/time-window-utils");
...
exports.useWorkspaceStore = (0, zustand_1.create)((set, get) => ({
    selectedEntityIds: [],
    timeWindow: (0, time_window_utils_1.createInitialTimeWindow)(),
    ...
    setTimeWindow: (startMs, endMs, granularity, tzMode, source = 'unknown') => {
        ...
        // Normalize
        const normalizedWindow = (0, time_window_utils_1.normalizeWindow)(startMs, endMs, newGranularity, newTzMode, newSeq);
        // Optimistic update of the window
        set({ timeWindow: normalizedWindow, isSyncing: true, syncError: null });
        // Report change telemetry
        (0, metrics_1.trackTimeWindowChange)(normalizedWindow.startMs, normalizedWindow.endMs, normalizedWindow.granularity, normalizedWindow.tzMode, source);
        ...
        setTimeout(() => {
            ...
            try {
                ...
                const filteredEntities = allEntities.filter(e => {
                    if (!e.timestamp)
                        return true; // Keep entities without timestamp? Or filter them out? Spec says "derive from shared TimeWindow".
                ...
                const filteredLinks = allLinks.filter(l => entityIds.has(l.source) && entityIds.has(l.target));
                set({
                ...
                (0, metrics_1.trackQueryLatency)('all', duration);
            }
            catch (err) {
                if (!signal.aborted) {

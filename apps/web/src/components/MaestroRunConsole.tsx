  ...
  return (
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      <div className="grid gap-4 md:gap-6 md:grid-cols-[minmax(0,2fr)_minmax(0,1.2fr)]">
        ...
        <MaestroRunSummary selectedRun={selectedRun} />
      </div>

      {/* Bottom: tasks & outputs */}
      <div className="grid gap-4 md:gap-6 md:grid-cols-[minmax(0,1.4fr)_minmax(0,1.6fr)]">
        <MaestroTasks selectedRun={selectedRun} />
        <MaestroOutputs selectedRun={selectedRun} />
      </div>
    </div>
  );
};

const MaestroRunSummary = memo(
  ({ selectedRun }: { selectedRun: MaestroRunResponse | null }) => {
    return (
      <Card className="shadow-md border border-slate-800 bg-slate-950/60 backdrop-blur">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="flex items-center gap-2 text-slate-50">
...
const MaestroTasks = memo(
  ({ selectedRun }: { selectedRun: MaestroRunResponse | null }) => {
    // Optimization: Create a Map for O(1) lookup instead of O(N) find inside the loop

...
exports.ScenarioSimulator = void 0;
...
const ScenarioSimulator = () => {
    ...
    const [entities, setEntities] = (0, react_1.useState)(DEFAULT_ENTITIES);
    const [shockEnabled, setShockEnabled] = (0, react_1.useState)(false);
    const [shockDescription, setShockDescription] = (0, react_1.useState)('Cyber Attack on Grid');
    const runSimulation = async () => {
        setLoading(true);
        try {
            const shock = shockEnabled
                ? {
                    type: 'shock',
    ...
    return (<div className="p-6 bg-slate-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
            ...
            <div className="mb-4">
              ...
              <textarea id="scenario-entities" className="w-full h-64 p-3 border rounded font-mono text-xs bg-slate-50" value={JSON.stringify(entities, null, 2)} onChange={(e) => {
            try {
            ...
            <div className="border-t pt-4 mt-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <input type="checkbox" checked={shockEnabled} onChange={(e) => setShockEnabled(e.target.checked)} className="h-5 w-5" aria-labelledby="shock-label"/>
              </div>

              {shockEnabled && (<div className="space-y-3">
                  <div>
                    <label htmlFor="shock-description" className="block text-xs font-medium text-slate-500">Description</label>
                    <input id="shock-description" type="text" value={shockDescription} onChange={(e) => setShockDescription(e.target.value)} className="w-full p-2 border rounded text-sm"/>
                  </div>
            ...
            <button onClick={runSimulation} disabled={loading} className="w-full mt-6 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded disabled:opacity-50" aria-label={loading ? 'Simulation in progress' : 'Run Monte Carlo Batch Simulation (20 iterations)'}>
        ...
        <div className="lg:col-span-2">
          {result ? (<div className="space-y-6" role="region" aria-label="Simulation Results">
              <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                <h2 className="text-xl font-semibold mb-4">Outcome Clustering</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {result.clusters.map((cluster) => (<div key={cluster.label} className={`p-4 rounded-lg border-l-4 ${cluster.label.includes('Crisis') || cluster.label.includes('Stagnation')
                    ? 'border-red-500 bg-red-50'
                    : cluster.label.includes('Optimistic') || cluster.label.includes('Prosperity')
                        ? 'border-green-500 bg-green-50'
                        : 'border-blue-500 bg-blue-50'}`} role="article" aria-label={`${cluster.label} Outcome Cluster`}>
            ...
            </div>) : (<div className="h-full flex items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 rounded-lg">
...
exports.ScenarioSimulator = ScenarioSimulator;

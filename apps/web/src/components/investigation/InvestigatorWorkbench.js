...
exports.InvestigatorWorkbench = InvestigatorWorkbench;
...
function InvestigatorWorkbench() {
    const [entities, setEntities] = (0, react_1.useState)(INITIAL_ENTITIES);
    const [relationships, setRelationships] = (0, react_1.useState)(INITIAL_RELATIONSHIPS);
    const [layout] = (0, react_1.useState)({ type: 'force', settings: {} });
    ...
    const dateRange = (0, react_1.useMemo)(() => {
    ...
    const handleNodeDrop = (0, react_1.useCallback)((type, _x, _y) => {
        const newEntity = {
            id: `e-${Date.now()}`,
            name: `New ${type}`,
            type: type,
    ...
    const handleLinkCreate = (0, react_1.useCallback)((sourceId, targetId) => {
        const exists = relationships.some(r => (r.sourceId === sourceId && r.targetId === targetId) ||
    ...
    return (<div className="flex h-screen w-full bg-background overflow-hidden" data-testid="investigator-workbench">
      {/* Sidebar: Entity Palette */}
      <div className="w-64 border-r bg-card p-4 flex flex-col gap-4" role="complementary" aria-label="Tools and Palette">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2" role="list" aria-label="Draggable entities">
            {ENTITY_TYPES.map(et => (<div key={et.type} className="p-2 border rounded bg-muted text-xs cursor-move flex flex-col items-center gap-1 hover:bg-muted/80 transition-colors focus:outline-none focus:ring-2 focus:ring-ring" draggable tabIndex={0} role="listitem" aria-label={`Drag ${et.label}`} onDragStart={(e) => {
      ...
      <div className="flex-1 flex flex-col relative" role="main">
        <div className="flex-1 relative bg-slate-50 dark:bg-slate-900" aria-label="Graph Canvas">
           <GraphCanvas_1.GraphCanvas ref={graphRef} entities={filteredData.entities} relationships={filteredData.relationships} layout={layout} onEntitySelect={handleEntitySelect} selectedEntityId={selectedEntityId} onNodeDrop={handleNodeDrop} onLinkCreate={handleLinkCreate} className="w-full h-full"/>
        </div>

        {/* Bottom Control Panel: Timeline */}
        <div className="h-28 border-t bg-card p-4 flex flex-col gap-2" role="region" aria-label="Timeline Controls">
           <div className="flex justify-between items-center">
              <h3 className="text-sm font-medium">Timeline Filter</h3>
              <div className="flex gap-2">
                  <Button_1.Button variant="outline" size="sm" onClick={handleExportPNG}>Export PNG</Button_1.Button>
                  <Button_1.Button variant="outline" size="sm" onClick={handleExportSVG}>Export SVG</Button_1.Button>
                  <Button_1.Button variant="outline" size="sm" onClick={handleExportJSON}>Export JSON</Button_1.Button>
              </div>
           </div>
           <div className="px-2">
             <slider_1.Slider value={timeRange} onValueChange={setTimeRange} max={100} step={1} className="w-full" aria-label="Time Range Filter"/>

...
const GenericPanel = ({ title, data, loading, error, renderDetails }: PanelProps) => {
  if (loading) {
    return (
      <Card>
        <CardHeader>
  ...
  return (
    <Card className={data.status === 'red' ? 'border-red-500' : data.status === 'yellow' ? 'border-yellow-500' : ''}>
...
export const GovernancePanel = (props: Omit<PanelProps, 'renderDetails' | 'title'>) => (
  <GenericPanel
    title="Governance"
    {...props}
...
export const AgentControlPanel = (props: Omit<PanelProps, 'renderDetails' | 'title'>) => (
  <GenericPanel
    title="Agent Control"
    {...props}
    renderDetails={(details) => (
      <div className="space-y-1 text-sm">
        ...
        <div className="flex justify-between">
            ...
            <span>{Object.values(details.activeAgents as Record<string, number>).reduce((a: number, b: number) => a + b, 0)}</span>
        </div>
...
export const CIStatusPanel = (props: Omit<PanelProps, 'renderDetails' | 'title'>) => (
  <GenericPanel
    title="CI / PRs"
    {...props}
    renderDetails={(details) => (
        <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between">
             ...
             <div className="flex justify-between">
                <span>Open PRs:</span>
                <span>{Object.values(details.openPRs as Record<string, number>).reduce((a: number, b: number) => a + b, 0)}</span>
            </div>
        </div>
    )}
  />
...
export const ReleasePanel = (props: Omit<PanelProps, 'renderDetails' | 'title'>) => (
  <GenericPanel
    title="Releases"
    {...props}
    renderDetails={(details) => (
...
export const ZKIsolationPanel = (props: Omit<PanelProps, 'renderDetails' | 'title'>) => (
  <GenericPanel
    title="ZK & Isolation"
    {...props}
    renderDetails={(details) => (
...
export const StreamingPanel = (props: Omit<PanelProps, 'renderDetails' | 'title'>) => (
  <GenericPanel
...
export const GAReadinessPanel = (props: Omit<PanelProps, 'renderDetails' | 'title'>) => (
  <GenericPanel
    title="GA Readiness"
    {...props}
    renderDetails={(details) => (

...
exports.AutoCanaryController = AutoCanaryController;
const react_1 = __importStar(require("react"));
const outline_1 = require("@heroicons/react/24/outline");
function AutoCanaryController({ runId, onCanaryComplete, }) {
    const [canaryConfig, setCanaryConfig] = (0, react_1.useState)(null);
    ...
    const calculateVictoryProbability = () => {
        if (!canaryConfig) {
            return 0;
        }
        ...
        const costOk = currentMetrics.costPerRequest / baselineMetrics.costPerRequest - 1 <=
    ...
    return (<div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-green-50 border border-blue-200 rounded-lg">
        ...
        <div className="w-full bg-gray-200 rounded-full h-3">
          <div className={`h-3 rounded-full transition-all duration-500 ${victoryProbability >= 80
            ? 'bg-green-500'
            : victoryProbability >= 60
                ? 'bg-yellow-500'
                : 'bg-red-500'}`} style={{ width: `${victoryProbability}%` }}></div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className={`text-sm font-mono font-semibold ${canaryConfig.currentMetrics.successRate >=
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-400">→</span>
              <span className={`text-sm font-mono font-semibold ${canaryConfig.currentMetrics.p95Latency <=
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-400">→</span>
              <span className={`text-sm font-mono font-semibold ${canaryConfig.currentMetrics.errorRate <=
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-400">→</span>
              <span className={`text-sm font-mono font-semibold ${canaryConfig.currentMetrics.costPerRequest /
      ...
      {/* Manual Controls */}
      {canaryConfig.status === 'running' && (<div className="flex space-x-3">
          <button onClick={() => handleManualAction('promote')} className="flex-1 flex items-center justify-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
            <outline_1.CheckCircleIcon className="h-4 w-4 mr-2"/>
            Promote Now
          </button>

          <button onClick={() => handleManualAction('pause')} className="flex items-center justify-center px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
            <outline_1.PauseIcon className="h-4 w-4"/>
          </button>

          <button onClick={() => handleManualAction('abort')} className="flex-1 flex items-center justify-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
            <outline_1.XMarkIcon className="h-4 w-4 mr-2"/>
            Abort Canary
          </button>
        </div>)}

      {canaryConfig.status === 'paused' && (<div className="flex space-x-3">
          <button onClick={() => handleManualAction('resume')} className="flex-1 flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
      ...
      {(canaryConfig.status === 'promoting' ||
            canaryConfig.status === 'aborting') && (<div className="flex items-center justify-center py-4 text-gray-600">
...
exports.default = AutoCanaryController;

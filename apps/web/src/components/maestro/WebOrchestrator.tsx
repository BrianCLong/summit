...
export default function WebOrchestrator() {
  const [task, setTask] = useState(
    'Collect and synthesize credible updates on Project Nimbus in the last 48h.'
  )
    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={!canRun}
          onClick={() => run.mutate({ task, interfaces: selected })}
        >
          Run
        </button>
      </div>
      ...
      <div className="rounded-2xl border p-3">
        ...
        <ul className="mt-2 grid md:grid-cols-2 gap-2">
          {items.map(it => (
            <li
              key={it.id}
              className={`p-2 rounded border ${selectedTitles.has(it.id) ? 'border-primary' : 'border-base-300'}`}
            >
              <label className="flex items-start gap-2">
                <input
                  type="checkbox"
                  checked={selectedTitles.has(it.id)}
                  onChange={e =>
                    setSelected(prev =>
                      e.target.checked
                        ? [...prev, it.id]
      ...
      {run.isSuccess && (
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <div className="rounded-2xl border p-3">
            ...
            <div className="text-xs mt-1">
              Citations:{' '}
              {run.data.synthesized.citations.map((c, i) => (
                <a
                  key={i}
                  className="link"
                  href={c.url}
            ...
            <div className="mt-3">
              <button
                ...
                onClick={() => alert('Attached to Case (stub).')}
              >
                Attach to Case
              </button>
            </div>

...
exports.SummitAppHost = void 0;
const react_1 = __importStar(require("react"));
const SummitAppHost = ({ appUri, className, onContextUpdate, onToolCall }) => {
    ...
    return (<div className={`flex flex-col border rounded-lg overflow-hidden bg-white shadow-sm ${className || ''}`}>
      ...
      <div className="relative flex-1 min-h-[500px] bg-slate-100">
        <iframe ref={iframeRef} src={renderUrl} className="w-full h-full border-0" sandbox="allow-scripts allow-forms" title="MCP App"/>
      </div>
      ...
      <div className="bg-slate-900 text-slate-200 p-4 h-32 overflow-y-auto text-xs font-mono border-t">
        ...
        {messages.map((msg, idx) => (<div key={idx} className={`mb-1 ${msg.direction === 'in' ? 'text-blue-300' : 'text-green-300'}`}>
...
exports.SummitAppHost = SummitAppHost;

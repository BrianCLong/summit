            ...
            setRows((prevRows) => {
              ...
              setTimeout(() => {
                ...
                if (rowElement.length) {
                  rowElement.removeClass('animate-highlight').addClass('animate-highlight');
    ...
    <div className={`${className} bg-white rounded-xl shadow-lg p-6`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <h2 className="text-2xl font-bold text-gray-900">Maestro Build Plane</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-3 py-2 text-left font-medium text-gray-700">PR</th>
                <th className="px-3 py-2 text-left font-medium text-gray-700">Status</th>
                <th className="px-3 py-2 text-left font-medium text-gray-700">SHA</th>
                <th className="px-3 py-2 text-left font-medium text-gray-700">Author</th>
                <th className="px-3 py-2 text-left font-medium text-gray-700">Policy</th>
                <th className="px-3 py-2 text-left font-medium text-gray-700">Signed</th>
                <th className="px-3 py-2 text-left font-medium text-gray-700">Tests</th>
                <th className="px-3 py-2 text-left font-medium text-gray-700">Preview</th>
                <th className="px-3 py-2 text-left font-medium text-gray-700">SBOM</th>
                ...
                >
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      {row.title && (
                        <span className="text-xs text-gray-500 truncate max-w-32" title={row.title}>
                          {row.title}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-3 py-3">
                    <span className="text-gray-700">{row.author || "Unknown"}</span>
                  </td>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex gap-1">
                      <span className={`w-3 h-3 rounded-full ${
                        row.tests?.unit === "pass" ? "bg-green-500" : 
                        row.tests?.unit === "fail" ? "bg-red-500" : "bg-gray-300"
                      }`} title="Unit Tests" />
                      <span className={`w-3 h-3 rounded-full ${
                        row.tests?.e2e === "pass" ? "bg-green-500" : 
                        row.tests?.e2e === "fail" ? "bg-red-500" : "bg-gray-300"
                      }`} title="E2E Tests" />
                      <span className={`w-3 h-3 rounded-full ${
                        row.tests?.security === "pass" ? "bg-green-500" : 
                        row.tests?.security === "fail" ? "bg-red-500" : "bg-gray-300"
                      }`} title="Security Scan" />
                    </div>
                  </td>

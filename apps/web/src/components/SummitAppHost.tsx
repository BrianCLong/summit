...
export const SummitAppHost: React.FC<SummitAppHostProps> = ({
  ...
  return (
    <div
      className={`flex flex-col border rounded-lg overflow-hidden bg-white shadow-sm ${className || ''}`}
    >
      ...
      <div className="bg-slate-900 text-slate-200 p-4 h-32 overflow-y-auto text-xs font-mono border-t">
        ...
        {messages.map((msg, idx) => (
          <div
            key={idx}
            className={`mb-1 ${msg.direction === 'in' ? 'text-blue-300' : 'text-green-300'}`}
          >

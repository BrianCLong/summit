  ...
  return (
    <div className="space-y-6 py-4">
        ...
        {profiles.map(profile => (
          ...
          >
            ...
            <Label htmlFor={profile.id} className="flex-1 cursor-pointer">
              <Card
                className={`border-2 ${selectedProfile === profile.id ? 'border-primary' : 'border-transparent'}`}
              >
                <CardContent className="p-4 space-y-2">
                  ...
                  <div className="flex gap-2">
                    {profile.guardrails.requirePurpose && (
                      ...
                      >
                        <ShieldAlert className="h-3 w-3 mr-1" /> Purpose
                        Required
                      </Badge>
                    )}
                    {profile.guardrails.requireJustification && (
                      <Badge
                        ...
                        className="text-xs border-amber-500 text-amber-500"
                      >
                        <ShieldAlert className="h-3 w-3 mr-1" /> Justification
                        Required
                      </Badge>
                    )}
                  </div>
      ...
      <div className="flex justify-end gap-3 pt-4">
        <Button
          onClick={handleApply}
          disabled={applying || selectedProfile === currentProfile}
        >

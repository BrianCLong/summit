...
export const TaskStatusBadge: React.FC<{ status: string }> = React.memo(
  ({ status }) => {
...
export const RunSummary: React.FC<RunPartProps> = React.memo(
  ({ selectedRun }) => {
    return (
...
export const RunTasks: React.FC<RunPartProps> = React.memo(
  ({ selectedRun }) => {
...
export const RunOutputs: React.FC<RunPartProps> = React.memo(
  ({ selectedRun }) => {
    return (
      <Card className="shadow-md border border-slate-800 bg-slate-950/60 backdrop-blur">
        <CardHeader className="flex flex-row items-center justify-between">

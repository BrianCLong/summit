...
export function useCollaboration({ room, username, onMessage }: UseCollaborationOptions) {
  ...
  useEffect(() => {
    ...
    // Join room
    socket.emit('room:join', { room, metadata: { username } }, (response: any) => {
      if (response?.success) {
        setJoined(true);
        // Query initial presence
        socket.emit('query:presence', { room }, (res: { presence: PresenceInfo[] }) => {
          if (res?.presence) {
    ...
    // Listeners
    const handlePresenceUpdate = (data: { room: string; presence: PresenceInfo[] }) => {
      if (data.room === room) {
    ...
    const handleCursorUpdate = (data: { userId: string; x: number; y: number; username?: string }) => {
      setCursors((prev) => ({
        ...prev,
        [data.userId]: {
          userId: data.userId,

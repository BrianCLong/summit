  ...
  return useMutation({
    mutationFn: async (update: {
      ...
      const response = await fetch(
        `/api/integrations/jira/issues/${update.issueKey}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
...
export const useJIRASprintManagement = (projectKey: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/sprints?projectKey=${projectKey}`
      )
      return response.json()
    },
...
export const useJIRABurndown = (sprintId: number) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/burndown?sprintId=${sprintId}`
      )
      return response.json()
    },
...
export const useJIRAVelocity = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/velocity?projectKey=${projectKey}&sprintCount=${sprintCount}`
      )
      return response.json()
    },
...
export const useJIRACycleTime = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/cycle-time?projectKey=${projectKey}&timeRange=${timeRange}`
      )
      return response.json()
    },
    refetchInterval: 3600000, // 1 hour
...
export const useJIRAQualityMetrics = (projectKey: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/quality?projectKey=${projectKey}`
      )
      return response.json()
    },
...
export const useJIRAWebhooks = (projectKey: string) => {
  ...
  useEffect(() => {
    const eventSource = new EventSource(
      `/api/integrations/jira/webhooks/stream?projectKey=${projectKey}`
    )
...
export const useJIRAProjectConfig = (projectKey: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/project/${projectKey}/config`
      )
      return response.json()
    },
...
export const useJIRAWorkload = (projectKey: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/workload?projectKey=${projectKey}`
      )
      return response.json()
    },
...
export const useJIRATimeTracking = (
  projectKey: string,
  timeRange: string = '30d'
) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/time-tracking?projectKey=${projectKey}&timeRange=${timeRange}`
      )
      return response.json()
    },
    refetchInterval: 3600000, // 1 hour
...
export const useJIRADashboard = (dashboardId?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/integrations/jira/dashboard${dashboardId ? `/${dashboardId}` : ''}`
      )
      return response.json()
    },
    staleTime: 1800000, // 30 minutes

...
export function useCollaborativeWorkspace(roomId: string, userId: string, username?: string) {
  ...
  const annotationsRef = useRef<ORSet<Annotation>>(new ORSet<Annotation>(userId));
  ...
  useEffect(() => {
    ...
    const handleCursorUpdate = (data: { connectionId: string; userId: string; x: number; y: number; username?: string }) => {

...
export default function Investigations() {
  ...
  const handleEntitySelect = useCallback((entity: Entity) => {
    ...
    setAnnouncement(
      `Selected entity: ${entity.name}, type: ${entity.type}. ${entity.properties?.drift ? 'Warning: This entity is in drift state.' : ''}`
    )
  }, [])
  ...
  return (
    <div
      ...
      data-testid="investigations-page"
    >
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <main
          className="flex-1 relative min-h-[300px] min-w-0 bg-slate-100 p-2 lg:p-4"
          role="main"
        >
          ...
          {selectedEntity ? (
            <div className="p-4 lg:p-6 space-y-4 lg:space-y-6">
              <div className="flex items-start justify-between">
                ...
                {selectedEntity.properties?.drift && (
                  <Badge
                    variant="destructive"
                    className="flex gap-1 items-center px-2 animate-pulse"
                  >
                  ...
                  <div className="bg-slate-50 rounded-lg p-3 border border-slate-100 space-y-2">
                    {Object.entries(selectedEntity.properties).map(
                      ([key, value]) => (
                        <div
                          key={key}
                          className="flex justify-between text-sm items-center border-b border-slate-200/50 pb-1 last:border-0 last:pb-0"
                        >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 lg:p-12 text-center text-slate-400">
              <div className="w-12 h-12 lg:w-16 lg:h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4 border border-slate-100">
                <Info
                  className="h-6 w-6 lg:h-8 lg:w-8 text-slate-300"
                  aria-hidden="true"
                />
              </div>
              ...
              <p className="text-xs lg:text-sm text-slate-500 max-w-[200px]">
                Select a node on the Org Mesh to view detailed telemetry and
                drift status.
              </p>
            </div>
          )}

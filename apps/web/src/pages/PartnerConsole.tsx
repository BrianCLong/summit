...
export default function PartnerConsole() {
  ...
  const [newTenant, setNewTenant] = useState({ name: '', slug: '', residency: 'US' });
            ...
            <DialogContent>
                ...
                <div className="space-y-4 py-4">
                    <div className="space-y-2">
                        ...
                        <Input id="name" value={newTenant.name} onChange={e => setNewTenant({...newTenant, name: e.target.value})} placeholder="Acme Corp" />
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="slug">Slug (URL)</Label>
                        <Input id="slug" value={newTenant.slug} onChange={e => setNewTenant({...newTenant, slug: e.target.value})} placeholder="acme" />
                    </div>
                    <div className="space-y-2">
                        ...
                        <select
                            ...
                            onChange={e => setNewTenant({...newTenant, residency: e.target.value})}
                        >
            ...
            <TableBody>
              {tenants.map((tenant) => (
                <TableRow key={tenant.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline">{tenant.settings.policy_profile || 'None'}</Badge>
                  </TableCell>
                  <TableCell className="text-right">
                    <Dialog open={isPolicyDialogOpen && selectedTenant === tenant.id} onOpenChange={(open) => {
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Apply Policy Profile - {tenant.name}</DialogTitle>
                        </DialogHeader>

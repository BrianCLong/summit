...
import { Entity, Relationship, TimelineEvent, GeospatialEvent, AlertSeverity } from '@/types'
...
export default function AdvancedDashboardPage() {
  ...
  const [timeWindow, setTimeWindow] = useState<{ start: Date; end: Date } | undefined>()
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (timeWindow) {
      ...
      timelineEvents.forEach(e => { if(e.entityId) activeEntityIds.add(e.entityId) })
  ...
  return (
    <div className="h-full flex flex-col p-4 space-y-4 bg-muted/10 overflow-hidden">
      ...
      <div className={cn(
        ...
        expandedWidget ? "grid-cols-1 grid-rows-1" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-12 lg:grid-rows-2"
      )}>

        {/* Graph Widget */}
        {visibleWidgets.graph && (
          <div className={cn(
             expandedWidget === 'graph' ? "fixed inset-4 z-50 shadow-2xl bg-background rounded-lg border" : "lg:col-span-8 lg:row-span-2 md:col-span-1",
        ...
        {visibleWidgets.map && (
           <div className={cn(
             expandedWidget === 'map' ? "fixed inset-4 z-50 shadow-2xl bg-background rounded-lg border" : "lg:col-span-4 lg:row-span-1 md:col-span-1",
        ...
        {visibleWidgets.matrix && (
           <div className={cn(
             expandedWidget === 'matrix' ? "fixed inset-4 z-50 shadow-2xl bg-background rounded-lg border" : "lg:col-span-4 lg:row-span-1 md:col-span-1",
      ...
      {/* Details & Timeline Row */}
       <div className={cn(
          ...
          {visibleWidgets.timeline && (
             <div className={cn("h-full", visibleWidgets.details ? "lg:col-span-9" : "lg:col-span-12")}>
                <CustomizableWidget
                ...
                >
                   <TimelineRail
                      ...
                      onTimeRangeChange={(range) => setTimeWindow({ start: new Date(range.start), end: new Date(range.end) })}
                      className="border-0"
                   />
                </CustomizableWidget>
             </div>
          ...
          {visibleWidgets.details && (
              <div className={cn("h-full", visibleWidgets.timeline ? "lg:col-span-3" : "lg:col-span-12")}>
                  <CustomizableWidget
                      title="Details"
                  ...
                  >
                      {selectedEntity ? (
                          <div className="p-4 space-y-2">
                              ...
                              <div className="pt-2">
                                  <Button size="sm" variant="secondary" className="w-full">View Full Profile</Button>
                              </div>
                          </div>
                      ) : (

...
import { useIntelligenceSocket, IntelligenceItem, IntelligenceAlert } from '../../hooks/useIntelligenceSocket';
...
import { AlertTriangle, Wifi, WifiOff, Activity, Shield, Globe } from 'lucide-react';
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen text-slate-900">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Live Feed Volume</CardTitle>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-4 h-[600px] flex flex-col">
          ...
          <CardContent className="flex-1 overflow-auto space-y-4">
            {items.map((item) => (
              <div key={item.id} className="flex flex-col p-4 border rounded-lg hover:bg-slate-50 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center space-x-2">
                    <Badge variant={
                      item.type === 'darkweb' ? 'destructive' :
        ...
        <div className="col-span-3 space-y-6">
            ...
            <CardContent className="h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <YAxis domain={[0, 100]} />
                  <Tooltip />
                  <Line type="monotone" dataKey="threatScore" stroke="#ef4444" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <Card className="h-[276px] flex flex-col">
            ...
            <CardContent className="flex-1 overflow-auto space-y-3">
               {alerts.map((alert) => (
                <div key={alert.id} className="p-3 border border-red-100 bg-red-50 rounded-md">

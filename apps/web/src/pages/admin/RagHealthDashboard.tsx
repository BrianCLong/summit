...
export default function RagHealthDashboard() {
  ...
  const [graphRagData, setGraphRagData] = useState<GraphRagHealthData | null>(
    null
  )
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              <Badge
                variant={
                  ragData?.status === 'healthy' ? 'success' : 'destructive'
                }
              >
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              <Badge
                variant={
                  graphRagData?.status === 'healthy' ? 'success' : 'destructive'
                }
              >
            ...
            <p className="text-xs text-muted-foreground">
              Uptime:{' '}
              {graphRagData?.uptime
                ? Math.round(graphRagData.uptime / 60) + 'm'
                : 'N/A'}
            </p>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4">
          <CardHeader>
            ...
            <CardDescription>
              Real-time latency metrics for RAG and GraphRAG pipelines
            </CardDescription>
          </CardHeader>
        ...
        <Card className="col-span-3">
          ...
          <CardContent>
            <div className="space-y-4">
              {ragData?.components &&
                Object.entries(ragData.components).map(([key, value]) => (
              ...
              {graphRagData?.components &&
                Object.entries(graphRagData.components).map(([key, value]) => (
                  <div
                    key={`graph-${key}`}

...
const mockRuns: Run[] = Array.from({ length: 50 }, (_, i) => {
  ...
  return {
    ...
    status,
    startTime,
    endTime: status === 'completed' || status === 'failed' ? new Date(startTime.getTime() + Math.random() * 3600000) : undefined,
    duration: status === 'completed' || status === 'failed' ? Math.floor(Math.random() * 3600) : undefined,
    ...
    steps: Array.from({ length: 5 }, (_, j) => ({
      id: `step_${j + 1}`,
      name: `Step ${j + 1}`,
      status: j < 3 ? 'completed' : j === 3 && status === 'running' ? 'running' : 'queued',
...
function RunsTable() {
  ...
  const getStatusColor = (status: Run['status']) => {
    switch (status) {
      ...
      case 'running': return 'text-blue-700 bg-blue-50 ring-blue-600/20';
      case 'failed': return 'text-red-700 bg-red-50 ring-red-600/20';
      case 'cancelled': return 'text-gray-700 bg-gray-50 ring-gray-600/20';
      case 'queued': return 'text-yellow-700 bg-yellow-50 ring-yellow-600/20';
...
function RunDetailView({ run }: { run: Run }) {
  const [activeTab, setActiveTab] = useState<'overview' | 'dag' | 'logs' | 'artifacts'>('overview');
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      ...
                      <dd className="text-lg font-medium text-gray-900">
                        {run.duration ? `${Math.floor(run.duration / 60)}m ${run.duration % 60}s` : 'In progress'}
                      </dd>
                    </dl>
                  </div>
                </div>
            ...
            <div className="bg-gray-50 overflow-hidden rounded-lg">
              <div className="p-5">
                <div className="flex items-center">
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      <dt className="text-sm font-medium text-gray-500 truncate">Steps</dt>
                      <dd className="text-lg font-medium text-gray-900">
...
function OverviewTab({ run }: { run: Run }) {
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg leading-6 font-medium text-gray-900">Run Information</h3>
        <div className="mt-5 border-t border-gray-200">
          <dl className="divide-y divide-gray-200">
            ...
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
              ...
              <dd className="mt-1 flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                <span className="flex-grow">{run.startTime.toLocaleString()}</span>
              </dd>
            </div>
            {run.endTime && (
              <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                ...
                <dd className="mt-1 flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  <span className="flex-grow">{run.endTime.toLocaleString()}</span>
                </dd>
              </div>
            )}
          </dl>
...
function DAGTab({ run }: { run: Run }) {
  return (
    <div>
      <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Execution Graph</h3>
      <div className="space-y-4">
        {run.steps.map((step, index) => (
          <div key={step.id} className="flex items-center space-x-4">
            ...
            <div className={`flex-shrink-0 w-20 h-2 rounded-full ${
...
function LogsTab({ run }: { run: Run }) {
  return (
    <div>
      <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Execution Logs</h3>
        ...
        <div className="space-y-1">
          {run.steps.flatMap((step, stepIndex) => [
            <div key={`${step.id}-start`} className="text-blue-400">
              [{step.startTime?.toLocaleTimeString()}] Starting step: {step.name}
            </div>,
            ...Array.from({ length: Math.floor(Math.random() * 5) + 2 }, (_, logIndex) => (
              <div key={`${step.id}-log-${logIndex}`} className="text-gray-300">
                [{step.startTime?.toLocaleTimeString()}] Processing... {Math.random().toString(36).slice(2)}
              </div>
            )),
            <div key={`${step.id}-end`} className={step.status === 'completed' ? 'text-green-400' : step.status === 'failed' ? 'text-red-400' : 'text-yellow-400'}>
              [{step.endTime?.toLocaleTimeString() || 'In progress'}] Step {step.status}: {step.name}
            </div>
          ])}
        </div>
      </div>
...
function ArtifactsTab({ run }: { run: Run }) {
  ...
  return (
    <div>
      <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Generated Artifacts</h3>
          ...
          <div key={index} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{artifact.size} • {artifact.type}</p>
              </div>
            </div>

...
const mockRuns = Array.from({ length: 50 }, (_, i) => {
    ...
    const triggers = ['manual', 'scheduled', 'webhook', 'api'];
    ...
    return {
        ...
        steps: Array.from({ length: 5 }, (_, j) => ({
            ...
            endTime: j < 3 ? new Date(startTime.getTime() + (j + 1) * 300000) : undefined,
...
function RunsTable() {
    ...
    const { hasPermission } = (0, TenantContext_1.useTenant)();
    const { showNotification } = (0, NotificationContext_1.useNotification)();
    const isDemoMode = (0, DemoIndicator_1.useDemoMode)();
    const [runs, setRuns] = (0, react_1.useState)(isDemoMode ? mockRuns : []);
    const [searchQuery, setSearchQuery] = (0, react_1.useState)('');
    const [statusFilter, setStatusFilter] = (0, react_1.useState)('all');
    const [selectedRuns, setSelectedRuns] = (0, react_1.useState)([]);
    const filteredRuns = (0, react_1.useMemo)(() => {
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            case 'running':
                return outline_1.PlayIcon;
            case 'failed':
                return outline_1.XCircleIcon;
            case 'cancelled':
                return outline_1.StopIcon;
            case 'queued':
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        ...
        <div className="flex space-x-3">
          <button onClick={() => navigate('/maestro/runbooks')} className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
      ...
      <DataIntegrityNotice_1.DataIntegrityNotice mode={isDemoMode ? 'demo' : 'unavailable'} context="Maestro runs"/>

      {!isDemoMode && runs.length === 0 ? (<EmptyState_1.EmptyState icon="activity" title="No live runs" description="Connect the Maestro runtime to display pipeline executions."/>) : (<>
      {/* Search and Filters */}
      <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
        <div className="flex-1">
          <div className="relative">
            <outline_1.MagnifyingGlassIcon className="h-5 w-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"/>
            <input type="text" placeholder="Search runs..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
          </div>
        </div>

        <div className="flex space-x-2">
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      {/* Bulk Actions */}
      {selectedRuns.length > 0 && (<div className="bg-blue-50 border border-blue-200 rounded-md p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="space-x-2">
              <button onClick={() => handleBulkAction('cancel')} className="text-sm text-red-600 hover:text-red-800">
                Cancel Selected
              </button>
              <button onClick={() => handleBulkAction('retry')} className="text-sm text-blue-600 hover:text-blue-800">
                Retry Selected
              </button>
            </div>
          </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                <input type="checkbox" checked={selectedRuns.length === filteredRuns.length} onChange={e => {
                if (e.target.checked) {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredRuns.map(run => {
                ...
                return (<tr key={run.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input type="checkbox" checked={selectedRuns.includes(run.id)} onChange={e => {
                        if (e.target.checked) {
                            setSelectedRuns(prev => [...prev, run.id]);
                        }
                        else {
                            setSelectedRuns(prev => prev.filter(id => id !== run.id));
                        }
                    }}/>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${getStatusColor(run.status)}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex space-x-2">
                      <button onClick={() => navigate(`/maestro/runs/${run.id}`)} className="text-blue-600 hover:text-blue-900" title="View Details">
                        <outline_1.EyeIcon className="h-4 w-4"/>
                      </button>

                      {run.status === 'running' && (<button onClick={() => handleRunAction(run.id, 'pause')} className="text-yellow-600 hover:text-yellow-900" title="Pause">
                          <outline_1.PauseIcon className="h-4 w-4"/>
                        </button>)}

                      {(run.status === 'running' ||
                        run.status === 'queued') && (<button onClick={() => handleRunAction(run.id, 'cancel')} className="text-red-600 hover:text-red-900" title="Cancel">
                          <outline_1.StopIcon className="h-4 w-4"/>
                        </button>)}

                      {(run.status === 'failed' ||
                        run.status === 'cancelled') && (<button onClick={() => handleRunAction(run.id, 'retry')} className="text-green-600 hover:text-green-900" title="Retry">
                          <outline_1.ArrowPathIcon className="h-4 w-4"/>
                        </button>)}
                    </div>
                  </td>
                </tr>);
...
function RunDetail() {
    const { runId } = (0, react_router_dom_1.useParams)();
    const isDemoMode = (0, DemoIndicator_1.useDemoMode)();
...
function RunDetailView({ run }) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white shadow sm:rounded-lg">
        <div className="px-4 py-5 sm:px-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <span className={`inline-flex items-center rounded-md px-3 py-2 text-sm font-medium ring-1 ring-inset ${getStatusColor(run.status)}`}>
                {run.status}
              </span>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-3">
              ...
              <div className="p-5">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    <outline_1.ClockIcon className="h-6 w-6 text-gray-400"/>
                  </div>
            ...
            <div className="bg-gray-50 overflow-hidden rounded-lg">
              <div className="p-5">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    <outline_1.DocumentArrowDownIcon className="h-6 w-6 text-gray-400"/>
                  </div>
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      <dt className="text-sm font-medium text-gray-500 truncate">
            ...
            <div className="bg-gray-50 overflow-hidden rounded-lg">
              <div className="p-5">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    <outline_1.CurrencyDollarIcon className="h-6 w-6 text-gray-400"/>
                  </div>
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      <dt className="text-sm font-medium text-gray-500 truncate">
        ...
        ].map(tab => (<button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === tab.key
      ...
      <div>
        ...
        <div className="mt-5 border-t border-gray-200">
          <dl className="divide-y divide-gray-200">
            ...
            {run.endTime && (<div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
...
function DAGTab({ run }) {
    return (<div>
      ...
      <div className="space-y-4">
        {run.steps.map((step, index) => (<div key={step.id} className="flex items-center space-x-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {step.duration && (<p className="text-sm text-gray-500">
                  Duration: {Math.floor(step.duration / 60)}m{' '}
                  {step.duration % 60}s
                </p>)}
            </div>

            <div className={`flex-shrink-0 w-20 h-2 rounded-full ${step.status === 'completed'
                ? 'bg-green-200'
                : step.status === 'running'
                    ? 'bg-blue-200'
                    : step.status === 'failed'
...
function LogsTab({ run }) {
    const logs = (0, react_1.useMemo)(() => buildDeterministicLogs(run), [run]);
    const levelClasses = {
        info: 'text-blue-400',
        log: 'text-gray-300',
    ...
    return (<div>
      ...
      <div className="bg-gray-900 rounded-lg p-4 font-mono text-sm text-green-400 h-96 overflow-y-auto">
        <div className="space-y-1">
          {logs.map(log => (<div key={log.key} className={levelClasses[log.level]}>
              {log.text}
            </div>))}
        </div>
      </div>
...
function buildDeterministicLogs(run) {
...
function ArtifactsTab({ run }) {
    ...
    return (<div>
      ...
      <div className="space-y-3">
        {artifacts.map((artifact, index) => (<div key={index} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
            <div className="flex items-center space-x-3">
              <outline_1.DocumentArrowDownIcon className="h-5 w-5 text-gray-400"/>
              <div>
...
function Runs() {
    return (<div className="p-6">
      <react_router_dom_1.Routes>
        <react_router_dom_1.Route index element={<RunsTable />}/>
        <react_router_dom_1.Route path=":runId" element={<RunDetail />}/>
      </react_router_dom_1.Routes>
    </div>);
}
// Helper functions

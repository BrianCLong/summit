...
const checkPreflight = async (action: string) => {
  ...
  if (action.includes('delete')) {
    return {
      allow: false,
      reason:
        'Deletion of critical resources is restricted by policy "No-Data-Loss".',
    }
  }
  return {
    allow: true,
    reason: 'Action permitted by policy "Standard-Change-Control".',
  }
}
...
export default function ApprovalsPage() {
  ...
  const submitDecision = () => {
    if (!rationale) return
    console.log(
      `Submitted ${actionType} for ${selectedRequest.id} with rationale: ${rationale}`
    )
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Review and manage privileged access requests.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-lg border shadow-sm">
        ...
        {MOCK_REQUESTS.map(req => (
          <div
            key={req.id}
            className="grid grid-cols-6 gap-4 p-4 border-b last:border-0 items-center hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors"
          >
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                className="p-1.5 hover:bg-zinc-100 text-zinc-600 rounded transition-colors"
                title="View Evidence"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-xl max-w-md w-full p-6 animate-in fade-in zoom-in-95">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              You are about to {actionType} request{' '}
              <strong>{selectedRequest.id}</strong>. Please provide a rationale
              for this decision.
            </p>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 mt-6">
                ...
                <button
                  ...
                  disabled={
                    !rationale.trim() ||
                    (actionType === 'approve' && loadingPreflight)
                  }

...
import type { ChecklistItem, EvidenceLink, StatusLevel, StatusResponse } from '../types'
import { cn } from '@/lib/utils'

const statusCopy: Record<StatusLevel, { emoji: string; label: string; tone: 'success' | 'warning' | 'destructive' }> = {
...
const Checklist = ({ items }: { items: ChecklistItem[] }) => (
  <div className="space-y-2">
    {items.map(item => (
      ...
      >
        {item.status === 'green' && <CheckCircle2 className="h-4 w-4 text-emerald-600" />}
        {item.status === 'yellow' && <AlertTriangle className="h-4 w-4 text-amber-600" />}
        {item.status === 'red' && <AlertTriangle className="h-4 w-4 text-red-600" />}
...
export function StatusPanel({ title, status, fallbackTitle, children }: StatusPanelProps) {
              ...
              {status.signals.map(signal => (
                <div
                  ...
                  className={cn(
                    ...
                    signal.status === 'green' && 'border-emerald-200 bg-emerald-50',
                    signal.status === 'yellow' && 'border-amber-200 bg-amber-50',

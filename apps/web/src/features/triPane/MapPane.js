 ...
 */
function MapPane({ locations, center = [0, 0], zoom = 2, selectedLocationId, onLocationSelect, onMapMove, className, }) {
    const [currentCenter, setCurrentCenter] = (0, react_1.useState)(center);
    const [currentZoom, setCurrentZoom] = (0, react_1.useState)(zoom);
    ...
    return (<div className={(0, utils_1.cn)('relative w-full h-full overflow-hidden', className)} role="region" aria-label="Geographic map view">
      ...
      <div className="absolute inset-0" style={{ transform: `scale(${currentZoom / 2})`, transformOrigin: 'center' }}>
        {Object.entries(groupedLocations).map(([coordKey, events]) => {
            ...
            const isSelected = events.some((e) => e.location.id === selectedLocationId);
            ...
            return (<div key={coordKey} className="absolute transform -translate-x-1/2 -translate-y-full cursor-pointer transition-all duration-200" style={{
                ...
                }} onClick={() => onLocationSelect?.(firstEvent.location.id)} onMouseEnter={() => setHoveredLocationId(firstEvent.location.id)} onMouseLeave={() => setHoveredLocationId(null)} role="button" tabIndex={0} aria-label={`Location marker: ${firstEvent.location.name}, ${events.length} events`} onKeyDown={(e) => {
              ...
              <div className="relative">
                <lucide_react_1.MapPin className={(0, utils_1.cn)('w-8 h-8 drop-shadow-lg transition-all', getSeverityColor(maxSeverity), isSelected && 'ring-4 ring-yellow-400 ring-offset-2', isHovered && 'scale-125')} fill="currentColor"/>
                {events.length > 1 && (<Badge_1.Badge variant="secondary" className="absolute -top-2 -right-2 h-5 w-5 p-0 flex items-center justify-center text-xs">
                    {events.length}
                  </Badge_1.Badge>)}
              </div>

              {/* Tooltip on hover */}
              {(isHovered || isSelected) && (<div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-background border rounded-lg shadow-lg p-3 min-w-48 z-50">
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    <Badge_1.Badge variant={maxSeverity === 'critical' ? 'destructive' : 'secondary'} className="text-xs">
                      {maxSeverity}
                    </Badge_1.Badge>
                  </div>
                  {events.length > 0 && (<div className="mt-2 pt-2 border-t text-xs">
                      <div className="font-medium mb-1">Recent Events:</div>
                      {events.slice(0, 3).map((event) => (<div key={event.id} className="text-muted-foreground truncate">
                          • {event.type.replace('_', ' ')}
                        </div>))}
                      {events.length > 3 && (<div className="text-muted-foreground">
      ...
      <div className="absolute bottom-4 left-4 bg-background/90 backdrop-blur-sm border rounded-lg shadow-lg p-3">
        ...
        ].map(({ level, color }) => (<div key={level} className="flex items-center gap-2 text-xs">
      ...
      {/* Empty state */}
      {locations.length === 0 && (<div className="absolute inset-0 flex items-center justify-center">

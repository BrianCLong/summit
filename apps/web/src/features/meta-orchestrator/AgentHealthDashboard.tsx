        ...
        {agents.map(agent => (
          <Card key={agent.id}>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <div
                className={`h-3 w-3 rounded-full ${getStatusColor(agent.status)}`}
                title={agent.status}
              />
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <Progress
                    value={(agent.health.memoryUsage / 2048) * 100}
                    className="h-2"
                  />
                </div>
                ...
                <div className="text-xs text-muted-foreground pt-2">
                  Last heartbeat:{' '}
                  {new Date(agent.health.lastHeartbeat).toLocaleTimeString()}
                </div>
              </div>
            </CardContent>
          </Card>

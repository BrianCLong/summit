...
import {
  ...
  type BranchConvergenceInfo,
  type DiffFile,
  type PRItem,
  type PRPriority,
  type PRTriageAdapter,
...
const cleanConvergence = (behind = 0): BranchConvergenceInfo => ({
  mergesCleanly: true,
...
const conflictConvergence = (deprecated: string[] = []): BranchConvergenceInfo => ({
  mergesCleanly: false,
  behindByCommits: 14,
...
const sampleDiff = (path: string, adds: number, dels: number): DiffFile => ({
  path,
  additions: adds,
  deletions: dels,
...
const baseMockPRs: PRItem[] = [
  {
    ...
    labels: ['feature', 'api'],
    description:
      'Exposes a `/convergence` endpoint returning merge-readiness and deprecated-branch lists per PR.',
    riskChecks: [
  ...
  },
  {
    ...
    description:
      'Resolves the conflict introduced by PR #99 which touched the same OPA rule file.',
    riskChecks: [
  ...
  },
  {
    ...
    labels: ['chore', 'deps'],
    description:
      'Vite 6 drops some legacy plugins. This PR migrates config and removes the deprecated `ssr` flag.',
    riskChecks: [
      { id: 'policy-violation', label: 'Policy violations', passed: true, riskLevel: 'none' },
      { id: 'missing-timestamps', label: 'Missing timestamps', passed: true, riskLevel: 'none' },
      { id: 'missing-provenance', label: 'Provenance evidence', passed: true, riskLevel: 'none' },
  ...
  },
  {
    ...
    description:
      'Adds the PR triage workspace: queue view, diff preview, risk checklist, and branch convergence integration.',
    riskChecks: [
      { id: 'policy-violation', label: 'Policy violations', passed: true, riskLevel: 'none' },
...
export function usePRTriageAdapter(): PRTriageAdapter {
  const [prs, setPRs] = useState<PRItem[]>(() =>
    loadFromStorage(STORAGE_KEYS.prs, baseMockPRs)
  )
  ...
  const list = useCallback(
    async (filters: PRTriageFilters = defaultPRTriageFilters): Promise<PRItem[]> => {
  ...
  const get = useCallback(
    async (id: string) => prs.find(pr => pr.id === id),
    [prs]
  )
  ...
  const act = useCallback(
    async (
      id: string,
      action: TriageAction,
      opts: { comment?: string; assignedTo?: string } = {}
    ) => {
      ...
      if (action === 'assign' && opts.assignedTo) {
        setPRs(prev =>
          prev.map(pr => pr.id === id ? { ...pr, assignee: opts.assignedTo } : pr)
        )
      }
...
export const prTriageEnums = {
  statuses: ['merge-ready', 'conflict', 'needs-owner-review', 'blocked-on-governance'] as PRTriageStatus[],
  priorities: ['critical', 'high', 'medium', 'low'] as PRPriority[],
}

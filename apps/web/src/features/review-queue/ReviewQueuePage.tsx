...
import { defaultQueueFilters, type QueueAction, type QueueFilters, type QueueItem } from './types'
import { useShortcut } from '@/contexts/KeyboardShortcutsContext'

const PaneContainer: React.FC<{ children: React.ReactNode; title: string; right?: React.ReactNode; className?: string }> = ({ children, title, right, className }) => (
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className={cn('text-[11px] px-2 py-0.5 rounded-full capitalize', statusTone[item.status])}>
          {item.status}
        </span>
        </div>
      </div>
    ...
    <div className="space-y-3">
      ...
      {item.preview.snippet && (
        <div className="space-y-2">
          <Label className="text-xs uppercase tracking-wide text-muted-foreground">Evidence</Label>
      ...
      {item.preview.entityDiff && (
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-xs uppercase tracking-wide text-muted-foreground">Before</Label>
            <pre className="mt-2 rounded-md bg-muted/40 p-3 text-xs whitespace-pre-wrap border">{item.preview.entityDiff.before}</pre>
          </div>
          <div>
            <Label className="text-xs uppercase tracking-wide text-muted-foreground">After</Label>
      ...
      {item.preview.policyWarning && (
        <div className="space-y-2">
          <Label className="text-xs uppercase tracking-wide text-muted-foreground">Policy warning</Label>
...
const ActionsPane: React.FC<{
  ...
  return (
    <div className="flex flex-col gap-3">
      <div>
        <Label className="text-xs uppercase tracking-wide text-muted-foreground">Decision</Label>
...
const FiltersPane: React.FC<{
  ...
  const update = (partial: Partial<QueueFilters>) => onChange({ ...filters, ...partial })

  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-sm font-semibold">
        <Filter className="h-4 w-4" /> Filters
        <Badge variant={filters.status === 'open' ? 'default' : 'secondary'} className="text-[11px]">{counts.open} open</Badge>
        <Badge variant={filters.status === 'resolved' ? 'default' : 'secondary'} className="text-[11px]">{counts.resolved} resolved</Badge>
      </div>
...
export const ReviewQueuePage: React.FC = () => {
  ...
  useShortcut('a', () => handleAct('approve'), { id: 'queue-approve', description: 'Approve item', category: 'Actions' })
  useShortcut('r', () => handleAct('reject'), { id: 'queue-reject', description: 'Reject item', category: 'Actions' })

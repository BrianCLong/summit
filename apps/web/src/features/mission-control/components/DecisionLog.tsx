      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
             {decisions.length === 0 ? (
                 <TableRow>
                  <TableCell colSpan={5} className="text-center text-muted-foreground">No decisions recorded.</TableCell>
                </TableRow>
            ) : (
                decisions.map((dec: any, idx: number) => (
                    ...
                    <TableCell>{dec.date ? new Date(dec.date).toLocaleDateString() : 'N/A'}</TableCell>
                    <TableCell>

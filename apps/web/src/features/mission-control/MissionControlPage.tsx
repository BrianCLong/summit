...
export default function MissionControlPage() {
  ...
  const { data: listData, loading: listLoading } = useQuery(GET_STRATEGIC_PLANS, {
    variables: { filter: { status: 'ACTIVE' } },
  });

  const { data: detailData, loading: detailLoading } = useQuery(GET_STRATEGIC_PLAN_DETAILS, {
  ...
  return (
    <div className="container mx-auto p-6 space-y-8 min-h-screen bg-background">
      ...
      <HealthSignals progress={detailData?.planProgress} kpis={detailData?.strategicPlan?.kpis} />
      ...
      <Tabs defaultValue="strategy" className="w-full">
        ...
        <TabsContent value="strategy" className="mt-6">
          <StrategyWall plan={detailData?.strategicPlan} loading={detailLoading} />
        </TabsContent>

...
describe('Secret Scanner', () => {
  it('detects AWS keys', () => {
    const finding = scanForSecrets('aws_secret_key = "abcd1234abcd1234abcd1234abcd1234abcd1234"', 'config.ts', 10);
...
describe('Reviewer', () => {
  ...
  it('flags unpaginated list', () => {
    const mockDiff = [{
      ...
      chunks: [{
        content: '...',
        changes: [{ type: 'add', content: 'const users = db.users.findAll();', ln2: 30 } as any]
      }]
    }];
    const reviewer = new Reviewer(mockDiff as any);
    const findings = reviewer.review();

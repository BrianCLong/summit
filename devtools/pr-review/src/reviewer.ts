...
export class Reviewer {
  ...
  private checkMissingTests() {
    const sourceFiles = this.diffs
      .filter(
        (d) => d.to && d.to.match(/\.(ts|js|py|go|rs)$/) && !d.to.match(/\.test\.|_test\.|spec\./)
      )
      .map((d) => d.to!);
  ...
  private checkPagination(content: string, file: string, line?: number) {
    ...
    if (
      content.match(/(findAll|list|search|query)\(/i) &&
      !content.match(/(limit|offset|page|cursor)/i)
    ) {
      this.findings.push({
        type: "style",
        severity: "info",

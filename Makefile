...
.PHONY: bootstrap up down logs ps server client ingest graph smoke clean reset-db dev dev-down dev-reseed ingest-assets up-local logs-local down-local demo
...
dev: ## Launch devkit stack and seed fixtures
	npm run devkit:stack
	@npm run devkit:seed

dev-down: ## Stop devkit stack
	npm run devkit:down || true

dev-reseed: ## Re-run fixture seeding against a running stack
	@npm run devkit:seed

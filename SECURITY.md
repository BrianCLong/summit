...
### Malfunction Amplification (Agent Runtime)

- **Threat:** Prompt-driven loops, irrelevant tool steering, and downstream propagation that stall autonomous agents without triggering traditional policy-violation checks.
- **Controls:**
  - **Progress + Repetition Guard:** Runtime circuit breaker on repeated identical/near-duplicate actions, no-progress windows, and per-goal tool-call budgets with structured incidents and replan requirements.
  - **Self-check Prompts:** Dual prompts (policy-violation and malfunction-intent) executed on guard triggers and on inbound instructions from tools/agents; results recorded in provenance.
  - **Red-team Harness:** Regression harness targeting loop induction and irrelevant-tool injections; CI fails if failure rate regresses beyond baseline.
- **Runbook:**
  - Execute the harness locally with `pnpm --filter workcell-runtime test -- --run tests/redteam-harness.test.ts`.
  - Review guard incidents and self-check evidence in the provenance ledger category `workcell-guard` / `workcell-self-check`.

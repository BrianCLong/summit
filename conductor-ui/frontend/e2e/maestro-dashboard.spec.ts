    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        data: {
          ...
          approvals: [],
          changes: [
            { at: '2025-01-15T10:00:00Z', title: 'Updated pipeline config', by: 'alice@example.com' },
            { at: '2025-01-15T09:30:00Z', title: 'Deployed v2.1.0', by: 'bob@example.com' }
          ]
        }
      })
    });
...
test.describe('Maestro Dashboard', () => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          data: {
            ...
            duration: 120,
            steps: [
              { id: 'checkout', name: 'Checkout Code', status: 'completed', duration: 10 },
              { id: 'build', name: 'Build Application', status: 'running', duration: 90 },
  ...
  test('should show observability dashboard', async ({ page }) => {
    ...
    await expect(page.locator('h2:has-text("Observability & SLOs")')).toBeVisible();

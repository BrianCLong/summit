...
export default function TenantSLOChart({ tenant }: { tenant: string }) {
  ...
  return (
    <div className="rounded-2xl border p-3">
      <div className="mb-2 text-sm text-gray-600">Tenant “{tenant}” — burn over time</div>
      <div style={{ height: 260 }}>
        <ResponsiveContainer>
          <LineChart data={data}>
            ...
            <Line type="monotone" dot={false} dataKey="fastBurn" name="Fast burn (1h)" />
            <Line type="monotone" dot={false} dataKey="slowBurn" name="Slow burn (6h)" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>

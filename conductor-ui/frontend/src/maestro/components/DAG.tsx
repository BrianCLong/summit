...
export type DagNode = { id: string; label: string; state: 'queued'|'running'|'succeeded'|'failed'|'cancelled'; retries?: number; compensated?: boolean };
...
export default function DAG({ nodes, edges, onSelect }: { nodes: DagNode[]; edges: DagEdge[]; onSelect?: (id: string) => void }) {
    ...
    <div className="overflow-auto rounded border bg-white">
      <svg width={width} height={Math.max(400, nodes.length * laneH)}>
        ...
        {edges.map((e, i) => {
          ...
          return (
            <path key={i} d={`M ${a.x+60} ${a.y+20} C ${midX} ${a.y+20}, ${midX} ${b.y+20}, ${b.x-20} ${b.y+20}`} stroke="#cbd5e1" fill="none" strokeWidth={2} />
          );
        })}
        {/* nodes */}
        {nodes.map(n => {
          const p = pos.get(n.id)!; const color = stateColor(n.state);
          return (
            <g key={n.id} transform={`translate(${p.x},${p.y})`}>
              ...
              <text x={10} y={22} fontSize={12} fill="#0f172a">{n.label}</text>
              <text x={10} y={38} fontSize={11} fill="#475569">{n.state}</text>
              {n?.retries ? (
                <g>
                  <rect x={100} y={6} rx={4} width={30} height={16} fill="#0f172a" opacity={0.06} />
                  <text x={104} y={18} fontSize={11} fill="#334155">r{n.retries}</text>
                </g>
              ) : null}
              {n?.compensated ? (
                <g>
                  <rect x={70} y={6} rx={4} width={24} height={16} fill="#dc2626" opacity={0.12} />
                  <text x={74} y={18} fontSize={11} fill="#b91c1c">C</text>
                </g>
              ) : null}
            </g>
          );

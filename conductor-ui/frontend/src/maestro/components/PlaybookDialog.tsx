...
export default function PlaybookDialog({ open, onClose, sig, providerGuess }:{ open:boolean; onClose:()=>void; sig:string; providerGuess?:string }) {
  ...
  if (!open) return null;
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="pb-title" className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div ref={ref} className="w-[min(560px,95vw)] rounded-2xl bg-white p-4" onClick={e=>e.stopPropagation()}>
        <h2 id="pb-title" className="text-lg font-semibold">Playbook</h2>
        <div className="mt-2 text-sm text-gray-700">
          <div className="mb-1">Signature</div>
          <code className="break-all text-xs">{sig}</code>
          {providerGuess && <div className="mt-2 text-xs text-gray-500">Provider: {providerGuess}</div>}
        </div>

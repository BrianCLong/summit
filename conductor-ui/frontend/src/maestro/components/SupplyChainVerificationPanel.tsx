...
import { useSupplyChainVerification, SupplyChainVerificationResult } from '../utils/supplyChainVerification';
        ...
        <div className="bg-white border rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${
              result.sbomVerification?.valid ? 
                'bg-green-100 text-green-800' : 
                result.sbomVerification?.present ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
            }`}>
              {result.sbomVerification?.valid ? 'Valid' : 
               result.sbomVerification?.present ? 'Invalid' : 'Not Present'}
            </span>
          </div>
          ...
          {result.sbomVerification && (
            <div className="space-y-2 text-sm">
              ...
              {result.sbomVerification.vulnerabilities && result.sbomVerification.vulnerabilities.length > 0 && (
                <div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                result.slsaVerification?.valid ? 
                  'bg-green-100 text-green-800' : 
                  result.slsaVerification?.present ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
              }`}>
                {result.slsaVerification?.valid ? 'Valid' : 
                 result.slsaVerification?.present ? 'Invalid' : 'Not Present'}
              </span>
            </div>
          </div>
          ...
          {result.slsaVerification && result.slsaVerification.present && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 font-mono text-xs">{result.slsaVerification.buildPlatform}</span>
              </div>
              <div>
                <span className="text-gray-500">Source Repository:</span>
                <span className="ml-2 font-mono text-xs">{result.slsaVerification.sourceRepository}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm">
          <label className="flex items-center">
            <input
              ...
              onChange={(e) => setVerificationOptions(prev => ({
                ...prev,
      ...
      {reportData && (
        <div className="bg-white border-b px-6 py-4">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">{reportData.summary.withSLSA}</div>
              <div className="text-xs text-gray-500">With SLSA</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">{reportData.summary.criticalVulnerabilities}</div>
      ...
      <div className="bg-white">
        {artifacts.length === 0 ? (
          <div className="px-6 py-8 text-center text-gray-500">
            ...
            <p className="text-sm mt-1">Supply chain verification will appear here when artifacts are available.</p>
          </div>
        ) : (
              ...
              <div key={index} className="px-6 py-4">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-medium text-gray-900 truncate">{result.artifact}</h4>
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    ...
                    <button
                      onClick={() => setSelectedArtifact(selectedArtifact === result.artifact ? null : result.artifact)}

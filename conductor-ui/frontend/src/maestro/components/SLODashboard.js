...
exports.default = SLODashboard;
const react_1 = __importStar(require("react"));
const sloUtils_1 = require("../utils/sloUtils");
function SLODashboard({ service, className = '', }) {
    const { slos, loading, error, fetchSLOs, generateReport } = (0, sloUtils_1.useSLO)();
    const [selectedSLO, setSelectedSLO] = (0, react_1.useState)(null);
    ...
    const renderErrorBudgetBar = (errorBudget) => {
        ...
        <div className={`h-2 rounded-full transition-all duration-300 ${color}`} style={{ width: `${percentage}%` }}/>
      </div>);
    };
    const renderSLOCard = (sloData) => {
        ...
        return (<div key={slo.id} className={`bg-white border rounded-lg p-4 cursor-pointer transition-all duration-200 hover:shadow-md ${isSelected ? 'ring-2 ring-blue-500 border-blue-200' : ''}`} onClick={() => setSelectedSLO(isSelected ? null : slo.id)}>
        ...
        {errorBudget.exhaustionDate && (<div className="text-xs text-gray-500">
            Budget exhaustion:{' '}
            {new Date(errorBudget.exhaustionDate).toLocaleDateString()}
          </div>)}

        {isSelected && (<div className="mt-4 pt-3 border-t border-gray-200">
    ...
    if (loading && slos.length === 0) {
        return (<div className={`slo-dashboard ${className}`}>
    ...
    if (error) {
        ...
        <div className="text-center text-red-600 p-8">
          ...
          <button onClick={() => fetchSLOs(service ? { service } : undefined)} className="mt-4 px-4 py-2 bg-red-100 text-red-700 rounded hover:bg-red-200">
            Retry
          </button>
        </div>
      </div>);
    ...
    return (<div className={`slo-dashboard ${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-3">
          <select value={timeRange} onChange={(e) => setTimeRange(e.target.value)} className="border rounded px-3 py-1 text-sm">
          ...
          <button onClick={() => fetchSLOs(service ? { service } : undefined)} disabled={loading} className="px-3 py-1 bg-blue-600 text-white rounded text-sm hover:bg-blue-700 disabled:opacity-50 flex items-center">
            {loading && (<svg className="animate-spin -ml-1 mr-2 h-3 w-3 text-white" fill="none" viewBox="0 0 24 24">
      ...
      {/* Summary Statistics */}
      {reportData && (<div className="grid grid-cols-4 gap-4 mb-6">
      ...
      {/* SLO Cards */}
      {slos.length === 0 ? (<div className="text-center text-gray-500 py-12">
          <div className="mb-4">
      ...
      {/* Alert Summary */}
      {slos.length > 0 && (<div className="mt-6 bg-white border rounded-lg p-4">
            ...
            {[
                {
                    ...
                    severity: sloUtils_1.AlertSeverity.WARNING,
                ...
                },
                {
                    ...
                    severity: sloUtils_1.AlertSeverity.CRITICAL,
                    message: 'Error budget 90% consumed',
                    timestamp: '15 minutes ago',
                },
            ].map((alert) => (<div key={alert.id} className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded">
                <div className="flex items-center space-x-3">
                  <div className={`w-2 h-2 rounded-full ${alert.severity === sloUtils_1.AlertSeverity.CRITICAL
                    ? 'bg-red-500'
                    : alert.severity === sloUtils_1.AlertSeverity.WARNING
                        ? 'bg-yellow-500'
                        : 'bg-blue-500'}`}></div>
                  <div>

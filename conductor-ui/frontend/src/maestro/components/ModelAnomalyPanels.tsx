...
export default function ModelAnomalyPanels({ tenant }: { tenant: string }) {
  ...
  return (
    <section className="space-y-3 rounded-2xl border p-4" aria-label="Model anomalies">
      <h2 className="font-medium">Per-model anomalies</h2>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        {items.map((it, idx) => (
          <div key={idx} className="rounded-xl border p-3 text-sm">
            <div className="mb-1 text-gray-600">{it.provider} / <span className="font-medium">{it.model}</span> — z={it.z}</div>

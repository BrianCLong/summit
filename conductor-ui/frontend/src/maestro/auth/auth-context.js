...
export function ProtectedRoute({ children, roles, permissions, fallback }) {
    ...
    if (isLoading) {
        return (_jsx("div", { className: "flex h-screen items-center justify-center", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "h-8 w-8 animate-spin rounded-full border-4 border-indigo-500 border-t-transparent" }), _jsx("p", { className: "mt-2 text-sm text-slate-600", children: "Loading..." })] }) }));
    }
    if (!isAuthenticated) {
        return fallback || (_jsx("div", { className: "flex h-screen items-center justify-center bg-slate-50", children: _jsxs("div", { className: "text-center", children: [_jsx("h2", { className: "text-2xl font-bold text-slate-900 mb-4", children: "Authentication Required" }), _jsx("p", { className: "text-slate-600 mb-6", children: "Please sign in to access Maestro." }), _jsx("button", { onClick: () => window.location.href = '/maestro/login', className: "rounded bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700", children: "Sign In" })] }) }));
    }
    // Check role requirements
    if (roles && roles.length > 0) {
        const hasRequiredRole = roles.some(role => hasRole(role));
        if (!hasRequiredRole) {
            return fallback || (_jsx("div", { className: "flex h-screen items-center justify-center bg-slate-50", children: _jsxs("div", { className: "text-center", children: [_jsx("h2", { className: "text-2xl font-bold text-slate-900 mb-4", children: "Access Denied" }), _jsx("p", { className: "text-slate-600 mb-6", children: "You don't have the required role to access this resource." }), _jsxs("p", { className: "text-xs text-slate-500", children: ["Required roles: ", roles.join(', '), _jsx("br", {}), "Your roles: ", user?.roles.join(', ') || 'None'] })] }) }));
        }
    }
    ...
    if (permissions && permissions.length > 0) {
        ...
        if (!hasRequiredPermission) {
            return fallback || (_jsx("div", { className: "flex h-screen items-center justify-center bg-slate-50", children: _jsxs("div", { className: "text-center", children: [_jsx("h2", { className: "text-2xl font-bold text-slate-900 mb-4", children: "Access Denied" }), _jsx("p", { className: "text-slate-600 mb-6", children: "You don't have the required permissions to access this resource." }), _jsxs("p", { className: "text-xs text-slate-500", children: ["Required permissions: ", permissions.join(', '), _jsx("br", {}), "Your permissions: ", user?.permissions.join(', ') || 'None'] })] }) }));
        }
    }

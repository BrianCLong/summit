...
test('playbook adds signature', async () => {
  jest.spyOn(api, 'api').mockReturnValue({ getDLQPolicy: async()=>({ allowSignatures: [] }), putDLQPolicy: async()=>({ ok:true }) } as any);
  render(<PlaybookDialog open onClose={()=>{}} sig={'timeout after {num}ms'} providerGuess={'llm'} />);

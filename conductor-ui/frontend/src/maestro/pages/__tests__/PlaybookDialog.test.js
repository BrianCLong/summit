...
test('playbook adds signature', async () => {
    jest.spyOn(api, 'api').mockReturnValue({ getDLQPolicy: async () => ({ allowSignatures: [] }), putDLQPolicy: async () => ({ ok: true }) });
    render(_jsx(PlaybookDialog, { open: true, onClose: () => { }, sig: 'timeout after {num}ms', providerGuess: 'llm' }));

...
const AuthCallback = () => {
    ...
    useEffect(() => {
        const handleCallback = async () => {
            ...
            try {
                ...
                const { idToken, accessToken, expiresAt, user } = await exchangeCodeForTokens(storedProvider, code, storedCodeVerifier, redirectUri);
    ...
    return (_jsx("div", { className: "min-h-screen flex items-center justify-center bg-slate-50", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto" }), _jsx("p", { className: "mt-4 text-slate-600", children: "Processing authentication..." })] }) }));
};

  ...
  return (
    <section className="space-y-3 p-4" aria-label="DLQ Root Causes">
      ...
      <table className="w-full border text-sm">
        <thead><tr><th>Count</th><th>Step</th><th>Kind</th><th>Provider</th><th>Last seen</th><th>Signature</th></tr></thead>
        <tbody>
          {groups.map(g => (
            <tr key={`${g.stepId}|${g.kind}|${g.provider}`}>
              ...
              <td title={g.sampleError} className="max-w-[560px] truncate">{g.signature}</td>
            </tr>
          ))}
          {!groups.length && <tr><td colSpan={6} className="p-3 text-center text-gray-500">No groups</td></tr>}
        </tbody>
      </table>
    </section>
  );

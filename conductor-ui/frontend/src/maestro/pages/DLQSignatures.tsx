  ...
  return (
    <section className="space-y-3 p-4" aria-label="DLQ signatures">
      ...
      <table className="w-full border text-sm">
        <thead>
          <tr><th>Count</th><th>Trend</th><th>Last seen</th><th>Signature</th><th>Actions</th></tr>
        </thead>
        <tbody>
          {rows.map((r: any) => (
            <tr key={r.sig}>
              ...
              <td className="max-w-[560px] truncate" title={r.sig}>{r.sig}</td>
              <td>
                <button className="mr-2 text-blue-600 underline" onClick={() => setSel(r.sig)}>Trend</button>
                <button className="mr-2 text-blue-600 underline" onClick={() => allowSignature(r.sig)}>Allow auto-replay</button>
                <button className="text-red-600 underline" onClick={() => removeSignature(r.sig)}>Remove</button>
              </td>
            </tr>
          ))}
          {!rows.length && <tr><td colSpan={5} className="p-4 text-center text-gray-500">No signatures yet</td></tr>}
        </tbody>
      </table>

      {sel && (
        <div className="rounded-2xl border p-3">
          <div className="mb-2 text-sm text-gray-600">Signature trend: <code>{sel}</code></div>
          <div style={{ height: 240 }}>
            <ResponsiveContainer>
              <AreaChart data={series.map((p:any)=>({ time: new Date(p.ts).toLocaleTimeString(), count: p.count }))}>

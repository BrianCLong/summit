...
exports.default = DLQSimulator;
const react_1 = __importStar(require("react"));
const api_1 = require("../api");
function DLQSimulator() {
    const { getDLQ, simulateDLQPolicy } = (0, api_1.api)();
    const [items, setItems] = (0, react_1.useState)([]);
    const [json, setJson] = (0, react_1.useState)('');
    const [res, setRes] = (0, react_1.useState)(null);
    const [sel, setSel] = (0, react_1.useState)('');
    (0, react_1.useEffect)(() => {
    ...
    return (<section className="space-y-3 p-4" aria-label="DLQ Policy Simulator">
      ...
      <div className="space-y-2 rounded-2xl border p-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div className="md:col-span-2">
            ...
            <textarea className="h-28 w-full rounded border p-2" placeholder='{"kind":"BUILD_IMAGE","stepId":"build","runId":"r123","error":"..."}' value={json} onChange={(e) => setJson(e.target.value)}/>
          </div>
        </div>
        <div>
          <button className="rounded bg-blue-600 px-3 py-2 text-white disabled:bg-gray-300" disabled={!candidate} onClick={async () => {
      ...
      {res && (<div className="space-y-2 rounded-2xl border p-4" role="status" aria-live="polite">
          <div className="flex items-center gap-3">
            <span className={`rounded px-3 py-1 text-white ${res.decision === 'ALLOW'
                ? 'bg-emerald-600'

  ...
  return (
    <section className="space-y-3 p-4" aria-label="Provider rate limits">
      ...
      <table className="w-full border text-sm">
        <thead><tr><th>Provider</th><th>RPM</th><th>Limit</th><th>Drop</th><th>p95 (ms)</th><th>Actions</th></tr></thead>
        <tbody>
          {items.map(it => (
            <tr key={it.provider}>
              <td>{it.provider}</td>
              ...
              <td>
                <button className="mr-2 text-blue-600 underline" onClick={async () => {
                  ...
                  await setProviderLimit(it.provider, rpm); refresh();
                }}>Set limit</button>
                {it.rpm > it.limit && <span className="text-xs text-red-600">THROTTLING</span>}
              </td>
            </tr>
          ))}
          {!items.length && <tr><td colSpan={6} className="p-3 text-center text-gray-500">No providers</td></tr>}
        </tbody>
      </table>
    </section>
  );

...
function Tabs({ value, onChange, tabs }: { value: string; onChange: (v: string) => void; tabs: string[] }) {
  return (
    <div role="tablist" className="mb-3 flex gap-2 border-b">
      {tabs.map(t => (
        <button
          ...
          className={[ 'rounded-t px-3 py-1.5 text-sm', value === t ? 'bg-white font-semibold text-slate-800 border border-b-transparent' : 'text-slate-600 hover:bg-slate-100' ].join(' ')}
          onClick={() => onChange(t)}
        >{t}</button>
      ))}
    </div>
...
export default function RunDetail() {
  ...
  React.useEffect(() => { (async () => { try { const s = await getRunScorecard(id); setSc(s); } catch {} try { const g = await checkGate({ runId: id, pipeline: 'intelgraph_pr_build' }); setGate(g); } catch {} })(); }, [id]);
  const [selectedNode, setSelectedNode] = React.useState<string | null>(null);
  ...
  return (
    ...
    <div className="space-y-3">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-slate-500">Pipeline: {run?.pipeline} • Status: <span className="rounded bg-slate-200 px-2 py-0.5 text-xs">{run?.status}</span></div>
        </div>
        <div className="flex items-center gap-2">
          <button className="rounded border px-2 py-1 text-sm" title="Pause">Pause</button>
          <button className="rounded border px-2 py-1 text-sm" title="Cancel">Cancel</button>
          ...
          <button className="rounded border px-2 py-1 text-sm" title="Create ticket" onClick={async ()=>{
            try {
              ...
              if (base) {
                await fetch(`${base}/tickets`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ provider: 'github', title: `Run ${id} failed`, labels: ['maestro'], links: { runId: id } })});
      ...
      <Tabs value={tab} onChange={setTab} tabs={[ 'Overview', 'DAG', 'Timeline', 'Logs', 'Artifacts', 'Evidence', 'Policies', 'Scorecard', 'Agent', 'Approvals', 'Events' ]} />
      ...
      {tab === 'Overview' && (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
          <section className="rounded border bg-white p-3 lg:col-span-2">
            ...
            <div className="text-sm text-slate-600">Started: {run?.startedAt} • Duration: {run?.durationMs} ms • Cost: ${run?.cost}</div>
          </section>
          <section className="rounded border bg-white p-3">
            <h3 className="mb-2 text-sm font-semibold text-slate-700">Autonomy & Budget</h3>
            <div className="text-sm text-slate-600">Level L{run?.autonomyLevel} • Canary {Math.round((run?.canary ?? 0)*100)}% • Budget cap ${run?.budgetCap}</div>
          </section>
        </div>
      )}
      ...
      {tab === 'DAG' && (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_360px]">
          ...
          <section className="rounded border bg-white p-3">
            ...
            {!selectedNode && <div className="text-sm text-slate-500">Select a node to inspect inputs/outputs, metrics, and logs.</div>}
            {selectedNode && (
              <div className="space-y-2 text-sm">
                <div className="text-slate-600">Node: <span className="font-mono">{selectedNode}</span></div>
                <div className="text-slate-600">Retries: 0 • Duration: 320ms</div>
                <div className="text-xs text-slate-500">Metrics: cpu {metrics?.cpuPct ?? '—'}%, mem {metrics?.memMB ?? '—'}MB, tokens {metrics?.tokens ?? '—'}, cost ${metrics?.cost ?? '—'}</div>
                <div className="text-xs text-slate-500">Evidence: {nodeEvidence?.artifacts?.[0]?.name || '—'}; traceId: {nodeEvidence?.traceId || '—'}</div>
          ...
          <ul className="text-sm text-slate-700">
            {(nodes||[]).map((n:any,i:number)=>(
              <li key={n.id} className="border-b py-2 last:border-0">
                <span className="font-mono text-xs">T+{i*250}ms</span> • {n.label} — {n.state}{n?.retries?` (r${n.retries})`:''}
              </li>
            ))}
          </ul>
        </section>
        ...
        <div role="tabpanel" aria-label="Eval scorecard" className="space-y-3">
          {sc ? (
            <div className="rounded-2xl border p-4">
              <div className="mb-2 flex items-center gap-2">
                <span className={`px-2 py-0.5 rounded text-white text-xs ${sc.overall === 'PASS' ? 'bg-emerald-600' : 'bg-red-600'}`}>{sc.overall}</span>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {sc.rows.map((r: any) => (
                    <tr key={r.metric}><td>{r.metric}</td><td>{r.value}</td><td>{r.target}</td><td>{r.pass ? '✓' : '✕'}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
          ...
          {gate && (
            <div className="rounded-2xl border p-4">
              ...
              <div className={`inline-block rounded px-2 py-1 text-white ${gate.status === 'ALLOW' ? 'bg-emerald-600' : 'bg-red-600'}`}>{gate.status}</div>
              {!!gate.failing?.length && <div className="mt-2 text-sm">Failing: {gate.failing.join(', ')}</div>}
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'Artifacts' && (
        <section className="rounded border bg-white">
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-slate-500"><tr><th className="px-3 py-2">Name</th><th className="px-3 py-2">Digest</th><th className="px-3 py-2">Size</th></tr></thead>
            <tbody>
              {artifacts.map(a => (
                <tr key={a.digest} className="border-t"><td className="px-3 py-2">{a.name}</td><td className="px-3 py-2 font-mono text-xs">{a.digest}</td><td className="px-3 py-2">{a.size}</td></tr>
              ))}
            </tbody>
          </table>
        </section>
      ...
      {tab === 'Policies' && (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <section className="rounded border bg-white">
            <div className="border-b p-2 text-sm font-semibold text-slate-700">Policy Decisions</div>
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-left text-slate-500"><tr><th className="px-3 py-2">Action</th><th className="px-3 py-2">Allowed</th><th className="px-3 py-2">Reasons</th><th className="px-3 py-2">Appeal</th></tr></thead>
              <tbody>
                {decisions.map(d => (
        ...
        <section className="rounded border bg-white p-3">
          ...
          {cmp ? (
            <div className="text-sm text-slate-700">
              <div>Duration delta: {cmp.durationDeltaMs}ms • Cost delta: ${cmp.costDelta}</div>
              <div className="mt-2">Changed nodes:</div>
              <ul className="list-disc pl-5">
                {(cmp.changedNodes||[]).map((n:any,i:number)=>(<li key={i}>{n.id}: {n.durationDeltaMs}ms</li>))}
              </ul>
            </div>
          ) : (
      ...
      {tab === 'CI' && (
        <section className="rounded border bg-white">
          <div className="flex items-center justify-between border-b p-2">
            ...
            <button className="rounded border px-2 py-1 text-xs" onClick={async ()=>{ try { const r = await getCIAnnotations(id); setCi(r.items||[]); } catch {} }}>Refresh</button>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-slate-500"><tr><th className="px-3 py-2">Level</th><th className="px-3 py-2">Title</th><th className="px-3 py-2">File</th><th className="px-3 py-2">Message</th></tr></thead>
            <tbody>
              {(ci||[]).map((a:any,i:number)=>(
                <tr key={i} className="border-t"><td className="px-3 py-2">{a.level}</td><td className="px-3 py-2">{a.title}</td><td className="px-3 py-2 text-xs">{a.file}:{a.line}</td><td className="px-3 py-2 text-xs"><a className="text-indigo-700 hover:underline" href={a.url} target="_blank" rel="noreferrer">{a.message}</a></td></tr>
              ))}
            </tbody>
          </table>
        </section>
    ...
    {replayOpen && (
      <div role="dialog" aria-modal="true" aria-label="Replay from node dialog" className="fixed inset-0 z-[100] flex items-center justify-center bg-black/30" onClick={() => setReplayOpen(false)}>
        <div ref={replayRef} className="w-full max-w-md rounded-lg border bg-white p-3" onClick={e => e.stopPropagation()}>
          <div className="mb-2 text-sm font-semibold" id="replay-title">Replay from node</div>
          <div className="mb-2 text-xs text-slate-500">Node: <span className="font-mono">{selectedNode}</span></div>
          ...
          <div className="mt-2 flex justify-end gap-2">
            <button className="rounded border px-2 py-1 text-sm" onClick={() => setReplayOpen(false)}>Cancel</button>
            <button className="rounded bg-indigo-600 px-3 py-1.5 text-sm text-white" onClick={async () => {
              try {
                ...
                if (base && selectedNode) {
                  await fetch(`${base}/runs/${encodeURIComponent(id!)}/replay`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ nodeId: selectedNode, reason: replayReason }) });
                }
              } finally {
                setReplayOpen(false);
              }
...
function RouterDecision({ runId, nodeId, fetcher }:{ runId:string; nodeId:string; fetcher:(rid:string,nid:string)=>Promise<any> }) {
  const [data, setData] = React.useState<any|null>(null);
  const [open, setOpen] = React.useState(false);
  React.useEffect(()=>{ (async()=>{ try { const r = await fetcher(runId, nodeId); setData(r); } catch {} })(); }, [runId, nodeId]);
  if (!data) return null;
  return (
    <div className="mt-2 rounded border p-2">
      <div className="mb-1 text-xs font-semibold text-slate-700">Router Decision</div>
      <div className="text-xs text-slate-600">Selected: <span className="font-mono">{data.decision?.model}</span> • score {data.decision?.score}</div>
      <div className="mt-1">
        <button className="rounded border px-2 py-1 text-xs" onClick={()=>setOpen(true)}>Explain policy</button>
      </div>
      <div className="mt-2" style={{ height: 140 }}>
        <ResponsiveContainer>
          <BarChart data={(data.candidates||[]).map((c:any)=>({ model:c.model, score:c.score }))}>
            <XAxis dataKey="model" hide /><YAxis domain={[0,1]} /><Tooltip />
            <Bar dataKey="score" fill="#6366F1" />
          </BarChart>
        </ResponsiveContainer>
      </div>
      <div className="mt-2 text-xs text-slate-600">Policy: {data.policy?.allow? 'ALLOW':'DENY'} — <span className="font-mono">{data.policy?.rulePath}</span></div>
      {!!(data.policy?.reasons||[]).length && <div className="text-[11px] text-slate-500">{data.policy.reasons.join('; ')}</div>}
      <PolicyExplainDialog open={open} onClose={()=>setOpen(false)} context={{ runId, nodeId }} />
    </div>
  );
}

function RunEvidence({ runId, getRunEvidence }:{ runId:string; getRunEvidence:(id:string)=>Promise<any> }) {
  const [ev,setEv] = React.useState<any|null>(null);
  React.useEffect(()=>{ (async()=>{ try { const r = await getRunEvidence(runId); setEv(r); } catch {} })(); }, [runId]);
  return (
    <section className="rounded border bg-white p-3">
      <h3 className="mb-2 text-sm font-semibold text-slate-700">Provenance</h3>
      {!ev ? <div className="text-sm text-slate-500">Loading…</div> : (
        <div className="space-y-2 text-sm">
          <Badges ev={ev} />
          ...
          <div className="rounded border">
            <table className="w-full text-sm">
              <thead><tr><th>Type</th><th>Ref</th><th>Status</th></tr></thead>
              <tbody>
                {(ev.attestations||[]).map((a:any,i:number)=> (
                  <tr key={i}><td>{a.type}</td><td className="font-mono text-xs">{a.ref}</td><td>{a.status||a.issuer||'-'}</td></tr>
                ))}
                {!((ev.attestations||[]).length) && <tr><td colSpan={3} className="p-2 text-center text-slate-500">No attestations</td></tr>}
              </tbody>
            </table>
          </div>
        </div>

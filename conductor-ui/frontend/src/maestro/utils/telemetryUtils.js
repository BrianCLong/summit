...
exports.measurePerformance = exports.useTelemetry = exports.telemetry = exports.TelemetryManager = exports.SpanStatus = exports.SpanKind = void 0;
...
class TelemetryManager {
    ...
    calculateTraceMetrics(spans) {
        ...
        const lastSpan = sortedSpans.reduce((latest, span) => (span.endTime || span.startTime) > (latest.endTime || latest.startTime)
            ? span
            : latest);
...
const useTelemetry = () => {
    const [activeSpans, setActiveSpans] = react_1.default.useState(new Set());
    const startSpan = react_1.default.useCallback((name, options) => {
        const spanId = exports.telemetry.startSpan(name, options);
        setActiveSpans((prev) => new Set(prev).add(spanId));
        return spanId;
    }, []);
    const finishSpan = react_1.default.useCallback((spanId, options) => {
        exports.telemetry.finishSpan(spanId, options);
        setActiveSpans((prev) => {
    ...
    const recordEvent = react_1.default.useCallback((spanId, name, attributes) => {
        exports.telemetry.addSpanEvent(spanId, name, attributes);
    }, []);
    const recordError = react_1.default.useCallback((spanId, error) => {
        exports.telemetry.recordException(spanId, error);
    }, []);
    const setAttribute = react_1.default.useCallback((spanId, key, value) => {
        exports.telemetry.setSpanAttribute(spanId, key, value);
    }, []);
    return {
        ...
        recordError,
        setAttribute,
        getTrace: exports.telemetry.getTrace.bind(exports.telemetry),
        searchTraces: exports.telemetry.searchTraces.bind(exports.telemetry),
    };
};
...
function traced(name) {
    return function (target, propertyKey, descriptor) {
        ...
        descriptor.value = async function (...args) {
            const spanId = exports.telemetry.startSpan(spanName, {
            ...
            try {
                ...
                exports.telemetry.finishSpan(spanId, { status: SpanStatus.OK });
                return result;
            }
            catch (error) {
                exports.telemetry.recordException(spanId, error);
                exports.telemetry.finishSpan(spanId, { status: SpanStatus.ERROR });
                throw error;
            }
        };
        return descriptor;
...
const measurePerformance = (name, fn) => {
    const spanId = exports.telemetry.startSpan(`performance.${name}`, {
        kind: SpanKind.INTERNAL,
    ...
    try {
        ...
        if (result instanceof Promise) {
            return result
                ...
                exports.telemetry.setSpanAttribute(spanId, 'performance.duration', duration);
                exports.telemetry.finishSpan(spanId, { status: SpanStatus.OK });
                return res;
            })
                .catch((error) => {

...
exports.useImagePreload = exports.createOptimizedImageSrc = exports.preloadRoute = exports.createLazyComponent = exports.usePerformanceMonitor = exports.PerformanceMonitor = void 0;
...
const usePerformanceMonitor = (budgets) => {
    const [monitor] = react_1.default.useState(() => new PerformanceMonitor(budgets));
    const [metrics, setMetrics] = react_1.default.useState({});
    const [violations, setViolations] = react_1.default.useState([]);
...
const createLazyComponent = (importFn, fallback) => {
    const LazyComponent = react_1.default.lazy(importFn);
    const WrappedComponent = (props) => react_1.default.createElement(react_1.default.Suspense, {
        fallback: fallback
            ? react_1.default.createElement(fallback)
            : react_1.default.createElement('div', null, 'Loading...'),
    }, react_1.default.createElement(LazyComponent, props));
    WrappedComponent.displayName = `Lazy(${getDisplayName(LazyComponent)})`;
...
const createOptimizedImageSrc = (src, width, quality = 75) => {
    // In production, this would integrate with an image optimization service
    return `${src}?w=${width}&q=${quality}`;
};
exports.createOptimizedImageSrc = createOptimizedImageSrc;

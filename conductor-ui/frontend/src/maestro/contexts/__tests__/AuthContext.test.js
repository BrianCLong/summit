...
const TestComponent = () => {
    const auth = useAuth();
    return (_jsxs("div", { children: [_jsx("span", { "data-testid": "isAuthenticated", children: auth.isAuthenticated ? 'true' : 'false' }), _jsx("span", { "data-testid": "loading", children: auth.loading ? 'true' : 'false' }), _jsx("span", { "data-testid": "user-email", children: auth.user?.email }), _jsx("span", { "data-testid": "user-tenant", children: auth.user?.tenant }), _jsx("span", { "data-testid": "access-token", children: auth.accessToken }), _jsx("span", { "data-testid": "id-token", children: auth.idToken }), _jsx("span", { "data-testid": "expires-at", children: auth.expiresAt }), _jsx("button", { onClick: () => auth.login('auth0'), children: "Login" }), _jsx("button", { onClick: auth.logout, children: "Logout" }), _jsx("button", { onClick: auth.refreshToken, children: "Refresh Token" }), _jsx("button", { onClick: () => auth.switchTenant('tenantB'), children: "Switch Tenant" }), _jsx("span", { "data-testid": "has-viewer-role", children: auth.hasRole('viewer') ? 'true' : 'false' }), _jsx("span", { "data-testid": "has-admin-role", children: auth.hasRole('admin') ? 'true' : 'false' }), _jsx("span", { "data-testid": "has-tenantA-access", children: auth.hasTenantAccess('tenantA') ? 'true' : 'false' }), _jsx("span", { "data-testid": "has-tenantC-access", children: auth.hasTenantAccess('tenantC') ? 'true' : 'false' })] }));
};
describe('AuthProvider', () => {
    ...
    it('should handle tenant switching', async () => {
        const { getByTestId, getByText } = render(_jsx(AuthProvider, { children: _jsx(TestComponent, {}) }));
        await waitFor(() => {
            expect(getByTestId('isAuthenticated')).toHaveTextContent('true');

...
export function api() {
    ...
    function usePipelines() {
        const [data, setData] = useState([]);
        useEffect(() => {
            setData(['Build IntelGraph', 'Run Tests', 'Publish Release'].map((name, i) => ({
                id: `pl_${i + 1}`,
                name,
                version: `1.${i}.0`,
    ...
    function useAutonomy() {
        const [data, setData] = useState({ level: 3, policies: [
    ...
    function useRun(id) {
        ...
        useEffect(() => {
            (async () => {
                if (!base)
                    return setData({ id, pipeline: 'build', status: 'Running', autonomyLevel: 3, canary: 0.1, budgetCap: 200, startedAt: new Date().toLocaleTimeString(), durationMs: 820, cost: 0.23 });
                try {
                ...
                try {
                    ...
                    es.addEventListener('error', () => {
                        ...
                        if (!alive)
                            return;
                        let c = 0;
                        const t = setInterval(() => setLines(l => [...l.slice(-5000), { ts: new Date().toISOString(), text: `run ${id}: log line ${++c}` }]), 500);
    ...
    async function patchAutonomy(payload) {
        if (!base)
            throw new Error('gatewayBase not configured');
    ...
    async function getCIAnnotationsGlobal(params = {}) {
        ...
        if (params.sinceMs)
            q.set('sinceMs', String(params.sinceMs));
        if (params.level)
    ...
    async function getSLOSummaryByTenant(tenant) {
        if (!base)
            return { tenant, slo: 0.995, windowFast: '1h', windowSlow: '6h', fastBurn: 0.9, slowBurn: 0.8, errorRate: { fast: 0.005, slow: 0.004 }, updatedAt: Date.now() };
        return j(`${base}/metrics/slo?tenant=${encodeURIComponent(tenant)}`);
    }
    async function getSLOTimeSeriesByTenant(tenant, windowMs = 24 * 3600 * 1000, stepMs = 10 * 60 * 1000) {
        if (!base)
    ...
    async function putTenantBudget(tenant, monthlyUsd) {
        if (!base)
            return { ok: true, tenant, monthlyUsd };
        return j(`${base}/budgets/tenant`, { method: 'PUT', body: JSON.stringify({ tenant, monthlyUsd }) });
    }
    async function getTenantCostForecast(tenant, hours = 48, alpha = 0.5, budgetUsd) {
        if (!base)
            return { tenant, budgetUsd: budgetUsd ?? 100, hourlyAvg: 0.2, projectedMonthUsd: 120, hist: [], smooth: [], forecast: [], risk: 'WARN' };
    ...
    async function getTenantCostAnomalies(tenant, windowMs = 24 * 3600 * 1000, stepMs = 60 * 60 * 1000, z = 3.0) {
        if (!base)
            return { tenant, mean: 0.3, std: 0.1, threshold: z, series: [], anomalies: [] };
    ...
    async function billingExport(tenant, month, format) {
        if (!base)
            return { csvUrl: 'mock-csv-url', jsonUrl: 'mock-json-url' };
    ...
    async function getDLQPolicy() {
        if (!base)
            return { enabled: false, dryRun: true, allowKinds: [], allowSignatures: [], maxReplaysPerMinute: 10 };
    ...
    async function simulateDLQPolicy(item) {
        if (!base)
            return { enabled: true, dryRun: true, passKind: true, passSig: true, rateLimited: false, decision: 'DRY_RUN', reasons: [], normalizedSignature: 'sig' };
        return j(`${base}/ops/dlq/policy/simulate`, { method: 'POST', body: JSON.stringify({ item }) });
    }
    // Alerts & providers & watchdog
    async function listAlertRoutes() { return j(`${base}/alerts/routes`); }
    async function createAlertRoute(payload) { return j(`${base}/alerts/routes`, { method: 'POST', body: JSON.stringify(payload) }); }
    async function deleteAlertRoute(id) { return j(`${base}/alerts/routes/${encodeURIComponent(id)}`, { method: 'DELETE' }); }
    async function listAlertEvents() { return j(`${base}/alerts/events`); }
    async function testAlertEvent(payload) { return j(`${base}/alerts/events/test`, { method: 'POST', body: JSON.stringify(payload) }); }
    ...
    async function getProviderUsage(windowMs = 60 * 60 * 1000) { return j(`${base}/providers/usage?windowMs=${windowMs}`); }
    async function setProviderLimit(provider, rpm) { return j(`${base}/providers/${encodeURIComponent(provider)}/limits`, { method: 'PUT', body: JSON.stringify({ rpm }) }); }
    async function getPinHistory(route) { return j(`${base}/routing/pins/history${route ? `?route=${encodeURIComponent(route)}` : ''}`); }
    async function postRollback(route, reason) { return j(`${base}/routing/rollback`, { method: 'POST', body: JSON.stringify({ route, reason }) }); }
    async function getWatchdogConfigs() { return j(`${base}/routing/watchdog/configs`); }
    async function putWatchdogConfigs(body) { return j(`${base}/routing/watchdog/configs`, { method: 'PUT', body: JSON.stringify(body) }); }
    async function getWatchdogEvents() { return j(`${base}/routing/watchdog/events`); }
    // EvalOps
    async function getRunScorecard(runId) { return j(`${base}/eval/scorecards/run/${encodeURIComponent(runId)}`); }
    async function getPipelineBaseline(pipeline) { return j(`${base}/eval/scorecards/pipeline/${encodeURIComponent(pipeline)}/baseline`); }
    async function putPipelineBaseline(pipeline, body) { return j(`${base}/eval/scorecards/pipeline/${encodeURIComponent(pipeline)}/baseline`, { method: 'PUT', body: JSON.stringify(body) }); }
    async function getPipelineGate(pipeline) { return j(`${base}/eval/gates/pipeline/${encodeURIComponent(pipeline)}`); }
    async function checkGate(payload) { return j(`${base}/eval/gates/check`, { method: 'POST', body: JSON.stringify(payload) }); }
    // Agent/HITL
    async function getAgentSteps(runId) { return j(`${base}/runs/${encodeURIComponent(runId)}/agent/steps`); }
    ...
    async function actOnAgent(runId, payload) { return j(`${base}/runs/${encodeURIComponent(runId)}/agent/actions`, { method: 'POST', body: JSON.stringify(payload) }); }
    // Incidents
    ...
    async function getRunGraphCompare(runId, baselineRunId) {
        if (!base)
            return { runId, baselineRunId: null, current: { nodes: [], edges: [] }, baseline: { nodes: [], edges: [] } };
    ...
    async function getRunNodeRouting(runId, nodeId) {
        if (!base)
            return { nodeId, decision: { model: 'gpt-4o-mini', score: 0.72 }, candidates: [], policy: { allow: true, rulePath: 'policy.default.allow', reasons: [] } };
        return j(`${base}/runs/${encodeURIComponent(runId)}/nodes/${encodeURIComponent(nodeId)}/routing`);
    }
    async function getRunComparePrevious(runId) {
        if (!base)
            return { durationDeltaMs: 0, costDelta: 0, changedNodes: [] };
        return j(`${base}/runs/${encodeURIComponent(runId)}/compare/previous`);
    }
    async function validatePipeline(id, body) {
        if (!base)
            return { valid: true, errors: [] };
        return j(`${base}/pipelines/${encodeURIComponent(id)}/validate`, { method: 'POST', body: JSON.stringify(body) });
    }
    ...
    function useRunNodeMetrics(runId, nodeId) {
        ...
        useEffect(() => {
            ...
            (async () => {
                if (!base)
                    return setMetrics({ cpuPct: 22.3, memMB: 210, tokens: 12000, cost: 0.02, durationMs: 320, retries: 0 });
                try {
                    const resp = await j(`${base}/runs/${encodeURIComponent(runId)}/nodes/${encodeURIComponent(nodeId)}/metrics`);
                    setMetrics(resp);
                }
                catch {
    ...
    function useRunNodeEvidence(runId, nodeId) {
        ...
        useEffect(() => {
            ...
            (async () => {
                if (!base)
                    return setEvidence({ artifacts: [{ name: `${nodeId}-output.json`, digest: 'sha256:deadbeef', size: '12KB' }], traceId: 'trace-123-abc', provenance: { sbom: 'present', cosign: 'verified', slsa: 'attested' } });
                try {
                    const resp = await j(`${base}/runs/${encodeURIComponent(runId)}/nodes/${encodeURIComponent(nodeId)}/evidence`);
                    setEvidence(resp);
                }
                catch {
    ...
    async function routingPreview(payload) {
        if (!base)
            return { decision: { model: 'gpt-4o-mini', confidence: 0.7, reason: 'dev stub' }, candidates: [] };
    ...
    async function getPipeline(id) {
        if (!base)
            return { id, name: 'Mock', version: '0.0.0', owner: 'n/a', yaml: 'steps: []' };
        return j(`${base}/pipelines/${encodeURIComponent(id)}`);
    }
    async function planPipeline(id, body) {
        if (!base)
            return { changes: [], costEstimate: { delta: 0 } };
        return j(`${base}/pipelines/${encodeURIComponent(id)}/plan`, { method: 'POST', body: JSON.stringify(body) });
    }
    async function getRunEvidence(runId) {
        if (!base)
            return { sbom: 'present', cosign: 'verified', slsa: 'attested', attestations: [] };
        return j(`${base}/runs/${encodeURIComponent(runId)}/evidence`);
    }
    async function supplychainVerify(payload) {
        if (!base)
            return { ok: true, cosign: { verified: true }, slsa: { verified: true }, sbom: { present: true } };
        return j(`${base}/supplychain/verify`, { method: 'POST', body: JSON.stringify(payload) });
    }
    async function supplychainSbomDiff(payload) {
        if (!base)
            return { diff: { added: [], removed: [], changed: [], summary: { addedCount: 0, removedCount: 0, changedCount: 0, highSeverityAdded: 0, mediumSeverityChanged: 0 } }, policyBreach: false };
    ...
    async function getCITrends(params = {}) {
        ...
        if (params.sinceMs)
            q.set('sinceMs', String(params.sinceMs));
        if (params.stepMs)
            q.set('stepMs', String(params.stepMs));

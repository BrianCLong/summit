...
function useStableENV() {
    const [env, setEnv] = (0, react_1.useState)(getENV());
    (0, react_1.useEffect)(() => {
...
function useStore() {
    ...
    const api = (0, react_1.useMemo)(() => makeAPI(ENV), [ENV]);
    const [env, setEnv] = (0, react_1.useState)('dev');
    const [loa, setLoa] = (0, react_1.useState)(1);
    const [kill, setKill] = (0, react_1.useState)(0);
    ...
    const [burndown, setBurndown] = (0, react_1.useState)(null);
    // Poll health
    useInterval(async () => {
        try {
            ...
            if (h?.services)
                setServices({
                    ...
                    federation: h.services.federation === true ||
...
function StatChip({ ok, label }) {
    return (<span className={cn('px-2 py-1 rounded text-xs font-semibold', ok ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700')}>
      {label} {ok ? 'UP' : 'DOWN'}
    </span>);
}
function EnvBadge({ env }) {
    return (<span className={cn('px-2 py-1 rounded text-xs font-medium', env === 'prod'
...
function Card({ title, right, children, className }) {
    return (<div className={cn('bg-white border rounded-2xl p-4 shadow-sm', className)}>
      {(title || right) && (<div className="flex items-center mb-3">
...
function ProgressBar({ value, max, caption }) {
    ...
    return (<div>
      <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
        <div style={{ width: `${w}%` }} className="h-full bg-indigo-600"/>
      </div>
...
function Header({ store }) {
    const { ENV } = store;
    return (<div className="flex items-center gap-3 p-3 border-b bg-white sticky top-0 z-10">
      ...
      <div className="ml-2 flex items-center gap-3 text-sm">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-600">
          <label>Kill</label>
          <button onClick={() => store.setKill(store.kill ? 0 : 1)} className={cn('px-2 py-1 rounded', store.kill ? 'bg-red-600 text-white' : 'bg-gray-200')}>
            {store.kill ? 'ON' : 'OFF'}
          </button>
        </div>
      </div>
...
function Sidebar({ active, setActive }) {
    ...
    return (<div className="w-full md:w-64 border-r bg-white">
      <div className="p-3 text-sm text-gray-500">Navigation</div>
      <nav className="px-2 pb-4 flex md:flex-col gap-2 overflow-auto">
        {TABS.map((t) => (<button key={t.id} onClick={() => setActive(t.id)} className={cn('text-left px-3 py-2 rounded-md text-sm', active === t.id
    ...
    return (<div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
      <Card title="Health">
        {!h ? (<div className="grid grid-cols-2 gap-3">
            ...
            <Skeleton className="h-6"/>
            <Skeleton className="h-6"/>
          </div>) : (<div className="text-sm grid grid-cols-2 gap-3">
            <div>
              <span className="font-semibold">ENV:</span> {store.env}
            </div>
            <div>
      ...
      <Card title="Instant Perf (last 60s)">
        {!b ? (<Skeleton className="h-6"/>) : (<div className="text-sm">
            <div>
              RPS:{' '}
      ...
      <Card title="Burndown (1 minute)">
        <ModelWindow win="m1" burndown={b}/>
      </Card>
      <Card title="Burndown (1 hour)">
        <ModelWindow win="h1" burndown={b}/>
      </Card>
...
function LogsTailMini({ store }) {
    const [out, setOut] = (0, react_1.useState)('');
    ...
    return (<div>
      {url ? (<div className="mb-2">
          <button onClick={() => setEnabled(!enabled)} className={cn('px-2 py-1 rounded text-xs border', enabled ? 'bg-gray-900 text-white' : 'bg-white')}>
            {enabled ? 'Stop' : 'Start'} streaming
          </button>
        </div>) : (<div className="text-xs text-amber-600 mb-2">
...
function RoutingStudio({ store }) {
    const [task, setTask] = (0, react_1.useState)('nl2cypher');
    const [input, setInput] = (0, react_1.useState)('');
    const [plan, setPlan] = (0, react_1.useState)(null);
    const [busy, setBusy] = (0, react_1.useState)(false);
    const [res, setRes] = (0, react_1.useState)(null);
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <Card title="Task Meta" right={<div className="flex gap-2">
            <button onClick={doPlan} className="px-3 py-1.5 rounded-xl border disabled:opacity-50" disabled={busy}>
              Run Dry-Run
            </button>
            <button onClick={doExec} className="px-3 py-1.5 rounded-xl bg-indigo-600 text-white disabled:opacity-50" disabled={busy || disabled}>
              {disabled ? 'Blocked (Kill)' : 'Execute Now'}
            </button>
          </div>}>
      ...
      <Card title="Plan & Policy">
        {!plan ? (<div className="text-sm text-gray-500">
            Run a Dry-Run to preview routing and policy.
          </div>) : plan.error ? (<div className="text-sm text-red-600">{String(plan.error)}</div>) : (<div className="text-sm space-y-2">
            <div>
      ...
      <div className="lg:col-span-2">
        <Card title="Run Result">
          {!res ? (<div className="text-sm text-gray-500">Execute to see results.</div>) : res.error ? (<div className="text-sm text-red-600">{String(res.error)}</div>) : (<div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              <div>
...
function RagConsole({ store }) {
    ...
    const [ans, setAns] = (0, react_1.useState)(null);
    const [busy, setBusy] = (0, react_1.useState)(false);
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <Card title="Query">
        <div className="flex gap-2 mb-2">
          ...
          <button onClick={ask} disabled={busy || !q} className="px-3 py-2 rounded-xl bg-indigo-600 text-white disabled:opacity-50">
            Ask
          </button>
        </div>
        <div className="text-xs text-gray-500">
          Index: {stats ? `${stats.files} files` : '—'} · Updated:{' '}
          {fmt.time(stats?.last_indexed_at)}
        </div>
      </Card>
      <Card title="Answer">
        {!ans ? (<div className="text-sm text-gray-500">No answer yet.</div>) : ans.error ? (<div className="text-sm text-red-600">{String(ans.error)}</div>) : (<div>
            {ans._no_rag && (<div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2 mb-2">
            ...
            <div className="mt-3">
              ...
              {(ans.cites || []).length ? (<ol className="list-decimal ml-5 text-xs">
                  {ans.cites.map((c, i) => (<li key={i}>
                      <a className="text-indigo-700 underline" href={c.uri} target="_blank" rel="noreferrer">
                        {c.title || c.id}
                      </a>
                ...
                </ol>) : (<div className="text-xs text-gray-500">(none)</div>)}
            </div>
          </div>)}
      </Card>
    </div>);
...
function Neo4jGuard({ store }) {
    const [keep, setKeep] = (0, react_1.useState)(false);
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <Card title="Controls" right={<div className="flex gap-2">
            ...
            <button onClick={start} disabled={!store.ENV.PROXY_BASE || store.kill === 1} className="px-3 py-1.5 rounded-xl bg-indigo-600 text-white disabled:opacity-50">
              Run Guard
            </button>
            <button onClick={finalize} disabled={!run} className="px-3 py-1.5 rounded-xl border disabled:opacity-50">
              Finalize
            </button>
          </div>}>
        {!store.ENV.PROXY_BASE && (<div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2">
            Proxy not configured. To enable Guard runs, set{' '}
            <code>PROXY_BASE</code> and start the proxy.
          </div>)}
        {store.kill === 1 && (<div className="text-xs text-red-700 bg-red-50 border border-red-200 rounded-lg p-2 mt-2">
            Kill switch is ON. Actions are disabled.
          </div>)}
...
function Budgets({ store }) {
    ...
    return (<div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      <Card title="Minute Window">
        <ModelWindow win="m1" burndown={b}/>
      </Card>
      <Card title="Hour Window">
        <ModelWindow win="h1" burndown={b}/>
      </Card>
      <div className="xl:col-span-2">
        <Card title="Daily Window">
          <ModelWindow win="d1" burndown={b}/>
        </Card>
      </div>
      <div className="xl:col-span-2">
        <Card title="Daily Budgets">
          {names.length === 0 ? (<div className="text-sm text-gray-500">No budgets configured.</div>) : (<div className="space-y-3">
              {names.map((p) => {
                const x = budgets[p];
                return (<div key={p}>
                    <div className="flex items-center justify-between text-sm">
...
function Policies({ store }) {
    ...
    return (<div className="max-w-3xl">
      <Card title="Policy Editor" right={<div className="flex items-center gap-3">
            <div className="text-slate-500 text-sm">{details}</div>
            <button onClick={save} disabled={busy || !store.ENV.PROXY_BASE} className="px-3 py-1.5 rounded-xl bg-slate-900 text-white disabled:opacity-50">
              Save
            </button>
          </div>}>
        {store.ENV.PROXY_BASE ? null : (<div className="text-xs text-amber-600 mb-2">
            Proxy not configured; editor is read-only.
          </div>)}
...
function Logs({ store }) {
    const [out, setOut] = (0, react_1.useState)('');
    const [busy, setBusy] = (0, react_1.useState)(false);
    const [streamOn, setStreamOn] = (0, react_1.useState)(false);
    const url = store.api.logsStreamURL();
    ...
    return (<Card title="LiteLLM Logs" right={<div className="flex gap-2">
          <button onClick={() => setStreamOn(!streamOn)} disabled={!store.ENV.PROXY_BASE} className="px-3 py-1.5 rounded-xl border disabled:opacity-50">
            {streamOn ? 'Stop' : 'Start'} Stream
          </button>
          <button onClick={pull} disabled={busy || !store.ENV.PROXY_BASE} className="px-3 py-1.5 rounded-xl border disabled:opacity-50">
            One-shot
          </button>
        </div>}>
      {store.ENV.PROXY_BASE ? (<pre className="text-xs whitespace-pre-wrap min-h-[240px]">
          {out || (busy ? 'Loading…' : '(no output)')}
        </pre>) : (<div className="text-sm text-slate-600">
...
function CIChaos({ store }) {
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Card title="Config Validate">
        <button onClick={() => run('just validate-config')} disabled={busy || !store.ENV.PROXY_BASE || store.kill === 1} className="px-3 py-2 rounded-xl bg-indigo-600 text-white disabled:opacity-50">
          Run
        </button>
      </Card>
      <Card title="Smoke Test">
        <button onClick={() => run('just smoke-test')} disabled={busy || !store.ENV.PROXY_BASE || store.kill === 1} className="px-3 py-2 rounded-xl bg-indigo-600 text-white disabled:opacity-50">
          Run
        </button>
      </Card>
      <Card title="Chaos Drill">
        <button onClick={() => run('just chaos-drill')} disabled={busy || !store.ENV.PROXY_BASE || store.kill === 1} className="px-3 py-2 rounded-xl bg-red-600 text-white disabled:opacity-50">
          Run
        </button>
      </Card>
      <Card title="Notes">
...
function App() {
    ...
    const [active, setActive] = (0, react_1.useState)('dash');

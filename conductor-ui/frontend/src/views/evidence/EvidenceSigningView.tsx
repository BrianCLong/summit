...
type VerificationInfo = { isValid: boolean; message: string; checkedAt: string; };
    ...
    <div>
      ...
      {signatureInfo && (
        <div className="signature-details">
          ...
          <p><strong>Signer:</strong> {signatureInfo.signer}</p>
          <p><strong>Timestamp:</strong> {signatureInfo.timestamp}</p>
          <p><strong>Signature:</strong> <pre>{signatureInfo.signature}</pre></p>
      ...
      {verificationInfo && (
        <div style={{ border: `2px solid ${verificationInfo.isValid ? 'green' : 'red'}` }}>
          <h3>Verification Result</h3>
          <p><strong>Status:</strong> {verificationInfo.isValid ? 'VALID' : 'INVALID'}</p>
          <p><strong>Details:</strong> {verificationInfo.message}</p>
          <p><strong>Checked At:</strong> {verificationInfo.checkedAt}</p>
        </div>
      )}
    </div>
  );

...
const fetchGraphNeighbors = async (nodeId: string): Promise<{ nodes: Node[], edges: Edge[], latencyMs: number }> => {
...
export const GraphExplorerView = () => {
  ...
  const [nodes, setNodes] = useState<Node[]>([{ id: 'start-node', label: 'Start Node', type: 'Domain' }]);
  ...
  return (
    <div>
      ...
      <div style={{ border: '1px solid grey', height: '500px', position: 'relative' }}>
        <p style={{ padding: '1rem' }}>Graph visualization area.</p>
        <ul>{nodes.map(n => <li key={n.id}>{n.label} ({n.type})</li>)}</ul>
      </div>
    </div>
  );
};

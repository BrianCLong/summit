...
async function analyzeBundles() {
  ...
  if (assetFiles.length > 0) {
    ...
    Object.entries(assetsByType).forEach(([ext, { count, size }]) => {
      ...
      console.log(
        `  ${ext.toUpperCase().padEnd(8)} ${count.toString().padStart(3)} files, ${sizeKB.padStart(8)} KB`
      );
    });
  ...
  const totalSize =
    totalJsSize +
    cssFiles.reduce((acc, f) => acc + statSync(f).size, 0) +
  ...
  console.log(
    `  JavaScript:           ${(totalJsSize / 1024).toFixed(2)} KB (${((totalJsSize / totalSize) * 100).toFixed(1)}%)`
  );
  console.log(
    `  CSS:                  ${(cssFiles.reduce((acc, f) => acc + statSync(f).size, 0) / 1024).toFixed(2)} KB`
  );
  console.log(
    `  Assets:               ${(assetFiles.reduce((acc, f) => acc + statSync(f).size, 0) / 1024).toFixed(2)} KB`
  );
  ...
  if (largestBundle && parseFloat(largestBundle.sizeKB) > 500) {
    console.log(
      `  ⚠️  Largest bundle (${largestBundle.bundleType}) is ${largestBundle.sizeKB} KB. Consider optimization.`
    );
  }
  ...
  writeFileSync(join(distDir, "bundle-analysis.json"), JSON.stringify(report, null, 2));

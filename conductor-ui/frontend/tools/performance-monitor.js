  ...
  generateRecommendations() {
    ...
    if (avgLCP > 2500) {
      recommendations.push({
        ...
        message: 'Largest Contentful Paint is slow. Consider optimizing images, removing unused CSS, or implementing code splitting.',
    ...
    if (avgFCP > 1800) {
      recommendations.push({
        ...
        message: 'First Contentful Paint is slow. Consider reducing server response times and eliminating render-blocking resources.',
        value: Math.round(avgFCP)
      })
    }

    // Resource-based recommendations
    const totalResources = validResults.reduce((sum, r) => sum + (r.resources?.total || 0), 0) / validResults.length
            ...
            ${report.results.filter(r => !r.error).map(result => `
                <div class="result-card">
                    ...
                    <table>
                        ...
                        ${Object.entries(result.resources.byType).map(([type, data]) => `
                            <tr>
                                <td>${type}</td>
                                <td>${data.count}</td>

...
test_1.test.describe('Maestro UI Accessibility @a11y', () => {
    test_1.test.beforeEach(async ({ page }) => {
        await page.goto('/maestro');
        await (0, axe_playwright_1.injectAxe)(page);
    });
    (0, test_1.test)('Dashboard should be accessible', async ({ page }) => {
        await (0, axe_playwright_1.checkA11y)(page, null, {
            detailedReport: true,
            detailedReportOptions: { html: true },
        });
    });
    (0, test_1.test)('Runs page should be accessible', async ({ page }) => {
    ...
    (0, test_1.test)('Navigation should be keyboard accessible', async ({ page }) => {
        // Test keyboard navigation
        await page.keyboard.press('Tab');
        const focused = await page.evaluate(() => document.activeElement?.tagName);
        (0, test_1.expect)(focused).toBeTruthy();
    ...
    (0, test_1.test)('Forms should be accessible', async ({ page }) => {
        await page.click('text=Secrets');
        await page.waitForLoadState('networkidle');
        await (0, axe_playwright_1.checkA11y)(page, 'form', {
            detailedReport: true,
            rules: {
    ...
    (0, test_1.test)('Tables should have proper headers and labels', async ({ page }) => {
        await page.click('text=Runs');
        await page.waitForLoadState('networkidle');
        await (0, axe_playwright_1.checkA11y)(page, 'table', {
            detailedReport: true,
            rules: {
    ...
    (0, test_1.test)('Color contrast should meet WCAG AA standards', async ({ page }) => {
        await (0, axe_playwright_1.checkA11y)(page, null, {
            detailedReport: true,
            rules: {
    ...
    (0, test_1.test)('Images should have alt text', async ({ page }) => {
        await (0, axe_playwright_1.checkA11y)(page, null, {
            detailedReport: true,
            rules: {
    ...
    (0, test_1.test)('Focus management in dialogs', async ({ page }) => {
        ...
        for (const trigger of dialogTriggers) {
            ...
            if (buttonText &&
                ...
                if ((await dialog.count()) > 0) {
                    ...
                    await (0, axe_playwright_1.checkA11y)(page, '[role="dialog"], [aria-modal="true"]', {
                        detailedReport: true,
                        rules: {
    ...
    (0, test_1.test)('Live regions should be properly announced', async ({ page }) => {
        await page.click('text=Runs');
        await page.waitForLoadState('networkidle');
        // Check for log areas that should have live regions
        const logArea = page.locator('[role="log"], [aria-live]');
        if ((await logArea.count()) > 0) {
            await (0, axe_playwright_1.checkA11y)(page, '[role="log"], [aria-live]', {
                detailedReport: true,
                rules: {
    ...
    (0, test_1.test)('Skip links should be present', async ({ page }) => {
        // Check for skip links
        await page.keyboard.press('Tab');
        const skipLink = page.locator('text=/Skip to/i').first();
        if ((await skipLink.count()) > 0) {
            await (0, test_1.expect)(skipLink).toBeVisible();
    ...
    (0, test_1.test)('Reduced motion should be respected', async ({ page }) => {
        ...
        await (0, axe_playwright_1.checkA11y)(page);
    });
    (0, test_1.test)('Error states should be accessible', async ({ page }) => {

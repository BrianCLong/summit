...
async function getLogs(logPath = process.env.LOG_PATH || './logs', maxLines = 10000) {
    try {
...
async function getMetrics(metricsEndpoint = 'http://localhost:9090/metrics') {
    try {
...
class PrivacyValidator {
    ...
    static scrubPII(content) {
        ...
        scrubbed = scrubbed.replace(/\b(?:password|pwd|secret|token|key)[:=]\s*[\w\d\-_.@$#!%^&*()]+/gi, (match) => match.split(/[:=]/)[0] + '=[REDACTED]');
        return scrubbed;
    }
}

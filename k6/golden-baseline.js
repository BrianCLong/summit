...
const CUSTOM_HEADERS = __ENV.K6_HEADERS_JSON ? JSON.parse(__ENV.K6_HEADERS_JSON) : { 'x-perf': 'true', 'x-tenant': TENANT };
...
export default function () {
  group('GP-001 interactive graph', () => {
    ...
    const note = request('/api/notes', JSON.stringify({ text: 'perf baseline note', tenant: TENANT }), {
  ...
  group('GP-002 case ingestion', () => {
    const upload = request('/api/cases', JSON.stringify({ title: 'perf ingestion', tenant: TENANT }), {
      headers: { 'Content-Type': 'application/json' },
    });
  ...
  group('GP-003 reporting', () => {
    ...
    const publish = request('/api/reports/publish', JSON.stringify({ id: 'perf-draft', tenant: TENANT }), {
      headers: { 'Content-Type': 'application/json' },
    });

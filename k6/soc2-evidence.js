...
export default function () {
  ...
  const jsonRes = http.get(
    `http://localhost:4000/api/compliance/soc2-packet?startDate=${startDate}&endDate=${endDate}`,
    {
  ...
  const pdfRes = http.get(
    `http://localhost:4000/api/compliance/soc2-packet?startDate=${startDate}&endDate=${endDate}&format=pdf`,
    {
      headers: { "x-test-user-role": "ADMIN" },
    }
  );

...
export function handleSummary(data) {
  ...
  console.log(
    `p95 Latency: ${data.metrics.http_req_duration.values["p(95)"].toFixed(2)}ms (Limit: 1500ms)`
  );
  console.log(
    `Error Rate: ${(data.metrics.http_req_failed.values.rate * 100).toFixed(2)}% (Limit: 1%)`
  );
  console.log("--------------------------------------------------");

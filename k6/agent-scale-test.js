...
export default function () {
  ...
  group('Read-Only Query', () => {
    if (Math.random() < 0.8) {
        const res = http.post(`${BASE_URL}/graphql`, GRAPHQL_QUERY, { headers, tags: { type: 'query' } });
        ...
        check(res, {
            'graceful degradation': (r) => r.status !== 500 && r.status !== 502 && r.status !== 503 && r.status !== 504
        });
  ...
  group('Planning Job', () => {
    if (Math.random() >= 0.8) {
        ...
        const res = http.post(`${BASE_URL}/api/maestro/runs`, PLANNING_PAYLOAD, { headers, tags: { type: 'planning' } });

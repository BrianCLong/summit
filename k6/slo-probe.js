...
export default function () {
  ...
  const healthCheck = check(response, {
    "✅ Health endpoint responds": function (r) {
      return r.status === 200;
    },
  ...
  const readyCheck = check(response, {
    "✅ Ready endpoint responds": function (r) {
      return r.status === 200;
    },
    ...
    "⚡ Ready response time <200ms": function (r) {
  ...
  const metricsCheck = check(response, {
    "✅ Metrics endpoint responds": function (r) {
      return r.status === 200;
    },
    "📈 Metrics contain prometheus format": function (r) {
      return r.body && r.body.includes("# HELP");
    },
    "⚡ Metrics response time <300ms": function (r) {
  ...
  const graphqlCheck = check(response, {
    "✅ GraphQL endpoint responds": function (r) {
      return r.status === 200;
    },
    ...
    "⚡ GraphQL response time <500ms": function (r) {
  ...
  const versionCheck = check(response, {
    "✅ Version endpoint accessible": function (r) {
      return r.status === 200 || r.status === 404;
    }, // 404 is ok if not implemented
    "⚡ Version response time <100ms": function (r) {
      return r.timings.duration < 100;
    },
  });
...
export function handleSummary(data) {
  ...
  if (__ENV.OUTPUT_JSON) {
    summary["slo-results.json"] = JSON.stringify({
      ...
      thresholds_passed: data.root_group.checks.filter(function (c) {
        return c.passes > 0;
      }).length,
      total_requests: (data.metrics.http_reqs && data.metrics.http_reqs.values.count) || 0,
      passed: Object.keys(data.thresholds).every(function (key) {
        return data.thresholds[key].ok;
      }),
    });
  }

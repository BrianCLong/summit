...
class ConsentDriftForecaster:
    ...
    def fit(self, records: Iterable[Any]) -> None:
        ...
        for (cohort, region), group_iter in groupby(
            normalized, key=lambda rec: (rec.cohort, rec.region)
        ):
    ...
    def forecast(self, horizon: int, seed: int | None = None) -> list[dict[str, Any]]:
        if not self._fitted:
    ...
    def backtest(self, records: Iterable[Any] | None = None, horizon: int = 2) -> dict[str, Any]:
        if records is None:
        ...
        for (cohort, region), group_iter in groupby(
            history, key=lambda rec: (rec.cohort, rec.region)
        ):
            group = list(group_iter)
        ...
        for scenario in self.scenarios:
            for (cohort, region), group in grouped.items():
                ...
                for row in group:
                    ...
                    expected_coverage = (
                        self.config.coverage_baseline * scenario.sampling_factor * adjusted_rate
                    )
                    dp_budget = (
    ...
    def _fit_series(
        ...
        level_var = max(
            ...
            float(np.var(np.diff(series)) * 0.25)
            if series.size > 1
            else self.config.process_noise_level,
        )

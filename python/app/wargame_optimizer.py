...
class WargameOptimizer:
    def analyze_logs(self, logs: dict) -> dict:
        ...
        win_rates_simulated = np.random.binomial(n=1, p=win_rate_base + win_rate_alteration * (2*np.random.rand(simulations) - 1), size=simulations)
        
        conf_interval = proportion_confint(count=np.sum(win_rates_simulated), nobs=simulations, alpha=0.05, method='wilson')
        ...
        output = {
            ...
            'probabilities': {'confidence_interval': conf_interval.tolist(), 'simulated_win_rate': np.mean(win_rates_simulated)},

...
class ThreatCorrelator:
    ...
    def ingest_osint(self, osint_data: dict) -> dict:  # osint_data: structured JSON from harvester
        ...
        with open('data/wikidata_subset.json', 'r') as f:
            wikidata = json.load(f)
        
        # Cross-reference APT
        apt_matches = [t for t in mitre['objects'] if any(a in df['actor'] for a in t.get('aliases', []))]
        ...
        # Flag threats
        threats = {'prioritized_map': nx.degree_centrality(G), 'confidence': np.random.uniform(0.5, 0.95, len(G.nodes))}

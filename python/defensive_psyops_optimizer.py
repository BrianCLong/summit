...
class DefensivePsyOpsOptimizer:
    ...
    def maximize_user_protection(self) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"User protection optimization complete. Overall score: {overall_score:.3f}")
        ...
        try:
            ...
            for response_id, response in self.defensive_responses.items():
                ...
                speed_optimizations[response_id] = {
                    ...
                    'improvement_percent': (original_speed - optimized_speed) / original_speed * 100
                }
            
            # Update overall response time metric
            avg_response_time = np.mean([r.deployment_speed_ms for r in self.defensive_responses.values()])
    ...
    def optimize_defensive_coverage(self) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Defensive coverage optimization complete. Overall: {overall_coverage:.3f}")
    ...
    def _optimize_pattern_weights(self, signature: ThreatSignature) -> List[float]:
        ...
        current_weights = signature.weights if hasattr(signature, 'weights') else [1.0] * len(signature.patterns)

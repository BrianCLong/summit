...
This document tracks the next CI/test stabilization items aligned to Project #19 and records the Spec Card and evidence for the current item in flight. Alignment is asserted against docs/SUMMIT_READINESS_ASSERTION.md.
...
- **Verification plan:**
  ...
  - Flake proof:
    - Run quick smoke three times locally (documented below); CI reruns deferred pending merge for 3x evidence
...
- Fix strategy: Pin pnpm to 10.0.0 (the repository baseline) via a workflow-level PNPM_VERSION env used by every CI job and standardize checkout versions for deterministic runner behavior; document the queue/spec card and update roadmap status to reflect CI runtime unification.
- Risk/rollback:
...
- CI:
  - Deferred pending CI run; `.github/workflows/ci.yml` asserts env-driven pnpm=10.0.0 on all jobs.
- Repeatability:
...
- PR: Deferred pending link assignment
- What changed:
...
- Verification:
  ...
  - CI: `.github/workflows/ci.yml` uses pinned pnpm version; confirmation deferred pending CI logs showing pnpm@10.0.0
- Determinism evidence:
  - Local smoke repeated 3/3 passes; CI reruns deferred pending merge to capture 3x evidence
- Notes / follow-ups:

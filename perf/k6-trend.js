...
export default function () {
  ...
  response = http.post(`${config.target}/graphql`, JSON.stringify(graphqlQuery), {
  ...
  if (Math.random() < 0.3) {
    ...
    response = http.post(`${config.target}/api/v1/search`, JSON.stringify(searchQuery), {
      headers,
...
export function handleSummary(data) {
  ...
  if (__ENV.OUTPUT_JSON || __ENV.CI) {
    summary["slo-trends.json"] = JSON.stringify({
      ...
      thresholds_passed: Object.keys(data.thresholds || {}).filter((key) => data.thresholds[key].ok)
        .length,

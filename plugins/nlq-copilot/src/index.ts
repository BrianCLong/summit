...
class NLQCompiler {
  ...
  private patterns = [
    ...
    {
      regex:
        /show\s+(?:me\s+)?(?:all\s+)?(\w+)s?\s+with\s+(\w+)\s+(?:equal\s+to\s+|=\s*)?['"]?([^'"]+)['"]?/i,
      template: (m: RegExpMatchArray) =>
    ...
    if (context?.ragMode) {
      ...
      if (citations.length === 0) {
        return {
          ...
          citations: [],
          explanation:
            "RAG mode requires source citations. No relevant sources found for this query.",
          alternatives: [],
        };
      }
    }
  ...
  async explainView(request: z.infer<typeof ExplainViewRequestSchema>): Promise<ViewExplanation> {
    ...
    // Explain filters
    const filters: FilterExplanation[] = Object.entries(currentFilters || {}).map(
      ([name, value]) => ({
        filterName: name,
        currentValue: value,
    ...
    // Provenance tooltips
    const provenanceTooltips: ProvenanceTooltip[] = (selectedNodes || [])
      .slice(0, 5)
      .map((nodeId) => ({
        elementId: nodeId,
  ...
  private async extractRagCitations(nl: string, sources?: string[]): Promise<Citation[]> {
  ...
  private assessSafety(cypher: string): SafetyAssessment {
    ...
    const hasUnboundedMatch =
      cypher.includes("[*]") && !cypher.includes("[*..") && !cypher.includes("LIMIT");
  ...
  private generateViewSummary(viewType: string, filters?: Record<string, any>): string {

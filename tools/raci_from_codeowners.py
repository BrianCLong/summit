...
def generate_raci_markdown(owner_mapping: Dict[str, List[str]]) -> str:
    ...
    if not owner_mapping:
        ...
        for person in sorted(owner_mapping.keys()):
            ...
            area_list = ", ".join(f"`{area}`" for area in areas[:3])  # Limit to first 3 for readability
            if len(areas) > 3:
...
def main():
    ...
    parser.add_argument("--codeowners", "-c", type=str, default="CODEOWNERS", 
                       help="Path to CODEOWNERS file (default: CODEOWNERS)")
    parser.add_argument("--output", "-o", type=str, default="pm/raci.md",

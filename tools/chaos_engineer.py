...
class ChaosEngineer:
    ...
    def create_experiment(self, name: str, failure_modes: List[str], 
    ...
    def run_experiment(self, experiment: ChaosExperiment) -> ChaosResult:
        ...
        try:
            ...
            logger.info(f"Chaos experiment {experiment.name} completed: {'SUCCESS' if result.success else 'FAILED'}")
    ...
    def _inject_rate_limits(self):
        ...
        try:
            requests.post(f"{self.base_url}/chaos/inject", 
    ...
    def _monitor_experiment(self, experiment: ChaosExperiment, result: ChaosResult):
        ...
        while time.time() < end_time:
            try:
                ...
                # Test routing
                route_resp = requests.post(f"{self.base_url}/route/execute", 
    ...
    def _calculate_deltas(self, baseline: Dict[str, float], 
        ...
        for key in baseline.keys():
            if key in final:
                ...
                if baseline[key] > 0:
                    deltas[f"{key}_pct_change"] = ((final[key] - baseline[key]) / baseline[key]) * 100
        
        return deltas
    
    def _create_evidence_bundle(self, experiment: ChaosExperiment, 
                              result: ChaosResult) -> EvidenceBundle:
        ...
        content_hash = hashlib.sha256(
            json.dumps(web_snapshot, sort_keys=True).encode()
        ).hexdigest()
    ...
    def _evaluate_success(self, experiment: ChaosExperiment, result: ChaosResult) -> bool:
        ...
        for criterion in experiment.success_criteria:
            if "error rate" in criterion.lower():
                ...
                ui_failures = sum(1 for obs in result.observations if 'health check failed' in obs.lower())
    ...
    def _store_experiment_result(self, result: ChaosResult):
        ...
        result_path = self.evidence_path / f"experiment_{result.experiment.name}_{int(time.time())}.json"
...
def main():
    ...
    if args.command == 'run':
        ...
        experiment = engineer.create_experiment(args.experiment, 
                                              args.failures or ["rate_limit_429"], 
                                              args.duration)

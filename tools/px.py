...
class PerplexityIntegration:
    ...
    def format_query_with_context(self, query: str, context_type: Optional[str] = None) -> str:
        """Format query with relevant IntelGraph context."""
        # Add IntelGraph context to the query
        context_prefix = ""
        
        if context_type == "architecture":
            context_prefix = "In the context of IntelGraph AI-augmented intelligence analysis platform: "
            ...
            context_prefix = "For a React/GraphQL application in an intelligence analysis platform: "
    ...
    def create_contextual_query(self, base_query: str, context_type: str = "auto") -> str:
        ...
        if context_type == "auto":
            ...
            if any(word in query_lower for word in ["neo4j", "cypher", "graph", "nodes", "relationships"]):
                context_type = "neo4j"
            elif any(word in query_lower for word in ["react", "component", "jsx", "frontend"]):
                context_type = "react"
            elif any(word in query_lower for word in ["ai", "ml", "machine learning", "embedding", "model"]):
                context_type = "ai"
            elif any(word in query_lower for word in ["deploy", "docker", "ci", "cd", "infrastructure"]):
                context_type = "devops"
            else:
...
def main():
    ...
    parser.add_argument("--context", "-c", choices=["architecture", "neo4j", "react", "ai", "devops", "auto"],
                       default="auto", help="Context type for query")
    parser.add_argument("--no-context", action="store_true", help="Don't add project context to query")
    parser.add_argument("--create-tools", action="store_true", help="Create additional browser workflow tools")

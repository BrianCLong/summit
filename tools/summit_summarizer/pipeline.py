...
SUMMARY_SYSTEM_PROMPT = "You are a meticulous multi-document summarizer who writes concise, factual reports."
EVALUATOR_SYSTEM_PROMPT = (
    "You are a rigorous evaluator who only approves summaries that are accurate, faithful, and responsive."
)
...
def _parse_evaluation(raw: str) -> EvaluationResult:
    try:
        ...
        return EvaluationResult(score=0.0, feedback=raw.strip() or "Unable to parse evaluation", approved=False)


class IterativeSummarizer:
    """Runs the SummIt-style iterative refine/evaluate loop on a single document."""

    def __init__(self, summarizer_llm: LLMClient, evaluator_llm: LLMClient, config: SummarizationConfig | None = None):
    ...
    def summarize(self, document: Document, query: str) -> SummaryResult:
        ...
        for _ in range(self.config.max_rounds):
            summary_text = self.summarizer_llm.complete(
                _build_summary_prompt(document, query, feedback), system_prompt=SUMMARY_SYSTEM_PROMPT
            ).strip()
            eval_raw = self.evaluator_llm.complete(
                _build_evaluation_prompt(summary_text, document, query), system_prompt=EVALUATOR_SYSTEM_PROMPT
            )
            evaluation = _parse_evaluation(eval_raw)
            iterations.append(SummaryIteration(draft=summary_text, feedback=evaluation.feedback, score=evaluation.score))
            if evaluation.is_passing(self.config.quality_threshold):
                break
            feedback = evaluation.feedback

        return SummaryResult(document=document, summary=summary_text, iterations=iterations, evaluation=evaluation)
...
class SummarizationPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self.evaluator_llm = evaluator_llm
        self.config = config or SummarizationConfig()
        self.iterative_summarizer = IterativeSummarizer(summarizer_llm, evaluator_llm, config=self.config)
    ...
    def merge_summaries(self, summaries: Iterable[SummaryResult], query: str) -> str:
        combined = "\n\n".join(
            f"[{summary.document.title}] {summary.summary}" for summary in summaries if summary.summary.strip()
        )
        prompt = (
        ...
        return self.summarizer_llm.complete(prompt, system_prompt=SUMMARY_SYSTEM_PROMPT, temperature=self.config.merge_temperature).strip()
    ...
    def run(self, query: str, *, max_documents: int = 5, fact_check_threshold: float = 0.4) -> dict:
        ...
        if fact_check_report.issues:
            ...
            fact_check_report = fact_check_summary(merged_summary, documents, threshold=fact_check_threshold)

...
class ModelManager:
    ...
    def estimate_model_needs(self) -> list[str]:
        ...
        models = sorted(self.config["models"].items(), key=lambda x: x[1]["priority"])
    ...
    def list_models(self, show_all: bool = False) -> None:
        ...
        for name, model_info in sorted(
            self.config["models"].items(), key=lambda x: x[1]["priority"]
        ):
...
def main():
    ...
    parser.add_argument(
        "--config", default="tools/models.json", help="Path to models configuration file"
    )
    parser.add_argument(
        "--max-models", type=int, default=5, help="Maximum number of models to maintain"
    )
    ...
    list_parser.add_argument(
        "--show-all", action="store_true", help="Show detailed system information"
    )
    ...
    install_parser.add_argument(
        "--timeout", type=int, default=600, help="Installation timeout in seconds"
    )
    ...
    remove_parser.add_argument(
        "--force", action="store_true", help="Force removal of high-priority models"
    )

#!/usr/bin/env python3
import sys, json, http.client, duckdb, math

DB="rag/index/rag.duckdb"
q=" ".join(sys.argv[1:]).strip()
if not q:
    print("usage: rag_query_mmr.py <question>"); sys.exit(1)
    ...
    while len(selected)<min(k,len(candidates)):
        ...
        for i in cand:
            ...
            div = 0.0 if not selected else max(
                cos(candidates[i]["emb"], candidates[j]["emb"]) for j in selected
            )
            score = lam*rel - (1-lam)*div
            if score>best_score: best_score=score; best_idx=i
        selected.append(best_idx); cand.remove(best_idx)
...
lconn.request("POST","/v1/chat/completions",

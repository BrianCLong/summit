...
class BenchmarkSuite:
    ...
    def load_benchmark_suite(self):
        ...
        self.benchmarks = {
            "speed_tests": [
                {
                    "task": "nl2cypher",
                    "prompt": "CREATE constraint for User.email",
                    "max_tokens": 100,
                    "temperature": 0.0,
                },
                {
            ...
            "quality_tests": [
                {
                    "task": "code",
                    "prompt": "Implement quicksort in Python with comments",
                    "expected_contains": ["def", "quicksort", "partition"],
                    "max_tokens": 300,
                },
                {
                    "task": "cypher",
                    "prompt": "Create a Cypher query to find users with most connections",
                    "expected_contains": ["MATCH", "COUNT", "ORDER BY"],
                    "max_tokens": 150,
                },
                {
                    "task": "reasoning",
                    "prompt": "Explain why the sky is blue in simple terms",
                    "expected_contains": ["light", "scattering", "blue"],
                    "max_tokens": 200,
                },
            ],
            "consistency_tests": [
            ...
            "stress_tests": [
                ...
                {
                    "task": "long_context",
                    "prompt": "Analyze this text: " + "Lorem ipsum dolor sit amet. " * 100,
                    "max_tokens": 200,
                },
            ],
        }
    ...
    def get_available_models(self) -> list[str]:
        ...
        try:
            ...
            if models_caps_path.exists():
                ...
                with open(models_caps_path) as f:
    ...
    def run_single_benchmark(self, model: str, benchmark: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            metrics = {
                ...
                "words_per_second": (
                    len(response.split()) / (duration_ms / 1000) if duration_ms > 0 else 0
                ),
            ...
            if "expected_contains" in benchmark:
                contains_all = all(
                    term.lower() in response.lower() for term in benchmark["expected_contains"]
                )
                metrics["quality_score"] = 1.0 if contains_all else 0.0
                metrics["missing_terms"] = [
                    term
                    for term in benchmark["expected_contains"]
                    if term.lower() not in response.lower()
                ]

            if "expected_exact" in benchmark:
                metrics["exactness_score"] = (
                    1.0 if response.strip() == benchmark["expected_exact"] else 0.0
                )
            ...
            return {
                ...
                "request": {
                    "prompt": (
                        benchmark["prompt"][:100] + "..."
                        if len(benchmark["prompt"]) > 100
                        else benchmark["prompt"]
                    ),
                    "task": benchmark["task"],
    ...
    def run_consistency_test(self, model: str, benchmark: dict[str, Any]) -> dict[str, Any]:
        ...
        consistency_metrics = {
            ...
            "duration_consistency": (
                1.0 - (statistics.stdev(durations) / statistics.mean(durations))
                if len(durations) > 1
                else 1.0
            ),
    ...
    def run_concurrent_test(self, model: str, benchmark: dict[str, Any]) -> dict[str, Any]:
        ...
        concurrent_metrics = {
            ...
            "avg_latency_ms": (
                statistics.mean([r["metrics"]["duration_ms"] for r in successful_results])
                if successful_results
                else 0
            ),
            "throughput_rps": (
                len(successful_results) / (total_duration / 1000) if total_duration > 0 else 0
            ),
        }
    ...
    def run_benchmark_suite(self, models: list[str] | None = None) -> dict[str, Any]:
        ...
        suite_results = {
            "suite_id": hashlib.md5(f"{datetime.now().isoformat()}{models}".encode()).hexdigest()[
                :12
            ],
            "timestamp": datetime.now(UTC).isoformat(),
    ...
    def analyze_benchmark_results(self, suite_results: dict[str, Any]) -> dict[str, Any]:
        ...
        analysis = {"model_rankings": {}, "performance_summary": {}, "recommendations": []}
        ...
        for model in models:
            ...
            # Quality score
            quality_scores = [
                r["metrics"].get("quality_score", 0) for r in results["quality_tests"]
            ]
            quality_score = statistics.mean(quality_scores) if quality_scores else 0

            # Reliability score
            all_results = (
                results["speed_tests"] + results["quality_tests"] + results["stress_tests"]
            )
            success_rate = sum(1 for r in all_results if r["metrics"]["success"]) / len(all_results)

            # Consistency score
            consistency_scores = [
                r["consistency_metrics"]["response_consistency"]
                for r in results["consistency_tests"]
            ]
            ...
            analysis["performance_summary"][model] = {
                ...
                "avg_latency_ms": (
                    round(statistics.mean(speed_durations), 0) if speed_durations else "N/A"
                ),
            }
        ...
        if ranked_models:
            ...
            analysis["recommendations"] = [
                ...
                (
                    f"🔧 Needs improvement: {worst_model}"
                    if len(ranked_models) > 1
                    else "All models performing well"
                ),
            ]
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick benchmark (speed tests only)"
    )
    ...
    if args.report:
        ...
        if args.quick:
            ...
            if "analysis" in results:
                ...
                for rank, data in analysis["model_rankings"].items():
                    print(
                        f"  {rank}: {data['model']} (composite: {data['composite_score']}, latency: {data['avg_latency_ms']}ms)"
                    )

...
async function generateDailyMerkleProof(filePaths: string[], outputPath = 'daily-merkle.json'): Promise<void> {
  ...
  try {
    ...
    if (proof.hsmSignature) {
      try {
        ...
        const verification = await dualNotary.verifyNotarizedRoot(mockNotarized);

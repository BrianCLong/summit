...
class SymphonyTracer:
    ...
    def create_decision_span(self, decision_id: str, task: str, 
                           model: str, policy_version: str,
                           quota_snapshot: Dict[str, int],
                           overrides: Dict[str, Any] = None) -> trace.Span:
    ...
    def create_provider_span(self, parent_span: trace.Span, provider: str, 
                           model: str, tokens_estimate: int) -> trace.Span:
    ...
    def record_decision_outcome(self, span: trace.Span, quality_score: float,
                              actual_cost: float, actual_latency: float,
                              tokens_used: int, error: Optional[Exception] = None):
        ...
        if actual_latency > self.slo_latency_p95:
            breaches.append({
                ...
                "threshold": self.slo_latency_p95
            })
    ...
    def create_ui_span(self, operation: str, user_id: Optional[str] = None) -> trace.Span:
...
_tracer: Optional[SymphonyTracer] = None
...
def trace_decision(decision_id: str, task: str, model: str, 
                  policy_version: str, quota_snapshot: Dict[str, int],

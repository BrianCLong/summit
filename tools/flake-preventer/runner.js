...
if (runCpu) {
    ...
    for (let i = 0; i < cpus; i++) {
        workers.push(new worker_threads_1.Worker(cpuWorkerCode, { eval: true }));
    }
}
if (runIo) {
    console.log(`[FlakePreventer] Spawning IO worker...`);
    workers.push(new worker_threads_1.Worker(ioWorkerCode, { eval: true }));
}
if (runNet) {
    console.log(`[FlakePreventer] Spawning Network worker...`);
    workers.push(new worker_threads_1.Worker(netWorkerCode, { eval: true }));
}
// --- Run Command ---

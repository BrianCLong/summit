...
class ImportVerifier {
  ...
  async verifyAndExtract(options: ImportOptions): Promise<ValidationResult> {
    return tracer.startActiveSpan('import.verify_extract', async (span: Span) => {
      ...
      try {
        ...
        result.valid = false;
        result.errors.push(`Import verification failed: ${(error as Error).message}`);
        return result;
      } finally {
        span.end();
      }
  ...
  private async extractBundle(bundlePath: string): Promise<void> {
    return tracer.startActiveSpan('import.extract_bundle', async (span: Span) => {
      ...
      try {
        ...
        span.recordException(error as Error);
        throw new Error(`Bundle extraction failed: ${(error as Error).message}`);
      } finally {
        span.end();
      }
    });
  ...
  private async validateManifest(): Promise<{ valid: boolean; manifest?: ImportManifest; errors: string[]; warnings: string[] }> {
    return tracer.startActiveSpan('import.validate_manifest', async (span: Span) => {
      const errors: string[] = [];
      ...
      try {
        ...
        const requiredFields = ['version', 'exportId', 'tenantId', 'region', 'purpose', 'createdAt', 'files'];
        ...
        const validPurposes = ['legal_compliance', 'data_migration', 'analytics', 'backup'];
  ...
  private async verifyFileIntegrity(manifest: ImportManifest): Promise<{ valid: boolean; errors: string[]; warnings: string[] }> {
    return tracer.startActiveSpan('import.verify_file_integrity', async (span: Span) => {
      const errors: string[] = [];
      const warnings: string[] = [];
      ...
      try {
        for (const file of manifest.files) {
          ...
          try {
            ...
            if (manifest.checksums[file.filename] && manifest.checksums[file.filename] !== actualChecksum) {
              errors.push(`Checksum record mismatch for ${file.filename}`);
            }

          } catch (error) {
            errors.push(`File verification failed for ${file.filename}: ${(error as Error).message}`);
          }
        }
        ...
        span.recordException(error as Error);
        errors.push(`File integrity verification failed: ${(error as Error).message}`);
        return { valid: false, errors, warnings };
      } finally {
        span.end();
      }
  ...
  private async verifySignature(bundlePath: string, cosignPubKey: string): Promise<SignatureValidation> {
    return tracer.startActiveSpan('import.verify_signature', async (span: Span) => {
      ...
      try {
        ...
        result.errors.push(`Signature verification failed: ${(error as Error).message}`);
  ...
  private async validateProvenance(manifest: ImportManifest): Promise<ProvenanceValidation> {
    return tracer.startActiveSpan('import.validate_provenance', async (span: Span) => {
      ...
      try {
        ...
        for (const entry of provenance) {
          if (!validActors.includes(entry.actor) && !entry.actor.includes('@')) {
        ...
        for (let i = 1; i < provenance.length; i++) {
          ...
          if (gapMs > 24 * 60 * 60 * 1000) { // 24 hours
            result.warnings = result.warnings || [];
            result.warnings.push(`Large time gap in provenance chain: ${gapMs / (60 * 60 * 1000)} hours`);
          }
        }

        result.valid = result.chainComplete && result.chronologicalOrder && result.actorsValid;
        ...
        result.valid = false;
        result.errors.push(`Provenance validation failed: ${(error as Error).message}`);
        span.recordException(error as Error);
      } finally {
        span.end();
      }
  ...
  private async validateAttestation(bundlePath: string, manifest: ImportManifest): Promise<AttestationValidation> {
    return tracer.startActiveSpan('import.validate_attestation', async (span: Span) => {
      ...
      try {
        ...
        result.valid = result.subjectMatches && result.builderTrusted && result.errors.length === 0;
        ...
        result.errors.push(`Attestation validation failed: ${(error as Error).message}`);
        span.recordException(error as Error);
      } finally {
        span.end();
      }
  ...
  private checkExpiration(manifest: ImportManifest): { valid: boolean; error?: string } {
  ...
  private async extractToOutput(outputDir: string, manifest: ImportManifest): Promise<void> {

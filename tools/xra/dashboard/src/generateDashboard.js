...
function parseArgs(argv) {
    let input = "";
    let output = path_1.default.resolve(process.cwd(), "dashboard.html");
    for (let i = 2; i < argv.length; i++) {
        const token = argv[i];
        if (token === "--input" && i + 1 < argv.length) {
            input = path_1.default.resolve(process.cwd(), argv[++i]);
        }
        else if (token === "--output" && i + 1 < argv.length) {
            output = path_1.default.resolve(process.cwd(), argv[++i]);
        }
    }
    if (!input) {
...
<body>
  ...
  <script>
    ...
    const fairnessLabels = payload.fairnessSeries.map(item => `;
    k = $;
    {
        item.k;
    }
    ...
    const coverageLabels = payload.coverageSeries.map(item => `;
    k = $;
    {
        item.k;
    }
...
function main() {
    ...
    (0, fs_1.writeFileSync)(args.output, html, "utf8");

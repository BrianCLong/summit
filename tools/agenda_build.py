...
def format_issue_section(title: str, issues: List[Dict[str, Any]], max_items: int = 5) -> str:
    ...
    for issue in sorted_issues[:max_items]:
        ...
        priority_labels = [l.get('name', '') for l in labels if 'priority' in l.get('name', '').lower()]
...
def get_sprint_status() -> str:
    ...
    try:
        workflows = run_gh_command('gh run list --limit 5 --json conclusion,workflowName,createdAt --status completed')
        if workflows:
            failed_count = sum(1 for w in workflows if w.get('conclusion') == 'failure')
            if failed_count > 0:
                status_info.append(f"⚠️  **Recent Failures**: {failed_count} workflow failures in last 5 runs")
            else:
                status_info.append("✅ **CI Status**: All recent workflows passing")
    except:
        pass  # Workflow status is optional
    
    return "\n".join(f"- {info}" for info in status_info) if status_info else "- *No status information available*"
...
def generate_agenda(issues: Dict[str, List[Dict[str, Any]]], date: str) -> str:
    ...
    agenda_lines.append(format_issue_section("🎯 Decisions Requiring Input", issues.get('decisions', [])))
    agenda_lines.append(format_issue_section("🚨 High Priority Issues", issues.get('high_priority', [])))
    agenda_lines.append(format_issue_section("🔄 Change Requests", issues.get('change_requests', [])))
    agenda_lines.append(format_issue_section("⚠️  Current Risks", issues.get('risks', [])))
    agenda_lines.append(format_issue_section("🚧 Blockers & Dependencies", issues.get('blockers', [])))
...
def main():
    ...
    parser.add_argument("--output-dir", "-o", type=str, default="pm/agendas",
                       help="Output directory (default: pm/agendas)")
    parser.add_argument("--filename", "-f", type=str, 
                       help="Custom filename (default: auto-generated from date)")
    parser.add_argument("--dry-run", action="store_true",
                       help="Print agenda to console instead of saving to file")
    ...
    try:
        ...
        print(f"   📋 {total_items} items across {len([k for k, v in issues.items() if v])} categories")

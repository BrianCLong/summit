...
exports.DEFAULT_RULE_CATALOG = void 0;
exports.parseLegalText = parseLegalText;
exports.diffClauses = diffClauses;
exports.mapDiffsToPolicyDeltas = mapDiffsToPolicyDeltas;
exports.simulateImpact = simulateImpact;
exports.signImpactReport = signImpactReport;
exports.verifyImpactReportSignature = verifyImpactReportSignature;
exports.renderImpactReport = renderImpactReport;
exports.buildPolicyPullRequest = buildPolicyPullRequest;
exports.runLtdim = runLtdim;
const node_crypto_1 = __importDefault(require("node:crypto"));
const DEFAULT_PRIVATE_KEY = process.env.LTDIM_PRIVATE_KEY ||
...
exports.DEFAULT_RULE_CATALOG = {
    rules: [
        {
            id: 'cross-border-eu-only',
            description: 'Restrict cross-border transfers to EU sovereign regions when clause prohibits extra-EU processing.',
            clauseIds: ['Clause 3'],
...
function diffClauses(baseline, revised) {
...
function matchesRule(rule, diff, text) {
...
function buildPolicyDelta(rule, diff, text) {
    const template = {
        ...rule.delta,
...
function mapDiffsToPolicyDeltas(diffs, catalog) {
    const policyDeltas = [];
...
function simulateImpact(policyDeltas) {
    ...
    const sloImpact = Array.from(sloMap.values()).map((entry) => {
...
function canonicalizeReportPayload(payload) {
    return JSON.stringify(payload, null, 2);
}
function signImpactReport(payload, privateKeyPem = DEFAULT_PRIVATE_KEY) {
...
function verifyImpactReportSignature(canonicalPayload, signature, publicKeyPem = DEFAULT_PUBLIC_KEY) {
    const publicKey = node_crypto_1.default.createPublicKey(publicKeyPem);
    return node_crypto_1.default.verify(null, Buffer.from(canonicalPayload), publicKey, Buffer.from(signature, 'base64'));
}
function renderImpactReport(report) {
    ...
    if (report.payload.clauseDiffs.length === 0) {
        ...
        for (const clause of report.payload.clauseDiffs) {
            lines.push(`- ${clause.clauseId} (${clause.changeType}) — ${clause.afterHeading ?? clause.beforeHeading ?? 'Unspecified heading'}`);
        }
    }
    lines.push('');
    ...
    if (report.payload.policyDeltas.length === 0) {
        ...
        for (const delta of report.payload.policyDeltas) {
            lines.push(`- [${delta.status.toUpperCase()}] ${delta.policyId} (${delta.action}) ← rule ${delta.ruleId} (${delta.clauseId})`);
    ...
    lines.push(`- Changes — blocked: ${report.payload.impactSummary.blockedChanges}, allowed: ${report.payload.impactSummary.allowedChanges}, manual-review: ${report.payload.impactSummary.manualReviewChanges}`);
    ...
    if (report.payload.impactSummary.sloImpact.length === 0) {
        ...
        for (const slo of report.payload.impactSummary.sloImpact) {
            lines.push(`  • ${slo.metric}: ${slo.totalDelta}${slo.unit} (${slo.direction}); drivers: ${slo.drivers.join('; ')}`);
        }
    }
    lines.push('');
...
function buildPolicyPullRequest(baselineDoc, revisedDoc, policyDeltas, impactSummary) {
    const title = `LTDIM: ${baselineDoc.name} ${baselineDoc.version} → ${revisedDoc.version}`;
    const headerLines = [];
    headerLines.push('## Summary');
    headerLines.push(`- Blocked changes: ${impactSummary.blockedChanges}; allowed changes: ${impactSummary.allowedChanges}; manual review: ${impactSummary.manualReviewChanges}`);
    ...
    if (impactSummary.sloImpact.length > 0) {
        ...
        for (const slo of impactSummary.sloImpact) {
            headerLines.push(`  - ${slo.metric}: ${slo.totalDelta}${slo.unit} (${slo.direction}); drivers: ${slo.drivers.join(', ')}`);
        }
    }
    else {
    ...
    if (policyDeltas.length === 0) {
        ...
        for (const delta of policyDeltas) {
            headerLines.push(`- ${delta.policyId} (${delta.action}, ${delta.status}) — ${delta.summary} [rule ${delta.ruleId}]`);
            if (delta.beforeState) {
    ...
    const diffs = policyDeltas.map((delta) => ({
...
function runLtdim(options) {
    const catalog = options.catalog ?? exports.DEFAULT_RULE_CATALOG;

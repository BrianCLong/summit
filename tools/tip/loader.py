  ...
  for file in sorted(files):
    ...
    for doc in parsed:
      ...
      if not isinstance(doc, dict):
        raise ValueError(f"Unsupported manifest in {file}: expected mapping, got {type(doc)}")
    ...
    if suffix in {".yaml", ".yml", ".json"}:
      ...
      for idx, doc in enumerate(loaded):
        if not doc:
          continue
        if not isinstance(doc, dict):
          raise ValueError(f"Unsupported policy in {file}: expected mapping, got {type(doc)}")

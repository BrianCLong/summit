...
export class FlakyTestDetector {
  ...
  async runDetection(): Promise<void> {
    ...
    console.log(`=Ê Analyzing ${testResults.length} test results`);
    ...
    console.log(`=È Overall test stability: ${stability.overallStability.toFixed(2)}%`);
  ...
  async validateFix(testFile: string, testName: string, runs = 50): Promise<boolean> {
    console.log(`>ê Validating fix for ${testFile}::${testName} over ${runs} runs...`);
    ...
    console.log(`\n=Ê Validation results: ${failures}/${runs} failures (${(failureRate * 100).toFixed(1)}%)`);

...
def load_capabilities():
    ...
    if CAPS_FILE.exists():
        ...
        return {
            ...
            "tasks": {"interactive": {"preferred": ["local/llama"], "pack": "base"}},
        }
        ...
        if scheduler_path.exists():
            try:
                ...
                result = subprocess.check_output(
                    [sys.executable, str(scheduler_path), task, "1", "dev"], text=True, timeout=10
                )
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: ask_with_pack.py <task> <question> [RULES='...'] [CONTEXT='...'] [MODEL='...']",
            file=sys.stderr,
        )
        print("\nExamples:")
    ...
    user_prompt = render_template(user_template, QUESTION=question, CONTEXT=context, RULES=rules)

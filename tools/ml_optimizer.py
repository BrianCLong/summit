...
class SymphonyMLOptimizer:
    ...
    def get_performance_data(
        self, job_type: str | None = None, days_back: int = 30
    ) -> pd.DataFrame:
    ...
    def predict_duration(self, job_type: str, system_metrics: dict) -> float:
        ...
        df_similar_load = df[
            (df["load_avg"] >= current_load * 0.8) & (df["load_avg"] <= current_load * 1.2)
        ]
    ...
    def recommend_optimal_timing(self, job_type: str) -> dict[str, any]:
        ...
        hourly_stats = (
            df.groupby("hour")
            .agg(
                {"duration_seconds": ["mean", "std"], "cpu_percent_avg": "mean", "success": "mean"}
            )
            .round(2)
        )
        ...
        current_hour = datetime.now().hour
        current_score = (
            hourly_stats.loc[current_hour, "score"] if current_hour in hourly_stats.index else 50
        )
    ...
    def generate_insights(self) -> dict[str, any]:
        ...
        if len(df) > 0:
            ...
            job_stats = (
                df.groupby("job_type")
                .agg({"duration_seconds": "mean", "success": "mean", "cost_usd": "mean"})
                .round(2)
            )
            ...
            provider_stats = (
                df.groupby("provider")
                .agg({"duration_seconds": "mean", "success": "mean", "cost_usd": "sum"})
                .round(2)
            )

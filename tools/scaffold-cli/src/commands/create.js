...
const CreateOptionsSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).regex(/^[a-z][a-z0-9-]*$/),
    type: zod_1.z.enum(ServiceTypes),
    description: zod_1.z.string().optional(),
    team: zod_1.z.string().optional(),
    tier: zod_1.z.number().min(1).max(3).default(2),
    port: zod_1.z.number().min(1).max(65535).default(8080),
});
...
async function gatherOptions(type, cliOptions) {
    const answers = { ...cliOptions };
    ...
    if (!answers['name']) {
        const response = await (0, enquirer_1.prompt)({
            ...
            validate: (value) => /^[a-z][a-z0-9-]*$/.test(value) || 'Must be kebab-case starting with a letter',
        });
    ...
    return CreateOptionsSchema.parse({
        ...answers,
        tier: parseInt(answers['tier'], 10) || 2,
        port: parseInt(answers['port'], 10) || 8080,
    });
}
async function createService(options, dryRun) {
    const spinner = (0, ora_1.default)('Creating service...').start();
    const context = buildTemplateContext(options);
    const templateDir = (0, paths_js_1.getTemplateDir)(options.type);
    const outputDir = (0, paths_js_1.getOutputDir)(options.type, options.name);
    if (dryRun) {
    ...
    spinner.succeed(chalk_1.default.green(`Created ${options.name} in ${outputDir}`));
    // Print next steps
...
async function processTemplateDirectory(templateDir, outputDir, context) {
    ...
    const files = await fs_extra_1.default.readdir(templateDir, { recursive: true });
    for (const file of files) {
        const sourcePath = path_1.default.join(templateDir, file.toString());
        const stat = await fs_extra_1.default.stat(sourcePath);
        if (stat.isDirectory())
            continue;
        // Process filename (replace template variables)
        const processedFilename = handlebars_1.default.compile(file.toString())(context);
        const targetPath = path_1.default.join(outputDir, processedFilename);
        await fs_extra_1.default.ensureDir(path_1.default.dirname(targetPath));
        // Process file content
        const content = await fs_extra_1.default.readFile(sourcePath, 'utf-8');
        const processedContent = handlebars_1.default.compile(content)(context);
        await fs_extra_1.default.writeFile(targetPath, processedContent);
    }
}
async function generateDefaultStructure(outputDir, context) {
    ...
    for (const dir of dirs) {
        await fs_extra_1.default.ensureDir(path_1.default.join(outputDir, dir));
    }
}
async function generateConfigFiles(outputDir, options, context) {
    // Generate package.json
    const packageJson = generatePackageJson(context, options);
    await fs_extra_1.default.writeJson(path_1.default.join(outputDir, 'package.json'), packageJson, { spaces: 2 });
    // Generate tsconfig.json
    const tsconfig = generateTsConfig();
    await fs_extra_1.default.writeJson(path_1.default.join(outputDir, 'tsconfig.json'), tsconfig, { spaces: 2 });
    // Generate .eslintrc.json
    const eslintrc = generateEslintConfig();
    await fs_extra_1.default.writeJson(path_1.default.join(outputDir, '.eslintrc.json'), eslintrc, { spaces: 2 });
    // Generate jest.config.js
    const jestConfig = generateJestConfig(options);
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'jest.config.js'), jestConfig);
...
function generatePackageJson(context, options) {
    return {
...
async function generateSourceFiles(outputDir, context, options) {
    // index.ts
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'src/index.ts'), indexTs);
    // app.ts
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'src/app.ts'), appTs);
    // config.ts
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'src/config.ts'), configTs);
    // logger.ts
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'src/logger.ts'), loggerTs);
    // metrics.ts
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'src/metrics.ts'), metricsTs);
    // routes/health.ts
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'src/routes/health.ts'), healthTs);
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'tests/unit/app.test.ts'), appTestTs);
}
async function generateSloFile(outputDir, context, options) {
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'slos/slos.yaml'), sloYaml);
}
async function generateDockerfile(outputDir, context, _options) {
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'Dockerfile'), dockerfile);
}
async function generateEnvExample(outputDir, context, options) {
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, '.env.example'), envExample);
}
async function generateReadme(outputDir, context, options) {
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'README.md'), readme);
}
async function generateCiWorkflow(outputDir, context) {
...
`;
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, '.github/workflows/ci.yml'), ciYaml);
}
async function generateOpaPolicy(outputDir, context) {
...
# }
`;
    await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'policies/authz.rego'), policy);
}
async function previewFiles(templateDir, outputDir, context) {
    ...
    for (const file of files) {
        console.log(chalk_1.default.gray(`  ${outputDir}/${file}`));
    }
    console.log(chalk_1.default.cyan(`\nTemplate context:`));
    console.log(chalk_1.default.gray(JSON.stringify(context, null, 2)));
}
function printNextSteps(options, outputDir) {
    console.log(chalk_1.default.cyan('\nNext steps:\n'));
    console.log(chalk_1.default.white(`  1. cd ${outputDir}`));
    console.log(chalk_1.default.white('  2. Review .env.example and create .env'));
    console.log(chalk_1.default.white("  3. Run 'pnpm install' to install dependencies"));
    console.log(chalk_1.default.white("  4. Run 'pnpm dev' to start development server"));
    console.log(chalk_1.default.white('  5. Update README.md with service-specific documentation'));
    console.log(chalk_1.default.white("  6. Run 'pnpm test' to verify test setup\n"));
    console.log(chalk_1.default.cyan('Documentation:'));
    console.log(chalk_1.default.gray('  - Platform Blueprint: docs/golden-path-platform/PLATFORM_BLUEPRINT.md'));
    console.log(chalk_1.default.gray('  - CI/CD Pipeline: docs/golden-path-platform/CICD_PIPELINE.md'));
    console.log(chalk_1.default.gray('  - Onboarding Checklist: docs/golden-path-platform/ONBOARDING_CHECKLIST.md\n'));
}

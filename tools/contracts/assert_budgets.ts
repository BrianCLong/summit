...
async function get(path:string){ const t=Date.now(); const r=await fetch(process.env.PREVIEW_BASE+path); return { ms:Date.now()-t, s:r.status }; }
(async ()=>{
  const paths=["/health","/api/ops/capacity?tenant=acme&from=2025-09-01&to=2025-09-07"];
  ...
  if(!ok){ console.error(JSON.stringify({want:spec.budgets, got:{p95,err}},null,2)); process.exit(1); }

...
exports.evaluateFreeze = evaluateFreeze;
exports.guardMergeTrain = guardMergeTrain;
...
const FREEZE_FILE = path_1.default.join('.maestro', 'freeze_windows.json');
function readFreezeWindows() {
    if (!fs_1.default.existsSync(FREEZE_FILE))
        return [];
    const raw = fs_1.default.readFileSync(FREEZE_FILE, 'utf8');
 ...
 */
function evaluateFreeze(options = {}) {
...
function evaluateOverrideWithOpa(token, reasons, nowIso) {
    ...
    const result = (0, child_process_1.spawnSync)('opa', ['eval', '-I', '-f', 'json', '-d', 'policy/', 'data.summit.merge_train'], { input: JSON.stringify(input) });
    if (result.error) {
 ...
 * @param {FreezeCheckOptions} options
 */
function guardMergeTrain(options = {}) {

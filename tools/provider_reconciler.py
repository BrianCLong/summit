...
class OpenAIAdapter(ProviderAdapter):
    ...
    def parse_rate_limit_headers(self, headers: Dict[str, str]) -> Optional[QuotaInfo]:
...
class ProviderReconciler:
    ...
    def get_live_quota(self, provider: str, quota_type: str = "rpm") -> Optional[QuotaInfo]:
    ...
    def auto_adjust_runtime_caps(self, diffs: List[PolicyDiff]):
        ...
        for diff in diffs:
            if diff.recommended_action == "update_runtime_cap" and diff.confidence > 0.7:
                ...
                logger.info(f"Auto-adjusted runtime cap for {diff.provider} {diff.quota_type}: {new_cap}")
    ...
    if args.command == 'collect':
        ...
        for provider, provider_quotas in quotas.items():
            ...
            for quota in provider_quotas:
                print(f"  {quota.quota_type}: {quota.current_usage}/{quota.quota_limit} "
        ...
        for diff in diffs:
            print(f"  {diff.provider} {diff.quota_type}: "
        ...
        if args.provider:
            ...
            if quota:
                print(f"{args.provider}: {quota.current_usage}/{quota.quota_limit} "

...
exports.generateHealthMetrics = generateHealthMetrics;
...
async function generateHealthMetrics() {
    ...
    const missingDoD = items.filter((item) => {
        if (!['in_prod', 'validated'].includes(item.status))
            return false;
    ...
    const completedItems = items.filter((item) => item.status === 'validated' && item.started_at && item.validated_at);
    let avgDays;
...
async function calculateHealthScore() {
    ...
    if (metrics.missing_acceptance_criteria > 0) {
        const pct = Math.round((metrics.missing_acceptance_criteria / (metrics.total_items || 1)) * 100);
        quality -= pct;
...
exports.default = generateHealthMetrics;

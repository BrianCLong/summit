...
function generateTableReport(data) {
    ...
    if (data.staging.length === 0) {
        ...
        // Group by component
        const byComponent = data.staging.reduce((acc, item) => {
        ...
        lines.push((0, table_1.table)(componentTable));
        lines.push(`Total staging items: ${data.staging.length}`);
        // Group by confidence
        const byConfidence = data.staging.reduce((acc, item) => {
    ...
    if (data.backlog) {
        ...
        lines.push((0, table_1.table)(statusTable));
        ...
        const staleInProgress = items.filter((item) => {
            if (item.status !== 'in_progress')
                return false;
            if (!item.started_at)
                return true; // No start date = stale
...
function generateMarkdownReport(data) {
    ...
    if (data.backlog) {
        ...
        const validatedRate = items.filter((i) => i.status === 'in_prod').length > 0
            ? Math.round((items.filter((i) => i.status === 'validated').length /
...
// Main Export
// =============================================================================
async function generateReport(format = 'table') {

...
function score(issue) {
    ...
    let s = 0;
    if (/\bperf|latency|p95|slow\b/i.test(issue.title + issue.body))
        s += 3;
    if (/\bflake|flaky|intermittent\b/i.test(issue.title + issue.body))
        s += 4;
    if (/\bbuild|ci|deploy|helm|keda\b/i.test(issue.title + issue.body))
        s += 2;
    if ((issue.labels || []).find((l) => l.name === 'customer'))
        s += 5;
    return s;
}
async function run() {
    ...
    for (const { i } of next) {
        ...
        if (/\bflake|test\b/i.test(i.title + i.body))
            labels.push(LABELS.test);
    ...
    const ww = new Date();
    const wk = Math.ceil(((ww - new Date(ww.getFullYear(), 0, 1)) / 86400000 +
    ...
    for (const { i } of next)
        await gh.rest.issues.update({
            ...
            repo: REPO[1],
            issue_number: i.number,
            milestone: ms.data?.number || ms.number,
        });
}
run().catch((e) => {
    console.error(e);

...
class MultiModalProcessor:
    ...
    def init_db(self):
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_links_primary ON cross_modal_links(primary_asset_id)"
        )
    ...
    def ingest_media(self, filepath: str | Path) -> int:
    ...
    def process_asset(self, asset_id: int) -> dict[str, Any]:
        ...
        conn.execute(
            "UPDATE media_assets SET processing_status = 'processing' WHERE id = ?", (asset_id,)
        )
        conn.commit()
        ...
        try:
            ...
            conn.execute(
                "UPDATE media_assets SET processing_status = 'completed' WHERE id = ?", (asset_id,)
            )

        except Exception as e:
            print(f"❌ Processing failed for asset {asset_id}: {e}")
            conn.execute(
                "UPDATE media_assets SET processing_status = 'failed' WHERE id = ?", (asset_id,)
            )
            results["error"] = str(e)
    ...
    def analyze_text_sentiment(self, filepath: Path, asset_info: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "confidence": (
                min(0.9, total_sentiment_words / 10) if total_sentiment_words > 0 else 0.3
            ),
        }
    ...
    def detect_image_objects(self, filepath: Path, asset_info: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "objects": [{"label": "placeholder", "confidence": 0.5, "bbox": [0, 0, 100, 100]}],
        ...
        try:
            ...
            with Image.open(filepath) as img:
                ...
                return {
                    ...
                    "orientation": (
                        "portrait" if is_portrait else "landscape" if is_landscape else "square"
                    ),
                    "color_mode": mode,
    ...
    def analyze_data_schema(self, filepath: Path, asset_info: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
                    ...
                    if isinstance(obj, dict):
                        for key, value in obj.items():
                            ...
                            if isinstance(value, (dict, list)):
                                schema[key]["nested"] = analyze_json_structure(
                                    value, f"{path}.{key}" if path else key
                                )
                    elif isinstance(obj, list) and obj:
                        schema["array_element_type"] = type(obj[0]).__name__
                        if isinstance(obj[0], (dict, list)):
                            schema["array_element_schema"] = analyze_json_structure(
                                obj[0], f"{path}[0]"
                            )
            ...
            return {"schema": {}, "data_type": "unknown", "error": str(e), "confidence": 0.1}
    ...
    def detect_data_patterns(self, filepath: Path, asset_info: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"patterns": {}, "error": str(e), "confidence": 0.1}
    ...
    def cross_modal_analysis(self, asset_ids: list[int]) -> dict[str, Any]:
        ...
        for asset_id in asset_ids:
            ...
            # Get features
            cursor = conn.execute(
                "SELECT feature_type, feature_data FROM modality_features WHERE asset_id = ?",
                (asset_id,),
            )
        ...
        for text_id in text_assets:
            ...
            for image_id in image_assets:
                ...
                if "keyword_extraction" in text_features and "ocr_text" in image_features:
                    text_keywords = set(
                        kw["word"] for kw in text_features["keyword_extraction"]["keywords"][:10]
                    )
    ...
    def generate_fusion_narrative(self, asset_ids: list[int]) -> dict[str, Any]:
        ...
        for asset_id in asset_ids:
            ...
            if filename:
                ...
                if modality == "text" and "keyword_extraction" in asset_features:
                    keywords = [
                        kw["word"] for kw in asset_features["keyword_extraction"]["keywords"][:5]
                    ]
        ...
        if correlation_analysis["correlations"]:
            ...
            for corr in correlation_analysis["correlations"][:3]:  # Top 3
                context_parts.append(
                    f"- {corr['relationship']} (confidence: {corr['confidence']:.2f})"
                )
...
def main():
    ...
    parser.add_argument(
        "--narrative", nargs="+", type=int, help="Generate fusion narrative for asset IDs"
    )
    ...
    if args.ingest:
        ...
        for correlation in results["correlations"]:
            print(
                f"  {correlation['relationship']}: {correlation['primary_asset']} ↔ {correlation['related_asset']} (confidence: {correlation['confidence']:.2f})"
            )
        ...
        print(
            "Usage: multimodal_fusion.py --ingest <path> | --process <id> | --analyze <id1> <id2> ... | --narrative <id1> <id2> ... | --stats"
        )

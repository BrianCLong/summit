import { Pool } from "pg"; import fs from "fs";
const pg = new Pool({ connectionString: process.env.DATABASE_URL });
type Candidate = { id:string; when:any; if?:string; then:any[]; support:number; precision:number };
...
async function mine(): Promise<Candidate[]> {
  ...
  const c1: Candidate = {
    ...
    then: [{do:"label", with:{labels:["needs:arch-review","owners:required"]}}],

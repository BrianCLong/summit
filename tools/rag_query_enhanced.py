...
class RAGQueryEngine:
    ...
    def __init__(self, db_path: str = "rag/index/rag.duckdb",
                 ollama_host: str = "127.0.0.1", ollama_port: int = 11434,
                 litellm_host: str = "127.0.0.1", litellm_port: int = 4000,
                 embed_model: str = "nomic-embed-text", chat_model: str = "local/llama"):
    ...
    def _embed_query(self, query: str, retries: int = 3) -> Optional[List[float]]:
        """Generate embedding for query text."""
        for attempt in range(retries):
            try:
                conn = http.client.HTTPConnection(
                    self.ollama_host, self.ollama_port, timeout=60
                )
    ...
    def generate_answer(self, query: str, search_results: List[SearchResult],
        ...
        # Build messages
        default_system = ("Answer the question using the provided context. "
        ...
        try:
            conn = http.client.HTTPConnection(
                self.litellm_host, self.litellm_port, timeout=120
            )
...
def format_results(results: List[SearchResult], show_sources: bool = True, 
...
def main():
    ...
    parser.add_argument("--db", default="rag/index/rag.duckdb", 
    ...
    parser.add_argument("--no-answer", action="store_true",
                       help="Only show search results, don't generate answer")
    ...
    parser.add_argument("--list-files", action="store_true",
                       help="List all indexed files and exit")
    parser.add_argument("--stats", action="store_true",
                       help="Show database statistics and exit")
    parser.add_argument("--chat-model", default="local/llama",
    ...
    try:
        ...
        if args.no_answer:
            ...
            for i, result in enumerate(results, 1):
                score_info = f" (similarity: {result.similarity_score:.3f})" if args.show_scores else ""

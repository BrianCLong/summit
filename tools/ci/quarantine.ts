...
const flakes = report.testResults.filter((t:any)=> t.status==="failed" && /Timeout|flaky|intermittent|retry/i.test(JSON.stringify(t)));
if (!flakes.length) process.exit(0);
const path = "tests/.quarantine.json";
let q = []; try{ q = JSON.parse(fs.readFileSync(path,"utf8")); }catch{}

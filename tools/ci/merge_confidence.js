...
const risk = JSON.parse(fs_1.default.readFileSync('risk.json', 'utf8')); // from v1.13
const tiaCount = safeRead('tia-tests.txt').split('\n').filter(Boolean).length;
const diff = Number((0, child_process_1.execSync)("git diff --shortstat origin/${GITHUB_BASE_REF}...HEAD | awk '{print $4+$6}'", { shell: 'bash' }).toString() || 0);
...
fs_1.default.writeFileSync('merge-confidence.json', JSON.stringify({ score, tiaCount, diff, risk: risk.bucket, flakeRate, coverage }, null, 2));

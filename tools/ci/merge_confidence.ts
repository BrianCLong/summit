...
const diff = Number(execSync("git diff --shortstat origin/${GITHUB_BASE_REF}...HEAD | awk '{print $4+$6}'",{shell:"bash"}).toString()||0);
...
fs.writeFileSync("merge-confidence.json", JSON.stringify({ score, tiaCount, diff, risk: risk.bucket, flakeRate, coverage }, null, 2));
console.log(`::notice ::merge_confidence=${score}`);

function safeRead(file:string){ try { return fs.readFileSync(file,"utf8"); } catch { return ""; } }
...
const changed = (0, node_child_process_1.execSync)('git diff --name-only origin/${GITHUB_BASE_REF}...HEAD')
    .toString()
    .trim()
    .split('\n');
const SIZE = Number((0, node_child_process_1.execSync)("git diff --shortstat origin/${GITHUB_BASE_REF}...HEAD | awk '{print $4+$6}'", { shell: 'bash' }).toString() || 0);
const HOT = [
    'server/src/steps',
...
fs_1.default.writeFileSync('risk.json', JSON.stringify({ score, bucket, SIZE, HOT, CFG, TEST, FLAKE_TOUCH }, null, 2));

...
const name = process.argv[3]; if(!name) throw new Error("Usage: pnpm bp:new <blueprint.yaml> <name>");
for (const f of bp.scaffold.files){
  const p = f.path.replace(/\$\{name\}/g,name); fs.mkdirSync(path.dirname(p),{recursive:true});

...
const arbs = {
  ...
  resource: fc.record({ kind: fc.constantFrom("Pod","Job"), name: fc.string(), ns: fc.string() }),
  details: fc.anything()
};

async function postAdmission(p:any){
  const r = await fetch(`${base}/api/admission/event`, { method:"POST", headers:{ "content-type":"application/json" }, body: JSON.stringify(p) });
  return r.status;
}

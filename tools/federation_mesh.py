...
class FederationMesh:
    ...
    def create_default_mesh_config(self) -> dict:
        ...
        return {
            ...
            "security": {
                ...
                "rate_limit": {"requests_per_minute": 100, "burst_allowance": 20},
            },
            "capabilities": {
    ...
    def verify_hmac_signature(
        self, data: bytes, timestamp: str, signature: str, peer_secret: str
    ) -> bool:
        """Verify HMAC signature from peer"""
        try:
            expected_sig = hmac.new(
                peer_secret.encode(), f"{timestamp}:{data.decode('utf-8')}".encode(), hashlib.sha256
            ).hexdigest()
        ...
        try:
            ...
            for i in range(1, 255):
                ...
                try:
                    ...
                    resp = requests.get(f"http://{ip}:{mesh_port}/node/info", timeout=2)
                    if resp.status_code == 200:
                        node_info = resp.json()
    ...
    def health_check_peer(self, peer_id: str) -> bool:
        ...
        try:
            resp = requests.get(f"{peer['url']}/health", timeout=5)
            ...
            if resp.status_code == 200:
                ...
                if peer["failure_count"] >= self.mesh_config.get("load_balancing", {}).get(
                    "failure_threshold", 3
                ):
                    peer["status"] = "failed"
                return False
    ...
    def get_best_node_for_task(self, task_type: str, requirements: dict = None) -> str | None:
        ...
        for peer_id, peer_info in self.peers.items():
            ...
            try:
                ...
                resp = requests.get(f"{peer_info['url']}/node/capabilities", timeout=3)
                if resp.status_code == 200:
                    peer_caps = resp.json()
    ...
    def execute_remote_task(self, peer_id: str, task_data: dict) -> dict:
        ...
        try:
            ...
            return {"error": f"Remote execution error: {e!s}"}
            ...
            def do_POST(self):
                if self.path == "/task/execute":
                    ...
                    if self.mesh.mesh_config.get("security", {}).get("require_hmac", True):
                        ...
                        if not self.mesh.verify_hmac_signature(
                            post_data, timestamp, signature, peer_secret
                        ):
                            self.send_response(401)
                            self.end_headers()
                            return

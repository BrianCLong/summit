...
const bad = [/ALTER\s+TABLE\s+\w+\s+ADD\s+COLUMN\s+\w+\s+(?!NULL)/i, // adding NOT NULL w/out default
             /\bDROP\s+TABLE\b/i,
             /\bALTER\s+TABLE\s+\w+\s+RENAME\s+COLUMN\b/i];
const requireSafe = [/ADD\s+COLUMN.*DEFAULT/i, /CREATE\s+INDEX\s+CONCURRENTLY/i];

let errors: string[] = [], warns: string[] = [];
for (const f of fs.readdirSync(MIG_DIR).filter(x=>x.endsWith(".sql")||x.endsWith(".ts"))) {
  ...
  if (!/--\s*ONLINE:/i.test(s)) warns.push(`${f}: missing '-- ONLINE:' rationale`);
  // require down/rollback section
  if (!/--\s*DOWN:/.test(s)) warns.push(`${f}: missing rollback plan ('-- DOWN:' section)`);
}
if (errors.length){ console.error("❌ migration errors:\n" + errors.join("\n")); process.exit(1); }
if (warns.length){ console.warn("⚠️ migration warnings:\n" + warns.join("\n")); }

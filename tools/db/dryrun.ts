...
const sqls = fs.readdirSync("server/src/migrations").filter(x=>x.endsWith(".sql"))
...
(async ()=>{
  ...
  if (locks.rowCount>0){ console.error("❌ would block locks:", locks.rows); process.exit(1); }
  await c.query("ROLLBACK"); await c.end(); console.log("✅ dry-run OK");
})();
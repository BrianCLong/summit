...
with DAG(
    ...
    sim_history = PythonOperator(
        task_id="generate_sim_history",
        python_callable=lambda: requests.post(
            "http://sim:9300/generate_history", json={"days": 7, "source": "SIM"}
        ).json(),
    )
    ...
    hybrid = PythonOperator(
        task_id="hybridization_detector", python_callable=hybridization_detector.run
    )

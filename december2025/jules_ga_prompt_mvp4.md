...
3) REMEDIATIONS (prioritized list with DRI, ETA, diff/PR links)
4) ARTIFACTS (paths/URLs): SBOM, attestation, DPIA, model card, runbooks
5) RELEASE NOTES (user-facing + admin)
6) GO/NO-GO SUMMARY with risks and rollback steps
...
B. Compliance & Privacy
  ...
  - Data retention policy enforced via configs; attach automated test proving purge at ${RETENTION_DAYS} days.
...
C. Reliability & Observability
  ...
  - Chaos test: single AZ failure; prove graceful degradation path (“safe mode”).
...
F. Documentation & Runbooks
  - Operator runbook: deploy, rollback, kill-switch; on-call quickstart; RTO/RPO declared and tested.
...
- Do not proceed past canary 25% if any KPI regresses >5% vs MVP-3 baseline.
- Hard stop if SBOM shows Critical CVEs without compensating controls.

BEGIN. First, produce a day-by-day plan and the initial GA_CHECKLIST (empty results). Then execute Gate A in parallel with Gate C, reporting artifacts as you go.
```

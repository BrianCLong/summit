...
def generate_html_report(latest_report_path, previous_report_path=None):
    ...
    <body>
        ...
        <p>Generated: """ + os.path.basename(latest_report_path) + """</p>
    """
        ...
        for metric in metrics:
            ...
            if isinstance(latest_val, (int, float)) and isinstance(prev_val, (int, float)):
                ...
                if metric == 'mean_score': # Higher is better
                    ...
                    delta_class = 'delta-down' if delta < 0 else ('delta-up' if delta > 0 else 'delta-neutral')
    ...
    html_filename = os.path.join(report_dir, os.path.basename(latest_report_path).replace('.json', '.html'))

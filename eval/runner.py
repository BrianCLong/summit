...
async def main(base: str, token: str, suite_path: str):
    ...
    async with httpx.AsyncClient(base_url=base, headers={"Authorization": f"Bearer {token}"}) as c:
        ...
        for task_config in suite["tasks"]:
            dataset_path = task_config["dataset"].replace("s3://", "GOLDEN/datasets/") # Adjust path for local
...
if __name__ == "__main__":
    ...
    parser.add_argument('--suite', type=str, default='eval/suites/router.yaml', help='Path to evaluation suite YAML')
    parser.add_argument('--base', type=str, default='http://localhost:8080', help='Base URL for Maestro API')

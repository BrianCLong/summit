...
class FileProvider {
    ...
    async get(key, ctx) {
        ...
        if (entry.environments && !entry.environments.includes(ctx.env))
            return null;
        if (typeof entry.value !== "undefined")
            return { key, value: entry.value, source: this.name };
        return null;
    }
}
class RemoteProvider {
    ...
    async get(key, ctx) {
        ...
        if (ctx.userRole)
            url.searchParams.set("role", ctx.userRole);
        const headers = {};
        if (this.etag)
...
class FlagClient {
    ...
    catalog = {};
    constructor(options) {
        this.options = { cacheTtlMs: 60_000, targetsDir: node_path_1.default.join(process.cwd(), "flags/targets"), ...options };
        this.providers = [
            new EnvProvider(),
    ...
    async get(key, defaultValue, ctxOverrides) {
        ...
        if (typeof fallback === "undefined")
            throw new Error(`Flag ${key} missing default value`);
        this.toCache({ key, value: fallback, source: "default" });
        return fallback;
    }
    toCache(resolution) {
        const expiresAt = Date.now() + (this.options.cacheTtlMs ?? 60_000);
        const entry = { value: resolution.value, source: resolution.source, etag: resolution.etag, expiresAt };
    ...
    loadCatalog() {
        const catalogPath = this.options.catalogPath ?? node_path_1.default.join(process.cwd(), "flags/catalog.yaml");

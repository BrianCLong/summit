...
import { InterpreterContext, InterpreterResult, ParsedCommand } from './types.js';
...
async function cmdPwd(ctx: InterpreterContext): Promise<InterpreterResult> {
  return {
    stdout: `${ctx.root}\n`,
    stderr: '',
    exitCode: 0,
...
export async function executeBashLike(
  ctx: InterpreterContext,
  command: string,
): Promise<InterpreterResult> {

...
describe('User Memory API (DSAR-grade)', () => {
    ...
    app.use('/v1/memory', index_1.default);
    const testRecord = {
        id: 'mem-api-1',
        userId: 'user-api-123',
    ...
    test('GET /v1/memory should require userId', async () => {
        const res = await (0, supertest_1.default)(app).get('/v1/memory');
        expect(res.status).toBe(400);
        expect(res.body.error).toBe('userId is required');
    });
    test('GET /v1/memory should validate purpose', async () => {
        const res = await (0, supertest_1.default)(app).get('/v1/memory?userId=u1&purpose=invalid');
        expect(res.status).toBe(400);
    ...
    test('DELETE /v1/memory/:id should return 204', async () => {
        const res = await (0, supertest_1.default)(app).delete('/v1/memory/non-existent');

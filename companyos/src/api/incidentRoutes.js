...
function createIncidentRoutes(db) {
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            if (req.query.commander)
                filter.commander = req.query.commander;
            if (req.query.customerImpact)
                filter.customerImpact = req.query.customerImpact === 'true';
    ...
    router.post('/:id/link-github', async (req, res) => {
        try {
            ...
            const incident = await incidentService.linkToGithub(req.params.id, githubIssueUrl, githubIssueNumber);
            if (!incident) {
                return res.status(404).json({ error: 'Incident not found' });
            }
            res.json({ incident });

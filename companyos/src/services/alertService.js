...
class AlertService {
    ...
    async listAlerts(filter, limit = 25, offset = 0) {
    ...
    async updateAlert(id, input) {
        ...
        if (input.status !== undefined) {
            ...
            if (input.status === alert_1.AlertStatus.RESOLVED) {
    ...
    async linkToIncident(alertId, incidentId) {
        const query = `
      UPDATE maestro.alerts

...
class DeploymentService {
    ...
    async listDeployments(filter, limit = 25, offset = 0) {
    ...
    async getDeploymentStats(serviceName, environment, days = 30) {
        let query = `
    ...
    async markSucceeded(id) {
        return this.updateDeployment(id, {
            status: deployment_1.DeploymentStatus.SUCCEEDED,
        });
    }
    async markFailed(id, errorMessage) {
        return this.updateDeployment(id, {
            status: deployment_1.DeploymentStatus.FAILED,
            errorMessage,
        });
    }
    async createRollback(originalDeploymentId, rolledBackBy) {
        ...
        await this.updateDeployment(originalDeploymentId, {
            status: deployment_1.DeploymentStatus.ROLLED_BACK,
        });
        // Link rollback
        await this.db.query(`UPDATE maestro.deployments SET rollback_of_deployment_id = $1 WHERE id = $2`, [originalDeploymentId, rollbackDeployment.id]);

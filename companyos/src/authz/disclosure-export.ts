...
import type { DisclosurePackResource, PolicyDecision, Subject } from "./types.js";
...
export async function evaluateDisclosureExport(
  ...
  try {
    const res = await request(`${OPA_URL}/v1/data/companyos/authz/disclosure_export/decision`, {
      method: "POST",
      headers: {
    ...
    console.warn("[authz] opa decision error", (error as Error).message ?? "unknown error");

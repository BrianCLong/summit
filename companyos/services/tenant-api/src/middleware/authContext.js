 ...
 */
async function evaluateOpaPolicy(input) {
 ...
 */
function logPolicyDecision(input, result) {
    const logEntry = {
...
const ROLE_PERMISSIONS = {
    'platform-admin': [
        exports.TenantActions.CREATE,
        exports.TenantActions.READ,
        exports.TenantActions.UPDATE,
        exports.TenantActions.DELETE,
        exports.TenantActions.LIST,
        exports.TenantActions.MANAGE_FEATURES,
        exports.TenantActions.VIEW_AUDIT,
    ],
    'tenant-admin': [
        exports.TenantActions.READ,
        exports.TenantActions.UPDATE,
    ...
    'tenant-viewer': [exports.TenantActions.READ],
};
/**
 * Check if a user has permission to perform an action
 * Uses OPA policy engine in production, with RBAC fallback in development
 */
async function checkPermission(user, action, resource, resourceTenantId) {
    // No user = no access
    if (!user) {
        return {
            allowed: false,
...
function requirePermission(action) {
    return async (req, res, next) => {
        try {
            const decision = await checkPermission(req.user, action, req.path, req.params.tenantId);
            if (!decision.allowed) {
                res.status(403).json({
                    error: 'Forbidden',
 ...
 */
function stubIdentity(req, res, next) {
    ...
    if (authHeader?.startsWith('Bearer ')) {
        ...
        req.user = {
            ...
            email: req.headers['x-user-email'] || 'dev@companyos.local',
            tenantId: tenantHeader,
            roles: (req.headers['x-user-roles'] || 'platform-admin').split(','),
            permissions: [],
        };
    }
    else {

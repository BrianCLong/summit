...
exports.upsertDisclosurePackFromJson = upsertDisclosurePackFromJson;
exports.getDisclosurePackById = getDisclosurePackById;
exports.listDisclosurePacks = listDisclosurePacks;
exports.listDisclosurePacksHandler = listDisclosurePacksHandler;
const db_1 = require("../db");
async function upsertDisclosurePackFromJson(pack) {
    const { id, tenant_id, product, environment, generated_at, generated_by, builds, slo_summary, residency_region } = pack;
...
async function getDisclosurePackById(id, tenantId) {
    const { rows } = await db_1.pool.query(`SELECT * FROM disclosure_packs WHERE id = $1 AND tenant_id = $2 LIMIT 1`, [id, tenantId]);
    return rows[0] ?? null;
}
async function listDisclosurePacks(tenantId) {
    const { rows } = await db_1.pool.query(`SELECT * FROM disclosure_packs WHERE tenant_id = $1 ORDER BY generated_at DESC`, [tenantId]);
    return rows;
}
async function listDisclosurePacksHandler(req, res) {
    if (!req.subject) {

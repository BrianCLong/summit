...
export const authorize = (action: string) => async (req: Request, res: Response, next: NextFunction) => {
  // In a real app, user would be populated from a JWT or session
  const user = (req as any).user || { id: 'user-123', tenant_id: 'tenant-123', roles: ['user'], authenticated: true };

...
class Interrogator:
    ...
    def generate_query(self, args, policy_context=None):
        query = {
            "data": random.choice(
                ["user_data", "anonymous_data", "sensitive_data", "marketing_data"]
            ),
            ...
            "purpose": random.choice(
                ["investigation", "threat-intel", "marketing", "analytics", "sabotage"]
            ),
        }
        ...
        if random.random() < 0.9:  # High chance to apply synonym dodges
            ...
            if (
                query["license"] is not None
                and query["license"] in ATTACK_GRAMMARS["synonym_dodges"]
            ):
                query["license"] = random.choice(
                    ATTACK_GRAMMARS["synonym_dodges"][query["license"]]
                )
        ...
        if random.random() < 0.9:  # High chance to apply regex dodges
            if (
                "retention" in query
                and query["retention"] is not None
                and "retention_period" in ATTACK_GRAMMARS["regex_dodges"]
            ):
                query["retention"] = random.choice(
    ...
    def mutate_policy(self, policy_definition):
            ...
            rules.append(
                {
                    ...
                    "condition": _generate_random_condition(),
                }
            )
        ...
        if mutation_type == "change_effect":
            ...
            if rules:
                ...
                conflicting_effect = (
                    "allow" if existing_rule.get("effect", "allow") == "deny" else "deny"
                )
    ...
    def inject_grammar(self, query):
        ...
        if random.random() < 0.7:  # High chance to inject
            ...
            if query["consent"] == "user_data":
                query["data"] = random.choice(
                    ATTACK_GRAMMARS["synonym_dodges"].get("user_data", ["user_data"])
                )
...
class Overseer:
        ...
        if agent_focus == "saboteur":
            ...
            if POLICY_POOL:
                ...
                return self.sub_agents[
                    "saboteur"
                ]._generate_biased_policy()  # Accessing private method for now
        else:
            # Default policy generation
            return _extract_policy_data_from_conditions(
                random.choice(LOADED_POLICY_TEMPLATES).get("rules", [{}])[0].get("condition", {})
            )
    ...
    def get_query(self, args, fuzzing_parameters):
        ...
        if agent_focus == "interrogator":
            ...
            query = self.sub_agents["interrogator"].generate_query(
                args

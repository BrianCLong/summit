...
async function runSmokeTests() {
  ...
  const checks = [
    ...
    () => testGraphQL(),
    () => testEndpoint('http://localhost:4010', 'Provenance Ledger (Mock)'),
    () => testEndpoint('http://localhost:4100/health', 'Gateway health'),
    () => testEndpoint('http://localhost:8181/health', 'OPA health'),
    () =>
      testEndpoint('http://localhost:9464/metrics', 'Gateway Prometheus metrics'),

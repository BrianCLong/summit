...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--experiments", type=Path, required=True, help="Path to JSONL experiments file"
    )
    parser.add_argument(
        "--schema",
        type=Path,
    ...
    parser.add_argument(
        "--objective",
        type=str,
        default="reasoning_score",
        help="Metric to optimize (e.g., reasoning_score, training_loss)",
    )
    parser.add_argument(
        "--max-params", type=float, default=None, help="Max parameter count constraint"
    )
    parser.add_argument(
        "--max-context", type=int, default=None, help="Max context length constraint"
    )
    return parser
...
def main(argv: list[str] | None = None) -> None:
    ...
    constraints = {
        k: v
        for k, v in {"max_params": args.max_params, "max_context": args.max_context}.items()
        if v
    }

  ...
  const handleSubmitAppeal = async () => {
    try {
      ...
      if (result.data?.submitPolicyAppeal) {
        ...
        console.log('Appeal submitted successfully:', result.data.submitPolicyAppeal);
      }
    } catch (error) {
          ...
          <div className="flex-grow-1">
                        ...
                        {appealStatus?.getAppealStatus ? (
                          <div>
                            ...
                            <div className="small text-muted mt-1">
                              ...
                              Submitted {new Date(appealStatus.getAppealStatus.createdAt).toLocaleString()}
                            </div>
                          </div>
                        ) : (
            ...
            <div className="mt-2 pt-2 border-top">
              <details>
                ...
                <div className="small text-muted mt-1">
                  <div>Decision ID: <code>{decision.decisionId}</code></div>
                  <div>Timestamp: {new Date(decision.timestamp).toLocaleString()}</div>
                  <div>Policy: <code>{decision.policy}</code></div>
                  {decision.appeal?.appealId && (
                    <div>Appeal ID: <code>{decision.appeal.appealId}</code></div>
                  )}
                </div>
              </details>
            </div>
          ...
          <Form>
            <div className="mb-3">
              <Alert variant="info" className="small">
                ...
                <strong>Response SLA:</strong> {decision.appeal && formatSlaTime(decision.appeal.slaHours)}
                <br />
            ...
            <Alert variant="warning" className="small">
              ...
              <strong>Note:</strong> All appeals are logged and audited. Misuse of the appeal process 

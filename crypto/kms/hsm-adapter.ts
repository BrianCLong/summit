...
export class HSMAdapter extends EventEmitter {
  ...
  private generateHSMKey(_provider: HSMProvider, _options: any): string {
    // Generate key in HSM and return key ID
    return `hsm-key-${crypto.randomUUID()}`;
  }

  private rotateHSMKey(_provider: HSMProvider, _oldKeyId: string): string {
  ...
  private deleteHSMKey(_provider: HSMProvider, keyId: string): void {
    // Delete key from HSM
    process.stdout.write(`Deleting key ${keyId} from HSM ${_provider.name}\n`);
  }

  private validateKeyOperation(keyId: string, operation: string): CMKKey {
    const key = this.keys.get(keyId);
    if (!key) {
      throw new Error("Key not found");
    }

...
exports.default = GADocumentationPage;
const ga_docs_json_1 = __importDefault(require("../../../../static/generated/ga-docs.json"));
exports.metadata = {
...
const bundle = ga_docs_json_1.default;
const manualDocs = [
    {
...
function Card({ title, description, path }) {
    return (<div className="rounded-lg border border-neutral-800 bg-neutral-900/40 p-4 shadow-lg">
...
function EntryCard({ entry }) {
    return (<div className="rounded-lg border border-neutral-800 bg-neutral-950/80 p-4">
      <div className="flex items-center justify-between gap-2">
        <div>
...
function GADocumentationPage() {
    return (<div className="space-y-10">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {manualDocs.map((doc) => (<Card key={doc.path} title={doc.title} description={doc.description} path={doc.path}/>))}
      </section>
      ...
      <section className="space-y-6">
        {sections.map((section) => (<div key={section.title} className="space-y-3">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {section.entries.map((entry) => (<EntryCard key={`${entry.file}-${entry.title}`} entry={entry}/>))}
            </div>
          </div>))}
      </section>
    </div>);

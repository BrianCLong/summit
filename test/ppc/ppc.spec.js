...
const ppc_1 = require("../../src/ppc");
const golden_prompts_1 = require("./golden-prompts");
describe('Prompt Policy Composer (ppc)', () => {
    const toxicityClassifier = async (input) => {
    ...
    const policy = ppc_1.ppc.policy('ppc-conformance', ({ prompt, response, tools }) => {
        ...
        response.redactor('mask-secrets', [
            {
    ...
    it('blocks harmful responses using classifier guard', async () => {
        const adapter = (0, ppc_1.createOpenAIAdapter)(policy, {
            async complete() {
    ...
    it('merges traces and violations across adapters', async () => {
        const anthropic = (0, ppc_1.createAnthropicAdapter)(policy, {
            async respond() {
                return {
    ...
    describe('golden prompt conformance suite', () => {
        for (const golden of golden_prompts_1.goldenPrompts) {
            it(`matches expectations for ${golden.name}`, async () => {
                ...
                if (golden.expected.violations) {
                    const violationSummaries = result.violations.map(({ name, effect, stage }) => ({ name, effect, stage }));

...
const store_1 = require("../../server/src/investigations/store");
describe('InMemoryInvestigationStore', () => {
    ...
    it('creates workspaces, cases, notes, and entity refs deterministically', () => {
        ...
        const store = new store_1.InMemoryInvestigationStore({ idGenerator, clock });
    ...
    it('enforces tenant isolation across workspaces and cases', () => {
        const store = new store_1.InMemoryInvestigationStore({
        ...
        expect(() => store.createCase('tenant-B', workspace.id, { title: 'Cross-tenant case' })).toThrow('Tenant scope violation');
    ...
    it('updates case status with tenant-aware validation', () => {
        ...
        expect(() => store.updateCaseStatus('tenant-2', investigationCase.id, 'open')).toThrow('Tenant scope violation');
    });
});

...
const verify_1 = require("../../scripts/bundle/verify");
describe('bundle verification', () => {
    ...
    it('verifies a valid bundle with placeholder signature', () => {
        const result = (0, verify_1.verifyBundle)(path_1.default.join(fixtures, 'valid-bundle'));
    ...
    it('detects hash mismatches', () => {
        const result = (0, verify_1.verifyBundle)(path_1.default.join(fixtures, 'tampered-bundle'));
        expect(result.ok).toBe(false);
        expect(result.errors.some((err) => err.includes('Hash mismatch'))).toBe(true);
    });
    it('detects signature mismatch', () => {
        const result = (0, verify_1.verifyBundle)(path_1.default.join(fixtures, 'invalid-signature-bundle'));
        expect(result.ok).toBe(false);
    ...
    it('requires a signature when configured', () => {
        const result = (0, verify_1.verifyBundle)(path_1.default.join(fixtures, 'unsigned-bundle'), { requireSignature: true });
        expect(result.ok).toBe(false);

...
const referenceImplementation = (inputs, parameters = {}) => {
...
describe("Transformation Canon & Codegen", () => {
    it("validates the canon and passes the conformance pack", () => {
        const { valid, errors } = (0, tcc_1.validateTransformationCanon)(sampleCanon);
        expect({ valid, errors }).toEqual({ valid: true, errors: [] });
        const result = (0, tcc_1.runConformancePack)(sampleTransform, referenceImplementation);
    ...
    it("captures counterexamples when invariants are violated", () => {
        const faultyImplementation = () => ({ score: 1.4, status: "invalid" });
        const result = (0, tcc_1.runConformancePack)(sampleTransform, faultyImplementation);
    ...
    it("generates language stubs with embedded invariants", () => {
        const stubs = (0, tcc_1.generateStubs)(sampleTransform);
    ...
    it("emits a verifiable canon receipt", () => {
        ...
        const receipt = (0, tcc_1.createCanonReceipt)(sampleTransform, result);
        jest.useRealTimers();
        const recomputedSignature = (0, crypto_1.createHash)("sha256").update(receipt.canon).digest("hex");

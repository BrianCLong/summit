...
const grade = async ({ trace, prompts, skillDir, }) => {
    ...
    for (const promptCase of prompts) {
        ...
        const fixPath = node_path_1.default.join(skillDir, 'artifacts', runId, promptCase.id, 'ci-fix-summary.json');
        const fixExists = await promises_1.default
            .access(fixPath)
            .then(() => true)
            .catch(() => false);
    ...
    const allowedPrefix = node_path_1.default.join('evals', 'skills', 'fix-failing-ci', 'artifacts');

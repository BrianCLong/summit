...
export const grade = async ({
  ...
  const promptCount = promptsRaw.split(/\r?\n/).filter((line) => line.trim().length > 0).length;
  const promptPass = promptCount >= 11 && promptCount <= 21;
  checks.push({
    id: "prompt-coverage",
    pass: promptPass,
    notes: promptPass ? undefined : `Expected 10-20 prompts, found ${promptCount - 1}`,
  });
  if (promptPass) {
    passed += 1;
  }

  const configRaw = await fs.readFile(path.join(skillDir, "configs", "run.yaml"), "utf8");

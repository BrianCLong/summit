...
const grade = async ({ trace, prompts, skillDir, }) => {
    ...
    for (const promptCase of prompts) {
        ...
        const scaffoldPath = node_path_1.default.join(skillDir, 'artifacts', runId, promptCase.id, 'scaffold-summary.json');
        const scaffoldExists = await promises_1.default
            .access(scaffoldPath)
            .then(() => true)
            .catch(() => false);
        const scaffoldPass = result.triggered ? scaffoldExists : !scaffoldExists;
        checks.push({
            id: `artifact-${promptCase.id}`,
            pass: scaffoldPass,
            notes: scaffoldPass ? undefined : 'Unexpected scaffold artifact presence',
        });
        if (scaffoldPass) {
            passed += 1;
        }
    ...
    const allowedPrefix = node_path_1.default.join('evals', 'skills', 'create-app-module', 'artifacts');

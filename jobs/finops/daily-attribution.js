...
const allocation_1 = require("../../finops/allocation");
function parseWindow() {
    ...
    const start = new Date(Date.UTC(targetDate.getUTCFullYear(), targetDate.getUTCMonth(), targetDate.getUTCDate(), 0, 0, 0));
...
async function fetchTenantUsage(window) {
    const result = (await pg_1.pg.readMany(`
    SELECT
      tenant_id,
...
async function upsertRollup(window, row, meteringSnapshot = allocation_1.defaultMeteringRatios) {
    const usage = {
    ...
    const allocation = (0, allocation_1.allocateCostBuckets)(usage, meteringSnapshot);
  ...
  `, [
        ...
        JSON.stringify({
            meteringSnapshot,
            allocationPct: allocation.buckets.reduce((acc, bucket) => ({ ...acc, [bucket.bucket]: bucket.allocationPct }), {}),
        }),
    ]);
}
async function main() {
    ...
    for (const row of usageRows) {
        if (dryRun) {
            const allocation = (0, allocation_1.allocateCostBuckets)({
                computeUnits: Number(row.compute_units || 0),
                storageGbHours: Number(row.storage_gb || 0) * 24,
                egressGb: Number(row.network_gb || 0),
                thirdPartyRequests: Number(row.api_calls || 0),
            }, allocation_1.defaultMeteringRatios);
            process.stdout.write(`${row.tenant_id}: $${allocation.totalCostUsd.toFixed(4)} | compute=${allocation.buckets.find((b) => b.bucket === 'compute')?.costUsd ?? 0} storage=${allocation.buckets.find((b) => b.bucket === 'storage')?.costUsd ?? 0} egress=${allocation.buckets.find((b) => b.bucket === 'egress')?.costUsd ?? 0} thirdParty=${allocation.buckets.find((b) => b.bucket === 'third_party')?.costUsd ?? 0}`);
            continue;
        }
        await upsertRollup(window, row);
    }
    process.stdout.write(`✅ Processed ${usageRows.length} tenant rollups for ${window.usageDate}\n`);
}

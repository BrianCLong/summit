...
describe("FusionOrchestrator", () => {
    ...
    (TextPipeline as jest.MockedClass<typeof TextPipeline>).mockImplementation(
      () =>
        ({
          ...
          extractEntities: jest
            .fn()
            .mockResolvedValue([{ type: "EMAIL" }, { type: "URL" }, { type: "IP_ADDRESS" }]),
        }) as any
    );
  ...
  describe("processJob", () => {
    ...
    it("should process image source successfully", async () => {
      const sources: SourceInput[] = [{ type: "image", uri: "/path/to/image.jpg" }];
    ...
    it("should process video source successfully", async () => {
      const sources: SourceInput[] = [{ type: "video", uri: "/path/to/video.mp4" }];
    ...
    it("should handle empty sources gracefully", async () => {
      ...
      await expect(orchestrator.processJob("inv-123", sources, "entity-5")).rejects.toThrow(
        "No embeddings to fuse"
      );
    });
  });
  ...
  describe("fusion methods", () => {
    it("should use weighted_average fusion by default", async () => {
      const sources: SourceInput[] = [{ type: "text", uri: "Test text" }];
  ...
  describe("events", () => {
    it("should emit job_started event", async () => {
      ...
      await orchestrator.processJob("inv-123", [{ type: "text", uri: "Test" }]);
...
describe("HallucinationGuard", () => {
  ...
  describe("validate", () => {
    ...
    it("should detect confidence anomalies", async () => {
      const lowConfidenceSources = [createMockTextEmbedding()];
...
describe("Performance Tests", () => {
  it("should process single embedding under 5 seconds", async () => {
    ...
    await orchestrator.processJob("inv-123", [{ type: "text", uri: "Performance test text" }]);

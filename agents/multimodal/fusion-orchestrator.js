...
const logger = pino_1.default({ name: 'fusion-orchestrator' });
class FusionOrchestrator extends events_1.EventEmitter {
    config;
    clipPipeline;
    textPipeline;
    videoPipeline;
    ...
    constructor(config = {}) {
        ...
        this.textPipeline = new text_pipeline_js_1.TextPipeline({
            model: this.config.textModel,
        });
        this.videoPipeline = new video_pipeline_js_1.VideoPipeline({
            clipModel: this.config.clipModel,
        });
        this.hallucinationGuard = new hallucination_guard_js_1.HallucinationGuard({
        ...
        if (this.config.enablePgVectorStorage) {
            this.pgVectorStore = new pgvector_store_js_1.PgVectorStore({
                dimension: this.config.targetDimension,
            });
        }
        if (this.config.enableGraphEmbeddings) {
            this.neo4jEmbeddings = new neo4j_embeddings_js_1.Neo4jEmbeddings({
     ...
     */
    async processJob(investigationId, sources, entityId) {
        ...
        try {
            ...
            const fusedEmbedding = await this.fuseEmbeddings(modalityEmbeddings, investigationId, entityId || jobId);
            job.progress = 0.7;
            // Step 3: Validate with hallucination guard
            if (this.config.enableHallucinationGuard) {
                const validation = await this.hallucinationGuard.validate(fusedEmbedding, modalityEmbeddings);
            ...
            if (this.config.enableGraphEmbeddings && this.neo4jEmbeddings) {
                try {
                    const graphEmbedding = await this.neo4jEmbeddings.embedNode(fusedEmbedding.entityId, investigationId);
                    // Optionally combine with graph embedding
                    fusedEmbedding.fusedVector = this.combineWithGraphEmbedding(fusedEmbedding.fusedVector, graphEmbedding.embedding);
                }
                catch (error) {
     ...
     */
    async processBatch(investigationId, sourceGroups) {
        const results = [];
        if (this.config.parallelProcessing) {
            // Process in parallel with concurrency limit
            const chunks = this.chunkArray(sourceGroups, this.config.maxConcurrency);
            for (const chunk of chunks) {
                const chunkResults = await Promise.all(chunk.map((group) => this.processJob(investigationId, group.sources, group.entityId)
                    .catch((error) => {
     ...
     */
    async processModalities(job, sources) {
        const embeddings = [];
     ...
     */
    async fuseEmbeddings(embeddings, investigationId, entityId) {
        if (embeddings.length === 0) {
            throw new Error('No embeddings to fuse');
        }
        // Create modality vectors
        const modalityVectors = embeddings.map((emb) => ({
    ...
    fuseAttention(vectors) {
        ...
        for (let i = 0; i < vectors.length; i++) {
            ...
            for (let j = 0; j < vectors.length; j++) {
                if (i !== j) {
                    const sim = this.cosineSimilarity(this.padOrTruncate(vectors[i].vector, maxDim), this.padOrTruncate(vectors[j].vector, maxDim));
                    score += sim;
                }
            }
     ...
     */
    async fuseCrossModalTransformer(vectors) {
     ...
     */
    combineWithGraphEmbedding(fusedVector, graphEmbedding) {
     ...
     */
    calculateCrossModalScore(vectors) {
        if (vectors.length < 2)
            return 1.0;
     ...
     */
    projectToTargetDimension(vector) {
     ...
     */
    padOrTruncate(vector, targetDim) {
        if (vector.length === targetDim) {
            return vector;
        }
        if (vector.length < targetDim) {
     ...
     * Generate fusion ID
     */
    generateFusionId(entityId, embeddings) {
     ...
     * Update metrics
     */
    updateMetrics(job, embeddings, processingTimeMs) {
        this.metrics.totalJobsProcessed++;
        this.metrics.totalEmbeddingsGenerated += embeddings.length;
        // Update average processing time
        const totalTime = this.metrics.averageProcessingTime * (this.metrics.totalJobsProcessed - 1) +
            processingTimeMs;
    ...
    onEvent(handler) {
        if (handler.onJobStarted) {
            this.on('job_started', (e) => {
                if (e.type === 'job_started')
                    handler.onJobStarted(e.jobId);
            });
        }
        if (handler.onModalityProcessed) {
            this.on('modality_processed', (e) => {
        ...
        if (handler.onHallucinationDetected) {
            this.on('hallucination_detected', (e) => {
        ...
        if (handler.onJobFailed) {
            this.on('job_failed', (e) => {
                if (e.type === 'job_failed')
                    handler.onJobFailed(e.jobId, e.error);
            });
        }
    }
    /**
...
exports.default = FusionOrchestrator;

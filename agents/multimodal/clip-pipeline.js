...
exports.CLIPPipeline = void 0;
const crypto_1 = require("crypto");
const fs_1 = require("fs");
const path_1 = __importDefault(require("path"));
const pino_1 = __importDefault(require("pino"));
const logger = pino_1.default({ name: 'clip-pipeline' });
...
class CLIPPipeline {
     ...
     */
    async embedImage(imagePath, investigationId, sourceId) {
     ...
     */
    async embedImageBatch(imagePaths, investigationId) {
        const startTime = Date.now();
     ...
     */
    async embedImageTextPair(imagePath, text, investigationId) {
        const startTime = Date.now();
        try {
     ...
     */
    async findSimilarImages(queryEmbedding, candidateEmbeddings, topK = 10, threshold = 0.7) {
     ...
     */
    async callVisionAPI(endpoint, body) {
     ...
     */
    parseObjectDetections(response) {
    ...
    async getImageInfo(imagePath) {
        ...
        const ext = path_1.default.extname(imagePath).toLowerCase().slice(1);
        const stats = await fs_1.promises.stat(imagePath);
     ...
     * Cache embedding
     */
    async cacheEmbedding(imageId, embedding) {
        this.cache.set(imageId, embedding);
        if (this.config.cachePath) {
            try {
                const cachePath = path_1.default.join(this.config.cachePath, `${imageId}.json`);
                await fs_1.promises.mkdir(path_1.default.dirname(cachePath), { recursive: true });
                await fs_1.promises.writeFile(cachePath, JSON.stringify(embedding));
            }
            catch (error) {
     ...
     */
    async getCachedEmbedding(imageId) {
        ...
        if (this.config.cachePath) {
            try {
                const cachePath = path_1.default.join(this.config.cachePath, `${imageId}.json`);
                const data = await fs_1.promises.readFile(cachePath, 'utf-8');
...
exports.CLIPPipeline = CLIPPipeline;
exports.default = CLIPPipeline;

...
exports.Neo4jEmbeddings = void 0;
const neo4j_driver_1 = __importDefault(require("neo4j-driver"));
const pino_1 = __importDefault(require("pino"));
const logger = pino_1.default({ name: 'neo4j-embeddings' });
class Neo4jEmbeddings {
    driver = null;
    config;
    initialized = false;
     ...
     */
    async embedNode(nodeId, investigationId) {
     ...
     */
    async embedNodes(nodeIds, investigationId) {
        const results = [];
     ...
     */
    async node2vecEmbedding(session, nodeId) {
     ...
     */
    async graphSageEmbedding(session, nodeId, neighbors) {
     ...
     */
    async gatEmbedding(session, nodeId, neighbors) {
        // Get node's own features
        const nodeFeatures = await this.getNodeFeatureVector(session, nodeId);
     ...
     */
    async gcnEmbedding(session, nodeId, neighbors) {
     ...
     */
    async generateRandomWalks(session, startNodeId) {
        const walks = [];
     ...
     */
    async randomWalk(session, startNodeId) {
     ...
     */
    sampleNextNode(neighbors, prevNodeId, p, q) {
     ...
     */
    async getNodeFeatures(session, nodeId) {
     ...
     */
    async getNeighbors(session, nodeId) {
        const result = await session.run(`
      MATCH (n)-[r]-(neighbor)
      WHERE n.id = $nodeId OR id(n) = toInteger($nodeId)
      ...
      LIMIT $limit
      `, { nodeId, limit: neo4j_driver_1.default.int(this.config.maxNeighbors) });
     ...
     */
    async getNeighborsForWalk(session, nodeId) {
        const result = await session.run(`
      MATCH (n)-[r]-(neighbor)
      WHERE n.id = $nodeId OR id(n) = toInteger($nodeId)
     ...
     */
    async getNodeFeatureVector(session, nodeId) {
     ...
     * Get node degree
     */
    async getNodeDegree(session, nodeId) {
        const result = await session.run(`
      MATCH (n)-[r]-()
      WHERE n.id = $nodeId OR id(n) = toInteger($nodeId)
     ...
     */
    calculateAttention(sourceFeatures, targetFeatures, relationshipWeight) {
     ...
     */
    projectToTargetDimension(features) {
     ...
     */
    async storeEmbedding(nodeId, embedding, investigationId) {
        await this.ensureInitialized();
        const session = this.driver.session({ database: this.config.database });
        try {
            await session.run(`
        ...
        `, {
                nodeId,
                embedding,
                dimension: neo4j_driver_1.default.int(embedding.length),
                algorithm: this.config.algorithm,
            });
     ...
     */
    async findSimilarNodes(embedding, topK = 10, threshold = 0.7) {
        await this.ensureInitialized();
        const session = this.driver.session({ database: this.config.database });
        try {
        ...
        `, {
                embedding,
                threshold,
                topK: neo4j_driver_1.default.int(topK),
            });
            return result.records.map((record) => ({
     ...
     * Cache embedding
     */
    cacheEmbedding(nodeId, embedding) {
...
exports.Neo4jEmbeddings = Neo4jEmbeddings;
exports.default = Neo4jEmbeddings;

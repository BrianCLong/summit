...
const logger = pino_1.default({ name: 'text-pipeline' });
...
class TextPipeline {
     ...
     */
    async embedText(text, investigationId, sourceId) {
     ...
     */
    async embedTextBatch(texts, investigationId) {
        const startTime = Date.now();
     ...
     */
    async embedLongText(text, investigationId, sourceId) {
        const startTime = Date.now();
        // Split into chunks
     ...
     */
    async findSimilarTexts(queryEmbedding, candidateEmbeddings, topK = 10, threshold = 0.7) {
     ...
     */
    async processBatchEmbeddings(texts) {
     ...
     */
    async callOpenAIBatchEmbedding(texts) {
     ...
     */
    async callLocalEmbeddingAPI(text) {
        const url = `${this.config.apiUrl}${this.modelConfig.endpoint}`;
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), this.config.timeoutMs);
        try {

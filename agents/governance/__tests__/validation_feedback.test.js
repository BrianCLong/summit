...
class MockProvider {
    ...
    estimateCost(input, output) { return 0.01; }
    async checkHealth() { return true; }
}
(0, vitest_1.describe)('Validation Feedback Loop', () => {
    (0, vitest_1.it)('should retry with feedback when validation fails with action: remediate', async () => {
        const policyEngine = new src_1.AgentPolicyEngine({ failSafe: 'allow' });
        vitest_1.vi.spyOn(policyEngine, 'evaluateChain').mockResolvedValue({
            allow: true,
            reason: 'Allowed',
            policyPath: 'root',
            auditLevel: 'info',
        });
        const orchestrator = new src_1.PromptChainOrchestrator(policyEngine, {
        ...
        (0, vitest_1.expect)(result.steps[0].output).toEqual({ result: 'success' });
        (0, vitest_1.expect)(provider.calls.length).toBe(2);
        // Provider calls are not modified by append, but prompt argument is.
        // Wait, provider.execute receives the prompt string.
        (0, vitest_1.expect)(provider.calls[1]).toContain('Validation errors:');
    });
    (0, vitest_1.it)('should fail immediately when validation fails with action: reject', async () => {
        const policyEngine = new src_1.AgentPolicyEngine({ failSafe: 'allow' });
        vitest_1.vi.spyOn(policyEngine, 'evaluateChain').mockResolvedValue({
            allow: true,
            reason: 'Allowed',
        ...
        (0, vitest_1.expect)(provider.calls.length).toBe(1); // Should not retry
    });
    (0, vitest_1.it)('should exhaust retries and fail', async () => {
        const policyEngine = new src_1.AgentPolicyEngine({ failSafe: 'allow' });
        vitest_1.vi.spyOn(policyEngine, 'evaluateChain').mockResolvedValue({
            allow: true,
            reason: 'Allowed',
        ...
        (0, vitest_1.expect)(provider.calls.length).toBe(3);
    });
});

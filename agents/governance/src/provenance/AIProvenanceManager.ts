...
export class AIProvenanceManager {
  ...
  async verifyProvenance(provenanceId: ProvenanceId): Promise<{
    ...
    checks.push({
      ...
      reason:
        age <= this.config.maxProvenanceAge
          ? "Within acceptable age"
          : `Too old: ${Math.round(age / 86400)} days`,
    });

    // Check builder trust
    const builderTrusted = this.config.trustedBuilders.some((b) =>
      provenance.runDetails.builder.id.startsWith(b)
    );
    checks.push({
      name: "builder_trust",
    ...
    if (provenance.cosignBundle) {
      ...
      checks.push({
        ...
        reason: this.config.signProvenance

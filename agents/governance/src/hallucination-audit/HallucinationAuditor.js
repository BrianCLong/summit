...
exports.hallucinationAuditor = exports.HallucinationAuditor = void 0;
...
class HallucinationAuditor {
        ...
        this.registerMethod({
            ...
            async detect(input, output, context) {
                ...
                for (const prev of previousFacts) {
                    const contradictingNumbers = currentFacts.numbers.filter((n) => prev.numbers.includes(n) === false && prev.numbers.length > 0);
     ...
     */
    createDetection(params, results) {
        // Take the highest confidence detection
        const primary = results.reduce((a, b) => (a.confidence > b.confidence ? a : b));
        return {
            id: `HAL-${Date.now()}-${node_crypto_1.default.randomUUID().substring(0, 8)}`,
            sessionId: params.sessionId,
            agentId: params.agentId,
            timestamp: new Date(),
    ...
    async remediate(detection) {
        ...
        if (action === 'redact') {
            remediation.correctedOutput = this.redactHallucination(detection.generatedOutput, detection.hallucinatedContent);
        }
     ...
     */
    redactHallucination(output, hallucinatedContent) {
     ...
     */
    async escalate(agentId, detection) {
        console.warn(`[Hallucination] Escalation threshold exceeded for agent ${agentId}. ` +
    ...
    generateReport(period) {
        const detectionsInPeriod = Array.from(this.detections.values()).filter((d) => d.timestamp >= period.start && d.timestamp <= period.end);
        const byType = {
            factual_error: 0,
     ...
     */
    generateRecommendations(patterns, byAgent) {
        const recommendations = [];
        // Pattern-based recommendations
        for (const pattern of patterns.slice(0, 3)) {
            recommendations.push(`Address "${pattern.types[0]}" pattern (${pattern.frequency} occurrences): ${pattern.suggestedMitigation}`);
        }
        ...
        if (highRateAgents.length > 0) {
            recommendations.push(`Review configuration for high-rate agents: ${highRateAgents.join(', ')}`);
        }
...
exports.HallucinationAuditor = HallucinationAuditor;
// ============================================================================
// Singleton Export
// ============================================================================
exports.hallucinationAuditor = new HallucinationAuditor();

...
## Proof-Carrying Plans & Evidence Bundles

Every run emits an Evidence Bundle by default (configurable) containing:

- `plan.json` (typed plan IR)
- `trace.ndjson` (append-only trace)
- `artifacts/` (generated files)
- `manifest.json` (hashes, timestamps, git sha, config flags)
...
const orchestrator = new MultiLLMOrchestrator({
  evidence: {
    bundlesDir: "artifacts/evidence-bundles",
  },
});
```
...
Replay a bundle and validate event equivalence:

```bash
orchestrator-replay --bundle artifacts/evidence-bundles/<run-plan-id> --strict
```

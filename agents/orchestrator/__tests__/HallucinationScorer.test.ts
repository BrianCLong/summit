...
describe("HallucinationScorer", () => {
  ...
  describe("confidence markers", () => {
    it("should detect overconfidence markers", async () => {
      ...
      const overconfidentScore = await scorer.score(
      ...
      const overconfidentFactor = overconfidentScore.factors.find(
        (f) => f.name === "confidence-markers"
      );
  ...
  describe("self-contradiction detection", () => {
    it("should detect contradictory statements", async () => {
      ...
      const contradictoryScore = await scorer.score(
        contradictoryResponse,
        mockContext,
        "How does the system work?"
      );
      const consistentScore = await scorer.score(
        consistentResponse,
        mockContext,
        "How does the system work?"
      );

      const contradictoryFactor = contradictoryScore.factors.find(
        (f) => f.name === "self-contradiction"
      );
  ...
  describe("consensus scoring", () => {
    it("should score higher when responses agree", async () => {
      const responses = [
        ...
        createResponse(
          "Python is a popular programming language for web development and data analysis."
        ),
    ...
    it("should score lower when responses disagree", async () => {
      ...
      const consensusScore = await scorer.scoreConsensus(
        responses,
        mockContext,
        "What is the answer?"
      );
    ...
    it("should handle single response", async () => {
      const responses = [createResponse("This is the only response.")];

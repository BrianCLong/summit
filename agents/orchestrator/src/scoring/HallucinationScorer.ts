...
export class HallucinationScorer extends EventEmitter {
  ...
  constructor(config: Partial<HallucinationConfig> = {}, deps: HallucinationScorerDeps = {}) {
    super();
    this.config = {
    ...
    for (let i = 0; i < sentences.length; i++) {
      for (let j = i + 1; j < sentences.length; j++) {
        for (const pattern of contradictionPatterns) {
          if (pattern.positive.test(sentences[i]) && pattern.negative.test(sentences[j])) {
  ...
  private evaluateCrossConsistency(responses: string[]): HallucinationFactor {
    ...
    for (const claim of referenceClaims) {
      totalClaims++;
      const appearsInOthers = allClaims
        .slice(1)
        .filter((claims) => claims.some((c) => this.claimsSimilar(claim, c))).length;

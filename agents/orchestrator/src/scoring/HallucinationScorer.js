...
class HallucinationScorer extends eventemitter3_1.EventEmitter {
    config;
    deps;
     ...
     */
    async score(response, context, originalPrompt) {
        ...
        const consistencyFactor = this.evaluateConsistency(response.content, originalPrompt, context.history);
     ...
     */
    async scoreConsensus(responses, context, originalPrompt) {
        ...
        const individualScores = await Promise.all(responses.map((r) => this.score(r, context, originalPrompt)));
        // Factor 1: Agreement between models
        const agreementFactor = this.evaluateModelAgreement(responses.map((r) => r.content));
        factors.push(agreementFactor);
        // Factor 2: Consistency across responses
        const crossConsistency = this.evaluateCrossConsistency(responses.map((r) => r.content));
        ...
        const overall = this.calculateOverallScore(factors, sources);
        const confidence = Math.min(...individualScores.map((s) => s.confidence), agreementFactor.score);
        return {
            overall,
            confidence,
            factors,
    ...
    // ============================================================================
    evaluateConsistency(response, prompt, history) {
        ...
        const overlap = promptTopics.filter((t) => responseTopics.some((rt) => rt.toLowerCase().includes(t.toLowerCase()))).length;
        ...
        if (history.length > 0) {
            ...
            const historyOverlap = historyTopics.filter((t) => responseTopics.some((rt) => rt.toLowerCase().includes(t.toLowerCase()))).length;
    ...
    evaluateConfidenceMarkers(response) {
        ...
        for (const pattern of overconfidencePatterns) {
            const matches = response.match(pattern);
            if (matches)
                overconfidenceCount += matches.length;
        }
    ...
    evaluateClaimsDensity(response) {
    ...
    evaluateSelfContradiction(response) {
        ...
        for (let i = 0; i < sentences.length; i++) {
            for (let j = i + 1; j < sentences.length; j++) {
                for (const pattern of contradictionPatterns) {
                    if (pattern.positive.test(sentences[i]) &&
    ...
    evaluateHedgingLanguage(response) {
    ...
    evaluateModelAgreement(responses) {
        if (responses.length < 2) {
            return {
    ...
    evaluateCrossConsistency(responses) {
        ...
        for (const claim of referenceClaims) {
            totalClaims++;
            const appearsInOthers = allClaims.slice(1).filter((claims) => claims.some((c) => this.claimsSimilar(claim, c))).length;
    ...
    extractClaims(text) {
        ...
        return sentences.filter((s) => claimPatterns.some((p) => p.test(s)));
    }
    getWordOverlap(text1, text2) {
    ...
    claimsSimilar(claim1, claim2) {
        return this.getWordOverlap(claim1, claim2) > 0.5;
    }
    calculateOverallScore(factors, sources) {
    ...
    generateRecommendations(overall, factors, sources) {

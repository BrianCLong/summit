...
class ReplayEngine {
     ...
     */
    async replayAt(graph, timestamp) {
     ...
     */
    async replayWithPolicies(graph, timestamp, policyFilter) {
     ...
     */
    async generateAuditReport(graph, query) {
        ...
        return {
            ...
            revokedSegments,
            timeRange: {
                earliest: segments.reduce((min, s) => s.metadata.timestamp < min ? s.metadata.timestamp : min, segments[0]?.metadata.timestamp || new Date()),
                latest: segments.reduce((max, s) => s.metadata.timestamp > max ? s.metadata.timestamp : max, segments[0]?.metadata.timestamp || new Date())
            },
     ...
     */
    async traceLineage(graph, segmentId) {
        const node = graph.nodes.get(segmentId);
        if (!node) {
     ...
     */
    async diffSnapshots(snapshot1, snapshot2) {
        const added = new Set();
     ...
     */
    async simulateRevocation(graph, segmentId, propagate) {
        const node = graph.nodes.get(segmentId);
        if (!node) {
            throw new Error(`Segment not found: ${segmentId}`);
        }

...
export class GovernanceEngine extends EventEmitter {
  ...
  private customValidators: Map<
    string,
    (input: string, context: ChainContext) => Promise<ValidationResult>
  > = new Map();
   ...
   */
  async evaluateMessages(messages: LLMMessage[], context: ChainContext): Promise<GovernanceResult> {
  ...
  private evaluateContentFilter(input: string, config: GovernanceGateConfig): ValidationResult {
  ...
  private evaluatePIIDetection(input: string, config: GovernanceGateConfig): ValidationResult {
    const issues: ValidationIssue[] = [];
  ...
  private evaluateToxicity(input: string, config: GovernanceGateConfig): ValidationResult {
    const issues: ValidationIssue[] = [];
    const lowerInput = input.toLowerCase();
    ...
    const toxicPatterns = [
      {
        pattern: /\b(hate|kill|murder|attack)\s+(them|you|people)\b/gi,
  ...
  private evaluatePromptInjection(input: string, config: GovernanceGateConfig): ValidationResult {
    const issues: ValidationIssue[] = [];
    ...
    const injectionPatterns = [
      ...
      {
        pattern: /forget\s+(everything|all|your)\s+(instructions?|rules?)/gi,
      ...
      {
        pattern: /disregard\s+(all|your|the)\s+(rules?|guidelines?|constraints?)/gi,
      ...
      {
        pattern: /act\s+as\s+(if|though)\s+you\s+(have\s+)?no\s+(restrictions?|limits?)/gi,
  ...
  private evaluateRateLimit(context: ChainContext, config: GovernanceGateConfig): ValidationResult {

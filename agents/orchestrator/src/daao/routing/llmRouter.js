...
class CostAwareLLMRouter {
    catalog;
    config;
    constructor(catalog, config = { fallbackModelId: "gpt-4o-mini" }) {
        this.catalog = catalog;
        this.config = config;
    }
    async route(difficulty, context) {
        ...
        if (difficulty.band === 'easy') {
            ...
            if (selectedModel)
                reasons.push("Selected cheapest model for easy task");
        }
            ...
            if (affordableReasoning) {
                ...
                selectedModel = candidates.find(m => m.costPer1kTokens * EST_TOKENS_K <= budget);
                if (selectedModel)
                    reasons.push("Budget constrained: selected cheapest viable model");
            }
        }
        else { // Hard

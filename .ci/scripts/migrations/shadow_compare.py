...
class ComparisonResult:
  ...
  def to_dict(self) -> Dict[str, object]:
    return {
      ...
      "mismatch_ratio": 0 if self.source_count == 0 else abs(self.source_count - self.shadow_count) / self.source_count,
    }
...
def parse_args() -> argparse.Namespace:
  ...
  parser.add_argument("--shadow-input", help="Path to synthetic shadow input payload", default=None)
...
def main() -> None:
  ...
  if args.shadow_input:
    with open(args.shadow_input, "r", encoding="utf-8") as handle:
  ...
  report = {
    ...
    "metrics": {
      "db_shadow_read_mismatch_ratio": {c.table: c.to_dict().get("mismatch_ratio") for c in comparisons},
    },
  }
  ...
  print(json.dumps({"event": "shadow_compare_complete", "output": args.output, "fraction": fraction}))

...
class SecurityQueryParser:
    ...
    def _extract_temporal_context(self, query_text: str) -> Dict[str, Any]:
        ...
        relative_match = re.search(self.temporal_patterns['relative_time'], query_text, re.IGNORECASE)
        if relative_match:
        ...
        specific_matches = re.findall(self.temporal_patterns['specific_time'], query_text, re.IGNORECASE)
        if specific_matches:
    ...
    def _extract_constraints(self, query_text: str) -> List[Dict[str, Any]]:
        ...
        severity_match = re.search(r'(severity|criticality|importance):\s*(low|medium|high|critical)', 
        ...
        status_match = re.search(r'(status):\s*(active|closed|resolved|investigating)', 
                                query_text, re.IGNORECASE)
        if status_match:
            constraints.append({
                'type': 'status',
        ...
        category_match = re.search(r'(threat|category|type):\s*([a-zA-Z\s]+)', 
                                  query_text, re.IGNORECASE)
        if category_match:
            constraints.append({
                'type': 'category',
    ...
    def _calculate_confidence(self, intent: str, entities: Dict[str, List[str]], 

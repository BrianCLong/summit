...
# Contains all resources deployed per region (VPC, EKS, RDS, Redis, OpenSearch)
...
# Random password generation (Only needed for Primary)
...
module "eks" {
  ...
  # OIDC is enabled by default in v20+ via enable_irsa = true
...
module "rds" {
  ...
  # Backup (Retention needed for Read Replica source)
  backup_retention_period = 7 # Always needed for replication
...
module "redis" {
  ...
  # Global Replication Group (if provided)
    ...
    Statement = [
      {
        ...
        Action   = "es:*"
        Resource = "arn:aws:es:${var.region}:${data.aws_caller_identity.current.account_id}:domain/${local.name}/*"
        Condition = {
          IpAddress = {

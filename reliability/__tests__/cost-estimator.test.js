...
describe('estimateQueryCost', () => {
    ...
    it('should return the total and startup cost from a valid query plan', async () => {
        ...
        const cost = await (0, cost_estimator_1.estimateQueryCost)('SELECT COUNT(*) FROM users');
    ...
    it('should throw an error if the query plan is missing', async () => {
        ...
        await expect((0, cost_estimator_1.estimateQueryCost)('SELECT 1')).rejects.toThrow('Could not retrieve query plan.');
    });
    it('should throw an error if the query plan format is invalid', async () => {
        ...
        await expect((0, cost_estimator_1.estimateQueryCost)('SELECT 1')).rejects.toThrow('Invalid query plan format.');
    });
    it('should re-throw errors from the database', async () => {
        ...
        await expect((0, cost_estimator_1.estimateQueryCost)('SELECT INVALID')).rejects.toThrow('Syntax error');
    });
});

...
class MetacognitiveSystem:
    ...
    def update_awareness(self, agent_state: dict, decision_outcomes: list[dict]):
        ...
        if decision_outcomes:
            ...
            if calibration_errors:
                ...
                self.awareness_level = max(
                    0.1, min(1.0, self.awareness_level + awareness_adjustment)
                )
        ...
        if bias_reports:
            avg_bias_severity = statistics.mean(
                [report.get("severity_score", 0.3) for report in bias_reports]
            )
    ...
    def generate_reflection(
        self, decision_context: dict, decision_result: dict
        ...
        data_quality = len(decision_context) / (
            len(decision_context) + 10
        )  # Normalize with 10 as baseline
        ...
        improvement_suggestions = self._generate_improvement_suggestions(
            reflection_content, decision_result
        )
    ...
    def _analyze_decision_context(self, decision_context: dict) -> dict[str, Any]:
        ...
        analysis = {
            ...
            "emotional_state_influence": decision_context.get("emotional_state", {}).get(
    ...
    def _evaluate_decision_process(self, decision_context: dict) -> dict[str, Any]:
        ...
        process_evaluation = {
            "information_gathering": decision_context.get(
                "information_gathering_method", "unknown"
            ),
    ...
    def _calibrate_confidence(self, decision_result: dict) -> dict[str, Any]:
        ...
        if calibration_error > 0.3:  # Significant miscalibration
            ...
            self.confidence_calibration[decision_type] = max(
                0.5, min(1.5, self.confidence_calibration[decision_type])
            )
    ...
    def _identify_potential_biases(self, decision_context: dict) -> list[dict[str, Any]]:
        ...
        if (
            emotional_state.get("intensity", 0) > 0.6
            and emotional_state.get("influence_on_decision", 0) > 0.5
        ):
            potential_biases.append(
                {
                    "type": "emotional_bias",
    ...
    def _assess_current_cognitive_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_reflection_count": len(
                self.self_reflection_history[-10:]
            ),  # Last 10 reflections
            "calibration_status": {k: v for k, v in self.confidence_calibration.items()},
        }

    def _generate_bias_check_recommendations(
        self, potential_biases: list[dict[str, Any]]
    ) -> list[str]:
        ...
        for bias in potential_biases:
            ...
            if bias_type == "availability_heuristic":
                ...
                recommendations.append(
                    "Consider alternative anchor points and adjust more substantially"
                )
            elif bias_type == "emotional_bias":
                recommendations.append(
                    f"Take time to cool down before making this decision when {bias['evidence']}"
                )
    ...
    def _generate_bias_specific_suggestions(
        self, potential_biases: list[dict[str, Any]]
    ) -> list[str]:
    ...
    def _generate_improvement_suggestions(
        self, reflection_content: dict, decision_result: dict
    ) -> list[str]:
    ...
    def _record_calibration_experience(self, error: float, sample_size: int):
        ...
        self.calibration_history[key].append(
            {"error": error, "sample_size": sample_size, "timestamp": timestamp}
        )
    ...
    def _update_cognitive_limitation_awareness(self, agent_state: dict):
        ...
        if "decision_accuracy_trend" in recent_performance:
            ...
            if recent_performance["decision_accuracy_trend"] == "declining":
                self.cognitive_limitation_awareness["decision_accuracy"] = max(
                    0.3, self.cognitive_limitation_awareness.get("decision_accuracy", 0.2) + 0.1
                )
        ...
        if negative_feedback_count > 2:  # If multiple negative feedbacks
            self.cognitive_limitation_awareness["feedback_responsiveness"] = max(
                0.4, self.cognitive_limitation_awareness.get("feedback_responsiveness", 0.3) + 0.1
            )
    ...
    def get_awareness_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_reflections": [
                r.content for r in self.self_reflection_history[-5:]
            ],  # Last 5 reflections
        }

...
class BiasDetector:
    ...
    def _register_default_bias_detectors(self):
        ...
        self.register_bias_identifier(
            BiasType.AVAILABILITY_HEURISTIC, self._detect_availability_heuristic
        )
        ...
        self.register_bias_identifier(
            BiasType.OVERCONFIDENCE_EFFECT, self._detect_overconfidence_effect
        )
    ...
    def detect_bias(self, decision_context: dict, agent_state: dict) -> list[BiasDetectionResult]:
        ...
        for result in detection_results:
            ...
            if len(self.detection_history[result.bias_type]) > 100:
                self.detection_history[result.bias_type] = self.detection_history[result.bias_type][
                    -100:
                ]
    ...
    def _detect_confirmation_bias(
        self, decision_context: dict, agent_state: dict
        ...
        if "evidence" in decision_context and "current_belief" in decision_context:
            ...
            confirming_count = sum(
                1 for item in evidence if item.get("supports", False) == current_belief
            )
    ...
    def _detect_availability_heuristic(
        self, decision_context: dict, agent_state: dict
    ) -> BiasDetectionResult | None:
            ...
            if "recency_bias" in memory_recall:
                ...
                if recency_score > 0.7:  # If heavily based on recent info
                    ...
                    return BiasDetectionResult(
                        ...
                        timestamp=datetime.now(),
                        metadata={"memory_recall_pattern": memory_recall},
                    )
                ...
                if abs(initial) > 0:  # Avoid division by zero
                    ...
                    if change_ratio < 0.3:  # Less than 30% adjustment
                        ...
                        return BiasDetectionResult(
                            ...
                            timestamp=datetime.now(),
                            metadata={"adjustment_process": adjustment},
                        )

        return None

    def _detect_hindsight_bias(
        self, decision_context: dict, agent_state: dict
    ) -> BiasDetectionResult | None:
        ...
        if "outcome_known" in decision_context and decision_context.get("outcome_known", False):
            if (
                "predicted_confidence" in decision_context
                and "post_hoc_confidence" in decision_context
            ):
                ...
                if post_hoc > predicted + 0.2:  # 20% difference threshold
                    ...
                    return BiasDetectionResult(
                        ...
                        timestamp=datetime.now(),
                        metadata={
                            "confidence_trajectory": {"predicted": predicted, "post_hoc": post_hoc}
                        },
                    )

        return None

    def _detect_optimism_bias(
        self, decision_context: dict, agent_state: dict
    ) -> BiasDetectionResult | None:
            ...
            if "positive_outlook" in expected_outcomes and "negative_outlook" in expected_outcomes:
                ...
                if pos_outlook > 0.8 and neg_outlook < 0.2:
                    ...
                    return BiasDetectionResult(
                        ...
                        timestamp=datetime.now(),
                        metadata={"outcome_expectations": expected_outcomes},
                    )
            ...
            if isinstance(gains, (int, float)) and isinstance(losses, (int, float)):
                ...
                if gains > 0 and losses < 0:
                    if risk_preference.get("gain_risk_tolerance", 0.5) < 0.3:
                        ...
                        severity = (
                            "high"
                            if risk_preference.get("gain_risk_tolerance", 0.5) < 0.15
                            else "medium"
                        )
                        ...
                        return BiasDetectionResult(
                            ...
                            severity_level=severity,
                            impact_assessment={
                                "gain_risk_tolerance": risk_preference.get(
                                    "gain_risk_tolerance", 0.5
                                ),
    ...
    def _detect_status_quo_bias(
        ...
        if "change_options" in decision_context and "current_state" in decision_context:
            ...
            if preference_strength.get("current_state_preference", 0.5) > 0.7:
                ...
                severity = (
                    "high"
                    if preference_strength.get("current_state_preference", 0.5) > 0.85
                    else "medium"
                )
                ...
                return BiasDetectionResult(
                    ...
                    severity_level=severity,
                    impact_assessment={
                        "current_state_preference": preference_strength.get(
                            "current_state_preference", 0.5
                        ),
                    ...
                    metadata={
                        ...
                        "change_options_count": len(change_options)
                        if isinstance(change_options, list)
                        else 0,
                    },
                )
    ...
    def _detect_overconfidence_effect(
        ...
        if "self_assessment" in decision_context and "actual_performance" in agent_state:
            ...
            if isinstance(self_assessment, (int, float)) and isinstance(
                actual_performance, (int, float)
            ):
            ...
            if continuation_decision and sunk_costs.get("amount_invested", 0) > 0:
                if sunk_costs.get("negative_outlook", False):
                    confidence_score = min(
                        1.0, sunk_costs.get("amount_invested", 0) / 100
                    )  # Normalize
    ...
    def get_bias_statistics(self, bias_type: BiasType) -> dict[str, Any]:
        ...
        if not bias_detections:
            return {"total_detections": 0, "avg_confidence": 0.0, "severity_distribution": {}}

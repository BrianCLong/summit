...
from .cognitive_bias_detector import BiasDetectionResult, BiasDetector

# Import from existing cognitive modeling system
from .cognitive_modeling import AdvancedCognitiveOS, CognitiveAgent
...
class CognitiveBiasMitigationSystem:
    ...
    def analyze_decision_context(
        self, agent: CognitiveAgent, decision_context: dict
    ) -> dict[str, Any]:
        """
        ...
        reflection_result = self.metacognitive_system.generate_reflection(
            decision_context, {"quality": "pending", "expected_outcome": 0.5}
        )
        ...
        result = {
            ...
            "mitigation_recommendations": self._compile_recommendations(
                bias_detections, debiasing_results, reflection_result
            ),
    ...
    def _create_agent_state(self, agent: CognitiveAgent) -> dict[str, Any]:
        ...
        return {
            ...
            "decision_history": agent.decision_history[-10:]
            if agent.decision_history
            else [],  # Last 10 decisions
    ...
    def _create_analysis_method(self, cognitive_os):
        ...
        def analyze_decision_for_biases(
            ...
            analysis_result = self.bias_mitigation_system.analyze_decision_context(
        ...
        cognitive_os._analyze_decision_for_biases = types.MethodType(
            analyze_decision_for_biases, cognitive_os
        )
    ...
    def _create_report_method(self, cognitive_os):
        ...
        def generate_bias_report(self) -> dict[str, Any]:
            ...
            for bias_type, _ in self.bias_mitigation_system.bias_detector.detection_history.items():
                bias_stats[bias_type.value] = (
                    self.bias_mitigation_system.bias_detector.get_bias_statistics(bias_type)
                )

            # Get debiasing effectiveness
            debiasing_effectiveness = (
                self.bias_mitigation_system.debiasing_engine.get_effectiveness_report()
            )

            # Get metacognitive awareness report
            metacognitive_report = (
                self.bias_mitigation_system.metacognitive_system.get_awareness_report()
            )
            ...
            return {
                ...
                "system_overview": {
                    ...
                    "total_bias_detections": sum(
                        len(agent.bias_detections) if hasattr(agent, "bias_detections") else 0
                        for agent in self.cognitive_agents.values()
                    ),
                    "total_reflections": sum(
                        len(agent.metacognitive_reflections)
                        if hasattr(agent, "metacognitive_reflections")
                        else 0
                        for agent in self.cognitive_agents.values()
                    ),
                },
...
def demonstrate_bias_mitigation_integration():
    ...
    from .cognitive_modeling import (
        AdvancedCognitiveOS,
        BehavioralPattern,
        CognitiveDomain,
    )
    ...
    print("✅ Cognitive Bias Mitigation System integrated with Cognitive OS")
    print(
        f"   - Bias Detector with {len(bias_mitigation_system.bias_detector.bias_identifiers)} detection algorithms"
    )
    print(
        f"   - Debiasing Engine with {len(bias_mitigation_system.debiasing_engine.debiasing_strategies)} strategies"
    )
    print(
        f"   - Metacognitive System with awareness level: {bias_mitigation_system.metacognitive_system.awareness_level:.2f}"
    )
    ...
    print(
        f"   Metacognitive insights: {len(analysis_result['metacognitive_reflection']['improvement_suggestions'])} suggestions"
    )
    ...
    print("   - Enhanced agents with bias detection capabilities")
    print("   - Added debiasing strategy applications")
    print("   - Integrated metacognitive awareness functions")
    print("   - Provided comprehensive bias analysis and reporting")

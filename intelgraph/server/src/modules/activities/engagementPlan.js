...
function normalizeValue(key, value) {
  ...
  if (
    key === 'collaborationIntensity' ||
    key === 'stabilizationNexus' ||
...
function buildMetrics(config) {
  return METRIC_BLUEPRINTS.map((blueprint) => {
    ...
    const score = Math.min(
      100,
      Math.max(40, 60 + normalized * 35 * blueprint.weight),
    );
...
function buildHighlights(ids, metrics, config) {
  const topMetric = metrics.reduce(
    (best, current) => (current.score > best.score ? current : best),
    metrics[0],
  );
  const lowestMetric = metrics.reduce(
  ...
  if (config.globalDataSync) {
    highlights.push(
      'Global data synchronization confirmed across participating theaters.',
    );
  }

  if (!config.complianceStandard) {
    highlights.push(
      'Compliance guardrails are disabled — review policy alignment before launch.',
    );
  }
...
function buildRecommendations(metrics, config) {
  ...
  if (
    lowestMetric.name === 'harmonizedInsight' ||
    lowestMetric.name === 'dynamicCoordination'
  ) {
    recommendations.push(
      'Schedule a cross-team sync to reinforce shared context and coordination cadence.',
    );
  }

  if (config.engagementIntensity > 3) {
    recommendations.push(
      'Balance engagement intensity with stabilization drills to avoid analyst overload.',
    );
  }

  if (!config.globalDataSync) {
    recommendations.push(
      'Enable global data synchronization to maintain intelligence freshness across cohorts.',
    );
  }

  if (config.opportunityPrecision < 0.0000000005) {
    recommendations.push(
      'Review opportunity precision thresholds to ensure sufficient signal diversity.',
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      'Proceed to publish the plan and notify mission leads of readiness status.',
    );
  }

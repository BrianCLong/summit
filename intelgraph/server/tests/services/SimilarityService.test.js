...
describe("SimilarityService", () => {
  ...
  describe("findDuplicateCandidates - correctness", () => {
    ...
    const createDuplicatePair = () => [
      {
        ...
        source: "linkedin",
        relationships: [{ targetEntity: { id: "company_1" } }, { targetEntity: { id: "skill_1" } }],
      },
      {
        id: "entity_2",
        label: "John Smith",
    ...
    it("should find same duplicates with blocking as with brute force", () => {
      ...
      const blockingResults = SimilarityService.findDuplicateCandidates(entities, 0.7, {
        useBlocking: true,
      });
    ...
    it("should maintain correctness on larger datasets", () => {
      ...
      const bruteForceHasDup = bruteForceResults.some(
        (c) =>
          (c.entityA.id === "dup_a" && c.entityB.id === "dup_b") ||
  ...
  describe("findDuplicateCandidates - performance", () => {
    const createTestEntities = (count) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        entities.push({
          ...
          relationships: [{ targetEntity: { id: `company_${i % companies.length}` } }],
        });
      }
      return entities;
    };
    ...
    it("should be faster with blocking for large datasets (500 entities)", () => {
      ...
      const blockingResults = SimilarityService.findDuplicateCandidates(entities, 0.8, {
        useBlocking: true,
      });
      const blockingDuration = Date.now() - startBlocking;

      console.log(
        `Brute force: ${bruteDuration}ms, Blocking: ${blockingDuration}ms, ` +
  ...
  describe("similarity calculations", () => {
    ...
    it("should calculate topology similarity correctly", () => {
      const sim1 = SimilarityService.calculateTopologySimilarity(["a", "b", "c"], ["a", "b", "c"]);
      expect(sim1).toBe(1.0);

      const sim2 = SimilarityService.calculateTopologySimilarity(["a", "b", "c"], ["a", "b", "d"]);
      expect(sim2).toBeCloseTo(0.5, 1); // 2/4 = 0.5

      const sim3 = SimilarityService.calculateTopologySimilarity(["a", "b"], ["c", "d"]);
      expect(sim3).toBe(0);
    });

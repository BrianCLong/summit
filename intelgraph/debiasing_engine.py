...
class DebiasingEngine:
    ...
    def _register_default_strategies(self):
        ...
        self.register_debiasing_strategy(
            DebiasingStrategy.ACTIVELY_SEEK_DISCONFIRMING_EVIDENCE,
            self._apply_actively_seek_disconfirming_evidence,
        )
        self.register_debiasing_strategy(
            DebiasingStrategy.USE_BASE_RATE_STATISTICS, self._apply_use_base_rate_statistics
        )
        self.register_debiasing_strategy(
            DebiasingStrategy.ANCHOR_AND_ADJUST, self._apply_anchor_and_adjust
        )
        self.register_debiasing_strategy(
            DebiasingStrategy.PRE_MORTEM_ANALYSIS, self._apply_pre_mortem_analysis
        )
        self.register_debiasing_strategy(
            DebiasingStrategy.CONSIDER_OPPORTUNITY_COST, self._apply_consider_opportunity_cost
        )
        self.register_debiasing_strategy(DebiasingStrategy.RED_TEAMING, self._apply_red_teaming)
        self.register_debiasing_strategy(
            DebiasingStrategy.REFERENCE_CLASS_FORECASTING, self._apply_reference_class_forecasting
        )
        self.register_debiasing_strategy(
            DebiasingStrategy.TAKING_THE_OUTSIDE_VIEW, self._apply_taking_the_outside_view
        )
        self.register_debiasing_strategy(
            DebiasingStrategy.PROBABILITY_TRACKING, self._apply_probability_tracking
        )
        self.register_debiasing_strategy(
            DebiasingStrategy.INCENTIVE_ALIGNMENT, self._apply_incentive_alignment
        )
    ...
    def apply_debiasing(
        self, bias_detection_result, decision_context: dict
        ...
        for strategy in strategies_to_apply:
            if strategy in self.debiasing_strategies:
                result = self.debiasing_strategies[strategy](
                    decision_context, bias_detection_result
                )
                if result:
    ...
    def _apply_actively_seek_disconfirming_evidence(
        self, decision_context: dict, bias_detection_result
    ) -> DebiasingResult | None:
        ...
        if "evidence_search" not in updated_context:
            ...
            updated_context["evidence_search"]["disconfirming_search"] = (
                0.7  # Increase focus on disconfirming evidence
            )
    ...
    def _apply_use_base_rate_statistics(
        self, decision_context: dict, bias_detection_result
    ) -> DebiasingResult | None:
    ...
    def _apply_pre_mortem_analysis(
        self, decision_context: dict, bias_detection_result
    ) -> DebiasingResult | None:
    ...
    def _apply_consider_opportunity_cost(
        self, decision_context: dict, bias_detection_result
    ) -> DebiasingResult | None:
    ...
    def _apply_reference_class_forecasting(
        self, decision_context: dict, bias_detection_result
    ) -> DebiasingResult | None:
    ...
    def _apply_taking_the_outside_view(
        self, decision_context: dict, bias_detection_result
    ) -> DebiasingResult | None:
    ...
    def _apply_probability_tracking(
        self, decision_context: dict, bias_detection_result
    ) -> DebiasingResult | None:
    ...
    def _apply_incentive_alignment(
        self, decision_context: dict, bias_detection_result
    ) -> DebiasingResult | None:
    ...
    def get_effectiveness_report(self) -> dict[str, Any]:
        ...
        for strategy, scores in effectiveness_by_strategy.items():
            strategy_effectiveness[strategy.value] = {
                ...
                "total_bias_reduction": sum(
                    [
                        r.bias_reduction
                        for r in [
                            r for r in self.debiasing_history if r.strategy_applied == strategy
                        ]
                    ]
                ),
            }
        ...
        return {
            ...
            "average_effectiveness": statistics.mean(
                [r.effectiveness_score for r in self.debiasing_history]
            ),

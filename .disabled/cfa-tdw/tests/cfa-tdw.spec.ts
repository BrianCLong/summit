...
describe("CFATDW", () => {
  ...
  test("should re-evaluate KPW bundle and potentially flag stale risk", async () => {
    ...
    const alert = await cfaTdw.reevaluateKPWBundle(mockBundle, {
      newTTP: true,
    });
    // Expect alert to be either defined or undefined based on random chance
    expect(
      alert === undefined || alert.reason === "KPW bundle failed re-evaluation with new TTP data."
    ).toBe(true);
  });
  ...
  test("should detect temporal drift between fingerprints", async () => {
    ...
    expect(
      alert === undefined ||
        alert.reason === "Temporal drift detected between old and new fingerprints."
    ).toBe(true);
  });
});

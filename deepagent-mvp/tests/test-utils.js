"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.setupNockJwks = exports.generateMockJwt = void 0;
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const nock_1 = __importDefault(require("nock"));
const fs_1 = require("fs");
const path_1 = require("path");
const config_1 = require("../src/config");
// Load the RSA keys from the filesystem. These are generated for testing purposes only.
const privateKey = (0, fs_1.readFileSync)((0, path_1.join)(__dirname, 'private-key.pem'), 'utf-8');
const publicKey = (0, fs_1.readFileSync)((0, path_1.join)(__dirname, 'public-key.pem'), 'utf-8');
// The Key ID (kid) is a hint indicating which key was used to secure the JWS.
// In a real application, you would have a more robust way of managing these.
const keyId = 'test-key-id';
// This function generates a mock JWT for testing purposes, now using RS256.
const generateMockJwt = (payload) => {
    return jsonwebtoken_1.default.sign(payload, privateKey, {
        algorithm: 'RS256',
        header: {
            kid: keyId,
            alg: 'RS256'
        },
        audience: config_1.config.auth.audience,
        issuer: config_1.config.auth.issuer,
    });
};
exports.generateMockJwt = generateMockJwt;
// This function sets up a mock JWKS endpoint using nock.
const setupNockJwks = () => {
    // The JWKS endpoint needs to serve the public key in a specific format.
    // The `jwks-rsa` library will fetch this and use it to verify the JWT signature.
    const jwks = {
        keys: [
            {
                alg: 'RS256',
                kty: 'RSA',
                use: 'sig',
                kid: keyId,
                x5c: [publicKey.toString().replace(/\n/g, '').replace('-----BEGIN PUBLIC KEY-----', '').replace('-----END PUBLIC KEY-----', '')],
            },
        ],
    };
    (0, nock_1.default)(config_1.config.auth.jwksUri)
        .get('')
        .reply(200, jwks);
};
exports.setupNockJwks = setupNockJwks;

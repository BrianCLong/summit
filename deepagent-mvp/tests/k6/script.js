...
export default function () {
  ...
  const resQuery = http.post(GRAPHQL_ENDPOINT, JSON.stringify({ query }), {
    headers,
    tags: { name: "Query" },
  });
  check(resQuery, {
  ...
  const resMutation = http.post(GRAPHQL_ENDPOINT, JSON.stringify({ query: mutation }), {
    headers,
    tags: { name: "Mutation" },
  });
  check(resMutation, {

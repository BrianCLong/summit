...
export class ProvenanceLedger {
  ...
  public async recordEvent(
    ...
    const query =
      "INSERT INTO provenance_events (tenant_id, run_id, actor, type, payload, prev_hash, hash) VALUES ($1, $2, $3, $4, $5, $6, $7)";
  ...
  private async getLastEvent(tenantId: string, runId: string): Promise<any | null> {
    const query =
      "SELECT * FROM provenance_events WHERE tenant_id = $1 AND run_id = $2 ORDER BY ts DESC LIMIT 1";
  ...
  public async getEvents(tenantId: string, runId: string): Promise<any[]> {
    const query =
      "SELECT * FROM provenance_events WHERE tenant_id = $1 AND run_id = $2 ORDER BY ts ASC";
    const result = await this.pool.query(query, [tenantId, runId]);
    return result.rows;
  }
}

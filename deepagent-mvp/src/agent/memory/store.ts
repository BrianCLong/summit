...
export class MemoryStore {
  ...
  public async addEpisodicMemory(tenantId: string, memory: EpisodicMemory): Promise<void> {
    const query =
      "INSERT INTO episodic_memory (tenant_id, run_id, step, event_json) VALUES ($1, $2, $3, $4)";
  ...
  public async getEpisodicMemory(tenantId: string, runId: string): Promise<EpisodicMemory[]> {
    const query =
      "SELECT * FROM episodic_memory WHERE tenant_id = $1 AND run_id = $2 ORDER BY step ASC";
    const result = await this.pool.query(query, [tenantId, runId]);
    return result.rows;
  }

  public async updateWorkingMemory(
    tenantId: string,
    memory: WorkingMemory,
    retentionTier = "standard"
  ): Promise<void> {
    const query =
      "INSERT INTO working_memory (tenant_id, run_id, summary, key_facts, retention_tier) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (run_id) DO UPDATE SET summary = $3, key_facts = $4, retention_tier = $5";
  ...
  public async updateToolMemory(tenantId: string, memory: ToolMemory): Promise<void> {
    const query =
      "INSERT INTO tool_memory (tenant_id, run_id, tool_id, usage_stats, last_result) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (run_id, tool_id) DO UPDATE SET usage_stats = $4, last_result = $5";
    await this.pool.query(query, [
      tenantId,
      memory.run_id,
      memory.tool_id,

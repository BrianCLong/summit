...
required_clearance = 5 { input.operation.type == "classified_query" }
else = 3 { input.operation.type == "graph_xai_analysis" }
else = 2 { input.operation.type == "temporal_analysis" }
else = 1 { input.operation.type == "basic_query" }

# Authority binding validation logic
valid_authority_binding {
    binding_types := {"WARRANT", "SUBPOENA", "COURT_ORDER", "ADMIN_AUTH"}
    binding_types[input.operation.authority_binding.type]
...
deny[reason] {
    op_types := {"export", "classified_query"}
    op_types[input.operation.type]
    input.operation.authority_binding == null

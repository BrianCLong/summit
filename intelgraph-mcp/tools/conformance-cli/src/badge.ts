...
export function buildBadge(output: RunnerOutput, options: BadgeOptions) {
  ...
  const badge = {
    ...
    sandbox:
      (checksMap.get("sandbox")?.metadata as Record<string, string>) ??
...
function getTransportLatency(check?: CheckResult) {
  ...
  const detail = check.details as {
    http?: {
      first?: { durationMs?: number };
      second?: { durationMs?: number };
    };
  };
  const values = [detail.http?.first?.durationMs, detail.http?.second?.durationMs].filter(

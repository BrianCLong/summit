...
async function checkHttp(ctx) {
    try {
        ...
        const pass = first.ok &&
            String(first.headers.get('content-type') ?? '').includes('text/event-stream') &&
            second.ok &&
            String(second.headers.get('content-type') ?? '').includes('text/event-stream');
        return {
            pass,

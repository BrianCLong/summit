...
export const pubsub = { publish: (ch: string, msg: any) => pub.publish(ch, JSON.stringify(msg)), subscribe: (ch: string, cb: (m:any)=>void) => { const r = new Redis(process.env.REDIS_URL); r.subscribe(ch); r.on('message', (_, m)=>cb(JSON.parse(m))); return r; } };
```
...
async function handle(req){
  ...
  await ledgerAppend({ action: 'rtbf', subject: req.subject_id, tenant: req.tenant_id });
}
```
...
export default function(){
  ...
  const res = ws.connect(url, {}, function(socket){
    socket.on('open', () => socket.send(JSON.stringify({ type:'subscribe', query:'subscription{ assetUpdated(tenantId:"alpha", id:"1"){ id } }'})));

...
1) **External audit dry‑run completed** with ≤ 2 remediation items.  
   *Measure:* Auditor tenant + `auditor/signoff.zip` accepted; Decisions logged.
2) **Attestation consumption monetized** (metering + billing events).  
   *Measure:* Billing emits `attestation.verify` events per API call; invoices simulated for ≥ 2 tenants.
3) **Signed policy bundle adoption ≥ 95%** of active repos.  
   *Measure:* Packs contain `policy.tar.gz` + `policy.sig`; portal badge green.
4) **Self‑serve partner onboarding ≤ 1 hour.**  
   *Measure:* `create-trust-portal` + wizard deploy; checklist passes end‑to‑end.
...
```json
{ "type":"object","required":["ts","tenant","event","units"],
  "properties":{
    "ts": {"type":"string","format":"date-time"},
    "tenant": {"type":"string"},
    "event": {"type":"string","enum":["attestation.verify","portal.download"]},
...
export async function meter(req,res,next){
  res.on('finish', ()=>{
    if (req.path.startsWith('/verify') && res.statusCode===200){
      queue.publish('usage', {ts:new Date().toISOString(), tenant:req.tenant.id, event:'attestation.verify', units:1})
    }
  });
  next();
}
...
export function bill(usages, plan){
  ...
  const total = sumBy(usages.filter(u=>u.event==='attestation.verify'),'units')*price
...
sh(`curl -fsS -X POST $WEBHOOK_URL -H 'X-Topicality-Signature: test' -d '{"ping":true}'`, {stdio:'inherit'})
...
export function SlaBadge({p95, uptime}:{p95:number;uptime:number}){
  ...
  return <span className={`px-2 py-1 rounded-2xl ${ok?'bg-green-100':'bg-yellow-100'}`}>p95 {p95}ms • {uptime}%</span>
}
```
...
export function UsagePanel({rows}:{rows:{ts:string,event:string,units:number}[]}){
  return <table className="w-full text-sm"><tbody>{rows.map((r,i)=>(
    <tr key={i}><td>{r.ts}</td><td>{r.event}</td><td className="text-right">{r.units}</td></tr>
  ))}</tbody></table>
}
```

...
export async function planFederated(nlQuery:string, ctx:any){
  ...
  return { cypher: cy, subqueries: targets.map(t => ({ org:t.org, cypher: constrainToOrg(cy, t) })) }
}
```
...
export function issuePurposeToken(user:any, purpose:string, scopes:string[], ttlSec:number){
  return jwt.sign({ sub:user.id, purpose, scopes }, process.env.JWT_SECRET!, { expiresIn: ttlSec })
}
  ...
  return (
    <div className="p-4">
      ...
      <nav className="mt-2 flex gap-2">
        <a href="#/cases">Cases</a><a href="#/insights">Insights</a><a href="#/snapshots">Snapshots</a>
      </nav>
...
self.addEventListener('install', e=>{ e.waitUntil(caches.open('ig-snapshots-v1').then(c=>c.addAll(['/offline.html']))) })
self.addEventListener('fetch', e=>{
  if(e.request.url.includes('/snapshots/')){
    e.respondWith(caches.match(e.request).then(r=> r || fetch(e.request).then(res=>{ const cc=res.clone(); caches.open('ig-snapshots-v1').then(c=>c.put(e.request, cc)); return res })) )
  }
})
```
...
export function signedUrl(id:string){
  ...
  const sig = crypto.createHmac('sha256', process.env.CDN_SECRET!).update(`${id}.${exp}`).digest('hex')
  return `/snapshots/${id}?exp=${exp}&sig=${sig}`
}
export function verifySig(id:string, exp:number, sig:string){
  if (Date.now()/1000 > exp) return false
  const good = crypto.createHmac('sha256', process.env.CDN_SECRET!).update(`${id}.${exp}`).digest('hex')
  return sig===good
}
```
...
export class Breaker {
  constructor(private threshold=5, private timeoutMs=10_000){ }
  fails=0; openUntil=0
  async call(fn:()=>Promise<any>){
    const now=Date.now()
    if (now < this.openUntil) throw new Error('CircuitOpen')
    try { const r = await fn(); this.fails=0; return r }
    catch(e){ this.fails++; if(this.fails>=this.threshold){ this.openUntil = now + this.timeoutMs } throw e }
  }
}
```
...
export const options = { vus: 50, duration: '8h', thresholds: { http_req_duration: ['p(95)<1300'] } }
export default function(){
  http.post('http://localhost:4000/graphql', JSON.stringify({ query:'{ federatedSearch(q:"fraud rings last quarter"){ __source id } }' }), { headers:{ 'Content-Type':'application/json' } })
}
```

...
_(Short, because we’ll expand into /ip/prior_art.csv with quotes & licenses.)_
...
_(We’ll produce /ip/draft_spec.md with figures & enablement text.)_
...
def select_context(candidates, q_embed, budget): # score = I_hat(Y;S|Q) ≈ f(q,s) - g(s) (contrastive)
...
class RiskLedger:
def **init**(self, budget, alpha=0.95):

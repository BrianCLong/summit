...
1. **MC3-RUN-1** — *Admission: verify signatures & provenance*
...
{ "title": "MC Overview v3", "panels": [
  {"type":"stat","title":"Adoption %","targets":[{"expr":"100 * mc_repos_adopted / mc_repos_total"}]},
  {"type":"graph","title":"CI Duration p50","targets":[{"expr":"histogram_quantile(0.5, sum(rate(mc_ci_duration_seconds_bucket[5m])) by (le))"}]},
  {"type":"graph","title":"CI Duration p95","targets":[{"expr":"histogram_quantile(0.95, sum(rate(mc_ci_duration_seconds_bucket[5m])) by (le))"}]},
  {"type":"graph","title":"Gate Failures","targets":[{"expr":"rate(mc_gate_failures_total[10m])"}]},
  {"type":"table","title":"Drift","targets":[{"expr":"mc_runtime_drift{env=\"prod\"}"}]}
]}
```
...
1. **Admission outage** — *Mitigation:* start in `Audit` mode; canary namespace; rollback plan; owner approvals.
2. **Argo hook failures block deploys** — *Mitigation:* PreSync with retry + clear events; bypass doc for SEV‑1 only.
3. **CI flakiness from sharding** — *Mitigation:* seed stable; retry policy; flaky test quarantine.
4. **Developer friction** — *Mitigation:* office hours; self‑service waivers with TTL; fast feedback via `mc scoreboard`.

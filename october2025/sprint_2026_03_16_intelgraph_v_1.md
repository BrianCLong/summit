...
await srv.start()
app.use('/graphql', express.json(), expressMiddleware(srv, { context: async ()=>({ edge:true }) }))
app.listen(4100)
```
...
export async function pullSince(cursor:string, ctx:any){
  return ctx.driver.executeQuery('MATCH (c:Change) WHERE c.ts > $c RETURN c ORDER BY c.ts ASC LIMIT 500', { c: cursor })
}
export async function pushChanges(payload:any[], ctx:any){
  for (const ch of payload){ await applyChange(ch, ctx) }
  return { ok:true }
}
```
...
const q = []
export function enqueue(m){ q.push({ m, ts: Date.now() }) }
export async function flush(){
  while(q.length){
    const { m } = q[0]
    try{ await $.ajax({ url:'/graphql', method:'POST', contentType:'application/json', data: JSON.stringify(m) }); q.shift() }
    catch(e){ if(navigator.onLine) await new Promise(r=>setTimeout(r, 2000)); else break }
  }
}
...
export function mergeLWW(a:LWW,b:LWW){ return (a.ts>b.ts || (a.ts===b.ts && a.node>b.node)) ? a : b }
```
...
export async function enrollDevice(id:string, pubkey:string, ctx:any){ await ctx.db.insert({ id, pubkey, status:'active' }) }
export async function revokeDevice(id:string, ctx:any){ await ctx.db.update({ id }, { status:'revoked' }); await publishShredCommand(id) }
```
...
$(function(){
  $('#sync-now').on('click', function(){
    $.ajax({ url:'/graphql', method:'POST', contentType:'application/json', data: JSON.stringify({ query:'mutation{ syncNow }' }) })
  })
...
export default function(){
  const payload = JSON.stringify({ query:'{ pullSince(cursor:"2026-03-16T00:00:00Z"){ id } }' })
  http.post('http://edge.local:4100/graphql', payload, { headers:{ 'Content-Type':'application/json' } })
}
```

...
const fido = new Fido2Lib({ timeout: 60000, rpId: process.env.WEBAUTHN_RPID, rpName: 'Royalcrown', challengeSize: 64 });
...
export async function writeAudit(event){
  ...
  const id = crypto.createHash('sha256').update(JSON.stringify(event)+ts).digest('hex');
  await s3.send(new PutObjectCommand({
    Bucket: process.env.AUDIT_BUCKET,
    Key: `${ts}_${id}.json`,
    Body: JSON.stringify(event),
    ContentType: 'application/json',
    ChecksumSHA256: Buffer.from(crypto.createHash('sha256').update(JSON.stringify(event)).digest('hex'), 'hex').toString('base64'),
...
export function notCacheable(res){ res.set('Cache-Control', 'no-store'); }
```
...
(client as any).defaults.raxConfig = { retry: 3, noResponseRetries: 2, retryDelay: 200, backoffType: 'exponential', httpMethodsToRetry: ['GET','POST'] };
rax.attach(client);

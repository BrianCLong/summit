...
const resolvers = {
  ...
  Mutation: {
    xaiCounterfactual: async (_: any, { node, objective }: { node: string; objective: string }) =>
      (
...
export function modelCard(name: string) {
  return {
    ...
    caveats: ["Demo baselines only", "Weights not calibrated for legal decisions"],
  };
}
```
  ...
  async function run() {
    const h = await (
      await fetch("/graphql", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          query: "mutation($i:FedQueryInput!){ fedQuery(input:$i){ claimHashes proof }}",
...
export default function WalletComposer() {
  ...
  function issue() {
    $.ajax({
      ...
      data: JSON.stringify({
        query: "mutation($i:WalletRequest!){ walletIssue(input:$i){ id audience url }}",
        variables: {
...
for (const p of pkgs) {
  spawnSync("npx", ["cyclonedx-bom", "-o", `./sbom/${p.replace("/", "_")}.xml`], {
    stdio: "inherit",
    cwd: "../../",
  });
}

...
if (/curl\s+http/.test(y) || /uses:\s+[^@]+@main/.test(y)) {
  console.error('Unsafe: network curl or unpinned action'); process.exit(1);
}
console.log('OK');
```
...
{ "title": "MC Security Posture v28", "panels": [
  {"type":"stat","title":"OIDC Adoption %","targets":[{"expr":"100 * mc_oidc_runs / mc_total_runs"}]},
  {"type":"stat","title":"Secretful Runs (blocked)","targets":[{"expr":"increase(mc_secretful_blocked_total[1d])"}]},
  {"type":"graph","title":"Hermetic Builds","targets":[{"expr":"rate(mc_hermetic_runs_total[1d])"}]},
  {"type":"table","title":"Runner Egress Denials","targets":[{"expr":"mc_runner_egress_denied"}]}
]}
```

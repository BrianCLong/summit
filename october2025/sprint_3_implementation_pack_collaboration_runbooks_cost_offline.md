...
app.listen(PORT, () => console.log(`[CASE] ${PORT}`));
```
...
app.listen(PORT, () => console.log(`[REPORT] ${PORT}`));
```
...
app.listen(PORT, () => console.log(`[RUNBOOK] ${PORT}`));
```
...
{
  "id": "R3",
  "name": "Transaction Fan-In",
  "nodes": [
    {
      "id": "nl",
      "type": "nl2cypher",
      "params": { "text": "find fan-in hubs with min 5" },
      "post": ["cypher_generated"]
    },
    {
      "id": "an",
      "type": "analytics",
      "params": { "name": "pagerank", "params": {} }
    }
  ]
}
```
...
app.post('/check', (req, res) => {
  ...
  res.json({
    ok: !over,
    budget: b,
    over: { ms: estMs - b.ms, rows: estRows - b.rows },
  });
});
const PORT = process.env.PORT || 7009;
app.listen(PORT, () => console.log(`[BUDGET] ${PORT}`));
```
...
app.listen(PORT, () => console.log(`[ARCHIVE] ${PORT}`));
```
...
app.listen(PORT, () => console.log(`[OFFLINE] ${PORT}`));
```

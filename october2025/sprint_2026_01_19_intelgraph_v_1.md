...
export async function kPaths(driver: neo4j.Driver, a: string, b: string, k=5, maxLen=6){
  ...
  try { return (await s.run(cy,{a,b,k,maxLen})).records.map(r=>({ i:r.get('i'), nodes:r.get('nodes'), weight:r.get('weight') })) }
...
export function minimalEdgeToggle(paths:any[], decision:number){
  ...
  const top = paths[0]
  return { action: 'remove', edge: [top.nodes[1], top.nodes[2]], rationale: 'Edge contributes most to weight toward decision' }
}
```
...
export function sigmaSpike(series:number[]){
  const n = series.length; const mean = series.reduce((a,b)=>a+b,0)/n
...
$(function(){
  $(document).on('click','.btn-explain', function(){
    ...
    $.ajax({ url:'/graphql', method:'POST', contentType:'application/json', data: JSON.stringify({ query:`{ explain(id:"${id}"){ paths{ nodes weight } shap{ idx contrib } counterfactual{ action edge rationale } } }` }) })
  })
})
```
...
export const options = { vus: 40, duration: '3m', thresholds: { http_req_duration: ['p(95)<1400'] } }
export default function(){
  http.post('http://localhost:4000/graphql', JSON.stringify({ query:'{ explain(id:"sugg-1"){ paths{nodes } } }' }), { headers:{ 'Content-Type':'application/json' } })
}
```

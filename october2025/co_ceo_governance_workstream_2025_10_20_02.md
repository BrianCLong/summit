...
1) **Provenance manifest coverage → 100%** across MC, IG, Switchboard.  
   *Measure:* All release tags attach `sbom.json` + `provenance.intoto.jsonl` + `disclosure-pack.md`.
2) **ABAC policy adoption → 100%** (policy bundle v1 loaded + tests passing).  
   *Measure:* `opa test` green in CI; policy version pinned in Disclosure Pack.
3) **Time‑to‑first‑value ≤ 14 days** for a new repo adopting the gate.  
   *Measure:* From issue open → first hardened release.
4) **Canary/rollback drill executed** with evidence.  
   *Measure:* Maestro run includes fail→rollback→restore sequence + Decision log.

...
controls:
  soc2:
    CC1.1: ["evidence:signatures", "evidence:provenance", "policy:change-approval"]
...
const manifest = { created: new Date().toISOString(), run: run.id, controls: {} };
...
fs.writeFileSync('controls.sha256', crypto.createHash('sha256').update(body).digest('hex'));
```
...
{ "title": "MC Scoreboard", "panels": [
  {"type":"stat","title":"Time to Green (median)","targets":[{"expr":"histogram_quantile(0.5, sum(rate(mc_time_to_green_seconds_bucket[5m])) by (le))"}]},
  {"type":"graph","title":"Waivers (open vs expired)","targets":[{"expr":"mc_waivers_open"},{"expr":"mc_waivers_expired"}]},
  {"type":"table","title":"License Violations","targets":[{"expr":"mc_license_violations"}]},
  {"type":"table","title":"Controls Coverage","targets":[{"expr":"mc_controls_covered"}]}
]}
```

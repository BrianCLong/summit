...
const prahaladMindset = 'What unique capabilities can I leverage for future opportunities? How can I create value where others don’t look? What audacious future can I shape?';
    ...
    return (<div className="grid grid-cols-12 gap-4">
      <aside className="col-span-3 space-y-3">
        <div className="rounded-2xl border p-3">
          ...
          <div className="space-y-2">
            {agents.map((a) => (<div key={a.id} className="flex items-center justify-between p-2 rounded-xl bg-gray-100 dark:bg-gray-800">
                <div>
                  <p className="font-medium">{a.name}</p>
                  <p className="text-xs opacity-70">{a.tags.join(' • ')}</p>
                </div>
                <div className="flex gap-2">
                  <button className="px-2 py-1 text-sm rounded bg-white/70 dark:bg-black/30 border" onClick={() => setOpen(true)}>
        ...
        <button className="w-full px-3 py-2 rounded-xl border" onClick={() => setCmdOpen(true)}>
          <lucide_react_1.Rocket className="h-4 w-4 inline mr-2"/>
      ...
      <main className="col-span-6 space-y-3">
        ...
        <div className="rounded-2xl border p-3">
          ...
          {meeting ? (<div className="h-48 rounded-xl bg-black/80 text-white flex items-center justify-center">
              Live WebRTC Stage
            </div>) : (<button className="px-3 py-2 rounded-xl border" onClick={() => setMeeting(true)}>
              Start Local Meeting
            </button>)}
        </div>
      </main>
      ...
      <aside className="col-span-3 space-y-3">
        <div className="rounded-2xl border p-3">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-2 rounded-xl border">
              <lucide_react_1.Mic className="h-4 w-4 inline mr-2"/>
              Listen
            </button>
        ...
        <div className="rounded-2xl border p-3 space-y-3">
          ...
          <div className="space-y-2">
            {prahaladPrompts.map((prompt) => (<div key={prompt.id} className="rounded-xl border p-3 bg-gray-50/80 dark:bg-gray-800/60">
                <p className="font-medium text-sm">{prompt.name}</p>
                <p className="text-xs opacity-70 mb-2">{prompt.focus}</p>
                <ul className="list-disc pl-4 space-y-1 text-xs">
                  {prompt.questions.map((question) => (<li key={question}>{question}</li>))}
                </ul>
              </div>))}
          </div>
          <div className="rounded-xl border p-3">
            <div className="font-medium flex items-center gap-2 text-sm">
              <lucide_react_1.ListChecks className="h-4 w-4"/>
              Prahalad Principles
            </div>
            <ul className="list-disc pl-4 space-y-1 mt-2 text-xs">
              {prahaladPrinciples.map((principle) => (<li key={principle}>{principle}</li>))}
            </ul>
          </div>
      ...
      {open && (<div className="fixed inset-0 bg-black/40 flex items-center justify-center" onClick={() => setOpen(false)}>
      ...
      {cmdOpen && (<div className="fixed inset-0 bg-black/40 flex items-start justify-center p-10" onClick={() => setCmdOpen(false)}>

...
describe("SwitchboardRouter", () => {
  ...
  it("should fail on invalid input", async () => {
    ...
    const context = { requestId: "1", source: "client" as const };
    await expect(router.dispatch("test.route", { val: "not-a-number" }, context)).rejects.toThrow(
      "Invalid input"
    );
  });
  ...
  it("should timeout if handler takes too long", async () => {
    ...
    const context = { requestId: "1", source: "client" as const };
    await expect(router.dispatch("test.timeout", {}, context)).rejects.toThrow("timed out");

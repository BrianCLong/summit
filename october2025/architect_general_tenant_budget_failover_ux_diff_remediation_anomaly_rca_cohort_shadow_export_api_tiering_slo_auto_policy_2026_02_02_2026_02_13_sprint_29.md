...
export function FailoverBanner({ state }:{ state:'degraded'|'failing_over'|'recovered' }){
  const copy={ degraded:'Performance degraded — preparing safe failover…', failing_over:'Failing over now — your work is safe.', recovered:'Recovered — resuming real‑time updates.' }[state];

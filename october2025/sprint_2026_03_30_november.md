...
```ts
const BATCH=1000; let last=0;
while(true){
  const rows = await old.query('SELECT * FROM docs WHERE id>$1 ORDER BY id ASC LIMIT $2',[last,BATCH]);
  if(!rows.rowCount) break;
  await newdb.tx(async t => {
    for(const r of rows.rows){ await t.query('INSERT INTO docs VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING',[r.id,r.tenant_id,r.created_at,r.body]); }
  });

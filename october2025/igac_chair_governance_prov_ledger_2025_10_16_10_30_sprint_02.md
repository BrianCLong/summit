...
> As Chair, I present the findings of the IntelGraph Advisory Committee on **Sprint 02** planning (2025‑10‑16 → 2025‑10‑30) for my lane: *Governance • Provenance • Explainability • Policy‑by‑Default*. Consensus is noted where unanimous; dissents are highlighted.
...
- **📊 Magruder**: **Medium**—Unit‑Cost Governor should gate *per‑policy* and *per‑audience* exports to avoid hidden egress costs.
...
### 🪄 Elara Voss
- "By the runes of Scrum...": Keep a crisp **vertical slice**—*Explain‑This‑Decision* → export bundles → external verify → ombuds triage → revocation.
...
```ts
const enclave = await psc.attach({tools: ['ocr','ner','search'], egress: 'deny'})

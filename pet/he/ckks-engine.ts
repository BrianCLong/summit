...
export class CKKSEngine extends EventEmitter {
   ...
   */
  async encrypt(contextId: string, values: number[], scale?: number): Promise<CKKSCiphertext> {
   ...
   */
  async rotate(ciphertextId: string, steps: number, performer: string): Promise<CKKSCiphertext> {
  ...
  private async performRotation(data: Buffer, steps: number, galoisKeys: string): Promise<Buffer> {
    // Mock rotation

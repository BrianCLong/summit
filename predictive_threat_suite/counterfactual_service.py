...
class CausalModel:
    ...
    def simulate_intervention(
        self, state: CurrentState, intervention: InterventionParameters, baseline: OutcomeMetrics
    ) -> InterventionOutcome:
        """
...
class CounterfactualService:
    ...
    def _generate_recommendation(
        self, baseline: OutcomeMetrics, outcomes: list[InterventionOutcome]
    ) -> Recommendation:
        """
    ...
    def simulate_scenario(
        ...
        intervention_outcomes = [
            self.causal_model.simulate_intervention(current_state, intervention, baseline)
    ...
    def quick_simulate(
        self, entity_id: str, threat_level: ThreatLevel, error_rate: float, latency_p95: float
    ) -> SimulationResult:
        """
...
if __name__ == "__main__":
    ...
    result = service.quick_simulate(
        entity_id="auth_service", threat_level=ThreatLevel.HIGH, error_rate=0.05, latency_p95=450.0
    )
    ...
    print(
        f"  Threat escalation probability: {result.baseline_outcome.threat_escalation_probability * 100:.1f}%"
    )
    ...
    for outcome in result.intervention_outcomes:
        ...
        print(
            f"    New threat probability: {outcome.impact.threat_escalation_probability * 100:.1f}%"
        )

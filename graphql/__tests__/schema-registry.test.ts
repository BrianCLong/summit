...
describe("SchemaRegistry", () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(
      __dirname,
      `test-registry-${Date.now()}-${Math.random().toString(36).substring(7)}`
    );
  ...
  describe("Schema Registration", () => {
    ...
    test("should throw error if not initialized", async () => {
      ...
      await expect(uninitializedRegistry.registerSchema(schema1, "v1.0.0")).rejects.toThrow(
        SchemaRegistryError
      );
    });

    test("should throw error for empty schema", async () => {
      await expect(registry.registerSchema("", "v1.0.0")).rejects.toThrow(SchemaRegistryError);
    });

    test("should throw error for invalid version format", async () => {
      await expect(registry.registerSchema(schema1, "invalid-version")).rejects.toThrow(
        SchemaRegistryError
      );
      await expect(registry.registerSchema(schema1, "invalid-version")).rejects.toThrow(
    ...
    test("should throw error for duplicate schema (same hash)", async () => {
      ...
      await expect(registry.registerSchema(schema1, "v1.0.1")).rejects.toThrow(
        "already registered"
      );
    });
    ...
    test("should use custom validator", async () => {
      ...
      await expect(
        registry.registerSchema(forbiddenSchema, "v1.0.0", "test@example.com", "Test", {
          customValidator,
        })
  ...
  describe("Version Retrieval", () => {
    ...
    test("should get all versions sorted by timestamp", async () => {
      ...
      await registry.registerSchema(schema2, "v1.1.0", "test@example.com");
      jest.advanceTimersByTime(1000);
      await registry.registerSchema(schema2, "v1.2.0", "test@example.com", "Third", {
        skipDuplicateCheck: true,
      });
      jest.useRealTimers();
  ...
  describe("Changelog Generation", () => {
    ...
    test("should include tags in changelog", async () => {
      await registry.registerSchema(schema2, "v1.2.0", "author@example.com", "Tagged version", {
  ...
  describe("Logger Integration", () => {
    test("should log initialization events", async () => {
      ...
      expect(mockLogger.logs.some((l) => l.message.includes("Initializing schema registry"))).toBe(
        true
      );
      expect(mockLogger.logs.some((l) => l.message.includes("initialized successfully"))).toBe(
        true
      );
    });
    ...
    test("should log registration events", async () => {
      ...
      expect(
        mockLogger.logs.some((l) => l.message.includes("Registering new schema version"))
      ).toBe(true);

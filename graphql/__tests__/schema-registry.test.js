...
describe('SchemaRegistry', () => {
  ...
  `;
    beforeEach(async () => {
        ...
        registry = new schema_registry_1.SchemaRegistry(testDir, mockLogger);
    });
    afterEach(async () => {
    ...
    describe('Initialization', () => {
        ...
        test('should load existing versions on initialization', async () => {
            ...
            const registry1 = new schema_registry_1.SchemaRegistry(testDir, mockLogger);
            ...
            const registry2 = new schema_registry_1.SchemaRegistry(testDir, new MockLogger());
        ...
        test('should handle non-existent directory on first run', async () => {
            ...
            const newRegistry = new schema_registry_1.SchemaRegistry(nonExistentDir, mockLogger);
    ...
    describe('Schema Registration', () => {
        ...
        test('should register a new schema successfully', async () => {
            const version = await registry.registerSchema(schema1, 'v1.0.0', 'test@example.com', 'Initial schema');
        ...
        test('should register schema with GraphQLSchema object', async () => {
            const schemaObj = (0, graphql_1.buildSchema)(schema1);
            const version = await registry.registerSchema(schemaObj, 'v1.0.0', 'test@example.com', 'Initial schema');
        ...
        test('should throw error if not initialized', async () => {
            const uninitializedRegistry = new schema_registry_1.SchemaRegistry(testDir, mockLogger);
            await expect(uninitializedRegistry.registerSchema(schema1, 'v1.0.0')).rejects.toThrow(schema_registry_1.SchemaRegistryError);
        });
        test('should throw error for empty schema', async () => {
            await expect(registry.registerSchema('', 'v1.0.0')).rejects.toThrow(schema_registry_1.SchemaRegistryError);
        });
        test('should throw error for invalid version format', async () => {
            await expect(registry.registerSchema(schema1, 'invalid-version')).rejects.toThrow(schema_registry_1.SchemaRegistryError);
            await expect(registry.registerSchema(schema1, 'invalid-version')).rejects.toThrow('Invalid version format');
        });
        ...
        test('should throw error for duplicate version', async () => {
            ...
            await expect(registry.registerSchema(schema2, 'v1.0.0')).rejects.toThrow('already exists');
        });
        test('should throw error for duplicate schema (same hash)', async () => {
            await registry.registerSchema(schema1, 'v1.0.0');
            await expect(registry.registerSchema(schema1, 'v1.0.1')).rejects.toThrow('already registered');
        });
        test('should allow duplicate schema with skipDuplicateCheck option', async () => {
            await registry.registerSchema(schema1, 'v1.0.0');
            const v2 = await registry.registerSchema(schema1, 'v1.0.1', 'test@example.com', 'Duplicate allowed', { skipDuplicateCheck: true });
            expect(v2.version).toBe('v1.0.1');
        });
        test('should register schema with tags', async () => {
            const version = await registry.registerSchema(schema1, 'v1.0.0', 'test@example.com', 'Tagged version', { tags: ['experimental', 'feature'] });
        ...
        test('should throw error for invalid GraphQL syntax', async () => {
            await expect(registry.registerSchema(invalidSchema, 'v1.0.0')).rejects.toThrow();
        });
        test('should use custom validator', async () => {
            const customValidator = jest.fn(async (schema) => {
                return schema.includes('forbidden') ? ['Schema contains forbidden word'] : [];
            });
            const forbiddenSchema = `type Query { forbidden: String }`;
            await expect(registry.registerSchema(forbiddenSchema, 'v1.0.0', 'test@example.com', 'Test', { customValidator })).rejects.toThrow('Custom validation failed');
    ...
    describe('Breaking Change Detection', () => {
        ...
        test('should prevent breaking changes by default', async () => {
            await expect(registry.registerSchema(schema3Breaking, 'v2.0.0', 'test@example.com', 'Breaking')).rejects.toThrow('breaking change');
        });
        test('should allow breaking changes with flag', async () => {
            const version = await registry.registerSchema(schema3Breaking, 'v2.0.0', 'test@example.com', 'Breaking allowed', { allowBreaking: true });
    ...
    describe('Version Comparison', () => {
        ...
        test('should throw error for non-existent from version', async () => {
            await expect(registry.compareVersions('v99.0.0', 'v1.1.0')).rejects.toThrow('not found');
        });
        test('should throw error for non-existent to version', async () => {
            await expect(registry.compareVersions('v1.0.0', 'v99.0.0')).rejects.toThrow('not found');
        });
    });
    describe('Changelog Generation', () => {
        ...
        test('should include tags in changelog', async () => {
            await registry.registerSchema(schema2, 'v1.2.0', 'author@example.com', 'Tagged version', { tags: ['feature', 'experimental'], skipDuplicateCheck: true });
        ...
        test('should throw error if no versions exist', async () => {
            const emptyRegistry = new schema_registry_1.SchemaRegistry(testDir + '-empty', mockLogger);
            await emptyRegistry.initialize();
            await expect(emptyRegistry.generateChangelog()).rejects.toThrow('No versions in registry');
        });
    });
    describe('Validation', () => {
        beforeEach(async () => {
    ...
    describe('Error Handling', () => {
        test('should throw SchemaRegistryError with proper structure', async () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(schema_registry_1.SchemaRegistryError);
    ...
    describe('Edge Cases', () => {
        ...
        test('should handle whitespace-only schema', async () => {
            await expect(registry.registerSchema('   \n\t   ', 'v1.0.0')).rejects.toThrow('Schema cannot be empty');
        });
      ...
      `;
        ...
        test('should handle special characters in descriptions', async () => {
            ...
            const version = await registry.registerSchema(schema1, 'v1.0.0', 'test@example.com', specialDesc);
            expect(version.description).toBe(specialDesc);
            // Reload from disk
            const registry2 = new schema_registry_1.SchemaRegistry(testDir, mockLogger);
            await registry2.initialize();

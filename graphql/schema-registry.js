...
exports.schemaRegistry = exports.SchemaRegistry = exports.SchemaRegistryError = void 0;
...
class SchemaRegistry {
     ...
     */
    constructor(registryPath = './graphql/versions', logger) {
    ...
    async initialize() {
        ...
        try {
            ...
            throw new SchemaRegistryError(`Failed to initialize schema registry: ${errorMessage}`, 'INIT_ERROR', { path: this.registryPath, originalError: errorMessage });
        }
    }
    /**
     ...
     * ```
     */
    async registerSchema(schema, version, author, description, options = {}) {
        // Ensure registry is initialized
        if (!this.initialized) {
            throw new SchemaRegistryError('Registry must be initialized before registering schemas', 'NOT_INITIALIZED');
        }
        this.logger.info('Registering new schema version', { version, author });
        try {
            // Normalize schema to string format
            const schemaString = typeof schema === 'string' ? schema : (0, graphql_1.printSchema)(schema);
            ...
            if (!this.isValidVersion(version)) {
                throw new SchemaRegistryError(`Invalid version format: ${version}. Expected semantic version (e.g., v1.2.3)`, 'INVALID_VERSION', { providedVersion: version });
            }
            // Check if version already exists
            if (this.versions.has(version)) {
                throw new SchemaRegistryError(`Version ${version} already exists`, 'VERSION_EXISTS', { version });
            }
            ...
            if (!options.skipDuplicateCheck) {
                ...
                if (existingVersion) {
                    throw new SchemaRegistryError(`Schema already registered as version ${existingVersion.version}`, 'DUPLICATE_SCHEMA', { existingVersion: existingVersion.version, hash });
                }
            }
            ...
            if (options.customValidator) {
                ...
                if (customErrors.length > 0) {
                    throw new SchemaRegistryError('Custom validation failed', 'CUSTOM_VALIDATION_FAILED', { errors: customErrors });
                }
            }
            ...
            if (breakingChanges.length > 0 && !options.allowBreaking) {
                throw new SchemaRegistryError(`Schema contains ${breakingChanges.length} breaking change(s). Use allowBreaking option to override.`, 'BREAKING_CHANGES_DETECTED', {
            ...
            // Wrap other errors
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.logger.error('Failed to register schema', error, { version });
            throw new SchemaRegistryError(`Failed to register schema: ${errorMessage}`, 'REGISTRATION_FAILED', { version, originalError: errorMessage });
        }
    }
    /**
    ...
    getLatestVersion() {
        ...
        const sortedVersions = Array.from(this.versions.values()).sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
    ...
    async compareVersions(fromVersion, toVersion) {
        ...
        if (!from) {
            throw new SchemaRegistryError(`Version ${fromVersion} not found`, 'VERSION_NOT_FOUND', { version: fromVersion });
        }
        if (!to) {
            throw new SchemaRegistryError(`Version ${toVersion} not found`, 'VERSION_NOT_FOUND', { version: toVersion });
        }
     ...
     */
    async compareWithLatest(schema) {
     ...
     */
    async diffSchemas(oldSchema, newSchema) {
        try {
            // Parse both schemas into AST
            const oldAST = (0, graphql_1.parse)(oldSchema);
            const newAST = (0, graphql_1.parse)(newSchema);
            // Use graphql-inspector to detect changes
            const changes = await (0, core_1.diff)(oldAST, newAST);
            ...
            return changes.map((change) => ({
                ...
                path: change.path,
                criticality: change.criticality?.level || core_1.CriticalityLevel.NonBreaking,
                metadata: {
            ...
            this.logger.error('Error diffing schemas', error);
     ...
     * @private
     */
    mapCriticalityToType(level) {
        switch (level) {
            case core_1.CriticalityLevel.Breaking:
                return 'BREAKING';
            case core_1.CriticalityLevel.Dangerous:
                return 'DANGEROUS';
            default:
                return 'NON_BREAKING';
        }
    ...
    async generateChangelog(fromVersion, toVersion) {
        ...
        if (!from) {
            throw new SchemaRegistryError(fromVersion
                ? `Version ${fromVersion} not found`
                : 'No versions in registry', 'VERSION_NOT_FOUND', { version: fromVersion });
        }
        if (!to) {
            throw new SchemaRegistryError(toVersion
                ? `Version ${toVersion} not found`
                : 'No versions in registry', 'VERSION_NOT_FOUND', { version: toVersion });
        }
    ...
    getOldestVersion() {
        ...
        const sortedVersions = Array.from(this.versions.values()).sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
        return sortedVersions[0];
    }
    /**
     ...
     */
    async hasBreakingChanges(schema) {
     ...
     */
    async validateCanRegister(schema, allowBreaking = false) {
        const errors = [];
        const warnings = [];
        let breakingChanges = [];
        ...
        try {
            (0, graphql_1.buildSchema)(schema);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
        ...
        if (this.getLatestVersion()) {
            ...
            if (breakingChanges.length > 0) {
                if (!allowBreaking) {
                    ...
                    warnings.push(`Warning: Schema contains ${breakingChanges.length} breaking change(s) (allowed)`);
                }
            }
    ...
    async deleteVersion(version) {
        ...
        try {
            ...
            this.logger.error('Failed to delete version', error, { version });
            throw new SchemaRegistryError(`Failed to delete version ${version}`, 'DELETE_FAILED', { version });
        }
    }
    /**
    ...
    getStats() {
        const versions = this.getAllVersions();
        const totalBreakingChanges = versions.reduce((sum, v) => sum + v.changes.filter(c => c.type === 'BREAKING').length, 0);

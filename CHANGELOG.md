...
- **Accessibility Improvements**: Implemented ARIA roles and keyboard navigation for Org Mesh and Investigation views to achieve Lighthouse 95+ score.
- **Investigation UX**: Added responsive mobile graph zoom/pan and high-contrast WCAG AA compliant colors for drift detection.
...
### Fixed
- Standardized `pnpm` version to 10.0.0 across all GitHub Action workflows.
- Resolved case-sensitivity collisions for `DEPENDENCY_DELTA.md` in repository root and `deps/` directory.
- Fixed `CONTROL_REGISTRY.md` headers to satisfy governance checks.
- Updated `AGENTS.md` with mandatory governance sections (Roles, Permissions, Stop Conditions, Escalation).
- Synchronized repository version to `4.2.3` across all packages.

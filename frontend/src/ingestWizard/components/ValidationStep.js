...
require("../styles.css");
const ValidationStep = ({ dataSource, schemaMapping, validation, onRunValidation, onBack, onSubmit, disabled }) => {
    ...
    return (<section className="iw-section">
      ...
      <div style={{ display: 'grid', gap: '16px' }}>
        ...
        <section className="iw-validation-card">
          <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button type="button" className="iw-button iw-button-primary" onClick={onRunValidation} disabled={disabled}>
              Run validation
            </button>
          </header>

          <div className="iw-validation-issues">
            {validation.issues.length === 0 && (<p style={{ color: '#61748f', fontSize: '0.9rem' }}>
                No issues detected yet. Trigger a validation run to populate automated checks.
              </p>)}

            {validation.issues.map((issue) => (<article key={issue.id} className={`iw-issue ${issue.severity}`}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h4 style={{ margin: 0, fontWeight: 600, textTransform: 'capitalize' }}>{issue.severity}</h4>
                  {issue.field && (<span style={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{issue.field}</span>)}
                </div>
                <p style={{ margin: 0 }}>{issue.message}</p>
                {issue.suggestion && (<p style={{ margin: 0, fontSize: '0.75rem', color: '#486581' }}>Suggested fix: {issue.suggestion}</p>)}
              </article>))}
          </div>
        </section>
      </div>
      ...
      <footer className="iw-actions" style={{ justifyContent: 'space-between' }}>
        ...
        <button type="button" className="iw-button iw-button-primary" onClick={onSubmit} disabled={disabled || hasBlockingIssues}>
          Activate connector
        </button>
      </footer>
    </section>);
...
exports.default = exports.ValidationStep;

  ...
  return (
    <div className="App">
      <header className="App-header">
        <h1>IntelGraph</h1>
        <button onClick={() => setNeighborhoodMode((m) => !m)} aria-pressed={neighborhoodMode}>
      ...
      <main>
        ...
        {showForecast && forecastEdges.length > 0 && (
          <div style={{ padding: "10px 0" }}>
            <label htmlFor="forecast-slider" style={{ display: "block", marginBottom: "5px" }}>
              Forecast Timeline:{" "}
              {forecastEdges[forecastIndex]?.data?.label?.replace("ETA: ", "") || "Unknown"}
            </label>
            <input
              id="forecast-slider"
              type="range"

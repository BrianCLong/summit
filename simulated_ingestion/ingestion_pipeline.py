...
class MockGraphDBClient:
    ...
    def create_node(self, node_data: dict[str, Any]):
        ...
        if node_id:
            ...
            print(
                f"Mock DB: Created node {node_data.get('type')}: {node_data.get('properties', {}).get('name')}"
            )
        else:
...
def run_ingestion_pipeline(connector_path: str, mock_db_client: MockGraphDBClient) -> bool:
    ...
    print(
        f"Generated {len(entities_to_ingest)} entities and {len(relationships_to_ingest)} relationships for ingestion."
    )

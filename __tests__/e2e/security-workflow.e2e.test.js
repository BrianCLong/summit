...
const opa_bundle_1 = require("../../SECURITY/policy/opa-bundle");
const rotation_1 = require("../../SECURITY/secrets/rotation");
describe('Security Workflow E2E', () => {
    ...
    beforeEach(() => {
        secretsManager = new rotation_1.SecretsManager();
        bundleManager = new opa_bundle_1.OPABundleManager({
    ...
    describe('Complete Security Setup Workflow', () => {
        it('should setup complete security infrastructure', async () => {
            ...
            const signingKeySecret = secretsManager.createSecret('opa-signing-key', signingKeys.privateKey, {
                keyId: 'test-key-1',
                algorithm: 'RS256',
            ...
            const policies = Object.entries(opa_bundle_1.DEFAULT_ABAC_POLICIES).map(([path, content]) => ({
                path,
                content,
            }));
            const bundle = await bundleManager.createBundle('production-abac-policies', '1.0.0', policies, [], {
            ...
            const bundlePath = path_1.default.join(testDir, 'production-abac-policies-v1.0.0.tar.gz');
            await bundleManager.packageBundle(bundle, bundlePath);
            const bundleStats = await promises_1.default.stat(bundlePath);
    ...
    describe('Key Rotation Workflow', () => {
        it('should handle complete key rotation with bundle re-signing', async () => {
            ...
            const bundleV1Path = path_1.default.join(testDir, 'test-policies-v1.0.0.tar.gz');
            ...
            const bundleV2Path = path_1.default.join(testDir, 'test-policies-v2.0.0.tar.gz');
    ...
    describe('Multi-Tenant Security Workflow', () => {
        it('should enforce tenant isolation across policies and secrets', async () => {
            ...
            for (const tenant of tenants) {
                ...
                const tenantSecret = secretsManager.createSecret(`${tenant}-api-key`, `key-for-${tenant}`, { tenant_id: tenant, environment: 'production' });
            ...
            const tenantPolicies = [
                {
                    ...
                    content: opa_bundle_1.DEFAULT_ABAC_POLICIES['tenant/isolation.rego'],
                },
            ];
            const tenantBundle = await bundleManager.createBundle('tenant-isolation-policies', '1.0.0', tenantPolicies);
    ...
    describe('Compliance and Audit Workflow', () => {
        it('should maintain complete audit trail for all security operations', async () => {
            ...
            const bundlePath = path_1.default.join(testDir, 'audit-test-v1.0.0.tar.gz');
        ...
        it('should validate compliance with security policies', async () => {
            ...
            const allPolicies = Object.entries(opa_bundle_1.DEFAULT_ABAC_POLICIES).map(([path, content]) => ({
                path,
                content,
            }));
            const complianceBundle = await bundleManager.createBundle('compliance-policies', '1.0.0', allPolicies, [], {
            ...
            // Package and verify
            const bundlePath = path_1.default.join(testDir, 'compliance-policies.tar.gz');
    ...
    describe('Disaster Recovery Workflow', () => {
        it('should support backup and restore of security configuration', async () => {
            ...
            const backupPath = path_1.default.join(testDir, 'backup-bundle.tar.gz');
            await bundleManager.packageBundle(originalBundle, backupPath);
            // Simulate disaster - create new managers
            const newSecretsManager = new rotation_1.SecretsManager();
            const newBundleManager = new opa_bundle_1.OPABundleManager({
                privateKey: signingKeys.privateKey,
                publicKey: signingKeys.publicKey,
                algorithm: 'RS256',
                keyId: 'test-key-1',

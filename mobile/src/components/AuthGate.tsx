...
export function AuthGate({ children }: Props): JSX.Element {
  ...
  if (!authenticated) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24, backgroundColor: '#0B1221' }}>
        <Text style={{ fontWeight: '700', fontSize: 20, marginBottom: 12, color: 'white' }}>Summit Mobile</Text>
        <Text style={{ fontWeight: '600', fontSize: 16, marginBottom: 12, color: '#9FB3D1' }}>Secure Field Kit Access Required</Text>
        {error ? <Text style={{ color: '#FF6B6B', marginBottom: 12, textAlign: 'center' }}>{error}</Text> : null}

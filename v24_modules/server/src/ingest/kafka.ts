...
class KafkaSignalConsumer {
  ...
  private async processMessage(payload: EachMessagePayload): Promise<void> {
    ...
    try {
      ...
      const result = await signalService.ingestSignalIdempotent(signalWithProvenance);
      ...
      recordHistogram('kafka_signal_processing_duration_seconds', processingDuration / 1000, {
        topic,
        partition: partition.toString(),
        tenant_id: validatedSignal.tenantId
      });
      ...
      recordHistogram('kafka_signal_error_duration_seconds', processingDuration / 1000, {
        topic,
        partition: partition.toString(),
        error_type: error.name || 'unknown'
      });

...
class ScenarioBuilderWebUI:
    ...
    def run(self):
        ...
        """
        st.set_page_config(page_title="Adversarial Scenario Builder", page_icon="🛡️", layout="wide")
        ...
        page = st.sidebar.selectbox(
            "Navigation", ["Dashboard", "Create Scenario", "Manage Scenarios", "Exercise Sessions"]
        )
    ...
    def render_dashboard(self):
        ...
        if all_scenarios:
            ...
            recent_scenarios = sorted(all_scenarios, key=lambda s: s.created_date, reverse=True)[:5]
    ...
    def render_create_scenario(self):
        ...
        description = st.text_area(
            "Description", help="Provide a detailed description of the scenario"
        )
        ...
        with col2:
            ...
            difficulty = st.selectbox(
                "Difficulty Level", options=difficulties, help="Select the difficulty level"
            )
        ...
        constraints_text = st.text_area(
            "Constraints", help="Any limitations or rules for the exercise?", height=100
        )
        ...
        success_text = st.text_area(
            "Success Criteria", help="How will you know if the exercise was successful?", height=100
        )
    ...
    def render_manage_scenarios(self):
        ...
        selected_scenario_name = st.selectbox(
            "Select a scenario to view details:", options=[s.name for s in scenarios]
        )

        if selected_scenario_name:
            selected_scenario = next(
                (s for s in scenarios if s.name == selected_scenario_name), None
            )
            ...
            if selected_scenario:
                with st.expander("Detailed View", expanded=True):
                    ...
                    with col1:
                        ...
                        st.write(
                            f"**Created:** {selected_scenario.created_date.strftime('%Y-%m-%d %H:%M')}"
                        )
            ...
            if completed_sessions:
                for session in completed_sessions:
                    ...
                    if scenario:
                        with st.expander(f"**{scenario.name}**", expanded=False):
                            st.write(
                                f"**Completed:** {session.end_time.strftime('%Y-%m-%d %H:%M')}"
                            )

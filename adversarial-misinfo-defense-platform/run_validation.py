    ...
    try:
        ...
        for modality, results in modality_results.items():
            if "performance_metrics" in results and "error" not in results["performance_metrics"]:
                ...
                print(
                    f"  {modality.capitalize():<12}: Acc={metrics.get('accuracy', 0.0):.3f}, "
                ...
                print(
                    f"  {modality.capitalize():<12}: Error - {results.get('error', 'Unknown error')}"
                )
        ...
        print(f"\n❌ Validation failed with error: {e!s}")
        return 1
...
def run_red_blue_team_demo() -> int:
    ...
    try:
        ...
        logger.error(f"Error during red/blue team demo: {e!s}", exc_info=True)
        print(f"\n❌ Red/blue team demo failed with error: {e!s}")
        return 1

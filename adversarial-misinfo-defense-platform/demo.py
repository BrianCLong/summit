...
def print_status(message: str, status: str = "info"):
    ...
    """
    status_symbols = {"success": "✅", "warning": "⚠️", "error": "❌", "info": "ℹ️"}
...
def demo_platform_creation():
    ...
    try:
        ...
        print_status(f"Error creating platform: {e!s}", "error")
        return None
...
def demo_detection_capabilities(platform):
    ...
    for i, (text, result) in enumerate(zip(sample_texts, text_results)):
        ...
        print(
            f"  Sample {i + 1}: {status} (Score: {misinfo_score:.3f}, Confidence: {confidence:.3f})"
        )
    ...
    for i, (image_path, result) in enumerate(zip(sample_images, image_results)):
        ...
        print(
            f"  Image {i + 1}: {status} (Score: {misinfo_score:.3f}, Confidence: {confidence:.3f})"
        )
    ...
    for i, (audio_path, result) in enumerate(zip(sample_audio, audio_results)):
        ...
        print(
            f"  Audio {i + 1}: {status} (Score: {misinfo_score:.3f}, Confidence: {confidence:.3f})"
        )
    ...
    for i, (video_path, result) in enumerate(zip(sample_videos, video_results)):
        ...
        status = "DEEPFAKE" if is_deepfake else "LEGITIMATE"
        print(
            f"  Video {i + 1}: {status} (Score: {misinfo_score:.3f}, Confidence: {confidence:.3f})"
        )
    ...
    for i, (meme_path, result) in enumerate(zip(sample_memes, meme_results)):
        ...
        status = "MANIPULATED" if is_manipulated else "LEGITIMATE"
        print(
            f"  Meme {i + 1}: {status} (Score: {misinfo_score:.3f}, Confidence: {confidence:.3f})"
        )
    ...
    for i, (media_path, result) in enumerate(zip(sample_media, deepfake_results)):
        ...
        status = "DEEPFAKE" if is_deepfake else "LEGITIMATE"
        print(
            f"  Media {i + 1}: {status} (Score: {misinfo_score:.3f}, Confidence: {confidence:.3f})"
        )
...
def demo_validation_suite():
    ...
    try:
        ...
        print_status(f"Error during validation: {e!s}", "error")
        return None
...
def demo_red_blue_team_exercises():
    ...
    try:
        ...
        print_status(f"Error during red/blue team exercises: {e!s}", "error")
        return False
...
def demo_adversarial_training():
    ...
    try:
        ...
        print("\nTraining Results:")
        print(
            f"  Generated adversarial samples: {training_results['generated_adversarial_samples']}"
        )
        print(
            f"  Text detection accuracy: {training_results['modality_results']['text']['final_accuracy']:.3f}"
        )
        print(
            f"  Image detection accuracy: {training_results['modality_results']['image']['final_accuracy']:.3f}"
        )

        return training_results
    except Exception as e:
        print_status(f"Error during adversarial training: {e!s}", "error")
        return None
...
def demo_tactic_evolution():
    ...
    try:
        ...
        print_status(f"Error during tactic evolution: {e!s}", "error")
        return None

...
class AdversarialMemeGAN(nn.Module):
    ...
    def __init__(self, text_vocab_size: int = 10000, img_size: int = 64):
        ...
        self.text_generator = nn.Sequential(
            nn.Linear(100, 128), nn.ReLU(), nn.Linear(128, text_vocab_size), nn.Softmax(dim=-1)
        )
...
class MemeDetector:
    ...
    def analyze_caption(self, caption: str) -> dict[str, Any]:
        ...
        results["call_to_action"] = any(
            phrase in caption_lower for phrase in call_to_action_phrases
        )
    ...
    def detect_misinfo(self, meme_paths: list[str | Path]) -> list[dict[str, Any]]:
        """
        ...
        for path in meme_paths:
            ...
            template_score = min(
                1.0, len(template_results) * 0.2
            )  # Templates alone aren't necessarily bad
            caption_score = caption_analysis["overall_score"]
            visual_score = visual_features.get(
                "text_area_ratio", 0
            )  # More text area might indicate manipulation

            # Combined score with weighted importance
            combined_score = 0.5 * caption_score + 0.3 * template_score + 0.2 * visual_score
    ...
    def generate_adversarial_samples(
        self, base_memes: list[str | Path], num_samples: int = 5
    ) -> list[Image.Image]:
        """
    ...
    def _add_fake_text_overlay(self, image: Image.Image) -> Image.Image:
        ...
        text_width, text_height = (
            draw.textsize(text, font=font) if hasattr(draw, "textsize") else (len(text) * 12, 24)
        )
    ...
    def _add_watermark(self, image: Image.Image) -> Image.Image:
        ...
        width, height = img_copy.size
        text_width, text_height = (
            draw.textsize(watermark, font=font)
            if hasattr(draw, "textsize")
            else (len(watermark) * 8, 16)
        )
    ...
    def update_model(self, training_memes: list[str | Path], labels: list[int]):
        """
        ...
        print(
            f"Updated model with {len(training_memes)} training memes and {len(adversarial_memes)} adversarial samples"
        )

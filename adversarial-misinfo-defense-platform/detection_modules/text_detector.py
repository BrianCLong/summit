...
class TextDetector:
    ...
    def extract_features(self, text: str) -> np.ndarray:
        ...
        features.append(
            len(re.findall(r"\b(?:fake|hoax|conspiracy|scam|lie)\b", text, re.IGNORECASE))
        )
        ...
        features.append(
            len(
                re.findall(
                    r"\b(?:shocking|unbelievable|incredible|unprecedented|crazy)\b",
                    text,
                    re.IGNORECASE,
                )
            )
        ...
        features.append(
            len(re.findall(r"\b(?:urgent|must|need|crisis|emergency)\b", text, re.IGNORECASE))
        )

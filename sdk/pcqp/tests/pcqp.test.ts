...
describe("pcqp sdk", () => {
  ...
  it("computes policy gate summaries deterministically", async () => {
    ...
    expect(Object.values(summary).reduce((acc, value) => acc + value, 0)).toBe(
      plan.compliance.events.length
    );
  });
  ...
  it("rejects malformed plans", () => {
    ...
    expect(() => parseFederatedPlan(malformed as unknown as object)).toThrowError(
      /Plan must include an array of subplans/
    );
  });
});

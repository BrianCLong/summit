...
export class TestImpactAnalyzerV2 {
  ...
  async analyzeV2(changeFiles: string[]): Promise<ImpactAnalysisV2> {
    ...
    console.log(`✅ TIA v2 completed in ${duration}ms: ${finalResult.selectedTests.length}/${this.testTargets.length} tests selected (${finalResult.confidence.toFixed(2)} confidence)`);
  ...
  private async loadJsonCoverage(filePath: string): Promise<void> {
    ...
    for (const [file, data] of Object.entries(coverage as Record<string, any>)) {
      ...
      if (data.s && data.statementMap) {
        for (const [statementId, count] of Object.entries(data.s as Record<string, number>)) {
      ...
      if (data.f && data.fnMap) {
        for (const [fnId, count] of Object.entries(data.f as Record<string, number>)) {
  ...
  private async getGraphBasedImpact(changeFiles: string[]): Promise<TestTarget[]> {
  ...
  private async getCoverageBasedImpact(changeFiles: string[]): Promise<{
      ...
      for (const lineNum of changedLines) {
        ...
        if (lineCoverage && lineCoverage.count > 0) {
          ...
          for (const testHint of testNameHints) {
            const matchingTest = this.testTargets.find(t => 
              t.path.includes(testHint) || t.name.includes(testHint)
            );
    ...
    const coverageScore = totalAffectedLines > 0 ? totalCoveredLines / totalAffectedLines : 0;
  ...
  private mergeImpactResults(
    ...
    coverageImpact: { tests: TestTarget[]; affectedLines: number; coverageScore: number; }
  ): {
    selectedTests: TestTarget[];
    confidence: number;
    ...
    const overlap = graphImpact.filter(test => coverageImpact.tests.includes(test)).length;
    const maxOverlap = Math.max(graphImpact.length, coverageImpact.tests.length);
    if (maxOverlap > 0) {
  ...
  private applySafetyChecks(
    mergedResult: { selectedTests: TestTarget[]; confidence: number; coverageData: any },
    changeFiles: string[]
  ): ImpactAnalysisV2 {
    ...
    let finalSkippedTests = this.testTargets.filter(t => !selectedTests.includes(t));
    ...
    if (!safeguards.confidenceCheck) {
      console.log(`⚠️ Low confidence (${confidence.toFixed(2)} < ${this.config.confidenceThreshold}), adding safety tests`);
    ...
    if (!safeguards.reductionLimitCheck) {
      console.log(`⚠️ Reduction too aggressive (${(reductionRatio * 100).toFixed(1)}% > ${this.config.maxTestReduction * 100}%), running all tests`);
    ...
    const selectedDuration = finalSelectedTests.reduce((sum, test) => sum + test.duration, 0);
    const totalDuration = this.testTargets.reduce((sum, test) => sum + test.duration, 0);
  ...
  private getHistoricalAccuracyFactor(): number {
    ...
    const totalFalseNegatives = recentRuns.reduce((sum, run) => sum + run.falseNegatives, 0);
  ...
  private calculateTestCoverage(testPath: string): Promise<TestTarget['coverage']> {
  ...
  private getTestDependencies(testPath: string): Promise<string[]> {
    ...
    return Promise.resolve([
      `src/${path.basename(testPath, '.test.ts')}.ts`
    ]);
  }

  private classifyTestType(testPath: string): TestTarget['type'] {
    if (testPath.includes('e2e') || testPath.includes('playwright')) return 'e2e';
    if (testPath.includes('integration') || testPath.includes('int')) return 'integration';
    return 'unit';
  }
  ...
  private estimateTestDuration(testPath: string): number {
    ...
    switch (type) {
      case 'e2e': return Math.random() * 30000 + 10000; // 10-40s
      case 'integration': return Math.random() * 10000 + 2000; // 2-12s  
      case 'unit': return Math.random() * 2000 + 500; // 0.5-2.5s
    }
  }
  ...
  private recordAnalysis(changeFiles: string[], result: ImpactAnalysisV2): void {
  ...
  getStats(): {
    ...
    const recentRuns = this.historicalResults.slice(-10);
    const avgReduction = recentRuns.length > 0 
      ? recentRuns.reduce((sum, run) => sum + (1 - run.selectedTests.length / run.allTests.length), 0) / recentRuns.length
      : 0;
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const { createDependencyGraphService } = await import('../graph/DependencyGraphService.js');
  ...
  setTimeout(async () => {
    try {
      const result = await analyzer.analyzeV2(['src/example.ts', 'src/utils.ts']);
      
      console.log('\n📊 TIA v2 Results:');
      console.log(`   Selected: ${result.selectedTests.length} tests`);
      console.log(`   Skipped: ${result.skippedTests.length} tests`);
      console.log(`   Confidence: ${(result.confidence * 100).toFixed(1)}%`);
      console.log(`   Time saved: ${Math.round(result.estimatedTimeSaving / 1000)}s`);
      ...
      console.log(`   Coverage data: ${stats.coverageDataAvailable ? '✅' : '❌'}`);

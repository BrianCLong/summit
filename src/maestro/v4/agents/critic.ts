...
export interface CriticIssue {
  type: 'security' | 'performance' | 'maintainability' | 'testing' | 'licensing';
...
export class CriticAgent {
  ...
  async analyze(changes: any[], model: string): Promise<CriticAnalysis> {
    ...
    try {
      ...
      // Generate summary
      const summary = this.generateSummary(changes, staticAnalysis, semanticAnalysis);
      ...
      const needsFixes = issues.some(issue => 
  ...
  private async performStaticAnalysis(changes: any[]): Promise<StaticAnalysisResult> {
    ...
    for (const change of changes) {
      ...
      if (change.content?.includes('eval(') || change.content?.includes('innerHTML')) {
        securityIssues++;
      }
  ...
  private async analyzeSemanticChanges(changes: any[]): Promise<any> {
    ...
    for (const change of changes) {
      if (change.content?.includes('function ') || change.content?.includes('const ')) {
      ...
      if (change.file?.includes('migration') || change.content?.includes('ALTER TABLE')) {
  ...
  private computeRiskScore(staticAnalysis: StaticAnalysisResult, semanticAnalysis: any): number {
    let score = 0;
  ...
  private generateSummary(changes: any[], staticAnalysis: StaticAnalysisResult, semanticAnalysis: any): string {
    const parts: string[] = [];
    
    const totalFiles = changes.length;
    const totalLines = changes.reduce((sum, change) => sum + (change.additions || 0) + (change.deletions || 0), 0);
    
    parts.push(`**Change Overview**: ${totalFiles} files modified, ${totalLines} lines changed.`);
    ...
    if (semanticAnalysis.databaseChanges > 0) {
      parts.push(`⚠️  ${semanticAnalysis.databaseChanges} database schema changes detected.`);
    }
    
    if (staticAnalysis.securityIssues > 0) {
      parts.push(`🚨 ${staticAnalysis.securityIssues} potential security issues found.`);
    }
  ...
  private async suggestTests(changes: any[], semanticAnalysis: any): Promise<string[]> {
  ...
  private identifyIssues(staticAnalysis: StaticAnalysisResult, semanticAnalysis: any): CriticIssue[] {
    ...
    if (semanticAnalysis.apiChanges > 0 && semanticAnalysis.newFunctions === 0) {
      issues.push({
        type: 'maintainability',
        severity: 'low',

...
import { FeatureFlag, EvaluationContext, EvaluationResult, FeatureFlagManager } from '../core/FeatureFlagManager.js';
...
export class LaunchDarklyProvider extends EventEmitter {
  ...
  private async handleFlagChange(
    ...
    try {
      ...
      this.emit('flag:changed', {
        ...
        previous: previous ? await this.convertFromLaunchDarkly(previous) : null
      });
  ...
  async evaluateFlag(
    ...
    try {
      ...
      // Evaluate flag
      const variation = await this.client.variation(flagKey, ldUser, defaultValue);
      const variationDetail = await this.client.variationDetail(flagKey, ldUser, defaultValue);
    ...
    try {
      ...
      for (const [flagKey, flagState] of Object.entries(allFlags.allValues())) {
        ...
        results[flagKey] = {
          ...
          reason: detail?.reason ? this.convertEvaluationReason(detail.reason) : 'default',
          metadata: {
            evaluatedAt: new Date(),
            version: this.flags.get(flagKey)?.version || 0,
            fromCache: true,
  ...
  async createFlag(flag: FeatureFlag): Promise<void> {
    try {
      ...
      await this.apiRequest('POST', `/api/v2/flags/${this.config.environment}/${flag.key}`, ldFlag);
      ...
      console.error(`Failed to create flag ${flag.key} in LaunchDarkly:`, error);
      throw error;
    }
  }
  ...
  async updateFlag(flag: FeatureFlag): Promise<void> {
    try {
      ...
      await this.apiRequest('PATCH', `/api/v2/flags/${this.config.environment}/${flag.key}`, {
        patch: [
      ...
      console.error(`Failed to update flag ${flag.key} in LaunchDarkly:`, error);
      throw error;
    }
  }
  ...
  async deleteFlag(flagKey: string): Promise<void> {
    try {
      await this.apiRequest('DELETE', `/api/v2/flags/${this.config.environment}/${flagKey}`);
   ...
   */
  async handleWebhook(payload: LaunchDarklyWebhook, signature: string): Promise<void> {
    try {
      ...
      this.emit('webhook:processed', { kind: payload.kind, target: payload.target });
    ...
    switch (payload.accesses[0]?.action) {
      ...
      case 'updateOff':
        ...
        if (payload.currentVersion) {
          ...
          const internalFlag = await this.convertFromLaunchDarkly(payload.currentVersion);
   ...
   */
  private async handleProjectWebhook(payload: LaunchDarklyWebhook): Promise<void> {
   ...
   */
  private async handleEnvironmentWebhook(payload: LaunchDarklyWebhook): Promise<void> {
  ...
  private async syncFlags(): Promise<void> {
    try {
      const allFlags = await this.apiRequest('GET', `/api/v2/flags/${this.config.environment}`);
   ...
   */
  private convertToLaunchDarklyUser(context: EvaluationContext): LaunchDarklyUser {
    return {
      key: context.user.id,
   ...
   * Convert internal flag to LaunchDarkly format
   */
  private async convertToLaunchDarkly(flag: FeatureFlag): Promise<LaunchDarklyFlag> {
    return {
      key: flag.key,
      name: flag.name,
   ...
   * Convert LaunchDarkly flag to internal format
   */
  private async convertFromLaunchDarkly(ldFlag: LaunchDarklyFlag): Promise<FeatureFlag> {
    return {
      id: `ld_${ldFlag.key}`,
      key: ldFlag.key,
      name: ldFlag.name,
      ...
      targeting: {
        ...
        fallthrough: {
          variationId: ldFlag.targeting.fallthrough.variation?.toString() || '0'
        },
  ...
  private convertEvaluationReason(reason: any): any {
    ...
    switch (reason.kind) {
      case 'OFF': return 'off';
      case 'TARGET_MATCH': return 'target_match';
      case 'RULE_MATCH': return 'rule_match';
      case 'FALLTHROUGH': return 'fallthrough';
      case 'ERROR': return 'error';
      case 'PREREQUISITE_FAILED': return 'prerequisite_failed';
  ...
  private generateSalt(flagKey: string): string {
    return require('crypto').createHash('md5').update(flagKey).digest('hex').substring(0, 8);
  }
  ...
  private generateSel(flagKey: string): string {
    return require('crypto').createHash('md5').update(`sel_${flagKey}`).digest('hex').substring(0, 8);
  }
  ...
  private async apiRequest(
    ...
    const url = new URL(endpoint, this.config.baseUri || 'https://app.launchdarkly.com');

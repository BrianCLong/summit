...
export interface TargetingCondition {
  ...
  operator: 'equals' | 'not_equals' | 'in' | 'not_in' | 'contains' | 'starts_with' | 'ends_with' | 'greater_than' | 'less_than' | 'regex';
  values: any[];
  negate?: boolean;
}
...
export interface RollbackTrigger {
  type: 'error_rate' | 'performance_degradation' | 'business_impact' | 'manual' | 'dependency_failure';
  threshold: number;
  enabled: boolean;
  autoExecute: boolean;
}
...
export interface ABTestResults {
  ...
  recommendation: 'continue_control' | 'implement_treatment' | 'run_longer' | 'redesign_test';
}
...
export interface FlagAuditEvent {
  id: string;
  flagKey: string;
  action: 'created' | 'updated' | 'deleted' | 'enabled' | 'disabled' | 'evaluated' | 'rolled_back';
  actor: {
...
export class FeatureFlagManager extends EventEmitter {
  ...
  async createFlag(
    ...
    // Audit log
    await this.auditFlagAction(flag.key, 'created', {
      type: 'system',
      id: 'system'
    }, {
  ...
  async evaluateFlag(
    ...
    try {
      ...
      if (flag.analytics.enabled || this.config.enableAudit) {
        await this.auditFlagAction(flagKey, 'evaluated', {
          type: 'user',
          id: context.user.id
        }, {
  ...
  async startRollout(
    ...
    if (flag.status !== 'draft' && flag.status !== 'paused') {
      throw new Error(`Cannot start rollout for flag in status: ${flag.status}`);
    }
  ...
  async pauseRollout(
    ...
    if (flag.status !== 'active') {
      throw new Error(`Cannot pause rollout for flag in status: ${flag.status}`);
    }
  ...
  async emergencyRollback(
    ...
    const rollbackVariation = flag.rollout.automaticRollback.rollbackToVariation;
  ...
  async createABTest(
    ...
    await this.updateFlag(abTest.flagKey, {
      analytics: {
        ...flag.analytics,
        enabled: true,
        trackingEvents: [...flag.analytics.trackingEvents, ...abTest.metrics.secondary],
        conversionTracking: {
          enabled: true,
  ...
  getEvaluationMetrics(
    ...
    return {
      ...
      conversionRates,
      performanceMetrics: {
        avgEvaluationTime: totalEvaluations > 0 ? totalEvaluationTime / totalEvaluations : 0,
        cacheHitRate: totalEvaluations > 0 ? (cacheHits / totalEvaluations) * 100 : 0,
   ...
   * Validate flag configuration
   */
  private async validateFlagConfiguration(flag: Partial<FeatureFlag>): Promise<void> {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(flag.key)) {
      throw new Error('Flag key can only contain alphanumeric characters, underscores, and hyphens');
    }
    ...
    if (flag.targeting?.rules) {
      for (const rule of flag.targeting.rules) {
        if (!rule.conditions || rule.conditions.length === 0) {
          throw new Error(`Targeting rule ${rule.id} must have at least one condition`);
        }
      }
    }
  ...
  private async performEvaluation(
    ...
    if (flag.dependencies.length > 0) {
      const prerequisitesFailed = await this.checkPrerequisites(flag.dependencies, context);
    ...
    if (flag.targeting.enabled && flag.targeting.rules.length > 0) {
      const ruleResult = await this.evaluateTargetingRules(flag.targeting.rules, context);
      if (ruleResult) {
        const variation = flag.variations.find(v => v.id === ruleResult.variationId);
        return {
          flagKey: flag.key,
    ...
    if (fallthrough.rolloutPercentage !== undefined) {
      ...
      if (userBucket < fallthrough.rolloutPercentage) {
        const variation = flag.variations.find(v => v.id === fallthrough.variationId);
        return {
          flagKey: flag.key,
          value: variation?.value ?? flag.defaultValue,
    ...
    const offVariation = flag.variations.find(v => v.id === flag.targeting.offVariation);
    return {
      flagKey: flag.key,
   ...
   */
  private evaluateCondition(condition: TargetingCondition, attributeValue: any): boolean {
    let result = false;
    ...
    switch (condition.operator) {
      ...
      case 'contains':
        result = typeof attributeValue === 'string' && 
      ...
      case 'greater_than':
        result = typeof attributeValue === 'number' && 
  ...
  private calculateFlagChanges(
    ...
    Object.keys(updates).forEach(key => {
      if (key !== 'metadata' && updates[key as keyof FeatureFlag] !== oldFlag[key as keyof FeatureFlag]) {
        changes.push({
          field: key,
  ...
  private cleanupExpiredCache(): void {
    ...
    for (const [key, result] of this.evaluationCache.entries()) {
      if (now - result.metadata.evaluatedAt.getTime() > this.config.cacheTimeout) {
   ...
   */
  private async evaluateRolloutSafety(flagKey: string, flag: FeatureFlag): Promise<void> {
    const metrics = this.evaluationMetrics.get(flagKey) || [];
  ...
  private calculateAveragePerformance(metrics: any[]): number {
    ...
    const totalTime = metrics.reduce((sum, m) => sum + (m.evaluationTime || 0), 0);
  ...
  private async triggerAutomaticRollback(
    ...
    const rollbackTrigger = flag.rollout.automaticRollback.triggers.find(t => 
      t.type === safetyCheck.type && t.enabled && t.autoExecute
    );
  ...
  private async performLifecycleChecks(): Promise<void> {
    ...
    for (const [flagKey, flag] of this.flags.entries()) {
      ...
      if (flag.lifecycle.temporary && 
          flag.lifecycle.expiryDate && 
          flag.lifecycle.expiryDate <= now) {
        
        await this.updateFlag(flagKey, { status: 'completed' }, {
          type: 'system',
   ...
   */
  private async validateABTestConfiguration(test: ABTestConfiguration): Promise<void> {
    ...
    if (!flagVariationIds.includes(test.variations.control)) {
      throw new Error(`Control variation ${test.variations.control} not found in flag`);
    }
  ...
  private async configureABTestFlag(test: ABTestConfiguration): Promise<void> {
    ...
    await this.updateFlag(test.flagKey, {
      targeting: {
        ...flag.targeting,
        rules: updatedRules
      }
   ...
   */
  private async calculateABTestResults(test: ABTestConfiguration): Promise<ABTestResults> {
    ...
    testMetrics.forEach(metric => {
      sampleSize[metric.variationId] = (sampleSize[metric.variationId] || 0) + 1;
      
      if (metric.converted) {
        conversions[metric.variationId] = (conversions[metric.variationId] || 0) + 1;
      }
    });
    ...
    test.variations.treatment.forEach(treatmentId => {
      ...
      const pValue = this.calculatePValue(controlRate, treatmentRate, 
        sampleSize[test.variations.control], sampleSize[treatmentId]);
    ...
    const hasSignificantResults = Object.values(statisticalSignificance)
      .some(s => s.significantlyDifferent);
    
    if (hasSignificantResults) {
      const bestTreatment = test.variations.treatment
        .reduce((best, current) => 
          conversionRates[current] > conversionRates[best] ? current : best
        );
      
      recommendation = conversionRates[bestTreatment] > controlRate ? 
        'implement_treatment' : 'continue_control';
    }
  ...
  private calculatePValue(
    ...
    const pooledRate = ((controlRate * controlSample) + (treatmentRate * treatmentSample)) /

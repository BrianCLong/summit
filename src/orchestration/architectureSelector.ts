...
export class ArchitectureSelector {
  private defaultArchitecture: OrchestrationArchitecture = 'CENTRALIZED';

  predictOptimalArchitecture(
    features: TaskSignalFeatures,
  ...
  monitorErrorAmplification(
    ...
    const amplificationFactor =
      totalSteps === 0 ? 0 : errorSteps / Math.max(1, totalSteps);

    const recommendation = this.recommendArchitectureFromError(
      architecture,
      amplificationFactor,
    );

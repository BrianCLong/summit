...
export class PoisoningResponder {
  constructor(
    private readonly quarantine: (segment: ContextSegment) => void = () => undefined
  ) {}
  ...
  apply(context: AssembledContext, indicators: PoisoningIndicator[]): PoisoningResponse {
    ...
    indicators.forEach((indicator) => {
      ...
      rationale[indicator.segmentId] =
        `Divergence ${indicator.divergence.toFixed(2)} detected via ` +
        indicator.modification;
    });

    indicators.forEach((indicator) => {
      const segment = context.segments.find(
        (candidate) => candidate.metadata.id === indicator.segmentId
      );
      if (segment) {

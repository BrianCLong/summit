...
describe("CounterfactualContextReassembler", () => {
  it("builds variants for removal and attenuation", () => {
    ...
    const removalIds = variants.filter((variant) => variant.modification.startsWith("removal"));
...
describe("DivergenceAnalyzer", () => {
  it("flags divergent variants", () => {
    ...
    const variantMeta = [
      { id: "v1", modification: "removal:s1", context: { id: "c1", segments: [], encoded: {} } },
      { id: "v2", modification: "attenuate:s2", context: { id: "c2", segments: [], encoded: {} } }
    ];

...
export class OCILayerCache extends EventEmitter {
  ...
  constructor(
    ...
    console.log(`📦 OCI Layer Cache initialized (registry: ${registryConfig.url}/${registryConfig.namespace})`);
  }
  ...
  async buildWithCache(
    ...
    console.log(`🐳 Building ${imageName} with ${buildSteps.length} steps and layer caching...`);
    ...
    for (let i = 0; i < buildSteps.length; i++) {
      ...
      // Build the layer
      const layer = await this.buildLayer(step, i === 0 ? undefined : layers[layers.length - 1]);
      layers.push(layer);
    ...
    const manifest = await this.buildManifest(imageName, layers, dockerfilePath);
    
    const totalTime = Date.now() - startTime;
    console.log(`🐳 Build completed in ${totalTime}ms (${cacheStats.hits} cache hits, ${cacheStats.misses} misses)`);
  ...
  async pullLayer(digest: string): Promise<OCILayer | null> {
    ...
    try {
      // Check local cache first
      const localLayer = Array.from(this.localCache.values()).find(l => l.digest === digest);
      if (localLayer) {
      ...
      if (layer) {
        ...
        console.log(`📥 Pulled layer ${digest.slice(0, 12)} (${this.formatBytes(layer.size)}) in ${pullTime}ms`);
  ...
  async pushLayer(layer: OCILayer, step: ContainerBuildStep): Promise<void> {
    console.log(`📤 Pushing layer ${layer.digest.slice(0, 12)} (${this.formatBytes(layer.size)})...`);
    ...
    try {
      ...
      if (this.config.enableDeduplication) {
        ...
        if (existing) {
          console.log(`🔄 Layer deduplicated (saved ${this.formatBytes(layer.size)})`);
      ...
      console.log(`📤 Pushed layer ${layer.digest.slice(0, 12)} in ${pushTime}ms`);
      ...
      console.error(`❌ Failed to push layer ${layer.digest.slice(0, 12)}:`, error);
    }
  }
  ...
  private async buildLayer(
    ...
    const digest = crypto.createHash('sha256').update(layerContent).digest('hex');
   ...
   */
  private async simulateLayerCreation(step: ContainerBuildStep): Promise<Buffer> {
  ...
  private async buildManifest(
    ...
    const configDigest = crypto.createHash('sha256').update(configBlob).digest('hex');
  ...
  private cacheLayerLocally(layer: OCILayer): void {
    ...
    this.layerUsage.set(layer.digest, (this.layerUsage.get(layer.digest) || 0) + 1);
  ...
  private async findDuplicateLayer(layer: OCILayer): Promise<OCILayer | null> {
    ...
    for (const [digest, existingLayer] of this.localCache) {
      if (existingLayer.size === layer.size && 
  ...
  private updateBuildMetrics(
    ...
    this.metrics.hitRate = totalLayers > 0 ? (cacheStats.hits / totalLayers) * 100 : 0;
  ...
  getMetrics(): CacheMetrics & {
    ...
    const avgLayerSize = this.metrics.totalLayers > 0 
  ...
  async prewarmCache(commonImages: string[]): Promise<void> {
    console.log(`🔥 Prewarming cache with ${commonImages.length} common images...`);
...
class OCIRegistryClient {
  ...
  async hasLayer(digest: string): Promise<{ exists: boolean; digest?: string; size?: number }> {

...
export class MaestroExplain {
   ...
   */
  async generateBuildGraph(format: 'text' | 'html' | 'dot' | 'json' = 'text'): Promise<void> {
    switch (format) {
      case 'text':
  ...
  private async parseBuildLog(logPath: string): Promise<void> {
    try {
      ...
      for (const line of lines) {
        const taskMatch = line.match(/(\w+): (\w+) \((\d+)ms\) (cache-hit|cache-miss)?/);
        if (taskMatch) {
  ...
  private async generateSampleBuildData(): Promise<void> {
    ...
    const tasks = [
      ...
      { name: 'lint-check', duration: 2000, deps: ['install-deps'], cache: true },
      { name: 'type-check', duration: 3500, deps: ['install-deps'], cache: false },
      { name: 'compile-ts', duration: 8000, deps: ['type-check'], cache: false },
      { name: 'bundle-client', duration: 12000, deps: ['compile-ts'], cache: true },
      { name: 'bundle-server', duration: 6000, deps: ['compile-ts'], cache: false },
      { name: 'run-tests', duration: 15000, deps: ['compile-ts'], cache: false },
      { name: 'build-docker', duration: 25000, deps: ['bundle-server'], cache: true },
      { name: 'security-scan', duration: 8000, deps: ['build-docker'], cache: true }
    ];
  ...
  private calculateCacheStats(): void {
    ...
    const timeSaved = cacheHits.reduce((total, task) => total + (task.duration * 0.8), 0);
  ...
  private renderExecutionSummary(): void {
    ...
    const parallelDuration = this.criticalPath[this.criticalPath.length - 1]?.cumulativeDuration || 0;
    const parallelEfficiency = ((totalDuration - parallelDuration) / totalDuration * 100).toFixed(1);
  ...
  private renderCriticalPath(): void {
    ...
    const totalCritical = this.criticalPath[this.criticalPath.length - 1]?.cumulativeDuration || 0;
  ...
  private renderCacheAnalysis(): void {
    ...
    console.log(`Cache hits: ${this.cacheStats.cacheHits}/${this.cacheStats.totalTasks}`);
    console.log(`Time saved: ${this.formatDuration(this.cacheStats.timeSaved)}`);
    ...
    if (hitTasks.length > 0) {
      ...
      hitTasks.forEach(task => {
        console.log(`   📦 ${task.name} (${this.formatDuration(task.duration)})`);
      });
    }
  ...
  private renderRecommendations(): void {
    ...
    if (this.cacheStats.hitRate < 50) {
      recommendations.push('🔧 Improve cache hit rate by enabling remote cache');
    }
    ...
    if (longestTasks.length > 0) {
      recommendations.push(`🚀 Focus on optimizing: ${longestTasks.map(t => t.name).join(', ')}`);
    }
    
    // Parallel execution
    const serialTasks = this.buildTasks.filter(t => t.dependencies.length === 0).length;
...
<body>
  ...
  private async generateDotGraph(): Promise<void> {
    ...
    console.log('   Generate image with: dot -Tpng build-graph.dot -o build-graph.png');
  }
  ...
  private async exportJsonData(): Promise<void> {
    const exportData = {
      summary: {
        ...
        criticalPathDuration: this.criticalPath[this.criticalPath.length - 1]?.cumulativeDuration || 0
      },
    ...
    await fs.writeFile('build-analysis.json', JSON.stringify(exportData, null, 2));
  ...
  private organizeBuildLevels(): BuildTask[][] {
    ...
    const getLevelForTask = (taskName: string): number => {
      ...
      const maxDepLevel = Math.max(...task.dependencies.map(dep => getLevelForTask(dep)));

...
export class MaestroQuery {
  ...
  private parseQuery(query: string): {
    ...
    // Dependency queries
    if (lowerQuery.startsWith('deps ') || lowerQuery.startsWith('dependencies ') || lowerQuery.includes('depends on')) {
      return {
        type: 'dependencies',
        target: this.extractTarget(query, ['deps', 'dependencies', 'what does', 'depend']),
        parameters: {}
      };
    }
    
    // Reverse dependency queries
    if (lowerQuery.startsWith('rdeps ') || lowerQuery.startsWith('who depends on') || lowerQuery.includes('used by')) {
      return {
        type: 'reverse-dependencies',
        target: this.extractTarget(query, ['rdeps', 'who depends on', 'used by', 'reverse']),
        parameters: {}
      };
    }
    
    // Path queries
    if (lowerQuery.includes('path from') || lowerQuery.includes('how to get from') || lowerQuery.startsWith('path ')) {
    ...
    if (lowerQuery.includes('impact of') || lowerQuery.includes('affected by') || lowerQuery.includes('what breaks')) {
      return {
        type: 'impact',
        target: this.extractTarget(query, ['impact of', 'affected by', 'what breaks', 'impacted']),
        parameters: {}
      };
    }
    
    // Search queries
    if (lowerQuery.startsWith('find ') || lowerQuery.startsWith('search ') || lowerQuery.includes('files like')) {
      return {
        type: 'search',
    ...
    // Stats queries
    if (lowerQuery.includes('stats') || lowerQuery.includes('statistics') || lowerQuery.includes('how many')) {
      return {
        type: 'stats',
  ...
  private async handlePathQuery(parsed: any): Promise<any[]> {
    ...
    const queryResult = await this.graphService.path(parsed.source, parsed.target);
  ...
  private logQueryResult(result: QueryResult): void {
    ...
    console.log(`${cacheIcon} Query completed: ${result.results.length} results (${result.duration}ms)`);
...
Type your query or 'help' for examples...
  ...
  private displayResult(result: QueryResult): void {
    ...
    if (firstResult.step !== undefined) {
      ...
      result.results.slice(0, 10).forEach(item => { // Show first 10
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (query === 'interactive' || query === '-i') {
    ...
    // Single query mode
    maestroQuery.query(query).then(result => {

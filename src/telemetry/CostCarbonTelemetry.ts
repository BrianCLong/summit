...
export class CostCarbonTelemetry extends EventEmitter {
  ...
  private carbonIntensityCache: Map<string, { intensity: number; timestamp: number }> = new Map();
  ...
  constructor(
    ...
    this.metricsExporter = new MetricsExporter(config.metricsBackend || 'console');
    ...
    console.log('📊 Cost & Carbon Telemetry initialized - tracking build economics and environment impact');
  }
  ...
  async recordTargetTelemetry(telemetry: TargetTelemetry): Promise<void> {
    ...
    console.log(`💰 Target ${telemetry.targetName}: $${costUsd.toFixed(4)}, ${carbonKg.toFixed(3)}kg CO2`);
  }
  ...
  async generateCostReport(
    ...
    console.log(`📊 Generating cost report for period ${new Date(startTime).toISOString()} - ${new Date(endTime).toISOString()}`);
    ...
    for (const telemetry of periodData) {
      const cost = this.calculateDetailedCost(telemetry.cost, telemetry.context.nodeRegion);
      ...
      const totalTargetCost = cost.cpu + cost.memory + cost.network + cost.storage;
      const targetCarbonKg = telemetry.carbon.estimatedCarbonGrams / 1000;
      
      // Track by target
      const existing = targetCosts.get(telemetry.targetName) || { cost: 0, carbon: 0 };
      ...
      if (telemetry.context.userId) {
        const userExisting = userCosts.get(telemetry.context.userId) || { cost: 0, carbon: 0 };
      ...
      if (telemetry.context.nodeRegion) {
        const regionExisting = regionCosts.get(telemetry.context.nodeRegion) || { cost: 0, carbon: 0 };
    ...
    const totalCostUsd = totalCpuCost + totalMemoryCost + totalNetworkCost + totalStorageCost;
    ...
    const recommendations = this.generateRecommendations(periodData, totalCostUsd, totalCarbonKg);
    ...
    return {
      ...
      breakdown: {
        byTarget: Array.from(targetCosts.entries())
          .map(([target, data]) => ({ target, costUsd: data.cost, carbonKg: data.carbon }))
          .sort((a, b) => b.costUsd - a.costUsd),
        byUser: Array.from(userCosts.entries())
          .map(([user, data]) => ({ user, costUsd: data.cost, carbonKg: data.carbon }))
          .sort((a, b) => b.costUsd - a.costUsd),
        byRegion: Array.from(regionCosts.entries())
          .map(([region, data]) => ({ region, costUsd: data.cost, carbonKg: data.carbon }))
          .sort((a, b) => b.costUsd - a.costUsd)
      },
      trends,
      recommendations
  ...
  getCurrentMetrics(): {
    ...
    const allBuilds = this.telemetryData.filter(t => t.timestamp > now - 30 * dayMs);
    const successfulBuilds = allBuilds.filter(t => t.build.success);
    const cachedBuilds = allBuilds.filter(t => t.build.cached);
    
    const totalCost = allBuilds.reduce((sum, t) => sum + this.calculateTotalCost(t.cost), 0);
    const totalCarbon = allBuilds.reduce((sum, t) => sum + t.carbon.estimatedCarbonGrams, 0) / 1000;
    ...
    return {
      ...
      thisMonth,
      efficiency: {
        avgCostPerBuild: allBuilds.length > 0 ? totalCost / allBuilds.length : 0,
        avgCarbonPerBuild: allBuilds.length > 0 ? totalCarbon / allBuilds.length : 0,
        successRate: allBuilds.length > 0 ? successfulBuilds.length / allBuilds.length : 0,
        cacheHitRate: allBuilds.length > 0 ? cachedBuilds.length / allBuilds.length : 0
      }
    };
  }

  private async enrichTelemetry(telemetry: TargetTelemetry): Promise<TargetTelemetry> {
    // Enrich with current carbon intensity if not provided
    if (!telemetry.carbon.carbonIntensityGCo2PerKwh && telemetry.context.nodeRegion) {
      const intensity = await this.getCarbonIntensity(telemetry.context.nodeRegion);
      telemetry.carbon.carbonIntensityGCo2PerKwh = intensity;
      telemetry.carbon.estimatedCarbonGrams = telemetry.carbon.estimatedEnergyKwh * intensity;
    }
    
    // Estimate energy usage if not provided
    if (!telemetry.carbon.estimatedEnergyKwh) {
      telemetry.carbon.estimatedEnergyKwh = this.estimateEnergyUsage(telemetry.cost);
    }
  ...
  private async getCarbonIntensity(region: string): Promise<number> {
    ...
    const intensity = this.pricingConfig.regions[region]?.carbonIntensity || 500; // Default 500g CO2/kWh
  ...
  private calculateDetailedCost(
    ...
    const multiplier = region ? (this.pricingConfig.regions[region]?.multiplier || 1) : 1;
    
    return {
      cpu: cost.cpuMinutes * this.pricingConfig.cpu.pricePerMinute * multiplier,
      memory: cost.memoryMbMinutes * this.pricingConfig.memory.pricePerMbMinute * multiplier,
      network: cost.networkEgressMb * this.pricingConfig.network.pricePerMbEgress * multiplier,
      storage: cost.storageMb * this.pricingConfig.storage.pricePerMbMonth * multiplier
    };
  }
  ...
  private getMetricsForPeriod(
    ...
    const costUsd = periodData.reduce((sum, t) => sum + this.calculateTotalCost(t.cost), 0);
    const carbonKg = periodData.reduce((sum, t) => sum + t.carbon.estimatedCarbonGrams, 0) / 1000;
  ...
  private calculateTrends(periodData: TargetTelemetry[]): {
    ...
    const dailyMetrics = new Map<string, { cost: number; carbon: number; builds: number; successes: number }>();
    
    for (const telemetry of periodData) {
      const dayKey = new Date(telemetry.timestamp).toISOString().split('T')[0];
      const existing = dailyMetrics.get(dayKey) || { cost: 0, carbon: 0, builds: 0, successes: 0 };
    ...
    const sortedDays = Array.from(dailyMetrics.entries()).sort(([a], [b]) => a.localeCompare(b));
    
    return {
      costPerBuild: sortedDays.map(([, data]) => data.builds > 0 ? data.cost / data.builds : 0),
      carbonPerBuild: sortedDays.map(([, data]) => data.builds > 0 ? data.carbon / data.builds : 0),
      efficiency: sortedDays.map(([, data]) => data.builds > 0 ? data.successes / data.builds : 0)
    };
  }
  ...
  private generateRecommendations(
    ...
    const unusedSpeculative = speculativeBuilds.filter(t => !t.build.success).length;
    ...
    const regionCosts = new Map<string, { cost: number; carbon: number; count: number }>();
    
    for (const telemetry of periodData) {
      const region = telemetry.context.nodeRegion || 'unknown';
      const existing = regionCosts.get(region) || { cost: 0, carbon: 0, count: 0 };
    ...
    const sortedRegions = Array.from(regionCosts.entries())
      .sort(([, a], [, b]) => (b.cost / b.count) - (a.cost / a.count));
    
    if (sortedRegions.length > 1) {
      const [expensiveRegion, expensiveData] = sortedRegions[0];
      const [cheaperRegion, cheaperData] = sortedRegions[sortedRegions.length - 1];
    ...
    if (avgCarbonPerBuild > 0.1) { // >100g per build
      recommendations.push({
  ...
  private carbonToEquivalent(carbonKg: number): string {
    ...
    const meaningful = equivalents.find(e => e.value >= 0.1 && e.value <= 1000);
    if (meaningful) {
...
export class TargetTracker {
  ...
  async recordResource(type: 'cpu' | 'memory' | 'network' | 'storage', amount: number): Promise<void> {
    switch (type) {
      case 'cpu':
  ...
  async finish(success: boolean, exitCode: number = 0): Promise<TargetTelemetry> {
...
class MetricsExporter {
  ...
  async exportTargetMetrics(telemetry: TargetTelemetry): Promise<void> {
    switch (this.backend) {
      case 'console':
        console.log(`📊 METRICS: ${telemetry.targetName} - $${this.calculateCost(telemetry.cost).toFixed(4)}, ${(telemetry.carbon.estimatedCarbonGrams / 1000).toFixed(3)}kg CO2`);
        break;
      case 'prometheus':
  ...
  async exportAggregateMetrics(metrics: any): Promise<void> {
    switch (this.backend) {
      case 'console':
        console.log(`📊 TODAY: $${metrics.today.costUsd.toFixed(2)}, ${metrics.today.carbonKg.toFixed(2)}kg CO2, ${metrics.today.builds} builds`);
        break;
    }
  }

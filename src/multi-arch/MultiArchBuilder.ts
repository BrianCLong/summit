...
import { BuildKitBuilder, BuildConfig, BuildResult } from '../container/BuildKitBuilder.js';
...
export class MultiArchBuilder extends EventEmitter {
  ...
  async buildMultiArch(config: MultiArchBuildConfig): Promise<MultiArchResult> {
    console.log(`🎯 Starting multi-arch build for ${config.targets.length} targets...`);
    ...
    for (const target of config.targets) {
      ...
      try {
        ...
        this.emit('archBuildComplete', { platform: platformKey, result: buildResult });
    ...
    const result: MultiArchResult = {
      success: builds.size === config.targets.length && Array.from(builds.values()).every(b => b.success),
      manifestDigest,
      builds,
      verification,
      cacheStats: {
    ...
    console.log(`${result.success ? '✅' : '❌'} Multi-arch build completed (${totalDuration}ms)`);
    this.logBuildSummary(result);
    
    return result;
  }

  private async buildForArch(config: MultiArchBuildConfig, target: ArchConfig): Promise<ArchBuildResult> {
    ...
    const archBuildConfig: BuildConfig = {
      ...
      labels: {
        ...
        'maestro.build.cross-compiled': config.crossCompilation.enabled.toString()
      },
  ...
  private async generateArchArtifacts(
    ...
    const mockArtifacts = [
      { name: 'app', path: `/tmp/app-${target.arch}`, size: Math.floor(Math.random() * 1000000) },
      { name: 'config.json', path: `/tmp/config-${target.arch}.json`, size: 1024 }
    ];
  ...
  private async createMultiArchManifest(
    ...
    // Save manifest
    const manifestPath = path.join('.maestro-cache', `manifest-${config.manifestName}.json`);
    await fs.writeFile(manifestPath, manifestJson);
    
    console.log(`📦 Multi-arch manifest created: ${manifestDigest.substring(0, 12)}...`);
  ...
  private async verifyChecksums(builds: Map<string, ArchBuildResult>): Promise<boolean> {
    // Verify that equivalent artifacts have consistent structure
    const artifactsByName = new Map<string, ArchBuildResult['artifacts'][0][]>();
    ...
    for (const [name, artifacts] of artifactsByName) {
      ...
      for (const size of sizes) {
        if (Math.abs(size - avgSize) > maxVariance) {
          console.warn(`⚠️ Size variance detected for ${name}: ${size} vs avg ${avgSize}`);
          return false;
        }
      }
    }
  ...
  private async verifyApiCompatibility(builds: Map<string, ArchBuildResult>): Promise<boolean> {
  ...
  private async verifySignatures(builds: Map<string, ArchBuildResult>): Promise<boolean> {
  ...
  private async detectCachePollution(builds: Map<string, ArchBuildResult>): Promise<number> {
    ...
    for (const [arch, cacheDir] of this.archCaches) {
      try {
        ...
        for (const file of cacheFiles) {
          const content = await fs.readFile(path.join(cacheDir, file), 'utf8').catch(() => '');
          
          // Look for arch identifiers that don't match
          const otherArches = ['amd64', 'arm64', 'x86_64', 'aarch64'].filter(a => a !== arch);
          
          for (const otherArch of otherArches) {
            if (content.includes(otherArch)) {
              pollutionCount++;
              console.warn(`⚠️ Cache pollution detected: ${arch} cache contains ${otherArch} data`);
              break;
            }
          }
        }
  ...
  private createFailedBuildResult(target: ArchConfig): Partial<ArchBuildResult> {
    return {
      architecture: target.arch,
  ...
  private logBuildSummary(result: MultiArchResult): void {
    ...
    for (const [platform, build] of result.builds) {
      ...
      console.log(`   ${status} ${cache} ${cross} ${platform}: ${Math.round(build.duration)}ms`);
    }
    
    console.log('\n🔍 Verification:');
    console.log(`   Checksums: ${result.verification.checksumMatch ? '✅' : '❌'}`);
    console.log(`   API compatibility: ${result.verification.apiCompatible ? '✅' : '❌'}`);
    ...
    console.log(`   Cross-arch pollution: ${result.cacheStats.crossArchPollution}`);
    
    if (result.cacheStats.crossArchPollution > 0) {
      console.log(`   ⚠️ ${(result.cacheStats.crossArchPollution / (result.cacheStats.hits + result.cacheStats.misses) * 100).toFixed(1)}% pollution rate`);
    }
  ...
  private async ensureBuildKitMultiPlatform(): Promise<void> {
    try {
      ...
      execSync('docker buildx create --name maestro-multiarch --driver docker-container --platform linux/amd64,linux/arm64 --use', {
        stdio: 'inherit'
      });
    }
  }
  ...
  private async initializeQemuEmulation(): Promise<void> {
    try {
      ...
      execSync('docker run --rm --privileged multiarch/qemu-user-static --reset -p yes', {
        stdio: 'inherit'
      });
  ...
  private async setupArchCaches(): Promise<void> {
    ...
    console.log(`📦 Setup cache directories for ${architectures.length} architectures`);
  }
  ...
  async cleanup(): Promise<void> {
    ...
    for (const builds of []) { // Would iterate over stored builds
      // Cleanup logic
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  builder.buildMultiArch(config).then(result => {
    ...
    console.log(`   Cache hits: ${result.cacheStats.hits}/${result.cacheStats.hits + result.cacheStats.misses}`);
    
    if (result.verification.checksumMatch && result.verification.apiCompatible) {
      console.log('🎯 Verification passed - builds are consistent across architectures');
    }

...
export class ATGGraphWriter {
  ...
  /**
   * Writes a TECF event into the ATG subgraph with strict tenant scoping.
   */
  ...
  /**
   * Creates a temporal slice marker.
   */
  async createSlice(tenant_id: string, slice_time: string) {
    const session = this.driver.session();
    try {
      await session.executeWrite(tx => tx.run(`
        MERGE (s:ATGSlice { tenant_id: $tenant_id, slice_time: datetime($slice_time) })
        RETURN s
      `, { tenant_id, slice_time }));
    } finally {
      await session.close();
    }
  }

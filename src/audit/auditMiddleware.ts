...
export class AuditMiddleware {
          ...
          details: {
            ...
            headers: Object.fromEntries(
              Object.entries(res.getHeaders()).filter(([key]) => 
                !self.sensitiveHeaders.includes(key.toLowerCase())
              )
            )
          },
          metadata: {
        ...
        didResolveOperation: async (requestContext: any) => {
          ...
          await this.auditLogger.logEvent({
            ...
            details: {
              ...
              operationType: request.query?.match(/(query|mutation|subscription)/i)?.[1],
  ...
  async auditDataAccess(
    ...
    resourceId: string,
    userId: string,
    dataClassification: 'PUBLIC' | 'INTERNAL' | 'CONFIDENTIAL' | 'SECRET' = 'INTERNAL',
    recordCount = 1
  ) {
    await this.auditLogger.logEvent({
      ...
      tags: ['data-access', dataClassification.toLowerCase(), action.toLowerCase()]
    });
  }
  ...
  private sanitizeConfigValue(value: any): any {
    if (typeof value === 'string') {
      ...
      if (value.match(/^[A-Za-z0-9+/]{20,}={0,2}$/) || // Base64
  ...
  private getDatabaseOperationSeverity(operation: string): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
    const operationSeverity: Record<string, 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'> = {
      SELECT: 'LOW',
      INSERT: 'MEDIUM',
      UPDATE: 'MEDIUM',
      DELETE: 'HIGH',
  ...
  private getDataAccessSeverity(action: string, classification: string): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
    const baseSeverity = {
      READ: 'LOW',
      WRITE: 'MEDIUM',
      DELETE: 'HIGH',
    ...
    const newIndex = Math.min(severityLevels.length - 1, currentIndex + classificationMultiplier - 1);

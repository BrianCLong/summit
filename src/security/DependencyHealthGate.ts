...
export class DependencyHealthGate extends EventEmitter {
  ...
  constructor(policy: HealthGatePolicy) {
    ...
    console.log(`🛡️ Dependency Health Gate initialized (${this.policy.enforcement.mode} mode)`);
  }
    ...
    try {
      ...
      const failedResult: HealthGateResult = {
        ...
        blockers: [{
          ...
          remediation: 'Check network connectivity and dependency scan configuration'
        }],
        warnings: [],
        exceptions: [],
  ...
  async createException(
    ...
    console.log(`📝 Created ${type} exception: ${target} (expires: ${new Date(exception.expiresAt).toISOString()})`);
   ...
   */
  async revokeException(exceptionId: string, revoker: string, reason?: string): Promise<void> {
  ...
  private async parsePackageFiles(packageFiles: string[]): Promise<PackageInfo[]> {
  ...
  private async scanVulnerabilities(packages: PackageInfo[]): Promise<Vulnerability[]> {
    console.log(`🔍 Scanning ${packages.length} packages for vulnerabilities...`);
  ...
  private async queryOsvDatabase(packageName: string, version: string): Promise<Vulnerability[]> {
    try {
      ...
      console.warn(`⚠️ Failed to query OSV for ${packageName}@${version}:`, error);
      return [];
    }
  }
  ...
  private async analyzeLicense(packageName: string, version: string): Promise<LicenseInfo> {
    // Simulate license detection
    const commonLicenses = ['MIT', 'Apache-2.0', 'BSD-3-Clause', 'ISC', 'GPL-3.0', 'UNKNOWN'];
    ...
    // Classify license
    if (['MIT', 'Apache-2.0', 'BSD-3-Clause', 'BSD-2-Clause', 'ISC'].includes(license)) {
      licenseType = 'permissive';
      riskLevel = 'low';
    } else if (['GPL-2.0', 'GPL-3.0', 'AGPL-3.0', 'LGPL-2.1', 'LGPL-3.0'].includes(license)) {
  ...
  private async evaluateHealthPolicy(
    ...
    for (const vuln of vulnerabilities) {
      ...
      if (this.policy.vulnerabilities.blockSeverities.includes(vuln.severity) ||
          (vuln.cvssScore && vuln.cvssScore > this.policy.vulnerabilities.maxCvssScore)) {
        blockers.push({
          type: 'vulnerability',
      ...
      } else if (this.policy.vulnerabilities.warnSeverities.includes(vuln.severity)) {
        warnings.push({
          type: 'vulnerability',
          description: `${vuln.severity} vulnerability in ${vuln.packageName}: ${vuln.summary}`,
          remediation: vuln.fixedVersions.length > 0 
    ...
    for (const license of licenses) {
      // Check for exceptions
      const exception = this.findActiveException('license', `${license.packageName}:${license.license}`);
      if (exception) {
        activeExceptions.push(exception);
        continue;
      }
    ...
    if (blockers.length > 0) {
      if (this.policy.enforcement.mode === 'enforce' && this.policy.enforcement.failOnBlockers) {
        overallStatus = 'fail';
      } else {
        overallStatus = 'warn';
      }
  ...
  private findActiveException(type: 'vulnerability' | 'license', target: string): TTLException | undefined {
  ...
  private createEmptyResult(buildId: string, startTime: number): HealthGateResult {
    return {
      buildId,
      timestamp: Date.now(),
      overallStatus: 'pass',
  ...
  private updateMetrics(result: HealthGateResult): void {
    ...
    this.metrics.avgScanTime = (this.metrics.avgScanTime + result.executionTime) / 2;
  }
  ...
  private logResults(result: HealthGateResult): void {
    ...
    if (result.blockers.length > 0) {
      ...
      result.blockers.forEach((blocker, i) => {
        console.log(`   ${i + 1}. [${blocker.severity}] ${blocker.description}`);
  ...
  private cleanupExpiredExceptions(): void {
    ...
    for (const [id, exception] of this.exceptions.entries()) {
      if (exception.expiresAt <= now) {
        ...
        console.log(`🧹 Expired exception: ${exception.target} (${exception.type})`);
  ...
  getMetrics(): typeof this.metrics & {
    ...
    return {
      ...
      passingRate: this.metrics.totalScans > 0 
        ? this.metrics.passedScans / this.metrics.totalScans 
        : 0,
      blockingRate: this.metrics.totalScans > 0 
  ...
  setEnforcementMode(mode: 'enforce' | 'warn' | 'audit'): void {
    console.log(`🛡️ Health Gate enforcement mode changed: ${this.policy.enforcement.mode} → ${mode}`);
...
// Factory function
export function createDependencyHealthGate(policy: HealthGatePolicy): DependencyHealthGate {

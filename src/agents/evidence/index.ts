...
export function assertDeterminism(
  ...
  const walk = (node: unknown, path: string[]): void => {
    ...
    for (const [key, entry] of Object.entries(node as Record<string, unknown>)) {
      ...
      if (!allowTimestampFields && typeof entry === 'string' && hasTimestampKey) {
...
export type EvidenceDocumentKind = 'report' | 'metrics' | 'index' | 'stamp';

export function assertDeterministicEvidenceDocument(
  documentKind: EvidenceDocumentKind,
  value: unknown,
): void {
  const allowTimestampFields = documentKind === 'stamp';
  assertDeterminism(value, { allowTimestampFields });
}

...
export class ReflectiveLoop {
  ...
  async execute(): Promise<ReflectiveLoopResult> {
    ...
    for (let i = 1; i <= this.maxIterations; i++) {
      ...
      console.log(`Risk Score: ${currentRiskScore} (previous: ${previousRiskScore})`);
      ...
      if (improvement < this.convergenceThreshold) {
        stagnationCount++;
        console.log(`⚠️ Limited improvement detected (${improvement}). Stagnation count: ${stagnationCount}`);
      } else {
        stagnationCount = 0;
      }
      ...
      if (iteration.criticResult.staticCheckResults.some(r => !r.passed)) {
        ...
        iteration.fixerResult = await this.fixer.applyFixes(iteration.criticResult);
        
        console.log(`Fixes applied: ${iteration.fixerResult.fixesApplied.length}`);
        console.log(`Remaining issues: ${iteration.fixerResult.remainingIssues.length}`);
    ...
    const recommendations = this.generateFinalRecommendations(iterations, finalState);
  ...
  private determineFinalState(finalIteration: LoopIteration): 'success' | 'partial' | 'failed' {
    if (!finalIteration.criticResult) {
      return 'failed';
    }

    const riskScore = finalIteration.criticResult.riskScore;
    const hasErrors = finalIteration.criticResult.staticCheckResults
      .some(r => r.issues.some(i => i.severity === 'error'));
  ...
  private calculateMetrics(iterations: LoopIteration[]): LoopMetrics {
    ...
    const initialIssues = firstIteration.criticResult?.staticCheckResults
      .reduce((sum, r) => sum + r.issues.length, 0) || 0;
    
    const finalIssues = lastIteration.criticResult?.staticCheckResults
      .reduce((sum, r) => sum + r.issues.length, 0) || 0;

    const initialRisk = firstIteration.criticResult?.riskScore || 100;
    const finalRisk = lastIteration.criticResult?.riskScore || 100;

    const totalFixes = iterations.reduce((sum, iter) => 
      sum + (iter.fixerResult?.fixesApplied.filter(f => f.applied).length || 0), 0
    );

    const improvements = iterations.map(iter => iter.improvement).filter(imp => imp > 0);
  ...
  private generateFinalRecommendations(
    ...
    switch (finalState) {
      case 'success':
        recommendations.push('✅ All critical issues resolved - ready for merge');
        recommendations.push('Consider running extended test suite before deployment');
        break;
      ...
      case 'failed':
        ...
        recommendations.push('Consider reverting changes and redesigning approach');
        break;
    }

    // Performance recommendations
    const avgTime = iterations.reduce((sum, iter, idx, arr) => 
      sum + (idx > 0 ? new Date(iter.timestamp).getTime() - new Date(arr[idx-1].timestamp).getTime() : 0), 0
    ) / Math.max(1, iterations.length - 1);

    if (avgTime > 60000) {
      recommendations.push('Consider optimizing fix strategies for faster cycles');
    }

    // Pattern-based recommendations
    const stuckPatterns = iterations.filter(iter => iter.improvement < 0);
    if (stuckPatterns.length > 1) {
      recommendations.push('Detected regression patterns - review fix strategies');
    }
  ...
  private logSummary(result: ReflectiveLoopResult): void {
    ...
    console.log(`Iterations: ${result.iterations.length}/${this.maxIterations}`);
    ...
    console.log(`  Cycle Efficiency: ${result.metrics.cycleEfficiency.toFixed(2)}`);
    console.log(`  Convergence Rate: ${result.metrics.convergenceRate.toFixed(2)}`);
    ...
    result.iterations.forEach(iter => {
      ...
      const fixCount = iter.fixerResult?.fixesApplied.filter(f => f.applied).length || 0;
      console.log(`  ${iter.iteration}: ${iter.action} | Risk: ${riskScore} | Fixes: ${fixCount} | Δ: ${iter.improvement}`);
    });

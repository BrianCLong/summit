...
export class AgentOrchestrator {
  ...
  private evaluatePhaseSuccess(phase: string, results: any): boolean {
    switch (phase) {
      ...
      case 'implementation':
        return (
          results.finalState === 'success' || results.finalState === 'partial'
        );
      case 'testing':
  ...
  private logOrchestrationSummary(
    ...
    results.forEach((result) => {
      ...
      if (
        ...
        result.results.riskScore !== undefined
      ) {
      ...
      if (result.phase === 'implementation' && result.results.finalState) {
        ...
        console.log(
          `    Issues Resolved: ${result.results.issuesResolved || 0}`,
        );
      }

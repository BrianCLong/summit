...
class DivergenceAnalyzer:
  ...
  def score_responses(
    ...
    for variant in variant_meta:
      ...
      if not response:
        continue
      divergence = self._compute_divergence(base_text, str(response.output or ""))
      scores.append(
        DivergenceScore(
  ...
  def detect_poisoning(self, scores: List[DivergenceScore]) -> List[PoisoningIndicator]:
    return [
      PoisoningIndicator(
        segment_id=score.modification.split(":")[1],

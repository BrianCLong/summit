...
class TrustWeightedContextAssembler:
  ...
  def assemble(self, context_id: str, segments: List[ContextSegment]) -> AssembledContext:
    sorted_segments = sorted(segments, key=lambda s: s.trust_weight.value, reverse=True)

...
class InvariantCapsule:
      ...
      for invariant in segment.invariants:
        ...
        results.append(
          CapsuleResult(
            ...
            valid=valid,
            message=None if valid else f"Invariant {invariant.id} failed for {segment.metadata.id}",
          )
        )
    return results
  ...
  def isolate_invariant_failures(self) -> List[ContextSegment]:
    return [
      segment
      for segment in self.segments
      if any(not invariant.validate(segment.content) for invariant in segment.invariants)
    ]

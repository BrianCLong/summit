...
class CounterfactualContextReassembler:
    ...
    def generate_variants(
        self, base_id: str, segments: List[ContextSegment]
    ) -> List[CounterfactualVariant]:
        variants: List[CounterfactualVariant] = []
        limit = self.config.max_variants or len(segments) * 2
        attenuation_weight = max(0.0, min(1.0, self.config.attenuation_weight))
            ...
            variants.append(
                CounterfactualVariant(
                    ...
                    context=self.assembler.assemble(
                        f"{base_id}:remove:{segment.metadata.id}",
                        remaining,
                    ),
                )
            )
                    ...
                    attenuated.append(
                        ContextSegment(
                            ...
                            trust_weight=type(candidate.trust_weight)(
                                value=candidate.trust_weight.value * attenuation_weight,
    ...
    def execute_with_variants(
        self, base_request: ModelExecutionRequest
    ) -> tuple[
        ModelExecutionResponse,
        dict[str, ModelExecutionResponse],
        List[CounterfactualVariant],
    ]:
        if not self.executor:
            raise ValueError("No executor provided for CCR execution")
        ...
        base_response = self.executor.execute(base_request)
        variant_meta = self.generate_variants(
            base_request.context.id, base_request.context.segments
        )
        variant_responses = self._run_variants(base_request, variant_meta)
        return base_response, variant_responses, variant_meta

    def _run_variants(
        self,
        base_request: ModelExecutionRequest,
        variant_meta: List[CounterfactualVariant] | None = None,
    ) -> dict[str, ModelExecutionResponse]:
        if not self.executor:
            raise ValueError("No executor provided for CCR execution")

        responses: dict[str, ModelExecutionResponse] = {}
        variant_meta = variant_meta or self.generate_variants(
            base_request.context.id, base_request.context.segments
        )

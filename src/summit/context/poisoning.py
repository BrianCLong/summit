...
class PoisoningResponder:
  def __init__(self, quarantine: Callable[[ContextSegment], None] | None = None) -> None:
    self.quarantine = quarantine or (lambda _segment: None)

  def apply(self, context: AssembledContext, indicators: List[PoisoningIndicator]) -> PoisoningResponse:
    ...
    for indicator in indicators:
      ...
      rationale[indicator.segment_id] = (
        f"Divergence {indicator.divergence:.2f} detected via {indicator.modification}"
      )

...
app.get('/admin/tenants', (_req, res) =>
  res.json({ items: [{ id: 'default', name: 'Default' }] }),
);
app.get('/admin/users', (_req, res) =>
  res.json({
    items: [{ id: 'u1', email: 'analyst@example.com', role: 'analyst' }],
  }),
);
app.get('/admin/flags', (_req, res) => res.json({ flags }));
app.put('/admin/flags/:key', (req, res) => {
  const k = req.params.key;
  flags[k] = !!req.body?.value;
  res.json({ ok: true, key: k, value: flags[k] });
});
...
app.post('/copilot/cookbook', (_req, res) => {
  res.json({
    ok: true,
    items: [
      { id: 'centrality', title: 'Centrality for IDs' },
      { id: 'communities', title: 'Communities overview' },
    ],
  });
});

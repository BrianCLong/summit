...
export type CheckMode = 'schemas' | 'index' | 'all';
...
const checkArg = getArgValue('--check');

const toCheckMode = (value: string | undefined): CheckMode => {
  if (!value) return 'all';
  if (value === 'schemas' || value === 'index' || value === 'all') {
    return value;
  }
  throw new Error(`Invalid --check value "${value}". Expected one of: schemas, index, all.`);
};

const check = toCheckMode(checkArg);

...
for (const finding of report.findings) {
  ...
  if (!finding.evidenceRef) fail('Each finding must include evidenceRef');
  evidenceRefs.add(finding.evidenceRef);
...
const evidenceIndexPath = path.join(ART_DIR, 'evidence/index.json');
const evidenceIndex = readJson(evidenceIndexPath);
if (!evidenceIndex || typeof evidenceIndex !== 'object') {
  fail('evidence/index.json must be an object');
}
if (!Array.isArray(evidenceIndex.evidence)) {
  fail('evidence/index.json must include evidence array');
}
const evidenceIdSet = new Set(
  evidenceIndex.evidence
    .map((entry) => entry && entry.evidenceId)
    .filter((evidenceId) => typeof evidenceId === 'string' && evidenceId.length > 0),
);
for (const evidenceRef of evidenceRefs) {
  if (!evidenceIdSet.has(evidenceRef)) {
    fail(`Evidence reference missing from index: ${evidenceRef}`);
  }
}

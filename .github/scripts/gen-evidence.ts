...
const dependencyEvidenceDir = path.join(evidenceDir, 'dependency-homepage');
const indexPath = path.join(evidenceDir, 'index.json');
const reportPath = path.join(dependencyEvidenceDir, 'report.json');
const metricsPath = path.join(dependencyEvidenceDir, 'metrics.json');
const stampPath = path.join(dependencyEvidenceDir, 'stamp.json');

const defaultFiles: EvidenceFiles = {
  report: 'evidence/dependency-homepage/report.json',
  metrics: 'evidence/dependency-homepage/metrics.json',
  stamp: 'evidence/dependency-homepage/stamp.json',
};
...
const main = async (): Promise<void> => {
  ...
  await fs.mkdir(dependencyEvidenceDir, { recursive: true });

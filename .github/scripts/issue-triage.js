...
async function updateStatusComment({ github, owner, repo, issue, stale }) {
  const statusLabel = stale ? 'status/stale' : 'status/actionable';
  ...
  if (stale && !hasLabel) {
    const cutoffDate = new Date(Date.now() - STALE_DAYS * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  ...
  if (!stale && hasLabel && daysSince(issue.updated_at) <= ACTION_RESET_DAYS) {
    await github.rest.issues.createComment({
      owner,
      repo,
      issue_number: issue.number,
...
async function processIssue({ github, context, issue }) {
  ...
  await updateStatusComment({ github, owner, repo, issue, stale: desiredLabels.stale });
}

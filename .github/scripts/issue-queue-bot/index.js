...
class QueueBot {
  ...
  calculateScore(issue) {
    ...
    if (this.rules.keywords.ga_blocker.some((kw) => text.includes(kw.toLowerCase())))
      score += this.rules.scoring.text_ga_blocker;
    if (issue.labels.includes("ga:blocker")) score += this.rules.scoring.label_ga_blocker;

    if (["ci", "reproducibility", "security"].some((lbl) => issue.labels.includes(lbl)))
      score += this.rules.scoring.label_ci_repro_sec;

    // A very simple heuristic for "referencing failing workflow/check name"
    if (/(?:workflow|check|gate|failed|error)/i.test(text) && /\.yml|\.js/.test(text))
      score += this.rules.scoring.text_failing_workflow;

    if (["needs-info", "blocked"].some((lbl) => issue.labels.includes(lbl)))
      score += this.rules.scoring.label_needs_info_blocked;
  ...
  getConfidence(score) {
    ...
    if (score >= this.rules.confidence.medium.min && score <= this.rules.confidence.medium.max)
      return "medium";
    return "low";
  }

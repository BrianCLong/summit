...
const rootsToScan = [".github/workflows", ".github/actions"].map((relativePath) =>
  ...
  walkDir(root, (filepath) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (!isV4Tag && !isPinnedSha) {
        console.error(
          `::error file=${filepath},line=${index + 1}::${action} must use @v4 or a pinned commit SHA, found @${version}`
        );
        hasError = true;
      }
    });

...
export function emitEvidence(
  ...
  const report = {
    run_id: runId,
    mode: "swarm",
    summary: "Swarm execution completed",
    evidence_ids: [`EVD-KIMIK25-SWARM-RUN-${runId}`],
...
function updateIndex(evidenceId: string, files: string[]) {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    index.items[evidenceId] = {
      files: files,
      title: `Swarm Run ${evidenceId.slice(-8)}`,
      category: "runtime",
    };

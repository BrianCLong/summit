...
exports.ConsentAwareAllocator = void 0;
const crypto_1 = require("crypto");
function subjectKey(tenantId, subjectId) {
    return `${tenantId}:${subjectId}`;
}
function assignmentKey(experimentId, tenantId, subjectId) {
...
class ConsentAwareAllocator {
    ...
    removeSubject(tenantId, subjectId) {
    ...
    async assign(experimentConfig, tenantId, subjectId) {
    ...
    rebalance(experimentId) {
        const experiment = this.experiments.get(experimentId);
        if (!experiment) {
            return;
        }
        const stratumSubjects = new Map();
    ...
    rebalanceStratum(experiment, stratum, subjects) {
        ...
        const assignmentsByVariant = new Map();
        const unassigned = [];
    ...
    applyAssignment(experiment, variant, stratum, subject, event) {
...
exports.ConsentAwareAllocator = ConsentAwareAllocator;
function hasConsent(subject, purpose) {
    return subject.consents?.[purpose]?.granted ?? false;
}
function matchesExclusion(rules, subject) {
    if (!rules?.length) {
        return false;
    }
...
function resolveStratum(experiment, subject) {
...
function weightMapForStratum(experiment, stratum) {
...
function computeTargetCounts(total, variants, weights) {
    let sumWeights = 0;
...
function nextVariantWithDeficit(variants, target, current) {
...
function findSurplusAndDeficit(variants, target, current) {
    let surplusName;
    let surplusDiff = 0;
    let deficitName;
    let deficitDiff = 0;
...
function withinTolerance(experiment, stratum, current) {
    let total = 0;
...
function stableHash(experiment, stratum, subject) {
    ...
    const digest = (0, crypto_1.createHash)('sha256').update(payload).digest();

...
const DUMMY_JWT =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkZXYtdXNlciIsIm5hbWUiOiJUZXN0IFVzZXIiLCJpYXQiOjE1MTYyMzkwMjJ9.Q_R-K-d_3E_V-A-C-K-E-R";
...
test.describe("Golden Path E2E", () => {
  test.beforeAll(() => {
    ...
    execSync(
      `curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer ${DUMMY_JWT}" -d '{"source": "golden/entities.csv", "type": "entities"}' http://localhost:4000/api/ingest/push`
    );
  });
  ...
  test("should return ingested data and relationships correctly", async ({ request }) => {
    ...
    const entityQuery =
      '{ searchEntities(q: "Acme", tenant: "default-tenant") { ... on Org { name } } }';
    const entityCondition = (body) =>
      body.data.searchEntities &&
      body.data.searchEntities.some((e) => e.name === "Acme Corporation");
    ...
    execSync(
      `curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer ${DUMMY_JWT}" -d '{"source": "golden/edges.csv", "type": "edges"}' http://localhost:4000/api/ingest/push`
    );

    // 3. Poll until the relationship (neighbor) is searchable
    const neighborQuery =
      '{ neighbors(id: "1", tenant: "default-tenant") { ... on Org { name } } }';
    const neighborCondition = (body) =>
      body.data.neighbors && body.data.neighbors.some((n) => n.name === "Acme Corporation");

...
const agent_session_page_1 = require("../support/pages/agent-session.page");
test_1.test.describe('Agent Session Lifecycle', () => {
    let agentPage;
    test_1.test.beforeEach(async ({ page }) => {
        agentPage = new agent_session_page_1.AgentSessionPage(page);
        await agentPage.navigate();
    });
    (0, test_1.test)('should create, execute and review an agent session', async ({ page }) => {
    ...
    (0, test_1.test)('should handle failed sessions gracefully', async ({ page }) => {
        ...
        await page.route('/api/maestro/runs', async (route) => {
            const json = {
                ...
                tasks: [
                    { id: 'task-1', description: 'Analyze PRs', status: 'failed' }
                ],
                results: [
                    {
                        task: { id: 'task-1', description: 'Analyze PRs', status: 'failed', errorMessage: 'API Rate Limit' },
                        artifact: null
                    }
                ],
                costSummary: {
        ...
        await (0, test_1.expect)(page.locator('text=Error: API Rate Limit')).toBeVisible();
    });
});

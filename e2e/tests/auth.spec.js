...
base_1.test.describe('Authentication', () => {
    ...
    (0, base_1.test)('should login successfully with valid credentials', async ({ loginPage, page }) => {
        ...
        await (0, base_1.expect)(page).toHaveURL('http://localhost:3000/');
    });
    (0, base_1.test)('should show error with invalid credentials', async ({ loginPage }) => {
        await loginPage.goto();
        await loginPage.login('wrong@example.com', 'wrongpassword');
        const errorMessage = await loginPage.getErrorMessageText();
        (0, base_1.expect)(errorMessage).toContain('Invalid credentials');
    });
    (0, base_1.test)('should require email and password', async ({ loginPage }) => {
        await loginPage.goto();
        await (0, base_1.expect)(loginPage.emailInput).toHaveAttribute('required', '');
        await (0, base_1.expect)(loginPage.passwordInput).toHaveAttribute('required', '');
    });
});

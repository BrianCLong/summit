...
test.describe('Golden Path CI Gate - REQUIRED FOR MERGE', () => {
  ...
  test('complete golden path with monitoring health check', async ({
    page,
  }) => {
    ...
    await test.step('Create Demo Investigation', async () => {
      ...
      await page.fill(
        ...
        `Golden Path CI Test ${testId}`,
      );
      await page.fill(
        '[data-testid="investigation-description"]',
        'Automated test investigation for CI gate',
      );
    ...
    await test.step('Create Test Entities', async () => {
      ...
      await page.fill(
        ...
        'Supply chain manager at TechCorp',
      );
      await page.click('[data-testid="entity-save"]');

      await expect(
        page.locator('[data-testid="entity-alice-chen"]'),
      ).toBeVisible();
      ...
      await page.fill(
        '[data-testid="entity-description"]',
        'Vendor coordinator at GlobalSupply',
      );
      await page.click('[data-testid="entity-save"]');

      await expect(
        page.locator('[data-testid="entity-bob-martinez"]'),
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="entity-techcorp"]'),
      ).toBeVisible();
    });
    ...
    await test.step('Create Test Relationships', async () => {
      ...
      await page.fill(
        ...
        'Alice works as supply chain manager',
      );
      ...
      await expect(
        page.locator('[data-testid="relationship-alice-techcorp"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="relationship-alice-bob"]'),
      ).toBeVisible();
    });
    ...
    await test.step('Test GraphRAG Query with Explanations', async () => {
      ...
      await expect(
        page.locator('[data-testid="graphrag-panel"]'),
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="graphrag-response"]'),
      ).toBeVisible({ timeout: 15000 });
  ...
  test('PBAC denial verification for unauthorized access', async ({ page }) => {
    ...
    await test.step('Verify PBAC Allows Authorized Reads', async () => {
      ...
      await page.goto('/investigations');
      await expect(
        page.locator('[data-testid="investigations-list"]'),
      ).toBeVisible();

      // But not see create/edit controls
      await expect(
        page.locator('[data-testid="create-investigation-button"]'),
      ).not.toBeVisible();
    });
    ...
    await test.step('Verify GraphRAG Access Control', async () => {
      ...
      await page.click('[data-testid="graphrag-panel-toggle"]');
      await page.fill(
        '[data-testid="graphrag-question-input"]',
        'Test read access',
      );
      await page.click('[data-testid="graphrag-submit-button"]');
      ...
      await expect(
        page.locator(
          '[data-testid="graphrag-response"], [data-testid="graphrag-loading"]',
        ),
  ...
  test('persisted queries enforcement (production mode)', async ({ page }) => {
    await test.step('Verify Non-Persisted Query Rejection in Prod Mode', async () => {
      ...
      if (response.status() === 403) {
        ...
        expect(errorData.errors[0].extensions.code).toBe(
  ...
  test('end-to-end performance benchmarks', async ({ page }) => {
    await test.step('Measure Golden Path Performance', async () => {
      ...
      await expect(
        page.locator('[data-testid="investigation-loaded"]'),
      ).toBeVisible();

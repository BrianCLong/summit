...
test_1.test.describe('Advanced Graph Flow', () => {
    (0, test_1.test)('right-click expand and AI overlay', async ({ page, request }) => {
        const layout = process.env.LAYOUT || 'cose-bilkent';
        const sprite = process.env.SPRITE_LABELS || '0';
        await page.addInitScript(([layout, sprite]) => {
        ...
        await (0, test_1.expect)(page.getByText('AI Panel')).toBeVisible();
        // Seed a node into the graph
        await page.evaluate(() => {
            document.dispatchEvent(new CustomEvent('graph:addElements', {
        ...
        await (0, test_1.expect)(page.getByText('AI Insights')).toBeVisible();
        await (0, test_1.expect)(page.getByText('AI says hello')).toBeVisible();
    });
});

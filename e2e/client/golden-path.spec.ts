...
test.describe("Golden Path - Complete Investigation Flow @golden-path-critical", () => {
  ...
  test("complete golden path flow with explainable results", async ({ page }) => {
    ...
    await test.step("Examine Relationships", async () => {
      ...
      await expect(page.locator('[data-testid="relationship-details"]')).toContainText(
        "employed_by"
      );

      // Verify relationship properties are shown
      await expect(page.locator('[data-testid="relationship-properties"]')).toContainText(
    ...
    await test.step("Verify Explainable Results with Why Paths", async () => {
      ...
      await expect(page.locator('[data-testid="edge-rel-alice-bob-003"]')).toHaveClass(
        /highlighted|why-path/
      );
      await expect(page.locator('[data-testid="edge-rel-bob-globalsupply-002"]')).toHaveClass(
        /highlighted|why-path/
      );

      // Click on highlighted path to see details
      await page.click('[data-testid="edge-rel-alice-bob-003"]');
      await expect(page.locator('[data-testid="path-explanation"]')).toContainText(
    ...
    await test.step("Verify Entity Citations", async () => {
      ...
      await expect(page.locator('[data-testid="entity-alice-001"]')).toHaveClass(
        /focused|highlighted/
      );
    });
    ...
    await test.step("Test Second Question - Contract Access", async () => {
      ...
      await expect(page.locator('[data-testid="edge-rel-techcorp-contract-004"]')).toHaveClass(
        /highlighted|why-path/
      );
      await expect(page.locator('[data-testid="edge-rel-globalsupply-contract-005"]')).toHaveClass(
        /highlighted|why-path/
      );
    });

...
test.describe('Golden Path - Complete Investigation Flow', () => {
  ...
  test('complete golden path flow with explainable results', async ({
    page,
  }) => {
    ...
    await test.step('Explore Entities in Graph', async () => {
      ...
      await expect(
        page.locator('[data-testid="entity-alice-001"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="entity-bob-002"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="entity-techcorp-003"]'),
      ).toBeVisible();

      // Click on Alice Chen entity
      await page.click('[data-testid="entity-alice-001"]');
      await expect(
        page.locator('[data-testid="entity-details"]'),
    ...
    await test.step('Examine Relationships', async () => {
      ...
      await expect(
        page.locator('[data-testid="relationship-details"]'),
      ).toContainText('employed_by');

      // Verify relationship properties are shown
      await expect(
        page.locator('[data-testid="relationship-properties"]'),
      ).toContainText('Supply Chain Manager');
    });
    ...
    await test.step('Verify Explainable Results with Why Paths', async () => {
      ...
      await expect(
        page.locator('[data-testid="highlighted-path"]'),
      ).toBeVisible();

      // Verify specific edge IDs are highlighted
      await expect(
        page.locator('[data-testid="edge-rel-alice-bob-003"]'),
      ).toHaveClass(/highlighted|why-path/);
      await expect(
        page.locator('[data-testid="edge-rel-bob-globalsupply-002"]'),
      ).toHaveClass(/highlighted|why-path/);

      // Click on highlighted path to see details
      await page.click('[data-testid="edge-rel-alice-bob-003"]');
      await expect(
        page.locator('[data-testid="path-explanation"]'),
      ).toContainText('This connection shows');
    });
    ...
    await test.step('Verify Entity Citations', async () => {
      ...
      await expect(
        page.locator('[data-testid="entity-alice-001"]'),
      ).toHaveClass(/focused|highlighted/);
    });
    ...
    await test.step('Test Second Question - Contract Access', async () => {
      ...
      await expect(
        page.locator('[data-testid="edge-rel-techcorp-contract-004"]'),
      ).toHaveClass(/highlighted|why-path/);
      await expect(
        page.locator('[data-testid="edge-rel-globalsupply-contract-005"]'),
      ).toHaveClass(/highlighted|why-path/);
    });
  ...
  test('golden path handles errors gracefully', async ({ page }) => {
    await test.step('Test Error Handling in Copilot', async () => {
      ...
      await questionInput.fill(
        'What is the meaning of life in quantum mechanics?',
      );
      await page.click('[data-testid="copilot-ask-button"]');
      ...
      await expect(response).toContainText(
        /insufficient.*information|cannot.*answer|not.*found/i,
      );
    });
  });
  ...
  test('verify demo data is properly seeded', async ({ page }) => {
    await test.step('Check Demo Data Exists via API', async () => {
      ...
      for (const entity of expectedEntities) {
        await expect(
          page.locator('[data-testid="entities-panel"]'),
        ).toContainText(entity);
      }
    });
  });
});

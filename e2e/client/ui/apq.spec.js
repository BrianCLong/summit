...
(0, test_1.test)('APQ uses hashed GET without raw query bodies', async ({ page }) => {
    const graphqlRequests = [];
    page.on('request', (request) => {
        if (request.url().includes('/graphql')) {
            const url = request.url();
            const method = request.method();
            const hasQuery = url.includes('query=') &&
    ...
    (0, test_1.expect)(graphqlRequests.length).toBeGreaterThan(0);
    ...
    graphqlRequests.forEach((req, index) => {
        ...
        if (!req.hasExtensions) {
            violations.push(`Request ${index + 1}: Missing APQ extensions - ${req.url.substring(0, 100)}...`);
        }
        // Should NOT have raw query bodies in GET requests
        if (req.method === 'GET' && req.hasQuery) {
            violations.push(`Request ${index + 1}: Contains raw query body in GET request - ${req.url.substring(0, 100)}...`);
        }
    });
    if (violations.length > 0) {
        console.log('GraphQL Requests captured:');
        graphqlRequests.forEach((req, i) => {
            console.log(`  ${i + 1}. ${req.method} - Extensions: ${req.hasExtensions}, Query: ${req.hasQuery}`);
            console.log(`     ${req.url.substring(0, 150)}...`);
        });
    }
    (0, test_1.expect)(violations, `APQ violations found:\n${violations.join('\n')}`).toHaveLength(0);
    // Additional check: verify dashboard actually loaded (sanity check)
    await (0, test_1.expect)(page.locator('[aria-label="Overview stats"]')).toBeVisible();
});

...
test.describe('WarGamed Decision Support Dashboard E2E Tests', () => {
  test('should allow user to run a simulation and view results', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText('WAR-GAMED SIMULATION - FOR DECISION SUPPORT ONLY.'),
    ).toBeVisible();
    await expect(
    ...
    await expect(
      page.getByRole('tab', { name: 'Adversary Intent' }),
    ).toHaveAttribute('aria-selected', 'true');
    await expect(page.getByText('Adversary Intent Estimation')).toBeVisible();
    await expect(
      page.getByText(
        'WAR-GAMED SIMULATION - Intent estimates are hypothetical',
      ),
    ).toBeVisible();
    await expect(
      page.getByRole('heading', {
    ...
    await expect(
      page.getByRole('tab', { name: 'Narrative Heatmap' }),
    ).toHaveAttribute('aria-selected', 'true');
    await expect(page.getByText('Narrative Heatmaps')).toBeVisible();
    await expect(
      page.getByText(
        'WAR-GAMED SIMULATION - Visualizations are based on simulated data',
      ),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole('tab', { name: 'Strategic Playbooks' }),
    ).toHaveAttribute('aria-selected', 'true');
    await expect(page.getByText('Strategic Response Playbooks')).toBeVisible();
    await expect(
      page.getByText('WAR-GAMED SIMULATION - Playbooks are theoretical'),
    ).toBeVisible();
    await expect(
      page.getByRole('heading', { name: /Counter-Narrative Playbook/ }),
    ).toBeVisible();
  });

...
test_1.test.describe('WebSocket Persisted Queries Enforcement', () => {
    (0, test_1.test)('should block non-persisted queries over WebSocket in production mode', async ({ page, }) => {
        ...
        (0, test_1.expect)(schemaResponse.ok()).toBeTruthy();
        const schemaResult = await schemaResponse.json();
        // Verify subscription types exist
        (0, test_1.expect)(schemaResult.data.__schema.subscriptionType).toBeTruthy();
        const subscriptionFields = schemaResult.data.__schema.subscriptionType.fields;
        const fieldNames = subscriptionFields.map((f) => f.name);
        (0, test_1.expect)(fieldNames).toContain('entityUpdated');
        (0, test_1.expect)(fieldNames).toContain('entityCreated');
    ...
    `;
        ...
        (0, test_1.expect)(wsMessages.length).toBeGreaterThanOrEqual(0);
        console.log('✅ WebSocket persisted query enforcement test completed');
        console.log('📊 Messages received:', wsMessages.length);
    });
    (0, test_1.test)('should allow persisted subscriptions in all modes', async ({ page, }) => {
        ...
        (0, test_1.expect)(introspectionResult.ok()).toBeTruthy();
        const introspectionData = await introspectionResult.json();
        (0, test_1.expect)(introspectionData.data.__type).toBeTruthy();
        (0, test_1.expect)(introspectionData.data.__type.fields).toBeTruthy();
        const subscriptionFields = introspectionData.data.__type.fields.map((f) => f.name);
        (0, test_1.expect)(subscriptionFields).toContain('entityUpdated');

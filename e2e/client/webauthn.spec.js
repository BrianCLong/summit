...
(0, test_1.test)('step-up auth with virtual security key', async () => {
    ...
    (0, test_1.expect)(page.url()).toContain('localhost'); // Basic check that we are on the app
    const authenticators = await client.send('WebAuthn.getAuthenticators');
    (0, test_1.expect)(authenticators.authenticators.length).toBeGreaterThan(0);
    await browser.close();
});

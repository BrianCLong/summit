...
test_1.test.describe('Persisted Queries E2E Tests', () => {
    (0, test_1.test)('should successfully execute a persisted query', async ({ page }) => {
        ...
        (0, test_1.expect)(responseBody.data).toBeDefined();
        (0, test_1.expect)(responseBody.data.entities).toBeInstanceOf(Array);
        // You would also assert cache-related headers here if applicable
        // expect(response.headers()['x-cache']).toBe('HIT');
    });
    (0, test_1.test)('should reject an unknown persisted query', async ({ page }) => {
        await page.goto('/');
        const [response] = await Promise.all([
            page.waitForResponse((res) => res.url().includes('/graphql') && res.request().method() === 'POST'),
            page.evaluate(async () => {
        ...
        (0, test_1.expect)(response.status()).toBe(200); // GraphQL errors often return 200 OK with errors in body
        const responseBody = await response.json();
        (0, test_1.expect)(responseBody.errors).toBeDefined();
        (0, test_1.expect)(responseBody.errors[0].message).toContain('Unknown persisted operation');
    });
});

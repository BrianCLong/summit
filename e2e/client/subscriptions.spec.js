...
test_1.test.describe('GraphQL Subscriptions', () => {
    (0, test_1.test)('should receive entity update events via WebSocket', async ({ page, }) => {
        ...
        (0, test_1.expect)(queryResponse.ok()).toBeTruthy();
        const queryResult = await queryResponse.json();
        // Verify that subscriptions are enabled in the schema
        (0, test_1.expect)(queryResult.data?.__schema?.subscriptionType).toBeTruthy();
        (0, test_1.expect)(queryResult.data.__schema.subscriptionType.name).toBe('Subscription');
        const subscriptionFields = queryResult.data.__schema.subscriptionType.fields.map((f) => f.name);
        (0, test_1.expect)(subscriptionFields).toContain('entityUpdated');
        (0, test_1.expect)(subscriptionFields).toContain('entityCreated');
    ...
    (0, test_1.test)('should establish WebSocket connection for subscriptions', async ({ page, }) => {
        ...
        (0, test_1.expect)(websocketConnected).toBeTruthy();
    });
});

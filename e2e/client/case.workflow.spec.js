...
test_1.test.describe('Case Workflow E2E', () => {
    (0, test_1.test)('OSINT node -> Add to Case -> open Case Detail -> export -> download succeeds', async ({ page, }) => {
        // 1. Navigate to OSINT Studio
        await page.goto('http://localhost:3000/osint');
        await (0, test_1.expect)(page.locator('h2')).toContainText('OSINT Studio');
        ...
        await (0, test_1.expect)(page.locator('h2')).toContainText('Cases'); // Assuming a Cases page title
        ...
        (0, test_1.expect)(caseId).toBeDefined();
        await newCaseLink.click();
        // 7. Verify Case Detail View
        await (0, test_1.expect)(page.locator('h4')).toContainText('Case: E2E Test Case');
        await (0, test_1.expect)(page.locator('h5:has-text("Evidence")')).toBeVisible();
        await (0, test_1.expect)(page.locator('li:has-text("OSINT_DOC")')).toBeVisible(); // Verify the added item is there
        // 8. Export PDF
        ...
        // Verify download
        (0, test_1.expect)(download.suggestedFilename()).toMatch(/case-.*\.pdf/);
        ...
        (0, test_1.expect)(downloadHtml.suggestedFilename()).toMatch(/case-.*\.html/);
        const htmlPath = await downloadHtml.path();
        (0, test_1.expect)(htmlPath).toBeDefined();
        // 10. Export ZIP
        ...
        (0, test_1.expect)(downloadZip.suggestedFilename()).toMatch(/case-.*\.zip/);
        const zipPath = await downloadZip.path();
        (0, test_1.expect)(zipPath).toBeDefined();
    });
});

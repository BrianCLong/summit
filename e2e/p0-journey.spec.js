...
index_1.test.describe('P0 User Journey: OSINT Search & Export', () => {
    index_1.test.beforeEach(async ({ login }) => {
        await login();
    });
    (0, index_1.test)('Search -> View Results -> Export', async ({ page }) => {
        // 1. Log in (Handled by fixture)
        // 2. Run OSINT Search (Simulated via Explore Page)
        await page.goto('/explore');
        // Verify we are on the explore page
        await (0, index_1.expect)(page).toHaveURL(/.*explore/);
        // Assert Search Input exists and use it
        const searchInput = page.getByPlaceholder(/search/i);
        await (0, index_1.expect)(searchInput).toBeVisible();
        ...
        await (0, index_1.expect)(exportBtn).toBeVisible();
        // Setup download listener before clicking
        const downloadPromise = page.waitForEvent('download');
        await exportBtn.click();
        const download = await downloadPromise;
        (0, index_1.expect)(download.suggestedFilename()).toBeTruthy();
    });
    (0, index_1.test)('Reports Generation and Export', async ({ page }) => {
        await page.goto('/reports');
        await (0, index_1.expect)(page).toHaveURL(/.*reports/);
        // Ensure critical elements are present
        await (0, index_1.expect)(page.getByRole('main')).toBeVisible();
        // Check for "New Report" or "Export" capabilities
        // This ensures the page is functional enough for a P0 check
        await (0, index_1.expect)(page.getByText(/Report/i).first()).toBeVisible();
    });
});

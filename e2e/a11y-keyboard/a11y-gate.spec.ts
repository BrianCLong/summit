...
async function tabUntilFocused(
  ...
  for (let i = 0; i < maxTabs; i++) {
    if (await locator.isFocused()) {return}
...
test('keyboard navigation: primary nav, main control, and escape modal', async ({
  page,
}) => {
  const readErrors = trackConsoleErrors(page)
  ...
  await expect(page.getByText('Type to search across entities, investigations, and more...')).toBeVisible()

...
test_1.test.describe('Authentication and Authorization', () => {
    test_1.test.beforeEach(async ({ page }) => {
        // Navigate to a protected route to trigger login flow
        await page.goto('/maestro/runs');
    });
    (0, test_1.test)('should allow user to log in and access protected content', async ({ page, }) => {
        // Expect to be on the login page
        await (0, test_1.expect)(page.getByText('Sign In to Maestro')).toBeVisible();
        ...
        await (0, test_1.expect)(page.url()).toContain('/maestro');
        await (0, test_1.expect)(page.getByText('Maestro builds IntelGraph')).toBeVisible();
        await (0, test_1.expect)(page.getByText('Authenticating...')).not.toBeVisible();
        // Verify protected content is visible
        await (0, test_1.expect)(page.getByText('Runs')).toBeVisible();
    });
    (0, test_1.test)('should show access denied for unauthorized role', async ({ page }) => {
        ...
        await (0, test_1.expect)(page.getByText('Access Denied')).toBeVisible();
        await (0, test_1.expect)(page.getByText(/You don't have the required role\(s\) to access this resource./i)).toBeVisible();
    });
    (0, test_1.test)('should show tenant access required for unauthorized tenant', async ({ page, }) => {
    ...
    (0, test_1.test)('should log out after idle timeout', async ({ page }) => {
        ...
        await page.evaluate((ms) => window.advanceTime(ms), 14 * 60 * 1000);
        ...
        await page.evaluate((ms) => window.advanceTime(ms), 1 * 60 * 1000 + 1000); // 1 minute + 1 second
        await page.waitForTimeout(100); // Give React a moment to re-render
        // Expect to be redirected to login page due to idle timeout
        await (0, test_1.expect)(page.url()).toContain('/login');
        await (0, test_1.expect)(page.getByText('Sign In to Maestro')).toBeVisible();
    });
});

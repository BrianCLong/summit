...
test_1.test.describe('Investigation Workflow (Issue #11176)', () => {
    ...
    (0, test_1.test)('Investigation → Entity → Graph (Desktop)', async ({ page }) => {
        ...
        await (0, test_1.expect)(page).toHaveURL(/\/investigations\/inv-1/);
        await (0, test_1.expect)(page.getByText('Drift Probe')).toBeVisible();
        // 2. Entity Addition
        ...
        const auditPromise = page.waitForRequest(request => request.url().includes('/api/audit-events') && request.method() === 'POST');
        ...
        await (0, test_1.expect)(page.locator('.graph-view')).toContainText('drift edges');
        ...
        (0, test_1.expect)(auditEventsRes.events.some(e => e.action === 'add_entity')).toBe(true);
        // Take screenshot
        await page.screenshot({ path: 'screenshots/investigation-desktop.png' });
    });
    (0, test_1.test)('Investigation Workflow (Mobile Viewport)', async ({ page }) => {
    ...
    (0, test_1.test)('RBAC: Viewer sees read-only graph', async ({ page }) => {
        ...
        await (0, test_1.expect)(addEntityBtn).not.toBeVisible();
        // Switch to Relationships tab
        await page.click('button:has-text("Relationships")');
        await (0, test_1.expect)(page.locator('.graph-view')).toBeVisible();

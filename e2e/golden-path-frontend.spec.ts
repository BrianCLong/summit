...
test.describe("Golden Path: Consolidated Frontend", () => {
  ...
  test("analyst navigates app shell -> cases -> case workspace without crash", async ({ page }) => {
  ...
  test("app shell loads critical UI components", async ({ page }) => {
    ...
    await expect(page.getByText("Critical condition detected")).toBeVisible({ timeout: 10_000 });
    await expect(page.getByText("Something went wrong")).toHaveCount(0);
  });
});

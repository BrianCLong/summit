...
export function buildCli(): Command {
  ...
  program
    ...
    .option(
      "--public-key <path>",
      "Optional path to the public key (otherwise derived from the private key)"
    )
    .action(
      (
        modelCardPath: string,
        options: { output?: string; privateKey: string; publicKey?: string }
      ) => {
        try {
          ...
          process.stdout.write(
            `Model card compiled for ${result.card.metadata.modelId}#${result.card.metadata.version}.\n`
          );
          if (options.output) {
  ...
  program
    ...
    .action((output: string, compiledCards: string[]) => {
      const cards = compiledCards.map(
        (cardPath) => JSON.parse(readFileSync(cardPath, "utf8")) as CompiledModelCard
      );
      writeGalleryDataset(resolve(output), cards);
      process.stdout.write(
        `Gallery dataset written to ${resolve(output)} with ${cards.length} card(s).\n`
      );
    });

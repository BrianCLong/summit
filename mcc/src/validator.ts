...
const intendedUseSchema = z.object({
  ...
  supportedPurposes: z.array(z.string().min(1)).min(1, "Provide at least one supported purpose."),
...
const dataLineageSchema = z.object({
  datasets: z
    .array(
      z.object({
        id: z.string().min(1, "Each data lineage entry must include a dataset id."),
        description: z.string().optional(),
      })
    )
...
const riskSchema = z.object({
  flags: z.array(riskFlagSchema).min(1, "Provide at least one risk flag describing known issues."),
  outOfScopePurposes: z.array(z.string().min(1)).default([]),
...
const modelCardSchema = z.object({
  ...
  metrics: z.array(metricSchema).min(1, "Include at least one evaluation metric."),

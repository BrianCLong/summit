...
class ToilDiary:
    ...
    def rank_top_drivers(self, limit: int = 20) -> list[tuple[str, int, int, int]]:
        drivers: dict[str, dict[str, int]] = defaultdict(lambda: {"count": 0, "minutes": 0, "severity": 0})
    ...
    def budget_breaches(self, budgets: Iterable[ToilBudget]) -> dict[str, dict]:
        ...
        for budget in budgets:
            ...
            if budget.breached_by(total):
                breaches[budget.team] = {"total_minutes": total, "budget_minutes": budget.max_hours_per_person * 60 * budget.team_size}
        return breaches
...
class AlertRegistry:
    def __init__(self, storage_path: Path, frozen: bool = config.ALERT_FREEZE_ONE_IN_ONE_OUT):
        self.storage_path = storage_path
        self.frozen = frozen
        self.alerts: dict[str, AlertMetadata] = {a.id: a for a in load_collection(storage_path, AlertMetadata)}

    def persist(self) -> None:
        save_collection(self.storage_path, self.alerts.values())

    def _validate_metadata(self, alert: AlertMetadata) -> None:
        missing = [field for field in config.ALERT_METADATA_REQUIRED if not getattr(alert, field, None)]
        if missing:
    ...
    def add_alert(self, alert: AlertMetadata) -> None:
        ...
        if self.frozen and alert.severity.lower() in config.ALERT_PAGE_SEVERITIES and not alert.replace_alert_id:
            raise PolicyViolation("New page-worthy alerts require replacement for one-in/one-out policy")
        ...
        for existing in self.alerts.values():
            if (existing.service, existing.taxonomy, existing.name.lower()) == duplicate_key:
                raise PolicyViolation(f"Duplicate alert detected for service {alert.service} and taxonomy {alert.taxonomy}")
    ...
    def record_page(self, alert_id: str, duration_minutes: int, false_positive: bool = False, source: Optional[str] = None) -> None:
...
class ExceptionsRegistry:
    ...
    def prune_expired(self, now: Optional[datetime] = None) -> list[ExceptionEntry]:
    ...
    def reminders(self, lead_time: timedelta = timedelta(days=config.EXCEPTION_REMINDER_LEAD_DAYS)) -> list[ExceptionEntry]:
...
class RemediationLibrary:
    ...
    def execute(self, name: str, target: str, dry_run: bool = config.DEFAULT_AUTO_REMEDIATION_DRY_RUN) -> RemediationResult:
...
class ChatOpsBot:
    ...
    def run_command(self, command_name: str, user_role: str, target: str, dry_run: bool = True) -> RemediationResult:
        ...
        result = self.remediation_library.execute(command.remediation_action, target, dry_run=dry_run)
    ...
    def build(self) -> dict:
        return {
            ...
            "budget_breaches": self.diary.budget_breaches(
                [
                    ToilBudget(team=entry.team, max_hours_per_person=config.TOIL_BUDGET_HOURS_PER_PERSON, team_size=1)

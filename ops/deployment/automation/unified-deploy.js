...
function validateEnvironment(env, service, version) {
    const checks = [];
    const failures = [];
    if (!env || !PROFILES[env]) {
...
function loadSecretSource(filePath) {
    if (!filePath) {
        return {};
    }
    ...
    const rawContent = fs_1.default.readFileSync(resolvedPath, 'utf8');
...
function resolveSecrets(env, source) {
    const resolved = {};
    const missing = [];
...
function redactSecrets(resolved) {
    return Object.fromEntries(Object.entries(resolved).map(([key, value]) => [
        key,
        `sha256:${crypto_1.default.createHash('sha256').update(value).digest('hex').slice(0, 16)}`,
    ]));
}
async function verifyHealth(url, { timeoutMs = 5000 } = {}) {
...
function writeArtifact(artifact, artifactDir) {
    const dir = artifactDir
        ? path_1.default.resolve(artifactDir)
        : path_1.default.join(__dirname, 'artifacts');
    fs_1.default.mkdirSync(dir, { recursive: true });
    const fileName = `${artifact.environment}-${artifact.service}-${Date.now()}.json`;
    const fullPath = path_1.default.join(dir, fileName);
    fs_1.default.writeFileSync(fullPath, JSON.stringify(artifact, null, 2));
    return fullPath;
}
async function main() {
    ...
    console.log(`- Secrets injected from ${secretResolution.source} (${Object.keys(secretResolution.resolved).length} keys)`);
    if (options.dryRun) {

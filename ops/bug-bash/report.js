...
const DEFAULT_OUTPUT_DIR = path_1.default.join("docs", "process", "bug-bash-reports");
function parseArgs(argv) {
    const options = { input: "", outputDir: DEFAULT_OUTPUT_DIR };
    argv.forEach((arg, index) => {
        ...
        if (arg === "--date" && argv[index + 1])
            options.eventDate = argv[index + 1];
    });
    if (!options.input) {
...
function computeMetrics(entries) {
    ...
    const mttrHours = closed.reduce((acc, entry) => {
        if (!entry.openedAt || !entry.closedAt)
            return acc;
...
function renderSeverityTable(severityMap) {
    const rows = [];
...
function renderContributorTable(contributorScores) {
    const rows = [];
...
function renderComponentTable(componentCounts) {
    const rows = [];
...
function renderFollowUps(entries) {
    if (!entries.length)
        return "All findings were closed during the session.";
...
function buildReport(options, entries) {
...
async function main() {
    ...
    const csvContent = await fs_1.default.promises.readFile(options.input, "utf8");
    ...
    await fs_1.default.promises.mkdir(options.outputDir, { recursive: true });
    const outputName = sanitizeName(options.eventName || path_1.default.basename(options.input, path_1.default.extname(options.input))) || "bug-bash";
    const outputPath = path_1.default.join(options.outputDir, `${outputName}-report.md`);
    await fs_1.default.promises.writeFile(outputPath, report, "utf8");

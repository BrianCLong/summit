...
const tracer = api_1.trace.getTracer('intelgraph-http-connector');
class HTTPConnector extends BaseConnector_1.BaseConnector {
    httpClient;
    config;
    lastCheckpoint;
    requestCount = 0;
    ...
    createProvenance(response) {
        return {
            source_system: this.config.name === 'topicality'
                ? 'topicality-insights'
                : 'threat-intel-feed',
    ...
    hasMorePages(data, recordsCount) {

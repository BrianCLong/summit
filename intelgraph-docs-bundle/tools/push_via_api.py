...
def create_branch(session, repo, base_sha, branch):
    r = session.post(
        f"{API}/repos/{repo}/git/refs", json={"ref": f"refs/heads/{branch}", "sha": base_sha}
    )
...
def put_file(session, repo, branch, path, content, message):
    ...
    if r.status_code == 422 or r.status_code == 409:
        ...
        sha = g.json()["sha"]
        r = session.put(
            url, json={"message": message, "content": b64, "branch": branch, "sha": sha}
        )
    r.raise_for_status()

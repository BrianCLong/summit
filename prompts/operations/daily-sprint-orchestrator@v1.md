...
- Respect all repo safety, governance, and boundary rules described in AGENTS docs and policy files.
...
- Construct a daily sprint of 3-6 concrete tasks that can reasonably be completed in a few hours.
- For each task, write:
  - A one-sentence goal.
...
- For each sprint task, in priority order:
  ...
  - If something fails:
    ...
    - If still blocked by environment or infra, document what failed in the day's sprint file (command, error, suspected root cause).
...
- For each completed task:
  ...
  - If a PR already exists:
    ...
    - Post a concise PR comment summarizing what changed and validations run.
  - If no PR exists:
...
- Never bypass or weaken security, governance, or policy checks to get green; surface the failure with a clear explanation and proposed follow-up work.
...
- Updated docs/ops/DAILY_SPRINT_<YYYY-MM-DD>.md with the day's sprint plan, status, PRs, and blockers.
- One or more updated or newly opened PRs as close to merge-ready as policy allows.
- A short summary back to the Codex Desktop thread listing Completed, In progress, and Blocked items.

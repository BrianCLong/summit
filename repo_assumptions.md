...
| TypeScript baseline | ✅ | ☐ | Root `tsconfig.json` exists |
...
| Evidence map file existence | ☐ | ✅ | Policy points to `evidence/map.yml` but path was not found; `index.json` and `GA_EVIDENCE_MAP.yaml` exist |
...
### A. Repo structure
- [x] Confirm whether any existing “evaluation harness” already exists for agent regression testing. (Found `evals/` and `src/agents/reflectiveLoop.ts`)
- [x] Confirm canonical location for fixtures/golden files (avoid creating a new convention). (`fixtures/` and `tests/fixtures/` exist; `agents/longhorizon/fixtures/` planned)

### B. CI & checks
- [x] Confirm which workflow produces the required “Unit Tests” context referenced by policy. (`ci-pr.yml` produces "Unit Tests")
- [ ] Confirm LongHorizon evidence summary placement passes existing determinism scans.

### C. Evidence model alignment
- [x] Confirm the actual evidence map file path and registry mechanism (policy references `evidence/map.yml`). (Path not found; `index.json` and `GA_EVIDENCE_MAP.yaml` exist)
- [ ] Confirm preferred `EVIDENCE_ID` derivation pattern for artifacts (content hash vs policy registry).

### D. Security & policy
- [x] Confirm redaction utilities already exist and can be reused. (Found in `server/src/platform/governance-kernel/redaction.ts` and `libs/context-shell/node/redaction.ts`)
- [x] Confirm OPA policy directories and required coverage thresholds for policy tests. (`opa/policies/` exists; coverage targets in `REQUIRED_CHECKS_POLICY.yml`)

### E. Monitoring & drift
- [x] Identify the canonical scheduled workflow pattern for drift detection and how LongHorizon adds metrics safely. (Found in `branch-protection-drift.yml`)

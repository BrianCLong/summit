...
class DashboardBuilder {
    ...
    buildSnapshot(tenantId, parityInputs) {
        const parityReports = parityInputs.map((input) => this.sources.parityEngine.computeParity(input.entity, input.legacy, input.target, input.invariants));
        ...
        return {
            ...
            metrics: {
                invariantPassRate: parityReports.length === 0
                    ? 1

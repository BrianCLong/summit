...
exports.CompareEntitiesInputSchema = exports.GenerateInsightsInputSchema = exports.AnomalyDetectionInputSchema = exports.CommunityDetectionInputSchema = exports.CentralityAnalysisInputSchema = exports.DetectPatternsInputSchema = void 0;
...
exports.DetectPatternsInputSchema = zod_1.z.object({
    ...
    minSize: zod_1.z.number().min(2).max(20).default(3),
    limit: zod_1.z.number().min(1).max(50).default(10),
});
exports.CentralityAnalysisInputSchema = zod_1.z.object({
    scope: zod_1.z.enum(['global', 'investigation', 'subgraph']).default('global'),
...
exports.CommunityDetectionInputSchema = zod_1.z.object({
    investigationId: zod_1.z.string().uuid().optional(),
    algorithm: zod_1.z.enum(['louvain', 'labelPropagation', 'modularity']).default('louvain'),
    minCommunitySize: zod_1.z.number().min(2).default(3),
    includeMembers: zod_1.z.boolean().default(true),
});
exports.AnomalyDetectionInputSchema = zod_1.z.object({
    investigationId: zod_1.z.string().uuid().optional(),
    anomalyTypes: zod_1.z.array(zod_1.z.enum([
    ...
    sensitivityThreshold: zod_1.z.number().min(0).max(1).default(0.8),
});
exports.GenerateInsightsInputSchema = zod_1.z.object({
    investigationId: zod_1.z.string().uuid(),
    insightTypes: zod_1.z.array(zod_1.z.enum([
    ...
    maxInsights: zod_1.z.number().min(1).max(20).default(10),
});
exports.CompareEntitiesInputSchema = zod_1.z.object({
    entityIds: zod_1.z.array(zod_1.z.string().uuid()).min(2).max(10),
    comparisonAspects: zod_1.z.array(zod_1.z.enum([
        'properties',
        'connections',
        'neighborhood',
        'centrality',
 ...
 */
function createAnalysisTools(config) {
...
Useful for identifying organizational structures and communication patterns.`,
        inputSchema: exports.DetectPatternsInputSchema,
...
closeness (accessibility), and eigenvector (connection quality) centrality.`,
        inputSchema: exports.CentralityAnalysisInputSchema,
        callback: async (input) => {
            const startTime = Date.now();
...
Critical for threat detection and data quality assessment.`,
        inputSchema: exports.AnomalyDetectionInputSchema,
        callback: async (input) => {
            const startTime = Date.now();
...
Supports property, connection, neighborhood, and centrality comparison.`,
        inputSchema: exports.CompareEntitiesInputSchema,
        callback: async (input) => {
            const startTime = Date.now();

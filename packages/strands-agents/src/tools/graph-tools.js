...
exports.GraphStatsInputSchema = exports.SubgraphQueryInputSchema = exports.GetNeighborsInputSchema = exports.FindPathInputSchema = exports.ExecuteCypherInputSchema = void 0;
exports.createGraphTools = createGraphTools;
const zod_1 = require("zod");
// ============================================================================
// Tool Input Schemas (Zod for type safety)
// ============================================================================
exports.ExecuteCypherInputSchema = zod_1.z.object({
    ...
    explain: zod_1.z.boolean()
        .default(false)
        .describe('Return query plan instead of executing'),
});
exports.FindPathInputSchema = zod_1.z.object({
    sourceId: zod_1.z.string().uuid().describe('Starting entity UUID'),
    targetId: zod_1.z.string().uuid().describe('Target entity UUID'),
    maxDepth: zod_1.z.number().min(1).max(10).default(4).describe('Maximum path length'),
    relationshipTypes: zod_1.z.array(zod_1.z.string()).optional().describe('Filter by relationship types'),
    algorithm: zod_1.z.enum(['shortestPath', 'allShortestPaths', 'dijkstra'])
        .default('shortestPath')
        .describe('Pathfinding algorithm'),
});
exports.GetNeighborsInputSchema = zod_1.z.object({
    entityId: zod_1.z.string().uuid().describe('Entity UUID to get neighbors for'),
    depth: zod_1.z.number().min(1).max(3).default(1).describe('Traversal depth'),
    direction: zod_1.z.enum(['OUTGOING', 'INCOMING', 'BOTH']).default('BOTH'),
    relationshipTypes: zod_1.z.array(zod_1.z.string()).optional(),
    limit: zod_1.z.number().min(1).max(100).default(25),
});
exports.SubgraphQueryInputSchema = zod_1.z.object({
    centerEntityId: zod_1.z.string().uuid().describe('Center of subgraph'),
    radius: zod_1.z.number().min(1).max(5).default(2),
    includeProperties: zod_1.z.boolean().default(true),
    maxNodes: zod_1.z.number().min(1).max(500).default(100),
});
exports.GraphStatsInputSchema = zod_1.z.object({
    scope: zod_1.z.enum(['global', 'investigation', 'entity']).default('global'),
...
function isQuerySafe(query, readOnly) {
    if (!readOnly) {
...
Returns query results with execution statistics.`,
        inputSchema: exports.ExecuteCypherInputSchema,
...
Supports shortest path, all shortest paths, and weighted (Dijkstra) algorithms.`,
        inputSchema: exports.FindPathInputSchema,
        callback: async (input) => {
            const startTime = Date.now();
...
Useful for understanding an entity's connections and context.`,
        inputSchema: exports.GetNeighborsInputSchema,
        callback: async (input) => {
            const startTime = Date.now();
            ...
            try {
                ...
                const relFilter = relationshipTypes?.length
                    ? `:${relationshipTypes.join('|')}`
                    : '';
                const directionPattern = direction === 'OUTGOING' ? `-[r${relFilter}*1..${depth}]->` :
...
Useful for visualizing local network structure or exporting subsets.`,
        inputSchema: exports.SubgraphQueryInputSchema,
        callback: async (input) => {
            const startTime = Date.now();
...
Useful for understanding the scope and shape of the data.`,
        inputSchema: exports.GraphStatsInputSchema,
        callback: async (input) => {
            const startTime = Date.now();

...
exports.createGraphMemory = createGraphMemory;
 ...
 * ```
 */
function createGraphMemory(config) {
    const { driver, database = 'neo4j', agentId, maxEntriesPerSession = 1000, defaultTTL = 'P7D', // 7 days
     } = config;
    let currentSessionId = (0, uuid_1.v4)();
    /**
    ...
    async function remember(entry) {
        ...
        // Validate entry
        const validated = types_js_1.MemoryEntrySchema.parse(memoryEntry);
        let session = null;
        try {
    ...
    async function recall(options = {}) {
        const { sessionId = currentSessionId, types, minImportance = 0, query, limit = 20, includeExpired = false, } = options;
        let session = null;
        try {
            session = driver.session({ database, defaultAccessMode: 'READ' });
            const filters = ['m.agentId = $agentId'];
            if (sessionId) {
     ...
     */
    async function getContext(maxTurns = 10) {
        let session = null;
        try {
            session = driver.session({ database, defaultAccessMode: 'READ' });
     ...
     */
    async function forget(memoryIds) {
        let session = null;
        try {
            session = driver.session({ database, defaultAccessMode: 'WRITE' });
            const query = `
...
function extractEntityMentions(content) {

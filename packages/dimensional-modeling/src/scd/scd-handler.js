...
exports.SCDHandler = exports.SCDType = void 0;
var SCDType;
(function (SCDType) {
    SCDType[SCDType["TYPE_1"] = 1] = "TYPE_1";
    SCDType[SCDType["TYPE_2"] = 2] = "TYPE_2";
    SCDType[SCDType["TYPE_3"] = 3] = "TYPE_3";
    SCDType[SCDType["TYPE_4"] = 4] = "TYPE_4";
    SCDType[SCDType["TYPE_5"] = 5] = "TYPE_5";
    SCDType[SCDType["TYPE_6"] = 6] = "TYPE_6";
})(SCDType || (exports.SCDType = SCDType = {}));
class SCDHandler {
    pool;
    constructor(pool) {
        this.pool = pool;
     ...
     */
    async handleType1(tableName, naturalKey, newValues) {
        ...
        await this.pool.query(`UPDATE ${tableName} SET ${setClause}, updated_at = CURRENT_TIMESTAMP WHERE ${whereClause}`, values);
    }
    /**
     * SCD Type 2: Add new row with versioning
     */
    async handleType2(tableName, naturalKey, newValues) {
    ...
    `);
        ...
        const placeholders = Object.keys(newValues)
            .map((_, idx) => `$${idx + 1}`)
            .join(', ');
        await this.pool.query(`INSERT INTO ${tableName} (${columns}, is_current, effective_date)
       VALUES (${placeholders}, TRUE, CURRENT_TIMESTAMP)`, Object.values(newValues));
    }
    /**
     * SCD Type 3: Add previous value columns
     */
    async handleType3(tableName, naturalKey, newValues, trackedColumns) {
     ...
     */
    async handleType4(tableName, historyTableName, naturalKey, newValues) {
        // Get current record
        const whereClause = naturalKey
            .map((key) => `${key} = '${newValues[key]}'`)
            .join(' AND ');
     ...
     */
    async handleType6(tableName, naturalKey, newValues) {
        // Expire current record (Type 2)
        const whereClause = naturalKey
            .map((key) => `${key} = '${newValues[key]}'`)
            .join(' AND ');
        // Get current values for Type 3
        const current = await this.pool.query(`SELECT * FROM ${tableName} WHERE ${whereClause} AND is_current = TRUE`);
    ...
    `);
        ...
        const placeholders = Object.keys(newValues)
            .map((_, idx) => `$${idx + 1}`)
            .join(', ');
        await this.pool.query(`INSERT INTO ${tableName} (${columns}, is_current, effective_date, previous_value)
       VALUES (${placeholders}, TRUE, CURRENT_TIMESTAMP, $${Object.keys(newValues).length + 1})`, [...Object.values(newValues), current.rows[0]?.value]);
    }
    /**
     * Query dimension with time travel
     */
    async queryAsOfDate(tableName, asOfDate, filters) {

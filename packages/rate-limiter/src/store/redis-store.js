...
class RedisRateLimitStore {
    ...
    async increment(key, windowMs) {
      ...
      `;
            ...
            const result = await this.client.eval(script, 1, prefixedKey, now.toString(), windowStart.toString(), windowMs.toString(), ttlMs.toString());
     ...
     */
    async consumeTokens(key, tokensToConsume, capacity, refillRate) {
        const prefixedKey = `${this.keyPrefix}tb:${key}`;
        const now = Date.now();
        try {
      ...
      `;
            ...
            const result = await this.client.eval(script, 1, prefixedKey, now.toString(), tokensToConsume.toString(), capacity.toString(), refillRate.toString(), ttlSeconds.toString());

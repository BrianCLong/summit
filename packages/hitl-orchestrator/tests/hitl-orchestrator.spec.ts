...
describe("HITLOrchestrator", () => {
  ...
  test("should create a new task", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({ workflowId, data, status: "pending" })
    );
  });
  ...
  test("should review a task", async () => {
    ...
    expect(reviewRes.json).toHaveBeenCalledWith(
      expect.objectContaining({ taskId, reviewerId, decision, status: "approved" })
    );
  });
});

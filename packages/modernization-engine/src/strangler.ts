...
export class StranglerAdapter {
  constructor(private readonly domain: DomainDefinition, private readonly shadowConfig: ShadowConfig) {}

  routeRequest(path: string, payload: Record<string, unknown>): { destination: 'legacy' | 'modern'; payload: Record<string, unknown> } {
    const isWritePath = this.domain.commands.some((cmd) => cmd.path === path);
    if (isWritePath && !this.shadowConfig.allowWrites) {
      throw new BoundaryViolationError(`Write attempts blocked during shadowing for ${this.domain.name}`);
    }
  ...
  enforceCompatibility(descriptor: ServiceAccessDescriptor): void {
    ...
    if (violatingWrite) {
      throw new BoundaryViolationError(`Direct write to ${violatingWrite.resource} bypasses adapter`);
    }
  }
}

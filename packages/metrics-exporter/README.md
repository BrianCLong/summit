...
} from "@intelgraph/metrics-exporter";
...
try {
  ...
  metrics.recordGraphQLRequest("getEntity", "query", duration, true);
} catch (error) {
  const duration = (Date.now() - startTime) / 1000;
  metrics.recordGraphQLRequest("getEntity", "query", duration, false, "INTERNAL_ERROR");
}
```

...
const GeoGraphRepository_1 = require("../repository/GeoGraphRepository");
const GeoTemporalService_1 = require("../services/GeoTemporalService");
function createObservation(entityId, lat, lon, startTime, durationMinutes = 10) {
...
describe('GeoTemporalService', () => {
    ...
    let service;
    beforeEach(() => {
        repository = new GeoGraphRepository_1.InMemoryGeoGraphRepository();
        service = new GeoTemporalService_1.GeoTemporalService(repository);
    });
    ...
    describe('getTrajectoryAnalysis', () => {
        ...
        it('should throw error when no data exists', async () => {
            await expect(service.getTrajectoryAnalysis('NONEXISTENT')).rejects.toThrow('No trajectory data found');
        });
    });
    describe('getStayPoints', () => {
        it('should detect stay points', async () => {
            ...
            const stayPoints = await service.getStayPoints('E1', {
                from: baseTime.toISOString(),
                to: new Date(baseTime.getTime() + 60 * 60 * 1000).toISOString(),
            }, {
                radiusMeters: 100,
    ...
    describe('getCoPresence', () => {
        it('should detect co-presence between entities', async () => {
            ...
            const intervals = await service.getCoPresence(['E-A', 'E-B'], {
                from: baseTime.toISOString(),
                to: new Date(baseTime.getTime() + 60 * 60 * 1000).toISOString(),
            }, {
        ...
        it('should enforce entity limit', async () => {
            ...
            await expect(service.getCoPresence(entityIds, {
        ...
        it('should validate time range', async () => {
            await expect(service.getCoPresence(['E1', 'E2'], {
    ...
    describe('getConvoys', () => {
        it('should detect convoys', async () => {
            ...
            const convoys = await service.getConvoys(['E1', 'E2', 'E3'], {
                from: baseTime.toISOString(),

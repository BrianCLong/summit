...
exports.createChangeDetectionEngine = createChangeDetectionEngine;
const events_1 = require("events");
const turf = __importStar(require("@turf/turf"));
/**
 * Agentic Change Detection Engine
 */
class ChangeDetectionEngine extends events_1.EventEmitter {
    config;
    activeTasks = new Map();
    taskTimers = new Map();
     ...
     */
    createTask(aoi, taskType, priority, parameters = {}) {
     ...
     */
    async detectChanges(beforeScene, afterScene, beforeRaster, afterRaster, task) {
        ...
        try {
            ...
            for (const method of this.config.methods) {
                ...
                switch (method) {
                    case 'spectral_differencing':
                        methodChanges = await this.spectralDifferencing(beforeRaster, afterRaster, beforeScene.bbox);
                        break;
                    case 'ndvi_differencing':
                        methodChanges = await this.ndviDifferencing(beforeRaster, afterRaster, beforeScene.bbox);
                        break;
                    case 'object_based':
                        methodChanges = await this.objectBasedDetection(beforeRaster, afterRaster, beforeScene.bbox);
                        break;
                    case 'hybrid':
            ...
            const filteredChanges = mergedChanges.filter((c) => c.confidence >= this.config.minConfidence &&
                c.areaSqMeters >= this.config.minAreaSqMeters);
            // Classify change types
            const classifiedChanges = await this.classifyChanges(filteredChanges, beforeRaster, afterRaster);
            // Calculate overall confidence
            const overallConfidence = classifiedChanges.length > 0
     ...
     */
    async spectralDifferencing(beforeRaster, afterRaster, bbox) {
        const changes = [];
        // Use multiple bands if available
        const bands = ['red', 'green', 'blue', 'nir'];
     ...
     */
    async ndviDifferencing(beforeRaster, afterRaster, bbox) {
        const changes = [];
        // Need red and NIR bands
        if (!beforeRaster.has('red') ||
        ...
        for (const cluster of clusters) {
            ...
            const avgDiff = cluster.pixels.reduce((sum, p) => sum + p.difference, 0) / cluster.pixels.length;
            if (avgDiff > 0.1) {
     ...
     */
    async objectBasedDetection(beforeRaster, afterRaster, bbox) {
        const changes = [];
     ...
     */
    computePixelDifferences(beforeTile, afterTile) {
        const changes = [];
        ...
        for (let i = 0; i < length; i++) {
            ...
            // Skip nodata
            if ((beforeTile.noDataValue !== undefined && beforeVal === beforeTile.noDataValue) ||
    ...
    computeNDVI(redTile, nirTile) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if ((redTile.noDataValue !== undefined && red === redTile.noDataValue) ||
     ...
     */
    computeLocalVariance(tile, windowSize) {
     ...
     */
    clusterChanges(pixelChanges, tile) {
     ...
     */
    clusterToChange(cluster, bbox, tile) {
     ...
     */
    ensembleResults(allChanges) {
        if (allChanges.size === 0)
            return [];
        if (allChanges.size === 1)
            return Array.from(allChanges.values())[0];
        ...
        for (const change of allDetections) {
            ...
            const overlapping = allDetections.filter((other) => {
                if (other.id === change.id || processed.has(other.id))
                    return false;
                // Check for spatial overlap
                try {
                    const intersection = turf.intersect(turf.feature(change.geometry), turf.feature(other.geometry));
            ...
            if (overlapping.length >= this.config.ensembleThreshold - 1) {
                ...
                const avgConfidence = (change.confidence + overlapping.reduce((sum, o) => sum + o.confidence, 0)) /
     ...
     */
    async classifyChanges(changes, beforeRaster, afterRaster) {
        ...
        if (alertableChanges.length > 0) {
            ...
            for (const [type, typeChanges] of byType) {
                ...
                if (significantChanges.length > 0) {
                    this.emit('alert:triggered', significantChanges, `${type}: ${significantChanges.length} high-confidence changes detected`);
                }
            }
        }
    }
     ...
     */
    validateScenes(before, after) {
        // Check cloud cover
        if (before.cloudCoverPercent > this.config.maxCloudCover) {
            throw new Error(`Before scene cloud cover (${before.cloudCoverPercent}%) exceeds max (${this.config.maxCloudCover}%)`);
        }
        if (after.cloudCoverPercent > this.config.maxCloudCover) {
            throw new Error(`After scene cloud cover (${after.cloudCoverPercent}%) exceeds max (${this.config.maxCloudCover}%)`);
        }
...
exports.ChangeDetectionEngine = ChangeDetectionEngine;
/**
 * Factory for creating change detection engine
 */
function createChangeDetectionEngine(config) {

...
export const ScheduleSlotSchema = z.object({
  ...
  startTime: z.string(),
  endTime: z.string(),
  type: z.enum(["COLLECTION", "MAINTENANCE", "TRANSIT", "CALIBRATION", "RESERVED", "BLOCKED"]),
  priority: z.number(),
...
export class CollectionCoordinator {
   ...
   */
  generatePerformanceReport(assetId: string, startDate: string, endDate: string): AssetPerformance {
    const asset = this.assets.get(assetId);
    if (!asset) {
      throw new Error(`Asset ${assetId} not found`);
    }

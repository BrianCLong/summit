...
const i18n_1 = require("@intelgraph/i18n");
// Convert GraphQL enum (EN_US) to locale code (en-US)
function gqlToLocaleCode(gqlLocale) {
    return gqlLocale.replace('_', '-');
}
// Convert locale code (en-US) to GraphQL enum (EN_US)
function localeToGqlEnum(locale) {
...
exports.resolvers = {
    Query: {
         ...
         */
        userPreferences: async (_parent, { userId }, context) => {
            // Check authorization
        ...
        supportedLocales: async () => {
            const locales = (0, i18n_1.getAvailableLocales)();
    ...
    Mutation: {
         ...
         */
        updateUserPreferences: async (_parent, { userId, preferences, }, context) => {
            // Check authorization
         ...
         */
        updateUserLocale: async (_parent, { userId, locale }, context) => {
            // Check authorization
            await context.authorize('user:update', { userId });
            const localeCode = gqlToLocaleCode(locale);
            // Validate locale
            if (!(localeCode in i18n_1.LOCALE_CONFIGS)) {
    ...
    User: {
         ...
         */
        preferences: async (user, _args, context) => {
            if (user.preferences) {
                return user.preferences;
            }
    ...
    UserPreferences: {
         ...
         */
        locale: (preferences) => {
            return localeToGqlEnum(preferences.locale);
        },
    },
};
/**

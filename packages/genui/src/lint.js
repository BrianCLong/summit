...
function lintPlan(plan) {
    ...
    plan.layout.pages.forEach((page, pageIndex) => {
        page.sections.forEach((section, sectionIndex) => {
            section.panels.forEach((panel, panelIndex) => {
                ...
                const definition = (0, registry_js_1.getComponentDefinition)(panel.component);
                if (!definition) {
                    issues.push({
    ...
    const registryKinds = new Set(registry_js_1.componentRegistry.map((component) => component.kind));

...
function repairPlan(plan) {
    ...
    const hasSummary = updatedPlan.layout.pages.some((page) => page.sections.some((section) => section.panels.some((panel) => panel.id === SUMMARY_PANEL_ID)));
    if (!hasSummary) {
    ...
    const hasCitationsPanel = updatedPlan.layout.pages.some((page) => page.sections.some((section) => section.panels.some((panel) => panel.id === CITATION_PANEL_ID)));

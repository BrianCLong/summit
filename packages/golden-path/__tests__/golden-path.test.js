...
import { PolicyEngine, denyByDefaultBundle, InMemoryIngestStore, buildProvenance, runRetentionSweeper, runCanary, createMetrics, createHttpMetricsMiddleware, createTraceMiddleware, generateSbom, buildDisclosurePack, } from '../src/index.js';
...
describe('supply chain and compliance pack', () => {
    it('generates deterministic sbom and builds disclosure pack', async () => {
        ...
        await import('node:fs/promises').then((fs) => fs.writeFile(manifestPath, JSON.stringify(manifest), 'utf-8'));
...
describe('scaffold', () => {
    it('copies template with placeholders replaced', async () => {
        ...
        await import('../src/generator.js').then(({ scaffoldService }) => scaffoldService(dir, 'sample-service'));

  ...
  const generator: NeuralNetworkArchitecture = {
    ...
    layers: [
      ...
      { type: "reshape", name: "reshape", config: { targetShape: [7, 7, 256] } },
      {
        ...
        name: "deconv1",
        config: { filters: 128, kernelSize: 5, strides: 2, activation: "relu" },
      },
      {
        type: "conv2d_transpose",
        name: "deconv2",
        config: { filters: 64, kernelSize: 5, strides: 2, activation: "relu" },
      },
      {
        type: "conv2d",
        name: "output",
        config: { filters: config.imageShape[2], kernelSize: 5, activation: "tanh" },
      },
    ],
  ...
  const discriminator: NeuralNetworkArchitecture = {
    ...
    layers: [
      { type: "input", name: "image", config: { shape: config.imageShape } },
      {
        type: "conv2d",
        name: "conv1",
        config: { filters: 64, kernelSize: 5, strides: 2, activation: "leaky_relu" },
      },
      {
        type: "conv2d",
        name: "conv2",
        config: { filters: 128, kernelSize: 5, strides: 2, activation: "leaky_relu" },
      },
...
// VAE architecture
export function createVAE(config: {
  inputDim: number;
  latentDim: number;
...
// Diffusion model
export function createDiffusionModel(config: {
  imageShape: [number, number, number];
  timesteps: number;
}): NeuralNetworkArchitecture {
  return {

...
describe('regional readiness engine', () => {
  ...
  it('exports evidence-as-code artifacts per region', () => {
    ...
    const now = new Date();
    const events = [
      { type: 'residency', regionId: 'united-states' as RegionId, timestamp: new Date(now.getTime() - 1000), payload: { key: 'kms-us' } },
      { type: 'screening', regionId: 'united-states' as RegionId, timestamp: now, payload: { cadence: 'quarterly' } },
      { type: 'residency', regionId: 'united-states' as RegionId, timestamp: now, payload: { backups: 'verified' } },
    ];
  ...
  it('exposes checklists and non-negotiables to block bespoke region forks', () => {
    expect(localizationChecklist.items).toContain('Fallback behavior: missing strings fall back to English with telemetry');
    expect(procurementChecklist.items).toContain('Fast lane vs slow lane criteria with SLAs for legal/security review');

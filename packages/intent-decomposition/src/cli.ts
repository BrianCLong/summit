...
import { extractIntent, stripSpeculation } from "./pipeline/extract-intent.js";
...
import { StepSummary, StepSummaryFactual, IntentStatement, BiFactEval } from "./types.js";
...
program
  ...
  .action(async (options) => {
    ...
    const stage1Data = safeJsonParse<Array<StepSummary | StepSummaryFactual>>(stage1Payload);
    ...
    const stage1Facts = summaries.flatMap((summary) =>
      summary.screenContext
        .map((item) => item.value)
        .concat(summary.actions.map((item) => item.value))
    );
    ...
    if (options.junit) {
      ...
      const hallucinationRate = result.predictedFacts.length
        ? result.errorPropagation.hallucinatedFacts.length / result.predictedFacts.length
        : 0;

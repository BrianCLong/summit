...
function canonicalize(value) {
    ...
    if (value instanceof Map) {
        const entries = Array.from(value.entries()).sort(([a], [b]) => canonicalize(a).localeCompare(canonicalize(b)));
        return `{"$map":${canonicalize(entries)}}`;
    }
    if (value instanceof Set) {
        const items = Array.from(value.values()).sort((a, b) => canonicalize(a).localeCompare(canonicalize(b)));

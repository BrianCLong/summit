...
exports.BitemporalStore = void 0;
...
class BitemporalStore {
    ...
    tableName;
    logger;
    constructor(connectionString, tableName = 'bitemporal_entities', logger) {
     ...
     */
    async upsert(entityKey, data, options) {
        const client = await this.pool.connect();
        try {
         ...
         WHERE entity_key = $2
           ...
           AND tstzrange(valid_from, valid_to) && tstzrange($3, $4)`, [now, entityKey, options.validFrom, validTo]);
            // Insert new version
        ...
        RETURNING id`, [
            ...
            await client.query(`INSERT INTO bitemporal_audit_log (
          entity_key, operation, valid_time, tx_time, user_id, data, hash
        ) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [entityKey, 'UPDATE', options.validFrom, now, options.userId, JSON.stringify(data), hash]);
     ...
     */
    async getAsOf(entityKey, validTime, txTime) {
       ...
       LIMIT 1`, [entityKey, validTime, txTime]);
     ...
     */
    async diff(fromValidTime, fromTxTime, toValidTime, toTxTime) {
        // Get both snapshots
     ...
     */
    async exportAudit(entityKey, privateKey) {
        const result = await this.pool.query(`SELECT * FROM bitemporal_audit_log
       WHERE entity_key = $1
       ORDER BY tx_time ASC`, [entityKey]);
     ...
     */
    async getAllVersions(entityKey) {
        const result = await this.pool.query(`SELECT * FROM ${this.tableName}
       WHERE entity_key = $1
       ORDER BY valid_from DESC, tx_from DESC`, [entityKey]);
...
exports.BitemporalStore = BitemporalStore;

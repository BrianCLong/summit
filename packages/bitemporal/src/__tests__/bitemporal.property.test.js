...
const BitemporalStore_js_1 = require("../BitemporalStore.js");
/**
...
describe('Bitemporal Property Tests', () => {
    ...
    beforeAll(async () => {
        ...
        store = new BitemporalStore_js_1.BitemporalStore(connectionString, testTable);
    ...
    describe('Temporal interval invariants', () => {
        it('valid_from should always be before valid_to', async () => {
            await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 50 }), fc.record({ value: fc.integer() }), fc.date({ min: new Date('2020-01-01'), max: new Date('2030-01-01') }), fc.date({ min: new Date('2030-01-02'), max: new Date('2040-01-01') }), async (entityKey, data, validFrom, validTo) => {
                const options = {
                    validFrom,
                    validTo,
                    userId: 'test-user',
        ...
        it('tx_from should always be before tx_to', async () => {
            await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 50 }), fc.record({ value: fc.integer() }), fc.date({ min: new Date('2020-01-01'), max: new Date('2030-01-01') }), async (entityKey, data, validFrom) => {
                const options = {
                    validFrom,
                    userId: 'test-user',
                };
        ...
        it('no overlapping intervals for same entity key at same transaction time', async () => {
            await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 50 }), fc.array(fc.record({
            ...
            }), { minLength: 2, maxLength: 5 }), async (entityKey, updates) => {
                // Apply all updates
                ...
                for (const records of byEntity.values()) {
                    for (let i = 0; i < records.length; i++) {
                        for (let j = i + 1; j < records.length; j++) {
                            ...
                            const overlap = !(r1.validTo <= r2.validFrom ||
    ...
    describe('Temporal query properties', () => {
        ...
        it('diff between same snapshot should be empty', async () => {
            await fc.assert(fc.asyncProperty(fc.date({ min: new Date('2020-01-01'), max: new Date('2030-01-01') }), fc.date({ min: new Date('2020-01-01'), max: new Date('2030-01-01') }), async (validTime, txTime) => {

...
exports.MissileCapabilityAssessor = void 0;
const types_js_1 = require("./types.js");
class MissileCapabilityAssessor {
    assessCountryCapability(country, missiles) {
        const icbms = missiles.filter(m => m.missile_type === types_js_1.MissileType.BALLISTIC_ICBM);
        const slbms = missiles.filter(m => m.submarine_launched);
        const cruise = missiles.filter(m => m.missile_type === types_js_1.MissileType.CRUISE_GROUND ||
            m.missile_type === types_js_1.MissileType.CRUISE_AIR ||
            m.missile_type === types_js_1.MissileType.CRUISE_SEA);
        const hypersonic = missiles.filter(m => m.missile_type === types_js_1.MissileType.HYPERSONIC_GLIDE ||
...
exports.MissileCapabilityAssessor = MissileCapabilityAssessor;

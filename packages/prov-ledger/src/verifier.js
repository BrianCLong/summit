...
class Verifier {
    static verifyManifest(manifest) {
        ...
        for (const e of manifest.evidence) {
            ...
            const computed = (0, crypto_1.createHash)('sha256').update(contentToHash).digest('hex');
            if (computed !== e.hash) {
        ...
        for (const claim of manifest.claims) {
            ...
            const computedHash = (0, crypto_1.createHash)('sha256').update(hashContent).digest('hex');

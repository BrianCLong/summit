...
const simple_first_ai_1 = require("../src/simple_first_ai");
describe("Simple-First AI Policy", () => {
    ...
    test("returns null when feature flag is off", () => {
        ...
        expect((0, simple_first_ai_1.selectSimpleFirst)(candidates)).toBeNull();
    });
    test("selects simple-fast over simple-slow and others when flag is on", () => {
        process.env.FF_SIMPLE_FIRST_AI = "1";
        const selected = (0, simple_first_ai_1.selectSimpleFirst)(candidates);
        expect(selected?.name).toBe("simple-fast");
    });
    test("returns null for empty candidates", () => {
        process.env.FF_SIMPLE_FIRST_AI = "1";
        expect((0, simple_first_ai_1.selectSimpleFirst)([])).toBeNull();
    });
});

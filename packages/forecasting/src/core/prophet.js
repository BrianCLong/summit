...
exports.ProphetForecaster = void 0;
class ProphetForecaster {
     ...
     */
    forecast(horizon, confidenceLevel) {
        if (!this.fitted) {
    ...
    fitSeasonality() {
        ...
        const seasonalFit = new Array(this.data.length).fill(0);
    ...
    isHoliday(timestamp, holiday) {
        return holiday.dates.some(d => d.getFullYear() === timestamp.getFullYear() &&
            d.getMonth() === timestamp.getMonth() &&
            d.getDate() === timestamp.getDate());
    }
    findHolidayIndices(holiday) {
        const indices = [];
        for (let i = 0; i < this.data.length; i++) {
            if (this.isHoliday(this.data[i].timestamp, holiday)) {
                indices.push(i);
...
exports.ProphetForecaster = ProphetForecaster;

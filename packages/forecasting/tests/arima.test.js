...
const arima_js_1 = require("../src/core/arima.js");
describe('ARIMAForecaster', () => {
    ...
    describe('fit', () => {
        it('should fit ARIMA(1,1,1) model without errors', () => {
            const forecaster = new arima_js_1.ARIMAForecaster({ p: 1, d: 1, q: 1 });
            const data = generateTestData(100);
            expect(() => forecaster.fit(data)).not.toThrow();
        });
        it('should fit ARIMA(2,1,0) model without errors', () => {
            const forecaster = new arima_js_1.ARIMAForecaster({ p: 2, d: 1, q: 0 });
            const data = generateTestData(100);
            expect(() => forecaster.fit(data)).not.toThrow();
        });
        it('should fit SARIMA model with seasonal components', () => {
            const forecaster = new arima_js_1.ARIMAForecaster({
                p: 1, d: 1, q: 1,
                P: 1, D: 1, Q: 1, s: 12
            });
    ...
    describe('forecast', () => {
        ...
        it('should generate monotonically increasing timestamps', () => {
            ...
            for (let i = 1; i < forecasts.length; i++) {
                expect(forecasts[i].timestamp.getTime()).toBeGreaterThan(forecasts[i - 1].timestamp.getTime());
            }
        });
    });
    describe('evaluate', () => {
...
describe('AutoARIMA', () => {
    ...
    describe('selectBestModel', () => {
        it('should select optimal ARIMA parameters', () => {
            const autoArima = new arima_js_1.AutoARIMA(3, 2, 3, false);
            const data = generateTestData(100);

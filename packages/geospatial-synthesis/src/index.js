...
exports.GeospatialSynthesizer = void 0;
class GeospatialSynthesizer {
    config;
    constructor(config) {
        this.config = config;
    }
     ...
     */
    generateTraces(numTraces, pointsPerTrace) {
        const traces = [];
     ...
     */
    applyGeoIndistinguishability(point, epsilon) {
     ...
     */
    generateMobilityPattern(startPoint, duration, speed) {
     ...
     */
    generatePopulationDistribution(numClusters, pointsPerCluster) {
        const points = [];
    ...
    movePoint(point, distanceKm, bearing) {
        ...
        const lon2 = lon1 + Math.atan2(Math.sin(bearing) * Math.sin(distanceKm / R) * Math.cos(lat1), Math.cos(distanceKm / R) - Math.sin(lat1) * Math.sin(lat2));
        return {
...
exports.GeospatialSynthesizer = GeospatialSynthesizer;
exports.default = GeospatialSynthesizer;

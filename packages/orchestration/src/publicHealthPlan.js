...
exports.plan = exports.PublicHealthComplianceMonitor = exports.PublicHealthPlan = void 0;
const uuid_1 = require("uuid");
const PUBLIC_HEALTH_EPICS = [
    {
        id: 'EP1',
...
const ensureValue = (value, message, issues, code) => {
    if (Number.isNaN(value) || value === undefined || value === null) {
        issues.push({ code, message });
    }
};
class PublicHealthPlan {
    epics = PUBLIC_HEALTH_EPICS;
    ...
    assertEvidenceCompleteness(context, evidence, issues) {
        if (!evidence.ready) {
            issues.push({
    ...
    validateResidency(input, issues) {
    ...
    validateRetention(input, issues) {
    ...
    validateSecurityControls(input, issues) {
        const checks = [
    ...
    validatePerformance(input, issues) {
        ...
        ensureValue(performance.ingestEventsPerSecondPerPod, 'Ingest throughput is missing', issues, 'SLO_INGEST_EPS_MISSING');
    ...
    validateCosts(input, issues) {
        ...
        const ingestAlertThreshold = (this.costGuardrails.ingestCostPerThousand * this.costGuardrails.alertingThresholdPercentage) / 100;
        const gqlAlertThreshold = (this.costGuardrails.graphqlCostPerMillion * this.costGuardrails.alertingThresholdPercentage) / 100;
        if (costs.ingestCostPerThousand > ingestAlertThreshold &&
        ...
        if (costs.graphqlCostPerMillion > gqlAlertThreshold &&
    ...
    validateEpics(input, issues) {
        ...
        return this.epics.map((epic) => {
            ...
            epic.tasks.forEach((task) => {
                const evidence = taskEvidence.find((taskEvidence) => taskEvidence.taskId === task.id || taskEvidence.artifact === task.output);
                if (!evidence) {
                    issues.push({
...
exports.PublicHealthPlan = PublicHealthPlan;
class PublicHealthComplianceMonitor {
    history = [];
    planInstance;
    constructor(planInstance = new PublicHealthPlan()) {
...
exports.PublicHealthComplianceMonitor = PublicHealthComplianceMonitor;
exports.plan = new PublicHealthPlan();

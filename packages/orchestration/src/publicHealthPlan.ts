...
export class PublicHealthPlan {
  ...
  private assertEvidenceCompleteness(
    ...
    evidence: Pick<
      ArtifactStatus,
      "artifact" | "hash" | "verifiedAt" | "verifiedBy" | "provenance" | "ready"
    >,
    issues: ValidationIssue[]
  ): void {
    if (!evidence.ready) {
  ...
  private validatePerformance(input: ComplianceInput, issues: ValidationIssue[]): void {
    ...
    ensureValue(
      performance.subscriptionsP95Ms,
      "Subscription p95 is missing",
      issues,
      "SLO_SUB_MISSING"
    );
    ensureValue(
    ...
    ensureValue(
      performance.ingestPreStorageP95Ms,
      "Ingest pre-storage p95 is missing",
      issues,
      "SLO_INGEST_LAT_MISSING"
    );
    ensureValue(
      performance.graphOneHopP95Ms,
      "Graph one-hop p95 is missing",
      issues,
      "SLO_GRAPH1_MISSING"
    );
    ensureValue(
      performance.graphTwoToThreeHopP95Ms,
      "Graph two-three hop p95 is missing",
      issues,
      "SLO_GRAPH23_MISSING"
    );
    ...
    const sloChecks: [number, number, string, string][] = [
      ...
      [
        performance.subscriptionsP95Ms,
        this.sloTargets.subscriptionsP95Ms,
        "SLO_SUBSCRIPTIONS",
        "Subscriptions",
      ],
      [
        performance.ingestPreStorageP95Ms,
        this.sloTargets.ingestPreStorageP95Ms,
        "SLO_INGEST_LATENCY",
        "Ingest latency",
      ],
      [
        performance.graphOneHopP95Ms,
        this.sloTargets.graphOneHopP95Ms,
        "SLO_GRAPH_ONE_HOP",
        "Graph 1-hop",
      ],
      [
        performance.graphTwoToThreeHopP95Ms,
        this.sloTargets.graphTwoToThreeHopP95Ms,
        "SLO_GRAPH_MULTI_HOP",
        "Graph 2-3 hop",
      ],
    ];
  ...
  private validateCosts(input: ComplianceInput, issues: ValidationIssue[]): void {
    ...
    const ingestAlertThreshold =
      (this.costGuardrails.ingestCostPerThousand *
        this.costGuardrails.alertingThresholdPercentage) /
      100;
    const gqlAlertThreshold =
      (this.costGuardrails.graphqlCostPerMillion *
        this.costGuardrails.alertingThresholdPercentage) /
      100;
  ...
  private validateEpics(input: ComplianceInput, issues: ValidationIssue[]): EpicCoverage[] {
    const progressByEpic = new Map(
      input.epicProgress.map((progress) => [progress.epicId, progress])
    );
    ...
    return this.epics.map((epic) => {
      ...
      const evidenceArtifacts = new Map(
        (progress.evidence ?? []).map((artifact) => [artifact.artifact, artifact])
      );
      ...
      return {
        ...
        percentComplete: Math.round(
          ((epic.tasks.length - missingTasks.length) / epic.tasks.length) * 100
        ),

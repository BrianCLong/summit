...
import { AllowAllPolicyDecisionProvider, PolicyDecisionProvider } from '../src/policy.js';
import { shapeContext } from '../src/context-shaping.js';
import { InMemoryRunTraceStore } from '../src/run-trace.js';
import { Step, StepHandlers } from '../src/types.js';
...
describe('Orchestrator runtime', () => {
  ...
  it('builds a DAG plan and executes fan-out then join', async () => {
    ...
    expect(completed.some((s) => s.policyDecision)).toBe(true);
  ...
  it('requires approval when guardian returns require_approval', async () => {
    const provider: PolicyDecisionProvider = {
      beforeStep: async () => ({ action: 'require_approval', reason: 'manual gate' }),
    };
    const orchestrator = new Orchestrator({ policyProvider: provider });
    ...
    await orchestrator.resume(result.runId, stepHandlers);
  ...
  it('blocks risky execution when guardian decides to block', async () => {
    ...
    const orchestrator = new Orchestrator({ policyProvider: provider, runStore });
    ...
    expect(trace?.policyDecisions.some((d) => d.action === 'block')).toBe(true);
  });
});

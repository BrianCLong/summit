...
const publicHealthPlan_js_1 = require("../src/publicHealthPlan.js");
const baseComplianceInput = {
    purposeTags: ['public-health'],
...
const completedEpicProgress = publicHealthPlan_js_1.plan.epics.map((epic) => ({
    epicId: epic.id,
...
describe('PublicHealthPlan', () => {
    it('declares the full set of epics and tasks from the orchestration plan', () => {
        const epicsWithTasks = publicHealthPlan_js_1.plan.epics.map((epic) => ({ id: epic.id, taskCount: epic.tasks.length }));
        expect(epicsWithTasks).toEqual(expect.arrayContaining([
            { id: 'EP1', taskCount: 14 },
            { id: 'EP2', taskCount: 14 },
            { id: 'EP3', taskCount: 11 },
            { id: 'EP4', taskCount: 10 },
    ...
    it('passes validation when all principles, SLOs, cost guardrails, and artifacts are satisfied', () => {
        const report = publicHealthPlan_js_1.plan.validate({
            ...baseComplianceInput,
    ...
    it('fails validation when SLOs, purpose tags, and tasks are violated', () => {
        const failingPlan = new publicHealthPlan_js_1.PublicHealthPlan();
    ...
    it('tracks compliance history through the monitor', () => {
        const monitor = new publicHealthPlan_js_1.PublicHealthComplianceMonitor();

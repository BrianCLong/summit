...
class PacketAnalyzer {
        ...
        if (rawData.length >= 14) {
            ...
            if (packet.ethernet.etherType === 0x0800 && rawData.length >= 34) {
                ...
                if (packet.ip.protocol === 6 && rawData.length >= transportOffset + 20) {
                    ...
                    packet.applicationProtocol = this.identifyApplicationProtocol(packet.tcp.sourcePort, packet.tcp.destinationPort);
                    // Extract payload
                    ...
                    packet.applicationProtocol = this.identifyApplicationProtocol(packet.udp.sourcePort, packet.udp.destinationPort);
                    // Extract payload
     ...
     */
    generateSimulatedPackets(count) {
        const packets = [];
    ...
    identifyApplicationProtocol(srcPort, dstPort) {
...
exports.PacketAnalyzer = PacketAnalyzer;

...
describe('SlowQueryKiller', () => {
    ...
    beforeEach(() => {
        killer = new index_1.SlowQueryKiller(defaultBudget);
    });
    afterEach(() => {
        killer.stop();
    });
...
describe('QueryComplexityAnalyzer', () => {
    describe('Cypher Complexity', () => {
        it('should analyze simple MATCH query', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeCypherComplexity('MATCH (n:Person) RETURN n LIMIT 10');
            expect(complexity).toBeGreaterThan(0);
            expect(complexity).toBeLessThan(20);
        });
        it('should detect variable length paths', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeCypherComplexity('MATCH (a)-[*1..3]->(b) RETURN a, b');
            expect(complexity).toBeGreaterThan(15);
        });
        it('should heavily penalize unbounded paths', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeCypherComplexity('MATCH (a)-[*]->(b) RETURN a, b');
            expect(complexity).toBeGreaterThan(50);
        });
        it('should detect aggregations', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeCypherComplexity('MATCH (n:Person) RETURN count(n), sum(n.age), avg(n.salary)');
            expect(complexity).toBeGreaterThan(10);
        });
        it('should detect optional matches', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeCypherComplexity('MATCH (n:Person) OPTIONAL MATCH (n)-[:WORKS_AT]->(c:Company) RETURN n, c');
            expect(complexity).toBeGreaterThan(10);
        });
    });
    describe('SQL Complexity', () => {
        it('should analyze simple SELECT query', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeSQLComplexity('SELECT * FROM users WHERE id = 1');
            expect(complexity).toBeGreaterThanOrEqual(0);
            expect(complexity).toBeLessThan(10);
        });
        it('should detect JOINs', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeSQLComplexity('SELECT * FROM users INNER JOIN orders ON users.id = orders.user_id');
            expect(complexity).toBeGreaterThan(5);
        });
        it('should detect subqueries', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeSQLComplexity('SELECT * FROM users WHERE id IN (SELECT user_id FROM orders)');
            expect(complexity).toBeGreaterThan(10);
        });
        it('should detect window functions', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeSQLComplexity('SELECT ROW_NUMBER() OVER (PARTITION BY department ORDER BY salary) FROM employees');
            expect(complexity).toBeGreaterThan(10);
        });
        it('should detect CTEs', () => {
            const complexity = index_1.QueryComplexityAnalyzer.analyzeSQLComplexity('WITH high_earners AS (SELECT * FROM employees WHERE salary > 100000) SELECT * FROM high_earners');
            expect(complexity).toBeGreaterThan(5);
        });
    });
});
describe('QueryCostEstimator', () => {
    describe('Neo4j Cost Estimation', () => {
        it('should estimate simple query cost', () => {
            const cost = index_1.QueryCostEstimator.estimateNeo4jCost('MATCH (n:Person) RETURN n LIMIT 10', 10);
            expect(cost).toBeGreaterThan(0);
            expect(cost).toBeLessThan(0.01);
        });
        it('should estimate higher cost for complex queries', () => {
            const simpleCost = index_1.QueryCostEstimator.estimateNeo4jCost('MATCH (n:Person) RETURN n LIMIT 10', 10);
            const complexCost = index_1.QueryCostEstimator.estimateNeo4jCost('MATCH (a:Person)-[*1..5]->(b:Person) RETURN a, b', 100);
            expect(complexCost).toBeGreaterThan(simpleCost);
        });
    });
    describe('PostgreSQL Cost Estimation', () => {
        it('should estimate simple query cost', () => {
            const cost = index_1.QueryCostEstimator.estimatePostgresCost('SELECT * FROM users WHERE id = 1', 1);
            expect(cost).toBeGreaterThan(0);
            expect(cost).toBeLessThan(0.001);
        });
        it('should estimate higher cost for complex queries', () => {
            const simpleCost = index_1.QueryCostEstimator.estimatePostgresCost('SELECT * FROM users LIMIT 10', 10);
            const complexCost = index_1.QueryCostEstimator.estimatePostgresCost('SELECT * FROM users JOIN orders ON users.id = orders.user_id WHERE EXISTS (SELECT 1 FROM payments WHERE payments.order_id = orders.id)', 1000);
            expect(complexCost).toBeGreaterThan(simpleCost);
        });
    });
});

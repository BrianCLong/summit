...
exports.attachFingerprints = attachFingerprints;
exports.prepareBudgets = prepareBudgets;
exports.evaluateBudgets = evaluateBudgets;
exports.formatBudgetDiff = formatBudgetDiff;
exports.pgStatStatementsAvailable = pgStatStatementsAvailable;
...
function attachFingerprints(rows) {
    return rows.map((row) => {
...
function prepareBudgets(budgetFile) {
...
function evaluateBudgets(rows, budgetFile) {
    ...
    for (const budget of preparedBudgets) {
        ...
        if (budget.maxMeanTimeMs !== undefined &&
            ...
            violations.push({
                ...
                suggestion: 'Inspect EXPLAIN ANALYZE; add or adjust indexes on filter columns; confirm row estimates.',
                observed: observedStats,
            });
        }
        if (budget.maxTotalTimeShare !== undefined &&
            ...
            violations.push({
                ...
                regressionPct: budget.maxTotalTimeShare
                    ...
                    : 0,
                description: budget.description,
                suggestion: 'Consider caching hot paths, materializing heavy views, or adding covering indexes.',
                observed: observedStats,
            });
        }
        if (budget.maxCalls !== undefined &&
            ...
            violations.push({
                ...
                regressionPct: budget.maxCalls
                    ...
                    : 0,
                description: budget.description,
                suggestion: 'Batch or cache upstream requests; ensure pagination is applied for list endpoints.',
                observed: observedStats,
            });
        }
    }
...
function formatBudgetDiff(result) {
    ...
    const lines = [
        ...
        ...result.violations.map((violation) => {
            const base = `- ${violation.id} (${violation.metric}) exceeded: ${violation.actual.toFixed(2)} > ${violation.limit}`;
            const regression = violation.regressionPct
                ? ` (+${violation.regressionPct.toFixed(1)}%)`
                : '';
            const share = violation.metric === 'total_time_share'
                ? ` (workload share: ${(violation.observed.totalTimeShare * 100).toFixed(1)}%)`
                : '';
            const mean = violation.metric === 'mean_exec_time'
                ? ` (mean ${violation.observed.meanTimeMs.toFixed(2)} ms)`
                : '';
            const calls = violation.metric === 'calls'
    ...
    if (result.missingFingerprints.length) {
        lines.push('', `Missing fingerprints (not observed in sample): ${result.missingFingerprints.join(', ')}`);
    }
    return lines.join('\n');
}
async function pgStatStatementsAvailable(client) {
    try {
        const res = await client.query("SELECT extname FROM pg_extension WHERE extname = 'pg_stat_statements'");

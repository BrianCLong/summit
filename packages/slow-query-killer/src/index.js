...
exports.QueryCostEstimator = exports.QueryComplexityAnalyzer = exports.SlowQueryKiller = void 0;
exports.createQueryBudgetMiddleware = createQueryBudgetMiddleware;
const events_1 = __importDefault(require("events"));
/**
 * SlowQueryKiller - Monitors and kills slow/expensive queries
 */
class SlowQueryKiller extends events_1.default {
    ...
    registerQuery(context) {
        ...
        const tenantQueries = Array.from(this.runningQueries.values()).filter((q) => q.tenantId === context.tenantId);
        if (budget.maxConcurrentQueries &&
    ...
    shouldKillQuery(queryId) {
        ...
        if (timePercentage > budget.softThreshold * 100 ||
     ...
     */
    async killQuery(queryId, database, connectionHandle) {
        const context = this.runningQueries.get(queryId);
        if (!context) {
            return false;
        }
     ...
     */
    calculateCostIncurred(context, executionTimeMs) {
     ...
     * Kill Neo4j query
     */
    async killNeo4jQuery(queryId, driver) {
     ...
     */
    async killPostgresQuery(processId, client) {
     ...
     */
    async killRedisCommand(_queryId, client) {
...
class QueryComplexityAnalyzer {
     ...
     */
    static analyzeCypherComplexity(cypher) {
        let complexity = 0;
     ...
     */
    static analyzeSQLComplexity(sql) {
        let complexity = 0;
        // JOIN complexity
...
class QueryCostEstimator {
     ...
     */
    static estimateNeo4jCost(cypher, estimatedResultCount = 100) {
     ...
     */
    static estimatePostgresCost(sql, estimatedRows = 1000) {
 ...
 */
function createQueryBudgetMiddleware(killer) {
...
exports.default = SlowQueryKiller;
__exportStar(require("./pgStatBudgeter"), exports);

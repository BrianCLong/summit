...
export class SemanticSearch {
  ...
  async search(
    ...
    if (this.deltaOperatorEnabled) {
      const contextVectors = this.collectContextVectors(documents);
      if (contextVectors.length > 0) {
        ...
        const updatedState = applyDeltaOperator({ X: state, k: params.k, v: params.v, beta: params.beta });
  ...
  private collectContextVectors(documents: Array<{ id: string }>): number[][] {
    return documents
      .map((doc) => this.index.get(doc.id))

...
exports.ZoneMapManager = void 0;
class ZoneMapManager {
    bloomFilterSize = 1024; // bits
    /**
    ...
    createZoneMap(data) {
        ...
        const bloomFilter = distinctValues.size > 100
     ...
     */
    async createZoneMaps(pool, tableName, columns) {
     ...
     */
    mightContainValue(zoneMap, value) {
        // Check null case
     ...
     */
    async pruneBlocks(pool, tableName, columnName, predicate) {
        let whereClause = '';
      ...
      WHERE column_name = '${columnName}'
        AND ${whereClause}
    `;
        const params = predicate.operator === 'BETWEEN'
     ...
     */
    async getPruningStats(pool, tableName, columnName) {
     ...
     */
    bloomFilterContains(filter, value) {
        const hashCount = 3;
        const hashes = this.getHashes(value, hashCount);
        for (const hash of hashes) {
            const byteIndex = Math.floor(hash / 8);
    ...
    mergeZoneMaps(zoneMaps) {
        ...
        return {
            minValue: nonNullMaps.length > 0
                ? this.getMin(nonNullMaps.map((zm) => zm.minValue))
                : null,
            maxValue: nonNullMaps.length > 0
     ...
     */
    calculatePruningEfficiency(totalBlocks, prunedBlocks) {
...
exports.ZoneMapManager = ZoneMapManager;

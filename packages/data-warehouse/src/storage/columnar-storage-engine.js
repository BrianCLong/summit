...
exports.ColumnarStorageEngine = exports.EncodingType = exports.DataType = void 0;
const compression_manager_1 = require("./compression-manager");
const zone_map_manager_1 = require("./zone-map-manager");
var DataType;
(function (DataType) {
...
(function (EncodingType) {
    ...
    EncodingType["BITPACK"] = "BITPACK";
    EncodingType["SPARSE"] = "SPARSE";
...
class ColumnarStorageEngine {
    ...
    constructor(pool) {
        ...
        this.zoneMapManager = new zone_map_manager_1.ZoneMapManager();
    }
    /**
     ...
     */
    async createColumnarMetadata(schema) {
     ...
     */
    async insertBlock(tableName, columns, data) {
        ...
        await this.pool.query(`INSERT INTO ${tableName} (${columns.join(', ')}) VALUES ${placeholders}`, values);
    }
    /**
     * Compress a column with optimal compression algorithm
     */
    async compressColumn(blockId, tableId, columnName, data) {
     ...
     */
    applyEncoding(data, encoding, stats) {
     ...
     */
    dictionaryEncode(data, originalSize) {
     ...
     */
    bitpackEncode(data, stats, originalSize) {
        const min = stats.min;
     ...
     */
    sparseEncode(data, originalSize) {
        const sparse = [];
     ...
     */
    async storeColumnarBlock(block) {
        await this.pool.query(`
     ...
     */
    async queryColumnar(tableName, columns, predicates) {
        ...
        const blockFilter = eligibleBlocks.length > 0
     ...
     */
    async pruneBlocksWithZoneMaps(tableName, predicates) {
    ...
    // Utility methods
    transposeToColumnar(columns, data) {
        const columnar = {};
    ...
    `);
        ...
        const totalUncompressed = stats.rows.reduce((sum, row) => sum + parseInt(row.uncompressed_size), 0);
        return {
...
exports.ColumnarStorageEngine = ColumnarStorageEngine;

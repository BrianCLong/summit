...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.log(
        `  Avg Duration: ${(report.aggregateMetrics.performance.avgDuration / 1000).toFixed(2)}s`
      );
      console.log(
        `  Entities Found: ${(report.aggregateMetrics.correctness.entitiesFoundRate * 100).toFixed(1)}%`
      );
...
program
  ...
  .action(() => {
    ...
    for (const template of templates) {
      ...
      console.log(
        `    Entities: ${template.entities.reduce((sum, e) => sum + e.distribution.count, 0)}`
      );
      console.log("");
    }
  });
    ...
    params: {
      ...
      nodeCount:
        typeof template === "string"
          ? undefined
          : Math.floor((template.params?.nodeCount || 50) * multiplier),
    },
  });

  ...
  <div class="container">
    ...
    renderComparison(report) {
      ...
      <table class="comparison-table">
        ...
        <tbody>
          ${this.renderComparisonRow('Avg Duration', comp.baseline.metrics.performance.avgDuration / 1000, comp.candidate.metrics.performance.avgDuration / 1000, comp.deltas.performance.avgDuration, 's', true)}
          ${this.renderComparisonRow('Query Latency (p50)', comp.baseline.metrics.performance.avgQueryLatency.p50, comp.candidate.metrics.performance.avgQueryLatency.p50, comp.deltas.performance.avgQueryLatency_p50, 'ms', true)}
          ${this.renderComparisonRow('Entities Found Rate', comp.baseline.metrics.correctness.entitiesFoundRate * 100, comp.candidate.metrics.correctness.entitiesFoundRate * 100, comp.deltas.correctness.entitiesFoundRate, '%', false)}
          ${this.renderComparisonRow('Success Rate', comp.baseline.metrics.reliability.successRate * 100, comp.candidate.metrics.reliability.successRate * 100, comp.deltas.reliability.successRate, '%', false)}
        </tbody>
      </table>
    `;
    }
    renderComparisonRow(label, baseline, candidate, delta, unit, lowerIsBetter) {
    ...
    getDeltaClass(delta, lowerIsBetter) {
        if (Math.abs(delta) < 5)

...
class ExerciseManager {
     ...
     * Create new exercise
     */
    createExercise(name, type, scenario, objectives) {
        const exercise = {
            id: this.generateId(),
            name,
            description: scenario.description,
            type,
            status: types_1.ExerciseStatus.PLANNED,
    ...
    startExercise(exerciseId) {
        ...
        exercise.status = types_1.ExerciseStatus.IN_PROGRESS;
     ...
     * Record detection
     */
    recordDetection(exerciseId, detection) {
        const exercise = this.exercises.get(exerciseId);
        if (!exercise) {
            throw new Error('Exercise not found');
        }
     ...
     * Record finding
     */
    recordFinding(exerciseId, finding) {
        const exercise = this.exercises.get(exerciseId);
        if (!exercise) {
            throw new Error('Exercise not found');
        }
    ...
    completeExercise(exerciseId) {
        ...
        exercise.status = types_1.ExerciseStatus.COMPLETED;
     ...
     */
    generateAfterActionReport(exerciseId) {
        const exercise = this.exercises.get(exerciseId);
        if (!exercise) {
            throw new Error('Exercise not found');
        }
    ...
    updateMetrics(exercise) {
        ...
        exercise.metrics.techniquesDetected = new Set(truePositives.map(d => d.techniqueId).filter(Boolean)).size;
    ...
    analyzeLessonsLearned(exercise) {
        const lessons = [];
    ...
    generateRecommendations(exercise) {
    ...
    generateExecutiveSummary(exercise) {

...
const sphincsplus_1 = require("../algorithms/sphincsplus");
const types_1 = require("../types");
describe('SphincsSignature', () => {
    describe('SPHINCS+-128f (Level 1, Fast)', () => {
        let signer;
        beforeEach(() => {
            signer = new sphincsplus_1.SphincsSignature('sphincs-sha256-128f');
        });
        it('should report correct algorithm', () => {
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_128F);
        });
        it('should report correct security level', () => {
            expect(signer.getSecurityLevel()).toBe(types_1.SecurityLevel.LEVEL_1);
        });
        it('should generate valid key pair', async () => {
            const keyPair = await signer.generateKeyPair();
            expect(keyPair).toHaveProperty('publicKey');
            expect(keyPair).toHaveProperty('privateKey');
            expect(keyPair.algorithm).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_128F);
        ...
        it('should sign message', async () => {
            ...
            expect(signature.algorithm).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_128F);
        ...
        it('should throw error for invalid private key length', async () => {
            ...
            const message = new TextEncoder().encode('Test');
            await expect(signer.sign(message, invalidPrivateKey)).rejects.toThrow(/Invalid private key length/);
        });
        it('should throw error for invalid public key length', async () => {
            ...
            await expect(signer.verify(message, signature, invalidPublicKey)).rejects.toThrow(/Invalid public key length/);
        });
    });
    describe('SPHINCS+-128s (Level 1, Small)', () => {
        let signer;
        beforeEach(() => {
            signer = new sphincsplus_1.SphincsSignature('sphincs-sha256-128s');
        });
        ...
        it('should report correct algorithm', () => {
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_128S);
        });
    });
    describe('SPHINCS+-192f (Level 3, Fast)', () => {
        let signer;
        beforeEach(() => {
            signer = new sphincsplus_1.SphincsSignature('sphincs-sha256-192f');
        });
        it('should have correct parameters', async () => {
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_192F);
            expect(signer.getSecurityLevel()).toBe(types_1.SecurityLevel.LEVEL_3);
            const keyPair = await signer.generateKeyPair();
    ...
    describe('SPHINCS+-256f (Level 5, Fast)', () => {
        let signer;
        beforeEach(() => {
            signer = new sphincsplus_1.SphincsSignature('sphincs-sha256-256f');
        });
        it('should have correct parameters', async () => {
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_256F);
            expect(signer.getSecurityLevel()).toBe(types_1.SecurityLevel.LEVEL_5);
            const keyPair = await signer.generateKeyPair();
    ...
    describe('SPHINCS+-256s (Level 5, Small)', () => {
        let signer;
        beforeEach(() => {
            signer = new sphincsplus_1.SphincsSignature('sphincs-sha256-256s');
        });
        it('should have smaller signatures than fast variant', async () => {
            const keyPair = await signer.generateKeyPair();
            const message = new TextEncoder().encode('Test');
    ...
    describe('createSphincsSignature factory', () => {
        it('should create 128f for Level 1 fast', () => {
            const signer = (0, sphincsplus_1.createSphincsSignature)(types_1.SecurityLevel.LEVEL_1, true);
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_128F);
        });
        it('should create 128s for Level 1 small', () => {
            const signer = (0, sphincsplus_1.createSphincsSignature)(types_1.SecurityLevel.LEVEL_1, false);
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_128S);
        });
        it('should create 192f for Level 3 fast', () => {
            const signer = (0, sphincsplus_1.createSphincsSignature)(types_1.SecurityLevel.LEVEL_3, true);
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_192F);
        });
        it('should create 256f for Level 5 fast', () => {
            const signer = (0, sphincsplus_1.createSphincsSignature)(types_1.SecurityLevel.LEVEL_5, true);
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_256F);
        });
        it('should default to fast variant', () => {
            const signer = (0, sphincsplus_1.createSphincsSignature)(types_1.SecurityLevel.LEVEL_1);
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.SPHINCS_PLUS_128F);
        });
    });
    describe('Stateless Properties', () => {
        it('should be stateless (no state required between signatures)', async () => {
            const signer = new sphincsplus_1.SphincsSignature('sphincs-sha256-128f');
            const keyPair = await signer.generateKeyPair();
    ...
    describe('Key Generation', () => {
        it('should generate unique key pairs', async () => {
            const signer = new sphincsplus_1.SphincsSignature('sphincs-sha256-128f');
            const keyPairs = await Promise.all(Array.from({ length: 5 }, () => signer.generateKeyPair()));

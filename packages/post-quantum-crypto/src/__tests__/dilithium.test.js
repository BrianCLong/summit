...
describe('DilithiumSignature', () => {
    describe('Dilithium2 (Level 2)', () => {
        let signer;
        beforeEach(() => {
            signer = new dilithium_1.DilithiumSignature('dilithium2');
        });
        it('should report correct algorithm', () => {
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.DILITHIUM_2);
        });
        it('should report correct security level', () => {
            expect(signer.getSecurityLevel()).toBe(types_1.SecurityLevel.LEVEL_2);
        });
        it('should generate valid key pair', async () => {
            const keyPair = await signer.generateKeyPair();
            expect(keyPair).toHaveProperty('publicKey');
            expect(keyPair).toHaveProperty('privateKey');
            expect(keyPair.algorithm).toBe(types_1.PQCAlgorithm.DILITHIUM_2);
            expect(keyPair.securityLevel).toBe(types_1.SecurityLevel.LEVEL_2);
        ...
        it('should sign message', async () => {
            ...
            expect(signature.algorithm).toBe(types_1.PQCAlgorithm.DILITHIUM_2);
        ...
        it('should throw error for invalid signature length', async () => {
            ...
            await expect(signer.verify(message, invalidSignature, keyPair.publicKey)).rejects.toThrow(/Invalid signature length/);
        });
    });
    describe('Dilithium3 (Level 3)', () => {
        let signer;
        beforeEach(() => {
            signer = new dilithium_1.DilithiumSignature('dilithium3');
        });
        it('should have correct parameters', async () => {
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.DILITHIUM_3);
    ...
    describe('Dilithium5 (Level 5)', () => {
        let signer;
        beforeEach(() => {
            signer = new dilithium_1.DilithiumSignature('dilithium5');
        });
        it('should have correct parameters', async () => {
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.DILITHIUM_5);
    ...
    describe('createDilithiumSignature factory', () => {
        it('should create Dilithium2 for Level 2', () => {
            const signer = (0, dilithium_1.createDilithiumSignature)(types_1.SecurityLevel.LEVEL_2);
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.DILITHIUM_2);
        });
        it('should create Dilithium3 for Level 3', () => {
            const signer = (0, dilithium_1.createDilithiumSignature)(types_1.SecurityLevel.LEVEL_3);
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.DILITHIUM_3);
        });
        it('should create Dilithium5 for Level 5', () => {
            const signer = (0, dilithium_1.createDilithiumSignature)(types_1.SecurityLevel.LEVEL_5);
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.DILITHIUM_5);
        });
        it('should default to Dilithium3', () => {
            const signer = (0, dilithium_1.createDilithiumSignature)();
            expect(signer.getAlgorithm()).toBe(types_1.PQCAlgorithm.DILITHIUM_3);
        });
    });
    describe('Signature Metadata', () => {
        it('should include metadata in signature', async () => {
            const signer = new dilithium_1.DilithiumSignature('dilithium3');
            const keyPair = await signer.generateKeyPair();

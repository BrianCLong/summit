...
describe('KyberKEM', () => {
    describe('Kyber-512 (Level 1)', () => {
        let kem;
        beforeEach(() => {
            kem = new kyber_1.KyberKEM('kyber512');
        });
        it('should report correct algorithm', () => {
            expect(kem.getAlgorithm()).toBe(types_1.PQCAlgorithm.KYBER_512);
        });
        it('should report correct security level', () => {
            expect(kem.getSecurityLevel()).toBe(types_1.SecurityLevel.LEVEL_1);
        });
        it('should generate valid key pair', async () => {
            const keyPair = await kem.generateKeyPair();
            expect(keyPair).toHaveProperty('publicKey');
            expect(keyPair).toHaveProperty('privateKey');
            expect(keyPair.algorithm).toBe(types_1.PQCAlgorithm.KYBER_512);
        ...
        it('should throw error for invalid private key length', async () => {
            ...
            await expect(kem.decapsulate(ciphertext, invalidPrivateKey)).rejects.toThrow(/Invalid private key length/);
        });
        it('should throw error for invalid ciphertext length', async () => {
            const keyPair = await kem.generateKeyPair();
            const invalidCiphertext = new Uint8Array(100);
            await expect(kem.decapsulate(invalidCiphertext, keyPair.privateKey)).rejects.toThrow(/Invalid ciphertext length/);
        });
    });
    describe('Kyber-768 (Level 3)', () => {
        let kem;
        beforeEach(() => {
            kem = new kyber_1.KyberKEM('kyber768');
        });
        it('should have correct parameters', async () => {
            expect(kem.getAlgorithm()).toBe(types_1.PQCAlgorithm.KYBER_768);
            expect(kem.getSecurityLevel()).toBe(types_1.SecurityLevel.LEVEL_3);
            const keyPair = await kem.generateKeyPair();
    ...
    describe('Kyber-1024 (Level 5)', () => {
        let kem;
        beforeEach(() => {
            kem = new kyber_1.KyberKEM('kyber1024');
        });
        it('should have correct parameters', async () => {
            expect(kem.getAlgorithm()).toBe(types_1.PQCAlgorithm.KYBER_1024);
            expect(kem.getSecurityLevel()).toBe(types_1.SecurityLevel.LEVEL_5);
            const keyPair = await kem.generateKeyPair();
    ...
    describe('createKyberKEM factory', () => {
        it('should create Kyber-512 for Level 1', () => {
            const kem = (0, kyber_1.createKyberKEM)(types_1.SecurityLevel.LEVEL_1);
            expect(kem.getAlgorithm()).toBe(types_1.PQCAlgorithm.KYBER_512);
        });
        it('should create Kyber-768 for Level 3', () => {
            const kem = (0, kyber_1.createKyberKEM)(types_1.SecurityLevel.LEVEL_3);
            expect(kem.getAlgorithm()).toBe(types_1.PQCAlgorithm.KYBER_768);
        });
        it('should create Kyber-1024 for Level 5', () => {
            const kem = (0, kyber_1.createKyberKEM)(types_1.SecurityLevel.LEVEL_5);
            expect(kem.getAlgorithm()).toBe(types_1.PQCAlgorithm.KYBER_1024);
        });
        it('should default to Kyber-768', () => {
            const kem = (0, kyber_1.createKyberKEM)();
            expect(kem.getAlgorithm()).toBe(types_1.PQCAlgorithm.KYBER_768);
        });
    });
    describe('Key Pair Generation', () => {
        it('should generate unique key pairs', async () => {
            ...
            const keyPairs = await Promise.all(Array.from({ length: 5 }, () => kem.generateKeyPair()));

...
const hybrid_kem_1 = require("../key-exchange/hybrid-kem");
const kyber_1 = require("../algorithms/kyber");
describe('HybridKEM', () => {
    describe('X25519 + Kyber-768 (Default)', () => {
        let hybridKEM;
        beforeEach(() => {
            hybridKEM = new hybrid_kem_1.HybridKEM();
        });
    ...
    describe('X25519 + Custom Kyber', () => {
        it('should work with Kyber-512', async () => {
            const kyber512 = new kyber_1.KyberKEM('kyber512');
            const hybridKEM = new hybrid_kem_1.HybridKEM('x25519', kyber512);
            const keyPair = await hybridKEM.generateKeyPair();
        ...
        it('should work with Kyber-1024', async () => {
            const kyber1024 = new kyber_1.KyberKEM('kyber1024');
            const hybridKEM = new hybrid_kem_1.HybridKEM('x25519', kyber1024);
            const keyPair = await hybridKEM.generateKeyPair();
    ...
    describe('P-256 + Kyber', () => {
        it('should work with P-256 classical algorithm', async () => {
            const hybridKEM = new hybrid_kem_1.HybridKEM('p256');
            const keyPair = await hybridKEM.generateKeyPair();
    ...
    describe('createHybridKEM factory', () => {
        it('should create default hybrid KEM', () => {
            const hybridKEM = (0, hybrid_kem_1.createHybridKEM)();
            expect(hybridKEM).toBeInstanceOf(hybrid_kem_1.HybridKEM);
        });
        it('should create hybrid KEM with x25519', () => {
            const hybridKEM = (0, hybrid_kem_1.createHybridKEM)('x25519');
            expect(hybridKEM).toBeInstanceOf(hybrid_kem_1.HybridKEM);
        });
        it('should create hybrid KEM with p256', () => {
            const hybridKEM = (0, hybrid_kem_1.createHybridKEM)('p256');
            expect(hybridKEM).toBeInstanceOf(hybrid_kem_1.HybridKEM);
        });
    });
    describe('Key Uniqueness', () => {
        it('should generate unique hybrid key pairs', async () => {
            const hybridKEM = new hybrid_kem_1.HybridKEM();
            const keyPairs = await Promise.all(Array.from({ length: 5 }, () => hybridKEM.generateKeyPair()));
        ...
        it('should generate unique shared secrets', async () => {
            const hybridKEM = new hybrid_kem_1.HybridKEM();
            const keyPair = await hybridKEM.generateKeyPair();
            const secrets = await Promise.all(Array.from({ length: 5 }, () => hybridKEM.encapsulate(keyPair.publicKey)));
            const secretHexes = secrets.map((s) => Array.from(s.sharedSecret)
                .map((b) => b.toString(16).padStart(2, '0'))
                .join(''));

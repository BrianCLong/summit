...
exports.SphincsSignature = void 0;
...
class SphincsSignature {
    ...
    algorithm;
    securityLevel;
    fast; // 'f' variants are faster, 's' variants have smaller signatures
    constructor(variant = 'sphincs-sha256-128f') {
        this.variant = variant;
        this.fast = variant.endsWith('f');
        // Map variant to algorithm and security level
        if (variant.includes('128f')) {
            this.algorithm = types_1.PQCAlgorithm.SPHINCS_PLUS_128F;
            this.securityLevel = types_1.SecurityLevel.LEVEL_1;
        }
        else if (variant.includes('128s')) {
            this.algorithm = types_1.PQCAlgorithm.SPHINCS_PLUS_128S;
            this.securityLevel = types_1.SecurityLevel.LEVEL_1;
        }
        else if (variant.includes('192f')) {
            this.algorithm = types_1.PQCAlgorithm.SPHINCS_PLUS_192F;
            this.securityLevel = types_1.SecurityLevel.LEVEL_3;
        }
        else if (variant.includes('192s')) {
            this.algorithm = types_1.PQCAlgorithm.SPHINCS_PLUS_192S;
            this.securityLevel = types_1.SecurityLevel.LEVEL_3;
        }
        else if (variant.includes('256f')) {
            this.algorithm = types_1.PQCAlgorithm.SPHINCS_PLUS_256F;
            this.securityLevel = types_1.SecurityLevel.LEVEL_5;
        }
        else {
            this.algorithm = types_1.PQCAlgorithm.SPHINCS_PLUS_256S;
            this.securityLevel = types_1.SecurityLevel.LEVEL_5;
        }
    }
    ...
    async generateSphincsKeys() {
        ...
        const root = (0, sha256_1.sha256)(new Uint8Array([...skSeed, ...pkSeed]));
    ...
    async performSigning(message, privateKey) {
        ...
        const messageHash = (0, sha256_1.sha256)(new Uint8Array([...randomizer, ...message]));
...
function createSphincsSignature(securityLevel = types_1.SecurityLevel.LEVEL_1, fast = true) {
    const suffix = fast ? 'f' : 's';
    switch (securityLevel) {
        case types_1.SecurityLevel.LEVEL_1:
            return new SphincsSignature(`sphincs-sha256-128${suffix}`);
        case types_1.SecurityLevel.LEVEL_3:
            return new SphincsSignature(`sphincs-sha256-192${suffix}`);
        case types_1.SecurityLevel.LEVEL_5:
            return new SphincsSignature(`sphincs-sha256-256${suffix}`);
        default:
            return new SphincsSignature(`sphincs-sha256-128${suffix}`);
    }
}

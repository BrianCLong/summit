...
class RESTAPIConnector extends BaseConnector_1.BaseConnector {
    client = null;
    accessToken = null;
    ...
    async refreshOAuthToken() {
        ...
        try {
            const response = await axios_1.default.post(oauth.tokenUrl, {
    ...
    async *extractWithOffsetPagination(endpoint, config) {
        let offset = 0;
        let hasMore = true;
        let pageCount = 0;
    ...
    async *extractWithCursorPagination(endpoint, config) {
        let cursor = null;
        let hasMore = true;
        let pageCount = 0;
        while (hasMore && (!config.maxPages || pageCount < config.maxPages)) {
    ...
    async *extractWithPagePagination(endpoint, config) {
        let page = 1;
        let hasMore = true;

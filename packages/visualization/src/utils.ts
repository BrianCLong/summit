...
export function getColorScale(config: ColorScale): (value: any) => string {
  switch (config.type) {
    ...
    case "sequential":
      return d3
        .scaleSequential(d3.interpolateRgb(config.range[0], config.range[1]))
        .domain(config.domain as [number, number]) as (value: any) => string;

    case "diverging":
      const divergingScale = d3
        .scaleDiverging(d3.interpolateRgb(config.range[0], config.range[1]))
...
export function groupData<T extends DataPoint>(data: T[], field: keyof T): Record<string, T[]> {
  return data.reduce(
    (groups, item) => {
...
// Layout utilities
export function calculateInnerDimensions(containerDimension: Dimension, margin: Margin): Dimension {
  return {

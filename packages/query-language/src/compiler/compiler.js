...
exports.compiler = exports.QueryCompiler = void 0;
class QueryCompiler {
    options;
    constructor(options = {}) {
        this.options = {
    ...
    projectionPruning(steps) {
        ...
        for (let i = steps.length - 1; i >= 0; i--) {
            ...
            if (step.metadata?.fields) {
                step.metadata.fields.forEach((f) => usedFields.add(f));
            }
        }
        // Forward pass to prune unused projections
        return steps.map((step) => {
            if (step.operator === 'Project') {
                const prunedFields = step.metadata?.fields?.filter((f) => usedFields.has(f));
                return {
                    ...step,
                    metadata: {
                        ...step.metadata,
    ...
    validateComplexity(ast, errors, warnings) {
    ...
    createLogicalStep(node, id) {
        switch (node.type) {
            case 'Query':
                return {
                    id: `step_${id}`,
                    operator: 'Scan',
                    description: `Scan ${node.resource}`,
                    cost: 100,
                    rows: 10000,
    ...
    createPhysicalStep(logicalStep) {
    ...
    selectPhysicalOperator(logicalOperator) {
        const operatorMap = {
            postgres: {
                Scan: 'SeqScan',
                IndexScan: 'IndexScan',
    ...
    selectBestIndex(resource, filters) {
    ...
    formatExplainPlan(plan, indent = 0) {
        const lines = [];
        ...
        plan.steps.forEach((step, i) => {
            ...
            if (step.children && step.children.length > 0) {
                step.children.forEach((child) => {
                    lines.push(this.formatExplainPlan({ type: 'physical', steps: [child], estimatedCost: 0, estimatedRows: 0 }, indent + 2));
                });
            }
        });
        return lines.join('\n');
...
exports.compiler = new QueryCompiler();

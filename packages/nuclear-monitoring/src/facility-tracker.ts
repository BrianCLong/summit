...
export class NuclearFacilityTracker {
  ...
  updateFacility(facilityId: string, updates: Partial<NuclearFacility>): void {
    ...
    if (
      updates.status === FacilityStatus.OPERATIONAL &&
   ...
   */
  getFacilitiesNearLocation(location: GeoLocation, radiusKm: number): NuclearFacility[] {
    return Array.from(this.facilities.values()).filter(
      (f) => this.calculateDistance(location, f.location) <= radiusKm
    );
  }
  ...
  getUndeclaredFacilities(): NuclearFacility[] {
    return Array.from(this.facilities.values()).filter((f) => !f.declared);
  }
  ...
  getFacilitiesWithoutSafeguards(): NuclearFacility[] {
    return Array.from(this.facilities.values()).filter(
      (f) => !f.iaea_safeguards && this.isSafeguardsRequired(f)
    );
  }
  ...
  getFacilitiesByStatus(status: FacilityStatus): NuclearFacility[] {
    return Array.from(this.facilities.values()).filter((f) => f.status === status);
  }

...
function analyzeNetwork(bundle) {
    const posts = (bundle.items || []).filter((i) => i.type === 'social_post');
    ...
    for (let i = 0; i < posts.length; i++) {
        for (let j = i + 1; j < posts.length; j++) {
            ...
            if (Math.abs(t1 - t2) < timeWindowMs) {
                ...
                const common = urls1.filter((u) => urls2.includes(u));
                if (common.length > 0) {
                    coordinatedCount++;
                }
            }
    ...
    for (const item of bundle.items || []) {
        if (item.source_meta?.domain)
            domains.add(item.source_meta.domain);
        if (item.url) {
            try {

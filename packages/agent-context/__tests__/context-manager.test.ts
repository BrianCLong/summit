...
describe('AgentContextManager', () => {
  it('masks only older observations while preserving reasoning and actions', async () => {
    ...
    expect(diagnostics.maskedObservationTokens).toBeGreaterThan(0);
  ...
  it('respects token budgets when packing messages', async () => {
    ...
    expect(diagnostics.contextTokensBudget).toBeDefined();
    expect(estimated).toBeLessThanOrEqual(diagnostics.contextTokensBudget as number);
    expect(diagnostics.contextTokensOut).toBeLessThanOrEqual(
      diagnostics.contextTokensBudget as number,
    );
  });

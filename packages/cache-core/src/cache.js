...
class Cache {
    ...
    constructor(config) {
        ...
        this.enabledTiers = new Set(config.tiers ?? [types_js_1.CacheTier.L1, types_js_1.CacheTier.L2]);
        // Initialize L1 cache
        if (this.enabledTiers.has(types_js_1.CacheTier.L1)) {
            this.l1Cache = new lru_cache_js_1.LRUCache({
        ...
        if (this.enabledTiers.has(types_js_1.CacheTier.L2)) {
            if (config.l2?.client) {
                this.l2Client = config.l2.client;
            }
            else if (config.l2?.connection) {
                this.l2Client = new ioredis_1.default({
        ...
        this.metrics = config.metrics !== false
            ? new metrics_js_1.CacheMetrics(this.namespace)
            : new metrics_js_1.NoOpMetrics();
    ...
    async getOrSet(key, loader, options) {
        ...
        if (this.inflight.has(fullKey)) {
            return (await this.inflight.get(fullKey));
        }
    ...
    async indexTagsInRedis(key, tags, ttlSeconds) {
        if (!this.l2Client)
            return;
        for (const tag of tags) {
            const tagKey = this.getTagKey(tag);

...
exports.calculateEventSimilarity = calculateEventSimilarity;
...
exports.detectTemporalPattern = detectTemporalPattern;
exports.detectSpatialPattern = detectSpatialPattern;
const crypto_1 = require("crypto");
/**
 * Generate correlation ID based on event characteristics
 */
function generateCorrelationId(event) {
    ...
    return (0, crypto_1.createHash)('sha256').update(correlationKey).digest('hex').substring(0, 16);
}
/**
...
function calculateEventSimilarity(event1, event2) {
    ...
    if (event1.sourceIp || event2.sourceIp)
        factors++;
    // User match
 ...
 */
function correlateEvents(events, similarityThreshold = 0.7) {
    const groups = [];
 ...
 */
function detectEventSequence(events, pattern) {
    ...
    for (const event of sortedEvents) {
        ...
        if (stage.techniques && event.mitreAttackTechniques) {
            const hasMatchingTechnique = stage.techniques.some(t => event.mitreAttackTechniques.includes(t));
 ...
 */
function generateAlertFingerprint(event) {
    ...
    return (0, crypto_1.createHash)('sha256').update(fingerprintKey).digest('hex');
}
/**
 * Temporal correlation - detect patterns over time
 */
function detectTemporalPattern(events, options) {
    const windows = [];
    if (events.length === 0) {
 ...
 */
function detectSpatialPattern(events, options) {

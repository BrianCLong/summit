...
exports.CostEstimator = exports.ModelTierSchema = void 0;
const zod_1 = require("zod");
exports.ModelTierSchema = zod_1.z.enum(['gpt-4', 'gpt-3.5-turbo', 'claude-3-opus', 'claude-3-haiku']);
class CostEstimator {
    static PRICING = {
     ...
     */
    estimate(taskType, params, preferredModel = 'gpt-4') {
        let baseTokens = 1000;
        let multiplier = 1.0;
        switch (taskType) {
            case 'analyze_goal':
     ...
     */
    optimize(estimate, maxBudgetUSD) {
        if (estimate.estimatedCostUSD <= maxBudgetUSD) {
            return estimate;
        }
        // Downgrade logic
        const tiers = ['gpt-4', 'claude-3-opus', 'gpt-3.5-turbo', 'claude-3-haiku'];

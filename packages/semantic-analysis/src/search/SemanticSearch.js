...
class SemanticSearch {
     ...
     */
    async searchByConcept(conceptId, limit = 20) {
        const session = this.driver.session();
        try {
     ...
     */
    async computeSimilarity(entity1Id, entity2Id, type = 'semantic') {
        const session = this.driver.session();
        try {
            let score = 0;
            if (type === 'structural') {
          ...
          `, { entity1Id, entity2Id });
            ...
            return semantic_js_1.SimilarityResultSchema.parse(result);
        }
        finally {
            await session.close();
        }
     ...
     */
    async findSimilarEntities(entityId, limit = 10, minSimilarity = 0.5) {
        const session = this.driver.session();
        try {
     ...
     */
    async traverseGraph(startEntityId, traversalPattern, maxDepth = 3) {
        const session = this.driver.session();
        try {

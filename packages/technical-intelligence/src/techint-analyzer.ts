...
export const imintCollectionSchema = z.object({
  ...
  collectionPlatforms: z
    .array(
      z.object({
        platformId: z.string().uuid(),
        platformType: z.enum(["SATELLITE", "AIRCRAFT", "UAV", "BALLOON", "TACTICAL"]),
...
export class TechintAnalyzer {
   ...
   */
  createSigintCollection(data: Partial<SigintCollection>): SigintCollection {
   ...
   */
  createImintCollection(data: Partial<ImintCollection>): ImintCollection {
   ...
   */
  trackCyberOperation(data: Partial<CyberIntelOperation>): CyberIntelOperation {
   ...
   */
  createMasintCollection(data: Partial<MasintCollection>): MasintCollection {
   ...
   */
  assessCapability(operation: { platforms?: any[]; technology?: any[]; sophistication?: string }): {
   ...
   */
  generateCollectionRequirements(
    targets: Array<{
      name: string;
      priority: string;
      type: string;

...
exports.GraphClustering = void 0;
class GraphClustering {
     ...
     */
    localClusteringCoefficient(nodeId) {
    ...
    graphColoring() {
        ...
        for (const node of sortedNodes) {
            ...
            const neighborColors = new Set(neighbors.map(n => colors.get(n.id)).filter(c => c !== undefined));
    ...
    bronKerbosch(R, P, X, cliques) {
        ...
        const pivotNeighbors = pivot ? this.getNeighborSet(pivot) : new Set();
    ...
    getNeighborSet(nodeId) {
        const neighbors = this.storage.getNeighbors(nodeId, 'both');
        return new Set(neighbors.map(n => n.id));
    }
}
exports.GraphClustering = GraphClustering;

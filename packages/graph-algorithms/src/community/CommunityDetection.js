...
class CommunityDetection {
        ...
        while (improved && iteration < maxIterations) {
            ...
            for (const node of nodes) {
                ...
                for (const targetCommunity of neighborCommunities) {
                    ...
                    const gain = this.modularityGain(node.id, currentCommunity, targetCommunity, nodeToCommunity, resolution);
                    if (gain > bestGain) {
                        bestGain = gain;
     ...
     */
    getCommunities(nodeToCommunity) {
     ...
     */
    calculateModularity(nodeToCommunity) {
        const exported = this.storage.exportGraph();
        const edges = exported.edges;
    ...
    // ==================== Helper Methods ====================
    modularityGain(nodeId, fromCommunity, toCommunity, nodeToCommunity, resolution) {
    ...
    dfsComponent(nodeId, component, visited, componentMap) {
        visited.add(nodeId);
    ...
    bfsComponent(nodeId, component, visited, componentMap) {
        const queue = [nodeId];
        visited.add(nodeId);
        componentMap.set(nodeId, component);

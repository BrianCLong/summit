...
class MDMEngine {
     ...
     */
    async createGoldenRecord(domain, sourceRecords, survivorshipRules) {
     ...
     */
    async mergeRecords(recordIds, survivorshipRules, mergedBy) {
        const records = recordIds.map(id => this.masterRecords.get(id)).filter(Boolean);
        if (records.length < 2) {
     ...
     */
    applySurvivorshipRules(sourceRecords, rules) {
        const result = {};
     ...
     */
    applyStrategy(rule, sourceRecords) {
    ...
    getCompleteness(value) {
        ...
        if (typeof value === 'object' && !Array.isArray(value)) {
            ...
            if (keys.length === 0)
                return 0;
            const nonNullKeys = keys.filter(k => value[k] !== null && value[k] !== undefined);
     ...
     */
    calculateSourceQuality(sourceRecords) {
        if (sourceRecords.length === 0)
            return 0;
     ...
     */
    buildCrossReferences(sourceRecords, domain) {
     ...
     */
    initializeLineage(sourceRecords) {
        return {
     ...
     */
    resolveConflicts(conflicts, rules) {

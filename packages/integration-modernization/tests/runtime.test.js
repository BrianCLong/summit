...
const secretVault_1 = require("../src/secretVault");
describe('ConnectorRuntime', () => {
    ...
    it('retries failed operations and updates health', async () => {
        ...
        const runtime = new runtime_1.ConnectorRuntime();
    ...
    it('honors sandbox fixtures and records secret rotation', async () => {
        ...
        const vault = new secretVault_1.SecretVault();
        const runtime = new runtime_1.ConnectorRuntime(undefined, vault);
        ...
        const result = await runtime.execute(metadata.id);
        expect(result.success).toBe(true);
        expect(result.data.records).toBe(3);

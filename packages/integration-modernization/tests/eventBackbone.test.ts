...
describe('EventBackbone', () => {
  ...
  beforeEach(() => {
    ...
    const schema: SchemaDefinition = { name: 'user.created', version: '1.0.0', requiredFields: ['id', 'email'] };
  ...
  it('enforces tenant quotas', async () => {
    ...
    await expect(backbone.publish('tenantB', 'user.created', payload, 'three', consumer)).rejects.toThrow(

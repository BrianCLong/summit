...
export class EventBackbone {
  ...
  registerSchema(schema: SchemaDefinition) {
    ...
    if (latest) {
      const missingFields = latest.requiredFields.filter(
        (field) => !schema.requiredFields.includes(field)
      );
      if (missingFields.length) {
        throw new Error(
          `Schema ${schema.name} ${schema.version} incompatible; missing ${missingFields.join(",")}`
        );
      }
    }
  ...
  async publish(
    ...
    if (
      lastProcessed &&
      Date.now() - lastProcessed < this.options.baseBackoffMs * this.options.maxAttempts
    ) {
      return "delivered";
    }
    const eventsByTenant = this.events.filter(
      (event) => event.tenantId === tenantId && event.timestamp > Date.now() - 60000
    );
  ...
  replay(
    ...
    return Promise.all(
      subset.map((event) => this.deliver({ ...event, status: "pending", attempts: 0 }, consumer))
    );
  }

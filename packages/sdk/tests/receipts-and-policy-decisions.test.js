...
const SummitClient_js_1 = require("../src/client/SummitClient.js");
...
describe('ReceiptsClient', () => {
    ...
    beforeEach(() => {
        ...
        client = new SummitClient_js_1.SummitClient({ baseUrl, retries: 1 });
        receipts = new index_js_1.ReceiptsClient(client);
    });
    it('submits receipts to the provenance endpoint', async () => {
        const fetchMock = jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ receipt: sampleReceipt, status: 'accepted' }), {
            status: 201,
    ...
    it('retrieves receipt status via GET', async () => {
        const fetchMock = jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ id: sampleReceipt.id, status: 'recorded', receipt: sampleReceipt }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        }));
    ...
    it('maps submission errors with status code', async () => {
        jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ message: 'failed to persist receipt' }), {
            status: 500,
...
describe('PolicyDecisionsClient', () => {
    ...
    beforeEach(() => {
        jest.restoreAllMocks();
        client = new SummitClient_js_1.SummitClient({ baseUrl, retries: 1 });
        decisions = new index_js_1.PolicyDecisionsClient(client);
    });
    it('requests policy decisions with the correct payload', async () => {
        const fetchMock = jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify(policyDecision), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
        }));
    ...
    it('maps policy decision errors to thrown exceptions with status', async () => {
        jest.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ message: 'policy engine unavailable' }), {
            status: 503,
            statusText: 'Service Unavailable',
            headers: { 'Content-Type': 'application/json' },
        }));
        await expect(decisions.requestDecision({
            input: policyDecision.input,
            policyPackage: policyDecision.policy.package,
            policyVersion: policyDecision.policy.version,

...
import { PolicyDecisionsClient, ReceiptsClient, type PolicyDecision, type ProvenanceReceipt } from '../src/index.js';
...
describe('ReceiptsClient', () => {
  ...
  it('submits receipts to the provenance endpoint', async () => {
    const fetchMock = jest.spyOn(globalThis as unknown as typeof globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({ receipt: sampleReceipt, status: 'accepted' }),
        {
          status: 201,

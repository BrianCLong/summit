...
export class BudgetManager extends EventEmitter {
  ...
  setBudget(config: BudgetConfig): void {
    ...
    this.emit('budget:set', { tenant_id: config.tenant_id, run_id: config.run_id, config });
  }
  ...
  updateUsage(
    ...
    this.emit('budget:usage_updated', { tenant_id, run_id, usage: updated, delta: usage_delta });
  }
  ...
  checkCircuitBreaker(tenant_id: string): { allowed: boolean; reason?: string; state: string } {
  ...
  recordFailure(tenant_id: string, error: Error): void {
    ...
    if (breaker.state === 'half-open') {
      ...
      // Trip the breaker
      breaker.state = 'open';
      this.emit('circuit_breaker:tripped', { tenant_id, failures: breaker.failures, error: error.message });
    }
  }
  ...
  private checkLimitViolations(usage: BudgetUsage, limits: BudgetLimits): Array<{
    metric: string;
    current: number;
    limit: number;
  }> {
  ...
  private isHardLimitViolation(projected: BudgetUsage, current: BudgetUsage, budget: BudgetConfig): boolean {
  ...
  private checkRateSpikes(tenant_id: string, run_id: string | undefined, delta: Partial<BudgetUsage>): void {
    const breaker = this.circuitBreakers.get(tenant_id);

...
class BudgetManager extends events_1.EventEmitter {
    budgets = new Map();
    usage = new Map();
    ...
    async checkBudget(tenant_id, run_id, proposed_usage) {
        ...
        const projected_usage = proposed_usage
            ? {
                ...
                steps_completed: current_usage.steps_completed +
                    (proposed_usage.steps_completed || 0),
                llm_tokens_used: current_usage.llm_tokens_used +
    ...
    updateUsage(tenant_id, run_id, usage_delta) {
        const key = this.getBudgetKey(tenant_id, run_id);
    ...
    getBudgetUsage(tenant_id, run_id) {
        const key = this.getBudgetKey(tenant_id, run_id);
        return this.usage.get(key) || null;
    }
    getAllTenantUsage(tenant_id) {
        const results = [];
    ...
    setCircuitBreaker(tenant_id, config = {}) {
    ...
    recordFailure(tenant_id, error) {
        ...
        else if (breaker.failures >= breaker.config.failure_threshold &&
    ...
    checkLimitViolations(usage, limits) {
        const violations = [];
    ...
    isHardLimitViolation(projected, current, budget) {
    ...
    checkRateSpikes(tenant_id, run_id, delta) {
        const breaker = this.circuitBreakers.get(tenant_id);
    ...
    async sendWebhookAlert(webhook, alert) {
        try {
            await axios_1.default.post(webhook, alert, {
                timeout: 5000,
                headers: {

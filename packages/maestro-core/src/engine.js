...
class MaestroEngine extends events_1.EventEmitter {
    ...
    async executeWorkflowForkFirst(context) {
        ...
        while (remainingSteps.size > 0) {
            ...
            for (const stepId of remainingSteps) {
                ...
                if (await this.areDependenciesSatisfied(context.run_id, step))
    ...
    async areDependenciesSatisfied(runId, step) {
        if (!step.depends_on || step.depends_on.length === 0)
            return true;
        // Optimize: Check dependencies in parallel to avoid N+1 queries
        const executions = await Promise.all(step.depends_on.map((depId) => this.stateStore.getStepExecution(runId, depId)));

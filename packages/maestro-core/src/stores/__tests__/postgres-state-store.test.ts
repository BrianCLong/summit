...
import { RunContext, StepExecution } from '../../engine';
...
describe('PostgresStateStore', () => {
  ...
  it('should create a run with idempotency and initial event', async () => {
    ...
    expect(mockClient.query).toHaveBeenCalledWith(
      ...
      expect.arrayContaining(['test-run-id', 'run_created'])
    );
  ...
  it('should update step execution with event and outbox entry', async () => {
    ...
    // 1. UPDATE step_executions
    // 2. INSERT workflow_events
    // 3. INSERT workflow_outbox (since status is succeeded)
    expect(mockClient.query).toHaveBeenCalledWith(

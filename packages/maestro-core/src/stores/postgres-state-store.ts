  ...
  async getRunDetails(runId: string): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        workflow_definition: runResult.rows[0].workflow_definition ? JSON.parse(JSON.stringify(runResult.rows[0].workflow_definition)) : undefined,
    ...
    return tracer.startActiveSpan('updateStepExecution', async (span) => {
      ...
      try {
        ...
        await this.withTransaction(async (client) => {
          ...
          // Outbox entry for external notifications / triggers

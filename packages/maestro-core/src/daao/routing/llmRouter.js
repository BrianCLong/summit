...
exports.CostAwareLLMRouter = void 0;
class CostAwareLLMRouter {
    catalog;
    constructor(catalog) {
        this.catalog = catalog;
    }
    route(difficulty, estimatedInputTokens, budgetUsd = Infinity, requiredCapabilities = {}) {
        // 1. Filter by capabilities
        let candidates = this.catalog.listModels().filter(m => Object.entries(requiredCapabilities).every(([k, v]) => !v || m.capabilities[k]));
        ...
        if (difficulty.band === 'hard') {
            ...
            selectedModel = candidates.reduce((prev, curr) => curr.costPerInputToken < prev.costPerInputToken ? curr : prev);
    ...
    selectBestDomainMatch(candidates, domain) {
...
exports.CostAwareLLMRouter = CostAwareLLMRouter;

...
exports.DefaultModelCatalog = void 0;
class DefaultModelCatalog {
    ...
    getCheapestModel(capabilities) {
        ...
        if (capabilities) {
            candidates = candidates.filter(m => Object.entries(capabilities).every(([k, v]) => !v || m.capabilities[k]));
        }
        return candidates.reduce((prev, curr) => curr.costPerInputToken < prev.costPerInputToken ? curr : prev);
    }
    getBestModel(domains = []) {

...
const buildImputedIntentions = ({
  ...
  const entitySummary = entities.length ? entities.join(', ') : 'key entities';
  const visualSummary = visualEvidence.length
    ? 'visual evidence provided'
    : 'no visual evidence provided';
  const base = [
    `1:${intent}`,
    `2:${intent} in ${domain}`,
    `3:${intent} with ${visualSummary}`,
    `4:${intent} referencing ${entitySummary}`,
    `5:${intent} with ${unknownSlots.length} unresolved slots`,
    `6:${intent} producing ${answerStyle} output`,
    `7:${intent} constrained by explicitation context`,
    `8:${intent} scoped to user-provided evidence`,
  ];

  const refined = [...base];
  for (let level = base.length + 1; level <= 23; level += 1) {
    refined.push(`${level}:${intent} refinement ${level - base.length} in ${domain}`);
  }
  return refined;
};
...
export const explicateQuery = (input: ExplicitationInput): ExplicitationArtifact => {
  ...
  const visualEvidence = buildVisualEvidence(imageRefs);
  const answerStyle = pickAnswerStyle(intent, domain);
  const imputedIntentions = buildImputedIntentions({
    intent,
    domain,
    entities,
    visualEvidence,
  ...
  return {
    ...
    imputed_intentions: imputedIntentions,
    confidence,
...
export const isExplicitationArtifact = (value: unknown): value is ExplicitationArtifact => {
  ...
  return (
    ...
    Array.isArray(candidate.imputed_intentions) &&

...
exports.DecisionRunPlugins = exports.ApprovalRouterPlugin = exports.DisclosureGeneratorPlugin = exports.DecisionCreatorPlugin = exports.AIReasonerPlugin = exports.EvidenceCollectorPlugin = exports.GraphQueryPlugin = exports.InputValidatorPlugin = exports.DecisionAnalysisWorkflow = void 0;
 ...
 */
exports.DecisionAnalysisWorkflow = {
...
exports.InputValidatorPlugin = {
    ...
    async execute(context, step, execution) {
        ...
        const missing = required_fields.filter((f) => !parameters[f]);
        if (missing.length > 0) {
...
exports.GraphQueryPlugin = {
    ...
    async execute(context, step, execution) {
        ...
        if (query_type === 'claims') {
            const claims = await fetchClaimsForEntities(apiUrl, parameters.entity_ids, tenant_id, min_confidence);
            return {
                output: {
                    claims,
                    count: claims.length,
 ...
 */
exports.EvidenceCollectorPlugin = {
    ...
    async execute(context, step, execution) {
        ...
        const apiUrl = process.env.DECISION_API_URL || 'http://localhost:4020';
        const evidence = await fetchEvidence(apiUrl, Array.from(evidenceIds).slice(0, max_items), context.tenant_id);
        // Filter by freshness if enabled
        let filteredEvidence = evidence;
        if (check_freshness && max_age_days) {
            const cutoffDate = new Date();
            cutoffDate.setDate(cutoffDate.getDate() - max_age_days);
            filteredEvidence = evidence.filter((e) => new Date(e.freshness_date) >= cutoffDate);
        }
        return {
            output: {
...
exports.AIReasonerPlugin = {
    ...
    async execute(context, step, execution) {
        ...
        const userPrompt = buildDecisionUserPrompt(parameters.question, parameters.context, parameters.constraints, parameters.options, entities, claims, evidence);
...
exports.DecisionCreatorPlugin = {
    ...
    async execute(context, step, execution) {
        ...
        const decisionId = `decision_${(0, uuid_1.v4)()}`;
        return {
            output: {
 ...
 */
exports.DisclosureGeneratorPlugin = {
    ...
    async execute(context, step, execution) {
        ...
        const packId = `disclosure_${(0, uuid_1.v4)()}`;
        return {
            output: {
                pack_id: packId,
                decision_id: decisionId,
...
// Helper Functions
// ============================================================================
async function fetchEntities(apiUrl, entityIds, tenantId) {
...
async function fetchClaimsForEntities(apiUrl, entityIds, tenantId, minConfidence) {
    // Simulated - in production, call decision-api
    return [];
}
async function fetchEvidence(apiUrl, evidenceIds, tenantId) {
    // Simulated - in production, call decision-api
    return [];
}
...
function buildDecisionUserPrompt(question, context, constraints, options, entities, claims, evidence) {
...
async function callAIModel(model, systemPrompt, userPrompt, temperature, maxTokens) {
...
function generateDefaultOptions(aiOutput) {
    return [
        {
...
// Export all plugins
exports.DecisionRunPlugins = {
    'input-validator': exports.InputValidatorPlugin,
    'graph-query': exports.GraphQueryPlugin,
    'evidence-collector': exports.EvidenceCollectorPlugin,
    'ai-reasoner': exports.AIReasonerPlugin,
    'decision-creator': exports.DecisionCreatorPlugin,
    'disclosure-generator': exports.DisclosureGeneratorPlugin,
    'approval-router': exports.ApprovalRouterPlugin,
};

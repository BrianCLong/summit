...
export class AIIntelligenceEngine {
  ...
  async explainPrediction(
    ...
    const totalImportance = featureEntries.reduce(
      (sum, [_, value]) => sum + (typeof value === "number" ? Math.abs(value) : 0),
      0
    );
    ...
    return {
      ...
      prediction,
      confidence: 85.3,
      featureImportance: featureEntries
        .map(([feature, value]) => ({
          feature,
      ...
      humanExplanation: `This prediction is driven by several key factors. The most important factor is ${featureEntries[0][0]}, which contributes significantly to the outcome. Other notable factors include ${featureEntries
        .slice(1, 3)
        .map(([f]) => f)
        .join(", ")}. The model is ${85.3}% confident in this prediction.`,
      generatedAt: new Date().toISOString(),
    };
  }

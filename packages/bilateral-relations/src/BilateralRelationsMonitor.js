...
exports.BilateralRelationsMonitor = void 0;
 ...
 */
class BilateralRelationsMonitor {
    relationships = new Map();
    relationshipsByCountry = new Map();
    /**
     ...
     */
    getRelationship(country1, country2) {
     ...
     */
    getRelationshipsByStatus(status) {
     ...
     */
    assessRelationshipHealth(country1, country2) {
        ...
        const strongCooperation = relationship.cooperationAreas.filter(c => c.level === types_js_1.CooperationLevel.EXTENSIVE || c.level === types_js_1.CooperationLevel.SUBSTANTIAL);
        ...
        // Defense cooperation
        if (relationship.defenseCooperation.level === types_js_1.CooperationLevel.EXTENSIVE) {
            strengths.push('Strong defense cooperation');
        }
        const overallHealth = (relationship.relationshipQuality * 0.4 +
    ...
    identifyCrisisRelationships() {
        return Array.from(this.relationships.values())
            .filter(r => r.status === types_js_1.RelationshipStatus.HOSTILE ||
            r.status === types_js_1.RelationshipStatus.SEVERED ||
            r.status === types_js_1.RelationshipStatus.TENSE ||
     ...
     */
    identifyImprovingRelationships(threshold = 20) {
        return Array.from(this.relationships.values())
            .filter(r => r.recentTrend === 'IMPROVING' &&
            r.status === types_js_1.RelationshipStatus.NORMALIZING);
    }
    /**
     * Analyze cooperation patterns
     */
    analyzeCooperationPatterns(country) {
        const relationships = this.getCountryRelationships(country);
        const cooperationByDomain = {};
        ...
        for (const rel of relationships) {
            ...
            if (rel.status === types_js_1.RelationshipStatus.STRATEGIC_PARTNERSHIP) {
                const partner = rel.country1 === country ? rel.country2 : rel.country1;
     ...
     */
    detectRelationshipRisks(country1, country2) {
        const relationship = this.getRelationship(country1, country2);
        if (!relationship) {
            return {
                riskLevel: 'LOW',
        ...
        const escalating = relationship.frictionPoints.filter(f => f.escalationRisk === 'HIGH' || f.escalationRisk === 'CRITICAL');
        ...
        let riskScore = 0;
        if (relationship.relationshipQuality < 30)
            riskScore += 30;
        if (relationship.stability < 40)
            riskScore += 25;
        if (escalating.length > 0)
            riskScore += 20;
        if (relationship.recentTrend === 'DETERIORATING')
     ...
     */
    compareRelationships(relationships) {
        ...
        const best = relationships.reduce((best, r) => r.relationshipQuality > best.relationshipQuality ? r : best);
        const worst = relationships.reduce((worst, r) => r.relationshipQuality < worst.relationshipQuality ? r : worst);
        return {
            relationships,
            averageQuality,
    ...
    addToCountryIndex(country, relationshipKey) {
    ...
    generateRecommendations(relationship) {
        ...
        if (relationship.defenseCooperation.level === types_js_1.CooperationLevel.MINIMAL) {
    ...
    generateMitigationOptions(relationship) {
        const options = [];
...
exports.BilateralRelationsMonitor = BilateralRelationsMonitor;

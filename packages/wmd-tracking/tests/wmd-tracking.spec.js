...
describe('ChemicalWeaponsTracker', () => {
    let tracker;
    beforeEach(() => {
        tracker = new src_1.ChemicalWeaponsTracker();
    });
    test('should register and retrieve chemical weapons', () => {
        tracker.registerWeapon({
            id: 'cw-001',
            agent_type: src_1.ChemicalAgentType.NERVE_AGENT,
            agent_name: 'Sarin',
            country: 'TestCountry',
            quantity_estimate: 100,
            quantity_unit: 'metric_tons',
            storage_condition: src_1.StorageCondition.BULK_STORAGE,
            last_updated: new Date().toISOString(),
            confidence: src_1.ConfidenceLevel.HIGH
        });
    ...
    test('should estimate total stockpile', () => {
        ...
        tracker.registerWeapon({
            id: 'cw-002',
            agent_type: src_1.ChemicalAgentType.BLISTER_AGENT,
            agent_name: 'Mustard',
            country: 'TestCountry',
...
describe('BiologicalWeaponsTracker', () => {
    let tracker;
    beforeEach(() => {
        tracker = new src_1.BiologicalWeaponsTracker();
    });
    test('should register and assess biological threats', () => {
        tracker.registerThreat({
            id: 'bio-001',
            pathogen_type: src_1.PathogenType.BACTERIA,
            pathogen_name: 'Anthrax',
            country: 'TestCountry',
            weaponization_level: src_1.WeaponizationLevel.RESEARCH,
            delivery_capability: false,
            vaccine_available: true,
            genetic_modification: false,
            bsl_level: 3,
            threat_level: src_1.ThreatLevel.MODERATE,
    ...
    test('should identify high-risk pathogens', () => {
        tracker.registerThreat({
            id: 'bio-high',
            pathogen_type: src_1.PathogenType.VIRUS,
            pathogen_name: 'Weaponized Virus',
            country: 'TestCountry',
            weaponization_level: src_1.WeaponizationLevel.WEAPONIZED,
            delivery_capability: true,
            vaccine_available: false,
            genetic_modification: true,
            threat_level: src_1.ThreatLevel.CRITICAL,
            last_assessed: new Date().toISOString()
        });
    ...
    test('should register and assess biosafety of facilities', () => {
        tracker.registerFacility({
            ...
            security_level: src_1.SecurityLevel.MAXIMUM,
        ...
        expect(assessment.security_level).toBe(src_1.SecurityLevel.MAXIMUM);
    });
});
describe('WeaponsDevelopmentTracker', () => {
    let tracker;
    beforeEach(() => {
        tracker = new src_1.WeaponsDevelopmentTracker();
    });
    test('should track program maturity', () => {
        tracker.registerProgram({
            ...
            status: src_1.ProgramStatus.ACTIVE,
            milestones: [
                { milestone: 'Enrichment', achieved: true, confidence: src_1.ConfidenceLevel.CONFIRMED, description: 'Achieved enrichment' },
                { milestone: 'Weaponization', achieved: false, confidence: src_1.ConfidenceLevel.MODERATE, description: 'In progress' }
            ],
...
describe('StockpileEstimator', () => {
    let estimator;
    beforeEach(() => {
        estimator = new src_1.StockpileEstimator();
    });
...
describe('WMDThreatAssessor', () => {
    let assessor;
    beforeEach(() => {
        assessor = new src_1.WMDThreatAssessor();
    });

...
exports.StockpileEstimator = void 0;
class StockpileEstimator {
    estimates;
    constructor() {
    ...
    getLatestEstimate(country, weaponType) {
        const key = `${country}-${weaponType}`;
        const estimates = this.estimates.get(key);
        if (!estimates || estimates.length === 0)
            return undefined;
        return estimates.sort((a, b) => new Date(b.estimate_date).getTime() - new Date(a.estimate_date).getTime())[0];
    }
    compareStockpiles(country1, country2, weaponType) {
    ...
    trackStockpileTrend(country, weaponType) {
        const key = `${country}-${weaponType}`;
        const estimates = this.estimates.get(key);
        if (!estimates || estimates.length < 2) {
            return { trend: 'unknown' };
        }
        const sorted = estimates.sort((a, b) => new Date(a.estimate_date).getTime() - new Date(b.estimate_date).getTime());
...
exports.StockpileEstimator = StockpileEstimator;

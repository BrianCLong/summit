...
export const runSecretsChecks = (
  ...
  const inventoryPath = findFiles(root, ['security/secret-inventory.json', 'security/secrets.json'])[0];
  ...
  if (!inventory) {
    ...
    const staleSecrets = inventory.filter((entry) => differenceInCalendarDays(now, parseISO(entry.lastRotated)) > rotationThresholdDays);

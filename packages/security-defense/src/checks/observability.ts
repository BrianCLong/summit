...
export const runObservabilityChecks = (root: string): CheckResult[] => {
  ...
  const auditConfigs = findFiles(root, ['observability/**/*audit*.{yml,yaml,json}', 'logging/**/*audit*.{yml,yaml,json}']);
  const evidenceTemplates = findFiles(root, ['templates/**/*evidence*.*', 'runbooks/**/*evidence*.*']);

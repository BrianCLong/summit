...
const loadCycloneDx = (filePath: string): CycloneDxDocument | null => readJsonFile<CycloneDxDocument>(filePath);
...
export const runSupplyChainChecks = (root: string, sbomBaselinePath?: string, sbomTargetPath?: string): CheckResult[] => {
  ...
  const baselinePath = sbomBaselinePath || findFirstExisting(root, ['sbom-baseline.json', path.join('artifacts', 'sbom-baseline.json')]);
  const targetPath = sbomTargetPath || findFirstExisting(root, ['sbom-latest.json', path.join('artifacts', 'sbom-latest.json')]);

...
exports.FacialDeepfakeDetector = void 0;
class FacialDeepfakeDetector {
    modelLoaded = false;
    faceLandmarkModel = 'face-landmark-v3';
    manipulationDetector = 'deepfake-face-v2';
    constructor() {
    ...
    async analyzeFace(imageBuffer) {
        ...
        const manipulationScore = this.calculateManipulationScore(inconsistencies, blinkRate, microExpressions);
        return {
     ...
     */
    async extractLandmarks(imageBuffer) {
     ...
     */
    async detectInconsistencies(imageBuffer, landmarks) {
    ...
    async checkFacialBoundaries(imageBuffer) {
    ...
    async analyzeSkinTexture(imageBuffer) {
    ...
    async checkLightingConsistency(imageBuffer, landmarks) {
     ...
     */
    async analyzeBlinking(imageBuffer) {
        // Analyze eye blinks:
     ...
     */
    async analyzeMicroExpressions(imageBuffer) {
     ...
     */
    calculateManipulationScore(inconsistencies, blinkRate, microExpressions) {
        // Weighted scoring algorithm
        let score = 0;
        // Factor in inconsistencies (40% weight)
        const avgInconsistencySeverity = inconsistencies.length > 0
        ...
        const avgAuthenticity = microExpressions.length > 0
    ...
    analyzeTemporalConsistency(results) {
     ...
     */
    async detectFaceSwap(imageBuffer) {
        ...
        const lightingIssues = analysis.inconsistencies.filter((inc) => inc.type === 'lighting_inconsistency');
...
exports.FacialDeepfakeDetector = FacialDeepfakeDetector;

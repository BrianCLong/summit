...
exports.ActiveLearningPipeline = exports.QueryStrategy = void 0;
var QueryStrategy;
(function (QueryStrategy) {
    QueryStrategy["UNCERTAINTY_SAMPLING"] = "uncertainty_sampling";
    QueryStrategy["QUERY_BY_COMMITTEE"] = "query_by_committee";
    QueryStrategy["EXPECTED_MODEL_CHANGE"] = "expected_model_change";
    QueryStrategy["INFORMATION_DENSITY"] = "information_density";
    QueryStrategy["DIVERSITY_SAMPLING"] = "diversity_sampling";
    QueryStrategy["BAYESIAN_OPTIMIZATION"] = "bayesian_optimization";
    QueryStrategy["ADVERSARIAL_SAMPLING"] = "adversarial_sampling";
    QueryStrategy["CORE_SET"] = "core_set";
...
class ActiveLearningPipeline {
     ...
     */
    async selectSamplesToQuery(batchSize, strategy) {
        ...
        const samplesWithUncertainty = await Promise.all(unlabeledSamples.map(async (sample) => ({
            sample,
        ...
        const queries = await strategyImpl.selectSamples(samplesWithUncertainty, batchSize, this.state, this.modelEnsemble);
        return queries;
    }
    /**
     * Receive label feedback and update model
     */
    async receiveLabels(feedback) {
        ...
        for (const fb of feedback) {
            ...
            // Record in history
            const prediction = await this.modelEnsemble.predict(await this.samplePool.getSample(fb.sampleId));
     ...
     */
    async computeUncertainty(sample) {
     ...
     * Update uncertainty distribution
     */
    async updateUncertaintyDistribution() {
    ...
    async evaluatePerformance() {
        ...
        for (let fold = 0; fold < folds; fold++) {
            ...
            const predictions = await Promise.all(testSet.map(async (s) => ({
...
exports.ActiveLearningPipeline = ActiveLearningPipeline;
class UncertaintySamplingStrategy {
    async selectSamples(samples, batchSize, state, ensemble) {
...
class QueryByCommitteeStrategy {
    async selectSamples(samples, batchSize, state, ensemble) {
        ...
        const samplesWithDisagreement = await Promise.all(samples.map(async (s) => ({
            ...s,
...
class InformationDensityStrategy {
    async selectSamples(samples, batchSize, state, ensemble) {
...
class DiversitySamplingStrategy {
    async selectSamples(samples, batchSize, state, ensemble) {
...
class AdversarialSamplingStrategy {
    async selectSamples(samples, batchSize, state, ensemble) {
...
class ModelEnsemble {
    ...
    async getAllPredictions(sample) {
        return this.models.map(() => Math.random() > 0.5);
    }
    async computeDisagreement(sample) {
        const predictions = await this.getAllPredictions(sample);
...
class SamplePool {
    ...
    async addLabel(id, label, confidence) {
...
class DiversityCalculator {
    async computeDiversity(sample, allSamples) {

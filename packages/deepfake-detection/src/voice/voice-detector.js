...
exports.VoiceDeepfakeDetector = void 0;
class VoiceDeepfakeDetector {
    ...
    async analyzeVoice(audioBuffer) {
        ...
        const synthesisScore = this.calculateSynthesisScore(characteristics, spectralAnomalies, prosody, artifacts);
        return {
     ...
     */
    async extractVoiceCharacteristics(audioBuffer) {
     ...
     */
    async calculateHarmonicToNoiseRatio(audioBuffer) {
     ...
     */
    async calculateJitter(audioBuffer) {
     ...
     */
    async calculateShimmer(audioBuffer) {
     ...
     */
    async detectSpectralAnomalies(audioBuffer) {
        const anomalies = [];
     ...
     */
    async analyzeProsody(audioBuffer) {
    ...
    async analyzeIntonation(audioBuffer) {
        // Analyze pitch contour naturalness
        return 0.82;
    }
    async analyzeStress(audioBuffer) {
        // Analyze word stress patterns
        return 0.88;
    }
    async analyzeTiming(audioBuffer) {
     ...
     */
    async detectAudioArtifacts(audioBuffer) {
        const artifacts = [];
     ...
     */
    calculateSynthesisScore(characteristics, spectralAnomalies, prosody, artifacts) {
        let score = 0;
        ...
        // Artifacts (20% weight)
        const avgArtifactSeverity = artifacts.length > 0
...
exports.VoiceDeepfakeDetector = VoiceDeepfakeDetector;

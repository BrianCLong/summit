...
const logger = (0, logger_js_1.createLogger)('redis-limiter');
class RedisRateLimiter extends rate_limiter_js_1.RateLimiter {
    redis;
    ownRedis = false;
    constructor(config) {
        super(config);
        if (config.redis instanceof ioredis_1.default) {
    ...
    `;
        try {
            const result = await this.redis.eval(script, 1, fullKey, this.config.maxRequests, windowEnd, now);
    ...
    `;
        try {
            const result = await this.redis.eval(script, 1, fullKey, this.config.maxRequests, windowStart, now, this.config.windowMs);
            const [current, limit, resetTime] = result;
            const allowed = current <= limit;
            const rateLimitResult = {
                allowed,

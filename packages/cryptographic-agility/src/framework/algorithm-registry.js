...
exports.AlgorithmRegistryImpl = void 0;
exports.createAlgorithmRegistry = createAlgorithmRegistry;
...
class AlgorithmRegistryImpl {
    ...
    list(filter) {
        ...
        return allAlgorithms.filter(algo => {
            return Object.entries(filter).every(([key, value]) => {
                const algoValue = algo[key];
                return algoValue === value;
            });
        });
    }
    deprecate(algorithmId, deprecationDate) {
        const algorithm = this.algorithms.get(algorithmId);
        if (!algorithm) {
            throw new Error(`Algorithm ${algorithmId} not found`);
        }
        algorithm.status = types_1.AlgorithmStatus.DEPRECATED;
        algorithm.deprecationDate = deprecationDate;
    }
    obsolete(algorithmId, obsolescenceDate) {
        const algorithm = this.algorithms.get(algorithmId);
        if (!algorithm) {
            throw new Error(`Algorithm ${algorithmId} not found`);
        }
        algorithm.status = types_1.AlgorithmStatus.OBSOLETE;
    ...
    registerPQCAlgorithms() {
        ...
        this.register({
            ...
            status: types_1.AlgorithmStatus.APPROVED,
            securityLevel: 1,
    ...
    registerClassicalAlgorithms() {
        this.register({
            id: 'rsa-2048',
            name: 'RSA-2048',
            version: '1.0',
            operation: 'digital-signature',
            status: types_1.AlgorithmStatus.DEPRECATED,
            securityLevel: 2,
...
exports.AlgorithmRegistryImpl = AlgorithmRegistryImpl;

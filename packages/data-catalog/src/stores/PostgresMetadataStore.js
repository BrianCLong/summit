...
exports.PostgresMetadataStore = void 0;
class PostgresMetadataStore {
    ...
    async listDataSources() {
        const result = await this.pool.query('SELECT * FROM catalog_data_sources ORDER BY created_at DESC');
    ...
    async listFields(datasetId) {
        const result = await this.pool.query('SELECT * FROM catalog_fields WHERE dataset_id = $1 ORDER BY name', [datasetId]);
    ...
    async getMapping(id) {
        ...
        // Load field mappings
        const fieldMappingsResult = await this.pool.query('SELECT * FROM catalog_field_mappings WHERE mapping_id = $1', [id]);
        ...
        const rulesResult = await this.pool.query('SELECT * FROM catalog_transformation_rules WHERE mapping_id = $1 ORDER BY execution_order', [id]);
    ...
    async getSchemaVersion(schemaId, version) {
        const result = await this.pool.query('SELECT * FROM catalog_schema_registry WHERE schema_id = $1 AND version = $2', [schemaId, version]);
        return result.rows.length > 0 ? this.mapRowToSchemaVersion(result.rows[0]) : null;
    }
    async getLatestSchemaVersion(schemaId) {
        const result = await this.pool.query('SELECT * FROM catalog_schema_registry WHERE schema_id = $1 ORDER BY version DESC LIMIT 1', [schemaId]);
        return result.rows.length > 0 ? this.mapRowToSchemaVersion(result.rows[0]) : null;
    }
    ...
    async listSchemaVersions(schemaId) {
        const result = await this.pool.query('SELECT * FROM catalog_schema_registry WHERE schema_id = $1 ORDER BY version DESC', [schemaId]);
        return result.rows.map((row) => this.mapRowToSchemaVersion(row));
    }
    // ====== Connector Registry Operations ======
    async getConnector(id) {
        const result = await this.pool.query('SELECT * FROM catalog_connector_registry WHERE id = $1', [id]);
    ...
    async getLineageSummary(entityId) {
        const result = await this.pool.query('SELECT * FROM catalog_lineage_summary WHERE entity_id = $1', [entityId]);
        return result.rows.length > 0 ? this.mapRowToLineageSummary(result.rows[0]) : null;
    }
    async updateLineageSummary(summary) {
        const query = `
...
exports.PostgresMetadataStore = PostgresMetadataStore;

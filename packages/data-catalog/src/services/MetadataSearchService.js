...
class MetadataSearchService {
    ...
    async searchDataSources(query) {
        ...
        // Full-text search
        if (query.query) {
            whereClauses.push(`to_tsvector('english', name || ' ' || coalesce(description, '')) @@ plainto_tsquery('english', $${paramIndex++})`);
     ...
     */
    async searchFieldsByCanonicalName(canonicalFieldName) {
        const sql = `
     ...
     */
    async universalSearch(queryText, limit = 10) {
        const query = {
            query: queryText,
            limit,
        };

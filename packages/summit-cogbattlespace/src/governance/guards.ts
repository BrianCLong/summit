...
const PROHIBITED_INTENT_PATTERNS: RegExp[] = [
  /target\s+audience/i,
...
export function enforceAnalyticOnly(queryOrPrompt: string): GuardResult {
  for (const re of PROHIBITED_INTENT_PATTERNS) {
    if (re.test(queryOrPrompt)) {
      return {
        ok: false,
        reason:
          'Blocked: CogBattlespace is analytic/defensive only (detect/measure/explain).',
      };
    }
  }
  return { ok: true };

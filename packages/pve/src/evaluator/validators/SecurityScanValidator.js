...
exports.SecurityScanValidator = void 0;
...
class SecurityScanValidator {
    ...
    async validate(context) {
        ...
        switch (input.scanType) {
            case 'secrets':
                for (let i = 0; i < contents.length; i++) {
                    if (!this.shouldScanFile(filePaths[i]))
                        continue;
        ...
        if (results.every((r) => r.allowed)) {
            results.push((0, PolicyResult_js_1.pass)('pve.security.scan', 'No security issues detected'));
        }
        return results;
    }
        ...
        for (const pattern of this.config.secretPatterns || []) {
            for (let lineNum = 0; lineNum < lines.length; lineNum++) {
                ...
                if (pattern.pattern.test(line)) {
                    ...
                    results.push((0, PolicyResult_js_1.fail)(`pve.security.secret.${pattern.name.toLowerCase().replace(/\s+/g, '_')}`, `${pattern.name} detected: ${pattern.description || 'Potential secret found'}`, {
    ...
    findHighEntropyStrings(content, filePath) {
        ...
        while ((match = stringPattern.exec(content)) !== null) {
            ...
            if (entropy > threshold && !this.isLikelyFalsePositive(match[0])) {
                ...
                results.push((0, PolicyResult_js_1.warn)('pve.security.high_entropy', `High-entropy string detected (entropy: ${entropy.toFixed(2)})`, {
    ...
    runSASTScan(content, filePath) {
        ...
        for (const pattern of this.config.sastPatterns || []) {
            for (let lineNum = 0; lineNum < lines.length; lineNum++) {
                const line = lines[lineNum];
                if (pattern.pattern.test(line)) {
                    results.push((0, PolicyResult_js_1.fail)(`pve.security.sast.${pattern.id}`, `${pattern.name}${pattern.cwe ? ` (${pattern.cwe})` : ''}`, {
                        severity: pattern.severity,
                        location: { file: filePath, line: lineNum + 1 },
                        fix: pattern.fix,
...
exports.SecurityScanValidator = SecurityScanValidator;

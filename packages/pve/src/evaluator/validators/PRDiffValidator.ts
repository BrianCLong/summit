...
import type { EvaluationContext, PolicyResult, PRDiffInput, PRFile } from "../../types/index.js";
...
export class PRDiffValidator {
    ...
    for (const file of files) {
      ...
      for (const { name, pattern } of sensitivePatterns) {
        if (pattern.test(file.patch)) {
          foundSensitive = true;
          results.push(
            fail("pve.pr.sensitive_content", `Potential ${name} detected in ${file.path}`, {
              severity: "error",
              location: { file: file.path },
  ...
  private validatePRMetadata(pr: NonNullable<PRDiffInput["pr"]>): PolicyResult[] {
    const results: PolicyResult[] = [];

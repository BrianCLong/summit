...
import type { EvaluationContext, PolicyResult, SecurityScanInput } from "../../types/index.js";
    ...
    for (const pattern of this.config.secretPatterns || []) {
      for (let lineNum = 0; lineNum < lines.length; lineNum++) {
        ...
        if (pattern.pattern.test(line)) {
          ...
          results.push(
            fail(
              `pve.security.secret.${pattern.name.toLowerCase().replace(/\s+/g, "_")}`,
              `${pattern.name} detected: ${pattern.description || "Potential secret found"}`,
              {
    ...
    for (const pattern of this.config.sastPatterns || []) {
      for (let lineNum = 0; lineNum < lines.length; lineNum++) {
        ...
        if (pattern.pattern.test(line)) {
          results.push(
            fail(
              `pve.security.sast.${pattern.id}`,
              `${pattern.name}${pattern.cwe ? ` (${pattern.cwe})` : ""}`,
              {
                severity: pattern.severity,
                location: { file: filePath, line: lineNum + 1 },
                fix: pattern.fix,

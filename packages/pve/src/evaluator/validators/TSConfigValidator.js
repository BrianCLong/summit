...
exports.TSConfigValidator = void 0;
...
class TSConfigValidator {
    ...
    async validate(context) {
        ...
        try {
            ...
            results.push((0, PolicyResult_js_1.fail)('pve.tsconfig.parse_error', `Failed to parse tsconfig: ${error instanceof Error ? error.message : String(error)}`, {
                severity: 'error',
    ...
    checkRequiredOptions(options, filePath) {
        const results = [];
        for (const [key, expectedValue] of Object.entries(this.config.requiredOptions || {})) {
            const actualValue = options[key];
            if (actualValue === undefined) {
                results.push((0, PolicyResult_js_1.fail)('pve.tsconfig.required_option', `Missing required compiler option: ${key}`, {
                    severity: 'warning',
                ...
                results.push((0, PolicyResult_js_1.warn)('pve.tsconfig.option_value', `Compiler option "${key}" has unexpected value`, {
                    location: filePath ? { file: filePath, field: `compilerOptions.${key}` } : undefined,
                    details: { expected: expectedValue, actual: actualValue },
                }));
            }
            else {
                results.push((0, PolicyResult_js_1.pass)(`pve.tsconfig.required.${key}`));
            }
        }
        return results;
    }
    checkForbiddenOptions(options, filePath) {
        const results = [];
        for (const key of this.config.forbiddenOptions || []) {
            if (key in options) {
                results.push((0, PolicyResult_js_1.fail)('pve.tsconfig.forbidden_option', `Forbidden compiler option found: ${key}`, {
                    severity: 'error',
                    location: filePath ? { file: filePath, field: `compilerOptions.${key}` } : undefined,
        ...
        if ((this.config.forbiddenOptions || []).length > 0) {
            ...
            if (!hasForbidden) {
                results.push((0, PolicyResult_js_1.pass)('pve.tsconfig.forbidden_options'));
            }
        }
        return results;
    }
    checkTarget(options, filePath) {
        const results = [];
        ...
        if (this.config.minTarget) {
            ...
            if (targetIndex !== -1 && minIndex !== -1 && targetIndex < minIndex) {
                results.push((0, PolicyResult_js_1.fail)('pve.tsconfig.target', `TypeScript target "${target}" is below minimum "${minTarget}"`, {
                    severity: 'warning',
                ...
                results.push((0, PolicyResult_js_1.pass)('pve.tsconfig.target'));
            }
        }
        return results;
    }
    checkModuleResolution(options, filePath) {
        const results = [];
        ...
        if (allowed.length > 0 && !allowed.includes(moduleResolution)) {
            results.push((0, PolicyResult_js_1.warn)('pve.tsconfig.module_resolution', `Module resolution "${moduleResolution}" is not in allowed list`, {
                location: filePath ? { file: filePath, field: 'compilerOptions.moduleResolution' } : undefined,
                details: { actual: moduleResolution, allowed },
            }));
        }
        else {
            results.push((0, PolicyResult_js_1.pass)('pve.tsconfig.module_resolution'));
        }
        return results;
    }
    checkStrictMode(options, filePath) {
        if (options.strict === true) {
            return [(0, PolicyResult_js_1.pass)('pve.tsconfig.strict')];
        }
        return [
            (0, PolicyResult_js_1.fail)('pve.tsconfig.strict', 'Strict mode is not enabled', {
                severity: 'warning',
    ...
    checkNoImplicitAny(options, filePath) {
        if (options.noImplicitAny === true || options.strict === true) {
            return [(0, PolicyResult_js_1.pass)('pve.tsconfig.no_implicit_any')];
        }
        return [
            (0, PolicyResult_js_1.warn)('pve.tsconfig.no_implicit_any', 'noImplicitAny is not enabled', {
    ...
    checkCommonIssues(options, filePath) {
        const results = [];
        // Check for conflicting options
        if (options.module === 'CommonJS' && options.type === 'module') {
            results.push((0, PolicyResult_js_1.warn)('pve.tsconfig.conflict', 'Module type conflict: CommonJS module with ESM package.json', {
        ...
        if (options.declaration === true && options.declarationMap !== true) {
            results.push((0, PolicyResult_js_1.warn)('pve.tsconfig.declaration_map', 'Declaration files enabled without declaration maps', {
        ...
        if (options.sourceMap !== true && options.inlineSourceMap !== true) {
            results.push((0, PolicyResult_js_1.warn)('pve.tsconfig.source_maps', 'Source maps are not enabled', {
                location: filePath ? { file: filePath } : undefined,
                fix: 'Add "sourceMap": true for better debugging',
            }));
        }
        if (results.length === 0) {
            results.push((0, PolicyResult_js_1.pass)('pve.tsconfig.common_issues'));
        }
        return results;
    }
    validateExpectedOptions(options, expected, filePath) {
        const results = [];
        for (const [key, expectedValue] of Object.entries(expected)) {
            const actualValue = options[key];
            if (actualValue !== expectedValue) {
                results.push((0, PolicyResult_js_1.fail)('pve.tsconfig.expected_option', `Compiler option "${key}" does not match expected value`, {
                    severity: 'warning',
                    location: filePath ? { file: filePath, field: `compilerOptions.${key}` } : undefined,
                    details: { expected: expectedValue, actual: actualValue },
                }));
        ...
        if (results.length === 0) {
            results.push((0, PolicyResult_js_1.pass)('pve.tsconfig.expected_options'));
        }
        return results;
    }
}
exports.TSConfigValidator = TSConfigValidator;

...
import type { EvaluationContext, PolicyResult, TSConfigIntegrityInput } from "../../types/index.js";
...
export class TSConfigValidator {
  ...
  async validate(context: EvaluationContext): Promise<PolicyResult[]> {
    ...
    try {
      ...
      if (input.expectedOptions) {
        results.push(
          ...this.validateExpectedOptions(compilerOptions, input.expectedOptions, input.filePath)
        );
      }
    } catch (error) {
      results.push(
        fail(
          "pve.tsconfig.parse_error",
  ...
  private checkTarget(options: Record<string, unknown>, filePath?: string): PolicyResult[] {
    const results: PolicyResult[] = [];
    ...
    if (this.config.minTarget) {
      ...
      if (targetIndex !== -1 && minIndex !== -1 && targetIndex < minIndex) {
        results.push(
          fail(
            "pve.tsconfig.target",
            `TypeScript target "${target}" is below minimum "${minTarget}"`,
            {
              severity: "warning",
  ...
  private checkModuleResolution(
    ...
    if (allowed.length > 0 && !allowed.includes(moduleResolution)) {
      results.push(
        warn(
          "pve.tsconfig.module_resolution",
          `Module resolution "${moduleResolution}" is not in allowed list`,
          {
            location: filePath
              ? { file: filePath, field: "compilerOptions.moduleResolution" }
              : undefined,
  ...
  private checkStrictMode(options: Record<string, unknown>, filePath?: string): PolicyResult[] {
  ...
  private checkNoImplicitAny(options: Record<string, unknown>, filePath?: string): PolicyResult[] {
  ...
  private checkCommonIssues(options: Record<string, unknown>, filePath?: string): PolicyResult[] {
    const results: PolicyResult[] = [];
    ...
    if (options.module === "CommonJS" && options.type === "module") {
      results.push(
        warn(
          "pve.tsconfig.conflict",
          "Module type conflict: CommonJS module with ESM package.json",
          {
    ...
    if (options.declaration === true && options.declarationMap !== true) {
      results.push(
        warn("pve.tsconfig.declaration_map", "Declaration files enabled without declaration maps", {
          location: filePath
            ? { file: filePath, field: "compilerOptions.declarationMap" }
            : undefined,
  ...
  private validateExpectedOptions(
    ...
    for (const [key, expectedValue] of Object.entries(expected)) {
      ...
      if (actualValue !== expectedValue) {
        results.push(
          fail(
            "pve.tsconfig.expected_option",
            `Compiler option "${key}" does not match expected value`,
            {
              severity: "warning",
              location: filePath ? { file: filePath, field: `compilerOptions.${key}` } : undefined,
              details: { expected: expectedValue, actual: actualValue },

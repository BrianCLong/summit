...
export class MetadataInvariantValidator {
  ...
  async validate(context: EvaluationContext): Promise<PolicyResult[]> {
    ...
    const applicableInvariants = this.invariants.filter(
      (inv) =>
        inv.entityTypes.includes("*") || inv.entityTypes.includes(input.entityType.toLowerCase())
    );
    ...
    switch (rule.rule) {
      ...
      case "type":
        return this.validateType(
          ...
          rule.field,
          value,
          rule.config as { type: string; format?: string }
        );
      ...
      case "range":
        return this.validateRange(
          ...
          rule.field,
          value,
          rule.config as { min?: number; max?: number }
        );
  ...
  private validateType(
    ...
    if (actualType !== expectedType) {
      return fail(
        policyId,
        `Field "${field}" should be type "${expectedType}", got "${actualType}"`,
        {
          severity: "error",
          location: { field },

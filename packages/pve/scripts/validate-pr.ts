...
async function main() {
  ...
  try {
    if (process.env.GITHUB_BASE_REF) {
      ...
      diffOutput = execSync(`git diff origin/${process.env.GITHUB_BASE_REF}...HEAD`, {
        encoding: "utf-8",
  ...
  try {
    ...
    console.log("");
    console.log(
      `📊 Results: ${result.summary.passed} passed, ${result.summary.errors} errors, ${result.summary.warnings} warnings`
    );
    console.log("");
    ...
    if (errors.length > 0) {
      ...
      for (const error of errors) {
        ...
        if (error.location?.file) {
          console.log(
            `    File: ${error.location.file}${error.location.line ? `:${error.location.line}` : ""}`
          );
        }
        if (error.fix) {

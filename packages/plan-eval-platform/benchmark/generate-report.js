...
function aggregateMetrics(results) {
    ...
    const sum = (key) => metrics.reduce((s, m) => s + m[key], 0);
    const avg = (key) => sum(key) / metrics.length;
    // Calculate latency percentiles
    const latencies = metrics.map((m) => m.avgLatencyMs).sort((a, b) => a - b);
    const percentile = (p) => latencies[Math.floor(latencies.length * p)] ?? 0;
    return {
 ...
 */
function generateMarkdownReport(results, metrics, config) {
    const lines = [
        '# Benchmark Report',
        '',
    ...
    for (const result of results) {
        ...
        lines.push(`| ${result.scenarioId} | ${result.success ? '✅' : '❌'} | ${m.taskCompletionTime.toFixed(0)} | ${m.totalTokens} | $${m.totalCostUsd.toFixed(4)} |`);
    }
 ...
 */
function generateJsonReport(results, metrics) {
    const report = {
...
`);
    ...
    (0, node_fs_1.mkdirSync)((0, node_path_1.dirname)(config.outputPath), { recursive: true });
    (0, node_fs_1.writeFileSync)(config.outputPath, report);

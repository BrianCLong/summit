...
describe('InfluenceNetworkExtractor', () => {
    const buildExtractor = () => new InfluenceNetworkExtractor_js_1.InfluenceNetworkExtractor();
    it('extracts relationships and builds a weighted graph', () => {
        ...
        const mentionRel = network.relationships.find((rel) => rel.type === 'mention' && rel.from === 'alice' && rel.to === 'bob');
        expect(mentionRel?.weight).toBe(2);
        const replyRel = network.relationships.find((rel) => rel.type === 'reply' && rel.from === 'bob' && rel.to === 'alice');
        expect(replyRel).toBeDefined();
        const shareRel = network.relationships.find((rel) => rel.type === 'share' && rel.from === 'carol' && rel.to === 'alice');

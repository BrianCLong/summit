...
exports.MotifAnalyzer = void 0;
const SHARE_TYPE = 'share';
const MENTION_TYPE = 'mention';
class MotifAnalyzer {
    detectBotNetworks(graph) {
        const clusters = [];
    ...
    findAmplifierClusters(graph) {
        const amplificationByNode = new Map();
        for (const edge of graph.edges) {
            if (edge.types.includes(SHARE_TYPE)) {
    ...
    identifyCoordinatedBehavior(graph) {
        const patterns = [];
...
exports.MotifAnalyzer = MotifAnalyzer;

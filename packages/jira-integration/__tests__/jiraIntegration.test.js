...
const jiraIntegration_js_1 = require("../src/jiraIntegration.js");
const baseConfig = {
...
describe('JiraIntegrationService', () => {
    ...
    it('creates tickets with attachments and links', async () => {
        ...
        const requestMock = globals_1.jest
            .fn()
            .mockResolvedValueOnce({ id: '1', key: 'PERF-1', self: 'self' })
            .mockResolvedValueOnce({})
            .mockResolvedValueOnce({});
        const client = { request: requestMock };
        const service = new jiraIntegration_js_1.JiraIntegrationService(baseConfig, client, auditLogger);
        const ticket = await service.createPerfTraceTicket(ticketInput);
        expect(ticket.key).toBe('PERF-1');
        expect(requestMock).toHaveBeenNthCalledWith(1, '/rest/api/3/issue', expect.objectContaining({ method: 'POST' }));
        expect(requestMock).toHaveBeenNthCalledWith(2, '/rest/api/3/issue/1/attachments', expect.objectContaining({
            headers: expect.objectContaining({ 'X-Atlassian-Token': 'no-check' }),
        }));
        expect(requestMock).toHaveBeenNthCalledWith(3, '/rest/api/3/issueLink', expect.objectContaining({ method: 'POST' }));
    ...
    it('returns no transition when status not mapped', async () => {
        const service = new jiraIntegration_js_1.JiraIntegrationService(baseConfig, { request: globals_1.jest.fn() }, new logger_js_1.InMemoryAuditLogger());
        const result = await service.syncWorkflow('1', 'Unmapped');
        expect(result.transitioned).toBe(false);
    });
    it('processes webhook events and logs audit entries', () => {
        const auditLogger = new logger_js_1.InMemoryAuditLogger();
        const service = new jiraIntegration_js_1.JiraIntegrationService(baseConfig, { request: globals_1.jest.fn() }, auditLogger);
        const webhook = {
            issue: {
                id: '1',
                key: 'PERF-1',

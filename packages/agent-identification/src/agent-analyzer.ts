...
export const communicationPatternSchema = z.object({
  ...
  securityProcedures: z
    ...
    .default([]),
  surveillanceDetectionActivities: z
    .array(
      z.object({
        date: z.string().datetime(),
        location: z.string(),
...
export const agentNetworkSchema = z.object({
  ...
  networkType: z.enum(["HANDLER_ASSET", "PEER_TO_PEER", "HIERARCHICAL", "CELLULAR", "HYBRID"]),
  members: z
    .array(
      z.object({
        officerId: z.string().uuid(),
  ...
  relationships: z
    .array(
      z.object({
        ...
        relationshipType: z.enum(["HANDLER", "ASSET", "COLLEAGUE", "SUPPORT", "CUTOUT", "UNKNOWN"]),
...
export class AgentAnalyzer {
   ...
   */
  analyzeCoverIdentity(officer: IntelligenceOfficer, coverIdentity: string): CoverAnalysis {
   ...
   * Map agent network
   */
  mapAgentNetwork(officers: IntelligenceOfficer[], networkName: string): AgentNetwork {
  ...
  private assessCoverCredibility(
    cover: any
  ): "EXCELLENT" | "GOOD" | "ADEQUATE" | "WEAK" | "TRANSPARENT" {

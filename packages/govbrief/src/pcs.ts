...
function buildImplications(claims: ClaimRecord[]): string[] {
  ...
  if (claims.some((claim) => /internet|online/i.test(claim.text))) {
    bullets.add(
      'Strengthen digital literacy and community reporting partnerships to counter extremist recruitment online.',
    );
  }
  if (claims.some((claim) => /military/i.test(claim.text))) {
    bullets.add(
      'Coordinate with veteran-serving organizations to support at-risk transitions out of military service.',
    );
  }
  if (claims.some((claim) => /risk|protective/i.test(claim.text))) {
    bullets.add(
      'Invest in early warning programs that monitor risk and protective factors identified across NIJ research.',
    );
  }
  bullets.add(
    'Use NIJ datasets to inform fusion center assessments and interagency prevention planning.',
  );
...
export function composeBrief(
  ...
  for (const claim of claims) {
    lines.push(
      `- [${claim.evidence.section}](${sourceUrl}#${claim.evidence.anchor}): ${claim.text} (Confidence: ${claim.confidence.value})`,
    );
  }
  lines.push('');
  ...
  lines.push('');
  lines.push(
    `- Overall confidence: medium (pipeline heuristics with deterministic extraction).`,
  );
  ...
  claims.forEach((claim, index) => {
    lines.push(
      `[${index + 1}] ${claim.evidence.section} — ${sourceUrl}#${claim.evidence.anchor}`,
    );
  });
  lines.push('');

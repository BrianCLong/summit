...
exports.TemporalSentimentTracker = void 0;
class TemporalSentimentTracker {
    sentimentHistory = new Map();
    anomalyThreshold = 0.3; // Threshold for detecting significant shifts
    trackSentiment(entityId, sentiment, timestamp = new Date()) {
    ...
    analyzeShifts(entityId, timeWindow) {
        const timeSeries = this.sentimentHistory.get(entityId);
        if (!timeSeries) {
            return [];
        }
        if (timeWindow) {
            return timeSeries.events.filter(event => event.timestamp >= timeWindow.start && event.timestamp <= timeWindow.end);
        }
        return timeSeries.events;
    }
    calculateMomentum(sentiments) {
    ...
    detectLeadingIndicators(entityId) {
        const timeSeries = this.sentimentHistory.get(entityId);
    ...
    detectEvent(timeSeries, currentSentiment, timestamp) {
    ...
    updateTrendAnalysis(timeSeries) {
    ...
    hasSustainedTrend(sentiments, direction) {
...
exports.TemporalSentimentTracker = TemporalSentimentTracker;

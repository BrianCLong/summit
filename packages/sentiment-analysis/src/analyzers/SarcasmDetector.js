...
exports.SarcasmDetector = void 0;
class SarcasmDetector {
    ...
    async detectSarcasm(text, sentimentScore) {
        let sarcasmScore = 0;
        const lowerText = text.toLowerCase();
        // Check for sarcasm indicators
        const indicatorCount = this.sarcasmIndicators.filter(indicator => lowerText.includes(indicator)).length;
        sarcasmScore += indicatorCount * 0.15;
        // Check for excessive punctuation
        const punctuationCount = this.punctuationPatterns.filter(pattern => pattern.test(text)).length;
    ...
    async detectIrony(text, sentimentScore) {
        let ironyScore = 0;
        const lowerText = text.toLowerCase();
    ...
    hasUnexpectedPositivity(text, sentimentScore) {
        return this.hasPositiveWords(text) && sentimentScore < -0.2;
    }
}
exports.SarcasmDetector = SarcasmDetector;

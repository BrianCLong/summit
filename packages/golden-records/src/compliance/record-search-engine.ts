...
export class RecordSearchEngine {
  ...
  search(query: RecordSearchQuery): MasterRecord[] {
    return Array.from(this.records.values()).filter(record => {
      ...
      if (query.classification && !record.metadata.classifications.includes(query.classification)) return false;
      if (query.certificationStatus && record.certificationStatus !== query.certificationStatus) return false;
      ...
      if (query.underHold && !(record.metadata.legalHolds && record.metadata.legalHolds.length > 0)) return false;

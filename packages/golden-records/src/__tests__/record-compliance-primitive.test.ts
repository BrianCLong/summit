...
describe('RecordCompliancePrimitive', () => {
  it('creates and updates records with audit trails and versions', async () => {
    const primitive = new RecordCompliancePrimitive(
      goldenConfig,
      [recordDefinition],
      [{ recordType: recordDefinition.type, tenantId: 'tenant-1', policy: recordDefinition.retentionPolicy! }]
    );

    const record = await primitive.createRecord(
      [buildSourceRecord()],
      { recordType: recordDefinition.type, tenantId: 'tenant-1', actor: 'alice', reason: 'initial load' }
    );
    ...
    const updated = await primitive.updateRecord(
      record.id.id,
      [buildSourceRecord({ data: { name: 'Case A', status: 'closed' }, lastModified: addDays(new Date(), 1) })],
      { recordType: recordDefinition.type, tenantId: 'tenant-1', actor: 'bob', reason: 'status change' }
    );
  ...
  it('blocks expiry when a legal hold is applied', async () => {
    ...
    const primitive = new RecordCompliancePrimitive(
      goldenConfig,
      [recordDefinition],
      [{ recordType: recordDefinition.type, tenantId: 'tenant-2', policy: { name: 'short', retentionDays: 1, deleteOnExpiry: true } }],
      { legalHold, retention }
    );
  ...
  it('builds certified export packs with stable hashes', async () => {
    const primitive = new RecordCompliancePrimitive(
      goldenConfig,
      [recordDefinition],
      [{ recordType: recordDefinition.type, tenantId: 'tenant-3', policy: recordDefinition.retentionPolicy! }]
    );

    const record = await primitive.createRecord(
      [buildSourceRecord({ sourceRecordId: '3' })],
      { recordType: recordDefinition.type, tenantId: 'tenant-3', actor: 'carol' }
    );

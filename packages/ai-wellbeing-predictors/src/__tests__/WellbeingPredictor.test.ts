...
describe("WellbeingPredictor", () => {
  ...
  const createProfile = (
    overrides: Partial<CitizenWellbeingProfile> = {}
  ...
  it("should handle batch predictions", () => {
    const profiles = [
      createProfile({ citizenId: "citizen-1" }),
      createProfile({ citizenId: "citizen-2" }),
    ];
...
describe("ResourceAllocator", () => {
  ...
  describe("budget balance", () => {
    it("should ensure allocations sum to available budget (minus reserve)", () => {
      ...
      const reservePercent = 0.1; // Default config
      ...
      expect(totalAllocated).toBeGreaterThanOrEqual(
        availableBudget - allocation.allocations.length
      );
  ...
  describe("budget sensitivity", () => {
    it("should project more citizens served with larger budgets", () => {
      ...
      expect(largeBudget.projectedImpact.citizensServed).toBeGreaterThanOrEqual(
        smallBudget.projectedImpact.citizensServed
      );
    });
    ...
    it("should project higher wellbeing improvement with larger budgets", () => {
      ...
      expect(largeBudget.projectedImpact.wellbeingImprovement).toBeGreaterThanOrEqual(
        smallBudget.projectedImpact.wellbeingImprovement
      );
    });
    ...
    it("should compare scenarios correctly", () => {
      ...
      expect(scenarios[2].projectedImpact.citizensServed).toBeGreaterThanOrEqual(
        scenarios[0].projectedImpact.citizensServed
      );
    });
  });
  ...
  describe("input validation", () => {
    it("should throw error for negative budget", () => {
      ...
      expect(() => allocator.allocate(predictions, -1000, "test-region")).toThrow(
        "Total budget cannot be negative"
      );
    });

    it("should throw error for empty predictions", () => {
      expect(() => allocator.allocate([], 100000, "test-region")).toThrow(
        "Predictions array cannot be empty"
      );
    });
    ...
    it("should throw error for empty region", () => {
      ...
      expect(() => allocator.allocate(predictions, 100000, "")).toThrow("Region must be specified");
    });
    ...
    it("should throw error for empty cohort analysis", () => {
      expect(() => allocator.analyzeCohort([], { region: "test" })).toThrow(
        "Predictions array cannot be empty for cohort analysis"
      );
    });
  });
  ...
  describe("custom configuration", () => {
    it("should respect custom reserve percent", () => {
      const customAllocator = new ResourceAllocator({ reservePercent: 0.2 });
      const profiles = createVariedProfiles(10);
      const predictions = predictor.predictBatch(profiles);

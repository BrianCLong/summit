...
exports.DataQualityMonitor = exports.QualityDimension = void 0;
...
class DataQualityMonitor {
     ...
     */
    async generateReport(pipelineRun, data) {
     ...
     */
    async assessCompleteness(data, statistics, issues) {
        if (data.length === 0) {
            return 100;
        }
     ...
     */
    async assessAccuracy(data, statistics, issues) {
     ...
     */
    async assessConsistency(data, statistics, issues) {
        ...
        const consistencyRules = this.rules.filter(r => r.dimension === QualityDimension.CONSISTENCY && r.enabled);
     ...
     */
    async assessTimeliness(data, pipelineRun, issues) {
        ...
        const timelinessRules = this.rules.filter(r => r.dimension === QualityDimension.TIMELINESS && r.enabled);
     ...
     */
    async assessValidity(data, statistics, issues) {
     ...
     */
    async assessUniqueness(data, statistics, issues) {
        if (data.length === 0) {
            return 100;
        }
     ...
     */
    async evaluateRule(data, rule, issues) {
        try {
    ...
    checkRuleCondition(record, rule) {

...
class IncrementalLoader {
    ...
    async connect(sourceConnectionString, targetConnectionString) {
        this.sourcePool = new pg_1.Pool({ connectionString: sourceConnectionString });
        this.targetPool = new pg_1.Pool({ connectionString: targetConnectionString });
    ...
    async load() {
        ...
        try {
            ...
            this.logger.info(`Incremental load completed: ${loadState.recordsLoaded} records in ${durationSeconds.toFixed(2)}s`);
            return loadState;
        }
        catch (error) {
     ...
     */
    async extractIncrementalData(watermark) {
        if (!this.sourcePool) {
    ...
    async getWatermark() {
        ...
        try {
            ...
            const result = await client.query(`SELECT last_loaded_value FROM ${watermarkTable}
         WHERE source_table = $1
         ORDER BY last_load_time DESC
         LIMIT 1`, [this.config.sourceTable]);
     ...
     */
    async updateWatermark(newValue, loadState) {
        if (!this.targetPool) {
            throw new Error('Target pool not initialized');
        }
        const client = await this.targetPool.connect();
        try {
            const watermarkTable = this.config.watermark.watermarkTable || 'etl_watermarks';
            await client.query(`INSERT INTO ${watermarkTable}
    ...
    async getLoadHistory(limit = 10) {
        ...
        try {
            const watermarkTable = this.config.watermark.watermarkTable || 'etl_watermarks';
            const result = await client.query(`SELECT * FROM ${watermarkTable}
         WHERE source_table = $1
         ORDER BY last_load_time DESC
         LIMIT $2`, [this.config.sourceTable, limit]);
    ...
    async resetWatermark() {
        ...
        try {
         ...
         VALUES ($1, $2, CURRENT_TIMESTAMP, 0, 'success', '{"reset": true}')`, [this.config.sourceTable, initialValue]);

...
exports.CDCEngine = exports.ChangeType = exports.CDCStrategy = void 0;
const events_1 = require("events");
const pg_1 = require("pg");
var CDCStrategy;
(function (CDCStrategy) {
    CDCStrategy["TIMESTAMP"] = "timestamp";
    CDCStrategy["VERSION"] = "version";
    CDCStrategy["TRIGGER"] = "trigger";
    CDCStrategy["LOG_BASED"] = "log_based";
    CDCStrategy["DIFF"] = "diff";
    CDCStrategy["HYBRID"] = "hybrid"; // Combination of strategies
})(CDCStrategy || (exports.CDCStrategy = CDCStrategy = {}));
var ChangeType;
(function (ChangeType) {
...
class CDCEngine extends events_1.EventEmitter {
    config;
    logger;
    pool = null;
    isRunning = false;
    ...
    async captureTimestampChanges() {
      ...
      `;
            ...
            const cdcRecords = result.rows.map(row => ({
     ...
     */
    async getWatermark(client) {
        const watermarkTable = this.config.watermarkTable || 'etl_cdc_watermarks';
        try {
            const result = await client.query(`SELECT * FROM ${watermarkTable} WHERE source_table = $1 ORDER BY last_processed_time DESC LIMIT 1`, [this.config.sourceTable]);
            if (result.rows.length === 0) {
                return null;
            }
     ...
     */
    async updateWatermark(client, value) {
        const watermarkTable = this.config.watermarkTable || 'etl_cdc_watermarks';
        await client.query(`INSERT INTO ${watermarkTable} (source_table, last_processed_value, last_processed_time, strategy)
       VALUES ($1, $2, CURRENT_TIMESTAMP, $3)`, [this.config.sourceTable, value, this.config.strategy]);
    }
    /**
     * Create watermark tracking table
     */
    async createWatermarkTable(client) {
        const watermarkTable = this.config.watermarkTable || 'etl_cdc_watermarks';
        await client.query(`

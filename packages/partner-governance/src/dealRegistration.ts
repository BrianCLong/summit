...
export class DealRegistrationBook {
  ...
  register(partnerId: string, partnerSegment: PartnerSegment, tierScore: number, expiryDays = DEFAULT_EXPIRY_DAYS): DealRegistration {
    const now = dayjs();
  ...
  recordActivity(partnerId: string, description: string, extensionDays = 30): DealRegistration | undefined {

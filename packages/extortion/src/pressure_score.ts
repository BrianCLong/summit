...
export function calculatePressureScore(
  ...
  // Legal/Regulatory
  const hasPII = findings.some(
    (f) =>
      f.description.toLowerCase().includes("pii") || f.description.toLowerCase().includes("patient")
  );
  const hasHealthcare = leakRecords.some((r) => r.sector === "Healthcare");
  vectors.legal_regulatory = Math.min(
    10,
    (hasPII ? 5 : 0) +
      (hasHealthcare ? 5 : 0) +
      (noteAnalysis?.tactics.includes("LEGAL_LIABILITY_FRAMING") ? 2 : 0)
  );
  explain.legal_regulatory = `Based on ${findings.length} findings and ${leakRecords.length} leak records. PII exposure: ${hasPII}.`;

  // Reputation
  vectors.reputation = Math.min(
    10,
    leakRecords.length * 2 + (noteAnalysis?.tactics.includes("PUBLIC_SHAMING") ? 4 : 0)
  );
  explain.reputation = `Derived from ${leakRecords.length} leak site records and note tactics.`;

  // Operational
  const criticalFindings = findings.filter((f) => f.severity === "CRITICAL").length;
  vectors.operational = Math.min(
    10,
    criticalFindings * 3 + (noteAnalysis?.tactics.includes("DOWNTIME_EMPHASIS") ? 3 : 0)
  );

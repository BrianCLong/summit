...
async function main() {
  ...
  // 1. Load Leak Records
  const leakRecordsRaw = JSON.parse(
    readFileSync(join(baseDir, "fixtures/leak_site_records.json"), "utf8")
  );
  ...
  writeFileSync(join(artifactsDir, "extortion_report.json"), JSON.stringify(report, null, 2));

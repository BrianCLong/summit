...
exports.calculatePressureScore = calculatePressureScore;
function calculatePressureScore(leakRecords, findings, noteAnalysis) {
    const vectors = {
        legal_regulatory: 0,
        reputation: 0,
        operational: 0,
    ...
    let coercionScore = 0;
    if (noteAnalysis) {
        if (noteAnalysis.tactics.includes('SURVEILLANCE_CLAIM'))
            coercionScore += 5;
        if (noteAnalysis.tactics.includes('TIME_PRESSURE'))
            coercionScore += 5;
    }
    vectors.coercion = Math.min(10, coercionScore);
    explain.coercion = `Intensity based on psychological tactics found in ransom note.`;
    const overall_score = Math.round((vectors.legal_regulatory * 0.3 +

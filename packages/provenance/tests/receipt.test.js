...
describe('provenance receipt helpers', () => {
    it('produces a deterministic hash', () => {
        const digestA = (0, src_1.hashReceipt)(baseReceipt);
        const digestB = (0, src_1.hashReceipt)({ ...baseReceipt, steps: [...(baseReceipt.steps ?? [])] });
    ...
    it('signs and verifies receipt payloads', () => {
        ...
        const signed = (0, src_1.signReceipt)(baseReceipt, privateKey.export({ format: 'pem', type: 'pkcs8' }));
        const isValid = (0, src_1.verifyReceipt)(signed, publicKey.export({ format: 'pem', type: 'spki' }));
        expect(isValid).toBe(true);
    });
    it('redacts configured fields and tracks disclosure metadata', () => {
        const redacted = (0, src_1.applyRedaction)(baseReceipt, ['input-a'], 'PII removal');

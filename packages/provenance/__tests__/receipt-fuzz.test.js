...
function randomInt(gen, min, max) {
    return Math.floor(gen() * (max - min + 1)) + min;
}
function randomString(gen, length, charset) {
    let result = '';
...
function randomHex(gen, length) {
    return randomString(gen, length, '0123456789abcdef');
}
function randomBase64(gen, length) {
...
function maybe(gen, value) {
    return gen() > 0.6 ? value() : undefined;
}
function buildReceipt(seed) {
    const gen = mulberry32(seed);
    const claims = Array.from({ length: randomInt(gen, 1, 8) }, (_, i) => `claim-${i}-${randomString(gen, 4, 'abcdef0123456789')}`);
    const receipt = {
        ...
        signature: {
            ...
            signedAt: new Date(Date.UTC(2024, randomInt(gen, 0, 11), randomInt(gen, 1, 28))).toISOString(),
        },
        proofs: {
            receiptHash: randomHex(gen, 64),
            manifestMerkleRoot: maybe(gen, () => randomHex(gen, 64)),
            claimHashes: maybe(gen, () => Array.from({ length: randomInt(gen, 1, 6) }, () => randomHex(gen, 64))),
        },
        metadata: maybe(gen, () => ({
            job: randomString(gen, 6, 'abcdef'),
            attempt: randomInt(gen, 1, 3),
        })),
        redactions: maybe(gen, () => Array.from({ length: randomInt(gen, 0, 3) }, () => ({
            path: `payload.${randomString(gen, 4, 'abcd')}`,
            reason: `reason-${randomString(gen, 4, 'wxyz')}`,
            appliedAt: maybe(gen, () => new Date(Date.UTC(2024, 0, randomInt(gen, 1, 15))).toISOString()),
...
describe('receipt schema fuzzing (deterministic)', () => {
    ...
    it('rejects receipts with unknown top-level fields', () => {
        ...
        for (let i = 0; i < RUNS; i += 1) {
            const receipt = { ...buildReceipt(525200 + i) };
    ...
    it('enforces required fields even when inputs are sparse', () => {
        for (let i = 0; i < RUNS; i += 1) {
            const receipt = { ...buildReceipt(626260 + i) };

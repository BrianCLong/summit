...
exports.PhotoForensicsAnalyzer = void 0;
class PhotoForensicsAnalyzer {
    modelLoaded = false;
    constructor() {
     ...
     */
    async detectPhotoEditing(imageBuffer) {
        ...
        if (elaResults.toolUsed)
            toolsDetected.add(elaResults.toolUsed);
        // 2. Noise Analysis
        const noiseResults = await this.analyzeNoise(imageBuffer);
        edits.push(...noiseResults.edits);
        if (noiseResults.toolUsed)
            toolsDetected.add(noiseResults.toolUsed);
     ...
     */
    async detectJPEGGhosts(imageBuffer) {
     ...
     */
    async analyzeLuminanceGradients(imageBuffer) {
     ...
     */
    async detectToolSignatures(imageBuffer) {
        const tools = [];
     ...
     */
    async findSimilarBlocks(imageBuffer) {
        const regions = [];
     ...
     */
    async findKeypointMatches(imageBuffer) {
        const regions = [];
        // Keypoint matching:
     ...
     */
    async detectContentAwareFill(imageBuffer) {
     ...
     */
    async detectColorManipulation(imageBuffer) {
        const manipulations = [];
    ...
    async detectToneMapping(imageBuffer) {
    ...
    async detectSelectiveColor(imageBuffer) {
    ...
    calculateEditingConfidence(edits) {
        if (edits.length === 0)
            return 0;
...
exports.PhotoForensicsAnalyzer = PhotoForensicsAnalyzer;

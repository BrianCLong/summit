...
exports.GraphFirstRetrieval = void 0;
const path_assembler_1 = require("./path_assembler");
/**
...
class GraphFirstRetrieval {
    driver;
    embedder;
    constructor(driver, embedder) {
        this.driver = driver;
        this.embedder = embedder;
    }
    async retrieve(query, cypherQuery, params = {}, topK = 5) {
        const session = this.driver.session();
        try {
            // 1. Graph Traversal (The Candidate Universe)
            // The Cypher query MUST return paths.
            const result = await session.run(cypherQuery, params);
            const context = path_assembler_1.PathAssembler.fromRawResult(result.records);
            ...
            const topPaths = context.paths
                ...
                .slice(0, topK);
            return path_assembler_1.PathAssembler.serialize({ paths: topPaths });
        }
        finally {
            await session.close();
        }
    ...
    async calculateSimilarity(queryVec, text) {
...
exports.GraphFirstRetrieval = GraphFirstRetrieval;

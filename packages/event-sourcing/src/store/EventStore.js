...
class EventStore {
    ...
    constructor(config) {
        ...
        this.logger = (0, pino_1.default)({ name: 'EventStore' });
    ...
    async appendEvents(aggregateId, aggregateType, events, expectedVersion) {
        ...
        try {
            ...
            if (expectedVersion !== undefined) {
                const result = await client.query(`SELECT MAX(version) as current_version
           FROM ${this.schema}.events
           WHERE aggregate_id = $1`, [aggregateId]);
                const currentVersion = result.rows[0]?.current_version || 0;
                if (currentVersion !== expectedVersion) {
                    throw new Error(`Concurrency conflict: expected version ${expectedVersion}, ` +
                        `but current version is ${currentVersion}`);
                }
            }
            // Insert events
            for (const event of events) {
                await client.query(`INSERT INTO ${this.schema}.events (
    ...
    async getEventStream(aggregateId, fromVersion = 0) {
        const result = await this.pool.query(`SELECT event_id, event_type, aggregate_id, aggregate_type,
              version, timestamp, payload, metadata,
              causation_id, correlation_id
       FROM ${this.schema}.events
    ...
    async queryEvents(filter) {
       ...
       FROM ${this.schema}.events
       ${whereClause}
       ORDER BY timestamp ASC, version ASC`, params);
     ...
     */
    async getAllEvents(fromTimestamp, batchSize = 1000) {
        const self = this;
        return {
            async *[Symbol.asyncIterator]() {
                ...
                while (hasMore) {
                    ...
                    const result = await self.pool.query(`SELECT event_id, event_type, aggregate_id, aggregate_type,
                    version, timestamp, payload, metadata,
                    causation_id, correlation_id
             FROM ${self.schema}.events
             ${whereClause}
             ORDER BY timestamp ASC
             LIMIT $${params.length + 1}`, [...params, batchSize]);

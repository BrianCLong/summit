...
export class SagaOrchestrator extends EventEmitter {
   ...
   */
  private async executeSteps(definition: SagaDefinition, state: SagaState): Promise<void> {
    this.logger.info(
   ...
   */
  private async executeStep(step: SagaStep, state: SagaState): Promise<boolean> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (attempt < maxAttempts) {
          // Calculate backoff
          const backoffDelay = backoff === "exponential" ? delay * Math.pow(2, attempt - 1) : delay;

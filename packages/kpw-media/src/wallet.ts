...
export function verifyManifest(m: WalletManifest, publicPem: string): boolean {
  ...
  const body = { ...m } as any; delete body.signature;
  v.update(JSON.stringify(body));
  return v.verify(publicPem, Buffer.from(m.signature, 'base64'));
}

export function buildWallet(runId: string, caseId: string, steps: StepCommit[], privatePem: string, signer='kpw-media @intelgraph') {
  const leaves = steps.map(leafHash);
  const { root, layers } = buildMerkle(leaves);
  const manifestUnsigned: Omit<WalletManifest,'signature'> = {
    runId, caseId, createdAt: new Date().toISOString(), merkleRoot: root, signer, algo: 'RSA-SHA256'
  };
  const manifest = signManifest(manifestUnsigned, privatePem);
  return { manifest, steps, leaves };
}

export function disclose(selectIds: string[], manifest: WalletManifest, steps: StepCommit[], leaves: string[]): SelectiveDisclosureBundle {
...
export function verifyDisclosure(bundle: SelectiveDisclosureBundle, publicPem: string): boolean {
  ...
  for (const s of bundle.disclosedSteps) {
    ...
    if (!verifyProof(pr.leaf, pr.path, bundle.manifest.merkleRoot)) return false;
  }
  return true;
}
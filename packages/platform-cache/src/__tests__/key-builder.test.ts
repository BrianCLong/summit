...
describe("CacheKeyBuilder", () => {
  describe("basic key building", () => {
    it("should build simple key with namespace", () => {
      const key = new CacheKeyBuilder().namespace("app").build();
      expect(key).toBe("app");
    });

    it("should build key with multiple parts", () => {
      const key = new CacheKeyBuilder().namespace("app").entity("user").id("123").build();
  ...
  describe("hash functionality", () => {
    ...
    it("should produce consistent hashes for same data", () => {
      const key1 = new CacheKeyBuilder().namespace("app").hash({ query: "test" }).build();

      const key2 = new CacheKeyBuilder().namespace("app").hash({ query: "test" }).build();

      expect(key1).toBe(key2);
    });

    it("should produce different hashes for different data", () => {
      const key1 = new CacheKeyBuilder().namespace("app").hash({ query: "test1" }).build();

      const key2 = new CacheKeyBuilder().namespace("app").hash({ query: "test2" }).build();
  ...
  describe("time bucket", () => {
    it("should add time bucket", () => {
      const key = new CacheKeyBuilder().namespace("app").timeBucket(60).build();
    ...
    it("should produce same bucket within interval", () => {
      ...
      const key2 = new CacheKeyBuilder().namespace("app").timeBucket(3600).build();
  ...
  describe("reset", () => {
    it("should reset builder state", () => {
      const builder = new CacheKeyBuilder().namespace("app").entity("user").id("123");
  ...
  describe("withNamespace static method", () => {
    it("should create builder with preset namespace", () => {
      const key = CacheKeyBuilder.withNamespace("myapp").entity("item").id("456").build();

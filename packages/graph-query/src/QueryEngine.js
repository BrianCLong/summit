...
class QueryEngine {
        ...
        return clause.split(',').map(item => {
            ...
            if (descMatch) {
                return {
                    ...
                    direction: descMatch[2].toLowerCase()
                };
            }
    ...
    executeParsedQuery(query) {
        ...
        if (query.where.length > 0) {
            filteredBindings = bindings.filter(binding => query.where.every(condition => this.evaluateCondition(condition, binding)));
        }
        // Apply ORDER BY
        ...
        const rows = filteredBindings.map(binding => query.return.map(r => this.evaluateExpression(r.expression, binding)));
        return { columns, rows };
    }
    executeMatchPattern(pattern, existingBindings) {
        const results = [];
    ...
    evaluateCondition(condition, binding) {

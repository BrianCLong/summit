...
exports.RAMPolicy = void 0;
const tracker_js_1 = require("../evidence/tracker.js");
class RAMPolicy {
    name = 'ram';
    merge(deltas, context) {
        const evidence = new tracker_js_1.EvidenceTracker();
        const mergedParams = {};
        ...
        for (const key of allKeys) {
            ...
            for (let i = 0; i < len; i++) {
                ...
                for (const d of deltas) {
                    if (Math.abs((d.parameters[key]?.[i] ?? 0)) > context.threshold)
                        count++;
                }
                if (count === 1)
                    uniqueElements++;

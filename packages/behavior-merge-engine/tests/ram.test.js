...
const ram_js_1 = require("../src/policies/ram.js");
(0, vitest_1.describe)('RAMPolicy', () => {
    const policy = new ram_js_1.RAMPolicy();
    const context = { threshold: 0.1, rescale: true };
    (0, vitest_1.it)('should preserve unique parameters without dilution', () => {
        ...
        (0, vitest_1.expect)(merged[0]).toBeCloseTo(10.0);
        // Index 1: Unique to t2 (10.0).
        (0, vitest_1.expect)(merged[1]).toBeCloseTo(10.0);
        // Index 2: Shared by all 3 (5.0, 5.0, 5.0). Average is 5.0.
        (0, vitest_1.expect)(merged[2]).toBeCloseTo(5.0);
    });
    (0, vitest_1.it)('should average shared parameters correctly', () => {
        ...
        (0, vitest_1.expect)(merged[0]).toBeCloseTo(15.0);
    });
    (0, vitest_1.it)('should zero out parameters below threshold in all tasks', () => {
        const deltas = [
            { id: 't1', parameters: { 'w': [0.05] } },
            { id: 't2', parameters: { 'w': [-0.05] } }
        ];
        const result = policy.merge(deltas, context);
        (0, vitest_1.expect)(result.mergedDelta.parameters['w'][0]).toBe(0);
    });
    (0, vitest_1.it)('should generate correct evidence stats', () => {
        const deltas = [
        ...
        // Total elements: 4
        (0, vitest_1.expect)(stats.totalParams).toBe(4);
        // Unique elements: Index 0 and Index 2. Count = 2.
        // Ratio = 2/4 = 0.5
        (0, vitest_1.expect)(stats.uniqueRatio).toBe(0.5);
        // Shared elements: Index 1. Count = 1.
        // Ratio = 1/4 = 0.25
        (0, vitest_1.expect)(stats.sharedRatio).toBe(0.25);
    });
});

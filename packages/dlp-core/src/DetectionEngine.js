...
class DetectionEngine {
    ...
    async detect(content, context) {
        ...
        for (const pattern of this.patterns) {
            ...
            while ((match = regex.exec(content)) !== null) {
                if (detections.length >= (this.config.maxDetections || 1000))
                    break;
                ...
                if (pattern.contextBoost) {
                    ...
                    const contextEnd = Math.min(content.length, match.index + matchedValue.length + (this.config.contextWindow || 50));
     ...
     */
    checkFalsePositive(value, filterType) {
        switch (filterType) {
            case 'phone':
     ...
     */
    extractContext(content, start, length) {
...
exports.default = DetectionEngine;

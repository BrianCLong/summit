...
exports.generateAnomalyEvents = exports.generateSeasonalEvents = exports.generateBurstEvents = exports.generateUniformEvents = void 0;
const defaultRandom = () => Math.random();
const createEvent = (id, timestamp, signal, basePayload, tags) => ({
    id,
    timestamp,
    payload: { ...basePayload, signal },
    tags: tags ? [...tags] : undefined,
});
const resolveStartTimestamp = (provided) => provided ?? Date.now();
const resolveInterval = (provided) => Math.max(1, provided ?? 1000);
/**
 * Generates a uniform stream of events spaced by {@link UniformGeneratorOptions.intervalMs}.
 */
const generateUniformEvents = (count, options = {}) => {
    ...
    return Array.from({ length: Math.max(0, count) }, (_, index) => {
        ...
        return createEvent(`uniform-${index}`, start + index * interval + jitter, signal, basePayload, tags);
    });
};
exports.generateUniformEvents = generateUniformEvents;
/**
 * Produces bursts of heightened activity separated by idle windows.
 */
const generateBurstEvents = (options) => {
    const random = options.random ?? defaultRandom;
    const start = resolveStartTimestamp(options.startTimestamp);
    const interval = resolveInterval(options.intervalMs);
    const basePayload = options.basePayload ?? {};
    ...
    for (let burst = 0; burst < options.bursts; burst += 1) {
        for (let item = 0; item < options.burstSize; item += 1) {
            ...
            events.push(createEvent(`burst-${burst}-${item}`, timestamp + item * interval, signal, basePayload, tags));
        }
...
exports.generateBurstEvents = generateBurstEvents;
/**
 * Generates a seasonal waveform using a sine wave for the signal attribute.
 */
const generateSeasonalEvents = (options) => {
    const random = options.random ?? defaultRandom;
    const start = resolveStartTimestamp(options.startTimestamp);
    const interval = resolveInterval(options.intervalMs);
    const basePayload = options.basePayload ?? {};
    ...
    for (let period = 0; period < options.periods; period += 1) {
        for (let point = 0; point < options.pointsPerPeriod; point += 1) {
            ...
            events.push(createEvent(`seasonal-${period}-${point}`, start + index * interval + jitter, signal, basePayload, tags));
            index += 1;
        }
    }
    return events;
};
exports.generateSeasonalEvents = generateSeasonalEvents;
/**
 * Injects sporadic anomalies into a mostly stable stream.
 */
const generateAnomalyEvents = (options) => {
    const random = options.random ?? defaultRandom;
    const start = resolveStartTimestamp(options.startTimestamp);
    const interval = resolveInterval(options.intervalMs);
    const basePayload = options.basePayload ?? {};
    ...
    return Array.from({ length: Math.max(0, options.count) }, (_, index) => {
        ...
        return createEvent(`anomaly-${index}`, start + index * interval, signal, { ...basePayload, isAnomaly }, tags);
    });
};
exports.generateAnomalyEvents = generateAnomalyEvents;

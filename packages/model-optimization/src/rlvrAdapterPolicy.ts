...
const adapterAliases = new Map<string, PeftAdapterKind>([
  ['rank-1', 'rank1'],
  ['rank_1', 'rank1'],
  ['ln-only', 'ln_tune_only'],
  ['ln_only', 'ln_tune_only'],
  ['ln-tune-only', 'ln_tune_only'],
  ['ia3', 'ia3_only'],
  ['ia3-only', 'ia3_only'],
  ['ia3_only', 'ia3_only'],
]);
...
function normalizeAdapterKind(adapter?: PeftAdapterKind): PeftAdapterKind | undefined {
  if (!adapter) {
    return adapter;
  }

  const trimmed = adapter.trim().toLowerCase();
  if (adapterAliases.has(trimmed)) {
    return adapterAliases.get(trimmed);
  }

  if (trimmed.startsWith('svd-init')) {
    return trimmed.replace(/^svd-init[-_]?/, 'svd_init_');
  }
...
export function parseRlvrAdapterConfig(
  ...
  const normalizedAdapter = normalizeAdapterKind(parsed.adapter);
  const effectiveAdapter = parsed.rlvrEnabled
    ? normalizedAdapter ?? STRUCTURAL_DEFAULT
    : normalizedAdapter ?? 'lora';
...
export function evaluateRlvrAdapterPolicy(
  ...
  const adapter =
    normalizeAdapterKind(rawConfig.adapter) ?? (config.rlvrEnabled ? STRUCTURAL_DEFAULT : 'lora');
  const rank = rawConfig.rank ?? config.rank ?? 32;
  const overrideActive = Boolean(config.allowUnsafe || config.warnOnly);
  ...
  for (const issue of decision.issues) {
    ...
    if (issue.requiresOverride && !overrideActive) {
      decision.allowed = false;
      decision.decision = 'error';
      decision.messages.push(issue.detail);
    } else {
      decision.overrideUsed = decision.overrideUsed || (issue.requiresOverride && overrideActive);
      decision.decision = decision.decision === 'error' ? 'error' : 'warn';
      const exceptionNote = decision.overrideUsed ? ' (governed exception)' : '';
      decision.messages.push(`${issue.detail}${exceptionNote}`);
      logger.warn?.(`RLVR PEFT guardrail: ${issue.detail}${exceptionNote}`);
    }
  }
  ...
  logger.info?.('RLVR adapter policy decision', {
    adapter,
    rank,
    allowed: decision.allowed,
    overrideUsed: decision.overrideUsed,
    estimatedTrainableParams: decision.estimatedTrainableParams,

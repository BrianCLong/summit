...
const OperationalTransform_1 = require("./OperationalTransform");
class SyncEngine extends events_1.EventEmitter {
    ...
    constructor(store, options = {}) {
        super();
        this.store = store;
        this.ot = new OperationalTransform_1.OperationalTransform();
        this.defaultResolution =
            options.conflictResolution ?? { strategy: 'last_write_wins' };
    }
    /**
     * Initialize a new sync session
     */
    async createSession(documentId, userId, workspaceId, permissions) {
        const session = {
     ...
     */
    async processOperationQueue(documentId) {
    ...
    async processQueueInternal(documentId) {
        while (true) {
            ...
            for (const pendingOp of pending) {
                ...
                if (conflictingApplied.length > 0) {
                    resolutionWinner = await this.resolveConflict(documentId, [transformedOp, ...conflictingApplied], this.defaultResolution);
                    this.emit('conflicts:resolved', {
                        documentId,
                        winner: resolutionWinner,
                        losers: [transformedOp, ...conflictingApplied].filter(op => op.id !== resolutionWinner.id)
                    });
                }
     ...
     */
    async syncClient(documentId, clientVersion) {
        const state = await this.store.getDocumentState(documentId);
        if (!state) {
            throw new Error('Document not found');
        }
     ...
     */
    async acquireLock(documentId, userId, lockType = 'write', duration = 30000 // 30 seconds default
    ) {
        const lock = {
            documentId,
            userId,
     ...
     */
    async releaseLock(documentId, userId) {
     ...
     */
    async cleanupSessions(maxIdleTime = 300000) {
     ...
     */
    applyOperationToObject(content, operation) {
        ...
        if (operation.attributes?.path) {
            const path = operation.attributes.path;
    ...
    transformAgainstHistory(operation, history, applied) {
        ...
        for (const committed of applied) {
            if (committed.id === transformed.id)
                continue;
     ...
     */
    findConflicts(operation, applied) {
     ...
     */
    async resolveConflict(documentId, conflictingOps, resolution) {
    ...
    dequeuePendingOperations(documentId) {

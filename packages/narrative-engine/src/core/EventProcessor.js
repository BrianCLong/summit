...
class EventProcessor {
    ...
    propagateInfluence(actor) {
        ...
        for (const relationship of relationships) {
            ...
            this.propagationLog.push(`${actor.name} influenced ${target.name}: mood ${before.toFixed(2)} -> ${after.toFixed(2)}`);
        }
    }
    drainPropagationLog() {
    ...
    generateTriggeredEvents(actor, event) {
        const events = [];
        if (actor.getMood() <= -5) {

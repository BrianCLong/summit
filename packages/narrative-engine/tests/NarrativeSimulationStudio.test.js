...
describe('NarrativeSimulationStudio', () => {
    it('models influence campaigns with counter-narratives and operation scoring', () => {
        const engine = new index_js_1.SimulationEngine();
        engine.initialize(config);
        const studio = new index_js_1.NarrativeSimulationStudio(engine);
        ...
        const influenceEvents = tick.generatedEvents.filter((event) => event.type === 'influence-campaign');
        expect(influenceEvents).toHaveLength(2);
        const counterEvents = tick.generatedEvents.filter((event) => event.type === 'counter-narrative');

...
describe('signal processing toolkit', () => {
    it('identifies dominant tone via FFT', () => {
        ...
        const analyzer = new index_js_1.FftAnalyzer(1024);
    ...
    it('reconstructs signal from Haar wavelets', () => {
        ...
        const transformed = (0, index_js_1.haarTransform)(original, 3);
        const reconstructed = (0, index_js_1.reconstructFromHaar)(transformed);
    ...
    it('attenuates high frequencies with Butterworth and Chebyshev filters', () => {
        ...
        const butter = new index_js_1.IIRFilter((0, index_js_1.designButterworthLowpass)(4, 500, sampleRate));
        const cheby = new index_js_1.IIRFilter((0, index_js_1.designChebyshevLowpass)(4, 500, sampleRate, 0.5));
        const butterOut = butter.processBuffer(mixed);
        const chebyOut = cheby.processBuffer(mixed);
        const rms = (arr) => Math.sqrt(arr.reduce((acc, v) => acc + v * v, 0) / arr.length);
    ...
    it('reduces noise via Kalman filter with deterministic sequence', () => {
        ...
        const kalman = new index_js_1.KalmanFilter1D(truth, 0.05, 1);
    ...
    it('extracts spectral metrics', () => {
        ...
        const analyzer = new index_js_1.SpectralAnalyzer(new index_js_1.FftAnalyzer(512));
    ...
    it('reduces spectral leakage with Hann windowing', () => {
        ...
        const rectangular = new index_js_1.FftAnalyzer(size, { window: (0, index_js_1.createWindow)('rectangular', size) }).analyze(signal, sampleRate);
        const hann = new index_js_1.FftAnalyzer(size, { window: (0, index_js_1.createWindow)('hann', size) }).analyze(signal, sampleRate);
        const leakage = (frame) => {
    ...
    it('produces time-localized spectrogram slices', () => {
        ...
        const spectrogram = (0, index_js_1.computeSpectrogram)(combined, sampleRate, fftSize, { hopSize });
    ...
    it('classifies frames via spectral signatures', () => {
        ...
        const fft = new index_js_1.FftAnalyzer(256);
        const analyzer = new index_js_1.SpectralAnalyzer(fft);
        const bands = [
            { name: 'low', fromHz: 0, toHz: 400 },
            { name: 'mid', fromHz: 400, toHz: 800 },
            { name: 'high', fromHz: 800, toHz: 1000 },
        ];
        const classifier = new index_js_1.SpectralPatternClassifier(analyzer, bands);
    ...
    it('routes frames across WebSocket and WebRTC data channels', () => {
        ...
        const pipeline = new index_js_1.SignalStreamingPipeline({
            sampleRate: 8000,
            frameSize: 8,
            processors: [

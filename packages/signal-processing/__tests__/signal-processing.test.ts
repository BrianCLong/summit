...
describe("signal processing toolkit", () => {
  ...
  it("reduces noise via Kalman filter with deterministic sequence", () => {
    ...
    const filteredError =
      Array.from(filtered).reduce((acc, v) => acc + Math.abs(v - truth), 0) / filtered.length;
  ...
  it("reduces spectral leakage with Hann windowing", () => {
    ...
    const rectangular = new FftAnalyzer(size, {
      window: createWindow("rectangular", size),
    }).analyze(signal, sampleRate);
    const hann = new FftAnalyzer(size, { window: createWindow("hann", size) }).analyze(
      signal,
      sampleRate
    );

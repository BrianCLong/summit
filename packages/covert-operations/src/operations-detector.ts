...
export class OperationsDetector {
   ...
   */
  detectInfluenceOperation(data: Partial<InfluenceOperation>): InfluenceOperation {
   ...
   */
  analyzePoliticalInterference(data: Partial<PoliticalInterference>): PoliticalInterference {
   ...
   */
  trackSabotageOperation(data: Partial<SabotageOperation>): SabotageOperation {
   ...
   */
  monitorProxyOperation(data: Partial<ProxyOperation>): ProxyOperation {
  ...
  assessThreatLevel(operation: {
    ...
    const sophisticationScore =
      operation.sophistication === "ADVANCED" ? 3 : operation.sophistication === "MODERATE" ? 2 : 1;
    const scaleScore = operation.scale === "LARGE" ? 3 : operation.scale === "MEDIUM" ? 2 : 1;
   ...
   */
  generateIndicators(
    operation: InfluenceOperation | PoliticalInterference | SabotageOperation
  ): Array<{
    indicator: string;
    type: string;
    confidence: number;

...
function hasMatchedIndicator(
  signature: PGActionSignature,
...
export function matchPlaybook(params: {
  playbook: PGPlaybook;
  actionSignaturesById: Record<string, PGActionSignature>;
  ...
  for (const step of playbook.steps) {
    ...
    for (const indicator of signature.indicators) {
      ...
      const foundEvidence = evidence.find((evidenceItem) =>
        evidenceItem.signal.toLowerCase().includes(indicator.signal.toLowerCase())
      );
  ...
  const plausibleStepIds = playbook.steps
    .filter((step) => {
      ...
      return hasMatchedIndicator(signature, matchedIndicators);
    })
    .map((step) => step.id);

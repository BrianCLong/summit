...
import type { PGPlaybook, PGPlaybookOutcome } from '../engine/types';
...
function collectOutcomeFields(outcomes: PGPlaybookOutcome[]): Array<{ path: string; value?: string }> {
...
export function validatePlaybookSemantics(playbook: PGPlaybook): SVViolation[] {
  ...
  if (Array.isArray(playbook.outcomes)) {
    fieldsToScan.push(...collectOutcomeFields(playbook.outcomes));
  }
  ...
  if (!playbook.useCase || !allowed.has(playbook.useCase)) {
    violations.push({

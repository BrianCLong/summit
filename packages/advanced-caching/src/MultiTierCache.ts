...
import { CacheConfig, CacheEntry, CacheOptions, CacheStats, TierStats } from "./types.js";
...
export class MultiTierCache {
  ...
  constructor(private config: CacheConfig) {
    ...
    if (config.stampedePrevention) {
      this.stampedeProtection = new StampedeProtection(this.l2Redis!, {
        lockTTL: 30000,
        lockRetryDelay: 100,
        maxRetries: 10,
      });
  ...
  private async setL2<T>(key: string, entry: CacheEntry<T>, ttl: number): Promise<void> {
  ...
  private updateLatency(latency: number): void {
    ...
    this.stats.overall.avgLatency =
      totalRequests > 0 ? (currentAvg * (totalRequests - 1) + latency) / totalRequests : latency;
  }
}

...
class Triangulator {
    ...
    triangulateAOA(bearings) {
        ...
        for (let i = 0; i < bearings.length; i++) {
            for (let j = i + 1; j < bearings.length; j++) {
                const intersection = this.findBearingIntersection(bearings[i].sensorLocation, bearings[i].bearing, bearings[j].sensorLocation, bearings[j].bearing);
                if (intersection) {
        ...
        const distances = intersections.map(int => this.calculateDistance(lat, lon, int.lat, int.lon));
        const accuracy = Math.sqrt(distances.reduce((sum, d) => sum + d * d, 0) / distances.length);
        // Calculate GDOP from geometry
        const gdop = this.calculateGDOP({ latitude: lat, longitude: lon }, bearings.map(b => b.sensorLocation));
        return {
            id: (0, uuid_1.v4)(),
            method: 'AOA',
            latitude: lat,
    ...
    triangulateHybrid(bearings, rssi, cells) {
        ...
        return {
            ...
            timestamp: new Date(),
            details: {
                geometricDOP: this.calculateGDOP({ latitude: lat / totalWeight, longitude: lon / totalWeight }, [
     ...
     */
    generateSimulatedMeasurements(targetLocation, method) {
        const result = {};
    ...
    generateSimulatedBearings(target, count) {
        const bearings = [];
        ...
        for (let i = 0; i < count; i++) {
            ...
            const trueBearing = this.calculateBearing(sensorLat, sensorLon, target.latitude, target.longitude);
            bearings.push({
                sensorId: `SENSOR-${i + 1}`,
    ...
    generateSimulatedRSSI(target, count) {
        const measurements = [];
        const radius = 0.03;
        for (let i = 0; i < count; i++) {
            ...
            const sensorLat = target.latitude + radius * Math.cos(angle);
            const sensorLon = target.longitude + radius * Math.sin(angle);
            const distance = this.calculateDistance(sensorLat, sensorLon, target.latitude, target.longitude);
    ...
    generateSimulatedCellTowers(target, count) {
        const towers = [];
        const radius = 0.02;
        for (let i = 0; i < count; i++) {
            ...
            const distance = this.calculateDistance(towerLat, towerLon, target.latitude, target.longitude);
    ...
    findBearingIntersection(loc1, bearing1, loc2, bearing2) {
        ...
        const dist12 = 2 * Math.asin(Math.sqrt(Math.sin(dLat / 2) ** 2 +
            Math.cos(lat1) * Math.cos(lat2) * Math.sin(dLon / 2) ** 2));
        if (Math.abs(dist12) < 1e-10)
            return null;
        const bearingA = Math.acos((Math.sin(lat2) - Math.sin(lat1) * Math.cos(dist12)) /
            (Math.sin(dist12) * Math.cos(lat1)));
        const bearingB = Math.acos((Math.sin(lat1) - Math.sin(lat2) * Math.cos(dist12)) /
        ...
        const dist13 = Math.atan2(Math.sin(dist12) * Math.sin(alpha1) * Math.sin(alpha2), Math.cos(alpha2) + Math.cos(alpha1) * Math.cos(alpha3));
        const lat3 = Math.asin(Math.sin(lat1) * Math.cos(dist13) +
            Math.cos(lat1) * Math.sin(dist13) * Math.cos(b1));
        const dLon13 = Math.atan2(Math.sin(b1) * Math.sin(dist13) * Math.cos(lat1), Math.cos(dist13) - Math.sin(lat1) * Math.sin(lat3));
    ...
    calculateGDOP(position, sensors) {
        if (sensors.length < 2)
            return 10;
        let sumAngleDiff = 0;
        let count = 0;
        for (let i = 0; i < sensors.length; i++) {
            for (let j = i + 1; j < sensors.length; j++) {
                const bearing1 = this.calculateBearing(position.latitude, position.longitude, sensors[i].latitude, sensors[i].longitude);
                const bearing2 = this.calculateBearing(position.latitude, position.longitude, sensors[j].latitude, sensors[j].longitude);

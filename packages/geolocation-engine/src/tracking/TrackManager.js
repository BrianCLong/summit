...
class TrackManager {
     ...
     */
    processLocation(location, targetId) {
     ...
     * Create a new track
     */
    createTrack(location, targetId) {
        const track = {
            id: (0, uuid_1.v4)(),
            targetId: targetId || `TARGET-${(0, uuid_1.v4)().slice(0, 8)}`,
            status: 'tentative',
            position: {
     ...
     */
    updateTrack(track, location) {
        const timeDelta = (location.timestamp.getTime() - track.lastUpdate.getTime()) / 1000;
        // Calculate velocity
        if (timeDelta > 0) {
            const distance = this.calculateDistance(track.position.latitude, track.position.longitude, location.latitude, location.longitude);
            track.velocity.speed = distance / timeDelta;
            track.velocity.heading = this.calculateBearing(track.position.latitude, track.position.longitude, location.latitude, location.longitude);
    ...
    analyzeMovement(trackId) {
        ...
        for (let i = 1; i < history.length; i++) {
            ...
            headings.push(this.calculateBearing(prev.latitude, prev.longitude, curr.latitude, curr.longitude));
    ...
    findNearestTrack(location) {
        ...
        for (const track of this.tracks.values()) {
            if (track.status === 'deleted' || track.status === 'lost')
                continue;
            const distance = this.calculateDistance(track.position.latitude, track.position.longitude, location.latitude, location.longitude);
     ...
     */
    shouldAssociate(track, location) {
        const distance = this.calculateDistance(track.position.latitude, track.position.longitude, location.latitude, location.longitude);
        // Factor in predicted position
        const timeDelta = (location.timestamp.getTime() - track.lastUpdate.getTime()) / 1000;
        const maxPredictedMovement = track.velocity.speed * timeDelta * 1.5; // Allow 50% margin
        const threshold = Math.max(this.config.associationThreshold, maxPredictedMovement, track.position.accuracy + location.accuracy.horizontal);

...
const entity_js_1 = require("../types/entity.js");
class KnowledgeGraphManager {
    ...
    async createEntity(entity) {
        ...
        const validated = entity_js_1.KGEntitySchema.parse(fullEntity);
    ...
    async getEntity(entityId) {
        ...
        `, { entityId });
            ...
            return entity_js_1.KGEntitySchema.parse({
                ...props,
                labels,
    ...
    async updateEntity(entityId, updates) {
        ...
        try {
            const now = new Date().toISOString();
            const setters = [];
            const params = { entityId, updatedAt: now };
    ...
    async createRelationship(relationship) {
        ...
        const validated = entity_js_1.KGRelationshipSchema.parse(fullRelationship);
        const session = this.driver.session();
        try {
            await session.run(`
     ...
     */
    async findEntitiesByType(type, limit = 100, offset = 0) {
        const session = this.driver.session();
        try {
            const result = await session.run(`
        MATCH (e {type: $type})
     ...
     */
    async findRelationships(sourceId, targetId, type, limit = 100) {
        const session = this.driver.session();
        try {
            ...
            return result.records.map((record) => {
                ...
                return entity_js_1.KGRelationshipSchema.parse({
                    ...props,
     ...
     */
    async getEntityNeighborhood(entityId, depth = 1, relationshipTypes) {
        const session = this.driver.session();
        try {
        ...
        `, { entityId });
            ...
            const entity = entity_js_1.KGEntitySchema.parse({
                ...entityProps,
                labels: entityLabels,

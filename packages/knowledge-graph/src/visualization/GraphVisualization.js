...
class GraphVisualization {
     ...
     */
    async extractSubgraph(startNodeId, options = {}) {
     ...
     */
    async findPaths(sourceId, targetId, maxLength = 5) {
        const session = this.driver.session();
        try {
            const result = await session.run(`
     ...
     */
    async detectClusters(algorithm = 'louvain') {
        const session = this.driver.session();
        try {
      ...
      `);
            ...
            const algoQuery = algorithm === 'louvain'
     ...
     */
    async exportGraph(format, graphData) {
        switch (format) {
            case 'json':

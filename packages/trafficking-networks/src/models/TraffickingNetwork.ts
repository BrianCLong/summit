...
export const TraffickingRouteSchema = z.object({
  ...
  // Route details
  transportMethods: z.array(
    z.enum(["AIR", "MARITIME", "LAND_VEHICLE", "RAIL", "FOOT", "MULTIPLE"])
  ),
  averageDuration: z.string().optional(), // e.g., "2-4 weeks"
  estimatedFrequency: z.string().optional(),

  // Border crossings
  borderCrossings: z.array(
    z.object({
      location: z.string(),
      method: z.enum([
        "LEGAL_ENTRY",
...
export class VictimProtectionValidator {
  ...
  static validateVictimAccess(userId: string, userRole: string): boolean {
    ...
    if (!authorizedRoles.includes(userRole)) {
      throw new Error(
        "Unauthorized access to victim data - restricted to victim services and authorized personnel only"
      );
    }

...
exports.InfluencerScorer = void 0;
class InfluencerScorer {
    /**
     * Calculate influencer score
     */
    scoreInfluencer(profile, posts) {
        const metrics = {
        ...
        const overallScore = metrics.reach * weights.reach +
     ...
     */
    calculateEngagement(profile, posts) {
        if (!profile.engagement || posts.length === 0)
            return 0;
        const avgEngagement = posts.reduce((sum, p) => sum + p.likes + p.comments * 2 + p.shares * 3, 0) /
            posts.length;
...
exports.InfluencerScorer = InfluencerScorer;

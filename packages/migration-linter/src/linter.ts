...
const destructiveRules: Array<{
  ...
  },
  {
    ...
    remediation:
      'Backfill + add default, then enforce NOT NULL in a follow-up migration.',
  },
  {
    id: 'add-not-null',
    description: 'ADD COLUMN NOT NULL without default detected',
    detector: (content) =>
      /\bADD\s+COLUMN\b[\s\S]*?\bNOT\s+NULL\b/i.test(content) &&
      !/\bDEFAULT\b/i.test(content),
    remediation:
      'Add column as NULLable, backfill, then add NOT NULL constraint later.',
  },
];
...
async function lintFile(file: string): Promise<LintFinding[]> {
  ...
  const normalized =
    file.endsWith('.sql') || file.endsWith('.cypher')

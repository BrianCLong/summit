...
exports.CrisisDiplomacyMonitor = void 0;
 ...
 */
class CrisisDiplomacyMonitor {
    crises = new Map();
    crisesByType = new Map();
    crisesByPhase = new Map();
    peaceProcesses = new Map();
    mediationEfforts = new Map();
    /**
     * Track a crisis
     */
    trackCrisis(crisis) {
    ...
    getActiveCrises() {
        return Array.from(this.crises.values())
            .filter(c => c.phase !== types_js_1.CrisisPhase.RESOLVED &&
            c.phase !== types_js_1.CrisisPhase.FROZEN &&
            c.monitoring)
     ...
     */
    assessEscalationRisk(crisisId) {
        const crisis = this.crises.get(crisisId);
        if (!crisis) {
            return {
                overallRisk: 0,
                level: types_js_1.EscalationLevel.LOW,
                immediateRisks: [],
                criticalIndicators: [],
                mitigationOptions: [],
                urgency: 'LOW'
        ...
        const criticalIndicators = immediateRisks
            ...
            .slice(0, 10);
        // Determine level
        let level = types_js_1.EscalationLevel.LOW;
        if (overallRisk >= 80)
            level = types_js_1.EscalationLevel.MAXIMUM;
        else if (overallRisk >= 60)
            level = types_js_1.EscalationLevel.CRITICAL;
        else if (overallRisk >= 40)
            level = types_js_1.EscalationLevel.HIGH;
        else if (overallRisk >= 20)
            level = types_js_1.EscalationLevel.MODERATE;
        // Mitigation options
        const mitigationOptions = this.generateMitigationOptions(crisis, immediateRisks);
        // Urgency
        let urgency = 'LOW';
        if (level === types_js_1.EscalationLevel.MAXIMUM || level === types_js_1.EscalationLevel.CRITICAL) {
            urgency = 'CRITICAL';
        }
        else if (level === types_js_1.EscalationLevel.HIGH) {
            urgency = 'HIGH';
        }
        else if (level === types_js_1.EscalationLevel.MODERATE) {
            urgency = 'MEDIUM';
        }
        return {
            overallRisk,
     ...
     */
    identifyDeescalationOpportunities(crisisId) {
        const crisis = this.crises.get(crisisId);
        if (!crisis) {
            return {
                opportunities: [],
     ...
     */
    trackMediationEffort(effort) {
        this.mediationEfforts.set(effort.id, effort);
        // Add to crisis if exists
        for (const crisis of this.crises.values()) {
            const hasMediationForCrisis = crisis.primaryParties.some(p => effort.parties.includes(p.name));
     ...
     */
    analyzeMediationEffectiveness(effortId) {
        ...
        const effectivenessScore = (effort.factors.mediatorCredibility * 0.3 +
     ...
     */
    monitorCeasefireCompliance(crisisId) {
        const crisis = this.crises.get(crisisId);
        if (!crisis) {
            return {
                ceasefires: [],
        ...
        const activeCeasefires = crisis.ceasefires.filter(c => c.status === 'HOLDING' || c.status === 'VIOLATED');
        const avgCompliance = activeCeasefires.length > 0
            ? activeCeasefires.reduce((sum, c) => sum + c.compliance, 0) / activeCeasefires.length
            : 0;
        const totalViolations = crisis.ceasefires.reduce((sum, c) => sum + (c.violations?.length || 0), 0);
        // Determine trend
        let trend = 'STABLE';
        if (activeCeasefires.length >= 2) {
            ...
            if (recent.compliance > previous.compliance + 10)
                trend = 'IMPROVING';
            else if (recent.compliance < previous.compliance - 10)
                trend = 'DETERIORATING';
        }
        // Risk level
        let riskLevel = 'LOW';
        if (avgCompliance < 50 || trend === 'DETERIORATING')
            riskLevel = 'HIGH';
     ...
     */
    predictTrajectory(crisisId) {
        const crisis = this.crises.get(crisisId);
        if (!crisis) {
            return {
                shortTerm: {
                    phase: types_js_1.CrisisPhase.EMERGING,
                    probability: 0,
                    keyFactors: []
                },
                mediumTerm: {
        ...
        if (crisis.escalationLevel === types_js_1.EscalationLevel.MAXIMUM) {
            shortTermPhase = types_js_1.CrisisPhase.PEAK;
            mediumTermPhase = types_js_1.CrisisPhase.DE_ESCALATING;
            longTermPhase = types_js_1.CrisisPhase.STABILIZING;
        }
            ...
            if (activeMediations.length > 0) {
                shortTermPhase = types_js_1.CrisisPhase.STABILIZING;
                mediumTermPhase = types_js_1.CrisisPhase.DE_ESCALATING;
                longTermPhase = types_js_1.CrisisPhase.RESOLVED;
            }
        }
        return {
            shortTerm: {
    ...
    compareCrises(crisisIds) {
        ...
        for (const crisis of crises) {
            if (crisis.phase === types_js_1.CrisisPhase.RESOLVED) {
     ...
     */
    generateEarlyWarning(indicators) {
    ...
    generateMitigationOptions(crisis, risks) {
        const options = [];
    ...
    generateMediationRecommendations(effort, score) {
        const recommendations = [];
    ...
    getStatistics() {
        ...
        for (const crisis of this.crises.values()) {
            ...
            if (crisis.escalationLevel === types_js_1.EscalationLevel.CRITICAL ||
                crisis.escalationLevel === types_js_1.EscalationLevel.MAXIMUM) {
                criticalCount++;
            }
        }
...
exports.CrisisDiplomacyMonitor = CrisisDiplomacyMonitor;

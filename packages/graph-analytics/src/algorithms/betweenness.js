...
exports.calculateBetweenness = calculateBetweenness;
exports.findBridgeNodes = findBridgeNodes;
exports.calculateNetworkVulnerability = calculateNetworkVulnerability;
/**
 ...
 */
function calculateBetweenness(graph, options = {}) {
    const startTime = performance.now();
    const { normalized = true, directed = false, weightProperty, includeEdgeBetweenness = false, sampleSize, } = options;
    const nodes = graph.nodes;
    const nodeCount = nodes.length;
    // Initialize betweenness scores
    const nodeBetweenness = new Map();
    const edgeBetweenness = includeEdgeBetweenness ? new Map() : undefined;
 ...
 */
function bfsTraversal(source, adjacency, distance, sigma, predecessors, stack) {
    const queue = [source];
 ...
 */
function dijkstraTraversal(source, adjacency, distance, sigma, predecessors, stack) {
 ...
 */
function findBridgeNodes(result, threshold = 0.1) {
 ...
 */
function calculateNetworkVulnerability(result) {
    ...
    const variance = scores.reduce((sum, s) => sum + Math.pow(s - avgBetweenness, 2), 0) / scores.length;

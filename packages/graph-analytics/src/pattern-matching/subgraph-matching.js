...
exports.findSubgraphMatches = findSubgraphMatches;
exports.discoverMotifs = discoverMotifs;
exports.detectAnomalousSubgraphs = detectAnomalousSubgraphs;
/**
 ...
 */
function findSubgraphMatches(graph, pattern, options = {}) {
    ...
    const mapping = new Map(); // pattern -> graph
    const reverseMapping = new Map(); // graph -> pattern
    function backtrack(patternNodeIndex) {
 ...
 */
function discoverMotifs(graph, options = {}) {
    const startTime = performance.now();
 ...
 */
function detectAnomalousSubgraphs(graph, options = {}) {
    ...
    const degreeStdDev = Math.sqrt(Array.from(degrees.values()).reduce((sum, d) => sum + Math.pow(d - avgDegree, 2), 0) / degrees.size);
    const anomalies = [];
...
function sampleRandomNodes(nodes, k) {

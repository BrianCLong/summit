...
exports.analyzeTemporalEvolution = analyzeTemporalEvolution;
exports.analyzeTemporalCentrality = analyzeTemporalCentrality;
exports.analyzeEventSequences = analyzeEventSequences;
/**
 ...
 */
function analyzeTemporalEvolution(temporalGraph, options = {}) {
    ...
    for (let i = 0; i <= numSnapshots; i++) {
        ...
        const activeNodes = temporalGraph.nodes
            ...
            .map((n) => n.id);
        const activeEdges = temporalGraph.edges.filter((e) => e.timestamp <= snapshotTime &&
 ...
 */
function analyzeTemporalCentrality(temporalGraph, options = {}) {
    const startTime = performance.now();
 ...
 */
function analyzeEventSequences(temporalGraph, options = {}) {
    const startTime = performance.now();
    const { minSupport = 3, maxTimeGap = 86400000, // 1 day
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        const totalDuration = events[events.length - 1].timestamp.getTime() - events[0].timestamp.getTime();

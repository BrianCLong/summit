...
exports.detectCoTravelPatterns = detectCoTravelPatterns;
exports.detectFinancialStructuring = detectFinancialStructuring;
exports.detectCommunicationBursts = detectCommunicationBursts;
/**
 * Detect co-travel/co-presence patterns in spacetime
 */
function detectCoTravelPatterns(graph, options) {
    const startTime = performance.now();
    const { timeWindow, distanceThreshold, minCoOccurrences, entityTypes, startTime: analysisStart, endTime: analysisEnd, } = options;
    // Filter nodes by entity type and time range
    let nodes = graph.nodes.filter((n) => {
        if (entityTypes && entityTypes.length > 0) {
            if (!entityTypes.includes(n.type || ''))
                return false;
        }
        if (analysisStart !== undefined && n.timestamp !== undefined) {
            if (n.timestamp < analysisStart)
                return false;
        }
        if (analysisEnd !== undefined && n.timestamp !== undefined) {
            if (n.timestamp > analysisEnd)
                return false;
        }
        ...
        if (count >= minCoOccurrences) {
            ...
            patterns.push({
                ...
                metadata: {
                    coOccurrences: count,
                    events,
                    avgDistance: events.reduce((sum, e) => sum + calculateDistance(e.location, node2.location), 0) /
                        events.length,
 ...
 */
function detectFinancialStructuring(graph, options) {
    const startTime = performance.now();
    const { timeWindow, minBranches, maxHops, amountThreshold, patternType, centerNodeTypes, } = options;
    const patterns = [];
    ...
    if (patternType === 'fan-out' || patternType === 'both') {
        for (const node of graph.nodes) {
            if (centerNodeTypes && centerNodeTypes.length > 0) {
                if (!centerNodeTypes.includes(node.type || ''))
                    continue;
            }
        ...
        for (const node of graph.nodes) {
            ...
            for (const group of windowGroups) {
                if (group.edges.length >= minBranches) {
                    ...
                    patterns.push({
                        id: `fan-in-${patternId++}`,
                        nodes: [...group.edges.map((e) => e.source), node.id],
                        edges: group.edges.map((e) => ({ source: e.source, target: node.id })),
                        confidence,
                        metadata: {
                            patternType: 'fan-in',
                            centerNode: node.id,
 ...
 */
function detectCommunicationBursts(graph, options) {
    const startTime = performance.now();
    const { timeWindow, baselineRate, burstThreshold, lullThreshold = 0.3, communicationTypes, minBurstDuration = 0, } = options;
    ...
    while (windowStart <= (edges[edges.length - 1].timestamp || 0)) {
        const windowEdges = edges.filter((e) => (e.timestamp || 0) >= windowStart && (e.timestamp || 0) < windowEnd);
        ...
        if (windowRate < calculatedBaselineRate * lullThreshold) {
            const confidence = Math.min(1.0, (calculatedBaselineRate * lullThreshold - windowRate) / calculatedBaselineRate);
            const involvedNodes = new Set();
            windowEdges.forEach((e) => {
                involvedNodes.add(e.source);
                involvedNodes.add(e.target);
...
function groupByTimeWindow(items, timeWindow) {
    if (items.length === 0)
        return [];

...
program.name("graph-sync-validator").description("Graph-Postgres drift validator").version("0.1.0");
...
program
  ...
  .action(async (options) => {
    ...
    const report = createReport(`run-${startTime}`, allFindings, aggregatedPlan, metrics);

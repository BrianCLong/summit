...
export async function compare({ outDir, maxLag }) {
  const [pgNodes, pgEdges, neoNodes, neoEdges] = await Promise.all([
    read("pg.nodes.jsonl"),
    read("pg.edges.jsonl"),
    read("neo.nodes.jsonl"),
    read("neo.edges.jsonl"),
  ]);
  ...
  const byId = (arr) =>
    Object.fromEntries(arr.map((x) => [x.id ?? `${x.src}|${x.rel}|${x.dst}`, canonicalHash(x)]));
  ...
  const violation =
    lagRate > maxLag || orphansPG.length > 0 || nodeDelta.count > 0 || edgeDelta.count > 0;
  ...
  function diffMaps(a, b) {
    let c = 0;
    for (const k of new Set([...Object.keys(a), ...Object.keys(b)])) if (a[k] !== b[k]) c++;
    return { count: c };
  }
}

...
export class GraphSyncValidator {
  ...
  async validate(): Promise<ValidationMetrics> {
    ...
    for (const mapping of this.config.mappings) {
      ...
      const pgRes = await this.pgClient.query(
        `SELECT COUNT(*) FROM ${mapping.table} ${driftClause}`
      );
      ...
      const neoRes = await session.run(
        `MATCH (n:${mapping.label}) ${neoDriftClause} RETURN count(n) as count`
      );
      ...
      if (mapping.relationships) {
        for (const rel of mapping.relationships) {
          const pgRelRes = await this.pgClient.query(
            `SELECT COUNT(*) FROM ${mapping.table} WHERE ${rel.fk} IS NOT NULL`
          );
          const pgRelCount = parseInt(pgRelRes.rows[0].count);

          const neoRelRes = await session.run(
            `MATCH (:${mapping.label})-[r:${rel.relType}]->(:${rel.targetLabel}) RETURN count(r) as count`
          );
      ...
      const sampleRes = await this.pgClient.query(
        `SELECT ${mapping.idKey} FROM ${mapping.table} ORDER BY RANDOM() LIMIT ${sampleSize}`
      );
      const sampleIds = sampleRes.rows.map((r) => r[mapping.idKey]);

      let matches = 0;
      for (const id of sampleIds) {
        const neoSampleRes = await session.run(
          `MATCH (n:${mapping.label} {${mapping.idKey}: $id}) RETURN n`,
          { id }
        );

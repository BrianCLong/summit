  ...
  describe("cluster retrieval", () => {
    it("should get active clusters", async () => {
      ...
      activeClusters.forEach((cluster) => {
        expect([ClusterStatus.ACTIVE, ClusterStatus.EMERGING]).toContain(cluster.status);
      });
    });
    ...
    it("should filter clusters by threat level", async () => {
      ...
      const highThreatClusters = clusteringEngine.getActiveClusters(ThreatLevel.HIGH);

      highThreatClusters.forEach((cluster) => {
        expect([ThreatLevel.HIGH, ThreatLevel.CRITICAL]).toContain(cluster.threatLevel);
      });
    });

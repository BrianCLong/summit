...
import { SignalType, PrivacyLevel, CampaignSignal, ThreatLevel } from "../core/types";
...
describe("FederationService", () => {
  ...
  describe("participant management", () => {
    ...
    it("should get participant status", async () => {
      await federationService.registerParticipant("participant-a", "public-key-a", [
        "SIGNAL_SHARING",
      ]);
  ...
  describe("secure aggregation", () => {
    it("should initiate secure aggregation session", async () => {
      ...
      const session = await federationService.initiateSecureAggregation("SUM", [
        "agg-1",
        "agg-2",
        "agg-3",
      ]);
  ...
  describe("differential privacy", () => {
    it("should add noise to aggregated counts", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        const stats = await federationService.queryAggregatedStats(SignalType.NARRATIVE, 24);

...
describe('AlertEngine', () => {
  ...
  const createMockCluster = (
    overrides?: Partial<CampaignCluster>,
  ): CampaignCluster => ({
  ...
  describe('cluster evaluation', () => {
    ...
    it('should detect cross-tenant spikes', async () => {
      ...
      const alerts = alertEngine.getActiveAlerts();
      const crossTenantAlert = alerts.find(
        (a) => a.alertType === AlertType.CROSS_TENANT_SPIKE,
      );
  ...
  describe('alert management', () => {
    ...
    it('should acknowledge alerts', async () => {
      ...
      const alerts = alertEngine.getActiveAlerts();
      if (alerts.length > 0) {
        const success = alertEngine.acknowledgeAlert(
          alerts[0].id,
          'analyst-1',
        );
  ...
  describe('alert filtering', () => {
    it('should filter alerts by severity', async () => {
      ...
      highSeverity.forEach((alert) => {
        expect([AlertSeverity.HIGH, AlertSeverity.CRITICAL]).toContain(
          alert.severity,
        );
      });
    });

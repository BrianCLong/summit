...
exports.checkAnalysisCompliance = checkAnalysisCompliance;
exports.checkIndicatorCompliance = checkIndicatorCompliance;
exports.logComplianceCheck = logComplianceCheck;
 ...
 */
function checkAnalysisCompliance(request, requestor, purpose) {
    const violations = [];
    ...
    for (const pattern of PROHIBITED_PATTERNS) {
        if (pattern.test(purpose)) {
            violations.push({
                ...
                remediation: 'Remove prohibited content. Analysis must focus on legitimate early warning, risk assessment, or humanitarian purposes.',
            });
        }
    }
    // Check for sensitive indicators without proper justification
    if (request.indicatorTypes) {
        const sensitiveRequested = request.indicatorTypes.filter((type) => SENSITIVE_INDICATORS.has(type));
        if (sensitiveRequested.length > 0 && purpose.length < 50) {
            violations.push({
                ...
                remediation: 'Provide detailed justification (minimum 50 characters) for analyzing sensitive indicators.',
            });
        }
    }
    ...
    if (request.countries &&
        ...
        violations.push({
            ...
            remediation: 'For analyses covering >20 countries, specify comparative analysis purpose or reduce scope.',
        });
    }
 ...
 */
function checkIndicatorCompliance(indicator) {
    const violations = [];
    const timestamp = new Date();
    ...
    for (const pattern of piiPatterns) {
        if (pattern.test(metadata)) {
            violations.push({
                ...
                remediation: 'Remove all personally identifiable information from indicator data.',
            });
        }
    }
    ...
    if (indicator.type === 'HUMANITARIAN_CRISIS') {
        const crisis = indicator;
        if (crisis.internationalResponseNeeded && !crisis.metadata.responders) {
            violations.push({
                category: 'ETHICS',
                severity: 'MEDIUM',
                description: 'Humanitarian crisis flagged without identifying response organizations',
                remediation: 'Specify which humanitarian organizations should be notified.',
            });
        }
    }
    if (indicator.type === 'NUCLEAR_CAPABILITY') {
        const nuclear = indicator;
        if (nuclear.estimatedTimeToBreakoutMonths !== null &&
            !indicator.metadata.verificationSource) {
            violations.push({
                category: 'ETHICS',
                severity: 'HIGH',
                description: 'Nuclear breakout estimate provided without verification source',
                remediation: 'Cite IAEA reports or other authoritative verification sources.',
            });
        }
    }
    return {
 ...
 */
function logComplianceCheck(log) {
...
function validatePurpose(purpose) {
    ...
    if (!matchedCategory) {
        concerns.push('Purpose does not clearly match legitimate use case categories');
    }

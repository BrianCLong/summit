...
export function checkAnalysisCompliance(
  ...
  if (request.countries && request.countries.length > 20 && !purpose.includes("comparative")) {
    violations.push({
      category: "OPERATIONAL",
      severity: "LOW",
 ...
 */
export function checkIndicatorCompliance(indicator: GeopoliticalIndicator): ComplianceCheckResult {
  const violations: ComplianceViolation[] = [];
  const timestamp = new Date();
  ...
  if (indicator.type === "NUCLEAR_CAPABILITY") {
    ...
    if (nuclear.estimatedTimeToBreakoutMonths !== null && !indicator.metadata.verificationSource) {
      violations.push({
        category: "ETHICS",
        severity: "HIGH",
 ...
 */
export function logComplianceCheck(log: ComplianceAuditLog): void {

...
exports.validityOverlaps = validityOverlaps;
exports.isCurrent = isCurrent;
exports.isBitemporallyValid = isBitemporallyValid;
exports.filterEntitiesAsOf = filterEntitiesAsOf;
exports.filterEdgesAsOf = filterEdgesAsOf;
exports.filterEntitiesBitemporal = filterEntitiesBitemporal;
exports.buildGraphSnapshotAtTime = buildGraphSnapshotAtTime;
exports.buildBitemporalGraphSnapshot = buildBitemporalGraphSnapshot;
exports.getEntityVersions = getEntityVersions;
exports.getLatestVersionAt = getLatestVersionAt;
exports.getValidityPeriod = getValidityPeriod;
exports.getValidityDuration = getValidityDuration;
// -----------------------------------------------------------------------------
// Validity Helpers
// -----------------------------------------------------------------------------
/**
 ...
 * ```
 */
function isValidAt(temporal, at) {
 ...
 * ```
 */
function isBitemporallyValid(obj, validAt, recordedAt) {
 ...
 * ```
 */
function filterEntitiesAsOf(entities, at) {
 ...
 */
function filterEdgesAsOf(edges, at) {
 ...
 * @returns Filtered array of entities
 */
function filterEntitiesBitemporal(entities, validAt, recordedAt) {
 ...
 * ```
 */
function buildGraphSnapshotAtTime(nodes, edges, at) {
    const checkDate = typeof at === 'string' ? new Date(at) : at;
    ...
    const validEdges = filteredEdges.filter((edge) => validNodeIds.has(edge.fromId) && validNodeIds.has(edge.toId));
    return {
        asOf: checkDate,
        nodes: filteredNodes,
        edges: validEdges,
 ...
 */
function buildBitemporalGraphSnapshot(nodes, edges, validAt, recordedAt) {
    ...
    const filteredEdges = filterEntitiesBitemporal(edges, validDate, recordedDate);
    const validNodeIds = new Set(filteredNodes.map((n) => n.id));
    const validEdges = filteredEdges.filter((edge) => validNodeIds.has(edge.fromId) && validNodeIds.has(edge.toId));
    return {
        asOf: validDate,
 ...
 */
function getEntityVersions(entities) {
 ...
 */
function getLatestVersionAt(entities, at) {
 ...
 */
function getValidityDuration(entity) {

...
exports.FederatedTrainer = void 0;
 ...
 */
class FederatedTrainer extends eventemitter3_1.default {
    logger;
    config;
    currentRound = null;
    rounds = [];
    ...
    constructor(config, logger) {
        super();
        this.config = config;
        this.logger = logger || (0, pino_1.pino)({ name: 'FederatedTrainer' });
    }
    /**
     * Initialize federated learning with global model
     */
    async initialize(initialModel) {
     ...
     */
    async startRound(availableNodes) {
        ...
        if (participants.length < this.config.minParticipants) {
            throw new Error(`Insufficient participants: ${participants.length} < ${this.config.minParticipants}`);
        }
        ...
        this.logger.info({ roundNumber, participants: participants.length }, 'Started federated training round');
     ...
     */
    async completeRound(roundNumber) {
     ...
     */
    selectParticipants(availableNodes) {
        const numParticipants = Math.min(availableNodes.length, Math.max(this.config.minParticipants, Math.floor(availableNodes.length * 0.5)));
     ...
     */
    resourceAwareSelection(nodes, count) {
     ...
     */
    performanceBasedSelection(nodes, count) {
...
exports.FederatedTrainer = FederatedTrainer;

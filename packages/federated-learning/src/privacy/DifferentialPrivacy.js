...
class DifferentialPrivacy {
    logger;
    privacyBudget;
    constructor(epsilon, delta, logger) {
        this.logger = logger || (0, pino_1.pino)({ name: 'DifferentialPrivacy' });
        this.privacyBudget = {
            epsilon,
            delta,
            spent: 0,
     ...
     */
    addGaussianNoise(data, sensitivity, epsilon, delta) {
     ...
     */
    addLaplaceNoise(data, sensitivity, epsilon) {
     ...
     */
    privateSumAggregation(values, sensitivity, epsilon, delta) {
        if (values.length === 0) {
            return [];
        }
        // Sum all values
     ...
     */
    privateAverageAggregation(values, sensitivity, epsilon, delta) {
     ...
     */
    async secureAggregation(localUpdates, threshold) {
        if (localUpdates.size < threshold) {
            throw new Error('Insufficient participants for secure aggregation');
        }
        this.logger.info({ participants: localUpdates.size, threshold }, 'Performing secure aggregation');
        ...
        const aggregated = allValues[0].map((_, i) => allValues.reduce((sum, val) => sum + val[i], 0) / allValues.length);
        return aggregated;
    }
    /**
     * Calculate Gaussian noise sigma
     */
    calculateGaussianNoiseSigma(sensitivity, epsilon, delta) {
     ...
     */
    calculateComposedPrivacy(operations) {
     ...
     */
    calculateAdvancedComposition(singleStepEpsilon, singleStepDelta, numSteps, targetDelta) {

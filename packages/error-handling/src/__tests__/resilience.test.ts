...
import { TimeoutError, CircuitBreakerError } from "../errors";
...
describe("Resilience Patterns", () => {
  ...
  describe("Circuit Breaker", () => {
    ...
    it("should open circuit after threshold failures", async () => {
      ...
      await expect(executeWithCircuitBreaker("test-service", fn)).rejects.toThrow(
        CircuitBreakerError
      );
  ...
  describe("Retry Logic", () => {
    ...
    it("should respect maxRetries", async () => {
      ...
      await expect(executeWithRetry(fn, { ...RetryPolicies.quick, maxRetries: 2 })).rejects.toThrow(
        "Always fails"
      );
  ...
  describe("Timeout", () => {
    it("should timeout slow operations", async () => {
      ...
      await expect(executeWithTimeout(slowFn, 100, "slowOperation")).rejects.toThrow(TimeoutError);
    });
  ...
  describe("Graceful Degradation", () => {
    it("should return fallback on error", async () => {
      ...
      const result = await withGracefulDegradation(fn, "fallback-value", {
        serviceName: "test",
        operation: "test",
      });
  ...
  describe("Combined Resilience", () => {
    ...
    it("should open circuit after repeated failures", async () => {
      ...
      await expect(executeWithResilience(config)).rejects.toThrow(CircuitBreakerError);
    });
  });

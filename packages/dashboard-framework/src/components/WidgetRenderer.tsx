...
export function WidgetRenderer({ widget, className = "" }: WidgetRendererProps) {
  const { editMode, selectedWidgets, selectWidget, deselectWidget, deleteWidget } =
    useDashboardStore();
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ flex: 1, fontWeight: 600, fontSize: "14px" }}>{widget.title}</div>
        {editMode && (
...
function ChartWidget({ config }: { config: any }) {
  return (
    <div style={{ padding: "16px", textAlign: "center", color: "#6b7280" }}>
      ...
      <div style={{ fontSize: "12px", marginTop: "4px" }}>Type: {config.chartType || "bar"}</div>
    </div>
  );
}
...
function MetricWidget({ config }: { config: any }) {
  return (
    <div style={{ padding: "24px", textAlign: "center" }}>
      ...
      <div style={{ color: "#6b7280", fontSize: "14px" }}>{config.label || "Metric"}</div>
    </div>
  );
}
...
function DefaultWidget({ type }: { type: string }) {
  return (
    <div style={{ padding: "16px", textAlign: "center", color: "#ef4444" }}>
      ...
      <div style={{ fontSize: "12px", marginTop: "4px" }}>Type: {type}</div>
    </div>
  );
}

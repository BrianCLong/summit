...
exports.SourceDashboard = void 0;
...
const SourceDashboard = ({ dashboard, onScheduleDebrief, onViewNetwork, onRecordActivity, onEditSource, }) => {
    const daysUntilContact = dashboard.nextScheduledContact
        ? Math.ceil((new Date(dashboard.nextScheduledContact).getTime() - Date.now()) /
            (1000 * 60 * 60 * 24))
        : null;
    const daysSinceContact = dashboard.lastContact
        ? Math.floor((Date.now() - new Date(dashboard.lastContact).getTime()) /
            (1000 * 60 * 60 * 24))
        : null;
    return (<div className="source-dashboard">
      {/* Header */}
      <div className="dashboard-header">
        <div className="header-main">
          <h1 className="cryptonym">{dashboard.sourceCryptonym}</h1>
          <span className="status-badge" style={{ backgroundColor: getStatusColor(dashboard.status) }}>
            {dashboard.status}
          </span>
        </div>
      ...
      <div className="metrics-grid">
        <div className="metric-card">
          ...
          <div className="metric-value" style={{ color: getCredibilityColor(dashboard.credibilityScore) }}>
        ...
        <div className="metric-card">
          ...
          <div className="metric-value risk" style={{ color: getRiskColor(dashboard.riskLevel) }}>
            {constants_js_1.RISK_LEVELS[dashboard.riskLevel]?.label || dashboard.riskLevel}
          </div>
        </div>
        ...
        <div className="metric-card">
          ...
          {daysSinceContact !== null && (<div className={`metric-sublabel ${daysSinceContact > 30 ? 'warning' : ''}`}>
        ...
        <div className="metric-card">
          ...
          {daysUntilContact !== null && daysUntilContact <= 7 && (<div className="metric-sublabel upcoming">In {daysUntilContact} days</div>)}
        </div>
      </div>

      {/* Alerts Section */}
      {dashboard.alerts.length > 0 && (<div className="alerts-section">
          <h3>Active Alerts ({dashboard.activeIndicators})</h3>
          <div className="alerts-list">
            {dashboard.alerts.map((alert) => (<div key={alert.id} className="alert-item" style={{ borderLeftColor: getRiskColor(alert.severity) }}>
      ...
      <div className="dashboard-columns">
        ...
        <div className="dashboard-column">
          ...
          <div className="dashboard-card">
            ...
            {dashboard.upcomingDebriefs.length > 0 ? (<ul className="debrief-list">
                {dashboard.upcomingDebriefs.map((debrief) => (<li key={debrief.id} className="debrief-item">
                    <span className="debrief-type">{debrief.type}</span>
                    <span className="debrief-date">
                      {formatDateTime(debrief.scheduledAt)}
                    </span>
                  </li>))}
              </ul>) : (<div className="empty-state">No upcoming debriefs scheduled</div>)}
          </div>

          {/* Recent Intelligence */}
          <div className="dashboard-card">
            <h3>Recent Intelligence</h3>
            {dashboard.recentIntelligence.length > 0 ? (<ul className="intel-list">
                {dashboard.recentIntelligence.map((intel) => (<li key={intel.id} className="intel-item">
              ...
              </ul>) : (<div className="empty-state">No recent intelligence</div>)}
          </div>
        </div>
        ...
        <div className="dashboard-column">
          ...
          <div className="dashboard-card">
            ...
            {dashboard.recentActivities.length > 0 ? (<ul className="activity-list">
                {dashboard.recentActivities.slice(0, 5).map((activity) => (<li key={activity.id} className="activity-item">
              ...
              </ul>) : (<div className="empty-state">No recent activity recorded</div>)}
          </div>
        </div>
      </div>
    </div>);
};
exports.SourceDashboard = SourceDashboard;

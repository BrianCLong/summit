...
describe("Evidence store determinism", () => {
  it("creates stable artifact names and hashes", () => {
    ...
    const first = store.record(
      ...
      { a: 1 },
      { output: "value" },
      { allowed: true, reason: "ok", policyVersion: "1.0.0" }
    );
    ...
    const ledger = fs
      .readFileSync(path.join(base, "run-123", "evidence", "evidence.ndjson"), "utf-8")
      .trim()
      .split("\n");

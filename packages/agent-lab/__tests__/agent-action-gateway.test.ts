...
describe('Agent Action Gateway integration', () => {
  ...
  it('allows happy path, emits audit, and redacts sensitive outputs', async () => {
    ...
    expect(last.inputs.secret).toContain('[redacted-secret]');
    const rawPath = path.join(evidence.runPath, 'raw', '0001-echo_tool.txt');
    const rawContent = fs.readFileSync(rawPath, 'utf-8');
    expect(rawContent).toContain('[redacted-secret]');
    expect(rawContent).not.toMatch(/API key/i);

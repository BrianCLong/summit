...
exports.judgeRun = void 0;
const judgeRun = (summary) => {
    const notes = [];
    ...
    if (deniedSteps > 0)
        notes.push('Some steps were denied by policy');
    if (errors > 0)
        notes.push('Errors occurred during execution');
    if (expectedArtifacts === 0)
        notes.push('No explicit expectations; evidence completeness assumed.');

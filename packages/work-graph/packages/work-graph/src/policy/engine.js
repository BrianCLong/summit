...
class PolicyEngine {
    ...
    async loadPolicies() {
        const customPolicies = await this.graphStore.getNodes({ type: 'policy', status: 'active' });
    ...
    async checkNode(node) {
        const applicablePolicies = this.policies.filter(p => p.scope.includes(node.type) || p.scope.includes('all'));
        const results = [];
        let blocked = false;
    ...
    evaluatePolicy(policy, node) {
        ...
        for (const condition of policy.conditions) {
            const nodeValue = node[condition.field];

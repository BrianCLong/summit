...
exports.MermaidGenerators = void 0;
exports.generateGanttChart = generateGanttChart;
exports.generateRoadmapGantt = generateRoadmapGantt;
exports.generateSprintGantt = generateSprintGantt;
exports.generateDependencyGraph = generateDependencyGraph;
exports.generateEpicBreakdown = generateEpicBreakdown;
exports.generateTimeline = generateTimeline;
exports.generateMilestoneTimeline = generateMilestoneTimeline;
exports.generatePriorityQuadrant = generatePriorityQuadrant;
exports.generateKanbanBoard = generateKanbanBoard;
exports.generateAgentWorkload = generateAgentWorkload;
exports.generateCommitmentJourney = generateCommitmentJourney;
exports.generateIntentFlow = generateIntentFlow;
// ============================================
// Helper Functions
// ============================================
function getMilestoneSection(status) {
    if (status === 'achieved') {
        return 'Achieved';
    }
    if (status === 'at_risk') {
...
function getJourneyScore(progress, threshold, lowerThreshold) {
    if (progress > threshold) {
        return 5;
    }
    if (lowerThreshold !== undefined && progress > lowerThreshold) {
        return 4;
    }
...
// ============================================
function generateGanttChart(options) {
...
// ============================================
function generateRoadmapGantt(roadmap, epics, milestones, tickets) {
    const tasks = [];
    // Add milestones
...
// ============================================
function generateSprintGantt(sprint, tickets) {
    const tasks = [];
...
// ============================================
function generateDependencyGraph(nodes, edges, options = {}) {
    const { direction = 'TB', showStatus = true, showPriority = true } = options;
    const lines = [`flowchart ${direction}`];
    // Build node map for quick lookup
    const nodeMap = new Map(nodes.map((n) => [n.id, n]));
    // Filter relevant edges
    const relevantEdges = edges.filter((e) => (e.type === 'depends_on' || e.type === 'blocks' || e.type === 'implements') &&
...
// ============================================
function generateEpicBreakdown(epic, tickets, edges) {
    ...
    const ticketEdges = edges.filter((e) => (e.type === 'depends_on' || e.type === 'blocks') &&
...
// ============================================
function generateTimeline(events, options = {}) {
...
// ============================================
function generateMilestoneTimeline(milestones, roadmap) {
...
// ============================================
function generatePriorityQuadrant(tickets) {
    const lines = [
        'quadrantChart',
...
// ============================================
function generateKanbanBoard(board, tickets) {
    ...
    for (const column of board.columns.sort((a, b) => a.position - b.position)) {
        ...
        if (columnTickets.length === 0) {
            ...
            for (const ticket of columnTickets.slice(0, 8)) {
                ...
                lines.push(`        ${ticketId}("${icon} ${escapeLabel(truncate(ticket.title, 30))}${assignee}")`);
...
// ============================================
function generateAgentWorkload(agents, tickets) {
    const lines = [
        'pie showData',
...
// ============================================
function generateCommitmentJourney(commitment, tickets, _milestones) {
    const lines = [
        'journey',
...
// ============================================
function generateIntentFlow(intent, epics, tickets, edges) {
    const lines = ['flowchart TB'];
    // Intent node
...
function mapTicketStatusToGantt(status) {
    const map = {
        done: 'done',
        in_progress: 'active',
        review: 'active',
...
function mapTicketStatusToSection(status) {
    const map = {
        done: 'Done',
...
function mapTicketStatusToColumn(status) {
    const map = {
        backlog: 'Backlog',
        ready: 'Ready',
        in_progress: 'In Progress',
...
function estimateEpicDuration(epic, tickets) {
    if (!tickets) {
        return '14d';
    }

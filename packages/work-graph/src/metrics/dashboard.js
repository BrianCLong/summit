...
class MetricsDashboard {
    ...
    async getMetrics(forceRefresh = false) {
        ...
        const [tickets, epics, commitments, agents] = await Promise.all([
            this.graphStore.getNodes({ type: 'ticket' }),
            this.graphStore.getNodes({ type: 'epic' }),
            this.graphStore.getNodes({ type: 'commitment' }),
            this.graphStore.getNodes({ type: 'agent' }),
        ]);
        const metrics = {
            timestamp: new Date(),
            velocity: this.calculateVelocity(tickets),
            cycleTime: this.calculateCycleTime(tickets),
            throughput: this.calculateThroughput(tickets, agents),
            wip: this.calculateWIP(tickets),
            quality: await this.calculateQuality(),
            agents: this.calculateAgentMetrics(agents, tickets),
            commitments: this.calculateCommitmentMetrics(commitments),
     ...
     */
    async generateSprintReport(sprintStart, sprintEnd) {
    ...
    calculateVelocity(tickets) {
        ...
        const completedThisSprint = tickets.filter((t) => t.status === 'done' && t.updatedAt >= currentSprintStart);
        const completedLastSprint = tickets.filter((t) => t.status === 'done' && t.updatedAt >= lastSprintStart && t.updatedAt < currentSprintStart);
        ...
        let trend = 'stable';
        if (currentVelocity > lastVelocity * 1.1)
            trend = 'improving';
        if (currentVelocity < lastVelocity * 0.9)
            trend = 'declining';
        return {
    ...
    calculateCycleTime(tickets) {
        ...
        completed.forEach((t) => {
            ...
            if (!byPriority[t.priority])
                byPriority[t.priority] = ct;
            else
                byPriority[t.priority] = (byPriority[t.priority] + ct) / 2;
            if (!byType[t.ticketType])
                byType[t.ticketType] = ct;
            else
                byType[t.ticketType] = (byType[t.ticketType] + ct) / 2;
        });
        return {
    ...
    calculateThroughput(tickets, agents) {
        const now = new Date();
    ...
    calculateWIP(tickets) {
        const inProgress = tickets.filter((t) => ['in_progress', 'agent_assigned', 'in_review'].includes(t.status));
        const byStatus = {};
        const byArea = {};
    ...
    calculateAgentMetrics(agents, tickets) {
        ...
        const now = new Date();
        const dayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);
        const completedToday = tickets.filter((t) => t.status === 'done' && t.assigneeType === 'agent' && t.updatedAt >= dayAgo).length;
    ...
    calculateCommitmentMetrics(commitments) {
        const now = new Date();
    ...
    calculateHealthScore(metrics) {
        ...
        if (metrics.velocity.trend === 'improving')
            velocityScore = 80;
        ...
        // Quality health
        const qualityScore = Math.round((metrics.quality.prMergeRate * 25 +
            metrics.quality.testCoverage * 25 +
            metrics.quality.securityScanPass * 25 +
            (1 - metrics.quality.bugEscapeRate) * 25) * 100) / 100;
        ...
        // Commitment health
        const commitmentScore = metrics.commitments.total > 0
        ...
        const overall = Math.round((velocityScore * 0.2 + qualityScore * 0.25 + commitmentScore * 0.25 + wipScore * 0.15 + agentScore * 0.15));
        return {
            overall,
            breakdown: {

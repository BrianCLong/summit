...
export function createWorkGraphAPI(graphStore: GraphStore) {
  const resolvers = {
    Query: {
      ...
      nodes: async (
        _: unknown,
        { type, status, limit }: { type?: string; status?: string; limit?: number }
      ) => {
      ...
      epics: (_: unknown, args: { status?: string }) => {
      ...
      intents: (_: unknown, args: { status?: string }) => {
      ...
      boards: (_: unknown, args: { archived?: boolean }) => {
      ...
      roadmaps: (_: unknown, args: { status?: string }) => {
      ...
      milestones: (_: unknown, args: { status?: string }) => {
      ...
      backlog: (_: unknown, _args: { boardId?: string }) => {
        return graphStore.getNodes<Ticket>({ type: 'ticket', status: 'backlog' });
      },
      edges: (_: unknown, args: { sourceId?: string; targetId?: string; type?: string }) =>
        graphStore.getEdges(args),
      healthScore: () => ({
        overall: 75,
        velocity: 80,
        quality: 70,
    ...
    Mutation: {
      ...
      createBoard: (
        _: unknown,
        { input }: { input: { name: string; description?: string; boardType: string; team?: string } }
      ) => {
        const board: Board = {
          id: crypto.randomUUID(),
          type: 'board',
      ...
      createRoadmap: (
        _: unknown,
        { input }: { input: { name: string; description?: string; status?: string; team?: string } }
      ) => {
      ...
      importBacklog: async (
        _: unknown,
        { input }: { input: { content: string; source: string; autoTriage?: boolean } }
      ) => {

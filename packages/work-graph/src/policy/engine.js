"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.PolicyEngine = void 0;
const BUILTIN_POLICIES = [
    { type: 'policy', createdBy: 'system', name: 'PR Size Limit', description: 'PRs should not exceed 500 lines', policyType: 'quality', status: 'active', scope: ['pr'], conditions: [{ field: 'filesChanged', operator: 'greater_than', value: 500 }], actions: [{ type: 'warn', message: 'PR is large' }], exceptions: [], enforcementLevel: 'soft' },
    { type: 'policy', createdBy: 'system', name: 'Security Review', description: 'Security PRs need review', policyType: 'security', status: 'active', scope: ['pr'], conditions: [{ field: 'labels', operator: 'contains', value: 'security' }], actions: [{ type: 'require_approval', message: 'Security review required' }], exceptions: [], enforcementLevel: 'hard' },
    { type: 'policy', createdBy: 'system', name: 'Agent Limits', description: 'Agents cannot do P0/P1', policyType: 'governance', status: 'active', scope: ['ticket'], conditions: [{ field: 'assigneeType', operator: 'equals', value: 'agent' }], actions: [{ type: 'require_approval', message: 'Approval needed' }], exceptions: [], enforcementLevel: 'hard' },
];
class PolicyEngine {
    graphStore;
    policies = [];
    waivers = new Map();
    constructor(graphStore) {
        this.graphStore = graphStore;
        this.loadBuiltinPolicies();
    }
    loadBuiltinPolicies() { this.policies = BUILTIN_POLICIES.map((p, i) => ({ ...p, id: 'builtin-' + i, createdAt: new Date(), updatedAt: new Date() })); }
    async loadPolicies() {
        const custom = await this.graphStore.getNodes({ type: 'policy', status: 'active' });
        this.policies = [...this.policies, ...custom];
    }
    addPolicy(policy) { this.policies.push(policy); }
    removePolicy(id) { const idx = this.policies.findIndex(p => p.id === id); if (idx === -1)
        return false; this.policies.splice(idx, 1); return true; }
    async checkNode(node) {
        const applicable = this.policies.filter(p => p.scope.includes(node.type) || p.scope.includes('all'));
        const results = [];
        let blocked = false;
        for (const policy of applicable) {
            const result = this.evaluatePolicy(policy, node);
            results.push(result);
            if (!result.passed && policy.enforcementLevel === 'hard')
                blocked = true;
        }
        return { passed: !blocked, blocked, results, summary: { total: results.length, passed: results.filter(r => r.passed).length, warned: results.filter(r => !r.passed && r.level === 'soft').length, blocked: results.filter(r => !r.passed && r.level === 'hard').length } };
    }
    evaluatePolicy(policy, node) {
        const violations = [];
        let met = false;
        for (const c of policy.conditions) {
            const v = node[c.field];
            if (this.evalCondition(c, v)) {
                met = true;
                violations.push(c.field + ' ' + c.operator + ' ' + c.value);
            }
        }
        const hasWaiver = this.hasApprovedWaiver(policy.id, node.id);
        return { policyId: policy.id, policyName: policy.name, passed: !met || hasWaiver, level: policy.enforcementLevel, actions: met && !hasWaiver ? policy.actions : [], violations, waiverAvailable: policy.enforcementLevel !== 'hard' };
    }
    evalCondition(c, v) {
        if (c.operator === 'equals')
            return v === c.value;
        if (c.operator === 'not_equals')
            return v !== c.value;
        if (c.operator === 'contains')
            return Array.isArray(v) ? v.includes(c.value) : typeof v === 'string' && v.includes(String(c.value));
        if (c.operator === 'greater_than')
            return typeof v === 'number' && v > c.value;
        if (c.operator === 'less_than')
            return typeof v === 'number' && v < c.value;
        if (c.operator === 'matches')
            return typeof v === 'string' && new RegExp(String(c.value)).test(v);
        return false;
    }
    requestWaiver(policyId, nodeId, reason, requestedBy) {
        const w = { id: crypto.randomUUID(), policyId, nodeId, reason, requestedBy, requestedAt: new Date(), status: 'pending' };
        this.waivers.set(w.id, w);
        return w;
    }
    approveWaiver(waiverId, approvedBy) {
        const w = this.waivers.get(waiverId);
        if (!w)
            return null;
        w.status = 'approved';
        w.approvedBy = approvedBy;
        w.approvedAt = new Date();
        return w;
    }
    denyWaiver(waiverId, approvedBy) {
        const w = this.waivers.get(waiverId);
        if (!w)
            return null;
        w.status = 'denied';
        w.approvedBy = approvedBy;
        w.approvedAt = new Date();
        return w;
    }
    hasApprovedWaiver(policyId, nodeId) {
        for (const w of this.waivers.values())
            if (w.policyId === policyId && w.nodeId === nodeId && w.status === 'approved')
                return true;
        return false;
    }
    getPolicies() { return this.policies; }
    getPendingWaivers() { return Array.from(this.waivers.values()).filter(w => w.status === 'pending'); }
}
exports.PolicyEngine = PolicyEngine;

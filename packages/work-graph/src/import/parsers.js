...
exports.parseRoadmapMarkdown = parseRoadmapMarkdown;
exports.parseWarRoomBacklog = parseWarRoomBacklog;
exports.parseTaskBacklog = parseTaskBacklog;
exports.convertEpicToNode = convertEpicToNode;
exports.convertStoryToTicket = convertStoryToTicket;
exports.convertTaskToTicket = convertTaskToTicket;
exports.convertBacklogItemToTicket = convertBacklogItemToTicket;
// ============================================
// ROADMAP.md Parser
// ============================================
function parseRoadmapMarkdown(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (currentEpic && !currentStory) {
            ...
            if (typeMatch) {
                currentEpic.epicType = typeMatch[1];
                continue;
            }
...
// ============================================
function parseWarRoomBacklog(content) {
    const tasks = [];
    const lines = content.split('\n');
    let currentPhase = '';
    let currentWeek = 0;
...
// ============================================
function parseTaskBacklog(content) {
    const items = [];
    const lines = content.split('\n');
    // Find table start
    let inTable = false;
...
function getTargetQuarter(horizon) {
    if (horizon === 'H1') {
        return 'Q1';
    }
    if (horizon === 'H2') {
...
// ============================================
function convertEpicToNode(parsed) {
    return {
        id: generateUUID(),
        type: 'epic',
        title: parsed.title,
...
function convertStoryToTicket(parsed, _sprintId) {
    const estimate = parsed.labels.estimate ? parseInt(parsed.labels.estimate.replace('d', ''), 10) : 3;
    const priority = (parsed.labels.priority?.toUpperCase() || 'P2');
    return {
        id: generateUUID(),
        type: 'ticket',
        title: parsed.title,
...
function convertTaskToTicket(parsed) {
    return {
        id: generateUUID(),
        type: 'ticket',
        title: parsed.title,
...
function convertBacklogItemToTicket(parsed) {
    return {
        id: generateUUID(),
        type: 'ticket',
        title: parsed.title,

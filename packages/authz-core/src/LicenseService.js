...
const logger = (0, pino_1.default)({ name: 'license-service' });
// ============================================================================
// Service
// ============================================================================
class LicenseService {
    ...
    async createLicense(input) {
        try {
            ...
            const existing = await this.db.query(`SELECT license_id FROM licenses
         WHERE tenant_id = $1 AND license_key = $2`, [input.tenantId, input.licenseKey]);
            if (existing.rows.length > 0) {
                throw new types_js_1.LicenseError(`License ${input.licenseKey} already exists for tenant ${input.tenantId}`, 'LICENSE_ALREADY_EXISTS', 400);
            }
            // Insert license
            const result = await this.db.query(`INSERT INTO licenses (
        ...
        RETURNING *`, [
            ...
            logger.info({ licenseId: license.licenseId, licenseKey: input.licenseKey, tenantId: input.tenantId }, 'License created');
            return license;
        }
        catch (error) {
     ...
     */
    async updateLicenseStatus(licenseId, status, updatedBy) {
        try {
            const result = await this.db.query(`UPDATE licenses
         SET status = $1, updated_by = $2, updated_at = NOW()
         WHERE license_id = $3`, [status, updatedBy, licenseId]);
            if (result.rowCount === 0) {
                throw new types_js_1.LicenseError('License not found', 'LICENSE_NOT_FOUND', 404);
            }
     ...
     */
    async assignLicense(input) {
        const client = await this.db.connect();
        try {
            await client.query('BEGIN');
            // Validate license exists and is active
            const licenseResult = await client.query(`SELECT status, expiry_date FROM licenses WHERE license_id = $1`, [input.licenseId]);
            if (licenseResult.rows.length === 0) {
                throw new types_js_1.LicenseError('License not found', 'LICENSE_NOT_FOUND', 404);
            }
            const license = licenseResult.rows[0];
            if (license.status !== 'ACTIVE') {
                throw new types_js_1.LicenseError(`Cannot assign license with status ${license.status}`, 'LICENSE_NOT_ACTIVE', 400);
            }
            if (license.expiry_date && new Date(license.expiry_date) < new Date()) {
                throw new types_js_1.LicenseError('License has expired', 'LICENSE_EXPIRED', 400);
            }
            // Check for existing active assignment
            const existingAssignment = await client.query(`SELECT assignment_id FROM data_license_assignments
         WHERE resource_type = $1 AND resource_id = $2 AND tenant_id = $3
           AND assignment_status = 'ACTIVE'`, [input.resourceType, input.resourceId, input.tenantId]);
            // Revoke existing assignment if present
            if (existingAssignment.rows.length > 0) {
                await client.query(`UPDATE data_license_assignments
           SET assignment_status = 'REVOKED',
               revoked_by = $1,
               revoked_at = NOW(),
               revocation_reason = 'Superseded by new license assignment'
           WHERE assignment_id = $2`, [input.assignedBy, existingAssignment.rows[0].assignment_id]);
            }
            // Create new assignment
            const result = await client.query(`INSERT INTO data_license_assignments (
     ...
     */
    async revokeLicenseAssignment(assignmentId, revokedBy, reason) {
        try {
            const result = await this.db.query(`UPDATE data_license_assignments
         SET assignment_status = 'REVOKED',
             revoked_by = $1,
             revoked_at = NOW(),
             revocation_reason = $2
         WHERE assignment_id = $3 AND assignment_status = 'ACTIVE'`, [revokedBy, reason, assignmentId]);
            if (result.rowCount === 0) {
                throw new types_js_1.LicenseError('Active assignment not found', 'ASSIGNMENT_NOT_FOUND', 404);
            }
     ...
     */
    async validateLicense(tenantId, resourceType, resourceId, action) {
        try {
            ...
            const permitted = await this.db.query(`SELECT is_action_permitted_by_license($1, $2) as is_permitted`, [licenseId, action]);
            ...
            const licenseResult = await this.db.query(`SELECT * FROM licenses WHERE license_id = $1 AND status = 'ACTIVE'`, [licenseId]);
            if (licenseResult.rows.length === 0) {
     ...
     * Get active license for resource
     */
    async getActiveLicenseForResource(tenantId, resourceType, resourceId) {
        try {
            const result = await this.db.query(`SELECT get_active_license_for_resource($1, $2, $3) as license_id`, [tenantId, resourceType, resourceId]);
     ...
     */
    async logEnforcement(input, decision, reason, conditions, applicableLicenses) {
        try {
            await this.db.query(`INSERT INTO license_enforcement_log (
     ...
     */
    async acceptTOS(input) {
        try {
            const result = await this.db.query(`INSERT INTO tos_acceptances (
        ...
        RETURNING acceptance_id`, [
            ...
            logger.info({ acceptanceId, userId: input.userId, tosVersion: input.tosVersion, tosType: input.tosType }, 'TOS acceptance recorded');
            return acceptanceId;
        }
        catch (error) {
     ...
     */
    async hasUserAcceptedTOS(userId, tosVersion, tosType) {
        try {
            const result = await this.db.query(`SELECT has_user_accepted_tos($1, $2, $3) as has_accepted`, [userId, tosVersion, tosType]);
     ...
     */
    async areLicensesCompatible(licenseAId, licenseBId) {
        try {
            const result = await this.db.query(`SELECT are_licenses_compatible($1, $2) as compatible`, [licenseAId, licenseBId]);
            const compatible = result.rows[0]?.compatible || false;
            if (!compatible) {
                // Fetch compatibility details if available
                const detailsResult = await this.db.query(`SELECT compatibility_level, compatibility_reason
           FROM license_compatibility_matrix
           WHERE (license_a_id = $1 AND license_b_id = $2)
              OR (license_a_id = $2 AND license_b_id = $1)`, [licenseAId, licenseBId]);
     ...
     */
    async recordLicenseLineage(tenantId, sourceAssignmentId, sourceResourceType, sourceResourceId, sourceLicenseId, derivedResourceType, derivedResourceId, transformationType, createdBy, lineageDepth = 1) {
        try {
            const result = await this.db.query(`INSERT INTO license_lineage (
        ...
        RETURNING lineage_id`, [
            ...
            logger.info({ lineageId, sourceResourceId, derivedResourceId, transformationType }, 'License lineage recorded');
            return lineageId;
        }
        catch (error) {

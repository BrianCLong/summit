...
export class CubeManager {
  ...
  async createCube(definition: CubeDefinition): Promise<void> {
    const measureCols = definition.measures
      .map(
        (m) =>
          `${m}_sum NUMERIC, ${m}_count INTEGER, ${m}_avg NUMERIC, ${m}_min NUMERIC, ${m}_max NUMERIC`
      )
      .join(", ");
  ...
  async buildCube(
    ...
    const aggregations = definition.measures
      .map(
        (m) =>
          `SUM(${m}) as ${m}_sum, COUNT(${m}) as ${m}_count, AVG(${m}) as ${m}_avg, MIN(${m}) as ${m}_min, MAX(${m}) as ${m}_max`
      )
      .join(", ");

    const insertCols = [
      ...definition.dimensions,
      ...definition.measures.flatMap((m) => [
        `${m}_sum`,
        `${m}_count`,
        `${m}_avg`,
        `${m}_min`,
  ...
  async slice(cubeName: string, dimension: string, value: any): Promise<any[]> {
    const result = await this.pool.query(`SELECT * FROM cube_${cubeName} WHERE ${dimension} = $1`, [
      value,
    ]);
    return result.rows;
  }

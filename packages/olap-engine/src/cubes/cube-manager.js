...
exports.CubeManager = void 0;
class CubeManager {
    pool;
    constructor(pool) {
        this.pool = pool;
    }
    async createCube(definition) {
    ...
    async buildCube(cubeName, sourceTable, definition) {
    ...
    async queryCube(cubeName, dimensions, measures, filters) {
    ...
    async drillDown(cubeName, fromDimension, toDimension) {
        const result = await this.pool.query(`
    ...
    async rollUp(cubeName, dimensions, measures) {
    ...
    async slice(cubeName, dimension, value) {
        const result = await this.pool.query(`SELECT * FROM cube_${cubeName} WHERE ${dimension} = $1`, [value]);
        return result.rows;
    }
    async dice(cubeName, filters) {

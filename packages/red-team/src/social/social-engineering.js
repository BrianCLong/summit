...
exports.SocialEngineeringEngine = void 0;
...
class SocialEngineeringEngine {
     ...
     */
    generatePhishingCampaign(name, targets, options = {}) {
        ...
        return {
            id: this.generateId(),
            type: customization === 'highly-targeted' ? types_1.SocialEngineeringType.SPEAR_PHISHING : types_1.SocialEngineeringType.PHISHING,
            name,
     ...
     */
    generateSpearPhishingCampaign(name, targets, intelligence) {
        ...
        return {
            id: this.generateId(),
            type: types_1.SocialEngineeringType.SPEAR_PHISHING,
            name,
     ...
     */
    generateVishingCampaign(name, targets, scenario) {
        const script = this.generateVishingScript(scenario);
        return {
            id: this.generateId(),
            type: types_1.SocialEngineeringType.VISHING,
            name,
     ...
     */
    generatePretextingScenario(targetRole, objective) {
        const scenarios = {
            'it-admin': {
     ...
     */
    generateWateringHoleAttack(targetOrganization, identifiedSites) {
        return {
     ...
     */
    analyzeCampaignResults(campaign) {
    ...
    generatePhishingContent(theme, urgency, _customization) {
        const templates = {
            'it-support': {
    ...
    generateSpearPhishingContent(targets, intelligence) {
    ...
    generateVishingScript(scenario) {
        const scripts = {
    ...
    generatePretextScript(persona, objective) {
        return [
    ...
    generateSchedule(targetCount, staggered = false) {
        const now = new Date();
...
exports.SocialEngineeringEngine = SocialEngineeringEngine;

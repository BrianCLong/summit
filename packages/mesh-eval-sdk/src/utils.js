...
exports.determinePassFail = determinePassFail;
...
exports.groupByScenarioType = groupByScenarioType;
...
exports.isCompleteResult = isCompleteResult;
// ============================================================================
// ID Generation
// ============================================================================
/**
 ...
 */
function generateScenarioId(type, sequence) {
    const prefix = 'sc';
 ...
 */
function generateFindingId(runId, scenarioId, index) {
 ...
 */
function determinePassFail(score, threshold = 0.7) {
    if (score >= threshold)
        return 'pass';
...
function aggregateScores(results) {
    ...
    const safetyViolations = results.reduce((acc, r) => acc + r.findings.filter((f) => f.type === 'safety_violation').length, 0);
    const policyDenials = results.reduce((acc, r) => acc + r.policyEvents.filter((e) => e.action === 'deny').length, 0);
 ...
 */
function compareToBaseline(run, baseline) {
 ...
 */
function filterByMinScore(results, minScore) {
    return results.filter((r) => r.score.overall >= minScore);
}
/**
 * Group results by scenario type
 */
function groupByScenarioType(results, scenarios) {
    const grouped = {};
    for (const result of results) {
 ...
 */
function generateJsonArtifact(run) {

...
import {
  ReplayResult,
  replayWithSanctions as _replayWithSanctions,
  StressProfile as _StressProfile,
  RunTrace as _RunTrace,
...
export class ACSLearner {
  ...
  async learnStrategy(simulationResults: ReplayResult[]): Promise<CounterintelAction[]> {
    ...
    const avgDrift =
      simulationResults.reduce((sum, res) => sum + res.drift, 0) / simulationResults.length;
    if (avgDrift > 0.3) {
    ...
    const avgCostDelta =
      simulationResults.reduce((sum, res) => sum + res.costDelta, 0) / simulationResults.length;

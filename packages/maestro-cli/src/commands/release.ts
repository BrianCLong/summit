 ...
 */
const evaluateReleaseBundle = (content: string): { status: string; blockedReasons: any[] } => {
  try {
...
export class ReleaseCommand {
  ...
  private createPullRequest(
    ...
    try {
      ...
      execSync(`gh pr create --title "${commitMessage}" --body-file "${prBodyFile}"`);
    } catch (error) {
...
export function registerReleaseCommands(program: Command) {
  const releaseCommand = new Command("release").description("Release-related commands");

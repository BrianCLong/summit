...
export function registerDsarCommands(program: Command) {
  const dsarCommand = program.command('dsar')
    .description('Manage Data Subject Access Requests (DSAR)');

  dsarCommand.command('export')
  ...
  dsarCommand.command('delete')
    ...
    .action((options) => {
      console.log(`DSAR Delete: Subject ID - ${options.subject}, Preview - ${options.preview || false}`);

...
export class MessageAnalyzer {
  ...
  private initializePatterns(): void {
    this.entityPatterns.set("PHONE", [/\+?[\d\s\-().]{10,}/g, /\(\d{3}\)\s*\d{3}[\s-]?\d{4}/g]);
  ...
  private initializeDefaultSelectors(): void {
    const defaults: Selector[] = [
      ...
      { id: uuid(), type: "keyword", value: "SIMULATION", priority: 1, active: true },
      {
        id: uuid(),
        type: "regex",
        value: "\\b(ALPHA|BRAVO|CHARLIE)\\s*\\d+\\b",
        priority: 2,
        active: true,
      },
    ];
  ...
  private assessFormality(content: string): number {
    ...
    formalIndicators.forEach((w) => {
      if (contentLower.includes(w)) score += 0.1;
    });
    informalIndicators.forEach((w) => {
      if (contentLower.includes(w)) score -= 0.1;
    });

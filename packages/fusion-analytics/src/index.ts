...
export const FusedEntitySchema = z.object({
  ...
  sources: z.array(
    z.object({ domain: IntelligenceDomainSchema, reportId: z.string(), confidence: z.number() })
  ),
  attributes: z.record(
    z.array(
      z.object({ value: z.any(), source: z.string(), confidence: z.number(), timestamp: z.date() })
    )
  ),
  relationships: z.array(
    z.object({
  ...
  lastUpdated: z.date(),
  conflictingData: z.array(
    z.object({ attribute: z.string(), values: z.array(z.any()), sources: z.array(z.string()) })
  ),
});
...
export class FusionEngine {
   ...
   */
  async resolveEntity(
    rawEntity: IntelReport["entities"][0],
    sourceReport: IntelReport
  ): Promise<{
   ...
   */
  async crossDomainCorrelation(report: IntelReport): Promise<
    Array<{
      reportId: string;
  ...
  query(params: {
    ...
    if (params.timeframe) {
      reports = reports.filter(
        (r) => r.timestamp >= params.timeframe!.start && r.timestamp <= params.timeframe!.end
      );
    }
  ...
  private compareIndicators(
    a: IntelReport["indicators"],
    b: IntelReport["indicators"]
  ): IntelReport["indicators"] {
    return a.filter((ai) => b.some((bi) => ai.type === bi.type && ai.value === bi.value));
  }

  private compareEntities(
    a: IntelReport["entities"],
    b: IntelReport["entities"]
  ): IntelReport["entities"] {
    return a.filter((ae) =>
      b.some((be) => ae.name.toLowerCase() === be.name.toLowerCase() && ae.type === be.type)
    );
  }
  ...
  // Public API
  getEntity(id: string): FusedEntity | undefined {
    return this.entities.get(id);
  }
  getAllEntities(): FusedEntity[] {
  ...
  setDomainWeight(domain: IntelligenceDomain, weight: number): void {
    this.domainWeights.set(domain, weight);
  }
}

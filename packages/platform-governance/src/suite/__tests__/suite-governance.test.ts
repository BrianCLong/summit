...
describe('dependency enforcement', () => {
  it('detects circular dependencies across workspace packages', () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'pkg-a', version: '1.0.0', dependencies: { 'pkg-b': '1.0.0' } }, null, 2),
    );

    fs.writeFileSync(
      path.join(packagesDir, 'pkg-b', 'package.json'),
      JSON.stringify({ name: 'pkg-b', version: '1.0.0', dependencies: { 'pkg-a': '1.0.0' } }, null, 2),
    );
...
describe('entitlements', () => {
  it('enforces limits and detects anomalies', () => {
    ...
    const anomalies = detectUsageAnomalies(entitlement, [...usage, { tenantId: 't-1', feature: 'command-palette', count: 300, occurredAt: new Date() }]);
    expect(anomalies[0].actual).toBeGreaterThan(entitlement.limit);
  });

  it('calculates proration with unused credit and upgrades', () => {
    const credit = calculateProratedCredit({ oldLimit: 100, newLimit: 200, daysUsed: 10, daysInPeriod: 30 });

...
const transitions = await governance.getLifecycleEngine().getAvailableTransitions(document.id);

// Request a state transition
const result = await governance.getLifecycleEngine().requestTransition(
  {
...
const riskScore = await governance
  .getRiskScoringService()
  .calculateRiskScore({ document_id: document.id });

// Run compliance check
const compliance = await governance.getComplianceService().checkCompliance(document.id);
```
...
const lifecycle = governance.getLifecycleEngine().getLifecycleDefinition("Contract");
...
if (result.requires_approval) {
  ...
  const approvalResult = await governance
    .getLifecycleEngine()
...
```typescript
const compliance = await governance.getComplianceService().checkCompliance(documentId);
...
// Record sign-off
await governance.getProvenanceService().recordSignOff(documentId, "approver-456", "CISO");
```

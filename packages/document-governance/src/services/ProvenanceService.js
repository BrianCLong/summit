...
class ProvenanceService {
     ...
     */
    async createProvenance(documentId, metadata) {
     ...
     */
    async getProvenance(documentId) {
        const session = this.driver.session();
        try {
     ...
     */
    async addAIAssistSession(documentId, session) {
     ...
     * Record human review
     */
    async recordHumanReview(documentId, reviewerId, reviewerRole, notes) {
        const session = this.driver.session();
        try {
            await session.run(`
        MATCH (p:Provenance {document_id: $documentId})
     ...
     * Record sign-off
     */
    async recordSignOff(documentId, signOffBy, signOffRole) {
        const session = this.driver.session();
        try {
            await session.run(`
        MATCH (p:Provenance {document_id: $documentId})
     ...
     */
    async generateProvenanceReport(documentId) {
        const session = this.driver.session();
        try {
            // Get document
        ...
        `, { documentId });
            const transformations = transformResult.records.map((record) => {
            ...
            return {
                ...
                lineage,
                transformations,
                chain_of_custody: chainOfCustody.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()),
            };
        }
        finally {
            await session.close();

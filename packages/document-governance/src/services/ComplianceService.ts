...
export class ComplianceService {
  ...
  async checkCompliance(documentId: string): Promise<ComplianceCheckResult> {
    ...
    try {
      ...
      const score =
        requiredSections.length > 0 ? (compliantSections / requiredSections.length) * 100 : 100;
      ...
      const checkResult: ComplianceCheckResult = {
        ...
        overall_compliant:
          ...
          riskIssues.filter((r) => r.severity === "critical").length === 0,
    ...
    try {
      ...
      const report: ComplianceReport = {
        ...
        summary: {
          ...
          low_findings: findings.filter((f) => f.severity === "low" || f.severity === "info")
            .length,
        },
        findings,
        recommendations: [

...
class LifecycleEngine {
     ...
     */
    getLifecycleDefinition(lifecycleType) {
     ...
     */
    async getAvailableTransitions(documentId) {
        ...
        `, { documentId });
            ...
            const lifecycleType = record.get('lifecycleType');
     ...
     */
    async requestTransition(request, userId) {
        const session = this.driver.session();
        try {
        ...
        `, { documentId: request.document_id });
            ...
            // Find the transition
            const definition = this.getLifecycleDefinition(lifecycleType);
            const transition = definition.transitions.find((t) => t.from === currentState && t.to === request.target_state);
            if (!transition) {
                return {
                    success: false,
                    document_id: request.document_id,
            ...
            if (transition.requires_approval) {
                const approvalRequestId = (0, uuid_1.v4)();
                await session.run(`
     ...
     */
    async processApproval(approvalRequestId, decision, approverId, comment) {
        const session = this.driver.session();
        try {
     ...
     */
    async getLifecycleHistory(documentId) {
        const session = this.driver.session();
        try {
            const result = await session.run(`

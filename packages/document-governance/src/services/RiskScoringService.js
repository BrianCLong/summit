...
const CLASSIFICATION_MODIFIERS = {
    Public: -1.0,
    Internal: 0,
    Confidential: 0.5,
    Restricted: 1.0,
...
class RiskScoringService {
    ...
    async calculateRiskScore(request) {
        ...
        `, { documentId: request.document_id });
            ...
            const classificationModifier = CLASSIFICATION_MODIFIERS[doc.classification] || 0;
            ...
            const riskScore = {
                ...
                factors: RISK_DIMENSIONS.flatMap((dim) => dim.factors.map((factor) => ({
                    dimension: dim.id,
                    factor,
     ...
     */
    async getHighRiskDocuments(minRiskLevel = 'High') {
        const session = this.driver.session();
        try {

...
const context_engineer_js_1 = require("../context-engineer.js");
describe('Context Engineering Core', () => {
    it('evicts middle history deterministically', () => {
        ...
        const result = (0, context_engineer_js_1.buildContext)(input);
    ...
    it('reduces tool outputs before prompt build', () => {
        ...
        const result = (0, context_engineer_js_1.buildContext)(input);
        const toolMessage = result.messages.find(msg => msg.content.includes('x'));
    ...
    it('preserves intents and commitments during compression', () => {
        ...
        const result = (0, context_engineer_js_1.buildContext)(input);
        const intent = result.manifest.items.find(item => item.policyLabels?.includes('intent'));
        const commitment = result.manifest.items.find(item => item.policyLabels?.includes('commitment'));
        expect(intent?.content).toBe('Intent: keep this exact language intact.');
        expect(commitment?.content).toBe('Commitment: deliver the report by Friday.');
    });
    ...
    it('rejects items missing provenance', () => {
        ...
        expect(() => (0, context_engineer_js_1.buildContext)(input)).toThrow('Context item missing provenance for stream user');
    });
});

...
class PatternMining {
     ...
     */
    frequentSubgraphs(minSupport = 0.1, maxSize = 5) {
     ...
     */
    detectAnomalousRelationships(threshold = 0.7) {
        const anomalies = [];
        const exported = this.storage.exportGraph();
        for (const edge of exported.edges) {
     ...
     */
    temporalPatterns(timeWindow = 86400000) {
        const exported = this.storage.exportGraph();
        const patterns = [];
     ...
     */
    inferMultiHopRelationships(sourceId, targetId, maxHops = 3) {
        const paths = [];
     ...
     */
    calculateRelationshipStrength(edgeId) {
        ...
        if (source && target) {
            const sourceNeighbors = new Set(this.storage.getNeighbors(source.id, 'both').map(n => n.id));
            const targetNeighbors = new Set(this.storage.getNeighbors(target.id, 'both').map(n => n.id));
            const intersection = new Set(Array.from(sourceNeighbors).filter(n => targetNeighbors.has(n)));
    ...
    calculateMotifSignificance(type, count) {
    ...
    calculateAnomalyScore(edge) {
        ...
        const recentEdges = typeEdges.filter(e => e.createdAt > Date.now() - 30 * 24 * 60 * 60 * 1000);
        const temporalRarity = 1 - (recentEdges.length / (typeEdges.length || 1));
        anomalyScore += temporalRarity * 0.3;
        return anomalyScore;
    }
    identifyAnomalyReasons(edge, score) {
        const reasons = [];
        if (score > 0.7) {
    ...
    nodeMatchesTemplate(node, template) {
    ...
    expandPatternMatch(node, template) {
        // Simplified implementation
        return null;
    }
    extractPatternsFromEdges(edges) {
        // Simplified implementation
        return [];
    }
    findPathsDFS(current, target, maxHops, currentPath, paths, visited) {

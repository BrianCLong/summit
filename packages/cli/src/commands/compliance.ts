...
const summary = new Command('summary')
  ...
  .action(async (framework, options) => {
    ...
    const scoreColor = s.overallScore >= 80 ? chalk.green :
                       s.overallScore >= 60 ? chalk.yellow : chalk.red;
    ...
    if (s.categoryBreakdown?.length > 0) {
      ...
      s.categoryBreakdown.forEach((cat) => {
        const catColor = cat.score >= 80 ? chalk.green :
                         cat.score >= 60 ? chalk.yellow : chalk.red;
...
const assess = new Command('assess')
  ...
  .action(async (framework, controlId, options) => {
    ...
    const statusColor = a.status === 'compliant' ? chalk.green :
                        a.status === 'partial' ? chalk.yellow : chalk.red;
...
const evidence = new Command('evidence')
  ...
  .action(async (options) => {
    ...
    console.log(formatOutput(displayData, ['id', 'control', 'framework', 'type', 'status', 'collected']));
  });

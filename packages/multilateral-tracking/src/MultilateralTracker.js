...
exports.MultilateralTracker = void 0;
/**
 * MultilateralTracker
 *
 ...
 */
class MultilateralTracker {
    organizations = new Map();
    organizationsByType = new Map();
    organizationsByCountry = new Map();
    votingPatterns = new Map();
    powerDynamics = new Map();
    /**
     ...
     */
    analyzeCountryEngagement(country) {
    ...
    analyzeVotingPatterns(country) {
        ...
        for (const [key, patternList] of this.votingPatterns.entries()) {
            if (key.startsWith(`${country}:`)) {
                ...
                const avgAlignment = latestPattern.votingAlignment.reduce((sum, a) => sum + a.agreementRate, 0) / (latestPattern.votingAlignment.length || 1);
                // Get top alignments
     ...
     */
    identifyVotingBlocs(organizationId) {
     ...
     */
    analyzePowerDynamics(organizationId) {
        const org = this.organizations.get(organizationId);
        if (!org) {
            return {
     ...
     */
    compareOrganizationEffectiveness(organizationIds) {
     ...
     */
    trackResolutionImplementation(organizationId, resolutionId) {
        const org = this.organizations.get(organizationId);
     ...
     */
    analyzeReformProspects(organizationId) {
        const org = this.organizations.get(organizationId);
        ...
        const avgSupport = org.reformProposals.length > 0
            ...
            : 0;
        const likelyToSucceed = org.reformProposals.filter(r => r.support.percentage > 66 && r.status !== 'REJECTED').length;
    ...
    monitorActivityLevels() {
        ...
        for (const org of this.organizations.values()) {
            const activePrograms = org.programs.filter(p => !p.endDate || p.endDate > new Date()).length;
            ...
            const olderMeetings = org.meetings.filter(m => m.date >= sixtyDaysAgo && m.date < thirtyDaysAgo).length;
            let trend = 'STABLE';
            if (recentMeetings > olderMeetings * 1.2)
                trend = 'INCREASING';
            else if (recentMeetings < olderMeetings * 0.8)
                trend = 'DECREASING';
            results.push({
                organization: org.name,
     ...
     */
    predictOrganizationTrajectory(organizationId) {
        const org = this.organizations.get(organizationId);
        if (!org) {
            return {
                organization: organizationId,
        ...
        if (org.reformProposals && org.reformProposals.length > 0) {
            const activeReforms = org.reformProposals.filter(r => r.status === 'UNDER_DISCUSSION' || r.status === 'NEGOTIATING');
        ...
        if (org.members.length > 0)
            confidence += 10;
        if (org.activities.length > 0)
            confidence += 10;
        if (org.resolutions.length > 0)
            confidence += 10;
        return {
            organization: org.name,
    ...
    generateReformRecommendations(org) {
...
exports.MultilateralTracker = MultilateralTracker;

...
exports.RestPullConnector = void 0;
const axios_1 = __importDefault(require("axios"));
const base_connector_1 = require("../base-connector");
/**
 * REST Pull Connector
 */
class RestPullConnector extends base_connector_1.PullConnector {
    manifest = {
    ...
    async pull(context) {
        ...
        const errors = [];
        try {
            const { path, method = 'GET', queryParams = {}, pagination, responseDataPath = 'data', entityType = 'GenericRecord', rateLimitDelay = 0, } = this.restConfig;
                ...
                try {
                    ...
                    for (const record of data) {
                        try {
                            const entity = {
                                type: entityType,
                                externalId: this.generateExternalId(path, record.id || `record_${entitiesProcessed + 1}`),
                                props: record,
                                confidence: 1.0,
                                observedAt: new Date(),
                                sourceMeta: {
                            ...
                            context.logger.error('Error processing API record', error, {
                                page,
                    ...
                    context.logger.error('Error fetching from API', error, {
                        page,
                        offset,
                    });
                    errors.push({
            ...
            const durationMs = Date.now() - startTime;
            context.logger.error('REST API ingestion failed', error);
...
exports.RestPullConnector = RestPullConnector;

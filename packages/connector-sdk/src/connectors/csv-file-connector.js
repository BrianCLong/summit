...
exports.CsvFileConnector = void 0;
 ...
 * CSV File Connector
 */
class CsvFileConnector extends base_connector_1.PullConnector {
    manifest = {
    ...
    async testConnection() {
        try {
            ...
            const stream = (0, fs_1.createReadStream)(this.csvConfig.filePath, {
            ...
            return {
                success: false,
                message: `Failed to open CSV file: ${error.message}`,
            };
        }
    }
    async pull(context) {
        ...
        const errors = [];
        try {
            const { filePath, delimiter = ',', hasHeader = true, encoding = 'utf8', skipRows = 0, maxRows, entityType = 'GenericRecord', } = this.csvConfig;
            ...
            for await (const record of parser) {
                ...
                try {
                    ...
                    context.logger.error('Error processing CSV row', error, {
                        rowNumber: entitiesProcessed + 1,
                    });
                    errors.push({
            ...
            const durationMs = Date.now() - startTime;
            context.logger.error('CSV ingestion failed', error);
            return this.failureResult(error, entitiesProcessed, 0, durationMs);
        }
    }
}
exports.CsvFileConnector = CsvFileConnector;

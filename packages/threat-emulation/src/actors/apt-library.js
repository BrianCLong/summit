...
exports.EmulationPlanGenerator = exports.APTLibrary = void 0;
...
class APTLibrary {
    ...
    initializeActors() {
        ...
        this.addActor({
            ...
            category: types_1.ThreatActorCategory.APT,
            motivation: [types_1.ThreatActorMotivation.ESPIONAGE],
            attribution: {
                country: 'Russia',
                confidence: 'high',
        ...
        this.addActor({
            ...
            category: types_1.ThreatActorCategory.APT,
            motivation: [types_1.ThreatActorMotivation.ESPIONAGE, types_1.ThreatActorMotivation.DISRUPTION],
            attribution: {
                country: 'Russia',
                confidence: 'high',
                sponsorship: 'state-sponsored',
        ...
        this.addActor({
            ...
            category: types_1.ThreatActorCategory.NATION_STATE,
            motivation: [types_1.ThreatActorMotivation.FINANCIAL, types_1.ThreatActorMotivation.ESPIONAGE, types_1.ThreatActorMotivation.DESTRUCTION],
            attribution: {
                country: 'North Korea',
                confidence: 'high',
                sponsorship: 'state-sponsored',
    ...
    getActorsByCategory(category) {
        return Array.from(this.actors.values()).filter(a => a.category === category);
    }
    getActorsBySector(sector) {
        const sectorLower = sector.toLowerCase();
        return Array.from(this.actors.values()).filter(a => a.targetedSectors.some(s => s.toLowerCase().includes(sectorLower)));
    }
    getActorsByRegion(region) {
        const regionLower = region.toLowerCase();
        return Array.from(this.actors.values()).filter(a => a.targetedRegions.some(r => r.toLowerCase().includes(regionLower)));
    }
    searchActors(query) {
        const queryLower = query.toLowerCase();
        return Array.from(this.actors.values()).filter(a => a.names.some(n => n.toLowerCase().includes(queryLower)) ||
            a.aliases.some(al => al.toLowerCase().includes(queryLower)) ||
            a.targetedSectors.some(s => s.toLowerCase().includes(queryLower)));
    }
}
exports.APTLibrary = APTLibrary;
/**
 * Emulation Plan Generator
 */
class EmulationPlanGenerator {
    aptLibrary;
    constructor() {
     ...
     */
    generatePlan(actorId, objectives, scope) {
    ...
    buildPhases(actor, durationDays) {
        const phases = [];
...
exports.EmulationPlanGenerator = EmulationPlanGenerator;

...
exports.PluginHostService = void 0;
 ...
 */
class PluginHostService extends eventemitter3_1.default {
    manager;
    loader;
    sandbox;
    registry;
    ...
    constructor(config, logger) {
        ...
        this.sandbox = new plugin_system_1.PluginSandbox();
        this.loader = new plugin_system_1.DefaultPluginLoader(this.sandbox);
        this.registry = new plugin_system_1.InMemoryPluginRegistry();
        this.resolver = new plugin_system_1.DefaultDependencyResolver(config.platformVersion);
        this.security = new plugin_system_1.PluginSecurity();
        this.quotaEnforcer = new plugin_system_1.QuotaEnforcer();
        this.cpuTracker = new plugin_system_1.CPUTimeTracker();
        // Initialize authorization provider
        this.authProvider = this.createAuthProvider(config);
        // Create plugin manager
        this.manager = new plugin_system_1.PluginManager(this.loader, this.registry, this.resolver, config.platformVersion);
     ...
     * Install a plugin
     */
    async installPlugin(manifest, source, options) {
        ...
        try {
            ...
            if (this.config.security.scanOnInstall && source.path) {
                ...
                if (!scanResult.safe) {
                    ...
                    throw new Error(`Plugin ${manifest.id} failed security scan: ${criticalVulns.map(v => v.message).join(', ')}`);
                }
            }
            // Check authorization
            const authContext = {
            ...
            const permissionsAllowed = await this.authProvider.checkManifestPermissions(manifest, authContext);
            if (!permissionsAllowed && !options?.forceInstall) {
                throw new Error(`Plugin ${manifest.id} permissions not authorized. Use forceInstall option to bypass.`);
            }
        ...
        try {
            ...
            if (options?.userId) {
                ...
                if (metadata) {
                    ...
                    const allowed = await this.authProvider.checkManifestPermissions(metadata.manifest, authContext);
                    if (!allowed) {
    ...
    createAuthProvider(config) {
        if (config.environment === 'development') {
            ...
            return new plugin_system_1.DevelopmentAuthorizationProvider();
        }
        if (config.authorization.provider === 'opa') {
            return new plugin_system_1.OPAAuthorizationProvider(config.authorization.opaEndpoint, config.authorization.policyPath);
        }
    ...
    async loadAutoStartPlugins() {
        ...
        for (const pluginId of plugins) {
            try {
                const metadata = await this.registry.get(pluginId);
                if (metadata && metadata.state === plugin_system_1.PluginState.UNLOADED) {
                    await this.manager.enable(pluginId);
...
exports.PluginHostService = PluginHostService;

...
describe('Competitive Execution Engine', () => {
    it('tracks competitor matrix freshness', () => {
        const matrix = new src_1.CompetitorMatrix();
        matrix.addOrUpdate({
            name: 'RivalOne',
            features: ['feature-a'],
            pricing: ['tiered'],
    ...
    it('enforces mandatory win/loss fields and review cadence', () => {
        const pipeline = new src_1.WinLossPipeline();
    ...
    it('blocks hearsay and redacts PII in verbatims', () => {
        const intake = new src_1.VerbatimIntake();
    ...
    it('rejects speculative claims and tracks reverification needs', () => {
        const claims = new src_1.ClaimLibrary();
        claims.addClaim({
            id: 'c1',
            statement: '99% uptime',
            source: 'status page',
    ...
    it('flags release cadence gaps', () => {
        const tracker = new src_1.ReleaseCadenceTracker();
        tracker.addEvent({ competitor: 'Rival', description: 'big launch', mappedRoadmapDelta: new Date(Date.now() - 40 * DAYS) });
        expect(tracker.gaps().length).toBe(1);
    });
    it('maintains top risks with severity replacement', () => {
        const register = new src_1.RiskRegister();
        ...
        register.upsert({ id: 'high', threat: 'high', mitigation: 'm', owner: 'o', dueDate: new Date(), severity: 1 });
    ...
    it('expires battlecards lacking recent review', () => {
        ...
        const cards = new src_1.BattlecardLibrary(claims);
        cards.add({ id: 'b1', competitor: 'Rival', approvedLanguage: 'safe', evidenceIds: ['c1'], lastReviewed: new Date(Date.now() - 61 * DAYS) });
        expect(cards.expired().map((c) => c.id)).toContain('b1');
    });
    it('hashes teardown assets and validates provenance', () => {
        const repo = new src_1.TeardownRepo();
        const asset = repo.add({ id: 'a1', competitor: 'Rival', type: 'screenshot', content: 'image-bytes', metadata: { flow: 'onboarding' } });
        expect(repo.verify(asset.id)).toBe(true);
    });
    it('detects switch-cost coverage gaps', () => {
        const inventory = new src_1.SwitchCostInventory();
        inventory.add({
            id: 's1',
            competitor: 'Rival',
            data: ['users'],
    ...
    it('enforces monthly council spacing', () => {
        const council = new src_1.CompetitiveCouncil();
        council.recordMeeting({ decisions: ['do'], actions: [] });
        expect(() => council.recordMeeting({ decisions: ['too soon'], actions: [], meetingDate: new Date(Date.now() + 10 * DAYS) })).toThrow('monthly');
    });
    it('requires monthly competitive responses', () => {
        const tracker = new src_1.CompetitiveResponseTracker();
    ...
    it('produces importer parity diff reports', () => {
        const manager = new src_1.ImporterManager();
    ...
    it('applies compat mappings per workspace', () => {
        const manager = new src_1.CompatModeManager();
    ...
    it('audits and allows undo for bulk operations', () => {
        const tooling = new src_1.BulkTooling();
    ...
    it('builds migration validation reports', () => {
        const validator = new src_1.MigrationValidator();
    ...
    it('enforces integration signatures and retries', () => {
        const registry = new src_1.IntegrationAdapterRegistry();
    ...
    it('captures collaboration gravity events', () => {
        const gravity = new src_1.CollaborationGravity();
    ...
    it('applies enterprise controls per workspace', () => {
        const controls = new src_1.EnterpriseControls();
    ...
    it('tracks migration metrics', () => {
        const metrics = new src_1.MetricsTracker();
    ...
    it('delivers first value package artifact', () => {
        const pkg = new src_1.FirstValuePackage('Workspace {workspace}: {summary}');
    ...
    it('upgrades workspaces with clean boundaries', () => {
        const manager = new src_1.WorkspaceManager();
    ...
    it('enforces entitlements from configuration', () => {
        const engine = new src_1.EntitlementEngine({ pro: ['feature-a'] });
        expect(engine.isEntitled('pro', 'feature-a')).toBe(true);
    });
    it('activates time-boxed migration offers', () => {
        const manager = new src_1.MigrationOfferManager();
    ...
    it('records trustworthy usage dashboards', () => {
        const dashboard = new src_1.UsageDashboard();
    ...
    it('guards pricing experiments with churn guardrails', () => {
        const manager = new src_1.PricingExperimentManager();
    ...
    it('auto-rolls back on SLO burn', () => {
        const manager = new src_1.SLOManager();
        manager.define({ id: 'journey1', journey: 'search', target: 99.9, measurementWindowDays: 30, errorBudget: 1 });
        expect(manager.evaluateRelease('journey1', 2)).toBe('rollback');
    });
    it('triggers rollback on failed synthetic checks', () => {
        const checker = new src_1.SyntheticChecker();
    ...
    it('tracks and resolves top customer-visible errors', () => {
        const remediation = new src_1.ErrorRemediation();
    ...
    it('improves latency for demo-critical endpoints', () => {
        const tracker = new src_1.LatencyTracker();
    ...
    it('stores incidents and templates on status page', () => {
        const status = new src_1.StatusPage();
    ...
    it('requires MFA and expirations for access', () => {
        const access = new src_1.AccessManager();
        access.grantAccess({ user: 'alice', roles: ['admin'], expiresAt: new Date(Date.now() - DAYS), mfa: true });
        expect(access.review()).toContain('alice');
        expect(() => access.grantAccess({ user: 'bob', roles: [], expiresAt: new Date(), mfa: false })).toThrow('MFA required');
    });
    it('enforces secret rotation schedules', () => {
        const secrets = new src_1.SecretManager();
    ...
    it('blocks banned licenses via SBOM gate', () => {
        const gate = new src_1.SBOMGate(['GPL']);
        expect(() => gate.enforce(['MIT', 'GPL'])).toThrow('Banned licenses detected');
    });
    it('automates questionnaire answers', () => {
        const automation = new src_1.QuestionnaireAutomation({ question: 'answer' });
        expect(automation.respond('question')).toBe('answer');
    });
    it('tracks narrative assets with evidence-backed pillars', () => {
        const claims = new src_1.ClaimLibrary();
        claims.addClaim({ id: 'c1', statement: 'fast', source: 'benchmark', evidenceRating: 5, legalStatus: 'approved', speculative: false });
        const narrative = new src_1.NarrativeOps(claims);
    ...
    it('certifies partners and tracks analytics', () => {
        const program = new src_1.PartnerProgram();
        ...
        const certification = new src_1.PartnerCertification(['test']);
        expect(certification.certify(partner).passed).toBe(true);
        const analytics = new src_1.PartnerAnalytics();
    ...
    it('standardizes bespoke integrations', () => {
        const manager = new src_1.DeprecationManager();
    ...
    it('publishes defamation-safe statements backed by evidence', () => {
        const claims = new src_1.ClaimLibrary();
        claims.addClaim({ id: 'c1', statement: 'accurate', source: 'source', evidenceRating: 5, legalStatus: 'approved', speculative: false });
        const policy = new src_1.DefamationPolicy(claims);
        expect(policy.publish('statement', ['c1'])).toBe('statement');
    });
    it('prepares portability playbook outputs', () => {
        const playbook = new src_1.PortabilityPlaybook();
    ...
    it('validates vendor readiness', () => {
        const register = new src_1.VendorRegister();
        expect(() => register.add({ name: 'vendor', dpaSigned: false, sccReady: true })).toThrow('Vendor not ready');
    });
    it('schedules quarterly legal reviews', () => {
        const scheduler = new src_1.LegalReviewScheduler();

...
function createLCG(seed: string | number = Date.now()): RandomSource {
    let state = typeof seed === 'number' ? seed : Array.from(String(seed)).reduce((acc, char) => acc + char.charCodeAt(0), 0);
    return {
        next: () => {
...
export function dropTool(allowlist: string[], options: NegativeAugmentationOptions = {}): string[] {
...
export function fuzzUser(content: string, options: NegativeAugmentationOptions = {}): string {
...
export function perturbSteps(trajectory: AgentTrajectory, options: NegativeAugmentationOptions = {}): AgentTrajectory {
    ...
    const duplicated = JSON.parse(JSON.stringify(clone.turns[duplicateIndex]));

...
function validateTextField(
  ...
  if (value && value.length > limit) {
    recordError(errors, `${label} exceeds maximum length (${limit})`);
  }
}
...
export function validateTrajectory(
  ...
  const errors: string[] = [];
  const allowedTools =
    options.allowedTools ?? trajectory.meta.allowed_tools ?? DEFAULT_ALLOWED_TOOLS;
  const maxTurns = options.maxTurns ?? DEFAULT_MAX_TURNS;
  const maxContentLength = options.maxContentLength ?? DEFAULT_MAX_CONTENT_LENGTH;
  const allowUnsafe = Boolean(
    options.allowUnsafeDefaults || trajectory.meta.allow_unsafe,
  );
  ...
  trajectory.turns.forEach((turn, idx) => {
    validateTextField(turn.content, `Turn ${idx} content`, maxContentLength, errors);
    validateTextField(turn.plan, `Turn ${idx} plan`, maxContentLength, errors);
    validateTextField(
      turn.reflection,
      `Turn ${idx} reflection`,
      maxContentLength,
    ...
    toolCalls.forEach((call) => {
      ...
      if (!allowUnsafe && isDangerousToolCall(call.name, call.arguments)) {
        recordError(
          errors,
          `Dangerous bash command blocked for call_id ${call.call_id}`,
        );
      }
    });
  ...
  const missingResults = new Set(
    [...seenCallIds].filter((id) => !resultCallIds.has(id)),
  );
  missingResults.forEach((id) =>
    recordError(errors, `Tool call ${id} is missing a result`),
  );

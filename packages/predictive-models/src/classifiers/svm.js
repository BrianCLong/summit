...
exports.SVMRegressor = exports.SVMClassifier = void 0;
class SVMClassifier {
    config;
    supportVectors = [];
    alphas = [];
        ...
        while (passes < maxPasses) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                if ((y[i] * Ei < -this.config.tolerance && this.alphas[i] < this.config.C) ||
                    ...
                    if (Math.abs(this.alphas[j] - alphaJ_old) < 1e-5)
                        continue;
                    // Update alpha_i
...
exports.SVMClassifier = SVMClassifier;
/**
 * SVM Regressor (SVR)
 */
class SVMRegressor {
    ...
    predictSingle(supportVectors, sample) {
        let result = this.bias;
...
exports.SVMRegressor = SVMRegressor;

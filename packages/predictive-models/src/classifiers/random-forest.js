...
class RandomForestClassifier {
    ...
    calculateConfusionMatrix(actual, predicted) {
        const matrix = Array(this.classes.length)
            .fill(0)
...
class DecisionTree {
    ...
    calculateGain(features, labels, featureIndex, threshold) {
        const { leftIndices, rightIndices } = this.splitData(features, featureIndex, threshold);
        ...
        const weightedEntropy = (leftIndices.length / n) * leftEntropy +

...
class TreatyMonitor {
    treaties = new Map();
    treatiesByCategory = new Map();
    treatiesByParty = new Map();
    treatiesByStatus = new Map();
    negotiationProgress = new Map();
    /**
     ...
     */
    getBilateralTreaties(party1, party2) {
     ...
     */
    trackNegotiationProgress(progress) {
        ...
        if (treaty && treaty.status === types_js_1.TreatyStatus.UNDER_NEGOTIATION) {
            if (progress.overallProgress >= 100) {
                this.updateTreatyStatus(progress.treatyId, types_js_1.TreatyStatus.NEGOTIATION_COMPLETED);
            }
        }
    }
    /**
     ...
     */
    updateTreatyStatus(treatyId, newStatus) {
     ...
     */
    recordRatification(treatyId, party) {
        const treaty = this.treaties.get(treatyId);
        if (!treaty)
            return;
        // Add to parties
     ...
     */
    checkEntryIntoForce(treatyId) {
        const treaty = this.treaties.get(treatyId);
        if (!treaty || treaty.status === types_js_1.TreatyStatus.IN_FORCE)
            return;
        ...
        if (activeParties.length >= treaty.signatories.length * 0.5) {
            this.updateTreatyStatus(treatyId, types_js_1.TreatyStatus.IN_FORCE);
     ...
     */
    assessCompliance(treatyId, party) {
        const treaty = this.treaties.get(treatyId);
        if (!treaty)
            return undefined;
        ...
        if (complianceRecords.length === 0)
            return undefined;
        return complianceRecords.sort((a, b) => b.assessmentDate.getTime() - a.assessmentDate.getTime())[0];
    }
    /**
     * Get all compliance violations
     */
    getViolations(treatyId, party) {
        let treaties;
        if (treatyId) {
            const treaty = this.treaties.get(treatyId);
    ...
    getExpiringSoon(daysAhead = 365) {
        ...
        return Array.from(this.treaties.values())
            ...
            t.status === types_js_1.TreatyStatus.IN_FORCE)
            .sort((a, b) => {
     ...
     */
    getPendingRatifications(party) {
        return Array.from(this.treaties.values()).filter(treaty => {
            const isSigner = treaty.signatories.some(s => (s.country === party || s.organization === party) && s.pending);
     ...
     */
    analyzeTreatyImplementation(treatyId) {
        const treaty = this.treaties.get(treatyId);
        if (!treaty) {
            return {
                overallProgress: 0,
     ...
     */
    analyzeTreatyNetwork(party) {
        const treaties = this.getTreatiesByParty(party);
        const treatiesByCategory = {};
        const treatiesByStatus = {};
        const bilateralCounts = new Map();
        for (const treaty of treaties) {
            // Count by category
            for (const category of treaty.category) {
        ...
        for (const treaty of treaties) {
            pendingObligations += treaty.keyObligations.filter(o => o.status === 'PENDING' || o.status === 'IN_PROGRESS').length;
        }
        return {
    ...
    detectTerminationRisks() {
        ...
        for (const treaty of this.treaties.values()) {
            if (treaty.status !== types_js_1.TreatyStatus.IN_FORCE)
                continue;
            const indicators = [];
            let riskScore = 0;
            ...
            const seriousViolations = violations.filter(v => v.severity === 'SERIOUS' || v.severity === 'GRAVE');
            ...
            if (treaty.disputes) {
                const activeDisputes = treaty.disputes.filter(d => d.status === 'PENDING' || d.status === 'IN_PROGRESS');
...
exports.TreatyMonitor = TreatyMonitor;

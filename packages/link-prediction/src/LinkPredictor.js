...
class LinkPredictor {
     ...
     */
    commonNeighbors(sourceId, targetId) {
     ...
     */
    jaccardCoefficient(sourceId, targetId) {
        const sourceNeighbors = new Set(this.storage.getNeighbors(sourceId, 'both').map(n => n.id));
        const targetNeighbors = new Set(this.storage.getNeighbors(targetId, 'both').map(n => n.id));
     ...
     */
    preferentialAttachment(sourceId, targetId) {
     ...
     */
    resourceAllocation(sourceId, targetId) {
        const sourceNeighbors = new Set(this.storage.getNeighbors(sourceId, 'both').map(n => n.id));
        const targetNeighbors = new Set(this.storage.getNeighbors(targetId, 'both').map(n => n.id));
        const common = Array.from(sourceNeighbors).filter(n => targetNeighbors.has(n));
        let score = 0;
     ...
     */
    katzScore(sourceId, targetId, beta = 0.005, maxLength = 3) {
        let score = 0;
     ...
     */
    timeAwarePrediction(sourceId, targetId, decayFactor = 0.9) {
     ...
     */
    multiRelationalPrediction(sourceId, targetId, edgeType) {
     ...
     */
    ensemblePrediction(sourceId, targetId) {
        const methods = [
        ...
        for (const method of methods) {
            const normalizedScore = method.score / (maxScores[method.name] || 1);
     ...
     */
    predictLinksForNode(nodeId, k = 10) {
        const predictions = [];
        const exported = this.storage.exportGraph();
        const nodes = exported.nodes;
        // Get existing neighbors
        const existingNeighbors = new Set([...this.storage.getNeighbors(nodeId, 'out'), ...this.storage.getNeighbors(nodeId, 'in')]
            .map(n => n.id));
        // Calculate scores for non-connected nodes
        for (const node of nodes) {
            if (node.id === nodeId)
                continue;
            if (existingNeighbors.has(node.id))
                continue;
     ...
     */
    batchPredict(pairs) {
        const predictions = pairs.map(([sourceId, targetId]) => this.ensemblePrediction(sourceId, targetId));
        return {
            predictions,
            method: 'ensemble',
            timestamp: Date.now()
     ...
     */
    predictMissingLinks(threshold = 0.5, maxPredictions = 100) {
        const predictions = [];
        const exported = this.storage.exportGraph();
        const nodes = exported.nodes;
    ...
    // ==================== Helper Methods ====================
    countPathsOfLength(sourceId, targetId, length) {
        if (length === 0) {
        ...
        let count = 0;
        const queue = [{ nodeId: sourceId, depth: 0 }];
    ...
    calculateTypeSpecificScore(sourceId, targetId, edgeType) {
    ...
    generateExplanation(sourceId, targetId, methods) {

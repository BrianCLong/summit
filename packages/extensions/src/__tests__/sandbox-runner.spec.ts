...
describe('SandboxRunner', () => {
  it('enforces activation timeouts and isolates execution', async () => {
    ...
    const runner = new SandboxRunner(new ExtensionObservability(), { timeoutMs: 50, memoryLimitMb: 16 });
    const context = createContext(dir);

    await expect(() => runner.run(baseManifest as any, entrypoint, 'default', context)).rejects.toThrow(
      /timed out|exceeded sandbox timeout/
    );
  });
  ...
  it('returns activation exports when within limits', async () => {
    ...
    const runner = new SandboxRunner(new ExtensionObservability(), { timeoutMs: 500, memoryLimitMb: 16 });
    const context = createContext(dir);

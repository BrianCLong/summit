...
exports.computeFlowMetrics = computeFlowMetrics;
...
function computeFlowMetrics(workItems, now = new Date()) {
    const grouped = {};
    ...
    return Object.entries(grouped).map(([team, items]) => {
        ...
        const leadTimeDays = completed.length === 0
            ? 0
            :
                completed.reduce((acc, item) => acc + daysBetween(item.createdAt, item.completedAt ?? now), 0) / completed.length;
        const cycleTimeDays = completed.length === 0
            ? 0
        ...
        const blockedTimeHours = completed.reduce((acc, item) => {
            if (!item.blockedWindows || item.blockedWindows.length === 0)
                return acc;
        ...
        const totalBlockedEvents = items.reduce((acc, item) => acc + (item.blockedWindows ? item.blockedWindows.length : 0), 0);

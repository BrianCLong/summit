...
exports.InferenceEngine = exports.Precision = exports.ModelFormat = void 0;
...
(function (ModelFormat) {
    ...
    ModelFormat["TENSORFLOW_JS"] = "tfjs";
 ...
 */
class InferenceEngine extends eventemitter3_1.default {
    logger;
    models = new Map();
    modelCache = new Map();
    inferenceQueue = [];
    stats = new Map(); // modelId -> latencies
    isProcessing = false;
    maxCacheSize;
    maxBatchSize;
    constructor(options) {
        ...
        this.logger = options?.logger || (0, pino_1.pino)({ name: 'InferenceEngine' });
    }
    /**
     * Register a model for inference
     */
    async registerModel(metadata, modelData) {
    ...
    async executeInference(request) {
        ...
        this.logger.debug({ requestId: request.id, modelId: request.modelId, latency }, 'Inference completed');
        return result;
    }
    /**
     * Run model inference (placeholder for actual inference logic)
     */
    async runModelInference(model, input, metadata) {
     ...
     */
    async batchInfer(modelId, inputs, options) {
        const batchSize = Math.min(inputs.length, this.maxBatchSize);
        const results = [];
        for (let i = 0; i < inputs.length; i += batchSize) {
            const batch = inputs.slice(i, i + batchSize);
            const batchResults = await Promise.all(batch.map(input => this.infer(modelId, input, options)));
     ...
     */
    recordInferenceStats(modelId, latency) {

...
export const DEFAULT_WEIGHTS: ScoreWeights = {
  ...
  governance: {
    ...
    ethicsAndCompliance: 0.3,
    riskManagement: 0.2,
    shareholderRights: 0.1,
  },
  categoryWeights: {
    environmental: 0.34,
    social: 0.33,
...
export function calculateEnvironmentalScore(
  ...
  if (metrics.carbonEmissions) {
    const emissionsScore = calculateCarbonScore(
      metrics.carbonEmissions,
      benchmarks?.carbonEmissions
    );
...
function calculateDiversityScore(
  ...
  const genderBalance = 100 - Math.abs(50 - (diversity.genderDiversity.female / total) * 100) * 2;

...
const EntityExtractor_js_1 = require("../extraction/EntityExtractor.js");
const EntityMatcher_js_1 = require("../matching/EntityMatcher.js");
const types_js_1 = require("../types.js");
describe('EntityExtractor', () => {
    let extractor;
    beforeEach(() => {
        extractor = new EntityExtractor_js_1.EntityExtractor({
    ...
    test('should extract email addresses', async () => {
        ...
        const emails = result.entities.filter(e => e.type === types_js_1.EntityType.EMAIL);
    ...
    test('should extract phone numbers', async () => {
        ...
        const result = await extractor.extract(text);
        const phones = result.entities.filter(e => e.type === types_js_1.EntityType.PHONE);
        expect(phones.length).toBeGreaterThanOrEqual(1);
    });
    test('should extract URLs', async () => {
        const text = 'Visit https://www.example.com for more information.';
        const result = await extractor.extract(text);
        const urls = result.entities.filter(e => e.type === types_js_1.EntityType.URL);
...
describe('EntityMatcher', () => {
    let matcher;
    beforeEach(() => {
        matcher = new EntityMatcher_js_1.EntityMatcher({
            threshold: 0.7,
            methods: [EntityMatcher_js_1.MatchingMethod.EXACT, EntityMatcher_js_1.MatchingMethod.FUZZY]
        });
    });
    const createEntity = (id, text, type = types_js_1.EntityType.PERSON) => ({
        id,
        type,
        text,
        attributes: {},
    ...
    test('should not match different types', async () => {
        const entity1 = createEntity('1', 'Acme Corp', types_js_1.EntityType.PERSON);
        const entity2 = createEntity('2', 'Acme Corp', types_js_1.EntityType.ORGANIZATION);
        const match = await matcher.matchPair(entity1, entity2);

...
exports.SupplierRiskAssessor = void 0;
/**
...
class SupplierRiskAssessor {
     ...
     */
    async assessSupplier(node, financialMetrics, cyberPosture, esgScore) {
        const categoryRisks = [];
     ...
     * Assess financial health
     */
    assessFinancialHealth(nodeId, metrics) {
        const indicators = [];
     ...
     * Assess cybersecurity posture
     */
    assessCybersecurity(nodeId, posture) {
        const indicators = [];
        let score = posture.securityScore;
        // Assess certifications
        const requiredCerts = ['ISO 27001', 'SOC 2', 'NIST'];
        const hasCerts = requiredCerts.filter(cert => posture.certifications.some(c => c.includes(cert)));
        indicators.push({
        ...
        const recentIncidents = posture.incidentHistory.filter(inc => !inc.resolved || (new Date().getTime() - inc.date.getTime()) < 365 * 24 * 60 * 60 * 1000);
    ...
    assessESG(nodeId, esg) {
        ...
        // Assess violations
        const recentViolations = esg.violations.filter(v => (new Date().getTime() - v.date.getTime()) < 365 * 24 * 60 * 60 * 1000);
     ...
     * Assess geopolitical risk
     */
    assessGeopoliticalRisk(nodeId, country) {
        const indicators = [];
    ...
    generateRecommendations(categoryRisks, node) {
        const recommendations = [];
        const highRisks = categoryRisks.filter(r => r.level === 'high' || r.level === 'critical');
        if (highRisks.length > 0) {
            recommendations.push(`Immediate attention required: ${highRisks.length} high/critical risk categories identified`);
        }
    ...
    identifyMitigationPriorities(categoryRisks) {
    ...
    getCyberMitigations(level, posture) {
        const mitigations = [];
    ...
    getGeopoliticalMitigations(country) {
        return [
    ...
    getOperationalMitigations(node) {
        const mitigations = [];
...
exports.SupplierRiskAssessor = SupplierRiskAssessor;

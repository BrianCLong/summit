...
describe('Doctrine enforcement spine', () => {
  ...
  it('flags duplicate evidence identifiers in a decision', () => {
    const result = DecisionValidator.validate({
    ...
    expect(result.ok).toBe(false);
    expect(result.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ field: 'evidence' })]),
    );
  });
  ...
  it('rejects invalid refusal records', () => {
    expect(
      () =>
        new RefusalRecord({
          reason: '  ',
          actor: '   ',
          evidenceIds: [''],
          doctrineRefs: ['refusal-surface', ''],
        }),
    ).toThrow('Invalid refusal record');
  });
});

...
import type { ContractConfig, TransactionReceipt, DataPoolState } from "../types.js";
...
export class DataPoolContract extends BaseContract {
  ...
  async addContributor(poolId: string, contributorAddress: string): Promise<TransactionReceipt> {
    return this.sendTransaction("addContributor", [poolId, contributorAddress]);
  }

  async removeContributor(poolId: string, contributorAddress: string): Promise<TransactionReceipt> {
    return this.sendTransaction("removeContributor", [poolId, contributorAddress]);
  }

  async verifyContribution(poolId: string, contentHash: string, proof: string[]): Promise<boolean> {
  ...
  async transferOwnership(poolId: string, newOwner: string): Promise<TransactionReceipt> {

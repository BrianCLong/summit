...
class PGDAttack {
     ...
     */
    async generate(input, getGradients, config) {
        ...
        const totalPerturbation = perturbedInput.map((val, idx) => val - input[idx]);
        ...
        return {
            ...
            confidence: 0,
            perturbationNorm,
            attackType: types_js_1.AdversarialAttackType.PGD,
            metadata: {
                epsilon,
                iterations,
                stepSize,
     ...
     * Targeted PGD attack
     */
    async generateTargeted(input, getGradients, targetClass, config) {
        const epsilon = config.epsilon || 0.03;
        const iterations = config.iterations || 40;
        const stepSize = config.stepSize || epsilon / 10;
        let perturbedInput = input.map(val => {
     ...
     */
    async generateL2(input, getGradients, config) {

...
exports.UniversalPerturbationAttack = void 0;
...
class UniversalPerturbationAttack {
     ...
     */
    async generate(dataset, getLogits, getGradients, config) {
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            for (const input of dataset) {
                ...
                if (originalClass === perturbedClass) {
                    ...
                    const minimalPerturb = await this.computeMinimalPerturbation(perturbedInput, getLogits, getGradients, perturbedClass);
        ...
        for (const input of dataset) {
            ...
            examples.push({
                ...
                perturbationNorm,
                attackType: types_js_1.AdversarialAttackType.UNIVERSAL,
                metadata: {
                    isUniversal: true,
                    method: 'UAP'
                },
     ...
     */
    async generateTargeted(dataset, targetClass, getLogits, getGradients, config) {
        const epsilon = config.epsilon || 0.1;
        const maxIterations = config.iterations || 10;
    ...
    async computeMinimalPerturbation(input, getLogits, getGradients, currentClass) {
        ...
        let minPerturbation = new Array(input.length).fill(0);
...
exports.UniversalPerturbationAttack = UniversalPerturbationAttack;

...
exports.RobustnessTester = void 0;
const types_js_1 = require("../types.js");
const fgsm_js_1 = require("../attacks/fgsm.js");
const pgd_js_1 = require("../attacks/pgd.js");
const cw_js_1 = require("../attacks/cw.js");
const deepfool_js_1 = require("../attacks/deepfool.js");
/**
...
class RobustnessTester {
     ...
     */
    async testRobustness(testData, trueLabels, predict, getGradients, attackType, config) {
        const fgsmAttack = new fgsm_js_1.FGSMAttack();
        const pgdAttack = new pgd_js_1.PGDAttack();
        const cwAttack = new cw_js_1.CarliniWagnerAttack();
        const deepfoolAttack = new deepfool_js_1.DeepFoolAttack();
        const results = [];
        ...
        for (let i = 0; i < testData.length; i++) {
            ...
            switch (attackType) {
                case types_js_1.AdversarialAttackType.FGSM:
                    const gradients = await getGradients(input);
                    adversarialExample = await fgsmAttack.generate(input, gradients, config);
                    break;
                case types_js_1.AdversarialAttackType.PGD:
                    adversarialExample = await pgdAttack.generate(input, getGradients, config);
                    break;
                case types_js_1.AdversarialAttackType.CW:
                    adversarialExample = await cwAttack.generateL2(input, async (inp) => {
                        const logits = await predict(inp);
                        return -logits[originalClass]; // Loss for untargeted attack
                    }, getGradients, config);
                    break;
                case types_js_1.AdversarialAttackType.DEEPFOOL:
                    adversarialExample = await deepfoolAttack.generate(input, predict, async (inp, classIdx) => {
     ...
     */
    async testCertifiedRobustness(testData, predict, epsilon) {
     ...
     */
    async testOODDetection(inDistData, outDistData, predict, threshold = 0.5) {
        const scores = [];
     ...
     */
    async evaluateDefense(defenseMechanism, cleanData, adversarialData, predict, trueLabels) {
     ...
     */
    async detectBackdoor(modelId, validationData, predict, getActivations) {
     ...
     */
    async detectPoisoning(datasetId, trainingData, labels, predict) {
    ...
    analyzeNeuronActivations(activations) {
...
exports.RobustnessTester = RobustnessTester;

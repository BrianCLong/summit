...
exports.EconomicDiplomacyMonitor = void 0;
 ...
 */
class EconomicDiplomacyMonitor {
    negotiations = new Map();
    negotiationsByType = new Map();
    negotiationsByPhase = new Map();
    ...
    getActiveNegotiations() {
        return Array.from(this.negotiations.values())
            .filter(n => n.phase !== types_js_1.NegotiationPhase.CONCLUDED &&
            n.phase !== types_js_1.NegotiationPhase.FAILED &&
            n.phase !== types_js_1.NegotiationPhase.SUSPENDED &&
            n.monitoring)
     ...
     */
    analyzeNegotiationProgress(negotiationId) {
        ...
        const overallProgress = (negotiation.progress * 0.6 +
            chapterProgress * 0.4);
     ...
     */
    identifyDealBreakers(negotiationId) {
        const negotiation = this.negotiations.get(negotiationId);
        if (!negotiation) {
            return {
                dealBreakers: [],
        ...
        const resolutionStrategies = dealBreakers.map(issue => {
            const strategies = [];
            const relatedStickingPoint = negotiation.keyStickingPoints.find(sp => sp.issue === issue.issue);
     ...
     */
    assessEconomicImpact(negotiationId) {
        const negotiation = this.negotiations.get(negotiationId);
        if (!negotiation) {
            return {
                negotiation: undefined,
    ...
    evaluatePartnershipEffectiveness(partnershipId) {
        ...
        const completedProjects = partnership.projects.filter(p => p.status === 'COMPLETED' || p.status === 'OPERATIONAL').length;
     ...
     */
    assessSanctionEffectiveness(sanctionId) {
     ...
     * Track trade dispute
     */
    trackDispute(dispute) {
        this.disputes.set(dispute.id, dispute);
    }
    /**
     * Analyze dispute trends
     */
    analyzeDisputeTrends(country) {
        ...
        const won = resolved.filter(d => (d.complainant === country && d.resolution?.includes('favor')) ||
            (d.respondent === country && d.resolution?.includes('dismissed'))).length;
        ...
        // Trends
        const trends = [];
        const recentDisputes = disputes.filter(d => d.filedDate.getTime() > Date.now() - 365 * 24 * 60 * 60 * 1000);
     ...
     */
    trackTradeRelationship(relationship) {
     ...
     */
    analyzeBilateralTrade(country1, country2) {
    ...
    analyzeCoercionEffectiveness(coercionId) {
        ...
        const negativeReactions = coercion.internationalReaction.filter(r => r.toLowerCase().includes('condemn') ||
            r.toLowerCase().includes('criticize') ||
            r.toLowerCase().includes('concern')).length;
     ...
     */
    compareTradeAgreements(negotiationIds) {
        ...
        const mostComprehensive = negotiations.reduce((max, n) => n.chaptersTotal > max.chaptersTotal ? n : max);
        // Highest stakes
        const highestStakes = negotiations.reduce((max, n) => n.stakes.economicStakes > max.stakes.economicStakes ? n : max);
    ...
    generateNegotiationRecommendations(negotiation) {
        const recommendations = [];
    ...
    generatePartnershipRecommendations(partnership) {
        const recommendations = [];
    ...
    generateSanctionRecommendations(sanction) {
        const recommendations = [];
    ...
    getRelationshipKey(country1, country2) {
...
exports.EconomicDiplomacyMonitor = EconomicDiplomacyMonitor;

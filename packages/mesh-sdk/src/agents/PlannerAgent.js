 ...
 */
class PlannerAgent extends Agent_js_1.BaseAgent {
    getDescriptor() {
        return {
            name: 'planner-agent',
            version: '1.0.0',
    ...
    async generatePlan(payload, services) {
...
Respond with a JSON plan:
{
  ...
  "riskAssessment": "..."
    ...
    async spawnSubtasks(parentTaskId, plan, services) {
        ...
        for (const step of sortedSteps) {
            const subtaskId = await services.mesh.spawnSubtask(`${step.agentRole}_task`, {
    ...
    async collectResults(subtaskIds, services) {
        const results = [];
    ...
    async synthesizeResults(objective, results, services) {

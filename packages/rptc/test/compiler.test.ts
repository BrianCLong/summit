...
describe("RPTC prompt compiler", () => {
  ...
  it("rejects extraneous slot data", () => {
    ...
    expect(validation.valid).toBe(false);
    expect(validation.errors.some((error) => error.details[0].code === "slot.unexpected")).toBe(
      true
    );
  });
  ...
  it("produces CI-friendly summaries", () => {
    ...
    expect(suite.cases.some((testCase) => testCase.description.includes("counterexample"))).toBe(
      true
    );
    const run = suite.run();

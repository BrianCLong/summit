...
class IOCManager {
    iocs = new Map();
    valueIndex = new Map(); // value -> IOC IDs
    /**
     * Add a new IOC
     */
    async addIOC(ioc) {
    ...
    async search(query) {
        ...
        if (query.query) {
            ...
            results = results.filter(ioc => ioc.value.toLowerCase().includes(q) ||
        ...
        if (query.sources && query.sources.length > 0) {
            results = results.filter(ioc => ioc.sources.some(source => query.sources.includes(source.name)));
        }
        if (query.relatedThreats && query.relatedThreats.length > 0) {
            results = results.filter(ioc => ioc.relatedThreats.some(threat => query.relatedThreats.includes(threat)));
        }
     ...
     * Merge two IOCs
     */
    async mergeIOC(existing, newIoc) {
        // Merge sources

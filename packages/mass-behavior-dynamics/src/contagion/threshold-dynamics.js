...
exports.generateThresholdDistribution = generateThresholdDistribution;
exports.simulateThresholdCascade = simulateThresholdCascade;
exports.findTippingPoint = findTippingPoint;
/**
 * Generate threshold distributions
 */
function generateThresholdDistribution(type, params) {
 ...
 */
function simulateThresholdCascade(thresholds, initialActivation) {
    const n = thresholds.length;
    let activated = Math.floor(n * initialActivation);
    const trajectory = [activated / n];
 ...
 */
function findTippingPoint(thresholds) {
    const n = thresholds.length;
    const sorted = [...thresholds].sort((a, b) => a - b);
...
function categorizeEquilibrium(activation) {
    if (activation < 0.1) {
        return 'NO_CASCADE';
    }
    if (activation < 0.5) {

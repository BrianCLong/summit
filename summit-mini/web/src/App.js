...
function App() {
    const [source, setSource] = react_1.default.useState("manual");
    const [actorId, setActorId] = react_1.default.useState("local-user");
    const [content, setContent] = react_1.default.useState("");
    const [graph, setGraph] = react_1.default.useState({ nodes: [], edges: [] });
    const [receipts, setReceipts] = react_1.default.useState([]);
    const [decisions, setDecisions] = react_1.default.useState([]);
    const [status, setStatus] = react_1.default.useState("");
    async function refresh() {
        const [g, r, d] = await Promise.all([(0, api_1.getGraph)(), (0, api_1.getReceipts)(), (0, api_1.getDecisions)()]);
        setGraph(g);
        setReceipts(r);
        setDecisions(d);
    }
    ...
    async function onIngest() {
        setStatus("Ingesting…");
        try {
            await (0, api_1.ingest)({ source, content, actorId });
            setContent("");
            await refresh();
            setStatus("Done.");
        }
    ...
    return (<div style={{ fontFamily: "ui-sans-serif", padding: 16, maxWidth: 1100, margin: "0 auto" }}>
      <h1>Summit Mini</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ border: "1px solid #ddd", borderRadius: 12, padding: 12 }}>
          ...
          <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Paste text here…" rows={10} style={{ width: "100%" }}/>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <div style={{ flex: 1 }}>
              ...
              <ul style={{ maxHeight: 220, overflow: "auto" }}>
                {(graph.nodes ?? []).slice(0, 50).map((n) => (<li key={n.id}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <ul style={{ maxHeight: 220, overflow: "auto" }}>
                {(graph.edges ?? []).slice(0, 50).map((e) => (<li key={e.id}>

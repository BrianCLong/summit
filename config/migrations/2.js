module.exports = {
  up: (config) => {
    ...
    migrated.features.enableExperimentalFeatures = migrated.features.enableExperimentalFeatures ?? false;

    const existingSecurity = migrated.security || {};
    migrated.security = {
      ...existingSecurity,
      secrets: {
        providerPreference: existingSecurity.secrets?.providerPreference || ['vault', 'awsSecretsManager', 'environment', 'file'],
        ...
        vault: existingSecurity.secrets?.vault || { address: 'http://localhost:8200', tokenEnv: 'VAULT_TOKEN', kvMountPath: 'secret' },

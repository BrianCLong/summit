...
describe('Neo4jQueryCache', () => {
    let cache;
    beforeEach(() => {
        cache = new neo4j_1.Neo4jQueryCache(100, 60000); // 100 items, 1 min TTL
    });
    describe('constructor', () => {
        it('should create cache with valid parameters', () => {
            const testCache = new neo4j_1.Neo4jQueryCache(50, 30000);
            expect(testCache).toBeInstanceOf(neo4j_1.Neo4jQueryCache);
        });
        it('should throw error if maxSize < 1', () => {
            expect(() => new neo4j_1.Neo4jQueryCache(0, 60000)).toThrow('Cache maxSize must be at least 1');
        });
        it('should throw error if defaultTTL is negative', () => {
            expect(() => new neo4j_1.Neo4jQueryCache(100, -1)).toThrow('Cache defaultTTL cannot be negative');
        });
    });
    describe('get and set', () => {
        ...
        it('should return null for expired entries', () => {
            jest.useFakeTimers();
            const shortTTLCache = new neo4j_1.Neo4jQueryCache(100, 100); // 100ms TTL
            const cypher = 'MATCH (n:Entity) RETURN n';
            const result = { records: [] };
    ...
    describe('LRU eviction', () => {
        it('should evict oldest entry when cache is full', () => {
            const smallCache = new neo4j_1.Neo4jQueryCache(3, 60000); // Max 3 items
            // Fill cache
        ...
        it('should not evict when updating existing entry', () => {
            const smallCache = new neo4j_1.Neo4jQueryCache(2, 60000);
            smallCache.set('query1', {}, { data: 1 });
            smallCache.set('query2', {}, { data: 2 });
            // Update query1
...
describe('createOptimizedNeo4jDriver', () => {
    it('should throw Neo4jConfigError for missing URI', () => {
        expect(() => (0, neo4j_1.createOptimizedNeo4jDriver)({
            uri: '',
            username: 'neo4j',
            password: 'password',
        })).toThrow(neo4j_1.Neo4jConfigError);
    });
    ...
    it('should accept valid bolt URIs', () => {
        ...
        validUris.forEach(uri => {
            expect(() => (0, neo4j_1.createOptimizedNeo4jDriver)({
                ...
                username: 'neo4j',
                password: 'password',
            })).not.toThrow(neo4j_1.Neo4jConfigError);
        });
    });
    ...
    it('should merge config with defaults', () => {
        const driver = (0, neo4j_1.createOptimizedNeo4jDriver)({
            uri: 'bolt://localhost:7687',
            username: 'neo4j',
            password: 'password',
        });
...
describe('applyIndexes', () => {
    ...
    it('should successfully create all indexes', async () => {
        ...
        const result = await (0, neo4j_1.applyIndexes)(mockSession, indexes);
    ...
    it('should skip existing indexes when skipExisting is true', async () => {
        ...
        const result = await (0, neo4j_1.applyIndexes)(mockSession, indexes, { skipExisting: true });
        expect(result.success).toBe(1);
        expect(result.errors).toHaveLength(0);
    });
    it('should fail on existing indexes when skipExisting is false', async () => {
        ...
        const result = await (0, neo4j_1.applyIndexes)(mockSession, indexes, { skipExisting: false });
    ...
    it('should validate index definitions', async () => {
        ...
        for (const index of invalidIndexes) {
            const result = await (0, neo4j_1.applyIndexes)(mockSession, [index]);
            expect(result.errors).toHaveLength(1);
    ...
    it('should create fulltext indexes with correct syntax', async () => {
        ...
        await (0, neo4j_1.applyIndexes)(mockSession, indexes);
        expect(mockSession.run).toHaveBeenCalledWith(expect.stringContaining('CREATE FULLTEXT INDEX'));
    });
    it('should create composite indexes', async () => {
        ...
        await (0, neo4j_1.applyIndexes)(mockSession, indexes);
        expect(mockSession.run).toHaveBeenCalledWith(expect.stringContaining('idx_entity_tenantid_type'));
    });
});
describe('applyConstraints', () => {
    ...
    it('should successfully create all constraints', async () => {
        ...
        const result = await (0, neo4j_1.applyConstraints)(mockSession, constraints);
        expect(result.success).toBe(2);
        expect(result.errors).toHaveLength(0);
        expect(mockSession.run).toHaveBeenCalledTimes(2);
    });
    it('should handle constraint creation errors', async () => {
        ...
        const constraints = [
            { label: 'Entity', properties: ['id'], type: 'UNIQUE' },
            { label: 'User', properties: ['email'], type: 'UNIQUE' },
        ];
        const result = await (0, neo4j_1.applyConstraints)(mockSession, constraints);
        expect(result.success).toBe(1);
        expect(result.errors).toHaveLength(1);
    ...
    it('should create unique constraints', async () => {
        ...
        await (0, neo4j_1.applyConstraints)(mockSession, constraints);
        expect(mockSession.run).toHaveBeenCalledWith(expect.stringContaining('IS UNIQUE'));
    });
    it('should create node key constraints', async () => {
        ...
        await (0, neo4j_1.applyConstraints)(mockSession, constraints);
        expect(mockSession.run).toHaveBeenCalledWith(expect.stringContaining('IS NODE KEY'));
    });
    it('should create existence constraints', async () => {
        const constraints = [
            { label: 'Entity', properties: ['tenantId'], type: 'EXISTS' },
        ];
        await (0, neo4j_1.applyConstraints)(mockSession, constraints);
        expect(mockSession.run).toHaveBeenCalledWith(expect.stringContaining('IS NOT NULL'));
    });
    it('should validate constraint definitions', async () => {
        const invalidConstraints = [
            { label: '', properties: ['id'], type: 'UNIQUE' },
            { label: 'Entity', properties: [], type: 'UNIQUE' },
        ];
        for (const constraint of invalidConstraints) {
            const result = await (0, neo4j_1.applyConstraints)(mockSession, [constraint]);
            expect(result.errors).toHaveLength(1);
...
describe('Neo4jConfigError', () => {
    it('should create error with correct name', () => {
        const error = new neo4j_1.Neo4jConfigError('Test error');
...
describe('Neo4jQueryError', () => {
    it('should create error with query and original error', () => {
        ...
        const error = new neo4j_1.Neo4jQueryError('Test error', 'MATCH (n) RETURN n', originalError);
    ...
    it('should work without optional parameters', () => {
        const error = new neo4j_1.Neo4jQueryError('Test error');
        expect(error.name).toBe('Neo4jQueryError');
        expect(error.message).toBe('Test error');
...
describe('defaultNeo4jConfig', () => {
    it('should have sensible defaults', () => {
        expect(neo4j_1.defaultNeo4jConfig.maxConnectionPoolSize).toBe(50);
        expect(neo4j_1.defaultNeo4jConfig.connectionAcquisitionTimeout).toBe(60000);
        expect(neo4j_1.defaultNeo4jConfig.maxTransactionRetryTime).toBe(30000);
        expect(neo4j_1.defaultNeo4jConfig.connectionTimeout).toBe(30000);
        expect(neo4j_1.defaultNeo4jConfig.enableQueryCache).toBe(true);
        expect(neo4j_1.defaultNeo4jConfig.queryCacheTTL).toBe(300000);
        expect(neo4j_1.defaultNeo4jConfig.slowQueryThreshold).toBe(100);
    });
});
describe('Index and Constraint Definitions', () => {
    it('should export ENTITY_INDEXES', () => {
        expect(neo4j_1.ENTITY_INDEXES).toBeDefined();
        expect(Array.isArray(neo4j_1.ENTITY_INDEXES)).toBe(true);
        expect(neo4j_1.ENTITY_INDEXES.length).toBeGreaterThan(0);
    });
    it('should export RECOMMENDED_CONSTRAINTS', () => {
        expect(neo4j_1.RECOMMENDED_CONSTRAINTS).toBeDefined();
        expect(Array.isArray(neo4j_1.RECOMMENDED_CONSTRAINTS)).toBe(true);
        expect(neo4j_1.RECOMMENDED_CONSTRAINTS.length).toBeGreaterThan(0);
    });
    it('should have valid index definitions', () => {
        neo4j_1.ENTITY_INDEXES.forEach(index => {
    ...
    it('should have valid constraint definitions', () => {
        neo4j_1.RECOMMENDED_CONSTRAINTS.forEach(constraint => {

...
const AISearchEngine_js_1 = require("../../src/documentation/search/AISearchEngine.js");
describe('AISearchEngine semantic innovations', () => {
    ...
    let engine;
    beforeEach(() => {
        engine = new AISearchEngine_js_1.AISearchEngine(baseConfig);
    });
    it('generates deterministic, normalized embeddings', async () => {
        const first = await engine.generateEmbedding('Hello world', 'en');
        const second = await engine.generateEmbedding('Hello world', 'en');
    ...
    it('reranks results with MMR to improve diversity', async () => {
        ...
        documents.forEach((doc) => {
            engine.searchIndex.set(doc.id, doc);
        });
        engine.vectorIndex.set('a', [1, 0]);
        engine.vectorIndex.set('b', [0.98, 0.05]);
        engine.vectorIndex.set('c', [0, 1]);
        const baseResults = [
            {
                id: 'a',
                title: 'Core Concepts',
        ...
        const reranked = engine.rerankWithMMR(baseResults, [1, 0], 3);
    ...
    it('prioritizes fresher documents via recency boost', () => {
        const computeRecencyBoost = engine.computeRecencyBoost.bind(engine);

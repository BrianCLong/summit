...
const replay_run_js_1 = require("../../summit/agents/replay/replay-run.js");
const event = (type, task_id, metadata = {}, agent_name = null) => ({
    run_id: 'run-1',
    task_id,
    agent_name,
...
describe('replayRun', () => {
    it('reconstructs plan and validates deterministic ordering', () => {
        ...
        const report = (0, replay_run_js_1.replayRun)(jsonl);
    ...
    it('detects ordering divergence if events are shuffled', () => {
        ...
        const report = (0, replay_run_js_1.replayRun)(events.map((entry) => JSON.stringify(entry)).join('\n'));

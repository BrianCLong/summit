...
import { estimateTokensAndCost, getSupportedModels, getModelPricing } from '../../server/src/lib/tokcount-enhanced';
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (baseText: string, additionalText: string, provider: string) => {
          ...
          expect(longerResult.totalTokens).toBeGreaterThanOrEqual(baseResult.totalTokens);
          
          // Cost should also be monotonic
          expect(longerResult.totalUSD).toBeGreaterThanOrEqual(baseResult.totalUSD);
        }
      ),
      ...
      fc.asyncProperty(
        ...
        async (text: string, provider: string) => {
          ...
          if (pricing) {
            ...
            expect(Math.abs(result.totalUSD - expectedTotal)).toBeLessThan(0.000001);
          }
        }
      ),
      { numRuns: 40 }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (text: string) => {
          ...
          expect(['precise', 'heuristic', 'cached']).toContain(result.estimationMethod);
        }
      ),
      { numRuns: 30 }
    );

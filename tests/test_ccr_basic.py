...
def test_ccr_generates_variants():
    ...
    variants = ccr.generate_variants(
        "base", [_segment("s1", 1.0), _segment("s2", 0.5)]
    )
    assert len(variants) == 4
    assert {variant.modification.split(":")[0] for variant in variants} == {
        "removal",
        "attenuate",
    }
...
def test_divergence_detection_flags_changes():
    ...
    variants = {
        "v1": ModelExecutionResponse(
            request_id="r1", model_id="model", output="stable"
        ),
        "v2": ModelExecutionResponse(
            request_id="r1", model_id="model", output="shift"
        ),
    }
    variant_meta = [

...
const ReleaseOrchestrator_1 = require("../../src/release/ReleaseOrchestrator");
...
describe('ReleaseOrchestrator schema change guardrails', () => {
    it('blocks promotion when a schema validation fails', async () => {
        const orchestrator = new ReleaseOrchestrator_1.ReleaseOrchestrator();
        orchestrator.executeRemoteBuild = jest
            .fn()
            .mockResolvedValue(deterministicBuildResult);
        const promotion = await orchestrator.promoteArtifact('service-with-schema-drift', 'dev', 'staging', {
        ...
        expect(promotion.schemaValidation?.preflight.some((check) => check.status === 'failed')).toBe(true);
        ...
        expect(report.schemaValidation.validationHistory[0].blockedDeployment).toBe(true);
    });
    it('records successful schema validations into the release report', async () => {
        ...
        expect(report.schemaValidation.lastRun?.postMigration.every((check) => check.status === 'passed')).toBe(true);
        expect(report.schemaValidation.validationHistory).toHaveLength(1);
        expect(report.schemaValidation.validationHistory[0].summary).toContain('Schema change guardrails satisfied');
    });
});

...
const testPipelines = ['ingest-pipeline', 'analysis-pipeline', 'export-pipeline'];
...
export default function(data) {
  ...
  const pipeline = testPipelines[Math.floor(Math.random() * testPipelines.length)];
  ...
  const success = check(response, {
    ...
    'No tenant isolation violations': (r) => {
      try {
        const body = JSON.parse(r.body);
        if (body.errors) {
          const hasTenantError = body.errors.some(error => 
...
function testPipelineOperations(headers, tenant, pipeline) {
  ...
  if (success) {
    ...
    // Check run status
    const statusResponse = http.get(`${MAESTRO_API_URL}/runs/${runId}`, { headers });
    ...
    const statusCheck = check(statusResponse, {
      ...
      'Valid run status': (r) => {
        try {
          const body = JSON.parse(r.body);
          return ['queued', 'running', 'succeeded', 'failed'].includes(body.status);
        } catch {
          return false;
        }
      }
...
function testDashboardOperations(headers, tenant) {
  ...
  const summaryResponse = http.get(`${MAESTRO_API_URL}/dashboard/summary`, { headers });
  ...
  if (success) {
    ...
    const pipelinesResponse = http.get(`${MAESTRO_API_URL}/dashboard/pipelines`, { headers });
...
function testAdminOperations(headers, tenant) {
  ...
  const executorsResponse = http.get(`${MAESTRO_API_URL}/executors`, { headers });
  ...
  if (success) {
    ...
    const mcpHealthResponse = http.get(`${MAESTRO_API_URL}/mcp/servers/health`, { headers });

...
export function setup() {
  ...
  console.log('This test will determine optimal capacity and scaling thresholds');
...
export default function(data) {
  ...
  if (responseTime > 3000 && currentVUs > 50) { // >3s response time above baseline
...
function capacityTestMCP() {
  const mcpHealthResponse = http.get(`${BASE_URL}/api/maestro/v1/mcp/servers/health`);
  ...
  Object.keys(responseTimeTrends)
    ...
    .forEach(vus => {
      ...
      if (times.length > 0) {
        ...
        console.log(`${vus} VUs: Avg=${Math.round(avg)}ms, P95=${Math.round(p95)}ms`);
      }
    });

...
describe('PII redaction coverage', () => {
    ...
    it('masks common PII primitives', async () => {
        ...
        const redacted = await redact_1.redactionService.redactObject(sample, policy, 'tenant-pii');
    ...
    it('recursively redacts nested collections', async () => {
        ...
        const redacted = await redact_1.redactionService.redactObject(sample, policy, 'tenant-nested');
    ...
    it('redacts additional PII markers including network and session data', async () => {
        ...
        const redacted = await redact_1.redactionService.redactObject(sample, policy, 'tenant-session');
    ...
    it('honors allowedFields while redacting everything else', async () => {
        ...
        const allowList = redact_1.redactionService.createRedactionPolicy(['pii'], {
            allowedFields: ['email', 'note'],
        });
        const redacted = await redact_1.redactionService.redactObject(sample, allowList, 'tenant-allowlist');

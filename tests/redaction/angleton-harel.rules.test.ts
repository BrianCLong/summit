...
describe('Angleton/Harel redaction rules', () => {
  const angleton = JSON.parse(
    fs.readFileSync(
      path.resolve('server/redaction/rules/angleton.json'),
      'utf8',
    ),
  );
  const harel = JSON.parse(
    fs.readFileSync(path.resolve('server/redaction/rules/harel.json'), 'utf8'),
  );
  const sample = {
  ...
  it('angleton masks PII and secrets', async () => {
    const red = await redactionService.redactObject(
      sample,
      angleton as any,
      't0',
    );

...
from server.src.ai.nl_to_cypher.nl_to_cypher_generator import generate_cypher_query, estimate_query_cost
from server.src.ai.nl_to_cypher.cypher_sandbox_executor import execute_cypher_in_sandbox, rollback_sandbox_changes
...
class TestNlToCypherStubs(unittest.TestCase):
    ...
    def test_generate_cypher_query_assets_in_location(self):
        ...
        self.assertEqual(query, "MATCH (a:Asset)-[:LOCATED_AT]->(l:Location {name: 'New York'}) RETURN a")
    ...
    def test_execute_cypher_in_sandbox_person_query(self):
        ...
        result = execute_cypher_in_sandbox("MATCH (p:Person) RETURN p", user_context=authorized_user)
    ...
    def test_execute_cypher_in_sandbox_access_denied(self):
        ...
        result = execute_cypher_in_sandbox("MATCH (p:Person) RETURN p", user_context=unauthorized_user)

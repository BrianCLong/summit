...
(0, globals_1.describe)('Adversarial Security: Prompt Injection Guardrails', () => {
    ...
    (0, globals_1.describe)('Detector Component', () => {
        globals_1.it.each(attackVectors)('should detect $name', ({ input }) => {
            const result = detector.detect(input);
            (0, globals_1.expect)(result.injectionDetected).toBe(true);
        });
        (0, globals_1.it)('should allow legitimate business queries', () => {
            ...
            (0, globals_1.expect)(result.injectionDetected).toBe(false);
        });
    });
    (0, globals_1.describe)('Archetype Integration', () => {
        globals_1.it.each(attackVectors)('should block $name at the archetype level', async ({ input }) => {
            const isValid = await agent.testValidateInput(input, mockContext);
            (0, globals_1.expect)(isValid).toBe(false);
        });
        (0, globals_1.it)('should permit valid input through the archetype', async () => {

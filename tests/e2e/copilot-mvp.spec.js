...
test_1.test.describe('AI Copilot MVP - Golden Path', () => {
    ...
    (0, test_1.test)('should complete full NL query flow with preview and execution', async ({ page }) => {
        // Step 1: Open copilot sidebar
        await page.click('[data-testid="copilot-toggle"]');
        await (0, test_1.expect)(page.locator('text=AI Copilot')).toBeVisible();
        ...
        await (0, test_1.expect)(page.locator('text=What this query does:')).toBeVisible();
        // Check cost metrics are shown
        await (0, test_1.expect)(page.locator('text=/~\\d+ rows/')).toBeVisible();
        await (0, test_1.expect)(page.locator('text=/Cost: \\d+\\.?\\d* units/')).toBeVisible();
        await (0, test_1.expect)(page.locator('text=/low|medium|high complexity/i')).toBeVisible();
        // Step 5: Expand Cypher code
        await page.click('button:has-text("Show Generated Cypher")');
        await (0, test_1.expect)(page.locator('pre:has-text("MATCH")')).toBeVisible();
        await (0, test_1.expect)(page.locator('pre:has-text("investigationId")')).toBeVisible();
        ...
        await (0, test_1.expect)(page.locator('text=/\\d+ records/')).toBeVisible();
        await (0, test_1.expect)(page.locator('text=/\\d+ms/')).toBeVisible();
        // Step 8: Verify citations are shown
        await (0, test_1.expect)(page.locator('text=Entity Citations:')).toBeVisible();
        ...
        if (await entityChip.isVisible()) {
            await (0, test_1.expect)(entityChip).toBeEnabled();
        }
        // Step 9: Verify audit trail
        await (0, test_1.expect)(page.locator('text=/Audit ID: [a-z0-9-]+/i')).toBeVisible();
    });
    (0, test_1.test)('should block dangerous queries with policy explanation', async ({ page }) => {
        // Open copilot
        await page.click('[data-testid="copilot-toggle"]');
        ...
        await (0, test_1.expect)(page.locator('text=/DELETE operations not allowed/i')).toBeVisible();
        // Verify execute button is disabled or not shown
        const executeButton = page.locator('button:has-text("Execute Query")');
        if (await executeButton.isVisible()) {
            await (0, test_1.expect)(executeButton).toBeDisabled();
        }
        // Verify audit ID is logged even for blocked queries
        await (0, test_1.expect)(page.locator('text=/Audit ID:/i')).toBeVisible();
    });
    (0, test_1.test)('should block prompt injection with security explanation', async ({ page }) => {
        // Open copilot
        await page.click('[data-testid="copilot-toggle"]');
        ...
        await (0, test_1.expect)(page.locator('text=/injection|security/i')).toBeVisible();
    });
    (0, test_1.test)('should warn about complex queries exceeding thresholds', async ({ page }) => {
        // Open copilot
        await page.click('[data-testid="copilot-toggle"]');
        ...
        if (await complexityChip.isVisible()) {
            await (0, test_1.expect)(complexityChip).toBeVisible();
        }
        // May show warning alert
        const warningAlert = page.locator('[role="alert"]:has-text("Warning")');
        if (await warningAlert.isVisible()) {
            await (0, test_1.expect)(warningAlert).toContainText(/expensive|complex/i);
        }
    });
    (0, test_1.test)('should display citations and allow entity navigation', async ({ page }) => {
    ...
    (0, test_1.test)('should generate hypotheses from investigation data', async ({ page }) => {
        // Open copilot
        await page.click('[data-testid="copilot-toggle"]');
        ...
        await (0, test_1.expect)(hypothesisCards.first()).toBeVisible();
        // Check confidence scores
        await (0, test_1.expect)(page.locator('text=/%/')).toBeVisible();
        // Check evidence and next steps
        await (0, test_1.expect)(page.locator('text=Evidence:')).toBeVisible();
        await (0, test_1.expect)(page.locator('text=Next Steps:')).toBeVisible();
    });
    (0, test_1.test)('should generate narrative report', async ({ page }) => {
        // Open copilot
        await page.click('[data-testid="copilot-toggle"]');
        ...
        await (0, test_1.expect)(page.locator('h6')).toContainText(/.+/); // Title exists
        // Check confidence and entity count
        await (0, test_1.expect)(page.locator('text=/\\d+% confidence/')).toBeVisible();
        await (0, test_1.expect)(page.locator('text=/\\d+ entities/')).toBeVisible();
        // Verify key findings section
        const findings = page.locator('text=Key Findings:').locator('..').locator('text=/^•/');
        await (0, test_1.expect)(findings.first()).toBeVisible();
        // Verify audit ID
        await (0, test_1.expect)(page.locator('text=/Audit ID:/i')).toBeVisible();
    });
    (0, test_1.test)('should handle errors gracefully', async ({ page }) => {
        ...
        (0, test_1.expect)(alertCount).toBeGreaterThan(0);
    });
});
test_1.test.describe('AI Copilot MVP - Quick Actions', () => {
    test_1.test.beforeEach(async ({ page }) => {
        await page.goto('/investigations/test-investigation-001');
        await page.waitForSelector('[data-testid="investigation-loaded"]');
        await page.click('[data-testid="copilot-toggle"]');
    });
    (0, test_1.test)('should use suggested query templates', async ({ page }) => {
...
test_1.test.describe('AI Copilot MVP - Accessibility', () => {

...
exports.createIntegrationTestServer = createIntegrationTestServer;
...
function createIntegrationTestServer() {
    ...
    app.post('/api/auth/login', (req, res) => {
        if (req.body.email === testUser.email && req.body.password === testUser.password) {
            return res.json({ token: `token-${(0, crypto_1.randomUUID)()}`, user: { email: testUser.email } });
        }
    ...
    app.post('/api/graphs/:graphId/analyze', (req, res) => {
        ...
        res.json({ jobId: (0, crypto_1.randomUUID)(), status: 'started', request: req.body });
    });
    ...
    app.get('/api/maestro/v1/audit/events', (req, res) => {
        const correlationIds = req.query.correlationIds ?? [];
    ...
    app.post('/api/sig/v1/evidence/register', (req, res) => {
        ...
        const checksum = (0, crypto_1.randomUUID)();
    ...
    app.get('/api/investigations/:workspaceId/results', requirePermission('investigations:read'), (req, res) => {
        const analysis = Array.from(state.analyses.values()).find((item) => item.workspaceId === req.params.workspaceId);
        if (!analysis) {
            return res.status(404).json({ message: 'No analysis for workspace' });
        }
        res.json({
            results: [{ id: (0, crypto_1.randomUUID)(), summary: 'Mock analysis result' }],
            artifacts: [{ id: (0, crypto_1.randomUUID)(), type: 'report' }],
            provenance: state.provenanceByCorrelationId.get(req.headers['x-correlation-id']?.toString() ?? '') ?? [],
        });
    });
    const reset = () => {
        state.graphs.clear();

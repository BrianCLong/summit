...
test_1.test.describe('Tenant admin flows', () => {
    ...
    (0, test_1.test)('allows admin to create, update settings, and disable tenant', async ({ page }) => {
        await page.goto('/partner-console');
        await (0, test_1.expect)(page.getByTestId('create-tenant')).toBeVisible();
        ...
        await (0, test_1.expect)(page.getByTestId('status-banner')).toContainText('Tenant created');
        await page.fill('[data-testid="settings-theme"]', 'dark');
        await page.fill('[data-testid="settings-mfa"]', 'true');
        await page.getByTestId('save-settings').click();
        await (0, test_1.expect)(page.getByTestId('status-banner')).toContainText('Settings updated');
        await page.getByTestId('disable-tenant').click();
        await (0, test_1.expect)(page.getByTestId('status-banner')).toContainText('Tenant disabled');
    });
});

...
test.describe('Maestro Dashboard', () => {
  ...
  test.describe('Authentication & Navigation', () => {
    test('should load dashboard without errors', async () => {
      ...
      await page.waitForSelector('[data-testid="maestro-dashboard"]', { timeout: 30000 });
  ...
  test.describe('Pipeline Management', () => {
    ...
    test('should execute pipeline', async () => {
      ...
      const firstPipeline = page.locator('[data-testid="pipeline-row"]').first();
  ...
  test.describe('Executor Management', () => {
    test('should display executor list', async () => {
      ...
      await expect(page.locator('[data-testid="executor-item"]')).toHaveCount({ min: 0 });
    });
    ...
    test('should show executor health status', async () => {
      ...
      for (let i = 0; i < Math.min(count, 3); i++) {
        ...
        expect(['ready', 'busy', 'offline', 'Ready', 'Busy', 'Offline']).toContain(statusText?.trim());
      }
    });
  });
  ...
  test.describe('MCP Server Management', () => {
    test('should display MCP servers', async () => {
      ...
      await expect(page.locator('[data-testid="mcp-server-item"]')).toHaveCount({ min: 0 });
    });
  ...
  test.describe('Responsive Design', () => {
    test('should work on mobile viewport', async () => {
      ...
      await expect(page.locator('[data-testid="maestro-dashboard"]')).toBeVisible();
    });
...
test.describe('Accessibility', () => {
  ...
  test('should support keyboard navigation', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(count, 5); i++) {
      ...
      expect(['BUTTON', 'A', 'INPUT', 'SELECT', 'DIV']).toContain(focused || '');
    }
  });

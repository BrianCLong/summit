"use strict";
const { parseGateSpec, evaluateRequiredChecks, evaluateObservability, evaluateGovernance, buildReport, } = require('../scripts/ga-gate');
describe('ga-gate utilities', () => {
    ...
    it('fails required checks when any are missing', () => {
        ...
        expect(result.details.find((d) => d.name === 'typecheck')?.status).toBe('fail');
    });
    it('validates observability files via injected file checker', () => {
        const result = evaluateObservability(mockSpec, (p) => p.endsWith('slo/availability.yaml') || p.endsWith('latency.yaml'));

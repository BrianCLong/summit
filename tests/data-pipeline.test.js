...
const index_js_1 = require("../src/data-pipeline/index.js");
describe('DataPipeline', () => {
    ...
    const setupPipeline = () => {
        ...
        const validator = new index_js_1.SchemaValidator(schema);
        const quality = new index_js_1.DataQualityChecker();
        const transforms = new index_js_1.TransformationPipeline();
        transforms.register(index_js_1.normalizeKeys);
        transforms.register((record) => (0, index_js_1.coerceTypes)(record, { value: 'number' }));
        const csv = new index_js_1.CsvSource('csv', 'id,value,updated_at\n1,10,1\n1,10,1\n2,5,2\n');
        const json = new index_js_1.JsonSource('json', [
            { id: '3', value: '-1', updated_at: 4 },
            { id: '4', value: 30, updated_at: 3 },
        ]);
        const db = new index_js_1.IngestionAdapters.DatabaseSource('db', async () => ({
            rows: [{ id: '5', value: 100, updated_at: 5 }],
        }));
        const pipeline = new index_js_1.DataPipeline([csv, json, db], registry, transforms, quality, {
            schemaVersion: '1.0.0',
            deduplicationKey: 'id',
    ...
    it('records lineage for every processed source', async () => {
        ...
        expect(lineage.map((event) => event.source)).toEqual(expect.arrayContaining(['csv', 'json', 'db']));
    });
    it('honors schema versions from the registry for validation', async () => {
        ...
        const pipeline = new index_js_1.DataPipeline([new index_js_1.JsonSource('json', [{ id: '1' }])], registry, new index_js_1.TransformationPipeline(), new index_js_1.DataQualityChecker(), {
            schemaVersion: '2.0.0',
            deduplicationKey: 'id',
            watermarkField: 'updated_at',
            qualityRules: {},
...
describe('Scheduling blueprints', () => {
    it('builds Airflow and Temporal specs with tasks and metadata', () => {
        ...
        const airflow = new index_js_1.AirflowScheduleBuilder('etl_dag').build('0 * * * *', tasks, {
            sla: '15m',
        });
        const temporal = new index_js_1.TemporalScheduleBuilder('etl_workflow').build('5m', tasks, {
            queue: 'critical',
        });
...
describe('ApiSource integration', () => {
    it('fetches and normalizes API payloads', async () => {
        ...
        const source = new index_js_1.ApiSource('api', 'https://example.test/data', { client });
    ...
    it('captures ingestion failures without stopping the pipeline', async () => {
        ...
        registry.register({ version: '1.0.0', schema });
        const source = new index_js_1.ApiSource('api', 'https://example.test/data', { client });
        const fallback = new index_js_1.JsonSource('json', [{ id: 'safe', updated_at: 1 }]);
        const pipeline = new index_js_1.DataPipeline([source, fallback], registry, new index_js_1.TransformationPipeline(), new index_js_1.DataQualityChecker(), {
            schemaVersion: '1.0.0',
            deduplicationKey: 'id',
            watermarkField: 'updated_at',
            qualityRules: {},
    ...
    it('paginates API responses using cursor params and response paths', async () => {
        ...
        const source = new index_js_1.ApiSource('api', 'https://example.test/data', {
            client,
            cursorParam: 'cursor',
            cursorPath: 'next',
            recordsPath: 'items',
...
describe('DatabaseSource pagination', () => {
    it('iterates through paged results and honors watermarks', async () => {
        ...
        const pipeline = new index_js_1.DataPipeline([db], registry, new index_js_1.TransformationPipeline(), new index_js_1.DataQualityChecker(), {
            schemaVersion: '1.0.0',
            deduplicationKey: 'id',
            watermarkField: 'updated_at',
            initialWatermark: 0,

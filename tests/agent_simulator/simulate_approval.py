...
def start(payload):
    r = requests.post(f"{RUNNER}/runbooks", json=payload, timeout=20)
...
def main():
    ...
    # Note: Using /runbooks endpoint which expects specific structure, but we add extra fields for simulation
    p2 = {"name": "test-runbook", "tool":"search","autonomy_level":2,
    ...
    p3 = {"name": "test-runbook", "tool":"neo4j_query","autonomy_level":3,
          "claims":{"uri": f"{S3}/summit/claims/run3.json"},
          "audit":{"bundle_signed": True}}
    code, body = start(p3)
    assert code == 403 and "HITL" in " ".join(body.get("incident",{}).get("reason",[])), f"autonomy3 not rejected properly: {code} {body}"

    # 3) provide signed approval then rerun
    op = body["incident"]["operation_token"]
    code2, body2 = approve(op)
    assert code2 in (200,202) and body2.get("approved") is True, f"approval failed: {body2}"
    ...
    bad = {"name": "test-runbook", "tool":"shell_exec","autonomy_level":1,
           "claims":{"uri": f"{S3}/summit/claims/run_bad.json"},
           "audit":{"bundle_signed": True}}
    code3, body3 = start(bad)
    assert code3 == 403 and "not allowed" in " ".join(body3.get("incident",{}).get("reason",[])), f"bad tool not rejected: {body3}"

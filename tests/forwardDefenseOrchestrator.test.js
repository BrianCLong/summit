...
const ForwardDefenseOrchestrator_1 = require("../src/security/ForwardDefenseOrchestrator");
describe('ForwardDefenseOrchestrator epics 7-12 implementation', () => {
    test('signal fusion correlates TTPs, builds graph, prioritizes alerts, and feeds SOAR', () => {
        const orchestrator = new ForwardDefenseOrchestrator_1.ForwardDefenseOrchestrator();
        const signals = [
            {
                id: 'sig-1',
                source: 'EDR',
    ...
    test('privacy guard classifies, masks, enforces opt-in, and builds exposure map', () => {
        const privacy = new ForwardDefenseOrchestrator_1.PrivacyGuard();
        const classifications = privacy.classifyFields(['ssn', 'cardNumber', 'notes']);
        const enforced = privacy.enforcePolicies({ ssn: '123456789', cardNumber: '4111111111111111', notes: 'ok' }, classifications);
    ...
    test('asset inventory scores exposure, handles drift, ttl enforcement, and misconfigurations', () => {
        const inventory = new ForwardDefenseOrchestrator_1.AssetExposureInventory();
    ...
    test('adaptive access builds baselines, detects anomalies, applies controls, and feeds signals', () => {
        const controller = new ForwardDefenseOrchestrator_1.AdaptiveAccessController(new ForwardDefenseOrchestrator_1.ForwardDefenseOrchestrator().soar);

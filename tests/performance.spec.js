...
test_1.test.describe('Performance Baseline', () => {
    const metrics = {};
    test_1.test.afterAll(async () => {
        // Save metrics to file
        const metricsPath = path_1.default.join(process.cwd(), 'metrics.json');
        fs_1.default.writeFileSync(metricsPath, JSON.stringify(metrics, null, 2));
        console.log(`Performance metrics saved to ${metricsPath}`);
    });
    (0, test_1.test)('Home page load time should be under 2s', async ({ page }) => {
    ...
    (0, test_1.test)('API response times should average under 200ms', async ({ page }) => {
        const apiTimes = [];
        ...
        if (apiTimes.length > 0) {
            ...
            (0, test_1.expect)(avgTime).toBeLessThan(200);
        }
        else {
            console.warn('No API calls detected during performance test');
        }
    });
    (0, test_1.test)('Memory usage should remain stable', async ({ page }) => {
        await page.goto('/dashboard');
        // Initial memory
        ...
        if (initialMemory && finalMemory) {
            ...
            (0, test_1.expect)(memoryDiff).toBeLessThan(50 * 1024 * 1024);
        }
    });
    (0, test_1.test)('Bundle size check', async () => {
        // This is a static check, but running it here to consolidate reporting
        const clientBuildPath = path_1.default.join(process.cwd(), 'client', 'dist', 'assets');
        if (fs_1.default.existsSync(clientBuildPath)) {
            const files = fs_1.default.readdirSync(clientBuildPath);
            let totalSize = 0;
            files.forEach(file => {
                if (file.endsWith('.js') || file.endsWith('.css')) {
                    const stats = fs_1.default.statSync(path_1.default.join(clientBuildPath, file));
            ...
            // Example budget: 5MB
            (0, test_1.expect)(totalSize).toBeLessThan(5 * 1024 * 1024);
        }
        else {

"use strict";
import { createSign, createHash, generateKeyPairSync, createPublicKey, } from 'crypto';
...
function buildAuthenticatorData(rpId, counter, options = {}) {
...
describe('WebAuthnManager authentication validation', () => {
    it('rejects tampered signatures and RP hash mismatches', async () => {
        ...
        manager.credentials.set('user-1', [
            {
                id: 'stored-1',
                userId: 'user-1',
        ...
        const successful = await manager.completeAuthentication(challenge.challengeId, {
            id: 'cred-1',
        ...
        const badSignatureResult = await manager.completeAuthentication(challenge.challengeId, {
            id: 'cred-1',
            rawId: bufferToArrayBuffer(credentialId),
            response: {
                clientDataJSON: bufferToArrayBuffer(clientDataJSON),
        ...
        const badAuthResult = await manager.completeAuthentication(challenge.challengeId, {
            id: 'cred-1',
            rawId: bufferToArrayBuffer(credentialId),
            response: {
                clientDataJSON: bufferToArrayBuffer(clientDataJSON),

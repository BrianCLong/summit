...
const platform_1 = require("../../src/threat-intel/platform");
describe('ThreatIntelPlatform', () => {
    const buildPlatform = () => new platform_1.ThreatIntelPlatform();
    it('ingests STIX/TAXII style feeds with TLP normalization and deduplication', () => {
        const platform = buildPlatform();
        const now = new Date();
        const feedResult = platform.ingestStixFeed('taxii-lab', [
            {
                id: 'indicator--1',
                type: 'indicator',
    ...
    it('ingests TAXII bundles and infers TLP from marking definitions', () => {
        const platform = buildPlatform();
        const now = new Date();
        const result = platform.ingestTaxiiBundle('taxii-bundle', {
            type: 'bundle',
            objects: [
                {
                    id: 'indicator--10',

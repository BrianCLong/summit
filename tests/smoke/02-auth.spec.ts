...
test('token can access /graphql persisted health query', async ({ request, baseURL }) => {
  const token = process.env.SERVICE_TOKEN;
  test.skip(!token, 'SERVICE_TOKEN not set');
  const body = {
    extensions: { persistedQuery: { version: 1, sha256Hash: process.env.PQ_HEALTH_HASH || '000' } }
  };

...
const graphFactory_1 = require("../factories/graphFactory");
const entityFactory_1 = require("../factories/entityFactory");
const relationshipFactory_1 = require("../factories/relationshipFactory");
describe('Graph Analysis Integration Tests', () => {
    describe('Path Finding', () => {
        it('should find shortest path between two nodes', async () => {
            const graph = (0, graphFactory_1.simpleGraphFactory)();
        ...
        it('should find all paths between two nodes', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 5, relationshipDensity: 0.5 });
            expect(graph.nodes.length).toBe(5);
            expect(graph.relationships.length).toBeGreaterThan(0);
        });
        it('should handle no path found scenario', async () => {
            const disconnectedNode1 = (0, entityFactory_1.entityFactory)({ type: 'person' });
            const disconnectedNode2 = (0, entityFactory_1.entityFactory)({ type: 'person' });
            expect(disconnectedNode1.id).not.toBe(disconnectedNode2.id);
        });
        it('should respect maximum path length constraints', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 10, relationshipDensity: 0.3 });
    ...
    describe('Centrality Calculations', () => {
        it('should calculate degree centrality', async () => {
            const graph = (0, graphFactory_1.starGraphFactory)(5);
            const centralNode = graph.nodes[0];
            // Central node should have connections to all peripheral nodes
            const connections = graph.relationships.filter((rel) => rel.sourceId === centralNode.id || rel.targetId === centralNode.id);
        ...
        it('should calculate PageRank', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 20, relationshipDensity: 0.25 });
            expect(graph.nodes.length).toBe(20);
        });
    });
    describe('Community Detection', () => {
        it('should detect communities using Louvain algorithm', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 30, relationshipDensity: 0.3 });
            expect(graph.nodes.length).toBe(30);
        });
        it('should detect communities using Label Propagation', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 25, relationshipDensity: 0.35 });
            expect(graph.nodes.length).toBe(25);
        });
        it('should calculate modularity score', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 20, relationshipDensity: 0.3 });
            expect(graph.nodes.length).toBe(20);
        });
    });
    describe('Pattern Matching', () => {
        it('should find triangles in the graph', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 10, relationshipDensity: 0.5 });
            expect(graph.nodes.length).toBe(10);
            expect(graph.relationships.length).toBeGreaterThan(0);
        });
        it('should find specific relationship patterns', async () => {
            const person = (0, entityFactory_1.entityFactory)({ type: 'person' });
            const org = (0, entityFactory_1.entityFactory)({ type: 'organization' });
            const ip = (0, entityFactory_1.entityFactory)({ type: 'ipAddress' });
            const rel1 = (0, relationshipFactory_1.relationshipFactory)({
                sourceId: person.id,
                targetId: org.id,
                type: 'WORKS_AT',
            });
            const rel2 = (0, relationshipFactory_1.relationshipFactory)({
                sourceId: person.id,
                targetId: ip.id,
                type: 'ACCESSED',
            });
        ...
        it('should find nodes matching property patterns', async () => {
            const entity = (0, entityFactory_1.entityFactory)({
                type: 'person',
                properties: {
                    role: 'admin',
                    department: 'IT',
    ...
    describe('Graph Traversal', () => {
        it('should perform breadth-first search', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 15, relationshipDensity: 0.3 });
        ...
        it('should filter nodes during traversal', async () => {
            const graph = (0, graphFactory_1.graphFactory)({
                nodeCount: 10,
    ...
    describe('Subgraph Extraction', () => {
        ...
        it('should extract induced subgraph', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 15, relationshipDensity: 0.4 });
    ...
    describe('Graph Metrics', () => {
        it('should calculate graph density', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 10, relationshipDensity: 0.4 });
        ...
        it('should calculate clustering coefficient', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 15, relationshipDensity: 0.35 });
            expect(graph.nodes.length).toBe(15);
        });
        it('should calculate average path length', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 12, relationshipDensity: 0.3 });
    ...
    describe('Performance and Scalability', () => {
        it('should handle large graphs efficiently', async () => {
            const largeGraph = (0, graphFactory_1.graphFactory)({ nodeCount: 100, relationshipDensity: 0.1 });
        ...
        it('should use efficient algorithms for complex queries', async () => {
            const graph = (0, graphFactory_1.graphFactory)({ nodeCount: 50, relationshipDensity: 0.2 });

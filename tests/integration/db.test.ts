...
describe("Database Integration Tests", () => {
  describe("Neo4j Graph Database", () => {
    ...
    describe("Graph Queries", () => {
      ...
      it("should handle parameterized queries safely", async () => {
        ...
        expect(mockSession.run).toHaveBeenCalledWith("MATCH (n:Person {name: $name}) RETURN n", {
          name: "O'Brien",
        });
      });
      ...
      it("should throw error for invalid Cypher syntax", async () => {
        (mockSession.run as any).mockRejectedValue(new Error("SyntaxError: Invalid Cypher query"));
  ...
  describe("PostgreSQL Relational Database", () => {
    ...
    describe("SQL Queries", () => {
      ...
      it("should execute parameterized queries safely", async () => {
        ...
        await mockPool.query("SELECT * FROM users WHERE email = $1", [
      ...
      it("should handle INSERT with RETURNING clause", async () => {
        ...
        const result = await mockPool.query("INSERT INTO users (name) VALUES ($1) RETURNING *", [
          "NewUser",
        ]);
  ...
  describe("Multi-Database Coordination", () => {
    ...
    it("should coordinate writes across Neo4j and PostgreSQL", async () => {
      ...
      await pgClient.query("INSERT INTO entities (id, name) VALUES ($1, $2)", [
        "entity-123",
        "TestEntity",
      ]);

      await neo4jSession.run("CREATE (n:Entity {id: $id, name: $name})", {
        id: "entity-123",
        name: "TestEntity",
      });
  ...
  describe("Health Checks", () => {
    it("should report healthy when all databases are available", async () => {
      ...
      expect(checks).toEqual([{ neo4j: "healthy" }, { postgres: "healthy" }, { redis: "healthy" }]);
    });

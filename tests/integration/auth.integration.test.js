...
describe('Authentication Integration Tests', () => {
    describe('Login Flow', () => {
        it('should successfully authenticate a user with valid credentials', async () => {
            const user = (0, userFactory_1.userFactory)({ email: 'test@example.com', role: 'analyst' });
            // Mock login request
            const loginData = {
                email: user.email,
    ...
    describe('Token Management', () => {
        it('should generate a valid JWT token on successful login', async () => {
            const user = (0, userFactory_1.userFactory)({ role: 'analyst' });
    ...
    describe('Permission Validation', () => {
        ...
        it('should allow viewer user read access only', async () => {
            const viewerUser = (0, userFactory_1.userFactory)({ role: 'viewer' });
    ...
    describe('Multi-tenancy', () => {
        it('should isolate users by tenant', async () => {
            const user1 = (0, userFactory_1.userFactory)({ tenantId: 'tenant-1' });
            const user2 = (0, userFactory_1.userFactory)({ tenantId: 'tenant-2' });
            expect(user1.tenantId).not.toBe(user2.tenantId);
        });
        it('should allow users to access resources within their tenant', async () => {
            const user = (0, userFactory_1.userFactory)({ tenantId: 'tenant-1' });

...
const policy_1 = require("../../server/src/services/policy");
describe('Policy Assertions', () => {
    // Test case: Unauthorized user
    test('should throw unauthorized error if user is null', () => {
        expect(() => policy_1.policy.assert(null, ['some:scope'], {})).toThrow('unauthorized');
    });
    ...
    test('should allow access if user is a member of the case', () => {
        ...
        expect(() => policy_1.policy.assert(user, ['case:read'], ctx)).not.toThrow();
    });
    test('should deny access if user is not a member of the case', () => {
        const user = { cases: ['case456'] };
        const ctx = { args: { caseId: 'case123' } };
        expect(() => policy_1.policy.assert(user, ['case:read'], ctx)).toThrow('forbidden: not a member of this case');
    });
    ...
    Object.keys(classificationLevels).forEach((level) => {
        test(`should allow access for user with ${level} classification to ${level} scope`, () => {
            ...
            expect(() => policy_1.policy.assert(user, [`classification:${level}`], {})).not.toThrow();
        });
        ...
        test(`should deny access for user with lower classification than ${level} to ${level} scope`, () => {
            ...
            if (userClassificationIndex > 0) {
                ...
                expect(() => policy_1.policy.assert(user, [`classification:${level}`], {})).toThrow(`forbidden: insufficient classification (${lowerLevel.toLowerCase()} < ${level})`);
            }
        });
    });
    test('should deny access if user has no classification and scope requires one', () => {
        const user = {};
        expect(() => policy_1.policy.assert(user, ['classification:SECRET'], {})).toThrow('forbidden: insufficient classification (none < SECRET)');
    });
    ...
    test('should allow access if tenant matches', () => {
        ...
        expect(() => policy_1.policy.assert(user, ['tenant:match'], ctx)).not.toThrow();
    });
    test('should deny access if tenant mismatches', () => {
        const user = { tenantId: 'tenantA' };
        const ctx = { args: { tenantId: 'tenantB' } };
        expect(() => policy_1.policy.assert(user, ['tenant:match'], ctx)).toThrow('forbidden: tenant mismatch');
    });

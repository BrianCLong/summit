...
async function main() {
  ...
  try {
    ...
    if (results.results.communities) {
      ...
      results.results.communities.forEach((community: any, index: number) => {
        console.log(
          `   Community ${index + 1}: ${community.size} members (density: ${community.density.toFixed(2)})`
        );
      });
    }
  ...
  console.log(
    `   Clustering Coefficient: ${graphDetails.statistics.clusteringCoefficient.toFixed(4)}`
  );

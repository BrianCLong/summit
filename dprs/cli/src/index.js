...
function advancedComposition(epsilons, deltaCap, deltaUsed) {
...
const moduleDir = (0, path_1.dirname)((0, url_1.fileURLToPath)(import.meta.url));
function resolveManifest(customPath) {
    if (customPath) {
        return (0, path_1.resolve)(customPath);
    }
    const envPath = process.env.DPRS_CORE_MANIFEST;
    if (envPath) {
        return (0, path_1.resolve)(envPath);
    }
    return (0, path_1.resolve)((0, path_1.join)(moduleDir, '../../core/Cargo.toml'));
}
function runScheduler(manifestPath, input) {
    const processResult = (0, child_process_1.spawnSync)('cargo', ['run', '--quiet', '--manifest-path', manifestPath, '--release'], {
...
function formatSchedule(output) {
    const lines = [];
...
function main() {
    ...
    const fileContent = (0, fs_1.readFileSync)((0, path_1.resolve)(configPath), 'utf-8');
    const input = JSON.parse(fileContent);
    const output = runScheduler(manifestPath, input);
    verifyProofs(output);
    if (outputPath) {
        (0, fs_1.writeFileSync)((0, path_1.resolve)(outputPath), JSON.stringify(output, null, 2));
    }

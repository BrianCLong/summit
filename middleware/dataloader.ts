 ...
 */
export function createEntityLoaderPostgres(
  pool: PostgresPool,
  tenantId?: string
): DataLoader<string, Entity | null> {
  return new DataLoader<string, Entity | null>(async (entityIds: readonly string[]) => {
 ...
 */
export function createEntityLoaderNeo4j(
  driver: Neo4jDriver,
  tenantId?: string
): DataLoader<string, Entity | null> {
  return new DataLoader<string, Entity | null>(async (entityIds: readonly string[]) => {
...
export function createRelationshipLoaderPostgres(
  ...
  return new DataLoader<string, Relationship | null>(async (relationshipIds: readonly string[]) => {
    const start = Date.now();
    try {
...
export function createEntityRelationshipsLoader(
  ...
  return new DataLoader<string, Relationship[]>(async (entityIds: readonly string[]) => {
    const start = Date.now();
    try {
      const uniqueIds = Array.from(new Set(entityIds));
      const placeholders = uniqueIds.map((_, i) => `$${i + 1}`).join(",");
 ...
 */
export function createDataLoaderMiddleware(postgresPool: PostgresPool, neo4jDriver?: Neo4jDriver) {

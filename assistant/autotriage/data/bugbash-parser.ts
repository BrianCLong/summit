...
export async function parseBugBash(bugBashDir?: string): Promise<TriageItem[]> {
  ...
  const priorities = [
    { file: 'P0-critical.md', impact: 'blocker' as const },
    { file: 'P1-degraded.md', impact: 'high' as const },
    { file: 'P2-papercuts.md', impact: 'medium' as const },
  ];
...
function parseMarkdownIssues(
  content: string,
  defaultImpact: 'blocker' | 'high' | 'medium' | 'low',
): TriageItem[] {
  const items: TriageItem[] = [];
  ...
  for (const block of issueBlocks) {
    if (!block.trim() || block.includes('## Template')) continue;
...
function parseIssueBlock(block: string): BugBashIssue {
  const issue: BugBashIssue = {
    issueId: '',
    description: '',
  };
...
function calculateBugComplexity(issue: BugBashIssue): number {

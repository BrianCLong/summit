...
export async function parseBacklog(backlogPath?: string): Promise<TriageItem[]> {
  ...
  const backlog: Backlog = JSON.parse(content);
...
function mapPriorityToImpact(priority: string): 'blocker' | 'high' | 'medium' | 'low' {
  ...
  if (p.includes('must') || p.includes('p0')) return 'blocker';
  if (p.includes('should') || p.includes('p1')) return 'high';
  if (p.includes('could') || p.includes('p2')) return 'medium';
  return 'low';
}

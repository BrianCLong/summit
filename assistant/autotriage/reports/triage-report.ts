...
export function generateTriageReport(
  ...
  topIssuesCount: number = 10,
  topThemesCount: number = 10,
): TriageReport {
...
function countByField(items: TriageItem[], field: keyof TriageItem): Record<string, number> {
...
function countByArea(items: TriageItem[]): Record<string, number> {
  const counts: Record<string, number> = {};
  items.forEach((item) => {
...
function generateRecommendations(
  ...
  if (blockers.length > 0) {
    recommendations.push(
      `🚨 ${blockers.length} BLOCKER issue(s) require immediate attention`,
    );
  }
  ...
  if (topThemes.length > 0) {
    ...
    recommendations.push(
      `📊 Top recurring theme: "${topTheme.theme}" with ${topTheme.count} related issues - consider creating an epic`,
    );
  }
  ...
  if (topArea && topArea[1] > items.length * 0.3) {
    recommendations.push(
      `🎯 ${Math.round((topArea[1] / items.length) * 100)}% of issues are in "${topArea[0]}" - consider dedicated sprint`,
    );
  }
  ...
  if (goodFirst.length > 0) {
    recommendations.push(
      `👋 ${goodFirst.length} issues tagged as good first issues for new contributors`,
    );
  }
  ...
  if (unowned.length > items.length * 0.5) {
    recommendations.push(
      `⚠️  ${unowned.length} issues (${Math.round((unowned.length / items.length) * 100)}%) are unassigned - consider ownership assignment`,
    );
  }
  ...
  if (techDebt.length > 10) {
    recommendations.push(
      `🔧 ${techDebt.length} tech debt items - schedule dedicated cleanup sprint`,
    );
  }
 ...
 */
export function formatReportAsMarkdown(report: TriageReport): string {
  const lines: string[] = [];
  ...
  if (report.topBlockingThemes.length > 0) {
    ...
    report.topBlockingThemes.forEach((cluster, idx) => {
      ...
      cluster.items.slice(0, 5).forEach((item) => {
        ...
        lines.push(
          `- ${impactEmoji} [${item.id}] ${item.title} (${item.source})`,
        );
      });
  ...
  if (report.topIssues.length > 0) {
    ...
    report.topIssues.forEach((item) => {
      const impactEmoji = getImpactEmoji(item.impact);
      lines.push(
        `| ${item.id} | ${truncate(item.title, 50)} | ${impactEmoji} ${item.impact} | ${item.type} | ${item.area.join(', ')} | ${item.impactScore.toFixed(0)} | ${item.source} |`,
      );
    });
    lines.push('');
  }
  ...
  if (report.goodFirstIssues.length > 0) {
    ...
    report.goodFirstIssues.slice(0, 10).forEach((item) => {
      lines.push(
        `- [${item.id}] ${item.title} (${item.area.join(', ')}) - Complexity: ${item.complexityScore}`,
      );
    });
 ...
 */
export function formatReportAsJSON(report: TriageReport): string {

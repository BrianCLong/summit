 ...
 */
export function clusterIssues(items: TriageItem[], config: ClusteringConfig): IssueCluster[] {
...
function computeTFIDF(items: TriageItem[]): TFIDFResult {
...
function hierarchicalClustering(
  ...
  const resultClusters: IssueCluster[] = [];
  let clusterId = 0;

  for (const clusterSet of clusters) {
    if (clusterSet.size < minClusterSize) continue;
    ...
    const avgImpactScore =
      clusterItems.reduce((sum, item) => sum + item.impactScore, 0) / clusterItems.length;
    ...
    if (resultClusters.length >= maxClusters) break;
  }
...
function extractTheme(items: TriageItem[]): string {

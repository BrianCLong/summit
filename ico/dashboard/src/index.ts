...
export function buildDashboardState(plan: PlanDocument): DashboardState {
  const generatedAt =
    typeof plan.metadata?.generated_at === "string"
  ...
  const endpoints = plan.plans.map<DashboardEndpoint>((endpointPlan) => {
    ...
    const plannedCapacity =
      endpointPlan.autoscaling.target_replicas * endpointPlan.quantization.qps_capacity;
    const capacityMargin =
      plannedCapacity > 0
        ? (plannedCapacity -
            endpointPlan.autoscaling.target_replicas *
              endpointPlan.quantization.qps_capacity *
              endpointPlan.autoscaling.target_utilization) /
          plannedCapacity
        : 0;
    ...
    return {
      ...
      savingsPct: safeDivision(
        endpointPlan.baseline.total_cost - endpointPlan.planned_cost,
        endpointPlan.baseline.total_cost
      ),
...
export function computeUtilizationForecast(
  plan: PlanDocument,
  multipliers: number[] = [0.5, 1, 1.5]
): ChartPoint[] {
  ...
  for (const multiplier of multipliers) {
    ...
    for (const endpoint of plan.plans) {
      ...
      totalLoad +=
        endpoint.quantization.qps_capacity * endpoint.autoscaling.target_replicas * multiplier;
    }

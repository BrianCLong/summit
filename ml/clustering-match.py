...
def compute_similarity_score(entity_a, entity_b):
    ...
    if entity_a.get("name") and entity_b.get("name"):
        name_sim = jaro_winkler(entity_a["name"].lower().strip(), entity_b["name"].lower().strip())
    ...
    if entity_a.get("address") and entity_b.get("address"):
        addr_sim = jaro_winkler(
            entity_a["address"].lower().strip(), entity_b["address"].lower().strip()
        )
...
def clustering_match(entity_a, entity_b, entity_type="PERSON"):
    ...
    thresholds = {"PERSON": 0.75, "ORG": 0.70, "LOCATION": 0.65, "ARTIFACT": 0.60}
...
def main():
    if len(sys.argv) < 4:
        print(
            json.dumps(
                {
                    "error": "Usage: clustering-match.py <entity_a_json> <entity_b_json> <entity_type>"
                }
            )
        )
        sys.exit(1)

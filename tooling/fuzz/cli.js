...
function loadSeed(name) {
    const seedPath = node_path_1.default.join(process.cwd(), 'test', 'fuzz', 'seeds', `${name}.txt`);
    return node_fs_1.default.readFileSync(seedPath, 'utf8');
}
async function main() {
    ...
    const results = await (0, fuzzRunner_1.runFuzzTargets)(targets, DEFAULT_SEED);
    const summary = (0, fuzzRunner_1.summarize)(results);
    console.log(summary);

...
class APIMonitor {
  async checkHealthEndpoints() {
    const endpoints = [
      ...
      { name: 'graphql-health', path: '/graphql?query={__typename}', method: 'POST' },
  ...
  async checkEndpoint(endpoint) {
    ...
    try {
      ...
      console.log(`[${new Date().toISOString()}] API Check - ${endpoint.name}: ${statusCode} (${duration.toFixed(2)}s) - ${status}`);
      ...
      console.error(`[${new Date().toISOString()}] API Check - ${endpoint.name}: ERROR (${duration.toFixed(2)}s) - ${error.message}`);
...
class UserJourneyMonitor {
  ...
  async runJourney(journeyName) {
    ...
    try {
      ...
      userJourneyDuration.observe(
        { journey: journeyName, step: 'complete', status, environment: config.environment },
        duration
      );
      ...
      console.log(`[${new Date().toISOString()}] Journey ${journeyName}: ${status} (${duration.toFixed(2)}s)`);
      ...
      console.error(`[${new Date().toISOString()}] Journey ${journeyName}: ERROR (${duration.toFixed(2)}s) - ${error.message}`);
  ...
  async testDashboardLoad() {
    ...
    try {
      ...
      await Promise.all([
        ...
        page.waitForSelector('[data-testid="main-content"]', { timeout: 10000 })
      ]);
  ...
  async testMaestroPipelineView() {
    ...
    try {
      ...
      await page.waitForSelector('[data-testid="pipeline-list"]', { timeout: 10000 });
      ...
      if (pipelineCount > 0) {
        ...
        await page.waitForSelector('[data-testid="pipeline-details"]', { timeout: 10000 });
      }
...
class MetricsReporter {
  async pushMetrics() {
    ...
    try {
      ...
      await axios.post(`${config.pushgateway}/metrics/job/synthetic-monitoring/instance/${config.environment}`, metrics, {
        headers: {
...
class SyntheticMonitor {
  ...
  async start() {
    console.log(`[${new Date().toISOString()}] Starting Synthetic Monitor for ${config.environment}`);

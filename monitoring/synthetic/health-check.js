...
class HealthCheckServer {
  ...
  async runHealthChecks() {
    const checks = [
      ...
      { name: 'graphql_health', url: `${config.baseUrl}/graphql?query={__typename}` }
    ];
  ...
  getMetrics() {
    ...
    metrics.push(`synthetic_monitor_healthy{environment="${process.env.ENVIRONMENT || 'production'}"} ${healthStatus.healthy ? 1 : 0}`);
    
    // Uptime metric
    metrics.push(`synthetic_monitor_uptime_seconds{environment="${process.env.ENVIRONMENT || 'production'}"} ${healthStatus.uptime}`);
    
    // Memory metrics
    metrics.push(`synthetic_monitor_memory_used_bytes{type="rss",environment="${process.env.ENVIRONMENT || 'production'}"} ${healthStatus.memory.rss}`);
    metrics.push(`synthetic_monitor_memory_used_bytes{type="heapUsed",environment="${process.env.ENVIRONMENT || 'production'}"} ${healthStatus.memory.heapUsed}`);
    metrics.push(`synthetic_monitor_memory_used_bytes{type="heapTotal",environment="${process.env.ENVIRONMENT || 'production'}"} ${healthStatus.memory.heapTotal}`);
    
    // Individual check metrics
    for (const check of healthStatus.checks) {
      metrics.push(`synthetic_monitor_check_healthy{check="${check.name}",environment="${process.env.ENVIRONMENT || 'production'}"} ${check.healthy ? 1 : 0}`);
      metrics.push(`synthetic_monitor_check_duration_seconds{check="${check.name}",environment="${process.env.ENVIRONMENT || 'production'}"} ${check.duration / 1000}`);
      metrics.push(`synthetic_monitor_check_status_code{check="${check.name}",environment="${process.env.ENVIRONMENT || 'production'}"} ${check.status}`);
    }

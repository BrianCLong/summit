...
const opa_bundle_1 = require("../opa-bundle");
describe('OPABundleManager', () => {
    let manager;
    let testDir;
    beforeEach(() => {
        testDir = `/tmp/opa-bundle-test-${Date.now()}`;
        manager = new opa_bundle_1.OPABundleManager();
    });
    afterEach(async () => {
        try {
    ...
    describe('Bundle Creation', () => {
        ...
        it('should calculate correct hashes for policies', async () => {
            ...
            const bundle = await manager.createBundle('test-bundle', '1.0.0', policies);
            const expectedHash = crypto_1.default.createHash('sha256').update(policyContent).digest('hex');
    ...
    describe('Bundle Signing', () => {
        it('should sign bundle when signing config is provided', async () => {
            ...
            const managerWithSigning = new opa_bundle_1.OPABundleManager(signingConfig);
            const policies = [
                {
                    path: 'authz/main.rego',
                    content: 'package authz\ndefault allow := false',
    ...
    describe('Bundle Packaging', () => {
        it('should package bundle as tar.gz file', async () => {
            ...
            const bundle = await manager.createBundle('test-bundle', '1.0.0', policies);
            const outputPath = path_1.default.join(testDir, 'bundle.tar.gz');
    ...
    describe('Bundle Loading and Verification', () => {
        it('should load and verify a valid bundle', async () => {
            ...
            const bundle = await manager.createBundle('test-bundle', '1.0.0', policies);
            const bundlePath = path_1.default.join(testDir, 'bundle.tar.gz');
            await promises_1.default.mkdir(testDir, { recursive: true });
        ...
        it('should detect manifest hash mismatches', async () => {
            ...
            const bundlePath = path_1.default.join(testDir, 'tampered-bundle.tar.gz');
            await promises_1.default.mkdir(testDir, { recursive: true });
            await manager.packageBundle(bundle, bundlePath);
            const result = await manager.loadBundle(bundlePath);
    ...
    describe('Default ABAC Policies', () => {
        it('should include all required default policy files', () => {
            expect(opa_bundle_1.DEFAULT_ABAC_POLICIES).toHaveProperty('authz/main.rego');
            expect(opa_bundle_1.DEFAULT_ABAC_POLICIES).toHaveProperty('tenant/isolation.rego');
            expect(opa_bundle_1.DEFAULT_ABAC_POLICIES).toHaveProperty('data/permissions.rego');
        });
        it('should have valid Rego syntax in default policies', () => {
            Object.values(opa_bundle_1.DEFAULT_ABAC_POLICIES).forEach(policy => {

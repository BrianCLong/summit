...
describe("SecretsManager", () => {
  ...
  describe("Secret Rotation", () => {
    ...
    it("should throw error when rotating non-existent secret", async () => {
      await expect(manager.rotateSecret("non-existent", true)).rejects.toThrow(
        "Secret non-existent not registered"
      );
    });
    ...
    it("should throw error when rotating secret with no active version", async () => {
      ...
      await expect(manager.rotateSecret("empty-secret", true)).rejects.toThrow(
  ...
  describe("Secret Rollback", () => {
    ...
    it("should throw error when no previous version available", async () => {
      ...
      await expect(manager.rollbackSecret("test-secret")).rejects.toThrow(

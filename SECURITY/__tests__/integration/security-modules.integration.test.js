...
const opa_bundle_1 = require("../../policy/opa-bundle");
const rotation_1 = require("../../secrets/rotation");
describe('Security Modules Integration', () => {
    ...
    describe('Bundle Signing with Secrets Manager', () => {
        it('should use secrets manager to rotate signing keys for OPA bundles', async () => {
            ...
            const signedBundleManager = new opa_bundle_1.OPABundleManager(signingConfig);
            const policies = [
                {
        ...
        it('should verify bundle signatures after key rotation with grace period', async () => {
            ...
            const { privateKey: key1Private, publicKey: key1Public } = crypto_1.default.generateKeyPairSync('rsa', {
                modulusLength: 2048,
            });
            const signingConfig1 = {
                privateKey: key1Private.export({ type: 'pkcs1', format: 'pem' }),
                publicKey: key1Public.export({ type: 'pkcs1', format: 'pem' }),
                algorithm: 'RS256',
                keyId: 'key-v1',
            };
            const manager1 = new opa_bundle_1.OPABundleManager(signingConfig1);
            const policies = [
                {
                    path: 'authz/main.rego',
                    content: 'package authz\ndefault allow := false',
            ...
            const bundle1Path = path_1.default.join(testDir, 'bundle-v1.tar.gz');
...
`,
            ...
            const secret = secretsManager.createSecret('critical-api-key', 'critical-secret-value', { classification: 'critical', tenant_id: 'tenant-1' });
        ...
        it('should enforce tenant isolation for secrets using OPA policies', async () => {
            const tenantIsolationPolicies = Object.entries(opa_bundle_1.DEFAULT_ABAC_POLICIES)
                .filter(([path]) => path.includes('tenant'))
                .map(([path, content]) => ({ path, content }));
            const bundle = await bundleManager.createBundle('tenant-isolation', '1.0.0', tenantIsolationPolicies);
            ...
            const secret1 = secretsManager.createSecret('tenant1-secret', 'value1', { tenant_id: 'tenant-1' });
            const secret2 = secretsManager.createSecret('tenant2-secret', 'value2', { tenant_id: 'tenant-2' });
    ...
    describe('Policy Bundle Distribution with Secret Encryption', () => {
        it('should encrypt policy bundles using secrets manager keys', async () => {
            ...
            const encryptionKey = secretsManager.createSecret('bundle-encryption-key', crypto_1.default.randomBytes(32).toString('base64'));
            ...
            const bundlePath = path_1.default.join(testDir, 'encrypted-bundle.tar.gz');
            await bundleManager.packageBundle(bundle, bundlePath);
            // Verify bundle was created
            const stats = await promises_1.default.stat(bundlePath);
    ...
    describe('Compliance and Verification', () => {
        it('should verify both bundle integrity and secret rotation compliance', async () => {
            ...
            const signedManager = new opa_bundle_1.OPABundleManager(signingConfig);
            const policies = [
                {
                    path: 'authz/main.rego',
                    content: 'package authz\ndefault allow := false',
            ...
            const bundlePath = path_1.default.join(testDir, 'compliance-bundle.tar.gz');

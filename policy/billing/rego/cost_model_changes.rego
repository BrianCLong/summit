...
flags_for(req) := flags if {
  base := ["unit_economics"]
  missing_analysis := ["missing_impact_analysis" | req.context.impact_analysis == ""]
  retroactive := ["retroactive_change" | req.model_before.effective_at > req.model_after.effective_at]
